...
void enable_ux_jank_detection(bool enable, const char *info, int tgid, int pid) {
  if (is_target_device != 1 && target_device_only == 1) {
    return;
  }
  ...
  if (enable) {
    ...
    if (detect_tgid != tgid) {
      ...
      if (target_app_only == 1) {
        is_target_app = 0;
        // if (strcmp(info, "<apk_name>") == 0) is_target_app = 1;
...
static int __init jank_detection_init(void) {
  ...
  hrtimer_setup(&timer_forcepull, timer_forcepull_callback, CLOCK_MONOTONIC,
      HRTIMER_MODE_REL);
#endif

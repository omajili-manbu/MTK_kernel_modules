...
#else
		hrtimer_setup(hrtimer, met_hrtimer_notify, CLOCK_MONOTONIC,
				HRTIMER_MODE_REL);
#endif
		...
		if (cpu == curr_polling_cpu) {
			...
			if (preferred_polling_cpu != -1) {
				...
				if (cpu_related_cnt == 0) {
					 ...
					 *		__func__, __LINE__, curr_polling_cpu);
					 */
					smp_call_function_single(curr_polling_cpu,
						__met_hrtimer_register, NULL, 1);
...
#else
		hrtimer_setup(&met_cpu_ptr->hrtimer, met_hrtimer_notify, CLOCK_MONOTONIC,
				HRTIMER_MODE_REL);
#endif

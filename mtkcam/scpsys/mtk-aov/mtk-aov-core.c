...
int aov_core_copy(struct mtk_aov *aov_dev, struct aov_dqevent *dequeue)
{
	...
	if ((event->detect_mode) || (debug_mode == AOV_DEBUG_MODE_DUMP) ||
		...
		if (event->apu_size) {
			...
			//FR OFFSET START
				// Copy fr_info output from event->fr_info to buffer
				get_user(buffer, (void **)((uintptr_t)dequeue +
					offsetof(struct aov_dqevent, fr_info)));
				AOV_DEBUG_LOG(*(aov_dev->enable_aov_log_flag),
					"%s: fr_info_t count %d, offset 0x%08x, width:%d, height:%d, stride:%d \n", __func__,
					event->fr_info.count, event->fr_info.offset[0], event->fr_info.offset[1],
					event->fr_info.offset[2], event->fr_info.offset[3]);
				AOV_DEBUG_LOG(*(aov_dev->enable_aov_log_flag),
					"%s: copy fr_info output from(%p) to(%p) size(%d)\n",
					__func__, ALIGN16(&(event->fr_info)),
					buffer, AOV_MAX_AAA_SIZE);
				ret = copy_to_user((void *)buffer,
					ALIGN16(&(event->fr_info)), AOV_MAX_AAA_SIZE);

				if (ret) {
					buffer_release(core_info, event);
					dev_info(aov_dev->dev,
						"%s: failed to copy fr_info output(%d)\n", __func__, ret);
					return -EFAULT;
				}
			//FR OFFSET END
		}

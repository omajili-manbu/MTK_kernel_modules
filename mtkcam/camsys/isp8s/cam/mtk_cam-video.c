				   ...
				   struct device *alloc_devs[])
{
	...
	int i;
	int min_buf_sz;
	struct device *alloc_dev = cam->smmu_dev ? : cam->dev;
				       ...
				       bool dbg_log, char *prefix)
{
	...
	/* 2 case:
	 * a. stream off one node for release vb2 buffer => owned_by_drv_count == 0
	 * b. to stop all node => owned_by_drv_count != 0 and must do ctx_stream_off
	 *    (or v4l2 checker warn_on during vb2 queue cancel)
	 */

	vb2_q_buf_cnt = atomic_read(&node->vb2_q.owned_by_drv_count);
	if (vb2_q_buf_cnt || mtk_cam_ctx_all_nodes_idle(ctx))
...
EXIT:
	if (CAM_DEBUG_ENABLED(V4L2) || dbg_log || vb2_q_buf_cnt)
		dev_info(ctx->cam->dev,
			"%s: %s %s vb2_q_buf_cnt:%d->%d node_cnt:%d\n",
...
__poll_t mtk_cam_v4l2_file_poll(struct file *file,
				    poll_table *wait)
{
	if (!file || !file->private_data) {
		pr_info("%s: null file pointer", __func__);

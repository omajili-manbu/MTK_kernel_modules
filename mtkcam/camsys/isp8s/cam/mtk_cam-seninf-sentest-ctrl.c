...
enum SENTEST_IDLE_OUTMUX_SRC {
	CHOOSE_FROM_CORE_LIST,
	CHOOSE_FROM_CTX_LIST,
};
...
static int get_lastest_outmux_id_by_vc_cnt(struct seninf_ctx *ctx,
	u32 target_count, u32 *outmux, enum SENTEST_IDLE_OUTMUX_SRC src)
{
	...
	head = (src == CHOOSE_FROM_CORE_LIST) ? &core->list_outmux : &ctx->list_outmux;
...
static int get_lastest_outmux_for_seamless(struct seninf_ctx *ctx, u32 target_count, u32 *outmux)
{
	/* chose new outmux from core list first */
	if (!get_lastest_outmux_id_by_vc_cnt(ctx, target_count,
		outmux, CHOOSE_FROM_CORE_LIST)) {
		return 0;
	}

	/* if core is run out of outmux, then take outmux from current ctx list */
	if (!get_lastest_outmux_id_by_vc_cnt(ctx,
		ctx->sentest_seamless_switch_outmux_current_list_idx,
		outmux, CHOOSE_FROM_CTX_LIST)) {
		ctx->sentest_seamless_switch_outmux_current_list_idx++;
		return 0;
	}

	pr_info("[Error][%s] get_lastest_outmux_id_from current list with cnt %u return failed",
		__func__, ctx->sentest_seamless_switch_outmux_current_list_idx);
	return -EINVAL;
}
...
static int seninf_sentest_set_camtg_for_seamless(struct seninf_ctx *ctx)
{
	...
	/* force reset current list id = 0 , prevent using outmux which is out of bound */
	ctx->sentest_seamless_switch_outmux_current_list_idx = 0;

	/* prepared outmux which to be enable */
	for (i = 0; i < cur_vcinfo->cnt; i++) {

		if (get_lastest_outmux_for_seamless(ctx, i, &outmux_id)) {
			pr_info("[Error][%s] get_lastest_outmux_for_seamless return failed", __func__);
			return -EFAULT;
		}

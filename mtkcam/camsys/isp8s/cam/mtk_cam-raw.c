...
static void init_raw_ddren(struct mtk_raw_device *dev, int is_srt, int frm_time_us)
{
	...
	if ((!GET_PLAT_HW(qof_support)) || debug_ddren_sw_mode) {
...
void initialize(struct mtk_raw_device *dev, struct engine_callback *cb,
			    int is_slave, int is_srt, int frm_time_us, bool chk_first_cq_start)
{
	u32 val;
	...
	dev->target_dcif_cq_start = (chk_first_cq_start) ?
		FBIT(CAMCTL_DCIF_FIRST_CQ_START_INT_ST) :
		FBIT(CAMCTL_DCIF_LAST_CQ_START_INT_ST);
							 ...
							 int max_p1_delay_mul)
{
	...
	if (scq_ms == -1 || max_p1_delay_mul != 1)
		max_p1_delay = max_p1_delay_mul * frame_ms * scq_cnt_rate_khz(val);
...
static void raw_dump_debug_lsci_status(struct mtk_raw_device *dev)
{
	mtk_cam_dump_lsc_debug(dev);

	mtk_cam_dump_dma_debug(dev,
			       dev->dmatop_base, /* DMATOP_BASE */
			       "LSCI_R1",
			       dbg_LSCI_R1, ARRAY_SIZE(dbg_LSCI_R1));
}
...
static bool is_rawi_r5_read_done(struct mtk_raw_device *dev)
{
	u32 rawi_r5_line_cnt = raw_readl_relaxed(dev, dev->dmatop_base,
			REG_CAMRAWDMATOP_DC_DBG_LINE_CNT_RAWI_R5);

	return (READ_FIELD(rawi_r5_line_cnt, CAMRAWDMATOP_DC_DBG_W_LINE_CNT_RAWI_R5) - 1) ==
		READ_FIELD(rawi_r5_line_cnt, CAMRAWDMATOP_DC_DBG_R_LINE_CNT_RAWI_R5);
}
...
static irqreturn_t mtk_irq_raw_yuv(int irq, void *data)
{
	...
	if (tg1_status & FBIT(CAMCTL_TG_SOF_INT_ST) ||
		...
		dcif_status & raw->target_dcif_cq_start) {
		...
		else if (dcif_status & FBIT(CAMCTL_DCIF_LAST_CQ_START_INT_ST))
			irq_info.fbc_empty = is_rawi_r5_read_done(raw);
		else
			irq_info.fbc_empty = 0;
				      ...
				      unsigned int fh_cookie)
{
	...
	if (cnt < OVERRUN_DUMP_CNT) {
		raw_dump_debug_lsci_status(raw_dev);
		dump_dma_soft_rst_stat(raw_dev);
		dump_raw_slice_gals(raw_dev);
		dump_vcore_gals(raw_dev);
		dump_wla_2_0(raw_dev);
		dump_dcm_status(raw_dev);
...
{
	int cnt;
	u32 mod10_en = raw_readl(raw_dev, raw_dev->base, REG_CAMCTL_MOD10_EN);
	u32 csr = readl(raw_dev->dmatop_base + REG_CAMRAWDMATOP_DMA_DBG_SEL);

	writel(csr | FBIT(CAMRAWDMATOP_DMA_DBG_EN),
		   raw_dev->dmatop_base + REG_CAMRAWDMATOP_DMA_DBG_SEL);

	raw_writel(mod10_en |
			   FBIT(CAMCTL_CQI_R1_EN) | FBIT(CAMCTL_CQI_R2_EN) |
			   FBIT(CAMCTL_CQI_R3_EN) | FBIT(CAMCTL_CQI_R4_EN),
			   raw_dev, raw_dev->base, REG_CAMCTL_MOD10_EN);
	raw_writel(mod10_en |
			   FBIT(CAMCTL_CQI_R1_EN) | FBIT(CAMCTL_CQI_R2_EN) |
			   FBIT(CAMCTL_CQI_R3_EN) | FBIT(CAMCTL_CQI_R4_EN),
			   raw_dev, raw_dev->base_inner, REG_CAMCTL_MOD10_EN);

	cnt = raw_dev->cq_err_handle_cnt++;

	if (cnt == 0) {
		mtk_cam_dump_cq_debug(raw_dev);
		dev_info(raw_dev->dev, "%s: force trigger CQ", __func__);
		raw_writel(FBIT(CAMCTL_CQ_THR0_START),
				   raw_dev, raw_dev->base, REG_CAMCTL_START);
	}
	...
	if (cnt < 32) {
		mtk_cam_dump_dma_debug(raw_dev,
			raw_dev->dmatop_base, /* DMATOP_BASE */
			"CQ bus",
			dbg_cq_bus, ARRAY_SIZE(dbg_cq_bus));
		/* dump twice */
		mtk_cam_dump_dma_debug(raw_dev,
			raw_dev->dmatop_base, /* DMATOP_BASE */
			"CQ bus",
	...
	raw_writel(mod10_en, raw_dev, raw_dev->base, REG_CAMCTL_MOD10_EN);
	raw_writel(mod10_en, raw_dev, raw_dev->base_inner, REG_CAMCTL_MOD10_EN);
	writel(csr, raw_dev->dmatop_base + REG_CAMRAWDMATOP_DMA_DBG_SEL);
...
void mtk_raw_push_dc_dbg_ln(struct mtk_raw_device *raw, u64 irq_boottime_ns)
{
	u32 idx = raw->dc_dbg_ln_idx;

	raw->dc_dbg_ln[idx].cur_boottime_ns = ktime_get_boottime_ns();
	raw->dc_dbg_ln[idx].irq_boottime_ns = irq_boottime_ns;
	raw->dc_dbg_ln[idx].rawi5 =
		readl(raw->dmatop_base + REG_CAMRAWDMATOP_DC_DBG_LINE_CNT_RAWI_R5);
	raw->dc_dbg_ln[idx].ufdi5 =
		readl(raw->dmatop_base + REG_CAMRAWDMATOP_DC_DBG_LINE_CNT_UFDI_R5);
	raw->dc_dbg_ln[idx].rawi2 =
		readl(raw->dmatop_base + REG_CAMRAWDMATOP_DC_DBG_LINE_CNT_RAWI_R2);
	raw->dc_dbg_ln[idx].ufdi2 =
		readl(raw->dmatop_base + REG_CAMRAWDMATOP_DC_DBG_LINE_CNT_UFDI_R2);

	raw->dc_dbg_ln_idx = ((raw->dc_dbg_ln_idx + 1) % DC_DBG_LN_STORE);
}

...
{
	/* invalid sets */
	bool invalid = (s->pixel_mode_max > 1) ?
...
mtk_cam_resource_update_work_buf(struct mtk_cam_resource_v2 *user_ctrl)
{
	...
	case MTK_CAM_SCEN_NORMAL:
		...
		/* for small size sensor, reserve at least 2x wbuf for */
		/* camsv to write rotationally, avoid ufbc len table overwrited */
		if ((s->width <= SEN_4K_W_HALF) && (s->height <= SEN_4K_H_HALF))
			buf_require *= 2;
...
CALC_RESOURCE:
	...
	if (GET_PLAT_HW(has_pixel_mode_constraints))
		stepper.pixel_mode_min = (s->width <= PIX_MODE_SIZE_CONSTRAIN) ?
			1 : GET_PLAT_HW(pixel_mode_max);
	else
...
static int mtk_raw_get_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	} else if (ctrl->id == V4L2_CID_MTK_CAM_RAWS_SUPPORT) {
...
static struct v4l2_ctrl_config cfg_get_available_raws_ctrl = {
	.ops = &cam_ctrl_ops,
	.id = V4L2_CID_MTK_CAM_RAWS_SUPPORT,
...
static void mtk_raw_pipeline_ctrl_setup(struct mtk_raw_pipeline *pipe)
{
	...
	v4l2_ctrl_new_custom(ctrl_hdlr, &cfg_get_available_raws_ctrl, NULL);

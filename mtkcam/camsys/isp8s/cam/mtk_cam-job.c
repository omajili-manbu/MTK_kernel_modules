...
static void set_engines_mux_cq_ready(struct mtk_cam_ctx *ctx,
				  ...
				  unsigned long engines, bool ready)
{
	...
	for (i = 0; i < cam->engines.num_raw_devices && mask; i++) {
		...
		raw_id = get_master_raw_id(BIT(i));  /* get raw itself */
		raw_tg_idx = raw_to_tg_idx(raw_id);
		mtk_cam_seninf_set_mux_cq_rdy(seninf, raw_tg_idx, ready);
	}
	...
	for (i = 0; i < cam->engines.num_camsv_devices && mask; i++) {
		...
		sv_dev = dev_get_drvdata(cam->engines.sv_devs[i]);
		mtk_cam_seninf_set_mux_cq_rdy(seninf, sv_dev->cammux_id, ready);
	}
				 ...
				 struct mtk_cam_request *req)
{
	...
	if (job->req) {
		dev_info(dev,
			 "%s job#%d %s might be leaked, need to check cam->req_cnt\n",
			 __func__, job->req_seq, job->req->debug_str);
	}
	...
	if (raw_data && (raw_data->ctrl.req_info.req_type == SENSOR_REQUEST)) {
		...
		if (job->req_sensor) {
			dev_info(dev,
				 "%s job#%d %s might be leaked, need to check cam->req_cnt\n",
				 __func__, job->req_seq, job->req_sensor->debug_str);
		}
		media_request_get(&req->req);
...
#define SEN_4K_W_HALF		(2048)
#define SEN_4K_H_HALF		(1536)
static int get_max_p1_delay_multi(struct mtk_cam_job *job)
{
	/* see mtk_cam_resource_update_work_buf() */
	if ((get_sensor_w(job) <= SEN_4K_W_HALF) &&
		(get_sensor_h(job) <= SEN_4K_H_HALF))
		return 2;
...
_stream_on(struct mtk_cam_job *job, bool on)
{
	...
	/* ois compensation */
	if (is_ois_compensation(job) && !ctx->ois_comp_en) {
	...
	for (i = 0; i < ARRAY_SIZE(ctx->hw_raw); i++) {
		if (ctx->hw_raw[i]) {
			...
			update_scq_start_period(raw_dev, job->scq_period,
				...
				get_max_p1_delay_multi(job));
			if (job->enable_hsf_raw) {
				without_tg =  is_dc_mode(job) || is_m2m(job) ||
					is_offline_timeshare(job);
				mtk_cam_hsf_stream_ctrl(ctx, without_tg);
			}
...
static bool rms_off_support(struct mtk_cam_job *job)
{
	/* temporarily by scenario */
	return true;
}

static void handle_rms_disable(struct mtk_cam_job *job)
{
	if (!rms_off_support(job))
		return;
			       ...
			       const char *desc)
{
	...
	if (!job_debug_exception_dump(job, desc)) {
		...
		if (!strcmp(desc, MSG_DC_SKIP_FRAME))
			job->is_dc_mode_skip_error = 1;
...
static void dump_job_info(struct mtk_cam_job *job, const char *desc)
{
	...
	fp = (struct mtkcam_ipi_frame_param *)job->ipi.vaddr;
	dev_info(dev, "%s: ctx-%d pipe %lx job type %d req-%d-0x%x eng %x master %x\n",
		 __func__,
		 ctx->stream_id, ctx->used_pipe,
		 job->job_type,
		 job->req_seq, job->frame_seq_no,
		 job->used_engine, job->master_engine);
...
static int job_raw_change_hw_init(struct mtk_cam_job *job, int pda_idx)
{
	...
	if (selected_need_init) {
		...
		if (job->raw_change == JOB_RAW_MASTER_UNCHANGED) {
			...
			if (qof_enabled && mtk_cam_job_not_support_qof(job)) {
				/* NOTE: ctx->used_engine has just updated */
				qof_mtcmos_voter_handle(&ctx->cam->engines,
					ctx->used_engine, &ctx->unsupport_scen);
			}
...
_common_seamless_after_frame_done(struct mtk_cam_job *job)
{
	...
	mtk_cam_dvc_unint(&cam->dvfs.dvc, raw_dev->id);
...
int mtk_cam_job_uninit_engine(struct mtk_cam_job *job, int unit_engs)
{
	...
	qof_mtcmos_voter_handle_clear(&cam->engines, bit_map_subset_of(MAP_HW_RAW, unit_engs),
		  &ctx->unsupport_scen);
...
static void update_job_state_init_sensor_param(struct mtk_cam_job *job)
{
	...
	u64 real_interval_ns, interval_from_ctrl_data_ns =
		mtk_cam_query_interval_from_ctrl_data(job->src_ctx);
	...
	real_interval_ns =
		max(interval_from_ctrl_data_ns, ctrl->frame_interval_ns);
	...
	if (job->src_ctx->last_req_exposue.long_exposure_flow) {
		...
		job->job_state.s_params.i2c_thres_ns =
			infer_i2c_deadline_ns(job, real_interval_ns);
	...
	if (ctrl_data && ctrl_data->rc_data.exp_ns.long_exposure_flow) {
		job->job_state.cq_trigger_thres_ns = ULLONG_MAX;
	} else {
		job->job_state.cq_trigger_thres_ns =
			(ctrl_data && ctrl_data->trigger_cq_deadline > 0) ?
			ctrl_data->trigger_cq_deadline :
			infer_cq_trigger_deadline_ns(job, interval_from_ctrl_data_ns);
	}
	...
	if (CAM_DEBUG_ENABLED(JOB) ||
		...
		(ctrl_data && ctrl_data->rc_data.exp_ns.long_exposure_flow))
...
static void update_preframe_cq_rdy_mask_en(struct mtk_cam_job *job)
{
	...
	if (!job->seamless_switch && ctrl_data &&
		...
		/* force cq ready once to gurantee receive one SOF */
		set_engines_mux_cq_ready(job->src_ctx,
				job->seninf, job->used_engine, true);
	}
}
		...
		if (is_dcg_with_vs(job)) {
			...
			if (sink) {
				...
				for (int i = 0; i < result.linetime_cnt; i++) {
					if (i == 0)
						job->dcg_vs_min_lint = result.lut_read_linetimes_in_ns[i];
					pr_info("%s: dcg_vs line_t result[%d]: %d",
						__func__, i, result.lut_read_linetimes_in_ns[i]);
					job->dcg_vs_min_lint = min(job->dcg_vs_min_lint,
...
static void job_dump_engines_debug_status(struct mtk_cam_job *job)
{
	...
	mtk_engine_dump_debug_status(cam, job->used_engine, dma_debug_dump, job);
	if (ctx->seninf) {
...
int mtk_cam_job_update_clk_switching(struct mtk_cam_job *job, bool begin)
{
	...
	if (!begin)
		ret = mtk_cam_dvfs_switch_end(&cam->dvfs, ctx->stream_id,
					 raw_id, freq_hz);
	else
		ret = mtk_cam_dvfs_switch_begin(&cam->dvfs, ctx->stream_id,
	...
	for (i = 0; i < cam->engines.num_raw_devices; i++) {
		if (BIT(i) & subset) {
			raw_dev = dev_get_drvdata(cam->engines.raw_devs[i]);
			mtk_cam_dvc_dbg_dump(&ctx->cam->dvfs.dvc, raw_dev->id);
		}
	}

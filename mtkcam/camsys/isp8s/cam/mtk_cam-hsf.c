...
void ccu_stream_on(struct mtk_cam_ctx *ctx, unsigned int without_tg)
{
	...
	int time_start = 0, time_end = 0, us = 0;
	...
	time_start = ts.tv_sec * 1000000ULL + ts.tv_nsec / 1000ULL;
#endif
	pData.tg_idx = raw_id;
...
#ifdef PERFORMANCE_HSF
	ktime_get_real_ts64(&ts);
	time_end = ts.tv_sec * 1000000ULL + ts.tv_nsec / 1000ULL;
...
/**
 * for 26m on 6993 useage
 * is_pkvm_enabled : check pkvm enable or not, if enabled, do not use ccu.
 */
static u32 get_pkvm_status(struct mtk_cam_device *cam)
{
	struct device_node *camisp_np = NULL;
	u32 use_pkvm = 0;
	u32 ret = 0, pkvm_enable = 0;

	camisp_np = of_find_compatible_node(NULL, NULL, "mediatek,mt6993-camisp");
	if (!camisp_np) {
		pr_info("%s: Cannot find mt6993 camisp node, try mt6881 camisp node\n", __func__);
		camisp_np = of_find_compatible_node(NULL, NULL, "mediatek,mt6881-camisp");

		if (!camisp_np) {
			pr_info("%s: get_camisp_pkvm_value: Cannot find camisp node\n", __func__);
			return 0;
		}
	}

	/* read camsys-pkvm-enable property */
	ret = of_property_read_u32(camisp_np, "camsys-pkvm-enable", &pkvm_enable);
	if (!ret){
		if(pkvm_enable == 0x1)
			use_pkvm = 1;
		pr_info("%s: camsys-pkvm-enable = %u\n", __func__, pkvm_enable);
	} else
		pr_info("%s: camsys-pkvm-enable property not found\n", __func__);

	of_node_put(camisp_np);
...
int mtk_cam_hsf_init(struct mtk_cam_ctx *ctx)
{
	...
	pkvm_enabled = get_pkvm_status(cam);
...
#endif
		dev_info(cam->dev, "hsf(ccu) initial ready, pkvm(%d)\n", pkvm_enabled);
	} else {
		if (is_protected_kvm_enabled()){
			mtk_cam_pkvm_init(ctx);
			dev_info(cam->dev, "hsf(pkvm) initial ready, pkvm(%d)\n", pkvm_enabled);
		}
	}
...
int mtk_cam_hsf_stream_ctrl(struct mtk_cam_ctx *ctx, unsigned int without_tg)
{
	if(!pkvm_enabled){
		ccu_stream_on(ctx, without_tg);
	} else {
		if (is_protected_kvm_enabled())
			mtk_cam_pkvm_stream_ctrl(ctx, without_tg);
	}
	return 0;
}

int mtk_cam_sv_hsf_config(struct mtk_cam_ctx *ctx, bool En)
{
	if(!pkvm_enabled){
		ccu_hsf_camsv_config(ctx, En);
	} else {
		if (is_protected_kvm_enabled())
			mtk_cam_pkvm_sv_config(ctx, En);
	}
	return 0;
}
...
#define CQ_SIZE 0x1000
	...
	/*
	 * dma_map_cq: cq buffer is not allocated here.
	 * therefore, dma_map_cq will not be used.
	 * keep for future use.
	 * dma_map_chk: chunk buffer is for trust chain,
	 * to handshake with seninf.
	 */
	...
	if (pkvm_enabled) {
		...
		dev_info(cam->dev, "dev_to_attach raw_devs\n");
	} else if (smmu_v3_enabled()) {
		dev_to_attach = ctx->cam->smmu_dev;
		dev_info(cam->dev, "dev_to_attach smmu_dev\n");
	} else {
		dev_to_attach = &(hsf_config->ccu_pdev->dev);
		dev_info(cam->dev, "dev_to_attach ccu dev\n");
	}

	if (!dev_to_attach){
		dev_info(cam->dev, "dev_to_attach is null!\n");
	}
	...
	if (is_protected_kvm_enabled() && pkvm_enabled)
		mtk_cam_pkvm_setcam(ctx);
	...
	time_end = ts.tv_sec * 1000000ULL + ts.tv_nsec / 1000ULL;
	us = time_end - time_start;
	dev_info(cam->dev, "hsf ccu config time %d us\n", us);
#endif
	return 0;
}

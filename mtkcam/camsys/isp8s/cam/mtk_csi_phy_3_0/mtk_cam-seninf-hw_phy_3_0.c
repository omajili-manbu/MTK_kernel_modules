...
#define MT6881_IOMOM_VERSIONS "mt6881"
...
struct mtk_cam_seninf_ctle_setting_info {
	struct mtk_cam_seninf_cdphy_ctle_setting *setting_table;
	u8 table_size;
};

struct mtk_cam_seninf_ctle_channel_info {
	struct mtk_cam_seninf_ctle_setting_info intermediate;
	struct mtk_cam_seninf_ctle_setting_info standard;
};

struct mtk_cam_seninf_ctle_platform_table {
	char *platform;
	struct mtk_cam_seninf_ctle_channel_info cphy;
	struct mtk_cam_seninf_ctle_channel_info dphy;
};
...
static int mtk_cam_seninf_set_outmux_rdy_msk_cq_rdy_status(
	struct seninf_ctx *ctx, u8 outmux, bool rdy_status)
{
	void *pSeninf_outmux = NULL;
	...
	if (rdy_status == true)
		SENINF_BITS(pSeninf_outmux, SENINF_OUTMUX_CAM_CFG_RDY_CQ, SENINF_OUTMUX_CAM_CFG_RDY_CQ_SET, true);
	else
		SENINF_BITS(pSeninf_outmux, SENINF_OUTMUX_CAM_CFG_RDY_CQ, SENINF_OUTMUX_CAM_CFG_RDY_CQ_CLR, true);
...
{
	void *pSeninf_outmux_inner = NULL;
	void *pSeninf_outmux = NULL;
	...
	pSeninf_outmux = ctx->reg_if_outmux[outmux];
	pSeninf_outmux_inner = ctx->reg_if_outmux_inner[outmux];

	/* need add lock for register protection */
	SENINF_BITS(pSeninf_outmux, SENINF_OUTMUX_CSR_CFG_CTRL3,
				SENINF_OUTMUX_CAM_CFG_RDY_CQ_EN, cq_rdy_en);

	SENINF_BITS(pSeninf_outmux_inner, SENINF_OUTMUX_CSR_CFG_CTRL3,
				SENINF_OUTMUX_CAM_CFG_RDY_CQ_EN, cq_rdy_en);
...
static int mtk_cam_seninf_get_outmux_curr_tag_vc_by_tag(struct seninf_ctx *ctx,
	u8 outmux, u8 tag_id, u8 *vc, bool *vc_en)
{
	void *pSeninf_outmux = NULL;
	...
	if (unlikely(vc_en == NULL)) {
		pr_info("[%s][Err]vc_en is NULL\n", __func__);
		return -EINVAL;
	}
	...
	/* used inner register status to check current setting status */
	pSeninf_outmux = ctx->reg_if_outmux_inner[outmux];

	switch (tag_id) {
	case 0:
		*vc = SENINF_READ_BITS(pSeninf_outmux, SENINF_OUTMUX_SOURCE_CONFIG_1, SENINF_OUTMUX_TAG_VC_0);
		*vc_en = SENINF_READ_BITS(pSeninf_outmux, SENINF_OUTMUX_TAG_VCDT_FILT_0, SENINF_OUTMUX_FILT_EN_0);
		break;
	case 1:
		*vc = SENINF_READ_BITS(pSeninf_outmux, SENINF_OUTMUX_SOURCE_CONFIG_1, SENINF_OUTMUX_TAG_VC_1);
		*vc_en = SENINF_READ_BITS(pSeninf_outmux, SENINF_OUTMUX_TAG_VCDT_FILT_1, SENINF_OUTMUX_FILT_EN_1);
		break;
	case 2:
		*vc = SENINF_READ_BITS(pSeninf_outmux, SENINF_OUTMUX_SOURCE_CONFIG_1, SENINF_OUTMUX_TAG_VC_2);
		*vc_en = SENINF_READ_BITS(pSeninf_outmux, SENINF_OUTMUX_TAG_VCDT_FILT_2, SENINF_OUTMUX_FILT_EN_2);
		break;
	case 3:
		*vc = SENINF_READ_BITS(pSeninf_outmux, SENINF_OUTMUX_SOURCE_CONFIG_1, SENINF_OUTMUX_TAG_VC_3);
		*vc_en = SENINF_READ_BITS(pSeninf_outmux, SENINF_OUTMUX_TAG_VCDT_FILT_3, SENINF_OUTMUX_FILT_EN_3);
		break;
	case 4:
		*vc = SENINF_READ_BITS(pSeninf_outmux, SENINF_OUTMUX_SOURCE_CONFIG_2, SENINF_OUTMUX_TAG_VC_4);
		*vc_en = SENINF_READ_BITS(pSeninf_outmux, SENINF_OUTMUX_TAG_VCDT_FILT_4, SENINF_OUTMUX_FILT_EN_4);
		break;
	case 5:
		*vc = SENINF_READ_BITS(pSeninf_outmux, SENINF_OUTMUX_SOURCE_CONFIG_2, SENINF_OUTMUX_TAG_VC_5);
		*vc_en = SENINF_READ_BITS(pSeninf_outmux, SENINF_OUTMUX_TAG_VCDT_FILT_5, SENINF_OUTMUX_FILT_EN_5);
		break;
	case 6:
		*vc = SENINF_READ_BITS(pSeninf_outmux, SENINF_OUTMUX_SOURCE_CONFIG_2, SENINF_OUTMUX_TAG_VC_6);
		*vc_en = SENINF_READ_BITS(pSeninf_outmux, SENINF_OUTMUX_TAG_VCDT_FILT_6, SENINF_OUTMUX_FILT_EN_6);
		break;
	case 7:
		*vc = SENINF_READ_BITS(pSeninf_outmux, SENINF_OUTMUX_SOURCE_CONFIG_2, SENINF_OUTMUX_TAG_VC_7);
		*vc_en = SENINF_READ_BITS(pSeninf_outmux, SENINF_OUTMUX_TAG_VCDT_FILT_7, SENINF_OUTMUX_FILT_EN_7);
		break;
	default:
		break;
	}
...
static int csirx_phyA_init(struct seninf_ctx *ctx)
{
	...
	for (i = 0; i <= ctx->is_4d1c; i++) {
		...
		} else if (!strcasecmp(_seninf_ops->iomem_ver, MT6881_IOMOM_VERSIONS)) {
			SENINF_BITS(base, CDPHY_RX_ANA_1, RG_CSI0_BG_VREF_SEL, 0x4);/* mt6881 ana phy Page 5 */
			SENINF_BITS(base, CDPHY_RX_ANA_3, RG_CSI0_EQ_DES_VREF_SEL, 0x10);/* mt6881 ana phy Page 5 */
		} else {
	...
	if (ctx->is_cphy) {
		if (ctx->csi_param.not_fixed_trail_settle) {
			...
			if (settle_delay_dt == 0) {
				...
				seninf_logd(ctx, "cphy settle val: (%u, %llu) => %llu\n",
...
int eq_param_custom(struct seninf_ctx *ctx){
	int i, port = 0;
	void *base;
	struct mtk_csi_param* csi_param = &ctx->csi_param;
	dev_info(ctx->dev, "[%s] enable = %d, bw = %d, dg0_en = %d, sr0 = %d, dg1_en = %d, sr1 = %d\n", __func__,
		csi_param->eq_enable, csi_param->eq_bw, csi_param->eq_dg0_en, csi_param->eq_sr0, csi_param->eq_dg1_en, csi_param->eq_sr1);
	if(!csi_param->eq_enable){
		return 0;
	}
	if((csi_param->eq_bw != 0 && csi_param->eq_bw != 1 && csi_param->eq_bw != 3) ||
	   csi_param->eq_dg0_en > 1 || csi_param->eq_sr0 >= 16 ||
	   csi_param->eq_dg1_en > 1 || csi_param->eq_sr1 >= 16 ){
		return 0;
	}
	for (i = 0; i <= ctx->is_4d1c; i++) {
		port = i ? ctx->portB : ctx->port;
		...
		SENINF_BITS(base, CDPHY_RX_ANA_5,
				RG_CSI0_CDPHY_EQ_BW, csi_param->eq_bw);
		SENINF_BITS(base, CDPHY_RX_ANA_5,
				RG_CSI0_CDPHY_EQ_DG0_EN, csi_param->eq_dg0_en);
		SENINF_BITS(base, CDPHY_RX_ANA_5,
				RG_CSI0_CDPHY_EQ_SR0, csi_param->eq_sr0);
		SENINF_BITS(base, CDPHY_RX_ANA_5,
				RG_CSI0_CDPHY_EQ_DG1_EN, csi_param->eq_dg1_en);
		SENINF_BITS(base, CDPHY_RX_ANA_5,
				RG_CSI0_CDPHY_EQ_SR1, csi_param->eq_sr1);
	}
	return 0;
}
int cdr_delay_custom(struct seninf_ctx *ctx){
	u32 i, port = 0, val;
	void *base;
	struct mtk_csi_param* csi_param = &ctx->csi_param;
	dev_info(ctx->dev, "[%s] enable = %d, is_cphy = %d,cdr_delay = %d\n", __func__, csi_param->cdr_delay_enable, ctx->is_cphy, csi_param->cdr_delay);
	val =  csi_param->cdr_delay;
	if(ctx->is_cphy){
		if(!csi_param->cdr_delay_enable || (csi_param->cdr_delay > 31)){
			return 0;
		}
		for (i = 0; i <= ctx->is_4d1c; i++) {
			port = i ? ctx->portB : ctx->port;
			...
			if (!strcasecmp(_seninf_ops->iomem_ver, MT6991_IOMOM_VERSIONS) ||
				...
				!strcasecmp(_seninf_ops->iomem_ver, MT6881_IOMOM_VERSIONS)) {
...
static int csirx_dphy_lrte_setting(struct seninf_ctx *ctx)
{
	...
	int rg_wc_dummy_valid_cnt_val = SENINF_READ_BITS(csirx_mac_csi,
		CSIRX_MAC_CSI2_RESYNC_MERGE_CTRL2, RG_WC_DUMMY_VALID_CNT);
	int dphy_idle_sync_init_cnt = 0;

	if (!strcasecmp(_seninf_ops->iomem_ver, MT6881_IOMOM_VERSIONS)) {
		SENINF_BITS(dphy_base, DPHY_DPHYV21_CTRL, RG_HSIDLE_EN, 0);
		SENINF_BITS(dphy_base, DPHY_IDLE_SYNC_INIT_CTRL, DPHY_IDLE_0A_CK_EN, 0);
		SENINF_BITS(dphy_base, DPHY_IDLE_SYNC_INIT_CTRL, DPHY_IDLE_0B_CK_EN, 0);
		seninf_logd(ctx, "not support dphy_lrte\n");
		return 0;
	}
	...
	if (ctx->csi_param.dphy_lrte_support) {
		...
		dphy_idle_sync_init_cnt = 6 + rg_afifo_dummy_valid_prepare_num_val +
			rg_afifo_dummy_valid_num_val * (1 + afifo_dummy_valid_gap_num_val) + rg_wc_dummy_valid_cnt_val;

		SENINF_BITS(dphy_base, DPHY_IDLE_SYNC_INIT_CTRL, DPHY_IDLE_SYNC_INIT_CNT,
			dphy_idle_sync_init_cnt);
		SENINF_BITS(dphy_base, DPHY_DPHYV21_CTRL, RG_HSIDLE_EN, 1);
		...
		SENINF_BITS(dphy_base, DPHY_IDLE_SYNC_INIT_CTRL, DPHY_IDLE_0B_CK_SEL, 0x3);
...
static int csirx_cphy_lrte_setting(struct seninf_ctx *ctx)
{
	...
	if (ctx->is_cphy && ctx->csi_param.cphy_lrte_support) {
		...
		SENINF_BITS(csirx_mac_csi, CSIRX_MAC_CSI2_RESYNC_MERGE_CTRL2, RG_RESYNC_LRTE_WC_DMY_OPTION, 0);
		...
		seninf_logi(ctx, "cphy lrte support, enable CPHY_POST_ENCODE_EN|REPLACE_EN, port:%d\n",
			 ctx->port);
		seninf_logi(ctx,
...
static void csirx_phyA_dphy_setting(void *base, u64 data_rate)
{
	...
	if (!strcasecmp(_seninf_ops->iomem_ver, MT6993_IOMOM_VERSIONS)) {
		...
		/* data rate < 1.5 Gbps */
		if (data_rate < 1500000000) {
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_SR1, 0x0);/* mt6881 ana p15 */
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_SR0, 0x0);/* mt6881 ana p15 */
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_DG1_EN, 0x0);/* mt6881 no need */
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_DG0_EN, 0x0);/* mt6881 ana p15 */
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_IS, 0x2);/* mt6881 ana p15 */
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_BW, 0x0);/* mt6881 ana p15 */
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_IS_RDC, 0x0);/* mt6881 no need */
			SENINF_BITS(base, CDPHY_RX_ANA_14, RG_CSI0_CDPHY_EQ_OS_IS, 0x1);/* mt6881 ana p15 */
		}
		/* 1.5 Gbps <= data date < 2.5 Gbps */
		else if (data_rate < 2500000000) {
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_SR1, 0x0);/* mt6881 ana p15 */
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_SR0, 0x0);/* mt6881 ana p15 */
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_DG1_EN, 0x0);/* mt6881 no need */
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_DG0_EN, 0x1);/* mt6881 ana p15 */
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_IS, 0x2);/* mt6881 ana p15 */
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_BW, 0x1);/* mt6881 ana p15 */
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_IS_RDC, 0x0);/* mt6881 no need */
			SENINF_BITS(base, CDPHY_RX_ANA_14, RG_CSI0_CDPHY_EQ_OS_IS, 0x1);/* mt6881 ana p15 */
		}
		/* 2.5 Gbps <= data date < 4.5 Gbps */
			...
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_DG1_EN, 0x0);/* mt6881 no need */
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_DG0_EN, 0x1);/* mt6881 ana p15 */
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_IS, 0x1);/* mt6881 ana p15 */
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_BW, 0x2);/* mt6881 ana p15 */
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_IS_RDC, 0x0);/* mt6881 no need */
			SENINF_BITS(base, CDPHY_RX_ANA_14, RG_CSI0_CDPHY_EQ_OS_IS, 0x0);/* mt6881 ana p15 */
		}
	}
}
...
static void csirx_phyA_cphy_setting(void *base, u64 data_rate)
{
	...
	if (!strcasecmp(_seninf_ops->iomem_ver, MT6993_IOMOM_VERSIONS)) {
		...
		SENINF_BITS(base, CDPHY_RX_ANA_14, RG_CSI0_LDO_X26M_EN, 0x0);
		SENINF_BITS(base, CDPHY_RX_ANA_14, RG_CSI0_LDO_LP_EN, 0x1);
		SENINF_BITS(base, CDPHY_RX_ANA_3, RG_CSI0_EQ_DES_VREF_SEL, 0x10);/* mt6881 ana phy Page 17 */
		SENINF_BITS(base, CDPHY_RX_ANA_0, RG_CSI0_CPHY_EN, 1);/* mt6881 ana phy Page 17 */
		/* data rate < 1.5 Gsps */
		if (data_rate < 1500000000) {
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_BW, 0x1);/* mt6881 ana phy Page 17 */
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_DG0_EN, 0x0);/* mt6881 ana phy Page 17 */
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_DG1_EN, 0x0);/* mt6881 no need */
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_SR0, 0x0);/* mt6881 ana phy Page 17 */
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_SR1, 0x0);/* mt6881 no need */
			SENINF_BITS(base, CDPHY_RX_ANA_0, RG_CSI0_CPHY_T0_HSMODE_EN, 0);/* mt6881 no need */
			SENINF_BITS(base, CDPHY_RX_ANA_0, RG_CSI0_CPHY_T1_HSMODE_EN, 0);/* mt6881 no need */
			SENINF_BITS(base, CDPHY_RX_ANA_6, RG_CSI0_CPHY_T0_CDR_AB_WIDTH, 0xA);
			/* mt6881 ana phy Page 17 */
			SENINF_BITS(base, CDPHY_RX_ANA_6, RG_CSI0_CPHY_T0_CDR_BC_WIDTH, 0xA);
			/* mt6881 ana phy Page 17 */
			SENINF_BITS(base, CDPHY_RX_ANA_6, RG_CSI0_CPHY_T0_CDR_CA_WIDTH, 0xA);
			/* mt6881 ana phy Page 17 */
			SENINF_BITS(base, CDPHY_RX_ANA_6, RG_CSI0_CPHY_T0_CDR_CK_DELAY, 0x14);
			/* mt6881 ana phy Page 17 */
			SENINF_BITS(base, CDPHY_RX_ANA_7, RG_CSI0_CPHY_T1_CDR_AB_WIDTH, 0xA);
			/* mt6881 ana phy Page 17 */
			SENINF_BITS(base, CDPHY_RX_ANA_7, RG_CSI0_CPHY_T1_CDR_BC_WIDTH, 0xA);
			/* mt6881 ana phy Page 17 */
			SENINF_BITS(base, CDPHY_RX_ANA_7, RG_CSI0_CPHY_T1_CDR_CA_WIDTH, 0xA);
			/* mt6881 ana phy Page 17 */
			SENINF_BITS(base, CDPHY_RX_ANA_7, RG_CSI0_CPHY_T1_CDR_CK_DELAY, 0x14);
			/* mt6881 ana phy Page 17 */
			SENINF_BITS(base, CDPHY_RX_ANA_13, RG_CSI0_CPHY_T0_CDR_SEL_CODE, 0x0);/* mt6881 no need */
			SENINF_BITS(base, CDPHY_RX_ANA_13, RG_CSI0_CPHY_T1_CDR_SEL_CODE, 0x0);/* mt6881 no need */
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_IS, 0x2);/* mt6881 ana phy Page 17 */
			SENINF_BITS(base, CDPHY_RX_ANA_14, RG_CSI0_CDPHY_EQ_OS_IS, 0x1);/* mt6881 ana phy Page 17 */
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_IS_RDC, 0x1);/* mt6881 no need */
		}
		/* 1.5 Gsps<= data rate < 2.5 Gsps */
		else if (data_rate < 2500000000) {
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_BW, 0x1);/* mt6881 ana phy Page 17 */
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_DG0_EN, 0x1);/* mt6881 ana phy Page 17 */
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_DG1_EN, 0x0);/* mt6881 no need */
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_SR0, 0x0);/* mt6881 ana phy Page 17 */
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_SR1, 0x0);/* mt6881 no need */
			SENINF_BITS(base, CDPHY_RX_ANA_0, RG_CSI0_CPHY_T0_HSMODE_EN, 0);/* mt6881 no need */
			...
			SENINF_BITS(base, CDPHY_RX_ANA_6, RG_CSI0_CPHY_T0_CDR_CA_WIDTH, 0xA);
			/* mt6881 ana phy Page 17 */
			SENINF_BITS(base, CDPHY_RX_ANA_6, RG_CSI0_CPHY_T0_CDR_CK_DELAY, 0xC);
			/* mt6881 ana phy Page 17 */
			SENINF_BITS(base, CDPHY_RX_ANA_7, RG_CSI0_CPHY_T1_CDR_AB_WIDTH, 0xA);
			/* mt6881 ana phy Page 17 */
			SENINF_BITS(base, CDPHY_RX_ANA_7, RG_CSI0_CPHY_T1_CDR_BC_WIDTH, 0xA);
			...
			SENINF_BITS(base, CDPHY_RX_ANA_7, RG_CSI0_CPHY_T1_CDR_CA_WIDTH, 0xA);
			/* mt6881 ana phy Page 17 */
			SENINF_BITS(base, CDPHY_RX_ANA_7, RG_CSI0_CPHY_T1_CDR_CK_DELAY, 0xC);
			/* mt6881 ana phy Page 17 */
			SENINF_BITS(base, CDPHY_RX_ANA_13, RG_CSI0_CPHY_T0_CDR_SEL_CODE, 0x0);/* mt6881 no need */
			SENINF_BITS(base, CDPHY_RX_ANA_13, RG_CSI0_CPHY_T1_CDR_SEL_CODE, 0x0);/* mt6881 no need */
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_IS, 0x2);/* mt6881 ana phy Page 17 */
		...
		/* 2.5 Gsps<= data rate < 4.5 Gsps */
			...
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_BW, 0x3);/* mt6881 ana phy Page 17 */
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_DG0_EN, 0x1);/* mt6881 ana phy Page 17 */
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_DG1_EN, 0x0);/* mt6881 no need */
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_SR0, 0x0);/* mt6881 ana phy Page 17 */
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_SR1, 0x0);/* mt6881 no need */
			...
			SENINF_BITS(base, CDPHY_RX_ANA_6, RG_CSI0_CPHY_T0_CDR_CA_WIDTH, 0xA);
			/* mt6881 ana phy Page 17 */
			SENINF_BITS(base, CDPHY_RX_ANA_6, RG_CSI0_CPHY_T0_CDR_CK_DELAY, 0x5);
			/* mt6881 ana phy Page 17 */
			SENINF_BITS(base, CDPHY_RX_ANA_7, RG_CSI0_CPHY_T1_CDR_AB_WIDTH, 0xA);
			/* mt6881 ana phy Page 17 */
			SENINF_BITS(base, CDPHY_RX_ANA_7, RG_CSI0_CPHY_T1_CDR_BC_WIDTH, 0xA);
			...
			SENINF_BITS(base, CDPHY_RX_ANA_7, RG_CSI0_CPHY_T1_CDR_CA_WIDTH, 0xA);
			/* mt6881 ana phy Page 17 */
			SENINF_BITS(base, CDPHY_RX_ANA_7, RG_CSI0_CPHY_T1_CDR_CK_DELAY, 0x5);
			/* mt6881 ana phy Page 17 */
			SENINF_BITS(base, CDPHY_RX_ANA_13, RG_CSI0_CPHY_T0_CDR_SEL_CODE, 0x0);/* mt6881 no need */
			SENINF_BITS(base, CDPHY_RX_ANA_13, RG_CSI0_CPHY_T1_CDR_SEL_CODE, 0x0);/* mt6881 no need */
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_IS, 0x1);/* mt6881 ana phy Page 17 */
			SENINF_BITS(base, CDPHY_RX_ANA_14, RG_CSI0_CDPHY_EQ_OS_IS, 0x0);/* mt6881 ana phy Page 17 */
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_IS_RDC, 0x0);/* mt6881 no need */
		}
	}
}
...
static int mtk_cam_seninf_set_idle(struct seninf_ctx *ctx)
{
	...
	for (i = 0; i < SENINF_OUTMUX_NUM; i++) {
		...
		seninf_logd(ctx, "[%s] force reset outmux %d done\n", __func__, i);
	}

	memset(ctx->outmux_force_disable_list, 0, sizeof(ctx->outmux_force_disable_list));
	seninf_logd(ctx, "release all outmux set all pd2cam to 0xff\n");
	...
	struct device_attribute *attr, char *buf)
{
	...
	list_for_each_entry(ctx, &core->list, list) {
		...
		SHOW(buf, len,
			"MipiRx_ANA%d:CDPHY_RX_ANA_9:(0x%x)/ANA_8:(0x%x)/ANA_7:(0x%x)/ANA_6:(0x%x)/ANA_5:(0x%x)\n",
			ctx->port,
			SENINF_READ_REG(base_ana, CDPHY_RX_ANA_9),
...
static int mtk_cam_seninf_debug(struct seninf_ctx *ctx)
{
	...
	void *pSeninf_outmux = NULL, *pSeninf_outmux_inner = NULL;
	struct mtk_cam_seninf_lprx_info lprx_info;
	// check sensor output 
	bool pkcntChange = true;
	bool CheckIrqstatus = true;
	bool CheckmuxTagDone = true;
	bool vcEnable = false;
	bool checkSocketStatus = true;
			...
			for (k = 0; k < ctx->vcinfo.vc[j].dest_cnt; k++) {
				...
				for (i = 0; i < _seninf_ops->outmux_num; i++) {
					if ((used_outmux == i) && mtk_cam_seninf_is_outmux_used(ctx, i)) {
						...
						pSeninf_outmux_inner = ctx->reg_if_outmux_inner[used_outmux];
						...
						seninf_logi(ctx,
							...
							SENINF_READ_BITS(pSeninf_outmux,
								...
								SENINF_OUTMUX_CAM_RDY_GRP_SEL),
							SENINF_READ_BITS(pSeninf_outmux_inner,
	...
	if (sot_err) {
		...
		"[ERROR] sensor sot err is_cphy %d sot_err: 0x%x\n",
			ctx->is_cphy, sot_err);
		seninf_logi(ctx,"sensor_output_check: check sensor output fail, SOT ERR!!");
	...
	if ((mac_irq & 0xD0) || seninf_irq){
		...
		CheckIrqstatus = false;
		seninf_logi(ctx,
			"sensor_output_check: check irq staus fail: multi lanes sync error, crc error, ecc error, irq status = 0x%x\n",mac_irq);
	}
			...
			for (k = 0; k < ctx->vcinfo.vc[j].dest_cnt; k++) {
				...
				for (i = 0; i < _seninf_ops->outmux_num; i++) {
					if ((used_outmux == i) && mtk_cam_seninf_is_outmux_used(ctx, i)) {
						...
						//check tag done
						if(pkcntChange && ((irq_st & (1 << (used_tag + 24))) == 0)){
							CheckmuxTagDone =false;
							seninf_logi(ctx,"sensor_output_check: check tag done failed: vc =0x%x,dt =0x%x, outmux %d, tag %d ,expect size =%d*%d, rec size =%d*%d", ctx->vcinfo.vc[j].vc,
							ctx->vcinfo.vc[j].dt, used_outmux, used_tag,((exp_sz & 0xffff)+1)*8/rawbit, ((exp_sz>>16)+1),(res>>16)*8/rawbit,res&0xffff);
						}
							vcEnable = true;
					}
				}
	...
	if((mac_irq & 0x324) != 0x324){
		if(!CheckmuxTagDone){
			seninf_logi(ctx,"sensor_output_check: mac_irq(0x%x) != 0x324, CheckmuxTagDone is false, set CheckIrqstatus is false\n", mac_irq);
			CheckIrqstatus = false;
		}
	}
	if(!vcEnable){
		seninf_logi(ctx,"sensor_output_check: vcEnable is false, set CheckmuxTagDone is	false\n");
		CheckmuxTagDone = false;
	}

	if(pkcntChange  && CheckIrqstatus && checkSocketStatus && CheckmuxTagDone){
		seninf_logi(ctx,"sensor_output_check: check sensor output pass\n");
	}else{
		if(!pkcntChange){
			seninf_logi(ctx,"sensor_output_check: check sensor output fail, pkcntChange is false");
		}
		if(!CheckIrqstatus){
			seninf_logi(ctx,"sensor_output_check: check sensor output fail, CheckIrqstatus is false");
		}
		if(!checkSocketStatus){
			seninf_logi(ctx,"sensor_output_check: check sensor output fail, socket error happened");
		}
		if(!CheckmuxTagDone){
			seninf_logi(ctx,"sensor_output_check: check sensor output fail, CheckmuxTagDone is false,sensor output size may be not enough");
		}
	}
...
static int mtk_cam_seninf_debug_current_status(struct seninf_ctx *ctx)
{
	...
	if((ctx->debug_cur_mac_irq & 0x324) != 0x324){
		if(!CheckmuxTagDone){
			seninf_logi(ctx,"sensor_output_check: mac_irq(0x%x) != 0x324, CheckmuxTagDone is false, set CheckIrqstatus is false\n", ctx->debug_cur_mac_irq);
			CheckIrqstatus = false;
		}
	...
	if(pkcntChange  && CheckIrqstatus && CheckmuxTagDone){
		seninf_logi(ctx,"sensor_output_check: check sensor output pass\n");
	}else{
		if(!pkcntChange){
			seninf_logi(ctx,"sensor_output_check: check sensor output fail, pkcntChange is false");
		...
		if(!CheckmuxTagDone){
			seninf_logi(ctx,"sensor_output_check: check sensor output fail, CheckmuxTagDone is false,sensor output size is not enough");
		}
	}
...
static void mtk_cam_seninf_irq_event_st_init(struct seninf_core *core)
{
	...
	if (likely(vsync_detect_seninf_irq_event.msg_buffer == NULL)) {
		vsync_detect_seninf_irq_event.msg_buffer = vzalloc(
			...
			u8 cfg_mode, struct outmux_tag_cfg *tag_cfg)
{
	...
	if (!strcasecmp(_seninf_ops->iomem_ver, MT6881_IOMOM_VERSIONS)) {
		SENINF_BITS(pSeninf_mux, SENINF_OUTMUX_CSR_CFG_CTRL, SENINF_OUTMUX_VERIF_EN, 0x1);
	}
...
int mtk_cam_seninf_set_outmux_cfg_done(struct seninf_ctx *ctx, u8 outmux_idx)
{
	...
	seninf_logd(ctx, "raise outmux%u cfg done, curr irq st:0x%x", outmux_idx,
...
static int mtk_cam_seninf_config_cphy_insertion_loss_table(
	struct seninf_ctx *ctx, struct mtk_cam_seninf_cdphy_ctle_param *param)
{
	int i, port;
	void *base;
...
static int mtk_cam_seninf_config_dphy_insertion_loss_table(
	struct seninf_ctx *ctx, struct mtk_cam_seninf_cdphy_ctle_param *param)
{
	int i, port;
	void *base;
...
static struct mtk_cam_seninf_ctle_setting_info *get_ctle_setting_table_by_insertion_loss(
	struct seninf_ctx *ctx, const struct mtk_sensor_insertion_loss *insertion_param)
{
	u8 i, ctle_list_size;
	struct mtk_cam_seninf_ctle_setting_info *ctle_setting_info = NULL;
	struct mtk_cam_seninf_ctle_channel_info *ctle_channel_info = NULL;
	struct mtk_cam_seninf_ctle_platform_table ctle_platform_list[] = {
		{
			.platform = "mt6993",
			.cphy = {
				.intermediate = {
					.setting_table = cphy_intermediate_channel,
					.table_size = ARRAY_SIZE(cphy_intermediate_channel),
				},
				.standard = {
					.setting_table = cphy_standard_channel,
					.table_size = ARRAY_SIZE(cphy_standard_channel),
				},
			},
			.dphy = {
				.intermediate = {
					.setting_table = dphy_intermediate_channel,
					.table_size = ARRAY_SIZE(dphy_intermediate_channel),
				},
				.standard = {
					.setting_table = dphy_standard_channel,
					.table_size = ARRAY_SIZE(dphy_standard_channel),
				},
			},
		},
	};
	...
	if (unlikely(insertion_param == NULL)) {
		pr_info("[%s]insertion_param is NULL\n", __func__);
		return NULL;
	}

	ctle_list_size = ARRAY_SIZE(ctle_platform_list);

	for (i = 0; i < ctle_list_size; i++) {
		if (strcasecmp(_seninf_ops->iomem_ver, ctle_platform_list[i].platform))
			continue;

		ctle_channel_info = (ctx->is_cphy) ?
			&ctle_platform_list[i].cphy : &ctle_platform_list[i].dphy;

		if (ctle_channel_info == NULL) {
			pr_info("[%s]can not target any ctle table on %s\n",
				__func__, _seninf_ops->iomem_ver);
			return NULL;
		}

		if (insertion_param->loss >= 2500)
			ctle_setting_info = &ctle_channel_info->standard;
		else if (insertion_param->loss >= 1800 && insertion_param->loss < 2500)
			ctle_setting_info = &ctle_channel_info->intermediate;

		break;
	}

	return ctle_setting_info;
}

static struct mtk_cam_seninf_cdphy_ctle_param *get_ctle_setting_param_by_data_rate(struct seninf_ctx *ctx,
	struct mtk_cam_seninf_ctle_setting_info *ctle_setting_info)
{
	struct seninf_vc *vc = NULL;
	struct mtk_cam_seninf_cdphy_ctle_setting *table;
	u64 data_rate = 0;
	u8 i;
	...
	if (unlikely(ctle_setting_info == NULL)) {
		pr_info("[%s]ctle_setting_info is NULL\n", __func__);
		return NULL;
	}
	...
	for (i = 0; i < ctle_setting_info->table_size; i++) {
		table = &ctle_setting_info->setting_table[i];

		if (data_rate > table->max_data_rate)
			continue;

		if (data_rate < table->min_data_rate)
			continue;
...
{
	struct mtk_cam_seninf_ctle_setting_info *ctle_setting_info;
	struct mtk_cam_seninf_cdphy_ctle_param *ctle_setting_param;
	...
	ctle_setting_info = get_ctle_setting_table_by_insertion_loss(ctx, param);

	if (ctle_setting_info == NULL) {
		pr_info("[%s]ctle_setting_info is NULL\n", __func__);
		return -EFAULT;
	}

	ctle_setting_param = get_ctle_setting_param_by_data_rate(ctx, ctle_setting_info);

	if (ctle_setting_param == NULL) {
		pr_info("[%s]ctle_setting_param is NULL\n", __func__);
		return -EFAULT;
	}

	return (ctx->is_cphy) ?
		mtk_cam_seninf_config_cphy_insertion_loss_table(ctx, ctle_setting_param) :
		mtk_cam_seninf_config_dphy_insertion_loss_table(ctx, ctle_setting_param);
}
		...
		struct mtk_sensor_ctle_param *param)
{
	...
	for (i = 0; i <= ctx->is_4d1c; i++) {
		...
		if (!strcasecmp(_seninf_ops->iomem_ver, MT6993_IOMOM_VERSIONS)) {
			/* only set value when eq_is is exceed hw default value, cannot lower than this */
			if (param->eq_is > 0x2)
				SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_IS, param->eq_is);
			else
				SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_IS, 0x2);
		...
		/* only set value when eq_bw is exceed hw default value, cannot lower than this */
		if (param->eq_bw > 0x1)
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_BW, param->eq_bw);
		else
			SENINF_BITS(base, CDPHY_RX_ANA_5, RG_CSI0_CDPHY_EQ_BW, 0x1);
...
struct mtk_cam_seninf_ops mtk_csi_phy_3_0 = {
	...
	._set_outmux_rdy_msk_cq_rdy_status = mtk_cam_seninf_set_outmux_rdy_msk_cq_rdy_status,
	...
	._get_outmux_curr_tag_vc_by_tag = mtk_cam_seninf_get_outmux_curr_tag_vc_by_tag,

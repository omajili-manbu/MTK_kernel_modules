...
/*
 *  struct mtk_cam_uapi_ae_downsample_block_cfg - downsampled block statistics info for AE
 *
 *  @downsample_block_enable:   enable switch for downsample block stats (AEDO)
 *  @downsample_block_num_x:   horizontal block number of downsample block stats (AEDO)
 *  @downsample_block_num_y:   vertical block number of downsample block stats (AEDO)
 *  @downsample_block_size_x:   horizontal size of each downsample block
 *  @downsample_block_size_y:   vertical size of each downsample block
 */

struct mtk_cam_uapi_ae_downsample_block_cfg {
__u16 downsample_block_num_x;
__u16 downsample_block_num_y;
__u16 downsample_block_size_x;
__u16 downsample_block_size_y;
};
...
struct mtk_cam_uapi_ae_downsample_block_cfg downsample_block_cfg;
...
/**
 * struct mtk_cam_uapi_pda_stats - statistics of pd
 *
 * @stats_src:     source width and heitgh of the statistics.
 * @stride:     stride value used by
 * @pda_buf:     The buffer for PDA statistic hardware output.
 *
 * This is the PD statistic returned to user.
 */
struct mtk_cam_uapi_pda_dc_stats {
struct  mtk_cam_uapi_meta_size stats_src;
__u32   stride;
struct  mtk_cam_uapi_meta_hw_buf pda_buf;
};
 ...
 *
 */
struct mtk_cam_uapi_pda_dc_param {
__u32 pda_index;
__u32 width;
__u32 height;
__u32 stride;
__u32 pack_mode;
__u32 dilation_cfg;
__u32 cfg[MTK_CAM_UAPI_PDA_DC_MAX_CFG_NUM];
};
...
struct mtk_cam_uapi_pda_dc_param pda_dc_param;
};
...
__u8   cpi_0_stats_enabled;
__u8   cpi_1_stats_enabled;

struct mtk_cam_uapi_pdp_stats pdp_0_stats;
struct mtk_cam_uapi_pdp_stats pdp_1_stats;
struct mtk_cam_uapi_cpi_stats cpi_0_stats;
struct mtk_cam_uapi_cpi_stats cpi_1_stats;
};

/**
 * struct mtk_cam_uapi_meta_pda_dc_stats_0 - capture buffer returns from
 *   camsys's mraw module after the frame is done. The buffer are
 *   not be pushed the other driver such as dip.
 *
 * @stats_enabled:   indicate that stats is ready or not in
 *       this buffer
 */
struct mtk_cam_uapi_meta_pda_dc_stats_0 {
__u8   pda_dc_stats_enabled;
__u8   pda_dc_status;

struct mtk_cam_uapi_pda_dc_stats pda_dc_stats;
};
#define SHIFT_NUM 5  /* 2^5 = 32 */
#define ALIGN_SIZE(input) \
(((input + (1 << SHIFT_NUM) - 1) >> SHIFT_NUM) << SHIFT_NUM)
...
#define MTK_CAM_UAPI_AEO_DUMMY_BLK_XNUM (4)
#define MTK_CAM_UAPI_AEO_MAX_BUF_SIZE (MTK_CAM_UAPI_AEO_BLK_SIZE \
* (MTK_CAM_UAPI_AEO_MAX_BLK_X + MTK_CAM_UAPI_AEO_DUMMY_BLK_XNUM) \
...
#define MTK_CAM_UAPI_AEDO_BLK_SIZE (32)
#define MTK_CAM_UAPI_AEDO_MAX_BLK_X (30)
#define MTK_CAM_UAPI_AEDO_MAX_BLK_Y (30)
#define MTK_CAM_UAPI_AEDO_MAX_BUF_SIZE (MTK_CAM_UAPI_AEDO_BLK_SIZE \
* MTK_CAM_UAPI_AEDO_MAX_BLK_X \
* MTK_CAM_UAPI_AEDO_MAX_BLK_Y)

#define MTK_CAM_UAPI_STATCOLO_REAL_DMAO_SIZE (280)
 ...
 * @aedo_buf: The buffer for AEDO statistic hardware output.
 *        The maximum size of the buffer is defined with
 *        MTK_CAM_UAPI_AEDO_MAX_BUF_SIZE.
 * @statcolo_buf: The buffer for STATCOLO statistic hardware output.
 *        The maximum size of the buffer is defined with
...
__u32 ae_stat_en_status;  /* aeo & aeho */
__u32 aedo_stat_en_status;
struct mtk_cam_uapi_meta_hw_buf aeo_buf;
struct mtk_cam_uapi_meta_hw_buf aeho_buf;
struct mtk_cam_uapi_meta_hw_buf aedo_buf;
};

struct mtk_cam_uapi_statcolo_stats {
__u32 statcolo_stat_en_status;
struct mtk_cam_uapi_meta_hw_buf statcolo_buf;
};
...
#define MTK_CAM_UAPI_PDO_MAX_BUF_NUM (24)

/**
 * struct mtk_cam_uapi_pd_stats - statistics of pd
 *
 * @pd_stat_mode:    p1 mode, 0:normal, 1:slice, 2:twin
 * @num_of_pd_stat:  the number of the statistics.
 * @pd_stats:        The buffer for PD statistic hardware output.
 *
 * This is the PD statistic returned to user.
 */
...
struct mtk_cam_uapi_pde_meta_size {
__u32 width;
__u32 height;
__u32 offset;
__u32 size;
...
struct mtk_cam_uapi_pde_stats {
__u32 pd_stat_mode;
__u32 num_of_pd_stat;
__u32 num_of_slice;
struct mtk_cam_uapi_pde_meta_size stats_src[MTK_CAM_UAPI_PDO_MAX_BUF_NUM];
__u32 stride;
struct mtk_cam_uapi_meta_hw_buf pdo_buf;
};
...
#define MTK_CAM_UAPI_TCYSO_RAW_SIZE (68)
#define MTK_CAM_UAPI_TCYSO_SIZE ALIGN_SIZE(MTK_CAM_UAPI_TCYSO_RAW_SIZE * 3)
...
struct mtk_cam_uapi_meta_raw_stats_isz_cfg {
struct mtk_cam_uapi_meta_raw_stats_cfg meta;
struct mtk_cam_uapi_meta_raw_stats_rawb_cfg meta_b;
};
...
struct mtk_cam_uapi_statcolo_stats statcolor_stats;
...
struct mtk_cam_uapi_pde_stats pde_stats;
...
#define MTK_CAM_META_CHIP_NAME "mt6881"

...
#define CAM_DEBUG 0
#define ENABLE_CCU
#define CAMSYS_RAW_MASK 0x7  /* 0b0001 RAW A | 0b0010 RAW B | 0b0100 RAW C */
#define CAMSYS_RAW_MASK_DEFAULT 0xff /* let middleware decide*/
...
#endif
#ifdef CAMSYS_ISP8S_MT6881
		{.compatible = "mediatek,mt6881-camisp", .data = &mt6881_data},
#endif
		{}
};
#endif
...
#endif
#ifdef CAMSYS_ISP8S_MT6881
		{.compatible = "mediatek,mt6881-camisp-vcore",},
#endif
		{}
};
#endif
			     ...
			     bool is_proc)
{
	...
	if (unlikely(list_empty(&done_list_sensor))) {
		dev_info(dev,
			 "%s: req:%s(%d) failed to find pipe_id:%d node_id:%d%s\n",
			 __func__, req->debug_str, job->req_seq,
			 pipe_id, node_id,
			 is_buf_empty_sensor ? " (empty)" : "");
		goto REQ_PUT;
	}
...
#define OBJ_LIST_DEPTH 16  /* how to determine size */
void mtk_cam_ctx_cleanup_pending(struct mtk_cam_ctx *ctx)
{
	struct mtk_cam_device *cam = ctx->cam;
	struct mtk_cam_request *req, *req_prev;
	struct media_request_object *obj, *obj_list[OBJ_LIST_DEPTH];
	struct list_head cleanup_list;
	unsigned long handled_pipe = 0, tmp_pipe, ids;
	int pipe_id, req_cnt = 0, obj_cnt = 0;
	bool is_buf_empty;
	...
	if (list_empty(&cam->pending_job_list)) {
		if (CAM_DEBUG_ENABLED(V4L2))
			dev_info(cam->dev, "%s: pending list is empty\n",__func__);
		goto UNLOCK_PENDING;
	}

	list_for_each_entry_safe(req, req_prev, &cam->pending_job_list, list) {
		/* make sure that ctx is updated */
		if (mtk_cam_req_try_update_used_ctx(&req->req))
			continue;

		if (!(req->used_ctx & bit_map_bit(MAP_STREAM, ctx->stream_id)))
			continue;

		handled_pipe = req->used_pipe & ctx->used_pipe;
		if (!handled_pipe)
			continue;

		/* clean up the buf belonged to ctx in req */
		ids = 0;
		is_buf_empty = false;
		for (pipe_id = 0, tmp_pipe = handled_pipe;
		     pipe_id <= MTKCAM_SUBDEV_MAX && tmp_pipe;
		     pipe_id++, tmp_pipe >>= 1) {
			if (!(tmp_pipe & 0x1))
				continue;
			is_buf_empty =
				!mtk_cam_req_collect_vb_bufs(req,
							     pipe_id,
							     -1,
							     false,
							     &cleanup_list,
		...
		/* clean up the ctrls belonged to ctx in req */
		/* raw ctrl is handled before appended in to pending */
		/* sensor ctrl */
		obj = (ctx->sensor) ?
			mtk_cam_req_find_ctrl_obj(req, ctx->sensor->ctrl_handler) :
			NULL;
		if (obj && (obj_cnt < OBJ_LIST_DEPTH))
			obj_list[obj_cnt++] = obj;

		/* remove flags */
		req->used_pipe &= ~handled_pipe;
		req->used_ctx &= ~bit_map_bit(MAP_STREAM, ctx->stream_id);
		...
		if (CAM_DEBUG_ENABLED(V4L2))
			dev_info(cam->dev,
				"%s:%s ctx-%d pipe:%#lx, bufs:%#lx, sensor_obj:%d\n",
				__func__, req->debug_str, ctx->stream_id,
				handled_pipe, ids, !!obj);
	}

	dev_info(cam->dev, "%s: ctx-%d req_cnt:%d obj_cnt:%d, %s\n",
		__func__, ctx->stream_id, req_cnt, obj_cnt,
		list_empty(&cam->pending_job_list) ? "pending is empty" : "");

UNLOCK_PENDING:
	spin_unlock(&cam->pending_job_lock);

	/* handle buf/obj after lock */
	if (!list_empty(&cleanup_list))
		mark_each_buffer_done(&cleanup_list, VB2_BUF_STATE_ERROR, 0, 0);

	for (obj_cnt--; obj_cnt >= 0; obj_cnt--)
		mtk_cam_req_complete_ctrl_obj(obj_list[obj_cnt]);
}
			       ...
			       struct v4l2_event *event)
{
	...
	if(WARN_ON(ctx->raw_subdev_idx < 0))
		return -1;
			       ...
			       struct v4l2_event *event)
{
	...
	if(WARN_ON(ctx->sv_subdev_idx[0] < 0))
		return -1;
...
static inline bool support_rms_freerun(void)
{
	return (rms_freerun && GET_PLAT_HW(rms_mtc_freerun_support));
}
...
void mtk_cam_dump_ufbc_header(struct mtk_cam_device *cam,
				     struct mtk_cam_job *job)
{
	int i, j;

	if (!job || !job->ufbc_header.used)
		return;

	for (i = 0; i < UBFC_HEADER_PARAM_MAX; i++) {
		struct mtk_cam_ufbc_header_entry *entry = &job->ufbc_header.entry[i];

		if (entry->ipi_id == MTKCAM_IPI_CAMSV_MAIN_OUT && entry->vaddr) {
			dev_info(cam->dev,
				"[UFBC_DUMP] job:%d entry[%d] ipi_id:%d size:%d vaddr:%p, stride:%d, height:%d\n",
				job->req_seq, i, entry->ipi_id, entry->size, entry->vaddr,
				entry->stride, entry->height);

			/* Dump UFBC header content */
			if (entry->size > 0) {
				unsigned int offset = entry->stride * entry->height +
					sizeof(struct UfbcBufferHeader);
				int dump_size = entry->size - offset;
				u8 *data = (u8 *)entry->vaddr + offset;

				int total_elements = dump_size;

				dev_info(cam->dev, "[UFBC_DUMP] Header content (%d bytes) offset(%d bytes)\n",
					dump_size, offset);
				for (j = 0; j < total_elements; j += 32) {
					dev_info(cam->dev, "[UFBC_DUMP] [%04x]: %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n",
						(unsigned int)j,
						(j < total_elements) ? data[j] : 0,
						(j + 1 < total_elements) ? data[j + 1] : 0,
						(j + 2 < total_elements) ? data[j + 2] : 0,
						(j + 3 < total_elements) ? data[j + 3] : 0,
						(j + 4 < total_elements) ? data[j + 4] : 0,
						(j + 5 < total_elements) ? data[j + 5] : 0,
						(j + 6 < total_elements) ? data[j + 6] : 0,
						(j + 7 < total_elements) ? data[j + 7] : 0,
						(j + 8 < total_elements) ? data[j + 8] : 0,
						(j + 9 < total_elements) ? data[j + 9] : 0,
						(j + 10 < total_elements) ? data[j + 10] : 0,
						(j + 11 < total_elements) ? data[j + 11] : 0,
						(j + 12 < total_elements) ? data[j + 12] : 0,
						(j + 13 < total_elements) ? data[j + 13] : 0,
						(j + 14 < total_elements) ? data[j + 14] : 0,
						(j + 15 < total_elements) ? data[j + 15] : 0,
						(j + 16 < total_elements) ? data[j + 16] : 0,
						(j + 17 < total_elements) ? data[j + 17] : 0,
						(j + 18 < total_elements) ? data[j + 18] : 0,
						(j + 19 < total_elements) ? data[j + 19] : 0,
						(j + 20 < total_elements) ? data[j + 20] : 0,
						(j + 21 < total_elements) ? data[j + 21] : 0,
						(j + 22 < total_elements) ? data[j + 22] : 0,
						(j + 23 < total_elements) ? data[j + 23] : 0,
						(j + 24 < total_elements) ? data[j + 24] : 0,
						(j + 25 < total_elements) ? data[j + 25] : 0,
						(j + 26 < total_elements) ? data[j + 26] : 0,
						(j + 27 < total_elements) ? data[j + 27] : 0,
						(j + 28 < total_elements) ? data[j + 28] : 0,
						(j + 29 < total_elements) ? data[j + 29] : 0,
						(j + 30 < total_elements) ? data[j + 30] : 0,
						(j + 31 < total_elements) ? data[j + 31] : 0);
				}
			}
		}
	}
				  ...
				  struct mtk_cam_job *job)
{
	...
	if (job && job->is_dc_mode_skip_error) {
		mtk_cam_dump_ufbc_header(cam, job);
		job->is_dc_mode_skip_error = 0;
	}
...
static u8 mtk_cam_read_segment_efuse(struct device *dev)
{
	...
	efuse_cell = nvmem_cell_get(dev, "efuse_segment_cell");
	if (IS_ERR(efuse_cell)) {
		dev_err(dev, "cannot get efuse_segment_cell %ld\n",
			PTR_ERR(efuse_cell));
		goto exit;
	}
	...
	if (IS_ERR(efuse_buf)) {
		dev_err(dev, "@%s: cannot get efuse_buf\n", __func__);
		goto exit;
	}

	efuse_id =  (*efuse_buf & 0xff);
exit:
	dev_info(dev, "efuse_segment_cell: 0x%x\n", efuse_id);
	if (efuse_buf != NULL)
		kfree(efuse_buf);
	return efuse_id;
}
...
{
	u8 res = CAMSYS_RAW_MASK_DEFAULT;

	struct nvmem_cell *cell = NULL;
	size_t len = 0;
	u32 *buf = NULL;
	u8 segment_code = mtk_cam_read_segment_efuse (dev) & 0xf; /* keep low 4 bits */
	...
	if (segment_code  == 0x2
		...
		* isp camsys efuse:
		*
		* dx5 efuse address: (0x10165b54)
		* efuse data bitmap(uint32_t):
		* RAW A NG bit: 0x01000000
		* RAW B NG bit: 0x02000000
		* RAW C NG bit: 0x04000000
		*/
		if ((*buf) & (CAMSYS_RAW_MASK << 24)) {
			res = (u8)((*buf) >> 24);
...
static int mtk_cam_runtime_resume(struct device *dev)
{
	...
	mtk_cam_vcore_ccu_qos_remap(cam_dev); /* TODO: remove CCU by platform */

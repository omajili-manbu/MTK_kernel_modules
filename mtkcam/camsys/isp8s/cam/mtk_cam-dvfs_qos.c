	...
	for (i = 0; i < CAMSV_MAX_TAGS; i++) {
		...
		case SMI_PORT_SV_TYPE2_NUM:
			if ((is_dcg_ap_merge(job) &&
				is_raw_twin && slc_mode == SLC_WITH_DISCARD &&
				ipifmt_is_raw_ufo(in->fmt.format)) ||
				(i == SVTAG_2 && is_raw_twin &&
				slc_mode == SLC_WITH_DISCARD &&
				ipifmt_is_raw_ufo(in->fmt.format))) {
...
static void apply_sv_qos(struct mtk_cam_job *job)
{
	...
	if (sv_dev) {
		...
		for (i = 0; i < min(port_num, SMI_PORT_SV_NUM); i++) {

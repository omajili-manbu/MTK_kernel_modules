...
enum mtk_cam_seninf_rdy_set_cmd {
	...
	MTK_CAM_SENINF_RDY_SET_CQ_STATUS,
};
...
struct seninf_vc *mtk_cam_seninf_get_vc_by_pad(struct seninf_ctx *ctx, int idx)
{
	...
	format_code = to_std_fmt_code(ctx->fmt[PAD_SINK].format.code);
		...
		for (j =0; j < vc->dest_cnt; j++) {
			...
			case MTK_CAM_SENINF_RDY_SET_CQ_ENABLE:
				ret |= g_seninf_ops->_set_outmux_rdy_msk_cq_rdy_en(
					ctx, ctx->pad2cam[vc->out_pad][j], value);
				break;
			case MTK_CAM_SENINF_RDY_SET_CQ_STATUS:
				ret |= g_seninf_ops->_set_outmux_rdy_msk_cq_rdy_status(
					ctx, ctx->pad2cam[vc->out_pad][j], value);
				break;
			default:
	...
	struct v4l2_subdev *sd, u8 camtg, bool sw_rdy_status)
{
	...
	if ((sw_rdy_status == true ) && (!ctx->is_test_model) && (ctx->sensor_sd))
...
static int get_seamless_delay_frame(struct seninf_ctx *ctx, int *grp_delay)
{
	struct v4l2_subdev *sensor_sd = NULL;
	struct v4l2_ctrl *ctrl;

	if (unlikely(ctx->sensor_sd == NULL)) {
		seninf_logd(ctx, "[WRN][%s] no sensor subdev, skip\n", __func__);
		/* for test pattern flow, it is normal case */
		return 0;
	}

	if (unlikely(grp_delay == NULL)) {
		pr_info("[Error][%s] grp_delay is NULL\n", __func__);
		return -EINVAL;
	}
	...
	*grp_delay = v4l2_ctrl_g_ctrl(ctrl);

	if (*grp_delay < 0) {
		pr_info("[Error][%s] v4l2_ctrl_g_ctrl return failed\n", __func__);
		return -EINVAL;
	}
	...
	bool is_defered_by_tsrec)
{
	...
	if (get_seamless_delay_frame(ctx, &sensor_grp_delay)) {
		pr_info("[%s][Err]get_seamless_delay_frame retrun failed\n", __func__);
		return -EINVAL;
	}

	if ((sw_rdy_status == true) &&
		((is_defered_by_tsrec == true) || sensor_grp_delay)) {
...
bool mtk_cam_seninf_set_mux_cq_rdy(struct v4l2_subdev *sd, u8 camtg, bool cq_rdy_status)
{
	struct seninf_ctx *ctx = container_of(sd, struct seninf_ctx, subdev);
	...
	return mtk_cam_seninf_set_mux_rdy_msk_cfg_by_cmd(
		ctx, camtg, MTK_CAM_SENINF_RDY_SET_CQ_STATUS, cq_rdy_status);
}
		...
		__u32 fmt_code, int input_pad_id)
{
	...
	u64 fsync_ext_vsync_pad_code = 0;
	int ret = EXPOSURE_FIRST;  /* default return last exposure */
	int i = 0;
	int exposure_num = 0;
	int scenario = 0;
	int desc;

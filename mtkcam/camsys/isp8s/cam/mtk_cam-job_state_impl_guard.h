...
static inline bool cq_trigger_always_allow(struct transition_param *p)
{
	return (p->cq_trigger_thres == ULLONG_MAX);
}
...
static inline bool valid_cq_execution(struct transition_param *p)
{
	...
	spin_lock(p->info_lock);
	ret = cq_trigger_always_allow(p);
	ret = ret || (valid_first_last_sof_order(p) &&
				  (valid_cq_thres_from_first_sof(p) ||
				   valid_cq_thres_from_last_sof(p)));
...
static inline bool valid_cq_execution_xvs(struct transition_param *p)
{
	...
	ret = cq_trigger_always_allow(p);
	ret = ret || (valid_first_last_sof_order(p) && valid_cq_thres_from_xvs(p));

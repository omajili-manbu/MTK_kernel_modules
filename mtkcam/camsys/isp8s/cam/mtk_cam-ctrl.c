...
#define MAX_EXPOSURE_SWITCH_TIMEOUT_MS		30000
...
#define VALID_SWITCH_PERIOD_30FPS_FROM_VSYNC_MS		30000000
		...
		if (ctrl->r_info.extisp_enable)
			...
			for (i = 0; i < ARRAY_SIZE(ctx->hw_raw); i++) {
				if (ctx->hw_raw[i]) {
					...
					mtk_raw_push_dc_dbg_ln(raw_dev, irq_info->ts_ns);
				}
			}
		}

		/* for ois compensation use */
		if (irq_info->irq_type & BIT(CAMSYS_IRQ_FRAME_START)) {
			if (!vsync_res.is_last) {
				ctrl->r_info.sv_sof_l_ts_ns = irq_info->ts_ns;
				ctrl->r_info.sv_sof_l_ts_mono_ns = ktime_get_ns();
			} else {
				ctrl->r_info.sv_sof_l_ts_ns = ctrl->r_info.sof_l_ts_ns;
				ctrl->r_info.sv_sof_l_ts_mono_ns = ctrl->r_info.sof_l_ts_ns;
			}
		}
...
static int dynamic_raw_change_stream_on(struct mtk_cam_job *job, int unit_engs)
{
	...
	if (job->raw_change) {
		if (job->raw_change == JOB_RAW_MASTER_UNCHANGED) {
			...
			vsync_set_desired(&ctrl->vsync_col, get_master_engines(ctx->used_engine));
			call_jobop(job, stream_on, true);
		}
	}
...
static void mtk_cam_ctrl_dynamic_raws_change_flow(struct mtk_cam_job *job)
{
	...
	qof_mtcmos_voter_handle_clear(&ctx->cam->engines,
			raw_uninit, &ctx->unsupport_scen);
	...
	if (mtk_cam_ctrl_wait_event(ctrl, check_for_inner, &check_args, 10000 + MAX_EXPOSURE_SWITCH_TIMEOUT_MS)) {
...
SWITCH_FAILURE:
	...
	if (mtk_cam_seninf_dump(ctx->seninf, job->frame_seq_no, true, true)
		!= -ESTRPIPE) {
		mtk_engine_dump_debug_status(ctx->cam, job->used_engine, DD_DUMP_NONE, job);
...
static void mtk_cam_ctrl_seamless_switch_flow(struct mtk_cam_job *job)
{
...
#ifdef SEAMLESS_PROFLIE
	u64 start;
	u64 isp_setup_time, eng_uninit_time;
	u64 ftr_start, fd_to_ready_mask;
#endif
	...
	check_args.force_last_sof = true;
...
#ifdef SEAMLESS_PROFLIE
	ftr_start = ktime_get_boottime_ns();
#endif
	...
	for (i = 0; i < cam->engines.num_raw_devices && qof_enabled; i++) {
		bool is_master = false;
		struct mtk_raw_device *raw_dev;
...
#ifdef SEAMLESS_PROFLIE
	isp_setup_time = ktime_get_boottime_ns() - start;
#endif

	mtk_cam_seninf_force_disable_out_mux(ctx->seninf);  /* after prev p1 done */

	qof_mtcmos_voter_handle_clear(&ctx->cam->engines,
			engine_uninit, &ctx->unsupport_scen);
	if (mtk_cam_job_not_support_qof(job))
		qof_mtcmos_voter_handle(&ctx->cam->engines,
			raw_after_change, &ctx->unsupport_scen);
	else
...
#ifdef SEAMLESS_PROFLIE
	fd_to_ready_mask = ktime_get_boottime_ns() - ftr_start;
#endif
...
#ifdef SEAMLESS_PROFLIE
	eng_uninit_time = ktime_get_boottime_ns() - start;
#endif
	...
	if (mtk_cam_ctrl_wait_event(ctrl, check_for_inner, &check_args,
				    10000 + MAX_EXPOSURE_SWITCH_TIMEOUT_MS)) {
...
#ifdef SEAMLESS_PROFLIE
	dev_info(dev, "[%s] proflie: frame done to ready mask: %llu ns",
			 __func__, fd_to_ready_mask);
	dev_info(dev, "[%s] proflie: isp setup: %llu ns",
			 __func__, isp_setup_time);
	dev_info(dev, "[%s] proflie: engine uninit: %llu ns",
			 __func__, eng_uninit_time);
#endif
	return;
...
static void mtk_cam_ctrl_raw_switch_flow(struct mtk_cam_job *job)
{
	...
	prev_seq = prev_frame_seq(job->frame_seq_no);
	if (mtk_cam_ctrl_wait_event(ctrl, check_done, &prev_seq, 10000 + MAX_EXPOSURE_SWITCH_TIMEOUT_MS))
		dev_info(dev, "[%s] check_done timeout: prev_seq=0x%x\n",
			 __func__, prev_seq);
			    ...
			    struct mtk_cam_job *job)
{
	...
	if (cam_ctrl->initial_req) {
		...
		vsync_set_desired(&cam_ctrl->vsync_col,
						  get_master_engines(job->src_ctx->used_engine));
...
void mtk_cam_ctrl_stop(struct mtk_cam_ctrl *cam_ctrl)
{
	...
	list_for_each_entry(job_s, &cam_ctrl->camsys_state_list, list) {
		...
		/* mark the buffer done which is in the running list */
		call_jobop(job, cancel);
	}
	read_unlock(&cam_ctrl->list_lock);

	/* mark the buffer done which is in the pending list */
	mtk_cam_ctx_cleanup_pending(ctx);
...
static inline u64 mtk_cam_ctrl_latest_sof(struct mtk_cam_ctrl *ctrl)
{
	...
	spin_lock(&ctrl->info_lock);
	/* NOTE: xvs could be first exp */
	ts_ns = max(ctrl->r_info.sof_l_ts_ns,
				ctrl->r_info.xvs_ts_ns);
	spin_unlock(&ctrl->info_lock);
...
static void mtk_dump_debug_for_no_vsync(struct mtk_cam_ctx *ctx)
{
	...
	job = mtk_cam_ctrl_get_job(ctrl, cond_first_job, 0);
	if (job) {
		mtk_engine_dump_debug_status(cam, job->used_engine, DD_DUMP_NONE, job);
		mtk_cam_job_put(job);
	} else {
		mtk_engine_dump_debug_status(cam, ctx->used_engine, DD_DUMP_NONE, NULL);
	}
}
...
static int launch_monitor_work(struct mtk_cam_watchdog *wd)
{
	...
	queue_work(system_unbound_wq, &monitor->work);
	return 0;
}
				...
				int inner_cookie, int count_down_dis)
{
	...
	if (!current_job || current_job->raw_change)
		return 0;

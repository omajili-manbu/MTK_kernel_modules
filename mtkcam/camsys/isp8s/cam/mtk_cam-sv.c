	...
	struct mtk_camsv_device *slave_sv_dev)
{
	...
	CAMSV_WRITE_REG(slave_sv_dev->base + REG_CAMSVCENTRAL_ERR_STATUS_EN, 0x701);
	...
	void *vaddr, struct mtk_mraw_pipeline *mraw_pipe)
{
	...
	CALL_PLAT_V4L2(is_sv_pdp_supported, sv_dev->id, tag_idx, &is_supported);
...
int mtk_cam_sv_debug_dump(struct mtk_camsv_device *sv_dev, unsigned int dump_tags)
{
	...
	dev_info(sv_dev->dev,
		"tg_sen_mode:0x%x tg_vf_con:0x%x tg_path_cfg:0x%x sof_delay_en:0x%x sv_dev first tag:0x%x, sv_dev first tag:0x%x\n",
		tg_sen_mode, tg_vf_con, tg_path_cfg, sof_delay_en, sv_dev->first_tag, sv_dev->last_tag);
	...
	struct mtk_camsys_irq_info *data)
{
	...
	if (atomic_read(&sv_dev->is_fifo_full) == 0 && sv_dev->camsv_error_count == 1) {
		...
		if (!mtk_cam_sv_is_zero_fbc_cnt(sv_dev, enable_tags)) {
			mtk_cam_ctrl_notify_hw_hang(sv_dev->cam,
...
static irqreturn_t mtk_irq_camsv_sof(int irq, void *data)
{
	...
	sv_dev->first_tag =
		readl_relaxed(sv_dev->base_inner + REG_CAMSVCENTRAL_FIRST_TAG);
	sv_dev->last_tag =
		readl_relaxed(sv_dev->base_inner + REG_CAMSVCENTRAL_FRM_SPARE1);
	for (i = 0; i < CAMSV_MAX_TAGS; i++) {
	...
	if (CAM_DEBUG_ENABLED(RAW_INT))
		dev_info(sv_dev->dev, "camsv-%d: sof status:0x%x channel status:0x%x seq_no:0x%x_0x%x group_tags:0x%x_%x_%x_%x (hw)first_tag:0x%x (hw)last_tag:0x%x (sw)first_tag:0x%x (sw)last_tag:0x%x tg_cnt:%d/%lld dcif sel/set:0x%x_%x pda_dcif0x%x",
		...
		readl_relaxed(sv_dev->base_inner + REG_CAMSVCENTRAL_PDA_DCIF_SET));
			    ...
			    struct mtk_camsv_device *sv_dev)
{
	...
	struct resource *res;
	unsigned int i, j;
	int ret, larbs, num_clks, num_iommus, num_ports, smmus, cam_mraw_base = 0;
	...
	CALL_PLAT_HW(query_module_base, CAM_MRAW, &cam_mraw_base);
	sv_dev->top = ioremap(cam_mraw_base, 0x1000);
	...
	sv_dev->num_larbs = (larbs <= 0) ? 0 : larbs;
...
bool mtk_camsv_check_stress_mode_on(struct mtk_camsv_device *sv_dev)
{
	...
	CALL_PLAT_V4L2(get_slave_sv_idx, &slave_sv_idx);

	if (slave_sv_idx == -1)
		return 0;

	struct device *slave_dev = sv_dev->cam->engines.sv_devs[slave_sv_idx];

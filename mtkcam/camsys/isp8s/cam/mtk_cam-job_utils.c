...
bool check_first_dcif_cq_start(struct mtk_cam_job *job)
{
	return (is_dcg_with_vs(job) &&
			job_sensor_exp_num(job) != job_exp_num(job));
}
...
int _add_entry_to_ufbc_header(struct mtk_cam_ufbc_header *ufbc_header,
		...
		struct mtkcam_ipi_img_ufo_param *param, unsigned int stride, unsigned int height)
{
...
int add_ufbc_header_entry(struct req_buffer_helper *helper,
		...
		int subsample, unsigned int size, unsigned int stride, unsigned int height)
{
	struct mtkcam_ipi_frame_param *fp = helper->fp;
	void *vaddr;
	int ret = 0;
	...
	case MTKCAM_IPI_RAW_IMGO_W:
		ret = _add_entry_to_ufbc_header(helper->ufbc_header, ipi_video_id,
				...
				&fp->img_ufdo_params.imgo, stride, height);
		break;
	case MTKCAM_IPI_CAMSV_MAIN_OUT:
		ret = _add_entry_to_ufbc_header(helper->ufbc_header, ipi_video_id,
				vaddr + offset,  subsample, size, NULL, stride, height);
		break;
	case MTKCAM_IPI_RAW_YUVO_1:
		ret = _add_entry_to_ufbc_header(helper->ufbc_header, ipi_video_id,
				vaddr + offset, subsample, size,
				&fp->img_ufdo_params.yuvo1, stride, height);
		break;
	case MTKCAM_IPI_RAW_YUVO_3:
		ret = _add_entry_to_ufbc_header(helper->ufbc_header, ipi_video_id,
				vaddr + offset, subsample, size,
				&fp->img_ufdo_params.yuvo3, stride, height);
		break;
	default:
		break;
	}
			...
			int subsample_ratio)
{
	...
	add_ufbc_header_entry(helper, buf->image_info.v4l2_pixelformat,
					  ...
					  buf->image_info.bytesperline[0],
					  buf->image_info.height);
...
{
	unsigned int size = buf->image_info.size[0];
	unsigned int stride = buf->image_info.bytesperline[0];
	unsigned int height = buf->image_info.height;
	unsigned int valid_plane = 0;
	dma_addr_t daddr = 0, buf_offset = 0;
	bool is_valid_mp_buf;
	...
	add_ufbc_header_entry(helper, buf->image_info.v4l2_pixelformat,
						  id, buf, valid_plane, buf_offset, 1, size, stride, height);
			...
			int sub_ratio)
{
	...
	/* revert yuv header to as-is, fill only first buf */
	add_ufbc_header_entry(helper, buf->image_info.v4l2_pixelformat,
						  io->uid.id, buf, 0, 0, 1,
						  buf->image_info.size[0], buf->image_info.bytesperline[0],
						  buf->image_info.height);
				   ...
				   struct mtk_cam_ctrl_runtime_info *info)
{
	...
	if (info->sof_l_ts_ns >= info->sof_ts_ns) {
		job->hdr_ts_cache.se = is_ois_comp ?
			info->sv_sof_l_ts_ns : info->sof_l_ts_ns;
		job->hdr_ts_cache.se_mono = is_ois_comp ?
			info->sv_sof_l_ts_mono_ns : info->sof_l_ts_mono_ns;
	} else {
				   ...
				   struct mtk_cam_ctrl_runtime_info *info)
{
	...
	case JOB_TYPE_STAGGER:
		if (exp_order == MTKCAM_IPI_ORDER_SE_NE) {
			job->hdr_ts_cache.le = is_ois_comp ?
				info->sv_sof_l_ts_ns : info->sof_l_ts_ns;
			job->hdr_ts_cache.le_mono = is_ois_comp ?
				info->sv_sof_l_ts_mono_ns : info->sof_l_ts_mono_ns;

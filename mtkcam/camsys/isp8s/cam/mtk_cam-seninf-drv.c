...
static int set_ctle_param(struct seninf_ctx *ctx)
{
	...
	if (ret == -ENODATA) {
		seninf_logd(ctx, "[%s] sensor may not porting CTLE (optional porting param)\n", __func__);
		return 0;

	} else if (ret != 0) {
		seninf_logi(ctx, "[%s][ERR] adaptor cmd return CTLT param error red %d\n", __func__, ret );
		return -EFAULT;
	}
...
static int set_cust_ctle_config(struct seninf_ctx *ctx)
{
	...
	if (ret) {
		seninf_logd(ctx, "[%s] get cust ctle config failed, skip\n", __func__);
		return 0;
	}
	...
	if (core->refcnt >= 0) {
		...
		if (core->refcnt == 0) {
			...
			if (core->clk[CLK_TOP_CAMTM]) {
				...
				seninf_logd(ctx, "clk_disable_unprepare CLK_TOP_CAMTM\n");
			}

			       ...
			       unsigned int dequeued_frame_seq_no)
{
	...
	if (!req->ctx_link_update) {
		dev_dbg(raw_dev->dev, "%s next req (%d) no link setup\n", __func__, frame_seq);
		return;
	}
	...
	for (i = 0; i < cam->max_stream_num; i++) {
		if ((req->ctx_used & 1 << i) && (req->ctx_link_update & (1 << i))) {
			...
			if (req_stream_data->state.estate == E_STATE_CAMMUX_OUTER_CFG_DELAY) {
				 ...
				 * queue_work(cam->link_change_wq, &req->link_work);
				 */
				dev_info(raw_dev->dev, "%s:req(%d) Exchange streams at req(%d), update link ctx (0x%x)\n",
					__func__, frame_seq, ctx->stream_id, req->ctx_link_update);
				mtk_cam_req_seninf_change(req);
				return;
			}
		}
				       ...
				       struct mtk_camsys_irq_info *irq_info)
{
	...
	if (mtk_cam_is_ext_isp(ctx)) {
		dev_info(raw_dev->dev, "ts=%llu irq_type %d, req:%d/%d\n",
		irq_info->ts_ns / 1000,
		irq_info->irq_type,
		irq_info->frame_idx_inner,
		irq_info->frame_idx);
					 ...
					 struct mtk_camsys_irq_info *irq_info)
{
	...
	if (camsv_dev->pipeline->hw_scen &
		...
		if (mtk_cam_is_ext_isp(ctx)) {
			dev_info(camsv_dev->dev, "ts=%llu irq_type %d, req:%d/%d\n",
			irq_info->ts_ns / 1000,
			irq_info->irq_type,
			irq_info->frame_idx_inner,
			irq_info->frame_idx);
				       ...
				       struct mtk_camsys_irq_info *irq_info)
{
	...
	if (list_empty(&ctx->composed_buffer_list.list)) {
		...
		req_stream_data = mtk_cam_ctrl_state_to_req_s_data(current_state);
		dev_info(raw_dev->dev,
		"SOF[ctx:%d-#%d], CQ-%d is update, composed:%d, cq_addr:0x%llx\n",
		ctx->stream_id, dequeued_frame_seq_no, req_stream_data->frame_seq_no,
		ctx->composed_frame_seq_no, base_addr);
			...
			struct mtk_camsys_irq_info *irq_info)
{
	...
	if (stream_data->timestamp == 0 &&
		...
		dev_info(ctx->cam->dev, "[%s] fix ts:ctx:%d req:%lld(ns) s_data:%lld \n",

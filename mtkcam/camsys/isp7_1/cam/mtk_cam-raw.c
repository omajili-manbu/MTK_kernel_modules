			  ...
			  struct mtk_cam_resource *res_user, char *dbg_str, bool log)
{
	...
	if (log)
		dev_info(dev,
			 "%s:%s:pipe(%d): from user: sensor: %d/%d/0x%x/%d/%d/%lld/%lld/%d/%d, raw:%lld/%d/%d/%d/%d/%d/%d/%d/%lld/%lld\n",
			 __func__, dbg_str, pipeline->id,
			 res_user->sensor_res.width, res_user->sensor_res.height,
			 ...
			 char *dbg_str, bool log)
{
	...
	if (sink_fmt->width <= 0 || sink_fmt->height <= 0) {
		dev_info(dev, "%s:invalid sink_fmt w/h: %d/%d",
			__func__, sink_fmt->width, sink_fmt->height);
		return -EINVAL;
	}
...
static int mtk_cam_raw_set_res_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	sink_fmt.width = res_user->sensor_res.width;
	sink_fmt.height = res_user->sensor_res.height;
	sink_fmt.code = res_user->sensor_res.code;

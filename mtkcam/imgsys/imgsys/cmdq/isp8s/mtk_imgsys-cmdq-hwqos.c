...
static int g_hwqos_active_multiply = 1;
...
static void imgsys_qos_set_floor_bw(struct cmdq_pkt *pkt,
	const uint16_t cpr_rw,
	const uint16_t map_idx,
	const uint32_t floor_bw)
{
	uint16_t cpr_idx;
	uint32_t floor_bwr_bw;

	GCE_OP_DECLARE;
	GCE_COND_DECLARE;
	GCE_COND_ASSIGN(pkt, SPR1);

	cpr_idx = cpr_rw + map_idx * QOS_CPR_STEP;
	floor_bwr_bw = floor_bw << BWR_MB_RIGHT_SHIFT;

	GCE_IF(IDX, cpr_idx, R_CMDQ_LESS,
		VAL, floor_bwr_bw);
	{
		cmdq_pkt_assign_command(pkt, cpr_idx, floor_bwr_bw);
	}
	GCE_FI;
}
...
static void imgsys_qos_set_bw(struct cmdq_pkt *pkt)
{
		...
		VAL, (1 << QOS_AVERAGE_SHIFT));
	{
		...
		imgsys_qos_set_floor_bw(pkt, CPR_AVG_R, 0, 69);
		imgsys_qos_set_floor_bw(pkt, CPR_AVG_W, 0, 56);

		imgsys_qos_set_floor_bw(pkt, CPR_AVG_R, 5, 143);
		imgsys_qos_set_floor_bw(pkt, CPR_AVG_W, 5, 59);

		imgsys_qos_set_floor_bw(pkt, CPR_AVG_R, 4, 179);
		imgsys_qos_set_floor_bw(pkt, CPR_AVG_W, 4, 34);

		imgsys_qos_set_floor_bw(pkt, CPR_AVG_R, 1, 394);
		imgsys_qos_set_floor_bw(pkt, CPR_AVG_W, 1, 215);

		imgsys_qos_set_floor_bw(pkt, CPR_AVG_R, 3, 231);
		imgsys_qos_set_floor_bw(pkt, CPR_AVG_W, 3, 179);

		imgsys_qos_set_floor_bw(pkt, CPR_AVG_R, 2, 292);
		imgsys_qos_set_floor_bw(pkt, CPR_AVG_W, 2, 66);

		imgsys_qos_set_floor_bw(pkt, CPR_AVG_R, 7, 107);
		imgsys_qos_set_floor_bw(pkt, CPR_AVG_W, 7, 100);

		imgsys_qos_set_floor_bw(pkt, CPR_AVG_R, 6, 309);
		imgsys_qos_set_floor_bw(pkt, CPR_AVG_W, 6, 228);

...
#if DVFS_QOS_READY
void mtk_imgsys_mmdvfs_init_plat8s(struct mtk_imgsys_dev *imgsys_dev)
{
	...
	if (!mmdvfs_get_version()) {
		dvfs_info->mmdvfs_user = VCP_PWR_USR_IMG;
		return;
	}
...
void mtk_imgsys_mmdvfs_uninit_plat8s(struct mtk_imgsys_dev *imgsys_dev)
{
	struct mtk_imgsys_dvfs *dvfs_info = &imgsys_dev->dvfs_info;
	int volt = 0, ret = 0;
...
void mtk_imgsys_mmdvfs_set_plat8s(struct mtk_imgsys_dev *imgsys_dev,
				...
				bool isSet)
{
	...
	if (IS_ERR_OR_NULL(dvfs_info->reg) && IS_ERR_OR_NULL(dvfs_info->mmdvfs_clk)) {
		...
		if (dvfs_info->cur_volt != volt) {
			if (imgsys_dvfs_dbg_enable_plat8s())
...
void mtk_imgsys_mmdvfs_reset_plat8s(struct mtk_imgsys_dev *imgsys_dev)
{
	struct mtk_imgsys_dvfs *dvfs_info = &imgsys_dev->dvfs_info;
	...
	for (opp_idx = 0; opp_idx < dvfs_info->opp_num; opp_idx++) {
		if (imgsys_fine_grain_dvfs_enable_plat8s()) {
		...
		dev_info(dvfs_info->dev, "[%s] fine grain(%d), opp=%d, clk_num=%d\n",
			__func__, imgsys_fine_grain_dvfs_enable_plat8s(),
...
void mtk_imgsys_mmdvfs_mmqos_cal_plat8s(struct mtk_imgsys_dev *imgsys_dev,
				struct swfrm_info_t *frm_info,
				bool isSet)
{

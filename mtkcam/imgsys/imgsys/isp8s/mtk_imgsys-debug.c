...
void __iomem *imgsysEfuseRxDoneRegBA;
int imgsys_ddr_en;
...
void imgsys_main_init(struct mtk_imgsys_dev *imgsys_dev)
{
	...
	imgsysEfuseRxDoneRegBA = of_iomap(imgsys_dev->dev->of_node, REG_MAP_E_MAIN_EFUSE_RX_DONE);
	if(!imgsysEfuseRxDoneRegBA) {
		dev_info(imgsys_dev->dev, "%s Unable to ioremap imgsys_EfuseRxDone registers\n",
								__func__);
		dev_info(imgsys_dev->dev, "%s of_iomap fail, devnode(%s).\n",
				__func__, imgsys_dev->dev->of_node->name);
		return;
	...
	if (imgsys_dev->dev_ver == 0) {
		...
		while ((value & 0x1fc) != 0x1fc) {
			...
			if (count > DDREN_ACK_TIMEOUT_CNT) {
				pr_err("[%s][%d] wait platorm resources done timeout 0x%08x",
...
void imgsys_main_uninit(struct mtk_imgsys_dev *imgsys_dev)
{
	...
	if(imgsysEfuseRxDoneRegBA) {
		iounmap(imgsysEfuseRxDoneRegBA);
		imgsysEfuseRxDoneRegBA = 0L;
	}

...
const struct mtk_imgsys_init_array mtk_imgsys_me_init_ary_mt6881[] = {
	{0x198, 0x00000001}, /* ME_DDREN */
};
...
const struct mtk_imgsys_init_array mtk_imgsys_mmg_init_ary_mt6881[] = {
	{0x70, 0x00000001}, /* MMG_DDREN */
};
...
int imgsys_me_tfault_callback_mt6881(int port, dma_addr_t mva, void *data)
{
	...
	for (i = MMG_CTL_OFFSET; i <= MMG_CTL_OFFSET + MMG_CTL_RANGE_TF; i += 0x10) {
		pr_info("%s: 0x%08X %08X, %08X, %08X, %08X", __func__,
		(unsigned int)(MMG_BASE_mt6881 + i),
		(unsigned int)ioread32((void *)(mmgRegBA + i)),
		(unsigned int)ioread32((void *)(mmgRegBA + (i+0x4))),
		(unsigned int)ioread32((void *)(mmgRegBA + (i+0x8))),
		(unsigned int)ioread32((void *)(mmgRegBA + (i+0xC))));
...
void imgsys_me_set_hw_initial_value_mt6881(struct mtk_imgsys_dev *imgsys_dev)
{
	...
	if (!mmgRegBA) {
		...
		/* MMG HW mode ddren */
		iowrite32(0x00000001, (void *)(mmgRegBA + 0x00000070));
...
void imgsys_me_cmdq_set_hw_initial_value_mt6881(struct mtk_imgsys_dev *imgsys_dev, void *pkt, int hw_idx)
{
	...
	/* init registers */
	for (i = 0; i < ARRAY_SIZE(mtk_imgsys_me_init_ary_mt6881); i++) {
		ofset = me_base + mtk_imgsys_me_init_ary_mt6881[i].ofset;
		cmdq_pkt_write(package, NULL, ofset /*address*/,
				mtk_imgsys_me_init_ary_mt6881[i].val, 0xffffffff);
	}

	/* init registers */
	for (i = 0; i < ARRAY_SIZE(mtk_imgsys_mmg_init_ary_mt6881); i++) {
		ofset = mmg_base + mtk_imgsys_mmg_init_ary_mt6881[i].ofset;
		cmdq_pkt_write(package, NULL, ofset /*address*/,
				mtk_imgsys_mmg_init_ary_mt6881[i].val, 0xffffffff);
	}
}

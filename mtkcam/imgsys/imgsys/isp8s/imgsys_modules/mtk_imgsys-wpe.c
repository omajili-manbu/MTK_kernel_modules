// SPDX-License-Identifier: GPL-2.0
/*
 * Copyright (c) 2020 MediaTek Inc.
 *
 * Author: Floria Huang <floria.huang@mediatek.com>
 *
 */

#include <linux/device.h>
#include <linux/of_address.h>
#include <linux/pm_runtime.h>
#include <linux/remoteproc.h>
#include <linux/version.h>

// GCE header
#include <linux/soc/mediatek/mtk-cmdq-ext.h>

#include "iommu_debug.h"

#include "mtk_imgsys-wpe.h"
#include "mtk-hcp.h"
#include "mtk_imgsys-v4l2-debug.h"
#include "../../cmdq/isp8s/mtk_imgsys-cmdq-qof.h"

/* The number of all WPE type (WPE_EIS, WPE_TNR, WPE_LITE) */
#define WPE_HW_NUM        3

/* wpe base address in different platform */
unsigned int g_wpe_base;
#define WPE_A_BASE        (0x34240000)  /* imgsys_dev->dev_ver is 0 */
#define WPE_A_BASE_P      (0x15240000)  /* imgsys_dev->dev_ver is 1 */

/* wpe base address offset in different WPE type {WPE_EIS, WPE_TNR, WPE_LITE} */
const unsigned int *g_mtk_imgsys_wpe_base_ofst;
const unsigned int mtk_imgsys_wpe_base_ofst[WPE_HW_NUM] = {0x0, 0x300000, 0x400000};
const unsigned int mtk_imgsys_wpe_base_ofst_1[WPE_HW_NUM] = {0x0, 0x2F0000, 0x0};

/* wpe type supported in different platform */
const bool *g_wpe_hw_support;
const bool wpe_hw_support[WPE_HW_NUM] = {true, false, true};  /* imgsys_dev->dev_ver is 0 */
const bool wpe_hw_support_1[WPE_HW_NUM] = {true, true, false};  /* imgsys_dev->dev_ver is 1 */

const unsigned int mtk_imgsys_wpe_reg_size[WPE_HW_NUM] = {0x1200, 0x1200, 0x1200};
unsigned int g_wpe_ufod_p2_desc_ofst;

//CTL_MOD_EN
#define PQDIP_DL  0x40    // WPE_TOP_MDPCROP_EN (6,6)
#define DIP_DL    0x80    // WPE_TOP_ISPCROP_EN (7,7)
#define TRAW_DL   0x1000  // WPE_TOP_TILERAW_CROP_EN: (12,12)
#define DECOMP_EN 0x400   // WPE_TOP_DECOMP_EN (10,10)

// register ofst
#define WPE_REG_DBG_SET     (0x3C)  // WPE_E1A_WPE_TOP_CTL_DBG_SET
#define WPE_REG_DBG_PORT    (0x40)  // WPE_E1A_WPE_TOP_CTL_DBG_PORT
#define WPE_REG_DMA_DBG_SET     (0xEA8) // WPE_DMATOP_E1A_WPE_DMATOP_DMA_DBG_SEL
#define WPE_REG_DMA_DBG_PORT    (0xEB0)  // WPE_DMATOP_E1A_WPE_DMATOP_DMA_DBG_PORT
#define WPE_REG_CQ_THR0_CTL (0xC48)  // DIPCQ_E1A_DIPCQ_CQ_THR0_CTL
#define WPE_REG_CQ_THR1_CTL (0xC58)  // DIPCQ_E1A_DIPCQ_CQ_THR1_CTL
#define WPE_REG_DEC_CTL1    (0xFC0)  // WPE_DEC_E1A_WPE_DEC_DEC_CTL1
#define WPE_REG_PSP_MERGE_CTL2    (0x5A4)  /* WPE_PSP_E1A_WPE_PSP_MERGE_CTL2 */
#define WPE_REG_PSP_MERGE_CTL3    (0x5A8)  /* WPE_PSP_E1A_WPE_PSP_MERGE_CTL3 */
// #define SW_RST   (0x000C)

const struct mtk_imgsys_init_array
			mtk_imgsys_wpe_init_ary[] = {
	{0x0014, 0x80000000}, /* WPE_TOP_CTL_INT_EN, en w-clr */
	{0x001C, 0xFFFFFFFF}, /* WPE_TOP_CTL_INT_STATUSX, w-clr */
	{0x0028, 0x00000001}, /* WPE_TOP_CTL_DDREN_SW_CTL */
	{0x00C8, 0x80000000}, /* WPE_TOP_CQ_IRQ_EN, en w-clr */
	{0x00D8, 0xFFFFFFFF}, /* WPE_TOP_CQ_IRQ_STX, w-clr */
	{0x00E0, 0xFFFFFFFF}, /* WPE_TOP_CQ_IRQ_STX2, w-clr */
	{0x00E8, 0xFFFFFFFF}, /* WPE_TOP_CQ_IRQ_STX3, w-clr */
	{0x0118, 0x080A0A82}, /* WPE_TOP_STG_CTL */
	{0x011C, 0x00002001}, /* WPE_TOP_STG_CTL_RANGE_UD */
	{0x0120, 0x00010010}, /* WPE_TOP_STG_CTL_RANGE_LR jayer*/
	// {0x0124, 0x00010010}, /* WPE_TOP_STG_CTL_RANGE_LR  liber*/

	// Disable WPE DCE
	{0x004C, 0xFFFFFFFF}, /* WPE_TOP_CTL_WPE_DCM_DIS */
	{0x0050, 0xFFFFFFFF}, /* WPE_TOP_CTL_DMA_DCM_DIS */
	{0x00B8, 0xFFFFFFFF}, /* WPE_TOP_CTL_WPE_DCM_DIS2 */
	{0x00BC, 0xFFFFFFFF}, /* WPE_TOP_CTL_DMA_DCM_DIS2 */
	{0x0FC4, 0x0000003F}, /* WPE_DEC_DEC_CTL2 */

	// {0x04A0, 0x10000040}, /* SVECI_CON, fifo size 0x40 */
	// {0x04A4, 0x10400040}, /* SVECI_CON2, set pre-ultra */
	// {0x04A8, 0x00400040}, /* SVECI_CON3, disable ultra */
	// {0x0660, 0x10000040}, /* VECI_CON, fifo size 0x40 */
	// {0x0664, 0x10400040}, /* VECI_CON2, set pre-ultra */
	// {0x0668, 0x00400040}, /* VECI_CON3, disable ultra */
	// {0x06E0, 0x10000040}, /* VEC2I_CON, fifo size 0x40 */
	// {0x06E4, 0x10400040}, /* VEC2I_CON2, set pre-ultra */
	// {0x06E8, 0x00400040}, /* VEC2I_CON3, disable ultra */
	// {0x0760, 0x10000040}, /* SVEC2I_CON, fifo size 0x40 */
	// {0x0764, 0x10400040}, /* SVEC2I_CON2, set pre-ultra */
	// {0x0768, 0x00400040}, /* SVEC2I_CON3, disable ultra */
	// {0x0B44, 0x00000002}, /* WPE_CACHE_RWCTL_CTL */
	// {0x0B88, 0x00400040}, /* WPE_CACHE_CACHI_CON2_0 */
	// {0x0B8C, 0x00400040}, /* WPE_CACHE_CACHI_CON3_0 */
	// {0x07E0, 0x10000040}, /* WPEO_CON, fifo size 0x40 */
	// {0x07E4, 0x10400040}, /* WPEO_CON2, set pre-ultra */
	// {0x07E8, 0x00400040}, /* WPEO_CON3, disable ultra */
	// {0x08A0, 0x10000040}, /* WPEO2_CON, fifo size 0x40 */
	// {0x08A4, 0x10400040}, /* WPEO2_CON2, set pre-ultra */
	// {0x08A8, 0x00400040}, /* WPEO2_CON3, disable ultra */
	// {0x0960, 0x10000040}, /* MSKO_CON, fifo size 0x40 */
	// {0x0964, 0x10400040}, /* MSKO_CON2, set pre-ultra */
	// {0x0968, 0x00400040}, /* MSKO_CON3, disable ultra */
	// {0x0A80, 0x00000000}, /* WPE_STG_EN_CTRL */
	// {0x0EA0, 0x80000000}, /* WPE_DMA_DMA_ERR_CTRL */
};
#define WPE_INIT_ARRAY_COUNT  ARRAY_SIZE(mtk_imgsys_wpe_init_ary)

struct imgsys_reg_range {
	uint32_t str;
	uint32_t end;
};

// Need to align with userspace's wpe_hw.h
union wpe_cq_cmd_desc_t {
	struct {
		// 1st word
		uint32_t reg_offset_address : 24;  // apb initial address
		uint32_t dummy_0 : 6;  // all zeros
		uint32_t code : 2;  // descriptor code, 0x0: pab, 0x1: null, 0x3: end_marker

		// 2nd word
		uint32_t dram_addr_lsb : 32;  // dram lsb address

		// 3rd word
		uint32_t dram_addr_msb : 4;  // dram msb address
		uint32_t dummy_1 : 12;  // all zeros
		uint32_t reg_count : 14;  // register count
		uint32_t dummy_2 : 2;  // all zeros
	};
	uint32_t raw[3];
};

void __iomem *gWpeRegBA[WPE_HW_NUM] = {0L};
static void __iomem *gVcoreRegBA;

int imgsys_wpe_tfault_callback(int port,
	dma_addr_t mva, void *data)
{
	void __iomem *wpeRegBA = 0L;
	unsigned int larb = 0;
	unsigned int engine = 0;
	int ret = 0;
	bool is_qof_wpe1_eis = false;
	bool is_qof_wpe3_lite = false;
	unsigned int ofst_idx;
	int i;

	pr_debug("%s: +\n", __func__);
	/* port: [10:5] larb / larb11: wpe_eis; larb22: wpe_tnr; larb23: wpe_lite */
	larb = ((port>>5) & 0x3F);

	pr_info("%s: iommu port:0x%x, larb:%d, idx:%d, addr:0x%08lx, +\n", __func__,
		port, larb, (port & 0x1F), (unsigned long)mva);

	/* iomap registers */
	engine = (larb == 11) ? REG_MAP_E_WPE_EIS : ((larb == 22) ? REG_MAP_E_WPE_TNR : REG_MAP_E_WPE_LITE);
	ofst_idx = engine - REG_MAP_E_WPE_EIS;
	wpeRegBA = gWpeRegBA[ofst_idx];
	if (!wpeRegBA) {
		pr_info("%s: WPE_%d, RegBA=0", __func__, port);
		return 1;
	}

	ret = smi_isp_wpe1_eis_get_if_in_use((void *)&is_qof_wpe1_eis);
	if (ret == -1) {
		pr_info("smi_isp_wpe1_eis_get_if_in_use = -1. return. stop dump\n");
		return 1;
	}

	ret = smi_isp_wpe3_lite_get_if_in_use((void *)&is_qof_wpe3_lite);
	if (ret == -1) {
		smi_isp_wpe1_eis_put((void *)&is_qof_wpe1_eis);
		pr_info("smi_isp_wpe3_lite_get_if_in_use = -1. return. stop dump\n");
		return 1;
	}
	pr_info("%s: ==== Dump WPE_%d, TF port: 0x%x =====",
		__func__, ofst_idx, port);

	/* Dump all wpe register */
	for (i = 0; i < mtk_imgsys_wpe_reg_size[ofst_idx]; i += 0x10) {
		pr_info("%s: [0x%08X] 0x%08X 0x%08X 0x%08X 0x%08X", __func__,
			(unsigned int)(g_wpe_base + g_mtk_imgsys_wpe_base_ofst[ofst_idx] + i),
			(unsigned int)ioread32((void *)(wpeRegBA + i)),
			(unsigned int)ioread32((void *)(wpeRegBA + i + 0x4)),
			(unsigned int)ioread32((void *)(wpeRegBA + i + 0x8)),
			(unsigned int)ioread32((void *)(wpeRegBA + i + 0xC)));
	}

	pr_info("[gals_tx dgb addr] 0x34780048 = 0x%08X",
		(unsigned int)ioread32((void *)(gVcoreRegBA + 0x48)));
	pr_info("[gals_tx dgb addr] 0x3478004C = 0x%08X",
		(unsigned int)ioread32((void *)(gVcoreRegBA + 0x4C)));
	pr_info("[gals_tx dgb addr] 0x34780050 = 0x%08X",
		(unsigned int)ioread32((void *)(gVcoreRegBA + 0x50)));
	pr_info("[gals_tx dgb addr] 0x34780054 = 0x%08X",
		(unsigned int)ioread32((void *)(gVcoreRegBA + 0x54)));
	pr_info("[ACK dgb addr] 0x347800C4 = 0x%08X",
		(unsigned int)ioread32((void *)(gVcoreRegBA + 0xC4)));
	pr_info("[ACK dgb addr] 0x347800C8 = 0x%08X",
		(unsigned int)ioread32((void *)(gVcoreRegBA + 0xC8)));
	pr_info("[ACK dgb addr] 0x347800CC = 0x%08X",
		(unsigned int)ioread32((void *)(gVcoreRegBA + 0xCC)));
	smi_isp_wpe1_eis_put((void *)&is_qof_wpe1_eis);
	smi_isp_wpe3_lite_put((void *)&is_qof_wpe3_lite);

	return 1;
}

int imgsys_wpe_tfault_callback_1(int port,
	dma_addr_t mva, void *data)
{
	void __iomem *wpeRegBA = 0L;
	unsigned int larb = 0;
	unsigned int engine = 0;
#if 0
	int ret = 0;
	bool is_qof_wpe1_eis = false;
	bool is_qof_wpe3_lite = false;
#endif
	unsigned int ofst_idx;
	int i;

	pr_debug("%s: +\n", __func__);
	/* port: [10:5] larb / larb11: wpe_eis; larb48: wpe_tnr; */
	larb = ((port>>5) & 0x3F);

	pr_info("%s: iommu port:0x%x, larb:%d, idx:%d, addr:0x%08lx, +\n", __func__,
		port, larb, (port & 0x1F), (unsigned long)mva);

	/* iomap registers */
	engine = (larb == 48) ? REG_MAP_E_WPE_TNR : REG_MAP_E_WPE_EIS;
	ofst_idx = engine - REG_MAP_E_WPE_EIS;
	wpeRegBA = gWpeRegBA[ofst_idx];
	if (!wpeRegBA) {
		pr_info("%s: WPE_%d, RegBA=0", __func__, port);
		return 1;
	}

#if 0
	ret = smi_isp_wpe1_eis_get_if_in_use((void *)&is_qof_wpe1_eis);
	if (ret == -1) {
		pr_info("smi_isp_wpe1_eis_get_if_in_use = -1. return. stop dump\n");
		return 1;
	}

	ret = smi_isp_wpe3_lite_get_if_in_use((void *)&is_qof_wpe3_lite);
	if (ret == -1) {
		smi_isp_wpe1_eis_put((void *)&is_qof_wpe1_eis);
		pr_info("smi_isp_wpe3_lite_get_if_in_use = -1. return. stop dump\n");
		return 1;
	}
#endif

	pr_info("%s: ==== Dump WPE_%d, TF port: 0x%x =====",
		__func__, ofst_idx, port);

	/* Dump all wpe register */
	for (i = 0; i < mtk_imgsys_wpe_reg_size[ofst_idx]; i += 0x10) {
		pr_info("%s: [0x%08X] 0x%08X 0x%08X 0x%08X 0x%08X", __func__,
			(unsigned int)(g_wpe_base + g_mtk_imgsys_wpe_base_ofst[ofst_idx] + i),
			(unsigned int)ioread32((void *)(wpeRegBA + i)),
			(unsigned int)ioread32((void *)(wpeRegBA + i + 0x4)),
			(unsigned int)ioread32((void *)(wpeRegBA + i + 0x8)),
			(unsigned int)ioread32((void *)(wpeRegBA + i + 0xC)));
	}

#if 0
	pr_info("[gals_tx dgb addr] 0x34780048 = 0x%08X",
		(unsigned int)ioread32((void *)(gVcoreRegBA + 0x48)));
	pr_info("[gals_tx dgb addr] 0x3478004C = 0x%08X",
		(unsigned int)ioread32((void *)(gVcoreRegBA + 0x4C)));
	pr_info("[gals_tx dgb addr] 0x34780050 = 0x%08X",
		(unsigned int)ioread32((void *)(gVcoreRegBA + 0x50)));
	pr_info("[gals_tx dgb addr] 0x34780054 = 0x%08X",
		(unsigned int)ioread32((void *)(gVcoreRegBA + 0x54)));
	pr_info("[ACK dgb addr] 0x347800C4 = 0x%08X",
		(unsigned int)ioread32((void *)(gVcoreRegBA + 0xC4)));
	pr_info("[ACK dgb addr] 0x347800C8 = 0x%08X",
		(unsigned int)ioread32((void *)(gVcoreRegBA + 0xC8)));
	pr_info("[ACK dgb addr] 0x347800CC = 0x%08X",
		(unsigned int)ioread32((void *)(gVcoreRegBA + 0xCC)));
	smi_isp_wpe1_eis_put((void *)&is_qof_wpe1_eis);
	smi_isp_wpe3_lite_put((void *)&is_qof_wpe3_lite);
#endif

	return 1;
}

void imgsys_wpe_set_initial_value(struct mtk_imgsys_dev *imgsys_dev)
{
	unsigned int hw_idx = 0, ary_idx = 0;

	switch (imgsys_dev->dev_ver) {
	case 1:
		g_mtk_imgsys_wpe_base_ofst = mtk_imgsys_wpe_base_ofst_1;
		g_wpe_base = WPE_A_BASE_P;
		g_wpe_hw_support = wpe_hw_support_1;
		g_wpe_ufod_p2_desc_ofst = WPE_UFOD_P2_DESC_OFST_1;
		break;
	default:
		g_mtk_imgsys_wpe_base_ofst = mtk_imgsys_wpe_base_ofst;
		g_wpe_base = WPE_A_BASE;
		g_wpe_hw_support = wpe_hw_support;
		g_wpe_ufod_p2_desc_ofst = WPE_UFOD_P2_DESC_OFST;
		break;
	}

	if (imgsys_wpe_8s_dbg_enable())
		dev_dbg(imgsys_dev->dev, "%s: +\n", __func__);

	for (hw_idx = REG_MAP_E_WPE_EIS; hw_idx <= REG_MAP_E_WPE_LITE; hw_idx++) {
		/* iomap registers */
		ary_idx = hw_idx - REG_MAP_E_WPE_EIS;
		if (!g_wpe_hw_support[ary_idx])
			continue;

		gWpeRegBA[ary_idx] = of_iomap(imgsys_dev->dev->of_node, hw_idx);
		if (!gWpeRegBA[ary_idx]) {
			dev_info(imgsys_dev->dev,
				"%s: error: unable to iomap wpe_%d registers, devnode(%s).\n",
				__func__, hw_idx, imgsys_dev->dev->of_node->name);
			continue;
		}
	}

	gVcoreRegBA = of_iomap(imgsys_dev->dev->of_node, REG_MAP_E_IMG_VCORE);
	if (!gVcoreRegBA) {
		pr_info("%s:unable to iomap Vcore reg, devnode()\n",
			__func__);
	}
	dev_info(imgsys_dev->dev, "%s: -\n", __func__);
}

void imgsys_wpe_set_hw_initial_value(struct mtk_imgsys_dev *imgsys_dev)
{
	void __iomem *ofset = NULL;
	unsigned int i = 0;
	unsigned int hw_idx = 0, ary_idx = 0;

	if (imgsys_wpe_8s_dbg_enable())
		dev_info(imgsys_dev->dev, "%s: +\n", __func__);

	for (hw_idx = REG_MAP_E_WPE_EIS; hw_idx <= REG_MAP_E_WPE_LITE; hw_idx++) {
		/* iomap registers */
		ary_idx = hw_idx - REG_MAP_E_WPE_EIS;
		if (!g_wpe_hw_support[ary_idx])
			continue;
		for (i = 0 ; i < WPE_INIT_ARRAY_COUNT ; i++) {
			ofset = gWpeRegBA[ary_idx] + mtk_imgsys_wpe_init_ary[i].ofset;
			writel(mtk_imgsys_wpe_init_ary[i].val, ofset);
		}
	}

	dev_info(imgsys_dev->dev, "%s: -\n", __func__);
}

bool imgsys_wpe_done_chk(struct mtk_imgsys_dev *imgsys_dev, uint32_t engine)
{
	void __iomem *wpeRegBA = 0L;
	unsigned int hw_idx = 0, ofst_idx;
	unsigned int wpeBase = 0;
	bool ret = true; //true: done
	uint32_t value = 0;
	uint32_t reg_ofst = 0x1c; //WPE_E1A_WPE_TOP_CTL_INT_STATUSX

	if (engine & IMGSYS_HW_FLAG_WPE_EIS)
		hw_idx = (unsigned int) REG_MAP_E_WPE_EIS;
	else if (engine & IMGSYS_HW_FLAG_WPE_TNR)
		hw_idx = (unsigned int) REG_MAP_E_WPE_TNR;
	else
		hw_idx = (unsigned int) REG_MAP_E_WPE_LITE;

	/* iomap registers */
	ofst_idx = hw_idx - (unsigned int) REG_MAP_E_WPE_EIS;
	if (ofst_idx >= WPE_HW_NUM) {
		pr_info("%s: ofst_idx(%d) >= WPE_HW_NUM(%d)",
			__func__, ofst_idx, (uint32_t)WPE_HW_NUM);
		return ret;
	}

	wpeBase = g_wpe_base + g_mtk_imgsys_wpe_base_ofst[ofst_idx];

	wpeRegBA = gWpeRegBA[ofst_idx];
	if (!wpeRegBA) {
		pr_info("%s: WPE_%d, RegBA = 0", __func__, hw_idx);
		return ret;
	}

	value = (uint32_t)ioread32((void *)(wpeRegBA + reg_ofst));

	if (!(value & 0x1)) {
		ret = false;
		pr_info(
		"%s: hw_comb:0x%x, polling WPE done fail!!! [0x%08x] 0x%x",
		__func__, engine,
		(unsigned int)(wpeBase + reg_ofst), value);
	}

	return ret;
}

void imgsys_wpe_updatecq(struct mtk_imgsys_dev *imgsys_dev,
			 struct img_swfrm_info *user_info,
			 struct private_data *priv_data,
			 int req_fd,
			 u64 tuning_iova,
			 unsigned int mode)
{
	const struct mtk_hcp_ops *hcp_ops = mtk_hcp_fetch_ops(imgsys_dev->scp_pdev);
	unsigned int j = 0;
	u64 u_iova_addr = 0;
	struct mtk_imgsys_req_fd_info *fd_info = NULL;
	struct dma_buf *dbuf = NULL;
	struct mtk_imgsys_request *req = NULL;
	struct mtk_imgsys_dev_buffer *dev_b = 0;
	u64 *u_cq_desc = NULL;
	union wpe_cq_cmd_desc_t *cq_desc = NULL;
	unsigned int tun_ofst = 0;
	struct flush_buf_info wpe_buf_info;
	void *cq_base = NULL;

	if (hcp_ops && hcp_ops->fetch_wpe_cq_mb_virt)
		cq_base = hcp_ops->fetch_wpe_cq_mb_virt(imgsys_dev->scp_pdev, mode);

	if (unlikely(cq_base == NULL)) {
		pr_err("[%s][%d] cq base is null!\n", __func__, __LINE__);
		return;
	}

	if (priv_data->need_update_desc) {
		// Update WPE_A_UFOD_P2 cq descriptor
		if (priv_data->buf_fd) {
			#ifndef MTK_IOVA_NOTCHECK
			dbuf = dma_buf_get(priv_data->buf_fd);
			#endif
			fd_info = &imgsys_dev->req_fd_cache.info_array[req_fd];
			req = (struct mtk_imgsys_request *) fd_info->req_addr_va;
			dev_b = req->buf_map[imgsys_dev->is_singledev_mode(req)];
			u_iova_addr = imgsys_dev->imgsys_get_iova(dbuf,
					priv_data->buf_fd,
					imgsys_dev, dev_b) + priv_data->buf_offset;

			u_cq_desc = (u64 *)((void *)(cq_base +
				priv_data->desc_offset +
				(g_wpe_ufod_p2_desc_ofst  * (sizeof(union wpe_cq_cmd_desc_t)))));

			cq_desc = (union wpe_cq_cmd_desc_t *)u_cq_desc;
			cq_desc->dram_addr_lsb = u_iova_addr & 0xFFFFFFFF;
			cq_desc->dram_addr_msb = (u_iova_addr >> 32) & 0xF;
			if (imgsys_wpe_8s_dbg_enable()) {
				pr_debug("%s: buf_fd(0x%08x) buf_ofst(0x%08x) buf_iova(0x%llx)\n",
					__func__, priv_data->buf_fd,
					priv_data->buf_offset, u_iova_addr);
				pr_debug("%s: des_ofst(0x%08x) cq_kva(0x%p) cq_desc(0x%x/0x%x/0x%x)\n",
					__func__, priv_data->desc_offset,
					u_cq_desc, cq_desc->raw[0], cq_desc->raw[1], cq_desc->raw[2]);
			}
		}

		// Update PSP tuning cq descriptor
		if (tuning_iova) {
			u_cq_desc = (u64 *)((void *)(cq_base + priv_data->desc_offset));
			cq_desc = (union wpe_cq_cmd_desc_t *)u_cq_desc;
			for (j = 0; j < WPE_CQ_DESC_NUM; j++) {
				if (cq_desc->dummy_1 == PSEUDO_DESC_TUNING) {
					tun_ofst = cq_desc->dram_addr_lsb;
					cq_desc->dram_addr_lsb = (tun_ofst + tuning_iova) & 0xFFFFFFFF;
					cq_desc->dram_addr_msb = ((tun_ofst + tuning_iova) >> 32) & 0xF;
					cq_desc->dummy_1 = 0x0; // set dummy_1 back to all zeros

					if (imgsys_wpe_8s_dbg_enable()) {
						pr_debug("%s: tuning_buf_iova(0x%llx) tun_ofst(0x%08x)\n",
							__func__, tuning_iova, tun_ofst);
						pr_debug("%s: des_ofst(0x%08x) cq_kva(0x%p) cq_desc(0x%x/0x%x/0x%x)\n",
							__func__, priv_data->desc_offset,
							u_cq_desc, cq_desc->raw[0], cq_desc->raw[1], cq_desc->raw[2]);
					}
				}
				cq_desc++;
			}
		}
		if (hcp_ops && hcp_ops->fetch_wpe_cq_mb_fd)
			wpe_buf_info.fd = hcp_ops->fetch_wpe_cq_mb_fd(imgsys_dev->scp_pdev, mode);
		wpe_buf_info.offset = priv_data->desc_offset;
		wpe_buf_info.len =
			((sizeof(union wpe_cq_cmd_desc_t) * WPE_CQ_DESC_NUM) + WPE_REG_SIZE);
		wpe_buf_info.mode = mode;
		wpe_buf_info.is_tuning = false;
		if (imgsys_wpe_8s_dbg_enable())
			pr_debug("imgsys_fw cq wpe_buf_info (%d/%d/%d), mode(%d)",
				wpe_buf_info.fd, wpe_buf_info.len,
				wpe_buf_info.offset, wpe_buf_info.mode);
		if (hcp_ops && hcp_ops->flush_mb && hcp_ops->fetch_wpe_cq_mb_id)
			hcp_ops->flush_mb(
				imgsys_dev->scp_pdev,
				hcp_ops->fetch_wpe_cq_mb_id(imgsys_dev->scp_pdev, mode),
				wpe_buf_info.offset,
				wpe_buf_info.len);

	}

	if (priv_data->need_flush_tdr) {
		// tdr buffer
		if (hcp_ops && hcp_ops->fetch_wpe_tdr_mb_fd)
			wpe_buf_info.fd = hcp_ops->fetch_wpe_tdr_mb_fd(imgsys_dev->scp_pdev, mode);
		wpe_buf_info.offset = priv_data->tdr_offset;
		wpe_buf_info.len = WPE_TDR_BUF_MAXSZ;
		wpe_buf_info.mode = mode;
		wpe_buf_info.is_tuning = false;
		if (imgsys_wpe_8s_dbg_enable())
			pr_debug("imgsys_fw tdr wpe_buf_info (%d/%d/%d), mode(%d)",
				wpe_buf_info.fd, wpe_buf_info.len,
				wpe_buf_info.offset, wpe_buf_info.mode);
		if (hcp_ops && hcp_ops->flush_mb && hcp_ops->fetch_wpe_tdr_mb_id)
			hcp_ops->flush_mb(
				imgsys_dev->scp_pdev,
				hcp_ops->fetch_wpe_tdr_mb_id(imgsys_dev->scp_pdev, mode),
				wpe_buf_info.offset,
				wpe_buf_info.len);

	}
}

void imgsys_wpe_cmdq_set_hw_initial_value(struct mtk_imgsys_dev *imgsys_dev,
		void *pkt, int hw_idx)
{
	unsigned int ofset;
	unsigned int i = 0;
	unsigned int ary_idx = 0;
	struct cmdq_pkt *package = NULL;
	unsigned int wpeBase = 0;

	if (imgsys_dev == NULL || pkt == NULL) {
		dump_stack();
		pr_err("[%s][%d] param fatal error!", __func__, __LINE__);
		return;
	}
	package = (struct cmdq_pkt *)pkt;

	if (imgsys_wpe_8s_dbg_enable())
		dev_info(imgsys_dev->dev, "%s: +\n", __func__);

	ary_idx = hw_idx - REG_MAP_E_WPE_EIS;
	wpeBase = g_wpe_base + g_mtk_imgsys_wpe_base_ofst[ary_idx];

	if (g_wpe_hw_support[ary_idx]) {
		/* iomap registers */
		for (i = 0 ; i < WPE_INIT_ARRAY_COUNT ; i++) {
			ofset = wpeBase + mtk_imgsys_wpe_init_ary[i].ofset;
			cmdq_pkt_write(package, NULL, ofset /*address*/,
					mtk_imgsys_wpe_init_ary[i].val, 0xffffffff);
		}
	} else {
		pr_err("[%s][%d] wpe %d: fail to write init value!",
				__func__, __LINE__, ary_idx);
	}

	dev_info(imgsys_dev->dev, "%s: -\n", __func__);
}

// TODO, this function haven't immgrate to jayer
void imgsys_wpe_debug_ufo_dump(struct mtk_imgsys_dev *imgsys_dev,
							void __iomem *wpeRegBA)
{
	unsigned int i;
	unsigned int debug_value[55] = {0x0};
	unsigned int sel_value = 0x0;

	writel((0xB<<12), (wpeRegBA + WPE_REG_DBG_SET));
	sel_value = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET));
	for (i = 0; i < 55; i++) {
		writel((i + 0xC00), (wpeRegBA + WPE_REG_DEC_CTL1));
		debug_value[i] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT));
	}

	pr_info("%s: [0x%x]dbg_sel: 0x%X, [0x%x]dec_ctrl1 [0x%x]ufo_st",
	  __func__, WPE_REG_DBG_SET, sel_value, WPE_REG_DEC_CTL1, WPE_REG_DBG_PORT);

	for (i = 0; i <= 10; i++) {
		pr_info("%s: [l][0x%x] 0x%08X 0x%08X 0x%08X 0x%08X 0x%08X",
		  __func__, (unsigned int)(0xC00+i*5),
		  debug_value[i*5+0], debug_value[i*5+1], debug_value[i*5+2],
		  debug_value[i*5+3], debug_value[i*5+4]);
	}

}

void imgsys_wpe_debug_dl_dump(struct mtk_imgsys_dev *imgsys_dev,
							void __iomem *wpeRegBA)
{
	unsigned int dbg_sel_value[3] = {0x0, 0x0, 0x0};
	unsigned int debug_value[3] = {0x0, 0x0, 0x0};
	unsigned int sel_value[3] = {0x0, 0x0, 0x0};

	dbg_sel_value[0] = (0xC << 12); //PQDIP
	dbg_sel_value[1] = (0xD << 12); //DIP
	dbg_sel_value[2] = (0xE << 12); //TRAW

	//line & pix cnt
	writel((dbg_sel_value[0] | (0x1 << 8)), (wpeRegBA + WPE_REG_DBG_SET));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT));

	writel((dbg_sel_value[1] | (0x1 << 8)), (wpeRegBA + WPE_REG_DBG_SET));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT));

	writel((dbg_sel_value[2] | (0x1 << 8)), (wpeRegBA + WPE_REG_DBG_SET));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT));

	pr_info("%s:[0x%x]dbg_sel,[0x%x](31:16)LnCnt(15:0)PixCnt:PQDIP[0x%x]0x%x,DIP[0x%x]0x%x,TRAW[0x%x]0x%x",
	  __func__, WPE_REG_DBG_SET, WPE_REG_DBG_PORT,
	  sel_value[0], debug_value[0], sel_value[1], debug_value[1],
	  sel_value[2], debug_value[2]);

	//req/rdy status (output)
	writel((dbg_sel_value[0] | (0x0 << 8)), (wpeRegBA + WPE_REG_DBG_SET));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT));

	writel((dbg_sel_value[1] | (0x0 << 8)), (wpeRegBA + WPE_REG_DBG_SET));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT));

	writel((dbg_sel_value[2] | (0x0 << 8)), (wpeRegBA + WPE_REG_DBG_SET));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT));

	pr_info("%s:[0x%x]dbg_sel,[0x%x]val/REQ/RDY:PQDIP[0x%x]0x%x/%d/%d,DIP[0x%x]0x%x/%d/%d,TRAW[0x%x]0x%x/%d/%d",
	  __func__, WPE_REG_DBG_SET, WPE_REG_DBG_PORT,
	  sel_value[0], debug_value[0],
	   ((debug_value[0] >> 24) & 0x1), ((debug_value[0] >> 23) & 0x1),
	  sel_value[1], debug_value[1],
	   ((debug_value[1] >> 24) & 0x1), ((debug_value[1] >> 23) & 0x1),
	  sel_value[2], debug_value[2],
	   ((debug_value[2] >> 24) & 0x1), ((debug_value[2] >> 23) & 0x1));
}

void imgsys_wpe_debug_module_dump(struct mtk_imgsys_dev *imgsys_dev,
							void __iomem *wpeRegBA, unsigned int ofst)
{
	unsigned int i = 0;
	unsigned int debug_value[4] = {0x0};
	unsigned int sel_value[4] = {0x0};

	//cache
	for (i = 0; i <= 3; i += 2) {
		writel(((0x2<<12) | (0x0<<8) | (i<<0)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
		sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
		debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));

		writel(((0x2<<12) | (0x1<<8) | (i<<0)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
		sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
		debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));

		writel(((0x2<<12) | (0x0<<8) | ((i+1)<<0)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
		sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
		debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));

		writel(((0x2<<12) | (0x1<<8) | ((i+1)<<0)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
		sel_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
		debug_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));

		pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, cache[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
			__func__, (WPE_REG_DBG_SET + ofst), (WPE_REG_DBG_PORT + ofst),
			sel_value[0], debug_value[0], sel_value[1], debug_value[1],
			sel_value[2], debug_value[2], sel_value[3], debug_value[3]);
	}

	/* 0x2248, 0x2348, 0x2448, 0x2548 */
	writel(((0x2<<12) | (0x2<<8) | ((4)<<4) | (8)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x3<<8) | ((4)<<4) | (8)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x4<<8) | ((4)<<4) | (8)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x5<<8) | ((4)<<4) | (8)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, cache[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
		__func__, (WPE_REG_DBG_SET + ofst), (WPE_REG_DBG_PORT + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1],
		sel_value[2], debug_value[2], sel_value[3], debug_value[3]);

	/* 0x2249, 0x2349, 0x2449, 0x2549 */
	writel(((0x2<<12) | (0x2<<8) | ((4)<<4) | (9)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x3<<8) | ((4)<<4) | (9)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x4<<8) | ((4)<<4) | (9)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x5<<8) | ((4)<<4) | (9)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, cache[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
		__func__, (WPE_REG_DBG_SET + ofst), (WPE_REG_DBG_PORT + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1],
		sel_value[2], debug_value[2], sel_value[3], debug_value[3]);

	/* 0x2258, 0x2358, 0x2458, 0x2558 */
	writel(((0x2<<12) | (0x2<<8) | ((5)<<4) | (8)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x3<<8) | ((5)<<4) | (8)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x4<<8) | ((5)<<4) | (8)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x5<<8) | ((5)<<4) | (8)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, cache[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
		__func__, (WPE_REG_DBG_SET + ofst), (WPE_REG_DBG_PORT + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1],
		sel_value[2], debug_value[2], sel_value[3], debug_value[3]);

	/* 0x2259, 0x2359, 0x2459, 0x2559 */
	writel(((0x2<<12) | (0x2<<8) | ((5)<<4) | (9)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x3<<8) | ((5)<<4) | (9)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x4<<8) | ((5)<<4) | (9)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x5<<8) | ((5)<<4) | (9)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, cache[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
		__func__, (WPE_REG_DBG_SET + ofst), (WPE_REG_DBG_PORT + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1],
		sel_value[2], debug_value[2], sel_value[3], debug_value[3]);

	/* 0x2268, 0x2368, 0x2468, 0x2568 */
	writel(((0x2<<12) | (0x2<<8) | ((6)<<4) | (8)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x3<<8) | ((6)<<4) | (8)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x4<<8) | ((6)<<4) | (8)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x5<<8) | ((6)<<4) | (8)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, cache[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
		__func__, (WPE_REG_DBG_SET + ofst), (WPE_REG_DBG_PORT + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1],
		sel_value[2], debug_value[2], sel_value[3], debug_value[3]);

	/* 0x2269, 0x2369, 0x2469, 0x2569 */
	writel(((0x2<<12) | (0x2<<8) | ((6)<<4) | (9)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x3<<8) | ((6)<<4) | (9)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x4<<8) | ((6)<<4) | (9)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x5<<8) | ((6)<<4) | (9)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, cache[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
		__func__, (WPE_REG_DBG_SET + ofst), (WPE_REG_DBG_PORT + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1],
		sel_value[2], debug_value[2], sel_value[3], debug_value[3]);

	/* 0x2278, 0x2378, 0x2478, 0x2578 */
	writel(((0x2<<12) | (0x2<<8) | ((7)<<4) | (8)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x3<<8) | ((7)<<4) | (8)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x4<<8) | ((7)<<4) | (8)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x5<<8) | ((7)<<4) | (8)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, cache[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
		__func__, (WPE_REG_DBG_SET + ofst), (WPE_REG_DBG_PORT + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1],
		sel_value[2], debug_value[2], sel_value[3], debug_value[3]);

	/* 0x2279, 0x2379, 0x2479, 0x2579 */
	writel(((0x2<<12) | (0x2<<8) | ((7)<<4) | (9)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x3<<8) | ((7)<<4) | (9)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x4<<8) | ((7)<<4) | (9)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x5<<8) | ((7)<<4) | (9)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, cache[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
		__func__, (WPE_REG_DBG_SET + ofst), (WPE_REG_DBG_PORT + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1],
		sel_value[2], debug_value[2], sel_value[3], debug_value[3]);

	/* 0x2042, 0x2242, 0x2442, 0x2842 */
	writel(((0x2<<12) | (0x0<<8) | ((4)<<4) | (2)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x2<<8) | ((4)<<4) | (2)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x4<<8) | ((4)<<4) | (2)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x8<<8) | ((4)<<4) | (2)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, cache[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
		__func__, (WPE_REG_DBG_SET + ofst), (WPE_REG_DBG_PORT + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1],
		sel_value[2], debug_value[2], sel_value[3], debug_value[3]);

	/* 0x2052, 0x2252, 0x2452, 0x2852 */
	writel(((0x2<<12) | (0x0<<8) | ((5)<<4) | (2)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x2<<8) | ((5)<<4) | (2)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x4<<8) | ((5)<<4) | (2)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x8<<8) | ((5)<<4) | (2)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, cache[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
		__func__, (WPE_REG_DBG_SET + ofst), (WPE_REG_DBG_PORT + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1],
		sel_value[2], debug_value[2], sel_value[3], debug_value[3]);

	/* 0x2062, 0x2262, 0x2462, 0x2862 */
	writel(((0x2<<12) | (0x0<<8) | ((6)<<4) | (2)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x2<<8) | ((6)<<4) | (2)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x4<<8) | ((6)<<4) | (2)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x8<<8) | ((6)<<4) | (2)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, cache[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
		__func__, (WPE_REG_DBG_SET + ofst), (WPE_REG_DBG_PORT + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1],
		sel_value[2], debug_value[2], sel_value[3], debug_value[3]);

	/* 0x2072, 0x2272, 0x2472, 0x2872 */
	writel(((0x2<<12) | (0x0<<8) | ((7)<<4) | (2)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x2<<8) | ((7)<<4) | (2)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x4<<8) | ((7)<<4) | (2)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x8<<8) | ((7)<<4) | (2)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, cache[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
		__func__, (WPE_REG_DBG_SET + ofst), (WPE_REG_DBG_PORT + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1],
		sel_value[2], debug_value[2], sel_value[3], debug_value[3]);

	/* 0x2044, 0x2144, 0x2844, 0x2a44 */
	writel(((0x2<<12) | (0x0<<8) | ((4)<<4) | (4)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x1<<8) | ((4)<<4) | (4)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x8<<8) | ((4)<<4) | (4)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0xa<<8) | ((4)<<4) | (4)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, cache[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
		__func__, (WPE_REG_DBG_SET + ofst), (WPE_REG_DBG_PORT + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1],
		sel_value[2], debug_value[2], sel_value[3], debug_value[3]);

	/* 0x2045, 0x2145, 0x2845, 0x2a45 */
	writel(((0x2<<12) | (0x0<<8) | ((4)<<4) | (5)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x1<<8) | ((4)<<4) | (5)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x8<<8) | ((4)<<4) | (5)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0xa<<8) | ((4)<<4) | (5)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, cache[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
		__func__, (WPE_REG_DBG_SET + ofst), (WPE_REG_DBG_PORT + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1],
		sel_value[2], debug_value[2], sel_value[3], debug_value[3]);

	/* 0x2054, 0x2154, 0x2854, 0x2a54 */
	writel(((0x2<<12) | (0x0<<8) | ((5)<<4) | (4)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x1<<8) | ((5)<<4) | (4)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x8<<8) | ((5)<<4) | (4)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0xa<<8) | ((5)<<4) | (4)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, cache[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
		__func__, (WPE_REG_DBG_SET + ofst), (WPE_REG_DBG_PORT + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1],
		sel_value[2], debug_value[2], sel_value[3], debug_value[3]);

	/* 0x2055, 0x2155, 0x2855, 0x2a55 */
	writel(((0x2<<12) | (0x0<<8) | ((5)<<4) | (5)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x1<<8) | ((5)<<4) | (5)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x8<<8) | ((5)<<4) | (5)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0xa<<8) | ((5)<<4) | (5)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, cache[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
		__func__, (WPE_REG_DBG_SET + ofst), (WPE_REG_DBG_PORT + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1],
		sel_value[2], debug_value[2], sel_value[3], debug_value[3]);

	/* 0x2064, 0x2164, 0x2864, 0x2a64 */
	writel(((0x2<<12) | (0x0<<8) | ((6)<<4) | (4)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x1<<8) | ((6)<<4) | (4)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x8<<8) | ((6)<<4) | (4)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0xa<<8) | ((6)<<4) | (4)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, cache[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
		__func__, (WPE_REG_DBG_SET + ofst), (WPE_REG_DBG_PORT + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1],
		sel_value[2], debug_value[2], sel_value[3], debug_value[3]);

	/* 0x2065, 0x2165, 0x2865, 0x2a65 */
	writel(((0x2<<12) | (0x0<<8) | ((6)<<4) | (5)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x1<<8) | ((6)<<4) | (5)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x8<<8) | ((6)<<4) | (5)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0xa<<8) | ((6)<<4) | (5)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, cache[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
		__func__, (WPE_REG_DBG_SET + ofst), (WPE_REG_DBG_PORT + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1],
		sel_value[2], debug_value[2], sel_value[3], debug_value[3]);

	/* 0x2074, 0x2174, 0x2874, 0x2a74 */
	writel(((0x2<<12) | (0x0<<8) | ((7)<<4) | (4)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x1<<8) | ((7)<<4) | (4)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x8<<8) | ((7)<<4) | (4)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0xa<<8) | ((7)<<4) | (4)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, cache[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
		__func__, (WPE_REG_DBG_SET + ofst), (WPE_REG_DBG_PORT + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1],
		sel_value[2], debug_value[2], sel_value[3], debug_value[3]);

	/* 0x2075, 0x2175, 0x2875, 0x2a75 */
	writel(((0x2<<12) | (0x0<<8) | ((7)<<4) | (5)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x1<<8) | ((7)<<4) | (5)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x8<<8) | ((7)<<4) | (5)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0xa<<8) | ((7)<<4) | (5)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, cache[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
		__func__, (WPE_REG_DBG_SET + ofst), (WPE_REG_DBG_PORT + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1],
		sel_value[2], debug_value[2], sel_value[3], debug_value[3]);

	/* 0x2040, 0x2050, 0x2060, 0x2070 */
	writel(((0x2<<12) | (0x0<<8) | ((4)<<4) | (0)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x0<<8) | ((5)<<4) | (0)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x0<<8) | ((6)<<4) | (0)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x0<<8) | ((7)<<4) | (0)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, cache[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
		__func__, (WPE_REG_DBG_SET + ofst), (WPE_REG_DBG_PORT + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1],
		sel_value[2], debug_value[2], sel_value[3], debug_value[3]);

	/* 0x2041, 0x2141, 0x2241 */
	writel(((0x2<<12) | (0x0<<8) | ((4)<<4) | (1)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x1<<8) | ((4)<<4) | (1)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x2<<8) | ((4)<<4) | (1)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, cache[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
		__func__, (WPE_REG_DBG_SET + ofst), (WPE_REG_DBG_PORT + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1],
		sel_value[2], debug_value[2]);

	/* 0x2051, 0x2151, 0x2251 */
	writel(((0x2<<12) | (0x0<<8) | ((5)<<4) | (1)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x1<<8) | ((5)<<4) | (1)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x2<<8) | ((5)<<4) | (1)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, cache[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
		__func__, (WPE_REG_DBG_SET + ofst), (WPE_REG_DBG_PORT + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1],
		sel_value[2], debug_value[2]);

	/* 0x2061, 0x2161, 0x2261 */
	writel(((0x2<<12) | (0x0<<8) | ((6)<<4) | (1)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x1<<8) | ((6)<<4) | (1)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x2<<8) | ((6)<<4) | (1)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, cache[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
		__func__, (WPE_REG_DBG_SET + ofst), (WPE_REG_DBG_PORT + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1],
		sel_value[2], debug_value[2]);

	/* 0x2071, 0x2171, 0x2271 */
	writel(((0x2<<12) | (0x0<<8) | ((7)<<4) | (1)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x1<<8) | ((7)<<4) | (1)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x2<<8) | ((7)<<4) | (1)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, cache[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
		__func__, (WPE_REG_DBG_SET + ofst), (WPE_REG_DBG_PORT + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1],
		sel_value[2], debug_value[2]);

	/* 0x2200, 0x2300, 0x2400, 0x2500 */
	writel(((0x2<<12) | (0x2<<8) | ((0)<<4) | (0)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x3<<8) | ((0)<<4) | (0)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x4<<8) | ((0)<<4) | (0)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x5<<8) | ((0)<<4) | (0)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, cache[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
		__func__, (WPE_REG_DBG_SET + ofst), (WPE_REG_DBG_PORT + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1],
		sel_value[2], debug_value[2], sel_value[3], debug_value[3]);

	/* 0x2210, 0x2310, 0x2410, 0x2510 */
	writel(((0x2<<12) | (0x2<<8) | ((1)<<4)  | (0)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x3<<8) | ((1)<<4)  | (0)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x4<<8) | ((1)<<4)  | (0)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	writel(((0x2<<12) | (0x5<<8) | ((1)<<4)  | (0)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));
	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, cache[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
		__func__, (WPE_REG_DBG_SET + ofst), (WPE_REG_DBG_PORT + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1],
		sel_value[2], debug_value[2], sel_value[3], debug_value[3]);

	for (i = 0xc; i <= 0xf; i += 2) {
		writel(((0x2<<12) | (0x0<<8) | (i<<0)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
		sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
		debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));

		writel(((0x2<<12) | (0x1<<8) | (i<<0)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
		sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
		debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));

		writel(((0x2<<12) | (0x0<<8) | ((i+1)<<0)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
		sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
		debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));

		writel(((0x2<<12) | (0x1<<8) | ((i+1)<<0)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
		sel_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
		debug_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));

		pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, cache[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
			__func__, (WPE_REG_DBG_SET + ofst), (WPE_REG_DBG_PORT + ofst),
			sel_value[0], debug_value[0], sel_value[1], debug_value[1],
			sel_value[2], debug_value[2], sel_value[3], debug_value[3]);
	}

	for (i = 0x1d; i <= 0x20; i += 2) {
		writel(((0x2<<12) | (0x0<<8) | (i<<0)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
		sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
		debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));

		writel(((0x2<<12) | (0x1<<8) | (i<<0)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
		sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
		debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));

		writel(((0x2<<12) | (0x0<<8) | ((i+1)<<0)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
		sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
		debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));

		writel(((0x2<<12) | (0x1<<8) | ((i+1)<<0)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
		sel_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
		debug_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));

		pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, cache[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
			__func__, (WPE_REG_DBG_SET + ofst), (WPE_REG_DBG_PORT + ofst),
			sel_value[0], debug_value[0], sel_value[1], debug_value[1],
			sel_value[2], debug_value[2], sel_value[3], debug_value[3]);
	}

	//psp, top
	writel(((0x4<<12) | (0x2<<8)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));

	writel(((0x4<<12) | (0x3<<8)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));

	writel(((0x4<<12) | (0x4<<8)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));

	writel((0x5<<12), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));

	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, psp[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x, top[0x%x]0x%x",
		__func__, (WPE_REG_DBG_SET + ofst), (WPE_REG_DBG_PORT + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1],
		sel_value[2], debug_value[2], sel_value[3], debug_value[3]);

	writel(((0x4<<12) | (0x2<<8)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));

	writel(((0x4<<12) | (0x3<<8)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));

	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, psp[0x%x]0x%x, [0x%x]0x%x",
		__func__, (WPE_REG_DBG_SET + ofst), (WPE_REG_DBG_PORT + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1]);

	writel(((0x0<<28) | (0x2<<24)), (wpeRegBA + WPE_REG_PSP_MERGE_CTL2 + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_PSP_MERGE_CTL2 + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_PSP_MERGE_CTL3 + ofst));

	writel(((0x1<<28) | (0x2<<24)), (wpeRegBA + WPE_REG_PSP_MERGE_CTL2 + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_PSP_MERGE_CTL2 + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_PSP_MERGE_CTL3 + ofst));

	writel(((0x2<<28) | (0x2<<24)), (wpeRegBA + WPE_REG_PSP_MERGE_CTL2 + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_PSP_MERGE_CTL2 + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_PSP_MERGE_CTL3 + ofst));

	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, psp[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
		__func__, (WPE_REG_PSP_MERGE_CTL2 + ofst), (WPE_REG_PSP_MERGE_CTL3 + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1], sel_value[2], debug_value[2]);

	writel(((0x0<<28) | (0x3<<24)), (wpeRegBA + WPE_REG_PSP_MERGE_CTL2 + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_PSP_MERGE_CTL2 + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_PSP_MERGE_CTL3 + ofst));

	writel(((0x1<<28) | (0x3<<24)), (wpeRegBA + WPE_REG_PSP_MERGE_CTL2 + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_PSP_MERGE_CTL2 + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_PSP_MERGE_CTL3 + ofst));

	writel(((0x2<<28) | (0x3<<24)), (wpeRegBA + WPE_REG_PSP_MERGE_CTL2 + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_PSP_MERGE_CTL2 + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_PSP_MERGE_CTL3 + ofst));

	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, psp[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
		__func__, (WPE_REG_PSP_MERGE_CTL2 + ofst), (WPE_REG_PSP_MERGE_CTL3 + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1], sel_value[2], debug_value[2]);

	//traw_chksum
	writel(((0xe<<12) | (0x0<<8)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));

	writel(((0xe<<12) | (0x1<<8)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));

	writel(((0xe<<12) | (0x2<<8)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));

	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, traw_chksum[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
		__func__, (WPE_REG_DBG_SET + ofst), (WPE_REG_DBG_PORT + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1],
		sel_value[2], debug_value[2]);

	//veci_chksum
	writel(0x100, (wpeRegBA + WPE_REG_DMA_DBG_SET + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DMA_DBG_SET + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DMA_DBG_PORT + ofst));

	writel(0x200, (wpeRegBA + WPE_REG_DMA_DBG_SET + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DMA_DBG_SET + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DMA_DBG_PORT + ofst));

	writel(0x300, (wpeRegBA + WPE_REG_DMA_DBG_SET + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DMA_DBG_SET + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DMA_DBG_PORT + ofst));

	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, veci_chksum[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
		__func__, (WPE_REG_DMA_DBG_SET + ofst), (WPE_REG_DMA_DBG_PORT + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1],
		sel_value[2], debug_value[2]);

	//vec2i_chksum
	writel(0x101, (wpeRegBA + WPE_REG_DMA_DBG_SET + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DMA_DBG_SET + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DMA_DBG_PORT + ofst));

	writel(0x201, (wpeRegBA + WPE_REG_DMA_DBG_SET + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DMA_DBG_SET + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DMA_DBG_PORT + ofst));

	writel(0x301, (wpeRegBA + WPE_REG_DMA_DBG_SET + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DMA_DBG_SET + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DMA_DBG_PORT + ofst));

	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, vec2i_chksum[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
		__func__, (WPE_REG_DMA_DBG_SET + ofst), (WPE_REG_DMA_DBG_PORT + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1],
		sel_value[2], debug_value[2]);

	//wpeo_chksum
	writel(0x103, (wpeRegBA + WPE_REG_DMA_DBG_SET + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DMA_DBG_SET + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DMA_DBG_PORT + ofst));

	writel(0x203, (wpeRegBA + WPE_REG_DMA_DBG_SET + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DMA_DBG_SET + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DMA_DBG_PORT + ofst));

	writel(0x303, (wpeRegBA + WPE_REG_DMA_DBG_SET + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DMA_DBG_SET + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DMA_DBG_PORT + ofst));

	writel(((0x10<<12)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));

	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, wpeo_chksum[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
		__func__, (WPE_REG_DMA_DBG_SET + ofst), (WPE_REG_DMA_DBG_PORT + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1],
		sel_value[2], debug_value[2], sel_value[3], debug_value[3]);

	//wpeo2_chksum
	writel(0x104, (wpeRegBA + WPE_REG_DMA_DBG_SET + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DMA_DBG_SET + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DMA_DBG_PORT + ofst));

	writel(0x204, (wpeRegBA + WPE_REG_DMA_DBG_SET + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DMA_DBG_SET + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DMA_DBG_PORT + ofst));

	writel(0x304, (wpeRegBA + WPE_REG_DMA_DBG_SET + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DMA_DBG_SET + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DMA_DBG_PORT + ofst));

	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, wpeo2_chksum[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
		__func__, (WPE_REG_DMA_DBG_SET + ofst), (WPE_REG_DMA_DBG_PORT + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1],
		sel_value[2], debug_value[2]);


	//masko_chksum
	writel(0x105, (wpeRegBA + WPE_REG_DMA_DBG_SET + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DMA_DBG_SET + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DMA_DBG_PORT + ofst));

	writel(0x205, (wpeRegBA + WPE_REG_DMA_DBG_SET + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DMA_DBG_SET + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DMA_DBG_PORT + ofst));

	writel(0x305, (wpeRegBA + WPE_REG_DMA_DBG_SET + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DMA_DBG_SET + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DMA_DBG_PORT + ofst));

	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, masko_chksum[0x%x]0x%x, [0x%x]0x%x, [0x%x]0x%x",
		__func__, (WPE_REG_DMA_DBG_SET + ofst), (WPE_REG_DMA_DBG_PORT + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1],
		sel_value[2], debug_value[2]);


	//wpeo_crop, wpeo2_crop, traw_crop
	writel(((0x15<<12) | (0x1<<8)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));

	writel(((0x16<<12) | (0x1<<8)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));

	writel(((0x17<<12) | (0x1<<8)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));

	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, wpeo_crop[0x%x]0x%x, wpeo2_crop[0x%x]0x%x, traw_crop[0x%x]0x%x",
		__func__, (WPE_REG_DBG_SET + ofst), (WPE_REG_DBG_PORT + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1],
		sel_value[2], debug_value[2]);

	//pak_c, pak_y
	writel(((0x1a<<12) | (0x1<<8)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));

	writel(((0x1a<<12) | (0x2<<8)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));

	writel(((0x1b<<12) | (0x1<<8)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));

	writel(((0x1b<<12) | (0x2<<8)), (wpeRegBA + WPE_REG_DBG_SET + ofst));
	sel_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET + ofst));
	debug_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT + ofst));

	pr_info("%s:[0x%x]dbg_sel,[0x%x]dbg_port, pak_c[0x%x]0x%x, [0x%x]0x%x, pak_y[0x%x]0x%x, [0x%x]0x%x",
		__func__, (WPE_REG_DBG_SET + ofst), (WPE_REG_DBG_PORT + ofst),
		sel_value[0], debug_value[0], sel_value[1], debug_value[1],
		sel_value[2], debug_value[2], sel_value[3], debug_value[3]);
}

void imgsys_wpe_debug_cq_dump(struct mtk_imgsys_dev *imgsys_dev,
							void __iomem *wpeRegBA)
{
	unsigned int dbg_sel_value = 0x0;
	unsigned int debug_value[5] = {0x0};
	unsigned int sel_value[5] = {0x0};

	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_CQ_THR0_CTL));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_CQ_THR1_CTL));
	if (!debug_value[0] || !debug_value[1]) {
		pr_info("%s: No cq_thr enabled! cq0:0x%x, cq1:0x%x",
			__func__, debug_value[0], debug_value[1]);
		return;
	}

	dbg_sel_value = (0x18 << 12);//cq_p2_eng

	//line & pix cnt
	writel((dbg_sel_value | 0x0), (wpeRegBA + WPE_REG_DBG_SET));
	sel_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET));
	debug_value[0] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT));

	writel((dbg_sel_value | 0x1), (wpeRegBA + WPE_REG_DBG_SET));
	sel_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET));
	debug_value[1] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT));

	writel((dbg_sel_value | 0x2), (wpeRegBA + WPE_REG_DBG_SET));
	sel_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET));
	debug_value[2] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT));

	writel((dbg_sel_value | 0x3), (wpeRegBA + WPE_REG_DBG_SET));
	sel_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET));
	debug_value[3] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT));

	writel((dbg_sel_value | 0x4), (wpeRegBA + WPE_REG_DBG_SET));
	sel_value[4] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_SET));
	debug_value[4] = (unsigned int)ioread32((void *)(wpeRegBA + WPE_REG_DBG_PORT));

	pr_info("%s:[0x%x]dbg_sel,[0x%x]cq_st[0x%x]0x%x,dma_dbg[0x%x]0x%x,dma_req[0x%x]0x%x,dma_rdy[0x%x]0x%x,dma_valid[0x%x]0x%x",
		__func__, WPE_REG_DBG_SET, WPE_REG_DBG_PORT,
		sel_value[0], debug_value[0], sel_value[1], debug_value[1],
		sel_value[2], debug_value[2], sel_value[3], debug_value[3],
		sel_value[4], debug_value[4]);

}


void imgsys_wpe_debug_dump(struct mtk_imgsys_dev *imgsys_dev,
							unsigned int engine)
{
	void __iomem *wpeRegBA = 0L;
	unsigned int ctl_en;
	unsigned int hw_idx = 0, ofst_idx;
	int i;

	pr_info("%s: +\n", __func__);

	if (engine & IMGSYS_HW_FLAG_WPE_EIS)
		hw_idx = (unsigned int) REG_MAP_E_WPE_EIS;
	else if (engine & IMGSYS_HW_FLAG_WPE_TNR)
		hw_idx = (unsigned int) REG_MAP_E_WPE_TNR;
	else
		hw_idx = (unsigned int) REG_MAP_E_WPE_LITE;

	/* iomap registers */
	ofst_idx = hw_idx - REG_MAP_E_WPE_EIS;
	if (ofst_idx >= WPE_HW_NUM) {
		pr_info("%s: ofst_idx(%d) >= WPE_HW_NUM(%d)",
			__func__, ofst_idx, (uint32_t)WPE_HW_NUM);
		return;
	}

	wpeRegBA = gWpeRegBA[ofst_idx];
	if (!wpeRegBA) {
		pr_info("%s: WPE_%d, RegBA = 0", __func__, hw_idx);
		return;
	}
	pr_info("%s: ==== Dump WPE_%d =====", __func__, ofst_idx);

	//DL
	ctl_en = (unsigned int)ioread32((void *)(wpeRegBA + 0x4));
	if (ctl_en & (PQDIP_DL|DIP_DL|TRAW_DL)) {
		pr_info("%s: WPE Done: %d", __func__,
			!(ioread32((void *)(wpeRegBA))) &&
			(ioread32((void *)(wpeRegBA + 0x1C)) & 0x1));
		pr_info("%s: WPE_DL: PQDIP(%d), DIP(%d), TRAW(%d)", __func__,
			(ctl_en & PQDIP_DL) > 0, (ctl_en & DIP_DL) > 0, (ctl_en & TRAW_DL) > 0);
		imgsys_wpe_debug_dl_dump(imgsys_dev, wpeRegBA);
	}

	imgsys_wpe_debug_cq_dump(imgsys_dev, wpeRegBA);
	imgsys_wpe_debug_module_dump(imgsys_dev, wpeRegBA, 0);

	// Dump all wpe register
	for (i = 0; i < mtk_imgsys_wpe_reg_size[ofst_idx]; i += 0x10) {
		pr_info("%s: [0x%08X] 0x%08X 0x%08X 0x%08X 0x%08X", __func__,
			(unsigned int)(g_wpe_base + g_mtk_imgsys_wpe_base_ofst[ofst_idx] + i),
			(unsigned int)ioread32((void *)(wpeRegBA + i)),
			(unsigned int)ioread32((void *)(wpeRegBA + i + 0x4)),
			(unsigned int)ioread32((void *)(wpeRegBA + i + 0x8)),
			(unsigned int)ioread32((void *)(wpeRegBA + i + 0xC)));

		ssize_t written = mtk_img_kernel_write(imgsys_dev->scp_pdev,
			"[0x%08X] 0x%08X 0x%08X 0x%08X 0x%08X\n",
			(unsigned int)(g_wpe_base + g_mtk_imgsys_wpe_base_ofst[ofst_idx] + i),
			(unsigned int)ioread32((void *)(wpeRegBA + i)),
			(unsigned int)ioread32((void *)(wpeRegBA + i + 0x4)),
			(unsigned int)ioread32((void *)(wpeRegBA + i + 0x8)),
			(unsigned int)ioread32((void *)(wpeRegBA + i + 0xC)));

		if(written < 0)
			pr_debug("Failed to write WPE register values to AEE buffer\n");
	}

	//UFO
	if (ctl_en & DECOMP_EN) {
		imgsys_wpe_debug_ufo_dump(imgsys_dev, wpeRegBA);
		imgsys_wpe_debug_ufo_dump(imgsys_dev, wpeRegBA);
	}

	pr_info("[gals_tx dgb addr] 0x34780048 = 0x%08X",
		(unsigned int)ioread32((void *)(gVcoreRegBA + 0x48)));
	pr_info("[gals_tx dgb addr] 0x3478004C = 0x%08X",
		(unsigned int)ioread32((void *)(gVcoreRegBA + 0x4C)));
	pr_info("[gals_tx dgb addr] 0x34780050 = 0x%08X",
		(unsigned int)ioread32((void *)(gVcoreRegBA + 0x50)));
	pr_info("[gals_tx dgb addr] 0x34780054 = 0x%08X",
		(unsigned int)ioread32((void *)(gVcoreRegBA + 0x54)));
	pr_info("[ACK dgb addr] 0x347800C4 = 0x%08X",
		(unsigned int)ioread32((void *)(gVcoreRegBA + 0xC4)));
	pr_info("[ACK dgb addr] 0x347800C8 = 0x%08X",
		(unsigned int)ioread32((void *)(gVcoreRegBA + 0xC8)));
	pr_info("[ACK dgb addr] 0x347800CC = 0x%08X",
		(unsigned int)ioread32((void *)(gVcoreRegBA + 0xCC)));
	//
	pr_info("%s: -\n", __func__);
}

void imgsys_wpe_uninit(struct mtk_imgsys_dev *imgsys_dev)
{
	unsigned int i;

	for (i = 0; i < WPE_HW_NUM; i++)
		if (g_wpe_hw_support[i]) {
			iounmap(gWpeRegBA[i]);
			gWpeRegBA[i] = 0L;
		}

	if (gVcoreRegBA) {
		iounmap(gVcoreRegBA);
		gVcoreRegBA = 0L;
	}
}
#if (KERNEL_VERSION(6, 13, 0) > LINUX_VERSION_CODE)
MODULE_IMPORT_NS(DMA_BUF);
#else
MODULE_IMPORT_NS("DMA_BUF");
#endif

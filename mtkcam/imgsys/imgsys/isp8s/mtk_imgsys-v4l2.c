...
#define CLK_READY
#define EFUSE_S2P_RX_DONE_BIT  BIT(29)
		...
		unsigned long flags, void *data)
{
	...
	if (flags == GENPD_NOTIFY_PRE_OFF || flags == GENPD_NOTIFY_PRE_ON) {
		dev_info(imgsys_dev->dev, "IMGSYS_FLOW: 0x%x", imgsys_dev->sw_pm_flow_cnt);
	} else if (flags == GENPD_NOTIFY_OFF || flags == GENPD_NOTIFY_ON) {
		/*
		 * MT6881 Requirement: Poll EFUSE_S2P_RX_DONE at post_off/on event.
		 * Address: 0x1C001E28, Bit: 29
		 */
		dev_info(imgsys_dev->dev,
				"%s: Imgsys Polling EFUSE_S2P_RX_DONE\n", __func__);
		if (imgsysEfuseRxDoneRegBA) {
			/* Wait indefinitely until bit is set */
			while (1) {
				val = readl(imgsysEfuseRxDoneRegBA);
				if (val & EFUSE_S2P_RX_DONE_BIT)
					break;
				udelay(10);
			}
		} else {
			dev_info(imgsys_dev->dev,
				"%s: imgsysEfuseRxDoneRegBA not mapped\n", __func__);
		}
	}

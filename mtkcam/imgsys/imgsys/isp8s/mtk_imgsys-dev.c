...
#endif
{
...
#else
	const singlenode_desc_ops_t *sn_ops = NULL;
	uint64_t meta;
#endif
...
#else
	sn_ops = fetch_singlenode_desc_ops(dev_buf->dev_fmt->format);
	meta = (uint64_t)src;
	tmax = fetch_singlenode_desc_tmax(dev_buf->dev_fmt->format);
	tnum = sn_ops->hdr_ops->_fparams_tnum(meta);
#endif
	/*fd2iova : need 2D array parsing*/
	if (tnum >= tmax) {
		tnum = tmax;
	...
	for (t = 0; t < tnum; t++) {
		for (j = 0; j < SCALE_MAX; j++) {
			if (iova_need)
				mtk_imgsys_desc_iova(pipe,
				sn_ops->hdr_ops->_fparams(meta, t), dev_buf);
			/* TODO: disable memcpy for imgstream fd */
			if (!dst)
				continue;
			memcpy(sn_ops->hdr_ops->_fparams(*dst, t),
					sn_ops->hdr_ops->_fparams(meta, t),
					sizeof(struct frameparams));
		}
	}
	if (dst)
		((struct header_desc *)dst)->fparams_tnum = sn_ops->hdr_ops->_fparams_tnum(meta);
#endif
}
#endif
...
#else
	for (t = 0; t < tnum; t++) {
		if (!sn_ops->_dmas_enable(desc, dma, t))
			continue;
		for (j = 0; j < SCALE_MAX; j++) {
			if (iova_need)
				mtk_imgsys_desc_iova(pipe,
...
#if defined (IMGSYS_SINGLE_NODE_DESC_V2)
static void mtk_imgsys_desc_set_v2(struct mtk_imgsys_pipe *pipe,
				struct mtk_imgsys_dev_buffer *dev_buf,
				const struct singlenode_desc_ops_t *sn_ops,
				uint64_t desc)

{
	unsigned int tmax, tnum, i;

	tmax = fetch_singlenode_desc_tmax(dev_buf->dev_fmt->format);
	tnum = sn_ops->_frame_num(desc);
	...
	for (i = 0; i < tnum; i++) {
		/*tuning_meta*/
		if (sn_ops->_dmas_enable(desc, MTK_IMGSYS_VIDEO_NODE_TUNING_OUT, i))
			mtk_imgsys_desc_iova(pipe,
			sn_ops->_tuning_meta(desc, i), dev_buf);
	}
}
#endif
...
#if !defined (IMGSYS_SINGLE_NODE_DESC_V2)
		mtk_imgsys_desc_set(pipe, desc_dma, (uint64_t *)desc, need_iova, buf_dma);
#endif
	}
#ifdef REQ_TIMESTAMP
...
void mtk_imgsys_sd_desc_map_iova(struct mtk_imgsys_request *req)
{
	...
	case V4L2_META_FMT_MTISP_SD:
		...
		req->req_stat = &desc_sd->req_stat;
		break;
	case V4L2_META_FMT_MTISP_SDNORM:
		desc_sd_norm =
			(struct singlenode_desc_norm *)buf_sd->va_daddr[0];
		req->req_stat = &desc_sd_norm->req_stat;
		break;
	}
#else
	sn_ops = fetch_singlenode_desc_ops(buf_sd->dev_fmt->format);
	desc = buf_sd->va_daddr[0];
	req->req_stat = sn_ops->_req_stat(desc);
#endif
	...
	src = NULL;
	dst = NULL;
	mtk_imgsys_desc_set_v2(pipe, buf_sd, sn_ops, desc);
#else
	/* tuning */
#ifndef MTK_DESC_OPS
	if (desc_sd)
...
#else
	src = (uint64_t *)sn_ops->_tuning_meta(desc);
#endif
...
#else
		dst = (uint64_t *)&dip_param->tuning_meta;
#endif
...
#else
	src = (uint64_t *)sn_ops->_ctrl_meta(desc);
#endif
...
#else
		dst = (uint64_t *)&dip_param->ctrl_meta;
#endif
					...
					bool isMENode)
{
	...
	for (i = 0; i < tnum ; i++) {
		mtk_imgsys_desc_fill_dmabuf(pipe,
			sn_ops->hdr_ops->_fparams(meta, i), isMENode);
	}
#endif
}
#endif
...
#if defined (IMGSYS_SINGLE_NODE_DESC_V2)
static void mtk_imgsys_singledevice_fill_ipi_param_v2(struct mtk_imgsys_pipe *pipe,
					struct mtk_imgsys_dev_buffer *dev_buf,
					const struct singlenode_desc_ops_t *sn_ops,
					uint64_t desc)
{
	...
	for (i = 0; i < tnum ; i++) {
		for (j = 0; j < SCALE_MAX; j++) {
			...
			if (sn_ops->_dmas_enable(desc, MTK_IMGSYS_VIDEO_NODE_TUNING_OUT, i))
				mtk_imgsys_desc_fill_dmabuf(pipe,
					sn_ops->_tuning_meta(desc, i), 0);
			/*ctrl_meta*/
			mtk_imgsys_desc_fill_dmabuf(pipe,
					sn_ops->_ctrl_meta(desc, i), 1);
			/*ctrl_meta_from_user*/
			mtk_imgsys_desc_fill_dmabuf(pipe,
					sn_ops->_ctrl_meta_from_user(desc, i), 1);
			/*ctrl_meta_from_user_mae*/
			mtk_imgsys_desc_fill_dmabuf(pipe,
					sn_ops->_ctrl_meta_from_user_mae(desc, i), 1);
		}
	}
}
#endif
...
#else
	for (i = 0; i < tnum ; i++) {
		if (!sn_ops->_dmas_enable(desc, dma, i))
			continue;
		for (j = 0; j < SCALE_MAX; j++)
			mtk_imgsys_desc_fill_dmabuf(pipe,
				sn_ops->hdr_ops->_fparams(meta, i), isMENode);
...
void mtk_imgsys_singledevice_ipi_params_config(struct mtk_imgsys_request *req)
{
	...
	case V4L2_META_FMT_MTISP_SD:
		...
		req->req_stat = &singledevice_desc_dma->req_stat;
		break;
	/* NORM */
	default:
	case V4L2_META_FMT_MTISP_SDNORM:
		...
		req->req_stat = &singledevice_desc_norm->req_stat;
		break;
	}
#else
	sn_ops = fetch_singlenode_desc_ops(buf_in->dev_fmt->format);
	desc = buf_in->va_daddr[0];
	req->req_stat = sn_ops->_req_stat(desc);
#if !defined (IMGSYS_SINGLE_NODE_DESC_V2)
	tuning_meta = (void *)sn_ops->_tuning_meta(desc);
	ctrl_meta = (void *)sn_ops->_ctrl_meta(desc);
	ctrl_meta_from_user = (void *)sn_ops->_ctrl_meta_from_user(desc);
#endif
#endif
	{
		req_track = (union request_track *)req->req_stat;
...
#if defined (IMGSYS_SINGLE_NODE_DESC_V2)
	mtk_imgsys_singledevice_fill_ipi_param_v2(pipe,
			buf_in, sn_ops, desc);
#else

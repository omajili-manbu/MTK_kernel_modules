		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	if (read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
		...
		CAM_CAL_LOG_DBG("read_data success [sensor ID] = 0x%x\n", pCamCalData->sensorID);
		err = CAM_CAL_ERR_NO_ERR;
	} else {
		CAM_CAL_LOG_ERR("Read Failed\n");
		show_cmd_error_log(pCamCalData->Command);
	...
	if (pCamCalData->sensorID == YILIOV08F10FRONT_SENSOR_ID) {
		read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID, 0x0201, 1, &vendor_id);
	} else if (pCamCalData->sensorID == PRAGUEGC08A8ULTRA_SENSOR_ID) {
		read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID, 0x14D0, 1, &vendor_id);
	} else {
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	/* get AWB enable bit */
	if (pCamCalData->sensorID == YILIOV08F10FRONT_SENSOR_ID){ /*yili ov08f*/
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				0x0943, 1, (unsigned char *)&AWBConfig);
	} else if(pCamCalData->sensorID == PRAGUEGC08A8ULTRA_SENSOR_ID){ /*prague gc08a8*/
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				0x4DE8, 1, (unsigned char *)&AWBConfig);
	} else {
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	if (table_size > 0) {
		...
		if (PRAGUEOVX8000WIDE_SENSOR_ID == pCamCalData->sensorID)
...
#ifdef __XIAOMI_CAMERA__
	if(WARHOLS5KJN5TELE_SENSOR_ID == pCamCalData->sensorID){
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
		...
		read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
								(start_addr + pCamCalData->PDAF.Size_of_PDAF + 2), 1004*3,
								(u8 *)&pCamCalData->PDAF.Data[496]);
		pCamCalData->PDAF.Size_of_PDAF = 3508;
	} else if (YILIOV13B10WIDE_SENSOR_ID == pCamCalData->sensorID) {
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				start_addr, 496, (unsigned char *)&pCamCalData->PDAF.Data[0]);
		if (read_data_size > 0) {
			pCamCalData->PDAF.Size_of_PDAF = 496;
		...
		if (WARHOLOVX9500WIDE_SENSOR_ID == pCamCalData->sensorID) {
			read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
							(start_addr + pCamCalData->PDAF.Size_of_PDAF + 11), 496, (unsigned char *)&pCamCalData->PDAF.Data[2600]);
			pCamCalData->PDAF.Size_of_PDAF = 3096;
			read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
							(start_addr + pCamCalData->PDAF.Size_of_PDAF + 13), 1004,
							(u8 *)&pCamCalData->PDAF.Data[3096]);
...
/*yqy test qcom pdaf calibration*/
// unsigned int xiaomi_do_qcom_pdaf(struct EEPROM_DRV_FD_DATA *pdata,
// 		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
// {
...
// 	int read_data_size;
// 	int err =  CamCalReturnErr[pCamCalData->Command];
// 	unsigned int mtk_start_addr = 0x16C1;
// 	(void) start_addr;
// 	(void) block_size;
	
// 	//qcom pd cailbration
// 		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
// 						start_addr, 994, (unsigned char *)&pCamCalData->PDAF.Data[0]);
// 		if (read_data_size > 0) {
// 			pCamCalData->PDAF.Size_of_PDAF = 994;
// 			err = CAM_CAL_ERR_NO_ERR;
// 			pCamCalData->PDAF.qcomflag[0] = 4;  //gain map
// 			pCamCalData->PDAF.qcomflag[1] = 8;  //left gain map
// 			pCamCalData->PDAF.qcomflag[2] = 450; //right gain map
// 			pCamCalData->PDAF.qcomflag[3] = 892; // q factory
// 			pCamCalData->PDAF.qcomflag[4] = 894; // DCC map
// 			pCamCalData->PDAF.qcomflag[5] = 0; // pd offset
...
// 		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
// 						mtk_start_addr, 496, (unsigned char *)&pCamCalData->PDAF.Data[994]);
// 		if (read_data_size > 0) {
// 			pCamCalData->PDAF.mtk_Size_of_PDAF = 496;
// 			err = CAM_CAL_ERR_NO_ERR;
// 		}
// 		else {
// 			CAM_CAL_LOG_ERR("Read Failed\n");
...
// 		read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
// 						(mtk_start_addr + pCamCalData->PDAF.mtk_Size_of_PDAF + 2), 1004,
// 						(u8 *)&pCamCalData->PDAF.Data[496+994]);

// 		pCamCalData->PDAF.mtk_Size_of_PDAF = 1500;
// 		pCamCalData->PDAF.Size_of_PDAF = 1500+994;
// 	CAM_CAL_LOG_DBG("======================Qcom PDAF Data==================\n");
// 	CAM_CAL_LOG_DBG("First eight %x, %x, %x, %x, %x, %x, %x, %x,qcom last %x\n",
// 		pCamCalData->PDAF.Data[0],
// 		pCamCalData->PDAF.Data[1],
// 		pCamCalData->PDAF.Data[2],
// 		pCamCalData->PDAF.Data[3],
// 		pCamCalData->PDAF.Data[4],
// 		pCamCalData->PDAF.Data[5],
// 		pCamCalData->PDAF.Data[6],
// 		pCamCalData->PDAF.Data[7],
// 		pCamCalData->PDAF.Data[993]);
// 	CAM_CAL_LOG_DBG("======================Qcom PDAF Data==================\n");
//         CAM_CAL_LOG_DBG("======================MTK PDAF Data==================\n");
//         CAM_CAL_LOG_DBG("First eight %x, %x, %x, %x, %x, %x, %x, %x,mtk last %x\n",
//                 pCamCalData->PDAF.Data[994],
//                 pCamCalData->PDAF.Data[995],
//                 pCamCalData->PDAF.Data[996],
//                 pCamCalData->PDAF.Data[997],
//                 pCamCalData->PDAF.Data[998],
//                 pCamCalData->PDAF.Data[999],
//                 pCamCalData->PDAF.Data[1000],
//                 pCamCalData->PDAF.Data[1001],
// 		pCamCalData->PDAF.Data[2493]);
//         CAM_CAL_LOG_DBG("======================MTK PDAF Data==================\n");

				...
				unsigned int sensorID, unsigned int *_cfg)
{
	...
	if ((check_id&mask) == (cfg->layout->header_id&mask)) {
		...
		must_log("header_id not matched 0x%08x 0x%08x\n",
			check_id, cfg->layout->header_id);
		...
		unsigned int offset, unsigned int length, unsigned char *data)
{
	...
	if (cam_cal_config->enable_preload && bufSize) {
		...
		must_log("read_data_func sensorid = 0x%x, offset = 0x%x, length = 0x%x, staAddr = 0x%x, bufSize = 0x%x,",cam_cal_config->sensor_id, offset, length, staAddr, bufSize);
		if (pdata->pdrv->config_info.mp_eeprom_preload != NULL && cam_cal_config->sensor_id == 0x08a8) {
				if (offset < staAddr) {
					must_log("Read data from memory, sensorid(0x%x), offset(0x%x), staAddr(0x%x)",cam_cal_config->sensor_id, offset, staAddr);
					return 0;
				}
				offset = (offset - staAddr) / 8 + staAddr;
				must_log("gc08a8_offset upstate as(0x%x)", offset);
		}
#endif
		if (!(pdata->pdrv->config_info.mp_eeprom_preload == NULL ||

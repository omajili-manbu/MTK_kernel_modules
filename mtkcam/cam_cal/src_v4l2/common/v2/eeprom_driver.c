				...
				unsigned int offset, unsigned int size)
{
	...
	if (plist && plist->readCamCalData) {
		...
		if (pdata->sensor_info.sensor_id == 0x5609)
		{
			must_log("customize for yili ov08f: 0x%x, 0x%x\n", pdata->pdrv->pi2c_client->addr, pdata->sensor_info.sensor_id);
			ret = YILI_OV08F_OTP_read_region(pdata->pdrv->pi2c_client,
						offset, buf, size);
		} else if (pdata->sensor_info.sensor_id == 0x08A8) {
			must_log("customize for prague gc08a8: 0x%x, 0x%x\n", pdata->pdrv->pi2c_client->addr, pdata->sensor_info.sensor_id);
			ret = PRAGUE_GC08A8_OTP_read_region(pdata->pdrv->pi2c_client,
						offset, buf, size);
		} else {
			must_log("no customized\n");

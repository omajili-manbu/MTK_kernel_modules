...
unsigned int YILI_OV08F_OTP_read_region(struct i2c_client *client, unsigned int addr,
				unsigned char *data, unsigned int size)
{
	unsigned int ret = 0;
	struct timespec64 t;

	must_log("YILI_OV08F_OTP_read_region : client 0x%x, addr 0x%x, size %d", client->addr, addr, size);
	EEPROM_PROFILE_INIT(&t);
...
#ifdef __XIAOMI_CAMERA__
static int Read_I2C_U16_U8_CAM_CAL(struct i2c_client *i2c_client, u16 reg, u8 *val)
{
	int ret;
	u8 buf[2];
...
static int Write_I2C_U16_U8_CAM_CAL(struct i2c_client *i2c_client, u16 reg, u8 val)
{
	int ret;
	u8 buf[3];
	struct i2c_msg msg;
	...
	// dev_info(&i2c_client->dev, "[%s] [addr] = 0x%02x,[reg] = 0x%04x,[val] = 0x%02x", __func__, i2c_client->addr, reg, val);
...
unsigned int PRAGUE_GC08A8_OTP_read_region(struct i2c_client *client, unsigned int addr,
				unsigned char *data, unsigned int size)
{
	int ret = 0;
	struct timespec64 t;
	const unsigned int startAddr = 0x14C8;
	const unsigned int whloeSize = 0x1D67;

	EEPROM_PROFILE_INIT(&t);
	u32 offset = 0;
	//read otp init setting
	Write_I2C_U16_U8_CAM_CAL(client, 0x031C, 0x60);
	Write_I2C_U16_U8_CAM_CAL(client, 0x0315, 0x80);
	Write_I2C_U16_U8_CAM_CAL(client, 0x0324, 0x42);
	Write_I2C_U16_U8_CAM_CAL(client, 0x0316, 0x09);
	Write_I2C_U16_U8_CAM_CAL(client, 0x0a67, 0x80);
	Write_I2C_U16_U8_CAM_CAL(client, 0x0313, 0x00);
	Write_I2C_U16_U8_CAM_CAL(client, 0x0a53, 0x0e);
	Write_I2C_U16_U8_CAM_CAL(client, 0x0a65, 0x17);
	Write_I2C_U16_U8_CAM_CAL(client, 0x0a68, 0xa1);
	Write_I2C_U16_U8_CAM_CAL(client, 0x0a47, 0x00);
	Write_I2C_U16_U8_CAM_CAL(client, 0x0a58, 0x00);
	Write_I2C_U16_U8_CAM_CAL(client, 0x0ace, 0x0c);
	mdelay(10);

	unsigned int eeprom_addr;
	unsigned int needSize;
	needSize = size;
	must_log("Read begin: addr 0x%x, offset %d, size %d, needsize:%d", addr, offset, size, needSize);
	if(addr == startAddr && (size == whloeSize)){ /*for dump*/
		eeprom_addr = startAddr;
		needSize = whloeSize;
		must_log("FOR preload, startAddr/size: 0x%x/0x%x", eeprom_addr, needSize);
	} else if (addr < startAddr || addr > (startAddr + (whloeSize - 1) * 8) || size > whloeSize) {
		must_log("gc08a8 read sensor otp: size or startAddr error\n");
		return ret;
	} else {
		unsigned int eeprom_flagAddr;
		eeprom_flagAddr = 0xFFF0;
		Write_I2C_U16_U8_CAM_CAL(client, 0x0a69, (eeprom_flagAddr >> 8) & 0xFF);
		Write_I2C_U16_U8_CAM_CAL(client, 0x0a6a, eeprom_flagAddr & 0xFF);

		Write_I2C_U16_U8_CAM_CAL(client, 0x0313, 0x20);
		Write_I2C_U16_U8_CAM_CAL(client, 0x0313, 0x12);
		
		u8 groupFlag1, groupFlag2;
		ret = Read_I2C_U16_U8_CAM_CAL(client, 0x0a6c, &groupFlag1);
		ret = Read_I2C_U16_U8_CAM_CAL(client, 0x0a6c, &groupFlag2);
		if (ret < 0) {
			must_log("gc08a8 read group flag error: flag1 0x%x, flag2 0x%x\n", groupFlag1, groupFlag2);
			return ret;
		}
		if (groupFlag1 == 0x55 && (groupFlag2 == 0x00 || groupFlag2 == 0xFF)) {
			offset = 0;    //group 0
		} else if (groupFlag2 == 0x55 && (groupFlag1 == 0x00 || groupFlag1 == 0xFF)) {
			offset = 30104; //group 1
		} else {
			must_log("gc08a8 read group flag error: flag1 0x%x, flag2 0x%x\n", groupFlag1, groupFlag2);
			return ret;
		}
		eeprom_addr = addr + offset;
		must_log("addr 0x%x, offset %d, eeprom_addr:0x%x size %d, flag: 0x%x/0x%x, needsize:%d", addr, offset, eeprom_addr, size, groupFlag1, groupFlag2, needSize);
	}

	Write_I2C_U16_U8_CAM_CAL(client, 0x0a69, (eeprom_addr >> 8) & 0xFF);
	Write_I2C_U16_U8_CAM_CAL(client, 0x0a6a, eeprom_addr & 0xFF);

	Write_I2C_U16_U8_CAM_CAL(client, 0x0313, 0x20);
	Write_I2C_U16_U8_CAM_CAL(client, 0x0313, 0x12);

	for(int i = 0; i < needSize; i++) {
		ret = Read_I2C_U16_U8_CAM_CAL(client, 0x0a6c, &data[i]);
		if (ret < 0) {
			must_log("i2c error, gc08a8 read sensor otp error\n");
			return ret;
		}
	}

	EEPROM_PROFILE(&t, "GC08A8_OTP_read_time");
	return needSize;
}

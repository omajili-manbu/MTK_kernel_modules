...
/* --------- define region -------- */
#define PDA_MMQOS
/* #define TWO_OFL_PDA_SUPPORT */
#define FOV_MAXIMUM 400
#define ROI_MAXIMUM 2048
/* -------------------------------- */
 ...
 *                               Porting Part
 ******************************************************************************/
#define CAMSYS_NODE_COMPATIBLE "mediatek,mt6881-camsys_mraw"
#define PDA_0_RESET_BITMASK (BIT(16) | BIT(17))
#define PDA_1_RESET_BITMASK (BIT(18) | BIT(19))
...
#else
#define PDA_MMQOS_PDA2_RDMA_NUM (0)
#endif
...
#else
#define PDA_MMQOS_PDA2_RDMA_B_NUM (0)
#endif
...
#else
#define PDA_MMQOS_PDA2_WDMA_NUM (0)
#endif
#endif
...
void pda_mmqos_bw_set(struct PDA_Data_t *pda_Pdadata)
{
	...
	if (FOV > ((B_N > 0) ? 100 : FOV_MAXIMUM)) {
		LOG_INF("FOV(%d) is out of range, max FOV is %d\n",
			FOV, (B_N > 0) ? 100 : FOV_MAXIMUM);
	...
	if (total_roi > ROI_MAXIMUM) {
		LOG_INF("total ROI num(%d) is out of range, max ROI num is %d\n",
			total_roi, ROI_MAXIMUM);
		total_roi = ROI_MAXIMUM;
	}
...
int pda_devm_clk_get(struct platform_device *pdev)
{
	...
	g_num_clks = of_count_phandle_with_args(pdev->dev.of_node, "clocks",
			"#clock-cells");
	g_num_clks = (g_num_clks < 0) ? 0 : g_num_clks;
	LOG_INF("clk_num:%d\n", g_num_clks);

	if (g_num_clks) {
		g_clks = devm_kcalloc(&pdev->dev, g_num_clks, sizeof(*g_clks),
					 GFP_KERNEL);
		if (!g_clks)
			return -ENOMEM;
	}
	...
	for (i = 0; i < g_num_clks; i++) {
		g_clks[i] = of_clk_get(pdev->dev.of_node, i);
		LOG_INF("of_clk_get index:%d done\n", i);
		if (IS_ERR(g_clks[i])) {
			LOG_INF("failed to get clk %d\n", i);
			return -ENODEV;
		}
	}
...
void pda_clk_prepare_enable(void)
{
	...
	for (i = 0; i < g_num_clks; i++) {
		...
		if (ret)
			LOG_INF("cannot enable clock index:%d\n", i);
		else
			LOG_INF("clk_prepare_enable index:%d done", i);
	}
}
...
void pda_clk_disable_unprepare(void)
{
	...
	for (i = g_num_clks-1; i >= 0; i--) {
		clk_disable_unprepare(g_clks[i]);

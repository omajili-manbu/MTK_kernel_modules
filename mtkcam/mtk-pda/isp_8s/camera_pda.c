...
#define PDA_CFG_0_DEFAULT 0x40014
...
// ****************** MIPS/MCPS optimization *****************************
unsigned int config_record[2][19];
unsigned int config_ROI_record[24][4];
// ***********************************************************************
...
static void pda_reset_nocheckclk(unsigned int PDA_Index)
{
	...
	LOG_INF("PDA%d reset pda timeout, DMA_RST:0x%x\n", PDA_Index,
		PDA_RD32(PDA_devs[PDA_Index].m_pda_base + PDA_PDA_DMA_RST_REG));
}
...
static inline void PDA_Prepare_Enable_ccf_clock(void)
{
	...
	if (g_PDA_quantity > 0) {
		...
		ret = pm_runtime_get_sync(g_dev1);
		if (ret < 0) {
	...
	if (g_PDA_quantity > 1) {
		/* consumer device starting work*/
		ret = pm_runtime_get_sync(g_dev2);
		if (ret < 0) {
...
static void EnableClock(bool En)
{
	...
	if (En) {			/* Enable clock. */
		...
		case 0:
			...
			//	enable_irq(PDA_devs[i].irq);
			//	LOG_INF("enable irq %d", PDA_devs[i].irq);
			//}
		...
		case 0:
			...
			for (i = 0; i < g_PDA_quantity; i++) {
				...
				//disable_irq(PDA_devs[i].irq);
				//LOG_INF("disable irq %d", PDA_devs[i].irq);
			}
...
static int pda_get_dma_buffer(struct pda_mmu *mmu, int fd)
{
	...
	if (pda_log_dbg_en == 1)
		LOG_INF("fd = %d, dma buffer size = %ld\n",fd , buf->size);
...
static void initHWDMASettings(void)
{
	...
	for (i = 0; i < g_PDA_quantity; i++) {
		...
		// clear AXSLC
		PDA_WR32(PDA_devs[i].m_pda_base + PDA_PDA_P1_AXSLC_REG, 0x0);
		PDA_WR32(PDA_devs[i].m_pda_base + PDA_PDA_P2_AXSLC_REG, 0x0);
		PDA_WR32(PDA_devs[i].m_pda_base + PDA_PDA_P3_AXSLC_REG, 0x0);
		PDA_WR32(PDA_devs[i].m_pda_base + PDA_PDA_P4_AXSLC_REG, 0x0);
		PDA_WR32(PDA_devs[i].m_pda_base + PDA_PDA_P5_AXSLC_REG, 0x0);
		PDA_WR32(PDA_devs[i].m_pda_base + PDA_PDAO_P1_AXSLC_REG, 0x0);

		// ddren set hw mode
		PDA_WR32(PDA_devs[i].m_pda_base + PDA_DDREN_CFG_REG, 0x8);

		PDA_WR32(PDA_devs[i].m_pda_base + PDA_PACK_MODE_REG, 0x0);
		PDA_WR32(PDA_devs[i].m_pda_base + PDA_DILATION_CFG_REG, 0x0);
...
{
	unsigned int i;
	unsigned int cfg_0_value = 0;

	for (i = 0; i < g_PDA_quantity; i++) {
		cfg_0_value = PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_0_REG);
		if (pda_log_dbg_en == 1)
			LOG_INF("config_record[i][0]:0x%x, config 0 value: 0x%x\n",
				config_record[i][0],
				cfg_0_value);

		// --------- Frame setting part -----------
		if (config_record[i][0] != pda_PdaConfig->PDA_HW_Register.PDA_CFG_0.Raw ||
				cfg_0_value == PDA_CFG_0_DEFAULT) {
			if (pda_log_dbg_en == 1)
				LOG_INF("config 0 resetting, before/after: 0x%x/0x%x\n",
					config_record[i][0],
					pda_PdaConfig->PDA_HW_Register.PDA_CFG_0.Raw);
			PDA_WR32(PDA_devs[i].m_pda_base + PDA_CFG_0_REG,
				pda_PdaConfig->PDA_HW_Register.PDA_CFG_0.Raw);
			config_record[i][0] = pda_PdaConfig->PDA_HW_Register.PDA_CFG_0.Raw;
		}
		if (config_record[i][1] != pda_PdaConfig->PDA_HW_Register.PDA_CFG_1.Raw ||
				cfg_0_value == PDA_CFG_0_DEFAULT) {
			if (pda_log_dbg_en == 1)
				LOG_INF("config 1 resetting, before/after: 0x%x/0x%x\n",
					config_record[i][1],
					pda_PdaConfig->PDA_HW_Register.PDA_CFG_1.Raw);
			PDA_WR32(PDA_devs[i].m_pda_base + PDA_CFG_1_REG,
				pda_PdaConfig->PDA_HW_Register.PDA_CFG_1.Raw);
			config_record[i][1] = pda_PdaConfig->PDA_HW_Register.PDA_CFG_1.Raw;
		}
		if (config_record[i][2] != pda_PdaConfig->PDA_HW_Register.PDA_CFG_2.Raw ||
				cfg_0_value == PDA_CFG_0_DEFAULT) {
			if (pda_log_dbg_en == 1)
				LOG_INF("config 2 resetting, before/after: 0x%x/0x%x\n",
					config_record[i][2],
					pda_PdaConfig->PDA_HW_Register.PDA_CFG_2.Raw);
			PDA_WR32(PDA_devs[i].m_pda_base + PDA_CFG_2_REG,
				pda_PdaConfig->PDA_HW_Register.PDA_CFG_2.Raw);
			config_record[i][2] = pda_PdaConfig->PDA_HW_Register.PDA_CFG_2.Raw;
		}
		if (config_record[i][3] != pda_PdaConfig->PDA_HW_Register.PDA_CFG_3.Raw ||
				cfg_0_value == PDA_CFG_0_DEFAULT) {
			if (pda_log_dbg_en == 1)
				LOG_INF("config 3 resetting, before/after: 0x%x/0x%x\n",
					config_record[i][3],
					pda_PdaConfig->PDA_HW_Register.PDA_CFG_3.Raw);
			PDA_WR32(PDA_devs[i].m_pda_base + PDA_CFG_3_REG,
				pda_PdaConfig->PDA_HW_Register.PDA_CFG_3.Raw);
			config_record[i][3] = pda_PdaConfig->PDA_HW_Register.PDA_CFG_3.Raw;
		}
		if (config_record[i][4] != pda_PdaConfig->PDA_HW_Register.PDA_CFG_4.Raw ||
				cfg_0_value == PDA_CFG_0_DEFAULT) {
			if (pda_log_dbg_en == 1)
				LOG_INF("config 4 resetting, before/after: 0x%x/0x%x\n",
					config_record[i][4],
					pda_PdaConfig->PDA_HW_Register.PDA_CFG_4.Raw);
			PDA_WR32(PDA_devs[i].m_pda_base + PDA_CFG_4_REG,
				pda_PdaConfig->PDA_HW_Register.PDA_CFG_4.Raw);
			config_record[i][4] = pda_PdaConfig->PDA_HW_Register.PDA_CFG_4.Raw;
		}
		if (config_record[i][5] != pda_PdaConfig->PDA_HW_Register.PDA_CFG_5.Raw ||
				cfg_0_value == PDA_CFG_0_DEFAULT) {
			if (pda_log_dbg_en == 1)
				LOG_INF("config 5 resetting, before/after: 0x%x/0x%x\n",
					config_record[i][5],
					pda_PdaConfig->PDA_HW_Register.PDA_CFG_5.Raw);
			PDA_WR32(PDA_devs[i].m_pda_base + PDA_CFG_5_REG,
				pda_PdaConfig->PDA_HW_Register.PDA_CFG_5.Raw);
			config_record[i][5] = pda_PdaConfig->PDA_HW_Register.PDA_CFG_5.Raw;
		}
		if (config_record[i][6] != pda_PdaConfig->PDA_HW_Register.PDA_CFG_6.Raw ||
				cfg_0_value == PDA_CFG_0_DEFAULT) {
			if (pda_log_dbg_en == 1)
				LOG_INF("config 6 resetting, before/after: 0x%x/0x%x\n",
					config_record[i][6],
					pda_PdaConfig->PDA_HW_Register.PDA_CFG_6.Raw);
			PDA_WR32(PDA_devs[i].m_pda_base + PDA_CFG_6_REG,
				pda_PdaConfig->PDA_HW_Register.PDA_CFG_6.Raw);
			config_record[i][6] = pda_PdaConfig->PDA_HW_Register.PDA_CFG_6.Raw;
		}
		if (config_record[i][7] != pda_PdaConfig->PDA_HW_Register.PDA_CFG_7.Raw ||
				cfg_0_value == PDA_CFG_0_DEFAULT) {
			if (pda_log_dbg_en == 1)
				LOG_INF("config 7 resetting, before/after: 0x%x/0x%x\n",
					config_record[i][7],
					pda_PdaConfig->PDA_HW_Register.PDA_CFG_7.Raw);
			PDA_WR32(PDA_devs[i].m_pda_base + PDA_CFG_7_REG,
				pda_PdaConfig->PDA_HW_Register.PDA_CFG_7.Raw);
			config_record[i][7] = pda_PdaConfig->PDA_HW_Register.PDA_CFG_7.Raw;
		}
		if (config_record[i][8] != pda_PdaConfig->PDA_HW_Register.PDA_CFG_8.Raw ||
				cfg_0_value == PDA_CFG_0_DEFAULT) {
			if (pda_log_dbg_en == 1)
				LOG_INF("config 8 resetting, before/after: 0x%x/0x%x\n",
					config_record[i][8],
					pda_PdaConfig->PDA_HW_Register.PDA_CFG_8.Raw);
			PDA_WR32(PDA_devs[i].m_pda_base + PDA_CFG_8_REG,
				pda_PdaConfig->PDA_HW_Register.PDA_CFG_8.Raw);
			config_record[i][8] = pda_PdaConfig->PDA_HW_Register.PDA_CFG_8.Raw;
		}
		if (config_record[i][9] != pda_PdaConfig->PDA_HW_Register.PDA_CFG_9.Raw ||
				cfg_0_value == PDA_CFG_0_DEFAULT) {
			if (pda_log_dbg_en == 1)
				LOG_INF("config 9 resetting, before/after: 0x%x/0x%x\n",
					config_record[i][9],
					pda_PdaConfig->PDA_HW_Register.PDA_CFG_9.Raw);
			PDA_WR32(PDA_devs[i].m_pda_base + PDA_CFG_9_REG,
				pda_PdaConfig->PDA_HW_Register.PDA_CFG_9.Raw);
			config_record[i][9] = pda_PdaConfig->PDA_HW_Register.PDA_CFG_9.Raw;
		}
		if (config_record[i][10] != pda_PdaConfig->PDA_HW_Register.PDA_CFG_10.Raw ||
				cfg_0_value == PDA_CFG_0_DEFAULT) {
			if (pda_log_dbg_en == 1)
				LOG_INF("config 10 resetting, before/after: 0x%x/0x%x\n",
					config_record[i][10],
					pda_PdaConfig->PDA_HW_Register.PDA_CFG_10.Raw);
			PDA_WR32(PDA_devs[i].m_pda_base + PDA_CFG_10_REG,
				pda_PdaConfig->PDA_HW_Register.PDA_CFG_10.Raw);
			config_record[i][10] = pda_PdaConfig->PDA_HW_Register.PDA_CFG_10.Raw;
		}
		if (config_record[i][11] != pda_PdaConfig->PDA_HW_Register.PDA_CFG_11.Raw ||
				cfg_0_value == PDA_CFG_0_DEFAULT) {
			if (pda_log_dbg_en == 1)
				LOG_INF("config 11 resetting, before/after: 0x%x/0x%x\n",
					config_record[i][11],
					pda_PdaConfig->PDA_HW_Register.PDA_CFG_11.Raw);
			PDA_WR32(PDA_devs[i].m_pda_base + PDA_CFG_11_REG,
				pda_PdaConfig->PDA_HW_Register.PDA_CFG_11.Raw);
			config_record[i][11] = pda_PdaConfig->PDA_HW_Register.PDA_CFG_11.Raw;
		}
		if (config_record[i][12] != pda_PdaConfig->PDA_HW_Register.PDA_CFG_12.Raw ||
				cfg_0_value == PDA_CFG_0_DEFAULT) {
			if (pda_log_dbg_en == 1)
				LOG_INF("config 12 resetting, before/after: 0x%x/0x%x\n",
					config_record[i][12],
					pda_PdaConfig->PDA_HW_Register.PDA_CFG_12.Raw);
			PDA_WR32(PDA_devs[i].m_pda_base + PDA_CFG_12_REG,
				pda_PdaConfig->PDA_HW_Register.PDA_CFG_12.Raw);
			config_record[i][12] = pda_PdaConfig->PDA_HW_Register.PDA_CFG_12.Raw;
		}
		if (config_record[i][13] != pda_PdaConfig->PDA_HW_Register.PDA_CFG_13.Raw ||
				cfg_0_value == PDA_CFG_0_DEFAULT) {
			if (pda_log_dbg_en == 1)
				LOG_INF("config 13 resetting, before/after: 0x%x/0x%x\n",
					config_record[i][13],
					pda_PdaConfig->PDA_HW_Register.PDA_CFG_13.Raw);
			PDA_WR32(PDA_devs[i].m_pda_base + PDA_CFG_13_REG,
				pda_PdaConfig->PDA_HW_Register.PDA_CFG_13.Raw);
			config_record[i][13] = pda_PdaConfig->PDA_HW_Register.PDA_CFG_13.Raw;
		}
		if (config_record[i][14] != pda_PdaConfig->PDA_HW_Register.PDA_CFG_14.Raw ||
				cfg_0_value == PDA_CFG_0_DEFAULT) {
			if (pda_log_dbg_en == 1)
				LOG_INF("config 14 resetting, before/after: 0x%x/0x%x\n",
					config_record[i][14],
					pda_PdaConfig->PDA_HW_Register.PDA_CFG_14.Raw);
			PDA_WR32(PDA_devs[i].m_pda_base + PDA_CFG_14_REG,
				pda_PdaConfig->PDA_HW_Register.PDA_CFG_14.Raw);
			config_record[i][14] = pda_PdaConfig->PDA_HW_Register.PDA_CFG_14.Raw;
		}
		if (config_record[i][15] != pda_PdaConfig->PDA_HW_Register.PDA_CFG_15.Raw ||
				cfg_0_value == PDA_CFG_0_DEFAULT) {
			if (pda_log_dbg_en == 1)
				LOG_INF("config 15 resetting, before/after: 0x%x/0x%x\n",
					config_record[i][15],
					pda_PdaConfig->PDA_HW_Register.PDA_CFG_15.Raw);
			PDA_WR32(PDA_devs[i].m_pda_base + PDA_CFG_15_REG,
				pda_PdaConfig->PDA_HW_Register.PDA_CFG_15.Raw);
			config_record[i][15] = pda_PdaConfig->PDA_HW_Register.PDA_CFG_15.Raw;
		}
		if (config_record[i][16] != pda_PdaConfig->PDA_HW_Register.PDA_CFG_16.Raw ||
				cfg_0_value == PDA_CFG_0_DEFAULT) {
			if (pda_log_dbg_en == 1)
				LOG_INF("config 16 resetting, before/after: 0x%x/0x%x\n",
					config_record[i][16],
					pda_PdaConfig->PDA_HW_Register.PDA_CFG_16.Raw);
			PDA_WR32(PDA_devs[i].m_pda_base + PDA_CFG_16_REG,
				pda_PdaConfig->PDA_HW_Register.PDA_CFG_16.Raw);
			config_record[i][16] = pda_PdaConfig->PDA_HW_Register.PDA_CFG_16.Raw;
		}
		if (config_record[i][17] != pda_PdaConfig->PDA_HW_Register.PDA_CFG_17.Raw ||
				cfg_0_value == PDA_CFG_0_DEFAULT) {
			if (pda_log_dbg_en == 1)
				LOG_INF("config 17 resetting, before/after: 0x%x/0x%x\n",
					config_record[i][17],
					pda_PdaConfig->PDA_HW_Register.PDA_CFG_17.Raw);
			PDA_WR32(PDA_devs[i].m_pda_base + PDA_CFG_17_REG,
				pda_PdaConfig->PDA_HW_Register.PDA_CFG_17.Raw);
			config_record[i][17] = pda_PdaConfig->PDA_HW_Register.PDA_CFG_17.Raw;
		}
		if (config_record[i][18] != pda_PdaConfig->PDA_HW_Register.PDA_CFG_18.Raw ||
				cfg_0_value == PDA_CFG_0_DEFAULT) {
			if (pda_log_dbg_en == 1)
				LOG_INF("config 18 resetting, before/after: 0x%x/0x%x\n",
					config_record[i][18],
					pda_PdaConfig->PDA_HW_Register.PDA_CFG_18.Raw);
			PDA_WR32(PDA_devs[i].m_pda_base + PDA_CFG_18_REG,
				pda_PdaConfig->PDA_HW_Register.PDA_CFG_18.Raw);
			config_record[i][18] = pda_PdaConfig->PDA_HW_Register.PDA_CFG_18.Raw;
		}
				...
				unsigned int ROIIndex)
{
	...
	for (i = 0; i < RoiProcNum; i++) {
		...
		if (pda_log_dbg_en == 1)
			pr_info("ROI:%d, x/y/w/h/iw/obx/oby/nbx/nby:%d/%d/%d/%d/%d/%d/%d/%d/%d\n",
				i, g_rgn_x_buf[i], g_rgn_y_buf[i], g_rgn_w_buf[i], g_rgn_h_buf[i], g_rgn_iw_buf[i],
				g_rgn_obx_buf[i], g_rgn_oby_buf[i], g_rgn_nbx_buf[i], g_rgn_nby_buf[i]);
	}
				...
				unsigned int PDA_Index)
{
	...
	for (i = 0; i <= ROI_MAX_INDEX; i++) {
		...
		xy_value = PDA_RD32((PDA_devs[PDA_Index].m_pda_base + 0x004*(RegIndex)));
		if (pda_log_dbg_en == 1)
			LOG_INF("xy_value = 0x%x\n", xy_value);

		if (config_ROI_record[i][0] != ((g_rgn_y_buf[i] << 16) + g_rgn_x_buf[i]) ||
				xy_value == 0x0) {
			config_ROI_record[i][0] = (g_rgn_y_buf[i] << 16) + g_rgn_x_buf[i];
			PDA_WR32((PDA_devs[PDA_Index].m_pda_base + 0x004*(RegIndex)),
				config_ROI_record[i][0]);
		}
		if (pda_log_dbg_en == 1)
			LOG_INF("config_ROI_record[%d][0] = 0x%x, PDA_CFG[%d]: 0x%x\n",
				i, config_ROI_record[i][0],
				RegIndex, PDA_RD32(PDA_devs[PDA_Index].m_pda_base + 0x004*RegIndex));
		RegIndex++;

		if (config_ROI_record[i][1] != ((g_rgn_h_buf[i] << 16) + g_rgn_w_buf[i]) ||
				xy_value == 0x0) {
			config_ROI_record[i][1] = (g_rgn_h_buf[i] << 16) + g_rgn_w_buf[i];
			PDA_WR32((PDA_devs[PDA_Index].m_pda_base + 0x004*(RegIndex)),
				config_ROI_record[i][1]);
		}
		if (pda_log_dbg_en == 1)
			LOG_INF("config_ROI_record[%d][1] = 0x%x, PDA_CFG[%d]: 0x%x\n",
				i, config_ROI_record[i][1],
				RegIndex, PDA_RD32(PDA_devs[PDA_Index].m_pda_base + 0x004*RegIndex));
		RegIndex++;


		if (config_ROI_record[i][2] != ((g_rgn_nby_buf[i] << 22) +
				(g_rgn_nbx_buf[i] << 16) + g_rgn_iw_buf[i]) || xy_value == 0x0) {
			config_ROI_record[i][2] = (g_rgn_nby_buf[i] << 22) +
				(g_rgn_nbx_buf[i] << 16) + g_rgn_iw_buf[i];
			PDA_WR32((PDA_devs[PDA_Index].m_pda_base + 0x004*(RegIndex)),
				config_ROI_record[i][2]);
		}
		if (pda_log_dbg_en == 1)
			LOG_INF("config_ROI_record[%d][2] = 0x%x, PDA_CFG[%d]: 0x%x\n",
				i, config_ROI_record[i][2],
				RegIndex, PDA_RD32(PDA_devs[PDA_Index].m_pda_base + 0x004*RegIndex));
		RegIndex++;


		if (config_ROI_record[i][3] != ((g_rgn_oby_buf[i] << 16) + g_rgn_obx_buf[i]) ||
				xy_value == 0x0) {
			config_ROI_record[i][3] = (g_rgn_oby_buf[i] << 16) + g_rgn_obx_buf[i];
			PDA_WR32((PDA_devs[PDA_Index].m_pda_base + 0x004*(RegIndex)),
				config_ROI_record[i][3]);
		}
		if (pda_log_dbg_en == 1)
			LOG_INF("config_ROI_record[%d][3] = 0x%x, PDA_CFG[%d]: 0x%x\n",
				i, config_ROI_record[i][3],
				RegIndex, PDA_RD32(PDA_devs[PDA_Index].m_pda_base + 0x004*RegIndex));
		RegIndex++;
...
static void LOGHWRegister(unsigned int i)
{
	...
	/* cfg from ALGO */
	LOG_INF("from ALGO, CFG_0/1/2/3/4/5/6: 0x%x/0x%x/0x%x/0x%x/0x%x/0x%x/0x%x\n",
		g_pda_Pdadata.PDA_HW_Register.PDA_CFG_0.Raw,
		g_pda_Pdadata.PDA_HW_Register.PDA_CFG_1.Raw,
		g_pda_Pdadata.PDA_HW_Register.PDA_CFG_2.Raw,
		g_pda_Pdadata.PDA_HW_Register.PDA_CFG_3.Raw,
		g_pda_Pdadata.PDA_HW_Register.PDA_CFG_4.Raw,
		g_pda_Pdadata.PDA_HW_Register.PDA_CFG_5.Raw,
		g_pda_Pdadata.PDA_HW_Register.PDA_CFG_6.Raw);
	...
	/* cfg from ALGO */
	LOG_INF("from ALGO, CFG_7/8/9/10/11/12/13: 0x%x/0x%x/0x%x/0x%x/0x%x/0x%x/0x%x\n",
		g_pda_Pdadata.PDA_HW_Register.PDA_CFG_7.Raw,
		g_pda_Pdadata.PDA_HW_Register.PDA_CFG_8.Raw,
		g_pda_Pdadata.PDA_HW_Register.PDA_CFG_9.Raw,
		g_pda_Pdadata.PDA_HW_Register.PDA_CFG_10.Raw,
		g_pda_Pdadata.PDA_HW_Register.PDA_CFG_11.Raw,
		g_pda_Pdadata.PDA_HW_Register.PDA_CFG_12.Raw,
		g_pda_Pdadata.PDA_HW_Register.PDA_CFG_13.Raw);

	LOG_INF("CFG_14/15/16/17/18: 0x%x/0x%x/0x%x/0x%x/0x%x\n",
	...
	/* cfg from ALGO */
	LOG_INF("from ALGO, CFG_14/15/16/17/18: 0x%x/0x%x/0x%x/0x%x/0x%x\n",
		g_pda_Pdadata.PDA_HW_Register.PDA_CFG_14.Raw,
		g_pda_Pdadata.PDA_HW_Register.PDA_CFG_15.Raw,
		g_pda_Pdadata.PDA_HW_Register.PDA_CFG_16.Raw,
		g_pda_Pdadata.PDA_HW_Register.PDA_CFG_17.Raw,
		g_pda_Pdadata.PDA_HW_Register.PDA_CFG_18.Raw);

	LOG_INF("ROI 0,1, CFG_19~26: 0x%x/0x%x/0x%x/0x%x/0x%x/0x%x/0x%x/0x%x\n",
	...
	LOG_INF("ROI 2,3, CFG_27~34: 0x%x/0x%x/0x%x/0x%x/0x%x/0x%x/0x%x/0x%x\n",
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_27_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_28_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_29_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_30_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_31_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_32_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_33_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_34_REG));
	LOG_INF("ROI 4,5, CFG_35~42: 0x%x/0x%x/0x%x/0x%x/0x%x/0x%x/0x%x/0x%x\n",
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_35_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_36_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_37_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_38_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_39_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_40_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_41_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_42_REG));
	LOG_INF("ROI 6,7, CFG_43~50: 0x%x/0x%x/0x%x/0x%x/0x%x/0x%x/0x%x/0x%x\n",
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_43_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_44_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_45_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_46_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_47_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_48_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_49_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_50_REG));
	LOG_INF("ROI 8,9, CFG_51~58: 0x%x/0x%x/0x%x/0x%x/0x%x/0x%x/0x%x/0x%x\n",
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_51_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_52_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_53_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_54_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_55_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_56_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_57_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_58_REG));
	LOG_INF("ROI 10,11, CFG_59~66: 0x%x/0x%x/0x%x/0x%x/0x%x/0x%x/0x%x/0x%x\n",
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_59_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_60_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_61_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_62_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_63_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_64_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_65_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_66_REG));
	LOG_INF("ROI 12,13, CFG_67~74: 0x%x/0x%x/0x%x/0x%x/0x%x/0x%x/0x%x/0x%x\n",
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_67_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_68_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_69_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_70_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_71_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_72_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_73_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_74_REG));
	LOG_INF("ROI 14,15, CFG_75~82: 0x%x/0x%x/0x%x/0x%x/0x%x/0x%x/0x%x/0x%x\n",
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_75_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_76_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_77_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_78_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_79_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_80_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_81_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_82_REG));
	LOG_INF("ROI 16,17, CFG_83~90: 0x%x/0x%x/0x%x/0x%x/0x%x/0x%x/0x%x/0x%x\n",
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_83_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_84_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_85_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_86_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_87_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_88_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_89_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_90_REG));
	LOG_INF("ROI 18,19, CFG_91~98: 0x%x/0x%x/0x%x/0x%x/0x%x/0x%x/0x%x/0x%x\n",
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_91_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_92_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_93_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_94_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_95_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_96_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_97_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_98_REG));
	LOG_INF("ROI 20,21, CFG_99~106: 0x%x/0x%x/0x%x/0x%x/0x%x/0x%x/0x%x/0x%x\n",
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_99_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_100_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_101_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_102_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_103_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_104_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_105_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_106_REG));
	LOG_INF("ROI 22,23, CFG_107~114: 0x%x/0x%x/0x%x/0x%x/0x%x/0x%x/0x%x/0x%x\n",
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_107_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_108_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_109_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_110_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_111_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_112_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_113_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_CFG_114_REG));

	for (int i = 0; i < PDA_MAXROI_PER_ROUND; i++) {
		LOG_INF("fd from ALGO, roi%d CFG: 0x%x/0x%x/0x%x/0x%x\n",
			i, (g_pda_Pdadata.roi_y[i] << 16) + g_pda_Pdadata.roi_x[i],
			(g_pda_Pdadata.roi_h[i] << 16) + g_pda_Pdadata.roi_w[i],
			(g_pda_Pdadata.roi_nby[i] << 22) + (g_pda_Pdadata.roi_nbx[i] << 16) +
			g_pda_Pdadata.roi_iw[i],
			(g_pda_Pdadata.roi_oby[i] << 16) + g_pda_Pdadata.roi_obx[i]);
	}

	LOG_INF("fd from ALGO, fd_left_image/fd_left_table/fd_output: %d/%d/%d\n",
		g_pda_Pdadata.fd_left_image[0],
		g_pda_Pdadata.fd_left_table,
	...
	LOG_INF("PACK/DILATION: 0x%x/0x%x\n",
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_PACK_MODE_REG),
		PDA_RD32(PDA_devs[i].m_pda_base + PDA_DILATION_CFG_REG));
}
			...
			unsigned long a_u4Param)
{
	...
	case PDA_ENQUE_WAITIRQ:
		...
		if (pda_log_dbg_en == 1)
			LOG_INF("PDA size image/table/output: %d/%d/%d\n",
				g_pda_Pdadata.image_size,
				g_pda_Pdadata.table_size,
				g_pda_Pdadata.output_size);
		...
		/* Check if the image buffer size is enough, if not return error */
		if (g_image_mmu[cur_img_idx].dma_buf->size < (2*g_pda_Pdadata.image_size)) {
			LOG_INF("Error: image buffer size less than required(2*image size), %ld/%d\n",
				g_image_mmu[cur_img_idx].dma_buf->size, 2*g_pda_Pdadata.image_size);
			g_pda_Pdadata.status = -33;
			goto EXIT;
		}
		...
		/* Check if the table buffer size is enough, if not return error */
		if (g_table_mmu[cur_tbl_idx].dma_buf->size < (2*g_pda_Pdadata.table_size)) {
			LOG_INF("Error: table buffer size less than required(2*table size), %ld/%d\n",
				g_table_mmu[cur_tbl_idx].dma_buf->size, 2*g_pda_Pdadata.table_size);
			g_pda_Pdadata.status = -34;
			goto EXIT;
		}
		...
		/* Check if the output buffer size is enough, if not return error */
		if (g_output_mmu[cur_out_idx].dma_buf->size < g_pda_Pdadata.output_size) {
			LOG_INF("Error: outpout buffer size less than required(output size), %ld/%d\n",
				g_output_mmu[cur_out_idx].dma_buf->size, g_pda_Pdadata.output_size);
			g_pda_Pdadata.status = -35;
			goto EXIT;
		}
...
int camera_pda_translation_fault_callback(int port, dma_addr_t mva, void *data)
{
...
static int PDA_probe(struct platform_device *pdev)
{
	...
	struct device_node *node;
	int i, larbs;
	struct device_node *vcore_node;
	struct platform_device *vcore_pdev;
	struct device_link *vcore_link;
	int smmus;
	...
	// device link with camisp_vcore
	vcore_node = of_parse_phandle(pdev->dev.of_node, "mediatek,camisp-vcore", 0);
	if (!vcore_node) {
		LOG_INF("failed to get camisp vcore phandle\n");
		/* return -ENODEV; */
	} else {
		vcore_pdev = of_find_device_by_node(vcore_node);
		if (WARN_ON(!vcore_pdev)) {
			of_node_put(vcore_node);
			LOG_INF("failed to get camisp vcore pdev\n");
			return -ENODEV;
		}
		of_node_put(vcore_node);

		vcore_link = device_link_add(&pdev->dev, &vcore_pdev->dev,
						DL_FLAG_PM_RUNTIME | DL_FLAG_STATELESS);
		if (!vcore_link)
			LOG_INF("unable to link vcore\n");

		LOG_INF("vcore pdev %p\n", pdev);
	}
	...
	if (PDA_devs[0].irq > 0) {
		if (PDA_devs[0].irq > 0 && g_PDA_quantity > 0) {
			...
			nRet = request_irq(PDA_devs[0].irq,
					...
					g_dev1);
			if (nRet) {
				LOG_INF("PDA1 request_irq irq 1st Fail: %d\n", nRet);
				return nRet;
			}
			LOG_INF("PDA1 request_irq irq 1st PASS: %d\n", nRet);

			//disable_irq(PDA_devs[0].irq);
			//LOG_INF("disable irq %d", PDA_devs[0].irq);
		} else {
	...
	for (i = 0; i < smmus; i++) {
		...
		if (!of_property_read_u32_index(
			...
			mtk_iommu_register_fault_callback(
				axid, camera_pda_translation_fault_callback,
				NULL, false);
		}
	}
	...
	if (PDA_devs[1].irq > 0) {
		if (PDA_devs[1].irq > 0 && g_PDA_quantity > 1) {
			...
			nRet = request_irq(PDA_devs[1].irq,
					...
					g_dev2);
			if (nRet) {
				LOG_INF("PDA2 request_irq irq 2nd Fail: %d\n", nRet);
				return nRet;
			}
			LOG_INF("PDA2 request_irq irq 2nd PASS: %d\n", nRet);

			//disable_irq(PDA_devs[1].irq);
			//LOG_INF("disable irq %d", PDA_devs[1].irq);
		} else {

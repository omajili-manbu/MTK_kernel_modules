...
#ifdef __XIAOMI_CAMERA__
	if(unlikely(mi_i2c_dump))
		dev_info(&i2c_client->dev, "[%s] [addr] = 0x%02x,[reg] = 0x%04x,[val] = 0x%04x", __func__, addr, reg, val);
#endif
...
#ifdef __XIAOMI_CAMERA__
            if(unlikely(mi_i2c_dump))
                dev_info(dev, "[%s] [addr] = 0x%02x,[reg] = 0x%04x,[val] = 0x%04x", __func__, addr, plist[0], plist[1]);
#endif
			plist += 2;
			pbuf += 4;
			pmsg++;

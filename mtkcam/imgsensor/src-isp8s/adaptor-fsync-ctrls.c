	...
	const u64 *ae_exp_arr, const u32 ae_exp_cnt)
{
	...
	for (i = 0; i < ae_exp_cnt; ++i) {
		...
		if ((exp_lc + ctx->subctx.margin) >=
				ctx->subctx.sw_max_frame_length) {
			has_long_exp = 1;
			break;
		}
	}
	...
	struct fs_streaming_st *s_info)
{
	...
	s_info->max_fl_lc = ctx->subctx.sw_max_frame_length;
...
{
	FSYNC_MGR_LOGD(ctx,
		"[%s] sidx:%d, seamless switch, fdelay:%u/max_fl_lc:%u, prop:(type_id:%u, orig_readout_time_us:%u, ctrl_receive_time_us:%u, hw_re_init_time_us:%u, prsh_length_lc:%u)\n",
		caller,
		ctx->idx,
		seamless_info->fl_active_delay,
		seamless_info->max_fl_lc,
	...
	u64 *ae_exp_arr, u32 orig_scenario_id, u32 target_scenario_id)
{
	...
	seamless_info.max_fl_lc = ctx->subctx.sw_max_frame_length;

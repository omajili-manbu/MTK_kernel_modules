...
#ifdef __XIAOMI_CAMERA__
static void set_reg_vs2_2(struct adaptor_ctx *ctx)
{
...
/* get mt6661-6 regmap */
	np = of_find_node_by_name(NULL, "mt6661-6");
	if (!np) {
		adaptor_logd(ctx, "mt6661-6\n");
		return;
	}
	...
	if (vs2_2_refcnt < 4){
		mutex_lock(&ctx->vs2_2_lock);
		vs2_2_refcnt++;
		mutex_unlock(&ctx->vs2_2_lock);
	}


	regmap_update_bits(map, 0x180b,0x1,0x1);//keep vs2_2 1.35

	adaptor_logi(ctx, "set_reg_vs2_2 1.35v vs2_2_refcnt(%d)\n",vs2_2_refcnt);

	return;
}

static void unset_reg_vs2_2(struct adaptor_ctx *ctx)
{
	struct device_node *np;
	struct platform_device *pmic_pdev = NULL;
	struct regmap *map;
...
/* get pmic6661 regmap */
	...
	if (vs2_2_refcnt < 1) {
		regmap_update_bits(map, 0x180c,0x1,0x1);//disable keep vs2_2 1.35
		vs2_2_refcnt = 0;
	}

	adaptor_logi(ctx, "set_reg_vs2_2 1.265v,vs2_2_refcnt(%d)\n",vs2_2_refcnt);
	return;
}

static void disable_mt6661cp_ldc3(void)
{
	struct device_node *np;
	struct platform_device *pmic_pdev = NULL;
	struct regmap *map;
...
/* get mt6661-5 regmap */
	np = of_find_node_by_name(NULL, "mt6661-5");
	if (!np) {
		printk("mt6661-5\n");
		return;
	}
	...
	regmap_update_bits(map, 0x1f09,0x1,0x1);

	pr_info("disable_6661CP_LDC3\n");
...
static int set_reg(struct adaptor_ctx *ctx, void *data, const struct subdrv_pw_val *val)
{
...
#ifdef __XIAOMI_CAMERA__
	if((ctx->subctx.s_ctx.regulator_oc_notice > 0) && (idx == HW_ID_DOVDD))
	{
		/* register oc notification for this regulator */
		if (ctx->subctx.s_ctx.regulator_oc_notice == 1) {
			ctx->subctx.s_ctx.oc_handle.notifier_call = regulator_oc_event;
			ret = devm_regulator_register_notifier(reg, &ctx->subctx.s_ctx.oc_handle);
			if (ret)
				adaptor_logi(ctx, "regulator notifier request failed\n");

			ctx->subctx.s_ctx.regulator_oc_notice = 0;

		} else {
			ctx->subctx.s_ctx.regulator_oc_notice--;
		}
		pr_info(" ctx->subctx.s_ctx.regulator_oc_notice(%d) ",ctx->subctx.s_ctx.regulator_oc_notice);
	}
#endif
	...
	adaptor_logd(ctx, "+ idx(%llu),val_min-max(%d-%d)\n", idx, min_v, max_v);
...
static int unset_reg(struct adaptor_ctx *ctx, void *data, const struct subdrv_pw_val *val)
{
	...
	if (!ctx->regulator[idx] && IS_ERR(ctx->regulator[idx])) {
		ctx->regulator[idx] = NULL;
		adaptor_loge(ctx,"regulator at index %llu is Invalid regulator pointer\n", idx);
		WRAP_AEE_EXCEPTION("unset_reg", "Err: Invalid regulator pointer");
		return -EINVAL;
	}
	...
	if (regulator_is_enabled(reg)) {
		...
		adaptor_loge(ctx,"regulator at index %llu is not enable\n", idx);
		ret = -EINVAL;
		goto exit_unset_reg;
	}
...
int adaptor_cam_pmic_on(struct adaptor_ctx *ctx)
{
	...
	ctx->pmic_on = 1;
	adaptor_logd(ctx, "pmic_wake_en = true\n");
...
int adaptor_pmic_ctrl(struct adaptor_ctx *ctx, bool bPmicEnable)
{
	...
	adaptor_logd(ctx, "[%s]+ bPmicEnable:%d, pmic_enable_cnt:%d ctx_pmic_on:%d+\n",
	...
	if (bPmicEnable) {
		if (pmic_enable_cnt == 0) {
			if ((atomic_read(&pmic_wake_en) && (ctx->pmic_on == 1))) {
				...
				adaptor_logd(ctx, "maximum delay:power_refcnt:%d pmic_delayus:%llu\n",
...
int adaptor_hw_power_on(struct adaptor_ctx *ctx)
{
	...
	//There are scenarios where aov frequently interswitches with normal camera, and it is not suitable to start a new thread for poweroff

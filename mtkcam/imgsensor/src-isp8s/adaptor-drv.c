...
static int imgsensor_probe(struct i3c_i2c_device *client)
{
	...
	mutex_init(&ctx->subctx.fast_mode_lock);
...
#else
	hrtimer_setup(&ctx->streamon_hrtimer, &trigger_streamon_work,
...
free_mutex:
	...
	mutex_destroy(&ctx->seamless_ts_info.seamless_switch_ts_mutex);
	mutex_destroy(&ctx->open_cnt_mutex);

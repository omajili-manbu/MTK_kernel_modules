...
static void dump_perframe_info(struct adaptor_ctx *ctx, struct mtk_hdr_ae *ae_ctrl)
{
	...
	adaptor_logi(ctx,
		...
		ae_ctrl->exposure.le_exposure*line_time_ns,
		ae_ctrl->exposure.me_exposure,
		ae_ctrl->exposure.me_exposure*line_time_ns,
		ae_ctrl->exposure.se_exposure,
		ae_ctrl->exposure.se_exposure*line_time_ns,
...
static int set_test_pattern_data_apply_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	if (data->Channel_R || data->Channel_Gr || data->Channel_Gb || data->Channel_B) {
		...
		adaptor_logd(ctx, "apply val=[R:%u,Gr:%u,Gb:%u,B:%u]\n",
				data->Channel_R,
				data->Channel_Gr,
				data->Channel_Gb,
				data->Channel_B);
...
#ifdef __XIAOMI_CAMERA__
static void get_dispatch_gain(struct adaptor_ctx *ctx, u32 tgain, u32 *again, u32 *dgain,u32 imgsensor_exposure)
{
	int i;
	u32 ag = tgain;
	u32 dg = 0;
	...
	if(ctx->subctx.s_ctx.mode[ctx->subctx.current_scenario_id].ana_gain_max){
		max_again = ctx->subctx.s_ctx.mode[ctx->subctx.current_scenario_id].ana_gain_max;
	} else {
		max_again = ctx->subctx.s_ctx.mode[ctx->subctx.current_scenario_id].multi_exposure_ana_gain_range[imgsensor_exposure].max;
	}
	...
	adaptor_logm(ctx, "again tlb cnt = %u sz(%u), gain(t/a/d) = %u / %u / %u, max_again(%d)\n",
		 ana_gain_table_cnt, ana_gain_table_size, tgain, ag, dg, max_again);
}
#else
...
#ifdef __XIAOMI_CAMERA__
	get_dispatch_gain(ctx, info->le_gain, again_exp, dgain_exp,IMGSENSOR_EXPOSURE_LE);
	get_dispatch_gain(ctx, info->me_gain, again_exp + 1, dgain_exp + 1,IMGSENSOR_EXPOSURE_ME);
	get_dispatch_gain(ctx, info->se_gain, again_exp + 2, dgain_exp + 2,IMGSENSOR_EXPOSURE_SE);
#else
...
static int set_hdr_gain_dual(struct adaptor_ctx *ctx, struct mtk_hdr_gain *info)
{
	...
	if (ctx->subctx.s_ctx.mode[ctx->cur_mode->id].hdr_mode == HDR_RAW_DCG_RAW || ctx->subctx.s_ctx.mode[ctx->cur_mode->id].hdr_mode == HDR_RAW_DCG_COMPOSE) {
		if(info->me_gain/info->le_gain > (ctx->subctx.s_ctx.mi_dxg_ratio/1024)){
	...
	adaptor_logd(ctx,"X! LE %d ,ME %d,mi_dxg_ratio %d",info->le_gain,info->me_gain,ctx->subctx.s_ctx.mi_dxg_ratio);
#endif
...
#ifdef __XIAOMI_CAMERA__
		get_dispatch_gain(ctx, dcg_gain, again_exp, dgain_exp,IMGSENSOR_EXPOSURE_LE);
#else
...
#ifdef __XIAOMI_CAMERA__
		get_dispatch_gain(ctx, ae_ctrl->gain.le_gain, again_exp, dgain_exp,IMGSENSOR_EXPOSURE_LE);
#else
	...
	while (i < SENSOR_SCENARIO_ID_MAX) {
		...
		if (!ret) {
			for (j = 0; write_to < MTK_FRAME_DESC_ENTRY_MAX && j < fd_tmp.num_entries;
				...
				adaptor_logd(ctx, "scenario %u desc %d/0x%x/%d/%d/%d\n",
						scenario_id,
...
static int ext_ctrl(struct adaptor_ctx *ctx, struct v4l2_ctrl *ctrl, struct sensor_mode *mode)
{
	...
	case V4L2_CID_MTK_STANDBY_MODE_QUERY:
		...
		break;
	case V4L2_CID_MTK_SEAMLESS_DELAY_FRAME:
		ctrl->val = ctx->subctx.s_ctx.seamless_switch_with_gph_delay;
		break;
	default:
		break;
	}
...
#ifdef __XIAOMI_CAMERA__
		adaptor_logd(ctx,
			"[scenario %d]:fps: %d/%d vb: %d hb: %d pixelrate: %u cust_pixel_rate: %d, w %d, h %d\n",
...
static int imgsensor_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_START_SEAMLESS_SWITCH:
		{
			...
			/* init fsync_exp */
			init_sensor_exp_by_ae_exp(ctx, info->target_scenario_id,
						  fsync_exp, IMGSENSOR_STAGGER_EXPOSURE_CNT,
						  info->ae_ctrl[0].exposure.arr,
						  IMGSENSOR_STAGGER_EXPOSURE_CNT);

			adaptor_logd(ctx, "fsync_exp[]={%llu,%llu,%llu,%llu,%llu}\n",
				     fsync_exp[0], fsync_exp[1], fsync_exp[2],
				     fsync_exp[3],fsync_exp[4]);

			clear_seamless_switch_ts_info(ctx);

			subdrv_call(ctx, disable_fast_mode);
	...
	case V4L2_CID_MTK_WAKEUP_CAMERA_PMIC:
		adaptor_logd(ctx, "V4L2_CID_MTK_WAKEUP_CAMERA_PMIC val = %d\n", ctrl->val);
		if (ctrl->val)
...
static const struct v4l2_ctrl_config cfg_mtkcam_seamless_delay_frame = {
	.ops = &ctrl_ops,
	.id = V4L2_CID_MTK_SEAMLESS_DELAY_FRAME,
	.name = "seamless_delay_frame",
	.type = V4L2_CTRL_TYPE_INTEGER,
	.flags = V4L2_CTRL_FLAG_VOLATILE,
	.max = 0xffff,
	.step = 1,
...
#ifdef __XIAOMI_CAMERA__
	queue_work(system_highpri_wq, &ctx->ctrl_work);
	if(!parallel_setting_enable || ctx->subctx.aov_sensor_support){
		flush_work(&ctx->ctrl_work);
	}
#else
...
int adaptor_init_ctrls(struct adaptor_ctx *ctx)
{
	...
	v4l2_ctrl_new_custom(&ctx->ctrls, &cfg_mtkcam_seamless_delay_frame, NULL);

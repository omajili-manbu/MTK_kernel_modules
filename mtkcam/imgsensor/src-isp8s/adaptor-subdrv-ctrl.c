...
static int mi_i2c_table_write(struct subdrv_ctx *ctx, u16 *list, u32 len)
{
	int i = 0, ret = 0;
...
void set_i2c_buffer(struct subdrv_ctx *ctx, u16 reg, u16 val)
{
	...
	if(unlikely(mi_i2c_dump))
		DRV_LOG(ctx, "[%s] [addr] = 0x%02x,[reg] = 0x%04x,[val] = 0x%02x", __func__, pmsg->addr, reg, val);
#endif
...
void check_write_frame_length_need_lshift(struct subdrv_ctx *ctx, u32 *fll)
{
	...
	u32 frame_length_max_without_lshift = ctx->s_ctx.frame_length_max_without_lshift ?
		ctx->s_ctx.frame_length_max_without_lshift : ctx->sw_max_frame_length;
...
void write_frame_length(struct subdrv_ctx *ctx, u32 fll)
{
	...
	if (ctx->s_ctx.mode[ctx->current_scenario_id].hdr_mode == HDR_RAW_STAGGER) {
		DRV_LOG_MUST(ctx, "dol skip write_frame_length\n");
		return;
	}
#endif
...
void set_frame_length(struct subdrv_ctx *ctx, u32 frame_length)
{
	bool gph = !ctx->is_seamless && (ctx->s_ctx.s_gph != NULL);
	...
	ctx->frame_length = min(ctx->frame_length, ctx->sw_max_frame_length);
	...
	u32 frame_length, u32 *frame_length_in_lut)
{
	...
	if (exp_cnt) {
		for (i = 0; i < exp_cnt; i++) {
			...
			ctx->frame_length_in_lut[i] =
				min(ctx->frame_length_in_lut[i], ctx->sw_max_frame_length);
		}
	} else {
...
void set_max_framerate(struct subdrv_ctx *ctx, u16 framerate, bool min_framelength_en)
{
	...
	if (ctx->s_ctx.mode[ctx->current_scenario_id].hdr_mode == HDR_RAW_LBMF) {
		...
		case 2:
			ctx->frame_length_in_lut[0] =
				max(ctx->frame_length_in_lut[0],
					...
					ctx->s_ctx.mode[ctx->current_scenario_id].read_margin);
			ctx->frame_length_in_lut[0] =
				min(ctx->frame_length_in_lut[0], ctx->sw_max_frame_length);
			ctx->frame_length_in_lut[1] =
			...
			ctx->frame_length_in_lut[1] =
				min(ctx->frame_length_in_lut[1], ctx->sw_max_frame_length);
		...
		case 3:
			...
			ctx->frame_length_in_lut[2] =
				min(ctx->frame_length_in_lut[2], ctx->sw_max_frame_length);
			ctx->frame_length_in_lut[3] = 0;
			ctx->frame_length_in_lut[4] = 0;
			/* update framelength */
			ctx->frame_length =
		...
		enum SENSOR_SCENARIO_ID_ENUM scenario_id, u32 framerate)
{
	...
	frame_length_max = ctx->sw_max_frame_length;
	...
	enum SENSOR_SCENARIO_ID_ENUM scenario_id, u32 framerate)
{
	...
	if (scenario_id != ctx->current_scenario_id) {
		memset(ctx->frame_length_in_lut, 0, sizeof(ctx->frame_length_in_lut));
		memset(ctx->frame_length_pre_store_in_lut, 0, sizeof(ctx->frame_length_pre_store_in_lut));
	}
	...
	case 2:
		...
		if (ctx->s_ctx.mode[scenario_id].delay_frame == 2) {
			...
			ctx->frame_length_in_lut[0] =
				min(calc_fl_in_lut[0], ctx->sw_max_frame_length);
			ctx->frame_length_in_lut[0] = frame_length_step ?
				roundup(ctx->frame_length_in_lut[0], frame_length_step) :
				ctx->frame_length_in_lut[0];
			...
			ctx->frame_length_in_lut[1] =
				min(calc_fl_in_lut[1], ctx->sw_max_frame_length);
			ctx->frame_length_in_lut[1] = frame_length_step ?
				roundup(ctx->frame_length_in_lut[1], frame_length_step) :
				ctx->frame_length_in_lut[1];
			ctx->frame_length_in_lut[2] = 0;
	...
	case 3:
		...
		if (ctx->s_ctx.mode[scenario_id].delay_frame == 2) {
			...
			ctx->frame_length_in_lut[2] =
				min(calc_fl_in_lut[2], ctx->sw_max_frame_length);
			ctx->frame_length_in_lut[2] = frame_length_step ?
				roundup(ctx->frame_length_in_lut[2], frame_length_step) :
				ctx->frame_length_in_lut[2];
			ctx->frame_length_in_lut[3] = 0;
...
#ifdef __XIAOMI_CAMERA__
	if (ctx->extraVB) {
		ctx->frame_length += ctx->extraVB * (ctx->pclk / 1000)  / ctx->line_length;
		DRV_LOG(ctx, "ctx->exposure[0] = %d, framelength = %d, extraVB(%d)", ctx->exposure[0], ctx->frame_length, ctx->extraVB);
	}
#endif
	/* write framelength */
...
void set_multi_shutter_frame_length(struct subdrv_ctx *ctx,
		u64 *shutters, u16 exp_cnt,	u32 frame_length)
{
	int i = 0;
	int fine_integ_line = 0;
	u16 last_exp_cnt = 1;
	...
	fll =	min(fll, ctx->sw_max_frame_length);
	/* restore shutter */
	memset(ctx->exposure, 0, sizeof(ctx->exposure));
	for (i = 0; i < exp_cnt; i++)
...
#ifdef __XIAOMI_CAMERA__
	DRV_LOG_MUST(ctx, "skip extend_frame_length\n");
	return;
#endif
...
void subdrv_ctx_init(struct subdrv_ctx *ctx)
{
	...
	ctx->sw_max_frame_length = ctx->s_ctx.frame_length_max;
...
void sensor_init(struct subdrv_ctx *ctx)
{
	...
	if (ctx->s_ctx.init_setting_table_v2 != NULL) {
		...
		for (i = 0; i < ctx->s_ctx.init_setting_table_v2_cnt; i++) {
			...
			udelay(delay_in_us);
			DRV_LOG_MORE(ctx, "init table v2: item[%d] size:%u, i2c time(us):%lld, delay(us):%u\n",
		...
		DRV_LOG(ctx, "X: size:%u, time(us):%lld\n", ctx->s_ctx.init_setting_len,
			ixc_time);
	} else {
		...
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	for (i = 0; i < ctx->s_ctx.sensor_mode_num; i++) {
		...
		if (ctx->s_ctx.mode[i].hdr_mode == HDR_RAW_STAGGER &&
			ctx->s_ctx.stagger_fl_type == IMGSENSOR_STAGGER_FL_AUTO_DIVIDED) {
			sensor_info->max_framelength[i] = ctx->s_ctx.mode[i].exp_cnt ?
				ctx->s_ctx.frame_length_max * ctx->s_ctx.mode[i].exp_cnt :
...
void update_mode_info(struct subdrv_ctx *ctx, enum SENSOR_SCENARIO_ID_ENUM scenario_id)
{
	...
	if (ctx->s_ctx.mode[scenario_id].hdr_mode == HDR_RAW_STAGGER &&
		ctx->s_ctx.stagger_fl_type == IMGSENSOR_STAGGER_FL_AUTO_DIVIDED) {
		ctx->sw_max_frame_length =
			ctx->s_ctx.mode[scenario_id].exp_cnt ?
			ctx->s_ctx.frame_length_max * ctx->s_ctx.mode[scenario_id].exp_cnt :
			ctx->s_ctx.frame_length_max;
	} else {
		ctx->sw_max_frame_length = ctx->s_ctx.frame_length_max;
	}
				...
				u8 *feature_para, u32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_SET_FRAMELENGTH:
		set_frame_length(ctx, (u32) (*feature_data));
		break;
	...
	case SENSOR_FEATURE_GET_STAGGER_MIN_VB:
		...
		break;
#ifdef __XIAOMI_CAMERA__
	case SENSOR_XIAOMI_FEATURE_SET_EXTRA_VB:
		ctx->extraVB = *feature_data_32;
		break;
#endif
	default:

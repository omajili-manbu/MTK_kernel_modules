 ...
 * Filename:
 * ---------
 *	 pragueovx8000widemipiraw_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
...
#include "pragueovx8000widemipiraw_Sensor.h"
...
static int pragueovx8000wide_get_imgsensor_id(struct subdrv_ctx *ctx, u32 *sensor_id);
static int pragueovx8000wide_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int pragueovx8000wide_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int init_ctx(struct subdrv_ctx *ctx,	struct i2c_client *i2c_client, u8 i2c_write_id);
static int vsync_notify(struct subdrv_ctx *ctx, unsigned int sof_cnt, u64 sof_ts);
static int pragueovx8000wide_set_awb_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int pragueovx8000wide_set_multi_dig_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int pragueovx8000wide_set_multi_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void pragueovx8000wide_set_multi_gain(struct subdrv_ctx *ctx, u32 *gains, u16 exp_cnt);
static int pragueovx8000wide_set_hdr_tri_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int pragueovx8000wide_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int pragueovx8000wide_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int pragueovx8000wide_get_period_by_scenario(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int pragueovx8000wide_get_readout_by_scenario(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void mi_stream(void *arg,bool enable);
static void mi_read_CGRatio(void *arg);
static int pragueovx8000wide_set_hdr_tri_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int pragueovx8000wide_set_max_framerate_by_scenario(struct subdrv_ctx *ctx, u8 *para, u32 *len);
...
#define SEAMLESS_SWITCH_GROUP_BOKEH                           6
#define SEAMLESS_SWITCH_GROUP_VIDEO_MODE_3PART                7

/* STRUCT */
static struct mtk_sensor_saturation_info pragueovx8000_saturation_info_14bit = {
	.gain_ratio = 3400,
	.OB_pedestal = 1024,
...
static struct mtk_sensor_saturation_info pragueovx8000_saturation_info_10bit = {
	.gain_ratio = 1000,
	.OB_pedestal = 64,
...
static unsigned int g_sof_cnt = 0;
static unsigned int g_sensor_output_cnt = 0;
static bool g_last_status = false;

static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, pragueovx8000wide_set_test_pattern},
	{SENSOR_FEATURE_SEAMLESS_SWITCH, pragueovx8000wide_seamless_switch},
	{SENSOR_FEATURE_SET_AWB_GAIN, pragueovx8000wide_set_awb_gain},
	{SENSOR_FEATURE_SET_MULTI_DIG_GAIN, pragueovx8000wide_set_multi_dig_gain},
	{SENSOR_FEATURE_SET_MULTI_SHUTTER_FRAME_TIME,pragueovx8000wide_set_multi_shutter_frame_length},
	{SENSOR_FEATURE_SET_ESHUTTER, pragueovx8000wide_set_shutter},
	{SENSOR_FEATURE_SET_SHUTTER_FRAME_TIME, pragueovx8000wide_set_shutter_frame_length},
	{SENSOR_FEATURE_SET_DUAL_GAIN,pragueovx8000wide_set_hdr_tri_gain},
	{SENSOR_FEATURE_SET_HDR_SHUTTER,pragueovx8000wide_set_hdr_tri_shutter},
	{SENSOR_FEATURE_GET_PERIOD_BY_SCENARIO,pragueovx8000wide_get_period_by_scenario},
	{SENSOR_FEATURE_GET_READOUT_BY_SCENARIO,pragueovx8000wide_get_readout_by_scenario},
	{SENSOR_FEATURE_SET_MAX_FRAME_RATE_BY_SCENARIO, pragueovx8000wide_set_max_framerate_by_scenario},
};
...
static struct mtk_mbus_frame_desc_entry frame_desc_cus14[] = {
	{
		.bus.csi2 = {
			...
			.user_data_desc = VC_STAGGER_NE,
			.fs_seq = MTK_FRAME_DESC_FS_SEQ_FIRST,
			.saturation_info = &pragueovx8000_saturation_info_14bit,
		},
	},
	{
		.bus.csi2 = {
...
//4096x3072_binning_4C2PlusSCG_10bit_30fps_PD_VC_4096x768_Max_A_gain_63.75x
//V1.3_MODE3
#define PRE_MODE  .frame_desc = frame_desc_prev,\
		.num_entries = ARRAY_SIZE(frame_desc_prev),\
		.mode_setting_table = pragueovx8000wide_preview_setting,\
		.mode_setting_len = ARRAY_SIZE(pragueovx8000wide_preview_setting),\
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_HD_CAP_MODE,\
		.seamless_switch_mode_setting_table = pragueovx8000wide_preview_setting,\
		.seamless_switch_mode_setting_len = ARRAY_SIZE(pragueovx8000wide_preview_setting),\
		.hdr_mode = HDR_NONE,\
		.raw_cnt = 1,\
		.exp_cnt = 1,\
		.pclk = 75162500,\
...
static struct subdrv_mode_struct mode_struct[] = {
	// preview mode 0
	// V1.3_MODE3 4096x3072_binning_4C2PlusSCG_10bit_60fps_PD_DT_4096x768_Max_A_gain_63.75x
	// PD size = 4096x768
	// Tline = 5.333us
	// VB = 16.949ms
	// version:2025-1114_v1.3
	{
		PRE_MODE
	},
	...
	// normal_video mode 2
	// V1.3_MODE17 4096x2304_binning_4C2PlusSCG_crop_10bit_30fps_PDVC_4096x576_Max_A_gain_63.75x
	// PD size = 4096x576
	// Tline = 5.333us
	// VB = 21.045 ms
	// version:2025-1114_v1.3
	{
		...
		.mode_setting_table = pragueovx8000wide_normal_video_setting,
		.mode_setting_len = ARRAY_SIZE(pragueovx8000wide_normal_video_setting),
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_VIDEO_MODE_3PART,
		.seamless_switch_mode_setting_table = pragueovx8000wide_normal_video_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(pragueovx8000wide_normal_video_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 75020000,
	...
	// V1.3_MODE16 1088x612_binning_4C1SCG_crop_10bit_480fps_no_PD_Max_A_gain_63.75x
	// no PD
	// Tline = 3.16us
	// VB = 0.145ms
	// version:2025-1114_v1.3
	{
		...
		.mode_setting_table = pragueovx8000wide_hs_video_setting,
		.mode_setting_len = ARRAY_SIZE(pragueovx8000wide_hs_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = pragueovx8000wide_hs_video_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(pragueovx8000wide_hs_video_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 75020000,
	...
	// custom1 mode 5 :video：60fps 16:9
	// V1.3_MODE17 4096x2304_binning_4C2PlusSCG_crop_10bit_30fps_PDVC_4096x576_Max_A_gain_63.75x
	// PD size = 4096x576
	// Tline = 5.333us
	// VB = 4.395 ms
	...
	// V1.3_MODE14 2048x1152_binning_4C2PlusSCG_crop_10bit_120fps_PDVC_2048x576_Max_A_gain_63.75x
	// PD Size = 2048*576
	// Tline = 5.333us
	// VB = 2.187ms
	// version:2025-1114_v1.3
	{
		...
		.mode_setting_table = pragueovx8000wide_custom2_setting,
		.mode_setting_len = ARRAY_SIZE(pragueovx8000wide_custom2_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = pragueovx8000wide_custom2_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(pragueovx8000wide_custom2_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 75020000,
	...
	// V1.3_MODE15 1920x1080_binning_4C1SCG_crop_10bit_240fps_no_PD_Max_A_gain_63.75x
	// no PD
	// Tline = 3.16us
	// VB = 0.752ms
	// version:2025-1114_v1.3
	{
		...
		.mode_setting_table = pragueovx8000wide_custom3_setting,
		.mode_setting_len = ARRAY_SIZE(pragueovx8000wide_custom3_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = pragueovx8000wide_custom3_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(pragueovx8000wide_custom3_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 75020000,
	...
	// V1.3_MODE1 8192x6144_full_quad_bayer_10bit_30fps_no_PD_Max_A_gain_15.9375x
	// Tline = 5.333us
	// VB = 0.576ms
	// version:2025-1114_v1.3
	{
		...
		.mode_setting_table = pragueovx8000wide_custom4_setting,
		.mode_setting_len = ARRAY_SIZE(pragueovx8000wide_custom4_setting),
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_NORMAL_CAP_MODE,
		.seamless_switch_mode_setting_table = pragueovx8000wide_custom4_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(pragueovx8000wide_custom4_setting),
	...
	// custom5 mode 9 : ISZ quad
	//  V1.3_MODE8 4096x3072_full_crop_quad_bayer_10bit_30fps_PD_VC_2048x768_Max_A_gain_15.9375x
	// PD Size = 2048x768
	// Tline = 5.333 us
	// VB = 16.96ms
	// version:2025-1114_v1.3
	{
		...
		.mode_setting_table = pragueovx8000wide_custom5_setting,
		.mode_setting_len = ARRAY_SIZE(pragueovx8000wide_custom5_setting),
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_NORMAL_CAP_MODE,
		.seamless_switch_mode_setting_table = pragueovx8000wide_custom5_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(pragueovx8000wide_custom5_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 75075000,
	...
	// custom6 mode 10 : 
	//same as preview(1117updata)
	{
		PRE_MODE
	},
	// custom7 mode 11 : 4 binning
	// V1.3_MODE20	2048x1536_binning_4C2PlusSCG_crop_10bit_30fps_PDVC_2048x768_Max_A_gain_63.75x
	// PD Size = 2048x768
	// Tline = 5.333us
	// VB = 16.96ms
	// version:2025-1114_v1.3
	{
		...
		.mode_setting_table = pragueovx8000wide_custom7_setting,
		.mode_setting_len = ARRAY_SIZE(pragueovx8000wide_custom7_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = pragueovx8000wide_custom7_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(pragueovx8000wide_custom7_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 75020000,
	...
	// custom8 mode 12 : full size bayer 10bit
	// V1.3_MODE2 8192x6144_full_bayer_10bit_30fps_no_PD_Max_A_gain_15.9375x
	// no PD
	// Tline = 5.333 us
	// VB = 0.576ms
	// version:2025-1114_v1.3
	{
		...
		.mode_setting_table = pragueovx8000wide_custom8_setting,
		.mode_setting_len = ARRAY_SIZE(pragueovx8000wide_custom8_setting),
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_HD_CAP_MODE,
		.seamless_switch_mode_setting_table = pragueovx8000wide_custom8_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(pragueovx8000wide_custom8_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 75075000,
	...
	// custom9 mode 13: ISZ bayer 10bit
	// V1.3_MODE7 4096x3072_full_crop_bayer_10bit_30fps_PD_VC_2048x768_Max_A_gain_15.9375x
	// PD Size = 2048x768
	// Tline = 5.333us
	// VB = 16.96ms
	// version:2025-1114_v1.3
	{
		...
		.mode_setting_table = pragueovx8000wide_custom9_setting,
		.mode_setting_len = ARRAY_SIZE(pragueovx8000wide_custom9_setting),
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_NORMAL_CAP_MODE,
		.seamless_switch_mode_setting_table = pragueovx8000wide_custom9_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(pragueovx8000wide_custom9_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 75075000,
	...
	//custom10 mode 14 : 1x bokeh(binning)
	//same as preview(1117update)
	// version:2025-1114_v1.3
	{
	...
	// custom11 mode 15 : 2x bokeh(ISZ quad)
	// same as (custom5 mode 9)
	// Tline = 5.333 us
	// VB = 16.96 ms
	// version:2025-1114_v1.3
	{
	...
	// custom12 mode 16 : bokeh fallback(binning)
	//same as preview(1225update)
	//24fps
	{
		.frame_desc = frame_desc_prev,
		.num_entries = ARRAY_SIZE(frame_desc_prev),
	...
	// custom13 mode 17: 4:3 binning 10bit
	// V1.3_MODE4 4096x3072_binning_4C2PlusSCG_10bit_60fps_PD_VC_4096x768_Max_A_gain_63.75x
	// Tline = 5.333us
	// VB = 16.384ms
	// version:2025-1114_v1.3
	{
		...
		.mode_setting_table = pragueovx8000wide_custom13_setting,
		.mode_setting_len = ARRAY_SIZE(pragueovx8000wide_custom13_setting),
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_NORMAL_CAP_MODE,
		.seamless_switch_mode_setting_table = pragueovx8000wide_custom13_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(pragueovx8000wide_custom13_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 75162500,
	...
	// V1.4_MODE6 4096x3072_binning_4C2Plus_DCG_Combine_MSB_14bit_30fps_PD_VC_4096x768_Max_A_gain_63.75x
	// PD Size = 4096x768
	// Tline = 10.333us
	// VB = 1.633ms
	// version:2025-1114_v1.3
	{
		...
		.mode_setting_table = pragueovx8000wide_custom14_setting,
		.mode_setting_len = ARRAY_SIZE(pragueovx8000wide_custom14_setting),
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_NORMAL_CAP_MODE,
		.seamless_switch_mode_setting_table = pragueovx8000wide_custom14_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(pragueovx8000wide_custom14_setting),
	...
	// custom15 mode 19: stagger exp2 10bit
	// V1.4_MODE5 4096x3072_binning_4C2PlusSCG_STG2_HDR_10bit_30fps_PD_VC_4096x768_Max_A_gain_63.75x
	// PD Size = 4096x768
	// Tline = 25.493us
	// VB = 13.715ms
	// version:2025-1114_v1.3
	{
		...
		.mode_setting_table = pragueovx8000wide_custom15_setting,
		.mode_setting_len = ARRAY_SIZE(pragueovx8000wide_custom15_setting),
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_NORMAL_CAP_MODE,
		.seamless_switch_mode_setting_table = pragueovx8000wide_custom15_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(pragueovx8000wide_custom15_setting),
	...
	// custom16 mode 20:30fps video 10bit 16:9
	//same as normal_video mode 2
	// V1.3_MODE17 4096x2304_binning_4C2PlusSCG_crop_10bit_30fps_PDVC_4096x576_Max_A_gain_63.75x
	{
		.frame_desc = frame_desc_vid,
		.num_entries = ARRAY_SIZE(frame_desc_vid),
	...
	// custom17 mode 21: video 10bit 16:10
	// V1.3_MODE9  4096x2560_binning_4C2PlusSCG_crop_10bit_30fps_PD_VC_4096x640_Max_A_gain_63.75x
	// PD Size = 4096x640
	// Tline = 5.333us
	// VB = 19.669ms
	// version:2025-1114_v1.3
	{
		...
		.mode_setting_table = pragueovx8000wide_custom17_setting,
		.mode_setting_len = ARRAY_SIZE(pragueovx8000wide_custom17_setting),
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_VIDEO_MODE,
		.seamless_switch_mode_setting_table = pragueovx8000wide_custom17_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(pragueovx8000wide_custom17_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 75020000,
	...
	// custom18 mode 22 : 14bit video DCG
	// V1.3_MODE11 4096x2560_binning_4C2Plus_crop_DCG_combine_MSB_14bit_30fps_PD_VC_4096x640_Max_A_gain_63.75x
	// PD Size = 4096x640
	// Tline = 5.333us
	// VB = 6.923ms
	// version:2025-1114_v1.3
	{
		...
		.mode_setting_table = pragueovx8000wide_custom18_setting,
		.mode_setting_len = ARRAY_SIZE(pragueovx8000wide_custom18_setting),
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_VIDEO_MODE,
		.seamless_switch_mode_setting_table = pragueovx8000wide_custom18_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(pragueovx8000wide_custom18_setting),
		.hdr_mode = HDR_RAW_DCG_COMPOSE,
		.raw_cnt = 1,
		.exp_cnt = 2,
		.pclk = 75020000,
	...
	// custom19 mode 23 : full size crop bayer 16:10 10 bit
	// V1.3_MODE13 4096x2560_full_crop_bayer_10bit_30fps_PD_VC_2048x640_Max_A_gain_15.9375x
	// PD Size = 2048x640
	// Tline = 5.333us
	// VB = 19.691ms
	// version:2025-1114_v1.3
	{
		...
		.mode_setting_table = pragueovx8000wide_custom19_setting,
		.mode_setting_len = ARRAY_SIZE(pragueovx8000wide_custom19_setting),
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_VIDEO_MODE,
		.seamless_switch_mode_setting_table = pragueovx8000wide_custom19_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(pragueovx8000wide_custom19_setting),
		.hdr_mode = PARAM_UNDEFINED,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 75075000,
	...
	// V1.3_MODE19 7680x4320_full_crop_bayer_10bit_30fps_PD_VC_3840x1080_Max_A_gain_15.9375x
	// PD Size = 3840x1080
	// Tline = 5.333us
	// VB = 10.304ms
	// version:2025-1114_v1.3
	{
		...
		.mode_setting_table = pragueovx8000wide_custom20_setting,
		.mode_setting_len = ARRAY_SIZE(pragueovx8000wide_custom20_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = pragueovx8000wide_custom20_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(pragueovx8000wide_custom20_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 75075000,
	...
	// V1.3_MODE18 4096x2304_binning_4C2PlusSCG_crop_10bit_120fps_no_PD_Max_A_gain_63.75x
	// no PD 
	// Tline = 3.16us
	// VB = 1.049ms
	// version:2025-1114_v1.3
	{
		...
		.mode_setting_table = pragueovx8000wide_custom21_setting,
		.mode_setting_len = ARRAY_SIZE(pragueovx8000wide_custom21_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = pragueovx8000wide_custom21_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(pragueovx8000wide_custom21_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 75020000,
	...
	// custom22 mode 26 : 14bit video DXG(16:9)
	// V1.6_MODE21 4096x2304_binning_4C2Plus_DCG_Combine_MSB_14bit_30fps_PD_VC_4096x576_Max_A_gain_63.75x
	// PD Size = 4096x576
	// Tline = 10.333 us
	// VB = 9.507 ms
	// version:2026-0106_v1.6
	{
		.frame_desc = frame_desc_cus22,
		.num_entries = ARRAY_SIZE(frame_desc_cus22),
		.mode_setting_table = pragueovx8000wide_custom22_setting,
		.mode_setting_len = ARRAY_SIZE(pragueovx8000wide_custom22_setting),
		.seamless_switch_group = SEAMLESS_SWITCH_GROUP_VIDEO_MODE_3PART,
		.seamless_switch_mode_setting_table = pragueovx8000wide_custom22_setting,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(pragueovx8000wide_custom22_setting),
		.hdr_mode = HDR_RAW_DCG_COMPOSE,
		.raw_cnt = 1,
		.exp_cnt = 2,
		.pclk = 75020000,
...
static struct subdrv_static_ctx static_ctx = {
	.sensor_id = PRAGUEOVX8000WIDE_SENSOR_ID,
	...
	.ana_gain_type = 0,
	.ana_gain_step = 1,
	.ana_gain_table = pragueovx8000wide_ana_gain_table,
	.ana_gain_table_size = sizeof(pragueovx8000wide_ana_gain_table),
	.tuning_iso_base = 50,
	.exposure_def = 0x3D0,
	.exposure_min = 8,
	...
	.exposure_step = 4,
	.exposure_margin = 32,
	.saturation_info = &pragueovx8000_saturation_info_10bit,
	...
	.init_setting_table = pragueovx8000wide_init_setting,
	.init_setting_len = ARRAY_SIZE(pragueovx8000wide_init_setting),
	.mode = mode_struct,
...
static struct subdrv_ops ops = {
	.get_id = pragueovx8000wide_get_imgsensor_id,
	.init_ctx = init_ctx,
	.open = common_open,
...
static struct subdrv_pw_seq_entry pw_seq[] = {
	{HW_ID_AFVDD, {3110000, 3110000},  1000},
...
const struct subdrv_entry pragueovx8000wide_mipi_raw_entry = {
	.name = "pragueovx8000wide_mipi_raw",
	.id = PRAGUEOVX8000WIDE_SENSOR_ID,
	.pw_seq = pw_seq,
...
static void mi_stream(void *arg,bool enable)
{
	...
	if(enable){
		pragueovx8000wide_set_awb_gain(ctx, (u8 *)&g_last_awb_gain, &len);
		i2c_table_write(ctx,pragueovx8000wide_stream_on, ARRAY_SIZE(pragueovx8000wide_stream_on));
		g_sof_cnt = 0;
		g_sensor_output_cnt = 0;
		g_last_status = false;
	}else{
		mi_stream_check_stream_off(ctx);
		i2c_table_write(ctx,pragueovx8000wide_stream_off, ARRAY_SIZE(pragueovx8000wide_stream_off));
	}
}
...
static void mi_read_CGRatio(void *arg){
	...
	if(ctx->s_ctx.mi_dxg_reg){
		subdrv_i2c_wr_u8(ctx,0x3624,0x80);
		subdrv_i2c_wr_u8(ctx,0x3625,0x8c);
...
int pragueovx8000wide_get_imgsensor_id(struct subdrv_ctx *ctx, u32 *sensor_id)
{
	u8 i = 0;
	u8 retry = 2;
...
{
	if (reg_gain < 1024) {
		set_i2c_buffer(ctx_ovx8000, 0x3506, ctx_ovx8000->s_ctx.mode[ctx_ovx8000->current_scenario_id].mode_setting_table[15]);
		set_i2c_buffer(ctx_ovx8000, 0x3546, ctx_ovx8000->s_ctx.mode[ctx_ovx8000->current_scenario_id].mode_setting_table[21]);
	} else {
		set_i2c_buffer(ctx_ovx8000, 0x3506, ctx_ovx8000->s_ctx.mode[ctx_ovx8000->current_scenario_id].mode_setting_table[15] + 0x02);
		set_i2c_buffer(ctx_ovx8000, 0x3546, ctx_ovx8000->s_ctx.mode[ctx_ovx8000->current_scenario_id].mode_setting_table[21] + 0x02);
	}
	...
	ctx_ovx8000->s_ctx.mode[ctx_ovx8000->current_scenario_id].mode_setting_table[15],
	ctx_ovx8000->s_ctx.mode[ctx_ovx8000->current_scenario_id].mode_setting_table[21]);
}
...
static int covert_seamless_setting_table(enum SENSOR_SCENARIO_ID_ENUM scenario_id, u16* seamless_setting)
{
	...
	for (i = 0; i< ctx_ovx8000->s_ctx.mode[scenario_id].seamless_switch_mode_setting_len/2; i++){
		...
		(ctx_ovx8000->s_ctx.mode[scenario_id].seamless_switch_mode_setting_table[2*i] == 0x3002))) {
...
void pragueovx8000wide_update_mode_info(struct subdrv_ctx *ctx, enum SENSOR_SCENARIO_ID_ENUM scenario_id)
{
	if (scenario_id >= ctx->s_ctx.sensor_mode_num)
	{
		DRV_LOGE(ctx, "invalid sid:%u, mode_num:%u\n",
...
void pragueovx8000wide_write_frame_length(struct subdrv_ctx *ctx, u32 fll)
{
...
bool support_vsync_offset(struct subdrv_ctx *ctx)
{
	bool ret = false;

	DRV_LOG_MUST(ctx, "sensormode:%d, is_seamless:%d is_streaming:%d, g_sof_cnt: %d, g_sensor_output_cnt: %d, g_last_status:%d, ctx->current_fps:%d\n",
		ctx->current_scenario_id, ctx->is_seamless, ctx->is_streaming, g_sof_cnt, g_sensor_output_cnt, g_last_status, ctx->current_fps);

	if ((ctx->current_scenario_id == 2 || ctx->current_scenario_id == 8 || ctx->current_scenario_id == 17 ||
		ctx->current_scenario_id == 18 || ctx->current_scenario_id == 21 || ctx->current_scenario_id == 22 ||
		ctx->current_scenario_id == 23 || ctx->current_scenario_id == 24 || ctx->current_scenario_id == 26)
		&& !ctx->is_seamless && ctx->is_streaming && ((g_sensor_output_cnt > 1 && g_last_status) || (g_sof_cnt > 1 && g_last_status) || 
		(g_sensor_output_cnt > 1 && g_sof_cnt > 1)) && ctx->current_fps <= 300) {
		ret = true;
	} else {
		ret = false;
	}
...
void init_vsync_offset(struct subdrv_ctx *ctx)
{
	u32 vsync_rise = 0, vsync_width = 0, offset = 0;
	...
	if ((ctx->current_scenario_id == 2 || ctx->current_scenario_id == 8 || ctx->current_scenario_id == 17 ||
		...
		if (ctx->current_scenario_id == 8 || ctx->current_scenario_id == 17 || ctx->current_scenario_id == 18) {
			offset = 13000000LL/(((int64_t)ctx->line_length * 1000000000LL) / ctx->pclk);	//capture 13ms
		} else {
			offset = 9000000LL/(((int64_t)ctx->line_length * 1000000000LL) / ctx->pclk);	//video 9ms
		}

		vsync_rise = ctx->frame_length - (ctx->exposure[0] + offset) + 16 + 8;
		if (vsync_rise > (ctx->frame_length - 9)) {
			vsync_rise = ctx->frame_length - 9;
		}

		vsync_width = ctx->s_ctx.mode[ctx->current_scenario_id].imgsensor_winsize_info.h2_tg_size + (ctx->exposure[0] + offset) - 1;

		if(ctx_ovx8000->s_ctx.mode[ctx_ovx8000->current_scenario_id].mi_mode_type != 1) {
			vsync_rise = vsync_rise / 2;
			vsync_width = vsync_width / 2;
		}

		set_i2c_buffer(ctx, 0x381a, (vsync_rise >> 8) & 0xFF);	//设置vsync rise offset1
		set_i2c_buffer(ctx, 0x381b, vsync_rise & 0xFF);

		set_i2c_buffer(ctx, 0x381e, (vsync_width >> 8) & 0xFF);	//设置vsync width offset2
		set_i2c_buffer(ctx, 0x381f, vsync_width & 0xFF);

    	commit_i2c_buffer(ctx);
    	DRV_LOG(ctx, "set seamless switch: shutter:%d, framelength:%d, offset:%d, vsync_rise:%d, vsync_width:%d\n",
    	        ctx->exposure[0], ctx->frame_length, offset, vsync_rise, vsync_width);
	}


}

void write_vsync_offset(struct subdrv_ctx *ctx)
{
	u32 vsync_rise = 0, vsync_width = 0;
	...
	rot = ctx->s_ctx.mode[ctx->current_scenario_id].imgsensor_winsize_info.h2_tg_size;
	vblanking = ctx->frame_length - shutter - rot;

	if ((shutter + offset) > ctx->frame_length) {
		offset = 0;
	}

	vsync_rise = ctx->frame_length - (shutter + offset) + 16 + 8;
	if (vsync_rise > (ctx->frame_length - 9)) {
		vsync_rise = ctx->frame_length - 9;
	}

	vsync_width = rot + (shutter + offset) - 1;
	if(ctx_ovx8000->s_ctx.mode[ctx_ovx8000->current_scenario_id].mi_mode_type != 1) {
		vsync_rise = vsync_rise / 2;
		vsync_width = vsync_width / 2;
	}
	DRV_LOG(ctx, "ois_vsync: shutter:%d, framelength:%d, offset:%d, rot:%d, extraVB:%d vblanking:%d, vsync_rise:%d, vsync_width:%d\n",
		shutter, ctx->frame_length, offset, rot, ctx->extraVB, vblanking, vsync_rise, vsync_width);
	...
	set_i2c_buffer(ctx, 0x381a, (vsync_rise >> 8) & 0xFF);	//设置vsync rise offset1
	set_i2c_buffer(ctx, 0x381b, vsync_rise & 0xFF);
	set_i2c_buffer(ctx, 0x3208, 0x12);	//group 2 end
	set_i2c_buffer(ctx, 0x3208, 0x03);	//group 3 start
	set_i2c_buffer(ctx, 0x381e, (vsync_width >> 8) & 0xFF);	//设置vsync width offset2
	set_i2c_buffer(ctx, 0x381f, vsync_width & 0xFF);
	set_i2c_buffer(ctx, 0x3208, 0x13);	//group 3 end
	set_i2c_buffer(ctx, 0x320d, 0x93);
	set_i2c_buffer(ctx, 0x320b, 0x01);
	set_i2c_buffer(ctx, 0x320c, 0x01);
	set_i2c_buffer(ctx, 0x320e, 0xa2);

	commit_i2c_buffer(ctx);

}

void pragueovx8000wide_set_multi_shutter_frame_length_dol(struct subdrv_ctx *ctx,
	u64 *shutters, u16 exp_cnt,	u16 frame_length)
{
	...
	bool gph_ois = false;
	gph_ois = support_vsync_offset(ctx);
	...
	/* group hold start */
	if (gph_ois) {
		set_i2c_buffer(ctx, 0x3208, 0x02);	//group 2 start
	} else {
		if (gph)
	...
	if (set_auto_flicker(ctx, 0) || frame_length || !ctx->s_ctx.reg_addr_auto_extend)
		pragueovx8000wide_write_frame_length(ctx, ctx->frame_length);
	else if (ctx->s_ctx.reg_addr_auto_extend)
		pragueovx8000wide_write_frame_length(ctx, ctx->min_frame_length);
	/* write shutter */
	switch (exp_cnt) {
	case 1:
...
static int pragueovx8000wide_set_hdr_tri_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	DRV_LOG(ctx, "pragueovx8000wide_set_hdr_tri_shutter");

	pragueovx8000wide_set_multi_shutter_frame_length_dol(ctx, values, exp_cnt, 0);
	return 0;
}
...
static int pragueovx8000wide_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	u32 frame_length_in_lut[IMGSENSOR_STAGGER_EXPOSURE_CNT] = {0};
	u32 exp_cnt = 0;
	u16 pragueovx8000wide_setting_seamless[700] = {0};
	...
	setting_seamless_len = covert_seamless_setting_table(scenario_id, pragueovx8000wide_setting_seamless);
	...
	pragueovx8000wide_update_mode_info(ctx, scenario_id);

	if (is_dol) {
		i2c_table_write(ctx,pragueovx8000wide_seamless_switch_step1_dol,
			ARRAY_SIZE(pragueovx8000wide_seamless_switch_step1_dol));
	} else {
		i2c_table_write(ctx,pragueovx8000wide_seamless_switch_step1,
			ARRAY_SIZE(pragueovx8000wide_seamless_switch_step1));
	}
	...
	if (ctx->s_ctx.mode[scenario_id].seamless_switch_group == 1 ||
		...
		i2c_table_write(ctx,pragueovx8000wide_setting_seamless, 2*setting_seamless_len);
	 }

	if (ctx->s_ctx.mode[scenario_id].mi_mode_type == 1)
		pragueovx8000wide_set_awb_gain(ctx, (u8 *)&g_last_awb_gain, &retLen);
	...
	if (ae_ctrl) {
		...
		case HDR_RAW_STAGGER:
			pragueovx8000wide_set_multi_shutter_frame_length_dol(ctx, (u64 *)&ae_ctrl->exposure, exp_cnt, 0);
		...
		case HDR_RAW_DCG_COMPOSE:
			pragueovx8000wide_set_shutter_frame_length(ctx, (u8 *)&ae_ctrl->exposure, len);
			if (ctx->s_ctx.mode[scenario_id].dcg_info.dcg_gain_mode == IMGSENSOR_DCG_DIRECT_MODE) {
				pragueovx8000wide_set_multi_gain(ctx, (u32 *)&ae_ctrl->gain, exp_cnt);
			}
			else
				set_gain(ctx, ae_ctrl->gain.le_gain);
			break;
		default:
			pragueovx8000wide_set_shutter(ctx, (u8 *)&ae_ctrl->exposure.le_exposure,len);
			set_gain(ctx, ae_ctrl->gain.le_gain);
			break;
		}
	}
	...
	if (is_dol) {
		i2c_table_write(ctx,pragueovx8000wide_seamless_switch_step2_dol,
			ARRAY_SIZE(pragueovx8000wide_seamless_switch_step2_dol));
	} else {
		i2c_table_write(ctx,pragueovx8000wide_seamless_switch_step2,
			ARRAY_SIZE(pragueovx8000wide_seamless_switch_step2));
	}
...
static u16 pragueovx8000wide_dgain2reg(struct subdrv_ctx *ctx, u32 dgain)
{
...
static int pragueovx8000wide_set_multi_dig_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	for (i = 0; i < exp_cnt; i++) {
		...
		gains[i] = pragueovx8000wide_dgain2reg(ctx, gains[i]);
	}
...
static int pragueovx8000wide_set_hdr_tri_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	pragueovx8000wide_set_multi_gain(ctx,values, exp_cnt);
	return 0;
}
...
static int pragueovx8000wide_get_period_by_scenario(struct subdrv_ctx *ctx, u8 *para, u32 *len){
	...
	periodNum = (ctx->s_ctx.mode[scenario_id].framelength << 16)
			+ (ctx->s_ctx.mode[scenario_id].linelength * ratio);
	*period = periodNum;

	DRV_LOG_MUST(ctx, "flag:%llu, scenario_id:%u, mode_num:%u, ratio = %u, periodNum = %u\n",
		flag, scenario_id, ctx->s_ctx.sensor_mode_num, ratio, periodNum);
...
static int pragueovx8000wide_get_readout_by_scenario(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	*readout_time =
		...
		* 1000000000 / ctx->s_ctx.mode[scenario_id].pclk;

	DRV_LOG_MUST(ctx, "sid:%u, readout_time = %lld\n",
			scenario_id, *readout_time);
	return 0;
}

static int pragueovx8000wide_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	pragueovx8000wide_set_shutter_frame_length(ctx, para, len);
	return 0;
}
...
static int vsync_notify(struct subdrv_ctx *ctx, unsigned int sof_cnt, u64 sof_ts)
{
	...
	sensor_output_cnt =  (subdrv_i2c_rd_u8(ctx, 0x387E) << 8) | subdrv_i2c_rd_u8(ctx, ctx->s_ctx.reg_addr_frame_count);

	g_sensor_output_cnt = sensor_output_cnt;

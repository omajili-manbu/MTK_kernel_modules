 ...
 *============================================================================
 ****************************************************************************/
#include "imx06clitemipiraw_Sensor.h"

#define IMX06CLITE_EMBEDDED_DATA_EN 1
...
static int imx06clite_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int imx06clite_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int imx06clite_set_test_pattern_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int imx06clite_cphy_lrte_mode(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int vsync_notify(struct subdrv_ctx *ctx,	unsigned int sof_cnt, u64 sof_ts);
static int imx06clite_mcss_init(void *arg);
static int imx06clite_mcss_set_mask_frame(struct subdrv_ctx *ctx, u32 num, u32 is_critical);
static int imx06clite_mcss_update_subdrv_para(void *arg, int scenario_id);
static int imx06clite_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void imx06clite_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int imx06clite_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int imx06clite_get_linetime_in_ns(void *arg, u32 scenario_id, u32 *linetime_in_ns,
	enum GET_LINETIME_ENUM linetime_type, enum IMGSENSOR_EXPOSURE exp_idx);

static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, imx06clite_set_test_pattern},
	{SENSOR_FEATURE_SET_TEST_PATTERN_DATA, imx06clite_set_test_pattern_data},
	{SENSOR_FEATURE_SEAMLESS_SWITCH, imx06clite_seamless_switch},
	{SENSOR_FEATURE_SET_CPHY_LRTE_MODE, imx06clite_cphy_lrte_mode},
	{ SENSOR_FEATURE_SET_ESHUTTER, imx06clite_set_shutter },
	{ SENSOR_FEATURE_SET_GAIN, imx06clite_set_gain },
};

static int imx06clite_get_linetime_in_ns(void *arg,
...
static struct subdrv_ops ops = {
	...
	.mcss_set_mask_frame = imx06clite_mcss_set_mask_frame,
};
...
static struct subdrv_static_ctx_ext_ops static_ext_ops = {
	...
	.mcss_update_subdrv_para = imx06clite_mcss_update_subdrv_para,
	.cust_get_linetime_in_ns = imx06clite_get_linetime_in_ns,
	.mcss_init = imx06clite_mcss_init,
#ifdef IMX06CLITE_ISF_DBG
	.debug_check_with_exist_s_ctx = &imx06clite_legacy_s_ctx,
#endif
};

const struct subdrv_entry imx06clite_mipi_raw_entry = {
	.name = "imx06clite_mipi_raw",
	.ops = &ops,
	.is_fw_support = TRUE,
...
static int imx06clite_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	u32 exp_cnt = 0;
	u64 seamless_i2c_start_time = 0;
	u64 seamless_i2c_finish_time = 0;
	...
	seamless_i2c_start_time = ktime_get_boottime_ns();
	commit_i2c_buffer(ctx);
	seamless_i2c_finish_time = ktime_get_boottime_ns();

	ctx->fast_mode_on = TRUE;
	ctx->ref_sof_cnt = ctx->sof_cnt;
	ctx->is_seamless = FALSE;
	DRV_LOG_MUST(ctx, "X: set seamless switch done, using (%llu)ns\n",
		seamless_i2c_finish_time - seamless_i2c_start_time);
	return ERROR_NONE;
}
static int imx06clite_cphy_lrte_mode(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
...
static int imx06clite_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	imx06clite_set_shutter_frame_length(ctx, para, 0);

	return 0;
}

void imx06clite_set_long_exposure(struct subdrv_ctx *ctx)
{
	u32 shutter = ctx->exposure[IMGSENSOR_STAGGER_EXPOSURE_LE];
	u32 l_shutter = 0;
	u16 l_shift = 0;

	if (shutter > (ctx->s_ctx.frame_length_max_without_lshift - ctx->s_ctx.exposure_margin)) {
		...
		for (l_shift = 1; l_shift < ctx->s_ctx.cit_lshift_max; l_shift++) {
			l_shutter = ((shutter - 1) >> l_shift) + 1;
			if (l_shutter
				< (ctx->s_ctx.frame_length_max_without_lshift - ctx->s_ctx.exposure_margin))
				break;
		}
		if (l_shift > ctx->s_ctx.cit_lshift_max) {
			DRV_LOGE(ctx, "unable to set exposure:%u, set to max\n", shutter);
			l_shift = ctx->s_ctx.cit_lshift_max;
		}
...
static void imx06clite_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	/* write shutter */
	imx06clite_set_long_exposure(ctx);
...
static int imx06clite_mcss_init(void *arg)
{
	...
	if (!(ctx->mcss_init_info.enable_mcss)) {
		...
		/*
		 * subdrv_i2c_wr_u8(ctx,
		 * ctx->s_ctx.reg_addr_mcss_mc_frm_lp_en, 0x00);
		*/
		/* FLL N+1/N+2 */

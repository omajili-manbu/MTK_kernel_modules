 ...
 * Filename:
 * ---------
 *	 yiliov08f10frontmipiraw_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
 ...
 *============================================================================
 ****************************************************************************/
#include "yiliov08f10frontmipiraw_Sensor.h"

static int init_ctx(struct subdrv_ctx *ctx,	struct i2c_client *i2c_client, u8 i2c_write_id);
static u16 get_gain2reg(u32 gain);
static int front_vsync_notify(struct subdrv_ctx *ctx, unsigned int sof_cnt, u64 sof_ts);
static int yiliov08f10front_close(struct subdrv_ctx *ctx);
static int yiliov08f10front_get_imgsensor_id(struct subdrv_ctx *ctx, u32 *sensor_id);
static int yiliov08f10front_return_sensor_id(struct subdrv_ctx *ctx);
static int yiliov08f10front_control(struct subdrv_ctx *ctx, enum MSDK_SCENARIO_ID_ENUM scenario_id,
		MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data);
static int yiliov08f10front_set_max_framerate_by_scenario(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int yiliov08f10front_streamon(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int yiliov08f10front_streamoff(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int yiliov08f10front_streaming_control(struct subdrv_ctx *ctx, bool enable);
static int yiliov08f10front_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int yiliov08f10front_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int yiliov08f10ultra_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int yiliov08f10front_set_multi_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int yiliov08f10front_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void yiliov08f10front_set_dummy(struct subdrv_ctx *ctx);
static void yiliov08f10front_soft_power_up(struct subdrv_ctx *ctx);
...
#define VBLANK_FULL (2501)
#define VBLANK_BINNING (1252)

/* STRUCT */
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_STREAMING_SUSPEND, yiliov08f10front_streamoff},
	{SENSOR_FEATURE_SET_STREAMING_RESUME, yiliov08f10front_streamon},
	{SENSOR_FEATURE_SET_GAIN, yiliov08f10front_set_gain},
	{SENSOR_FEATURE_SET_SHUTTER_FRAME_TIME, yiliov08f10front_set_shutter_frame_length},
	{SENSOR_FEATURE_SET_MAX_FRAME_RATE_BY_SCENARIO, yiliov08f10front_set_max_framerate_by_scenario},
	{SENSOR_FEATURE_SET_ESHUTTER, yiliov08f10ultra_set_shutter},
	{SENSOR_FEATURE_SET_MULTI_SHUTTER_FRAME_TIME, yiliov08f10front_set_multi_shutter_frame_length},
	{SENSOR_FEATURE_SET_TEST_PATTERN, yiliov08f10front_set_test_pattern},
};
...
static struct subdrv_mode_struct mode_struct[] = {
	...
	//setting 20260212 V2.4
	{
		.frame_desc = frame_desc_prev,
		.num_entries = ARRAY_SIZE(frame_desc_prev),
		.mode_setting_table = yiliov08f10front_preview_setting,
		.mode_setting_len = ARRAY_SIZE(yiliov08f10front_preview_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 34927200,
	...
	//setting 20260212 V2.4
	{
		...
		.mode_setting_table = yiliov08f10front_normal_video_setting,
		.mode_setting_len = ARRAY_SIZE(yiliov08f10front_normal_video_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 34927200,
	...
	//setting 20260212 V2.4
	{
		...
		.mode_setting_table = yiliov08f10front_custom1_setting,
		.mode_setting_len = ARRAY_SIZE(yiliov08f10front_custom1_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 35080000,
	...
	//setting 20260212 V2.4
	{
		...
		.mode_setting_table = yiliov08f10front_custom2_setting,
		.mode_setting_len = ARRAY_SIZE(yiliov08f10front_custom2_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 35080000,
	...
	//setting 20260212 V2.4
	{
		...
		.mode_setting_table = yiliov08f10front_custom3_setting,
		.mode_setting_len = ARRAY_SIZE(yiliov08f10front_custom3_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 35080000,
	...
	//setting 20260212 V2.4
	{
		...
		.mode_setting_table = yiliov08f10front_custom4_setting,
		.mode_setting_len = ARRAY_SIZE(yiliov08f10front_custom4_setting),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 35080000,
...
static struct subdrv_static_ctx static_ctx = {
	.sensor_id = YILIOV08F10FRONT_SENSOR_ID,
	...
	.ana_gain_type = 1,
	.ana_gain_step = 1,
	.ana_gain_table = yiliov08f10front_ana_gain_table,
	.ana_gain_table_size = sizeof(yiliov08f10front_ana_gain_table),
	.tuning_iso_base = 50,
	.exposure_def = 0x3D0,
	.exposure_min = 8,
	...
	.init_setting_table = yiliov08f10front_init_setting,
	.init_setting_len = ARRAY_SIZE(yiliov08f10front_init_setting),
	.mode = mode_struct,
...
static struct subdrv_ops ops = {
	.get_id = yiliov08f10front_get_imgsensor_id,
	.init_ctx = init_ctx,
	.open = common_open,
	.get_info = common_get_info,
	.get_resolution = common_get_resolution,
	.control = yiliov08f10front_control,
	.feature_control = common_feature_control,
	.close = yiliov08f10front_close,
	.get_frame_desc = common_get_frame_desc,
	.get_temp = common_get_temp,
	//.get_csi_param = yiliov08f10front_get_csi_param,
...
static struct subdrv_pw_seq_entry aov_pw_seq[] = {
	...
	{HW_ID_DVDD1,  {1200000, 1200000}, 1000},//dvdd_vin vcam ldo
	{HW_ID_AVDD, {2800000, 2800000}, 1000},
	{HW_ID_DVDD, {1200000, 1200000}, 6000},//dvdd vcam ldo
	{HW_ID_RST, {1}, 1000},
...
const struct subdrv_entry yiliov08f10front_mipi_raw_entry = {
	.name = "yiliov08f10front_mipi_raw",
	.id = YILIOV08F10FRONT_SENSOR_ID,
	.pw_seq = pw_seq,
...
static int yiliov08f10front_get_imgsensor_id(struct subdrv_ctx *ctx, u32 *sensor_id)
{
	...
	while (ctx->s_ctx.i2c_addr_table[i] != 0xFF) {
		...
		do {
			*sensor_id = yiliov08f10front_return_sensor_id(ctx);
...
static int yiliov08f10front_control(struct subdrv_ctx *ctx,
			...
			MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	yiliov08f10front_streaming_control(ctx, FALSE);// stream off
...
static int yiliov08f10front_streamon(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	if (ctx->s_ctx.aov_sensor_support && ctx->s_ctx.mode[ctx->current_scenario_id].aov_mode) {
		...
		yiliov08f10front_soft_power_up(ctx);
		return ERROR_NONE;
	}
	return yiliov08f10front_streaming_control(ctx, TRUE);
}
...
static int yiliov08f10front_streamoff(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	return yiliov08f10front_streaming_control(ctx, FALSE);
}
...
static void yiliov08f10front_set_dummy(struct subdrv_ctx *ctx)
{
	...
	if (ctx->s_ctx.aov_sensor_support && ctx->s_ctx.mode[ctx->current_scenario_id].aov_mode)
		dummy_line = (ctx->frame_length - VBLANK_BINNING);//BINNING MODE
else
		dummy_line = (ctx->frame_length - VBLANK_FULL)*2;//FULLSIZE MODE
...
static int yiliov08f10front_set_max_framerate_by_scenario(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	if (ctx->frame_length > (ctx->exposure[0] + ctx->s_ctx.exposure_margin))
		yiliov08f10front_set_dummy(ctx);
...
{
	DRV_LOG(ctx, "+\n");
	yiliov08f10front_set_shutter_frame_length(ctx, para, len);
	return ERROR_NONE;
}
...
static int yiliov08f10front_close(struct subdrv_ctx *ctx)
{
	DRV_LOG_MUST(ctx, "yiliov08f10front_close\n");
	return ERROR_NONE;
}
...
static int set_streaming_control(void *arg, bool enable)
{
	...
	if (enable) {
		ret = yiliov08f10front_streaming_control(ctx, TRUE); //stream on

...
static int unset_reg(struct adaptor_ctx *ctx, void *data, int val)
{
	...
	if (!ctx->regulator[idx] && IS_ERR(ctx->regulator[idx])) {
		ctx->regulator[idx] = NULL;
		adaptor_logi(ctx,"ERR: regulator at index %llu is Invalid regulator pointer\n", idx);
	...
	if (regulator_is_enabled(reg)) {
		...
		if (ret) {
			adaptor_logi(ctx,
				"ERR: disable(%s),ret(%llu)(fail)\n",
				reg_names[idx], ret);
			return ret;
		}
	} else {
		adaptor_logi(ctx,"ERR: regulator at index %llu is not enable\n", idx);
		WRAP_AEE_EXCEPTION("unset_reg", "Err: regulator is not enable");
		return -EINVAL;
	}

...
#if IS_ENABLED(CONFIG_MALI_MTK_LOG_BUFFER)
					mtk_logbuffer_type_print(kbdev, MTK_LOGBUFFER_TYPE_CRITICAL | MTK_LOGBUFFER_TYPE_EXCEPTION | MTK_LOGBUFFER_TYPE_DEFERRED,
...
#else /* CONFIG_MALI_MTK_LOG_BUFFER */
						dev_info(kctx->kbdev->dev, "context#seqno:%s dma_fence_array[%d] null and log bypass", info.name, i);
#endif /* CONFIG_MALI_MTK_LOG_BUFFER */
					}
				}
			}
				    ...
				    struct kbasep_printer *kbpr)
{
	...
	if ((fence_signal_command_timeout_counter == 2) || (fence_signal_command_timeout_counter == 3)
		...
		mtk_common_debug(MTK_COMMON_DBG_DUMP_AS_STATUS, kctx, MTK_DBG_HOOK_MALI_FENCE_SIGNAL_TIMEOUT);
		...
		mtk_common_debug(MTK_COMMON_DBG_DUMP_POI_KTRACE, kctx, MTK_DBG_HOOK_MALI_FENCE_SIGNAL_TIMEOUT);
#endif /* CONFIG_MALI_MTK_POI_KTRACE */
#endif /* CONFIG_MALI_MTK_DEBUG_DUMP */
#if IS_ENABLED(CONFIG_MALI_MTK_FW_ANALYSIS_DUMP)
		queue_work(kctx->kbdev->mtk_fw_analysis_dump_workq, &kctx->kbdev->mtk_fw_analysis_dump_work);

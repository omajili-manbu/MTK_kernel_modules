	...
	unsigned int *pui32Block, unsigned int *pui32Idle, void *Util_Ex)
{
	...
	if (psDevInfo && psDevInfo->pfnGetBasicGpuUtilStats) {
		psDevInfo->pfnGetBasicGpuUtilStats(psDevInfo->psDeviceNode, &psDevInfo->sGpuUtilStats);
...
PVRSRV_ERROR MTKTDSendFWImage(IMG_HANDLE hSysData, PVRSRV_FW_PARAMS *psTDFWParams)
{
	int ret ;

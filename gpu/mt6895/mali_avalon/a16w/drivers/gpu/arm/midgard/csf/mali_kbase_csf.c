...
#if IS_ENABLED(CONFIG_MALI_MTK_ADAPTIVE_POWER_POLICY)||IS_ENABLED(CONFIG_MALI_MTK_GPU_DVFS_HINT_26M_LOADING)
#include <ged_dvfs.h>
	...
	if (!kbase_is_region_invalid_or_free(region)) {
		...
		if (queue && (queue->bind_state == KBASE_CSF_QUEUE_BOUND)) {
			...
			dev_warn_ratelimited(kbdev->dev,
					     "CS_QUEUE_KICK: queue not found (buffer_addr=0x%.16llx, ctx %d_%d)",
					     kick->buffer_gpu_addr, kctx->tgid, kctx->id);
		} else {
			dev_warn_ratelimited(kbdev->dev,
					     "CS_QUEUE_KICK: queue not bound (state=%d, buffer_addr=0x%.16llx, ctx %d_%d)",
					     queue->bind_state, kick->buffer_gpu_addr, kctx->tgid, kctx->id);
		}
	} else {
		dev_warn_ratelimited(kbdev->dev,
				     "CS_QUEUE_KICK: no valid command buffer region (buffer_addr=0x%.16llx, ctx %d_%d",
				     kick->buffer_gpu_addr, kctx->tgid, kctx->id);
		err = -EFAULT;
	}
	kbase_gpu_vm_unlock(kctx);
...
#if IS_ENABLED(CONFIG_MALI_MTK_KBASE_THREAD_DEBUG)
	MALI_KTHREAD_WORK_START(queue, "oom_event_worker");
#endif /* CONFIG_MALI_MTK_KBASE_THREAD_DEBUG */
...
#if IS_ENABLED(CONFIG_MALI_MTK_KBASE_THREAD_DEBUG)
	MALI_KTHREAD_WORK_END(queue, "oom_event_worker");
#endif /* CONFIG_MALI_MTK_KBASE_THREAD_DEBUG */
...
#if IS_ENABLED(CONFIG_MALI_MTK_LOG_BUFFER)
		mtk_logbuffer_type_print(kbdev, MTK_LOGBUFFER_TYPE_CRITICAL | MTK_LOGBUFFER_TYPE_EXCEPTION,
			"queue not bound when handling an error event\n");
#endif /* CONFIG_MALI_MTK_LOG_BUFFER */
...
#endif /* CONFIG_MALI_MTK_LOG_BUFFER */
#if IS_ENABLED(CONFIG_MIEV)
		mi_disp_mievent_report(MI_EVENT_GPU_CS_FAULT);
#endif
	}

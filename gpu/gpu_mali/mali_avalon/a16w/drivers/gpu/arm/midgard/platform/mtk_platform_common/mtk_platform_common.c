...
#include <platform/mtk_platform_common/mtk_platform_debug_dump_poi_ktrace.h>
...
{
	return mtk_poi_ktrace_get_recording_entry(code);
}
#endif /* CONFIG_MALI_MTK_POI_KTRACE */
...
#if IS_ENABLED(CONFIG_MALI_MTK_POI_KTRACE)
		case MTK_COMMON_DBG_DUMP_POI_KTRACE:
			mtk_debug_dump_poi_ktrace(kbdev);
			break;
#endif /* CONFIG_MALI_MTK_POI_KTRACE */
#if IS_ENABLED(CONFIG_MALI_MTK_DIAGNOSIS_MODE)
...
#if IS_ENABLED(CONFIG_MALI_MTK_POI_KTRACE)
	mtk_debug_dump_poi_ktrace_init();
#endif /* CONFIG_MALI_MTK_POI_KTRACE */

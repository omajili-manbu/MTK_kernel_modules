...
static void kbase_file_delete(struct kbase_file *const kfile)
{
	...
	if (atomic_read(&kfile->setup_state) == KBASE_FILE_COMPLETE) {
		...
		if (execute_time >= KBASE_FUNCTION_EXECUTE_DEBUG_TIMEOUT) {
			dev_warn(kbdev->dev, "ctx:%d_%d kbase_release monitor (%llums)", kctx_tgid, kctx_id, execute_time);
#if IS_ENABLED(CONFIG_MALI_MTK_LOG_BUFFER)
			// dump critical msg when kbase release execution time > 1s
			if (execute_time >= 1000) {
				mtk_logbuffer_type_print(kbdev, MTK_LOGBUFFER_TYPE_CRITICAL | MTK_LOGBUFFER_TYPE_EXCEPTION,
					"ctx:%d_%d kbase_release too long! (%llums)\n", kctx_tgid, kctx_id, execute_time);
			}
#endif /* CONFIG_MALI_MTK_LOG_BUFFER */
		}
#endif /* kbase_file_delete */
...
{
#if IS_ENABLED(CONFIG_MIEV)
	mi_disp_mievent_report(MI_EVENT_GPU_REQUEST_TIMEOUT);
#endif
...
static long kbase_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	kctx = kbase_file_get_kctx_if_setup_complete(kfile);
	if (unlikely(!kctx)) {
		if (cmd == KBASE_IOCTL_CS_QUEUE_KICK)
			dev_warn(kbdev->dev,
				 "CS_QUEUE_KICK: context not ready (setup_state=%d)",
				 atomic_read(&kfile->setup_state));
		return -EPERM;
	}
...
#if IS_ENABLED(CONFIG_MALI_MTK_DYNAMIC_SCHEDULING)
	kbdev->csf.scheduler.csg_scheduling_period_debug = (kbdev->csf.scheduler.csg_scheduling_period_ms != 100);
#endif /* CONFIG_MALI_MTK_DYNAMIC_SCHEDULING */

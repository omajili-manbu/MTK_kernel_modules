...
enum mtk_common_debug_types {
	...
	MTK_COMMON_DBG_CSF_DUMP_ITER_HWIF_LOCKED,
	...
	MTK_COMMON_DBG_DUMP_AS_STATUS_NOLOCK,
#if IS_ENABLED(CONFIG_MALI_MTK_POI_KTRACE)
	MTK_COMMON_DBG_DUMP_POI_KTRACE,
...
int mtk_common_ged_dvfs_update_platform_step_size(int low_step, int med_step, int high_step ,
...
#if IS_ENABLED(CONFIG_MALI_MTK_POI_KTRACE)
struct mtk_common_poi_ktrace_data {
...
struct mtk_common_poi_ktrace_data *mtk_common_poi_ktrace_get_recording_entry(enum kbase_ktrace_code code);
#endif /* CONFIG_MALI_MTK_POI_KTRACE */

...
#if IS_ENABLED(CONFIG_MALI_MTK_POI_KTRACE)
#define MTK_POI_KTRACE_ADD(kbdev, code, kctx, info_val) \
	mtk_poi_ktrace_add_wrap(kbdev, KBASE_KTRACE_CODE(code), kctx, 0, info_val)
...
#define KBASE_KTRACE_ADD(kbdev, code, kctx, info_val)                             \
	do {                                                                      \
		...
		MTK_POI_KTRACE_ADD(kbdev, code, kctx, __info_val);                \
	} while (0)

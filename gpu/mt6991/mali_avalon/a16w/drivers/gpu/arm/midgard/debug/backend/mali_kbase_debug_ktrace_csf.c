...
#if IS_ENABLED(CONFIG_MALI_MTK_POI_KTRACE)
void mtk_poi_ktrace_add_csf_wrap(struct kbase_device *kbdev, enum kbase_ktrace_code code,
			   struct kbase_queue_group *group, struct kbase_queue *queue,
			   kbase_ktrace_flag_t flags, u64 info_val)
{
	struct kbase_context *kctx = NULL;
	struct mtk_common_poi_ktrace_data *recording_data =
		mtk_common_poi_ktrace_get_recording_entry(code);

	if (recording_data == NULL)
		return;

	memset(recording_data, 0, sizeof(struct mtk_common_poi_ktrace_data));

	ktime_get_boottime_ts64(&recording_data->timestamp);
	recording_data->code = code;
	...
	if (kctx) {
		recording_data->kctx_tgid = kctx->tgid;
		recording_data->kctx_id = kctx->id;
	}

	if (queue)
		recording_data->u.csf_group.queue_csi_index = queue->csi_index;

	if (group) {
		recording_data->u.csf_group.group_handle = group->handle;
		recording_data->u.csf_group.csg_slot = group->csg_nr;
		if (group->csg_nr > 0) {
			struct kbase_csf_csg_slot *csg_slot =
				&kbdev->csf.scheduler.csg_slots[group->csg_nr];
			recording_data->u.csf_group.csg_slot_prio = csg_slot->priority;
		}
	}

	recording_data->u.csf_group.info_val1 = info_val;
}

void mtk_poi_ktrace_add_csf_kcpu_wrap(struct kbase_device *kbdev, enum kbase_ktrace_code code,
				struct kbase_kcpu_command_queue *queue, u64 info_val1,
				u64 info_val2)
{
	struct mtk_common_poi_ktrace_data *recording_data =
	...
	ktime_get_boottime_ts64(&recording_data->timestamp);
	recording_data->code = code;
	recording_data->kctx_tgid = queue->kctx->tgid;
	recording_data->kctx_id = queue->kctx->id;
	recording_data->u.csf_kcpu.kcpu_queue_id = queue->id;
	recording_data->u.csf_kcpu.info_val1 = info_val1;
	recording_data->u.csf_kcpu.info_val2 = info_val2;
}
#endif /* CONFIG_MALI_MTK_POI_KTRACE */

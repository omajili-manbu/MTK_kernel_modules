...
wpa_remove_ptk(struct wpa_state_machine *sm) {
	...
	if (wpa_auth_set_key(sm->wpa_auth, 0, WPA_ALG_NONE, sm->addr,
			     0, NULL, 0) < 0) {
		wpa_printf(MSG_WARNING, "WARN! wpa_auth_set_key fail");
	}
...
void wpa_group_ensure_init(struct wpa_authenticator *wpa_auth,
			   struct wpa_group *group)
{
...
#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_FRAME_PROT == 1)
size_t ieee80211w_kde_len(struct wpa_state_machine *sm)
{
	size_t len = 0;
	struct wpa_auth_config *nmiConf = &sm->wpa_auth_nmi->conf;
	uint8_t ucIsIGTKSA = 0, ucIsBIGTKSA = 0, ucGroupSAUsed = 0;
	...
	if (prNdp)
		ucGroupSAUsed = prNdp->ucGroupSAUsed;
#ifdef DALE
	else
		ucGroupSAUsed = NAN_GROUP_SECURITY_ASSOC_USE_IGTKSA |
			NAN_GROUP_SECURITY_ASSOC_USE_BIGTKSA;
#endif

	ucIsIGTKSA = (ucGroupSAUsed &
		NAN_GROUP_SECURITY_ASSOC_USE_IGTKSA) ? TRUE : FALSE;
	ucIsBIGTKSA = (ucGroupSAUsed &
		NAN_GROUP_SECURITY_ASSOC_USE_BIGTKSA) ? TRUE : FALSE;

	if (sm->mgmt_frame_prot && ucIsIGTKSA) {
		len += 2 + RSN_SELECTOR_LEN + WPA_IGTK_KDE_PREFIX_LEN;
		len += wpa_cipher_key_len(nmiConf->group_mgmt_cipher);
	}

	if (sm->mgmt_frame_prot && nmiConf->beacon_prot && ucIsBIGTKSA) {
...
u8 *ieee80211w_kde_add(struct wpa_state_machine *sm, u8 *pos)
{
	struct wpa_igtk_kde igtk = {0};
	struct wpa_bigtk_kde bigtk = {0};
	struct wpa_group *gsm = sm->wpa_auth_nmi->group;
	u8 rsc[WPA_KEY_RSC_LEN] = {0};
	struct wpa_auth_config *nmiConf = &sm->wpa_auth_nmi->conf;
	size_t len = wpa_cipher_key_len(nmiConf->group_mgmt_cipher);
	uint8_t ucIsIGTKSA = 0, ucIsBIGTKSA = 0, ucGroupSAUsed = 0;
	struct _NAN_NDP_INSTANCE_T *prNdp = NULL;
	...
	if (gsm->wpa_group_state != WPA_GROUP_SETKEYSDONE ||
	    wpa_auth_get_seqnum(sm->wpa_auth_nmi,
	    NULL, gsm->GN_igtk, rsc) < 0)
		os_memset(igtk.pn, 0, sizeof(igtk.pn));
	else
		kalMemCpyS(igtk.pn,
			sizeof_field(struct wpa_igtk_kde, pn),
			rsc,
			sizeof(igtk.pn));
	kalMemCpyS(igtk.igtk,
		sizeof_field(struct wpa_igtk_kde, igtk),
		gsm->IGTK[gsm->GN_igtk - 4],
		len);
#ifdef NAN_R4_SECURITY_UNUSED
	...
	wpa_hexdump(MSG_DEBUG,
		"IGTK", &igtk, (WPA_IGTK_KDE_PREFIX_LEN + len));

	if (!nmiConf->beacon_prot || !ucIsBIGTKSA)
		return pos;

	bigtk.keyid[0] = (u8)gsm->GN_bigtk;
	bigtk.keyid[1] = 0;
	if (gsm->wpa_group_state !=
		WPA_GROUP_SETKEYSDONE ||
	    wpa_auth_get_seqnum(sm->wpa_auth_nmi,
	    NULL, gsm->GN_bigtk, rsc) < 0)
		os_memset(bigtk.pn, 0, sizeof(bigtk.pn));
	else
		kalMemCpyS(bigtk.pn,
			sizeof_field(struct wpa_bigtk_kde, pn),
			rsc,
			sizeof(bigtk.pn));

	kalMemCpyS(bigtk.bigtk,
		sizeof_field(struct wpa_bigtk_kde, bigtk),
		gsm->BIGTK[gsm->GN_bigtk - 6], len);

	pos = wpa_add_kde(pos,
		RSN_KEY_DATA_BIGTK,
		(const u8 *)&bigtk,
		WPA_BIGTK_KDE_PREFIX_LEN + len, NULL, 0);

	wpa_hexdump(MSG_DEBUG, "BIGTK",
		    &bigtk, (WPA_BIGTK_KDE_PREFIX_LEN + len));

	return pos;
}

uint32_t
nan_sec_wpa_send_eapol_4(
	struct wpa_sm *sm,
	const u8 *key_rsc,
	const u8 *kde,
	size_t kde_len,
...
{
	/* struct ieee802_1x_hdr *hdr; */
	struct wpa_eapol_key *reply = NULL;
	struct wpa_eapol_key_192 *reply192 = NULL;
	size_t mic_len = 0, keyhdrlen = 0; /* len */
	...
	mic_len = (size_t)wpa_mic_len((int)sm->key_mgmt);
	keyhdrlen = mic_len == 24 ? sizeof(*reply192) : sizeof(*reply);
	...
	if (force_version)
		version = force_version;
	else if (sm->key_mgmt == WPA_KEY_MGMT_OSEN ||
			 wpa_key_mgmt_suite_b((int)sm->key_mgmt))
		version = WPA_KEY_INFO_TYPE_AKM_DEFINED;
	else if (wpa_key_mgmt_sha256((int)sm->key_mgmt) ||
	 ...
	 */
#endif /* NAN's version only use AKM */
	if ((version == WPA_KEY_INFO_TYPE_AKM_DEFINED) && encr) {
		pad_len = key_data_len % 8;
		if (pad_len)
			pad_len = 8 - pad_len;
		key_data_len += pad_len + 8;
	...
	if (szTotalLen > NAN_KDE_ATTR_BUF_SIZE)
		DBGLOG(NAN, ERROR, "Invalid length: %lu\n", szTotalLen);

#ifdef NAN_TODO
	sm->szTmpKdeAttrLen = szTotalLen;
#endif

	sm->u4TmpKdeAttrLen = szTotalLen;
	...
	prNanSecKdeAttrHdr->u2AttrLen =
		(uint16_t)(keyhdrlen + key_data_len + 1); /*1:publishId*/
	...
	reply = (struct wpa_eapol_key *)
		(sm->au1TmpKdeAttrBuf + sizeof(struct _NAN_SEC_KDE_ATTR_HDR));

	key_len = sizeof_field(struct wpa_state_machine, au1TmpKdeAttrBuf) -
		sizeof(struct _NAN_SEC_KDE_ATTR_HDR);
	reply192 = (struct wpa_eapol_key_192 *)reply;

	key_data = ((u8 *)reply) + keyhdrlen;
	...
	key_info |= (uint32_t)version |
		WPA_KEY_INFO_KEY_TYPE;
	if (mic_len)
		key_info |= WPA_KEY_INFO_MIC;

	if (encr && reply->type == EAPOL_KEY_TYPE_RSN)
		key_info |= WPA_KEY_INFO_ENCR_KEY_DATA;
	if (reply->type == EAPOL_KEY_TYPE_RSN)
		key_info |= keyidx << WPA_KEY_INFO_KEY_INDEX_SHIFT;

	key_info |= WPA_KEY_INFO_INSTALL;

	WPA_PUT_BE16(reply->key_info, (u16)key_info);

	/* alg = pairwise ? sm->pairwise : wpa_auth->conf.wpa_group; */
	/* WPA_PUT_BE16(key->key_length, wpa_cipher_key_len(alg)); */
	WPA_PUT_BE16(reply->key_length, 0);

	if ((uint32_t) key_info & WPA_KEY_INFO_SMK_MESSAGE)
		WPA_PUT_BE16(reply->key_length, 0);


	kalMemCpyS(reply->replay_counter,
		sizeof_field(struct wpa_eapol_key, replay_counter),
		key->replay_counter,
		sizeof_field(struct wpa_eapol_key, replay_counter));
	...
	if (key_rsc)
		kalMemCpyS(reply->key_rsc,
			sizeof_field(struct wpa_eapol_key, key_rsc),
			key_rsc, WPA_KEY_RSC_LEN);
	...
	if (kde && !encr) {
		kalMemCpyS(key_data,
			...
			kde_len);
		if (mic_len == 24)
			WPA_PUT_BE16(reply192->key_data_length, (u16)kde_len);
		else
		 ...
		 */
#endif /* NAN's version only use AKM */
		if (version == WPA_KEY_INFO_TYPE_AKM_DEFINED) {

			if (INT_MAX < ((key_data_len - 8) / 8)) {
				DBGLOG(INIT, ERROR,
					"Line:[%d] ((key_data_len - 8) / 8)=[%lu] overflowed!!!\n",
					__LINE__, ((key_data_len - 8) / 8));
			}

			if (aes_wrap(sm->ptk.kek, sm->ptk.kek_len,
				(int)(INT_MAX & ((key_data_len - 8) / 8)),
				buf, key_data, key_len - keyhdrlen)) {
				/* os_free(hdr); */
			...
			if (mic_len == 24)
				WPA_PUT_BE16(reply192->key_data_length,
					     (u16)key_data_len);
			else
				WPA_PUT_BE16(reply->key_data_length,
					     (u16)key_data_len);
		} else {
			/* os_free(hdr); */
			DBGLOG(NAN, ERROR, "key_mgmt incorrect, %d, %u\n",
			       version, sm->key_mgmt);
			os_free(buf);
			return WLAN_STATUS_FAILURE;
		}
...
#ifdef NAN_R4_SECURITY_UNUSED
	if ((uint32_t) key_info & WPA_KEY_INFO_MIC) {
		/* u8 *key_mic; */

		if (!sm->PTK_valid) {
			wpa_auth_logger_prvc(
				wpa_auth, sm->addr, LOGGER_DEBUG, NORMAL,
...
int
nan_sec_wpa_send_gtk_in_M4(struct wpa_state_machine *wpaStateMachine,
			   struct wpa_sm *sm, const struct wpa_eapol_key *key)
{
	u8 rsc[WPA_KEY_RSC_LEN] = {0}, *_rsc = NULL, *gtk = NULL, *kde = NULL,
					*pos = NULL; /*dummy_gtk[32]*/
	size_t gtk_len = 0;
	size_t kde_len = 0;
	u8 dummy_gtk[32] = {0};
	struct wpa_group *gsm = wpaStateMachine->group;
	u8 *wpa_ie = NULL;
	int keyidx = 0, encr = 0;
	size_t wpa_ie_len = 0;
	...
	wpa_auth_get_seqnum(wpaStateMachine->wpa_auth,
		NULL, gsm->GN, rsc);
	/* If FT is used, wpa_auth->wpa_ie includes both RSNIE and MDIE */
	wpa_ie = wpaStateMachine->wpa_auth->wpa_ie;
	wpa_ie_len = wpaStateMachine->wpa_auth->wpa_ie_len;

	wpa_hexdump(MSG_DEBUG, "WPA IE in M4", wpa_ie, wpa_ie_len);

	if (wpaStateMachine->wpa == WPA_VERSION_WPA &&
	    ((uint32_t) wpaStateMachine->wpa_auth->conf.wpa &
	    WPA_PROTO_RSN) &&
	    (int)wpa_ie_len > wpa_ie[1] + 2 &&
		...
		if (wpa_ie[0] == WLAN_EID_MOBILITY_DOMAIN)
			wpa_ie = wpa_ie + wpa_ie[1] + 2;
		wpa_ie_len = (size_t)(wpa_ie[1] + 2);
	}

	wpa_auth_logger(wpaStateMachine->wpa_auth,
		wpaStateMachine->addr, LOGGER_DEBUG,
		"sending 4/4 msg of 4-Way Handshake");

	/* Skip compose GTK when GTK is disabled */
	ucGroupSAUsed =
		((struct _NAN_NDP_INSTANCE_T *)(sm->pvNdp))->ucGroupSAUsed;
	ucIsGTKSA = (ucGroupSAUsed &
		NAN_GROUP_SECURITY_ASSOC_USE_GTKSA) ? TRUE : FALSE;

	if ((wpaStateMachine->wpa == WPA_VERSION_WPA2) &&
		ucIsGTKSA) {
		wpa_printf(MSG_DEBUG,
		...
		wpa_hexdump(MSG_DEBUG, "GTK in M4", gtk, gtk_len);

		if (wpaStateMachine->wpa_auth->conf.disable_gtk) {
			/*
		...
		keyidx = 0;
		_rsc = NULL;
		if (wpaStateMachine->rx_eapol_key_secure) {
			/*
	...
	kde_len = ieee80211w_kde_len(wpaStateMachine);
	if (gtk)
		kde_len += 2 + RSN_SELECTOR_LEN + 2 + gtk_len;

	kde = os_malloc(kde_len);
	/* Return if kde_len != 0, but allocate kde fail */
	if (kde_len && kde == NULL)
		return -1;

	/* encrypt KDE if it's non-zero */
	if (kde) {
		encr = 1;
		...
		if (gtk) {
			...
			hdr[1] = 0;
			pos = wpa_add_kde(pos,
				RSN_KEY_DATA_GROUPKEY, hdr, 2, gtk,
				gtk_len);
		}
		pos = ieee80211w_kde_add(wpaStateMachine, pos);
	}

	sm->u1CurMsg = NAN_SEC_M4;

	if (nan_sec_wpa_send_eapol_4(
		sm, _rsc, kde, kde_len, keyidx, encr, 0,
		key) != WLAN_STATUS_SUCCESS) {
		wpa_printf(MSG_ERROR,
				   "Send eapol 4 fail!");
	}
	os_free(kde);
...
#else /* CONFIG_IEEE80211W */
#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_FRAME_PROT == 0)
...
SM_STATE(WPA_PTK, PTKINITNEGOTIATING) {
	...
	ucGroupSAUsed =
		((struct _NAN_NDP_INSTANCE_T *)(sm->pvNdp))->ucGroupSAUsed;
	ucIsGTKSA = !!(ucGroupSAUsed & NAN_GROUP_SECURITY_ASSOC_USE_GTKSA);

	if ((sm->wpa == WPA_VERSION_WPA2) && ucIsGTKSA) {
	...
	if (kde) {
		...
		if (gtk) {
			...
			hdr[0] = keyidx & 0x03;
			hdr[1] = 0;
			DBGLOG(NAN, INFO, "add gtk, gtk len=%u\n", gtk_len);
			pos = wpa_add_kde(pos, RSN_KEY_DATA_GROUPKEY, hdr, 2,
					  gtk, gtk_len);
		}
		DBGLOG(NAN, INFO, "add igtk, bigtk\n");
...
#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_FRAME_PROT == 1)
			if (kalMemCmp(sm->au1Psk, aucPMK, PMK_LEN))
#else
			  ...
			  struct wpa_group *group)
{
	...
	kalMemCpyS(group->GNonce,
		sizeof_field(struct wpa_group, GNonce),
		group->Counter,
		sizeof_field(struct wpa_group, Counter));
	inc_byte_array(group->Counter, WPA_NONCE_LEN);

#if (CFG_SUPPORT_NAN_R4_DALE_HOST_KEY == 0)
	/* In dale's platform, host will plumb key using IOCTL*/
	if (wpa_gmk_to_gtk(group->GMK,
		"Group key expansion",
		wpa_auth->addr,
		group->GNonce, group->GTK[group->GN - 1],
		group->GTK_len) < 0)
		ret = -1;
#endif /* CFG_SUPPORT_NAN_R4_DALE_HOST_KEY */
	...
	if (wpa_auth->conf.ieee80211w !=
		NO_MGMT_FRAME_PROTECTION) {
		len = wpa_cipher_key_len(
			wpa_auth->conf.group_mgmt_cipher);
		kalMemCpyS(group->GNonce,
			sizeof_field(struct wpa_group, GNonce),
			group->Counter,
...
wpa_group_config_group_keys(struct wpa_authenticator *wpa_auth,
	...
	if (wpa_auth->conf.ieee80211w != NO_MGMT_FRAME_PROTECTION) {
		...
		alg = (enum wpa_alg)wpa_cipher_to_alg(
			wpa_auth->conf.group_mgmt_cipher);
		len = wpa_cipher_key_len(
			wpa_auth->conf.group_mgmt_cipher);

...
wpa_add_kde(u8 *pos, u32 kde, const u8 *data, size_t data_len, const u8 *data2,
	...
	*pos++ = (u8)(RSN_SELECTOR_LEN + data_len + data2_len);
	RSN_SELECTOR_PUT(pos, kde);
	pos += RSN_SELECTOR_LEN;
	kalMemCpyS(pos, data_len, data, data_len);
	pos += data_len;
	if (data2) {
		kalMemCpyS(pos, data2_len, data2, data2_len);
		pos += data2_len;
	}
#else
	*pos++ = WLAN_EID_VENDOR_SPECIFIC;
...
wpa_parse_generic(const u8 *pos, const u8 *end, struct wpa_eapol_ie_parse *ie) {
	...
	if (pos[1] >= 6 && RSN_SELECTOR_GET(pos + 2) == WPA_OUI_TYPE &&
		...
		ie->wpa_ie = pos;
		ie->wpa_ie_len = (size_t)(pos[1] + 2);
		return 0;
	}

	ieLen = (size_t)(2 + pos[1]);
	...
	if (pos[1] > RSN_SELECTOR_LEN + 2 &&
		...
		ie->gtk_len = (size_t)(pos[1] - RSN_SELECTOR_LEN);
	...
	if (pos[1] > RSN_SELECTOR_LEN + 2 &&
		...
		ie->mac_addr_len = (size_t)pos[1] - RSN_SELECTOR_LEN;
		return 0;
	}
	...
	if (pos[1] > RSN_SELECTOR_LEN + 2 &&
		...
		ie->igtk_len = (size_t)(pos[1] - RSN_SELECTOR_LEN);
		wpa_hexdump(MSG_DEBUG,
	...
	if (pos[1] > RSN_SELECTOR_LEN + 2 &&
		...
		ie->bigtk_len = (size_t)(pos[1] - RSN_SELECTOR_LEN);
		wpa_hexdump(MSG_DEBUG,

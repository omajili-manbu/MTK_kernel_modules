...
hostapd_wpa_auth_get_seqnum(void *ctx, const u8 *addr, int idx, u8 *seq) {
	...
	DBGLOG(NAN, INFO,
	    "idx:%d,IPN:%x-%x-%x-%x-%x-%x,BIPN:%x-%x-%x-%x-%x-%x\n",
	    idx,
	...
	if (idx == 4 || idx == 5) {
		kalMemCpyS(seq, WPA_KEY_RSC_LEN, g_aucNanIgtkPn,
			   sizeof(g_aucNanIgtkPn));
	} else if (idx == 6 || idx == 7) {
		kalMemCpyS(seq, WPA_KEY_RSC_LEN, g_aucNanBigtkPn,
			   sizeof(g_aucNanBigtkPn));
	}

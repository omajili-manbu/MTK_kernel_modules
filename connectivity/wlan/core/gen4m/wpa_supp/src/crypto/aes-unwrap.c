...
aes_unwrap_supp(const u8 *kek, size_t kek_len, int n, const u8 *cipher,
	...
	u8 a[8] = {0}, *r = NULL, b[AES_BLOCK_SIZE] = {0};
	int i = 0, j = 0;
	void *ctx = NULL;
	unsigned int t = 0;
	size_t r_len = 0;
	...
	kalMemCpyS(a, sizeof(a), cipher, 8);
	r = plain;
	r_len = plain_len;
	kalMemCpyS(r, r_len, cipher + 8, (size_t)(8 * n));

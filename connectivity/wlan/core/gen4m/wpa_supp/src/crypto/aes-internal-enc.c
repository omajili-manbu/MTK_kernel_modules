...
aes_encrypt_init(const u8 *key, size_t len) {
	...
	rk = kalMemAlloc(AES_PRIV_SIZE, PHY_MEM_TYPE);
	if (rk == NULL)
		return NULL;
	res = rijndaelKeySetupEnc(rk, key, (int)(len * 8));
	if (res < 0) {
		kalMemFree(rk, PHY_MEM_TYPE, AES_PRIV_SIZE);
		return NULL;
	}
	rk[AES_PRIV_NR_POS] = (u32)res;
	return rk;
}
...
aes_encrypt(void *ctx, const u8 *plain, u8 *crypt) {
	...
	rijndaelEncrypt(ctx, (int)rk[AES_PRIV_NR_POS], plain, crypt);
...
aes_encrypt_deinit(void *ctx) {
	...
	kalMemFree(ctx, PHY_MEM_TYPE, AES_PRIV_SIZE);
}

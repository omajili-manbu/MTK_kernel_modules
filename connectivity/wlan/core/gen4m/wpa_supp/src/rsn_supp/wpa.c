...
wpa_eapol_key_send_wpa(struct wpa_sm *sm, const u8 *kck, size_t kck_len,
		       ...
		       size_t msg_len, u8 *key_mic, size_t key_mic_len) {
	size_t mic_len;
	...
	int key_info)
{
	...
	if (sm->group_cipher != WPA_CIPHER_GTK_NOT_USED &&
		(wpa_supplicant_check_group_cipher(sm,
		(int)sm->group_cipher,
		gtk_len,
		gtk_len,
		&gd.key_rsc_len,
...
/* Bypass IGTK install, NAN have independent WTBL usage for R4 Key installing*/
	...
	if (wpa_sm_set_key(sm, wpa_cipher_to_alg((int)sm->mgmt_group_cipher),
			   broadcast_ether_addr, keyidx, 0, igtk->pn,
			   sizeof(igtk->pn), igtk->igtk, len) < 0) {
		wpa_msg_prvc(sm->ctx->msg_ctx, MSG_WARNING, NORMAL,
			"WPA: Failed to configure IGTK to the driver");
		return -1;
	}
#endif
	...
	kalMemCpyS(sm->igtk.igtk,
		sizeof_field(struct wpa_igtk, igtk),
		igtk->igtk,
		len);
	kalMemCpyS(sm->igtk_pn,
...
{
	size_t len =
		wpa_cipher_key_len((int)sm->mgmt_group_cipher);
	u16 keyidx =
		WPA_GET_LE16(bigtk->keyid);
...
/* Bypass BIGTK install, NAN have independent WTBL usage for R4 Key installing*/
	...
	kalMemCpyS(sm->bigtk.bigtk,
		sizeof_field(struct wpa_bigtk, bigtk),
		bigtk->bigtk,
		len);
	kalMemCpyS(sm->bigtk_pn,
...
ieee80211w_set_keys(struct wpa_sm *sm, struct wpa_eapol_ie_parse *ie) {
	...
	if (ie->igtk) {
		len = wpa_cipher_key_len(
			(int)sm->mgmt_group_cipher);
		if (ie->igtk_len !=
			WPA_IGTK_KDE_PREFIX_LEN + len)
			return -1;

		igtk = (const struct wpa_igtk_kde *)ie->igtk;

		if (wpa_supplicant_install_igtk(sm,
			igtk) < 0)
			return -1;
	}
	...
	if (ie->bigtk &&
		...
		bigtk = (const struct wpa_bigtk_kde *)
			ie->bigtk;

		if (wpa_supplicant_install_bigtk(sm,
			bigtk) < 0)
			return -1;
	}
...
wpa_supplicant_process_3_of_4(
	...
	*/
	...
	/* UNREFERENCED_PARAMETER(key_data_len); */
#else /* CFG_NAN_SUPPORT_R4_GROUP_ADDR_FRAME_PROT */
...
wpa_supplicant_verify_eapol_key_mic(struct wpa_sm *sm,
	...
	if (sm->tptk_set) {
		...
		wpa_eapol_key_mic_wpa(sm->tptk.kck, sm->tptk.kck_len,
			...
			sizeof_field(struct wpa_eapol_key_192, key_mic));
#endif
		DBGLOG(NAN, INFO, "mic len=%u\n", mic_len);
		DBGDUMP_HEX(NAN, DEBUG, "mic", mic, mic_len);
		DBGDUMP_HEX(NAN, DEBUG, "key_mic", key->key_mic, mic_len);
...
wpa_sm_rx_eapol_wpa(struct wpa_sm *sm, const u8 *src_addr, const u8 *buf,
	...
	if (key_info & WPA_KEY_INFO_KEY_TYPE) {
		...
		if (key_info & WPA_KEY_INFO_MIC) {
			/* 3/4 4-Way Handshake */
			/* NAN_TODO: Not used? */
			wpa_supplicant_process_3_of_4(NULL, sm, key, ver,
						      key_data, key_data_len);
		} else {
	...
	u16 ver, u8 *key_data, size_t *key_data_len)
{
	wpa_hexdump(MSG_DEBUG,
		"RSN: encrypted key data", key_data,
		 *key_data_len);
	...
	if (ver == WPA_KEY_INFO_TYPE_HMAC_MD5_RC4 &&
		...
		kalMemCpyS(ek,
			sizeof(ek),
			key->key_iv,
			sizeof_field(struct wpa_eapol_key, key_iv));
		kalMemCpyS(
			ek +
			sizeof_field(struct wpa_eapol_key, key_iv),
			sizeof(ek) -
			sizeof_field(struct wpa_eapol_key, key_iv),
			wpaStateMachine->ptk.kek,
		...
		wpa_key_mgmt_suite_b((int)
		wpaStateMachine->wpa_key_mgmt)) {
		u8 *buf = NULL;
		...
		*key_data_len -= 8; /* AES-WRAP adds 8 bytes */
		...
		if (INT_MAX < (*key_data_len / 8)) {
			DBGLOG(INIT, ERROR,
				"Line:[%d] (*key_data_len / 8)=[%lu] overflowed!!!\n",
				__LINE__,
				(*key_data_len / 8));
		}
...
uint32_t nan_sec_wpa_rx_eapol_4(struct wpa_state_machine *wpaStateMachine,
				struct wpa_sm *wpaSm,
				struct wpa_eapol_key *keyData)
{
	...
	if ((ver == WPA_KEY_INFO_TYPE_HMAC_SHA1_AES ||
		...
		if (nan_sec_wpa_decrypt_key_data(
			wpaStateMachine,
			key,
			ver,
			key_data,
	...
	wpa_hexdump(MSG_DEBUG,
		"WPA: IE KeyData", key_data, key_data_len);

	if (wpa_supplicant_parse_ies_wpa(
		key_data,
		key_data_len,
		&ie) < 0) {
		goto out;

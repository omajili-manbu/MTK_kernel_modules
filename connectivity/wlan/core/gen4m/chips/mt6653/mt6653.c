...
#if defined(CFG_MTK_CCCI_EAP_IDC_SUPPORT)
#if (CFG_MTK_CCCI_EAP_IDC_SUPPORT == 1)
...
static void mt6653DumpDPDHwTransLog(struct ADAPTER *ad);
...
static void mt6653RecoveryMsiStatus(struct ADAPTER *prAdapter, u_int8_t fgForce)
{
	...
	uint32_t v_mcu2host_sw_int_sta = 0, v_mcu2host_sw_int_ena = 0;
	uint32_t v_wpdma2host_err_int_sta = 0, v_wpdma2host_err_int_ena = 0;
	uint32_t v_mcu_int_ena = 0, v_mcu_int_sta = 0;
#if (WFDMA_AP_MSI_NUM == 1)
	...
	uint32_t u4Addr[4] = {0}, u4IntSta = 0, u4AfterVal;
	uint32_t u4Vals[6] = {0};
...
recovery:
		...
		if (u4Val & u4IntMask) {
			...
			/* Re-read MSI mask after unmask for logging */
			{
				uint32_t u4ValAfter;

				u4ValAfter = mtk_pci_read_msi_mask(
					prAdapter->prGlueInfo);
				DBGLOG(HAL, WARN,
				       "unmask msi Rx[%u] MSI_MASK=[0x%08x] EP=[0x%08x]->[0x%08x]",
				       u4Cnt, u4IntMask, u4Val, u4ValAfter);
				/* If MSI mask bit0 is 1 after re-read, */
				/* dump DPD HW Trans Log */
				if (u4ValAfter & BIT(0))
					mt6653DumpDPDHwTransLog(prAdapter);
			}
		}
	...
	/* wfdma periodic interrupt */
	u4Addr[3] = WF_WFDMA_HOST_DMA0_HOST_PER_DLY_INT_CFG_ADDR;
	HAL_MCR_EMI_RD(prAdapter, u4Addr[3], &u4Vals[5], &fgRet);
	...
	DBGLOG(HAL, WARN,
	       "Emi=[0x%08x] WbIntSta=[0x%08x] [0x%08x]=[0x%08x] [0x%08x]=[0x%08x]->[0x%08x]",
	       u4IntSta, prHifInfo->u4WbIntSta, u4Addr[3], u4Vals[5],
	       u4Addr[0], u4Val, u4AfterVal);

	DBGLOG(HAL, WARN,
	       "[0x%08x]=[0x%08x] PCIe MAC [0x%08x][0x%08x][0x%08x][0x%08x] link_info: 0x%X",
	       u4Addr[1], u4Vals[0], u4Vals[1],
	       u4Vals[2], u4Vals[3], u4Vals[4], link_info);

	/* Extra WFDMA dump for MSI recovery */
	HAL_MCR_EMI_RD(prAdapter,
		WF_WFDMA_HOST_DMA0_MCU2HOST_SW_INT_STA_ADDR,
		&v_mcu2host_sw_int_sta, &fgRet);
	HAL_MCR_EMI_RD(prAdapter,
		WF_WFDMA_HOST_DMA0_MCU2HOST_SW_INT_ENA_ADDR,
		&v_mcu2host_sw_int_ena, &fgRet);
	HAL_MCR_EMI_RD(prAdapter,
		WF_WFDMA_HOST_DMA0_WPDMA2HOST_ERR_INT_STA_ADDR,
		&v_wpdma2host_err_int_sta, &fgRet);
	HAL_MCR_EMI_RD(prAdapter,
		WF_WFDMA_HOST_DMA0_WPDMA2HOST_ERR_INT_ENA_ADDR,
		&v_wpdma2host_err_int_ena, &fgRet);
	HAL_MCR_EMI_RD(prAdapter,
	...
	DBGLOG(HAL, WARN,
	       "WFDMA0 CR: SW_INT_STA[0x%08x] SW_INT_ENA[0x%08x] ERR_INT_STA[0x%08x]",
	       v_mcu2host_sw_int_sta, v_mcu2host_sw_int_ena,
	       v_wpdma2host_err_int_sta);

	DBGLOG(HAL, WARN,
	       "ERR_INT_ENA[0x%08x] MCU_INT_ENA[0x%08x] MCU_INT_STA[0x%08x]",
	       v_wpdma2host_err_int_ena, v_mcu_int_ena, v_mcu_int_sta);
#endif /* CFG_MTK_WIFI_WFDMA_WB */
}
...
#endif
#else /* !CFG_MTK_WIFI_PCIE_SUPPORT */
	spin_lock_init(&rPCIELock);
#endif /* CFG_MTK_WIFI_PCIE_SUPPORT */
...
#if CFG_MTK_WIFI_PCIE_SUPPORT
	struct GL_HIF_INFO *prHifInfo = &prGlueInfo->rHifInfo;
	uint32_t delay = 0, value1 = 0, u4PollTimeout = 0;
	struct mt66xx_chip_info *prChipInfo;
	struct BUS_INFO *prBusInfo;
...
#else
	if (fgEn) {
		HAL_MCR_RD(prGlueInfo->prAdapter, 0x74030150, &value);
		...
		HAL_MCR_WR(prGlueInfo->prAdapter, 0x74030150, 0x3ffff);
	}
#endif /* CFG_MTK_WIFI_PCIE_SUPPORT */
				...
				u_int8_t fgWakeup)
{
	...
	if (!prHifInfo->fgPcieKeepL0)
		kalSetHifHandleAllTokensUnusedEvent(prGlueInfo);
}
#endif //CFG_SUPPORT_PCIE_ASPM
...
static void mt6653CheckCbInfraBusCheck(struct ADAPTER *ad)
{
	uint32_t u4RegVal[2] = {0};
	uint32_t u4Value = 0x11223344;
	uint32_t u4Ret = 0;

	if (fgIsBusAccessFailed) {
		DBGLOG(INIT, WARN, "Bus Fail return\n");
		return;
	}

	/* 1. read pcie cfg.space 0x488, Readable check */
	u4Ret = glReadPcieCfgSpace(0x488, &u4RegVal[0]);
	if ((u4RegVal[0] & 0x3811) != 0x3811 ||
	    u4RegVal[0] == 0xffffffff || u4Ret != 0) {
		glReadPcieCfgSpace(0x48C, &u4RegVal[1]);
		DBGLOG(INIT, WARN,
		       "Cb_infra bus un-readable 0x488=0x%08x 0x48C=0x%08x, u4Ret=%u\n",
		       u4RegVal[0], u4RegVal[1], u4Ret);

		return;
	}

	/* 2. read 0x7002842C[29], cb_infra bus timeout check */
	HAL_RMCR_RD(ONOFF_DBG, ad, 0x7002842C, &u4Value);
	if (u4Value & BIT(29)) {
		DBGLOG(INIT, WARN,
		       "Cb_infra bus timeout 0x7002842C=0x%08x\n",
		       u4Value);
	} else
		return;

	/* 3. cb_infra bus timeout debug dump */
	DBGLOG(INIT, DEBUG, "0x7002842C=0x%08x\n", u4Value);
	HAL_RMCR_RD(ONOFF_DBG, ad, 0x70026A64, &u4Value);
	DBGLOG(INIT, DEBUG, "0x70026A64=0x%08x\n", u4Value);
	HAL_RMCR_RD(ONOFF_DBG, ad, 0x70026A68, &u4Value);
	DBGLOG(INIT, DEBUG, "0x70026A68=0x%08x\n", u4Value);
	HAL_RMCR_RD(ONOFF_DBG, ad, 0x70026A6C, &u4Value);
	DBGLOG(INIT, DEBUG, "0x70026A6C=0x%08x\n", u4Value);

	u4Ret = glReadPcieCfgSpace(0x494, &u4Value);
	if (u4Ret)
		return;

	u4Value |= BIT(7) | BIT(30) | BIT(31);
	u4Value &= ~BITS(20, 24);
	u4Ret = glWritePcieCfgSpace(0x494, u4Value);
	if (u4Ret)
		return;

	u4Ret = glReadPcieCfgSpace(0x488, &u4Value);
	if (u4Ret)
		return;
	DBGLOG(INIT, DEBUG, "cfg space 0x488=0x%08x\n", u4Value);
...
#if (CFG_MTK_CCCI_IDC_SUPPORT == 1) && \
		((CFG_MTK_CCCI_SUPPORT == 1) || \
		(CFG_MTK_CCCI_EAP_IDC_SUPPORT == 1))
#if defined(CFG_MTK_CCCI_EAP_IDC_SUPPORT) && (CFG_MTK_CCCI_EAP_IDC_SUPPORT == 1)
#define IOCTL_WDM_MAX_COMMAND _IOR('H', 0xA0, unsigned short)
#define MTK_IOC_SET_RX_BUF_SIZE _IOW('M', 0xA0, unsigned int)
#define MTK_IOC_ALLOW_DROP_PACKET _IO('M', 0xA1)
#define MTK_IOC_FORBID_DROP_PACKET _IO('M', 0xA2)
#endif
...
#if defined(CFG_MTK_CCCI_EAP_IDC_SUPPORT) && (CFG_MTK_CCCI_EAP_IDC_SUPPORT == 1)
int mt6653_ccci_notify_MD(uint8_t on_off)
{
	...
	if (prGlueInfo->u4IdcWfRdyFlag == on_off) {
		DBGLOG(INIT, INFO, "EAP IDC WF status not change\n");
		return 0;
	}

	prGlueInfo->u4IdcWfRdyFlag = on_off;

	ret = mtk_port_search("idc_wifi", &port_id, 1);

	if (ret != 1) {
		DBGLOG(INIT, INFO, "EAP IDC idc_wifi not found\n");
		return 0;
	}

	ret = mtk_port_open(port_id, O_NONBLOCK);
	if (ret != 0) {
		DBGLOG(INIT, INFO, "EAP IDC idc_wifi port open fail\n");
		return 0;
	}

	mtk_port_ioctl(port_id, MTK_IOC_FORBID_DROP_PACKET, 0);
	...
	DBGLOG(INIT, INFO, "EAP IDC CCCI send %d\n", on_off);

	ret = mtk_port_write(port_id, (void *)&idc_ccci_ctrl_msg, 8);

	if (ret < 0)
		DBGLOG(INIT, INFO, "EAP IDC CCCI send fail\n");

	mtk_port_close(port_id);
...
#if CFG_MTK_WIFI_MBU
static void mt6653MbuDebugReadCr(
	struct ADAPTER *prAdapter, const uint32_t u4DbgCr,
	uint32_t *pu4Val, u_int8_t fgIsDumpViaBT) {

	if (!fgIsDumpViaBT) {
		HAL_RMCR_RD(PLAT_DBG, prAdapter, u4DbgCr, pu4Val);
		return;
	}

#if (CFG_MTK_WIFI_CONNV3_SUPPORT == 1)
	kalDevRegReadViaBT(prAdapter->prGlueInfo, u4DbgCr, pu4Val);
#endif
}
static void mt6653MbuDebugWriteCr(
	struct ADAPTER *prAdapter, const uint32_t u4DbgCr,
	uint32_t u4Val, u_int8_t fgIsDumpViaBT) {

	if (!fgIsDumpViaBT) {
		HAL_MCR_WR(prAdapter, u4DbgCr, u4Val);
		return;
	}

#if (CFG_MTK_WIFI_CONNV3_SUPPORT == 1)
	kalDevRegWriteViaBT(prAdapter->prGlueInfo, u4DbgCr, u4Val);
#endif
}
				    ...
				    u_int8_t fgIsDumpViaBT)
{
	...
	for (u4Idx = 0; u4Idx < u4Size; u4Idx++) {
		mt6653MbuDebugReadCr(prAdapter, au4DbgCr[u4Idx], &u4Val,
				     fgIsDumpViaBT);
		u4Pos += kalSnprintf(
			aucBuf + u4Pos,
			u4BufferSize - u4Pos,
				     ...
				     u_int8_t fgIsDumpViaBT)
{
	...
	for (u4Idx = 0; u4Idx < u4Size; u4Idx++) {
		...
		if (arMbuDbg[u4Idx].read) {
			mt6653MbuDebugReadCr(
				prAdapter, arMbuDbg[u4Idx].r_addr, &u4Val,
				fgIsDumpViaBT);
			u4Pos += kalSnprintf(
				aucBuf + u4Pos,
				u4BufferSize - u4Pos,
...
static void mt6653MbuDumpDebugCr(struct GLUE_INFO *prGlueInfo)
{
	...
	mt6653MbuDebugReadCr(prAdapter, 0x70025010, &u4Val, fgIsDumpViaBT);
	u4Val |= BIT(24);
	mt6653MbuDebugWriteCr(prAdapter, 0x70025010, u4Val, fgIsDumpViaBT);

	mt6653MbuDumpDebugCrAry(prAdapter, au4DbgCr1, ARRAY_SIZE(au4DbgCr1),
				aucBuf, u4BufferSize, fgIsDumpViaBT);
	...
	kalMdelay(3);
	mt6653MbuDebugReadCr(prAdapter, 0x74130200, &u4Val, fgIsDumpViaBT);
...
/*
 * mt6653DumpDPDHwTransLog
 * Steps (per untitled.txt):
 * 1. Set SLR(0x700130A0) = 1 to stop logging
 * 2. Dump 0x7413D010 ~ 0x7413D01C
 *    - CMD_PTR LEL for AP2CONN: 0x7413D010
 *    - DATA_PTR LEL for AP2CONN: 0x7413D014
 *    - CMD_PTR LEL for CONN2AP: 0x7413D018
 *    - DATA_PTR LEL for CONN2AP: 0x7413D01C
 * 3. Dump CB_INFRA_HW_TRANS_LOG 0x74000000 ~ 0x74000300
 *    (AP2CONN/CONN2AP COMMEM/DATAMEM bases for reference)
 *    #define CB_INFRA_HW_TRANS_LOG_AP2CONN_COMMEM_A_BASE (0x74000000)
 *    #define CB_INFRA_HW_TRANS_LOG_AP2CONN_COMMEM_B_BASE (0x74000080)
 *    #define CB_INFRA_HW_TRANS_LOG_AP2CONN_DATAMEM_BASE  (0x74000100)
 *    #define CB_INFRA_HW_TRANS_LOG_CONN2AP_COMMEM_A_BASE (0x74000180)
 *    #define CB_INFRA_HW_TRANS_LOG_CONN2AP_COMMEM_B_BASE (0x74000200)
 *    #define CB_INFRA_HW_TRANS_LOG_CONN2AP_DATAMEM_BASE  (0x74000280)
 * 4. Set SLR(0x700130A0) = 0 to start logging
 */
static void mt6653DumpDPDHwTransLog(struct ADAPTER *ad)
{
	...
	if (!ad) {
		DBGLOG(HAL, ERROR, "mt6653DumpHwTransLog: NULL ADAPTER\n");
		return;
	}

	/* 1. Set SLR(0x700130A0) = 1 to stop logging */
	HAL_MCR_WR(ad, 0x700130A0, 0x1);
	DBGLOG(HAL, INFO, "SLR[0x700130A0] <- 0x1 (stop logging)\n");

	/* 2. Dump 0x7413D010 ~ 0x7413D01C */
	u4Addr = 0x7413D010;
	HAL_MCR_EMI_RD(ad, u4Addr, &u4Val, &fgRet);
	DBGLOG(HAL, INFO, "[CMD_PTR LEL AP2CONN] [0x%08x]=[0x%08x]\n",
		u4Addr, u4Val);

	u4Addr = 0x7413D014;
	HAL_MCR_EMI_RD(ad, u4Addr, &u4Val, &fgRet);
	DBGLOG(HAL, INFO, "[DATA_PTR LEL AP2CONN] [0x%08x]=[0x%08x]\n",
		u4Addr, u4Val);

	u4Addr = 0x7413D018;
	HAL_MCR_EMI_RD(ad, u4Addr, &u4Val, &fgRet);
	DBGLOG(HAL, INFO, "[CMD_PTR LEL CONN2AP] [0x%08x]=[0x%08x]\n",
		u4Addr, u4Val);

	u4Addr = 0x7413D01C;
	HAL_MCR_EMI_RD(ad, u4Addr, &u4Val, &fgRet);
	DBGLOG(HAL, INFO, "[DATA_PTR LEL CONN2AP] [0x%08x]=[0x%08x]\n",
		u4Addr, u4Val);

	/* 3. Dump CB_INFRA_HW_TRANS_LOG 0x74000000 ~ 0x74000300 */
	DBGLOG(HAL, INFO,
		"Dump CB_INFRA_HW_TRANS_LOG [0x74000000 ~ 0x74000300]\n");
	for (u4Addr = 0x74000000; u4Addr < 0x74000300; u4Addr += 16) {
		uint32_t v0 = 0, v1 = 0, v2 = 0, v3 = 0;

		HAL_MCR_EMI_RD(ad, u4Addr + 0x0, &v0, &fgRet);
		HAL_MCR_EMI_RD(ad, u4Addr + 0x4, &v1, &fgRet);
		HAL_MCR_EMI_RD(ad, u4Addr + 0x8, &v2, &fgRet);
		HAL_MCR_EMI_RD(ad, u4Addr + 0xC, &v3, &fgRet);

		DBGLOG(HAL, INFO, "[0x%08x] %08x %08x %08x %08x\n",
		       u4Addr, v0, v1, v2, v3);
	}

	/* 4. Set SLR(0x700130A0) = 0 to start logging */
	HAL_MCR_WR(ad, 0x700130A0, 0x0);
	DBGLOG(HAL, INFO, "SLR[0x700130A0] <- 0x0 (start logging)\n");
}

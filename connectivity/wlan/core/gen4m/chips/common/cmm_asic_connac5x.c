...
********************************************************************************
*/
#define CONNAC5X_HIF_DMASHDL_BASE 0x52000000
...
uint32_t g_au4UsbPollAddrTbl[] = {
	CONNAC5X_U3D_RX4CSR0,
	CONNAC5X_U3D_RX5CSR0,
	CONNAC5X_U3D_RX6CSR0,
	CONNAC5X_U3D_RX7CSR0,
	CONNAC5X_U3D_RX8CSR0,
	CONNAC5X_U3D_RX9CSR0,
	CONNAC5X_UDMA_WLCFG_0,
	CONNAC5X_UDMA_WL_TX_SCH_ADDR,
	CONNAC5X_UDMA_AR_CMD_FIFO_ADDR,
	WF_WFDMA_HOST_DMA0_WPDMA_GLO_CFG_EXT2_CSR_TX_DROP_MODE_TEST_ADDR,
	WF_WFDMA_EXT_WRAP_CSR_WFDMA_HIF_MISC_HIF_BUSY_ADDR,
	CONNAC5X_UDMA_WL_STOP_DP_OUT_ADDR
};
uint32_t g_au4UsbPollMaskTbl[] = {
	CONNAC5X_U3D_RX_FIFOEMPTY,
	CONNAC5X_U3D_RX_FIFOEMPTY,
	CONNAC5X_U3D_RX_FIFOEMPTY,
	CONNAC5X_U3D_RX_FIFOEMPTY,
	CONNAC5X_U3D_RX_FIFOEMPTY,
	CONNAC5X_U3D_RX_FIFOEMPTY,
	CONNAC5X_UDMA_WLCFG_0_WL_TX_BUSY_MASK,
	CONNAC5X_UDMA_WL_TX_SCH_MASK,
	CONNAC5X_UDMA_AR_CMD_FIFO_MASK,
	WF_WFDMA_HOST_DMA0_WPDMA_GLO_CFG_EXT2_CSR_TX_DROP_MODE_TEST_MASK,
	WF_WFDMA_EXT_WRAP_CSR_WFDMA_HIF_MISC_HIF_BUSY_MASK,
	CONNAC5X_UDMA_WL_STOP_DP_OUT_DROP_MASK
};
	...
	struct ADAPTER *prAdapter)
{
	...
	prChipInfo->u2CmdTxHdrSize = sizeof(struct CONNAC5X_WIFI_CMD);
	prChipInfo->asicFillInitCmdTxd = asicConnac5xFillInitCmdTxd;
	prChipInfo->asicFillCmdTxd = asicConnac5xFillCmdTxd;
#ifdef CFG_SUPPORT_UNIFIED_COMMAND
	prChipInfo->u2UniCmdTxHdrSize = sizeof(struct CONNAC5X_WIFI_UNI_CMD);
	prChipInfo->asicFillUniCmdTxd = asicConnac5xFillUniCmdTxd;
#endif
	prChipInfo->u2RxSwPktBitMap = CONNAC5X_RX_STATUS_PKT_TYPE_SW_BITMAP;
	prChipInfo->u2RxSwPktEvent = CONNAC5X_RX_STATUS_PKT_TYPE_SW_EVENT;
	prChipInfo->u2RxSwPktFrame = CONNAC5X_RX_STATUS_PKT_TYPE_SW_FRAME;
	prChipInfo->u4ExtraTxByteCount = 0;
	asicConnac5xInitTxdHook(prAdapter, prChipInfo->prTxDescOps);
	asicConnac5xInitRxdHook(prAdapter, prChipInfo->prRxDescOps);
#if (CFG_SUPPORT_MSP == 1)
	prChipInfo->asicRxProcessRxvforMSP = asicConnac5xRxProcessRxvforMSP;
#endif /* CFG_SUPPORT_MSP == 1 */
	prChipInfo->asicRxGetRcpiValueFromRxv =
			asicConnac5xRxGetRcpiValueFromRxv;
	prChipInfo->asicRxGetRxModeValueFromRxv =
			asicConnac5xRxGetRxModeValueFromRxv;
#if (CFG_SUPPORT_PERF_IND == 1)
	prChipInfo->asicRxPerfIndProcessRXV = asicConnac5xRxPerfIndProcessRXV;
#endif
	...
	case MT_DEV_INF_AXI:
		...
		prChipInfo->u4HifDmaShdlBaseAddr = CONNAC5X_HIF_DMASHDL_BASE;

		if (prBusInfo && prBusInfo->lowPowerOwnInit)
			prBusInfo->lowPowerOwnInit(prAdapter);
		else
			HAL_MCR_WR(prAdapter,
				   CONNAC5X_BN0_IRQ_ENA_ADDR,
				   BIT(0));

		if (prChipInfo->is_support_asic_lp) {
			HAL_MCR_WR(prAdapter,
				CONNAC5X_WPDMA_MCU2HOST_SW_INT_MASK
					(u4HostWpdamBase),
				BITS(0, 15));
		}
		break;
	...
	case MT_DEV_INF_USB:
		...
		prChipInfo->asicEnableFWDownload = asicConnac5xEnableUsbFWDL;
...
static void asicConnac5xFillInitCmdTxdInfo(
	struct ADAPTER *prAdapter,
	struct WIFI_CMD_INFO *prCmdInfo,
	u_int8_t *pucSeqNum)
{
	struct INIT_HIF_TX_HEADER *prInitHifTxHeader;
	struct HW_MAC_CONNAC5X_TX_DESC *prInitHifTxHeaderPadding;
	uint32_t u4TxdLen = sizeof(struct HW_MAC_CONNAC5X_TX_DESC);

	prInitHifTxHeaderPadding =
		(struct HW_MAC_CONNAC5X_TX_DESC *) (prCmdInfo->pucInfoBuffer);
	prInitHifTxHeader = (struct INIT_HIF_TX_HEADER *)
		(prCmdInfo->pucInfoBuffer + u4TxdLen);

	HAL_MAC_CONNAC5X_TXD_SET_TX_BYTE_COUNT(prInitHifTxHeaderPadding,
		prCmdInfo->u2InfoBufLen);
	if (!prCmdInfo->ucCID)
		HAL_MAC_CONNAC5X_TXD_SET_PKT_FORMAT(prInitHifTxHeaderPadding,
						INIT_PKT_FT_PDA_FWDL)
	else
		HAL_MAC_CONNAC5X_TXD_SET_PKT_FORMAT(prInitHifTxHeaderPadding,
						INIT_PKT_FT_CMD)
	HAL_MAC_CONNAC5X_TXD_SET_HEADER_FORMAT(prInitHifTxHeaderPadding,
						HEADER_FORMAT_COMMAND);
	HAL_MAC_CONNAC5X_TXD_SET_QUEUE_INDEX(prInitHifTxHeaderPadding, 0);
...
static void asicConnac5xFillCmdTxdInfo(
	struct ADAPTER *prAdapter,
	struct WIFI_CMD_INFO *prCmdInfo,
	u_int8_t *pucSeqNum)
{
	struct CONNAC5X_WIFI_CMD *prWifiCmd;
	uint32_t u4TxdLen = sizeof(struct HW_MAC_CONNAC5X_TX_DESC);

	prWifiCmd = (struct CONNAC5X_WIFI_CMD *)prCmdInfo->pucInfoBuffer;

	HAL_MAC_CONNAC5X_TXD_SET_TX_BYTE_COUNT(
		(struct HW_MAC_CONNAC5X_TX_DESC *)prWifiCmd,
		prCmdInfo->u2InfoBufLen);
	HAL_MAC_CONNAC5X_TXD_SET_PKT_FORMAT(
		(struct HW_MAC_CONNAC5X_TX_DESC *)prWifiCmd,
		INIT_PKT_FT_CMD);
	HAL_MAC_CONNAC5X_TXD_SET_HEADER_FORMAT(
		(struct HW_MAC_CONNAC5X_TX_DESC *)prWifiCmd,
		HEADER_FORMAT_COMMAND);
	HAL_MAC_CONNAC5X_TXD_SET_QUEUE_INDEX(
		(struct HW_MAC_CONNAC5X_TX_DESC *)prWifiCmd, 0);
...
#ifdef CFG_SUPPORT_UNIFIED_COMMAND
static void asicConnac5xFillUniCmdTxdInfo(
	struct ADAPTER *prAdapter,
	struct WIFI_UNI_CMD_INFO *prCmdInfo,
	u_int8_t *pucSeqNum)
{
	struct CONNAC5X_WIFI_UNI_CMD *prWifiCmd;
	uint32_t u4TxdLen = sizeof(struct HW_MAC_CONNAC5X_TX_DESC);

	prWifiCmd = (struct CONNAC5X_WIFI_UNI_CMD *)prCmdInfo->pucInfoBuffer;
...
void asicConnac5xFillInitCmdTxd(
	struct ADAPTER *prAdapter,
	struct WIFI_CMD_INFO *prCmdInfo,
	...
	u_int8_t *pucSeqNum, void **pCmdBuf)
{
	...
	if (!prCmdInfo->ucCID && pCmdBuf)
		*pCmdBuf = prCmdInfo->pucInfoBuffer;
		...
		prInitHifTxHeader = (struct INIT_HIF_TX_HEADER *)
			(prCmdInfo->pucInfoBuffer + u4TxdLen);
		asicConnac5xFillInitCmdTxdInfo(
			prAdapter,
			prCmdInfo,
			pucSeqNum);
		if (pCmdBuf)
...
u_int8_t asicConnac5xWfdmaDummyCrRead(
	struct ADAPTER *prAdapter,
	uint32_t *pu4Value)
{
	u_int32_t u4Addr = CONNAC5X_WFDMA_DUMMY_CR;
	...
	return (*pu4Value & CONNAC5X_WFDMA_NEED_REINIT_BIT) == 0 ? TRUE : FALSE;
}


void asicConnac5xWfdmaDummyCrWrite(
	struct ADAPTER *prAdapter)
{
#if !CFG_MTK_WIFI_WFDMA_TX_RING_BK_RS
	u_int32_t u4RegValue = 0;

	asicConnac5xWfdmaDummyCrRead(prAdapter, &u4RegValue);
	u4RegValue |= CONNAC5X_WFDMA_NEED_REINIT_BIT;

	HAL_MCR_WR(prAdapter,
		CONNAC5X_WFDMA_DUMMY_CR,
		u4RegValue);
#endif /* CFG_MTK_WIFI_WFDMA_BK_RS */
}

u_int8_t asicConnac5xWfdmaIsNeedReInit(
	struct ADAPTER *prAdapter)
{
	struct mt66xx_chip_info *prChipInfo;
	u_int32_t u4RegValue = 0;
	...
	fgNeedReInit = asicConnac5xWfdmaDummyCrRead(prAdapter, &u4RegValue);

	return fgNeedReInit;
}

static void asicConnac5xWfdmaReInitImpl(struct ADAPTER *prAdapter)
{
#if defined(_HIF_PCIE) || defined(_HIF_AXI)
	...
	struct ADAPTER *prAdapter)
{
	...
	fgResult = asicConnac5xWfdmaDummyCrRead(prAdapter, &u4Val);
	if (fgResult) {
		asicConnac5xWfdmaReInitImpl(prAdapter);
		asicConnac5xWfdmaDummyCrWrite(prAdapter);
	}
#endif /* CFG_MTK_WIFI_WFDMA_TX_RING_BK_RS */
}

void asicConnac5xFillCmdTxd(
	struct ADAPTER *prAdapter,
	struct WIFI_CMD_INFO *prCmdInfo,
	u_int8_t *pucSeqNum,
	void **pCmdBuf)
{
	...
	prWifiCmd = (struct CONNAC5X_WIFI_CMD *)prCmdInfo->pucInfoBuffer;
	asicConnac5xFillCmdTxdInfo(prAdapter, prCmdInfo, pucSeqNum);
	if (pCmdBuf)
		*pCmdBuf = &prWifiCmd->aucBuffer[0];
}

#ifdef CFG_SUPPORT_UNIFIED_COMMAND
void asicConnac5xFillUniCmdTxd(
	struct ADAPTER *prAdapter,
	struct WIFI_UNI_CMD_INFO *prCmdInfo,
	u_int8_t *pucSeqNum,
	void **pCmdBuf)
{
	...
	/* 2. Setup common CMD Info Packet */
	prWifiCmd = (struct CONNAC5X_WIFI_UNI_CMD *)prCmdInfo->pucInfoBuffer;
	asicConnac5xFillUniCmdTxdInfo(prAdapter, prCmdInfo, pucSeqNum);
	if (pCmdBuf)
		*pCmdBuf = &prWifiCmd->aucBuffer[0];
}
#endif
...
#if defined(_HIF_PCIE) || defined(_HIF_AXI)
uint32_t asicConnac5xWfdmaCfgAddrGet(
	struct GLUE_INFO *prGlueInfo,
	u_int8_t ucDmaIdx)
{
	...
	if (ucDmaIdx == 0)
		return CONNAC5X_WPDMA_GLO_CFG(prBusInfo->host_dma0_base);
	else
		return CONNAC5X_WPDMA_GLO_CFG(prBusInfo->host_dma1_base);
}

uint32_t asicConnac5xWfdmaIntRstDtxPtrAddrGet(
	struct GLUE_INFO *prGlueInfo,
	u_int8_t ucDmaIdx)
{
	struct BUS_INFO *prBusInfo = prGlueInfo->prAdapter->chip_info->bus_info;
	...
	if (ucDmaIdx == 0)
		return CONNAC5X_WPDMA_RST_DTX_PTR(prBusInfo->host_dma0_base);
	else
		return CONNAC5X_WPDMA_RST_DTX_PTR(prBusInfo->host_dma1_base);
}

uint32_t asicConnac5xWfdmaIntRstDrxPtrAddrGet(
	struct GLUE_INFO *prGlueInfo,
	u_int8_t ucDmaIdx)
{
	struct BUS_INFO *prBusInfo = prGlueInfo->prAdapter->chip_info->bus_info;
	...
	if (ucDmaIdx == 0)
		return CONNAC5X_WPDMA_RST_DRX_PTR(prBusInfo->host_dma0_base);
	else
		return CONNAC5X_WPDMA_RST_DRX_PTR(prBusInfo->host_dma1_base);
}

uint32_t asicConnac5xWfdmaHifRstAddrGet(
	struct GLUE_INFO *prGlueInfo,
	u_int8_t ucDmaIdx)
{
	struct BUS_INFO *prBusInfo = prGlueInfo->prAdapter->chip_info->bus_info;
	...
	if (ucDmaIdx == 0)
		return CONNAC5X_WPDMA_HIF_RST(prBusInfo->host_dma0_base);
	else
		return CONNAC5X_WPDMA_HIF_RST(prBusInfo->host_dma1_base);
}

void asicConnac5xWfdmaStop(struct GLUE_INFO *prGlueInfo, u_int8_t enable)
{
	struct ADAPTER *prAdapter = prGlueInfo->prAdapter;
	union WPDMA_GLO_CFG_STRUCT GloCfg;
	uint32_t u4DmaCfgCr;

	u4DmaCfgCr = asicConnac5xWfdmaCfgAddrGet(prGlueInfo, 0);
...
u_int8_t asicConnac5xWfdmaPollingAllIdle(struct GLUE_INFO *prGlueInfo)
{
	if (!asicConnac5xWfdmaWaitIdle(prGlueInfo, 0, 100, 1000)) {
		DBGLOG(HAL, WARN,
	...
	u_int8_t enable)
{
	...
	ASSERT(ucDmaIdx < CONNAC5X_MAX_WFDMA_COUNT);
	u4DmaCfgCr = asicConnac5xWfdmaCfgAddrGet(prGlueInfo, ucDmaIdx);
	...
	if (!enable) {
		...
		u4DmaRstDtxPtrCr = asicConnac5xWfdmaIntRstDtxPtrAddrGet(
			prGlueInfo, ucDmaIdx);
		u4DmaRstDrxPtrCr = asicConnac5xWfdmaIntRstDrxPtrAddrGet(
			prGlueInfo, ucDmaIdx);

		asicConnac5xWfdmaWaitIdle(prGlueInfo, ucDmaIdx, 100, 1000);
		/* Reset DMA Index */
...
u_int8_t asicConnac5xWfdmaWaitIdle(
	struct GLUE_INFO *prGlueInfo,
	u_int8_t index,
	uint32_t round,
	uint32_t wait_us)
{
	...
	if (index == 0 || index == 1)
		u4RegAddr = asicConnac5xWfdmaCfgAddrGet(prGlueInfo, index);
	else {
...
void asicConnac5xWfdmaTxRingBasePtrExtCtrl(
	struct GLUE_INFO *prGlueInfo,
...
void asicConnac5xWfdmaRxRingBasePtrExtCtrl(
	struct GLUE_INFO *prGlueInfo,
...
void asicConnac5xWfdmaTxRingExtCtrl(
	struct GLUE_INFO *prGlueInfo,
	struct RTMP_TX_RING *tx_ring,
	u_int32_t index)
{
...
void asicConnac5xWfdmaRxRingExtCtrl(
	struct GLUE_INFO *prGlueInfo,
	struct RTMP_RX_RING *rx_ring,
	u_int32_t index)
{
...
void asicConnac5xAllocWfdmaWbBuffer(struct GLUE_INFO *prGlueInfo,
				    bool fgAllocMem)
{
	struct GL_HIF_INFO *prHifInfo;
	struct mt66xx_chip_info *prChipInfo;
...
void asicConnac5xFreeWfdmaWbBuffer(struct GLUE_INFO *prGlueInfo)
{
	struct GL_HIF_INFO *prHifInfo;
	struct mt66xx_chip_info *prChipInfo;
	struct HIF_MEM_OPS *prMemOps;
...
void asicConnac5xEnablePlatformIRQ(struct ADAPTER *prAdapter)
{
...
void asicConnac5xDisablePlatformIRQ(struct ADAPTER *prAdapter)
{
	struct GL_HIF_INFO *prHifInfo = NULL;
	struct mt66xx_chip_info *prChipInfo;
	struct WIFI_VAR *prWifiVar;
...
#if defined(_HIF_AXI)
void asicConnac5xDisablePlatformSwIRQ(struct ADAPTER *prAdapter)
{
	struct GL_HIF_INFO *prHifInfo = NULL;
...
void asicConnac5xLowPowerOwnRead(
	struct ADAPTER *prAdapter,
	u_int8_t *pfgResult)
{
	struct mt66xx_chip_info *prChipInfo;
...
void asicConnac5xLowPowerOwnSet(
	struct ADAPTER *prAdapter,
	u_int8_t *pfgResult)
{
	struct mt66xx_chip_info *prChipInfo;
...
void asicConnac5xLowPowerOwnClear(
	struct ADAPTER *prAdapter,
	u_int8_t *pfgResult)
{
	struct mt66xx_chip_info *prChipInfo;
...
void asicConnac5xProcessSoftwareInterrupt(
	struct ADAPTER *prAdapter)
{
	struct GLUE_INFO *prGlueInfo;
	struct GL_HIF_INFO *prHifInfo;
	...
	if (prMem && prMem->va) {
		...
		u4Addr = CONNAC5X_WPDMA_MCU2HOST_SW_INT_STA(u4HostWpdamBase);
...
void asicConnac5xSoftwareInterruptMcu(
...
uint32_t asicConnac5xGetMdSoftwareInterruptStatus(
	struct ADAPTER *prAdapter)
{
	struct GLUE_INFO *prGlueInfo;
	struct GL_HIF_INFO *prHifInfo;
	...
	u4Addr = CONNAC5X_WPDMA_MCU2MD_SW_INT_STA(u4HostWpdamBase);
	HAL_MCR_EMI_RD(prAdapter, u4Addr, &u4Status, &fgRet);
	if (!fgRet)
		HAL_RMCR_RD(SER_READ, prAdapter, u4Addr, &u4Status);
...
void asicConnac5xHifRst(
	struct GLUE_INFO *prGlueInfo)
{
	uint32_t u4HifRstCr;

	u4HifRstCr = asicConnac5xWfdmaHifRstAddrGet(prGlueInfo, 0);
	/* Reset dmashdl and wpdma */
	kalDevRegWrite(prGlueInfo, u4HifRstCr, 0x00000000);
	kalDevRegWrite(prGlueInfo, u4HifRstCr, 0x00000030);

	u4HifRstCr = asicConnac5xWfdmaHifRstAddrGet(prGlueInfo, 1);
	/* Reset dmashdl and wpdma */
	kalDevRegWrite(prGlueInfo, u4HifRstCr, 0x00000000);
	kalDevRegWrite(prGlueInfo, u4HifRstCr, 0x00000030);
}
 ...
 */
void asicConnac5xWfdmaInitForUSB(
	struct ADAPTER *prAdapter,
	struct mt66xx_chip_info *prChipInfo)
{
	struct BUS_INFO *prBusInfo;
	...
	u4WfdmaAddr = CONNAC5X_TX_RING_EXT_CTRL_BASE(
	...
	for (idx = 0; idx < USB_TX_EPOUT_NUM; idx++) {
		...
		u4WfdmaCr &= ~CONNAC5X_WFDMA_DISP_MAX_CNT_MASK;
		u4WfdmaCr |= CONNAC5X_TX_RING_DISP_MAX_CNT;
		u4WfdmaCr &= ~CONNAC5X_WFDMA_DISP_BASE_PTR_MASK;
	...
	u4WfdmaAddr = CONNAC5X_WPDMA_GLO_CFG(
		prChipInfo->u4HostWfdmaBaseAddr);
	HAL_RMCR_RD(HIF_USB, prAdapter, u4WfdmaAddr, &u4WfdmaCr);
	u4WfdmaCr &= ~(CONNAC5X_WPDMA1_GLO_CFG_OMIT_RX_INFO);
	u4WfdmaCr |=
		(CONNAC5X_WPDMA1_GLO_CFG_OMIT_TX_INFO |
		 CONNAC5X_WPDMA1_GLO_CFG_OMIT_RX_INFO_PFET2 |
		 CONNAC5X_WPDMA1_GLO_CFG_FW_DWLD_Bypass_dmashdl |
		 CONNAC5X_WPDMA1_GLO_CFG_RX_DMA_EN |
		 CONNAC5X_WPDMA1_GLO_CFG_TX_DMA_EN);
	...
	if (!prChipInfo->is_support_dma_shdl) {
		 ...
		 */
		u4WfdmaAddr = CONNAC5X_WPDMA_GLO_CFG_EXT0(
			prChipInfo->u4HostWfdmaBaseAddr);
		HAL_RMCR_RD(HIF_USB, prAdapter, u4WfdmaAddr, &u4WfdmaCr);
		u4WfdmaCr &= ~CONNAC5X_WPDMA1_GLO_CFG_EXT0_TX_DMASHDL_EN;
		HAL_MCR_WR(prAdapter, u4WfdmaAddr, u4WfdmaCr);
		 ...
		 *	1: Enable
		 */
		u4WfdmaAddr = CONNAC5X_HOST_DMASHDL_SW_CONTROL(
			prChipInfo->u4DmaShdlBaseAddr);
		HAL_RMCR_RD(HIF_USB, prAdapter, u4WfdmaAddr, &u4WfdmaCr);
		u4WfdmaCr |= CONNAC5X_HIF_DMASHDL_BYPASS_EN;
		HAL_MCR_WR(prAdapter, u4WfdmaAddr, u4WfdmaCr);
	}
...
uint8_t asicConnac5xUsbEventEpDetected(struct ADAPTER *prAdapter)
{
	return USB_DATA_EP_IN;
}

#if CFG_ENABLE_FW_DOWNLOAD
void asicConnac5xEnableUsbFWDL(
	struct ADAPTER *prAdapter,
	u_int8_t fgEnable)
{
	struct GLUE_INFO *prGlueInfo;
...
u_int8_t asicConnac5xUsbResume(
	struct ADAPTER *prAdapter,
	struct GLUE_INFO *prGlueInfo)
{
	uint8_t count = 0;
	...
	if (asicConnac5xWfdmaIsNeedReInit(prAdapter)) {
		DBGLOG(INIT, DEBUG,
...
uint16_t asicConnac5xUsbRxByteCount(
	struct ADAPTER *prAdapter,
	struct BUS_INFO *prBusInfo,
	uint8_t *pRXD)
{
	...
	ucPacketType = HAL_MAC_CONNAC5X_RX_STATUS_GET_PKT_TYPE(
		(struct HW_MAC_CONNAC5X_RX_DESC *)pRXD);
	u2RxByteCount = HAL_MAC_CONNAC5X_RX_STATUS_GET_RX_BYTE_CNT(
		(struct HW_MAC_CONNAC5X_RX_DESC *)pRXD);
...
#if (CFG_SUPPORT_HOST_OFFLOAD == 1)
void fillConnac5xNicTxDescAppendWithSdo(
	struct ADAPTER *prAdapter,
...
void fillConnac5xNicTxDescAppendWithSdoV2(
	struct ADAPTER *prAdapter,
	struct MSDU_INFO *prMsduInfo,
	uint8_t *prTxDescBuffer)
{
...
void fillConnac5xTxDescAppendBySdo(
	struct ADAPTER *prAdapter,
	struct MSDU_INFO *prMsduInfo,
	uint16_t u4MsduId,
...
void fillConnac5xTxDescAppendByMawdSdo(
	struct ADAPTER *prAdapter,
	struct MSDU_INFO *prMsduInfo,
	uint16_t u4MsduId,
	dma_addr_t rDmaAddr,
	...
	uint8_t *pucBuffer)
{
	...
	prTxDesc = (struct HW_MAC_CONNAC5X_TX_DESC *) pTxCell->AllocVa;
#if CFG_ENABLE_MAWD_MD_RING
	HAL_MAC_CONNAC5X_TXD_SET_PACKET_SOURCE(prTxDesc, 1);
...
void fillConnac5xTxDescTxByteCountWithSdo(
	struct ADAPTER *prAdapter,
	struct MSDU_INFO *prMsduInfo,
	void *prTxDesc)
{
	...
	HAL_MAC_CONNAC5X_TXD_SET_HIF_VERSION(
		(struct HW_MAC_CONNAC5X_TX_DESC *)prTxDesc, 1);

	/* Calculate Tx byte count */
	HAL_MAC_CONNAC5X_TXD_SET_TX_BYTE_COUNT(
		(struct HW_MAC_CONNAC5X_TX_DESC *)prTxDesc, u4TxByteCount);
}
#endif /* CFG_SUPPORT_HOST_OFFLOAD == 1 */
...
void fillConnac5xTxDescTxByteCount(
	struct ADAPTER *prAdapter,
	struct MSDU_INFO *prMsduInfo,
	void *prTxDesc)
{
	...
	struct TX_DESC_OPS_T *prTxDescOps)
{
	...
	prTxDescOps->nic_txd_long_format_op = nic_txd_v5_long_format_op;
	prTxDescOps->nic_txd_tid_op = nic_txd_v5_tid_op;
	prTxDescOps->nic_txd_pkt_format_op = nic_txd_v5_pkt_format_op;
	prTxDescOps->nic_txd_queue_idx_op = nic_txd_v5_queue_idx_op;
#if (CFG_TCP_IP_CHKSUM_OFFLOAD == 1)
	prTxDescOps->nic_txd_chksum_op = nic_txd_v5_chksum_op;
#endif /* CFG_TCP_IP_CHKSUM_OFFLOAD == 1 */
	prTxDescOps->nic_txd_header_format_op = nic_txd_v5_header_format_op;
	prTxDescOps->nic_txd_fill_by_pkt_option = nic_txd_v5_fill_by_pkt_option;
	prTxDescOps->nic_txd_compose = nic_txd_v5_compose;
	prTxDescOps->nic_txd_set_pkt_fixed_rate_option =
		nic_txd_v5_set_pkt_fixed_rate_option;
	prTxDescOps->nic_txd_set_hw_amsdu_template =
		nic_txd_v5_set_hw_amsdu_template;
	...
	if (IS_FEATURE_ENABLED(prWifiVar->fgEnableSdo)) {
		...
		prTxDescOps->fillHifAppend =
			fillConnac5xTxDescAppendBySdo;
		prTxDescOps->fillTxByteCount =
			fillConnac5xTxDescTxByteCountWithSdo;
	}
	...
	if (IS_FEATURE_ENABLED(prWifiVar->fgEnableMawdTx)) {
		...
		if (prTxDescOps->fillNicSdoAppend)
			prTxDescOps->fillNicAppend =
				prTxDescOps->fillNicSdoAppend;
		prTxDescOps->fillHifAppend =
			fillConnac5xTxDescAppendByMawdSdo;
		prTxDescOps->fillTxByteCount =
			fillConnac5xTxDescTxByteCountWithSdo;
	}
#endif
...
void asicConnac5xInitRxdHook(
	struct ADAPTER *prAdapter,
	struct RX_DESC_OPS_T *prRxDescOps)
{
	ASSERT(prRxDescOps);
	prRxDescOps->nic_rxd_get_rx_byte_count = nic_rxd_v5_get_rx_byte_count;
	prRxDescOps->nic_rxd_get_pkt_type = nic_rxd_v5_get_packet_type;
	prRxDescOps->nic_rxd_get_wlan_idx = nic_rxd_v5_get_wlan_idx;
	prRxDescOps->nic_rxd_get_sec_mode = nic_rxd_v5_get_sec_mode;
	prRxDescOps->nic_rxd_get_sw_class_error_bit =
		nic_rxd_v5_get_sw_class_error_bit;
	prRxDescOps->nic_rxd_get_ch_num = nic_rxd_v5_get_ch_num;
	prRxDescOps->nic_rxd_get_rf_band = nic_rxd_v5_get_rf_band;
	prRxDescOps->nic_rxd_get_tcl = nic_rxd_v5_get_tcl;
	prRxDescOps->nic_rxd_get_ofld = nic_rxd_v5_get_ofld;
	prRxDescOps->nic_rxd_get_HdrTrans = nic_rxd_v5_get_HdrTrans;
	prRxDescOps->nic_rxd_fill_rfb = nic_rxd_v5_fill_rfb;
	prRxDescOps->nic_rxd_sanity_check = nic_rxd_v5_sanity_check;
#if CFG_SUPPORT_WAKEUP_REASON_DEBUG
	prRxDescOps->nic_rxd_check_wakeup_reason =
		nic_rxd_v5_check_wakeup_reason;
#endif
#ifdef CFG_SUPPORT_SNIFFER_RADIOTAP
	prRxDescOps->nic_rxd_fill_radiotap = nic_rxd_v5_fill_radiotap;
#endif
	prRxDescOps->nic_rxd_handle_host_rpt =
		nic_rxd_v5_handle_host_rpt;
}

#if (CFG_SUPPORT_MSP == 1)
void asicConnac5xRxProcessRxvforMSP(struct ADAPTER *prAdapter,
	  struct SW_RFB *prRetSwRfb)
{
	...
	if (prRetSwRfb->ucGroupVLD & BIT(RX_GROUP_VLD_3)) {
		...
		prRxV[0] = CONNAC5X_HAL_RX_VECTOR_GET_RX_VECTOR(prGroup3, 0);

		/* P-RXV0[32:63] in RXD Group3 */
		prRxV[1] = CONNAC5X_HAL_RX_VECTOR_GET_RX_VECTOR(prGroup3, 1);
...
uint8_t asicConnac5xRxGetRcpiValueFromRxv(
	uint8_t ucRcpiMode,
	struct SW_RFB *prSwRfb)
{
	...
	ucRcpi0 = CONNAC5X_HAL_RX_VECTOR_GET_RCPI0(prGroup3);
	ucRcpi1 = CONNAC5X_HAL_RX_VECTOR_GET_RCPI1(prGroup3);
	ucRcpi2 = CONNAC5X_HAL_RX_VECTOR_GET_RCPI2(prGroup3);
	ucRcpi3 = CONNAC5X_HAL_RX_VECTOR_GET_RCPI3(prGroup3);
...
uint8_t asicConnac5xRxGetRxModeValueFromRxv(struct SW_RFB *prSwRfb)
{
	...
	prGroup3 = (struct HW_MAC_RX_STS_GROUP_3_V2 *)
				prSwRfb->prRxStatusGroup3;
	ucRxModeValue = CONNAC5X_HAL_RX_VECTOR_GET_RX_MODE(prGroup3);

	return ucRxModeValue;
}

#if (CFG_SUPPORT_PERF_IND == 1)
void asicConnac5xRxPerfIndProcessRXV(struct ADAPTER *prAdapter,
			       struct SW_RFB *prSwRfb,
			       uint8_t ucBssIndex)
{
	struct GLUE_INFO *prGlueInfo;
...
void asicConnac5xDmashdlLiteSetTotalPlePsePageSize(
	struct ADAPTER *prAdapter,
...
void asicConnac5xDmashdlLiteSetQueueMapping(
...
void asicConnac5xDmashdlSetPlePsePktMaxPage(
	struct ADAPTER *prAdapter,
...
void asicConnac5xDmashdlSetRefill(struct ADAPTER *prAdapter, uint8_t ucGroup,
			       u_int8_t fgEnable)
{
	struct BUS_INFO *prBusInfo;
	struct DMASHDL_CFG *prCfg;
...
void asicConnac5xDmashdlSetMinMaxQuota(
...
void asicConnac5xDmashdlSetQueueMapping(
	struct ADAPTER *prAdapter, uint8_t ucQueue, uint8_t ucGroup)
{
	struct BUS_INFO *prBusInfo;
	struct DMASHDL_CFG *prCfg;
...
void asicConnac5xDmashdlSetUserDefinedPriority(
...
void asicConnac5xDmashdlSetSlotArbiter(
...
void asicConnac5xDmashdlSetOptionalControl(
	struct ADAPTER *prAdapter,
...
uint32_t asicConnac5xDynamicDmashdlGetInUsedMaxQuota(
	struct ADAPTER *prAdapter,
	uint32_t u4GroupIdx,
...
uint32_t asicConnac5xUpdateDynamicDmashdlQuota(
	struct ADAPTER *prAdapter,
	uint8_t ucWmmIndex,
	uint32_t u4MaxQuota)
{
...
uint32_t asicConnac5xDynamicDmashdlQuotaDecision(
	struct ADAPTER *prAdapter,
	uint8_t ucWmmIndex)
{
	struct mt66xx_chip_info *prChipInfo = prAdapter->chip_info;
...
u_int8_t asicConnac5xSwIntHandler(struct ADAPTER *prAdapter)
{
...
uint32_t asicConnac5xQueryPmicInfo(struct ADAPTER *prAdapter)
{
...
uint32_t asicConnac5xQueryDFDInfo(
...
uint32_t asicConnac5xGetFwVer(struct ADAPTER *prAdapter)
{
	uint32_t u4SwVer = 0;
...
static void register_connv3_cbs(void)
{
	...
	cb.rst_cb.pre_whole_chip_rst = wlan_pre_whole_chip_rst_v5;
	cb.rst_cb.post_whole_chip_rst = wlan_post_whole_chip_rst_v5;
#if CFG_MTK_WIFI_DFD_DUMP_SUPPORT
	cb.rst_cb.post_reset_on = wlan_post_reset_on_v5;
#endif
#endif
...
static void connac5xInitValidMmioReadReasonAry(
	struct mt66xx_chip_info *prChipInfo)
{
...
u_int8_t connac5xIsValidMmioReadReason(
	struct mt66xx_chip_info *prChipInfo, enum HIF_DEV_REG_REASON eReason)
{
	connac5xInitValidMmioReadReasonAry(prChipInfo);
...
u_int8_t connac5xIsNoMmioReadReason(
	struct mt66xx_chip_info *prChipInfo, enum HIF_DEV_REG_REASON eReason)
{
	if (eReason >= HIF_DEV_REG_MAX)
		return FALSE;
...
#if defined(_HIF_PCIE) || defined(_HIF_AXI)
void connac5xClearEvtRingTillCmdRingEmpty(struct ADAPTER *prAdapter)
{
	struct GL_HIF_INFO *prHifInfo;
	struct RTMP_TX_RING *prTxRing;
	uint32_t u4Idx = 0;

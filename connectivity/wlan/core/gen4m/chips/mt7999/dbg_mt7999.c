...
struct PLE_TOP_CR rMt7999PleTopCr = {
	.rAc0QueueEmpty0 = {
...
struct PSE_TOP_CR rMt7999PseTopCr = {
	.rFlQueCtrl0 = {
...
struct PP_TOP_CR rMt7999PpTopCr = {
	.rDbgCtrl = {
...
static void update_mbu_timeout(uint8_t is_timeout)
{
	...
	DBGLOG(INIT, DEBUG, "set timeout:%d\n", fgIsMbuTimeout);
}
...
static void mt7999_dump_debug_sop(struct ADAPTER *prAdapter,
...
#define REG_DUMP_ARRAY_SIZE		(MAX_REG_DUMP_NUM*9+16)
	...
	/* Header */
#if CFG_MTK_WIFI_MBU
	uTimeout = mt7999_get_mbu_timeout_status();
#endif
	DBGLOG(HAL, DEBUG, "[%s][H] [%s][Count: %d][MBUTO:%d]\n",
...
#if CFG_MTK_WIFI_MBU
		if (!mt7999_get_mbu_timeout_status() &&
...
{
	uint32_t u4DbgIdxAddr, u4DbgProbeAddr, u4DbgIdxVal, u4DbgProbeVal = 0;
	...
	for (u4DbgIdxVal = 0x100; u4DbgIdxVal < 0x120; u4DbgIdxVal++) {
		HAL_MCR_WR(prAdapter, u4DbgIdxAddr, u4DbgIdxVal);
		HAL_RMCR_RD(HIF_DBG, prAdapter,
			       u4DbgProbeAddr, &u4DbgProbeVal);
		DBGLOG(HAL, DEBUG, "\t Write(0x%2x) DBG_PROBE[0x%X]=0x%08X\n",
			u4DbgIdxVal, u4DbgProbeAddr, u4DbgProbeVal);
	}

	/* low priority debug CR */
	for (u4DbgIdxVal = 0x120; u4DbgIdxVal <= 0x131; u4DbgIdxVal++) {
		HAL_MCR_WR(prAdapter, u4DbgIdxAddr, u4DbgIdxVal);
		HAL_RMCR_RD(HIF_DBG, prAdapter,
			       u4DbgProbeAddr, &u4DbgProbeVal);
		DBGLOG(HAL, TRACE, "\t Write(0x%2x) DBG_PROBE[0x%X]=0x%08X\n",
			u4DbgIdxVal, u4DbgProbeAddr, u4DbgProbeVal);
	}
}
	...
	enum _ENUM_WFDMA_TYPE_T enum_wfdma_type)
{
	...
	DBGLOG(HAL, DEBUG, "WFDMA_AXI_SLPPROT_CTRL_0(0x%08x): 0x%08x\n",
	       u4DmaCfgCr,
	       u4RegValue);

	u4DmaCfgCr = WFDMA_WRAP_CSR_WFDMA_AXI_SLPPROT_CTRL_1_ADDR;
	HAL_RMCR_RD(HIF_DBG, prAdapter, u4DmaCfgCr, &u4RegValue);
	DBGLOG(HAL, DEBUG, "WFDMA_AXI_SLPPROT_CTRL_1(0x%08x): 0x%08x\n",
	       u4DmaCfgCr,
	       u4RegValue);

	u4DmaCfgCr = WFDMA_WRAP_CSR_WFDMA_SLPPROT_DBG_0_ADDR;
	HAL_RMCR_RD(HIF_DBG, prAdapter, u4DmaCfgCr, &u4RegValue);
	DBGLOG(HAL, DEBUG, "WFDMA_SLPPROT_DBG_0(0x%08x): 0x%08x\n",
	       u4DmaCfgCr,
	       u4RegValue);

	u4DmaCfgCr = WFDMA_WRAP_CSR_WFDMA_SLPPROT_DBG_1_ADDR;
	HAL_RMCR_RD(HIF_DBG, prAdapter, u4DmaCfgCr, &u4RegValue);
	DBGLOG(HAL, DEBUG, "WFDMA_SLPPROT_DBG_1(0x%08x): 0x%08x\n",
	       u4DmaCfgCr,
	       u4RegValue);

	u4DmaCfgCr = WFDMA_WRAP_CSR_WFDMA_SLPPROT_DBG_2_ADDR;
	HAL_RMCR_RD(HIF_DBG, prAdapter, u4DmaCfgCr, &u4RegValue);
	DBGLOG(HAL, DEBUG, "WFDMA_SLPPROT_DBG_2(0x%08x): 0x%08x\n",
	       u4DmaCfgCr,
	       u4RegValue);
}

static void mt7999ShowWfdmaLpInfo(struct GLUE_INFO *prGlueInfo)
{
	struct ADAPTER *prAdapter = prGlueInfo->prAdapter;
	uint32_t u4Addr, u4Val = 0;

	u4Addr = WFDMA_WRAP_CSR_WFDMA_HIF_MISC_ADDR;
	HAL_RMCR_RD(HIF_DBG, prAdapter, u4Addr, &u4Val);
	DBGLOG(HAL, DEBUG, "WFDMA_HIF_MISC(0x%08x): 0x%08x\n", u4Addr, u4Val);

	u4Addr = 0x20024108;
	HAL_RMCR_RD(HIF_DBG, prAdapter, u4Addr, &u4Val);
	DBGLOG(HAL, DEBUG, "WFDMA_CLK_EN_STATUS(0x%08x): 0x%08x\n",
	       u4Addr, u4Val);

}

static void mt7999ShowWfdmaIntInfo(struct GLUE_INFO *prGlueInfo)
{
	struct ADAPTER *prAdapter = prGlueInfo->prAdapter;
	uint32_t u4AddrEn, u4AddrSta, u4ValEn = 0, u4ValSta = 0;

	u4AddrSta = WF_P0_WFDMA_WPDMA2HOST_ERR_INT_STA_ADDR;
	u4AddrEn = WF_P0_WFDMA_WPDMA2HOST_ERR_INT_ENA_ADDR;
	HAL_RMCR_RD(HIF_DBG, prAdapter, u4AddrSta, &u4ValSta);
	HAL_RMCR_RD(HIF_DBG, prAdapter, u4AddrEn, &u4ValEn);
	DBGLOG(HAL, DEBUG,
	       "WPDMA2HOST_ERR_INT_STA/ENA(0x%08x/0x%08x): 0x%08x/0x%08x\n",
	       u4AddrSta, u4AddrEn, u4ValSta, u4ValEn);

	u4AddrSta = WF_P0_WFDMA_MCU2HOST_SW_INT_STA_ADDR;
	u4AddrEn = WF_P0_WFDMA_MCU2HOST_SW_INT_ENA_ADDR;
	HAL_RMCR_RD(HIF_DBG, prAdapter, u4AddrSta, &u4ValSta);
	HAL_RMCR_RD(HIF_DBG, prAdapter, u4AddrEn, &u4ValEn);
	DBGLOG(HAL, DEBUG,
	       "MCU2HOST_SW_INT_STA/ENA(0x%08x/0x%08x): 0x%08x/0x%08x\n",
	       u4AddrSta, u4AddrEn, u4ValSta, u4ValEn);

	u4AddrSta = WF_P0_WFDMA_SUBSYS2HOST_INT_STA_ADDR;
	u4AddrEn = WF_P0_WFDMA_SUBSYS2HOST_INT_ENA_ADDR;
	HAL_RMCR_RD(HIF_DBG, prAdapter, u4AddrSta, &u4ValSta);
	HAL_RMCR_RD(HIF_DBG, prAdapter, u4AddrEn, &u4ValEn);
	DBGLOG(HAL, DEBUG,
	       "SUBSYS2HOST_INT_STA/ENA(0x%08x/0x%08x): 0x%08x/0x%08x\n",
	       u4AddrSta, u4AddrEn, u4ValSta, u4ValEn);

	u4AddrSta = WF_P0_WFDMA_WFDMA_SUBMODULE_ERR_INT_ADDR;
	u4AddrEn = u4AddrSta + 0x4;
	HAL_RMCR_RD(HIF_DBG, prAdapter, u4AddrSta, &u4ValSta);
	HAL_RMCR_RD(HIF_DBG, prAdapter, u4AddrEn, &u4ValEn);
	DBGLOG(HAL, DEBUG,
	       "WFDMA_SUBMODULE_ERR_INT/ENA(0x%08x/0x%08x): 0x%08x/0x%08x\n",
	       u4AddrSta, u4AddrEn, u4ValSta, u4ValEn);

	u4AddrSta = WF_P0_WFDMA_HOST_INT_STA_MD_ADDR;
	u4AddrEn = WF_P0_WFDMA_HOST_INT_ENA_MD_ADDR;
	HAL_RMCR_RD(HIF_DBG, prAdapter, u4AddrSta, &u4ValSta);
	HAL_RMCR_RD(HIF_DBG, prAdapter, u4AddrEn, &u4ValEn);
	DBGLOG(HAL, DEBUG,
	       "HOST_INT_STA_MD/ENA(0x%08x/0x%08x): 0x%08x/0x%08x\n",
	       u4AddrSta, u4AddrEn, u4ValSta, u4ValEn);

	u4AddrSta = WF_P0_WFDMA_MD_INT_STA_ADDR;
	u4AddrEn = WF_P0_WFDMA_MD_INT_ENA_ADDR;
	HAL_RMCR_RD(HIF_DBG, prAdapter, u4AddrSta, &u4ValSta);
	HAL_RMCR_RD(HIF_DBG, prAdapter, u4AddrEn, &u4ValEn);
	DBGLOG(HAL, DEBUG,
	       "MD_INT_STA/ENA(0x%08x/0x%08x): 0x%08x/0x%08x\n",
	       u4AddrSta, u4AddrEn, u4ValSta, u4ValEn);
}
...
{
	mt7999ShowWfdmaLpInfo(prGlueInfo);
	mt7999ShowWfdmaIntInfo(prGlueInfo);
	mt7999ShowPcieDebugInfo(prGlueInfo);
}
...
void mt7999_dumpPcieReg(void)
{
	...
	DBGLOG(HAL, DEBUG, "Start dumpPcieReg.\n");
	connv3_hif_dbg_write(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74070000, 0x00000404);
	DBGLOG(HAL, DEBUG, "WR CR[0x74070000]=0x00000404\n");
	connv3_hif_dbg_write(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74070010, 0x00AD00AD);
	DBGLOG(HAL, DEBUG, "WR CR[0x74070010]=0x00AD00AD\n");
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x740700D0, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x740700D0] value[0x%08x]\n", u4Value);

	connv3_hif_dbg_write(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74070010, 0x00C500C5);
	DBGLOG(HAL, DEBUG, "WR CR[0x74070010]=0x00C500C5\n");
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x740700D0, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x740700D0] value[0x%08x]\n", u4Value);
	...
	DBGLOG(HAL, DEBUG, "WR CR[0x74070010]=0x008E008E\n");
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x740700D0, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x740700D0] value[0x%08x]\n", u4Value);
	...
	DBGLOG(HAL, DEBUG, "WR CR[0x74070010]=0x00940094\n");
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x740700D0, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x740700D0] value[0x%08x]\n", u4Value);
	...
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x70003014, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x70003014] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x70025014, &u4Value);
	...
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030014, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030014] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030018, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030018] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030080, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030080] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030098, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030098] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x740300C0, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x740300C0] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x740300C8, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x740300C8] value[0x%08x]\n", u4Value);
	...
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030180, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030180] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030184, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030184] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030188, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030188] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x7403018C, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x7403018C] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030194, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030194] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030198, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030198] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x7403019C, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x7403019C] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x740301A4, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x740301A4] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x740301A8, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x740301A8] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x740301AC, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x740301AC] value[0x%08x]\n", u4Value);

	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030D38, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030D38] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030D3C, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030D3C] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030D40, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030D40] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030D44, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030D44] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030D48, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030D48] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030D4C, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030D4C] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030D50, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030D50] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030D54, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030D54] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030D58, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030D58] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030D5C, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030D5C] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030D60, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030D60] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030D64, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030D64] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030D68, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030D68] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030D6C, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030D6C] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030D70, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030D70] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030D74, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030D74] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030D78, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030D78] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030D7C, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030D7C] value[0x%08x]\n", u4Value);

	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030E00, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030E00] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030E04, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030E04] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030E08, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030E08] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030E0C, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030E0C] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030E10, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030E10] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030E14, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030E14] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030E18, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030E18] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030E1C, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030E1C] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030E20, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030E20] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030E24, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030E24] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030E28, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030E28] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030E2C, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030E2C] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030E30, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030E30] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030E34, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030E34] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030E38, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030E38] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030E3C, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030E3C] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030E40, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030E40] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030E44, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030E44] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030E48, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030E48] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74030E4C, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74030E4C] value[0x%08x]\n", u4Value);

	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74031000, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74031000] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74031004, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74031004] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74031008, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74031008] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x7403100C, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x7403100C] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74031010, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74031010] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74031014, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74031014] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74031018, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74031018] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x7403101C, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x7403101C] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74031020, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74031020] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74031024, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74031024] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74031080, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74031080] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74031084, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74031084] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74031088, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74031088] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x7403108C, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x7403108C] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74031090, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74031090] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74031094, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74031094] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74031098, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74031098] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x7403109C, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x7403109C] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x740310A8, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x740310A8] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x740310E0, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x740310E0] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x740310E4, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x740310E4] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x740310EC, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x740310EC] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x740310F0, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x740310F0] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x740310F4, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x740310F4] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74031108, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74031108] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74031110, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74031110] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74031114, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74031114] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74031118, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74031118] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x7403111C, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x7403111C] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74031200, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74031200] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74031204, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74031204] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74031208, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74031208] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x7403120C, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x7403120C] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74031210, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74031210] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74031214, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74031214] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74031218, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74031218] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x7403121C, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x7403121C] value[0x%08x]\n", u4Value);

	/* dump PCIe debug CR Phy */
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74070094, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74070094] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74070098, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74070098] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x7407009C, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x7407009C] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x740700B0, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x740700B0] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x740700C0, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x740700C0] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x740700D0, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x740700D0] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x740700E0, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x740700E0] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x740700E4, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x740700E4] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x740730C0, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x740730C0] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x740730C4, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x740730C4] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x740730C8, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x740730C8] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x740730CC, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x740700CC] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x740700D0, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x740700D0] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x740750C0, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x740750C0] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x740750C4, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x740750C4] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x740750C8, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x740750C8] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x740750CC, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x740750CC] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x740750D0, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x740750D0] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x740750D4, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x740750D4] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x740750D8, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x740750D8] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74076018, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74076018] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74076098, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74076098] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x7407609C, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x7407609C] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74079050, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74079050] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x74079054, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x74079054] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x740790B0, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x740790B0] value[0x%08x]\n", u4Value);
	connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
		0x740790B4, &u4Value);
	DBGLOG(HAL, DEBUG, "CR[0x740790B4] value[0x%08x]\n", u4Value);
}

void mt7999_dumpPcieRegWithScanDump(void)
{
	uint32_t i = 0;
	uint32_t u4Value = 0;
	uint32_t u4RegVal = 0;
	...
	for (i = 0; i <= 0x1FC; i += 4) {
		...
		connv3_hif_dbg_read(CONNV3_DRV_TYPE_WIFI, CONNV3_DRV_TYPE_BT,
			u4RegVal, &u4Value);
		DBGLOG(HAL, DEBUG, "CR[0x%08x] value[0x%08x]\n",
			u4RegVal, u4Value);
	}
}
...
void mt7999_dumpCbInfraReg(struct ADAPTER *ad, uint8_t fgIsDumpViaBt)
{
	...
	mt7999_dump_debug_sop(ad, &mt7999_dump_list_cb_infra_a,
		fgIsDumpViaBt);

	/* SectionB - cb_infra vcore on */
	mt7999_dump_debug_sop(ad, &mt7999_dump_list_cb_infra_b,
		fgIsDumpViaBt);

	/* SectionC - cb_infra off, read check ok */
	mt7999_dump_debug_sop(ad, &mt7999_dump_list_cb_infra_c,
		fgIsDumpViaBt);

	/* SectionD - pcie */
	mt7999_dump_debug_sop(ad, &mt7999_dump_list_cb_infra_d,
		fgIsDumpViaBt);

	/* SectionE - cbtop CR */
	mt7999_dump_debug_sop(ad, &mt7999_dump_list_cb_infra_e,
		fgIsDumpViaBt);

	/* SectionG - dma dbg_ctl */
	mt7999_dump_debug_sop(ad, &mt7999_dump_list_cb_infra_g,
		fgIsDumpViaBt);

	/* SectionH - cb_infra_mbu */
	mt7999_dump_debug_sop(ad, &mt7999_dump_list_cb_infra_h,
		fgIsDumpViaBt);

	/* SectionI - cb_infra_bus (cb_infra_off_dma_bcrm) */
	mt7999_dump_debug_sop(ad, &mt7999_dump_list_cb_infra_i,
		fgIsDumpViaBt);

	/* SectionJ - pcie phy */
	mt7999_dump_debug_sop(ad, &mt7999_dump_list_cb_infra_j,
		fgIsDumpViaBt);
}
...
void mt7999_dumpPcGprLog(struct ADAPTER *ad, uint8_t fgIsDumpViaBt)
{
	...
	mt7999_dump_debug_sop(ad, &mt7999_dump_list_wf_mcu_hostcsr_a,
		fgIsDumpViaBt);

	/* SectionB - Dump WFMCU GPR_log */
	mt7999_dump_debug_sop(ad, &mt7999_dump_list_wf_mcu_hostcsr_b,
		fgIsDumpViaBt);

	/* SectionC - Dump WFMCU GPR */
	mt7999_dump_debug_sop(ad, &mt7999_dump_list_wf_mcu_hostcsr_c,
		fgIsDumpViaBt);

	/* SectionD - Dump WFMCU CSR */
	mt7999_dump_debug_sop(ad, &mt7999_dump_list_wf_mcu_hostcsr_d,
		fgIsDumpViaBt);
}
...
#define CTRL_LOG_NUM			5
	...
	connac5x_dump_format_memory32(general_dump,
		GENERAL_LOG_NUM,
	...
	connac5x_dump_format_memory32(ctl_status_dump,
		CTRL_LOG_NUM,
...
void mt7999_dumpWfTopReg(struct ADAPTER *ad, uint8_t fgIsDumpViaBt)
{
	...
	mt7999_dump_debug_sop(ad, &mt7999_dump_list_wf_top_a,
		fgIsDumpViaBt);

	/* SectionB - Dump wf_top_misc_von monflg */
	mt7999_dump_debug_sop(ad, &mt7999_dump_list_wf_top_b,
		fgIsDumpViaBt);

	/* SectionC - Dump wf_top_cfg_on debug CR */
	mt7999_dump_debug_sop(ad, &mt7999_dump_list_wf_top_c,
		fgIsDumpViaBt);

	/* SectionD - Dump wf_top_rgu_on debug CR */
	mt7999_dump_debug_sop(ad, &mt7999_dump_list_wf_top_d,
		fgIsDumpViaBt);

	/* SectionE - Dump wf_top_rgu_von monflg */
	mt7999_dump_debug_sop(ad, &mt7999_dump_list_wf_top_e,
		fgIsDumpViaBt);

	/* SectionF - Dump wf_top_slpprot_on debug CR */
	mt7999_dump_debug_sop(ad, &mt7999_dump_list_wf_top_f,
		fgIsDumpViaBt);
}
...
void mt7999_dumpWfBusReg(struct ADAPTER *ad, uint8_t fgIsDumpViaBt)
{
	...
	mt7999_dump_debug_sop(ad, &mt7999_dump_list_wf_bus_a,
		fgIsDumpViaBt);

	/* SectionB - Dump VDNR timeout wf side info */
	mt7999_dump_debug_sop(ad, &mt7999_dump_list_wf_bus_b,
		fgIsDumpViaBt);

	/* SectionC - Dump AHB APB timeout info */
	mt7999_dump_debug_sop(ad, &mt7999_dump_list_wf_bus_c,
		fgIsDumpViaBt);

	/* SectionD - Dump WF2AP bus status */
	mt7999_dump_debug_sop(ad, &mt7999_dump_list_wf_bus_d,
		fgIsDumpViaBt);

	/* SectionE - Dump WF2AP access detect info */
	mt7999_dump_debug_sop(ad, &mt7999_dump_list_wf_bus_e,
		fgIsDumpViaBt);
}
...
static void mt7999_dumpConninfraBus(struct ADAPTER *ad)
{
	...
	HAL_RMCR_RD(PLAT_DBG, ad, CONN_HOST_CSR_TOP_WF_MD_LPCTL_ADDR,
		&MDDrvOwnStat);
	DBGLOG(HAL, DEBUG, "WF DrvOwn stat=0x%08x, MD DrvOwn stat=0x%08x.\n",
...
void mt7999_DumpBusStatus(struct ADAPTER *ad)
{
	...
	/* Ensure config space dump is disabled to avoid interference */
	mtk_pcie_disable_cfg_dump(0);
#endif /* CFG_MTK_WIFI_PCIE_CONFIG_SPACE_ACCESS_DBG */


	DBGLOG(HAL, DEBUG, "Phase1: Trigger PCIe Scan Dump.\n");
#if (CFG_MTK_WIFI_CONNV3_SUPPORT == 1)
	/* Notify BT to start */
	...
	mt7999_dumpPcieRegWithScanDump();
...
start_dump_via_pcie:
	DBGLOG(HAL, DEBUG, "Phase2: Trigger Wi-Fi dump via PCIe.\n");
	...
	DBGLOG(HAL, DEBUG, "[PSOP_9_1] version=%s\n",
			MT7999_WIFI_DEBUGSOP_DUMP_VERSION);
	mt7999_dumpCbInfraReg(ad, FALSE);
	mt7999_dumpWfTopReg(ad, FALSE);
	mt7999_dumpWfBusReg(ad, FALSE);
	mt7999_dumpPcGprLog(ad, FALSE);
	mt7999_dumpWfsyscpupcr(ad);
	mt7999_dumpRV55CoreReg(ad);

	/* skip dump via BT if had dump via PCIe */
	goto dump_end;

start_dump_via_bt:
	DBGLOG(HAL, DEBUG, "Phase3: Trigger Wi-Fi dump via BT.\n");
#if (CFG_MTK_WIFI_CONNV3_SUPPORT == 1)
	if (debug_ops && debug_ops->dumpPcieStatus)
		readable = debug_ops->dumpPcieStatus(ad->prGlueInfo);
	...
	if (ret != 0) {
		...
		DBGLOG(HAL, DEBUG, "start BT dump.\n");
	...
	DBGLOG(HAL, DEBUG, "[PSOP_9_1] version=%s\n",
			MT7999_WIFI_DEBUGSOP_DUMP_VERSION);
	mt7999_dumpCbInfraReg(ad, TRUE);
	mt7999_dumpWfTopReg(ad, TRUE);
	mt7999_dumpWfBusReg(ad, TRUE);
	mt7999_dumpPcGprLog(ad, TRUE);
	mt7999_dumpWfsyscpupcr(ad);
...
wrong_rate:
	connac5x_dbg_invalid_rx_rate(prAdapter, prSwRfb, &rate);
#endif
}

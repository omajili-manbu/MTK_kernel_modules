...
#if IS_ENABLED(CFG_CHECK_DRVOWN_EINT)
...
static void soc7_0_SetupFwDateInfo(struct ADAPTER *prAdapter,
	enum ENUM_IMG_DL_IDX_T eDlIdx,
	uint8_t *pucDate);
static int wake_up_conninfra_off(struct GLUE_INFO *prGlueInfo,
	uint32_t eSrc, u_int8_t fgNeedRetry);
static void disable_conninfra_off_force_on(struct GLUE_INFO *prGlueInfo,
	uint32_t eSrc);
...
#if (BUILD_QA_DBG == 1)
struct WF_POS_CR_VALUE soc7_0_wf_pos_cr[WF_POS_NUM];
#endif
...
#if (BUILD_QA_DBG == 1)
static void dump_wf_pos_cr(enum _ENUM_WF_POS_DEBUG_CR eEnd)
{
	uint32_t i;
	uint32_t value = 0;

	for (i = 0; i <= eEnd; i++) {
		wf_ioremap_read(soc7_0_wf_pos_cr[i].u4Addr, &value);
		DBGLOG(HAL, DEBUG, "0x%08x: record=[0x%08x], read=[0x%08x]\n",
			soc7_0_wf_pos_cr[i].u4Addr,
			soc7_0_wf_pos_cr[i].u4Val,
			value);
	}
}
#endif
...
static int wake_up_conninfra_off(struct GLUE_INFO *prGlueInfo,
	uint32_t eSrc, u_int8_t fgNeedRetry)
{
#define WF_POS_WAKEUP_RETRY_MAX 10

	int i4Ret;
	uint32_t value = 0, u4CheckVal = 0;
	uint32_t u4RetryCnt = 0;
	uint32_t polling_count;
	uint32_t u4ConnsysVersion = 0;

	while (KAL_TEST_AND_SET_BIT(0, prGlueInfo->ulIsConnChanging)
	       == 1) {
		udelay(500);
	}

	if (KAL_TEST_BIT(eSrc, prGlueInfo->ulWakeUpConnSrc)) {
		DBGLOG(INIT, WARN,
			"Source %u already hold wakeup.\n", eSrc);
	} else {
		GLUE_INC_REF_CNT(prGlueInfo->u4WakeUpConnCnt);
		KAL_SET_BIT(eSrc, prGlueInfo->ulWakeUpConnSrc);
	}
...
wakeup_retry:
	...
	if (wf_ioremap_write_check(CONN_HOST_CSR_TOP_CONN_INFRA_WAKEPU_WF_ADDR,
				value, BIT(0), &u4CheckVal)) {
		DBGLOG(INIT, ERROR,
			"Write wakeup CR fail. (0x%x), retry=%u\n",
			u4CheckVal, u4RetryCnt);
		i4Ret = -1;
		goto exit;
	}
#if (BUILD_QA_DBG == 1)
	soc7_0_wf_pos_cr[WF_POS_WF_WAKEUP_CONNINFRA].u4Addr =
		CONN_HOST_CSR_TOP_CONN_INFRA_WAKEPU_WF_ADDR;
	soc7_0_wf_pos_cr[WF_POS_WF_WAKEUP_CONNINFRA].u4Val = u4CheckVal;
#endif
	...
	while (value != u4ConnsysVersion) {
		if (polling_count > 10) {
			DBGLOG(INIT, ERROR,
				"Polling CONNSYS version ID fail. (0x%x), retry=%u\n",
				value, u4RetryCnt);
			i4Ret = -1;
			goto exit;
		}
	...
	while ((value & CONN_INFRA_CFG_ON_CONN_INFRA_CFG_PWRCTRL1_CONN_INFRA_RDY_MASK) == 0) {
		if (polling_count > 10) {
			DBGLOG(INIT, ERROR,
				"Polling CONN_INFRA cmdbt restore done fail. (0x%x), retry=%u\n",
				value, u4RetryCnt);
			i4Ret = -1;
			goto exit;
		}
...
exit:
	if (i4Ret != 0 && u4RetryCnt < WF_POS_WAKEUP_RETRY_MAX && fgNeedRetry) {
		u4RetryCnt++;
		goto wakeup_retry;
	}

	if (i4Ret != 0) {
		KAL_CLR_BIT(eSrc, prGlueInfo->ulWakeUpConnSrc);
		GLUE_DEC_REF_CNT(prGlueInfo->u4WakeUpConnCnt);
	}

	KAL_CLR_BIT(0, prGlueInfo->ulIsConnChanging);
	...
	uint32_t eSrc)
{
	...
	if (KAL_TEST_BIT(eSrc, prGlueInfo->ulWakeUpConnSrc)) {
		KAL_CLR_BIT(eSrc, prGlueInfo->ulWakeUpConnSrc);
		u4Cnt = GLUE_DEC_REF_CNT(prGlueInfo->u4WakeUpConnCnt);
	} else {
		DBGLOG(INIT, WARN,
	...
	if (u4Cnt == 0) {
		...
		DBGLOG(INIT, TRACE,
			...
			u4Cnt, prGlueInfo->ulWakeUpConnSrc);
	}
...
static int wf_pwr_on_consys_mcu(struct ADAPTER *prAdapter)
{
...
#if (BUILD_QA_DBG == 1)
	kalMemZero(soc7_0_wf_pos_cr, sizeof(soc7_0_wf_pos_cr));
#endif
	...
	ret = wake_up_conninfra_off(prAdapter->prGlueInfo,
			WAKEUP_CONN_ON_SEQ, TRUE);
	if (ret)
		return ret;
...
#if (BUILD_QA_DBG == 1)
	wf_ioremap_write_check(CONN_INFRA_CFG_ON_CONN_INFRA_WF_SLP_CTRL_ADDR,
		value, 0xFFFFFFFF, &u4CheckVal);
	soc7_0_wf_pos_cr[WF_POS_WF2CONN_SLP_PROT_DISABLE].u4Addr =
		CONN_INFRA_CFG_ON_CONN_INFRA_WF_SLP_CTRL_ADDR;
	soc7_0_wf_pos_cr[WF_POS_WF2CONN_SLP_PROT_DISABLE].u4Val = u4CheckVal;
#endif
	...
	while ((value & BIT(30)) == 0) {
		if (polling_count > 10) {
			...
			DBGLOG(INIT, ERROR,
				...
				value);
#if (BUILD_QA_DBG == 1)
			dump_wf_pos_cr(WF_POS_WF2CONN_SLP_PROT_DISABLE);
#endif
			return ret;
		}
		udelay(500);
	...
	disable_conninfra_off_force_on(prAdapter->prGlueInfo,
			WAKEUP_CONN_ON_SEQ);
...
#if (CFG_WIFI_COREDUMP_SUPPORT == 1)
	while (wifi_coredump_get_status() != ENUM_COREDUMP_NOT_TRIGGER) {
...
static int soc7_0_ConnacPccifoff(struct ADAPTER *prAdapter)
{
	...
	if (kalGetChipID() == 0x6881)
		ret = wf_ioremap_write(0x10254014, 0x0ff);
	else
	...
	uint32_t *status)
{
	...
	check = wake_up_conninfra_off(prAdapter->prGlueInfo,
			WAKEUP_CONN_IRQ, TRUE);
	if (check)
		return FALSE;

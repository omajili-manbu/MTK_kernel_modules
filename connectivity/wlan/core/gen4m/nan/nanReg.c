				       ...
				       uint8_t ucPriChnlBitmap)
{
	...
	if (IS_6G_OP_CLASS(ucOperatingClass) && g_fgNanUseR4AvailAttr) {
		...
		DBGDUMP_HEX(NAN, INFO, "[6g Prim Chnl Map]",
	...
	for (j = 0; j < u4MaxChnlBitmap; j++) {
		...
		DBGLOG(NAN, LOUD, "Returning channel index=%u", j);
...
/* 6G ucOperatingClass is NOT supported */
uint8_t nanGetBitIndexByPrimaryChnl(uint8_t ucOperatingClass,
				    u_int8_t fgNonContinuousBw,
				    ...
				    uint8_t ucPriChnl)
{
	...
	ucOcIndex = nanRegFindRecordIdx(ucOperatingClass);
	if (ucOcIndex == REG_DB_ENTRY_NOT_FOUND)
		return REG_INVALID_INFO;

	DBGLOG(NAN, LOUD,
	       "find OC=%u, NC=%u, u2ChnlBitmap=0x%04x, ucPrimChnlBitmap=0x%02x, PriChnl=%u",
	       ucOperatingClass, fgNonContinuousBw, u2ChnlBitmap,
	       ucPrimaryChnlBitmap, ucPriChnl);

	u2Bw = g_rNanRegInfo[ucOcIndex].u2Bw;
	pucSupportChnlList = g_rNanRegInfo[ucOcIndex].aucSupportChnlList;

	for (i = 0; i < REG_2G_5G_MAX_SUPPORT_CHANNEL; i++) {
		if ((BIT(i) & u2ChnlBitmap) == 0)
			continue;

		ucCenterChannel = pucSupportChnlList[i];
		DBGLOG(NAN, LOUD, "check center channel=%u", ucCenterChannel);

		for (j = 0; j < sizeof(uint8_t) * CHAR_BIT; j++) {
			if ((BIT(j) & ucPrimaryChnlBitmap) == 0)
				continue;

			ucPrimary = nanRegGetPrimaryChannel(ucCenterChannel,
						    u2Bw, fgNonContinuousBw, j,
						    ucOperatingClass);
			DBGLOG(NAN, LOUD, "check pri bit=%zu, primary=%u",
			       j, ucPrimary);

			if (ucPrimary == ucPriChnl)
				return i;
		}
	}
...
/**
 * Determine these parameters by (primary + bw)
 * uint32_t u4ChnlType : 1;		NAN_BAND_CH_ENTRY_LIST_TYPE_CHNL
 * uint32_t u4ChnlRsvd : 7;		--
 * uint32_t u4OperatingClass : 8;	abc
 * uint32_t u4PrimaryChnl : 8;		ucPriChannel
 * uint32_t u4AuxCenterChnl : 8;	0
 */

...
struct wpa_authenticator g_rNanWpaAuth[NAN_NDI_INDEX_NUM];

#if (CFG_SUPPORT_NAN_R4_PAIRING == 1)
struct wpa_sm g_arNanPairingWpaSm[NAN_MAX_SUPPORT_NDL_CONFIG_NUM];
struct wpa_state_machine
g_arNanPairingWpaAuthSm[NAN_MAX_SUPPORT_NDL_CONFIG_NUM];
...
uint8_t g_aucNanGtkCipherSuiteList[] = {
...
#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_DATA == 1)
uint32_t
nan_sec_hostapd_wpa_auth_conf_group_cipher(struct wpa_auth_config *wconf);
#endif
...
#define TEMP_LOG_TEMPLATE \
	...
	DBGLOG(NAN, INFO, TEMP_LOG_TEMPLATE,
...
#define TEMP_LOG_TEMPLATE \
	"prAdapter:%p, pucLocalAddr:%p,pucPeerAddr:%p, pucKeyData:%p\n"

		DBGLOG(NAN, ERROR, TEMP_LOG_TEMPLATE,
			prAdapter, pucLocalAddr,
			pucPeerAddr, pucKeyData);
...
/* TODO: UNIFY CMD */
	...
	case NAN_KEY_TYPE_MC_TX_KEY:
		...
		prCmdManageKey->ucBssIdx = prWtbl[u2WtblEntry].ucBssIndex;
		prCmdManageKey->ucNdiIdx =
			nanDataEngineGetNdiIdxByAddr(prAdapter, pucLocalAddr);
		DBGLOG(NAN, STATE, "[MCDATA] NdiIdx = %d\n",
			prCmdManageKey->ucNdiIdx);
		break;
	...
	case NAN_KEY_TYPE_MC_MGMT_RX_KEY:
		for (szIdx = 0; szIdx < NAN_MAX_MC_RX_WTBL_NUM; szIdx++) {
			if (g_arNanMcRxWtbl[szIdx].u2WtblIdx == u2WtblEntry) {
				...
				prCmdManageKey->ucBssIdx =
					g_arNanMcRxWtbl[szIdx].ucBssIdx;
				break;
			}
		}
	...
	if (eKeyOp == NAN_KEY_OP_SET_KEY) {
		...
		if (eKeyType == NAN_KEY_TYPE_MC_MGMT_RX_KEY) {
			...
			kalMemCopy(&prCmdManageKey->aucRsvd,
			pucPn, NAN_PACKET_NUMBER_LEN);
		}

		kalMemCopy(&prCmdManageKey->aucKeyMaterial,
			pucKeyData, ucKeyLen);
	...
	uint8_t *pucPeerNmiAddr)
{
	...
	uint8_t *pucLocalNmiAddr = prAdapter->rDataPathInfo.aucLocalNMIAddr;
	...
	if (ucIdx < NAN_NUM_NMI_CXT_KEY) {
		...
		kalMemCopy(prNmiCxtKey->aucNmiAddr,
			pucPeerNmiAddr, MAC_ADDR_LEN);

		if (prNmiCxtKey->u2WtblIdx == WTBL_RESERVED_ENTRY) {
			prNmiCxtKey->u2WtblIdx =
				secPrivacySeekForNanEntry(prAdapter,
					pucLocalNmiAddr,
					pucPeerNmiAddr, 1, 0, &fgWtblReUsed);
...
/************************************************
 *               Mc Rx Wtbl MaintaRelated
 ************************************************
 */
#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_DATA == 1)

void
nanDumpMcRxWtbl(void)
{
	size_t szIdx = 0;
	struct _NAN_MC_RX_WTBL *prMcRxEntry = NULL;

	for (szIdx = 0; szIdx < NAN_MAX_MC_RX_WTBL_NUM; szIdx++) {
		prMcRxEntry = &g_arNanMcRxWtbl[szIdx];

		if (atomic_read(&(prMcRxEntry->mcRxRefCount)) == 0)
			continue;

#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_FRAME_PROT == 1)
		DBGLOG(NAN, INFO,
			"[MC RX DBG] Dump entry:%zu,wtbl:%u, Peer="
			MACSTR ",Igtk:%u,bigtk:%u\n",
			szIdx, prMcRxEntry->u2WtblIdx,
			MAC2STR(prMcRxEntry->aucPeerAddr),
			prMcRxEntry->fgIgtkInstalled,
			prMcRxEntry->fgBigtkInstalled);
#else
		DBGLOG(NAN, INFO,
			"[MC RX DBG] Dump entry:%zu,wtbl:%u,Peer=" MACSTR "\n",
			szIdx, prMcRxEntry->u2WtblIdx,
			MAC2STR(prMcRxEntry->aucPeerAddr));
#endif
	}
...
nanGetMcRxWtblIdx(uint8_t *pucPeerAddr)
{
	...
	for (szIdx = 0; szIdx < NAN_MAX_MC_RX_WTBL_NUM; szIdx++) {
		if (kalMemCmp(pucPeerAddr, g_arNanMcRxWtbl[szIdx].aucPeerAddr,
				MAC_ADDR_LEN) == 0) {
...
#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_FRAME_PROT == 1)
uint32_t nanSecCheckGtkKde(struct wpa_sm *sm,
	const u8 *key_data, size_t key_data_len)
{
	struct wpa_eapol_ie_parse ie = {0};
	uint8_t ucIsGTKSA = 0, ucGroupSAUsed = 0;
	uint32_t ret = WLAN_STATUS_SUCCESS;

	/* Skip compose GTK when GTK is disabled */
	ucGroupSAUsed =
		((struct _NAN_NDP_INSTANCE_T *)(sm->pvNdp))->ucGroupSAUsed;
	ucIsGTKSA =
		(ucGroupSAUsed & NAN_GROUP_SECURITY_ASSOC_USE_GTKSA) ?
		TRUE : FALSE;
	...
	if (ucIsGTKSA) {
		...
		if (ie.gtk == NULL) {
			DBGLOG(NAN, ERROR, "Need to include GTK KDE\n");
...
{
	kalMemCopy(g_aucNanIgtkPn, pucIgtkPn, NAN_PACKET_NUMBER_LEN);
	kalMemCopy(g_aucNanBigtkPn, pucBigtkPn, NAN_PACKET_NUMBER_LEN);

	DBGLOG(NAN, INFO, "IPN: %02x %02x %02x %02x %02x %02x\n",
		g_aucNanIgtkPn[0], g_aucNanIgtkPn[1], g_aucNanIgtkPn[2],
		g_aucNanIgtkPn[3], g_aucNanIgtkPn[4], g_aucNanIgtkPn[5]);
	DBGLOG(NAN, INFO, "BIPN: %02x %02x %02x %02x %02x %02x\n",
		g_aucNanBigtkPn[0], g_aucNanBigtkPn[1], g_aucNanBigtkPn[2],
		g_aucNanBigtkPn[3], g_aucNanBigtkPn[4], g_aucNanBigtkPn[5]);
}

void
nanSecUpdatePacketNumEvt(struct ADAPTER *prAdapter, uint8_t *pcuEvtBuf)
{
	struct _NAN_REPORT_PACKET_NUM_EVENT *prPnEvt;

	prPnEvt = (struct _NAN_REPORT_PACKET_NUM_EVENT *)pcuEvtBuf;

	nanSecPacketNumberUpdate(prPnEvt->aucIgtkPn, prPnEvt->aucBigtkPn);
}
	...
	uint8_t ucBssIdx)
{
	...
	for (szIdx = 0; szIdx < NAN_MAX_MC_RX_WTBL_NUM; szIdx++) {
		if ((kalMemCmp(pucPeerNmiAddr,
				...
				MAC_ADDR_LEN) == 0)
			&& (ucBssIdx == g_arNanMcRxWtbl[szIdx].ucBssIdx)) {
	...
	uint8_t fgAllocNew)
{
	...
	/* 1. use pucPeerNdiAddr to find entry that is already acquired */
	for (szIdx = 0; szIdx < NAN_MAX_MC_RX_WTBL_NUM; szIdx++) {
...
#define TEMP_LOG_TEMPLATE \
	"[MCDATA] Rx Wtbl isn't consistency with " \
	"global wtbl, %hu, %hhu, " MACSTR "\n"
...
#undef TEMP_LOG_TEMPLATE
	...
	for (szIdx = 0; szIdx < NAN_MAX_MC_RX_WTBL_NUM; szIdx++) {
		if (g_arNanMcRxWtbl[szIdx].u2WtblIdx == WTBL_RESERVED_ENTRY) {
			...
			g_arNanMcRxWtbl[szIdx].ucBssIdx = ucBssIdx;
			kalMemCopy(g_arNanMcRxWtbl[szIdx].aucPeerAddr,
			...
			if (prNanSpecBssInfo)
				...
				DBGLOG(NAN, ERROR, "BssInfo is NULL\n");
			...
			g_arNanMcRxWtbl[szIdx].rStaRec.ucWlanIndex =
				(uint8_t)u2WtblIdx;
			atomic_set(&(g_arNanMcRxWtbl[szIdx].mcRxRefCount), 0);

			nanSecManageKeyCmd(prAdapter,
				NAN_KEY_OP_ACTIVATE,
				NAN_KEY_TYPE_MC_RX_KEY, u2WtblIdx,
				aucBCAddr, pucPeerAddr,
				WPA_ALG_NONE, 0 /* ucKeyId */,
				0 /* ucKeyLen */, NULL /* pucKeyData */, NULL,
				TRUE /* fgInit */, FALSE /* fgKeyExist */);
			break;
		}
	}
	...
	uint8_t ucBssIdx)
{
	...
	szIdx = nanFindAndAllocMcRxWtbl(prAdapter, pucPeerAddr,
		0 /* NA */, ucBssIdx, FALSE /* NA */, FALSE, FALSE);
	if (szIdx >= NAN_MAX_MC_RX_WTBL_NUM) {
		DBGLOG(NAN, ERROR,
...
#ifdef DALE
	atomic_dec(&(g_arNanMcRxWtbl[szIdx].mcRxRefCount));
	if (atomic_read(&(g_arNanMcRxWtbl[szIdx].mcRxRefCount)) == 0)
#else
	if (atomic_dec_return(&(g_arNanMcRxWtbl[szIdx].mcRxRefCount)) == 0)
#endif
	{
		if (TRUE/*!isNanAtResetFlow()*/) {
#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_FRAME_PROT == 1)
			nanSecManageKeyCmd(prAdapter,
					NAN_KEY_OP_CLS_KEY,
		...
		g_arNanMcRxWtbl[szIdx].ucBssIdx = 0;
	...
	uint8_t ucBssIdx)
{
	...
	u2WtblIdx = secPrivacySeekForNanEntry(prAdapter,
		pucLocalAddr, pucNdcId, FALSE, ucBssIdx, &fgWtblReUsed);
	...
	if (!fgWtblReUsed) {
		...
		nanSecManageKeyCmd(prAdapter,
			NAN_KEY_OP_ACTIVATE,
			NAN_KEY_TYPE_MC_TX_KEY, u2WtblIdx,
			pucLocalAddr, pucNdcId,
			CIPHER_SUITE_NONE, 0 /* ucKeyId */,
			0 /* ucKeyLen */, NULL /* pucKeyData */, NULL,
			TRUE /* fgInit */, FALSE /* fgKeyExist */);
	} else {
		if (pfgFirst)
	...
	DBGLOG(NAN, INFO,
		"[MCDATA] MC Tx wtbl alloc idx[%hu], first %d, refCnt %d\n",
		u2WtblIdx, *pfgFirst, prNdcMgmt->mcTxRefCount[ucNdiIdx]);
	...
	struct _NAN_NDC_CTRL_T *prNdcCtrl)
{
	...
	DBGLOG(NAN, INFO,
		"[MCDATA] " MACSTR ", refCnt %d\n", MAC2STR(pucLocalAddr),
		prNdcMgmt->mcTxRefCount[ucNdiIndex]);

#ifdef DALE
	atomic_dec(&(prNdcMgmt->mcTxRefCount[ucNdiIndex]));
	if (atomic_read(&(prNdcMgmt->mcTxRefCount[ucNdiIndex])) == 0)
#else
	i8RefCnt = atomic_dec_return(&(prNdcMgmt->mcTxRefCount[ucNdiIndex]));
	if (i8RefCnt <= 0)
#endif
	{
		if (i8RefCnt < 0) {
			DBGLOG(NAN, WARN,
				"[MCDATA] McTxWTBL:%hu refCnt %d < 0!\n",
				u2WtblIdx, i8RefCnt);
		}
		u2WtblIdx = prNdcMgmt->au2BmcTxWtblIdx[ucNdiIndex];
		if (u2WtblIdx == WTBL_RESERVED_ENTRY) {
			DBGLOG(NAN, ERROR, "[MCDATA] mc wtbl release fail\n");
		} else if (TRUE/*!isNanAtResetFlow()*/) {
		...
		/* Free pending MC TX data */
		nicFreePendingTxMsduInfo(prAdapter, u2WtblIdx,
...
nan_sec_wlanSetAddKey(struct ADAPTER *prAdapter, void *pvSetBuffer,
	...
	if (!prBssInfo) {
		DBGLOG(REQ, DEBUG, "BSS Info not exist !!\n");
 ...
 */
	...
	if (prStaRec) {
		if (prCmdKey->ucKeyType) { /* RSN STA */
			...
			COPY_MAC_ADDR(prCmdKey->aucPeerAddr,
				      prCmdFWKey->aucPeerAddr);
		} else {
			DBGLOG(NAN, ERROR,
				"ucKeyType error!\n");
...
#if 1 /* DBG */
	DBGLOG(NAN, DEBUG, "Add key cmd to wlan index %d:",
	       prCmdKey->ucWlanIndex);
	DBGLOG(NAN, DEBUG, "(BSS = %d) " MACSTR "\n", prCmdKey->ucBssIdx,
	       MAC2STR(prCmdKey->aucPeerAddr));
	DBGLOG(NAN, DEBUG, "Tx = %d type = %d Auth = %d\n", prCmdKey->ucTxKey,
	       prCmdKey->ucKeyType, prCmdKey->ucIsAuthenticator);
	DBGLOG(NAN, DEBUG, "cipher = %d keyid = %d keylen = %d\n",
	       prCmdKey->ucAlgorithmId, prCmdKey->ucKeyId, prCmdKey->ucKeyLen);
	DBGLOG_MEM8(RSN, INFO, prCmdKey->aucKeyMaterial, prCmdKey->ucKeyLen);

	DBGLOG(NAN, DEBUG, "wepkeyUsed = %d\n",
	       prBssInfo->wepkeyUsed[prCmdKey->ucKeyId]);
	DBGLOG(NAN, DEBUG, "wepkeyWlanIdx = %d:", prBssInfo->wepkeyWlanIdx);
	DBGLOG(NAN, DEBUG, "ucBMCWlanIndexSUsed = %d\n",
	       prBssInfo->ucBMCWlanIndexSUsed[prCmdKey->ucKeyId]);
	DBGLOG(NAN, DEBUG, "ucBMCWlanIndexS = %d:",
...
nan_sec_wlanSetRemoveKey(struct ADAPTER *prAdapter, void *pvSetBuffer,
	...
	if (prAdapter->rAcpiState == ACPI_STATE_D3) {
		DBGLOG(REQ, WARN,
		       "Fail set remove key! (Adapter not ready). ACPI=D%d, Radio=%d\n",
...
/* ASSERT(prCmdKey->ucKeyId < MAX_KEY_NUM); */
	...
	COPY_MAC_ADDR(prCmdKey->aucPeerAddr, prCmdFWKey->aucPeerAddr);
	prCmdKey->ucBssIdx = prCmdFWKey->ucBssIdx;
...
nan_sec_wpas_setkey_glue(bool fgIsAp, u8 u1BssIdx, enum wpa_alg alg,
	...
	DBGLOG(NAN, DEBUG,
	       "fgIsAp:%d, u1BssIdx:%hhu, alg:%d, key_idx:%d, key_len:%zu\n",
	       fgIsAp ? 1 : 0, u1BssIdx, alg, key_idx,
	...
	if (kalMemCmp(addr, aucBCAddr, ETH_ALEN) == 0) {
		DBGLOG(NAN, DEBUG, "[%s] broadcast addr", __func__);
		fgIsBC = TRUE;
	}
	...
	if (fgIsAp) { /* AP */
		...
		if (fgIsBC) { /* Own AP */
			COPY_MAC_ADDR(&rCmdkey.aucPeerAddr,
				      prBssInfo->aucOwnMacAddr);
		} else { /* Remote STA */
			COPY_MAC_ADDR(&rCmdkey.aucPeerAddr, addr);
		}
	} else { /* STA */
...
nan_sec_wpa_eapol_key_mic(const u8 *key, size_t key_len, u32 cipher,
	...
	DBGDUMP_HEX(NAN, DEBUG, "KCK", key, key_len);

	DBGDUMP_HEX(NAN, DEBUG, "BUF_len", buf, len);

	DBGLOG(NAN, INFO, "[%s] cipher:%d\n", __func__, cipher);

	if (cipher == NAN_CIPHER_SUITE_ID_NCS_SK_GCM_256) {
		if (hmac_sha384(key, key_len, buf, len, hash)) {
			DBGLOG(NAN, DEBUG, "[%s] ERROR! hmac_sha384() failed",
			       __func__);
			return WLAN_STATUS_FAILURE;
		}
		os_memcpy(mic, hash, NCS_SK_256_MIC_LEN);

		DBGDUMP_HEX(NAN, DEBUG, "hmac_sha384() Result MIC",
			    mic, NCS_SK_256_MIC_LEN);
	} else {
		/* NAN_CIPHER_SUITE_ID_NCS_SK_CCM_128 */
		if (hmac_sha256(key, key_len, buf, len, hash)) {
			DBGLOG(NAN, DEBUG, "[%s] ERROR! hmac_sha256() failed",
			       __func__);
			return WLAN_STATUS_FAILURE;
		}
		os_memcpy(mic, hash, NCS_SK_128_MIC_LEN);

		DBGDUMP_HEX(NAN, DEBUG, "hmac_sha256() Result MIC",
			    mic, NCS_SK_128_MIC_LEN);
	}
...
/* len = sizeof(struct ieee802_1x_hdr) + keyhdrlen; */
	...
	if (u4TotalLen > NAN_KDE_ATTR_BUF_SIZE) {
		DBGLOG(NAN, ERROR, "Correct invalid length %u\n", u4TotalLen);
		u4TotalLen = NAN_KDE_ATTR_BUF_SIZE;
	}
	...
	DBGLOG(NAN, DEBUG, "[%s] u4TotalLen:%d\n", __func__, u4TotalLen);
	DBGLOG(NAN, DEBUG, "key_data_len=%u\n", key_data_len);

	key = (struct wpa_eapol_key *)(sm->au1TmpKdeAttrBuf +
				       sizeof(struct _NAN_SEC_KDE_ATTR_HDR));

	key_len = sizeof_field(struct wpa_state_machine, au1TmpKdeAttrBuf) -
		sizeof(struct _NAN_SEC_KDE_ATTR_HDR);
			   ...
			   size_t data_len, struct wpa_state_machine *sm) /*AP*/
{
	...
	if (data_len < sizeof(*hdr) + sizeof(struct wpa_eapol_key)) {
		DBGLOG(NAN, ERROR,
		       "ERROR! size mismatch, data_len:%zu, hdr+key:%zu",
		       data_len,
	...
	if (os_memcmp_const(mic, key192->key_mic, mic_len) != 0) {
		DBGDUMP_HEX(NAN, WARN, "WARN! MIC mismatch, remote MIC",
			    mic, mic_len);
		return WLAN_STATUS_FAILURE;
	}
	...
	struct wpa_sm *sm, const u8 *src_addr)
{
...
#if 0
	DBGLOG(NAN, DEBUG, "[%s] IEEE 802.1X RX: version=%d type=%d length=%lu",
	...
	tmp = buf; /* NAN, directly edit the buffer content from NDP */
	if (tmp == NULL)
		goto out;
#if 0
	...
	if ((key_info & WPA_KEY_INFO_MIC) && !peerkey &&
	    wpa_supplicant_verify_eapol_key_mic(sm, key192, ver, tmp,
						data_len)) {
		DBGLOG(NAN, DEBUG, "[%s] Quit3. MIC error!\n", __func__);
		goto out;
	}
	...
	if ((sm->proto == WPA_PROTO_RSN || sm->proto == WPA_PROTO_OSEN) &&
		...
		if (wpa_supplicant_decrypt_key_data(sm, key, ver, key_data,
						    &key_data_len)) {
			DBGLOG(NAN, DEBUG, "[%s] Quit4. decrypt data error!\n",
			       __func__);
			goto out;
		}
	}
	...
	if (key_info & WPA_KEY_INFO_KEY_TYPE) {
		if (key_info & WPA_KEY_INFO_KEY_INDEX_MASK) {
			 ...
			 *		non-zero key index");
			 */
			DBGLOG(NAN, DEBUG, "[%s] Quit5. non-zero key index\n",
			       __func__);
			goto out;
		}
		if (key_info & WPA_KEY_INFO_MIC) {
...
uint32_t
nan_sec_wpa_receive(
	struct wpa_authenticator *wpa_auth, /* AP */
...
/* TODO_CJ:remove IE */
	...
	if (key_data_length > data_len - sizeof(*hdr) - keyhdrlen) {
		DBGLOG(NAN, DEBUG,
		       "Quit1. key_data_length:%hu, data_len:%zu, sizeof(*hdr):%zu, keyhdrlen:%zu\n",
	...
	if (sm->wpa == WPA_VERSION_WPA2) {
		if (key->type == EAPOL_KEY_TYPE_WPA) {
			 ...
			 * msg 4/4 with incorrect type value WPA2 mode.
			 */
		} else if (key->type != EAPOL_KEY_TYPE_RSN) {
			DBGLOG(NAN, DEBUG, "[%s] Quit2. key->type:%d\n",
			       __func__, key->type);
			return WLAN_STATUS_FAILURE;
		}
	} else {
		if (key->type != EAPOL_KEY_TYPE_WPA) {
			DBGLOG(NAN, DEBUG, "Quit3. key->type:%d\n", key->type);
			return WLAN_STATUS_FAILURE;
		}
	}
	...
	wpa_hexdump(MSG_DEBUG, "WPA: Received Key Nonce", key->key_nonce,
		    WPA_NONCE_LEN);
	DBGDUMP_HEX(NAN, DEBUG, "Received Key Nonce",
		    key->key_nonce, WPA_NONCE_LEN);

	wpa_hexdump(MSG_DEBUG, "WPA: Received Replay Counter",
		    key->replay_counter, WPA_REPLAY_COUNTER_LEN);
	DBGDUMP_HEX(NAN, DEBUG, "Received Replay Counter",
		    key->replay_counter, WPA_REPLAY_COUNTER_LEN);

	DBGDUMP_HEX(NAN, DEBUG, "key info", key->key_info, 2);
...
#endif
	DBGLOG(NAN, INFO, "Mic Len=%zu, Key Data Len=%u\n",
	...
	if ((mic_len == 0 && key_data_length > 16) ||
		...
		msg = PAIRWISE_4;
		msgtxt = "4/4 Pairwise";
		DBGLOG(NAN, DEBUG, "[%s] Judge as M4\n", __func__);
	}
	...
	if (key_info & WPA_KEY_INFO_REQUEST) {
		if (sm->req_replay_counter_used &&
			 ...
			 *		replayed counter");
			 */
			DBGLOG(NAN, DEBUG, "[%s] Quit4. replayed counter\n",
			       __func__);
	...
	if (!(key_info & WPA_KEY_INFO_REQUEST) &&
		...
		DBGLOG(NAN, DEBUG, "[%s] Quit5. replay counter invalid\n",
		       __func__);
...
continue_processing:
	...
	case PAIRWISE_2:
		...
		if (sm->wpa_ie == NULL ||
			 ...
			 */
			if (sm->wpa_ie) {
				wpa_hexdump(MSG_DEBUG, "WPA IE AssocReq",
					sm->wpa_ie, sm->wpa_ie_len);
			}
			wpa_hexdump(MSG_DEBUG, "WPA IE msg 2/4",
					eapol_key_ie, eapol_key_ie_len);
	...
	if (key_info & WPA_KEY_INFO_REQUEST) {
		if (sm->MICVerified) {
			 ...
			 *	invalid MIC");
			 */
			DBGLOG(NAN, DEBUG, "[%s] Quit9. invalid MIC\n",
			       __func__);
		...
		if (msg == PAIRWISE_2) {
			/* Maintaa copy of the pending EAPOL-Key frames in
...
nan_sec_wpa_init(const u8 *addr, struct wpa_auth_config *conf,
		 ...
		 size_t szBssIdx, size_t szNdiIdx) {
	struct wpa_authenticator *wpa_auth;

	TRACE_FUNC(NAN, DEBUG, "[%s] Enter\n");

	if (szNdiIdx >= ARRAY_SIZE(g_rNanWpaAuth)) {
		DBGLOG(NAN, WARN, "NDI index out of bound\n");
		return NULL;
	}
	...
	kalMemCpyS(wpa_auth->addr,
		...
		addr, ETH_ALEN);
	kalMemCpyS(&wpa_auth->conf,
		sizeof_field(struct wpa_authenticator, conf),
		conf, sizeof(*conf));
	kalMemCpyS(&wpa_auth->cb,
...
nan_sec_hostapd_setup_wpa(struct hostapd_data *hapd) {
	...
	/* Set own mac */
	bss_index =
	nanGetSpecificBssInfo(g_prAdapter, NAN_BSS_INDEX_MAIN)->ucBssIndex;

	prnanBssInfo = GET_BSS_INFO_BY_INDEX(g_prAdapter, bss_index);
...
nan_sec_hostapd_setup_bss(struct hostapd_data *hapd) {
	hostapd_wpa_auth_hapd_data_alloc(
		hapd, nanGetSpecificBssInfo(
			g_prAdapter, NAN_BSS_INDEX_BAND0)->ucBssIndex);
...
nan_sec_hostapd_notif_assoc(struct hostapd_data *hapd, const u8 *addr,
	...
	if (addr == NULL) {
		 ...
		 * driver wrapper. This was seen at least one case where the
	...
	if (elems.wps_ie) {
		...
		DBGLOG(NAN, DEBUG, "STA included WPS IE (Re)AssocReq");
	} else if (elems.rsn_ie) {
		ie = elems.rsn_ie - 2;
		ielen = elems.rsn_ie_len + 2;
		DBGLOG(NAN, DEBUG, "STA included RSN IE (Re)AssocReq");
	} else if (elems.wpa_ie) {
		ie = elems.wpa_ie - 2;
		ielen = elems.wpa_ie_len + 2;
		DBGLOG(NAN, DEBUG, "STA included WPA IE (Re)AssocReq");
	} else {
		ie = NULL;
		ielen = 0;
		DBGLOG(NAN, DEBUG,
		       "STA did not include WPS/RSN/WPA IE (Re)AssocReq");
	}
	...
	if (hapd->conf->wpa) {
		if (ie == NULL || ielen == 0) {
			...
			DBGLOG(NAN, DEBUG, "No WPA/RSN IE from STA");
			return -1;
		}
...
nanSecGetNdpCsidAttr(struct _NAN_NDP_INSTANCE_T *prNdp,
...
#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_FRAME_PROT == 1)
	prCsidAttrHdr->u1Cap |=
		nanSecGetGroupSecurityCap(g_prAdapter, prNdp);
#else
	prCsidAttrHdr->u1Cap = 0;
#endif /* CFG_NAN_SUPPORT_R4_GROUP_ADDR_FRAME_PROT */

	/* Fill-static Cipher list */
	prCsidAttrListHdr =
	...
	if (prNdp->fgTxGtkRequired &&
		...
		prCsidAttrListHdr =
			prCsidAttrListHdr + 1;
		prCsidAttrListHdr->u1CipherType =
			prNdp->ucTxGtkCipherType;
		prCsidAttrListHdr->u1PublishId =
			prNdp->ucPublishId;
		prCsidAttrHdr->u2AttrLen +=
			sizeof(struct _NAN_SEC_CSID_ATTR_LIST);
		u4TotalLen +=
...
#if (ENABLE_SEC_UT_LOG == 1)
	DBGDUMP_HEX(NAN, DEBUG, "CSID attr", prCsidAttrHdr, u4TotalLen);
#endif
...
#if (ENABLE_SEC_UT_LOG == 1)
	DBGDUMP_HEX(NAN, DEBUG, "SCID attr", prScidAttrHdr, u4TotalLen);
#endif
...
#if (ENABLE_SEC_UT_LOG == 1)
	DBGLOG(NAN, DEBUG,
	       "len_ATTR_LIST:%zu, len_ATTR_HDR:%zu, u4NumElem:%u, u4CipherListLen:%u, u4TotalLen:%u\n",
	       sizeof(struct _NAN_SEC_CSID_ATTR_LIST),
...
nanSecInsertCipherList(uint32_t u4CipherType, uint16_t u2PublishId) {
	...
	DBGLOG(NAN, DEBUG, "[%s] Enter, u4CipherType:0x%x, u2PublishId:0x%x\n",
...
uint32_t nanSecNotify4wayBegin(struct _NAN_NDP_INSTANCE_T *prNdp)
{
	...
	wpa_auth = prNdp->prInitiatorSecSmInfo->wpa_auth
		= &g_rNanWpaAuth[prNdp->ucNdiIndex];
	...
	kalMemCpyS(wpa_auth->addr,
		...
		sizeof_field(struct _NAN_NDP_INSTANCE_T,
		aucLocalNDIAddr));
	...
	if (prNdp->eNDPRole == NAN_PROTOCOL_INITIATOR) {
		 ...
		 * (initiator is done in wpa_auth_sta_associated ->
	...
	prNdp->prResponderSecSmInfo->u1MicCalState =
		NAN_SEC_MIC_CAL_IDLE;
	prNdp->prResponderSecSmInfo->pvNdp =
		(void *)prNdp;

	nanSecUpdatePeerNDI(prNdp,
		prNdp->aucPeerNDIAddr);
...
nanSecNotify4wayBegin(struct _NAN_NDP_INSTANCE_T *prNdp)
{
	...
	if (prNdp->eNDPRole == NAN_PROTOCOL_INITIATOR) {
		...
		hostapd_wpa_auth_set_ownmac(g_prNanHapdData,
					    prNdp->aucLocalNDIAddr);
		COPY_MAC_ADDR(prNdp->prInitiatorSecSmInfo->addr,
			      prNdp->aucPeerNDIAddr);
...
{
	DBGLOG(NAN, DEBUG, "Enter, eNDPRole:%d, NDPID:%d\n",
	       prNdp->eNDPRole, prNdp->ucNDPID);
	...
	if (prNdp->eNDPRole == NAN_PROTOCOL_INITIATOR) {
		...
		DBGLOG(NAN, DEBUG, "reset prResponderSecSmInfo=%p\n",
				prNdp->prResponderSecSmInfo);
...
nanSecTxKdeAttrDone(struct _NAN_NDP_INSTANCE_T *prNdp, uint8_t u1DstMsg) {
	...
	DBGLOG(NAN, DEBUG, "Enter, eNDPRole:%d, u1DstMsg:%d\n",
...
nanSecRxKdeAttr(struct _NAN_NDP_INSTANCE_T *prNdp, uint8_t u1SrcMsg,
...
#if (ENABLE_SEC_UT_LOG == 1)
	DBGDUMP_HEX(NAN, DEBUG, "KDE attr", pu1KdeAttrBuf, u4KdeAttrLen);
	DBGDUMP_HEX(NAN, DEBUG, "RX Msg Buf", pu1RxMsgBuf, u4RxMsgLen);
#endif

	if (u1SrcMsg == NAN_SEC_END) {
		DBGLOG(NAN, DEBUG,
		       "[%s] Rcv NDP terminate, return\n", __func__);
		return rStatus;
	}
	...
	if (prNdp->eNDPRole == NAN_PROTOCOL_INITIATOR) {
		...
		if (nanSecIsDevSupportGroupSecurity(g_prAdapter)) {
			/* GTK required and GTK cipher type is from different
			 * attribute ID, therefore update here
			 */
			nanSecSetGroupCipherType(prNdp, FALSE);
			nanSecSetGroupSA(prNdp);
		}
		rStatus = nan_sec_wpa_receive(
			&g_rNanWpaAuth[prNdp->ucNdiIndex],
			prNdp->prInitiatorSecSmInfo,
			prNdp->prResponderSecSmInfo,
		...
		if (u1SrcMsg == NAN_SEC_M1) {
			...
			nanSecStaSmBufReset(prNdp->prResponderSecSmInfo);
			if (nanSecIsDevSupportGroupSecurity(g_prAdapter)) {
				/* GTK required and GTK cipher type are from
				 * different attribute ID, therefore update here
				 */
				nanSecSetGroupCipherType(prNdp, FALSE);
				nanSecSetGroupSA(prNdp);
			}
			...
			DBGLOG(NAN, DEBUG, "set fgIsAllocRxMsgForM1=%u\n",
			       prNdp->prResponderSecSmInfo
					->fgIsAllocRxMsgForM1);
...
nanSecUpdatePmk(struct _NAN_NDP_INSTANCE_T *prNdp) {
	...
	os_memset(
		g_prNanHapdData->conf->ssid.wpa_psk->psk,
		0,
		PMK_LEN);
	kalMemCpyS(
		g_prNanHapdData->conf->ssid.wpa_psk->psk,
...
nanSecSelPtkKeyId(struct _NAN_NDP_INSTANCE_T *prNdp, uint8_t *pu1PeerAddr) {
	...
	/* DBGLOG(NAN, DEBUG, "[%s] Enter, u1NdpIdx:%d", __func__, u1NdpIdx); */
...
nanSecMicCalStaSmStep(struct wpa_sm *sm) /* Send M2, M4 */
{
	...
	case NAN_SEC_MIC_CAL_WAIT: {
		DBGLOG(NAN, DEBUG, "[%s] CAL_MIC_BEGIN\n", __func__);
		...
		/* Fill-KDE for NDP */
		u4TmpKdeAttrLen = sm->u4TmpKdeAttrLen <  NAN_KDE_ATTR_BUF_SIZE
				? sm->u4TmpKdeAttrLen : NAN_KDE_ATTR_BUF_SIZE;
		...
		DBGLOG(NAN, DEBUG, "[%s] CAL_MIC_DONE\n", __func__);
...
nanSecStaSmBufReset(struct wpa_sm *sm) {
	...
	if (sm->fgIsAllocRxMsgForM1) {
		...
		DBGLOG(NAN, DEBUG, "free sm=%p, pu1GetRxMsgBodyBuf=%p\n",
		       sm, sm->pu1GetRxMsgBodyBuf);

		os_free(sm->pu1GetRxMsgKdeBuf);
		DBGLOG(NAN, DEBUG, "free sm=%p, pu1GetRxMsgKdeBuf=%p\n",
		       sm, sm->pu1GetRxMsgKdeBuf);

		sm->fgIsAllocRxMsgForM1 = FALSE;
		DBGLOG(NAN, DEBUG, "free sm=%p, fgIsAllocRxMsgForM1=%u\n",
		       sm, sm->fgIsAllocRxMsgForM1);
	}
#endif

	sm->pu1GetRxMsgBodyBuf = NULL;
	DBGLOG(NAN, DEBUG, "reset sm=%p, pu1GetRxMsgBodyBuf=%p\n",
	       sm, sm->pu1GetRxMsgBodyBuf);
	sm->u4GetRxMsgBodyLen = 0;

	sm->pu1GetRxMsgKdeBuf = NULL;
	DBGLOG(NAN, DEBUG, "reset sm=%p, pu1GetRxMsgKdeBuf=%p\n",
	       sm, sm->pu1GetRxMsgKdeBuf);
...
nanSecMicCalApSmStep(struct wpa_state_machine *sm) /* Send M1, M3 */
{
	...
	case NAN_SEC_MIC_CAL_IDLE: {
		if (sm->u1CurMsg == NAN_SEC_M1) {
			DBGLOG(NAN, DEBUG, "[%s] for M1, direct send out\n",
			       __func__);
			sm->u1MicCalState = NAN_SEC_MIC_CAL_DONE;
		} else
			sm->u1MicCalState = NAN_SEC_MIC_CAL_WAIT;
...
nanSecApSmBufReset(struct wpa_state_machine *sm) {
	DBGLOG(NAN, DEBUG, "[%s] Enter, sm:0x%p\n", __func__, sm);

	DBGDUMP_HEX(NAN, DEBUG, "au1AuthTokenBuf",
		    sm->au1AuthTokenBuf, NAN_AUTH_TOKEN_LEN);

	DBGDUMP_HEX(NAN, DEBUG, "au1M3MicMaterialBuf",
		    sm->au1M3MicMaterialBuf, sm->u4M3MicMaterialLen);

	DBGDUMP_HEX(NAN, DEBUG, "dump au1TmpKdeAttrBuf",
...
nanSecGenAuthToken(u32 cipher, const u8 *auth_token_data,
	...
	DBGLOG(NAN, DEBUG, "[%s] Enter, cipher:%d\n", __func__, cipher);

	if ((auth_token_data == NULL) || (auth_token_data_len == 0))
		DBGLOG(NAN, DEBUG, "[%s] ERROR! auth_token_data is NULL\n",
		       __func__);

#if (ENABLE_SEC_UT_LOG == 1)
	DBGDUMP_HEX(NAN, DEBUG, "auth token",
		    auth_token_data, auth_token_data_len);
#endif

	if (cipher == NAN_CIPHER_SUITE_ID_NCS_SK_GCM_256) {
		if (sha384_vector(1, &auth_token_data, &auth_token_data_len,
				  hash)) {
			DBGLOG(NAN, DEBUG, "ERROR! sha384_vector() failed");
			return WLAN_STATUS_FAILURE;
		}
		os_memcpy(auth_token, hash, NAN_AUTH_TOKEN_LEN);

#if (ENABLE_SEC_UT_LOG == 1)
		DBGDUMP_HEX(NAN, DEBUG, "sha384 auth token",
			    auth_token, NAN_AUTH_TOKEN_LEN);
#endif
	} else {
		/* NAN_CIPHER_SUITE_ID_NCS_SK_CCM_128 */
		if (sha256_vector(1, &auth_token_data, &auth_token_data_len,
				  hash)) {
			DBGLOG(NAN, DEBUG, "[%s] ERROR! sha256_vector() failed",
			       __func__);
			return WLAN_STATUS_FAILURE;
		}
		os_memcpy(auth_token, hash, NAN_AUTH_TOKEN_LEN);
...
#if (ENABLE_SEC_UT_LOG == 1)
		DBGDUMP_HEX(NAN, DEBUG, "sha256 auth token",
			    auth_token, NAN_AUTH_TOKEN_LEN);
#endif
	}
...
nanSecGenM3MicMaterial(uint8_t *pu1AuthTokenBuf, const u8 *pu1M3bodyBuf,
	...
	*pu4M3MicMaterialLen = u4TotalLen;
	...
	DBGDUMP_HEX(NAN, DEBUG, "pu1AuthTokenBuf",
		    pu1AuthTokenBuf, NAN_AUTH_TOKEN_LEN);

	DBGDUMP_HEX(NAN, DEBUG, "pu1M3bodyBuf", pu1M3bodyBuf, u4M3BodyLen);

	DBGDUMP_HEX(NAN, DEBUG, "pu1MicMaterialBuf",
		    pu1MicMaterialBuf, u4TotalLen);
...
#if (CFG_SUPPORT_NAN_R4_PAIRING == 1)
struct wpa_state_machine *
nanSecGetPairingInitiatorSm(uint8_t u1Index)
{
	return &g_arNanPairingWpaAuthSm[u1Index];
}

struct wpa_sm *
nanSecGetPairingResponderSm(uint8_t u1Index)
{
	return &g_arNanPairingWpaSm[u1Index];
}
#endif
...
nanSecInstallTk(struct _NAN_NDP_INSTANCE_T *prNdp,
	...
	DBGLOG(NAN, DEBUG, "Enter, StaIdx:%d, BssIdx:%d\n",
	...
	DBGDUMP_HEX(NAN, STATE, "TK", pu1Tk, u1TkLen);
...
nanSecDumpEapolKey(struct wpa_eapol_key *key) {
	TRACE_FUNC(NAN, DEBUG, "[%s] Enter\n");
	DBGLOG(NAN, DEBUG, "[%s] type:0x%x\n", __func__, key->type);
	DBGLOG(NAN, DEBUG, "[%s] key_info:0x%x, 0x%x\n", __func__,
	       key->key_info[0], key->key_info[1]);
	DBGLOG(NAN, DEBUG, "[%s] key_length:0x%x, 0x%x\n", __func__,
	       key->key_length[0], key->key_length[1]);

	DBGDUMP_HEX(NAN, DEBUG, "replay counter",
		    key->replay_counter, WPA_REPLAY_COUNTER_LEN);

	DBGDUMP_HEX(NAN, DEBUG, "key_nonce", key->key_nonce, WPA_NONCE_LEN);

	DBGDUMP_HEX(NAN, DEBUG, "key_iv", key->key_iv, 16);

	DBGDUMP_HEX(NAN, DEBUG, "key_rsc", key->key_rsc, WPA_KEY_RSC_LEN);

	DBGDUMP_HEX(NAN, DEBUG, "key_id", key->key_id, 8);

	DBGDUMP_HEX(NAN, DEBUG, "key_mic", key->key_mic, 16);

	DBGLOG(NAN, DEBUG, "[%s] key_data_length:0x%x, 0x%x\n", __func__,
...
nanSecUpdateAttrCmd(struct ADAPTER *prAdapter, uint8_t aucAttrId,
	...
	DBGLOG(NAN, DEBUG, "[%s] Enter, aucAttrId:0x%x, aucAttrLen:%d\n",
...
nanSecUpdatePeerNDI(struct _NAN_NDP_INSTANCE_T *prNdp,
	...
	DBGLOG(NAN, DEBUG, "[%s] Enter, role:%d\n", __func__, prNdp->eNDPRole);
...
nanSecGtkCipherWfaToNan(uint16_t u2CipherId)
{
	...
	default:
		DBGLOG(NAN, ERROR,
			"Wfa Gtk cipher is not supported\n");
		return NAN_CIPHER_SUITE_ID_NONE;
	}
}
	...
	uint8_t ucNdiIdx)
{
	...
	if (ucNdiIdx < NAN_NDI_INDEX_NUM) {
		...
		wpa_auth->conf.wpa_group =
			nanSecGtkCipherNanToWfa(ucCipherId);
		wpa_auth->group->GTK_len =
			ucKeyLen;
		kalMemCpyS(wpa_auth->group->GTK[0],
			ucKeyLen,
			pucKey,
			ucKeyLen);
	} else {
		DBGLOG(NAN, ERROR,
			"invalid ucNdiIdx: %hhu\n",
			ucNdiIdx);
	}
}

void nanSecSetIgtkToSm(struct ADAPTER *prAdapter, uint8_t *pucMacAddr,
		       uint8_t ucCipherId, uint8_t ucKeyLen, uint8_t *pucKey,
		       uint8_t fgSelfKey, uint16_t u2WtblIdx)
{
	uint16_t u2WfaCipher = nanSecIgtkCipherNanToWfa(ucCipherId);
	uint8_t aucBCAddr[] = BC_MAC_ADDR;
	uint8_t aucZeroPn[] = NULL_MAC_ADDR;
	uint8_t ucIgtkAlgoId = CIPHER_SUITE_NONE;
#if (NAN_R4_BIP_NEGATIVE_TEST == 1)
	uint8_t aucAllZeroKey[32] = {0};
#endif
	...
	if (fgSelfKey) {
		...
		g_prNanHapdData->wpa_auth->conf.group_mgmt_cipher = u2WfaCipher;
		...
		for (i = 0; i < NAN_BSS_INDEX_NUM; i++) {
			...
			DBGLOG(NAN, INFO, "Install TX IGTK, Wtbl:%d\n",
				prnanBssInfo->ucBMCWlanIndex);

#if (NAN_R4_BIP_NEGATIVE_TEST == 1)
			nan_sec_wpas_setkey_glue(FALSE,
				prnanBssInfo->ucBssIndex,
				(enum wpa_alg)wpa_cipher_to_alg(u2WfaCipher),
				aucBCAddr, 4, aucAllZeroKey, ucKeyLen);
#else
			nan_sec_wpas_setkey_glue(FALSE,
				prnanBssInfo->ucBssIndex,
				(enum wpa_alg)wpa_cipher_to_alg(u2WfaCipher),
				aucBCAddr, 4, pucKey, ucKeyLen);
#endif /* NAN_R4_BIP_NEGATIVE_TEST */
		}
	} else {
		if (u2WtblIdx == WTBL_RESERVED_ENTRY) {
			DBGLOG(NAN, ERROR, "u2WtblIdx is WTBL_RESERVED_ENTRY");
			return;
		}
		ucIgtkAlgoId = nanSecGetCipherWpaToHw(u2WfaCipher);
		nanSecManageKeyCmd(prAdapter,
				   NAN_KEY_OP_SET_KEY,
...
void nanSecSetBigtkToSm(struct ADAPTER *prAdapter, uint8_t *pucMacAddr,
			uint8_t ucCipherId, uint8_t ucKeyLen, uint8_t *pucKey,
			uint8_t fgSelfKey, uint16_t u2WtblIdx)
{
	uint16_t u2WfaCipher = nanSecIgtkCipherNanToWfa(ucCipherId);
	...
	if (fgSelfKey) {
		...
		for (i = 0; i < NAN_BSS_INDEX_NUM; i++) {
			...
			nan_sec_wpas_setkey_glue(FALSE,
				prnanBssInfo->ucBssIndex,
				(enum wpa_alg)wpa_cipher_to_alg(u2WfaCipher),
				aucBCAddr, 6, aucAllZeroKey, ucKeyLen);
#else
			nan_sec_wpas_setkey_glue(FALSE,
				prnanBssInfo->ucBssIndex,
				(enum wpa_alg)wpa_cipher_to_alg(u2WfaCipher),
				aucBCAddr, 6, pucKey, ucKeyLen);
#endif /* NAN_R4_BIP_NEGATIVE_TEST */
		}
	} else {
		if (u2WtblIdx == WTBL_RESERVED_ENTRY) {
		...
		if (u2WfaCipher == WPA_CIPHER_AES_128_CMAC) {
			ucBigtkAlgoId =
				CIPHER_SUIT_BCN_PROT_CMAC_128_AFTER_IP_37;
		} else {
			ucBigtkAlgoId =
				CIPHER_SUIT_BCN_PROT_CMAC_256_AFTER_IP_37;
		}
...
uint16_t nanSecEncryptKde(uint8_t *kek, size_t kek_len, uint8_t *kde,
			  size_t kde_len, uint8_t *ciphertext)
{
	...
	if (kde) {
		...
		if (buf == NULL) {
			DBGLOG(NAN, ERROR,
				"Buffur allocate fail\n");
			return 0;
		}
		pos = buf;
		kalMemCpyS(pos, key_data_len, kde, kde_len);
		pos += kde_len;
		...
		DBGLOG(NAN, INFO,
			"key_data_len=%zu, pad_len=%zu", key_data_len, pad_len);
	...
	return (uint16_t)key_data_len;
}

uint8_t *nanSecComposeNikKde(uint8_t *kde, uint8_t nik_length, uint8_t *nik,
			     uint8_t cipher_version)
{
	struct NIK_KDE_INFO *prNikKde = NULL;

	TRACE_FUNC(NAN, INFO, "Enter %s\n");
...
uint8_t *nanSecComposeNikLifetimeKde(uint8_t *kde, uint16_t key_bitmap,
...
uint16_t nanSecAppendKde(uint8_t fgNik, uint8_t nik_length, uint8_t *nik,
			 uint8_t cipher_version, uint8_t fgNikLifetime,
			 uint16_t key_bitmap, uint32_t key_lifttime,
			 uint8_t fgGroupKey, uint8_t *kek, size_t kek_len,
			 uint8_t *payload)
{
	...
	DBGLOG_MEM8(NAN, INFO,
		(const uint8_t *)kde, kde_len);
	key_data_len = nanSecEncryptKde(kek,
		kek_len, kde, kde_len, payload);
	DBGLOG(NAN, INFO, "Cipher text KDEs\n");
	DBGLOG_MEM8(NAN, INFO,
		(const uint8_t *)payload, key_data_len);
...
void nanSecComposeEapolKey(struct _NAN_ATTR_SKDA_T *prAttrSkda, uint8_t *prKdes,
			   ...
			   uint32_t u4CipherType)
{
	...
	if (u4CipherType == NAN_CIPHER_SUITE_ID_NCS_PK_PASN_128) {
		eapol_key =
			(struct wpa_eapol_key *)(prAttrSkda->aucKeyDescriptor);
		...
		kalMemCpyS(prAttrSkda->aucKeyDescriptor + sizeof(*eapol_key),
			u2KeyLength,
			prKdes,
			u2KeyLength);
		u2TotalLength = u2KeyLength + sizeof(*eapol_key);
		nan_sec_wpa_eapol_key_mic(kck,
			kck_len,
			u4CipherType,
		...
		eapol_key_192 =
		       (struct wpa_eapol_key_192 *)prAttrSkda->aucKeyDescriptor;

		kalMemZero(eapol_key_192,
			sizeof(*eapol_key_192));
		eapol_key_192->type = EAPOL_KEY_TYPE_RSN;
		WPA_PUT_BE16(eapol_key_192->key_info,
			key_info);
		WPA_PUT_BE16(eapol_key_192->key_data_length,
			u2KeyLength);
		kalMemCpyS(prAttrSkda->aucKeyDescriptor +
				   sizeof(*eapol_key_192),
			   u2KeyLength,
			   prKdes,
			   u2KeyLength);
...
nanSecRxSkdaToHostFormat(uint8_t *prKeyDescriptor, uint32_t u4CipherType)
{
	...
	DBGLOG(NAN, INFO,
		"Convert the SKDA endianness to host format\n");
...
#endif
void nanSecGetTxIgtkBigtk(uint32_t *pucIgtkCipher, uint8_t *pucIgtkKey,
			  size_t *pucIgtkLen, uint32_t *pucBigtkCipher,
			  uint8_t *pucBigtkKey, size_t *pucBigtkLen)
{
	struct wpa_authenticator *prWpaAuth = g_prNanHapdData->wpa_auth;

	*pucIgtkCipher = prWpaAuth->conf.group_mgmt_cipher;
	*pucIgtkLen = wpa_cipher_key_len(prWpaAuth->conf.group_mgmt_cipher);
	kalMemCpyS(pucIgtkKey,
		   *pucIgtkLen,
		   prWpaAuth->group->IGTK[0],
		   *pucIgtkLen);

	if (prWpaAuth->conf.beacon_prot) {
		*pucBigtkCipher = prWpaAuth->conf.group_mgmt_cipher;
		*pucBigtkLen =
			wpa_cipher_key_len(prWpaAuth->conf.group_mgmt_cipher);
		kalMemCpyS(pucBigtkKey,
			   *pucBigtkLen,
			   prWpaAuth->group->BIGTK[0],
			   *pucBigtkLen);
	}
}

void nanSecConfigNmiIgtk(struct ADAPTER *prAdapter, uint8_t fgSet,
			 uint16_t u2WtblEntry, uint8_t *pucLocalNMI,
			 uint8_t *pucPeerNMI, struct _NAN_NDP_INSTANCE_T *prNdp)
{
...
void nanSecConfigNmiBigtk(struct ADAPTER *prAdapter, uint8_t fgSet,
			  uint16_t u2WtblEntry, uint8_t *pucLocalNMI,
			  uint8_t *pucPeerNMI,
			  struct _NAN_NDP_INSTANCE_T *prNdp)
{
	...
	if (fgSet) {
		pu1Bigtk =
			prNdp->prResponderSecSmInfo->bigtk.bigtk;
		szBigtkLen =
			prNdp->prResponderSecSmInfo->bigtk.bigtk_len;
		u4Cipher =
			prNdp->prResponderSecSmInfo->mgmt_group_cipher;
		pucBigtkPn =
			prNdp->prResponderSecSmInfo->bigtk_pn;
...
uint8_t nanSecGetGtkCipherByNdi(uint8_t ucNdiIdx)
{
	struct wpa_authenticator *wpa_auth = NULL;

	wpa_auth = &g_rNanWpaAuth[ucNdiIdx];

	return nanSecGtkCipherWfaToNan(
		(uint16_t)wpa_auth->conf.wpa_group);
}

uint8_t nanSecIsGroupCipherSupported(uint8_t ucGtkCipherType)
{
...
uint8_t nanSecIsDevSupportGroupSecurity(struct ADAPTER *prAdapter)
{
	struct WIFI_VAR *prWifiVar = NULL;
...
uint8_t nanSecGetGroupSecurityCap(struct ADAPTER *prAdapter,
				  struct _NAN_NDP_INSTANCE_T *prNDP)
{
	struct WIFI_VAR *prWifiVar = NULL;
	...
	if (ucGroupSASupport ==
		...
		ucIgtkCipher =
			...
			(uint8_t)CSIA_CAP_IGTKSA_BIGTKSA_CS_SHFT) &
			(uint8_t)CSIA_CAP_IGTKSA_BIGTKSA_CS_MASK);
	}

	ucGroupSASupport =
		((ucGroupSASupport <<
		(uint8_t)CSIA_CAP_GTKSA_IGTKSA_BIGTKSA_SUPPORT_SHFT) &
		(uint8_t)CSIA_CAP_GTKSA_IGTKSA_BIGTKSA_SUPPORT_MASK);
	...
	int32_t *pi4TmpGroupCipher, int32_t *pi4TmpGroupMgmtCipher)
{
	...
	if (pfgGtk == NULL || pfgIgtk == NULL || pfgBigtk == NULL ||
	    pi4TmpGroupCipher == NULL || pi4TmpGroupMgmtCipher == NULL) {
	...
	/* Select chipher suit */
	if (*pfgGtk) {
		if (ucGtkCipherType == NAN_CIPHER_SUITE_ID_NCS_GTK_GCM_256) {
...
uint32_t nanSecSetGroupCipherType(struct _NAN_NDP_INSTANCE_T *prNdp,
				  uint8_t fgIsTx)
{
	...
	struct wpa_authenticator *wpaAuth = NULL;
	...
	DBGLOG(NAN, INFO, "GtkReq:%u\n", prNdp->fgTxGtkRequired);

	/* Assign into state machine */
	if (fgIsTx) {
		/* group data protection use NDI configuration */
		wpaAuth = &g_rNanWpaAuth[prNdp->ucNdiIndex];
		conf = &wpaAuth->conf;
		wpaStateMachine =
			prNdp->prInitiatorSecSmInfo;
		...
		if ((fgGtk == 0) || (prNdp->fgTxGtkRequired == 0)) {
			...
			if (conf->wpa_group != (int)i4TmpGroupCipher) {
				DBGLOG(NAN, ERROR,
					"Group Cipher mismatch %d, %d, Reconfigure %d\n",
					conf->wpa_group, i4TmpGroupCipher,
					!wpaAuth->group->first_sta_seen);
				/* Overwrite GTK if no STA connected */
				if (wpaAuth->group->first_sta_seen == 0) {
					int wpa_group;

					conf->wpa_group = i4TmpGroupCipher;
					wpa_group = conf->wpa_group;
					wpaAuth->group->GTK_len =
						wpa_cipher_key_len(wpa_group);
				}
			}
		}
		if (fgIgtk || fgBigtk)
...
uint32_t nanSecSetGroupSA(struct _NAN_NDP_INSTANCE_T *prNdp)
{
	...
	if (prNdp->fgTxGtkRequired && prNdp->fgRxGtkRequired)
		prNdp->ucGroupSAUsed |=
			NAN_GROUP_SECURITY_ASSOC_USE_GTKSA;
	else
		prNdp->ucGroupSAUsed &=
			~NAN_GROUP_SECURITY_ASSOC_USE_GTKSA;
...
int nan_sec_ndi_wpa_auth_init(size_t szNdiIdx, uint8_t ucGtkCipherType)
{
	struct wpa_authenticator *wpa_auth = NULL;
	...
	if (szNdiIdx < ARRAY_SIZE(g_rNanWpaAuth)) {
		...
		nanDevGetNdiAddress(g_prAdapter,
			(uint8_t)szNdiIdx, ucNDIAddr);

		cb.set_key =
			nan_sec_hostapd_wpa_auth_set_key;
		cb.get_seqnum =
			hostapd_wpa_auth_get_seqnum;
		cb.for_each_sta =
			hostapd_wpa_auth_for_each_sta;
...
#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_FRAME_PROT == 1)
		conf.wpa_group = nanSecGtkCipherNanToWfa(ucGtkCipherType);
...
nan_sec_hostapd_wpa_auth_conf_group_cipher(struct wpa_auth_config *wconf)
{
	...
	wconf->wpa_group = i4TmpGroupCipher;

	wconf->group_mgmt_cipher = i4TmpGroupMgmtCipher;
...
#ifdef NAN_TODO
	wconf->wpa_group = nanSecGtkCipherNanToWfa(0);
	wconf->group_mgmt_cipher = (int)WPA_CIPHER_NONE;
	wconf->ieee80211w = NO_MGMT_FRAME_PROTECTION;
	wconf->beacon_prot = 0;
	wconf->wpa = 2;
#endif
...
#ifdef DALE
		atomic_dec(&(prNmiCxtKey->ulRefCount));
		if (atomic_read(&(prNmiCxtKey->ulRefCount)) == 0)
#else
		if (atomic_dec_return(&(prNmiCxtKey->ulRefCount)) == 0)
#endif
		{
			fgRelease = TRUE;
		}
...
void nanSecReleaseAllNmiCxtKey(struct ADAPTER *prAdapter)
{
	...
	for (ucIdx = 0; ucIdx < NAN_NUM_NMI_CXT_KEY; ucIdx++) {
		...
		if (prNmiCxtKey->fgValid == TRUE) {
			 ...
			 * when NAN stack is unload so it will directly reset
			 * ref count to 1 then call nanSecReleaseNmiCxtKey.
			 **/
...
void
nanSecConfigNmiTk(struct ADAPTER *prAdapter,
...
nanSecConfigNdiGtk(struct ADAPTER *prAdapter,
	...
	if (fgSet) {
		...
		nanSecManageKeyCmd(prAdapter,
				...
				ucAlgoId, 1, (uint8_t)szGtkLen, pu1Gtk, NULL,
				FALSE, TRUE);
	} else {
		nanSecManageKeyCmd(prAdapter,
	...
	uint8_t *pucPeerNMI)
{
	...
	for (ucIdx = 0; ucIdx < NAN_NUM_NMI_CXT_KEY; ucIdx++) {
		prNmiKey = &g_arNanNmiCxtKey[ucIdx];
		if (prNmiKey->fgValid &&
		    kalMemCmp(prNmiKey->aucNmiAddr,
		    pucPeerNMI, MAC_ADDR_LEN) == 0)
			return prNmiKey;
	}
...
nanSecUtApKdeAttr(void) {
	...
	if (rStatus == WLAN_STATUS_SUCCESS) {
		...
		DBGLOG(NAN, DEBUG, "[%s] gen KDE failed!\n", __func__);
	}
}
...
nanSecUtPbkdf256(void) {
	...
	DBGLOG(NAN, DEBUG, "[%s] Enter, p_len:%d, s_len:%d\n", __func__,
	       sizeof(passphrase), sizeof(salt));

	DBGDUMP_HEX(NAN, DEBUG, "passphrase", passphrase, sizeof(passphrase));
	DBGDUMP_HEX(NAN, DEBUG, "salt", salt, sizeof(salt));

	PKCS5_PBKDF2_HMAC((unsigned char *)passphrase, sizeof(passphrase) - 1,
			  (unsigned char *)salt, sizeof(salt), 4096, 32,
			  (unsigned char *)key);

	DBGDUMP_HEX(NAN, DEBUG, "key", key, 32);
}

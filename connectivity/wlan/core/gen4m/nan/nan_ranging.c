...
#define CFG_RANGING_SESSION_TO_ENABLE 0
/*******************************************************************************
 *                             D A T A   T Y P E S
 *******************************************************************************
 */
...
nanRangingInstanceInit(struct ADAPTER *prAdapter,
	...
	COPY_MAC_ADDR(prRanging->ranging_ctrl.aucPeerAddr, puc_peer_mac);
	...
	prRanging->ranging_ctrl.rNanFtmParam.ucFTMNum = 4;
	prRanging->ranging_ctrl.rNanFtmParam.ucMinDeltaIn100US = 40;
...
nanRangingInstanceDel(struct ADAPTER *prAdapter,
	...
	if (pairingFsmSearch(prAdapter, prRanging->ranging_ctrl.aucPeerAddr))
		fgKeepStaRec = TRUE;
#endif /* CFG_SUPPORT_NAN_R4_PAIRING */

	if (fgKeepStaRec == FALSE) {
		for (ucIdx = NAN_BSS_INDEX_BAND0;
			ucIdx < NAN_BSS_INDEX_NUM; ucIdx++) {
			prNanSpecificBssInfo =
					nanGetSpecificBssInfo(prAdapter, ucIdx);
		     ...
		     struct _NAN_RANGING_INSTANCE_T *prRanging)
{
	...
	if (prRanging->ranging_ctrl.b4Status == NAN_RANGING_STATUS_ACCEPTED) {
		DBGLOG(NAN, DEBUG, "Allocate StaRec for Ranging\n");
		for (ucIdx = NAN_BSS_INDEX_BAND0;
		     ucIdx < NAN_BSS_INDEX_NUM;
		     ucIdx++) {
			prNanSpecificBssInfo =
				nanGetSpecificBssInfo(prAdapter, ucIdx);
			prStaRec = cnmStaRecAlloc(prAdapter, STA_TYPE_NAN,
				prNanSpecificBssInfo->ucBssIndex,
				prRanging->ranging_ctrl.aucPeerAddr);
...
nanRangingFtmParamCmd(struct ADAPTER *prAdapter,
	...
	kalMemCopy(&prFtmParam->rNanRangingCfg,
		   &prRanging->ranging_ctrl.ranging_cfg,
		   sizeof(struct NanRangingCfg));

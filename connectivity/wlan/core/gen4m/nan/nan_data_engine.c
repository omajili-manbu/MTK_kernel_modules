...
static struct _APPEND_ATTR_ENTRY_T txDataAttributeTable[] = {
	...
	{ NAN_ATTR_ID_COUNTRY_CODE, nanDataEngineCountryCodeAttrLength,
	  nanDataEngineCountryCodeAttrAppend },
	...
	{ NAN_ATTR_ID_UNALIGNED_SCHEDULE, nanDataEngineUlwAttrLength,
	...
	uint32_t u4NdpInstanceId)
{
	...
	DBGLOG(NAN, ERROR,
		"Invalid NDP (fg = %d, id = %d), instance ID = %d\n",
...
nanDataUtilSearchPeerSchDescByNdpInstanceId(struct ADAPTER *prAdapter,
					    uint32_t u4NdpInstanceId)
{
	uint8_t ucNdlIndex;
	uint8_t ucNdpIndex;
	...
	prDataPathInfo = &(prAdapter->rDataPathInfo);
	if (ucNdlIndex >= ARRAY_SIZE(prDataPathInfo->arNDL))
		return NULL;

	prNDL = &(prDataPathInfo->arNDL[ucNdlIndex]);

	if (ucNdpIndex >= ARRAY_SIZE(prNDL->arNDP))
		return NULL;

	if (!prNDL->arNDP[ucNdpIndex].fgNDPValid)
		return NULL;
	if (prNDL->arNDP[ucNdpIndex].ndp_instance_id != u4NdpInstanceId)
		return NULL;

	return nanSchedSearchPeerSchDescByNmi(prAdapter, prNDL->aucPeerMacAddr);
}
...
/*----------------------------------------------------------------------------*/
/*!
 * \brief Find an NDL in prAdapter->rDataPathInfo by matching MAC address
 *        pucAddr
 *
 * \param[in]
 *
 * \return return pointer to matched NDL if
 *	a) prNDL->aucPeerMacAddr equals pucAddr, or
 *	b) any valid NDP prNDL->arNDP[u4Idx].aucPeerNDIAddr equals pucAddr
 */
/*----------------------------------------------------------------------------*/
...
nanDataUtilSearchNdlByMac(struct ADAPTER *prAdapter, uint8_t *pucAddr)
{
...
#if (ENABLE_NDP_UT_LOG == 1)
	DBGLOG(NAN, DEBUG, "Search %02x:%02x:%02x:%02x:%02x:%02x\n",
	...
	for (ucNdlIndex = 0; ucNdlIndex < NAN_MAX_SUPPORT_NDL_NUM;
	     ucNdlIndex++) {
		if (!prDataPathInfo->arNDL[ucNdlIndex].fgNDLValid)
			continue;
		...
		for (u4Idx = 0; u4Idx < NAN_MAX_SUPPORT_NDP_NUM; u4Idx++) {
			if (!prNDL->arNDP[u4Idx].fgNDPValid)
				continue;
		   ...
		   uint8_t ucNDPIfaecId)
{
	...
	/* TODO */
	prNDP->ucNdiIndex = NAN_NDI_INDEX_0;
...
/*----------------------------------------------------------------------------*/
uint8_t *
nanDataGetLocalNDIAddr(struct ADAPTER *prAdapter,
			 uint8_t ucNdiIndex)
{
...
nanDataFreeNdl(struct ADAPTER *prAdapter, struct _NAN_NDL_INSTANCE_T *prNDL) {
	...
	if (prDataPathInfo->ucNDLNum == 1) {
		...
		if (!prAdapter->rWifiVar.ucNanVendorIoctl &&
			...
			DBGLOG(NAN, INFO,
			       "In suspend mode, disable FC and SetDWInterval 8\n");
			nanSetFlashCommunication(prAdapter, FALSE);
			nanDevSetDWInterval(prAdapter, 8);
		}
	}
#endif
...
#if (CFG_SUPPORT_NAN_MGMT_TX == 1)
	prMgmtTxReqInfo = &prAdapter->rNanMgmtTxInfo;
...
nanNdpAutoReplyDataRequest(struct ADAPTER *prAdapter,
	...
	prAttrNDP = nanRetrieveAttrById(prNDP->pucAttrList,
					prNDP->u2AttrListLength,
					NAN_ATTR_ID_NDP, 0);
	prAttrNDPE = nanRetrieveAttrById(prNDP->pucAttrList,
					 prNDP->u2AttrListLength,
					 NAN_ATTR_ID_NDP_EXTENSION, 0);
	prAttrNDL = nanRetrieveAttrById(prNDP->pucAttrList,
					prNDP->u2AttrListLength,
					NAN_ATTR_ID_NDL, 0);

	DBGDUMP_HEX(NAN, DEBUG, "Peer NMI",
		prNDL->aucPeerMacAddr, MAC_ADDR_LEN);
	...
	if (prDataPathInfo->fgAutoHandleDPRequest == TRUE) {
		...
		if (prAttrNDP != NULL) {
			COPY_MAC_ADDR(prAdapter->rDataPathInfo.aucRemoteAddr,
...
#if (CFG_SUPPORT_NAN_R4_PAIRING == 1)
			DBGLOG(NAN, INFO,
			"[%s] peerNDI=%02x:%02x:%02x:%02x:%02x:%02x\n",
			__func__,
		...
		if (nanGetFeatureNDPE(prAdapter)) {
			...
			if (prDataPathInfo->u2AppInfoLen > 0) {
				...
				DBGLOG(NAN, DEBUG, "App InfoLen = %d:\n",
				       prNDP->u2AppInfoLen);
			}
...
nanNdpProcessDataRequest(struct ADAPTER *prAdapter, struct SW_RFB *prSwRfb) {
	...
	prAttrNDP = nanRetrieveAttrById(pucAttrList, u2AttrListLength,
					NAN_ATTR_ID_NDP, 0);
	prAttrNDPE = nanRetrieveAttrById(pucAttrList, u2AttrListLength,
					 NAN_ATTR_ID_NDP_EXTENSION, 0);
	prAttrNDL = nanRetrieveAttrById(pucAttrList, u2AttrListLength,
					NAN_ATTR_ID_NDL, 0);
	...
	if (prNDL) {
		...
		if (prNDL) {
			...
			prNDP = nanDataAllocateNdp(
				...
				aucInitiatorNDI, ucNDPID,
				fgSecurityRequired, 0);
		}
	}
	...
	if (prNDL) {
		...
		if (prNDL->eCurrentNDLMgmtState ==
			...
			if (nanNdlParseAttributes(prAdapter,
				    NAN_ACTION_SCHEDULE_CONFIRM,
				    pucAttrList, u2AttrListLength, prNDL) ==
			    WLAN_STATUS_SUCCESS) {
				rStatus = nanSchedNegoChkRmtCrbProposal(
					prAdapter, &u4RejectCode);
				DBGLOG(NAN, INFO, "Status=%u\n", rStatus);
		  ...
		  struct _NAN_NDL_INSTANCE_T *prNDL)
{
	...
	do {
		...
		case NDL_SCHEDULE_ESTABLISHED:
			...
			prNDL->fgScheduleEstablished = TRUE;
			nanSchedSetLocalCommittedBand(prAdapter);
		...
		case NDL_TEARDOWN:
			...
			/* Prefer band might be switched */
			nanUpdateStaRecIdx(prAdapter, prNDL);
...
#if (CFG_SUPPORT_NAN_R4_PAIRING == 1)
		prPairingFsm =
			pairingFsmSearch(prAdapter, prNDL->aucPeerMacAddr);

		if (prPairingFsm) {
			DBGLOG(NAN, INFO, "nanPairingHandleNdlDisconnect1\n");
			nanPairingHandleNdlDisconnect(prAdapter, prPairingFsm);
		} else {
			DBGLOG(NAN, INFO, "nanPairingHandleNdlDisconnect\n");
		}
#endif /* CFG_SUPPORT_NAN_R4_PAIRING */
				nanDataFreeNdl(prAdapter, prNDL);
...
void nanSetNdpPmkid(struct ADAPTER *prAdapter,
		  ...
		  uint8_t *pu1NdpId, uint8_t *au1InitiatorDataAddr)
{
	...
	unsigned char fgAllocNDL = FALSE;
#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_DATA == 1)
	u_int8_t aucDefaultMcAddr[6] = {0x33, 0x33, 0, 0, 0, 0x1};
...
#endif /* CFG_SUPPORT_NAN_R4_PAIRING */
#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_DATA == 1)
	uint8_t aucMacAddrOverride[MAC_ADDR_LEN];
#endif /* CFG_NAN_SUPPORT_R4_GROUP_ADDR_DATA */
	...
	DBGDUMP_HEX(NAN, INFO, "ResponderDataAddress",
...
#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_DATA == 1)
	if (IS_BMCAST_MAC_ADDR(prNanCmdDataRequest->aucMulticastAddress)) {
		kalMemCopy(prNDP->aucNanMulticastAddr,
			prNanCmdDataRequest->aucMulticastAddress,
			NAN_MAC_ADDR_LEN);
	} else {
		kalMemCopy(prNDP->aucNanMulticastAddr,
			aucDefaultMcAddr, NAN_MAC_ADDR_LEN);
	}
	if (prAdapter->rWifiVar.ucNanMulticastAddrOverride) {
		wlanHwAddrToBin(
			prAdapter->rWifiVar.aucNanMulticastAddrStr,
			aucMacAddrOverride);
		COPY_MAC_ADDR(prNDP->aucNanMulticastAddr, aucMacAddrOverride);
	}

	DBGLOG(NAN, DEBUG,
		"[MCDATA]=> %02x:%02x:%02x:%02x:%02x:%02x\n",
		prNDP->aucNanMulticastAddr[0], prNDP->aucNanMulticastAddr[1],
		prNDP->aucNanMulticastAddr[2], prNDP->aucNanMulticastAddr[3],
		prNDP->aucNanMulticastAddr[4], prNDP->aucNanMulticastAddr[5]);
#endif /* CFG_NAN_SUPPORT_R4_GROUP_ADDR_DATA */
	...
	if (prNanCmdDataRequest->ucSecurity) {
		...
		(prPairingFsm->ePairingState == NAN_PAIRING_PAIRED)) {
			DBGLOG(NAN, INFO,
			"[%s]PAIRED. go nanSecSetPmk(initiator)\n", __func__);
			DBGDUMP_HEX(NAN, DEBUG,
				"NAN ND-PMK",
				prPairingFsm->key_info.body.
				pmk_info.pmk,
	...
	DBGLOG(NAN, DEBUG, "Update App info len = %d and QoS:%d\n",
	       prNDP->u2AppInfoLen, prNanCmdDataRequest->ucRequireQOS);
...
/* insert into queue for later handling */
	...
	*pu1NdpId = prNDP->ucNDPID;
	COPY_MAC_ADDR(au1InitiatorDataAddr, prNDP->aucLocalNDIAddr);
		   ...
		   struct _NAN_CMD_DATA_RESPONSE *prNanCmdDataResponse)
{
...
#endif /* CFG_SUPPORT_NAN_R4_PAIRING */
#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_DATA == 1)
	uint8_t aucMacAddrOverride[MAC_ADDR_LEN] = {0};
#endif /* CFG_NAN_SUPPORT_R4_GROUP_ADDR_DATA */
...
#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_DATA == 1)
	if (IS_BMCAST_MAC_ADDR(prNanCmdDataResponse->aucMulticastAddress)) {
		kalMemCopy(prNDP->aucNanMulticastAddr,
...
#if (CFG_SUPPORT_NAN_R4_PAIRING == 1)
	prPairingFsm = pairingFsmSearch(prAdapter, prNDL->aucPeerMacAddr);
	if (fgIsNanPairingEn && prPairingFsm &&
	    prPairingFsm->ePairingState == NAN_PAIRING_PAIRED) {
		prNanCmdDataResponse->ucSecurity =
			NAN_CIPHER_SUITE_ID_NCS_PK_PASN_128;
		DBGLOG(NAN, DEBUG, "Set Pairing Cipher %u\n",
			prNanCmdDataResponse->ucSecurity);
	}
#endif /* CFG_SUPPORT_NAN_R4_PAIRING */
	...
	if (prNDP->fgSecurityRequired) {
		struct _GL_NAN_INFO_T *prNANInfo =
			(struct _GL_NAN_INFO_T *)NULL;

		/* Multi-NDI: TODO */
		prNANInfo = prAdapter->prGlueInfo->aprNANDevInfo[0];
	...
	if (prNanCmdDataResponse->ucDecisionStatus ==
		...
		if (prNanCmdDataResponse->ucSecurity) {
			...
			(prPairingFsm->ePairingState == NAN_PAIRING_PAIRED)) {
				DBGLOG(NAN, INFO,
					"PAIRED. go nanSecSetPmk(responder)\n");
				DBGDUMP_HEX(NAN, DEBUG,
					"NAN ND-PMK",
					prPairingFsm->key_info.body.
					pmk_info.pmk,
	      ...
	      struct _NAN_CMD_DATA_END *prNanCmdDataEnd)
{
	...
	COPY_MAC_ADDR(prNanCmdDataEnd->aucInitiatorDataAddress,
		      prAdapter->rDataPathInfo.aucRemoteAddr);

#if (ENABLE_NDP_UT_LOG == 1)
	TRACE_FUNC(NAN, DEBUG, "[%s] Enter\n");
	DBGLOG(NAN, DEBUG, "ucNDPId:%d\n", prNanCmdDataEnd->ucNDPId);
	DBGLOG(NAN, DEBUG,
		    "InitiatorDataAddress %02x:%02x:%02x:%02x:%02x:%02x",
		    prNanCmdDataEnd->aucInitiatorDataAddress[0],
		    prNanCmdDataEnd->aucInitiatorDataAddress[1],
		    prNanCmdDataEnd->aucInitiatorDataAddress[2],
		    prNanCmdDataEnd->aucInitiatorDataAddress[3],
		    prNanCmdDataEnd->aucInitiatorDataAddress[4],
		    prNanCmdDataEnd->aucInitiatorDataAddress[5]);
#endif
...
nanUpdateNdlSchedule(struct ADAPTER *prAdapter,
	...
	if (prNDL == NULL ||
		...
		if (prNDL->eCurrentNDLMgmtState == NDL_IDLE ||
			...
			DBGLOG(NAN, DEBUG, "Update SCH MinTimeSlot %d MaxLT %d",
			       prNDL->ucMinimumTimeSlot,
			       prNDL->u2MaximumLatency);
...
nanNdpSendDataIndicationEvent(struct ADAPTER *prAdapter,
	...
	COPY_MAC_ADDR(rDataReqInd.peer_disc_mac_addr, prNDL->aucPeerMacAddr);
	...
	if (prNDP->fgSecurityRequired) {
		...
		DBGLOG(NAN, DEBUG, "uCipher:%d\n", rDataReqInd.uCipher);
		DBGDUMP_HEX(NAN, DEBUG, "SCID",
			    rDataReqInd.aucSCID, NAN_SCID_DEFAULT_LEN);
	}
...
nanNdpSendDataConfirmEvent(struct ADAPTER *prAdapter,
	...
	COPY_MAC_ADDR(rDataConfirmInd.peer_ndi_mac_addr, prNDP->aucPeerNDIAddr);
				   ...
				   struct _NAN_NDP_INSTANCE_T *prNDP)
{
	...
	if (prPairingFsm &&
		...
		DBGLOG(NAN, INFO,
		"[%s] get starec from pairing, StaRecIndex=%d\n",
	...
	uint8_t *pucLocalMacAddr)
{
	...
	for (szNdlIndex = 0;
		szNdlIndex < NAN_MAX_SUPPORT_NDL_CONFIG_NUM; szNdlIndex++) {
	...
	struct _NAN_NDP_INSTANCE_T *prNDP)
{
	...
	if ((prAdapter == NULL) || (prNDP == NULL)) {
		DBGLOG(NAN, ERROR, "prAdapter or prNDP is null\n");
		return NULL;
	}
...
)
{
	...
	if (!pucLocalNdiAddr) {
		DBGLOG(NAN, ERROR,
			"[MCDATA] MC Tx entry release fail, lack NDI\n");
		return;
	}
...
nanInitMcTxLookUpEntry(
	struct ADAPTER *prAdapter,
	struct _NAN_MC_TX_LOOKUP *prMcTxLookUpTbl,
...
)
{
	...
	if (prMcTxLookUpTbl->prNdcMgmt == NULL) {
		...
		kalMemCopy(prMcTxLookUpTbl->aucLocalNdiAddr,
			pucLocalNdiAddr, MAC_ADDR_LEN);

		if (pucNanMulticastAddr)
			kalMemCopy(prMcTxLookUpTbl->aucMulticastAddr,
...
void nanDumpMcTxWtbl(struct ADAPTER *prAdapter)
{
	size_t szMcTxTblIdx = 0, szIdx = 0;
	struct _NAN_MC_TX_LOOKUP *prMcTxLookUpTbl = NULL;
	struct _NAN_NDC_MGMT_T *prNdcMgmt = NULL;
	uint8_t aucNullAddr[] = NULL_MAC_ADDR;
	uint8_t *pucPeerNdiAddr = NULL;
	...
	for (szMcTxTblIdx = 0; szMcTxTblIdx < NAN_MC_TX_LOOKUP_TBL_NUM;
		...
		if (prMcTxLookUpTbl->prNdcMgmt == NULL)
			continue;
		prNdcMgmt = prMcTxLookUpTbl->prNdcMgmt;

		if (prMcTxLookUpTbl->prNDP)
			pucPeerNdiAddr = prMcTxLookUpTbl->prNDP->aucPeerNDIAddr;
		else
			pucPeerNdiAddr = aucNullAddr;

		DBGLOG(NAN, INFO,
			"[MC TX] WID:%zu,MC="MACSTR",L="MACSTR",P="MACSTR"\n",
			szMcTxTblIdx,
			MAC2STR(prMcTxLookUpTbl->aucMulticastAddr),
			MAC2STR(prMcTxLookUpTbl->aucLocalNdiAddr),
			MAC2STR(pucPeerNdiAddr));

		for (szIdx = 0; szIdx < NAN_MAX_MC_TX_WTBL_NUM; szIdx++) {
			if (atomic_read(&prNdcMgmt->mcTxRefCount[szIdx]) == 0)
				continue;

			DBGLOG(NAN, INFO,
				"[MC TX] wtbl entry:%zu,widx:%u,key:%u,dbdc:%u\n",
				szIdx, prNdcMgmt->au2BmcTxWtblIdx[szIdx],
				prNdcMgmt->fgIsKeyExist[szIdx],
				(uint32_t)prNdcMgmt->eTxDbdcIdx);
		}
	}
}

void
nanRegisterTxWtblForNdc(
	struct ADAPTER *prAdapter,
	...
	struct _NAN_NDP_INSTANCE_T *prNDP)
{
	...
	if (prNDL == NULL) {
		DBGLOG(NAN, ERROR, "[MCDATA] prNDL null\n");
		goto MC_ERR;
	}

	if (prNdcCtrl == NULL) {
		DBGLOG(NAN, ERROR,
			"[MCDATA] prNdcCtrl NULL\n");
		goto MC_ERR;
	}
	...
	/* Force update ndc band */
	nanSchedUpdateActiveNdcBands(prAdapter);

	DBGLOG(NAN, STATE, "[MCDATA] prNDL state: %d, ndcBand %d\n",
		prNDL->eCurrentNDLMgmtState, prScheduler->ucNdcBand);

	if (prNDL->eCurrentNDLMgmtState ==
		NDL_SCHEDULE_ESTABLISHED) {
		for (i = 0; i < NAN_BSS_INDEX_NUM; i++) {
			if (!(prScheduler->ucNdcBand & BIT(i)))
				continue;

			DBGLOG(NAN, STATE,
				"[MCDATA] Update tx mc WTBL bn:%d\n", i);
			...
			if (nanRegisterMcTxWtblIdx(prAdapter,
				...
				== WLAN_STATUS_SUCCESS) {
				nanDataEngineSetupTxMcWtbl(prAdapter,
					prNdcMgmt, prNdcCtrl,
					prNDP, fgFirst, prNDP->ucNdiIndex);
			}
		}
	}
MC_ERR:
			...
			uint8_t *pucMacAddr)
{
	...
	for (i = NAN_NDI_INDEX_0; i < NAN_MAX_MULTI_NDI_NUM; i++) {
		...
		if (kalMemCmp(pucMacAddr, pucNdiAddr, MAC_ADDR_LEN) == 0) {
			ucNdiIdx = i;
			break;
		}
	}

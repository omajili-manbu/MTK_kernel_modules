...
uint8_t
nanGetBssIdxByRole(struct ADAPTER *prAdapter,
	...
	uint8_t ucNdiIdx, uint8_t *pucNdiAddr)
{
	...
	DBGLOG(NAN, INFO,
		"NDI#%u, MAC dev addr in netdev [" MACSTR "]\n",
...
nanDevSendEnableRequestToCnm(struct ADAPTER *prAdapter)
{
	...
	for (ucIdx = 0; ucIdx < NAN_BSS_INDEX_NUM; ucIdx++) {
		...
		prnanBssInfo = GET_BSS_INFO_BY_INDEX(
					prAdapter, prNANSpecInfo->ucBssIndex);
		if (prnanBssInfo == NULL) {
			DBGLOG(NAN, ERROR, "prnanBssInfo [%d] is NULL\n",
			       prNANSpecInfo->ucBssIndex);
			return WLAN_STATUS_FAILURE;
		}
...
void nanDevSetDWInterval(struct ADAPTER *prAdapter, uint8_t ucDWInterval)
{
	...
	DBGLOG(NAN, DEBUG,
		"DW=%u, DB=%u, VendorIoctl=%u, Suspend=%u\n",
		ucDWInterval,
		prAdapter->rWifiVar.u2NanDiscBcnInterval,
	...
	prCmdNanDWInterval->ucNanVendorIoctl =
		prAdapter->rWifiVar.ucNanVendorIoctl;

	prCmdNanDWInterval->u2NanDiscBcnInterval =
		prAdapter->rWifiVar.u2NanDiscBcnInterval;
	...
	struct ADAPTER *prAdapter)
{
	...
	for (ucIdx = 0; ucIdx < NAN_BSS_INDEX_NUM; ucIdx++) {
		...
		ccmChannelSwitchProducer(prAdapter, nan, __func__);

		if (fgIsSingleSap)
			break;
	}

	ccmRegisterStableCb(prAdapter, nanCcmStableCallback);

				   ...
				   *prAdapter, struct STA_RECORD *prStaRec)
{
	...
	if (IS_BSS_INDEX_WON(prAdapter, prBssInfo->ucBssIndex)) {
		  ...
		  struct CMD_INFO *prCmdInfo, uint8_t ucTC)
{
	...
	if (prCmdInfo->eCmdType == COMMAND_TYPE_MANAGEMENT_FRAME) {
		...
		if (prMsduInfo->fgIs802_11 != TRUE) {
			DBGLOG(TX, ERROR, "fgIs802_11=%u, eSrc=%u",
			       prMsduInfo->fgIs802_11,
			       prMsduInfo->eSrc);
			DBGDUMP_HEX(TX, ERROR, "MGMT Frame:",
				    prMsduInfo->prPacket,
				    prMsduInfo->u2FrameLength);
			ASSERT(0);
		}
...
static void nicTxMarkCriticalPkt(struct ADAPTER *prAdapter,
	struct MSDU_INFO *prMsduInfo, void *prPacket)
{
	if (GLUE_TEST_PKT_FLAG(prPacket, ENUM_PKT_TCP_ACK))
		prMsduInfo->fgIsCritical = TRUE;
	else
		prMsduInfo->fgIsCritical = FALSE;
}
			   ...
			   struct MSDU_INFO *prMsduInfo, void *prPacket)
{
	...
	if (IS_BSS_INDEX_WON(prAdapter,
		...
		prWlanHeader = (struct WLAN_MAC_HEADER *)prSkb->data;
#if CFG_WONDERTAP_TX_RX_DEBUG
		DBGLOG(WON, DEBUG, "bss: %u\n", prMsduInfo->ucBssIndex);
		DBGLOG(WON, DEBUG, "aucEthDestAddr: " MACSTR "\n",
			MAC2STR(prMsduInfo->aucEthDestAddr));
#endif
		COPY_MAC_ADDR(prMsduInfo->aucEthDestAddr,
...
#if CFG_SUPPORT_NAN
	kalGetEthSrcAddr(prAdapter->prGlueInfo, prPacket,
			  prMsduInfo->aucEthSrcAddr);
#endif
...
#if CFG_SUPPORT_WONDERTAP
	if (IS_BSS_INDEX_WON(prAdapter, prMsduInfo->ucBssIndex)) {
		nicTxConfigPktControlFlag(prMsduInfo,
			MSDU_CONTROL_FLAG_DIS_MAT,
			TRUE);
#if CFG_WONDERTAP_TX_RX_DEBUG
		DBGLOG(WON, DEBUG,
			"Bssidx %u data set DIS_MAT",
			prMsduInfo->ucBssIndex);
#endif
	}
#endif
	...
	if (GLUE_IS_PKT_FLAG_SET(prPacket)) {
		nicTxMarkCriticalPkt(prAdapter, prMsduInfo, prPacket);
		...
		if (GLUE_TEST_PKT_FLAG(prPacket, ENUM_PKT_DHCP)
			...
			prMsduInfo->ucPktType = ENUM_PKT_DNS;
#if (CFG_SUPPORT_WONDERTAP == 1)
		else if (GLUE_TEST_PKT_FLAG(prPacket, ENUM_PKT_802_11))
			prMsduInfo->ucPktType = ENUM_PKT_802_11;
#endif
#if CFG_SUPPORT_TX_MGMT_USE_DATAQ
...
uint32_t nicTxMgmtDirectTxMsduMthread(struct ADAPTER *prAdapter)
{
	...
	while (1) {
		...
		GLUE_INC_REF_CNT(prAdapter->rHifStats.u4DataPendingTxCount);
		HAL_WRITE_TX_DATA(prAdapter, prMsduInfo);
		fgSetHifTx = TRUE;
	}
	...
	struct MSDU_INFO *prMsduInfo, uint8_t ucStaIdx, struct QUE *prQue)
{
	...
	if (prAdapter->rStaPendQueue[ucStaIdx].u4NumElem == 0)
		DBGLOG(TX, DEBUG,
		       "Sta %u BSS %u wlan %u fgIsTxAllowed isn't TRUE!\n",
		       ucStaIdx, prMsduInfo->ucBssIndex,
		       prMsduInfo->ucWlanIndex);
...
{
	if (!prSta) {
		DBGLOG(NAN, WARN, "NULL prSta to set Allow=%u", fgTxAllowed);
		return;
	}
		...
		uint8_t ucBssIndex)
{
	...
	if (pvPacket) {
		...
		if (mlrCheckIfDoFrag(prAdapter, prMsduInfo, (void *)pvPacket)) {
			...
			kalDoFragPacket(prAdapter, prMsduInfo, prFragmentedQue);

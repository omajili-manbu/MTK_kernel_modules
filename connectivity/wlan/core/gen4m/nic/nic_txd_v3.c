			...
			u_int8_t *prTxDescBuffer)
{
...
#if ((CFG_SUPPORT_NAN == 1) && (CFG_NAN_SUPPORT_R4_GROUP_ADDR_DATA == 1))
	/* Set wlan index for NAN BMC data packet */
	if (prBssInfo && (prBssInfo->eNetworkType == NETWORK_TYPE_NAN) &&
		(prMsduInfo->ucStaRecIndex == STA_REC_INDEX_BMCAST)) {
		/* NAN may use diff NDI tx data, find widx accordingly */
		prMsduInfo->ucWlanIndex =
			(uint8_t)nanFindMultiCastWidx(
				prAdapter,
				prMsduInfo->aucEthDestAddr,
				prMsduInfo->aucEthSrcAddr);

		/* If No MC TX widx, print error log. shuoud check before */
		if (prMsduInfo->ucWlanIndex == WTBL_RESERVED_ENTRY) {
			DBGLOG(NAN, ERROR,
				"[MCDATA] wtblIdx not found, Dest = "
				MACSTR ", Src = " MACSTR "\n",
				MAC2STR(prMsduInfo->aucEthDestAddr),
				MAC2STR(prMsduInfo->aucEthSrcAddr));
		} else {
			/* Set WlanIdx for BMC */
			HAL_MAC_CONNAC3X_TXD_SET_MLD_ID(prTxDesc,
				prMsduInfo->ucWlanIndex);
...
#endif /*((CFG_SUPPORT_NAN == 1) && (CFG_NAN_SUPPORT_R4_GROUP_ADDR_DATA == 1))*/
	...
	/* TODO: fail when enable hw amsdu */
#if CFG_SUPPORT_WON_HW
	else if (prBssInfo &&
		 IS_BSS_INDEX_WON(prAdapter, prBssInfo->ucBssIndex) &&

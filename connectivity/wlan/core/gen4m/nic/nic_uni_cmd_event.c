	...
	uint8_t *buf, struct CMD_SET_BSS_INFO *cmd)
{
	...
	DBGLOG(WON, DEBUG, "u2StaRecIdxOfAP=%u cmd->starec=%u",
		tag->u2StaRecIdxOfAP, cmd->ucStaRecIdxOfAP);
#endif
#endif
...
#endif
#if CFG_SUPPORT_WONDERTAP
	else if (legacy_sta_type == STA_TYPE_WON)
		return CONNECTION_WON;
#endif
	...
	uint8_t *buf, struct CMD_UPDATE_STA_RECORD *cmd)
{
	...
	if (!IS_BSS_INDEX_NAN(ad, prStaRec->ucBssIndex)) {
		DBGLOG(REQ, WARN,
			"NAN unicmd - Skip\n");
		return 0;
	}
...
uint32_t nicUniCmdStaRecTagHdrTrans(struct ADAPTER *ad,
	uint8_t *buf, struct CMD_UPDATE_STA_RECORD *cmd)
{
	struct STA_RECORD *prStaRec =
		cnmGetStaRecByIndex(ad, cmd->ucStaIndex);
	struct UNI_CMD_STAREC_HDR_TRANS *tag =
		(struct UNI_CMD_STAREC_HDR_TRANS *)buf;

	if (!prStaRec) {
		DBGLOG(REQ, WARN,
			"Hdr Trans unicmd - prStaRec is NULL\n");
		return 0;
	}

	if (!IS_BSS_INDEX_WON(ad, prStaRec->ucBssIndex)) {
		DBGLOG(REQ, TRACE,
			"Hdr Trans unicmd - Skip\n");
		return 0;
	}

	tag->u2Tag = UNI_CMD_STAREC_TAG_HDR_TRANS;
	tag->u2Length = sizeof(struct UNI_CMD_STAREC_HDR_TRANS);

	tag->ucFromDs = 0;
	tag->ucToDs = 0;
	tag->ucDisRxHdrTran = 1;

	DBGLOG(REQ, DEBUG,
		"Hdr Trans unicmd - StaRec[%u] WIDX[%u] DisRxHdrTran[%u]\n",
		cmd->ucStaIndex,
		cmd->ucWlanIndex,
		tag->ucDisRxHdrTran);
...
#endif
	{sizeof(struct UNI_CMD_STAREC_HDR_TRANS), nicUniCmdStaRecTagHdrTrans},
		...
		struct WIFI_UNI_SETQUERY_INFO *info)
{
	...
	/* bigtk */
	if ((cmd->ucKeyId >= 6 && cmd->ucKeyId <= 7) &&
		!IS_BSS_INDEX_NAN(ad, cmd->ucBssIdx))
		...
		struct WIFI_UNI_SETQUERY_INFO *info)
{
	...
	if (info->ucCID != CMD_ID_LAYER_0_EXT_MAGIC_NUM ||
	   ...
	   tag_id != TX_POWER_COMPOSITION))
		return WLAN_STATUS_NOT_ACCEPTED;

	if (tag_id == TX_POWER_SHOW_INFO)
		pfCmdDoneHandler = nicUniCmdEventTxPowerInfo;
	else if (tag_id == TX_POWER_COMPOSITION)
		pfCmdDoneHandler = nicUniCmdEventTxPowerComp;
	else
		pfCmdDoneHandler = nicUniCmdEventSetCommon;

	entry = nicUniCmdAllocEntry(ad, UNI_CMD_ID_TXPOWER, max_cmd_len,
		pfCmdDoneHandler, nicUniCmdTimeoutCommon);
...
void nicUniCmdEventTxPowerComp(struct ADAPTER
	*prAdapter, struct CMD_INFO *prCmdInfo, uint8_t *pucEventBuf)
{
	struct WIFI_UNI_EVENT *uni_evt = (struct WIFI_UNI_EVENT *) pucEventBuf;
	struct UNI_EVENT_TXPOWER *evt =
	...
	nicCmdEventQueryTxPowerComp(prAdapter, prCmdInfo, tag->aucBuffer);
}
	...
	struct WIFI_UNI_EVENT *evt)
{
	...
	TAG_FOR_EACH(tag, tags_len, offset) {
		...
		case UNI_EVENT_CNM_TAG_OPMODE_CHANGE_RDD: {
			...
			legacy.ucTrafficLimit = 0;
			RUN_RX_EVENT_HANDLER(EVENT_ID_RDD_OPMODE_CHANGE,
				&legacy);
		}
			break;
		case UNI_EVENT_CNM_TAG_OPMODE_CHANGE_RDD_V2: {
			struct UNI_EVENT_CNM_RDD_OPMODE_CHANGE_V2 *opmode =
				(struct UNI_EVENT_CNM_RDD_OPMODE_CHANGE_V2 *)
				tag;
			struct EVENT_RDD_OPMODE_CHANGE legacy = {0};
			...
			legacy.ucChBw = opmode->ucChBw;
			legacy.ucAction = opmode->ucAction;
			legacy.ucTrafficLimit = opmode->ucTrafficLimit;
			RUN_RX_EVENT_HANDLER(EVENT_ID_RDD_OPMODE_CHANGE,
				&legacy);
		}
			break;
...
void nicUniEventStaRec(struct ADAPTER *ad, struct WIFI_UNI_EVENT *evt)
{
	...
	TAG_FOR_EACH(tag, tags_len, offset) {
		...
		case UNI_EVENT_STAREC_TAG_MLO_LINK_STATE: {
			...
			mldStaRecEventLinkState(ad, common->u2WlanIdx, state);
#endif /* CFG_SUPPORT_802_11BE_MLO */
		}
			break;
...
void nicUniEventBssER(struct ADAPTER *ad, struct WIFI_UNI_EVENT *evt)
{
	...
	TAG_FOR_EACH(tag, tags_len, offset) {
		...
		case UNI_EVENT_MLR_TAG_FSM_UPDATE: {
			...
			legacy.ucMlrStateBmp = fsm->ucMlrStateBmp;
...
void nicUniEventBF(struct ADAPTER *ad, struct WIFI_UNI_EVENT *evt)
{
	...
	TAG_FOR_EACH(tag, tags_len, offset) {
		...
		case UNI_CMD_BF_TAG_PFMU_TAG_READ: {
			...
			DBGLOG(INIT, DEBUG,
				"========================== (R)Tag1 info ==========================\n"
				" Raw data0 : %x, Raw data1 : %x, Raw data2 : %x, Raw data3 : %x\n",
				au4RawDataTag1[0], au4RawDataTag1[1],
				au4RawDataTag1[2], au4RawDataTag1[3]);
			DBGLOG(INIT, DEBUG,
				" Raw data4 : %x, Raw data5 : %x, Raw data6 : %x\n",
			...
			DBGLOG(INIT, DEBUG,
				"========================== (R)Tag2 info ==========================\n"
				" Raw data0 : %x, Raw data1 : %x, Raw data2 : %x\n",
...
void nicUniEventMddp(struct ADAPTER *ad, struct WIFI_UNI_EVENT *evt)
{
	...
	TAG_FOR_EACH(tag, tags_len, offset) {
		...
		case UNI_EVENT_MDDP_PF_USING: {
			...
			mddpSetPFUsing(pf_using->fgIsMdPfUsing);

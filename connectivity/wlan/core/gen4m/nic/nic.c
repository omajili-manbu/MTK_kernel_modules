			...
			uint8_t fgClearStaRec)
{
	...
	uint8_t ucIotApAct = 0;
	uint64_t u8IotApAct = 0;
	...
	if (IS_BSS_AIS(prBssInfo) &&
		...
		if (prBssInfo->prStaRecOfAP)
			...
			DBGLOG(WON, TRACE, "prStaRecOfAP NULL\n");
	}
#endif
	else
...
#if (CFG_SUPPORT_IOT_AP_BLOCKLIST == 1)
	ucIotApAct = rCmdSetBssInfo.ucIotApAct;
	u8IotApAct = rCmdSetBssInfo.u8IotApAct;
#endif

	DBGLOG(BSS, INFO,
	       "Update Bss[%u] OMAC[%u] WMM[%u] ConnState[%u] OPmode[%u] BSSID["
	       MACSTR
	       "] AuthMode[%u] EncStatus[%u] IotAct[%u:%u] eBand[%u] Bw[%u] NetIfTh[%u:%u] 6GPwrReg[%u]",
	       ucBssIndex,
	       prBssInfo->ucOwnMacIndex,
	...
	if (IS_BSS_AIS(prBssInfo)) {
		if (ucCaller == PS_CALLER_COMMON &&
			...
			scheduledpm_action(prAdapter, SCHED_PM_TEARDOWN, NULL,
				FALSE, SCHED_TEARDOWN_BY_PSDISABLE);

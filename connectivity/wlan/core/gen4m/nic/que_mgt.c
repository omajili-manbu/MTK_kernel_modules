...
#if CFG_RX_PENDING_AHEAD
static u_int8_t paTryCommit(struct RX_BA_ENTRY *prEntry,
			    uint16_t *pu2SeqNo);
static u_int8_t paIsLargeGap(uint16_t u2SeqNo, uint16_t u2WinEnd,
			     uint16_t u2WinSize);
static inline void paTentativeClear(struct RX_BA_ENTRY *prEntry);
static u_int8_t paInsert(struct ADAPTER *prAdapter,
			 struct RX_BA_ENTRY *prEntry,
			 struct SW_RFB *prSwRfb,
			 struct QUE *prReturnedQue);
static void paForceCommit(struct RX_BA_ENTRY *prEntry);
static void paDiscard(struct ADAPTER *prAdapter,
		      struct RX_BA_ENTRY *prEntry,
		      struct QUE *prReturnedQue);
static u_int8_t paRollback(struct ADAPTER *prAdapter,
			   struct RX_BA_ENTRY *prEntry,
			   struct QUE *prReturnedQue,
			   uint16_t u2SeqNo);
#endif /* CFG_RX_PENDING_AHEAD */
...
#if CFG_RX_PARTIAL_AMSDU
static void qmPartialAmsduResetState(struct RX_BA_ENTRY *prReorderQueParm);
#endif /* CFG_RX_PARTIAL_AMSDU */
...
#if CFG_RX_PARTIAL_AMSDU
		qmPartialAmsduResetState(&prQM->arRxBaTable[u4Idx]);
#endif /* CFG_RX_PARTIAL_AMSDU */
		      ...
		      struct STA_RECORD *prStaRec)
{
	...
	if (IS_BSS_INDEX_WON(prAdapter, prStaRec->ucBssIndex)) {
		DBGLOG(WON, DEBUG,
			"WONDER interface skip generate TxDesc template\n");
	} else
#endif
	{
	...
	struct MSDU_INFO *prMsduInfo)
{
	...
	case OP_MODE_WON:
		...
		if (IS_BMCAST_MAC_ADDR(prMsduInfo->aucEthDestAddr)) {
			...
			DBGLOG(WON, DEBUG, "TX with DA = BMCAST\n");
			return;
		}
		break;
		...
		struct WLAN_MAC_HEADER *prWlanHeader, uint16_t u2FrameCtrl)
{
	...
	/* handler MC RX with sec */
	struct _NAN_MC_RX_WTBL *prNanMcRxWtbl = NULL;

	/* MC Rx with sec should hit wtbl and done header translation */
	if (prCurrSwRfb->prRxStatusGroup4 != NULL) {
		uint8_t aucTaAddr[MAC_ADDR_LEN] = {0};

		HAL_RX_STATUS_GET_TA(
			prCurrSwRfb->prRxStatusGroup4,
			aucTaAddr);

		/* Find MC Rx wtbl with TA */
		prNanMcRxWtbl = nanGetMcRxWtblIdx(aucTaAddr);
		if (prNanMcRxWtbl != NULL)
			prCurrSwRfb->prStaRec = &(prNanMcRxWtbl->rStaRec);
...
static inline uint16_t qmGetFrameCtrl(struct SW_RFB *prSwRfb)
{
	if (prSwRfb->fgHdrTran)
		return HAL_RX_STATUS_GET_FRAME_CTL_FIELD(
			prSwRfb->prRxStatusGroup4);

	return ((struct WLAN_MAC_HEADER *)prSwRfb->pvHeader)->u2FrameCtrl;
}

static struct RX_BA_ENTRY *qmGetRxReorderParam(
	struct ADAPTER *prAdapter,
	struct SW_RFB *prSwRfb,
	u_int8_t fgIsBMC)
{
	...
	if (prSwRfb->ucTid >= CFG_RX_MAX_BA_TID_NUM ||
		prSwRfb->fgFragFrame || fgIsBMC || !prStaRec)
		return NULL;

	u2FrameType = qmGetFrameCtrl(prSwRfb) & MASK_FRAME_TYPE;
	if (u2FrameType != MAC_FRAME_DATA &&
	    u2FrameType != MAC_FRAME_QOS_DATA)
		return NULL;
...
#undef __STR_FMT__
		...
		if (fgSwRxReordering &&
			...
			if (prCurrSwRfb->prStaRec &&
				...
				prReorderQueParm = qmGetRxReorderParam(
					prAdapter, prCurrSwRfb, fgIsBMC);
...
#if CFG_WONDERTAP_TX_RX_DEBUG
					DBGLOG(WON, DEBUG,
						"WONDER Reorder Rx Pkt BSSidx:%u, StaRecIdx:%u, widx:%u, tid:%u, SN:%u, PLfmt:%u, bmc:%u, pkttype:%u, FmType:0x%04x\n",
						GLUE_GET_PKT_BSS_IDX(prCurrSwRfb
						  ->pvPacket),
						prCurrSwRfb->ucStaRecIdx,
						prCurrSwRfb->ucWlanIdx,
						prCurrSwRfb->ucTid,
...
#if CFG_WONDERTAP_TX_RX_DEBUG
					DBGLOG(WON, DEBUG,
						"WONDER non-reorder Rx Pkt BSSidx:%u, StaRecIdx:%u, widx:%u, tid:%u, SN:%u, PLfmt:%u, bmc:%u, pkttype:%u, FmType:0x%04x\n",
						GLUE_GET_PKT_BSS_IDX(prCurrSwRfb
						  ->pvPacket),
						prCurrSwRfb->ucStaRecIdx,
						prCurrSwRfb->ucWlanIdx,
...
#if CFG_SUPPORT_WONDERTAP
			else if (nicRxHandleWonPkt(prAdapter, prCurrSwRfb)) {
				DBGLOG(RX, DEBUG,
					"By pass rx wondertap packet\n");
			}
#endif
			else {
				DBGLOG(RX, TEMP,
...
static void qmPartialAmsduResetState(struct RX_BA_ENTRY *prReorderQueParm)
{
	...
	prReorderQueParm->u2LastPopOutSN = 0xFFFF;
	prReorderQueParm->u2LastPopOutMsduCount = 0;
	prReorderQueParm->u2LastDropSN = 0xFFFF;
	prReorderQueParm->u2LastDropMsduCount = 0;
}

static void qmPartialAmsduSetDrop(struct RX_BA_ENTRY *prReorderQueParm,
	struct SW_RFB *prSwRfb)
{
	if (!prReorderQueParm || !prSwRfb)
		return;

	if (prSwRfb->u2SSN == prReorderQueParm->u2LastDropSN)
		prReorderQueParm->u2LastDropMsduCount++;
	else
		prReorderQueParm->u2LastDropMsduCount = 1;


	prReorderQueParm->u2LastDropSN = prSwRfb->u2SSN;
}

static void qmPartialAmsduSetPopOut(struct RX_BA_ENTRY *prReorderQueParm,
	struct SW_RFB *prSwRfb)
{
	if (!prReorderQueParm || !prSwRfb)
		return;

	if (prSwRfb->u2SSN == prReorderQueParm->u2LastPopOutSN) {
		prReorderQueParm->u2LastPopOutMsduCount++;
		if (IS_RX_MPDU_FINAL(prSwRfb->ucPayloadFormat))
			prReorderQueParm->u2LastDropMsduCount = 0;
	} else
		prReorderQueParm->u2LastPopOutMsduCount = 1;

	prReorderQueParm->u2LastPopOutSN = prSwRfb->u2SSN;
}

/**
 * Special case:
 * last pop out fall within SN taken as complete && indicated to
 * kernel, but the same SN comes with more subframes.
 * Pull back window on 1 to make it as fall within
 *
 * The RX SN is just received and indicated to advance the window, and
 * the drop count of this SN == the number of just indicated MSDU.
 */
static u_int8_t qmDetectPartialAmsdu(struct RX_BA_ENTRY *prReorderQueParm,
	struct SW_RFB *prSwRfb)
{
	uint16_t u2SeqNo;
	...
	u2SeqNo = prSwRfb->u2SSN;
	if (u2SeqNo == prReorderQueParm->u2LastPopOutSN &&
		SEQ_ADD(u2SeqNo, 1) == prReorderQueParm->u2WinStart &&
		/* passed (dropped) the leading same SN subframes */
		prReorderQueParm->u2LastDropSN == u2SeqNo &&
		prReorderQueParm->u2LastDropMsduCount ==
		prReorderQueParm->u2LastPopOutMsduCount) {
		DBGLOG_LIMITED(RX, WARN,
			"RX SN=%u, dropped=%u, pull back WinStart %u",
			u2SeqNo, prReorderQueParm->u2LastDropMsduCount,
			prReorderQueParm->u2WinStart);
		SEQ_DEC(prReorderQueParm->u2WinStart);
		SEQ_DEC(prReorderQueParm->u2WinEnd);
	...
	struct QUE *prReturnedQue)
{
	...
	/* Debug */
	DBGLOG(RX, TEMP,
		"QM: RxPkt=%p ipid=%d STA[%u] TID[%u] seq=%u Win{%u,%u}\n",
		prSwRfb,
...
#if CFG_RX_PARTIAL_AMSDU
	if (qmDetectPartialAmsdu(prReorderQueParm, prSwRfb)) {
		u2WinStart = prReorderQueParm->u2WinStart;
		u2WinEnd = prReorderQueParm->u2WinEnd;
	}
#endif /* CFG_RX_PARTIAL_AMSDU */
	...
	/* Case 1: Fall within */
	if (SEQ_WITHIN_WINDOW(u2SeqNo, u2WinStart, u2WinEnd)) {
		DBGLOG(RX, TEMP, "RxPkt=%p seq=%d fall within\n",
			prSwRfb, u2SeqNo);

#if CFG_RX_PENDING_AHEAD
		/* Any frame inside the BA window ends a tentative commit.
		 * Discard the far-ahead buffer if it still exists.
		 */
		paDiscard(prAdapter, prReorderQueParm, prReturnedQue);
#endif /* CFG_RX_PENDING_AHEAD */
...
#if CFG_RX_PENDING_AHEAD
		/* Pending-ahead: handle only *large* forward jumps */
		if (paIsLargeGap(u2SeqNo, u2WinEnd,
				 prReorderQueParm->u2WinSize)) {
			u_int8_t fgInserted;

			/* paInsert() returns TRUE if the frame was queued */
			fgInserted = paInsert(prAdapter, prReorderQueParm,
					      prSwRfb, prReturnedQue);

			/* Nothing queued – give control back */
			if (!fgInserted)
				return;

			/* Shift window once commit */
			if (!paTryCommit(prReorderQueParm, &u2SeqNo))
...
#if CFG_RX_PENDING_AHEAD
		/* Tentative commit proved wrong; roll back. If the frame
		 * now falls inside the old window, rerun the fall-within path.
		 */
		if (paRollback(prAdapter, prReorderQueParm,
			       prReturnedQue, u2SeqNo)) {
			qmInsertFallWithinReorderPkt(prAdapter, prSwRfb,
						     prReorderQueParm,
						     prReturnedQue);
			qmPopOutDueToFallWithin(prAdapter, prReorderQueParm,
...
#if CFG_RX_PARTIAL_AMSDU
		qmPartialAmsduSetDrop(prReorderQueParm, prSwRfb);
#endif /* CFG_RX_PARTIAL_AMSDU */
...
#if CFG_RX_PARTIAL_AMSDU
			qmPartialAmsduSetPopOut(prReorderQueParm,
					prReorderedSwRfb);
#endif /* CFG_RX_PARTIAL_AMSDU */
...
#if CFG_RX_PENDING_AHEAD
	/* Bubble timeout: lost frames; commit PA burst if ready. */
	if (!QUEUE_IS_EMPTY(&prReorderQueParm->rPendingAheadQue)) {
		uint16_t u2TmpSeq = 0;          /* dummy holder */

		/* Commit only if (length / timeout) conditions are met */
		paTryCommit(prReorderQueParm, &u2TmpSeq);
	}
#endif /* CFG_RX_PENDING_AHEAD */
...
#if CFG_RX_PARTIAL_AMSDU
		qmPartialAmsduResetState(prRxBaEntry);
#endif /* CFG_RX_PARTIAL_AMSDU */
...
#if CFG_RX_PENDING_AHEAD
	/* always merge the pending-ahead queue first. */
	if (HAL_IS_RX_DIRECT(prAdapter))
		RX_DIRECT_REORDER_LOCK(prAdapter->prGlueInfo, 0);

	paForceCommit(prRxBaEntry);
	...
	if (u2EtherType == ETH_P_1X) {
		...
		if (ucEapolType == ETH_EAPOL_KEY) {
			...
			if (prAdapter->rWifiVar.u4SwTestMode ==
			    ...
			    m == 3 && !prSwRfb->prStaRec->fgIsTxKeyReady) {
	...
	enum RX_REORDER_TIMEOUT_TYPE eType, uint32_t u4Timeout)
{
	...
	if (u4OldVal && u4OldVal != qmGetRxReorderTimeout(prStaRec)) {
		DBGLOG(QM, INFO,
			"STA[%u] BSS[%u] Rx Reorder Timeout changed from %u to %u eType[%u]\n",
			prStaRec->ucIndex, prStaRec->ucBssIndex,
			u4OldVal, qmGetRxReorderTimeout(prStaRec), eType);
	}
}
...
#if CFG_RX_PENDING_AHEAD
/* helpers for the 256-slot pending-ahead cache */
static inline void paCacheClear(struct RX_BA_ENTRY *prEntry)
{
	memset(prEntry->aprPendCache, 0, sizeof(prEntry->aprPendCache));
}

static inline void paCacheUpdate(struct RX_BA_ENTRY *prEntry,
				 struct SW_RFB *prSwRfb)
{
	uint16_t u2Idx = prSwRfb->u2SSN & RX_PA_CACHE_MASK;

	prEntry->aprPendCache[u2Idx] = prSwRfb;
}

static inline struct SW_RFB *paCacheLookup(struct RX_BA_ENTRY *prEntry,
	uint16_t u2SSN)
{
	struct SW_RFB *prHit = prEntry->aprPendCache[u2SSN & RX_PA_CACHE_MASK];

	return (prHit && prHit->u2SSN == u2SSN) ? prHit : NULL;
}

static inline void paResetState(struct RX_BA_ENTRY *prEntry)
{
	/* Clear direct-mapped cache and counters, restart timer */
	paCacheClear(prEntry);
	prEntry->u2PendDistinctSnCnt = 0;
	prEntry->rAheadStartTime = 0;
}

/* TRUE if SN is far enough *ahead* of WinEnd. */
static inline u_int8_t paIsLargeGap(uint16_t u2SeqNo, uint16_t u2WinEnd,
				    uint16_t u2WinSize)
{
	uint16_t u2Gap = SEQ_DIFF(u2WinEnd, u2SeqNo);
	uint16_t u2Thr = (u2WinSize * RX_FALL_AHEAD_GAP_PCT) / 100;

	if (u2Thr < RX_FALL_AHEAD_GAP_MIN)
		u2Thr = RX_FALL_AHEAD_GAP_MIN;
...
static inline void paTentativeClear(struct RX_BA_ENTRY *prEntry)
{
	prEntry->fgPaTentative = FALSE;
}

/* Mark commit as tentative and save old window. */
static inline void paTentativeMark(struct RX_BA_ENTRY *prEntry)
{
	prEntry->fgPaTentative = TRUE;
	prEntry->u2PaOldWinStart = prEntry->u2WinStart;
	prEntry->u2PaOldWinEnd = prEntry->u2WinEnd;
}

static inline enum ENUM_PA_READY_STATE
paIsReady(struct RX_BA_ENTRY *prEntry, OS_SYSTIME rNow)
{
	uint32_t u4Thr = prEntry->u2WinSize * RX_FALL_AHEAD_CNT_PCT / 100;

	if (u4Thr < RX_FALL_AHEAD_CNT_MIN)
		u4Thr = RX_FALL_AHEAD_CNT_MIN;

	if (prEntry->u2PendDistinctSnCnt >= u4Thr)
		return PA_READY_COUNT;

	if (CHECK_FOR_TIMEOUT(rNow, prEntry->rAheadStartTime,
			      MSEC_TO_SYSTIME(RX_FALL_AHEAD_TIMEOUT_MS)))
		return PA_READY_TIMEOUT;

	return PA_READY_NONE;
}

static void paForceCommit(struct RX_BA_ENTRY *prEntry)
{
	if (QUEUE_IS_EMPTY(&prEntry->rPendingAheadQue))
		return;

	QUEUE_CONCATENATE_QUEUES(&prEntry->rReOrderQue,
				 &prEntry->rPendingAheadQue);

	/* Commit is now final - no roll-back possible */
	paTentativeClear(prEntry);
	/* ready for the next round */
	paResetState(prEntry);
}

static u_int8_t paDropDup(struct ADAPTER *prAdapter,
			  struct RX_BA_ENTRY *prEntry,
			  struct SW_RFB *prSwRfb,
			  struct QUE *prReturnedQue,
			  const char *pcTag)
{
	/* Common duplicate-drop routine */
	qmPopOutReorderPkt(prAdapter, prEntry, prSwRfb, prReturnedQue,
			   RX_DUPICATE_DROP_COUNT);
	LINK_QUALITY_COUNT_DUP(prAdapter, prSwRfb);
	DBGLOG(QM, TRACE,
	       "QM:PA dup(%s) STA[%u] TID[%u] SN[%u] Win{%u,%u}\n",
	       pcTag,
	       prSwRfb->prStaRec ?
	       prSwRfb->prStaRec->ucIndex : 0xFF,
	       prSwRfb->ucTid, prSwRfb->u2SSN,
	       prEntry->u2WinStart, prEntry->u2WinEnd);
	return FALSE;
}

/* Insert frame into pending-ahead queue (ordered by SN) */
static u_int8_t paInsert(struct ADAPTER *prAdapter,
			 struct RX_BA_ENTRY *prEntry,
			 struct SW_RFB *prSwRfb,
			 struct QUE *prReturnedQue)
{
	struct QUE *prPendQue = &prEntry->rPendingAheadQue;
	uint16_t u2SN = prSwRfb->u2SSN;
	uint8_t ucTid = prSwRfb->ucTid;
	uint8_t ucSta = prSwRfb->prStaRec ?
		prSwRfb->prStaRec->ucIndex : 0xFF;
	struct SW_RFB *prTail;
	struct SW_RFB *prPos;
	struct SW_RFB *prSameTail;
	u_int8_t fgCompleted = FALSE; /* AMSDU finished? */
	const char *pzWhere;

	/* Fast-path 1: empty queue -> head */
	if (QUEUE_IS_EMPTY(prPendQue)) {
		QUEUE_INSERT_HEAD(prPendQue, prSwRfb);
		paCacheUpdate(prEntry, prSwRfb);
		prEntry->u2PendDistinctSnCnt = 1; /* first SN */

		DBGLOG(QM, TRACE,
		       "QM:PA head STA[%u] TID[%u] SN[%u] Win{%u,%u} len=1\n",
		       ucSta, ucTid, u2SN,
		       prEntry->u2WinStart, prEntry->u2WinEnd);
		return TRUE;
	}

	/* Fast-path 2: SN > tail -> tail append */
	prTail = QUEUE_GET_TAIL(prPendQue);
	if (prTail && SEQ_SMALLER(prTail->u2SSN, u2SN)) {
		QUEUE_INSERT_TAIL(prPendQue, prSwRfb);
		paCacheUpdate(prEntry, prSwRfb);
		prEntry->u2PendDistinctSnCnt++; /* new SN */

		DBGLOG(QM, TRACE,
		       "QM:PA tail STA[%u] TID[%u] SN[%u] Win{%u,%u} len=%u\n",
		       ucSta, ucTid, u2SN,
		       prEntry->u2WinStart, prEntry->u2WinEnd,
		       QUEUE_LENGTH(prPendQue));
		return TRUE;
	}

	/* Step 3: find first pkt with SN >= incoming */
	prPos = paCacheLookup(prEntry, u2SN);
	if (!prPos)
		prPos = QUEUE_GET_HEAD(prPendQue);

	while (prPos && SEQ_SMALLER(prPos->u2SSN, u2SN))
		prPos = (struct SW_RFB *)QUEUE_GET_NEXT_ENTRY(prPos);

	/* Step 4: SAME-SN handling */
	if (prPos && prPos->u2SSN == u2SN) {
		/* Walk through SAME-SN region to detect completion */
		prSameTail = prPos;
		do {
#if CFG_SUPPORT_RX_AMSDU
			if (IS_RX_MPDU_FINAL(prSameTail->ucPayloadFormat))
				fgCompleted = TRUE;
#else
			fgCompleted = TRUE;
#endif
			prSameTail = (struct SW_RFB *)
				QUEUE_GET_NEXT_ENTRY(prSameTail);
		} while (prSameTail && prSameTail->u2SSN == u2SN);

		/* Duplicate rules */
		if (fgCompleted)
			return paDropDup(prAdapter, prEntry, prSwRfb,
					 prReturnedQue, "completed");

		if (IS_RX_MPDU_BEGIN(prSwRfb->ucPayloadFormat))
			return paDropDup(prAdapter, prEntry, prSwRfb,
					 prReturnedQue, "begin");

		/* In-progress AMSDU - insert after SAME-SN block */
		prPos = prSameTail; /* may be NULL -> tail append */
		/* distinct-SN counter stays unchanged */
	} else {
		/* Brand-new SN - bump DISTINCT-SN counter once */
		prEntry->u2PendDistinctSnCnt++;
	}

	/* Step 5: do the actual insert */
	if (prPos)
		QUEUE_INSERT_BEFORE(prPendQue, prPos, prSwRfb);
	else
		QUEUE_INSERT_TAIL(prPendQue, prSwRfb);

	paCacheUpdate(prEntry, prSwRfb);

	/* Figure out where we inserted for trace message */
	if (prSwRfb == QUEUE_GET_HEAD(prPendQue))
		pzWhere = (QUEUE_LENGTH(prPendQue) == 1) ? "head" : "ins";
	else if (prSwRfb == QUEUE_GET_TAIL(prPendQue))
		pzWhere = "tail";
	else
		pzWhere = "mid";

	DBGLOG(QM, TRACE,
	       "QM:PA %s STA[%u] TID[%u] SN[%u] Win{%u,%u} len=%u\n",
	       pzWhere, ucSta, ucTid, u2SN,
	       prEntry->u2WinStart, prEntry->u2WinEnd,
	       QUEUE_LENGTH(prPendQue));
...
/* Discard the whole pending-ahead queue (triggered by fall-within) */
static void paDiscard(struct ADAPTER *prAdapter,
		      struct RX_BA_ENTRY *prEntry,
		      struct QUE *prReturnedQue)
{
	struct SW_RFB *prTmp;
	uint32_t u4Discarded = 0;
	uint16_t u2FirstSN = 0;

	/* Frame within window -> tentative commit is now final */
	paTentativeClear(prEntry);

	while (!QUEUE_IS_EMPTY(&prEntry->rPendingAheadQue)) {
		QUEUE_REMOVE_HEAD(&prEntry->rPendingAheadQue, prTmp,
				  struct SW_RFB *);
		if (!prTmp)
			break;

		if (!u2FirstSN)
			u2FirstSN = prTmp->u2SSN;

		qmPopOutReorderPkt(prAdapter, prEntry, prTmp, prReturnedQue,
				   RX_REORDER_AHEAD_DROP_COUNT);
		u4Discarded++;
	}

	if (u4Discarded)
		DBGLOG(QM, TRACE,
		       "QM:PA drop STA[%u] TID[%u] firstSN[%u] +%u pkt Win{%u,%u}\n",
		       prEntry->ucStaRecIdx, prEntry->ucTid,
		       u2FirstSN, u4Discarded,
		       prEntry->u2WinStart, prEntry->u2WinEnd);

	/* Flush cache and reset counters/timestamp for the next burst */
	paResetState(prEntry);
}

/* Commit the deferred shift if ready. Return TRUE on commit. */
static u_int8_t paTryCommit(struct RX_BA_ENTRY *prEntry,
			    uint16_t *pu2SeqNo)
{
	struct SW_RFB *prTail;
	OS_SYSTIME rNow;
	enum ENUM_PA_READY_STATE eReady;
	const char *pzReason;
	uint32_t u4PendLen;           /* snapshot of pending queue length */

	GET_CURRENT_SYSTIME(&rNow);

	/* Record the arrival time of the *first* far-ahead frame only once */
	if (prEntry->rAheadStartTime == 0)
		prEntry->rAheadStartTime = rNow;

	eReady = paIsReady(prEntry, rNow);
	if (eReady == PA_READY_NONE)
		return FALSE; /* keep waiting */
	else if (eReady == PA_READY_TIMEOUT) {
		/* Save roll-back information _before_ we merge. */
		paTentativeMark(prEntry);
	} else { /* PA_READY_COUNT -> this commit is final */
		paTentativeClear(prEntry);
	}

	/* Snapshot queue length BEFORE we merge, so the log is accurate */
	u4PendLen = QUEUE_LENGTH(&prEntry->rPendingAheadQue);

	/* Highest SN sits at the tail (ascending order) */
	prTail = (struct SW_RFB *)QUEUE_GET_TAIL(&prEntry->rPendingAheadQue);
	if (prTail)
		*pu2SeqNo = prTail->u2SSN;

	/* merge queues and clear pending list */
	QUEUE_CONCATENATE_QUEUES(&prEntry->rReOrderQue,
				 &prEntry->rPendingAheadQue);
	paResetState(prEntry);

	/* Text tag for trace message */
	pzReason = (eReady == PA_READY_COUNT) ? "cnt" : "tmo";

	DBGLOG(QM, TRACE,
	       "QM:PA commit(%s) STA[%u] TID[%u] tailSN=%u Win{%u,%u} pendLen=%u\n",
	       pzReason,
	       prEntry->ucStaRecIdx, prEntry->ucTid,
	       *pu2SeqNo,
	       prEntry->u2WinStart, prEntry->u2WinEnd,
...
/* Roll back a timeout-only commit if later traffic disproves it. */
/* Return TRUE  - rollback done and SN is inside *old* window
 *        FALSE - no rollback or SN still outside old window.
 */
static u_int8_t paRollback(struct ADAPTER *prAdapter,
			   struct RX_BA_ENTRY *prEntry,
			   struct QUE *prReturnedQue,
			   uint16_t u2SeqNo /* offending frame */)
{
	uint32_t u4Dropped = 0;
	struct SW_RFB *prTail;

	if (!prEntry->fgPaTentative)
		return FALSE; /* nothing to undo */

	/* Was the offending SN inside the *old* window? */
	if (!SEQ_WITHIN_WINDOW(u2SeqNo,
			       prEntry->u2PaOldWinStart,
			       prEntry->u2PaOldWinEnd))
		return FALSE; /* outside -> commit was correct */


	/* Fall within old window, just do rollback */
	paTentativeClear(prEntry);
	prEntry->u2WinStart = prEntry->u2PaOldWinStart;
	prEntry->u2WinEnd   = prEntry->u2PaOldWinEnd;

	while (!QUEUE_IS_EMPTY(&prEntry->rReOrderQue)) {
		prTail = QUEUE_GET_TAIL(&prEntry->rReOrderQue);
		if (!prTail)
			break;   /* queue just turned empty */

		/* remove any RFB from tail that is outside the old window */
		if (SEQ_WITHIN_WINDOW(prTail->u2SSN,
				prEntry->u2WinStart, prEntry->u2WinEnd))
			break;

		QUEUE_REMOVE_TAIL(&prEntry->rReOrderQue, prTail,
				  struct SW_RFB *);
		qmPopOutReorderPkt(prAdapter, prEntry, prTail, prReturnedQue,
				   RX_REORDER_AHEAD_DROP_COUNT);
		u4Dropped++;
	}

	DBGLOG(QM, WARN,
	       "QM:PA rollback STA[%u] TID[%u] SN=%u drop=%u Win->{%u,%u}\n",
	       prEntry->ucStaRecIdx, prEntry->ucTid,
	       u2SeqNo, u4Dropped,
	       prEntry->u2WinStart, prEntry->u2WinEnd);

	return TRUE; /* rollback done, offending SN should be re-processed */
}

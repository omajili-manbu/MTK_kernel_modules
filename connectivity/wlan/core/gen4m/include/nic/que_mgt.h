...
#define CFG_RX_PENDING_AHEAD 1

#if CFG_RX_PENDING_AHEAD
#define RX_FALL_AHEAD_CNT_PCT      20  /* N = 20 % of BA window (default)  */
#define RX_FALL_AHEAD_CNT_MIN       8  /* but never smaller than eight     */
#define RX_FALL_AHEAD_TIMEOUT_MS    8  /* or wait T ms before committing   */
#define RX_FALL_AHEAD_GAP_PCT      50  /* 50 % of BA window                */
#define RX_FALL_AHEAD_GAP_MIN      32  /* but never smaller than 32        */
#define RX_PA_CACHE_BITS            8  /* 2^8 = 256 entries, 1/4 of BA win */
#define RX_PA_CACHE_SIZE      (1 << RX_PA_CACHE_BITS)
#define RX_PA_CACHE_MASK      (RX_PA_CACHE_SIZE - 1)
...
/* True if _SEQ lies inside the inclusive window [_START ... _END]        */
#define SEQ_WITHIN_WINDOW(_SEQ, _START, _END) \
	((SEQ_SMALLER((_START), (_SEQ)) || (_SEQ) == (_START)) && \
	  (SEQ_SMALLER((_SEQ), (_END)) || (_SEQ) == (_END)))
...
#define CFG_RX_PARTIAL_AMSDU 1

#if CFG_RX_PENDING_AHEAD
enum ENUM_PA_READY_STATE {
	PA_READY_NONE,    /* not ready */
	PA_READY_COUNT,   /* enough distinct SNs buffered */
	PA_READY_TIMEOUT  /* timeout reached */
};
#endif /* CFG_RX_PENDING_AHEAD */
...
struct RX_BA_ENTRY {
	...
	uint16_t u2LastPopOutSN;
	uint16_t u2LastPopOutMsduCount;
	uint16_t u2LastDropSN;
	uint16_t u2LastDropMsduCount;
...
#if CFG_RX_PENDING_AHEAD
	/* Timestamp of the first far-ahead frame */
	OS_SYSTIME rAheadStartTime;

	/* Buffer for far-ahead frames and its lookup cache */
	struct QUE rPendingAheadQue;
	struct SW_RFB *aprPendCache[RX_PA_CACHE_SIZE]; /* lookup cache */

	uint16_t u2PendDistinctSnCnt; /* #distinct SNs in pending queue */

	/* Roll-back info for tentative (timeout-only) commit */
	uint16_t u2PaOldWinStart;
	uint16_t u2PaOldWinEnd;
	u_int8_t fgPaTentative;        /* 1 = commit may be rolled back */
#endif /* CFG_RX_PENDING_AHEAD */
};

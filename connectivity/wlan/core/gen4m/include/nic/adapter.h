...
struct BSS_INFO {
	...
	u_int8_t fgIsModifyBwForDfs;
	uint8_t ucP2p5gOriBwForDfs;
...
#if (CFG_SUPPORT_NAN_R4_PAIRING == 1)
	struct PAIRING_FSM_INFO arPairingFsmInfo[NAN_MAX_NDP_SESSIONS];
#endif /* CFG_SUPPORT_NAN_R4_PAIRING */
#endif

#if CFG_SUPPORT_WONDERTAP
	struct WON_DEV_FSM_INFO *prWonDevFsmInfo;
	struct WON_ROLE_FSM_INFO *aprWonRoleFsmInfo[KAL_WON_NUM];
#endif
...
/* P_P2P_FSM_INFO_T prP2pFsmInfo; */
	...
	uint8_t aucWonAddress[KAL_WON_NUM][MAC_ADDR_LEN];
	...
	uint32_t u4LinkStableTimeoutMs;
...
#if CFG_SUPPORT_WONDERTAP
	uint8_t ucWon2gBandwidth;
	uint8_t ucWon5gBandwidth;
	uint8_t ucWon6gBandwidth;
#endif
...
#if CFG_SUPPORT_WONDERTAP
	uint32_t u4RegWonIfAtProbe;
	uint8_t ucRegWonMode;
	u_int8_t fgWonNoDisconnect;
#endif
	...
	uint32_t u4NanGtkCipher;
#endif /* CFG_NAN_SUPPORT_R4_GROUP_ADDR_FRAME_PROT */
#if (CFG_SUPPORT_NAN_R4_PAIRING ==  1)
	uint32_t u4NanPairingCipher;
	uint32_t u4NanNikLifetime;
	uint32_t u4NanGroupKdeInFollowup;
#endif /* CFG_SUPPORT_NAN_R4_PAIRING */
	...
	uint8_t ucNanMulticastAddrOverride;
	uint8_t aucNanMulticastAddrStr[WLAN_CFG_VALUE_LEN_MAX];
#endif /* CFG_NAN_SUPPORT_R4_GROUP_ADDR_FRAME_PROT */
...
#define NAN_CUSTOM_BITMAP_LEVELS 4
	uint32_t u4NdpCustomThreshold[NAN_CUSTOM_BITMAP_LEVELS];

	/* {0xffffffff, 0x44104210, 0x0060060c} for  5G */
	uint32_t u4NdpCustomBitmap[NAN_CUSTOM_BITMAP_LEVELS];
	...
	u_int8_t fgNanNdpSkipNmiKey;
	uint8_t ucNanLogSlotStatistics;
#if (defined(CFG_SUPPORT_NAN_R4_PAIRING) && CFG_SUPPORT_NAN_R4_PAIRING == 1)
	u_int8_t ucNanEnablePairing;
	uint16_t u2NanPairingComeback;
	uint16_t u2NanPairingCbMethod;
	u_int8_t ucNanPairingNikExAtVerify;
#endif /* CFG_SUPPORT_NAN_R4_PAIRING */
	u_int8_t fgNanCustDwChnl;
#endif /* CFG_SUPPORT_NAN */
...
#define INVALID_TX_DELAY 0xFFFFFFFF
	...
	u_int8_t ucErrPos;
	uint32_t u4MlrCfg;
	u_int8_t fgEnMlrUevent;
	u_int8_t fgMlrDisSingleLink2g;
	...
	uint32_t fgEnFwCtrlTxDataDelay;
	...
	uint8_t ucCountryPwrLimitCtrl;
#ifdef SOC3_0
	uint8_t fgUseOneTxRing;
#endif

	u_int8_t fgDisCnmTBTC;
	u_int8_t fgEnCnmSTAP2PTBTC;
	u_int8_t fgDisCnmSTAWFDTBTC;
	u_int8_t fgDisCnmDualSTATBTC;
	u_int8_t fgDisBn2UNII2C;
};
...
/* the last entry is to hold the sum of all BSSes */
struct TX_RX_STATS_RECORD {
	uint64_t au8TxBytes[MAX_BSSID_NUM + 1];
	uint64_t au8RxBytes[MAX_BSSID_NUM + 1];
	uint64_t au8TxPackets[MAX_BSSID_NUM + 1];
	uint64_t au8RxPackets[MAX_BSSID_NUM + 1];
};

struct TX_RX_TPUT {
	uint64_t au8TxTput[MAX_BSSID_NUM + 1];
	uint64_t au8RxTput[MAX_BSSID_NUM + 1];
};
...
struct PERF_MONITOR {
	...
	union {
		struct {
			uint64_t ulLastTxBytes[MAX_BSSID_NUM + 1];
			uint64_t ulLastRxBytes[MAX_BSSID_NUM + 1];
			uint64_t ulLastTxPackets[MAX_BSSID_NUM + 1];
			uint64_t ulLastRxPackets[MAX_BSSID_NUM + 1];
		};
		struct TX_RX_STATS_RECORD rLastStats;
	};
	OS_SYSTIME rLastStatsUpdateTime;

	unsigned long ulTxPacketsDiffLastSec[MAX_BSSID_NUM];
	unsigned long ulRxPacketsDiffLastSec[MAX_BSSID_NUM];

	/* priv_driver_get_tp_info() calls kalPerMonGetInfo() to get tput of
	 * wlan and p2p
	 * wlanLowDataRateMonitor(), kalPerMonHandler() and kalGetTpMbps() and
	 * kalIsTputMode
	 * call kalGetTpMbpsByBssId()
	 */
	uint64_t ulThroughput; /* in bps */
	union {
		struct {
			uint64_t ulTxTp[MAX_BSSID_NUM + 1]; /* in Bps */
			uint64_t ulRxTp[MAX_BSSID_NUM + 1]; /* in Bps */
		};
		struct TX_RX_TPUT rTputBps;
	};

	/* ulThroughputInPPS = pps * ETHER_MAX_PKT_SZ */
	uint64_t ulThroughputInPPS; /* in pps */
	struct TX_RX_TPUT rTputPps;
...
struct ADAPTER {
...
#if CFG_SUPPORT_WONDERTAP
	uint8_t u4WonMode;
	u_int8_t fgIsWONRegistered;
	u_int8_t fgIsWONRunning;
	enum ENUM_NET_REG_STATE rWONNetRegState;
	enum ENUM_WON_REG_STATE rWONRegState;
	struct WON_DEV_INFO *prWonInfo;
#endif
	...
	/* Mgmt tx related. */
	struct NAN_MGMT_TX_REQ_INFO rNanMgmtTxInfo;
#endif
...
/*#endif*/
	...
	struct SCHED_PM_PARAMS rSchedPmParams;
#endif
#endif
...
#define IS_BSS_WON(_prBssInfo) \
	(_prBssInfo && \
	((_prBssInfo)->eNetworkType == NETWORK_TYPE_WON))

#define IS_BSS_INDEX_WON(_prAdapter, _BssIndex) \
	(GET_BSS_INFO_BY_INDEX(_prAdapter, _BssIndex) && \
	IS_BSS_WON(GET_BSS_INFO_BY_INDEX(_prAdapter, _BssIndex)))

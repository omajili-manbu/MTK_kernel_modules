...
#define AUTH_TYPE_FT			BIT(AUTH_ALGORITHM_NUM_FT)
#define AUTH_TYPE_SAE			BIT(AUTH_ALGORITHM_NUM_SAE)
#define AUTH_TYPE_FILS_SK		BIT(AUTH_ALGORITHM_NUM_FILS_SK)
...
enum ENUM_MLO_MODE {
	MLO_MODE_SLSR, /* Single link single radio */
	MLO_MODE_STR,
	MLO_MODE_MLSR,
	MLO_MODE_EMLSR,
...
enum ENUM_DRV_OWN_SRC {
	...
	DRV_OWN_SRC_SDIO_RESUME,
	DRV_OWN_SRC_WR_REG_MAC,
	DRV_OWN_SRC_ICS_OFF_CMD,
...
#endif
#if CFG_SUPPORT_WONDERTAP
#define STA_TYPE_WON_MASK                   BIT(STA_TYPE_WON_INDEX)
#endif
...
#endif
#if CFG_SUPPORT_WONDERTAP
#define IS_STA_WON_TYPE(_prStaRec) ((_prStaRec->eStaType) & STA_TYPE_WON_MASK)
#endif
...
#endif
#if CFG_SUPPORT_WONDERTAP
	STA_TYPE_WON = (STA_TYPE_WON_MASK),
#endif
};
...
/* If you want to add a new roaming reason, you need to also update both
 * ENUM_ROAMING_REPORT_REASON and apucRoamingReasonConvert accordingly.
 */
...
#define WLAN_SET_FIELD_16(_memAddr_p, _value) \
	{ \
		uint8_t *__cp = (uint8_t *)(_memAddr_p); \
		uint16_t __value = (uint16_t) _value; \
		__cp[0] = (uint8_t)(__value); \
		__cp[1] = (uint8_t)((__value) >> 8); \
	}

#define WLAN_SET_FIELD_64(_memAddr_p, _value) \
	{ \
		uint8_t *__cp = (uint8_t *)(_memAddr_p); \
		uint64_t __value = (uint64_t) _value; \
		__cp[0] = (uint8_t)((__value) & 0xff); \
		__cp[1] = (uint8_t)((__value) >> 8); \
		__cp[2] = (uint8_t)((__value) >> 16); \
		__cp[3] = (uint8_t)((__value) >> 24); \
		__cp[4] = (uint8_t)((__value) >> 32); \
		__cp[5] = (uint8_t)((__value) >> 40); \
		__cp[6] = (uint8_t)((__value) >> 48); \
		__cp[7] = (uint8_t)((__value) >> 56); \
	}
...
#define WLAN_SET_FIELD_BE16(_memAddr_p, _value) \
	{ \
		uint8_t *__cp = (uint8_t *)(_memAddr_p); \
		uint16_t __value = (uint16_t) _value; \
		__cp[0] = (uint8_t)((__value) >> 8); \
		__cp[1] = (uint8_t)(__value); \
	}

#define WLAN_SET_FIELD_24(_memAddr_p, _value) \
	{ \
		uint8_t *__cp = (uint8_t *)(_memAddr_p); \
		uint32_t __value = (uint32_t) _value; \
		__cp[0] = (uint8_t)(__value); \
		__cp[1] = (uint8_t)((__value) >> 8); \
		__cp[2] = (uint8_t)((__value) >> 16); \
	}
...
#define WLAN_SET_FIELD_BE24(_memAddr_p, _value) \
	{ \
		uint8_t *__cp = (uint8_t *)(_memAddr_p); \
		uint32_t __value = (uint32_t) _value; \
		__cp[0] = (uint8_t)((__value) >> 16); \
		__cp[1] = (uint8_t)((__value) >> 8); \
		__cp[2] = (uint8_t)(__value); \
	}
...
#define WLAN_SET_FIELD_BE32(_memAddr_p, _value) \
	{ \
		uint8_t *__cp = (uint8_t *)(_memAddr_p); \
		uint32_t __value = (uint32_t) _value; \
		__cp[0] = (uint8_t)((__value) >> 24); \
		__cp[1] = (uint8_t)((__value) >> 16); \

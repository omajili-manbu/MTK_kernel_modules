...
#define STAREC_D "[%s] Update STA_REC:Idx:%d,WtblIdx:%d,BssIdx:%d,staType:%d\n"
...
struct _NAN_NDP_INSTANCE_T {
	...
	uint8_t *pucAttrList; /* make a copy by nanNdpBufferNanAttrLists() */
...
struct _NAN_CMD_DATA_POWERSAVE {
	uint32_t u4NdpTransactionId;
	uint16_t u2UpdateConfig;
	uint32_t u4InstanceId;
	uint32_t u4Latency;
	uint32_t u4Tput;
};
...
void *nanRetrieveAttrById(uint8_t *pucNanAttrList,
			  uint16_t u2NanAttrListLength,
...
void nanDataEngineUlwAttrAppend(struct ADAPTER *prAdapter,
				struct MSDU_INFO *prMsduInfo,
				struct _NAN_NDL_INSTANCE_T *prNDL,
				struct _NAN_NDP_INSTANCE_T *prNDP);

...
#define NAN_6G_BW20_CERT_DEFAULT_CHANNEL	37
#endif
...
#define NAN_MAX_NDC_MGMT_NUM NAN_MAX_CONN_CFG
...
#define NAN_SLOTS_PER_DW_INTERVAL (NAN_DW_INTERVAL / NAN_SLOT_INTERVAL) /* 32 */
...
#define NAN_2G_DEFAULT_RANGING 0xFF000000 /* slot 24~31 */
#define NAN_5G_DEFAULT_RANGING 0xFF000000 /* slot 24~31 */
...
#define NAN_NON_DBDC_AIS 0x00FF00FC
...
/* For single map, do not restrict slot_1 for CERT */
...
({									\
	...
	if (nanGetTimelineNum() == 1 &&					\
		!nanGetFeatureIsSigma(_prAdapter))			\
		u4Bitmap = NAN_NON_DBDC_AIS;				\
	if (nanGetTimelineNum() >= 2 &&					\
		NAN_IS_2G_TIMELINE(_prAdapter, _szTimeLineIdx))		\
...
({									\
	...
	if (nanGetTimelineNum() >= 2 &&					\
		NAN_IS_2G_TIMELINE(_adapter, _szTimeLineIdx))		\
...
#define NAN_IS_CHANNEL_5G(_rChnlInfo) \
	(NAN_IS_CHANNEL_5G_LOW(_rChnlInfo) || \
	 NAN_IS_CHANNEL_5G_HIGH(_rChnlInfo))
...
struct _NAN_SCHEDULE_TIMELINE_T {
	uint8_t ucMapId; /* peer MapId or local generated MapId */
	uint8_t ucLocalMapId; /* TX NDC attribute, peer MapId ?!= local MapId */
	uint8_t aucRsvd[2];
...
enum NDP_PS_STATE {
	NDP_PS_NONE,
	NDP_ENTER_PS, /* in power aving state */
	NDP_RESUME_FROM_PS, /* active from power saving */
};

struct _NAN_NDL_CUSTOMIZED_T {
	u_int8_t fgBitmapSet;
	enum NDP_PS_STATE ePowerSaving;
	uint8_t ucOpChannel;
	uint32_t u4Bitmap;
};
...
struct _NAN_NDC_CTRL_T {
	...
	/* The ucMapId shall set according to prNanTimelineMgmt->ucMapId
	 * Ref: nanSchedNegoAllocNdcCtrl
	 */
	struct _NAN_SCHEDULE_TIMELINE_T arTimeline[NAN_NUM_AVAIL_DB];
};

struct _NAN_NDC_MGMT_T {
	struct _NAN_NDC_CTRL_T rNdcCtrl;
#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_DATA == 1)
	enum ENUM_MBMC_BN eTxDbdcIdx;
	atomic_t mcTxRefCount[NAN_MAX_MC_TX_WTBL_NUM];
	uint16_t au2BmcTxWtblIdx[NAN_MAX_MC_TX_WTBL_NUM];
	uint8_t fgIsKeyExist[NAN_MAX_MC_TX_WTBL_NUM];
#endif
};
...
struct _NAN_AVAILABILITY_TIMELINE_T {
	...
	union {
		uint32_t au4AvailMap[NAN_TOTAL_DW];
		uint8_t aucAvailMap[NAN_TOTAL_DW * sizeof(uint32_t)];
	};
};

struct _NAN_AVAILABILITY_DB_T {
	uint8_t ucMapId;
	uint8_t ucLastRxSeqId; /* for checking duplicate RX availability */
...
#define MAX_ULW_ATTR_DATA_SIZE 24
struct PEER_ULW_ATTR_DATA {
	uint8_t ucLen;
	uint8_t aucData[MAX_ULW_ATTR_DATA_SIZE];
};
...
struct _NAN_PEER_SCH_DESC_T {
	...
	/* Record band with sufficient committed slots */
	uint32_t u4CommittedBand;

	enum NAN_RX_PEER_SPECIAL_AVAIL ePeerForceAvailAttr;

	/* control to invalidate availability maps */
	uint8_t ucLastAvailabilitySequence;

	/* Collected from nanNd[pl]ParseAttributes() */
	uint8_t aucAttributesBitmap[256 / CHAR_BIT];

	struct PEER_ULW_ATTR_DATA arPeerUlwAttr[NAN_MAX_ULW_RECORD];
};
...
struct _NAN_PEER_SCHEDULE_RECORD_T {
	...
	u_int8_t fgInPowerSaving; /* intended PS with data stopped */

#if CFG_SUPPORT_NAN_RANGING
	enum ENUM_BAND eNanRangingBand;
#endif /* CFG_SUPPORT_NAN_RANGING */
};

#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_DATA == 1)

enum _ENUM_NAN_NDC_MGMT_CMD_TYPE {
	NAN_NDC_MGMT_CMD_TYPE_INVALID = 0,
	NAN_NDC_MGMT_CMD_TYPE_ACTIVATE,
	NAN_NDC_MGMT_CMD_TYPE_INACTIVATE,
	NAN_NDC_MGMT_CMD_TYPE_NUM
};

struct _NAN_CMD_NDC_CTRL_MGMT_T {
	uint32_t u4CmdActionType; /* enum _ENUM_NAN_NDC_MGMT_CMD_TYPE */
	uint8_t aucNdcId[NAN_NDC_ATTRIBUTE_ID_LENGTH];
	uint8_t ucNdcIdx;
	uint8_t ucDefaultDbdcIdx;
...
/* Ref: _NAN_ATTR_UNALIGNED_SCHEDULE_T */
struct _NAN_ULW_RECORD_T {
	u_int8_t fgValid; /* is this ULW record effective */
	uint8_t ucCountDown; /* 255 */
	uint8_t ucOverrideMap; /* 0, b1OverwriteAll, b4OverwriteMapId  */
	u_int8_t fgChnlAvailability; /* b1ChannelAvail */

	/* determine whether _NAN_ATTR_ULW_CONTROL presents */
	u_int8_t fgNoAvailability;
	uint8_t ucSchID; /* b4ScheduleId */
	uint8_t ucSeqID; /* b8SequenceId */
	uint8_t ucRxNss; /* _NAN_ATTR_ULW_CONTROL b4RxNss */

	/* _NAN_ATTR_ULW_CONTROL generally, b2Type = ULW_CTRL_BY_CHANNEL */
	union _NAN_BAND_CHNL_CTRL rBandChnlInfo; /* _NAN_CHNL_ENTRY_T */

	u_int32_t u4StartTime; /* u4StartingTime  */
	u_int32_t u4Period; /* u4Period */
	u_int32_t u4Duration; /* u4Duration */

/**
 * FC:
 *	Schedule ID: 0
 *	Sequecne ID: ++
 *	Starting: 2688155653		9043972		11141123
 *	Duration: 32768
 *	Period: 524288 (512 TUs)
 *	CountDown: 255
 *	Overwrite All: True
 *	Map ID: 0
 * ULW1:
 *	Schedule ID: 1
 *	Sequecne ID: ++
 *	Starting: 2688950161		2754691286	2809987551
 *	Duration: 47303(46.19)		47587(46.47)	47807(46.68)
 *	Period: 204800 (200 TUs)
 *	CountDown: 255
 *	Overwrite All: True
 *	Map ID: 0
 *	Control Type: 1 (Channel w/o Aux)
 *	Control Channel Avail: 1
 *	Control Rx Nss: 2
 *	Channel Entry OC: 128
 *	Channel Entry Channel Bitmap: 0x0001 (Center=42)
 *	Channel Entry Primary Bitmap: 0x04 (pri=44)
 *
 * Avail:   [2688950161+204800*n,      2688950161+204800*n+47303]
 * Unavail: [2688950161+204800*n47303, 2688950161+204800*n+204800]
 *
 *
 * ULW2:
 *	Schedule ID: 2
 *	Sequecne ID: ++
 *	Starting: 2689034620(+82)	2754776319(+83)	2810073148(+83)
 *	Duration: 65097(63.57)		64813(63.29)	64593(63.08)
 *	Period: 204800 (200 TUs)
 *	CountDown: 255
 *	Overwrite All: True
 *	Map ID: 0
 ...
 *	Channel Entry Primary Bitmap: 0x04 (pri=44)
 *
 * Avail:   [2689034620+204800*n,       2689034620+204800*n+65097]
 * Unavail: [2689034620+204800*n+65097, 2689034620+204800*n+204800]
 *
 * Common Avail:
 *	[2688950161+204800*n,      2688950161+204800*n+47303]
 *	[2689034620+204800*n,      2689034620+204800*n+65097]
 *
 * Common Unavail:
 *	[2688950161+47303, 2689034620]
 *	[2689034620+65097, 2688950161+204800]
 */
...
struct _NAN_SCHEDULER_T {
	...
	unsigned char fgAdoptCustChnl;		/* find out why */
	unsigned char fgAdoptCustNdcSched;	/* find out why */
	...
	struct _NAN_NDC_MGMT_T arNdcMgmt[NAN_MAX_NDC_MGMT_NUM];
	...
	struct _NAN_ATTR_COUNTRY_CODE_T rCountryCode;

	uint32_t au4NumOfPotentialChnlList[NAN_TIMELINE_MGMT_SIZE];
	/* Save potential channels in nanSchedCmdUpdatePotentialChnlList */
	...
	/* Local committed band */
	enum _NAN_SUPPORTED_BAND_BIT eLocalCommittedBand;

	/* store for reschedule after PS peers resumed */
	enum RESCHEDULE_SOURCE eRescheduleEventInPS;
	struct _NAN_NDL_INSTANCE_T *prRescheduleNdlInPS;

	u_int8_t fgFlashCommunication;

	unsigned char fgNanIs2gDwOnly;

	/* Local ULW record from summarized concurrent status */
	u_int8_t fgLocalUlwLockRequested;
	u_int8_t fgLocalUlwSet; /* set TRUE if countdown != 0 record from FW */
	struct _NAN_ULW_RECORD_T arNanFcUlw[1];
	struct _NAN_ULW_RECORD_T arNanLocalUlw[NAN_MAX_ULW_RECORD];

#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_DATA == 1)
	/* Record NDC Ctrl for doing re-schedule */
	struct _NAN_NDC_CTRL_T *prPreviousNdcCtrl;
#endif /* (CFG_NAN_SUPPORT_R4_GROUP_ADDR_DATA == 1) */
};
...
uint32_t nanSchedGetCountryCodeAttr(struct ADAPTER *prAdapter,
				    uint8_t **ppucCountryCodeAttr,
				    uint32_t *pu4CountryCodeAttrLength);
...
uint32_t nanSchedPeerUpdateUlwAttr(struct ADAPTER *prAdapter,
				   uint8_t *pucNmiAddr, uint8_t *pucUlwAttr);
...
uint32_t nanSchedulerEventDispatch(struct ADAPTER *prAdapter, uint8_t *pucBuf);
...
extern uint8_t g_aucNanIEBuffer[NAN_IE_BUF_MAX_SIZE];
...
void nanSchedSetLocalCommittedBand(struct ADAPTER *prAdapter);
...
/* Add 2G/5G DW slot to timeline according to 2G only or not */
uint32_t
nanSchedSetDwTimeline(struct ADAPTER *prAdapter, uint8_t fgDW2gOnly);
/* Set DW timeline when NAN enable and update to fw  */
uint32_t

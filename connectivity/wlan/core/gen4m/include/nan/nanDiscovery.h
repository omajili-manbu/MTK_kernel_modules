...
extern uint8_t g_aucPubServiceName[NAN_MAX_SERVICE_NAME_LEN];
...
#endif /* CFG_SUPPORT_NAN_R4_PAIRING */
#if (CFG_SUPPORT_NAN_EVENT_ALIGNMENT == 1)
} __KAL_ATTRIB_PACKED__ __KAL_ATTRIB_ALIGNED__(4);
#else
};
#endif /* CFG_SUPPORT_NAN_EVENT_ALIGNMENT */
...
void
nanDiscSetupInstance(uint8_t ucInstanceId, uint8_t type,
		uint8_t *service_hash, uint8_t *service_name);

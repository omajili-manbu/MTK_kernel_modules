...
static uint32_t u4ProjectId = 8875;
...
static int32_t mt8875_wlan_pinctrl_init(struct mt66xx_chip_info *chip_info);
static int32_t mt8875_wlan_pinctrl_action(struct mt66xx_chip_info *chip_info,
	enum WLAN_PINCTRL_MSG msg);

static struct WLAN_PINCTRL_OPS mt8875_pinctrl_ops = {
	.init = mt8875_wlan_pinctrl_init,
	.action = mt8875_wlan_pinctrl_action,
};
#endif
...
int32_t kalPlatOpsInit(void)
{
	...
	if (driver_data && driver_data->chip_info) {
		...
		chip->pinctrl_ops = &mt8875_pinctrl_ops;
	}

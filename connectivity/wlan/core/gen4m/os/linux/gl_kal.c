...
#define __BUFFER_SIZE 1024
	...
	buf = (char *) kalMemAlloc(__BUFFER_SIZE, PHY_MEM_TYPE);
	if (!buf) {
	...
	kalMemFree(buf, PHY_MEM_TYPE, __BUFFER_SIZE);
...
uint8_t kalQuery80211HeaderLength(void *pvPacket)
{
	struct sk_buff *skb = (struct sk_buff *)pvPacket;
	uint16_t u2FrameCtrl;
	uint8_t ucMacLen = 0;

	if (!skb || skb->len < sizeof(uint16_t))
		return 0;

	/* 802.11 Frame Control is always Little Endian */
	u2FrameCtrl = skb->data[0] | (skb->data[1] << 8);

	switch (u2FrameCtrl & MASK_FC_TYPE) {
	case MAC_FRAME_TYPE_MGT:
		ucMacLen = WLAN_MAC_MGMT_HEADER_LEN;
		break;

	case MAC_FRAME_TYPE_CTRL:
		switch (u2FrameCtrl & MASK_FRAME_TYPE) {
		case MAC_FRAME_PS_POLL:
		case MAC_FRAME_RTS:
		case MAC_FRAME_BLOCK_ACK_REQ:
		case MAC_FRAME_BLOCK_ACK:
			ucMacLen = PSPOLL_FRAME_LEN;
			break;
		case MAC_FRAME_CTS:
		case MAC_FRAME_ACK:
			ucMacLen = WLAN_MAC_CTS_ACK_HEADER_LEN;
			break;
		case MAC_FRAME_CF_END:
		case MAC_FRAME_CF_END_CF_ACK:
			ucMacLen = WLAN_MAC_CF_END_HEADER_LEN;
			break;
		default:
			ucMacLen = WLAN_MAC_CTS_ACK_HEADER_LEN;
			break;
	...
	case MAC_FRAME_TYPE_DATA:
		...
		/* Address 4: ToDS and FromDS both set (WDS) */
		if ((u2FrameCtrl & MASK_TO_DS_FROM_DS) == MASK_TO_DS_FROM_DS)
			ucMacLen += MAC_ADDR_LEN;

		/* QoS Control: Data type and Subtype bit 3 (bit 7 of FC) set */
		if (u2FrameCtrl & MASK_FC_SUBTYPE_QOS_DATA)
			ucMacLen += QOS_CTRL_LEN;
		break;
	...
	/* HT Control: "Order" bit set - applicable to Mgmt and Data frames */
	if ((u2FrameCtrl & MASK_FC_TYPE) != MAC_FRAME_TYPE_CTRL &&
	    (u2FrameCtrl & MASK_FC_ORDER)) {
		ucMacLen += HT_CTRL_LEN;
	}

	/* Boundary check: calculated length cannot exceed actual
	 * captured bytes
	 */
	if (ucMacLen > skb->len)
		ucMacLen = (uint8_t)skb->len;

#if CFG_WONDERTAP_TX_RX_DEBUG
	DBGLOG(INIT, DEBUG, "Calculated MacLenRet:%u Type:%u Subtype:%u\n",
		ucMacLen,
		(uint8_t)((u2FrameCtrl & MASK_FC_TYPE) >> 2),
		(uint8_t)((u2FrameCtrl & MASK_FC_SUBTYPE) >>
			   ...
			   void *apvPkts[], uint8_t ucPktNum)
{
	...
	KAL_WAKE_LOCK_TIMEOUT(prGlueInfo->prAdapter,
		prGlueInfo->rRxIndPktsWakeLock, MSEC_TO_JIFFIES(
...
void kal_netif_free(struct GLUE_INFO *prGlueInfo, struct net_device *dev)
{
	struct sk_buff *skb, *tmp;
	struct sk_buff_head *prRxNapiSkbQ = NULL;
	uint16_t skb_free_cnt = 0;

	if (prGlueInfo && dev) {
		prRxNapiSkbQ = &prGlueInfo->rRxNapiSkbQ;
		skb_queue_walk_safe(prRxNapiSkbQ, skb, tmp) {
			if (skb->dev == dev) {
				skb_unlink(skb, prRxNapiSkbQ);
				kfree_skb(skb);
				skb_free_cnt++;
			}
		}
		free_netdev(dev);
		DBGLOG(NIC, INFO, "Freed rRxNapiSkbQ count:%d", skb_free_cnt);
	}
}
			     ...
			     *prGlueInfo, void *pvPkt)
{
...
#if CFG_WONDERTAP_TX_RX_DEBUG
	DBGLOG(WON, DEBUG, "ucBssIdx:%u, netdev:0x%p, name:%s\n",
	...
	if (prGlueInfo->fgIsEnableMon ||
		IS_BSS_INDEX_WON(prGlueInfo->prAdapter, ucBssIdx)) {
		       ...
		       struct TX_PACKET_INFO *prTxPktInfo)
{
	...
	uint16_t u2IpId;
	uint8_t ucIpHLen;
	uint16_t u2IpTotalLen;
	uint16_t u2IpPayLoadLen;
#if CFG_SUPPORT_WIFI_SYSDVT
#if (CFG_TCP_IP_CHKSUM_OFFLOAD)
	struct ADAPTER *prAdapter = NULL;
#endif
	...
	/* get ip header len and ip total len */
	/* ip header len is filled with word count */
	ucIpHLen = (pucIpHdr[0] & 0x0F) << 2;
	u2IpTotalLen = (pucIpHdr[IPV4_HDR_IP_TOTAL_LENGTH_OFFSET] << 8) |
			pucIpHdr[IPV4_HDR_IP_TOTAL_LENGTH_OFFSET + 1];

	/* sanity check on ip header length */
	if (ucIpHLen < IPV4_HDR_LEN_MIN || ucIpHLen > IPV4_HDR_LEN_MAX ||
		u2IpTotalLen < ucIpHLen) {
		DBGLOG(TX, WARN, "Invalid IP Len [%u:%u]\n",
			ucIpHLen, u2IpTotalLen);
		return FALSE;
	}

	u2IpPayLoadLen = u2IpTotalLen - ucIpHLen;
	...
	if (ucIpProto == IP_PRO_UDP) {
		...
		pucTcpHdr = &pucIpHdr[ucIpHLen];
		/* TCP header len is filled with word count */
		ucTcpHLen = (pucTcpHdr[TCP_HDR_HEADER_LEN_OFFSET]
				& TCP_HDR_HEADER_LEN_MASK)
				>> TCP_HDR_HEADER_LEN_SHIFT;
		ucTcpHLen = ucTcpHLen << 2;

		/* check for tcp header len */
		if (ucTcpHLen < TCP_HDR_LEN_MIN ||
			ucTcpHLen > TCP_HDR_LEN_MAX ||
			ucTcpHLen > u2IpPayLoadLen) {
			DBGLOG(TX, WARN, "Invalid TCP Len [%u:%u:%u]\n",
				ucTcpHLen, u2IpPayLoadLen, u2IpTotalLen);
			return FALSE;
		}
		...
		DBGLOG(TX, TEMP,
			"TcpAck:%u u2IpId:%u Flag:%02x IpLen:%u IpHLen:%u u2IpPayLoadLen:%u TcpHLen:%u TotLen:%u\n",
			u2IpPayLoadLen == ucTcpHLen,
			u2IpId,
			pucTcpHdr[TCP_HDR_FLAG_OFFSET],
			u2IpTotalLen,
			ucIpHLen,
			u2IpPayLoadLen,
			ucTcpHLen,
			kalQueryPacketLength(prPacket));

		if (u2IpPayLoadLen == ucTcpHLen)
...
#if CFG_SUPPORT_NAN
	kalMemCopy(prTxPktInfo->aucEthSrcAddr,
		aucLookAheadBuf + PARAM_MAC_ADDR_LEN, PARAM_MAC_ADDR_LEN);

	DBGLOG_LIMITED(INIT, LOUD,
		"SA: " MACSTR ", DA: " MACSTR "\n",
		MAC2STR(prTxPktInfo->aucEthSrcAddr),
		MAC2STR(prTxPktInfo->aucEthDestAddr));
...
#if CFG_SUPPORT_NAN
u_int8_t kalGetEthSrcAddr(struct GLUE_INFO *prGlueInfo,
			   void *prPacket, uint8_t *pucEthSrcAddr)
{
	struct sk_buff *prSkb = (struct sk_buff *)prPacket;
	...
	COPY_MAC_ADDR(pucEthSrcAddr,
		pucLookAheadBuf + PARAM_MAC_ADDR_LEN);
	 ...
	 uint32_t *pu4QryInfoLen, uint8_t ucBssIndex)
{
	...
	KAL_WAKE_LOCK_TIMEOUT(prGlueInfo->prAdapter,
		prGlueInfo->rIoctlWakeLock, MSEC_TO_JIFFIES(
	...
	if (waitRet > 0) {
		...
		DBGLOG(OID, WARN,
			"wait main_thread timeout, duration:%llums, sched(x%llu/r%llu/i%llu), OID_bit:%d\n",
			schedstats.time, schedstats.exec,
			schedstats.runnable, schedstats.iowait,
			test_bit(GLUE_FLAG_OID_BIT, &prGlueInfo->ulFlag));
		KAL_WARN_ON(TRUE);
...
skip_tx_data:
		...
		if (test_and_clear_bit(HIF_FLAG_DUMP_PCIE_MSI_BIT,
				       &prGlueInfo->ulHifFlag)) {
			struct BUS_INFO *prBusInfo =
				prAdapter->chip_info->bus_info;
		...
		if (test_and_clear_bit(GLUE_FLAG_RX_TO_OS_BIT,
			...
			while (u4LoopCount--) {
				while (QUEUE_IS_NOT_EMPTY(
				    ...
				    KAL_WAKE_LOCK_TIMEOUT(prGlueInfo->prAdapter,
					prGlueInfo->rRxIndOnePktWakeLock,
...
#else
	hrtimer_setup(prTimer, NULL, CLOCK_BOOTTIME, HRTIMER_MODE_ABS);
#endif
}
...
uint32_t kalRxTaskWorkDone(struct GLUE_INFO *pr, u_int8_t fgIsInt)
{
	...
	sched_cnt = GLUE_TRY_DEC_REF_CNT(pr->u4RxTaskScheduleCnt);
	if (sched_cnt > 0) {
		/* pending schedule exists, reschedule RxTask */
#if CFG_SUPPORT_RX_WORK
		kalRxWorkSchedule(pr);
#else /* CFG_SUPPORT_RX_WORK */
		kalRxTaskletSchedule(pr);
		...
		/*
		 * Last tasklet done (sched_cnt==0), or no more schedule (<0),
		 * enable interrupt
		 */
		if (fgIsInt) {
#if CFG_SUPPORT_RX_NAPI
...
void kalSetHifDumpPcieMsi(struct GLUE_INFO *pr)
{
	set_bit(HIF_FLAG_DUMP_PCIE_MSI_BIT, &pr->ulHifFlag);
#if CFG_SUPPORT_MULTITHREAD
	wake_up_interruptible(&pr->waitq_hif);
#endif
}
...
void kalSetMgmtDirectTxEvent2Hif(struct GLUE_INFO *pr)
{
	...
	KAL_WAKE_LOCK_TIMEOUT(pr->prAdapter, pr->rMgmtTxEvtWakeLock,
...
void kalSetTxEvent2Hif(struct GLUE_INFO *pr)
{
	...
	KAL_WAKE_LOCK_TIMEOUT(pr->prAdapter, pr->rTxEvtWakeLock,
			      MSEC_TO_JIFFIES(
...
void kalSetFwOwnEvent2Hif(struct GLUE_INFO *pr)
{
	...
	KAL_WAKE_LOCK_TIMEOUT(pr->prAdapter, pr->rFwOwnWakeLock,
			      MSEC_TO_JIFFIES(
			      pr->prAdapter->rWifiVar.u4WakeLockThreadWakeup));
...
void kalSetTxEvent2Rx(struct GLUE_INFO *pr)
{
	...
	KAL_WAKE_LOCK_TIMEOUT(pr->prAdapter, pr->rTxEvtToRxWakeLock,
			      MSEC_TO_JIFFIES(
			      pr->prAdapter->rWifiVar.u4WakeLockThreadWakeup));
...
void kalSetTxCmdEvent2Hif(struct GLUE_INFO *pr)
{
	...
	if (!KAL_TEST_BIT(SUSPEND_FLAG_CLEAR_WHEN_RESUME, pr->fgIsInSuspend))
#endif
	KAL_WAKE_LOCK_TIMEOUT(pr->prAdapter, pr->rTxCmdEvtWakeLock,
			      MSEC_TO_JIFFIES(
			      pr->prAdapter->rWifiVar.u4WakeLockThreadWakeup));
...
u_int8_t wlanIsFirstNetInterfaceByNetdev(struct GLUE_INFO *prGlueInfo,
...
static void checkNanTput(struct ADAPTER *prAdapter, int32_t period,
			 struct TX_RX_TPUT *prTputBps,
			 struct TX_RX_TPUT *prTputPps)
{
	uint64_t nanTput;
	uint64_t nanPps;
	struct PERF_MONITOR *prPerMonitor;
	...
	for (ucBssIndex = 0; ucBssIndex < MAX_BSSID_NUM; ucBssIndex++) {
		...
		if (!bss || !IS_BSS_NAN(bss))
			continue;

		/* Bytes / sec */
		nanTput = prTputBps->au8TxTput[ucBssIndex] +
			  prTputBps->au8RxTput[ucBssIndex];
		nanTput <<= 3; /* bps */
		nanTput >>= 20; /* Mbps */

		/* Packets / sec */
		nanPps = prTputPps->au8TxTput[ucBssIndex] +
			 prTputPps->au8RxTput[ucBssIndex];
		...
		if (nanTput < PERF_MON_NAN_BOOST_CPU_THRESHOLD) {
			...
			DBGLOG(SW4, INFO,
			       "NAN tput: %llu(mbps), period=%d, TXpps=%llu, RXpps=%llu, boost=%u\n",
			       nanTput, period,
			       prTputPps->au8TxTput[ucBssIndex],
			       prTputPps->au8RxTput[ucBssIndex],
			       prPerMonitor->u4NanBoostCpu);
		}
	}
...
/**
 * Input: stats and system time on last sampling
 *
 * Return
 * 1. Current stats and system time
 * 2. Optional Diff of stats (TX_RX_STATS_RECORD)
 *   TX per BSS bytes, packets
 *   RX per BSS bytes, packets
 *   Sum of all BSS TX bytes, packets, saved in [MAX_BSSID_NUM]
 *   Sum of all BSS RX bytes, packets, saved in [MAX_BSSID_NUM]
 * 3. tput calculated by (currentTime - lastTime) (TX_RX_TPUT)
 *   prTputBps: Bytes/sec for per BSS, and sum of all BSS in [MAX_BSSID_NUM]
 *   prTputPps: Packets/sec for per BSS, and sum of all BSS in [MAX_BSSID_NUM]
 *
 * The caller shall save returned current stats and current time to
 * last stats and last time to be used in next call.
 */
uint32_t kalGetTput(struct ADAPTER *prAdapter,
		    const struct TX_RX_STATS_RECORD *prLast,
		    OS_SYSTIME lastTime,
		    struct TX_RX_STATS_RECORD *prCurrent,
		    OS_SYSTIME *currentTime,
		    struct TX_RX_STATS_RECORD *prDiff,
		    struct TX_RX_TPUT *prTputBps,
		    struct TX_RX_TPUT *prTputPps)
{
	...
	struct TX_RX_STATS_RECORD rCurrent = { 0 };
	struct TX_RX_STATS_RECORD rDiff = { 0 };
	...
	if (prDiff)
		kalMemZero(prDiff, sizeof(*prDiff));

	GET_BOOT_SYSTIME(&now);
	period = ((int32_t) now - (int32_t) lastTime) * MSEC_PER_SEC / KAL_HZ;
	...
	for (i = 0; i < MAX_BSSID_NUM; i++) {
		...
		if (fgIsValidNetDevice) {
			rCurrent.au8TxBytes[i] = ndev->stats.tx_bytes;
			rCurrent.au8RxBytes[i] = ndev->stats.rx_bytes;
			rCurrent.au8TxPackets[i] = ndev->stats.tx_packets;
			rCurrent.au8RxPackets[i] = ndev->stats.rx_packets;
		} else {
			rCurrent.au8TxBytes[i] = perf->ulLastTxBytes[i];
			rCurrent.au8RxBytes[i] = perf->ulLastRxBytes[i];
			rCurrent.au8TxPackets[i] = perf->ulLastTxPackets[i];
			rCurrent.au8RxPackets[i] = perf->ulLastRxPackets[i];
		}
		GLUE_RELEASE_SPIN_LOCK(glue, SPIN_LOCK_NET_DEV);


		rCurrent.au8TxBytes[MAX_BSSID_NUM] += rCurrent.au8TxBytes[i];
		rCurrent.au8RxBytes[MAX_BSSID_NUM] += rCurrent.au8RxBytes[i];
		rCurrent.au8TxPackets[MAX_BSSID_NUM] +=
			rCurrent.au8TxPackets[i];
		rCurrent.au8RxPackets[MAX_BSSID_NUM] +=
			rCurrent.au8RxPackets[i];

		/* In the unexpected case current < last, take as reset,
		 * increment only the current value
		 */
		rDiff.au8TxBytes[i] = rCurrent.au8TxBytes[i] -
			prLast->au8TxBytes[i] *
			(rCurrent.au8TxBytes[i] >= prLast->au8TxBytes[i]);

		rDiff.au8RxBytes[i] = rCurrent.au8RxBytes[i] -
			prLast->au8RxBytes[i] *
			(rCurrent.au8RxBytes[i] >= prLast->au8RxBytes[i]);

		rDiff.au8TxPackets[i] = rCurrent.au8TxPackets[i] -
			prLast->au8TxPackets[i] *
			(rCurrent.au8TxPackets[i] >= prLast->au8TxPackets[i]);

		rDiff.au8RxPackets[i] = rCurrent.au8RxPackets[i] -
			prLast->au8RxPackets[i] *
			(rCurrent.au8RxPackets[i] >= prLast->au8RxPackets[i]);

		/* Divsion first to avoid overflow */
		if (prTputBps) { /* Bytes / sec */
			uint64_t n;

			n = kal_div_u64(rDiff.au8TxBytes[i], period);
			prTputBps->au8TxTput[i] = n * MSEC_PER_SEC;

			n = kal_div_u64(rDiff.au8RxBytes[i], period);
			prTputBps->au8RxTput[i] = n * MSEC_PER_SEC;
		}

		if (prTputPps) { /* Packets / sec */
			uint64_t n;

			n = kal_div_u64(rDiff.au8TxPackets[i], period);
			prTputPps->au8TxTput[i] = n * MSEC_PER_SEC;

			n = kal_div_u64(rDiff.au8RxPackets[i], period);
			prTputPps->au8RxTput[i] = n * MSEC_PER_SEC;
		}
	}

	if (prCurrent)
		kalMemCopy(prCurrent, &rCurrent, sizeof(*prCurrent));

	rDiff.au8TxBytes[MAX_BSSID_NUM] =
		rCurrent.au8TxBytes[MAX_BSSID_NUM] -
			prLast->au8TxBytes[MAX_BSSID_NUM] *
			(rCurrent.au8TxBytes[MAX_BSSID_NUM] >=
			 prLast->au8TxBytes[MAX_BSSID_NUM]);
	rDiff.au8RxBytes[MAX_BSSID_NUM] =
		rCurrent.au8RxBytes[MAX_BSSID_NUM] -
			prLast->au8RxBytes[MAX_BSSID_NUM] *
			(rCurrent.au8RxBytes[MAX_BSSID_NUM] >=
			 prLast->au8RxBytes[MAX_BSSID_NUM]);
	rDiff.au8TxPackets[MAX_BSSID_NUM] =
		rCurrent.au8TxPackets[MAX_BSSID_NUM] -
			prLast->au8TxPackets[MAX_BSSID_NUM] *
			(rCurrent.au8TxPackets[MAX_BSSID_NUM] >=
			 prLast->au8TxPackets[MAX_BSSID_NUM]);
	rDiff.au8RxPackets[MAX_BSSID_NUM] =
		rCurrent.au8RxPackets[MAX_BSSID_NUM] -
			prLast->au8RxPackets[MAX_BSSID_NUM] *
			(rCurrent.au8RxPackets[MAX_BSSID_NUM] >=
			prLast->au8RxPackets[MAX_BSSID_NUM]);

	if (prDiff)
		kalMemCopy(prDiff, &rDiff, sizeof(*prDiff));

	if (prTputBps) { /* Bytes / sec */
		uint64_t n;

		n = kal_div_u64(rDiff.au8TxBytes[MAX_BSSID_NUM], period);
		prTputBps->au8TxTput[MAX_BSSID_NUM] = n * MSEC_PER_SEC;

		n = kal_div_u64(rDiff.au8RxBytes[MAX_BSSID_NUM], period);
		prTputBps->au8RxTput[MAX_BSSID_NUM] = n * MSEC_PER_SEC;
	}
	if (prTputPps) { /* Packets / sec */
		uint64_t n;

		n = kal_div_u64(rDiff.au8TxPackets[MAX_BSSID_NUM], period);
		prTputPps->au8TxTput[MAX_BSSID_NUM] = n * MSEC_PER_SEC;

		n = kal_div_u64(rDiff.au8RxPackets[MAX_BSSID_NUM], period);
		prTputPps->au8RxTput[MAX_BSSID_NUM] = n * MSEC_PER_SEC;
	}
...
static uint32_t kalPerMonCalculateTput(struct ADAPTER *prAdapter,
				       uint64_t *throughput,
				       int32_t *period,
				       struct TX_RX_STATS_RECORD *prDiff)
{
	...
	struct GLUE_INFO *glue = prAdapter->prGlueInfo;
	OS_SYSTIME now;
	struct TX_RX_STATS_RECORD rCurrentStats = { 0 };
	uint32_t u4Result;

	u4Result = kalGetTput(prAdapter,
			      &perf->rLastStats, perf->rLastStatsUpdateTime,
			      &rCurrentStats, &now,
			      prDiff, &perf->rTputBps, &perf->rTputPps);
	if (u4Result != WLAN_STATUS_SUCCESS)
		return u4Result;

	kalMemCopy(perf->ulTxPacketsDiffLastSec, prDiff->au8TxPackets,
		   sizeof(perf->ulTxPacketsDiffLastSec));
	kalMemCopy(perf->ulRxPacketsDiffLastSec, prDiff->au8RxPackets,
		   sizeof(perf->ulRxPacketsDiffLastSec));

	*period = now - perf->rLastStatsUpdateTime;
	perf->rLastStatsUpdateTime = now;

	*throughput = perf->rTputBps.au8TxTput[MAX_BSSID_NUM] +
		      perf->rTputBps.au8RxTput[MAX_BSSID_NUM];
	perf->fgIdle = (*throughput == 0 && glue->i4TxPendingFrameNum == 0);
	/* Bps => (x8)  => bps */
	perf->ulThroughput = (perf->rTputBps.au8TxTput[MAX_BSSID_NUM] +
			      perf->rTputBps.au8RxTput[MAX_BSSID_NUM]) << 3;

	/* Pps => (x1500 x8) => bps */
	perf->ulThroughputInPPS = ((perf->rTputPps.au8TxTput[MAX_BSSID_NUM] +
				    perf->rTputPps.au8RxTput[MAX_BSSID_NUM]) *
				   ETHER_MAX_PKT_SZ) << 3;

	kalMemCopy(&perf->rLastStats, &rCurrentStats, sizeof(perf->rLastStats));

	checkNanTput(prAdapter, *period, &perf->rTputBps, &perf->rTputPps);
			  ...
			  struct TX_RX_STATS_RECORD *prDiff)
{
	...
	for (i = 0; i < MAX_BSSID_NUM + 1; i++) {
		pos += kalSnprintf(pos, end - pos, "[%llu,%llu,%llu,%llu]",
				   prDiff->au8TxBytes[i],
				   prDiff->au8TxPackets[i],
				   prDiff->au8RxBytes[i],
				   prDiff->au8RxPackets[i]);
	}
...
static uint32_t kalPerMonUpdate(struct ADAPTER *prAdapter)
{
	...
	ret = kalPerMonCalculateTput(prAdapter, &throughput, &period, &rDiff);
	if (ret != WLAN_STATUS_SUCCESS)
		goto done;
	...
	if (prAdapter->rWifiVar.fgSapRpsEnable == 1)
		p2pFuncRpsKalCheck(prAdapter, period, rDiff.au8RxPackets);
#endif
	...
	pos += composeTput(prAdapter, pos, end, &rDiff);
	...
	for (ucBssIndex = 0; ucBssIndex < MAX_BSSID_NUM; ucBssIndex++) {
		prBssInfo = GET_BSS_INFO_BY_INDEX(prAdapter, ucBssIndex);
		if (prBssInfo &&
		    IS_BSS_INFO_IN_AIS(prBssInfo) &&
		    kalGetMediaStateIndicated(glue, ucBssIndex) ==
			MEDIA_STATE_CONNECTED) {
			if (wlanGetRxRateByBssid(glue, ucBssIndex,
	...
	pos += kalSnprintf(pos, end - pos,
		"drv[RM,IL,RI,RIE,NG|RF,PA,PAP,PF,DU,DA#RT,RM,RW,RA,RB|RTO,DT,NS,IB,HS_LS,DD,ME,BD,NI|RAD,RBD,TE,PE,CE^DN,FE,DE,IE,TME#CM,FB,ID,FD,NL]:");

	pos += kalSnprintf(pos, end - pos,
		"%lu,%lu,%lu,%lu,%lu|%lu,%lu,%lu,%lu,%lu,%lu#%lu,%lu,%lu,%lu,%lu|%lu,%lu,%lu,%lu,%lu_%lu,%lu,%lu,%lu,%lu|%lu,%lu,%lu,%lu,%lu^%lu,%lu,%lu,%lu,%lu#%lu,%lu,%lu,%lu,%lu",
		...
		RX_GET_CNT(&prAdapter->rRxCtrl, RX_REORDER_AHEAD_DROP_COUNT),
...
#define TPUT_LOG_TEMPLATE \
	...
	DBGLOG(SW4, DEBUG,
		"freq %s mask:online %lx, hif %lx, rx %lx, main %lx, ApFS=%d",
		buf,
	...
	for (i = 0; i < MAX_BSSID_NUM; i++) {
		...
		if (IS_BSS_ALIVE(prAdapter, prBssInfo) && prDevHandler) {
			...
			*/
			...
			roamingFsmInactiveMonitor(prAdapter, i);
...
void __weak kalReviseCfgsByPlatform(struct ADAPTER *prAdapter)
{
	...
	u_int8_t fgSuccess)
{
	...
	if (roam->eReason < ROAMING_REASON_NUM)
		log_info->roamingReason =
				apucRoamingReasonConvert[roam->eReason];
	else
		log_info->roamingReason = ROAMING_REPORT_REASON_UNSPECIFIC;
...
void kalNapiThreadedInit(struct GLUE_INFO *prGlueInfo)
{
	...
	if (dev_set_threaded(prGlueInfo->napi_dev, TRUE) != 0) {
...
#if KERNEL_VERSION(6, 14, 0) <= CFG80211_VERSION_CODE
	prGlueInfo->napi_dev = alloc_netdev_dummy(0);
	if (!prGlueInfo->napi_dev)
		panic("Failed to allocate Napi dummy netdev\n");
#else
	init_dummy_netdev(&prGlueInfo->napi_dev);
#endif
#if (KERNEL_VERSION(6, 14, 0) <= CFG80211_VERSION_CODE)
	netif_napi_add(prGlueInfo->napi_dev, &prGlueInfo->napi,
			kalNapiPoll);
...
#endif /* CFG_SUPPORT_RX_NAPI_THREADED */
#if KERNEL_VERSION(6, 14, 0) <= CFG80211_VERSION_CODE
	free_netdev(prGlueInfo->napi_dev);
#endif
...
#else
	hrtimer_setup(&pr->rNapiDelayTimer, kalNapiDelayTimeout,
...
static int kalNapiPollSwRfb(struct napi_struct *napi, int budget)
{
	...
	while ((work_done <= u4Cnt) && (work_done < budget) &&
...
#endif
#if CFG_SUPPORT_NAN_R4_PAIRING
uint8_t kalRequestNanPasnStart(struct ADAPTER *prAdapter,
		void *pvPasnReq,
		uint8_t ucBssIdx)
{
	struct sk_buff *skb = NULL;
	...
	struct net_device *netdev = NULL;
	struct wireless_dev *wdev = NULL;
	struct NAN_PASN_START_PARAM *prPasnReq =
		(struct NAN_PASN_START_PARAM *)pvPasnReq;
	uint32_t dataLen;
	...
	DBGLOG(NAN, INFO, "[pairing-pasn] enter\n");

	dataLen = NLMSG_HDRLEN;
	dataLen += (NLMSG_HDRLEN
		+ sizeof(enum mtk_wlan_vendor_nl80211_attr_nan_pasn_state));
	dataLen += (NLMSG_HDRLEN + sizeof(struct NAN_PASN_START_PARAM));

	skb = kalCfg80211VendorEventAlloc(wiphy, wdev,
		dataLen, WIFI_EVENT_SUBCMD_NAN_PASN, GFP_KERNEL);
	if (!skb) {
		DBGLOG(REQ, ERROR, "allocate skb failed\n");
		return -ENOMEM;
	}

	if (unlikely(nla_put_u32(skb, MTK_WLAN_VENDOR_ATTR_NAN_PASN_STATE,
		MTK_WLAN_VENDOR_ATTR_NAN_PASN_INITIATOR_START) < 0))
		goto nla_put_failure;

	if (unlikely(nla_put(skb,
			MTK_WLAN_VENDOR_ATTR_NAN_PASN_M1_REQ_PARAM,
			sizeof(struct NAN_PASN_START_PARAM),
			prPasnReq) < 0))
		goto nla_put_failure;

	cfg80211_vendor_event(skb, GFP_KERNEL);
	DBGLOG(NAN, INFO, "[pairing-pasn] leave\n");
	return TRUE;

nla_put_failure:
	DBGLOG(NAN, INFO, "[pairing-pasn] failed\n");
	kfree_skb(skb);
	return FALSE;
}


uint8_t kalReportNanPasnFrame(struct ADAPTER *prAdapter,
		void *pvPasnFrame, uint8_t ucBssIdx)
{
	struct sk_buff *skb = NULL;
	struct wiphy *wiphy = NULL;
	struct net_device *netdev = NULL;
	struct wireless_dev *wdev = NULL;
	struct NanPairingPASNMsg *prPasnframe = NULL;
	struct WLAN_AUTH_FRAME *prAuthFrame = NULL;
	uint32_t dataLen;
	enum mtk_wlan_vendor_nl80211_attr_nan_pasn_state state =
		MTK_WLAN_VENDOR_ATTR_NAN_PASN_STATE_INVALID;
	enum WIFI_VENDOR_NAN param_index =
		MTK_WLAN_VENDOR_ATTR_NAN_PASN_M1_IND_PARAM;

	prPasnframe = (struct NanPairingPASNMsg *)pvPasnFrame;
	if (!prPasnframe)
		return -EINVAL;
	...
	if (!wiphy || !wdev || !prAuthFrame)
		return -EINVAL;

	switch (prAuthFrame->u2AuthTransSeqNo) {
	case PASN_M1_AUTH_SEQ:
		state =
		MTK_WLAN_VENDOR_ATTR_NAN_PASN_RESPONDER_HANDLE_PASN_1;
		param_index =
		MTK_WLAN_VENDOR_ATTR_NAN_PASN_M1_IND_PARAM;
		break;
	case PASN_M2_AUTH_SEQ:
		state =
		MTK_WLAN_VENDOR_ATTR_NAN_PASN_INITIATOR_HANDLE_PASN_2;
		param_index =
		MTK_WLAN_VENDOR_ATTR_NAN_PASN_M2_IND_PARAM;
		break;
	case PASN_M3_AUTH_SEQ:
		state =
		MTK_WLAN_VENDOR_ATTR_NAN_PASN_RESPONDER_HANDLE_PASN_3;
		param_index =
		MTK_WLAN_VENDOR_ATTR_NAN_PASN_M3_IND_PARAM;
		break;
	default:
		DBGLOG(NAN, ERROR,
			"[pairing-pasn] invalid seq number :%u\n",
	...
	dataLen = NLMSG_HDRLEN
		+ sizeof(enum mtk_wlan_vendor_nl80211_attr_nan_pasn_state);
	dataLen += (NLMSG_HDRLEN + sizeof(struct NanPairingPASNMsg));
	...
	if (unlikely(nla_put(skb, param_index, sizeof(struct NanPairingPASNMsg),
			prPasnframe) < 0))
		goto nla_put_failure;

	DBGLOG(NAN, INFO,
		"[pairing-pasn] upload received PASN-M%u to suppplicant.\n",
		prAuthFrame->u2AuthTransSeqNo);
	cfg80211_vendor_event(skb, GFP_KERNEL);
	return TRUE;
...
void kalIndicateNanPairingRequest(struct ADAPTER *prAdapter,
		struct GLUE_INFO *prGlueInfo,
		struct SW_RFB *prSwRfb,
		uint8_t ucBssIdx) {
	struct _NanPairingRequestIndParams requestIndParams;
	struct WLAN_AUTH_FRAME *prAuthFrame;

	prAuthFrame = (struct WLAN_AUTH_FRAME *)prSwRfb->pvHeader;

	if (prAuthFrame) {
		/* initialized request indication params.*/
		memcpy(requestIndParams.peer_disc_mac_addr,
			prAuthFrame->aucSrcAddr, MAC_ADDR_LEN);
		requestIndParams.nan_pairing_request_type =
			NAN_PAIRING_SETUP_REQ_T;
		requestIndParams.enable_pairing_cache = 1;
		memset(&requestIndParams.nira, 0,
			sizeof(struct NanIdentityResolutionAttribute));

		/* parse PASN-M1 attrs*/
		nanPairingProcessAuthFrame(prAdapter, prSwRfb,
			&requestIndParams.nan_pairing_request_type,
			&requestIndParams.enable_pairing_cache,
			requestIndParams.nira.nonce,
			requestIndParams.nira.tag);

		DBGLOG(NAN, INFO,
			"[pairing-pasn] pump up Pairing request request_type=%u\n",
			requestIndParams.nan_pairing_request_type);
		DBGLOG(NAN, INFO,
			"nira.nonce=%llu, nira.tag=%llu\n",
			*(uint64_t *)requestIndParams.nira.nonce,
			*(uint64_t *)requestIndParams.nira.tag);
	}

	mtk_cfg80211_vendor_event_nan_pairing_indication(prAdapter,
		(uint8_t *) &requestIndParams, prSwRfb);
}
...
inline struct workqueue_struct *__kalWorkQueueInit(
	uint8_t *WqName, u_int8_t fgIsHighPri)
{
	struct workqueue_struct *prWorkQueue;

	prWorkQueue = alloc_workqueue(
		WqName, fgIsHighPri ? WQ_HIGHPRI | WQ_UNBOUND : 0, 1);
	if (!prWorkQueue)
 ...
 *
 */
static void kalWorkInit(struct GLUE_INFO *pr, enum ENUM_WORK eWork,
			uint8_t *WqName, GL_WORK_FUNC pfWorkFunc,
			u_int8_t fgIsHighPri)
{
	...
	prWork->prWorkQueue = __kalWorkQueueInit(WqName, fgIsHighPri);
}
	...
	const char *name, uint8_t ucIdx, PFN_CON_WORK_FUNC func)
{
	...
	prConWork->prWorkQueue = __kalWorkQueueInit(aucBuf, FALSE);
...
{
	kalWorkInit(pr, TX_FREE_MSDU_WORK, "TxFreeMsduWork",
		    kalTxFreeMsduWork, FALSE);
}
...
{
	kalWorkInit(pr, RX_NAPI_WORK, "RxNapiWork", kalRxNapiWork, FALSE);
}
...
{
	kalWorkInit(pr, RX_WORK, "RxWork", kalRxWork, TRUE);
...
{
	kalWorkInit(pr, TX_WORK, "TxWork", kalTxWork, FALSE);
}
...
{
	kalWorkInit(pr, HIF_TX_WORK, "HifTxWork", kalHifTxWork, TRUE);
}
...
/*----------------------------------------------------------------------------*/
/*!
 * @brief This function is send txpower limit
 *
 * @param[in] prGlueInfo : glue info
 *
 * @return status
 */
/*----------------------------------------------------------------------------*/
int32_t kalSetSarLimitByBitMap(struct GLUE_INFO *prGlueInfo)
{
	...
	if (prGlueInfo->prAdapter->rWifiVar.ucSarLmtCfgVer == 2) {
		...
		rStatus = rlmDomainSarGetRemapSubIdx(&u2SubIdx);
	...
	for (i = 0; i < SAR_NUM; i++) {
		...
		ucApplyAnt = rlmDomainGetSarApplyActAntStatus(i);

		if (rlmDomainIsSarEventConcurrent(&u4BitMap) &&
				ucApplyAnt != SAR_APPLY_DISABLE_ANT_ALL) {
			/* If SAR event concurrent, power limit will
			* take effect for all ant
			*/
			u2SubIdx = 1;
		} else {
			if (ucApplyAnt == SAR_APPLY_ENABLE_ANT_ALL)
				u2SubIdx = 1;
			else if (ucApplyAnt == SAR_APPLY_ENABLE_ONLY_WF0)
				u2SubIdx = 2;
			else if (ucApplyAnt == SAR_APPLY_ENABLE_ONLY_WF1)
				u2SubIdx = 3;
			else /* SAR_APPLY_DISABLE_ANT_ALL */
				u2SubIdx = 0;
		}
...
/*----------------------------------------------------------------------------*/
/*!
 * @brief This function is send txpower limit by ioctrl
 *
 * @param[in] prGlueInfo : glue info
 * @param[in] u2SubIdx : Sub index
 * @param[in] cName : Scenario name
 * @param[in] cNewSetting : New setting
 *
 * @return status
 */
/*----------------------------------------------------------------------------*/
int32_t kalSetPowerLimitByIoCtrl(
	struct GLUE_INFO *prGlueInfo,
	uint16_t u2SubIdx,
	char *cName,
	char *cNewSetting)
{
	...
	rPwrCtrlParam.fgApplied = (u2SubIdx == 0) ? FALSE : TRUE;
	rPwrCtrlParam.name = cName;
	rPwrCtrlParam.index = u2SubIdx;
	rPwrCtrlParam.newSetting = cNewSetting;
	...
	size_t szSrc, const char *pucFile, const size_t szLine)
{
	...
	if ((szDst > (16 * 1024 * 1024)) || (szDst < szSrc)) {
		DBGLOG(INIT, ERROR,
		"kalMemCpyS failed with invalid copy size at %s:%lu\n",
		pucFile, szLine);
		return -EINVAL;
	}
	...
	if ((pvDst == NULL) || (pvSrc == NULL)) {
		DBGLOG(INIT, ERROR,
		"kalMemCpyS failed with NULL pointer at %s:%lu\n",
		pucFile, szLine);
	return -EFAULT;
	}

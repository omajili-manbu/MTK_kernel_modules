...
#include "../../include/wpa_supp/src/ap/wpa_auth_i.h"
#include "../../include/wpa_supp/src/common/wpa_common.h"
#include "../../include/wpa_supp/src/rsn_supp/wpa_i.h"
#endif
		  ...
		  union iwreq_data *prIwReqData, char *pcExtra)
{
	...
	case ENUM_NAN_PUBLISH: {
		...
		DBGDUMP_HEX(NAN, INFO, "Publish SCID",
		...
		int i4TotalLen)
{
	...
	uint8_t aucLocalGtk[32] = {0};
	uint8_t aucPeerGtk[32] = {0};
	uint8_t aucLocalIgtk[32] = {0};
	uint8_t aucPeerIgtk[32] = {0};
	uint8_t aucLocalBigtk[32] = {0};
	uint8_t aucPeerBigtk[32] = {0};
	...
	for (i = 0; i < NAN_MAX_SUPPORT_NDL_NUM; i++) {
		...
		for (j = 0; j < NAN_MAX_SUPPORT_NDP_NUM; j++) {
			...
			if (prNDP->eCurrentNDPProtocolState != NDP_NORMAL_TR ||
				...
				prNDP->fgTxGtkRequired != TRUE)
				continue;
			kalMemCopy(aucLocalGtk,
				   prNDP->prInitiatorSecSmInfo->group->GTK[0],
				   16);
			LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
				"Local GTK: %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n",
				aucLocalGtk[0], aucLocalGtk[1],
				aucLocalGtk[2], aucLocalGtk[3],
				aucLocalGtk[4], aucLocalGtk[5],
				aucLocalGtk[6], aucLocalGtk[7],
				aucLocalGtk[8], aucLocalGtk[9],
				aucLocalGtk[10], aucLocalGtk[11],
				aucLocalGtk[12], aucLocalGtk[13],
				aucLocalGtk[14], aucLocalGtk[15]);
			kalMemCopy(aucLocalIgtk,
				   prNDP->prInitiatorSecSmInfo->group->IGTK[0],
				   16);
			LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
				"Local IGTK: %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n",
				aucLocalIgtk[0], aucLocalIgtk[1],
				aucLocalIgtk[2], aucLocalIgtk[3],
				aucLocalIgtk[4], aucLocalIgtk[5],
				aucLocalIgtk[6], aucLocalIgtk[7],
				aucLocalIgtk[8], aucLocalIgtk[9],
				aucLocalIgtk[10], aucLocalIgtk[11],
				aucLocalIgtk[12], aucLocalIgtk[13],
				aucLocalIgtk[14], aucLocalIgtk[15]);
			kalMemCopy(aucLocalBigtk,
				   prNDP->prInitiatorSecSmInfo->group->BIGTK[0],
				   16);
			LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
				"Local BIGTK: %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n",
				aucLocalBigtk[0], aucLocalBigtk[1],
				aucLocalBigtk[2], aucLocalBigtk[3],
				aucLocalBigtk[4], aucLocalBigtk[5],
				aucLocalBigtk[6], aucLocalBigtk[7],
				aucLocalBigtk[8], aucLocalBigtk[9],
				aucLocalBigtk[10], aucLocalBigtk[11],
				aucLocalBigtk[12], aucLocalBigtk[13],
				aucLocalBigtk[14], aucLocalBigtk[15]);

			if (prNDP->eCurrentNDPProtocolState != NDP_NORMAL_TR)
				continue;

			kalMemCopy(aucPeerGtk,
				   prNDP->prResponderSecSmInfo->gtk.gtk,
				   16);
			LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
				"Peer GTK: %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n",
				aucPeerGtk[0], aucPeerGtk[1],
				aucPeerGtk[2], aucPeerGtk[3],
				aucPeerGtk[4], aucPeerGtk[5],
				aucPeerGtk[6], aucPeerGtk[7],
				aucPeerGtk[8], aucPeerGtk[9],
				aucPeerGtk[10], aucPeerGtk[11],
				aucPeerGtk[12], aucPeerGtk[13],
				aucPeerGtk[14], aucPeerGtk[15]);

			kalMemCopy(aucPeerIgtk,
				   prNDP->prResponderSecSmInfo->igtk.igtk,
				   16);
			LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
				"Peer IGTK: %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n",
				aucPeerIgtk[0], aucPeerIgtk[1],
				aucPeerIgtk[2], aucPeerIgtk[3],
				aucPeerIgtk[4], aucPeerIgtk[5],
				aucPeerIgtk[6], aucPeerIgtk[7],
				aucPeerIgtk[8], aucPeerIgtk[9],
				aucPeerIgtk[10], aucPeerIgtk[11],
				aucPeerIgtk[12], aucPeerIgtk[13],
				aucPeerIgtk[14], aucPeerIgtk[15]);
			kalMemCopy(aucPeerBigtk,
				   prNDP->prResponderSecSmInfo->bigtk.bigtk,
				   16);
			LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
				"Peer BIGTK: %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n",
				aucPeerBigtk[0], aucPeerBigtk[1],
				aucPeerBigtk[2], aucPeerBigtk[3],
				aucPeerBigtk[4], aucPeerBigtk[5],
				aucPeerBigtk[6], aucPeerBigtk[7],
				aucPeerBigtk[8], aucPeerBigtk[9],
				aucPeerBigtk[10], aucPeerBigtk[11],
				aucPeerBigtk[12], aucPeerBigtk[13],
				aucPeerBigtk[14], aucPeerBigtk[15]);
#endif /* CFG_NAN_SUPPORT_R4_GROUP_ADDR_FRAME_PROT */
		}
	}
	...
	for (szTimeLineIdx = 0; szTimeLineIdx < NAN_TIMELINE_MGMT_SIZE;
		...
		for (i = 0; i < ARRAY_SIZE(prNanTimelineMgmt->arChnlList);
		     i++) {
			...
			LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
				"[%zu][%zu] MapId:%d, Raw:0x%x, Class:%u, Bw:%d, Commit Chnl:%u ",
				szTimeLineIdx, i,
		...
		for (i = 0; i < ARRAY_SIZE(prNanTimelineMgmt->arCondChnlList);
		     i++) {
	...
	/* ULW */
	for (i = 0; i < ARRAY_SIZE(prNanScheduler->arNanLocalUlw); i++) {
		struct _NAN_ULW_RECORD_T *prUlw;

		prUlw = &prNanScheduler->arNanLocalUlw[i];
		if (prUlw->ucCountDown != 255)
			continue;

		LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
		       "ULW[%zu] Start=%u, Period=%u, Duration=%u, OC=%u, Ch=%u\n",
		       i, prUlw->u4StartTime, prUlw->u4Period,
		       prUlw->u4Duration,
		       prUlw->rBandChnlInfo.u4OperatingClass,
		       prUlw->rBandChnlInfo.u4PrimaryChnl);
	}
...
#if CFG_SUPPORT_NAN_R4_PAIRING
int
priv_driver_set_sendnik(struct net_device *prNetDev, char *pcCommand,
		int i4TotalLen) {
	struct ADAPTER *prAdapter = NULL;
	struct GLUE_INFO *prGlueInfo = NULL;
	...
	if (nanGetFeatureIsSigma(prAdapter)) {
		sigma_nik_exchange_run = true;
		DBGLOG(NAN, INFO,
			"[pairing-keyEx] sigma trigger nik exchange\n");
	}
...
/**
 * The string in pcCommand could be these types:
 * iwpriv wlan0 driver "nan_setps enterApNAN"
 * iwpriv wlan0 driver "nan_setps enterFrPs"
 * iwpriv wlan0 driver "nan_setps terminate"
 */
int priv_driver_nan_set_powersave(struct net_device *prNetDev,
				  char *pcCommand, int i4TotalLen)
{
	struct GLUE_INFO *prGlueInfo;
	struct ADAPTER *prAdapter;
	...
	return nan_ext_set_powersave(prAdapter, pcCommand, i4TotalLen);
}
#endif
...
int priv_driver_set_country_power_control(struct net_device *prNetDev,
				  char *pcCommand, int i4TotalLen)
{
	int32_t i4BytesWritten = 0;
	struct GLUE_INFO *prGlueInfo = NULL;
	int32_t i4Argc = 0;
	int8_t *apcArgv[WLAN_CFG_ARGV_MAX] = { 0 };
	uint32_t u4Ret = 0, u4Action = 0, u4Cfg = 0;
	uint32_t u4SetInfoLen = 0;
	uint32_t rWlanStatus = WLAN_STATUS_SUCCESS;
	...
	/* example: set_country_pwr_ctrl 0 1 (set default /config table) */
	if (i4Argc >= 3) {
		u4Ret = kalkStrtou32(apcArgv[1], 0, &u4Action);
		if (u4Ret) {
		...
		u4Ret = kalkStrtou32(apcArgv[2], 0, &u4Cfg);
		if (u4Ret) {
...
int priv_driver_get_country_power_control(struct net_device *prNetDev,
				  char *pcCommand, int i4TotalLen)
{
	struct ADAPTER *prAdapter = NULL;
	int32_t i4BytesWritten = 0;
	...
	int32_t i4Argc = 0;
	int8_t *apcArgv[WLAN_CFG_ARGV_MAX] = { 0 };
	uint32_t u4Action = 0, u4Ret = 0, u4Cfg = 0;
	...
	/* example: get_country_pwr_ctrl 0 (get default /config table) */
	if (i4Argc >= 2) {
		u4Ret = kalkStrtou32(apcArgv[1], 0, &u4Action);
		if (u4Ret) {
			DBGLOG(REQ, ERROR, "parse apcArgv1 error u4Ret=%d\n",
		...
		if (u4Action == 0) {
			u4Cfg = prAdapter->rWifiVar.ucCountryPwrLimitCtrl;
			i4BytesWritten += kalSnprintf(
				pcCommand + i4BytesWritten,
				i4TotalLen - i4BytesWritten,
				"CountryPwrCtrl 0x%x\n",
			...
			i4BytesWritten += kalSnprintf(
				pcCommand + i4BytesWritten,
				i4TotalLen - i4BytesWritten,
				"- BIT(0) high will disable default table\n");

			i4BytesWritten += kalSnprintf(
				pcCommand + i4BytesWritten,
				i4TotalLen - i4BytesWritten,
				"- BIT(1) high will disable config table\n");
		}
	}
...
int priv_driver_dfs_zerowait_stop(struct net_device *prNetDev,
				  char *pcCommand, int i4TotalLen)
{
	int32_t i4Argc = 0;
	int8_t *apcArgv[WLAN_CFG_ARGV_MAX] = {0};
	...
	p2pFuncStopZerowait(prGlueInfo->prAdapter, ucBssIdx);
	...
	int i4TotalLen)
{
	...
	if (this_char) {
		...
		if (kalStrnCmp(this_char, "OpenAdapter", 11) == 0 ||
		    kalStrnCmp(this_char, "CloseAdapter", 11) == 0)
			fgSkipLock = TRUE;

		if (fgSkipLock == FALSE)
			mutex_lock(&g_rHqaLock);
		ret = mt_agent_hqa_cmd_handler(&prGlueInfo->rService,
		&local_hqa);
		if (fgSkipLock == FALSE)
			mutex_unlock(&g_rHqaLock);
	}
...
#endif
	) && (p2pFuncIsFixWfd1NssBw40(prAdapter, bss))) {
		ucMaxBw =
		((ucMaxBw > MAX_BW_40MHZ) ?
		MAX_BW_40MHZ : ucMaxBw);
	}
#endif
...
int priv_driver_get_pmic_uvlo(struct net_device *prNetDev,
			  char *pcCommand, int i4TotalLen)
{
	int32_t i4BytesWritten = 0;
	int32_t i4Argc = 0;
	...
	if (i4Argc >= 1) {
		kalSnprintf(pcCommand, i4TotalLen,
					 CMD_GET_CHIP " pmic 0x00 0x26");

		/* Get chip config from FW */
		i4BytesWritten = priv_driver_get_chip_config(prNetDev, pcCommand,
						    i4TotalLen);
...
int priv_driver_set_pmic_uvlo(struct net_device *prNetDev,
			  char *pcCommand, int i4TotalLen)
{
	uint32_t i4BytesWritten = 0;
	int32_t i4Argc = 0;
	...
	if (i4Argc >= 2) {
		/* ucMode = kalStrtoul(apcArgv[1], NULL, 0); */
		u4Ret = kalkStrtou32(apcArgv[1], 0, &u4Data);
		if (u4Ret)
			DBGLOG(REQ, ERROR, "parse u4Data error u4Ret=%d\n",
			       u4Ret);

		kalSnprintf(pcCommand, i4TotalLen,
			CMD_SET_CHIP " pmic 0x00 0x26 0x%08x", u4Data);

		DBGLOG(REQ, DEBUG, "%s: command is %s\n", __func__,
		       pcCommand);

 ...
 */
/*----------------------------------------------------------------------------*/
static irqreturn_t mtk_axi_int_top_handler(int irq, void *dev_instance)
{
...
static irqreturn_t mtk_axi_int_thread_handler(int irq, void *dev_instance)
{
	struct GLUE_INFO *prGlueInfo = (struct GLUE_INFO *)dev_instance;
...
int32_t glBusSetIrq(void *pvData, void *pfnIsr, void *pvCookie)
{
	...
	ret = request_threaded_irq(prHifInfo->u4IrqId,
			mtk_axi_int_top_handler,
			mtk_axi_int_thread_handler,
			IRQF_SHARED,
			prNetDevice->name,
			prGlueInfo);
	if (ret != 0) {

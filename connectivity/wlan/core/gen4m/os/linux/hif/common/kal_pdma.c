...
static bool kalIsPdmaScatterAllowed(struct GLUE_INFO *pr)
{
			     ...
			     uint16_t u2Port)
{
	...
	if (!kalIsPdmaScatterAllowed(prGlueInfo) && prRxRing->fgEnEmiDidx &&
		    ...
		    struct SW_RFB *prSwRfb)
{
	...
	if (pRxD->LastSec0 == 0 || prRxRing->fgRxSegPkt) {
		...
		/* Rx segmented packet */
		if (!kalIsPdmaScatterAllowed(prGlueInfo)) {
			DBGLOG(HAL, WARN,
				"Skip Segmented Data, Port[%u] SDL0[%u] LS0[%u]\n",
				u2Port,
				pRxD->SDLen0, pRxD->LastSec0);
			fgDebugSegment = TRUE;
		}
...
#if (CFG_SUPPORT_PDMA_SCATTER == 1)
	if (kalIsPdmaScatterAllowed(prGlueInfo) && fgRet == FALSE) {
...
int wf_ioremap_write_check(phys_addr_t addr, unsigned int val,
	unsigned int mask, unsigned int *retReadVal)
{
	/* Set a magic number, in case read_val = val = all zero */
	*retReadVal = 0xaaaeee77;

	if (wf_ioremap_read(addr, retReadVal) != 0)
		return -1;

	if ((*retReadVal & mask) != (val & mask)) {
		DBGLOG(INIT, WARN,
			"Check addr:0x%08x mask[0x%08x] read[0x%08x] != write[0x%08x]\n",
		       (uint32_t)addr, mask, *retReadVal, val);
		return -1;
	}

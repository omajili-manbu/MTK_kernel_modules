	...
	uint32_t u4LongestPending, uint8_t ucBssIndex)
{
	...
	if (prAdapter->u4SameTokenCnt > prWifiVar->u4SameTokenThr) {
		kalSendAeeWarning(aee_str,
			"Bss_Idx[%u] Tx timeout same token > %d , idle slot %d\n",
...
static bool halIsTxTimeout(struct ADAPTER *prAdapter, uint32_t *u4Token)
{
	...
	if (u4TimeoutSerTime == NIC_MSDU_REPORT_DISABLE_SER_TIME) {
		...
		if (CHECK_FOR_TIMEOUT64(u8Now,
			...
			DBGLOG(HAL, ERROR,
			       "Timeout > %ds, lastMsduRpt @ %lld, SameTokCnt[%u] trigger SER\n",
				u4TimeoutSerTime,
...
void halCheckWfdmaStallForWB(struct ADAPTER *prAdapter)
{
	...
	u_int8_t fgEnableCfgDump = FALSE;
#endif
	...
	for (i = 0; i < NUM_OF_RX_RING; i++) {
		...
		if (u4RxCnt > 0 && !halIsWfdmaRxCidxChanged(prAdapter, i)) {
			...
			if (prRxRing->u4CidxErrCnt ==
			    prWifiVar->u4WfdmaRxTimeoutRecoveryCnt - 1)
				fgEnableCfgDump = TRUE;
#endif
		} else {
...
#if (CFG_MTK_WIFI_PCIE_CONFIG_SPACE_ACCESS_DBG == 1)
	if (fgEnableCfgDump)
		halCfgDumpVoteOn(prAdapter, PCIE_CFG_DUMP_USER_WFDMA_STALL);
	else
		halCfgDumpVoteOff(prAdapter, PCIE_CFG_DUMP_USER_WFDMA_STALL);
#endif
}

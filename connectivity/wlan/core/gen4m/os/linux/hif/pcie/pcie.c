...
void mtk_pci_disable_irq(struct GLUE_INFO *prGlueInfo)
{
	...
	for (i = 0; i < prMsiInfo->u4MsiNum; i++) {
		...
		mtk_pci_msi_disable_irq(prGlueInfo, prMsiLayout->irq_num, i);
		KAL_SET_BIT(i, prMsiInfo->ulEnBits);
	}
}
	...
	pci_channel_state_t state)
{
	...
	/* bit[10]: PCIe SDES/link down, */
	/* always block PCIe access once detected */
	if (dump & BIT(10)) {
		DBGLOG(HAL, WARN, "PCIE SDES/link down (%u)\n", dump);
#if CFG_MTK_WIFI_PCIE_SUPPORT
		mtk_pcie_disable_data_trans(0);
	...
	if (fgIsPcieDataTransDisabled == FALSE &&
		...
		DBGLOG(HAL, WARN, "PCIE link down (%u)\n", dump);
		/* block PCIe access */
#if CFG_MTK_WIFI_PCIE_SUPPORT
		mtk_pcie_disable_data_trans(0);
		fgIsPcieDataTransDisabled = TRUE;
...
static int mtk_pci_suspend(struct pci_dev *pdev, pm_message_t state)
{
	...
	while (wait < 500) {
		 ...
		 * semaphore is downed to block any DRV_OWN until resumed.
		 */
		if (KAL_HIF_OWN_TRYLOCK(prAdapter)) {
			if ((prAdapter->u4PwrCtrlBlockCnt == 1)) {
				...
				|| prHifInfo->eCurPcieState == PCIE_STATE_L1_2
					...
					DBGLOG(HAL, ERROR,
						"Cannot enter FW own (fgIsFwOwn=%u)\n",
						prAdapter->fgIsFwOwn);
					ret = -EAGAIN;
				...
				/* Disable cfg dump during suspended */
				if (halPcieIsPcieProbed() == TRUE)
					halCfgDumpVoteOff(prAdapter,
						PCIE_CFG_DUMP_USER_DRV_OWN);
...
{
	uint16_t u2Reg = 0;
	uint16_t u2RegAfterR = 0;
	uint16_t u2RegBeforeW = 0;
	...
	DBGLOG(INIT, INFO, "afterR:0x%08X, beforeW:0x%08X, afterW:0x%08X\n",
		u2RegAfterR, u2RegBeforeW, u2Reg);
}
...
uint32_t glReadPcieCfgSpace(int offset, uint32_t *value)
{
	...
	struct GLUE_INFO *prGlueInfo = NULL;
	struct ADAPTER *prAdapter = NULL;
	struct device *prDev = &g_prDev->dev;
...
#if (CFG_MTK_WIFI_PCIE_CONFIG_SPACE_ACCESS_DBG == 1)
void halCfgDumpVoteOn(struct ADAPTER *prAdapter,
	enum PCIE_CFG_DUMP_USER_T eUser)
{
	struct GL_HIF_INFO *prHifInfo;
	struct GLUE_INFO *prGlueInfo;
	unsigned long flags;
	...
	spin_lock_irqsave(&prGlueInfo->rSpinLock[SPIN_LOCK_PCIE_CFG_DUMP],
			  flags);

	if (prHifInfo->u4CfgDumpVoteState == 0) {
		mtk_pcie_enable_cfg_dump(0);
		prHifInfo->fgEnablePcieCfgDump = TRUE;
		DBGLOG(HAL, TRACE, "Enable PCIE CFG Dump (User=%d)\n", eUser);
	}

	prHifInfo->u4CfgDumpVoteState |= BIT(eUser);

	spin_unlock_irqrestore(
		&prGlueInfo->rSpinLock[SPIN_LOCK_PCIE_CFG_DUMP], flags);
}

void halCfgDumpVoteOff(struct ADAPTER *prAdapter,
	enum PCIE_CFG_DUMP_USER_T eUser)
{
	struct GL_HIF_INFO *prHifInfo;
	struct GLUE_INFO *prGlueInfo;
	...
	if (prHifInfo->u4CfgDumpVoteState == 0 &&
	    prHifInfo->fgEnablePcieCfgDump) {
		mtk_pcie_disable_cfg_dump(0);
		prHifInfo->fgEnablePcieCfgDump = FALSE;
		DBGLOG(HAL, TRACE, "Disable PCIE CFG Dump (Last User=%d)\n",
			eUser);
	}

...
#if CFG_SUPPORT_NAN
struct CMD_VALIDATE_POLICY set_faw_policy[COMMON_CMD_SET_ARG_NUM(4)] = {
	[COMMON_CMD_ATTR_IDX(1)] = {.type = NLA_U8, .min = 0, .max = U8_MAX},
	[COMMON_CMD_ATTR_IDX(2)] = {.type = NLA_U8, .min = 0, .max = U8_MAX},
	[COMMON_CMD_ATTR_IDX(3)] = {.type = NLA_U32, .min = 0, .max = U32_MAX}
};
...
struct PRIV_CMD_HANDLER priv_cmd_handlers_customer[] = {
	...
	{
		.pcCmdStr  = CMD_DFS_ZEROWAIT_STOP,
		.pfHandler = priv_driver_dfs_zerowait_stop,
		.argPolicy = VERIFY_EXACT_ARG_NUM,
		.ucArgNum  = COMMON_CMD_SET_ARG_NUM(1),
		.policy    = NULL,
		.u4PolicySize = 0
	...
	{
		.pcCmdStr  = CMD_SET_PWR_COUNTRY_CTRL,
		.pfHandler = priv_driver_set_country_power_control,
		.argPolicy = VERIFY_MIN_ARG_NUM,
		.ucArgNum  = COMMON_CMD_SET_ARG_NUM(3),
		.policy    = u32_policy,
		.u4PolicySize = 0
	...
	{
		.pcCmdStr  = CMD_GET_PWR_COUNTRY_CTRL,
		.pfHandler = priv_driver_get_country_power_control,
		.argPolicy = VERIFY_MIN_ARG_NUM,
		.ucArgNum  = COMMON_CMD_GET_ARG_NUM(2),
		.policy    = u32_policy,
		.u4PolicySize = 0
	...
	{
		.pcCmdStr  = CMD_GET_PMIC_UVLO,
		.pfHandler = priv_driver_get_pmic_uvlo,
		.argPolicy = VERIFY_EXACT_ARG_NUM,
		.ucArgNum  = COMMON_CMD_GET_ARG_NUM(1),
		.policy    = NULL,
		.u4PolicySize = 0
	...
	{
		.pcCmdStr  = CMD_SET_PMIC_UVLO,
		.pfHandler = priv_driver_set_pmic_uvlo,
		.argPolicy = VERIFY_EXACT_ARG_NUM,
		.ucArgNum  = COMMON_CMD_SET_ARG_NUM(2),
		.policy    = NULL,
		.u4PolicySize = 0
...
struct PRIV_CMD_HANDLER priv_cmd_handlers_debug[] = {
	...
	{
		.pcCmdStr  = CMD_NANSETPS,
		.pfHandler = priv_driver_nan_set_powersave,
		.argPolicy = VERIFY_MIN_ARG_NUM,
		.ucArgNum  = COMMON_CMD_GET_ARG_NUM(2),
		.policy    = NULL,
		.u4PolicySize = 0
	...
	uint32_t u4PolicySize)
{
	...
	if ((argPolicy == VERIFY_EXACT_ARG_NUM && ucArgNum != i4Argc) ||
		...
		DBGLOG(REQ, ERROR,
		       "Arg num mismatch, argPolicy=%d, ucArgNum=%d, i4Argc=%d",
		       argPolicy, ucArgNum, i4Argc);
		return ret;
	}

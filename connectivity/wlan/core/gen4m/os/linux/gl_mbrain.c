...
static uint32_t mbrTRxPerfCalculateTput(struct ADAPTER *prAdapter,
	 uint64_t *throughput)
{
	static struct TX_RX_STATS_RECORD rLastStats;
...
void mbrTRxPerfEnqueue(struct ADAPTER *prAdapter)
{
	...
	static uint32_t u8LastTxTotalCount, u8LastTxFailCount;
	static uint32_t u8LastIdleSlotCount;
	...
	period = kalGetTimeTick() - u4LastTRxPerfEnQTime;
	if (period <= 0)
		return;
	...
	mbrTRxPerfCalculateTput(prAdapter, &prTRxPerfInfo->tput);

	if (prLinkQualityInfo->u8IdleSlotCount < u8LastIdleSlotCount) {
		prTRxPerfInfo->idle_slot = 0;
	} else {
		prTRxPerfInfo->idle_slot =
			(prLinkQualityInfo->u8IdleSlotCount -
				u8LastIdleSlotCount) * MSEC_PER_SEC;
		 do_div(prTRxPerfInfo->idle_slot, period);
	}
	...
	DBGLOG(REQ, LOUD,
		"BssIdx[%u] Widx[%u] Tokenid[%u] TxDuration[%u] OPmode[%u] RxDrop[%u,%u,%u] NapiFull[%u] tput[%lu(%lu.%lu)] idle_slot[%lu] per[%u/%u] rate[%u/%u] rssi[%d]\n",
		prTRxPerfInfo->bss_index, prTRxPerfInfo->wlan_index,
		prTRxPerfInfo->token_id, prTRxPerfInfo->timeout_duration,
		prTRxPerfInfo->operation_mode,
		prTRxPerfInfo->rx_drop_total, prTRxPerfInfo->rx_drop_reorder,
		prTRxPerfInfo->rx_drop_sanity,
		prTRxPerfInfo->rx_napi_full,
		prTRxPerfInfo->tput,
		(unsigned long long) (prTRxPerfInfo->tput >> 20),
		(unsigned long long) ((prTRxPerfInfo->tput >> 10) & BITS(0, 9)),
		prTRxPerfInfo->idle_slot,
		prTRxPerfInfo->tx_per, prTRxPerfInfo->rx_per,
		prTRxPerfInfo->tx_rate, prTRxPerfInfo->rx_rate,
		prTRxPerfInfo->rx_rssi);
	...
	u8LastTxTotalCount = prLinkQualityInfo->u8TxTotalCount;
	u8LastTxFailCount = prLinkQualityInfo->u8TxFailCount;
	u8LastIdleSlotCount = prLinkQualityInfo->u8IdleSlotCount;
}
...
void mbrWifiTRxPerfFlushQueue(struct ADAPTER *prAdapter)
{
	struct MBRAIN_TRXPERF_ENTRY *prTRxPerfEntry = NULL;

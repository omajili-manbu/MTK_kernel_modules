...
struct _NanPairingNdlDisconnectIndMsg {
	struct _NanMsgHeader fwHeader;
	struct _NanPairingNdlDisconnectIndParams pairingNdlDisconnectIndParams;
	u8 ptlv[];
} PACKED;
...
enum NanMsgId {
	...
	NAN_MSG_ID_PAIRING_NDLDISCONNECT    = 73,
...
int mtk_cfg80211_vendor_nan_pasn_setkey(struct wiphy *wiphy,
			struct wireless_dev *wdev,
			const void *data, int data_len);
int mtk_cfg80211_vendor_nan_pasn_confirm(struct wiphy *wiphy,
			struct wireless_dev *wdev,
			const void *data, int data_len);
#endif

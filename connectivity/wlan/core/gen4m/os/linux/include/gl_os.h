...
#define HIF_FLAG_DUMP_PCIE_MSI		BIT(4)
#define HIF_FLAG_DUMP_PCIE_MSI_BIT	(4)
...
struct GLUE_INFO {
	...
	struct GL_WON_DEV_INFO *prWONDevInfo;
	struct GL_WON_INFO *prWONInfo[KAL_WON_NUM];
#endif
#if CFG_ENABLE_WIFI_DIRECT
...
#endif
	KAL_WAKE_LOCK_T *rRxIndPktsWakeLock;
	KAL_WAKE_LOCK_T *rRxIndOnePktWakeLock;
	KAL_WAKE_LOCK_T *rIoctlWakeLock;
	KAL_WAKE_LOCK_T *rMgmtTxEvtWakeLock;
	KAL_WAKE_LOCK_T *rTxEvtWakeLock;
	KAL_WAKE_LOCK_T *rFwOwnWakeLock;
	KAL_WAKE_LOCK_T *rTxEvtToRxWakeLock;
	KAL_WAKE_LOCK_T *rTxCmdEvtWakeLock;
...
#endif
#if (CFG_WIFI_PCIE_L2_MOBILE_ONLY == 1)
	KAL_WAKE_LOCK_T *rOobWakeLock;
#endif
#if CFG_ENABLE_WAKE_LOCK && CFG_SUPPORT_WONDERTAP
	KAL_WAKE_LOCK_T *prWonWakeLock;
#endif
	...
	struct wireless_dev *prWonWdev[KAL_WON_NUM];
	struct wireless_dev *prWonRoleWdev[KAL_WON_NUM];
	struct net_device *prWonDev[KAL_WON_NUM];
	...
	unsigned long ulIsConnChanging;
	unsigned long ulWakeUpConnSrc;
	uint32_t u4WakeUpConnCnt;
...
#if (defined(CFG_MTK_CCCI_IDC_SUPPORT) && \
		(CFG_MTK_CCCI_IDC_SUPPORT == 1)) && \
		((defined(CFG_MTK_CCCI_SUPPORT) && \
		(CFG_MTK_CCCI_SUPPORT == 1)) || \
		(defined(CFG_MTK_CCCI_EAP_IDC_SUPPORT) && \
		(CFG_MTK_CCCI_EAP_IDC_SUPPORT == 1)))
	uint32_t u4IdcWfRdyFlag;	/* IDC check if WF is ready */
#endif
};
...
/*
 * Safely decrement the atomic ref count if > 0, otherwise do nothing.
 * Returns the new value if decremented, or negative if not decremented.
 */
#define GLUE_TRY_DEC_REF_CNT(_refCount) \
	GLUE_FENCE_ATOMIC(atomic_dec_if_positive, ((atomic_t *)&(_refCount)))

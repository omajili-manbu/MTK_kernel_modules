...
enum mtk_wlan_ndp_sub_cmd {
	...
	/* Command to set power save mode by NDP */
	MTK_WLAN_VENDOR_ATTR_NDP_POWERSAVE_MODE = 13,
...
enum mtk_wlan_vendor_attr_ndp_params {
	...
	/* Unsigned 32-bit value in order to choose NDP NDI */
	MTK_WLAN_VENDOR_ATTR_NDP_DATA_INTERFACE = 30,
	/* Initiator MAC address. An array of 6 Unsigned int8 */
	MTK_WLAN_VENDOR_ATTR_NDP_INITIATOR_MAC_ADDRESS = 31,
	/* Multicast address. An array of 6 Unsigned int8 */
	MTK_WLAN_VENDOR_ATTR_NDP_MULTICAST_ADDR = 32,
	MTK_WLAN_VENDOR_ATTR_NDP_UPDATE_CONFIG = 33,
	MTK_WLAN_VENDOR_ATTR_NDP_LATENCY = 34,
	MTK_WLAN_VENDOR_ATTR_NDP_TPUT = 35,
	/* keep last */

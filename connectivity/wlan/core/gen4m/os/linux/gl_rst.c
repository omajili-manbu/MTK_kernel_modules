...
#define UPGRATE_TO_L0_PATTERN " - Upgrade to L0"
	...
	if (kalIsResetting() || kalIsResetOnEnd()) {
		DBGLOG(INIT, DEBUG, "already in reset [%u:%u]\n",
...
#else
	if (u4RstFlag & RST_FLAG_DO_CORE_DUMP)
		wifi_coredump_set_status(ENUM_COREDUMP_TRIGGERED);
	else
		wifi_coredump_set_status(ENUM_COREDUMP_NOT_TRIGGER);
	...
	enum _ENUM_CHIP_RESET_REASON_TYPE_T resetReason)
{
	...
	if (g_IsWfsysBusNoAck == TRUE) {
		if (prAdapter) {
			...
			if (prGlueInfo && prGlueInfo->u4ReadyFlag) {
				fgIsDrvTriggerWholeChipReset = TRUE;
				glSetRstReasonString("fw trigger L0 reset");
				glResetWholeChipResetTrigger(g_reason);
				glResetCleanResetFlag();
				return;
			}
	...
	if (g_SubsysRstCnt > 3) {
		 ...
		 */
			...
			if (wifi_coredump_get_status() ==
			    ENUM_COREDUMP_TRIGGERED)
				wifi_coredump_start(
					coredump_source,
		...
		if (wifi_coredump_get_status() == ENUM_COREDUMP_TRIGGERED)
		...
		if (prGlueInfo && prGlueInfo->u4ReadyFlag) {
			...
			if (prBusInfo->wakeUpConninfra)
				prBusInfo->wakeUpConninfra(prGlueInfo,
					WAKEUP_L04_RESET, TRUE);
#endif
			glResetMsgHandler(ENUM_RST_MSG_L04_START);
			glResetMsgHandler(ENUM_RST_MSG_L04_END);
#if (CFG_SUPPORT_CONNINFRA == 1)
			if (prBusInfo->releaseConninfraWakeUp)
				prBusInfo->releaseConninfraWakeUp(prGlueInfo,
					WAKEUP_L04_RESET);
#endif
		} else {
			glResetCleanResetFlag();

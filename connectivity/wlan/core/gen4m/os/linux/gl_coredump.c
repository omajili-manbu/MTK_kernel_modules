...
int wifi_coredump_post_start(void)
{
	...
	ctx->eCoredumpStatus = ENUM_COREDUMP_ONGOING;
...
deinit:
	__coredump_deinit(ctx);
exit:
	ctx->eCoredumpStatus = ENUM_COREDUMP_NOT_TRIGGER;
	return ret;
}
#endif
...
void wifi_coredump_set_status(enum ENUM_COREDUMP_STATUS eStatus)
{
	...
	if (eStatus >= ENUM_COREDUMP_STATUS_NUM) {
		DBGLOG(INIT, WARN, "Invalid status[%d]\n", eStatus);
		return;
	}

	if (ctx->eCoredumpStatus != eStatus) {
		ctx->eCoredumpStatus = eStatus;
		DBGLOG(INIT, INFO, "Update coredump status[%u]\n", eStatus);
	} else
		DBGLOG(INIT, TRACE, "Update coredump status[%u]\n", eStatus);
}
...
u_int8_t is_wifi_coredump_processing(void)
{
	...
	return (ctx->eCoredumpStatus == ENUM_COREDUMP_ONGOING);
}

...
/* allocate netdev */
	...
	/* Customer request not check GTK bit by default */
	/* due to R3 & R4 compatibility issue. */
	nanDevEnableGtkReqChecker(prAdapter, (uint8_t)eRole, FALSE);
...

{
	...
	prNetDev = g_aprNanRoleWdev[ucIdx]->netdev;
	DBGLOG(NAN, DEBUG,
	       "Unregister g_aprNanRoleWdev[%d], freeing %p, netdev=%p",
	       ucIdx, g_aprNanRoleWdev[ucIdx], prNetDev);

	kfree(g_aprNanRoleWdev[ucIdx]);
	g_aprNanRoleWdev[ucIdx] = NULL;
	if (prNetDev)
		prNetDev->ieee80211_ptr = NULL;
	return TRUE;
}
void
...
#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_DATA == 1)
		prEth = (struct ETH_FRAME *)prSkb->data;

		/* Check TX MC wtbl */
		ucWlanIndex = (uint8_t)nanFindMultiCastWidx(
				prGlueInfo->prAdapter,
				prEth->aucDestAddr,
				prEth->aucSrcAddr);

		/* If no MC TX widx, drop it*/
		if (ucWlanIndex == WTBL_RESERVED_ENTRY) {
			DBGLOG(NAN, WARN,
				"[MCDATA] wtblIdx not found, Dest = "
				MACSTR ", Src = " MACSTR "\n",
...
#else /* (CFG_NAN_SUPPORT_R4_GROUP_ADDR_DATA == 1) */

...
#define CFG_GET_RF_CAL_RESULT 0

#if (CFG_MTK_ANDROID_WMT) || (CFG_GET_RF_CAL_RESULT)
#define PROC_RF_CAL_RESULT			"rf_cal"
#endif
...
#if (CFG_MTK_ANDROID_WMT) || (CFG_GET_RF_CAL_RESULT)
static ssize_t procRfCalRead(struct file *filp, char __user *buf,
	size_t count, loff_t *f_pos)
{
	struct ADAPTER *prAdapter = NULL;
	struct GLUE_INFO *prGlueInfo = NULL;
	uint32_t u4CopySize = 0, i = 0, u4TotalSize = 0;
	int32_t i4Ret = 0;
	size_t remain = 0;
	uint8_t *prRfEmiCalData = NULL;
...
#if defined(_HIF_PCIE) || defined(_HIF_AXI)
	for (i = 0; i < prChipInfo->rsvMemWiFiMiscSize; i++) {
		if (prChipInfo->rsvMemWiFiMisc[i].block_name ==
			WIFI_MISC_MEM_BLOCK_PRECAL)
			u4TotalSize = prChipInfo->rsvMemWiFiMisc[i].size;
	}

	if (u4TotalSize == 0) {
		DBGLOG(RFTEST, INFO, "[RFCAL] RF CAL EMI not register\n");
		return 0;
	}
	...
	if (prMemOps && prMemOps->getWifiMiscRsvEmi) {
		prMem = prMemOps->getWifiMiscRsvEmi(prChipInfo,
			WIFI_MISC_MEM_BLOCK_PRECAL);
		if (prMem) {
			prRfEmiCalData = (uint8_t *)prMem->va;
		} else {
			DBGLOG(RFTEST, INFO,
				"[RFCAL] Failed to obtain prMem\n");
			return 0;
		}
	}
#endif

	if (prRfEmiCalData == NULL) {
		DBGLOG(RFTEST, INFO, "[RFCAL] RF CAL EMI is NULL\n");
		return 0;
	}

	/* if *f_ops>u4TotalSize, we should return 0 to make cat command exit */
	if (*f_pos >= u4TotalSize || buf == NULL)
		return 0;

	remain = u4TotalSize - *f_pos;
	u4CopySize = PROC_MAX_BUF_SIZE;

	if (u4CopySize > count)
		u4CopySize = count;

	if (u4CopySize > remain)
		u4CopySize = remain;

	if (copy_to_user(buf, prRfEmiCalData + *f_pos, u4CopySize)) {
		DBGLOG(REQ, WARN, "copy to user failed\n");
		i4Ret = -EFAULT;
	}
...
#if KERNEL_VERSION(5, 6, 0) <= CFG80211_VERSION_CODE
static const struct proc_ops rfcal_ops = {
	.proc_read = procRfCalRead,
};
#else
static const struct file_operations rfcal_ops = {
	.owner = THIS_MODULE,
	.read = procRfCalRead,
};
#endif /* KERNEL_VERSION(5, 6, 0) <= CFG80211_VERSION_CODE */
#endif /* (CFG_MTK_ANDROID_WMT) || (CFG_GET_RF_CAL_RESULT) */
...
int32_t procRemoveProcfs(struct GLUE_INFO *prGlueInfo)
{
...
#if (CFG_MTK_ANDROID_WMT) || (CFG_GET_RF_CAL_RESULT)
	remove_proc_entry(PROC_RF_CAL_RESULT, prProcRoot);
#endif /* (CFG_MTK_ANDROID_WMT) || (CFG_GET_RF_CAL_RESULT) */
	...
	procRemoveFsWondertap(prGlueInfo, prProcRoot);
#endif
...
#if (CFG_MTK_ANDROID_WMT) || (CFG_GET_RF_CAL_RESULT)
	prEntry = PROC_CREATE(PROC_RF_CAL_RESULT, 0664, prProcRoot, &rfcal_ops);
	if (prEntry == NULL) {
		DBGLOG(INIT, ERROR,
			"Unable to create /proc entry for driver rfcal\n\r");
		return -1;
	}
#endif /* (CFG_MTK_ANDROID_WMT) || (CFG_GET_RF_CAL_RESULT) */
...
#if CFG_SUPPORT_WONDERTAP
	procCreateFsWondertap(prGlueInfo, prProcRoot);
#endif

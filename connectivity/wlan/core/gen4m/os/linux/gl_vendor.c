...
#define IS_INDOOR_CONCURRENT_ALLOWED(ad, filter) \
	(IS_STA_INDOOR_CHANNEL_ENABLED(ad) \
	&& (filter & CONCURRENCY))

#define IS_DFS_CONCURRENT_ALLOWED(ad, filter) \
	(IS_STA_DFS_CHANNEL_ENABLED(ad) \
	&& (filter & CONCURRENCY))
...
const struct nla_policy nla_parse_wifi_attribute[
	...
	[WIFI_ATTRIBUTE_SET_FAST_ACS] = {.type = NLA_U8},
	...
	nla_for_each_nested(attr, attrs[RTT_ATTRIBUTE_TARGET_INFO], tmp) {
		...
		if (nanIsOn(prGlueInfo->prAdapter) &&
			...
			enum ENUM_BAND eNanRangingBand =
				prAdapter->rWifiVar.eNanRangingBand;
...
// MIUI ADD: WIFI_BugFixUpstream
int wlan_cfg80211_set_fast_acs(struct wiphy *wiphy, struct wireless_dev *wdev,
				const void *data, int data_len)
{
	if (!wiphy || !wdev || !data || !data_len) {
		DBGLOG(REQ, ERROR, "input fast acs data null.\n");
		return -EINVAL;
	}
	is_sap_fast_acs = (bool) nla_get_u8((struct nlattr *) data);
	DBGLOG(REQ, INFO, "fast enabled %d", is_sap_fast_acs);
	return 0;
}

bool wlan_cfg80211_get_fast_acs(void)
{
	return is_sap_fast_acs;
}
//END WIFI_BugFixUpstream
	...
	struct wireless_dev *wdev, const void *data, int data_len)
{
	...
	if (prGlueInfo->prAdapter->rWifiVar.ucApfEnable == 0)
		...
		if (prGlueInfo->prAdapter->ucApfMode & BIT(1)) {
			aucCapablilities[0] = APF_VERSION4;
		} else if (prGlueInfo->prAdapter->ucApfMode & BIT(2)) {
	...
	struct wireless_dev *wdev, const void *data, int data_len)
{
	...
	uint8_t ucEnableOp = PKT_OFLD_OP_ENABLE_W_TPUT_DETECT;
...
#if (CFG_EXT_FEATURE == 1)
	if (prGlueInfo->prAdapter->ucApfMode & BIT(0)) {
		ucEnableOp = PKT_OFLD_OP_ENABLE;
		DBGLOG(REQ, INFO, "Disable Tput monitor due to APF mode = %dn",
			prGlueInfo->prAdapter->ucApfMode);
	}
#endif
...
static u_int8_t isStaConnectedChnls(struct ADAPTER *prAdapter,
	enum ENUM_BAND eBand,
	uint8_t ucChannelNum,
	struct RF_CHANNEL_INFO *prStaConnectedChnlInfo,
	uint32_t ucStaChnlNums)
{
	uint32_t i;

	/* channel is not dfs for P2P GO, if STA already connected to it*/
	for (i = 0; i < ucStaChnlNums; i++) {
		if ((prStaConnectedChnlInfo[i].eBand == eBand) &&
			(prStaConnectedChnlInfo[i].ucChannelNum ==
				ucChannelNum))
			return TRUE;
	}
	return FALSE;
...
u_int8_t isIfaceTypeSupportChannel(struct ADAPTER *prAdapter,
	...
	uint32_t ucStaChnlNums)
{
	...
	if (iface & (IFACE_MODE_STA | IFACE_MODE_P2P_CLIENT | IFACE_MODE_TDLS
		...
		if (eBand == BAND_5G) {
			isChnlConntedBySta = isStaConnectedChnls(prAdapter,
						ucChannelNum,
						eBand,
						prStaConnectedChnlInfo,
						ucStaChnlNums);
			if (rlmDomainIsIndoorChannel(
				prAdapter,
				BAND_5G,
				ucChannelNum) &&
				(!IS_INDOOR_CONCURRENT_ALLOWED(
					prAdapter,
					filter) ||
				!isChnlConntedBySta)){
				DBGLOG(REQ, TRACE,
	...
	} else if (iface & IFACE_MODE_NAN) {
		if (u2CountryCode == COUNTRY_CODE_WW) {
			if (eBand != BAND_2G4) {
				DBGLOG(REQ, INFO,
					"skip freq %d, country_code_WW\n",
					u4channel_freq);
				result = FALSE;
			}
			/* SS requirement NAN not use 2467 & 2472 in WW*/
			if (u4channel_freq >= 2467 &&
					u4channel_freq <= 2472) {
				DBGLOG(REQ, INFO,
					"skip freq %d, country_code_WW\n",
					u4channel_freq);
	...
	const void *data, int data_len)
{
	...
	struct RF_CHANNEL_INFO rStaConnectedChnlInfo[BSSID_NUM] = {0};
	uint32_t ucStaChnlNums = 0;
	...
	if (filter & CELLULAR_COEXISTENCE) {
		prLteSafeChn = kalMemZAlloc(sizeof(struct PARAM_GET_CHN_INFO),
			VIR_MEM_TYPE);
		if (!prLteSafeChn) {
			DBGLOG(REQ, ERROR, "allocate prLteSafeChn failed\n");
		} else {
			rStatus = kalIoctl(prGlueInfo,
	...
	rStatus = kalIoctl(prGlueInfo, wlanoidGetAisConnectedChnls,
		   rStaConnectedChnlInfo, sizeof(rStaConnectedChnlInfo),
		   &ucStaChnlNums);
	...
	for (i = 0; i < ucNumOfChannel; i++) {
		...
		if (supported_iface_mask != 0)
			DBGLOG(REQ, TRACE,
				"channels[%d], freq %d support_iface_mask 0x%x\n",
					i,
					u4channel_freq,
					supported_iface_mask);
	}
...
const struct nla_policy nla_pasn_rsp_policy[
	MTK_WLAN_VENDOR_ATTR_MAX + 1] = {
	[MTK_WLAN_VENDOR_ATTR_NAN_PASN_RAW_FRAME] = {
	.type = NLA_BINARY },
};
const struct nla_policy nla_pasn_setkey_policy[
	MTK_WLAN_VENDOR_ATTR_MAX + 1] = {
	[MTK_WLAN_VENDOR_ATTR_NAN_PASN_SET_KEY] = {
	.type = NLA_BINARY },
};

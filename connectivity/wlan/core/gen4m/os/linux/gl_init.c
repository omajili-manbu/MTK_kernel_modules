...
static const struct wiphy_vendor_command
	...
	// MIUI ADD: WIFI_BugFixUpstream
	{
		{
			.vendor_id = OUI_MTK,
			.subcmd = NL80211_VENDOR_SUBCMD_SET_FAST_ACS
		},
		.flags = WIPHY_VENDOR_CMD_NEED_WDEV
				| WIPHY_VENDOR_CMD_NEED_NETDEV
				| WIPHY_VENDOR_CMD_NEED_RUNNING,
		.doit = wlan_cfg80211_set_fast_acs
#if KERNEL_VERSION(5, 4, 0) <= LINUX_VERSION_CODE
		,
		.policy = nla_parse_wifi_attribute,
		.maxattr = WIFI_ATTRIBUTE_MAX
	...
	},
	{
		...
		.flags = WIPHY_VENDOR_CMD_NEED_WDEV |
				WIPHY_VENDOR_CMD_NEED_NETDEV |
				WIPHY_VENDOR_CMD_NEED_RUNNING,
		.doit = mtk_cfg80211_vendor_ndp_qca
#if KERNEL_VERSION(5, 4, 0) <= CFG80211_VERSION_CODE
		,
		.policy = mtk_wlan_vendor_ndp_policy,
		.maxattr = MTK_WLAN_VENDOR_ATTR_NDP_PARAMS_MAX
...
#if	CFG_SUPPORT_NAN_R4_PAIRING
	{
		{
			.vendor_id = OUI_MTK,
			.subcmd = MTK_SUBCMD_NAN_PASN_RSP
		},
		.flags = WIPHY_VENDOR_CMD_NEED_WDEV |
				WIPHY_VENDOR_CMD_NEED_NETDEV |
				WIPHY_VENDOR_CMD_NEED_RUNNING,
		.doit = mtk_cfg80211_vendor_nan_pasn_rsp
#if KERNEL_VERSION(5, 4, 0) <= CFG80211_VERSION_CODE
		,
		.policy = nla_pasn_rsp_policy,
		.maxattr = MTK_WLAN_VENDOR_ATTR_MAX
#endif
	},
	{
		{
			.vendor_id = OUI_MTK,
			.subcmd = MTK_SUBCMD_NAN_PASN_SETKEY
		},
		.flags = WIPHY_VENDOR_CMD_NEED_WDEV |
				WIPHY_VENDOR_CMD_NEED_NETDEV |
				WIPHY_VENDOR_CMD_NEED_RUNNING,
		.doit = mtk_cfg80211_vendor_nan_pasn_setkey
#if KERNEL_VERSION(5, 4, 0) <= CFG80211_VERSION_CODE
		,
		.policy = nla_pasn_setkey_policy,
		.maxattr = MTK_WLAN_VENDOR_ATTR_MAX
#endif
	},
	{
		{
			.vendor_id = OUI_MTK,
			.subcmd = MTK_SUBCMD_NAN_PASN_CONFIRM
		},
		.flags = WIPHY_VENDOR_CMD_NEED_WDEV |
				WIPHY_VENDOR_CMD_NEED_NETDEV |
				WIPHY_VENDOR_CMD_NEED_RUNNING,
		.doit = mtk_cfg80211_vendor_nan_pasn_confirm
#if KERNEL_VERSION(5, 4, 0) <= CFG80211_VERSION_CODE
		,
		.policy = nla_pasn_setkey_policy,
		.maxattr = MTK_WLAN_VENDOR_ATTR_MAX
...
#endif
#if CFG_SUPPORT_NAN_R4_PAIRING
	[WIFI_EVENT_SUBCMD_NAN_PASN] {
		.vendor_id = OUI_MTK,
		.subcmd = MTK_SUBCMD_NAN_PASN
	},
#endif /* CFG_SUPPORT_NAN_R4_PAIRING */
};
#endif
	...
	uint8_t ucAisIndex)
{
	...
	if (prMldBss) {
		LINK_FOR_EACH_ENTRY(prBssInfo, &prMldBss->rBssList,
			...
			ucBw = rlmVhtBw2OpBw(ucVhtChannelWidth,
					     prBssInfo->eBssSCO);
			...
			DBGLOG(P2P, INFO,
			       "STA band=%u, ch=%u, sco=%u, bw(vht)=%u, centerFreq=%u\n",
			       prBssInfo->eBand,
			       prBssInfo->ucPrimaryChannel,
			       prBssInfo->eBssSCO,
...
#else
	ucBw = rlmVhtBw2OpBw(prBssInfo->ucVhtChannelWidth,
			     prBssInfo->eBssSCO);
	u4CenterFreq = nicGetCenterChFreq(prBssInfo->eBand,
				prBssInfo->ucPrimaryChannel,
				prBssInfo->eBssSCO,
...
void wlanWakeLockInit(struct GLUE_INFO *prGlueInfo)
{
	...
	KAL_WAKE_LOCK_INIT(NULL, prGlueInfo->rIntrWakeLock,
			   "WLAN interrupt");
	KAL_WAKE_LOCK_INIT(NULL, prGlueInfo->rRxIndPktsWakeLock,
			   "WLAN_RxIndicatePkts");
	KAL_WAKE_LOCK_INIT(NULL, prGlueInfo->rRxIndOnePktWakeLock,
			   "WLAN_RxIndicateOnePkt");
	KAL_WAKE_LOCK_INIT(NULL, prGlueInfo->rIoctlWakeLock,
			   "WLAN_Ioctl");
	KAL_WAKE_LOCK_INIT(NULL, prGlueInfo->rMgmtTxEvtWakeLock,
			   "WLAN_MgmtTxEvent");
	KAL_WAKE_LOCK_INIT(NULL, prGlueInfo->rTxEvtWakeLock,
			   "WLAN_TxEvent");
	KAL_WAKE_LOCK_INIT(NULL, prGlueInfo->rFwOwnWakeLock,
			   "WLAN_FwOwn");
	KAL_WAKE_LOCK_INIT(NULL, prGlueInfo->rTxEvtToRxWakeLock,
			   "WLAN_TxEventToRx");
	KAL_WAKE_LOCK_INIT(NULL, prGlueInfo->rTxCmdEvtWakeLock,
			   "WLAN_TxCmdEvent");
	...
	KAL_WAKE_LOCK_INIT(NULL, prGlueInfo->prAlarmTimerWakeLock,
			   "Alarmtimer");
#endif
#if (CFG_WIFI_PCIE_L2_MOBILE_ONLY == 1)
	KAL_WAKE_LOCK_INIT(NULL, prGlueInfo->rOobWakeLock,
			   "OOB interrupt");
#endif
#if CFG_SUPPORT_WONDERTAP
	KAL_WAKE_LOCK_INIT(NULL, prGlueInfo->prWonWakeLock, "WLAN_WONDER");
#endif
#endif
}
...
void wlanWakeLockUninit(struct GLUE_INFO *prGlueInfo)
{
	...
	if (KAL_WAKE_LOCK_ACTIVE(NULL,
				prGlueInfo->rRxIndPktsWakeLock))
		KAL_WAKE_UNLOCK(NULL, prGlueInfo->rRxIndPktsWakeLock);
	KAL_WAKE_LOCK_DESTROY(NULL, prGlueInfo->rRxIndPktsWakeLock);

	if (KAL_WAKE_LOCK_ACTIVE(NULL,
				prGlueInfo->rRxIndOnePktWakeLock))
		KAL_WAKE_UNLOCK(NULL, prGlueInfo->rRxIndOnePktWakeLock);
	KAL_WAKE_LOCK_DESTROY(NULL, prGlueInfo->rRxIndOnePktWakeLock);

	if (KAL_WAKE_LOCK_ACTIVE(NULL,
				 prGlueInfo->rIoctlWakeLock))
		KAL_WAKE_UNLOCK(NULL, prGlueInfo->rIoctlWakeLock);
	KAL_WAKE_LOCK_DESTROY(NULL, prGlueInfo->rIoctlWakeLock);

	if (KAL_WAKE_LOCK_ACTIVE(NULL,
				 prGlueInfo->rMgmtTxEvtWakeLock))
		KAL_WAKE_UNLOCK(NULL, prGlueInfo->rMgmtTxEvtWakeLock);
	KAL_WAKE_LOCK_DESTROY(NULL, prGlueInfo->rMgmtTxEvtWakeLock);

	if (KAL_WAKE_LOCK_ACTIVE(NULL,
				 prGlueInfo->rTxEvtWakeLock))
		KAL_WAKE_UNLOCK(NULL, prGlueInfo->rTxEvtWakeLock);
	KAL_WAKE_LOCK_DESTROY(NULL, prGlueInfo->rTxEvtWakeLock);

	if (KAL_WAKE_LOCK_ACTIVE(NULL,
				 prGlueInfo->rFwOwnWakeLock))
		KAL_WAKE_UNLOCK(NULL, prGlueInfo->rFwOwnWakeLock);
	KAL_WAKE_LOCK_DESTROY(NULL, prGlueInfo->rFwOwnWakeLock);

	if (KAL_WAKE_LOCK_ACTIVE(NULL,
				 prGlueInfo->rTxEvtToRxWakeLock))
		KAL_WAKE_UNLOCK(NULL, prGlueInfo->rTxEvtToRxWakeLock);
	KAL_WAKE_LOCK_DESTROY(NULL, prGlueInfo->rTxEvtToRxWakeLock);

	if (KAL_WAKE_LOCK_ACTIVE(NULL,
				 prGlueInfo->rTxCmdEvtWakeLock))
		KAL_WAKE_UNLOCK(NULL, prGlueInfo->rTxCmdEvtWakeLock);
	KAL_WAKE_LOCK_DESTROY(NULL, prGlueInfo->rTxCmdEvtWakeLock);
	...
	if (KAL_WAKE_LOCK_ACTIVE(NULL,
		...
		KAL_WAKE_UNLOCK(NULL, prGlueInfo->prAlarmTimerWakeLock);
	KAL_WAKE_LOCK_DESTROY(NULL, prGlueInfo->prAlarmTimerWakeLock);
#endif
#if (CFG_WIFI_PCIE_L2_MOBILE_ONLY == 1)
	if (KAL_WAKE_LOCK_ACTIVE(NULL,
				 prGlueInfo->rOobWakeLock))
		KAL_WAKE_UNLOCK(NULL, prGlueInfo->rOobWakeLock);
	KAL_WAKE_LOCK_DESTROY(NULL, prGlueInfo->rOobWakeLock);
#endif
#if CFG_SUPPORT_WONDERTAP
	if (KAL_WAKE_LOCK_ACTIVE(NULL, prGlueInfo->prWonWakeLock))
		KAL_WAKE_UNLOCK(NULL, prGlueInfo->prWonWakeLock);
	KAL_WAKE_LOCK_DESTROY(NULL, prGlueInfo->prWonWakeLock);
#endif
#endif
}
	...
	struct PARAM_MULTICAST_LIST *prAddrList)
{
	...
	if (fgEnable) {
		...
		for (pmc = rtnl_dereference(in_dev->mc_list);
			...
			kalMemCopy(aucMcIp, &pmc->multiaddr, IPV4_ADDR_LEN);
			...
			u_int8_t fgEnable)
{
	...
	if (prGlueInfo->prAdapter->fgIsNANRegistered) {
		...
		if (prAdapter->rWifiVar.ucNanVendorIoctl) {
			DBGLOG(NAN, DEBUG,
			   "ucNanVendorIoctl is enabled, skip\n");
			return;
		}

		if (fgEnable) {
			if (nanGetActivePeerNum(prAdapter)) {
				DBGLOG(NAN, DEBUG,
				       "Enter suspend mode, active peers=%u skip disable FC and SetDWInterval\n",
				       nanGetActivePeerNum(prAdapter));
			} else {
				DBGLOG(NAN, DEBUG,
				       "Enter suspend mode, disable FC and SetDWInterval 8\n");
				nanSetFlashCommunication(prAdapter, FALSE);
				nanDevSetDWInterval(prAdapter, 8);
			}

			nanSchedChkAndSet2gDwOnly(prAdapter);
		} else {
			if (!nanGetFlashCommunication(prAdapter)) {
				DBGLOG(NAN, INFO,
					"Leave suspend mode, enable FC and SetDWInterval 1\n");
				nanSetFlashCommunication(prAdapter, TRUE);
				nanDevSetDWInterval(prAdapter, 1);
			} else {
				DBGLOG(NAN, INFO,
				       "Leave suspend mode, skip enable FC and SetDWInterval 1\n");
			}

			nanSchedChkAndRestoreFrom2gDwOnly(prAdapter);
		}
	}
#endif
}
...
#if (CFG_EXT_FEATURE == 1 && CFG_SUPPORT_APF == 1)
	prAdapter->ucApfMode = glGetApfMode();
#endif
}
...
#if (CFG_TESTMODE_FWDL_SUPPORT == 1)
	wlanOnWondertapReg(prGlueInfo, prAdapter, pprWdev[0],
		g_fgWlanOnOffHoldRtnlLock);
#else
	wlanOnWondertapReg(prGlueInfo, prAdapter, pprWdev[0],
		FALSE);
...
int32_t wlanOnAtReset(struct net_device *prDev)
{
	struct GLUE_INFO *prGlueInfo = NULL;
	struct ADAPTER *prAdapter = NULL;
	uint32_t rStatus = WLAN_STATUS_SUCCESS, rStatus2;
	uint32_t u4BufLen = 0;
	...
	if (rStatus == WLAN_STATUS_SUCCESS) {
		...
		for (u4Idx = 0; u4Idx < KAL_AIS_NUM; u4Idx++) {
			...
			/* Abort the pending scan req to prevent scan fail
			 * (scan state back to IDLE).
			 */
			rStatus2 = kalIoctlByBssIdx(prGlueInfo,
				wlanoidAbortScan,
				NULL, 1, &u4BufLen,
				u4Idx);

			if (rStatus2 != WLAN_STATUS_SUCCESS)
				DBGLOG(REQ, ERROR,
				"wlanoidAbortScan fail 0x%x\n", rStatus2);
...
#if CFG_SUPPORT_WONDERTAP
	if (prGlueInfo->prAdapter->fgIsWONRegistered) {
		DBGLOG(WON, DEBUG, "wonNetUnregister...\n");
#if (CFG_TESTMODE_FWDL_SUPPORT == 1)
		wonNetUnregister(prGlueInfo, g_fgWlanOnOffHoldRtnlLock, FALSE);
#else
		wonNetUnregister(prGlueInfo, FALSE, FALSE);
#endif
		DBGLOG(WON, DEBUG, "wonRemove...\n");
		/*wonRemove must before wlanAdapterStop */
#if (CFG_TESTMODE_FWDL_SUPPORT == 1)
		wonRemove(prGlueInfo, g_fgWlanOnOffHoldRtnlLock);
#else
		wonRemove(prGlueInfo, FALSE);
#endif
	}
#endif
	...
	if (IS_BSS_INDEX_AIS(prGlueInfo->prAdapter, ucBssIndex)) {
		...
		if (prAdapter->rNanNetRegState == ENUM_NET_REG_STATE_REGISTERED)
			prNetDevice = wlanGetNetInterfaceByBssIdx(prGlueInfo,
								  ucBssIndex);
#endif
	} else if (IS_BSS_WON(prBssInfo) && ucBssIndex < MAX_BSSID_NUM) {
#if CFG_SUPPORT_WONDERTAP

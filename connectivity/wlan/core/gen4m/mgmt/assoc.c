				  ...
				  uint16_t *pu2StatusCode)
{
	...
	/* For OWE, if ASSOC REQ ues PMKID, ASSOC RSP should include PMKID */
	if (IS_BSS_INDEX_AIS(prAdapter, prStaRec->ucBssIndex) &&
	    u2RxStatusCode == STATUS_CODE_SUCCESSFUL) {
		struct CONNECTION_SETTINGS *prConnSettings;
		...
		if (prConnSettings->eAuthMode == AUTH_MODE_WPA3_OWE &&
			...
			u_int8_t fgIsRsn = FALSE, fgIsOweDh = FALSE;
			...
			IE_FOR_EACH(pucIE, u2IELength, u2Offset) {
				if (IE_ID(pucIE) == ELEM_ID_RSN)
					fgIsRsn = rsnParseRsnIE(prAdapter,
						pucIE, &rRsnInfo);
			...
			if ((!fgIsRsn || rRsnInfo.u2PmkidCount == 0) &&
			     !fgIsOweDh) {
				DBGLOG(SAA, WARN,
					"No PMKID and OWE DH in %sASSOC RSP, RSN IE=%d, PMKID=%d, OWE DH=%d\n",
					u2RxFrameCtrl ==
					    MAC_FRAME_REASSOC_RSP ? "RE" : "",
					fgIsRsn,
					rRsnInfo.u2PmkidCount,
					fgIsOweDh);

				u2RxStatusCode =
					STATUS_CODE_ASSOC_RSP_OWE_NO_PMKID;

				/* Don't use PMKID and try again immediately */

...
void StatsEnvRxTime2Host(struct ADAPTER *prAdapter, void *pvPacket)
{
	...
	if (u8IntTime > u8RxTime)
		u8IntTime = u8RxTime;
	u4Delay = NSEC_TO_USEC(u8CurrTime - u8IntTime);
	...
	case IP_PRO_ICMP:
		...
		DBGLOG(RX, DEBUG,
	"IPID 0x%04x src %d dst %d UP %d,delay %u us,int2rx %llu us,IntTime %llu,%u/%u,leave at %02d:%02d:%02d.%06u\n",
			u2IPID, u2UdpSrcPort, u2UdpDstPort,
			((pucEth[1] & IPTOS_PREC_MASK) >> IPTOS_PREC_OFFSET),
			u4Delay,
			NSEC_TO_USEC(u8RxTime - u8IntTime),

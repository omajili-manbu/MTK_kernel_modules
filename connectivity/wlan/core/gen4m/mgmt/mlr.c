...
static u_int8_t mlrFillTcpUdpChecksum(void *pvPacket)
{
	...
	if (unlikely(!pvPacket)) {
		DBGLOG(TX, DEBUG, "MLR frag - pvPacket is NULL");
		return FALSE;
	}
		...
		struct QUE *prFragmentedQue)
{
	...
	if (MLR_CHECK_IF_ENABLE_DEBUG(prAdapter)) {
		DBGLOG(TX, DEBUG, "MLR frag - MSDU_INFO: Frag[0] dump...\n");
...
err:
	DBGLOG(TX, DEBUG, "MLR frag - %s ERR\n", __func__);
	return FALSE;
}
		...
		struct QUE *prFragmentedQue)
{
	...
	if (unlikely(!prStaRec->fgIsInUse)) {
		DBGLOG(TX, DEBUG, "MLR check - prStaRec->fgIsInUse is FALSE");
		return FALSE;
	}
	...
	MLR_DBGLOG(prAdapter, TX, DEBUG, "MLR frag - Start\n");
	/* KAL_REC_TIME_START(); */
	...
	if (MLR_CHECK_IF_ENABLE_DEBUG(prAdapter)) {
		DBGLOG(TX, DEBUG, "MLR frag - prSkb-after split dump...\n");
	...
	/* KAL_BOOT_TIME_END();
	 * DBGLOG(TX, DEBUG, "MLR frag - End[%lu us]\n",
	 * KAL_GET_BOOTTIME_INTERVAL());
	 */
	MLR_DBGLOG(prAdapter, TX, DEBUG, "MLR frag - End\n");
		...
		struct STA_RECORD *prStaRec)
{
	...
	if (unlikely(!prAdapter)) {
		DBGLOG(TX, DEBUG, "MLR check - prAdapter is NULL");
		return FALSE;
	}
	if (unlikely(!prBssInfo)) {
		DBGLOG(TX, DEBUG, "MLR check - prBssInfo is NULL");
		return FALSE;
	}
	if (unlikely(!prStaRec)) {
		DBGLOG(TX, DEBUG, "MLR check - prStaRec is NULL");
		return FALSE;
	}
		...
		void *prNativePacket)
{
	...
	if (unlikely(!prMsduInfo)) {
		DBGLOG(TX, DEBUG, "MLR check - prMsduInfo is NULL");
		return FALSE;
	}
	...
	if (unlikely(!prNativePacket)) {
		DBGLOG(TX, DEBUG, "MLR check - prNativePacket is NULL");
		return FALSE;
	}
	...
	if (fgForceTxFrag)
		DBGLOG(TX, DEBUG, "MLR check - Force Tx Frag\n");
	...
	uint8_t *uc2gTxEnValue, uint8_t *uc5gTxEnValue)
{
	...
	prWlanCfgEntry = wlanCfgGetEntry(prAdapter, "MlrCfg2gTxEn",
		WLAN_CFG_DEFAULT);
	if (prWlanCfgEntry) {
		MLR_DBGLOG(prAdapter, NIC, DEBUG, "Find key[%s]=%s\n",
		...
		uint16_t *pu2RateCode)
{
	...
	MLR_DBGLOG(prAdapter, NIC, DEBUG,
			 ...
			 struct MSDU_INFO *prMsduInfo, uint8_t *pucBuf)
{
	...
	if (IS_BSS_APGO(prBssInfo)) {
		...
		/* Check if disabling MLR while Single Link & 2.4G AP */
		if (mlrIsSingleLink2gBand(prAdapter, prStaRec))
			prMLR->ucLRBitMap = 0x00;
...
uint8_t mlrGetReasonIdFromStateBmp(uint8_t ucBitmap)
{
	uint8_t ucState = 0;

	if (ucBitmap & BIT(MLR_STATE_UPDATE_BY_DRV_OR_MLR_MODULE))
		ucState |= 0x01;

	if (ucBitmap & BIT(MLR_STATE_UPDATE_BY_RA))
		ucState |= 0x02;

	switch (ucState) {
	case 0x01:
		return MLR_STATE_UPDATE_REASON_BY_LOW_RSSI;
	case 0x02:
		return MLR_STATE_UPDATE_REASON_BY_RA;
	case 0x03:
		return MLR_STATE_UPDATE_REASON_BY_BOTH;
	default:
		return MLR_STATE_UPDATE_REASON_BY_IGNORE;
	}
}
		  ...
		  struct WIFI_EVENT *prEvent)
{
	...
	char uevent[300];
	uint8_t ucReasonId;
	uint8_t ucIsSendUevent = FALSE;
	static uint32_t u4LastUeventSendTime;
	static uint8_t ucLastReasonId = MLR_STATE_UPDATE_REASON_BY_IGNORE;
	uint32_t u4CurrentTime;
	...
	if ((IS_BSS_AIS(prBssInfo) &&
		...
		ucReasonId = mlrGetReasonIdFromStateBmp(
			prEvtMlrFsmUpdate->ucMlrStateBmp);
		ucBitmapAnd = prEvtMlrFsmUpdate->ucMlrMode
			& prStaRec->ucMlrSupportBitmap;

		/* Throttle kalSendUevent to once every 3000 ms (3 seconds) */
		u4CurrentTime = kalGetTimeTick();
		if (prAdapter->rWifiVar.fgEnMlrUevent
			&& (prStaRec->ucMlrState == MLR_STATE_IDLE
			&& prEvtMlrFsmUpdate->ucMlrState == MLR_STATE_START
			&& ucLastReasonId == MLR_STATE_UPDATE_REASON_BY_IGNORE
			&& ucReasonId <= MLR_STATE_UPDATE_REASON_BY_BOTH)
		|| (prStaRec->ucMlrState == MLR_STATE_START
			&& prEvtMlrFsmUpdate->ucMlrState == MLR_STATE_IDLE
			&& ucLastReasonId <= MLR_STATE_UPDATE_REASON_BY_BOTH
			&& ucReasonId == MLR_STATE_UPDATE_REASON_BY_IGNORE)
		|| (prStaRec->ucMlrState == MLR_STATE_START
			&& prEvtMlrFsmUpdate->ucMlrState == MLR_STATE_START
			&& ucLastReasonId <= MLR_STATE_UPDATE_REASON_BY_BOTH
			&& ucReasonId <= MLR_STATE_UPDATE_REASON_BY_BOTH
			&& CHECK_FOR_TIMEOUT(u4CurrentTime,
			u4LastUeventSendTime, 3000))) {
			kalSnprintf(uevent, sizeof(uevent),
				"mlr=M:0x%02x,S:%d,F:%d,R:%d,Bitmap:0x%02x(0x%02x & 0x%02x),EnTxFrag:%d\n",
			...
			u4LastUeventSendTime = u4CurrentTime;
			ucIsSendUevent = TRUE;
		}

		DBGLOG(NIC, DEBUG,
			   "MLR event - mode[%d->%d] state[%d->%d] frag[%d->%d] reason[%d->%d] sendU[%d] EnSendU[%d]\n",
			   prStaRec->ucMlrMode,
			   prEvtMlrFsmUpdate->ucMlrMode,
			   prStaRec->ucMlrState,
			   prEvtMlrFsmUpdate->ucMlrState,
			   ...
			   ucReasonId,
			   ucIsSendUevent,
			   prAdapter->rWifiVar.fgEnMlrUevent);

		ucLastReasonId = ucReasonId;
		...
		if (prMldStaRec)
			fgIsMultiLink = IS_MLD_STAREC_MULTI(prMldStaRec);
#endif
		if (prEvtMlrFsmUpdate->ucMlrState ==
			MLR_STATE_START
		...
		DBGLOG(NIC, DEBUG,
			"MLR event - BssIdx[%d]WlanIdx[%d]StaRecIdx[%d] M:S:F:SB[0x%02x, %d, %d, 0x%02x(R=%d)], Bitmap[0x%02x](0x%02x & 0x%02x), IsML=%d, EnTxFrag=%d\n",
			prBssInfo->ucBssIndex,
	...
	MLR_DBGLOG(prAdapter, NIC, DEBUG,
		"MLR event - BssIdx[%d]WlanIdx[%d]StaRecIdx[%d] Connected[%d] IsML[%d], M:S:F:SB[0x%02x, %d, %d, 0x%02x]\n",
		prBssInfo->ucBssIndex,
		prEvtMlrFsmUpdate->u2WlanIdx,
		ucStaIdx,
...
u_int8_t mlrIsSingleLink2gBand(struct ADAPTER *prAdapter,
	struct STA_RECORD *prStaRec)
{
	...
	if (unlikely(!prAdapter)) {
		DBGLOG(TX, WARN, "prAdapter is NULL");
		return FALSE;
	}
	...
	if (unlikely(!prBssInfo)) {
		DBGLOG(TX, WARN, "prBssInfo is NULL");
		return FALSE;
	}
	...
	DBGLOG(TX, INFO,
		"fgMlrDisSingleLink2g=%d, IsSingleLink=%d, Is2gBand=%d\n",
		prAdapter->rWifiVar.fgMlrDisSingleLink2g,
		!fgIsMultiLink,
		prBssInfo->eBand == BAND_2G4);

	return (prAdapter->rWifiVar.fgMlrDisSingleLink2g
		&& !fgIsMultiLink
		&& prBssInfo->eBand == BAND_2G4);
}

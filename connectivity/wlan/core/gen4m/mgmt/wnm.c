...
void wnmRecvBTMRequest(struct ADAPTER *prAdapter, struct SW_RFB *prSwRfb)
{
	...
	if (prAdapter->fgEnLowLatencyMode &&
	    prAdapter->rWifiVar.ucAllowBtmReqMode != 0xff &&
	...
	if (ucRequestMode == WNM_BSS_TM_REQ_PREF_CAND_LIST_INCLUDED &&
	    aisCheckNeighborApConnected(prAdapter, ucBssIndex)) {
		DBGLOG(WNM, WARN,
		       "All valid neighbors are connected and have non-zero pref value.\n");
		ucStatus = WNM_BSS_TM_REJECT_NO_SUITABLE_CANDIDATES;
		goto send_response;
	}

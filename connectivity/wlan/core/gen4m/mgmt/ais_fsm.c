			...
			uint8_t ucBssIndex)
{
	 ...
	 */
	if (entry &&
	    (rsnApInvalidPMK(prAdapter, entry->u2StatusCode,
	...
	struct STA_RECORD *prMainStaRec, uint8_t ucBssIndex)
{
	...
	if (prMainStaRec->u2StatusCode !=
	    ...
	    !rsnApInvalidPMK(prAdapter, prMainStaRec->u2StatusCode,
	...
	struct BSS_DESC_SET *prBssDescSet)
{
	 ...
	 */
	if ((POLICY_IS_BY_BSSID(prConnSettings) &&
	     prBssDescSet->fgIsMatchBssid) ||
	    (POLICY_IS_BY_BSSID_HINT(prConnSettings) &&
			    ...
			    struct MSG_HDR *prMsgHdr)
{
	...
	scheduledpm_action(prAdapter, SCHED_PM_RESUME, NULL, FALSE, 0);
#endif
	...
	uint8_t ucBcnTimeoutReason, uint8_t fgTryRecover, uint8_t ucBssIndex)
{
	...
	if (prAisBtoInfo->prBtoBssDesc && fgTryRecover) {
		DBGLOG(AIS, EVENT,
	...
	uint32_t u4ReqScan, uint8_t ucBssIndex)
{
	...
	if (prRoamingInfo->eReason != ROAMING_REASON_POOR_RCPI &&
	    prRoamingInfo->eReason != ROAMING_REASON_BEACON_TIMEOUT) {
...
// MIUI ADD: WIFI_SelfRecovery
void sapFuncValidateRxActionFrame(struct ADAPTER *prAdapter,
				  struct SW_RFB *prSwRfb)
{
	uint8_t ucBssIndex = 0;

	if (!prSwRfb) {
		DBGLOG(AIS, LOUD, "Null prSwRfb\n");
		return;
	}
	...
	/* All action frames indicate to hostapd */
	/* Leave the action frame to hostapd. */
	kalIndicateRxMgmtFrame(prAdapter, prAdapter->prGlueInfo,
		prSwRfb, ucBssIndex, MLD_LINK_ID_NONE);

	return;
}				/* sapFuncValidateRxActionFrame */
// END WIFI_SelfRecovery
...
u_int8_t aisCheckNeighborApConnected(struct ADAPTER *prAdapter,
	uint8_t ucBssIndex)
{
	struct AIS_SPECIFIC_BSS_INFO *prAisSpecBssInfo =
			aisGetAisSpecBssInfo(prAdapter, ucBssIndex);
	...
	struct BSS_DESC *prBssDesc = NULL;
	struct NEIGHBOR_AP *prNeighborAP = NULL;
	uint8_t i, ucConnectedCount = 0;

	if (!LINK_IS_EMPTY(prNeighborAPLink)) {
		LINK_FOR_EACH_ENTRY(prNeighborAP, prNeighborAPLink,
				    rLinkEntry, struct NEIGHBOR_AP) {
			for (i = 0; i < MLD_LINK_MAX; i++) {
		...
		if (prNeighborAPLink->u4NumElem == ucConnectedCount)
			return TRUE;
	}

				      ...
				      struct SW_RFB *prSwRfb)
{
	...
	if (prAdapter->rWifiVar.ucNanEnablePairing == 1) {
		...
		if ((prAuthFrame->u2AuthAlgNum == AUTH_ALGORITHM_NUM_PASN)
			&& IS_NAN_CLUSTER_ID(prAuthFrame->aucBSSID)) {
			nanPasnRxAuthFrame(prAdapter, prSwRfb);
			DBGLOG(NAN, INFO,
				"[pairing-pasn]Auth Alg(%02x) PASN-M%u to engine\n",
...
#if (CFG_SUPPORT_802_11BE_MLO == 1)
u_int8_t
authSaeCommitSkipFixedFields(uint8_t *pucIE, uint16_t u2IELength,
			     u_int8_t fgIsApMode, uint16_t u2StatusCode,
			     uint16_t *pu2Offset)
{
	uint16_t u2Group, u2Offset = 0;

	if (fgIsApMode && u2StatusCode == STATUS_CODE_SUCCESSFUL) {
		DBGLOG(AAA, WARN,
			"H2E not used, status=%u\n",
			u2StatusCode);
		return FALSE;
	} else if (u2StatusCode != WLAN_STATUS_SAE_HASH_TO_ELEMENT) {
		goto exit;
	}

	if (u2IELength < 2) {
		DBGLOG(AAA, WARN, "No SAE Group\n");
		return FALSE;
	}

	/* SAE H2E commit message (group, scalar, FFE) */
	WLAN_GET_FIELD_16(pucIE, &u2Group);
	switch (u2Group) {
	case 19:
		u2Offset = 2 + 32 + 32 * 2;
		break;
...
u_int8_t
authSaeConfirmSkipFixedFields(uint8_t *pucIE, uint16_t u2IELength,
			      u_int8_t fgIsApMode, uint16_t u2StatusCode,
			      uint16_t *pu2Offset)
{
	uint16_t u2Offset = 0;

	if (u2StatusCode == STATUS_REJECTED_WITH_SUGGESTED_BSS_TRANSITION)
		goto exit;

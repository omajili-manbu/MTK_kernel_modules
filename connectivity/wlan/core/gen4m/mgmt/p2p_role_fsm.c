	...
	uint8_t ucLinkIdx)
{
	...
	kalMemZero(prP2pSpecBssInfo, sizeof(*prP2pSpecBssInfo));
	...
	struct BSS_INFO *prP2pBssInfo)
{
	...
	if (p2pFuncIsRoleCsa(prAdapter,
			prP2pRoleFsmInfo->ucRoleIndex)) {
		p2pFuncSetRoleCsa(prAdapter,
			prP2pRoleFsmInfo->ucRoleIndex,
			FALSE);
		...
		struct STA_RECORD *prStaRec)
{
	...
	if (mld_starec) {
		...
		if (mld_starec->fgSkipDeauthTxDone) {
			DBGLOG(P2P, DEBUG,
				"Skip deauth tx done\n");
			return;
		}
		...
		struct MSG_HDR *prMsgHdr)
{
	...
	if (eBand == BAND_5G &&
		...
		bSkipRdd = FALSE;
	else if (eBand == BAND_5G && ucChannelNum >= 36 && ucChannelNum <= 48 &&
...
#if (CFG_SUPPORT_DFS_MASTER == 1)
	/* check if SCC with STA */
	for (i = 0; i < ARRAY_SIZE(prAdapter->aucDfsAisChnlReqEntries);
	     ++i) {
		struct WLAN_DFS_CHANNEL_REQ_ENTRY aisEntry =
			prAdapter->aucDfsAisChnlReqEntries[i];
		if (!aisEntry.fgValid)
			continue;
		DBGLOG(P2P, INFO,
		       "DfsAisEntry aisCh=%u, aisBw=%u, ApCh=%u, ApBw=%u\n",
		       aisEntry.rRfChnlInfo.ucChannelNum,
		       aisEntry.rRfChnlInfo.ucChnlBw,
		       ucChannelNum, ucRfBw);
		if (aisEntry.rRfChnlInfo.ucChannelNum == ucChannelNum) {
			if (aisEntry.rRfChnlInfo.ucChnlBw < ucRfBw &&
			    IS_BSS_GO(prAdapter, prBssInfo)) {
				/* Keep original BW and modify wifi.cfg */
				prBssInfo->fgIsModifyBwForDfs = TRUE;
				prBssInfo->ucP2p5gOriBwForDfs = ucRfBw;
				ucRfBw = aisEntry.rRfChnlInfo.ucChnlBw;
				prAdapter->rWifiVar.ucP2p5gBandwidth = ucRfBw;
			}
			if (aisEntry.rRfChnlInfo.ucChnlBw >= ucRfBw) {
				bSkipRdd = TRUE;
				break;
	...
	DBGLOG(P2P, INFO, "bSkipRdd=%u, bSkipCac=%u, role=%s, ch=%u, bw=%u\n",
	       bSkipRdd, bSkipCac,
		...
		struct MSG_HDR *prMsgHdr)
{
	...
	/* init p2pSpecificBssInfo for no group p2p */
	if (prP2pBssInfo->eIftype == IFTYPE_P2P_CLIENT ||
	    prP2pBssInfo->eIftype == IFTYPE_P2P_GO) {
		struct P2P_SPECIFIC_BSS_INFO *prP2pSpecBssInfo;

		prP2pSpecBssInfo = prAdapter->rWifiVar
			.prP2pSpecificBssInfo[prP2pRoleFsmInfo->ucRoleIndex];
		kalMemZero(prP2pSpecBssInfo, sizeof(*prP2pSpecBssInfo));
	}
			    ...
			    uint32_t *pau4SafeChnl)
{
	...
	for (i = 0; i < prMsgAcsRequest->u4NumChannel; i++) {
		...
		if (wlan_cfg80211_get_fast_acs() && prMsgAcsRequest->eChnlBw == MAX_BW_80MHZ &&
		    prAdapter->rWifiVar.u2CountryCode == COUNTRY_CODE_CN && u4NumChannel >= 1 &&
		    prRfChannelInfo->ucChannelNum == WLAN_REG_CH_165) {
			DBGLOG(REQ, INFO, "do fast acs");
			continue;
		}

...
#if (CFG_SUPPORT_NAN == 1)
	/* According to FW CNM design,
	 * need to leave NAN 2G DW only when concurrent with other network
	 */
			    ...
			    struct WIFI_EVENT *prEvent)
{
	...
	else if (prBssInfo && (prBssInfo->eNetworkType == NETWORK_TYPE_WON)) {
		DBGLOG(CNM, DEBUG, "WON channel granted.\n");
		prChResp->rMsgHdr.eMsgId = MID_CNM_WON_CH_GRANT;
	}
#endif
	else {
		       ...
		       uint8_t ucBssIndex)
{
	...
	if (IS_BSS_AIS(prBssInfo)) {
		...
		if (prBssDesc) {
			...
			/* 1. Revise BW by regulatory domain/country */
			...
			if (ucMaxBandwidth > ucMaxBandwidthOld) {
				...
				DBGLOG(CNM, TRACE,
					...
					ucMaxBandwidth,
					apucOpBw[ucMaxBandwidth],
					ucMaxBandwidthOld,
					apucOpBw[ucMaxBandwidthOld],
					(uint8_t) (u2Cc >> 8), (uint8_t) u2Cc);

				ucMaxBandwidth = ucMaxBandwidthOld;
			}

			/* 2. Check segment mismatch after all revisions */
			/* max bw is BW320_1 but ap is BW320_2, */
			/* downgrade to BW160 */
		...
		if (!prAdapter->rWifiVar.ucApChnlDefFromCfg
			...
			/* P2P mode */
				...
				(p2pFuncIsFixWfd1NssBw40(prAdapter,
				 prBssInfo))) {
					ucMaxBandwidth =
					((ucMaxBandwidth > MAX_BW_40MHZ) ?
					MAX_BW_40MHZ : ucMaxBandwidth);
				}
#endif
...
#endif
#if CFG_SUPPORT_WONDERTAP
	else if (IS_BSS_WON(prBssInfo)) {
		/* TODO: set correct BW */
		if (prBssInfo->eBand == BAND_2G4)
			ucMaxBandwidth = prAdapter->rWifiVar.ucWon2gBandwidth;
		else if (prBssInfo->eBand == BAND_5G)
			ucMaxBandwidth = prAdapter->rWifiVar.ucWon5gBandwidth;
#if (CFG_SUPPORT_WIFI_6G == 1)
		else if (prBssInfo->eBand == BAND_6G)
			ucMaxBandwidth = prAdapter->rWifiVar.ucWon6gBandwidth;
#endif
		else
			ucMaxBandwidth = MAX_BW_20MHZ;
	}
	...
	for (ucBssIndex = prWifiVar->ucBssIdStartValue;
		...
		if (prBssInfo && !prBssInfo->fgIsInUse) {
			...
			if (IS_BSS_NAN(prBssInfo)) {
				/* NAN will decide BMC wtbl later */
...
)
{
	...
	if (eRunReq < 0 || eRunReq > CNM_WMM_REQ_DEFAULT
		...
		if (eRunReq < CNM_WMM_REQ_NUM)
			u4QuotaFinal =
				prWmmQuotaCtrl->arReqPool[eRunReq].u4ReqQuota;
	}

...
void mldLinkStableTimeout(struct ADAPTER *prAdapter, uintptr_t ulParamPtr)
{
	struct MLD_STA_RECORD *prMldStaRec;
	struct BSS_INFO *prBssInfo;
	struct STA_RECORD *prStaRec;
	...
	DBGLOG(ML, INFO, "mldstarec=%d widx=%d bss=%d stable timeout!\n",
...
void mldStaRecEventLinkState(struct ADAPTER *ad,
	uint16_t widx, struct UNI_EVENT_STAREC_MLO_LINK_STATE *state)
{
	struct STA_RECORD *prStaRec;
	struct MLD_STA_RECORD *prMldStaRec;

	DBGLOG(ML, INFO, "widx=%d state=%d reason=%d\n",
		widx, state->ucLinkState, state->ucReason);

	prStaRec = cnmGetStaRecByWlanIndex(ad, widx);
	prMldStaRec = mldStarecGetByStarec(ad, prStaRec);
	if (prStaRec && prMldStaRec) {
		unsigned long long u8OriActiveStaBitmap;

		u8OriActiveStaBitmap = prMldStaRec->u8ActiveStaBitmap;
		...
		if (u8OriActiveStaBitmap != prMldStaRec->u8ActiveStaBitmap) {
			DBGLOG(ML, INFO,
				"<CONN> LINK_STATE bss=%d sta=%d widx=%d state=%s\n",
				prStaRec->ucBssIndex, prStaRec->ucIndex,
				prStaRec->ucWlanIndex,
				state->ucLinkState == MLO_LINK_STATE_ACTIVE ?
				"ACTIVE" : "INACTIVE");

#if (CFG_SUPPORT_MLD_LOG == 1)
			mldLogLink(ad,
				prStaRec,
				prMldStaRec,
				state->ucLinkState,
			...
			cnmTimerStopTimer(ad, &prMldStaRec->rLinkStableTimer);
			cnmTimerStartTimer(ad, &prMldStaRec->rLinkStableTimer,
					MLO_LINK_STABLE_TIMEOUT_MS);
		}
	}
}
	...
	struct MLD_BSS_INFO *prMldBssInfo, struct ML_INFO *prMlInfo)
{
	...
	for (i = 0; i < ARRAY_SIZE(prAdapter->aprMldStarec); i++) {
		...
		cnmTimerInitTimer(prAdapter,
			&prMldStarec->rLinkStableTimer,
			mldLinkStableTimeout,
			(uintptr_t)prMldStarec);
	...
	struct MLD_STA_RECORD *prMldStarec, struct STA_RECORD *prStarec)
{
	...
	cnmTimerStopTimer(prAdapter, &prMldStarec->rLinkStableTimer);
...
u_int8_t
mldParseMlInfoFromAuthReqFrame(struct ADAPTER *prAdapter,
			       struct BSS_INFO *prBssInfo,
			       struct WLAN_AUTH_FRAME *prAuthFrame,
			       uint16_t u2PacketLen,
			       struct MULTI_LINK_INFO *prMulLinkInfo)
{
	uint16_t u2IEOffset, u2IELength;
	uint8_t *pucIE;
	const uint8_t *pucMlIE = NULL;
	u_int8_t fgIsApMode = IS_BSS_AP(prAdapter, prBssInfo);

	u2IEOffset = (uint16_t) OFFSET_OF(struct WLAN_AUTH_FRAME,
					  aucInfoElem[0]);
	if (u2PacketLen < u2IEOffset) {
		DBGLOG(AAA, WARN,
	...
	u2IELength = u2PacketLen - u2IEOffset;
	if (u2IELength == 0)
		return FALSE;
	...
	{
		uint16_t u2StatusCode = prAuthFrame->u2StatusCode;
		uint16_t u2FieldOffset;

		if (prAuthFrame->u2AuthTransSeqNo == 1) {
			if (authSaeCommitSkipFixedFields(pucIE, u2IELength,
							 fgIsApMode,
							 u2StatusCode,
							 &u2FieldOffset) ==
			    FALSE)
				return FALSE;
		} else if (prAuthFrame->u2AuthTransSeqNo == 2) {
			if (authSaeConfirmSkipFixedFields(pucIE, u2IELength,
							 fgIsApMode,
							 u2StatusCode,
							 &u2FieldOffset) ==
			    FALSE)
			...
			DBGLOG(AAA, ERROR, "Invalid seq(%u)\n",
				prAuthFrame->u2AuthTransSeqNo);
			return FALSE;
		}

		if (u2FieldOffset >= u2IELength)
			return FALSE;

		pucIE += u2FieldOffset;
		u2IELength -= u2FieldOffset;
		break;
	}
	default:
		DBGLOG(AAA, WARN,
			"Unsupport algo(%u) for mlo\n",
			prAuthFrame->u2AuthAlgNum);
		return FALSE;
	}

	if (mldIsSingleLinkEnabled(prAdapter, NETWORK_TYPE_P2P,
				   fgIsApMode) &&
	    prBssInfo && RLM_NET_IS_11BE(prBssInfo))
		pucMlIE = mldFindMlIE(pucIE, u2IELength,
				      ML_CTRL_TYPE_BASIC);

	if (!pucMlIE)
		return FALSE;

	mldParseBasicMlIE(prMulLinkInfo, pucMlIE,
			  IE_SIZE(pucMlIE),
			  prAuthFrame->aucBSSID,
			  MAC_FRAME_AUTH,
			  __func__);
	if (!prMulLinkInfo->ucValid)
		return FALSE;

	prMulLinkInfo->ucMldType = mldCheckMldType(prAdapter, pucIE,
						   u2IELength);

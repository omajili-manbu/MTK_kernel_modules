			 ...
			 struct BSS_INFO *prBssInfo)
{
	...
	if (IS_BSS_NAN(prBssInfo)) {
		...
		for (ucEntry = 0;
			ucEntry <=
			(size_t)(prAdapter->ucTxDefaultWlanIndex - 1);
			ucEntry++) {
			if (prWtbl[ucEntry].ucUsed &&
				prWtbl[ucEntry].ucBssIndex ==
					prBssInfo->ucBssIndex) {
				secPrivacyFreeForEntry(prAdapter, ucEntry);
				DBGLOG(RSN, WARN,
					"free unRelease NAN bss/entry %d/%d\n",
					prBssInfo->ucBssIndex, ucEntry);
			}
		}
	}
#endif /* (CFG_NAN_SUPPORT_R4_GROUP_ADDR_DATA == 1) */
...
bool secIsUsedByStaRecEntry(struct ADAPTER *prAdapter, size_t Index)
{
	...
	for (i = 0; i < CFG_STA_REC_NUM; i++) {
		...
		if (prStaRec->fgIsInUse && prStaRec->ucWlanIndex == Index) {
			fgIsUsed = TRUE;
			DBGLOG(RSN, INFO,
				"[Wlan index]: Duplicated entry #%zu\n", Index);
			break;
		}
	}
	return fgIsUsed;
	...
	uint8_t *pucReUsed)
{
	...
	if (prAdapter->ucTxDefaultWlanIndex != 0)
		...
		DBGLOG(RSN, ERROR,
			"prAdapter->ucTxDefaultWlanIndex is 0, return FALSE\n");
	...
	for (i = 0; i <= (size_t)(prAdapter->ucTxDefaultWlanIndex - 1);
	  i++) {
		if (prWtbl[i].ucUsed
			&& EQUAL_MAC_ADDR(pucPeerAddr,
				prWtbl[i].aucMacAddr)
			&& EQUAL_MAC_ADDR(pucLocalAddr,

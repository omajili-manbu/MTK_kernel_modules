...
u_int8_t rttGetRSTABssIndex(struct ADAPTER *prAdapter,
	uint8_t *pucDestAddr, uint8_t *pucSrcAddr)
{
	uint8_t ucBssIndex = 0;
	struct BSS_INFO *prBssInfo = NULL;
#if CFG_SUPPORT_NAN_RANGING
	enum ENUM_BAND eNanRangingBand = prAdapter->rWifiVar.eNanRangingBand;
	...
	if (prPeerSchRecord &&
	    ...
	    prPeerSchRecord->eNanRangingBand > BAND_NULL &&
	    prPeerSchRecord->eNanRangingBand < BAND_NUM) {
		eNanRangingBand = prPeerSchRecord->eNanRangingBand;
	}
#endif /* CFG_SUPPORT_NAN_RANGING */
		...
		struct FTM_INFO_ELEM *prFtmInfoElem)
{
	...
	if (IS_BSS_INDEX_NAN(prAdapter, ucBssIndex)) {
		...
		if (eNanRangingBand > BAND_2G4)
			...
			channelWidth = WIFI_CHAN_WIDTH_20;
	} else
#endif /* CFG_SUPPORT_NAN_RANGING */
	{

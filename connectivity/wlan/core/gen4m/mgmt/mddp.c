...
uint32_t g_u4SerSn;
u_int8_t g_fgMddpPFUsing = FALSE;
	...
	uint8_t fgActivate)
{
	...
	prMddpTxd->wmmset = halRingDataSelectByWmmIndex(
		prAdapter, prBssInfo->ucWmmQueSet);

#ifdef SOC3_0
	if (prAdapter->rWifiVar.fgUseOneTxRing)
		prMddpTxd->wmmset = 0;
#endif
...
#undef TEMP_LOG_TEMPLATE
	...
	if (fgActivate) {
		...
		for (i = 0; i < MAX_BSSID_NUM; i++) {
			qmSetRxReorderTimeoutByBssIdx(prAdapter, i,
				RX_REORDER_TIMEOUT_TYPE_FW,
				u4RxReorderTimeoutTime);
		}

		DBGLOG(INIT, INFO,
			"mddp: STA connect, set RX reoder timeout: %d\n",
			u4RxReorderTimeoutTime);
	}
...
void mddpSetPFUsing(u_int8_t fgIsUsing)
{
	DBGLOG(INIT, DEBUG, "mddp: SetPFUsing %u\n", fgIsUsing);
	g_fgMddpPFUsing = fgIsUsing;
}

u_int8_t mddpGetPFUsing(void)
{
	DBGLOG(INIT, DEBUG, "mddp: GetPFUsing %u\n", g_fgMddpPFUsing);
	return g_fgMddpPFUsing;
}

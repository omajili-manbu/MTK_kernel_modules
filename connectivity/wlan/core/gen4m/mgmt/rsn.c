...
uint8_t rsnApInvalidPMK(struct ADAPTER *prAdapter,
	uint16_t status, uint16_t reason, enum ENUM_PARAM_AUTH_MODE AuthMode)
{
	switch (status) {
	...
	case STATUS_CODE_ASSOC_RSP_OWE_NO_PMKID:
		return TRUE;
	...
	if (IS_FEATURE_ENABLED(prAdapter->rWifiVar.fgStaPmkCheckDeauth)) {
		switch (reason) {

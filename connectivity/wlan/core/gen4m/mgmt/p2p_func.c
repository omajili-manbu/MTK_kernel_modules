...
drop:
		...
		if (p2pFuncNeedChangeConnState(prAdapter, eConnState))
		...
		struct BSS_INFO *prP2pBssInfo)
{
	...
	/* Recover 5G BW */
	if (prP2pBssInfo->fgIsModifyBwForDfs) {
		prAdapter->rWifiVar.ucP2p5gBandwidth =
			prP2pBssInfo->ucP2p5gOriBwForDfs;
		prP2pBssInfo->fgIsModifyBwForDfs = FALSE;
		prP2pBssInfo->ucP2p5gOriBwForDfs = 0;
	}
...
void p2pFuncStopZerowait(struct ADAPTER *prAdapter, uint8_t ucBssIdx)
{
	struct CMD_RDD_ON_OFF_CTRL *prCmdRddOnOffCtrl;
	struct BSS_INFO *prBssInfo;
	...
	if (!prBssInfo)
		goto exit;
	prBssInfo->fgIsDfsActive = FALSE;
	prCmdRddOnOffCtrl->ucDfsCtrl = ZEROWAIT_STOP;
	prCmdRddOnOffCtrl->ucBssIdx = ucBssIdx;

	DBGLOG(P2P, INFO,
		"Stop Zerowait - Bss index: %d\n",
		ucBssIdx);
		  ...
		  uint8_t ucSubType, u_int8_t fgIsLocallyGenerated)
{
	...
	do {
		...
		if (fgSendDeauth) {
			...
			prMldSta = mldStarecGetByStarec(prAdapter, prStaRec);
			if (prMldSta)
				prMldSta->fgSkipDeauthTxDone = TRUE;
#endif
...
uint8_t
p2pFuncAuthTimeCheck(struct ADAPTER *prAdapter,
		...
		struct STA_RECORD *prStaRec)
{
	...
	rTimeDiff =
		rAuthTime - prStaRec->rAuthInitTime;

	if (prStaRec->ucAuthCount == 0) {
		prStaRec->rAuthInitTime = rAuthTime;
		prStaRec->ucAuthCount++;
		rTimeDiff = 0;
	} else if (rTimeDiff <
		(MIN_AUTH_TIME_DIFF * AUTH_PACKET_CHECK_NUM)) {
		prStaRec->ucAuthCount++;
	} else {
		prStaRec->rAuthInitTime = rAuthTime;
		prStaRec->ucAuthCount = 1;
		rTimeDiff = 0;
	}
	DBGLOG(P2P, TRACE,
		"p2p widx: %u, AuthCount: %u, rTimeDiff: %u\n",
		prStaRec->ucWlanIndex,
		prStaRec->ucAuthCount,
		rTimeDiff);
	return
		(prStaRec->ucAuthCount > AUTH_PACKET_CHECK_NUM);
}				/* p2pFuncAuthTimeCheck */
		...
		uint16_t *pu2StatusCode)
{
...
#if (CFG_SUPPORT_802_11BE_MLO == 1)
	struct MULTI_LINK_INFO rMulLinkInfo = {0};
	struct MLD_BSS_INFO *prMldBss;
	struct MLD_STA_RECORD *prOldMldSta = NULL, *prNewMldSta = NULL;
#endif /* CFG_SUPPORT_802_11BE_MLO */
	struct STA_RECORD *prStaRec;
	...
	*pu2StatusCode = STATUS_CODE_REQ_DECLINED;
	...
	pucPeerAddr = prAuthFrame->aucSrcAddr;
#if (CFG_SUPPORT_802_11BE_MLO == 1)
	prMldBss = mldBssGetByBss(prAdapter, prP2pBssInfo);
	if (prMldBss &&
	    mldParseMlInfoFromAuthReqFrame(prAdapter, prP2pBssInfo,
					   prAuthFrame, prSwRfb->u2PacketLen,
					   &rMulLinkInfo) &&
	    rMulLinkInfo.ucValid) {
		DBGLOG(P2P, INFO, "mld_addr=" MACSTR "\n",
			MAC2STR(rMulLinkInfo.aucMldAddr));
		pucPeerAddr = rMulLinkInfo.aucMldAddr;
		prOldMldSta = mldStarecGetByMldAddr(prAdapter, prMldBss,
						    rMulLinkInfo.aucMldAddr);
	}
#endif /* CFG_SUPPORT_802_11BE_MLO */
	...
	if ((prAuthFrame->u2AuthTransSeqNo == 1) &&
		(u4StaUsedCount >= P2P_MAXIMUM_CLIENT_COUNT)) {
		DBGLOG(P2P, WARN,
	...
	else if ((prAuthFrame->u2AuthTransSeqNo == 1) &&
...
#if (CFG_SUPPORT_802_11BE_MLO == 1)
	if (prOldMldSta) {
		struct LINK *prStarecList = &prOldMldSta->rStarecList;
		struct STA_RECORD *prLinkSta, *prTempSta;
		struct BSS_INFO *prLinkBss;
		u_int8_t fgRemove = FALSE;

		LINK_FOR_EACH_ENTRY(prLinkSta, prStarecList, rLinkEntryMld,
				    struct STA_RECORD) {
			if (prLinkSta->ucStaState > STA_STATE_1 &&
			    IS_STA_IN_P2P(prAdapter, prLinkSta)) {
				fgRemove = TRUE;
				break;
			}
		}
		...
		if (fgRemove)
			LINK_FOR_EACH_ENTRY_SAFE(prLinkSta, prTempSta,
						 prStarecList,
						 rLinkEntryMld,
						 struct STA_RECORD) {
				prLinkBss = GET_BSS_INFO_BY_INDEX(prAdapter,
					prLinkSta->ucBssIndex);
				if (!prLinkBss)
					continue;

				bssRemoveClient(prAdapter, prLinkBss,
						prLinkSta);
				p2pFuncDisconnect(prAdapter, prLinkBss,
					prLinkSta, FALSE,
...
#if (CFG_SUPPORT_802_11BE_MLO == 1)
		if (rMulLinkInfo.ucValid) {
			COPY_MAC_ADDR(rMlInfo.aucMldAddr,
				      rMulLinkInfo.aucMldAddr);
			rMlInfo.u2EmlCap = rMulLinkInfo.u2EmlCap;
			rMlInfo.u2MldCap = rMulLinkInfo.u2MldCap;
			rMlInfo.fgMldType = rMulLinkInfo.ucMldType;

			prNewMldSta = mldStarecAlloc(prAdapter, prMldBss,
						     &rMlInfo);
			if (!prNewMldSta) {
				DBGLOG(AAA, ERROR,
					"Can't alloc mldstarec!\n");
				cnmStaRecFree(prAdapter, prStaRec);
				return FALSE;
			}
			mldStarecRegister(prAdapter, prNewMldSta, prStaRec,
					  prP2pBssInfo->ucLinkId);
...
u_int8_t p2pFuncIsChSwitchAvailable(
		struct ADAPTER *prAdapter)
{
	int8_t i;
	struct BSS_INFO *bss = NULL;
		...
		enum ENUM_P2P_FILTER_SCENARIO_TYPE eFilterScnario)
{
	...
	if (!p2pFuncIsChSwitchAvailable(prAdapter))
		goto exit;
...
p2pFunGetTopPreferFreqByBand(struct ADAPTER *prAdapter,
		...
		u_int8_t fgNoDfs, u_int8_t fgIsAp)
{
			     ...
			     struct BSS_INFO *prBssInfo, uint32_t *apu4FreqList)
{
	...
	for (ch = 0; ch < ucCandidateChnlNum; ++ch) {
		...
		/* Bn2 may have power attenuation at UNII-2C */
		if (prWifiVar->fgDisBn2UNII2C) {
			if (prRfChnlInfo1->ucChannelNum >= UNII2C_LOWER_BOUND &&
			    prRfChnlInfo1->ucChannelNum <= UNII2C_UPPER_BOUND) {
				DBGLOG(P2P, TRACE, "Skip UNII-2C ch=%u\n",
...
#if (CFG_SUPPORT_WIFI_6G == 1)
			|| (prRfChnlInfo1->eBand == BAND_6G)
#endif
		) &&
		(p2pFuncIsFixWfd1NssBw40(prAdapter, prBssInfo))) {
			prRfChnlInfo1->ucChnlBw  =
				 ((prRfChnlInfo1->ucChnlBw > MAX_BW_40MHZ) ?
				 MAX_BW_40MHZ : prRfChnlInfo1->ucChnlBw);
		}
#endif
 ...
 * \param[in] pau4FreqList Pointer to store output prefer list.
 * \param[in] pu4FreqListNum Number of freq in pau4FreqList.
 * \param[in] pau4FreqAllowList Pointer to IDC allow channel list.
 * \param[in] ucAllowFreqNum Number of freq in pau4FreqAllowList.
 * \param[in] fgIsSkipDfs Flag for skip DFS channel.
 *
 * \retval Status.
 */
/*---------------------------------------------------------------------------*/
		...
		uint8_t ucAllowFreqNum, u_int8_t fgIsSkipDfs)
{
	...
	if (prWifiVar->eDbdcMode == ENUM_DBDC_MODE_DISABLED &&
		...
		/* Treat SCC as safe channel to prevent falls into MCC */
	...
	if (ucNumAliveBss5g + ucNumAliveBss6g > 0) {	/* SCC */
...
u_int8_t
p2pFuncNeedChangeConnState(struct ADAPTER *prAdapter,
		enum ENUM_P2P_CONNECT_STATE eConnState)
{
	switch (eConnState) {
	case P2P_CNN_GO_NEG_REQ:
...
void
p2pFuncSetRoleCsa(struct ADAPTER *prAdapter,
		uint8_t ucRoleIdx,
		u_int8_t fgCsa)
{
	struct GLUE_INFO *prGlueInfo = prAdapter->prGlueInfo;
	...
	if (prP2pInfo)
		prP2pInfo->fgChannelSwitchReq = fgCsa;
}
...
u_int8_t p2pFuncIsPreferWfdAa(struct ADAPTER *prAdapter)
{
	...
	struct WFD_CFG_SETTINGS *prWfdCfgSettings =
		&(prWifiVar->rWfdConfigureSettings);

	return (ccmIsPreferAA(prAdapter) &&
		prWfdCfgSettings->ucWfdEnable == 1 &&
		!prWifiVar->fgDisCnmTBTC &&
		!prWifiVar->fgDisCnmSTAWFDTBTC);
}
#endif

#if (CFG_SUPPORT_WFD == 1 && CFG_SUPPORT_WFD_1NSS_BW40_CONFIG == 1)
u_int8_t p2pFuncIsFixWfd1NssBw40(struct ADAPTER *prAdapter,
			      struct BSS_INFO *prBssInfo)
{
	if (prAdapter->rWifiVar.ucFixWfd1NssBw40 &&
		IS_BSS_P2P(prBssInfo) &&
		wlanWfdEnabled(prAdapter)) {
		DBGLOG(SW4, LOUD, "Use 1NssBw40 due to miracast mode\n");
		return TRUE;
	}
	return FALSE;
}
			       ...
			       enum ENUM_BAND eTargetBand)
{
	...
	(p2pFuncIsFixWfd1NssBw40(prAdapter, prBssInfo))) {
		ucTargetBw  =
				((ucTargetBw > MAX_BW_40MHZ) ?
				MAX_BW_40MHZ : ucTargetBw);
	}
#endif
	if (eTargetBand == BAND_5G &&
...
#if (CFG_SUPPORT_WFD == 1 && CFG_SUPPORT_WFD_1NSS_BW40_CONFIG == 1)
		if (p2pFuncIsFixWfd1NssBw40(prAdapter, prBssInfo)) {
			rRfChnlInfo.ucChnlBw =
			((prAdapter->rWifiVar.ucP2p5gBandwidth > MAX_BW_40MHZ) ?
				 MAX_BW_40MHZ : rRfChnlInfo.ucChnlBw);
		}
#endif
		rRfChnlInfo.u2PriChnlFreq =

				...
				struct SW_RFB *prSwRfb)
{
	...
	if (IS_STA_IN_AIS(prAdapter, prStaRec)) {
		...
		if (IS_BSS_AP(prAdapter, prBssInfo) &&
			...
			p2pFuncDisconnect(prAdapter, prBssInfo, prStaRec,
					  TRUE, REASON_CODE_CLASS_2_ERR,
					  MAC_FRAME_DEAUTH, FALSE);
	...
	else if (IS_STA_WON_TYPE(prStaRec)) {
		DBGLOG(WON, WARN,
			   "Received a Deauth: wlanIdx[%d] from WON network\n",
			   ucWlanIdx);
	}
#endif
	else {
				 ...
				 struct STA_RECORD *prStaRec)
{
	...
	do {
		...
		if (!IS_BMCAST_MAC_ADDR(prDeauthFrame->aucDestAddr)) {
			...
			DBGLOG(RSN, INFO,
				"[%d] QM RX MGT: rsnStartSaQuery, count = %d\n",
				prStaRec->ucBssIndex,
				  ...
				  struct SW_RFB *prSwRfb)
{
	...
	if (IS_STA_IN_AIS(prAdapter, prStaRec)) {
		...
		DBGLOG(WON, WARN,
			"Received a Disassoc: wlanIdx[%d] from WON network\n",
			ucWlanIdx);
	}
#endif
	else {

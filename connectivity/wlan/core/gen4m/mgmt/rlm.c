...
static void rlmRecOpModeBwForClient(struct ADAPTER *prAdapter,
	 ...
	 struct MSDU_INFO *prMsduInfo)
{
	...
	if (IS_BSS_AP(prAdapter, prBssInfo) &&
		...
		(frame_ctrl == MAC_FRAME_PROBE_RSP ||
		frame_ctrl == MAC_FRAME_BEACON)) {
		struct IE_MTK_DFS_CONTROL *prDfsCtrl;
		uint8_t ucRoleIndex;
		...
		DBGLOG(RLM, INFO, "MTK_OUI_DFS_CONTROL\n");
		if (prP2pSpecificBssInfo &&
			prP2pSpecificBssInfo->ucRddZeroWaitInd !=
				P2P_DFS_TRAFFIC_IE_REMOVE) {

			prDfsCtrl = (struct IE_MTK_DFS_CONTROL *)
				(pucBuffer + IE_SIZE(pucBuffer));
			len = prMsduInfo->u2FrameLength;

			prDfsCtrl->ucId = MTK_OUI_ID_DFS_CONTROL;
			prDfsCtrl->ucLength = 0;

			if (prP2pSpecificBssInfo->ucRddZeroWaitInd ==
				P2P_DFS_TRAFFIC_IE_ADD_CTRL_DIS)
				prDfsCtrl->ucVar0 =
					P2P_DFS_TRAFFIC_IE_CTRL_DIS;
			else if (prP2pSpecificBssInfo->ucRddZeroWaitInd ==
				P2P_DFS_TRAFFIC_IE_ADD_CTRL_EN)
				prDfsCtrl->ucVar0 =
					P2P_DFS_TRAFFIC_IE_CTRL_EN;

			/* This magic number was added to avoid FW
			 * parsing beacon IE errors, so only when FW
			 * parsing var1 = 66, the value of var0 will
			 * be used.
			*/
			prDfsCtrl->ucVar1 = 66;

			prMsduInfo->u2FrameLength +=
				sizeof(struct IE_MTK_DFS_CONTROL);
			prDfsCtrl->ucLength =
				 (prMsduInfo->u2FrameLength - len - 2);
			MTK_OUI_IE(pucBuffer)->ucLength += IE_SIZE(prDfsCtrl);

			DBGLOG_MEM8(RLM, INFO, pucBuffer, IE_SIZE(pucBuffer));
		}
	}
...
#if (CFG_SUPPORT_802_11AX == 1)
	if (IS_BSS_AIS(prBssInfo) && prStaRec
	    && (prStaRec->ucDesiredPhyTypeSet & PHY_TYPE_SET_802_11AX
#if (CFG_SUPPORT_802_11BE == 1)
	    || (prStaRec->ucDesiredPhyTypeSet & PHY_TYPE_SET_802_11BE)
#endif /* CFG_SUPPORT_802_11BE */
	    )) {
		...
		if (prBssDesc &&
		    HE_IS_MAC_CAP_OM_CTRL(prBssDesc->ucHeMacCapInfo)) {
			DBGLOG(RLM, INFO,
				"Skip OMN IE in Assoc Req since we will send OMI later\n");
			return;
		}
	}
#endif /* CFG_SUPPORT_802_11AX */
...
VHT_CAP_INFO_COMPRESSED_STEERING_NUMBER_OF_BEAMFORMER_ANTENNAS_4_SUP;
	...
	if (IE_SIZE(prVhtCap) > (ELEM_HDR_LEN + ELEM_MAX_LEN_VHT_CAP)) {
		DBGLOG(RLM, ERROR, "Invalid VHT CAP IE Len: %u",
			IE_SIZE(prVhtCap));
		return;
	}
...
uint8_t rlmGetMaxBwByBand(struct ADAPTER *prAdapter,
	enum ENUM_BAND eBand, struct BSS_DESC *prBssDesc)
{
	...
	if (eBand == BAND_2G4) {
		...
		DBGLOG(RLM, ERROR, "Invalid BAND:%d!\n", eBand);
			...
			struct BSS_INFO *prBssInfo)
{
...
#else
	uint8_t ucMaxBw;
	uint8_t fgIsBwAtMax = FALSE;
	uint8_t fgIsNssAtMax = FALSE;
	struct BSS_DESC *prBssDesc = NULL;
	...
	ucMaxBw = rlmGetMaxBwByBand(prAdapter, prBssInfo->eBand, prBssDesc);
	fgIsBwAtMax = (ucOpBw >= ucMaxBw);

	/* In 6GHz, 40MHz and 80MHz support are indicated by the same bit
	 * in HE Physical Capabilities. To resolve this ambiguity, we send
	 * an OMI frame to explicitly inform the AP of our actual operating
	 * bandwidth.
	 */
	if (fgIsNssAtMax && fgIsBwAtMax) {
		DBGLOG(RLM, TRACE,
			"Op NSS == Max NSS %d && op BW is at max %d, return\n",
			prBssInfo->ucOpRxNss, ucOpBw);
		return;
	}

	DBGLOG(RLM, INFO,
		"Send OMI: fgIsNssAtMax=%u, fgIsBwAtMax=%u (Nss:%u/%u, Bw:%u/%u)\n",
		fgIsNssAtMax, fgIsBwAtMax,
		prBssInfo->ucOpRxNss,
		wlanGetSupportNss(prAdapter, prBssInfo->ucBssIndex),
		ucOpBw, ucMaxBw);
		    ...
		    uint8_t *pucPrimaryCh)
{
	...
	/* Revise AP maximum bandwidth that the primary channel can support */
	if (IS_BSS_AIS(prBssInfo) &&
	    aisGetTargetBssDesc(prAdapter, ucBssIndex)) {
		struct BSS_DESC *prBssDesc =
			aisGetTargetBssDesc(prAdapter, ucBssIndex);

		ucMaxBandwidth = ucCurrentBandwidth;
		...
		if (ucCurrentBandwidth > ucMaxBandwidth) {
			...
			ucCurrentBandwidth, apucOpBw[ucCurrentBandwidth],
	...
	/* Revise maximum bandwidth that we can support */
	...
	if (fgHasNewOPModeIE == TRUE) {
		if (prStaRec->ucStaState == STA_STATE_3) {
			if (fgUpdateToFWForDiffOPModeIE == TRUE) {
				...
				rlmRecOpModeBwForClient(prAdapter,
						ucVhtOpModeChannelWidth,
						prBssInfo);
...
/*----------------------------------------------------------------------------*/
static void rlmRecOpModeBwForClient(struct ADAPTER *prAdapter,
		uint8_t ucVhtOpModeChannelWidth, struct BSS_INFO *prBssInfo)
{
	struct STA_RECORD *prStaRec = NULL;
	enum ENUM_CHANNEL_WIDTH eRfChannelWidth;
		...
		if (((ucOperatingMode & VHT_OP_MODE_RX_NSS_TYPE) !=
			...
			if (prBssInfo->eCurrentOPMode == OP_MODE_ACCESS_POINT) {
				...
				rlmUpdateVhtOpCapInfo(prAdapter,
				   ...
				   OP_MODE_INFRASTRUCTURE)
				rlmRecOpModeBwForClient(prAdapter,
					ucVhtOpModeChannelWidth, prBssInfo);
			       ...
			       struct BSS_INFO *prBssInfo)
{
	...
	if (prBssInfo->fgIsOpChangeChannelWidth) {
		...
		if (prBssInfo->eBand == BAND_6G &&
			prBssInfo->ucOpChangeChannelWidth == MAX_BW_40MHZ) {
			/* 6G s1 calculation don't care sco */
			prBssInfo->ucVhtChannelFrequencyS1 = nicGetS1(
				prBssInfo->eBand,
				prBssInfo->ucPrimaryChannel,
				prBssInfo->eBssSCO,
			...
			if (prBssInfo->ucVhtChannelFrequencyS1 != 0) {
				if (prBssInfo->ucVhtChannelFrequencyS1 >
					prBssInfo->ucPrimaryChannel)
					prBssInfo->eBssSCO = CHNL_EXT_SCA;
				else if (prBssInfo->ucVhtChannelFrequencyS1 <
					prBssInfo->ucPrimaryChannel)
			...
			DBGLOG(RLM, INFO,
				"Update BSS[%d] 6G BW40 Channel Width Info to s1=%d sco=%d\n",
				prBssInfo->ucBssIndex,
				prBssInfo->ucVhtChannelFrequencyS1,
				prBssInfo->eBssSCO);
		}
		...
		if (prBssInfo->ucPhyTypeSet & PHY_TYPE_BIT_HT
			...
			if (prBssInfo->ucOpChangeChannelWidth == MAX_BW_20MHZ) {
				...
				if (prBssInfo->eCurrentOPMode ==
					...
					/* no HT OP IE for 6G, don't update */
	...
	uint8_t *pucPriChIdx)
{
	 ...
	 *      |      4        |             8           |
	 */
	ucTxPwrEnvCnt = TX_PWR_ENV_INFO_GET_TXPWR_COUNT(
	 ...
	 * TX_PWR_ENV_INFO_TXPWR_COUNT_MAX + 1(+1 is for consider Transmit
	 * Power Information field), we will consider Extension Maximum
	 * Transmit Power field.
	 */
	if (IE_LEN(prTxPwrEnvIE) > TX_PWR_ENV_INFO_TXPWR_COUNT_MAX + 1) {
		/* Get PSD Cnt Ext for check */
	...
	/* Check for Primary channel index */

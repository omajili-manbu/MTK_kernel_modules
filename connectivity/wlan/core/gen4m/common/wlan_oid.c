		      ...
		      uint32_t *pu4QueryInfoLen)
{
	...
	if (prAdapter->fgIsRadioOff == FALSE) {
		for (i = 0; i < prWlanInfo->u4ScanResultNum; i++) {
			/* If buflen reaches the maximum value,
	...
	*pu4SetInfoLen = 0;
...
#endif
	{
		...
		if (p2pFuncIsCsaBlockScan(prAdapter)) {
			...
			scheduledpm_action(prAdapter,
				SCHED_PM_SUSPEND, NULL, FALSE, 0);
#endif
			aisFsmScanRequestAdv(prAdapter, prScanRequest);
...
wlanoidSetCountryCode(struct ADAPTER *prAdapter,
	...
	/* Todo: Need to block FWK setting country code */
...
uint32_t
wlanoidQueryTxPowerComposition(struct ADAPTER *prAdapter,
	void *pvQueryBuffer, uint32_t u4QueryBufferLen,
	uint32_t *pu4QueryInfoLen)
{
#define MAX_CMD_TX_POWER_COMPOSITION_NUM 100
	struct CMD_TX_POWER_COMPOSITION_T *prCmdTxPCompos;
	uint32_t rWlanStatus = WLAN_STATUS_SUCCESS;
	uint8_t ucNum = 0;
	uint32_t u4Size = 0;
	...
	if (u4QueryBufferLen % sizeof(struct PARAM_TXPOWER_COMPOSITION_T) != 0)
		return WLAN_STATUS_FAILURE;

	ucNum = u4QueryBufferLen / sizeof(struct PARAM_TXPOWER_COMPOSITION_T);

	DBGLOG(OID, ERROR, "[TXPSTAT] Num : %d, MAX:%d\n", ucNum,
		MAX_CMD_TX_POWER_COMPOSITION_NUM);

	if (ucNum > MAX_CMD_TX_POWER_COMPOSITION_NUM)
		return WLAN_STATUS_FAILURE;

	u4Size = sizeof(struct CMD_TX_POWER_COMPOSITION_T) +
		sizeof(struct PARAM_TXPOWER_COMPOSITION_T) * ucNum;
	prCmdTxPCompos =
		(struct CMD_TX_POWER_COMPOSITION_T *) kalMemAlloc(u4Size,
		VIR_MEM_TYPE);

	if (prCmdTxPCompos == NULL) {
		DBGLOG(OID, ERROR, "[TXPSTAT] prCmdTxPCompos Null\n");
		return WLAN_STATUS_FAILURE;
	}

	kalMemSet(prCmdTxPCompos, 0, u4Size);
	prCmdTxPCompos->ucTag = TX_POWER_COMPOSITION;
	prCmdTxPCompos->ucNum = ucNum;
	kalMemCopy(prCmdTxPCompos->aucBuffer, pvQueryBuffer, u4QueryBufferLen);
	...
	rWlanStatus = wlanSendSetQueryExtCmd(prAdapter,
			     ...
			     TRUE,
			     TRUE,
			     nicCmdEventQueryTxPowerComp,
			     nicOidCmdTimeoutCommon,
			     u4Size,
			     (uint8_t *) (prCmdTxPCompos),
			     pvQueryBuffer,
			     u4QueryBufferLen);

	kalMemFree(prCmdTxPCompos, VIR_MEM_TYPE, u4Size);

	return rWlanStatus;

}

uint32_t wlanoidCountryTxPowerControl(struct ADAPTER *prAdapter,
			       void *pvSetBuffer,
			       uint32_t u4SetBufferLen,
			       uint32_t *pu4SetInfoLen)
{
	...
	if (!pvSetBuffer || !prAdapter || !pu4SetInfoLen)
		return WLAN_STATUS_INVALID_DATA;

	if (u4SetBufferLen < sizeof(uint32_t))
		return WLAN_STATUS_INVALID_LENGTH;

	u4Cfg = *(uint32_t *) pvSetBuffer;

	prAdapter->rWifiVar.ucCountryPwrLimitCtrl = u4Cfg;
...
uint32_t
wlanoidGetAisConnectedChnls(struct ADAPTER *prAdapter, void *pvQueryBuffer,
		    uint32_t u4QueryBufferLen, uint32_t *pu4QueryInfoLen)
{
	...
	i = 0;
	prChnlInfo = (struct RF_CHANNEL_INFO *) pvQueryBuffer;

	for (ucBssIndex = 0; ucBssIndex < BSSID_NUM; ucBssIndex++) {
		prBssInfo = GET_BSS_INFO_BY_INDEX(prAdapter, ucBssIndex);

		if (IS_BSS_AIS(prBssInfo) && IS_BSS_ALIVE(prAdapter,
			prBssInfo)) {
			prChnlInfo[i].eBand =
				prBssInfo->eBand;

		...
		const u_int8_t bAtResetFlow)
{
	...
	mbrWifiTRxPerfFlushQueue(prAdapter);
#endif
...
void wlanSendIcsOffCmd(struct ADAPTER *ad, enum ENUM_MBMC_BN eBand)
{
	...
	ACQUIRE_POWER_CONTROL_FROM_PM(ad, DRV_OWN_SRC_ICS_OFF_CMD);
	rIcsCmd.ucModule = 2;
	...
	RECLAIM_POWER_CONTROL_TO_PM(ad, FALSE, DRV_OWN_SRC_ICS_OFF_CMD);
	if (u4Status != WLAN_STATUS_SUCCESS)
...
u_int8_t wlanProcessTxFrame(struct ADAPTER *prAdapter, void *prPacket)
{
	...
	if (kalQoSFrameClassifierAndPacketInfo(
		...
		if (IS_BSS_INDEX_WON(prAdapter,
			GLUE_GET_PKT_BSS_IDX(prPacket)))
			ucMacHeaderLen = kalQuery80211HeaderLength(prPacket);
		else
...
void wlanCfgDumpIotApRule(struct ADAPTER *prAdapter)
{
	...
	for (ucRuleIdx = 0; ucRuleIdx < CFG_IOT_AP_RULE_MAX_CNT; ucRuleIdx++) {
		...
		DBGLOG(INIT, TRACE, "IOTAP%d NSS:%X HT:%X Band:%X Act:%lX\n",
...
static uint8_t __getIotApOption(uint8_t ucTokId, uint8_t *pCurTok,
				struct WLAN_IOT_AP_RULE_T *prIotApRule)
{
	uint8_t ucParsedLen = 0;

#define PARSE_IOT_AP_U8_DEC(_pCurTok, _field)		\
({							\
	int r;						\
	r = kalkStrtou8(_pCurTok, 10, &(_field));	\
	r == 0 ? strlen(_pCurTok) : 0;			\
})

#define PARSE_IOT_AP_U64_HEX(_pCurTok, _field)		\
({							\
	int r;						\
	r = kalkStrtou64(_pCurTok, 16, &(_field));	\
	r == 0 ? strlen(_pCurTok) : 0;			\
})

#define PARSE_IOT_AP_ARRAY(_pCurTok, _field)				       \
({									       \
	ucParsedLen = wlanHexStrToByteArray(_pCurTok, _field, sizeof(_field)); \
	ucParsedLen;							       \
})

	switch (ucTokId) {
	case WLAN_IOT_AP_FG_VERSION:
		ucParsedLen = PARSE_IOT_AP_U8_DEC(pCurTok,
						  prIotApRule->ucVersion);
		break;
	case WLAN_IOT_AP_FG_OUI:
		ucParsedLen = PARSE_IOT_AP_ARRAY(pCurTok,
						 prIotApRule->aVendorOui);
		break;
	case WLAN_IOT_AP_FG_DATA:
		ucParsedLen = PARSE_IOT_AP_ARRAY(pCurTok,
						 prIotApRule->aVendorData);
		break;
	case WLAN_IOT_AP_FG_DATA_MASK:
		ucParsedLen = PARSE_IOT_AP_ARRAY(pCurTok,
						 prIotApRule->aVendorDataMask);
		break;
	case WLAN_IOT_AP_FG_BSSID:
		ucParsedLen = PARSE_IOT_AP_ARRAY(pCurTok,
						 prIotApRule->aBssid);
		break;
	case WLAN_IOT_AP_FG_BSSID_MASK:
		ucParsedLen = PARSE_IOT_AP_ARRAY(pCurTok,
						 prIotApRule->aBssidMask);
		break;
	case WLAN_IOT_AP_FG_NSS:
		ucParsedLen = PARSE_IOT_AP_U8_DEC(pCurTok,
						  prIotApRule->ucNss);
		break;
	case WLAN_IOT_AP_FG_HT:
		ucParsedLen = PARSE_IOT_AP_U8_DEC(pCurTok,
						  prIotApRule->ucHtType);
		break;
	case WLAN_IOT_AP_FG_BAND:
		ucParsedLen = PARSE_IOT_AP_U8_DEC(pCurTok,
						  prIotApRule->ucBand);
		break;
	case WLAN_IOT_AP_FG_ACTION:
		ucParsedLen = PARSE_IOT_AP_U64_HEX(pCurTok,
						   prIotApRule->u8Action);
		break;
	default:
		break;
...
#undef PARSE_IOT_AP_U8_DEC
#undef PARSE_IOT_AP_U64_HEX
#undef PARSE_IOT_AP_ARRAY
}
...
void wlanCfgLoadIotApRule(struct ADAPTER *prAdapter)
{
	...
	for (ucCnt = 0; ucCnt < CFG_IOT_AP_RULE_MAX_CNT; ucCnt++) {
		...
		kalMemZero(prIotApRule, sizeof(struct WLAN_IOT_AP_RULE_T));
		kalMemZero(aucCfgVal, WLAN_CFG_VALUE_LEN_MAX);
		kalMemZero(aucCfgKey, WLAN_CFG_KEY_LEN_MAX);
		pCurTok = &aucCfgVal[0];
		pNexTok = &aucCfgVal[0];
		kalSnprintf(aucCfgKey, WLAN_CFG_KEY_LEN_MAX, "IOTAP%d", ucCnt);
		/* Skip empty rule */
		if (wlanCfgGet(prAdapter, aucCfgKey, aucCfgVal, NULL, 0,
			       FEATURE_TO_CUSTOMER) != WLAN_STATUS_SUCCESS)
			continue;
		...
		for (ucTokId = 0; ucTokId < WLAN_IOT_AP_FG_MAX; ucTokId++) {
			...
			if (pCurTok) {
				ucStatus = __getIotApOption(ucTokId, pCurTok,
							    prIotApRule);
			} else {
				DBGLOG(INIT, TRACE,
				       "Invalid Tok IOTAP%d\n", ucCnt);
			...
			DBGLOG(INIT, TRACE,
				"IOTAP%d tok:%d Str:%s status:%d flag:0x%x\n",
				ucCnt, ucTokId, pCurTok, ucStatus,
				prIotApRule->u2MatchFlag);
...
#define INIT_INT(__FEATURE, __KEY, __VAL, __DBG) \
	...
	INIT_UINT(prWifiVar->fgMlcSupport, "EnableMlc", FEATURE_ENABLED,
		FEATURE_TO_CUSTOMER);
#endif
	INIT_UINT(prWifiVar->u4LinkStableTimeoutMs,
		"LinkStableTimeoutMs", MLO_LINK_STABLE_TIMEOUT_MS,
		FEATURE_TO_CUSTOMER);
	...
	INIT_UINT(prWifiVar->u4RegWonIfAtProbe,
		"RegWonIfAtProbe", FEATURE_ENABLED, FEATURE_DEBUG_ONLY);
	INIT_UINT(prWifiVar->ucRegWonMode,
		"RegWonMode", 5, FEATURE_TO_CUSTOMER);
	INIT_UINT(prWifiVar->fgWonNoDisconnect,
		"WonNoDisconnect", FEATURE_DISABLED, FEATURE_TO_CUSTOMER);
#endif
	...
	INIT_UINT(prWifiVar->ucApAcsChannel[0], "ApAcs2gChannel", 0,
		  FEATURE_TO_CUSTOMER);
	INIT_UINT(prWifiVar->ucFollowHostapdBw, "FollowHostapdBw",
		  FEATURE_DISABLED, FEATURE_TO_CUSTOMER);
	...
	INIT_UINT(prWifiVar->ucAp6gBandwidth, "Ap6gBw", DEFAULT_SAP_6G_BW,
		  FEATURE_TO_CUSTOMER);
#if CFG_SUPPORT_WONDERTAP
	INIT_UINT(prWifiVar->ucWon2gBandwidth, "Won2gBw", MAX_BW_20MHZ,
		  FEATURE_TO_CUSTOMER);
	INIT_UINT(prWifiVar->ucWon5gBandwidth, "Won5gBw", MAX_BW_80MHZ,
		  FEATURE_TO_CUSTOMER);
	INIT_UINT(prWifiVar->ucWon6gBandwidth, "Won6gBw", MAX_BW_320_1MHZ,
		  FEATURE_TO_CUSTOMER);
#endif
...
#if (CFG_SUPPORT_WFD == 1 && CFG_SUPPORT_WFD_1NSS_BW40_CONFIG == 1)
	INIT_UINT(prWifiVar->ucFixWfd1NssBw40, "FixWfd1NssBw40",
		 FEATURE_DISABLED, FEATURE_TO_CUSTOMER);
#endif
...
#undef __STR_MIN_RESERVED
	...
	INIT_UINT(prWifiVar->u4NanGtkCipher, "NanGtkCipher",
		  NAN_CIPHER_SUITE_ID_NONE,
		  FEATURE_TO_CUSTOMER);
	INIT_UINT(prWifiVar->ucNanMulticastAddrOverride, "NanMcAddrOverride", 0,
		  FEATURE_TO_CUSTOMER);
	INIT_STR(prWifiVar->aucNanMulticastAddrStr,
		  "NanMulticastMacAddr",
		  "33:33:00:00:00:01",
		  FEATURE_TO_CUSTOMER);
#endif /* CFG_NAN_SUPPORT_R4_GROUP_ADDR_FRAME_PROT */
#if (CFG_SUPPORT_NAN_R4_PAIRING == 1)
	INIT_UINT(prWifiVar->u4NanPairingCipher, "NanPairingCipher",
		  NAN_CIPHER_SUITE_ID_NONE,
		  FEATURE_TO_CUSTOMER);
	INIT_UINT(prWifiVar->u4NanNikLifetime, "NanNikLifetime",
		  0,
		  FEATURE_TO_CUSTOMER);
	INIT_UINT(prWifiVar->u4NanGroupKdeInFollowup, "NanGroupKdeInFollowup",
		  0,
		  FEATURE_TO_CUSTOMER);
...
#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_FRAME_PROT == 1)
	INIT_UINT(prWifiVar->ucNanGroupSecCap, "NanSecCap",
		  CSIA_CAP_GTKSA_IGTKSA_BIGTKSA_UNSUPPORTED,
		  FEATURE_TO_CUSTOMER);
	INIT_UINT(prWifiVar->ucNanIgtkCipher, "NanIgtkCs", 0,
		  FEATURE_TO_CUSTOMER);
#endif /* CFG_NAN_SUPPORT_R4_GROUP_ADDR_FRAME_PROT */
	...
	INIT_UINT(prWifiVar->ucDftNdcStartOffset, "NanDftNdcStartOffset", 0,
		  FEATURE_TO_CUSTOMER);
	INIT_UINT(prWifiVar->ucNanMinNdpCrbSlots, "NanMinNdpCrbSlots", 16,
		  FEATURE_TO_CUSTOMER);
	...
	INIT_UINT(prWifiVar->fgNanMccUlwEnabled, "NanMccUlw", 0,
		  FEATURE_TO_CUSTOMER);
	...
	INIT_UINT(prWifiVar->ucNanVendorIoctl, "NanVendorIoctl", 0,
		  FEATURE_TO_CUSTOMER);
	INIT_UINT(prWifiVar->u2NanDiscBcnInterval, "NanDiscBcnInterval", 100,
		  FEATURE_TO_CUSTOMER);
	 ...
	 * [3] 520: 00-06-00-00
	 */
	INIT_UINT(prWifiVar->u4NdpCustomThreshold[0],
		"NdpCustomThreshold_0", 0, FEATURE_TO_CUSTOMER);
	INIT_UINT(prWifiVar->u4NdpCustomThreshold[1],
		"NdpCustomThreshold_1", 100, FEATURE_TO_CUSTOMER);
	INIT_UINT(prWifiVar->u4NdpCustomThreshold[2],
		"NdpCustomThreshold_2", 200, FEATURE_TO_CUSTOMER);
	INIT_UINT(prWifiVar->u4NdpCustomThreshold[3],
		"NdpCustomThreshold_3", 520, FEATURE_TO_CUSTOMER);

	INIT_UINT(prWifiVar->u4NdpCustomBitmap[0],
		"NdpCustomBitmap_0", 0xffffffff, FEATURE_TO_CUSTOMER);
	INIT_UINT(prWifiVar->u4NdpCustomBitmap[1],
		"NdpCustomBitmap_1", 0x44104210, FEATURE_TO_CUSTOMER);
	INIT_UINT(prWifiVar->u4NdpCustomBitmap[2],
		"NdpCustomBitmap_2", 0x0060060c, FEATURE_TO_CUSTOMER);
	INIT_UINT(prWifiVar->u4NdpCustomBitmap[3],
		"NdpCustomBitmap_3", 0x00000600, FEATURE_TO_CUSTOMER);
	 ...
	 */
	INIT_UINT(prWifiVar->eNanRangingBand, "NanRangingBand", BAND_5G,
		FEATURE_TO_CUSTOMER);
	...
	INIT_UINT(prWifiVar->fgNanNdpSkipNmiKey,
		"NanNdpSkipNmiKey", 0, FEATURE_TO_CUSTOMER);

#if (defined(CFG_SUPPORT_NAN_R4_PAIRING) && CFG_SUPPORT_NAN_R4_PAIRING == 1)
	INIT_UINT(prWifiVar->ucNanEnablePairing,
		"NanEnablePairing", FEATURE_ENABLED, FEATURE_TO_CUSTOMER);
	wlanCfgSetUint32(prAdapter, "NanEnablePairing",
		prWifiVar->ucNanEnablePairing);
	INIT_UINT(prWifiVar->u2NanPairingComeback,
		"NanPairingComeback", 0, FEATURE_TO_CUSTOMER);
	INIT_UINT(prWifiVar->u2NanPairingCbMethod,
		"NanPairingCbMethod", 0x02, FEATURE_TO_CUSTOMER);
	INIT_UINT(prWifiVar->ucNanPairingNikExAtVerify,
		"NanPairingNikExAtVerify",
		FEATURE_ENABLED, FEATURE_TO_CUSTOMER);
#endif /* CFG_SUPPORT_NAN_R4_PAIRING */
	...
	INIT_UINT(prWifiVar->fgNanCustDwChnl, "NanCustDwChnl", 0,
		FEATURE_TO_CUSTOMER);
	wlanCfgSetUint32(prAdapter, "NanCustDwChnl",
		prWifiVar->fgNanCustDwChnl);
	...
	INIT_UINT(prWifiVar->u4MlrCfg, "MlrCfg", 0, FEATURE_TO_CUSTOMER);
	INIT_UINT(prWifiVar->fgEnMlrUevent,
		"EnMlrUevent", FEATURE_DISABLED, FEATURE_TO_CUSTOMER);
	INIT_UINT(prWifiVar->fgMlrDisSingleLink2g,
		"MlrDisSingleLink2g", FEATURE_DISABLED, FEATURE_TO_CUSTOMER);
	...
	INIT_UINT(prWifiVar->fgStaPmkCheckDeauth,
		"StaPmkCheckDeauth", FEATURE_ENABLED, FEATURE_TO_CUSTOMER);
	...
	INIT_UINT(prWifiVar->u4TxDataDelayCnt, "TxDataDelayCnt", 10,
		  FEATURE_TO_CUSTOMER);
	INIT_UINT(prWifiVar->fgEnFwCtrlTxDataDelay,
		  "EnFwCtrlTxDataDelay", FEATURE_ENABLED, FEATURE_TO_CUSTOMER);
	...
	INIT_UINT(prWifiVar->u4DlyIntTime, "DlyIntTime", 5,
		FEATURE_TO_CUSTOMER); /* unit: 20us */
	/* 0: No check by count */
...
#ifdef SOC3_0
	INIT_UINT(prWifiVar->fgUseOneTxRing, "UseOneTxRing",
		  FEATURE_DISABLED, FEATURE_TO_CUSTOMER);
#endif

	kalReviseCfgsByPlatform(prAdapter);

	INIT_UINT(prWifiVar->fgDisCnmTBTC, "DisCnmTBTC",
		  FEATURE_DISABLED, FEATURE_TO_CUSTOMER);
	INIT_UINT(prWifiVar->fgEnCnmSTAP2PTBTC, "EnCnmSTAP2PTBTC",
		  FEATURE_DISABLED, FEATURE_TO_CUSTOMER);
	INIT_UINT(prWifiVar->fgDisCnmSTAWFDTBTC, "DisCnmSTAWFDTBTC",
		  FEATURE_DISABLED, FEATURE_TO_CUSTOMER);
	INIT_UINT(prWifiVar->fgDisCnmDualSTATBTC, "DisCnmDualSTATBTC",
		  FEATURE_DISABLED, FEATURE_TO_CUSTOMER);
	INIT_UINT(prWifiVar->fgDisBn2UNII2C, "DisBn2UNII2C",
		  FEATURE_DISABLED, FEATURE_TO_CUSTOMER);
#if (CFG_TC10_FEATURE == 1)
	INIT_UINT(prWifiVar->ucSarMethod, "SarMethod", 0);
	INIT_UINT(prWifiVar->ucSarLmtCfgVer, "SarLmtCfgVer", 1);
#endif
	    ...
	    uint32_t u4Flags, enum ENUM_FEATURE_SUPPORT_SCOPE fgIsDebugUsed)
{
	...
	// Miui Add: CFG_RM_WIFI_LIMIT to remove userload limit
	if (u4Flags == WLAN_CFG_DEFAULT && !BUILD_QA_DBG && !CFG_RM_WIFI_LIMIT
		&& fgIsDebugUsed) {
		/* do not pass to FW */
...
__KAL_NOINLINE__ void wlanCfgRecordValue(struct ADAPTER *prAdapter,
	...
	uint32_t u4ValueDef, enum ENUM_FEATURE_SUPPORT_SCOPE fgIsDebugUsed)
{
	...
	// Miui Add: CFG_RM_WIFI_LIMIT to remove userload limit
	if (!BUILD_QA_DBG && !CFG_RM_WIFI_LIMIT && fgIsDebugUsed) {
		/* do not pass to FW */
		wlanCfgSet(prAdapter, pucKey, NULL, WLAN_CFG_DEFAULT);
		goto done;
	}
 ...
 *
 * Return: strlen of hexString, excluding '\0'
 */
...
u_int8_t wlanIsChipNoAck(struct ADAPTER *prAdapter)
{
	...
	if (!fgIsNoAck) {
		...
		if (!kalIsResetting() && fgIsWarningTriggered == FALSE) {
			++u4ChipNoAckCnt;
	...
	case TX_PROF_TAG_DRV_TX_DONE:
		if (prPktProfile->fgIsValid) {
			...
			if (prWifiVar->fgPacketLatencyLog)
				DBGLOG(TX, DEBUG,
				     "Latency(us) HIF_D:%llu, DEQ_D:%llu, ENQ_D:%llu A:%llu BSSIDX:WIDX:PID[%u:%u:%u] IPID:0x%04x SeqNo:%d\n",
				     NSEC_TO_USEC(prPktProfile->u8HifTxTime -
						  prPktProfile->u8XmitArrival),
				     NSEC_TO_USEC(prPktProfile->u8DeqTime -
						  prPktProfile->u8XmitArrival),
				     NSEC_TO_USEC(prPktProfile->u8EnqTime -
						  prPktProfile->u8XmitArrival),
				     NSEC_TO_USEC(prPktProfile->u8XmitArrival),
				     prMsduInfo->ucBssIndex,
				     prMsduInfo->ucWlanIndex,
				     prMsduInfo->ucPID,
			...
			halAddTxLatencyCount(prAdapter, DRIVER_TX_DELAY,
				prMsduInfo->ucBssIndex,
				NSEC_TO_MSEC(prPktProfile->u8HifTxTime -
					     prPktProfile->u8XmitArrival));
#endif
	...
	case TX_PROF_TAG_ACQR_MSDU_TOK:
		if (prPktProfile->fgIsValid) {
			...
			halAddTxLatencyCount(prAdapter, DRIVER_HIF_TX_DELAY,
				prMsduInfo->ucBssIndex,
				NSEC_TO_MSEC(prPktProfile->u8HifAcqrMsduTime -
...
#if CFG_ENABLE_PKT_LIFETIME_PROFILE
#if CFG_SUPPORT_TX_LATENCY_STATS
	u4DelayXmitToHif = NSEC_TO_USEC(prMsduInfo->rPktProfile.u8HifTxTime -
					prMsduInfo->rPktProfile.u8XmitArrival);
	u4DelayHifToDone = NSEC_TO_USEC(u8Now -
...
#endif
	 ) && (p2pFuncIsFixWfd1NssBw40(prAdapter, prBssInfo))) {
		DBGLOG(SW4, INFO, "Use 1x1 due to miracast mode\n");
		ucRetValNss = 1;
	}
#endif
			 ...
			 uint8_t ucLimitMcsIdx)
{
	...
	if (txmode == TX_RATE_MODE_CCK) { /* 11B */
		...
		ucMaxRateIdx = MCS_IDX_MAX_RATE_EHT;
		if (ucLimitMcsIdx < ucMaxRateIdx)
			ucMaxRateIdx = ucLimitMcsIdx;
		u4CurRate = g_rAxDataRateMappingTable.nsts[nsts - 1]
				.bw[frmode].gi[gi].rate[rate];
		 ...
		 uint32_t *pu4CurRate, uint32_t *pu4MaxRate)
{
	...
	*pu4MaxRate = 0;
	...
	if (prBssInfo->eHwBandIdx == ENUM_BAND_2) {
		/* In common case, use supported max NSS;
		 * band2 native capability is limited
		 */
		if (ucNss > prBssInfo->ucOpTxNss)
			ucNss = prBssInfo->ucOpTxNss;

#if (CFG_SUPPORT_CONNAC3X == 1)
		if (ucTxMode == TX_RATE_MODE_EHT_MU)
			ucMaxRateIdx = MCS_IDX_MAX_RATE_HE;
#endif
	}
#endif
	...
	DBGLOG(SW4, TRACE,
	       "txmode=[%u], mcs idx=[%u], bandwidth=[%u], sgi=[%u], nsts=[%u], limit=[%u]\n",
	       ucTxMode, ucMcsIdx, ucBw, ucSgi, ucNss, ucMaxRateIdx);

	/* aucBWConvertTable convert driver bw to Phy bw type
	 * [driver bw enum ENUM_MAX_BANDWIDTH]
	 * 0:20M, 1:40M, 2:80M, 3:160M, 4:80M+80M, 5:320M, 6:320M
	 * [Phy bw type]
	 * 0:20M, 1:40M, 2:80M, 3:160M/80M+80M, 4:320M, 5:320M
	 * If BW get from Driver enum and more than idx 4,
	 * it needs to covert Phy bw type.
	 */
	if (ucBw < MAX_BW_NUM)
		ucRxvBw = aucBwConvertTable[ucBw];
	else
		goto errhandle;

	if (wlanQueryRateByTable(ucTxMode, ucMcsIdx, ucRxvBw,
		ucSgi, ucNss, pu4CurRate, pu4MaxRate, ucMaxRateIdx) < 0)
		goto errhandle;
...
#if CFG_SUPPORT_LINK_QUALITY_MONITOR
static u_int8_t wlanGetBssIdxByPrimaryInterface(
	struct GLUE_INFO *prGlueInfo, uint8_t *pucBssIndex)
{
...
uint32_t wlanLinkQualityMonitor(struct GLUE_INFO *prGlueInfo, bool bFgIsOid)
{
	...
	if (wlanGetBssIdxByPrimaryInterface(prGlueInfo, &ucBssIndex)) {
		DBGLOG(NIC, TRACE, "Select BssIdx:%u\n", ucBssIndex);
		prBssInfo = GET_BSS_INFO_BY_INDEX(prAdapter, ucBssIndex);
	}
	...
	DBGLOG(NIC, TRACE, "LastLinkQuality:%u BssIdx:%u\n",
			prAdapter->u4LastLinkQuality, ucBssIndex);
...
void wlanFinishCollectingLinkQuality(struct GLUE_INFO *prGlueInfo)
{
	...
	/* get current trx rate with primary interface*/
	if (wlanGetBssIdxByPrimaryInterface(prGlueInfo, &ucBssIndex)) {
		struct LINK_SPEED_EX_ *prLq;

		prLq = &prAdapter->rLinkQuality.rLq[ucBssIndex];
		if (prLq->u2TxLinkSpeed != 0) {
			prLinkQualityInfo->u4CurTxRate =
						prLq->u2TxLinkSpeed;
			do_div(prLinkQualityInfo->u4CurTxRate, 1000);
		}

		if (wlanGetRxRateByBssid(prGlueInfo, ucBssIndex,
...
uint32_t
wlanSetFixRate(struct ADAPTER *prAdapter,
		      void *pvSetBuffer,

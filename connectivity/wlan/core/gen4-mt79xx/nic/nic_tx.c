	...
	if (prAdapter->fgIsSupportCsumOffload
		...
		if (prAdapter->u4CSUMFlags &
			...
			if (prMsduInfo->ucPktType == ENUM_PKT_IP_FRAG
				...
				ucChksumFlag &=
					(~(TX_CS_IP_GEN | TX_CS_TCP_UDP_GEN));
#endif
			     ...
			     IN struct WIFI_EVENT *prEvent)
{
	...
	if (prTxDone->ucStatus >= TX_RESULT_NUM) {
		DBGLOG(NIC, ERROR, "TxStatus out of range: %u!\n",
			prTxDone->ucStatus);
		return;
	}
...
/* fos_change begin */
	...
	if (prTxDone->ucFlag & BIT(TXS_WITH_ADVANCED_INFO)) {
		...
		if (prTxDone->ucFlag & BIT(TXS_IS_EXIST)) {
			...
			if (prTxDone->ucBandwidth >= sizeof(apucBandwidt) /
				sizeof(apucBandwidt[0])) {
				DBGLOG(NIC, ERROR,
					"TxBandwidth out of range: %u!\n",
...
void nicTxSetWtblEntry(struct MSDU_INFO *prMsduInfo,
			 uint8_t ucWtblEntry)
{
	prMsduInfo->ucWtblEntry = ucWtblEntry;
	prMsduInfo->u4Option |= MSDU_OPT_MANUAL_WTBL_ENTRY;
}
		...
		uint8_t ucBssIndex)
{
	...
	if (prSkb) {
		if (ppe_hook_tx_wifi != NULL) {
			DBGLOG(TX, LOUD,
				"ppe_hook_tx_wifi exist call ppe_hook_tx_wifi\n");
			if (ppe_hook_tx_wifi(prSkb, 0)) {
				DBGLOG(TX, LOUD,
					"ppe_hook_tx_wifi return 1,the skb can be sent out\n");
			} else {
				DBGLOG(TX, LOUD,
					"ppe_hook_tx_wifi return 0,drop the skb, only used to keep alive\n");
				dev_kfree_skb(prSkb);
				return WLAN_STATUS_SUCCESS;
			}
		} else {
			DBGLOG(TX, LOUD,
				"ppe_hook_tx_wifi NULL, do original tx behavior\n");
		}
	}
#endif
			      ...
			      struct GLUE_INFO *prGlueInfo)
{
	...
	if (prSkb) {
		...
		if (prMsduInfo == NULL) {
			DBGLOG_LIMITED(TX, ERROR, "cnmPktAlloc NULL\n");

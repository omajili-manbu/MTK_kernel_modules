	...
	IN struct MSDU_INFO *prMsduInfo)
{
	...
	if (prBssInfo->eCurrentOPMode == OP_MODE_NAN) {
		prTempStaRec = nanDataEngineSearchStaRec(prAdapter,
			NULL, prMsduInfo->aucEthSrcAddr,
			prMsduInfo->aucEthDestAddr);
		if (prTempStaRec) {
		...
		if (prCurrSwRfb->fgHdrTran) {
			...
			if (prCurrSwRfb->prStaRec == NULL) {
				...
				HAL_RX_STATUS_GET_TA(
					...
					aucTaAddr);
#if (CFG_SUPPORT_NAN == 1)
				if (nanRxLookforStaRec(prAdapter,
					prCurrSwRfb,
#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_DATA == 1)
					&g_rNanMcDataRxTestPath,
	...
	IN struct WIFI_EVENT *prEvent)
{
	...
	if (!IS_BSS_INDEX_VALID(prEventBssStatus->ucBssIndex)) {
		DBGLOG(QM, WARN, "NAF:BSS IDX is invalid\n");
		return;
	}
			  ...
			  struct SW_RFB *prSwRfb)
{
	...
	/* Key ID 4 ~ 7 are used for BIP MGMT.
	 * We can skip data frame replay detection
	 */
	if (ucKeyID >= MAX_PGTK_KEY_NUM) {
		DBGLOG(QM, ERROR, "KeyID[%u] oversize[%u]\n",
			ucKeyID, MAX_PGTK_KEY_NUM);
		return FALSE;
	}

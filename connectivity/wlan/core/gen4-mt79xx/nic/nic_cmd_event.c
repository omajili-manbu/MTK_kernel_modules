...
#define FMCW_BURST_TX_DATA_OFFSET  100
#endif
...
const struct NIC_CAPABILITY_V2_REF_TABLE
	...
	NIC_FILL_CAP_V2_REF_TBL(TAG_CAP_NAN_FEATURE_SET_MOBILE_CE,
				nicCfgChipCapNAN),
#endif
};
	...
	IN struct CMD_INFO *prCmdInfo, IN uint8_t *pucEventBuf)
{
	...
	if (prCmdInfo->u4SetInfoLen >= OFFSET_OF(
			   struct CMD_IPV6_NETWORK_ADDRESS_LIST,
			   arNetAddress)) {
		u4Count = (prCmdInfo->u4SetInfoLen - OFFSET_OF(
				struct CMD_IPV6_NETWORK_ADDRESS_LIST,
				arNetAddress))
			/ sizeof(struct CMD_IPV6_NETWORK_ADDRESS_LIST);
	}
...
#if CFG_SUPPORT_FMCW_RADAR
void nicCmdEventQueryRfTestFmcwATInfo(IN struct ADAPTER
				  *prAdapter, IN struct CMD_INFO *prCmdInfo,
				  IN uint8_t *pucEventBuf)
{
	union EVENT_FMCW_TEST_STATUS *prTestStatus, *prQueryBuffer;
	struct GLUE_INFO *prGlueInfo;
	uint32_t u4QueryInfoLen;
	...
	prTestStatus = (union EVENT_FMCW_TEST_STATUS *) pucEventBuf;

	if (prCmdInfo->fgIsOid) {
		prGlueInfo = prAdapter->prGlueInfo;
		prQueryBuffer = (union EVENT_FMCW_TEST_STATUS *)
				prCmdInfo->pvInformationBuffer;
		...
		u4QueryInfoLen = sizeof(union EVENT_FMCW_TEST_STATUS);
...
void nicCmdEventFmcwTestFlowCtrl(IN struct ADAPTER *prAdapter,
		IN struct CMD_INFO *prCmdInfo, IN uint8_t *pucEventBuf)
{
	struct RADAR_TEST_FLOW_CTRL *prCmdRadarInfo = NULL;
	struct RADAR_TEST_FLOW_CTRL *prInfoEvent = NULL;
	struct GLUE_INFO *prGlueInfo = NULL;
	uint32_t u4QueryInfoLen;

	if (!prAdapter || !prAdapter->prGlueInfo) {
		DBGLOG(NIC, ERROR, "NULL prAdapter or NULL prGlueInfo!\n");
		return;
	}
	...
	if (prCmdInfo->fgIsOid) {
		prCmdRadarInfo = (struct RADAR_TEST_FLOW_CTRL *)
			   prCmdInfo->pvInformationBuffer;
		prInfoEvent = (struct RADAR_TEST_FLOW_CTRL *)pucEventBuf;

		kalMemCopy(prCmdRadarInfo, prInfoEvent,
			   sizeof(struct RADAR_TEST_FLOW_CTRL));

		prGlueInfo = prAdapter->prGlueInfo;
		u4QueryInfoLen = sizeof(struct RADAR_TEST_FLOW_CTRL);
...
void nicOneTimeCalSendCmdDoneHandler(struct ADAPTER *prAdapter,
	struct CMD_INFO *prCmdInfo,
	uint8_t *pucEventBuf)
{
	struct ONE_TIME_CAL_BLK_INFO *prCmdBlkInfo;
	struct ONE_TIME_CAL_SEND_CTX *prctx =
		(struct ONE_TIME_CAL_SEND_CTX *) prCmdInfo->pvInformationBuffer;
	if (!prctx) {
		DBGLOG(NIC, ERROR,
			"One time cal done Send handler get null context!\n");
		return;
	}

	/* Advance to next fragment or finish */
	prctx->ucFragCtr++;
	if (prctx->ucFragCtr < prctx->ucFragSeqNum) {
		rlmSendFWOneTimeCalData_next(prctx);
		return;
	}
	prctx->ucBlkCtr++;
	if (prctx->ucBlkCtr < prctx->ucBlkSeqNum[prctx->u4SeqIdx]) {
		prctx->ucFragCtr = 0;
		/* Get new frag count for next block */
		prCmdBlkInfo = &prctx->rCmdBlk.rOneTimeCalBlkInfo;
		prCmdBlkInfo->ucCalType = prctx->eCalType;
		prCmdBlkInfo->u4CalParam = prctx->u4CalParam;
		prCmdBlkInfo->ucBlkSeqNum =
			(prctx->ucBlkSeqNum[prctx->u4SeqIdx] << 4) +
			prctx->ucBlkCtr;
	...
	/* Finished this cal type, check if more in sequence */
	prctx->u4SeqIdx++;
	if (prctx->u4SeqIdx < prctx->u4SeqTotal) {
		rlmSendFWOneTimeCalData_Sequence_next(prctx);
		return;
	}
	kalMemFree(prctx, VIR_MEM_TYPE, sizeof(struct ONE_TIME_CAL_SEND_CTX));
	return;
}

void nicOneTimeCalSendCmdTimeOutHandler(struct ADAPTER *prAdapter,
	struct CMD_INFO *prCmdInfo)
{
	struct ONE_TIME_CAL_SEND_CTX *prctx =
		(struct ONE_TIME_CAL_SEND_CTX *) prCmdInfo->pvInformationBuffer;
	...
	if (prctx) {
		kalMemFree(prctx,
			VIR_MEM_TYPE, sizeof(struct ONE_TIME_CAL_SEND_CTX));
	} else
		DBGLOG(NIC, ERROR,
			"One time cal Timeout handler get null context!\n");

	DBGLOG(NIC, ERROR, "rlmSendFWOneTimeCalData Timeout!\n");
		  ...
		  uint32_t u4GetWf1)
{
	...
	*prIQAry = prAdapter->rIcapInfo.au4IQData;
	...
	kalSnprintf(aucPath, 50, "/tmp/dump_out_%05hu_WF%u.txt",
	...
	for (u4SrcOffset = 0,
		...
		if (prEventDumpMem->eIcapContent == ICAP_CONTENT_FIIQ ||
			...
			u4DataWLenF1 = kalScnprintf(pucDataWF1, u4DataWBufSize,
			   ...
			   ICAP_CONTENT_FIIQ + 1000
			   || prEventDumpMem->eIcapContent ==
			   ICAP_CONTENT_FDIQ + 1000) {
			u4Iqc160WF0Q0 =
			  ...
			  IN uint8_t *pucEventBuf)
{
	...
	if (prEventDumpMem->u4RemainLength == 0
		...
		kalSnprintf(aucPath_done, 300, "%s", "/file_dump_done.txt");
		if (kalCheckPath(aucPath_done) == -1) {
			kalMemSet(aucPath_done, 0x00, 256);
			kalSnprintf(aucPath_done, 300, "%s",
...
#define _PATH_LEN 256
	static uint8_t aucPath[_PATH_LEN];
	...
	if (1) {
		...
		if (prEventDumpMem->ucFragNum == 1) {
			 ...
			 * the dump files wouls put on it
			 */
			iRet += snprintf(aucPath, _PATH_LEN, "/dump_%05hu.hex",
				prAdapter->rIcapInfo.u2DumpIndex);
			if (kalCheckPath(aucPath) == -1) {
				kalMemSet(aucPath, 0x00, 256);
				iRet += snprintf(aucPath, _PATH_LEN,
					"/data/dump_%05hu.hex",
					prAdapter->rIcapInfo.u2DumpIndex);
			} else
				   ...
				   IN uint8_t *pucEventBuf)
{
	...
	if (prEvent->u4Flags & BIT(0)) {
		...
		for (ucDbdcIdx = 0; ucDbdcIdx < ENUM_BAND_NUM; ucDbdcIdx++) {
			prAdapter->arMibInfo[ucDbdcIdx].u4RxMpduCnt +=
				prStaStatistics->rMibInfo[ucDbdcIdx].
				u4RxMpduCnt;
			prAdapter->arMibInfo[ucDbdcIdx].u4FcsError +=
				prStaStatistics->rMibInfo[ucDbdcIdx].
				u4FcsError;
			prAdapter->arMibInfo[ucDbdcIdx].u4RxFifoFull +=
				prStaStatistics->rMibInfo[ucDbdcIdx].
				u4RxFifoFull;
			prAdapter->arMibInfo[ucDbdcIdx].u4AmpduTxSfCnt +=
				prStaStatistics->rMibInfo[ucDbdcIdx].
				u4AmpduTxSfCnt;
			prAdapter->arMibInfo[ucDbdcIdx].u4AmpduTxAckSfCnt +=
				prStaStatistics->rMibInfo[ucDbdcIdx].
				u4AmpduTxAckSfCnt;

			for (ucIdx = 0; ucIdx <= AGG_RANGE_SEL_NUM; ucIdx++)
				prAdapter->
				arMibInfo[ucDbdcIdx].au2TxRangeAmpduCnt[ucIdx]
					+= prStaStatistics->rMibInfo[ucDbdcIdx].
...
#if CFG_SUPPORT_NAN
	prAdapter->rWifiVar.ucNanNSS = (!prPhyCap->ucDbdc) ? 2 : 1;
#if CFG_SUPPORT_CFG_FILE
	wlanCfgSetUint32(prAdapter, "NanNss",
...
#if (CFG_SUPPORT_NAN == 1)
uint32_t nicCfgChipCapNAN(IN struct ADAPTER *prAdapter,
	IN uint8_t *pucEventBuf)
{
	struct CAP_NAN_FEATURE_T *prNanCap =
		(struct CAP_NAN_FEATURE_T *)pucEventBuf;
	...
	if (prAdapter->rWifiVar.fgEnNanPairing) {
		if (prNanCap->u4NanFeatureSet &
			BIT(ENUM_NAN_FEATURE_R4_PAIRING)) {
			prAdapter->u8NanCardCap |=
				WLAN_CAPABILITIES_NAN_PAIRING;
		} else {
			prAdapter->u8NanCardCap &=
				~WLAN_CAPABILITIES_NAN_PAIRING;
			prAdapter->rWifiVar.fgEnNanPairing = FALSE;
		}
	}
	...
	if (prAdapter->rWifiVar.fgEnNanKeyMgmt) {
		if (prNanCap->u4NanFeatureSet &
			BIT(ENUM_NAN_FEATURE_KEY_MANAGEMENT)) {
			prAdapter->u8NanCardCap |=
				WLAN_CAPABILITIES_NAN_KEY_MANAGEMENT;
		} else {
			prAdapter->rWifiVar.fgEnNanKeyMgmt = FALSE;
		}
	}

#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_DATA == 1)
	if (prNanCap->u4NanFeatureSet &
		BIT(ENUM_NAN_FEATURE_R4_MC_DATA)) {
		prAdapter->u8NanCardCap |=
	...
	if ((prAdapter->rWifiVar.ucNanGroupSecCap ==
		    CSIA_CAP_GTKSA_IGTKSA_SUPPORTED_BIGTKSA_UNSUPPORTED) ||
		  (prAdapter->rWifiVar.ucNanGroupSecCap ==
		    CSIA_CAP_GTKSA_IGTKSA_BIGTKSA_SUPPORTED)) {
		if (prNanCap->u4NanFeatureSet &
			BIT(ENUM_NAN_FEATURE_R4_GROUP_KEY)) {
			prAdapter->u8NanCardCap |=
				WLAN_CAPABILITIES_NAN_GROUP_KEY;
		} else {
			prAdapter->rWifiVar.ucNanGroupSecCap =
				CSIA_CAP_GTKSA_IGTKSA_BIGTKSA_UNSUPPORTED;
		}
	}
#endif /* CFG_NAN_SUPPORT_R4_GROUP_ADDR_FRAME_PROT */

	DBGLOG(INIT, INFO, "NanCardCapability=%x\n",
		prAdapter->u8NanCardCap);
	...
	for (u4SrcOffset = 0,
		...
		if (prIcap->u4CapNode == ICAP_CONTENT_FIIQ
			...
			u4DataWLenF1 = kalScnprintf(pucDataWF1, u4DataWBufSize,
				...
				icapBusData.rIqcBusData.u4Iqc1I,
				icapBusData.rIqcBusData.u4Iqc1Q);
		} else if (prIcap->u4CapNode == ICAP_CONTENT_FIIQ + 1000 ||
			   prIcap->u4CapNode == ICAP_CONTENT_FDIQ + 1000) {
			...
			u4DataWLenF1 = kalScnprintf(pucDataWF1, u4DataWBufSize,
				...
				icapBusData.rPackedAdcBusData.u4AdcI1T5,
				icapBusData.rPackedAdcBusData.u4AdcQ1T5);
		} else if (prIcap->u4CapNode == ICAP_CONTENT_ADC + 2000) {
			u4DataWLenF0 = kalScnprintf(pucDataWF0, u4DataWBufSize,
				"%8d,%8d\n%8d,%8d\n%8d,%8d\n",
				icapBusData.rPackedAdcBusData.u4AdcI0T0,
				icapBusData.rPackedAdcBusData.u4AdcQ0T0,
			   ...
			   IN uint8_t *pucEventBuf)
{
	...
	if (prPhyIcsEvent->u4DataLen > MAX_PHY_ICS_DUMP_DATA_CNT) {
		DBGLOG(RFTEST, ERROR,
			"u4DataLen %u out of valid event length\n",
			     ...
			     IN uint8_t *pucEventBuf)
{
	...
	kalSnprintf(aucPath, 256, "/dump_%05hu.hex",
		prAdapter->rIcapInfo.u2DumpIndex);
	...
	if (kalCheckPath(aucPath) == -1) {
		kalMemSet(aucPath, 0x00, 256);
		kalSnprintf(aucPath, 256, "/data/dump_%05hu.hex",
			prAdapter->rIcapInfo.u2DumpIndex);
	}
			    ...
			    IN struct WIFI_EVENT *prEvent)
{
	...
	case EXT_EVENT_ID_EFUSE_ACCESS:
	{
		...
		case EXT_EVENT_ID_SECEFUSE_ACCESS:
		{
			struct EVENT_ACCESS_SECEFUSE *prEventEfuseAccess;
			uint32_t i;

			u4QueryInfoLen = sizeof
				(struct PARAM_CUSTOM_ACCESS_SECEFUSE);
			prCmdInfo = nicGetPendingCmdInfo(prAdapter,
							 prEvent->ucSeqNum);
			prEventEfuseAccess = (struct EVENT_ACCESS_SECEFUSE *) (
							 prEvent->aucBuffer);

			for (i = 0; i < 32; i++)
				DBGLOG(NIC, INFO, "aucData[0x%x]:0x%x\n",
					i, prEventEfuseAccess->aucData[i]);

			kalMemCopy(prAdapter->aucEepromVaule,
				   prEventEfuseAccess->aucData, 32);

			DBGLOG(NIC, INFO, "u4Valid:0x%x\n",
				prEventEfuseAccess->u4Valid);
			prAdapter->aucEepromValid = prEventEfuseAccess->u4Valid;
			break;
		}
#endif
...
#if CFG_WOW_SUPPORT
	if ((prAdapter->rWifiVar.ucSchedScanDelayWakeup) &&
		(prAdapter->rWowCtrl.ucReason ==
			ENUM_PF_CMD_TYPE_SCHED_SCN_MATCH) &&
...
static void handleBtmForRoamingEnhance(
	...
	IN struct BSS_INFO *prBssInfo)
{
	...
	kalIndicateCqmBeaconLossNotify(prAdapter->prGlueInfo,
		...
		if (IS_BSS_AIS(prBssInfo)) {
			if (nicBeaconTimeoutFilterPolicy(prAdapter,
				...
				if (prAdapter->prGlueInfo->
					u4RoamingEnhanceEnable) {
					prBssInfo->u2BeaconTimeoutReason =
						prEventBssBeaconTimeout->
						ucReasonCode;
					handleBtmForRoamingEnhance(
						prAdapter,
						prBssInfo);
				} else
#endif
...
nicEventCSIData(IN struct ADAPTER *prAdapter, IN struct WIFI_EVENT *prEvent)
{
	...
	if (u4EventLen > CSI_EVENT_MAX_SIZE ||
		u4EventLen < sizeof(struct CSI_TLV_ELEMENT)) {
	...
	if (!prCSIInfo->prCsiInfoData) {
		DBGLOG(NIC, ERROR, "prCsiInfoData is NULL\n");
		return;
	}
#endif
	...
	if (prCSITlvData->body_len > (u4EventLen - u2Offset)) {
		DBGLOG(NIC, WARN,
			"[CSI] Invalid body_len: %u, tag_type=%u\n",
			prCSITlvData->body_len, prCSITlvData->tag_type);
		goto out;
	}
	...
	if (prCSIInfo->ucValue1[CSI_CONFIG_OUTPUT_FORMAT] ==
		...
		!prCSIData->bIsCck) {
#if CFG_CSI_DYNAMIC_BUFFER
		kalMemCopy(prCSIInfo->prCsiInfoData->ai2TempIData,
			prCSIData->ac2IData,
			sizeof(int16_t) * prCSIData->u2DataCount);
		kalMemCopy(prCSIInfo->prCsiInfoData->ai2TempQData,
			prCSIData->ac2QData,
			sizeof(int16_t) * prCSIData->u2DataCount);
		wlanShiftCSI(prCSIData->ucRxMode,
			prCSIData->ucBw, prCSIData->ucDataBw,
			prCSIData->ucPrimaryChIdx,
			prCSIInfo->prCsiInfoData->ai2TempIData,
			prCSIInfo->prCsiInfoData->ai2TempQData,
			prCSIData->ac2IData,
			prCSIData->ac2QData);
#else
	...
	wlanVniOutputCSIData(prAdapter, prCSIData);
#else
...
uint32_t nicDumpTlv(IN void *prCmdBuffer)
{
	...
	for (u2ElementNum = 1; u2ElementNum <= prTlvCommon->u2TotalElementNum;
		...
		if (!prTlvElement) {
			DBGLOG(TX, ERROR, "prTlvElement is null\n");
			return WLAN_STATUS_FAILURE;
		}
	    ...
	    IN uint8_t *pcuEvtBuf)
{
	...
	g_rDiscMatchInd.peer_sdea_params.fgDataType = NAN_DATA_PATH_UNICAST_MSG;
...
{
	struct NAN_EVENT_NDL_FLOW_CTRL_T *prFlowCtrlEvt;
	struct STA_RECORD *prStaRec;
	uint16_t u2SchId = 0;
	uint32_t u4Idx;
	...
	prFlowCtrlEvt = (struct NAN_EVENT_NDL_FLOW_CTRL_T *)pcuEvtBuf;
		      ...
		      IN struct WIFI_EVENT *prEvent)
{
	...
	DBGLOG(NAN, INFO, "nicNanEventDispatcher [%s]\n",
	    ...
	    IN struct CMD_INFO *prCmdInfo, IN uint8_t *pucEventBuf)
{
	...
	if (prCmdInfo->fgIsOid) {
		...
		if (!prTlvElement) {
			DBGLOG(REQ, ERROR, "prTlvElement is null\n");
			return;
		}
		prEventContent =
		...
		if (!prEventContent || !prQueryInfoContent) {
			DBGLOG(REQ, ERROR, "pr*Content is null\n");
			return;
		}
...
#if CFG_SUPPORT_QA_TOOL
#if ((CFG_CONNAC2_LISTMODE_ENABLE == 1) || (CONFIG_WLAN_SERVICE == 1))
...
#if (CFG_SUPPORT_RADAR_CMD == 1)
void nicCmdEventRadarCtrl(IN struct ADAPTER *prAdapter,
		IN struct CMD_INFO *prCmdInfo, IN uint8_t *pucEventBuf)
{
	struct CMD_RADAR_CTRL_PARA *prRadarCtrl = NULL;
	struct RADAR_RAW_DATA *prInfoEvent = NULL;
	...
	if (prCmdInfo->fgIsOid) {
		prRadarCtrl = (struct CMD_RADAR_CTRL_PARA *)
			   prCmdInfo->pvInformationBuffer;

		prInfoEvent = (struct RADAR_RAW_DATA *)pucEventBuf;

		if (prInfoEvent->ucSubEventId ==
			ENUM_RADAR_RESP_SUBEVENT_SETTING) {
			prRadarSetting = (struct CMD_RADAR_CTRL_SETTING_T *)
				prRadarCtrl->aucBody;
			u4QueryInfoLen = sizeof(struct CMD_RADAR_CTRL_PARA) +
				sizeof(struct CMD_RADAR_CTRL_SETTING_T);
			kalMemZero(prRadarSetting,
				sizeof(struct CMD_RADAR_CTRL_SETTING_T));

			prRadarReport = (struct RADAR_CMD_RESP_REPORT *)
				prRadarCtrl->aucBody;
			prRadarReport->ucStatus = prInfoEvent->ucStatus;
		} else if (prInfoEvent->ucSubEventId ==
			ENUM_RADAR_RESP_SUBEVENT_ENABLE) {
			prRadarEnable = (struct CMD_RADAR_CTRL_ENABLE_T *)
				prRadarCtrl->aucBody;
			u4QueryInfoLen = sizeof(struct CMD_RADAR_CTRL_PARA) +
				sizeof(struct CMD_RADAR_CTRL_ENABLE_T);
			kalMemZero(prRadarEnable,
				sizeof(struct CMD_RADAR_CTRL_ENABLE_T));
...
void nicEventRadarRecvData(IN struct ADAPTER *prAdapter,
		     IN struct WIFI_EVENT *prEvent)
{
	struct WIFI_VAR *prWifiVar = &prAdapter->rWifiVar;
	struct RADAR_RAW_DATA *prRadarRawData = NULL;
	struct CMD_INFO *prCmdInfo = NULL;
	uint8_t ucRawDataNum = 0;
	uint8_t ucRadarCouplingCnt = 0;
	OS_SYSTIME rCurrentTime;
	uint32_t rCurrentSec;
	uint32_t rCurrentMsec;

	rCurrentTime = kalGetTimeTick();
	rCurrentMsec = SEC_REM_TO_MSEC(rCurrentTime);
	rCurrentSec = SYSTIME_TO_SEC(rCurrentTime);

	if (!prAdapter) {
		DBGLOG(NIC, ERROR, "prAdapter error!\n");
		return;
	}
	if (!prEvent) {
		DBGLOG(NIC, ERROR, "prEvent error!\n");
		return;
	}

	prRadarRawData = (struct RADAR_RAW_DATA *)prEvent->aucBuffer;
	if (!prRadarRawData) {
		DBGLOG(NIC, ERROR, "prRadarRawData error!\n");
		return;
	}

	switch (prRadarRawData->ucSubEventId) {
	case ENUM_RADAR_RESP_SUBEVENT_SETTING:
	case ENUM_RADAR_RESP_SUBEVENT_ENABLE:
		prCmdInfo = nicGetPendingCmdInfo(prAdapter, prEvent->ucSeqNum);
		...
		if (prCmdInfo != NULL) {
			if (prCmdInfo->pfCmdDoneHandler)
				prCmdInfo->pfCmdDoneHandler(prAdapter,
							prCmdInfo,
							prEvent->aucBuffer);
			else if (prCmdInfo->fgIsOid != FALSE)
				kalOidComplete(prAdapter->prGlueInfo,
				       prCmdInfo->fgSetQuery, 0,
				       WLAN_STATUS_SUCCESS);
			/* return prCmdInfo */
		...
		break;
	case ENUM_RADAR_RESP_SUBEVENT_PERIODICAL_EVENT:
		prRadarRawData->rCurrentSec = rCurrentSec;
		prRadarRawData->rCurrentMsec = rCurrentMsec;

		if (IS_FEATURE_ENABLED(prWifiVar->fgRadarCoupling) ||
			(prRadarRawData->ucReasonCode == ENUM_DUMP_ACC_DATA))
			ucRadarCouplingCnt = FMCW_BURST_TX_DATA_OFFSET;
		else
			ucRadarCouplingCnt = FMCW_BURST_TX_DATA_OFFSET / 2;

		if (prRadarRawData->ucStatus ==
			ENUM_RADAR_CTRL_RESULT_SUCCESS) {
			DBGLOG(NIC, WARN,
				"[%u.%03u][RadarInfo] Reason Code: %d, Burst Index:%d, Thermal:%d, RSSI:%d, CBP_Retry:%d, TxAntIdx:%d\n",
				prRadarRawData->rCurrentSec,
				prRadarRawData->rCurrentMsec,
				prRadarRawData->ucReasonCode,
				prRadarRawData->u4BurstCnt,
				prRadarRawData->u4TxChainAdcTemp,
				prRadarRawData->u4FMCWRssi,
				prRadarRawData->u4FMCWCBPRetryCnt,
				prRadarRawData->ucFMCWTxAntIdx);
			DBGLOG(NIC, WARN, "rlmFMCWRadarReadData\n");
			for (ucRawDataNum = 0; ucRawDataNum <
				ucRadarCouplingCnt; ucRawDataNum++)
				DBGLOG(NIC, WARN, "FMCW Dump data[%d]=%08x\n",
					ucRawDataNum,
					prRadarRawData->
					au4RawData[ucRawDataNum]);
		} else {
			DBGLOG(NIC, WARN, "Radar Fail Reason Code: %d\n",
				prRadarRawData->ucReasonCode);
		}

#if (CFG_SUPPORT_FMCW_VNI == 1)
		if (IS_FEATURE_ENABLED(prWifiVar->fgRadarVniEnable))
			wlanVniOutputFmcwData(prAdapter,
				prRadarRawData, ucRadarCouplingCnt);
#endif /* (CFG_SUPPORT_FMCW_VNI == 1) */

		break;
	case ENUM_RADAR_RESP_SUBEVENT_INVALID:
	default:
		DBGLOG(NIC, ERROR, "Radar FW error.\n");
		break;
	}
...
void nicEventRttResp(struct ADAPTER *prAdapter,
		struct WIFI_EVENT *prEvent)
{
	struct EVENT_LOC_RESP *prLocResp = NULL;
	struct CMD_INFO *prCmdInfo = NULL;
	struct LOC_EVENT_RTT_RESULT *prLocResult = NULL;
	struct EVENT_RTT_RESULT *prRttResult = NULL;
	struct RTT_RESULT *prResult = NULL;
	struct LOC_EVENT_RTT_DONE *prLocDone = NULL;
	struct EVENT_RTT_DONE *prRttDone = NULL;
	uint32_t u4Size;

	if ((prAdapter == NULL) || (prEvent == NULL)) {
		DBGLOG(RTT, ERROR, "prAdapter or prEvent error!\n");
		return;
	}

	prLocResp = (struct EVENT_LOC_RESP *)prEvent->aucBuffer;
	if (!prLocResp) {
		DBGLOG(RTT, ERROR, "prLocResp error!\n");
		return;
	}

	switch (prLocResp->ucSubEventId) {
	case EVENT_LOC_CAPA:
		DBGLOG(RTT, STATE, "RTT EVENT_LOC_CAPA.\n");
		/* command response handling */
		prCmdInfo = nicGetPendingCmdInfo(prAdapter, prEvent->ucSeqNum);
		if (prCmdInfo != NULL) {
			if (prCmdInfo->pfCmdDoneHandler)
		...
		break;
	case EVENT_LOC_RESULT:
		DBGLOG(RTT, STATE, "RTT EVENT_LOC_RESULT.\n");
		prLocResult =
			(struct LOC_EVENT_RTT_RESULT *) prLocResp->aucBody;
		u4Size = sizeof(struct EVENT_RTT_RESULT);
		if (prLocResult->u2IELen < 512) /* LCI, LCR */
			u4Size += prLocResult->u2IELen;
		else
			DBGLOG(RTT, WARN, "u2IELen is too long\n");
		prRttResult = kalMemAlloc(u4Size, VIR_MEM_TYPE);
		if (!prRttResult) {
			DBGLOG(RTT, ERROR, "prRttDone error\n");
			return;
		}
		prResult = &prRttResult->rResult;
		kalMemCopy(prResult->aucMacAddr, prLocResult->aucMacAddr,
				MAC_ADDR_LEN);
		prResult->u4BurstNum = prLocResult->u4BurstNum;
		prResult->u4MeasurementNumber =
				prLocResult->u4MeasurementNumber;
		prResult->u4SuccessNumber = prLocResult->u4SuccessNumber;
		prResult->ucNumPerBurstPeer = prLocResult->ucNumPerBurstPeer;
		prResult->eStatus = prLocResult->eStatus;
		prResult->ucRetryAfterDuration =
				prLocResult->ucRetryAfterDuration;
		prResult->eType = prLocResult->eType;
		prResult->i4Rssi = prLocResult->i4Rssi;
		prResult->i4RssiSpread = prLocResult->i4RssiSpread;
		kalMemCopy(&prResult->rTxRate, &prLocResult->rTxRate,
				sizeof(struct WIFI_RATE));
		kalMemCopy(&prResult->rRxRate, &prLocResult->rRxRate,
				sizeof(struct WIFI_RATE));
		prResult->i8Rtt = prLocResult->i8Rtt;
		prResult->i8RttSd = prLocResult->i8RttSd;
		prResult->i8RttSpread = prLocResult->i8RttSpread;
		prResult->i4DistanceMM = prLocResult->i4DistanceMM;
		prResult->i4DistanceSdMM = prLocResult->i4DistanceSdMM;
		prResult->i4DistanceSpreadMM =
				prLocResult->i4DistanceSpreadMM;
		prResult->i8Ts = prLocResult->i8Ts;
		prResult->i4BurstDuration = prLocResult->i4BurstDuration;
		prResult->i4NegotiatedBustNum =
				prLocResult->i4NegotiatedBustNum;
		/* LCI, LCR not support now */
		prResult->pLCI = NULL;
		prResult->pLCR = NULL;

		if (prLocResult->u2IELen < 512) { /* LCI, LCR */
			prRttResult->u2IELen = prLocResult->u2IELen;
			kalMemCopy(prRttResult->aucIE,
				prLocResult->aucIE, prLocResult->u2IELen);
		} else {
			prRttResult->u2IELen = 0;
		}
		rttEventResult(prAdapter, prRttResult);
		kalMemFree(prRttResult, VIR_MEM_TYPE, u4Size);
		break;
	case EVENT_LOC_DONE:
		DBGLOG(RTT, STATE, "RTT EVENT_LOC_DONE.\n");
		prLocDone =
			(struct LOC_EVENT_RTT_DONE *) prLocResp->aucBody;

		prRttDone = kalMemAlloc(sizeof(struct EVENT_RTT_DONE),
						VIR_MEM_TYPE);
		if (!prRttDone) {
			DBGLOG(RTT, ERROR, "prRttDone error\n");
			return;
		}
		prRttDone->ucSeqNum = prLocDone->ucSeqNum;
		rttEventDone(prAdapter, prRttDone);
		kalMemFree(prRttDone, VIR_MEM_TYPE,
				sizeof(struct EVENT_RTT_DONE));
		break;
	case EVENT_LOC_TRIGGER:
	case EVENT_LOC_REQ_MC_BURST:
	default:
		DBGLOG(RTT, ERROR, "RTT EVENT Invalid.\n");
		break;
	}
}
...
void nicEventAntSwDivSwitch(IN struct ADAPTER *prAdapter,
		     IN struct WIFI_EVENT *prEvent)
{
	struct EVENT_ANT_SMART_DIV_INFO *prEventAntSwDiv = NULL;
	static uint8_t *apucDebugStatus[ANT_SW_DIV_STATUS_NUM] = {
		(uint8_t *) DISP_STRING("SUCCESS"),
		(uint8_t *) DISP_STRING("NO_NEED_SWITCH"),
		(uint8_t *) DISP_STRING("BLOCKED"),
		(uint8_t *) DISP_STRING("STAREC_NULL"),
		(uint8_t *) DISP_STRING("SIG_DETECT")
	};
	static uint8_t *apucDebugIdx[ANT_SW_DIV_ANT_IDX_NUM] = {
		(uint8_t *) DISP_STRING("ANT_MAIN"),
		(uint8_t *) DISP_STRING("ANT_AUX"),
	};
	...
	prEventAntSwDiv = (struct EVENT_ANT_SMART_DIV_INFO *)prEvent->aucBuffer;

	if (prEventAntSwDiv->ucStatus < ANT_SW_DIV_STATUS_NUM)
		DBGLOG(NIC, STATE, "Status : [%s]\n",
			apucDebugStatus[prEventAntSwDiv->ucStatus]);

	switch (prEventAntSwDiv->ucStatus) {
	case ANT_SW_DIV_STATUS_SUCCESS:
	case ANT_SW_DIV_STATUS_NO_NEED_SWITCH:
		prAdapter->rWifiVar.fgIsAntSwDivProbed = 1;
		break;
	case ANT_SW_DIV_STATUS_BLOCKED:
		break;
	case ANT_SW_DIV_STATUS_STAREC_NULL:
		break;
	case ANT_SW_DIV_STATUS_DETECTION_SUCCESS:
		break;
	case ANT_SW_DIV_STATUS_NUM:
		break;
	default:
		DBGLOG(NIC, ERROR, "AntSwitch error.\n");
		break;
	}

	if ((prEventAntSwDiv->ucStatus == ANT_SW_DIV_STATUS_SUCCESS) &&
		(prEventAntSwDiv->ucUsingAntRcpiAvg == 0)) {

		DBGLOG(NIC, STATE, "Force Switch Ant to [%s]  done\n",
			apucDebugIdx[prEventAntSwDiv->ucUsingAntIdx]);
	} else if (prEventAntSwDiv->ucStatus == ANT_SW_DIV_STATUS_SUCCESS) {
		DBGLOG_MEM8(NIC, STATE, prEventAntSwDiv,
			sizeof(struct EVENT_ANT_SMART_DIV_INFO));

		DBGLOG(NIC, STATE,
			"Switch Ant from [%s(%d)] to [%s(%d)] done\n",
			apucDebugIdx[prEventAntSwDiv->ucWaittingAntIdx],
			RCPI_TO_dBm(prEventAntSwDiv->ucWaittingAntRcpiAvg),
			apucDebugIdx[prEventAntSwDiv->ucUsingAntIdx],
			RCPI_TO_dBm(prEventAntSwDiv->ucUsingAntRcpiAvg));
	} else if (prEventAntSwDiv->ucStatus
		== ANT_SW_DIV_STATUS_NO_NEED_SWITCH) {
		DBGLOG_MEM8(NIC, STATE, prEventAntSwDiv,
			sizeof(struct EVENT_ANT_SMART_DIV_INFO));

		DBGLOG(NIC, STATE,
			"No need switch [Current using %s(%d)] :  [%s(%d)]\n",
			apucDebugIdx[prEventAntSwDiv->ucUsingAntIdx],
			RCPI_TO_dBm(prEventAntSwDiv->ucUsingAntRcpiAvg),
			apucDebugIdx[prEventAntSwDiv->ucWaittingAntIdx],
			RCPI_TO_dBm(prEventAntSwDiv->ucWaittingAntRcpiAvg));
	} else if (prEventAntSwDiv->ucStatus
		== ANT_SW_DIV_STATUS_DETECTION_SUCCESS) {
		DBGLOG(NIC, STATE, "Current RSSI(%d)\n",
			RCPI_TO_dBm(prEventAntSwDiv->ucUsingAntRcpiAvg));
		kalAntSwDivDynamicSwitch(prAdapter,
			RCPI_TO_dBm(prEventAntSwDiv->ucUsingAntRcpiAvg));
	}

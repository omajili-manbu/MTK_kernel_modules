 ...
 *******************************************************************************
 */
#if defined(_HIF_USB) && (CFG_SUPPORT_MULTI_CARD == 0)
...
void nicReleaseAdapterMemory(IN struct ADAPTER *prAdapter)
{
	...
	if (!prAdapter) {
		DBGLOG(NIC, WARN, "prAdapter is NULL!\n");
		return;
	}
...
uint8_t nicIsChBwValid(enum ENUM_BAND eBand, uint8_t ucCh, uint8_t ucBw)
{
	uint8_t ucOriBw = ucBw;

	nicReviseBwByCh(eBand, ucCh, &ucBw);
 ...
 */
/*----------------------------------------------------------------------------*/
void nicReviseBwByCh(enum ENUM_BAND eBand, uint8_t ucCh, uint8_t *pucBw)
{
	uint8_t eNewBw = *pucBw;

	if (*pucBw >= MAX_BW_UNKNOWN) {
		DBGLOG(NIC, ERROR, "unknown bandwidth");
		return;
	}

	if (eBand == BAND_2G4 && *pucBw >= MAX_BW_80MHZ) {
		if (ucCh >= 1 && ucCh <= 13 && *pucBw >= MAX_BW_80MHZ)
			/* downgrade to 2G default BW */
			eNewBw = MAX_BW_20MHZ;
		else if (ucCh == 14 && *pucBw >= MAX_BW_40MHZ)
			eNewBw = MAX_BW_20MHZ;
	} else if (eBand == BAND_5G) {
#if (CFG_SUPPORT_UNII4 == 0)
		if (ucCh >= 132 && ucCh <= 161 && *pucBw >= MAX_BW_160MHZ)
			eNewBw = MAX_BW_80MHZ;
		else if (ucCh == 165)
			eNewBw = MAX_BW_20MHZ;
#else
		if (ucCh >= 132 && ucCh <= 177 && *pucBw >= MAX_BW_160MHZ)
			eNewBw = MAX_BW_80MHZ;
#endif
	}
#if (CFG_SUPPORT_WIFI_6G == 1)
	else if (eBand == BAND_6G) {
		if (ucCh >= 225 && ucCh <= 229 && *pucBw >= MAX_BW_80MHZ)
			/* downgrade to 2G default BW */
			eNewBw = MAX_BW_20MHZ;
		else if (ucCh == 233)
			eNewBw = MAX_BW_20MHZ;
	...
	if (*pucBw != eNewBw) {
		DBGLOG(NIC, LOUD, "b:%u c:%u bw not support %u, fallback to %u",
		       eBand, ucCh, *pucBw, eNewBw);
		*pucBw = eNewBw;
	}
}
...
/* disabled for BOW to finish ucBssIndex migration */
	...
	if (IS_BSS_ACTIVE(prBssInfo) &&
		prAdapter->e6GPwrMode[ucBssIndex] == PWR_MODE_6G_VLP)
	...
	DBGLOG(BSS, INFO,
	       ...
	       "] AuthMode[%u] EncStatus[%u] 6GPwrReg[%u]\n", ucBssIndex,
	       prBssInfo->eConnectionState,
...
#if 1
uint8_t nicGetChipHwVer(struct GLUE_INFO *prGlueInfo)
{
#if CFG_SUPPORT_MULTI_CARD
	return prGlueInfo->eco_info.ucHwVer;
#else
	return g_eco_info.ucHwVer;
#endif
}

uint8_t nicGetChipSwVer(struct GLUE_INFO *prGlueInfo)
{
#if CFG_SUPPORT_MULTI_CARD
	return prGlueInfo->eco_info.ucRomVer;
#else
	return g_eco_info.ucRomVer;
#endif
}

uint8_t nicGetChipFactoryVer(struct GLUE_INFO *prGlueInfo)
{
#if CFG_SUPPORT_MULTI_CARD
	return prGlueInfo->eco_info.ucFactoryVer;
#else
	return g_eco_info.ucFactoryVer;
#endif
}

uint8_t nicSetChipHwVer(struct GLUE_INFO *prGlueInfo, uint8_t value)
{
#if CFG_SUPPORT_MULTI_CARD
	prGlueInfo->eco_info.ucHwVer = value;
#else
...
uint8_t nicSetChipSwVer(struct GLUE_INFO *prGlueInfo, uint8_t value)
{
#if CFG_SUPPORT_MULTI_CARD
	prGlueInfo->eco_info.ucRomVer = value;
#else
...
uint8_t nicSetChipFactoryVer(struct GLUE_INFO *prGlueInfo, uint8_t value)
{
#if CFG_SUPPORT_MULTI_CARD
	prGlueInfo->eco_info.ucFactoryVer = value;
#else
...
uint8_t nicGetChipEcoVer(IN struct ADAPTER *prAdapter)
{
	...
	ucCurSwVer = nicGetChipSwVer(prAdapter->prGlueInfo);
	ucCurHwVer = nicGetChipHwVer(prAdapter->prGlueInfo);
	ucCurFactoryVer = nicGetChipFactoryVer(prAdapter->prGlueInfo);
	...
	IN unsigned long plParamPtr)
{
	...
	cnmTimerStartTimer(prAdapter, &SER_SYN_TIMER(prAdapter), u4TimeoutMs);
}
#endif
...
void nicSerInit(IN struct ADAPTER *prAdapter, IN const u_int8_t bAtResetFlow)
{
	...
	if (prAdapter->chip_info->fgIsSupportL0p5Reset) {
		if (!bAtResetFlow)
			glResetInitL0p5Work(prAdapter->prGlueInfo);
...
#if CFG_CHIP_RESET_SUPPORT
	if (prAdapter->chip_info->fgIsSupportL0p5Reset)
		glResetWaitL0p5WorkDone(prAdapter->prGlueInfo);
#endif
	...
	struct CMD_DFSP_CONFIG *ptrDfspCfg)
{
	...
	struct CMD_DFSP_CONFIG *rCmdDfspConfig = NULL;
	...
	rStatus = nicAddNewTlvElement(NAN_CMD_DFSP_CONFIG,
				      sizeof(struct CMD_DFSP_CONFIG),
	...
	rCmdDfspConfig = (struct CMD_DFSP_CONFIG *)prTlvElement->aucbody;
	kalMemCopy(rCmdDfspConfig, ptrDfspCfg,
		   sizeof(struct CMD_DFSP_CONFIG));

...
static struct RX_EVENT_HANDLER arEventTable[] = {
...
#endif
#if CFG_WIFI_RAM_MQM_BA_DELAY_SUPPORT
	...
	{EVENT_ID_RADAR_CTRL, nicEventRadarRecvData},
#endif
#if (CFG_SUPPORT_RTT == 1)
	{EVENT_ID_LOCATION, nicEventRttResp},
#endif /* CFG_SUPPORT_RTT */
#if (CFG_SUPPORT_ANT_SMART_DIVERSITY == 1)
	{EVENT_ID_ANT_SMART_DIV_SWITCH, nicEventAntSwDivSwitch},
#endif
};
...
void nicRxInitialize(IN struct ADAPTER *prAdapter)
{
	...
	kalCreateHifSkbList(prAdapter->prGlueInfo);
#else
...
#if CFG_SUPPORT_MULTI_SKB_SIZE
	for (eSkbSizeType = SKB_SIZE_TYPES_1K_SLAB;
		eSkbSizeType < SKB_SIZE_TYPES_NUM;
		eSkbSizeType++)
		QUEUE_INITIALIZE(&prRxCtrl->rFreeSwRfbList[eSkbSizeType]);
#else
	...
	for (eSkbSizeType = SKB_SIZE_TYPES_1K_SLAB;
		eSkbSizeType < SKB_SIZE_TYPES_NUM;
		eSkbSizeType++) {
		for (i = getSkbNum(prAdapter, eSkbSizeType); i != 0; i--) {
			prSwRfb = (struct SW_RFB *) pucMemHandle;
			if (nicRxSetupRFB(prAdapter, prSwRfb, eSkbSizeType)) {
				DBGLOG(RX, ERROR,
					"allocate packet buffer failed!\n");
				return;
			}
		...
		if (prRxCtrl->rFreeSwRfbList[eSkbSizeType].u4NumElem !=
			getSkbNum(prAdapter, eSkbSizeType))
			ASSERT_NOMEM();
	}
#else
...
void nicRxUninitialize(IN struct ADAPTER *prAdapter)
{
	...
	do {
		...
		if ((++u1SkbSizeType) >= SKB_SIZE_TYPES_NUM)
			break;
#else
...
#if CFG_SUPPORT_RX_PAGE_POOL
#if CFG_SUPPORT_RX_PAGE_POOL_USE_CMA
	kalReleaseHifSkbList(prAdapter->prGlueInfo);
#else
				   ...
				   *prAdapter, IN struct SW_RFB *prSwRfb)
{
	...
	u4CurrentRxBufferCount = getTotalFreeRfbCount(prAdapter);
			    ...
			    IN struct SW_RFB *prSwRfb)
{
	...
	freeRfbCount = getTotalFreeRfbCount(prAdapter);
				...
				*prAdapter, IN struct SW_RFB *prSwRfb)
{
...
#if CFG_SUPPORT_MULTI_SKB_SIZE
	pFreeRfbQueue = getFreeRfbList(prAdapter, prSwRfb->u2RxByteCount, 0);
#else
	pFreeRfbQueue = &prRxCtrl->rFreeSwRfbList;
#endif
	freeRfbCount = getTotalFreeRfbCount(prAdapter);
	...
	if (freeRfbCount
		...
		KAL_ACQUIRE_SPIN_LOCK(prAdapter, SPIN_LOCK_RX_FREE_QUE);
		QUEUE_REMOVE_HEAD(pFreeRfbQueue,
...
#if (CFG_SUPPORT_4ADDR_STATION == 1)
	ucIndex = secGetBssIdxByWlanIdx(prAdapter, prSwRfb->ucWlanIdx);

	if (ucIndex < MAX_BSSID_NUM + 1)
		prBssInfo = prAdapter->aprBssInfo[ucIndex];

	if (prBssInfo
		&& prBssInfo->ucWdsEnable
		&& prSwRfb->fgHdrTran
		&& prSwRfb->fgIsBC
		&& prSwRfb->prRxStatusGroup4
		&& RXM_IS_FROM_DS(prSwRfb->prRxStatusGroup4->u2FrameCtl)) {
		fgDrop = TRUE;
		DBGLOG(RX, INFO, "Enable WDS Drop Rx 3addr BC frame\n");
	}
#endif /* end CFG_SUPPORT_4ADDR_STATION */
			     ...
			     IN OUT struct SW_RFB *prSwRfb)
{
	...
	if (prEvent->u2PacketLength > RX_GET_PACKET_MAX_SIZE(prAdapter)
		...
		DBGLOG(NIC, ERROR,
			"Invalid RX event len: ID[0x%02X] SEQ[%u] LEN[%u]\n",
			     ...
			     IN OUT struct SW_RFB *prSwRfb)
{
	...
	if (prSwRfb->prRxStatusGroup3 == NULL) {
		...
		glSetRstReason(prAdapter, RST_GROUP3_NULL);
	...
	IN struct SW_RFB *prSwRfb)
{
	...
	if (!prIcsAggHeader) {
		DBGLOG(NIC, ERROR, "prRxStatus is invalid\n");
		return;
	}
	if (prIcsAggHeader->rxByteCount > ETHER_MAX_PKT_SZ) {
		DBGLOG(NIC, ERROR, "rxByteCount too large[%u]\n",
...
#if CFG_SUPPORT_RX_PAGE_POOL
		pvPacket = kalAllocRxSkbFromPp(prAdapter->prGlueInfo,
					&pucRecvBuff, -1);
#else
#if CFG_SUPPORT_MULTI_SKB_SIZE
		pvPacket = kalPacketAlloc(prAdapter->prGlueInfo,
			getSkbSize(prAdapter, eSkbSizeType), &pucRecvBuff);
#else
		pvPacket = kalPacketAlloc(prAdapter->prGlueInfo,
...
#if CFG_SUPPORT_MULTI_SKB_SIZE
		prSwRfb->eSkbSizeType = eSkbSizeType;
#endif
	} else {
		    ...
		    IN struct SW_RFB *prSwRfb)
{
	...
	if (halIsPendingRx(prAdapter)
	    && (getTotalFreeRfbCount(prAdapter) > 0))
		kalSetIntEvent(prAdapter->prGlueInfo);
		 ...
		 IN struct SW_RFB *prSwRfb)
{
	...
	if (prSwRfb->prStaRec &&
		(prSwRfb->prStaRec->eStaType == STA_TYPE_NAN)) {
		fgIsTxKeyReady = prSwRfb->prStaRec->fgIsTxKeyReady;
	} else if (prAdapter->rWifiVar.fgUseNmiCxtKey &&
		prAdapter->rWifiVar.fgEnNanKeyMgmt &&
	    nanDataEngineIsNmiContext(prAdapter,
			prActionFrame->aucDestAddr,
		...
		if (prNmiCxtKey &&
			...
			fgIsTxKeyReady = prNmiCxtKey->fgIsKeyExist;
	}

	if (fgIsTxKeyReady == TRUE) {
#if CFG_SUPPORT_CONNAC2X
			      ...
			      IN struct SW_RFB *prSwRfb)
{
	...
	if (ucOuiType == VENDOR_OUI_TYPE_NAN_NAF) {
		if (nicRxNANPMFCheck(prAdapter, prSwRfb) ==
		    WLAN_STATUS_FAILURE) {
			DBGLOG(NAN, ERROR, "PMF check fail\n");
			return WLAN_STATUS_FAILURE;
		}
	}
	...
	if (ucOuiType == VENDOR_OUI_TYPE_NAN_NAF ||
		...
		case NAN_ACTION_SCHEDULE_UPDATE_NOTIFICATION:
			rWlanStatus = nanNdlProcessScheduleUpdateNotification(
				prAdapter, prSwRfb);
			break;
		case NAN_ACTION_RESERVED:
			rWlanStatus = nanNdlProcessOob(prAdapter, prSwRfb,
				OOB_TYPE_ACTION);
			break;
		default:
			break;

	...
	struct CMD_SCAN_REQ_V2 *cmd)
{
	...
	uint8_t ssid_num = kal_min_t(int, cmd->ucSSIDNum, 4);
	uint8_t ssid_ext_num = kal_min_t(int, cmd->ucSSIDExtNum, 6);
	...
	struct CMD_SCAN_REQ_V2 *cmd)
{
	...
	uint8_t chnl_num = kal_min_t(int, cmd->ucChannelListNum, 32);
	uint8_t chnl_ext_num = kal_min_t(int, cmd->ucChannelListExtNum, 32);
	uint16_t len = sizeof(*tag) +

...
/* host(wifip2pd) will plumb key using IOCTL */
#if (CFG_SUPPORT_NAN_R4_VENDOR_HOST_KEY == 0)
static int
...
#endif /* CFG_SUPPORT_NAN_R4_VENDOR_HOST_KEY */
...
wpa_verify_key_mic(int akmp, struct wpa_ptk *PTK, u8 *data, size_t data_len) {
	...
	key192 = (struct wpa_eapol_key_192 *)(hdr + 1);
	key_info = WPA_GET_BE16(key->key_info);
	kalMemCpyS(mic, sizeof(mic), key192->key_mic,
		sizeof_field(struct wpa_eapol_key_192, key_mic));
...
ieee80211w_kde_len(struct wpa_state_machine *sm)
{
	...
	if (sm->pvNdp != NULL)
		ucGroupSAUsed = ((struct _NAN_NDP_INSTANCE_T *)
			(sm->pvNdp))->ucGroupSAUsed;
	else
		ucGroupSAUsed =
	...
	if (sm->mgmt_frame_prot && nmiConf->beacon_prot && ucIsBIGTKSA) {
		len += (size_t)
			(2 + RSN_SELECTOR_LEN + WPA_BIGTK_KDE_PREFIX_LEN);
		len += wpa_cipher_key_len(nmiConf->group_mgmt_cipher);
	}
...
ieee80211w_kde_add(struct wpa_state_machine *sm, u8 *pos)
{
	...
	igtk.keyid[1] = 0;
	DBGLOG(NAN, STATE, "wpa_group_state: %u\n", gsm->wpa_group_state);
	if (gsm->wpa_group_state != WPA_GROUP_SETKEYSDONE ||
	    wpa_auth_get_seqnum(sm->wpa_auth_nmi, NULL, gsm->GN_igtk, rsc) < 0)
	...
	wpa_hexdump(MSG_DEBUG, "IGTK", &igtk, (WPA_IGTK_KDE_PREFIX_LEN + len));
	...
	if (gsm->wpa_group_state != WPA_GROUP_SETKEYSDONE ||
		wpa_auth_get_seqnum(sm->wpa_auth_nmi,
			NULL, gsm->GN_bigtk,
			rsc) < 0)
		nan_os_memset(bigtk.pn, 0, sizeof(bigtk.pn));
	else
		nan_os_memcpyS(bigtk.pn,
			sizeof_field(struct wpa_bigtk_kde, pn),
			rsc, sizeof(bigtk.pn));
	nan_os_memcpyS(bigtk.bigtk,
		sizeof_field(struct wpa_bigtk_kde, bigtk),
		gsm->BIGTK[gsm->GN_bigtk - 6],
		len);

	pos = wpa_add_kde(pos, RSN_KEY_DATA_BIGTK, (const u8 *)&bigtk,
			  WPA_BIGTK_KDE_PREFIX_LEN + len, NULL, 0);
	wpa_hexdump(MSG_DEBUG, "BIGTK",
		&bigtk,
		(WPA_BIGTK_KDE_PREFIX_LEN + len));
...
SM_STATE(WPA_PTK, PTKINITNEGOTIATING) {
	...
	if ((sm->wpa == WPA_VERSION_WPA2) && ucIsGTKSA) {
		...
		if (sm->wpa_auth->conf.disable_gtk) {
			 ...
			 * of GTK in the BSS.
			 */
			if (random_get_bytes(tmp_gtk, gtk_len) < 0)
				return;
			gtk = tmp_gtk;
		}
		keyidx = gsm->GN;
	...
	if (kde) {
		...
		if (gtk) {
			...
			hdr[0] = keyidx & 0x03;
			hdr[1] = 0;
			pos = wpa_add_kde(pos, RSN_KEY_DATA_GROUPKEY,
				hdr, 2, gtk,
				gtk_len);
		}
...
nan_sec_wpa_send_eapol_4(
	struct wpa_sm *sm,
	const u8 *key_rsc,
	const u8 *kde, size_t kde_len, int keyidx, int encr,
	int force_version,
...
if ((version == WPA_KEY_INFO_TYPE_HMAC_SHA1_AES ||
	...
	key_info |= version | WPA_KEY_INFO_KEY_TYPE;
	if (mic_len)
	...
	nan_os_memcpyS(reply->replay_counter,
	...
	if (key_rsc)
		nan_os_memcpyS(reply->key_rsc,
			sizeof_field(struct wpa_eapol_key, key_rsc),
			key_rsc, WPA_KEY_RSC_LEN);

	if (kde && !encr) {
		nan_os_memcpyS(key_data, key_data_len, kde, kde_len);
		if (mic_len == 24)
	...
	if (version == WPA_KEY_INFO_TYPE_HMAC_SHA1_AES ||
		...
		if (version == WPA_KEY_INFO_TYPE_AKM_DEFINED) {
			...
			if (INT_MAX < ((key_data_len - 8) / 8))
				DBGLOG(INIT, ERROR,
					"((key_data_len-8)/8)=[%lu] overflowed!!!\n",
					((key_data_len - 8) / 8));

			if (aes_wrap(sm->ptk.kek, sm->ptk.kek_len,
				(int)(INT_MAX & ((key_data_len - 8) / 8)), buf,
				key_data,
				key_len - keyhdrlen)) {
				/* os_free(hdr); */
	...
	struct wpa_sm *sm, const struct wpa_eapol_key *key)
{
	 ...
	 *  GTK[GN], IGTK, [FTIE], [TIE * 2])
	 */
	nan_os_memset(rsc, 0, WPA_KEY_RSC_LEN);
	wpa_auth_get_seqnum(wpaStateMachine->wpa_auth, NULL, gsm->GN, rsc);
	/* If FT is used, wpa_auth->wpa_ie includes both RSNIE and MDIE */
	...
	if (wpaStateMachine->wpa == WPA_VERSION_WPA &&
	    ((uint32_t) wpaStateMachine->wpa_auth->conf.wpa & WPA_PROTO_RSN) &&
	    (int)wpa_ie_len > wpa_ie[1] + 2 && wpa_ie[0] == WLAN_EID_RSN) {
	...
	wpa_auth_logger(wpaStateMachine->wpa_auth, wpaStateMachine->addr,
		LOGGER_DEBUG,
	...
	if ((wpaStateMachine->wpa == WPA_VERSION_WPA2) && ucIsGTKSA) {
		wpa_printf(MSG_DEBUG,
			   "WPA2 send GTK in the 4-way handshake");

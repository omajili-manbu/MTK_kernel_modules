...
wpa_eapol_key_send_wpa(struct wpa_sm *sm, const u8 *kck, size_t kck_len,
		       ...
		       size_t msg_len, u8 *key_mic, u32 key_mic_len) {
	size_t mic_len;
	...
	if (key_mic && wpa_eapol_key_mic_wpa(kck, kck_len, sm->key_mgmt, ver,
		msg, msg_len, key_mic, key_mic_len)) {
...
wpa_supplicant_send_2_of_4(struct wpa_sm *sm, const unsigned char *dst,
	...
	wpa_eapol_key_send_wpa(sm, ptk->kck, ptk->kck_len, ver, dst,
		...
		key_mic, sizeof_field(struct wpa_eapol_key_192, key_mic));
	return 0;
}
...
/*if (sm->cur_pmksa && sm->cur_pmksa->opportunistic) {
	...
	*/
	...
	/* wpa_sm should be cleared after set key to firmware */
...
/* Bypass GTK install, NAN have independent WTBL usage for R4 Key installing*/
#if 0
...
#endif
	wpa_printf(MSG_INFO,
		"Note: Bypass GTK install, NAN have independent WTBL usage\n");
	os_memset(gtk_buf, 0, sizeof(gtk_buf));
				   ...
				   int key_info)
{
	...
	nan_os_memcpyS(gd.gtk,
		sizeof_field(struct wpa_gtk_data, gtk),
		gtk, gtk_len);
	gd.gtk_len = gtk_len;

	if (sm->group_cipher != WPA_CIPHER_GTK_NOT_USED &&
		(wpa_supplicant_check_group_cipher(sm, (int)sm->group_cipher,
						   gtk_len, gtk_len,
...
wpa_supplicant_install_igtk(struct wpa_sm *sm,
	...
	size_t len = wpa_cipher_key_len((int)sm->mgmt_group_cipher);
...
/* Bypass IGTK install, NAN have independent WTBL usage for R4 Key installing*/
#if 0
	if (wpa_sm_set_key(sm, wpa_cipher_to_alg((int)sm->mgmt_group_cipher),
...
#endif
	wpa_printf(MSG_INFO,
		"Note: Bypass IGTK install, NAN have independent WTBL usage\n");
	sm->igtk.igtk_len = len;
	os_memcpy(sm->igtk.igtk, igtk->igtk, sm->igtk.igtk_len);
	nan_os_memcpyS(sm->igtk_pn,
		sizeof_field(struct wpa_sm, igtk_pn),
		igtk->pn, NAN_PACKET_NUMBER_LEN);
	return 0;
}

static int
wpa_supplicant_install_bigtk(struct wpa_sm *sm,
			const struct wpa_bigtk_kde *bigtk)
{
	...
	if (sm->bigtk.bigtk_len == len &&
		...
		wpa_printf(
			MSG_INFO,
			"[%s] WPA: Not reinstalling already in-use BIGTK to the driver (keyidx=%d)",
			__func__, keyidx);
	...
	wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG,
		"WPA: BIGTK keyid %d pn %02x%02x%02x%02x%02x%02x", keyidx,
		MAC2STR(bigtk->pn));
...
/* Bypass BIGTK install, NAN have independent WTBL usage for R4 Key installing*/
#if 0
	if (wpa_sm_set_key(sm, wpa_cipher_to_alg((int)sm->mgmt_group_cipher),
			   broadcast_ether_addr, keyidx, 0, bigtk->pn,
			   sizeof(bigtk->pn), bigtk->bigtk, len) < 0) {
		wpa_msg(sm->ctx->msg_ctx, MSG_WARNING,
...
#endif
	wpa_printf(MSG_INFO,
		"Note: Bypass BIGTK install, NAN have independent WTBL usage\n");
	sm->bigtk.bigtk_len = len;
	nan_os_memcpyS(sm->bigtk.bigtk,
		sizeof_field(struct wpa_bigtk, bigtk),
		bigtk->bigtk, len);
	nan_os_memcpyS(sm->bigtk_pn,
		sizeof_field(struct wpa_sm, bigtk_pn),
		bigtk->pn, NAN_PACKET_NUMBER_LEN);
...
ieee80211w_set_keys(struct wpa_sm *sm, struct wpa_eapol_ie_parse *ie) {
#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_FRAME_PROT == 1)
	size_t len = 0;
	const struct wpa_igtk_kde *igtk = NULL;
	const struct wpa_bigtk_kde *bigtk = NULL;

	if (!wpa_cipher_valid_mgmt_group((int)sm->mgmt_group_cipher))
		return 0;

	if (ie->igtk) {
		len = wpa_cipher_key_len((int)sm->mgmt_group_cipher);
	...
	if (ie->bigtk && sm->beacon_prot) {
		...
		if (wpa_supplicant_install_bigtk(sm, bigtk) < 0)
			return -1;
	}
...
void
wpa_supplicant_process_3_of_4(struct wpa_state_machine *wpaStateMachine,
	struct wpa_sm *sm,
	const struct wpa_eapol_key *key, u16 ver,
	const u8 *key_data, size_t key_data_len) {
	u16 key_info = 0; /*(, keylen;*/
#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_FRAME_PROT == 1)
	...
	*/
	...
	if (ie.igtk && wpa_cipher_valid_mgmt_group(
		(int)sm->mgmt_group_cipher) &&
		ie.igtk_len !=
			WPA_IGTK_KDE_PREFIX_LEN + (unsigned int)
			wpa_cipher_key_len(
			(int)sm->mgmt_group_cipher)) {
		wpa_msg(sm->ctx->msg_ctx, MSG_WARNING,
	...
	//ver = WPA_KEY_INFO_TYPE_AES_128_CMAC;
#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_FRAME_PROT == 1)
	if (nan_sec_wpa_send_gtk_in_M4(wpaStateMachine, sm, key))
		goto failed;
#else
...
wpa_supplicant_verify_eapol_key_mic(struct wpa_sm *sm,
	...
	kalMemCpyS(mic, sizeof(mic), key->key_mic, mic_len);
	if (sm->tptk_set) {
...
wpa_supplicant_decrypt_key_data(struct wpa_sm *sm, struct wpa_eapol_key *key,
	...
	if (ver == WPA_KEY_INFO_TYPE_HMAC_MD5_RC4 && sm->ptk.kek_len == 16) {
		...
		os_memset(ek, 0, sizeof(ek));
		   ...
		   ver == WPA_KEY_INFO_TYPE_AES_128_CMAC ||
#if CFG_SUPPORT_NAN /* NAN's version only use AKM */
		   ver == WPA_KEY_INFO_TYPE_AKM_DEFINED ||
#endif
...
/* Decrypt RSN EAPOL-Key key data (RC4 or AES-WRAP) */
int
nan_sec_wpa_decrypt_key_data(
	struct wpa_state_machine *wpaStateMachine,
	struct wpa_eapol_key *key,
	u16 ver, u8 *key_data, size_t *key_data_len)
{
	wpa_hexdump(MSG_DEBUG, "RSN: encrypted key data", key_data,
		    *key_data_len);
	if (!wpaStateMachine->PTK_valid) {
		wpa_msg(wpaStateMachine->ctx->msg_ctx, MSG_WARNING,
			"WPA: PTK not available, cannot decrypt EAPOL-Key Key Data");
		return -1;
	}
	 ...
	 */
	if (ver == WPA_KEY_INFO_TYPE_HMAC_MD5_RC4 &&
		wpaStateMachine->PTK.kek_len == 16) {
#ifdef CONFIG_NO_RC4
		...
		nan_os_memcpyS(ek, sizeof(ek),
			key->key_iv, sizeof_field(struct wpa_eapol_key,
			key_iv));
		nan_os_memcpyS(ek + sizeof_field(struct wpa_eapol_key, key_iv),
			sizeof(ek) - sizeof_field(struct wpa_eapol_key, key_iv),
			wpaStateMachine->ptk.kek, wpaStateMachine->ptk.kek_len);

		if (rc4_skip(ek, 32, 256, key_data, *key_data_len)) {
			nan_os_memset(ek, 0, sizeof(ek));
			wpa_msg(wpaStateMachine->ctx->msg_ctx,
				MSG_ERROR, "WPA: RC4 failed");
			return -1;
		}
		...
		nan_os_memset(ek, 0, sizeof(ek));
		   ...
		   ver == WPA_KEY_INFO_TYPE_AKM_DEFINED ||
#endif
		   wpaStateMachine->wpa_key_mgmt == WPA_KEY_MGMT_OSEN ||
		   wpa_key_mgmt_suite_b((int)wpaStateMachine->wpa_key_mgmt)) {
		u8 *buf = NULL;
		...
		*key_data_len -= 8; /* AES-WRAP adds 8 bytes */
		...
		if (INT_MAX < (*key_data_len / 8))
			DBGLOG(INIT, ERROR,
			"(*key_data_len / 8)=[%lu] overflowed!!!\n",
			(*key_data_len / 8));

		if (aes_unwrap_supp(wpaStateMachine->PTK.kek,
			wpaStateMachine->PTK.kek_len,
			(int)(INT_MAX & (*key_data_len / 8)),
			key_data, buf,
			*key_data_len)) {
			os_free(buf);
			wpa_printf(MSG_WARNING,
		...
		nan_os_memcpyS(key_data, *key_data_len, buf, *key_data_len);
		os_free(buf);
		WPA_PUT_BE16(key->key_data_length, (u16)*key_data_len);
	} else {
		wpa_printf(MSG_WARNING,
			"WPA: Unsupported key_info type %d", ver);
		return -1;
...
uint32_t
nan_sec_wpa_rx_eapol_4(
	struct wpa_state_machine *wpaStateMachine,
	struct wpa_sm *wpaSm,
	struct wpa_eapol_key *keyData)
{
	size_t key_data_len = 0;
	u16 key_info = 0, ver = 0;
	uint32_t ret = WLAN_STATUS_FAILURE;
	u8 *mic = NULL, *key_data = NULL;
	struct wpa_eapol_ie_parse ie = {0};
	struct wpa_eapol_key *key = NULL;
	struct wpa_eapol_key_192 *key192 = NULL;
	size_t mic_len = 0;

	DBGLOG(NAN, INFO, "Enter\n");

	key = (struct wpa_eapol_key *)keyData;
	key192 = (struct wpa_eapol_key_192 *)keyData;

	key_info = WPA_GET_BE16(key->key_info);
	ver = key_info & WPA_KEY_INFO_TYPE_MASK;
	mic_len = wpa_mic_len(wpaStateMachine->wpa_key_mgmt);
	mic = (u8 *)(key + 1);
	...
	/* key_data_len already checked by caller,
	 * this checker is to confirm the value is within a permissible range
	 */
	if (key_data_len > (size_t)NAN_KDE_ATTR_BUF_SIZE) {
		DBGLOG(NAN, INFO,
			   "Quit1. key_data_length:%zu over threshold\n",
		       key_data_len);
		goto out;
	}
	...
	if ((ver == WPA_KEY_INFO_TYPE_HMAC_SHA1_AES ||
		wpaStateMachine->wpa_key_mgmt == WPA_KEY_MGMT_OSEN ||
		wpa_key_mgmt_suite_b(wpaStateMachine->wpa_key_mgmt) ||
#if CFG_SUPPORT_NAN /* NAN's version only use AKM */
		    ver == WPA_KEY_INFO_TYPE_AKM_DEFINED ||
#endif
			ver == WPA_KEY_INFO_TYPE_AES_128_CMAC) &&
			(key_info & WPA_KEY_INFO_ENCR_KEY_DATA)) {
		if (nan_sec_wpa_decrypt_key_data(wpaStateMachine, key,
			ver, key_data,
			&key_data_len)) {
...
#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_FRAME_PROT == 1)
	if (nanSecCheckGtkKde(wpaSm, key_data, key_data_len)
		!= WLAN_STATUS_SUCCESS)
		goto out;
#endif
	...
	if (ie.igtk &&
		wpa_cipher_valid_mgmt_group(
			(int)wpaSm->mgmt_group_cipher) &&
		ie.igtk_len !=
			WPA_IGTK_KDE_PREFIX_LEN + wpa_cipher_key_len(
				(int)wpaSm->mgmt_group_cipher)) {
		wpa_printf(MSG_WARNING,
			"WPA: Invalid IGTK KDE length %lu",
			(unsigned long)ie.igtk_len);
	...
	wpa_sm_set_state(wpaSm, WPA_GROUP_HANDSHAKE);

	if (ie.gtk &&
		 wpa_supplicant_pairwise_gtk(wpaSm, key,
					 ie.gtk, ie.gtk_len, key_info) < 0){
		wpa_msg(wpaSm->ctx->msg_ctx, MSG_DEBUG,
				"RSN: Failed to configure GTK");
		goto out;
	}

	if (ieee80211w_set_keys(wpaSm, &ie) < 0) {
		wpa_msg(wpaSm->ctx->msg_ctx, MSG_DEBUG,
				"RSN: Failed to configure IGTK");
		goto out;
	}

	if (wpaSm->group_cipher == WPA_CIPHER_GTK_NOT_USED
		|| ie.gtk) {
		wpa_supplicant_key_neg_complete(wpaSm, wpaSm->bssid,
					key_info & WPA_KEY_INFO_SECURE);
	}

...
uint8_t
nanBssInit(IN struct ADAPTER *prAdapter, uint8_t ucIdx) {
...
nanDevEnable(struct ADAPTER *prAdapter)
{
	...
	/* Initialize NAN pairing info */
	nanPairingInit(prAdapter);
#endif

	/* initialize NAN sync info */
	nanSyncInfoInit(prAdapter);
...
{
	nanSecReleaseAllNmiCxtKey(prAdapter);
...
uint8_t
nanGetBssIdxByRole(IN struct ADAPTER *prAdapter,
		      size_t szRole /* enum NAN_BSS_ROLE_INDEX */)
{
	struct _NAN_SPECIFIC_BSS_INFO_T *prNANSpecInfo = NULL;

	prNANSpecInfo = nanGetSpecificBssInfo(prAdapter, szRole);
...
void
nanDevGetNdiAddress(struct ADAPTER *prAdapter,
	uint8_t ucNdiIdx, uint8_t *pucNdiAddr)
{
	struct _GL_NAN_INFO_T *prNANInfo = (struct _GL_NAN_INFO_T *)NULL;

	prNANInfo = prAdapter->prGlueInfo->aprNANDevInfo[ucNdiIdx];
	kalMemCpyS(pucNdiAddr,
		MAC_ADDR_LEN,
		prNANInfo->prDevHandler->dev_addr,
		MAC_ADDR_LEN);

	DBGLOG(NAN, INFO, "NDI#%u, MAC dev addr in netdev [" MACSTR "]\n",
		ucNdiIdx, MAC2STR(prNANInfo->prDevHandler->dev_addr));
}
...
void
nanDevGetDeviceCapabilities(struct ADAPTER *prAdapter,
	struct NanDeviceCapabilities *prNanDvcCapabilities)
{
	if (prAdapter == NULL) {
		DBGLOG(NAN, ERROR, "prAdapter is NULL\n");
		return;
	...
	if (prNanDvcCapabilities == NULL) {
		DBGLOG(NAN, ERROR, "prNanDvcCapabilities is NULL\n");
		return;
	}

	prNanDvcCapabilities->supported_bands = WFPAL_NAN_BAND_GHZ_2_4;
	if (prAdapter->rWifiVar.ucConfig5gChannel)
		prNanDvcCapabilities->supported_bands |=
		WFPAL_NAN_BAND_GHZ_4_9_to_5_0;

	prNanDvcCapabilities->operation_mode_ht_vht =
		WFPAL_NAN_OPERATION_MODE_VHT;
	prNanDvcCapabilities->number_of_antennas = 0x11; /* Fixed DBDC Mode */

	prNanDvcCapabilities->max_channel_switch_time = g_u2MaxChnlSwitchTimeUs;
	prNanDvcCapabilities->max_publishers = NAN_MAX_PUBLISH_NUM;
	prNanDvcCapabilities->max_subscribers = NAN_MAX_SUBSCRIBE_NUM;
	prNanDvcCapabilities->max_peer_entries = NAN_MAX_CONN_CFG;
	prNanDvcCapabilities->max_datapath_sessions =
		NAN_MAX_SUPPORT_NDP_NUM * NAN_MAX_CONN_CFG;
	prNanDvcCapabilities->max_avail_maps = NAN_TIMELINE_MGMT_SIZE;
	prNanDvcCapabilities->max_nan_interfaces = NAN_NDI_INDEX_NUM;
	prNanDvcCapabilities->unicast_insecure_datapath_restore_supported =
		TRUE;
#if (CFG_SUPPORT_NAN_R4_VENDOR_PAIRING == 1)
	prNanDvcCapabilities->nan_host_pairing =
		prAdapter->rWifiVar.fgEnNanPairing;
#endif
	/* other field filled by PAL */

	DBGLOG(NAN, INFO,
		"sb[0x%x] opmode[%u] an[0x%x] cs[%u] pub[%u] sub[%u] peer[%u]",
		prNanDvcCapabilities->supported_bands,
		prNanDvcCapabilities->operation_mode_ht_vht,
		prNanDvcCapabilities->number_of_antennas,
		prNanDvcCapabilities->max_channel_switch_time,
		prNanDvcCapabilities->max_publishers,
		prNanDvcCapabilities->max_subscribers,
		prNanDvcCapabilities->max_peer_entries);
	DBGLOG(NAN, INFO,
		"dp[%u] map[%u] ndi[%u] restore[%u] pairing[%u]",
		prNanDvcCapabilities->max_datapath_sessions,
		prNanDvcCapabilities->max_avail_maps,
		prNanDvcCapabilities->max_nan_interfaces,
		prNanDvcCapabilities->
			unicast_insecure_datapath_restore_supported,
		prNanDvcCapabilities->nan_host_pairing);
}

#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_FRAME_PROT == 1)
enum NanStatusType
nanDevEnableGtkReqChecker(struct ADAPTER *prAdapter,
			uint8_t ucNdiIdx, uint8_t fgEnable)
{
	struct _GL_NAN_INFO_T *prNANInfo = (struct _GL_NAN_INFO_T *)NULL;

	prNANInfo = prAdapter->prGlueInfo->aprNANDevInfo[ucNdiIdx];
	prNANInfo->fgRejectNdpByGtkReq = fgEnable;

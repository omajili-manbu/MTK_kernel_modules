...
nanRetrieveAttrById(uint8_t *pucAttrList, uint16_t u2Length,
	...
	while (u2RemainSize >= OFFSET_OF(struct _NAN_ATTR_HDR_T, aucAttrBody)) {
		...
		u2Length = (uint16_t) NAN_GET_U16(pucPtr + 1);
...
nanNdpAttrUpdateNdp(struct ADAPTER *prAdapter, enum _NAN_ACTION_T eNanAction,
	...
	if (!prAttrNDP) {
		DBGLOG(NAN, ERROR, "prAttrNDP error\n");
		return WLAN_STATUS_INVALID_DATA;
	}
	...
	if (eNanAction == NAN_ACTION_DATA_PATH_REQUEST) {
		...
		kalMemCopy(prNDP->aucPeerNDIAddr, prAttrNDP->aucInitiatorNDI,
			   MAC_ADDR_LEN);
		DBGLOG(NAN, INFO, "Update aucPeerNDIAddr:" MACSTR
...
nanNdpeAttrUpdateNdp(IN struct ADAPTER *prAdapter,
	...
	if (!prAttrNDPE) {
		DBGLOG(NAN, ERROR, "prAttrNDPE error\n");
		return WLAN_STATUS_INVALID_DATA;
	}
	...
	if (eNanAction == NAN_ACTION_DATA_PATH_REQUEST) {
		...
		DBGLOG(NAN, INFO, "ctrl:0x%x, Gtk:%u\n",
			prAttrNDPE->ucNDPEControl, prNDP->fgRxGtkRequired);

		if ((prNDP->fgTxGtkRequired) &&
			(prNDP->fgRxGtkRequired == FALSE)) {
			DBGLOG(NAN, WARN,
				"Rejected by GtkEnforced in NDPE checker\n");
			prNDP->fgRejectPending = TRUE;
			prNDP->ucNDPSetupStatus = NAN_ATTR_NDP_STATUS_REJECTED;
			prNDP->ucReasonCode = NAN_REASON_CODE_SECURITY_POLICY;
			return WLAN_STATUS_FAILURE;
		}
#endif
	} else if (eNanAction == NAN_ACTION_DATA_PATH_RESPONSE) {
	...
	DBGLOG(NAN, INFO, "NDPE TLV len = %d\n",
...
nanNdlParseAttributes(struct ADAPTER *prAdapter,
	...
	if (!pucNanAttrList) {
		DBGLOG(NAN, ERROR, "pucNanAttrList error\n");
		return WLAN_STATUS_INVALID_DATA;
	}
...
nanNdlAttrUpdateNdl(struct ADAPTER *prAdapter,
	...
	if (!prAttrNDL) {
		DBGLOG(NAN, ERROR, "prAttrNDL error\n");
		return WLAN_STATUS_INVALID_DATA;
	}
...
nanNdlQosAttrUpdateNdl(struct ADAPTER *prAdapter,
	...
	if (!prNdlQosAttr) {
		DBGLOG(NAN, ERROR, "prNdlQosAttr error\n");
		return WLAN_STATUS_INVALID_DATA;
	}
...
nanCipherSuiteAttrHandler(
	...
	DBGLOG_MEM8(NAN, INFO, (uint8_t *)prCipherSuiteAttr,
		(sizeof(struct _NAN_ATTR_CIPHER_SUITE_INFO_T) + u2Length - 1));
#endif

#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_FRAME_PROT == 1)
	prNDP->ucRxSecurityCap = prCipherSuiteAttr->ucCapabilities;

	while (u2Length >= sizeof(struct _NAN_CIPHER_SUITE_ATTRIBUTE_T)) {
		...
		u2Length -= sizeof(struct _NAN_CIPHER_SUITE_ATTRIBUTE_T);
		prCipherSuite++;
	}

	DBGLOG(NAN, INFO, "SecCap:%u, IsGtk:%u, Csid:%u\n",
		prNDP->ucRxSecurityCap, prNDP->fgRxGtkRequired,
		prNDP->ucRxGtkCipherType);

	if (prNDP->fgTxGtkRequired) {
		ucGtkSupport = prCipherSuiteAttr->ucCapabilities &
		...
		if ((ucGtkSupport ==
			...
			(prNDP->ucRxGtkCipherType == 0)) {
			DBGLOG(NAN, WARN,
				"Rejected by GtkEnforced in CSIA checker\n");
			prNDP->fgRejectPending = TRUE;
			prNDP->ucNDPSetupStatus = NAN_ATTR_NDP_STATUS_REJECTED;
			prNDP->ucReasonCode = NAN_REASON_CODE_SECURITY_POLICY;
			return WLAN_STATUS_FAILURE;
	...
	DBGLOG(NAN, INFO, "u1RmtCipher:%d, u1RmtCipherPId:%d\n",
...
nanSecContextAttrHandler(
	...
	DBGLOG(NAN, INFO, "u1RmtScidPId:%d\n",
	       prNDP->u1RmtScidPId);
...
#if (ENABLE_NDP_UT_LOG == 1)
	DBGLOG(NAN, INFO, "Enter, u2RxSkdAttrOffset:%d\n",
	       prNDP->u2RxSkdAttrOffset);
#endif

	if (!prAttrSharedKeyDescriptor) {
		DBGLOG(NAN, ERROR,
			"prAttrSharedKeyDescriptor error\n");
		return WLAN_STATUS_INVALID_DATA;
	}
...
nanNdpParseAttributes(struct ADAPTER *prAdapter,
	...
	if (fgExistSKD) {
		rStatus = nanSecRxKdeAttr(prNDP,
			(uint8_t) NAN_ACTION_TO_MSG(eNanAction),
			prNDP->u2KdeLen,
			prNDP->pucKdeInfo,
			prNDP->u2RxMsgLen,
			prNDP->pucRxMsgBuf);
		...
		if ((prNDP->fgTxGtkRequired) &&
			(rStatus != WLAN_STATUS_SUCCESS)) {
			DBGLOG(NAN, ERROR, "reject due to security error\n");
			prNDP->fgRejectPending = TRUE;
			prNDP->ucNDPSetupStatus = NAN_ATTR_NDP_STATUS_REJECTED;
			prNDP->ucReasonCode = NAN_REASON_CODE_SECURITY_POLICY;
		}
...
nanDataEngineNDPAttrAppendImpl(IN struct ADAPTER *prAdapter,
	...
	uint8_t *pucOffset = NULL;
	uint16_t u2AttrOptlLength = 0;
	...
	if (prNDP == NULL && prPeerAttrNDP == NULL) {
		DBGLOG(NAN, ERROR,
		       "unexpected condition: invalid parameters\n");
		return;
	}
	...
	u2AttrLength = nanDataEngineNDPAttrLength(prAdapter, prNDL, prNDP);
	u2AttrOptlLength = sizeof(struct _NAN_ATTR_NDP_T)
		- OFFSET_OF(struct _NAN_ATTR_NDP_T, ucPublishID);
	...
	if (u2AttrLength != 0) {
		...
		if (prNDP == NULL) {
			...
			/* TODO: should use msdu_len -
			 * u2FrameLength as dst len
			 */
			if (prAttrNDP->ucNDPControl &
			    NAN_ATTR_NDP_CTRL_RESP_NDI_PRESENT) {
				if (prNDP->eNDPRole == NAN_PROTOCOL_INITIATOR)
					kalMemCpyS(pucOffset, u2AttrOptlLength,
						prNDP->aucPeerNDIAddr,
						MAC_ADDR_LEN);
...
nanDataEngineNDLAttrLength(struct ADAPTER *prAdapter,
	...
	if (fgGenerateNDL == TRUE) {
		...
		nanSchedNegoGetImmuNdlScheduleList(prAdapter,
			prNDL->aucPeerMacAddr, &pucScheduleList,
...
nanDataEngineElemContainerAttrLength(struct ADAPTER *prAdapter,
...
#if (CFG_SUPPORT_NAN_R4 == 1) && (CFG_SUPPORT_802_11AX == 1)
	uint8_t aucElements[50] = {0};
	struct BSS_INFO *prBssInfo = NULL;
	uint8_t ucHeCapLen = 0, ucHeOpLen = 0;
	...
	if ((prNDP == NULL) || (prNDL == NULL)) {
		DBGLOG(NAN, INFO, "prNDP/prNDL error\n");
		return 0;
	}
...
nanDataEngineNdcAttrAppend(struct ADAPTER *prAdapter,
	...
	nanSchedNegoGetSelectedNdcAttr(prAdapter, prNDL->aucPeerMacAddr,
		&pucNdcAttr,
		&pu4NdcAttrLength);
...
nanDataEngineNdlQosAttrLength(struct ADAPTER *prAdapter,
	...
	nanSchedNegoGetQosAttr(prAdapter,
		prNDL->aucPeerMacAddr, &pucQosAttr,
		&u4QosLength);
...
nanDataEngineNdlQosAttrAppend(struct ADAPTER *prAdapter,
	...
	nanSchedNegoGetQosAttr(prAdapter, prNDL->aucPeerMacAddr,
...
/*----------------------------------------------------------------------------*/
/*!
 * \brief            NAN Attribute Length Estimation - NAN Vendor Specific ATTR
 *
 * \param[in]
 *
 * \return Status
...
/*----------------------------------------------------------------------------*/
uint16_t
nanDataEngineVendorSpecificAttrLength(struct ADAPTER *prAdapter,
				 struct _NAN_NDL_INSTANCE_T *prNDL,
				 struct _NAN_NDP_INSTANCE_T *prNDP)
{
...
/*----------------------------------------------------------------------------*/
/*!
 * \brief            NAN Attribute Length Generation - NAN Vendor Specific ATTR
 *
 * \param[in]
 *
 * \return Status
...
/*----------------------------------------------------------------------------*/
void
nanDataEngineVendorSpecificAttrAppend(struct ADAPTER *prAdapter,
				 struct MSDU_INFO *prMsduInfo,
				 struct _NAN_NDL_INSTANCE_T *prNDL,
				 struct _NAN_NDP_INSTANCE_T *prNDP)
{
...
nanDataEngineNDPEAttrAppendImpl(IN struct ADAPTER *prAdapter,
	...
	u2AttrLength = nanDataEngineNDPEAttrLength(prAdapter, prNDL, prNDP);
	u2AttrOptlLength = sizeof(struct _NAN_ATTR_NDPE_T)
		- OFFSET_OF(struct _NAN_ATTR_NDPE_T, ucPublishID);
...
nanDataEngineSetupStaRec(IN struct ADAPTER *prAdapter,
	...
	if (nanGetPeerDevCapability(prAdapter, ENUM_NAN_DEVCAP_RX_ANT_NUM,
		...
		ucPeerBW = nanGetPeerMinBw(prAdapter, prNDL->aucPeerMacAddr,
		       prBssInfo->eBand);
		DBGLOG(NAN, INFO, "PeerBW %d , PeerNSS %d\n",
		       ucPeerBW, u4PeerNSS);
	} else {
		ucPeerBW = 20;
		/* whsu */
		u4PeerNSS = 1;
		DBGLOG(NAN, INFO, "Use Default PeerBW %d , PeerNSS %d\n",
		       ucPeerBW, u4PeerNSS);
	}
...
nanDataEngineAllocStaRec(IN struct ADAPTER *prAdapter,
	...
	if (*pprStaRec == NULL) {
		...
		*pprStaRec = cnmStaRecAlloc(prAdapter, STA_TYPE_NAN, ucBssIndex,
							pucPeerAddr);
		if (*pprStaRec == NULL) {
			DBGLOG(NAN, ERROR, "STA-REC allocation failure\n");
			return WLAN_STATUS_FAILURE;
		} else
			DBGLOG(NAN, INFO, "alloc NDP wtbl:%u, staIdx:%u\n",
		...
		DBGLOG(NAN, INFO,
		       "Update STA_REC: ADDR=%02x:%02x:%02x:%02x:%02x:%02x\n",
		...
		DBGLOG(NAN, INFO,
		       "Update STA_REC: Idx:%d, WtblIdx:%d, BssIdx:%d\n",
		       (*pprStaRec)->ucIndex,
		       (*pprStaRec)->ucWlanIndex, ucBssIndex);
		DBGLOG(NAN, INFO,
		       "BSS OwnMac=%02x:%02x:%02x:%02x:%02x:%02x\n",
...
nanGetNdlInstanceByNDI(struct ADAPTER *prAdapter, uint8_t *pucPeerMacAddr)
{
	size_t szNdlIndex = 0;
	size_t szNdpCxtIdx = 0;
	...
	for (szNdlIndex = 0;
		  szNdlIndex < NAN_MAX_SUPPORT_NDL_NUM;
		  szNdlIndex++) {
		...
		if (EQUAL_MAC_ADDR(prNDL->aucPeerMacAddr, pucPeerMacAddr))
			return prNDL;

		for (szNdpCxtIdx = 0; szNdpCxtIdx < NAN_MAX_SUPPORT_NDP_CXT_NUM;
			  szNdpCxtIdx++) {
			if (prNDL->arNdpCxt[szNdpCxtIdx].fgValid == FALSE)
				continue;

			if (EQUAL_MAC_ADDR(
				prNDL->arNdpCxt[szNdpCxtIdx].aucPeerNDIAddr,
				pucPeerMacAddr))
				return prNDL;
		}
	}
...
uint8_t
nanGetNdlCurrentBssIdxByNDI(struct ADAPTER *prAdapter, uint8_t *pucPeerMacAddr)
{
	struct _NAN_NDL_INSTANCE_T *prNDL = NULL;
	enum ENUM_BAND eBand = BAND_NULL;

	prNDL = nanGetNdlInstanceByNDI(prAdapter, pucPeerMacAddr);
	if (prNDL)
		eBand = nanSchedGetSchRecBandByMac(prAdapter,
			prNDL->aucPeerMacAddr);

	return nanGetBssIdxbyBand(prAdapter, eBand);
}
...
nanDataEngineFreeStaRec(IN struct ADAPTER *prAdapter,
	...
	if (atomic_dec_return(&((*pprStaRec)->NanRefCount)) == 0) {
		if ((*pprStaRec)->ucWlanIndex != WTBL_RESERVED_ENTRY)
...
uint8_t
nanDataEngineCmpPeerNmiNdi(IN struct ADAPTER *prAdapter,
			IN struct _NAN_NDL_INSTANCE_T *prNDL,
			IN struct _NAN_NDP_INSTANCE_T *prNDP)
{
	if (kalMemCmp(prNDL->aucPeerMacAddr,
			prNDP->aucPeerNDIAddr, MAC_ADDR_LEN) == 0)
		return TRUE;
...
uint8_t
nanDataEngineCmpLocalNmiNdi(IN struct ADAPTER *prAdapter,
			IN struct _NAN_NDP_INSTANCE_T *prNDP)
{
	if (kalMemCmp(prAdapter->rDataPathInfo.aucLocalNMIAddr,
...
/*----------------------------------------------------------------------------*/
uint32_t
nanDataEngineActiveStaRec(IN struct ADAPTER *prAdapter,
			IN struct _NAN_NDL_INSTANCE_T *prNDL,
			IN uint8_t ucRcpi,
			struct STA_RECORD **pprStaRec)
{
	...
	if (*pprStaRec != NULL) {
		...
		DBGLOG(NAN, INFO,
			"Update STA_REC: Idx:%d, WtblIdx:%d, BssIdx:%d\n",
			(*pprStaRec)->ucIndex,
			(*pprStaRec)->ucWlanIndex, (*pprStaRec)->ucBssIndex);
		DBGLOG(NAN, INFO,
			"BSS OwnMac=%02x:%02x:%02x:%02x:%02x:%02x\n",
			prBssInfo->aucOwnMacAddr[0],
		...
		/* NAN devices always support QoS */
		(*pprStaRec)->fgIsQoS = TRUE;
...
nanDataEngineEnrollNMIContext(IN struct ADAPTER *prAdapter,
	...
	struct _NAN_NDP_CONTEXT_T *prNdpCxt = NULL;
	struct _NAN_NDP_INSTANCE_T *prTargetNdpSA = NULL;
	struct _NAN_NDP_INSTANCE_T *prNdpTmp = NULL;
	unsigned char fgSecurityRequired = 0;
	...
	prNmiCxtKey = nanSecGetNmiCxtKey(prAdapter, pucPeerNMI);

	if (nanDataEngineCmpLocalNmiNdi(prAdapter, prNDP) &&
			nanDataEngineCmpPeerNmiNdi(prAdapter, prNDL, prNDP)) {

		if (prAdapter->rWifiVar.fgEnNanKeyMgmt) {
			/* reuse WTBL entry from NDP context */
			prNdpCxt = nanDataEngineSearchNDPContext(prAdapter,
					prNDL, pucLocalNMI, pucPeerNMI);
			if (prNmiCxtKey && prNdpCxt &&
				(prNdpCxt->prNanStaRec != NULL)) {
				if ((prNmiCxtKey->u2WtblIdx !=
					WTBL_RESERVED_ENTRY) &&
				...
				nanSecManageKeyCmd(prAdapter,
					...
					0, 0, 0, NULL, NULL,
					FALSE, prNmiCxtKey->fgIsKeyExist);
			}
		}
	...
	if (u4Idx == prNdpCxt->ucNumEnrollee) {
		...
		if (prNdpCxt->ucNumEnrollee > NAN_MAX_SUPPORT_NDP_NUM) {
			DBGLOG(NAN, ERROR,
				"Exceed NAN_MAX_SUPPORT_NDP_NUM\n");
			return WLAN_STATUS_FAILURE;
		}
	...
	if (prAdapter->rWifiVar.fgUseNmiCxtKey &&
		...
		if (prNmiCxtKey && (prNmiCxtKey->u2WtblIdx !=
			WTBL_RESERVED_ENTRY)) {
			prNmiCxtKey->fgIsKeyExist = fgSecurityRequired;
			nanSecConfigNmiTk(prAdapter,
				fgSecurityRequired,
				prNmiCxtKey->u2WtblIdx,
				pucLocalNMI, pucPeerNMI,
				prTargetNdpSA);
			...
			DBGLOG(NAN, ERROR,
				"[NmiCxt]u2NmiCxtWidx error (prNmiCxtKey:%p, wtbl:%u)\n",
				prNmiCxtKey,
				(prNmiCxtKey ?
				prNmiCxtKey->u2WtblIdx :
				(uint32_t)WTBL_RESERVED_ENTRY));
			return WLAN_STATUS_FAILURE;
		}
	} else {
		for (u4Idx = 0;
			u4Idx < NAN_MAX_SUPPORT_PER_NDP_STA_REC_NUM;
			u4Idx++) {
			if (nanDataEngineAllocStaRec(prAdapter,
				prNDL, ucBssIndex,
				pucPeerNMI,
				prNDP->ucRCPI,
				&prNdpCxt->prNanStaRec) !=
		...
		for (u4Idx = 0;
			...
			prNanStaRec = prNdpCxt->prNanStaRec;
			if (!prNanStaRec) {
				DBGLOG(NAN, ERROR, "prNanStaRec error\n");
				return WLAN_STATUS_FAILURE;
			}

			if (nanDataEngineActiveStaRec(prAdapter,
				prNDL,
				prNDP->ucRCPI,
				&prNanStaRec) !=
				WLAN_STATUS_SUCCESS)
			...
			if (prAdapter->rWifiVar.fgEnNanKeyMgmt) {
				...
				if (prNmiCxtKey) {
					...
					DBGLOG(NAN, ERROR,
						"[NmiCxt]u2NmiCxtWidx error (prNmiCxtKey:NULL)\n");
				}
			}
		}
	}
#if (ENABLE_NDP_UT_LOG == 1)
	DBGLOG(NAN, INFO, "Summary\n");
	DBGLOG(NAN, INFO,
...
nanDataEngineUnrollNMIContext(IN struct ADAPTER *prAdapter,
	...
	if (nanDataEngineCmpLocalNmiNdi(prAdapter, prNDP) &&
		...
		if (prAdapter->rWifiVar.fgEnNanKeyMgmt) {
			/* reserve WTBL entry from NDP context */
			if (prNdpCxt->prNanStaRec &&
				(prNDP->fgNDPEstablish == TRUE) &&
			    (atomic_read(
				&(prNdpCxt->prNanStaRec->NanRefCount)) == 1))
				prNdpCxt->prNanStaRec->ucWlanIndex =
					WTBL_RESERVED_ENTRY;
		}
	...
	if (u4Idx == NAN_MAX_SUPPORT_NDP_NUM)
		return WLAN_STATUS_FAILURE;
	for (u4Idx2 = u4Idx; u4Idx2 < (NAN_MAX_SUPPORT_NDP_NUM - 1); u4Idx2++)
		prNdpCxt->aprEnrollNdp[u4Idx2] =
			prNdpCxt->aprEnrollNdp[u4Idx2 + 1];
	prNdpCxt->ucNumEnrollee--;
	...
	if (prAdapter->rWifiVar.fgUseNmiCxtKey &&
		...
		/* update SA with strongest security */
		prTargetNdpSA = NULL;
		if ((u4Idx == 0) && (prNdpCxt->ucNumEnrollee > 0)) {
			prTargetNdpSA = prNdpCxt->aprEnrollNdp[0];
			fgSecurityRequired = prTargetNdpSA->fgSecurityRequired;
		...
		if (prNdpCxt->ucNumEnrollee == 0) {
			if (prNdpCxt->prNanStaRec &&
				...
				> 0) {
				DBGLOG(NAN, WARN, "STA-REC RefCount:%u\n",
					(uint32_t)atomic_read(
					&(prNdpCxt->prNanStaRec->NanRefCount)));
			}
...
nanDataEngineEnrollNDPContext(IN struct ADAPTER *prAdapter,
	...
	struct _NAN_NDC_CTRL_T *prNDC = NULL;
	...
	for (u4Idx = 0;
		...
		if (nanDataEngineActiveStaRec(prAdapter, prNDL,
						prNDP->ucRCPI,
						&prNdpCxt->prNanStaRec) !=
			WLAN_STATUS_SUCCESS)
			return WLAN_STATUS_FAILURE;
	...
	if (IS_NAN_CAPABILITY_ENABLED(prAdapter,
		...
		/* enroll WTBL to NDC */
		prNDC = nanFindNdcForNdp(prAdapter, prNDP);
		if (prNDC == NULL) {
			DBGLOG(NAN, ERROR,
				"[MCDATA] NDC can not found by NDP\n");
			goto MC_ERR;
		} else {
			prNdcMgmt = nanSchedGetNdcMgmtByIdx(
				prAdapter,
				(size_t)(prNDC->ucNdcIdx));
			if (prNdcMgmt == NULL) {
				DBGLOG(NAN, ERROR,
					"[MCDATA] NDC Mgmt can not found by NDC idx\n");
				goto MC_ERR;
			}
		}

		DBGLOG(NAN, INFO,
			"[MCDATA] Mapping StaIdx:%u,MAC"MACSTR"NDCID"MACSTR"\n",
			prNanStaRec->ucIndex,
			MAC2STR(prNanStaRec->aucMacAddr),
			MAC2STR(prNDC->aucNdcId));

		/* For MC Tx: pucLocalAddr = local NDI/NMI
		 * pucPeerAddr = NDC ID
		 */
		if (nanRegisterMcTxWtblIdx(prAdapter,
			ucNdiIdx,
			prNdcMgmt, &fgFirst) == WLAN_STATUS_SUCCESS) {
...
#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_FRAME_PROT == 1)
		if (nanGetIgtkBigtkInMcRxWtbl(prAdapter,
			prNDL->aucPeerMacAddr) >
			NO_KEY_EXIST_IND) {
			DBGLOG(NAN, STATE,
				"Skip I/BIGTK installation, it may have already been installed.\n");
			prNDL->fgNmiIgtkReady = TRUE;
		}

		if ((prNDL->fgNmiIgtkReady == FALSE) &&
			(prNDP->ucGroupSAUsed >
			NAN_GROUP_SECURITY_ASSOC_USE_GTKSA)) {
			u2WtblIdx = secPrivacySeekForNanEntry(
				prAdapter, aucBCAddr,
				prNDL->aucPeerMacAddr,
				FALSE, 0,
				&fgWtblReUsed);
			if (u2WtblIdx == WTBL_RESERVED_ENTRY) {
				...
				if (prNDP->ucGroupSAUsed >=
					...
					nanSecConfigNmiBigtk(prAdapter,
					TRUE,
					u2WtblIdx,
					aucBCAddr,
					prNDL->aucPeerMacAddr,
...
nanDataEngineUnrollNDPContext(IN struct ADAPTER *prAdapter,
			      IN struct _NAN_NDL_INSTANCE_T *prNDL,
			      IN struct _NAN_NDP_INSTANCE_T *prNDP) {
	uint32_t u4Idx, u4Idx2 = 0;
	uint32_t u4NdpCxtIdx = 0;
	struct _NAN_NDP_CONTEXT_T *prNdpCxt;
	struct _NAN_NDP_INSTANCE_T *prTargetNdpSA;
	enum NAN_BSS_ROLE_INDEX eRole = NAN_BSS_INDEX_MAIN;
	struct _NAN_DATA_PATH_INFO_T *prDataPathInfo;
	unsigned char fgSecurityRequired = 0;
	unsigned char fgNmiCxt = FALSE;
	struct _NAN_KEY_ENTRY_T *prNmiCxtKey = NULL;
	...
	prNmiCxtKey = nanSecGetNmiCxtKey(prAdapter, prNDL->aucPeerMacAddr);
	nanDataEngineUnrollNMIContext(prAdapter, prNDL, prNDP);

	/* Local and peer NDI == NMI */
	if (nanDataEngineCmpLocalNmiNdi(prAdapter, prNDP) &&
	  nanDataEngineCmpPeerNmiNdi(prAdapter, prNDL, prNDP))
		fgNmiCxt = TRUE;
	...
	if ((u4Idx == 0) && (prNdpCxt->ucNumEnrollee > 0)) {
		...
		if (prAdapter->rWifiVar.fgEnNanKeyMgmt && fgNmiCxt) {
			if (prNmiCxtKey &&
				(prNmiCxtKey->u2WtblIdx !=
				WTBL_RESERVED_ENTRY)) {
				/* update key exist flag in FW */
	...
	if ((IS_NAN_CAPABILITY_ENABLED(prAdapter,
		 ...
		 */
		nanReleaseMcTxLookUpEntry(prAdapter,
			NULL,
			prNDP->aucLocalNDIAddr,
			prNDP);
		...
		nanReleaseMcRxWtblIdx(prAdapter,
			prNDP->aucPeerNDIAddr);
#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_FRAME_PROT == 1)
		if ((prNDL->ucNDPNum == 0) && (prNDL->fgNmiIgtkReady == TRUE)) {
			nanReleaseMcRxWtblIdx(prAdapter,
				prNDL->aucPeerMacAddr);
			prNDL->fgNmiIgtkReady = FALSE;
		}
#endif /* CFG_NAN_SUPPORT_R4_GROUP_ADDR_FRAME_PROT */
	}
#endif /* CFG_NAN_SUPPORT_R4_GROUP_ADDR_DATA */
...
#if (CFG_SUPPORT_NAN_R4_VENDOR_PAIRING == 1)
	prPairingInfo = nanPairingInfoSearch(prAdapter, prNDL->aucPeerMacAddr);
	if ((prNDL->ucNDPNum == 0) && (prPairingInfo) &&
		(prPairingInfo->prNmiCxtKey == prNmiCxtKey))
		nanPairingInfoRelease(prAdapter, prPairingInfo);
#endif
...
nanDataEngineSearchNDPContext(IN struct ADAPTER *prAdapter,
	      IN struct _NAN_NDL_INSTANCE_T *prTargetNDL,
	      IN uint8_t *pucLocalAddr, IN uint8_t *pucPeerAddr)
{
	size_t szNdpCxtIdx = 0;
	struct _NAN_NDP_CONTEXT_T *prNdpCxt = NULL;
	size_t szNdlIndex = 0;
	size_t szNdlNum = NAN_MAX_SUPPORT_NDL_NUM;
	struct _NAN_NDL_INSTANCE_T *prNDL = NULL;
	...
	if (prTargetNDL) {
		szNdlIndex = prTargetNDL->ucIndex;
		szNdlNum = szNdlIndex + 1;
	}

	for (; szNdlIndex < szNdlNum; szNdlIndex++) {
		...
		for (szNdpCxtIdx = 0; szNdpCxtIdx < NAN_MAX_SUPPORT_NDP_CXT_NUM;
			...
			if (!EQUAL_MAC_ADDR(prNdpCxt->aucPeerNDIAddr,
				pucPeerAddr))
				continue;

			if (pucLocalAddr &&
				!EQUAL_MAC_ADDR(prNdpCxt->aucLocalNDIAddr,
				pucLocalAddr))
				continue;
...
/* TODO: check */
u_int8_t nanRxLookforStaRec(struct ADAPTER *prAdapter,
	struct SW_RFB *prCurrSwRfb,
#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_DATA == 1)
	struct _NAN_MCDATA_RX_TEST_PATH *prNanMcDataRxTestPath,
#endif
	uint8_t *pucTaAddr,
	...
	void *prRxStatus)
{
	...
	if (prAdapter == NULL || prCurrSwRfb == NULL) {
		DBGLOG(NAN, ERROR, "prAdapter  or prCurrSwRfb error\n");
		return FALSE;
	}
...
#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_DATA == 1)
	if ((prNanMcDataRxTestPath->fgMcDataRxTestCmdIsSet == TRUE) &&
		(kalMemCmp(pucTaAddr, prNanMcDataRxTestPath->aucNewPeerAddr,
		MAC_ADDR_LEN) == 0)) {
		DBGLOG(NAN, INFO, "SA:"MACSTR",DA:"MACSTR"\n",
			MAC2STR(pucTaAddr),
			MAC2STR(pucEthDestAddr));
		prCurrSwRfb->ucStaRecIdx = secLookupStaRecIndexFromTA(
			prAdapter,
			prNanMcDataRxTestPath->aucOriginPeerAddr);
		return TRUE;
	}
#endif /* CFG_NAN_SUPPORT_R4_GROUP_ADDR_DATA */
	if (prRxDescOps && (prRxDescOps->nic_rxd_get_bssid) &&
		(nanGetSpecificBssInfo(prAdapter,
		NAN_BSS_INDEX_MAIN) != NULL) &&
		(prRxDescOps->nic_rxd_get_bssid(prRxStatus) ==
			GET_BSS_INFO_BY_INDEX(prAdapter,
			nanGetBssIdxByRole(prAdapter,
			NAN_BSS_INDEX_MAIN))->ucOwnMacIndex)) {
		if (fgIsBMC) {
#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_DATA == 1)
			prNanMcRxWtbl =
				nanGetMcRxWtblIdx(
			...
			if (prNanMcRxWtbl != NULL)
				prCurrSwRfb->prStaRec =
					&(prNanMcRxWtbl->rStaRec);
			else
				DBGLOG(NAN, ERROR,
					"prNanMcRxWtbl is NULL\n");
#else
			prCurrSwRfb->prStaRec =
				nanDataEngineSearchStaRec(
				prAdapter, NULL, NULL, pucTaAddr);
#endif /* CFG_NAN_SUPPORT_R4_GROUP_ADDR_DATA */
		} else {
			prCurrSwRfb->prStaRec =
				nanDataEngineSearchStaRec(
...
uint8_t
nanDataEngineIsNmiContext(IN struct ADAPTER *prAdapter,
	      IN uint8_t *pucLocalAddr, IN uint8_t *pucPeerAddr)
{
	uint8_t *pucLocalNMI = NULL;
	struct _NAN_NDL_INSTANCE_T *prNdlInstance = NULL;

	pucLocalNMI = prAdapter->rDataPathInfo.aucLocalNMIAddr;

	if (kalMemCmp(pucLocalNMI, pucLocalAddr, MAC_ADDR_LEN) != 0)
		return FALSE;

	prNdlInstance = nanGetNdlInstanceByNDI(prAdapter, pucPeerAddr);
	if (!prNdlInstance ||
		(kalMemCmp(prNdlInstance->aucPeerMacAddr,
...
struct STA_RECORD *
nanDataEngineSearchStaRec(IN struct ADAPTER *prAdapter,
	      IN struct _NAN_NDL_INSTANCE_T *prNDL,
	      IN uint8_t *pucLocalAddr, IN uint8_t *pucPeerAddr)
{
	struct _NAN_NDP_CONTEXT_T *prNdpCxt = NULL;

	prNdpCxt = nanDataEngineSearchNDPContext(prAdapter, prNDL,
		pucLocalAddr, pucPeerAddr);

	return prNdpCxt ? prNdpCxt->prNanStaRec : NULL;
}

u_int8_t
nanIsStaKeyExist(struct STA_RECORD *prStaRec)
{
	...
	IN uint8_t *pucLocalMacAddr)
{
	...
	for (eRole = 0; eRole < NAN_BSS_INDEX_NUM; eRole++) {
		...
		if (cnmGetDbdcIdxByRfBand(prAdapter, prNanBssInfo->eBand) ==
			prMcTxLookUpTbl->prNdcMgmt->eTxDbdcIdx)
			break;
	}

...
struct _NAN_NDC_CTRL_T g_rVendorNdcCtrl = {0};
#endif
...
} __KAL_ATTRIB_PACKED__ __KAL_ATTRIB_ALIGN_4__;
...
struct _NAN_SCHED_EVENT_DW_INTERVAL_T {
	uint8_t uc2gDwInterval;
	uint8_t uc5gDwInterval;
} __KAL_ATTRIB_PACKED__ __KAL_ATTRIB_ALIGN_4__;

uint8_t g_aucNanIEBuffer[NAN_IE_BUF_MAX_SIZE];

uint16_t g_u2MaxChnlSwitchTimeUs = 5000;

struct _NAN_CRB_NEGO_CTRL_T g_rNanSchNegoCtrl = {0};
struct _NAN_CRB_NEGO_TRANSACTION_T
	g_arNegoTrans[NAN_CRB_NEGO_MAX_TRANSACTION] = {0};
...
uint8_t
nanGetTimelineMgmtIndexByBand(struct ADAPTER *prAdapter, enum ENUM_BAND eBand)
{
#if CFG_SUPPORT_DBDC
...
nanGetTimelineMgmtByBand(struct ADAPTER *prAdapter, enum ENUM_BAND eBand)
{
	...
	if (szIdx >= NAN_TIMELINE_MGMT_SIZE)
		szIdx = 0;
...
nanGetTimelineSupportedBand(struct ADAPTER *prAdapter, uint8_t ucIdx)
{
	...
	if ((prNanScheduler->fgEn2g) &&
		(ucIdx == nanGetTimelineMgmtIndexByBand(prAdapter, BAND_2G4)))
		u4SuppBandIdMask |= BIT(BAND_2G4);

	if ((prNanScheduler->fgEn5gH || prNanScheduler->fgEn5gL) &&
		(ucIdx == nanGetTimelineMgmtIndexByBand(prAdapter, BAND_5G)))
...
size_t
nanGetTimelineSupportedChNumByBand(struct ADAPTER *prAdapter,
	enum ENUM_BAND eBand)
{
	if (eBand == BAND_2G4)
		return NAN_SUPPORTED_2G_FAW_CH_NUM;
	else
		return NAN_SUPPORTED_5G_FAW_CH_NUM;
}
...
{
	if (ucIdx < NAN_TIMELINE_MGMT_SIZE)
		return &prAdapter->arNanPotentialChnlList[ucIdx];
	else
		return &prAdapter->arNanPotentialChnlList[0];
}

struct _NAN_POTENTIAL_CHNL_LIST_T *
nanGetPotentialChnlListByMapId(struct ADAPTER *prAdapter, uint8_t ucMapId)
{
	size_t szIdx = 0, szAvailableIdx = NAN_INVALID_MAP_ID;
	size_t szNanActiveTimelineNum = nanGetActiveTimelineMgmtNum(prAdapter);

	for (szIdx = 0;
		szIdx < szNanActiveTimelineNum;
		szIdx++) {
		if (prAdapter->arNanPotentialChnlList[szIdx].ucMapId
			== ucMapId)
			return &prAdapter->arNanPotentialChnlList[szIdx];

		if (prAdapter->arNanPotentialChnlList[szIdx].ucMapId
			== NAN_INVALID_MAP_ID) {
			if (szAvailableIdx == NAN_INVALID_MAP_ID)
				szAvailableIdx = szIdx;
		}
	}

	if (szAvailableIdx >= NAN_TIMELINE_MGMT_SIZE)
		return NULL;

	return &prAdapter->arNanPotentialChnlList[szAvailableIdx];
}
...
static struct _NAN_CRB_NEGO_TRANSACTION_T *
nanGetNegoTransDataByIdx(struct ADAPTER *prAdapter, size_t szIdx)
{
	if (!prAdapter)
		return NULL;

	if (szIdx < NAN_CRB_NEGO_MAX_TRANSACTION)
		return &g_arNegoTrans[szIdx];
	else
		return NULL;
}
	...
	uint16_t u2SlotIdx, uint8_t ucTimeLineIdx)
{
	...
	uint8_t ucIndex2G = nanGetTimelineMgmtIndexByBand(prAdapter, BAND_2G4);
	uint8_t ucIndex5G = nanGetTimelineMgmtIndexByBand(prAdapter, BAND_5G);
...
nanSchedAcquirePeerSchedRecord(struct ADAPTER *prAdapter, uint8_t *pucNmiAddr) {
	...
	for (szSchIdx = 0; szSchIdx < NAN_MAX_CONN_CFG; szSchIdx++) {
		prPeerSchRecord = nanSchedGetPeerSchRecord(prAdapter, szSchIdx);
	...
	prPeerSchDesc->u4SchIdx = szSchIdx;

	nanSchedResetPeerSchedRecord(prAdapter, szSchIdx);
	prPeerSchRecord->fgActive = TRUE;
	kalMemCpyS(prPeerSchRecord->aucNmiAddr,
		sizeof_field(struct _NAN_PEER_SCHEDULE_RECORD_T, aucNmiAddr),
		pucNmiAddr, MAC_ADDR_LEN);
	prPeerSchRecord->prPeerSchDesc = prPeerSchDesc;

	nanSchedCmdManagePeerSchRecord(prAdapter, szSchIdx, TRUE);
	nanSchedCmdUpdatePeerCapability(prAdapter, szSchIdx);

	if (prAdapter->rWifiVar.fgEnNanKeyMgmt)
		nanSecAcquireNmiCxtKey(prAdapter, pucNmiAddr);
...
nanSchedReleasePeerSchedRecord(struct ADAPTER *prAdapter, uint32_t u4SchIdx) {
	...
	if (prAdapter->rWifiVar.fgEnNanKeyMgmt) {
		prNmiCxtKey = nanSecGetNmiCxtKey(
			prAdapter,
...
nanSchedInit(struct ADAPTER *prAdapter) {
	...
	uint8_t ucNanActiveTimelineNum = 0;
	u_int8_t fgDbdcMode = FALSE;
	...
	if (prAdapter->rWifiVar.eDbdcMode > 0)
		fgDbdcMode = TRUE;

	/* Dual Map while DBDC cap is true, Single while off */
	if (fgDbdcMode) {
		prAdapter->fgNanMultipleMapTimeline = TRUE;
		ucNanActiveTimelineNum = NAN_TIMELINE_MGMT_SIZE;
	} else {
		prAdapter->fgNanMultipleMapTimeline = FALSE;
		ucNanActiveTimelineNum = 1;
	}

	prNegoCtrl = nanGetNegoControlBlock(prAdapter);
	prNanScheduler = nanGetScheduler(prAdapter);

	DBGLOG(NAN, INFO, "IN, Init:%d Dbdc:%d\n",
		prNanScheduler->fgInit, fgDbdcMode);
	DBGLOG(NAN, INFO, "Supported Timeline number:%d\n",
		ucNanActiveTimelineNum);
	...
	prNanScheduler->uc2gCommitDwInterval =
		prAdapter->rWifiVar.ucNan2gCommittedDw;
	prNanScheduler->uc5gCommitDwInterval =
		prAdapter->rWifiVar.ucNan5gCommittedDw;
	prNanScheduler->fgAttrDevCapValid = FALSE;

	kalMemZero(prNanScheduler->arNdcMgmt,
		sizeof(prNanScheduler->arNdcMgmt));

#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_DATA == 1)
	for (u4Idx = 0; u4Idx < NAN_MAX_NDC_MGMT_NUM; u4Idx++) {
		for (szNdiIdx = 0; szNdiIdx < NAN_MAX_MC_TX_WTBL_NUM;
			szNdiIdx++) {
			prNanScheduler->arNdcMgmt[u4Idx].
				au2BmcTxWtblIdx[szNdiIdx] =
				WTBL_RESERVED_ENTRY;
			atomic_set(
				&(prNanScheduler->arNdcMgmt[u4Idx].
					mcTxRefCount[szNdiIdx]),
				0);
		}
		prNanScheduler->arNdcMgmt[u4Idx].rNdcCtrl.ucNdcIdx =
			(uint8_t)u4Idx;
	}
#endif

	for (ucTimeLineIdx = 0; ucTimeLineIdx < ucNanActiveTimelineNum;
		ucTimeLineIdx++) {
		prNanTimelineMgmt =
		...
		kalMemZero(prAdapter->arNanPotentialChnlList,
				sizeof(prAdapter->arNanPotentialChnlList));
	...
	if (prAdapter->rWifiVar.ucNanVendorIoctl) {
		...
		kalMemZero((uint8_t *)&g_rVendorNdcCtrl,
...
#if (CFG_SUPPORT_NAN_CUST_DW_CHNL == 0)
		prScheduler = nanGetScheduler(prAdapter);
		ucIndex2G = nanGetTimelineMgmtIndexByBand(prAdapter, BAND_2G4);
		ucIndex5G = nanGetTimelineMgmtIndexByBand(prAdapter, BAND_5G);
...
nanGetPeerChnlInfoBySlot(struct ADAPTER *prAdapter, uint32_t u4SchIdx,
	...
	prPeerSchDesc = nanSchedGetPeerSchDesc(prAdapter, u4SchIdx);
	if (prPeerSchDesc == NULL || u4AvailDbIdx == NAN_INVALID_AVAIL_DB_INDEX)
		return g_rNullChnl;
...
nanSchedAcquireNdcCtrl(struct ADAPTER *prAdapter) {
	...
	for (szIdx = 0; szIdx < NAN_MAX_NDC_MGMT_NUM; szIdx++) {
		if (prScheduler->arNdcMgmt[szIdx].rNdcCtrl.fgValid == FALSE) {
			...
			for (szIdx2 = 0; szIdx2 < NAN_TIMELINE_MGMT_SIZE;
				szIdx2++) {
				prNdcCtrl->arTimeline[szIdx2].ucMapId =
					NAN_INVALID_MAP_ID;
			}
#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_DATA == 1)
			for (szIdx2 = 0; szIdx2 < NAN_MAX_MC_TX_WTBL_NUM;
				szIdx2++) {
				prScheduler->arNdcMgmt[szIdx].
					au2BmcTxWtblIdx[szIdx2] =
					WTBL_RESERVED_ENTRY;
				atomic_set(&(prScheduler->arNdcMgmt[szIdx].
					mcTxRefCount[szIdx2]), 0);
			}
			prScheduler->arNdcMgmt[szIdx].eTxDbdcIdx =
				ENUM_BAND_NUM;
#endif
...
nanSchedAcquireChnlTimeline(struct ADAPTER *prAdapter,
	...
	size_t szSupported2gChNum =
		nanGetTimelineSupportedChNumByBand(prAdapter, BAND_2G4);
	size_t szSupported5gChNum =
		nanGetTimelineSupportedChNumByBand(prAdapter, BAND_5G);
	size_t sz2gChNum = 0;
	size_t sz5gChNum = 0;
	...
	if (u4InvalidIdx != NAN_TIMELINE_MGMT_CHNL_LIST_NUM) {
		if (nanRegGetNanChnlBand(*prChnlInfo) == BAND_2G4) {
			sz2gChNum++;
			if (sz2gChNum > szSupported2gChNum) {
				DBGLOG(NAN, ERROR,
					"2g chnl num (%zu) exceed cap (%zu), map (%u)\n",
					sz2gChNum, szSupported2gChNum,
					prAdapter->fgNanMultipleMapTimeline);
				return NULL;
			}
		} else {
			sz5gChNum++;
			if (sz5gChNum > szSupported5gChNum) {
				DBGLOG(NAN, ERROR,
					"5g chnl num (%zu) exceed cap (%zu), map (%u)\n",
					sz5gChNum, szSupported5gChNum,
					prAdapter->fgNanMultipleMapTimeline);
				return NULL;
			}
		}
...
nanSchedMergeAvailabileChnlList(struct ADAPTER *prAdapter,
	...
	struct _NAN_CHANNEL_TIMELINE_T *prChnlTimelineList;
	struct _NAN_TIMELINE_MGMT_T *prNanTimelineMgmt;
	uint8_t ucNanActiveTimelineNum = nanGetActiveTimelineMgmtNum(prAdapter);

	for (ucTimeLineIdx = 0;
		ucTimeLineIdx < ucNanActiveTimelineNum; ucTimeLineIdx++) {
...
nanSchedPeerPurgeOldCrb(struct ADAPTER *prAdapter, uint32_t u4SchIdx) {
	...
	do {
		...
		for (u4TimeLineIdx = 0; u4TimeLineIdx < ucNanActiveTimelineNum;
			u4TimeLineIdx++) {
			kalMemZero((uint8_t *)
				prPeerSchRecord
...
nanSchedPeerChkQos(struct ADAPTER *prAdapter,
	...
	uint32_t i4Latency;
	uint32_t u4Idx1;
	uint32_t au4UsedAvailMap[NAN_TOTAL_DW] = {0};
	uint8_t ucTimeLineIdx;
	uint8_t ucNanActiveTimelineNum = nanGetActiveTimelineMgmtNum(prAdapter);
...
nanSchedPeerUpdateCommonFAW(struct ADAPTER *prAdapter, uint32_t u4SchIdx) {
	...
	for (icTimeLineIdx = (ucNanActiveTimelineNum - 1);
		icTimeLineIdx >= 0;
		icTimeLineIdx--) {

		ucTimeLineIdx = (uint8_t) icTimeLineIdx;
...
nanSchedReleaseUnusedCommitSlot(struct ADAPTER *prAdapter) {
	...
	struct _NAN_SCHEDULER_T *prScheduler;
	uint8_t ucTimeLineIdx;
	uint8_t  ucNanActiveTimelineNum =
...
#endif
	{
		prScheduler = nanGetScheduler(prAdapter);
		for (ucTimeLineIdx = 0;
			ucTimeLineIdx < ucNanActiveTimelineNum;
			ucTimeLineIdx++) {
		...
		if (!nanGetFeatureIsSigma(prAdapter)) {
			/* Keep DW timeline */
			nanSchedSetDwTimeline(prAdapter);
		}
#endif
	...
	do {
		...
		for (szTimeLineIdx = 0;
			...
			for (szIdx = 0;
				...
				szIdx++) {
				prChnlTimeline =
					&prNanTimelineMgmt->arCustChnlList
						[szIdx];
				if (prChnlTimeline->fgValid == FALSE)
					continue;
			...
			/* 1.2 bitmap check */
			szNum = nanUtilCheckBitOneCnt(prAdapter,
				(uint8_t *)prTimeline->au4AvailMap,
				sizeof(prTimeline->au4AvailMap));
		...
		nanUtilDump(prAdapter, "Config NDC ID",
			    prNdcCtrl->aucNdcId,
			    NAN_NDC_ATTRIBUTE_ID_LENGTH);

		/* Copy to global variable to
		 * prevent being cleared
		 * when drop resource
		 */
		kalMemCpyS(&g_rVendorNdcCtrl,
			   sizeof(struct _NAN_NDC_CTRL_T),
			   prNdcCtrl,
			   sizeof(struct _NAN_NDC_CTRL_T));
...
uint32_t
nanSchedNegoCustFawEnableDualMapCmd(struct ADAPTER *prAdapter)
{
#if CFG_SUPPORT_DBDC
...
nanSchedNegoCustFawConfigCmd(struct ADAPTER *prAdapter, uint8_t ucChnl,
...
#endif
	enum ENUM_BAND eBand;
	size_t szSupported2gChNum = 0;
	size_t szSupported5gChNum = 0;
	size_t sz2gChNum = 0;
	size_t sz5gChNum = 0;
	uint8_t fgAddNewCustFaw = TRUE;
	...
	for (u4Idx = 0;
		u4Idx < ARRAY_SIZE(prNanTimelineMgmt->arCustChnlList);
		u4Idx++) {
		...
		/* empty conflict slots-
		 * new cust faw config command overrides the old one
		 */
		for (u4DwIdx = 0; u4DwIdx < NAN_TOTAL_DW; u4DwIdx++)
	...
	if (u4TargetIdx != NAN_TIMELINE_MGMT_CHNL_LIST_NUM) {
		...
		/* check ch capability */
		if (prChnlTimeline->fgValid == FALSE) {
			if (nanRegGetNanChnlBand(rChnlInfo) == BAND_2G4) {
				sz2gChNum++;
				if (sz2gChNum > szSupported2gChNum) {
					DBGLOG(NAN, ERROR,
					"2g chnl num (%zu) exceed cap (%zu), map (%u)\n",
		...
		if (fgAddNewCustFaw) {
			...
			for (u4DwIdx = 0; u4DwIdx < NAN_TOTAL_DW; u4DwIdx++)
				prChnlTimeline->au4AvailMap[u4DwIdx] |=
					u4SlotBitmap;
			prChnlTimeline->i4Num = (int32_t)nanUtilCheckBitOneCnt(
				prAdapter,
				(uint8_t *)prChnlTimeline->au4AvailMap,
				sizeof(prChnlTimeline->au4AvailMap));
...
static size_t
nanSchedNegoLookupTransIdx(struct ADAPTER *prAdapter, uint8_t *pucNmiAddr)
{
	struct _NAN_CRB_NEGO_TRANSACTION_T *prNegoTrans = NULL;
	size_t szSchIdx = 0, szTransIdx = 0;

	if (pucNmiAddr == NULL)
		return NAN_CRB_NEGO_MAX_TRANSACTION;

	szSchIdx = nanSchedLookupPeerSchRecordIdx(prAdapter, pucNmiAddr);

	if (szSchIdx == NAN_INVALID_SCH_RECORD)
		return NAN_CRB_NEGO_MAX_TRANSACTION;

	for (szTransIdx = 0;
		szTransIdx < NAN_CRB_NEGO_MAX_TRANSACTION;
		szTransIdx++) {
		prNegoTrans = nanGetNegoTransDataByIdx(prAdapter, szTransIdx);
		if (prNegoTrans && (prNegoTrans->szSchIdx == szSchIdx))
			return szTransIdx;
	}
...
uint32_t
nanSchedNegoClearDb(struct ADAPTER *prAdapter,
	size_t szIdx)
{
	struct _NAN_CRB_NEGO_TRANSACTION_T *prNegoTrans = NULL;
	...
	prNegoTrans = nanGetNegoTransDataByIdx(prAdapter, szIdx);

	kalMemZero(prNegoTrans,
		   sizeof(struct _NAN_CRB_NEGO_TRANSACTION_T));
...
uint32_t
nanSchedNegoUpdateDb(struct ADAPTER *prAdapter,
	size_t szIdx)
{
	struct _NAN_CRB_NEGO_CTRL_T *prNegoCtrl =
	...
	prNegoTrans->eState = prNegoCtrl->eState;
	prNegoTrans->u4QosMinSlots = prNegoCtrl->u4QosMinSlots;
	prNegoTrans->u4QosMaxLatency = prNegoCtrl->u4QosMaxLatency;
	prNegoTrans->prSelectedNdcCtrl = prNegoCtrl->prSelectedNdcCtrl;
	kalMemCpyS(prNegoTrans->arImmuNdlTimeline,
		sizeof_field(struct _NAN_CRB_NEGO_TRANSACTION_T,
		arImmuNdlTimeline),
		prNegoCtrl->arImmuNdlTimeline,
		sizeof_field(struct _NAN_CRB_NEGO_CTRL_T,
		arImmuNdlTimeline));
	kalMemCpyS(prNegoTrans->arRangingTimeline,
		sizeof_field(struct _NAN_CRB_NEGO_TRANSACTION_T,
		arRangingTimeline),
	...
	DBGLOG(NAN, INFO,
		"TransIdx,%lu,SchIdx,%lu,Role:%u,Type:%u,State,%u\n",
		szIdx,
		prNegoTrans->szSchIdx,
		prNegoCtrl->eRole,
		prNegoCtrl->eType,
		prNegoTrans->eState);
...
uint32_t
nanSchedNegoReloadDb(struct ADAPTER *prAdapter,
	uint8_t *pucNmiAddr)
{
	size_t szTransIdx = 0;
	struct _NAN_CRB_NEGO_CTRL_T *prNegoCtrl =
	...
	szTransIdx = nanSchedNegoLookupTransIdx(prAdapter, pucNmiAddr);

	if (prNegoCtrl->ucNegoCurTransIdx == szTransIdx)
		return WLAN_STATUS_SUCCESS;

	DBGLOG(NAN, INFO, "TransIdx,%lu,CurIdx,%lu,mac:"MACSTR"\n",
		szTransIdx, prNegoCtrl->ucNegoCurTransIdx, MAC2STR(pucNmiAddr));
	...
	/* Reset Db */
	nanSchedNegoInitDb(prAdapter,
			prNegoTrans->szSchIdx,
			prNegoTrans->eType,
			prNegoTrans->eRole);

	prNegoCtrl->ucNegoCurTransIdx = (uint8_t) szTransIdx;

	/* Restore state */
	prNegoCtrl->eState = prNegoTrans->eState;

	/* Restore QoS value */
	prNegoCtrl->u4QosMinSlots = prNegoTrans->u4QosMinSlots;
	prNegoCtrl->u4QosMaxLatency = prNegoTrans->u4QosMaxLatency;

	/* Restore NDC */
	prNegoCtrl->prSelectedNdcCtrl = prNegoTrans->prSelectedNdcCtrl;

	/* Restore ImmuNdl, and Ranging timeline */
	kalMemCpyS(prNegoCtrl->arImmuNdlTimeline,
		sizeof_field(struct _NAN_CRB_NEGO_CTRL_T,
		arImmuNdlTimeline),
		prNegoTrans->arImmuNdlTimeline,
		sizeof_field(struct _NAN_CRB_NEGO_TRANSACTION_T,
		arImmuNdlTimeline));
	kalMemCpyS(prNegoCtrl->arRangingTimeline,
		sizeof_field(struct _NAN_CRB_NEGO_CTRL_T,
		arRangingTimeline),
		prNegoTrans->arRangingTimeline,
		sizeof_field(struct _NAN_CRB_NEGO_TRANSACTION_T,
		arRangingTimeline));
...
nanSchedNegoDispatchTimeout(struct ADAPTER *prAdapter, unsigned long ulParam) {
	...
	struct _NAN_TIMELINE_MGMT_T *prNanTimelineMgmt = NULL;
	uint8_t ucNanActiveTimelineNum = nanGetActiveTimelineMgmtNum(prAdapter);
	uint8_t ucTransNum = 0;
	struct _NAN_NDL_INSTANCE_T *prNDL = NULL;
	struct _NAN_CRB_NEGO_TRANSACTION_T *prNegoTrans = NULL;

	prNegoCtrl = nanGetNegoControlBlock(prAdapter);

	DBGLOG(NAN, INFO, "TransNum:%d,Pending:%d\n",
		prNegoCtrl->ucNegoTransNum,
		prNegoCtrl->ucNegoPendingTransNum);

	ucTransNum = prNegoCtrl->ucNegoTransNum;
	...
	while (ucTransNum > 0) {
		...
		u4SchIdx = nanSchedLookupPeerSchRecordIdx(
			prAdapter, prNegoTrans->aucNmiAddr);
		/* Skip if peer record not found */
		if (u4SchIdx == NAN_INVALID_SCH_RECORD) {
			DBGLOG(NAN, ERROR,
				"u4SchIdx equals NAN_INVALID_SCH_RECORD!");
			DBGLOG(NAN, ERROR,
				"Idx,%lu,mac," MACSTR "\n",
				szTransIdx, MAC2STR(prNegoTrans->aucNmiAddr));
			szTransIdx = (szTransIdx + 1) %
				NAN_CRB_NEGO_MAX_TRANSACTION;
			continue;
		}

		DBGLOG(NAN, INFO,
			"TransNum,%u,Pending,%u,TransIdx,%lu,State,%u\n",
			prNegoCtrl->ucNegoTransNum,
			prNegoCtrl->ucNegoPendingTransNum,
			szTransIdx,
			prNegoTrans->eState);

		/* Find the pending negotiation transaction */
		if (prNegoTrans->eState == ENUM_NAN_CRB_NEGO_STATE_IDLE) {
			nanSchedNegoInitDb(prAdapter, u4SchIdx,
					   prNegoTrans->eType,
					   prNegoTrans->eRole);
			prNegoTrans->szSchIdx = (size_t)u4SchIdx;
			prNegoCtrl->ucNegoCurTransIdx = szTransIdx;
			prNegoCtrl->ucNegoPendingTransNum--;
			...
			prNDL = nanDataUtilSearchNdlByMac(prAdapter,
				prNegoTrans->aucNmiAddr);

			if (prNDL != NULL)
				nanSchedNegoAddQos(prAdapter,
					prNDL->ucMinimumTimeSlot,
					prNDL->u2MaximumLatency);

			nanSchedNegoUpdateDb(prAdapter,
				prNegoCtrl->ucNegoCurTransIdx);

			/* nanSchedNegoDumpState(prAdapter,
			 *  __func__, __LINE__);
			 */

			prNegoTrans->pfCallback(
				prAdapter, prNegoTrans->aucNmiAddr,
				prNegoTrans->eType,
				prNegoTrans->eRole,
				prNegoTrans->pvToken);
		...
		szTransIdx = (szTransIdx + 1) % NAN_CRB_NEGO_MAX_TRANSACTION;

	}

#if (CFG_SUPPORT_NAN_SCHED_PARALLEL_NEGO == 1)
	if (prNegoCtrl->ucNegoPendingTransNum > 0) {
		if (!timerPendingTimer(&(prNegoCtrl->rCrbNegoDispatchTimer))) {
...
nanSchedNegoInitDb(struct ADAPTER *prAdapter, uint32_t u4SchIdx,
	...
	prNegoCtrl->prSelectedNdcCtrl = NULL;
	...
	for (ucTimeLineIdx = 0;
		...
		prNegoCtrl->arImmuNdlTimeline[ucTimeLineIdx].ucMapId =
			NAN_INVALID_MAP_ID;
		prNegoCtrl->arRangingTimeline[ucTimeLineIdx].ucMapId =
			NAN_INVALID_MAP_ID;
	}
	...
	nanSchedNegoAddQos(prAdapter, u4DftNdlQosQuotaVal,
			   u4DftNdlQosLatencyVal);
#if (CFG_SUPPORT_NAN_AVAILABILITY_CONTROL_BY_UPPER_LAYER == 1)
	/* Assign local NDC CRB */
	if (prAdapter->rWifiVar.ucNanVendorIoctl != 0)
		nanSchedNegoGenNdcCrb(prAdapter);
#endif
}
...
nanSchedNegoStart(
	...
	struct _NAN_CRB_NEGO_CTRL_T *prNegoCtrl = NULL;
	struct _NAN_CRB_NEGO_TRANSACTION_T *prNegoTrans = NULL;
	uint8_t aucNmiAddr[MAC_ADDR_LEN] = {0};
	...
	/* Find an empty transaction */
	for (u4Idx = 0; u4Idx < NAN_CRB_NEGO_MAX_TRANSACTION; u4Idx++) {
		prNegoTrans = nanGetNegoTransDataByIdx(prAdapter, u4Idx);
		if ((kalMemCmp(prNegoTrans->aucNmiAddr,
			aucNmiAddr, MAC_ADDR_LEN) == 0) &&
			(prNegoTrans->eState ==
			ENUM_NAN_CRB_NEGO_STATE_IDLE))
			break;
	}

	if (u4Idx >= NAN_CRB_NEGO_MAX_TRANSACTION)
		return WLAN_STATUS_FAILURE;

	prNegoCtrl->ucNegoTransNum++;
	prNegoCtrl->ucNegoPendingTransNum++;

	kalMemCpyS(prNegoTrans->aucNmiAddr,
		sizeof_field(struct _NAN_CRB_NEGO_TRANSACTION_T, aucNmiAddr),
		pucNmiAddr, MAC_ADDR_LEN);
	prNegoTrans->eRole = eRole;
	prNegoTrans->eType = eType;
	prNegoTrans->pfCallback = pfCallback;
	prNegoTrans->pvToken = pvToken;

	nanSchedPeerPrepareNegoState(prAdapter, pucNmiAddr);

	DBGLOG(NAN, INFO, "Num,%u,Pending,%u,Idx,%u,State,%u\n",
		prNegoCtrl->ucNegoTransNum,
		prNegoCtrl->ucNegoPendingTransNum,
		u4Idx,
		prNegoTrans->eState);

#if (CFG_SUPPORT_NAN_SCHED_PARALLEL_NEGO == 0)
	if (prNegoCtrl->eState == ENUM_NAN_CRB_NEGO_STATE_IDLE)
#endif
	{
		if (!timerPendingTimer(&(prNegoCtrl->rCrbNegoDispatchTimer))) {
...
void
nanSchedNegoStop(struct ADAPTER *prAdapter, uint8_t *pucPeerNmiAddr) {
	struct _NAN_CRB_NEGO_CTRL_T *prNegoCtrl = NULL;
	struct _NAN_TIMELINE_MGMT_T *prNanTimelineMgmt = NULL;
	struct _NAN_PEER_SCHEDULE_RECORD_T *prPeerSchRecord = NULL;
	uint8_t ucTimeLineIdx = 0, ucIdx = 0;
	uint8_t ucNanActiveTimelineNum = nanGetActiveTimelineMgmtNum(prAdapter);

	DBGLOG(NAN, LOUD, "IN\n");

	/* Load peer nego info */
	if (nanSchedNegoReloadDb(prAdapter, pucPeerNmiAddr)
		!= WLAN_STATUS_SUCCESS) {
		DBGLOG(NAN, ERROR, "Peer Not found! "MACSTR"\n",
			MAC2STR(pucPeerNmiAddr));
		return;
	}
	...
	/* Write back Nego status */
	nanSchedNegoClearDb(prAdapter, prNegoCtrl->ucNegoCurTransIdx);
	nanSchedNegoDumpState(prAdapter,
	...
	if (prNegoCtrl->ucNegoPendingTransNum > 0) {
		cnmTimerStartTimer(prAdapter,
			&(prNegoCtrl->rCrbNegoDispatchTimer), 1);
	} else if (prNegoCtrl->ucNegoTransNum == 0) {

		/* Clear conditional bitmap after all negotiation done */
		for (ucTimeLineIdx = 0;
			ucTimeLineIdx < ucNanActiveTimelineNum;
			ucTimeLineIdx++) {
			prNanTimelineMgmt =
				nanGetTimelineMgmt(prAdapter, ucTimeLineIdx);
			prNanTimelineMgmt->fgChkCondAvailability = FALSE;
			for (ucIdx = 0; ucIdx < NAN_TIMELINE_MGMT_CHNL_LIST_NUM;
				ucIdx++)
				prNanTimelineMgmt
				->arCondChnlList[ucIdx].fgValid = FALSE;
		}
...
nanSchedNegoGenQosCriteria(struct ADAPTER *prAdapter) {
	...
	if (u4QosMinSlots > NAN_INVALID_QOS_MIN_SLOTS) {
		if (u4QosMinSlots > NAN_QOS_MIN_SLOTS_UP_BOUND)
...
nanSchedNegoIsCrbEqual(
	...
	for (u4Idx = 0; u4Idx < NAN_TOTAL_DW; u4Idx++) {
		...
		for (u4Idx1 = 0; u4Idx1 < ucNanActiveTimelineNum; u4Idx1++) {
...
nanSchedNegoAddNdcCrb(struct ADAPTER *prAdapter,
	...
	do {
		...
		prNdcCtrl = prNegoCtrl->prSelectedNdcCtrl;
		if (prNdcCtrl->fgValid == FALSE) {
...
static void nanSchedNegoGenDefCrbExt(
	struct ADAPTER *prAdapter,
	struct _NAN_CRB_NEGO_CTRL_T *prNegoCtrl,
	union _NAN_BAND_CHNL_CTRL *prRmtChnlInfo,
	union _NAN_BAND_CHNL_CTRL *prLocalChnlInfo,
	enum ENUM_BAND *prRmtBand, enum ENUM_BAND eBand,
	unsigned char fgChkRmtCondSlot,
	uint32_t u4SlotIdx,
	union _NAN_BAND_CHNL_CTRL *prSelChnlInfo,
	uint32_t *pu4DefCrbNum, unsigned char *pfgFound,
	uint8_t ucTimeLineIdx)
{
	uint32_t u4AvailDbIdx = 0;

	for (; u4AvailDbIdx < NAN_NUM_AVAIL_DB;
		u4AvailDbIdx++) {
		if (nanSchedPeerAvailabilityDbValidByID(
			prAdapter, prNegoCtrl->u4SchIdx,
			u4AvailDbIdx) == FALSE)
		...
		*prRmtChnlInfo = nanGetPeerChnlInfoBySlot(
			prAdapter, prNegoCtrl->u4SchIdx,
			u4AvailDbIdx, u4SlotIdx,
			fgChkRmtCondSlot);
		DBGLOG(NAN, LOUD,
			...
			prRmtChnlInfo->rChannel.u4PrimaryChnl);

		*prRmtBand = nanRegGetNanChnlBand(*prRmtChnlInfo);
		if ((prRmtChnlInfo->rChannel.u4PrimaryChnl != 0) &&
			(*prRmtBand != eBand)) {
			DBGLOG(NAN, LOUD,
				"Skip diff band! TIdx,%zu,Slot,%zu,rmtChnl,%u,B=%u,SupB=0x%x\n",
				ucTimeLineIdx, u4SlotIdx,
				prRmtChnlInfo->rChannel.u4PrimaryChnl,
				*prRmtBand, eBand);
			continue;
		}

		if ((prRmtChnlInfo->rChannel.u4PrimaryChnl ==
			 0) &&
			!fgChkRmtCondSlot) {
			if (prLocalChnlInfo->rChannel
				.u4PrimaryChnl == 0) {
				*prSelChnlInfo =
				nanSchedNegoSelectChnlInfo(
					prAdapter,
			...
			*pfgFound = TRUE;
			(*pu4DefCrbNum)--;
			break;
		} else if (prRmtChnlInfo->rChannel.u4PrimaryChnl != 0) {
			if (!nanIsAllowedChannel(
				prAdapter,
				prRmtChnlInfo->rChannel
				.u4PrimaryChnl))
				continue;
			...
			if ((prLocalChnlInfo->rChannel
				...
				*pfgFound = TRUE;
				(*pu4DefCrbNum)--;
				break;
			} else if (prLocalChnlInfo->rChannel
				.u4PrimaryChnl != 0) {
				continue;
			}

			/* no local channel allocation case */
			*prSelChnlInfo = nanSchedConvergeChnlInfo(
				prAdapter, *prRmtChnlInfo);
			if (prSelChnlInfo->rChannel.u4PrimaryChnl
				!= 0) {
				nanSchedAddCrbToChnlList(
				prAdapter,
				prSelChnlInfo,
	...
	for (icTimeLineIdx = (ucNanActiveTimelineNum - 1);
		...
		for (u4DwIdx = 0; u4DwIdx < NAN_TOTAL_DW; u4DwIdx++) {
			...
			for ((u4DefCrbNum = u4DefCrbNumBackup),
				...
				for (eBand = (BAND_NUM - 1);
					!fgFound && (eBand > BAND_NULL);
					eBand--) {
					if (!(u4SuppBandIdMask & BIT(eBand)))
						continue;
...
nanSchedNegoGenNdcCrb(struct ADAPTER *prAdapter) {
	...
	for (u4Idx = 0; u4Idx < NAN_MAX_NDC_MGMT_NUM; u4Idx++) {
		...
		if (ucTimeLineIdx < ucNanActiveTimelineNum)
			break;
	}

	if (u4Idx < NAN_MAX_NDC_MGMT_NUM) {
		/* find a local NDC CRB */
		prNegoCtrl->prSelectedNdcCtrl = prNdcCtrl;
		DBGLOG(NAN, INFO, "Reuse NDC ID [ " MACSTR " ]",
			MAC2STR(prNdcCtrl->aucNdcId));
		rRetStatus = WLAN_STATUS_SUCCESS;
		goto GEN_NDC_DONE;
	}

#if (CFG_SUPPORT_NAN_AVAILABILITY_CONTROL_BY_UPPER_LAYER == 1)
	/* If no existed NDC, request host assign new NDC */
	if (prAdapter->rWifiVar.ucNanVendorIoctl != 0) {
		DBGLOG(NAN, WARN, "No existed NDC, Host need config it\n");
		goto GEN_NDC_DONE;
	}
#endif
	...
	for (ucTimeLineIdx = 0;
		...
		if (u4Num < NAN_SLOTS_PER_DW_INTERVAL) {
			prNegoCtrl->prSelectedNdcCtrl = prNdcCtrl =
				nanSchedAcquireNdcCtrl(prAdapter);
			if (prNdcCtrl == NULL) {
				rRetStatus = WLAN_STATUS_FAILURE;
				break;
			}
			...
			DBGLOG(NAN, INFO,
				"NDC MapID:%d,TIdx:%zu,Bmap,0x%x,0x%x\n",
				prNdcCtrl->arTimeline[ucTimeLineIdx].ucMapId,
				ucTimeLineIdx,
				prNdcCtrl->arTimeline[ucTimeLineIdx]
				.au4AvailMap[0],
			...
			nanUtilDump(prAdapter, "New NDC Map",
				(uint8_t *)prNdcCtrl->arTimeline[ucTimeLineIdx].
					au4AvailMap,
				sizeof(prNdcCtrl->arTimeline[ucTimeLineIdx].
					au4AvailMap));
#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_DATA == 1)
			nanSchedNdcMgmtCmd(prAdapter, prNdcCtrl,
				NAN_NDC_MGMT_CMD_TYPE_ACTIVATE);
			...
			rRetStatus = WLAN_STATUS_SUCCESS;
			break;
			/* just need to create one NDC
			 * from any timeline success
			 */
		} else {
			rRetStatus = WLAN_STATUS_FAILURE;
		}
...
/* create NDC record from peer's NDC proposal */
uint32_t
nanSchedNegoAllocNdcCtrl(struct ADAPTER *prAdapter,
	...
	do {
		...
		for (ucTimeLineIdx = 0;
			...
			DBGLOG(NAN, INFO,
				"NDC MapID:%hhu,TIdx:%u,SlotCnt:%u,Bmap:0x%x,0x%x\n",
				prNdcCtrl->arTimeline[ucTimeLineIdx].ucMapId,
				ucTimeLineIdx, u4Num,
				prNdcCtrl->arTimeline[ucTimeLineIdx]
				.au4AvailMap[0],
	...
	uint8_t *pucPeerNmiAddr)
{
	...
	case ENUM_NAN_CRB_NEGO_STATE_INITIATOR:
		if (prNegoCtrl->eType == ENUM_NAN_NEGO_DATA_LINK) {
			...
			if ((prNegoCtrl->prSelectedNdcCtrl == NULL) ||
				prNegoCtrl->prSelectedNdcCtrl->fgValid ==
				FALSE) {
	...
	nanSchedNegoUpdateDb(prAdapter, prNegoCtrl->ucNegoCurTransIdx);
...
nanSchedNegoDataPathChkRmtCrbProposalForRspState(
...
#if (CFG_SUPPORT_NAN_AVAILABILITY_CONTROL_BY_UPPER_LAYER == 1)
	/* Don't send counter proposal but request host assist */
	if (fgCounterProposal && prAdapter->rWifiVar.ucNanVendorIoctl) {
		u4ReasonCode = NAN_REASON_CODE_IMMUTABLE_UNACCEPTABLE;
		goto DATA_RESPONDER_STATE_DONE;
	}
#endif
	...
	if (prNegoCtrl->prSelectedNdcCtrl &&
		(prNegoCtrl->prSelectedNdcCtrl->fgValid == TRUE)) {
		if ((prPeerSchDesc->rSelectedNdcCtrl.fgValid == TRUE) &&
		    (nanSchedNegoIsCrbEqual(
			     prAdapter,
			     prNegoCtrl->prSelectedNdcCtrl->arTimeline,
...
nanSchedNegoGetImmuNdlScheduleList(struct ADAPTER *prAdapter,
	...
	if (ppucScheduleList)
		*ppucScheduleList = NULL;
	/* Find peer nego info */
	szTransIdx = nanSchedNegoLookupTransIdx(prAdapter, pucPeerNmiAddr);
	if (szTransIdx >= NAN_CRB_NEGO_MAX_TRANSACTION)
		return rRetStatus;

	prNegoTrans = nanGetNegoTransDataByIdx(prAdapter, szTransIdx);

	do {
		if ((prNegoTrans->eState !=
		...
		if (prNegoTrans->eType != ENUM_NAN_NEGO_DATA_LINK)
			break;

		if ((ucNanActiveTimelineNum *
			(1/* Map ID(1) */ + NAN_TIME_BITMAP_FIELD_MAX_LENGTH)) >
			NAN_IE_BUF_MAX_SIZE) {
			*pu4ScheduleListLength = 0U;
		...
		for (ucTimeLineIdx = 0;
			...
			if (prNegoTrans->arImmuNdlTimeline
				[ucTimeLineIdx].ucMapId !=
			    NAN_INVALID_MAP_ID) {
				prScheduleEntry =
	...
	if (pu4ScheduleListLength)
		*pu4ScheduleListLength = (uint32_t)(pucPos - g_aucNanIEBuffer);
...
nanSchedNegoGetRangingScheduleList(struct ADAPTER *prAdapter,
	...
	do {
		...
		if (prNegoTrans->eType != ENUM_NAN_NEGO_RANGING)
			break;
		...
		for (ucTimeLineIdx = 0;
			...
			if (prNegoTrans
				->arRangingTimeline[ucTimeLineIdx]
				.ucMapId !=
			    NAN_INVALID_MAP_ID) {
				prScheduleEntry =
					(struct _NAN_SCHEDULE_ENTRY_T *)pucPos;
...
uint32_t
nanSchedNegoGetQosAttr(struct ADAPTER *prAdapter,
			uint8_t *pucPeerNmiAddr,
			uint8_t **ppucQosAttr,
	...
	do {
		...
		if ((prNegoTrans->u4QosMaxLatency <
		     NAN_INVALID_QOS_MAX_LATENCY) ||
		    (prNegoTrans->u4QosMinSlots >
		    NAN_INVALID_QOS_MIN_SLOTS)) {
			...
			prNdlQosAttr->u2MaxLatency =
				(uint16_t)prNegoTrans->u4QosMaxLatency;
			prNdlQosAttr->ucMinTimeSlot =
				(uint8_t)prNegoTrans->u4QosMinSlots;
		}
		rRetStatus = WLAN_STATUS_SUCCESS;
	} while (FALSE);
	...
	if (pu4QosLength)
		*pu4QosLength = (uint32_t)(pucPos - g_aucNanIEBuffer);

	return rRetStatus;
}

uint32_t
nanSchedNegoGetSelectedNdcAttr(struct ADAPTER *prAdapter,
			uint8_t *pucPeerNmiAddr,
			uint8_t **ppucNdcAttr,
	...
	*ppucNdcAttr = NULL;
	...
	do {
		...
		/* Find peer nego info */
		szTransIdx = nanSchedNegoLookupTransIdx(prAdapter,
			pucPeerNmiAddr);
		if (szTransIdx >= NAN_CRB_NEGO_MAX_TRANSACTION)
			return rRetStatus;
		prNegoTrans = nanGetNegoTransDataByIdx(prAdapter, szTransIdx);
		prNdcCtrl = prNegoTrans->prSelectedNdcCtrl;
		...
		if ((OFFSET_OF(struct _NAN_ATTR_NDC_T, aucScheduleEntryList) +
			ucNanActiveTimelineNum *
			(1/* Map ID(1) */ + NAN_TIME_BITMAP_FIELD_MAX_LENGTH)) >
			NAN_IE_BUF_MAX_SIZE) {
			*pu4NdcAttrLength = 0U;
		...
		kalMemCpyS(prNdcAttr->aucNDCID,
			sizeof_field(struct _NAN_ATTR_NDC_T, aucNDCID),
			prNdcCtrl->aucNdcId, NAN_NDC_ATTRIBUTE_ID_LENGTH);
	...
	*pu4NdcAttrLength = (uint32_t)(pucPos - g_aucNanIEBuffer);
...
nanSchedGetDevCapabilityAttr(struct ADAPTER *prAdapter,
	...
	prAttrDevCap->u2CommittedDWInfo |=
		(prScheduler->uc2gCommitDwInterval
		 << NAN_COMMITTED_DW_INFO_24G_OFFSET);
	if (prScheduler->fgEn5gL || prScheduler->fgEn5gH)
		prAttrDevCap->u2CommittedDWInfo |=
			(prScheduler->uc5gCommitDwInterval
			 << NAN_COMMITTED_DW_INFO_5G_OFFSET);

	prNanTimelineMgmt = nanGetTimelineMgmtByBand(prAdapter, BAND_2G4);
	if (prNanTimelineMgmt->ucMapId != NAN_INVALID_MAP_ID)
		prAttrDevCap->u2CommittedDWInfo |=
			(prNanTimelineMgmt->ucMapId
			 << NAN_COMMITTED_DW_INFO_24G_DW_OVERWRITE_OFFSET);
	...
	if (prScheduler->fgEn5gL || prScheduler->fgEn5gH) {
		prNanTimelineMgmt = nanGetTimelineMgmtByBand(prAdapter,
								BAND_5G);
		if (prNanTimelineMgmt->ucMapId != NAN_INVALID_MAP_ID)
			prAttrDevCap->u2CommittedDWInfo |=
			    (prNanTimelineMgmt->ucMapId
	...
	/* Support VHT Mode */
	prAttrDevCap->ucOperationMode |= NAN_DEV_CAP_OP_MODE_PHY_MODE_VHT;

	/* Support HE Mode */
#if (CFG_SUPPORT_NAN_R4 == 1) && (CFG_SUPPORT_802_11AX == 1)
	if (prAdapter->rWifiVar.fgEnNanHE)
		prAttrDevCap->ucOperationMode |=
			BIT(NAN_DEV_CAP_OP_MODE_PHY_MODE_HE);
#endif
	...
	prAttrDevCap->ucNumOfAntennas = (prBssInfo->ucOpRxNss << 4)
					 | (prBssInfo->ucOpTxNss);
	DBGLOG(NAN, INFO, "Bss idx:%d, Nss:%d\n", prBssInfo->ucBssIndex,
	       prBssInfo->ucOpRxNss);

	prAttrDevCap->u2MaxChannelSwitchTime = g_u2MaxChnlSwitchTimeUs;
	...
	IN struct ADAPTER *prAdapter)
{
	...
	union _NAN_BAND_CHNL_CTRL rFixChnl = {0};
	struct _NAN_CHNL_ENTRY_T rChnlEntry = {0};
	struct _NAN_CHNL_ENTRY_T *prPotentialChnlList = NULL;
	...
	for (ucTimelineIdx = 0;
		...
		if (rFixChnl.rChannel.u4PrimaryChnl != 0) {
			...
			for (prPotentialChnlMap = g_arPotentialChnlMap;
				...
				while (eBwMap > 0 && prPotentialChnlMap
...
#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_DATA == 1)
		kalMemCpyS(&(prCmdUpdateCRB->rCommNdcCtrl),
			sizeof_field(struct _NAN_SCHED_CMD_UPDATE_CRB_T,
				rCommNdcCtrl),
...
nanSchedCmdUpdateAvailabilityDb(struct ADAPTER *prAdapter,
	...
	prCmdUpdateAvailability->fgMultipleMap =
		(uint8_t)prAdapter->fgNanMultipleMapTimeline;
...
nanSchedCmdUpdateAvailability(IN struct ADAPTER *prAdapter) {
	...
	do {
		...
		if (fgChange) {
			...
			DBGLOG(NAN, INFO, "Skip update availability");
			return rStatus;
		}
		...
		uint8_t *pucBuf)
{
	...
	g_u2MaxChnlSwitchTimeUs = prEventDevCap->u2MaxChnlSwitchTimeUs;
	DBGLOG(NAN, INFO, "MaxChnlSwitchTime:%u us\n", g_u2MaxChnlSwitchTimeUs);
...
nanSchedEventDwInterval(struct ADAPTER *prAdapter, uint32_t u4SubEvent,
	...
	prScheduler = nanGetScheduler(prAdapter);
	prScheduler->uc2gCommitDwInterval = prEventDwInterval->uc2gDwInterval;
	prScheduler->uc5gCommitDwInterval = prEventDwInterval->uc5gDwInterval;
...
nanScheduleNegoTestFunc(struct ADAPTER *prAdapter, uint8_t *pucNmiAddr,
	...
	if (pvToken == (void *)5) {
		...
		nanSchedNegoGetSelectedNdcAttr(prAdapter,
			aucNmiAddr, &pucBuf, &u4Length);
		nanUtilDump(prAdapter, "[NDC Attr]", pucBuf, u4Length);

		nanSchedNegoStop(prAdapter, aucNmiAddr);
	} else if (pvToken == (void *)7) {
		nanSchedNegoGenLocalCrbProposal(prAdapter, aucNmiAddr);
		...
		DBGLOG(NAN, INFO, "nanSchedNegoGenLocalCrbProposal: %x\n",
		       rRetStatus);
		nanSchedNegoGetRangingScheduleList(prAdapter, aucNmiAddr,
				&pucBuf,
				&u4Length);
		...
		nanSchedNegoGetImmuNdlScheduleList(prAdapter, aucNmiAddr,
				&pucBuf,
				&u4Length);
...
nanDisableHandler(struct ADAPTER *prAdapter)
{
	...
	if (!prAdapter->fgNanEnable) {
		DBGLOG(NAN, ERROR, "NAN Not Enable yet, try enable first\n");
		return NAN_STATUS_SUCCESS;
	}

	if (prAdapter->fgIsNanSendRequestToCnm && isNanAtResetFlow() == FALSE)
		nanDevSendAbortRequestToCnm(prAdapter);

	nanDevDisable(prAdapter);
	nanDevBssDeactivate(prAdapter, FALSE);
	prAdapter->fgNanEnable = FALSE;
	...
	uint8_t ucNetIdx)
{
	if (ucNetIdx < NAN_MAX_NONNAN_TIMELINE_NUM)
		return &prAdapter->arNonNanTimeline[ucNetIdx];
	else
		return &prAdapter->arNonNanTimeline[0];
}
...
nanSchedInitDefChnlMap(struct ADAPTER *prAdapter)
{
	...
	kalMemCopy(prAdapter->rDefChnlMap, arDefChnlMap,
			sizeof(prAdapter->rDefChnlMap));
	kalMemZero(prAdapter->arNonNanTimeline,
			sizeof(prAdapter->arNonNanTimeline));
...
nanSchedUpdateDefChnlInfo(struct ADAPTER *prAdapter)
{
	...
	prDefChnlMap = prAdapter->rDefChnlMap;
	...
	for (u4Idx = 0;
		u4Idx < (sizeof(prAdapter->rDefChnlMap)/
	...
	enum ENUM_BAND eBand, uint32_t u4SlotBitmap, uint8_t ucApply)
{
	...
	if ((ucChnl == 0) || (eBand == 0) || (u4SlotBitmap == 0)) {
		kalMemZero(prAdapter->rDefChnlMap,
				sizeof(prAdapter->rDefChnlMap));
		return rRetStatus;
	}
	...
	uint32_t *pu4AvailMap)
{
	...
	if (prPotChnlList == NULL) {
		DBGLOG(NAN, ERROR,
			"prPotChnlList is NULL!\n");
		return WLAN_STATUS_FAILURE;
	}
...
#if (CFG_SUPPORT_NAN_AVAILABILITY_CONTROL_BY_UPPER_LAYER == 1)
		/* Skip clear NDC if it's configured by upperlayer */
		if (g_rVendorNdcCtrl.fgValid
			&& (kalMemCmp(prNdcCtrl->aucNdcId,
					g_rVendorNdcCtrl.aucNdcId,
					NAN_NDC_ATTRIBUTE_ID_LENGTH) == 0))
			continue;
#endif
...
nanSchedSetDwTimeline(struct ADAPTER *prAdapter)
{
	...
	for (szTimeLineIdx = 0;
		...
		for (szSlotOffset = 0;
			szSlotOffset < NAN_SLOTS_PER_DW_INTERVAL;
			szSlotOffset++) {
			...
			for (szDwIdx = 0; szDwIdx < NAN_TOTAL_DW; szDwIdx++) {
				...
				szSlotIdx = szSlotOffset +
					    szDwIdx * NAN_SLOTS_PER_DW_INTERVAL;
	...
	uint32_t u45gChnlVal)
{
	...
	if (u45gChnlVal == NAN_5G_LOW_DISC_CHANNEL)
		fgEn5gL = TRUE;
	else if (u45gChnlVal == NAN_5G_HIGH_DISC_CHANNEL)
		fgEn5gL = fgEn5gH = TRUE;
	...
	{
		/* update timeline according to supported band */
		nanSchedConfigAllowedBand(prAdapter, fgEn2g, fgEn5gH, fgEn5gL);
		nanSchedSetDwTimeline(prAdapter);
		nanSchedCmdUpdateAvailability(prAdapter);
	}
	rRetStatus = WLAN_STATUS_SUCCESS;
...
#endif /* CFG_SUPPORT_NAN_CUST_DW_CHNL */


#if (CFG_NAN_P2P_CONCURRENT_SUPPORT == 1)
void
nanSchedP2pNotifyNanListenMode(struct ADAPTER *prAdapter,
	uint32_t u4P2pIndCh)
{
	if (!prAdapter)
		return;
	...
	/* P2P listen */
	mtk_cfg80211_vendor_event_nan_report_p2p_state(
		prAdapter,
		ENUM_NAN_P2P_STATE_LISTENING,
		u4P2pIndCh,
		CW_20_40MHZ);
}

void
nanSchedP2pNotifyNanAbortMode(struct ADAPTER *prAdapter)
{
	uint32_t u4Channel = g_r2gDwChnl.rChannel.u4PrimaryChnl;
	...
	/* P2P disable */
	/* If infra on connection state
	 * shall indicate AP channel to host
	 * appling new channel sequence
	 */
	for (ucBssIndex = 0; ucBssIndex < prAdapter->ucHwBssIdNum;
		...
		if (IS_BSS_AIS(prBssInfo) &&
			...
			prBssInfo->eConnectionState ==
			MEDIA_STATE_CONNECTED) {
			u4Channel = prBssInfo->ucPrimaryChannel;
			break;
		}
	}
	...
	mtk_cfg80211_vendor_event_nan_report_p2p_state(
		prAdapter,
		ENUM_NAN_P2P_STATE_DISABLED,
		u4Channel,
		CW_20_40MHZ); /* TODO */
}

void
nanSchedP2pNotifyNanConnectMode(struct ADAPTER *prAdapter,
	struct MSG_HDR *prMsgHdr)
{
	struct MSG_P2P_NAN_DP_UPDATE *prP2pNanDpUpdateMsg = NULL;

	if (!prAdapter || !prMsgHdr)
		return;

	prP2pNanDpUpdateMsg = (struct MSG_P2P_NAN_DP_UPDATE *)prMsgHdr;

	DBGLOG(NAN, INFO, "Concurrent[%u], fgConnect[%u] ch[%u]\n",
		prP2pNanDpUpdateMsg->fgDpConcurrent,
		prP2pNanDpUpdateMsg->fgConnect,
		prP2pNanDpUpdateMsg->rChannelInfo.ucChannelNum);

	if (prP2pNanDpUpdateMsg->fgConnect) {
		/* Host will disable NAN 5G
		 * Driver need to disconnect NAN
		 */
		mtk_cfg80211_vendor_event_nan_report_p2p_state(
		prAdapter,
			ENUM_NAN_P2P_STATE_OPERATING, /* exclusive mode */
			prP2pNanDpUpdateMsg->rChannelInfo.ucChannelNum,
			CW_20_40MHZ); /* TODO */

		/* Disable all NDP */
		nanTerminateAllNdp(prAdapter);
	} else {
		/* P2P Disable */
...
/* To Notify P2P that
 * NAN Data Path gonnas establish
 * The timing to call should be cared
 */
void
nanSchedNotifyP2pNdpMode(struct ADAPTER *prAdapter)
{
	struct MSG_NAN_P2P_DP_UPDATE *prMsgChReq = NULL;

	if (!prAdapter)
		return;

	prMsgChReq =
		(struct MSG_NAN_P2P_DP_UPDATE *)cnmMemAlloc(prAdapter,
		RAM_TYPE_MSG,
		sizeof(struct MSG_NAN_P2P_DP_UPDATE));
	if (!prMsgChReq) {
		DBGLOG(NAN, ERROR, "Can't indicate CNM\n");
		return;
	}

	prMsgChReq->rMsgHdr.eMsgId = MID_NAN_P2P_DP_UPDATE;

	/* Other struct member assign field here */

	DBGLOG(NAN, INFO, "NDP Connecting msg update!\n");

	/* WFD is going to disconnect
	 * report the event to host
	 * so that host can update map
	 */
	nanSchedP2pNotifyNanAbortMode(prAdapter);
...
#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_DATA == 1)
enum NanStatusType
nanSchedNdcMgmtCmd(IN struct ADAPTER *prAdapter,
		struct _NAN_NDC_CTRL_T *prNdcCtrl,
		enum _ENUM_NAN_NDC_MGMT_CMD_TYPE eNdcMgmtCmdType)
{
	...
	kalMemCpyS(prCmdNanNdcCtrlMgmt->aucNdcId,
		sizeof_field(struct _NAN_CMD_NDC_CTRL_MGMT_T, aucNdcId),
		prNdcCtrl->aucNdcId,
		sizeof_field(struct _NAN_NDC_CTRL_T, aucNdcId));
	if (prCmdNanNdcCtrlMgmt->u4CmdActionType ==
		NAN_NDC_MGMT_CMD_TYPE_ACTIVATE) {
		kalMemCpyS(prCmdNanNdcCtrlMgmt->arTimeline,
			sizeof_field(struct _NAN_CMD_NDC_CTRL_MGMT_T,
				arTimeline),
			prNdcCtrl->arTimeline,
			sizeof_field(struct _NAN_NDC_CTRL_T,
				arTimeline));
	}
	if (eNdcMgmtCmdType == NAN_NDC_MGMT_CMD_TYPE_ACTIVATE) {
		...
		if (prNdcMgmt && (prNdcMgmt->eTxDbdcIdx == ENUM_BAND_NUM)) {
			if (prAdapter->rWifiVar.eDbdcMode ==
				...
				for (szTimeLineIdx = 0;
					szTimeLineIdx < NAN_TIMELINE_MGMT_SIZE;
					szTimeLineIdx++) {
					prTimeline =
						&(prNdcMgmt->rNdcCtrl.arTimeline
						[szTimeLineIdx]);
					szBandSlotNum[szTimeLineIdx] =
						nanUtilCheckBitOneCnt(
						prAdapter,
						(uint8_t *)prTimeline->
							au4AvailMap,
				...
				if (szBandSlotNum[1] >= szBandSlotNum[0])
					ucDefaultDbdcIdx = (uint8_t)
						cnmGetDbdcIdxByRfBand(prAdapter,
							BAND_5G);
				else
					ucDefaultDbdcIdx = (uint8_t)
						cnmGetDbdcIdxByRfBand(prAdapter,
				...
				prCmdNanNdcCtrlMgmt->ucDefaultDbdcIdx =
					ucDefaultDbdcIdx;
				prNdcMgmt->eTxDbdcIdx = ucDefaultDbdcIdx;
			}
		}
	}

	rStatus = wlanSendSetQueryCmd(prAdapter, CMD_ID_NAN_EXT_CMD,
			TRUE, FALSE, FALSE,
			NULL,	NULL,
			(uint32_t)szCmdBufferLen,
			(uint8_t *)prCmdBuffer,
...
uint32_t
nanVendorOobActionTxHandler(struct ADAPTER *prAdapter,
	struct _NAN_CMD_OOB_ACTION *prNanCmdOOBAction)
{
	uint32_t rStatus = WLAN_STATUS_SUCCESS;
	struct _NAN_CMD_OOB_ACTION rNanCmdOOBAction = {0};
	...
	if (prNanCmdOOBAction == NULL) {
		DBGLOG(NAN, ERROR, "prNanCmdOOBAction is null\n");
		return -EINVAL;
	}

	rNanCmdOOBAction.u8BitMap = prNanCmdOOBAction->u8BitMap;
	kalMemCopy(rNanCmdOOBAction.aucDestAddress,
		prNanCmdOOBAction->aucDestAddress,
		MAC_ADDR_LEN);
	kalMemCopy(rNanCmdOOBAction.aucSrcAddress,
		prNanCmdOOBAction->aucSrcAddress,
		MAC_ADDR_LEN);
	kalMemCopy(rNanCmdOOBAction.aucBssid,
		prNanCmdOOBAction->aucBssid,
		MAC_ADDR_LEN);
	rNanCmdOOBAction.ucMapId = prNanCmdOOBAction->ucMapId;
	rNanCmdOOBAction.u2Timeout = prNanCmdOOBAction->u2Timeout;
	rNanCmdOOBAction.ucFrameType = prNanCmdOOBAction->ucFrameType;
	rNanCmdOOBAction.ucSecurity = prNanCmdOOBAction->ucSecurity;
	rNanCmdOOBAction.u2Token = prNanCmdOOBAction->u2Token;
	rNanCmdOOBAction.u2PayloadLen = prNanCmdOOBAction->u2PayloadLen;
	kalMemCopy(rNanCmdOOBAction.aucPayload,
		prNanCmdOOBAction->aucPayload,
		prNanCmdOOBAction->u2PayloadLen);

	DBGLOG(NAN, INFO, "[OobAF] bitmap[%llu] DestAddr"MACSTR"\n",
		rNanCmdOOBAction.u8BitMap,
		MAC2STR(rNanCmdOOBAction.aucDestAddress));
	DBGLOG(NAN, INFO, "[OobAF] SrcAddr"MACSTR" Bssid"MACSTR"\n",
		MAC2STR(rNanCmdOOBAction.aucSrcAddress),
		MAC2STR(rNanCmdOOBAction.aucBssid));
	DBGLOG(NAN, INFO, "[OobAF] MapId[%u] Token[%u] Payload_len[%u]\n",
		rNanCmdOOBAction.ucMapId,
		rNanCmdOOBAction.u2Token,
		rNanCmdOOBAction.u2PayloadLen);
	DBGLOG(NAN, INFO, "[OobAF] FrameType[%u] Security[%u] Timeout[%u]\n",
		rNanCmdOOBAction.ucFrameType,
		rNanCmdOOBAction.ucSecurity,
		rNanCmdOOBAction.u2Timeout);

#if (CFG_NAN_SUPPORT_OOB_AF_SLOT_BASE == 1)
	nanCmdSendNanOobActionAuthFrameSlotBase(prAdapter,
		&rNanCmdOOBAction);
#endif
...
/*----------------------------------------------------------------------------*/
/*!
 * \brief Get Peer available bitmap by
 * calculating the intersection of local & remote channel sequence
 *
 * \param[in]
 *
 * \return Status
...
/*----------------------------------------------------------------------------*/
uint32_t
nanSchedGetPeerOobAvailableBitmap(IN struct ADAPTER *prAdapter,
	IN uint8_t u1Mapid, IN uint64_t *pu8Bitmap, uint8_t *pucNmiAddr)
{
	struct _NAN_PEER_SCHEDULE_RECORD_T *prPeerSchRecord = NULL;
	struct _NAN_PEER_SCH_DESC_T *prPeerSchDesc = NULL;
	size_t szIdx = 0, szAvailDbIdx = 0, szSlotIdx = 0;
	size_t szTimeLineIdx = 0;
	size_t szNanActiveTimelineNum = 0;
	struct _NAN_AVAILABILITY_DB_T *prAvailabilityDB = NULL;
	struct _NAN_AVAILABILITY_TIMELINE_T *prNanAvailEntry = NULL;
	struct _NAN_TIMELINE_MGMT_T *prNanTimelineMgmt = NULL;
	union _NAN_BAND_CHNL_CTRL rLocalChnlInfo = {0};
	union _NAN_BAND_CHNL_CTRL rRmtChnlInfo = {0};
	uint64_t u8BitmapTemp = 0;
	uint8_t fgFound = FALSE;

	if (pu8Bitmap == NULL || prAdapter == NULL || pucNmiAddr == NULL) {
		DBGLOG(NAN, ERROR, "Null input\n");
		return WLAN_STATUS_INVALID_DATA;
	}

	u8BitmapTemp = *pu8Bitmap;
	szNanActiveTimelineNum = nanGetActiveTimelineMgmtNum(prAdapter);
	prPeerSchRecord = nanSchedLookupPeerSchRecord(prAdapter, pucNmiAddr);

	if (prPeerSchRecord) {
		u8BitmapTemp &=
			(((uint64_t)prPeerSchRecord->rCommFawTimeline[u1Mapid].
			au4AvailMap[1] << 32) |
			prPeerSchRecord->rCommFawTimeline[u1Mapid].
			au4AvailMap[0]);
	} else {
		DBGLOG(NAN, INFO, "Can not find peer schedule record\n");

		prPeerSchDesc =
			nanSchedSearchPeerSchDescByNmi(
				prAdapter, pucNmiAddr);
		if (prPeerSchDesc == NULL) {
			DBGLOG(NAN, INFO,
				"Can not find peer schedule desc\n");
			goto end;
		}
		...
		for (szTimeLineIdx = 0;
			...
			prNanTimelineMgmt =
				nanGetTimelineMgmt(prAdapter,
				szTimeLineIdx);
			if (prNanTimelineMgmt->ucMapId == u1Mapid)
				break;
		}

		if (szTimeLineIdx >= szNanActiveTimelineNum) {
			DBGLOG(NAN, ERROR,
				"Can not find Tidx,%zu,%zu,MapId,%u\n",
				szTimeLineIdx, szNanActiveTimelineNum,
				u1Mapid);
			goto end;
		}

		for (szSlotIdx = 0; szSlotIdx < 64; szSlotIdx++) {
			if (!(BIT64(szSlotIdx) & u8BitmapTemp))
				continue;

			rLocalChnlInfo =
				nanQueryChnlInfoBySlot(prAdapter,
					szSlotIdx, NULL,
					TRUE,
					szTimeLineIdx);
			...
			for (szAvailDbIdx = 0;
				szAvailDbIdx < NAN_NUM_AVAIL_DB;
				szAvailDbIdx++) {
				prAvailabilityDB =
					&prPeerSchDesc->
					arAvailAttr[szAvailDbIdx];

				if (prAvailabilityDB->ucMapId ==
					NAN_INVALID_MAP_ID)
					continue;

				for (szIdx = 0;
				(szIdx < NAN_NUM_AVAIL_TIMELINE);
				szIdx++) {
					prNanAvailEntry =
					&prAvailabilityDB->
					arAvailEntryList
					...
					if (rRmtChnlInfo.rChannel.u4Type !=
						NAN_BAND_CH_ENTRY_LIST_TYPE_CHNL
						)
						continue;
			...
			if (!fgFound)
				u8BitmapTemp =
					u8BitmapTemp & ~BIT64(szSlotIdx);
		}

	}

end:
	DBGLOG(NAN, INFO, "[OobAF]Bitmap: 0x%llx -> 0x%llx\n",
		*pu8Bitmap, u8BitmapTemp);

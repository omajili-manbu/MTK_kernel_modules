...
nanDiscInstanceAdd(struct ADAPTER *prAdapter,
	...
	uint8_t ucServiceNameLength, u_int8_t fgGtkRequired)
{
	...
	prInstance->fgGtkRequired = fgGtkRequired;
	kalMemCpyS(prInstance->aucServiceHash,
		sizeof_field(struct _NAN_INSTANCE_T, aucServiceHash),
		pucServiceHashName, NAN_SERVICE_HASH_LENGTH);
	prInstance->ucServiceNameLength = ucServiceNameLength;
	kalMemCpyS(prInstance->aucServiceName,
		sizeof_field(struct _NAN_INSTANCE_T, aucServiceName),
		pucServiceName, ucServiceNameLength);
...
nanCancelPublishRequest(struct ADAPTER *prAdapter,
	...
	prNanCancelReq = (struct NanFwCancelRequest *)prTlvElement->aucbody;
	prNanCancelReq->publish_or_subscribe = 1;
	prNanCancelReq->publish_subscribe_id = msg->publish_id;
...
nanPublishRequest(struct ADAPTER *prAdapter, struct NanPublishRequest *msg) {
	...
	kalMemCpyS(&prPublishReq->sdea_params,
		sizeof_field(struct NanFWPublishRequest, sdea_params),
		&msg->sdea_params,
		sizeof_field(struct NanPublishRequest, sdea_params));
...
/*----------------------------------------------------------------------------*/
uint32_t
nanDiscGetCustomAttrImpl(struct ADAPTER *prAdapter,
		uint8_t **ppucCustomAttr, uint16_t *pu2CustomAttrLength,
		struct NanTransmitFollowupRequest *prNanFollowupReq)
{
	...
	if (!ppucCustomAttr) {
		DBGLOG(NAN, ERROR, "ppuCustomAttr error\n");
		return WLAN_STATUS_INVALID_DATA;
	}

	if (!pu2CustomAttrLength) {
		DBGLOG(NAN, ERROR, "pu2CustomAttrLength error\n");
		return WLAN_STATUS_INVALID_DATA;
	}

	*ppucCustomAttr = (uint8_t *)prNanFollowupReq->custom_attributes;
	*pu2CustomAttrLength = prNanFollowupReq->custom_attribute_length;

	DBGLOG(NAN, INFO,
		"followup custom attr len(%d)\n",
		prNanFollowupReq->custom_attribute_length);

	dumpMemory8(prNanFollowupReq->custom_attributes,
		prNanFollowupReq->custom_attribute_length);
...
/*----------------------------------------------------------------------------*/
/*!
 * \brief            Send customer attribute of Follow-up
 *
 * \param[in]
 *
 * \return Status
...
/*----------------------------------------------------------------------------*/
uint32_t
nanDiscCmdUpdateCustomAttrCache(struct ADAPTER *prAdapter,
		struct NanTransmitFollowupRequest *prNanFollowupReq)
{
	uint32_t rStatus = WLAN_STATUS_FAILURE;
	void *prCmdBuffer = NULL;
	...
	struct _CMD_EVENT_TLV_COMMOM_T *prTlvCommon = NULL;
	struct _CMD_EVENT_TLV_ELEMENT_T *prTlvElement = NULL;
	struct _NAN_CMD_UPDATE_ATTR_CACHE_T *prCmdUpdateCustomCache =
		NULL;
	uint8_t *pucCustomAttr = NULL;
	uint16_t u2CustomAttrLength = 0;

	DBGLOG(NAN, INFO, "Enter\n");

	nanDiscGetCustomAttrImpl(prAdapter, &pucCustomAttr,
				   &u2CustomAttrLength, prNanFollowupReq);

	szCmdBufferLen =
		sizeof(struct _CMD_EVENT_TLV_COMMOM_T) +
		sizeof(struct _CMD_EVENT_TLV_ELEMENT_T) +
		offsetof(struct _NAN_CMD_UPDATE_ATTR_CACHE_T, aucAvailAttr) +
		u2CustomAttrLength;
	...
	prCmdUpdateCustomCache =
		(struct _NAN_CMD_UPDATE_ATTR_CACHE_T *)
			prTlvElement->aucbody;
	prCmdUpdateCustomCache->u4AvailAttrSize = (uint32_t)u2CustomAttrLength;
	kalMemCpyS(prCmdUpdateCustomCache->aucAvailAttr,
		u2CustomAttrLength, pucCustomAttr,
		u2CustomAttrLength);

	DBGLOG(NAN, INFO, "Dump custom attr cmd\n");
	dumpMemory8(prCmdUpdateCustomCache->aucAvailAttr, u2CustomAttrLength);

	rStatus = wlanSendSetQueryCmd(prAdapter, CMD_ID_NAN_EXT_CMD, TRUE,
			FALSE, FALSE, nanDevCommonSetCb,
			nicCmdTimeoutCommon,
			(uint32_t)szCmdBufferLen, (uint8_t *)prCmdBuffer,
			NULL, 0);
	if (rStatus != WLAN_STATUS_FAILURE)
		DBGLOG(NAN, STATE, "Element Container Attr update success\n");
		   ...
		   struct NanTransmitFollowupRequest *msg)
{
	...
	DBGLOG(NAN, INFO,
	       "publish_subscribe_id: %d, requestor_instance_id: %d\n",
	...
	DBGLOG(NAN, INFO,
		"custom_attribute_length: %u\n",
		msg->custom_attribute_length);
	if (msg->custom_attribute_length)
		nanDiscCmdUpdateCustomAttrCache(prAdapter, msg);
...
nanCancelSubscribeRequest(struct ADAPTER *prAdapter,
	...
	prNanCancelReq = (struct NanFwCancelRequest *)prTlvElement->aucbody;
	prNanCancelReq->publish_or_subscribe = 0;
	prNanCancelReq->publish_subscribe_id = msg->subscribe_id;
...
nanSubscribeRequest(struct ADAPTER *prAdapter,
	...
	kalMemZero(prSubscribeReq, sizeof(*prSubscribeReq));

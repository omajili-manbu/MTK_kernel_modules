...
#define SECEFUSE_BLOCK_SIZE		32

/* For MT7961 SECEEPROM Layout */
#define SECEFUSE_KEYHASH_OFFSET_BEGIN	0x20
#define SECEFUSE_KEYHASH_OFFSET_END	0x80 /* off by 1 */
#define SECEFUSE_KEYHASH_SIZE		32
...
#define CMD_GET_FMCW_RADAR_REPORT_TYPE (0xA)
#endif /* #if CFG_WIFI_FMCW_RADAR_TRAFFIC_REPORT */
#endif
...
struct RADAR_TEST_FLOW_CTRL {
	uint32_t u4SubCmdId;
	uint32_t u4FmcwVal;
};

struct PARAM_MTK_FMCW_TEST_STRUCT {
	uint32_t u4FuncIndex;
	uint32_t u4FuncData;
	uint32_t u4FuncFmcwData[100];
	uint32_t u4TxChainAdcTemp;
};
#endif

struct _RBIST_IQ_DATA_T {
	int32_t i4IQArray[MAX_IQ_ARRAY_WF_CNT][2]; /* IQ_Array[WF][IQ] */
};
...
enum RADAR_RESP_SUBEVENT_ID {
	ENUM_RADAR_RESP_SUBEVENT_SETTING,
	ENUM_RADAR_RESP_SUBEVENT_ENABLE,
	ENUM_RADAR_RESP_SUBEVENT_PERIODICAL_EVENT,
	ENUM_RADAR_RESP_SUBEVENT_INVALID
};

enum RADAR_FAIL_REASON_CODE {
	ENUM_RADAR_CORRECT_DATE = 0,
	ENUM_RADAR_FAIL_ERROR_CENTRAL_FREQ,
	ENUM_RADAR_FAIL_CHANEL_BUSY,
	ENUM_RADAR_FAIL_CTS_FAIL
};

struct RADAR_CMD_RESP_REPORT {
	uint8_t ucStatus;
	uint8_t aucReserved[3];
};
...
uint32_t
wlanoidFmcwTestFlowCtrl(IN struct ADAPTER *prAdapter,
			 IN void *pvSetBuffer,
			 IN uint32_t u4SetBufferLen,
			 OUT uint32_t *pu4SetInfoLen);
#endif
...
uint32_t wlanoidSetCsiMode(struct ADAPTER *prAdapter, void *pvSetBuffer,
			   uint32_t u4SetBufferLen,
			   uint32_t *pu4SetInfoLen);
#endif /* CFG_SUPPORT_CSI_VNI */

#if (CFG_SUPPORT_FMCW_VNI == 1)
uint32_t wlanoidSetFmcwMode(struct ADAPTER *prAdapter, void *pvSetBuffer,
			   uint32_t u4SetBufferLen,
			   uint32_t *pu4SetInfoLen);
...
#if (CFG_SUPPORT_RADAR_CMD == 1)
uint32_t wlanoidSetRadarParams(IN struct ADAPTER *prAdapter,
	IN void *pvSetBuffer,
	IN uint32_t u4SetBufferLen,
	OUT uint32_t *pu4SetInfoLen
);

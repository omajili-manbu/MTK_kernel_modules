...
#define FMCW_REASON_CODE_NUM 10
#endif
...
struct CMD_ACCESS_SECEFUSE {
	uint32_t u4CmdVer;
	uint32_t u4Address;
	/* Compatible to ori CMD field. Currently no use */
	uint32_t u4ReservedValid;
	uint32_t u4Size;
	uint32_t u4MagicNum;
...
/* FMCW Radar Traffic Report Counter */
struct CMD_FMCW_RADAR_TRAFFIC_INFO {
	uint16_t u2Type;
	uint16_t u2Len;
	uint8_t ucAction;
	uint32_t u4CbpChIdle;
	uint32_t u4CbpChBusy;
	uint32_t u4ReasonCode[FMCW_REASON_CODE_NUM];
};

/* FMCW Radar Report Related */
enum ENUM_FMCW_RADAR_REPORT_ACTION_T {
	CMD_FMCW_RADAR_REPORT_GET = 1,
	CMD_FMCW_RADAR_REPORT_RESET
};
#endif
...
enum ENUM_EXT_CMD_ID {
	...
	EXT_CMD_ID_SECEFUSE_ACCESS = 0xBC,
...
enum ENUM_SCN_FUNC_EXT_MASK {
	...
	ENUM_SCN_ENABLE_DBDC_SCAN = (1 << 5),
	/* reserved for firmware */
	ENUM_SCN_NOT_FORCE_6G_ACTIVE_PROBE = (1 << 12),
};
...
enum ENUM_WOW_WAKEUP_REASON {
	...
	ENUM_PF_CMD_TYPE_SCHED_SCN_MATCH               = 17,
...
#endif
#define EXT_EVENT_ID_SECEFUSE_ACCESS 0xBC
/*#endif*/
...
#if CFG_SUPPORT_NAN
enum ENUM_NAN_FEATURE_SET {
	ENUM_NAN_FEATURE_R4_6G,
	ENUM_NAN_FEATURE_R4_PAIRING,
	ENUM_NAN_FEATURE_R4_USD,
	ENUM_NAN_FEATURE_DYN_SDB,
	ENUM_NAN_FEATURE_KEY_MANAGEMENT,
	ENUM_NAN_FEATURE_R4_MC_DATA,
	ENUM_NAN_FEATURE_R4_GROUP_KEY,
	ENUM_NAN_FEATURE_NUM
};
...
#if CFG_SUPPORT_FMCW_RADAR
union EVENT_FMCW_TEST_STATUS {
	struct PARAM_MTK_FMCW_TEST_STRUCT rATInfo;
	/* PARAM_CUSTOM_RFTEST_TX_STATUS_STRUCT_T   rTxStatus; */
	/* PARAM_CUSTOM_RFTEST_RX_STATUS_STRUCT_T   rRxStatus; */
};
#endif
...
struct CMD_802_11_PMKID {
	...
	struct PMKID_ENTRY *arPMKIDInfo[];
};
...
enum NIC_CAPABILITY_V2_TAG {
	...
	TAG_CAP_NAN_FEATURE_SET_MOBILE_CE = 0x44,
#endif
	TAG_CAP_TOTAL
};
...
#if (CFG_SUPPORT_NAN == 1)
struct CAP_NAN_FEATURE_T {
	uint32_t u4NanFeatureSet;
};
#endif
...
struct EVENT_PMKID_CANDIDATE_LIST {
	...
	struct PARAM_PMKID_CANDIDATE arCandidateList[];
};
...
struct CMD_ANT_SW_DIV_CTRL {
	uint8_t  ucForceSwitch;  /*True: forect switch  False: dynamic switch */
	uint8_t  ucCurrentRCPI; /*bring current rcpi to save time*/
	uint8_t  ucBssIndex; /*BSS index, default only support AIS BSS */
	/*DetectMode 0: both group detection 1: only detection current group*/
	uint8_t ucDetectionMode;
	uint8_t ucReserve[16];
};
#endif
...
#if CFG_SECEFUSE_ACCESS
struct EVENT_ACCESS_SECEFUSE {
	uint32_t u4EventVer;
	uint32_t u4Address;
	/* Compatible to original CMD field
...
struct _NAN_EVENT_DEVICE_INFO {
	...
	uint8_t ucNanDeviceState;
	uint8_t ucMasPreference;
	uint8_t ucRandomFactor;
	uint8_t ucHopCount;
	uint8_t aucClusterID[MAC_ADDR_LEN];
	uint8_t aucAnchorMasMacAddr[MAC_ADDR_LEN];
	uint8_t ucAmMasPreference;
	uint8_t ucAmRandomFactor;
	uint8_t aucParentMacAddr[MAC_ADDR_LEN];
	uint8_t ucParentMasPreference;
	uint8_t ucParentRandomFactor;
	uint8_t ucRsvd;
	uint32_t u4AMBTT;
	uint32_t au4Tsf[2];
	uint8_t aucIgtkPn[6];
	uint8_t aucBigtkPn[6];
};
...
struct NAN_EVENT_REPORT_DW_T {
	struct _NAN_EVENT_DEVICE_INFO device_info;
...
struct NAN_REPORT_BEACON_EVENT {
	uint32_t eRfBand;
	int32_t i4Rssi;
...
struct _NAN_SYNC_INFO_T {
	uint8_t ucCurrentHopCount;
	uint8_t ucCurrentRandomfactor;
	uint8_t ucCurrentMasPref;
};
...
enum _ENUM_NAN_SUB_CMD {
	...
	NAN_CMD_SKDA_ATTR_CACHE = 100,
	NAN_CMD_TX_OOB_ACTION_FRAME = 101,
	NAN_CMD_KEY_MGMT = 102,
	NAN_CMD_NDC_MGMT = 103,
...
enum _ENUM_NAN_SUB_EVENT {
	...
	NAN_EVENT_VENDOR_DISCOVERY_RESULT = 50,
	...
	/* EXT_CMD Part */
	/* Reserve for vendor r, 100 ~ 199 */
	NAN_EVENT_ID_OOB_ACTION_TX_STATUS = 100,
...
enum ENUM_RADAR_CTRL_SUB_CMD_ID {
	ENUM_RADAR_CTRL_SET_RADAR,
	ENUM_RADAR_CTRL_SET_ENABLE_RADAR,
	ENUM_RADAR_CTRL_SET_MAX
};

enum EVENT_ENUM_FMCW_RADAR_FAIL_REASON_CODE {
	ENUM_CORRECT_DATA,
	ENUM_FAIL_ERROR_CENTRAL_FREQ,
	ENUM_FAIL_CHANNEL_BUSY,
	ENUM_FAIL_CTS_FAIL,
	ENUM_TSSI_CALIBRATION,
	ENUM_DUMP_ACC_DATA,
	ENUM_FAIL_PHASE_CAL,
	ENUM_FORCE_CBP_TX,
	ENUM_MOVE_DATA_FAIL,
...
enum ENUM_RADAR_CTRL_RESULT_STATUS {
	ENUM_RADAR_CTRL_RESULT_SUCCESS,
	ENUM_RADAR_CTRL_RESULT_FAILURE
};

struct CMD_RADAR_CTRL_SETTING_T {
	uint32_t u4Freg;
...
struct CMD_RADAR_CTRL_ENABLE_T {
	uint8_t ucRadarEnable; /* Timer Enable */
	uint8_t ucPadding;
	uint8_t ucFpsCnt; /* Frame Per Second Count */
	uint8_t ucBurstCnt; /* Tx Burst Count */
	uint8_t aucReserved[4];
};

struct CMD_RADAR_CTRL_PARA {
	uint8_t ucSubCmdId;
...
enum ENUM_LOC_SUB_CMD {
	CMD_LOC_GET_CAPA = 0,
	CMD_LOC_RANGE_MC_ISTA = 1,
	CMD_LOC_RANGE_MC_RSTA = 2,
	CMD_LOC_RANGE_REQ_AZ_NTB = 3, /* Currently unused */
	CMD_LOC_REQ_AZ_NTB_PHY_SEC = 4, /* Currently unused */
	CMD_LOC_RSP_ENABLE = 5,
	CMD_LOC_DEBUG = 6, /* Currently unused */
	CMD_LOC_REQ_MC_BURST = 7, /* Currently unused */
	CMD_LOC_TOAE_CAL_CFG = 8, /* Currently unused */
	CMD_LOC_FTMR_TIMEOUT = 9, /* Currently unused */
	CMD_LOC_NUM
};

enum ENUM_LOC_SUB_EVENT {
	EVENT_LOC_CAPA = 0,
	EVENT_LOC_RESULT = 1,
	EVENT_LOC_DONE = 2,

	/* Currently unused, sync with FW*/
	EVENT_LOC_TRIGGER = 3,
	EVENT_LOC_REQ_MC_BURST = 7,
	EVENT_LOC_NUM
};
...
struct EVENT_LOC_RESP {
	uint8_t ucSubEventId;
	uint8_t aucPadding[3];
	uint8_t aucBody[];
};

struct LOC_EVENT_RTT_RESULT {
	uint8_t aucMacAddr[MAC_ADDR_LEN];
...
#if (CFG_SUPPORT_ANT_SMART_DIVERSITY == 1)
enum ENUM_ANT_SMART_DIV_STATUS_COD_T {
	ANT_SW_DIV_STATUS_SUCCESS = 0,	/* switch success*/
	ANT_SW_DIV_STATUS_NO_NEED_SWITCH,	/* decide not to switch */
	ANT_SW_DIV_STATUS_BLOCKED,			/* switch blocked*/
	ANT_SW_DIV_STATUS_STAREC_NULL,		/* ap station record not find*/
	ANT_SW_DIV_STATUS_DETECTION_SUCCESS,/* detection success*/
	ANT_SW_DIV_STATUS_NUM,
};

enum ENUM_ANT_SMART_DIV_ANT_IDX_T {
	ANT_SW_DIV_ANT_IDX_MAIN = 0,
	ANT_SW_DIV_ANT_IDX_AUX,
	ANT_SW_DIV_ANT_IDX_NUM
};

struct EVENT_ANT_SMART_DIV_INFO {
	uint8_t ucStatus;
	uint8_t ucUsingAntIdx;
	uint8_t ucWaittingAntIdx;
	uint8_t ucUsingAntRcpiAvg;
	uint8_t ucWaittingAntRcpiAvg;
	uint8_t ucReserved[15];
};
#endif
/*******************************************************************************
 *                            P U B L I C   D A T A
...
void nicCmdEventFmcwTestFlowCtrl(IN struct ADAPTER *prAdapter,
		IN struct CMD_INFO *prCmdInfo, IN uint8_t *pucEventBuf);
#endif /* (CFG_SUPPORT_FMCW_RADAR == 1) */

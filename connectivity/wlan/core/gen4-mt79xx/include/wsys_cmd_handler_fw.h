...
#define SCAN_COUNT                              2
#define MAX_CHANNEL_NUM    SCAN_DONE_EVENT_MAX_CHANNEL_NUM
...
enum ENUM_CMD_ID {
	...
	CMD_ID_FMCW_TEST_CTRL = 0x6F,       /* 0x6F (Query) */
	...
	CMD_ID_RADAR_CTRL = 0xB6,           /* 0xB6 (Set / Query) */
#endif
#if (CFG_SUPPORT_FMCW_RADAR == 1)
	CMD_ID_RADAR_TEST_FLOW_CTRL = 0xB8, /* 0xB8 (Set / Query) */
#endif /* (CFG_SUPPORT_FMCW_RADAR == 1) */

#if (CFG_SUPPORT_ANT_SMART_DIVERSITY == 1)
	CMD_ID_ANT_SMART_DIV_SWITCH_TRIGGER = 0xB9, /* 0xB9 (Set) */
#endif
	...
	CMD_ID_SET_P2P_DEV = 0xEC,	/* 0xEC (Set) */
...
#if (CFG_WIFI_RAM_MQM_BA_DELAY_SUPPORT == 1)
...
#if (CFG_SUPPORT_RADAR_CMD == 1)
	EVENT_ID_RADAR_CTRL = 0xB6,             /* 0xB6 (Set / Query) */
#endif
#if (CFG_SUPPORT_FMCW_RADAR == 1)
	EVENT_ID_RADAR_TEST_FLOW_CTRL = 0xB8,   /* 0xB8 (Set / Query) */
#endif /* (CFG_SUPPORT_FMCW_RADAR == 1) */

#if (CFG_SUPPORT_ANT_SMART_DIVERSITY == 1)
	EVENT_ID_ANT_SMART_DIV_SWITCH = 0xB9,   /* 0xB9 (Query) */
#endif
...
struct CMD_UPDATE_STA_RECORD {
	...
	uint8_t    ucWdsEnable;
	uint8_t    aucPadding1[2];
#else /* CFG_SUPPORT_4ADDR_STATION == 0 */
...
#if (CFG_SUPPORT_SCAN_RADIOINFO == 1)
	uint16_t	 u2ChannelScanTime[SCAN_DONE_EVENT_MAX_CHANNEL_NUM];
#endif /* end CFG_SUPPORT_SCAN_RADIOINFO */
};
...
#define SINGLE_SKU_VLP_SECTION_NUM 11
struct CMD_TXPOWER_CHANNEL_POWER_LIMIT_VLP {
	uint8_t u1CentralCh;
	int8_t i1PwrLimit[SINGLE_SKU_VLP_SECTION_NUM];
};
...
#if (CFG_SUPPORT_6G_PWR_REGS == 1)
	TXPOWER_LIMIT_FORMAT_CH_SKU_VLP = 2,
	TXPOWER_LIMIT_FORMAT_TPE = 3,
#endif  /*CFG_SUPPORT_6G_PWR_REGS == 1*/

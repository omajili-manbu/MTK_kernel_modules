...
struct CONNECTION_SETTINGS {
	...
	struct IE_WFA_RSN_SELECTION rRnsSelectionInfo;
#endif
...
struct BSS_INFO {
	...
	uint16_t u2BeaconTimeoutReason;
#endif
...
#if CFG_MTK_SUPPLICANT_ROAMING_ENHANCE
	uint8_t fgIgnorSaQueryReq;
	uint8_t fgDelayReportCarrierOff;
#endif
#endif
...
struct WIFI_VAR {
	...
	struct RTT_BIAS_TABLE rRttBiasInfo;
	...
	struct PAIRING_INFO arPairingInfo[NAN_MAX_NDP_SESSIONS];
	uint8_t fgEnNanPairing;
...
/* P_P2P_FSM_INFO_T prP2pFsmInfo; */
	...
	/* Index of ACS
	 * Dirtiness only by default
	 * Idle includes the air time
	 */
	uint8_t ucWeightDirt;
	uint8_t ucWeightIdle;
	...
	uint8_t ucSchedScanDelayWakeup;
#if (CFG_MTK_WIFI_DRV_OWN_MODE_CONTROL == 1)
	uint32_t u4DrvOwnMode; /* driver own mode control, default 0 */
	/* 0: legacy polling mode (default), 1: delayed polling mode */
#endif
	...
	u_int8_t fgScanAddCapIE;
	u_int8_t fgEnDbdcScan;
	...
	uint8_t ucNan2gCommittedDw;
	uint8_t ucNan5gCommittedDw;
	uint8_t fgNoPmf;
	...
	uint8_t fgEnNanKeyMgmt;
	uint8_t fgUseNmiCxtKey;
#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_FRAME_PROT == 1)
	uint8_t ucNanGroupSecCap;
	uint8_t ucNanIgtkCipher;
	...
	uint8_t fgRadarCoupling;
	uint8_t fgRadarForceCBPTx;
#if (CFG_SUPPORT_FMCW_VNI == 1)
	uint8_t fgRadarVniEnable;
	...
	/*
	 * the numbers of 1K slab
	 */
	uint16_t u2SkbNum1kSlab;

	/*
	 * the numbers of 2K slab
	 */
	uint16_t u2SkbNum2kSlab;

	/*
	 * the numbers of 4K slab
	 */
	uint16_t u2SkbNum4kSlab;
#endif

#if (CFG_SUPPORT_ANT_SMART_DIVERSITY == 1)
	u_int8_t fgAntSwDivEnable;
	u_int8_t fgIsAntSwDivProbed;
	int32_t ucAntSwDivTriggerTH;
	int32_t i4AntSwDivTheshold;
	int32_t i4AntSwDivMaxRssi;
#endif
...
struct WIFI_VER_INFO {
	...
	uint8_t aucReleaseFeatureMap[16];
...
struct CSI_INFO_T {
	...
	struct CSI_INFO_DATA_T *prCsiInfoData;
#if CFG_SUPPORT_DBDC
	uint8_t ucBandIdx;
...
#endif /* CFG_CSI_DYNAMIC_BUFFER */
#endif /* CFG_SUPPORT_CSI */

#if (CFG_SUPPORT_ANT_SMART_DIVERSITY == 1)
#define ANTENNA_SW_DIV_DETECT_INTERVAL  5000
#endif
/*
...
struct ADAPTER {
	...
	u_int8_t fgIsNANRegistered;
	u_int8_t fgIsNANfromHAL;
	bool fgIsNanSendRequestToCnm;
	uint8_t ucNanReqTokenId;
	uint8_t ucNanMacAddr[PARAM_MAC_ADDR_LEN];
	uint8_t ucforce_present;
	u_int8_t fgNanMultipleMapTimeline;
	u_int8_t fgNanEnable;
	uint64_t u8NanCardCap;
	...
	struct _NAN_POTENTIAL_CHNL_LIST_T
		arNanPotentialChnlList[NAN_TIMELINE_MGMT_SIZE];

	/* Container for Sync module configuration */
	struct _NAN_SYNC_INFO_T rNanSyncInfo;

#if (CFG_SUPPORT_NAN_CUSTOMIZATION_VERSION == 1)
	/* Record default channel bitmap when NO NDP */
	struct _NAN_CUST_TIMELINE_PROFILE_T
		rDefChnlMap[NAN_MAX_DEFAULT_TIMELINE_NUM];

	/* Record Non-NAN network channel bitmap */
	struct _NAN_NONNAN_NETWORK_TIMELINE_T
		arNonNanTimeline[NAN_MAX_NONNAN_TIMELINE_NUM];
#endif /* CFG_SUPPORT_NAN_CUSTOMIZATION_VERSION */
	...
	enum ENUM_NET_REG_STATE rCsiNetRegState;
	u_int8_t fgIsCsiRegistered;
#endif /* CFG_SUPPORT_CSI_VNI */
#if CFG_SUPPORT_FMCW_VNI
	enum ENUM_NET_REG_STATE rFmcwNetRegState;
	u_int8_t fgIsFmcwRegistered;
#endif /* (CFG_SUPPORT_FMCW_VNI == 1) */
...
#if CFG_SUPPORT_FMCW_RADAR
	/* FMCW_RADAR_VAR_T */
	struct FMCW_RADAR_VAR rFmcwRadarVar;
#endif
	...
	uint8_t aucOneTimeCalFileBuf[ONE_TIME_CAL_FILE_LEN];
...
/*#if (CFG_EEPROM_PAGE_ACCESS == 1)*/
	uint8_t aucEepromVaule[32]; /* HQA CMD for Efuse Block size contents */
	uint32_t aucEepromValid;
...
/*#endif*/
	...
	uint8_t fgFwLogBinLoad;
#endif /* CFG_SUPPORT_FW_IDX_LOG_TRANS */

#if (CFG_SUPPORT_6G_PWR_REGS == 1)
	u_int8_t fgTxPwrEnvEnable;
#endif  /* CFG_SUPPORT_6G_PWR_REGS == 1 */
#if CFG_SUPPORT_MULTI_CARD
#if CFG_SUPPORT_DBDC
	...
	uint32_t u4FWLastUpdateTime;
#endif /* CFG_SUPPORT_FW_LOG_TIMESYNC */

	struct MIB_INFO_STAT arMibInfo[ENUM_BAND_NUM];
};				/* end of _ADAPTER_T */
...
#define IS_BSS_AP(_prAdapter, _prBssInfo) \
	(IS_BSS_APGO(_prBssInfo) && \
	 p2pFuncIsAPMode( \
	(_prAdapter)->rWifiVar. \
	prP2PConnSettings[(_prBssInfo)->u4PrivateData]))

#define IS_BSS_INDEX_APGO(_prAdapter, _BssIndex) \
	(((_BssIndex) < MAX_BSSID_NUM) && \
	(IS_BSS_APGO((_prAdapter)->aprBssInfo[(_BssIndex)])))

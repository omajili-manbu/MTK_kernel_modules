...
#define NAN_GROUP_SECURITY_ASSOC_USE_GTKSA		BIT(0)
#define NAN_GROUP_SECURITY_ASSOC_USE_IGTKSA		BIT(1)
#define NAN_GROUP_SECURITY_ASSOC_USE_BIGTKSA		BIT(2)
/****************************************************
 *                    Local part
 ****************************************************
 */
...
/* Support 4 NDP context for 2NDI*2NDI connection between local and peer */
#define NAN_MAX_SUPPORT_NDL_FIXED_NUM 8
#define NAN_MAX_SUPPORT_PER_NDP_STA_REC_NUM 1
...
struct _NAN_MC_TX_LOOKUP {
	uint8_t aucMulticastAddr[MAC_ADDR_LEN];
	uint8_t aucLocalNdiAddr[MAC_ADDR_LEN];
	struct _NAN_NDC_MGMT_T *prNdcMgmt;
	struct _NAN_NDP_INSTANCE_T *prNDP;
	uint8_t ucLocalNdiIdx;
};

struct _NAN_MCDATA_RX_TEST_PATH {
	uint8_t fgMcDataRxTestCmdIsSet;
	uint8_t aucReserved[3];
	uint8_t aucOriginPeerAddr[NAN_MAC_ADDR_LEN];
	uint8_t aucNewPeerAddr[NAN_MAC_ADDR_LEN];
};

extern struct _NAN_MCDATA_RX_TEST_PATH g_rNanMcDataRxTestPath;
...
struct _NAN_NDP_INSTANCE_T {
	...
	unsigned char fgTxGtkRequired;
	uint8_t ucTxSecurityCap;
	uint8_t ucTxGtkCipherType;
	unsigned char fgRxGtkRequired;
	uint8_t ucRxSecurityCap;
	uint8_t ucRxGtkCipherType;
	uint8_t ucGroupSAUsed;
#endif
...
struct _NAN_CMD_DATA_END {
	...
	uint8_t aucInitiatorDataAddress[MAC_ADDR_LEN];
	uint8_t aucReserved[2];
};

struct _NAN_CMD_OOB_ACTION {
	uint64_t u8BitMap;
	uint8_t aucDestAddress[MAC_ADDR_LEN];
	uint8_t aucSrcAddress[MAC_ADDR_LEN];
	uint8_t aucBssid[MAC_ADDR_LEN];
	uint8_t ucMapId;	/* 0: 2G4, 1: 5G */
	uint16_t u2Timeout;	/* unit: ms */
	uint8_t ucFrameType;
...
struct _NAN_CMD_OOB_ACTION_AUTH_FRAME_T {
	...
	uint16_t u2Timeout;
	uint16_t u2Token;
	uint8_t aucOobActionAuthFrame[];
};

struct _NAN_OOB_ACTION_TXS_EVT_T {
	uint16_t u2Token;
	uint16_t u2Timeout;
	uint8_t ucBitmapId;
	uint8_t aucRsv[3];
	uint32_t au4Bitmap[2];
	uint32_t u4TxStatus;
	uint8_t aucRawTxS[NIC_TX_STATUS_LENGTH];
};
...
nanDataEngineGetNdiIdxByAddr(struct ADAPTER *prAdapter,
			uint8_t *pucMacAddr);
u_int8_t
nanCheckScanAbort(struct ADAPTER *prAdapter);

void
nanTerminateAllNdp(struct ADAPTER *prAdapter);

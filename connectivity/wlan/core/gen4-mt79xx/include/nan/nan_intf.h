...
#define NAN_KDE_ATTR_BUF_SIZE 256
#define NAN_MIC_BUF_SIZE 600
#define NAN_SKDA_ATTR_BUF_SIZE 200
...
#define NAN_MAX_MULTI_NDI_NUM 1
...
enum NAN_BSS_ROLE_INDEX {
	NAN_BSS_INDEX_MAIN,
	NAN_BSS_INDEX_2G_BAND = NAN_BSS_INDEX_MAIN,
	NAN_BSS_INDEX_5G_BAND,
	NAN_BSS_INDEX_NUM
};

enum NAN_NDI_INDEX {
	NAN_NDI_INDEX_0 = 0,
#if (NAN_MAX_MULTI_NDI_NUM > 1)
	NAN_NDI_INDEX_1,
#endif
#if (NAN_MAX_MULTI_NDI_NUM > 2)
	NAN_NDI_INDEX_2,
#endif
#if (NAN_MAX_MULTI_NDI_NUM > 3)
	NAN_NDI_INDEX_3,
#endif
#if (NAN_MAX_MULTI_NDI_NUM > 4)
	NAN_NDI_INDEX_4,
#endif
#if (NAN_MAX_MULTI_NDI_NUM > 5)
	NAN_NDI_INDEX_5,
#endif
#if (NAN_MAX_MULTI_NDI_NUM > 6)
	NAN_NDI_INDEX_6,
#endif
	NAN_NDI_INDEX_NUM
};
...
#define WFPAL_NAN_MAX_NIK_LENGTH   256
struct NanSkdaNikKde {
	uint8_t nik_length;
	uint8_t nik[WFPAL_NAN_MAX_NIK_LENGTH];
	uint8_t cipher_version;
} __KAL_ATTRIB_PACKED__;

struct NanSkdaNikLifetimeKde {
	uint16_t key_bitmap;
...
struct NanSkdaInfo {
	/// 1 = true, 0 = false
	/* if this is true, firmware can generate
	 * the SKDA given the information below
	 */
	uint8_t append_nik_kde;
	struct NanSkdaNikKde nik;
	/// 1 = true, 0 = false
	/* if this is true, firmware can generate
	 *the SKDA given the information below
	 */
	uint8_t append_nik_lifetime_kde;
	struct NanSkdaNikLifetimeKde nik_lifetime;
	/// 1 = true, 0 = false
	/* if this is true, firmware can append
	 * the group keys based on what
	 * has been configured
	 */
	uint8_t append_igtk_kde;
} __KAL_ATTRIB_PACKED__;
...
struct NanTransmitFollowupRequest {
	...
	uint16_t custom_attribute_length;
	uint8_t custom_attributes[NAN_CUSTOM_ATTRIBUTE_MAX_SIZE];
	/* host assign by skda_info to driver
	 * but driver need to append it as
	 * custom_attr so that no need
	 * this element
	 */
//	struct NanSkdaInfo skda_info;
} __KAL_ATTRIB_PACKED__;
...
struct NanDataPathInitiatorRequest {
	...
	/* Multicast MAC addr of the NDP */
	uint8_t multicast_address[NAN_MAC_ADDR_LEN];

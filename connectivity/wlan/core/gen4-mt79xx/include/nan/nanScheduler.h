...
struct NAN_EVENT_NDL_FLOW_CTRL_T {
...
struct _NAN_CMD_UPDATE_ATTR_CACHE_T  {
	uint32_t u4AvailAttrSize;
	uint8_t aucAvailAttr[];
} __KAL_ATTRIB_PACKED__ __KAL_ATTRIB_ALIGN_4__;
...
uint32_t nanSchedNegoChkRmtCrbProposal(struct ADAPTER *prAdapter,
			uint8_t *pucPeerNmiAddr, uint32_t *pu4RejectCode);
...
void nanSchedNegoStop(struct ADAPTER *prAdapter, uint8_t *pucPeerNmiAddr);
...
extern uint16_t g_u2MaxChnlSwitchTimeUs;

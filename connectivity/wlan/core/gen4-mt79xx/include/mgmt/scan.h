...
#define SCN_VENDOR_COMMAND_SCAN_START	0
#define SCN_VENDOR_COMMAND_SCAN_DONE	1
#endif
...
/* P_LINK_T prP2pDeviceList; */
	...
	int8_t acTxPwrEnvMaxTxPwr[TX_PWR_ENV_MAX_TXPWR_BW_NUM];
	uint8_t fgIsTxPwrEnvChange;
...
struct SCAN_RADIO_INFO {
	uint8_t aucChannelNum;
	uint16_t au2ChannelIdleTime;
	/* Mdrdy Count in each Channel  */
	uint8_t aucChannelMDRDYCnt;
	/* Beacon and Probe Response Count in each Channel */
	uint8_t aucChannelBAndPCnt;
	uint16_t u2ChannelScanTime;
	/* eBand infor for differing the 2g4/6g */
	enum ENUM_BAND aeChannelBand;
};
...
#if (CFG_SUPPORT_SCAN_RADIOINFO == 1)
	uint8_t ucSparseChannelArrayValidNum[SCAN_COUNT];
	uint8_t ucScanCount;
...
#if (CFG_SUPPORT_SCAN_RADIOINFO == 1)
	struct SCAN_RADIO_INFO rScanRadioInfo[SCAN_COUNT][MAX_CHANNEL_NUM];
#else /* CFG_SUPPORT_SCAN_RADIOINFO is 0 */
...
#define IS_6G_PSC_CHANNEL(_ch)		((_ch % 16) == 5)

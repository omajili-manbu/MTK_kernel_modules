...
struct ONE_TIME_CAL_SEND_CTX {
	struct ADAPTER *prAdapter;
	enum ONE_TIME_CAL_TYPE aeCalType[ONE_TIME_CAL_SEQ_MAX];
	uint32_t au4CalParam[ONE_TIME_CAL_SEQ_MAX];
	uint8_t u4SeqTotal;   /* total types in sequence */
	uint8_t u4SeqIdx;     /* current type index */
	enum ONE_TIME_CAL_TYPE eCalType;
	uint32_t u4CalParam;
	uint8_t ucBlkSeqNum[ONE_TIME_CAL_SEQ_MAX];
	uint8_t ucFragSeqNum;
	uint8_t ucBlkCtr;
	uint8_t ucFragCtr;
	struct ONE_TIME_CAL_DATA_BLK rCmdBlk;
};
...
void rlmSendFWOneTimeCalData_Sequence_next(
	struct ONE_TIME_CAL_SEND_CTX *prctx
);
void rlmSendFWOneTimeCalData_next(
	struct ONE_TIME_CAL_SEND_CTX *prctx
);
#endif

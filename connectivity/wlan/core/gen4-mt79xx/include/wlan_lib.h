...
#define WLAN_CFG_ENTRY_NUM_MAX	550	/* SDIO has over 200 entries */
...
#if defined(_HIF_USB)
#define POWER_OFF_WIFI_MAX_WAIT_TIME 50 /* unit: 5ms */
#endif
...
#if CFG_SUPPORT_MULTI_CARD && defined(_HIF_USB)
#define SER_SYN_TIMER(_prAdapter) ((_prAdapter->rSerSyncTimer))
#else
#define SER_SYN_TIMER(_prAdapter) (rSerSyncTimer)
...
#define RTT_MAX_CANDIDATES 10
#define RTT_MAX_CLIENTS 1
#define RTT_REQUEST_DONE_TIMEOUT_SEC 4
#define RTT_REQUEST_CONT_TIMEOUT_SEC 2
#define RTT_BIAS_TABLE_FILE CFG_WIFI_RTT_BIAS_TABLE
 ...
 * [MAC address][MAC address][Type] use 14byte, so set 4 INFO_NUM
 * [TimeStamp-sec][TimeStamp-ms][ReasonCode][BurstIndex] ...
 * [Thermal][RSSI][CBP_Retry][TxAntIdx]
 */
#define FMCW_NONDATA_INFO_NUM 12
#endif /* CFG_SUPPORT_RADAR_CMD */
...
#if CFG_SUPPORT_NAN
	uint8_t aucEthSrcAddr[MAC_ADDR_LEN];
#endif
	uint16_t u2Flag;
...
#if (CFG_SUPPORT_RADAR_CMD == 1)
struct RADAR_RAW_DATA {
	uint8_t ucSubEventId;
	uint8_t ucStatus;
	uint8_t ucReasonCode;
	uint8_t ucFMCWTxAntIdx;
	uint8_t aucReserved[16];
	uint32_t au4RawData[100];
	uint32_t u4TxChainAdcTemp;
	uint32_t u4Fps;
	uint32_t u4BurstCnt;
	uint32_t u4FMCWCBPRetryCnt;
	uint32_t u4FMCWRssi;
	uint64_t rCurrentSec;
	uint64_t rCurrentMsec;
};
#endif /* (CFG_SUPPORT_RADAR_CMD == 1) */

#if (CFG_SUPPORT_RTT == 1)
/* Sync with Android ENUM wifi_rtt_status */
...
/* Sync with Android ENUM rtt_peer_type */
...
/* Sync with Android ENUM wifi_rtt_bw */
...
/* Sync with Android ENUM wifi_rtt_preamble */
...
/* Sync with Android ENUM wifi_rtt_type */
enum ENUM_RTT_TYPE {
...
/* Sync with Android struct wifi_rtt_capabilities */
...
struct RTT_CONFIG {
	/* Sync with Android struct wifi_rtt_config */
	...
	uint16_t u2PreferencePartialTsfTimer; /* Sync with FW, not android */
	...
	/* bellow are for internal useages, sync with FW */
	...
	uint8_t ucBssIndex;
	uint8_t eEventType;  /* enum ENUM_RTT_EVENT_TYPE*/
	uint8_t ucASAP;
	uint8_t ucFtmMinDeltaTime; /* mc: UNIT:100us */
	uint8_t ucReserved; /* 4 byte align */

	/* bias compensate */
	uint32_t u4BiasPsValueBW20;
	uint32_t u4BiasPsValueBW40;
	uint32_t u4BiasPsValueBW80;
	uint32_t u4BiasPsValueBW160;
};

/* Sync with Android struct wifi_rtt_result */
struct RTT_RESULT {
	uint8_t aucMacAddr[MAC_ADDR_LEN];
	uint32_t u4BurstNum;

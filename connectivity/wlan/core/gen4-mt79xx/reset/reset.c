...
const char *eventName[RFSM_EVENT_MAX + 1] = {
	...
	"RFSM_EVENT_RESET_OFF_DONE",
	"RFSM_EVENT_All"
};

const char *notifyName[MODULE_NOTIFY_MAX + 1] = {
	"MODULE_NOTIFY_MESSAGE",
	"MODULE_NOTIFY_PRE_POWER_OFF",
	"MODULE_NOTIFY_RESET_DONE",
	"MODULE_NOTIFY_PRE_FUNCTION_OFF",
	"MODULE_NOTIFY_POWER_OFF_DONE",
	"MODULE_NOTIFY_POWER_ON_DONE",
	"MODULE_NOTIFY_MAX"
};
...
static bool tryRegisterModule(uint32_t bus_id, enum ModuleType module,
				uint32_t *dongle_id);
static bool tryFindDongle(uint32_t bus_id, uint32_t *dongle_id);
#endif
/**********************************************************************
*                            P U B L I C   D A T A
***********************************************************************
...
struct ResetInfo resetInfo[MAX_DONGLE_NUM];
static bool fgExit;
				...
				unsigned long event, void *unused)
{
	...
	if (event == SYS_RESTART ||
		...
		for (dongle_id = 0; dongle_id < MAX_DONGLE_NUM; dongle_id++) {
			if (!resetInfo[dongle_id].fgIsReady)
				continue;
...
static void resetkoNotifyEventToAll(uint32_t dongle_id,
				    enum ModuleNotifyEvent event)
{
	struct FsmEntity *fsm_temp;
	enum ModuleType module;

	for (module = RESET_MODULE_TYPE_WIFI;
	     module < RESET_MODULE_TYPE_MAX;
	     module++) {
		fsm_temp = findResetFsm(dongle_id, module);
		if (!fsm_temp)
			continue;
		resetkoNotifyEvent(fsm_temp, event);
	}
}
...
void resetkoNotifyWork(struct work_struct *work)
{
	...
	while (1) {
		...
		MR_Info("[%s_%d] %s %s\n",
			fsm->name, fsm->dongle_id,
			__func__, notifyName[prEvent->event]);
#if CFG_RESETKO_SUPPORT_MULTI_CARD
...
void clearAllModuleReady(uint32_t dongle_id)
{
	...
	for (id = 0; id < MAX_DONGLE_NUM; id++) {
		if (id == dongle_id ||
			resetHif_isReuseSomeHifInfo(id, dongle_id)) {
			list_for_each_entry_safe(fsm, next_fsm,
					&resetInfo[dongle_id].moduleList,
					node) {
				fsm->fgReady = false;
			}
		}
	...
	for (id = 0; id < MAX_DONGLE_NUM; id++) {
		if (id == dongle_id ||
			...
			list_for_each_entry_safe(fsm, next_fsm,
					&resetInfo[dongle_id].moduleList,
					node) {
				MR_Info("[%s_%d] %s: %s and %s power off\n",
					fsm->name, fsm->dongle_id, __func__,
					fsm->fgReady ? "ready" : "not ready",
					fsm->fgPowerOff ? "request" :
					"not request");
				/* power off request will be always ready */
				if (!fsm->fgPowerOff && !fsm->fgReady)
					ret = false;
			}
		}
	}
...
void powerOff(uint32_t dongle_id)
{
	 ...
	 * 3. power off
	 */
	resetHif_SdioRemoveHost(dongle_id);
	/*pcie host remove*/
	resetHif_PcieRemoveHost(dongle_id);

	resetHif_ResetGpioPull(dongle_id);
	resetHif_PowerGpioSwitchOff(dongle_id);
	resetkoNotifyEventToAll(dongle_id, MODULE_NOTIFY_POWER_OFF_DONE);
...
void powerOn(uint32_t dongle_id)
{
	...
	resetHif_SdioAddHost(dongle_id);
	/*pcie host add*/
	resetHif_PcieAddHost(dongle_id);

	resetkoNotifyEventToAll(dongle_id, MODULE_NOTIFY_POWER_ON_DONE);
...
static void resetkoResetWork(struct work_struct *work)
{
	enum ModuleType module;
	struct delayed_work *delay_work;
...
#else
	send_reset_event(RESET_MODULE_TYPE_WIFI, RFSM_EVENT_RESET_OFF_DONE);
#endif
	msleep(50);
	...
	mutex_lock(&resetInfo->moduleMutex);
	resetkoNotifyEventToAll(resetInfo->dongle_id, MODULE_NOTIFY_RESET_DONE);
	for (module = RESET_MODULE_TYPE_WIFI;
	     module < RESET_MODULE_TYPE_MAX;
	     module++) {
					   ...
					   enum ResetFsmEvent event)
{
	...
	if ((dongle_id >= MAX_DONGLE_NUM) ||
		...
		MR_Err("send_reset_event: args error %d %d %d\n",
	...
	if (!resetEvent) {
		MR_Err("[%s_%d] send_reset_event: allocResetEvent fail\n",
...
/*----------------------------------------------------------------------------*/
/*!
 * @brief send reset event to resetko
 * @param   bus_id    bus id of the module, only need if multi card supported
...
static enum ReturnStatus _send_msg_to_module(uint32_t dongle_id,
				    ...
				    struct ModuleMsg *msg)
{
	...
	if (!msg)
		MR_Err("send_msg_to_module: %d -> %d, msg is NULL\n",
			srcModule, dstModule);

	mutex_lock(&resetInfo[dongle_id].moduleMutex);
	srcfsm = findResetFsm(dongle_id, srcModule);
	if (!srcfsm) {
		MR_Err("send_msg_to_module: src module (%d) not exist\n",
			srcModule);
		goto SEND_MSG_FAIL;
	}
	dstfsm = findResetFsm(dongle_id, dstModule);
	if (!dstfsm) {
		MR_Err("send_msg_to_module: dst module (%d) not exist\n",
			dstModule);
		goto SEND_MSG_FAIL;
	}
...
/*----------------------------------------------------------------------------*/
/*!
 * @brief send message to another module which is regisgered in resetko
 * @param   bus_id    bus id of the module, only need if multi card supported
 * @param   srcModule source module type
 * @param   dstModule destination module type
...
/*----------------------------------------------------------------------------*/
/*!
 * @brief update sdio hif info, for sdio chip reset
 * @param   bus_id    bus id of the module, only need if multi card supported
 * @param   type      type of the hif info, now it's HIF_INFO_SDIO_HOST
 * @param   info      sdio hif info
 * @retval  ReturnStatus
 */
/*----------------------------------------------------------------------------*/
#if CFG_RESETKO_SUPPORT_MULTI_CARD
...
#if CFG_RESETKO_SUPPORT_MULTI_CARD
	if (!tryFindDongle(bus_id, &dongle_id))
		return RESET_RETURN_STATUS_FAIL;
#endif

	if (type == HIF_INFO_SDIO_HOST)
		return resetHif_UpdateSdioHost(dongle_id, info);

	if (type == HIF_INFO_PCIE_HOST)
		return resetHif_UpdatePcieHost(dongle_id, info);

	return RESET_RETURN_STATUS_FAIL;
}
EXPORT_SYMBOL(update_hif_info);

static enum ReturnStatus _resetko_power_request(uint32_t dongle_id,
					enum ModuleType module,
					enum PowerRequestType request,
					uint32_t timeout_ms)
{
	struct FsmEntity *fsm;
	enum ReturnStatus ret = RESET_RETURN_STATUS_FAIL;
	ktime_t start, now;
	s64 elapsed_ms;
	...
	if ((dongle_id >= MAX_DONGLE_NUM) ||
	    ((unsigned int)module >= RESET_MODULE_TYPE_MAX) ||
	    ((unsigned int)request >= POWER_MAX)) {
		MR_Err("%s: args error %d %d %d %d\n",
			__func__, dongle_id, module, request, timeout_ms);
		return ret;
	}

	MR_Err("%s: %s request %s\n", __func__,
		resetInfo[dongle_id].moduleName[module],
		request == POWER_OFF ? "POWER_OFF" : "POWER_ON");

	mutex_lock(&resetInfo[dongle_id].moduleMutex);
	fsm = findResetFsm(dongle_id, module);
	if (!fsm) {
		MR_Err("%s: fail to find fsm\n", __func__);
		goto out;
	}
	if (request == POWER_OFF) {
		/* Current state to pre_power_off */
		resetFsmHandlevent(fsm, RFSM_EVENT_TRIGGER_POWER_OFF);
		/* pre_power_off to power_off */
		resetFsmHandlevent(fsm, RFSM_EVENT_READY);
	} else {
		/* power_off to power_up */
		resetFsmHandlevent(fsm, RFSM_EVENT_TRIGGER_POWER_ON);
	}
...
out:
	...
	if (request == POWER_ON) {
		ret = RESET_RETURN_STATUS_SUCCESS;
	} else {
		while (fsm != NULL) {
			/* for power off request,
			 * need check and wait request done
			 */
			if (strcmp(fsm->fsmState->name, "POWER_OFF") == 0) {
				ret = RESET_RETURN_STATUS_SUCCESS;
				break;
			}
			...
			msleep(20);
			now = ktime_get();
			elapsed_ms = ktime_to_ms(ktime_sub(now, start));
			if (elapsed_ms > timeout_ms) {
				MR_Err("%s: power off timeout, %lldms > %dms\n",
					__func__, elapsed_ms, timeout_ms);
				break;
			}
		}
	}
...
/*----------------------------------------------------------------------------*/
/*!
 * @brief module call this function to request power on or power off
 * @param   bus_id    bus id of the module, only need if multi card supported
 * @param   module    module type
 * @param   request   power request type, POWER_ON or POWER_OFF
 * @retval  ReturnStatus
 */
/*----------------------------------------------------------------------------*/
#if CFG_RESETKO_SUPPORT_MULTI_CARD
enum ReturnStatus resetko_power_request(uint32_t bus_id,
					enum ModuleType module,
					enum PowerRequestType request,
					uint32_t timeout_ms)
{
	...
	if (tryRegisterModule(bus_id, module, &dongle_id))
		return _resetko_power_request(dongle_id, module,
					      request, timeout_ms);
...
#else
enum ReturnStatus resetko_power_request(enum ModuleType module,
					enum PowerRequestType request,
					uint32_t timeout_ms)
{
	return _resetko_power_request(0, module, request, timeout_ms);
}
#endif
EXPORT_SYMBOL(resetko_power_request);
...
static int resetko_probe(struct platform_device *pdev)
{
	...
	for (dongle_id = 0; dongle_id < MAX_DONGLE_NUM; dongle_id++) {
		if (resetInfo[dongle_id].fgIsReady == false) {
			resetInfo[dongle_id].fgIsReady = true;
	...
	for (dongle_id = 0; dongle_id < MAX_DONGLE_NUM; dongle_id++) {
		if (resetInfo[dongle_id].bus_id == bus_id) {
			MR_Info(
			   ...
			   dongle_id, bus_id);
			resetInfo[dongle_id].fgIsReady = false;
			...
			mutex_destroy(&resetInfo[dongle_id].moduleMutex);
			break;
		}
	}
...
static enum ReturnStatus _resetko_register_module(uint32_t dongle_id,
					enum ModuleType module,
					char *name,
					void *notifyFunc)
{
	...
	if (findResetFsm(dongle_id, module) != NULL) {
		mutex_unlock(&resetInfo[dongle_id].moduleMutex);
		MR_Err("resetko_register_module: insmod module(%d) existed\n",
			module);
		freeResetFsm(fsm);
		return RESET_RETURN_STATUS_FAIL;
	}
...
static bool tryFindDongle(uint32_t bus_id, uint32_t *dongle_id)
{
	...
	MR_Err("can not find dongle for bus %d\n", bus_id);
	return false;
}
...
/*----------------------------------------------------------------------------*/
/*!
 * @brief call this function to register module to resetko
 * @param   module    module type
 * @param   name      module name
 * @param   notifyFunc  module notify callback function
 * @retval  ReturnStatus
 */
/*----------------------------------------------------------------------------*/
...
static enum ReturnStatus _resetko_unregister_module(uint32_t dongle_id,
					     enum ModuleType module)
{
	struct FsmEntity *fsm;
	...
	mutex_destroy(&fsm->notifyEventMutex);
	mutex_unlock(&resetInfo[dongle_id].moduleMutex);
...
/*----------------------------------------------------------------------------*/
/*!
 * @brief call this function to unregister module from resetko
 * @param   module    module type
 * @retval  ReturnStatus
 */
/*----------------------------------------------------------------------------*/
...
static int __init resetInit(void)
{
	...
	resetInfo[0].fgIsPowerOff = true;
#else
	resetInfo[0].fgIsPowerOff = false;
#endif
	resetInfo[0].fgIsReady = true;
...
static void __exit resetExit(void)
{
	...
	resetHif_Uninit(0);
	mutex_destroy(&resetInfo[0].moduleMutex);
#endif
	fgExit = true;

		...
		IN struct P2P_SCAN_REQ_INFO *prScanReqInfo)
{
	...
	do {
		...
		if (prScanReqV2->ucSeqNum == OBSS_SCAN_SEQ_NUM)
			prScanReqV2->ucSeqNum =
		...
		prScanReqV2->u2ChannelDwellTime =
			prScanReqInfo->u2PassiveDwellTime;
		...
		IN u_int8_t fgNonCckRate)
{
	...
	do {
		...
		case MAC_FRAME_REASSOC_RSP:
			...
			if (prStaRec != NULL)
				prMgmtTxMsdu =
					p2pFuncProcessP2pAssocResp(prAdapter,
					prStaRec, ucBssIndex, prMgmtTxMsdu);
			else
				DBGLOG(P2P, WARN,
				"[OWE] StrRec is NULL use supplicant\n");
		...
		IN struct P2P_CHNL_REQ_INFO *prP2pChnlReqInfo)
{
	...
	do {
		...
		if ((prAisBssInfo != NULL) && (prAisBssInfo->eConnectionState ==
			MEDIA_STATE_CONNECTED) && (prWifiVar->ucP2pGoAutoSCC ==
			FEATURE_ENABLED) &&
			cnmIsSCCAutoSwitch(prAdapter,
		...
		if (prP2pChnlReqInfo->eBand == BAND_5G &&
			...
			p2pFuncStopRdd(prAdapter, prBssInfo->ucBssIndex);
		}
...
#if (CFG_SUPPORT_AUTO_SCC == 1)
		prP2pBssInfo->fgGoStarted = FALSE;
#endif
	} while (FALSE);
}				/* p2pFuncStopGO */
...
void p2pFuncStartRdd(IN struct ADAPTER *prAdapter, IN uint8_t ucBssIdx)
{
	...
	switch (rlmDomainGetDfsRegion(prAdapter)) {
		...
		IN struct P2P_CHNL_REQ_INFO rP2pChnlReqInfo)
{
	...
	if (!IS_BSS_APGO(prBssInfo)) {
		DBGLOG(P2P, ERROR, "Bss should be AP/GO!\n");
		return;
	}
...
/* To Support Cross Band Channel Swtich */
	...
	if (prBssInfo->eBand == BAND_5G
		...
		/* Depend on eCurrentOPMode and ucPhyTypeSet */
		prBssInfo->ucConfigAdHocAPMode = AP_MODE_11A;
	} else if (fgIsPureAp) { /* Only SAP mode should enter this function */
		prBssInfo->ucPhyTypeSet |= PHY_TYPE_SET_802_11BG;
		/* Depend on eCurrentOPMode and ucPhyTypeSet */
		prBssInfo->ucConfigAdHocAPMode = AP_MODE_MIXED_11BG;
	} else { /* GO Mode should enter this function */
		prBssInfo->ucPhyTypeSet |= PHY_TYPE_SET_802_11G;
		/* Depend on eCurrentOPMode and ucPhyTypeSet */
		prBssInfo->ucConfigAdHocAPMode = AP_MODE_11G_P2P;
	}
...
uint32_t p2pFuncGetCacRemainingTime(void)
{
	...
	u8CacRemainingBootTime = kalGetBootTime() - g_u8CacStartBootTime;
...
u_int32_t p2pFuncPreStartRdd(struct ADAPTER *prAdapter,
	uint8_t ucRoleIdx, struct cfg80211_chan_def *chandef,
	unsigned int cac_time_ms)
{
	...
	if (!prAdapter || !chandef)
		goto out;

	prGlueInfo = prAdapter->prGlueInfo;
	if (!prGlueInfo || !prGlueInfo->prP2PInfo[ucRoleIdx])
		goto out;

	if (prGlueInfo->prP2PInfo[ucRoleIdx]->chandef == NULL) {
		prGlueInfo->prP2PInfo[ucRoleIdx]->chandef =
			(struct cfg80211_chan_def *)
				kalMemAlloc(sizeof(struct cfg80211_chan_def),
				VIR_MEM_TYPE);
		if (prGlueInfo->prP2PInfo[ucRoleIdx]->chandef == NULL) {
			i4Rslt = -ENOMEM;
			goto out;
		...
		prGlueInfo->prP2PInfo[ucRoleIdx]->chandef->chan =
			(struct ieee80211_channel *)
				kalMemAlloc(sizeof(struct ieee80211_channel),
				VIR_MEM_TYPE);
	...
	kalMemZero(prGlueInfo->prP2PInfo[ucRoleIdx]->chandef->chan,
		sizeof(struct ieee80211_channel));
	...
	kalMemCopy(prGlueInfo->prP2PInfo[ucRoleIdx]->chandef->chan,
	...
	DBGLOG(P2P, INFO, "PreStartRdd (role %d)\n", ucRoleIdx);
...
u_int32_t p2pFuncStartCAC(struct ADAPTER *prAdapter,
	uint8_t ucRoleIdx, struct cfg80211_chan_def *chandef)
{
	int32_t i4Rslt = -EINVAL;
	...
	DBGLOG(P2P, INFO, "StartCAC (role %d)\n", ucRoleIdx);
	...
	mboxSendMsg(prAdapter, MBOX_ID_0,
		(struct MSG_HDR *) prP2pDfsCacMsg, MSG_SEND_METHOD_BUF);
		...
		IN uint8_t *pucIEInfo, IN uint32_t u4IELen)
{
	...
	if (!pucIEInfo) {
		DBGLOG(P2P, ERROR, "IE pointer is NULL.\n");
		return;
	}
		...
		OUT uint32_t *num_freq_list)
{
	...
	if (!prAisBssInfo) {
		...
		/* 2.4G should be always included */
		...
		if (prAisBssInfo->eBand == BAND_2G4) {
			...
			if (prAisBssInfo->eBand == BAND_5G
				...
				freq_list[ucIdx] = nicChannelNum2Freq(
					prAisBssInfo->ucPrimaryChannel,
					prAisBssInfo->eBand) / 1000;
				(*num_freq_list)++;
				ucIdx++;
			...
			if (prWifiVar->ucP2pGoACS == FEATURE_ENABLED) {
#if (CFG_SUPPORT_AUTO_SCC == 1)
				if (prWifiVar->ucP2pGoAutoSCC ==
					FEATURE_ENABLED) {
					p2pFunGetAcsBestChList(prAdapter,
						BIT(prAisBssInfo->eBand),
						MAX_BW_20MHZ,
						BITS(0, 31), BITS(0, 31),
						BITS(0, 31), BITS(0, 31),
						&ucNumOfChannel,
			...
			for (i = 0; i < ucNumOfChannel; i++) {
				freq_list[i + ucIdx] = nicChannelNum2Freq(
					aucChannelList[i].ucChannelNum,
					aucChannelList[i].eBand) / 1000;
				(*num_freq_list)++;
			}
		...
		IN uint32_t u4LteSafeChnMask_6G)
{
	...
	struct WIFI_VAR *prWifiVar;
	uint8_t j = 0, ucRetChannel = 0;
	struct PARAM_PREFER_CHN_INFO *prPreferChannel;
	uint8_t num_channels = 0;
	uint32_t min_d = 0xFFFFFFFF, max_d = 0;
	uint16_t min_idle_slot = 0xFFFF, max_idle_slot = 0;
	...
	num_channels +=
		p2pFuncAcsCountBits(u4LteSafeChnMask_2G) +
		p2pFuncAcsCountBits(u4LteSafeChnMask_5G_1) +
		p2pFuncAcsCountBits(u4LteSafeChnMask_5G_2) +
		p2pFuncAcsCountBits(u4LteSafeChnMask_6G);

	if (!num_channels) {
		DBGLOG(P2P, WARN, "no available LTE safe channel\n");
		return 0;
	}

	prPreferChannel = kalMemAlloc(num_channels *
		sizeof(struct PARAM_PREFER_CHN_INFO), VIR_MEM_TYPE);

	if (!prPreferChannel) {
		DBGLOG(P2P, WARN, "can't alloc prPreferChannel\n");
		return 0;
	}

	kalMemZero(prPreferChannel,
		num_channels * sizeof(struct PARAM_PREFER_CHN_INFO));
	...
	if (aucChannelList == NULL) {
		DBGLOG(P2P, WARN, "can't alloc aucChannelList\n");
		kalMemFree(prPreferChannel, VIR_MEM_TYPE,
			num_channels * sizeof(struct PARAM_PREFER_CHN_INFO));
		return 0;
	}
	 ...
	 */
	prGetChnLoad = &(prWifiVar->rChnLoadInfo);
	...
	for (i = 0; i < ucNumOfChannel; i++) {
		...
		max_d = kal_max_t(uint32_t, max_d,
			prGetChnLoad->rEachChnLoad[ucIdx].u4Dirtiness);
		min_d = kal_min_t(uint32_t, min_d,
			prGetChnLoad->rEachChnLoad[ucIdx].u4Dirtiness);
		max_idle_slot = kal_max_t(uint16_t, max_idle_slot,
			prGetChnLoad->rEachChnLoad[ucIdx].u2IdleSlot);
		min_idle_slot = kal_min_t(uint16_t, min_idle_slot,
			prGetChnLoad->rEachChnLoad[ucIdx].u2IdleSlot);

		prPreferChannel[j].ucChannel =
			prGetChnLoad->rEachChnLoad[ucIdx].ucChannel;
		prPreferChannel[j].u2IdleSlot =
			prGetChnLoad->rEachChnLoad[ucIdx].u2IdleSlot;
		prPreferChannel[j].u4Dirtiness =
			prGetChnLoad->rEachChnLoad[ucIdx].u4Dirtiness;

		DBGLOG(P2P, LOUD,
			"Channel: %3d, Dirtiness: %4d, IdleSlot: %4d\n",
			prPreferChannel[j].ucChannel,
			prPreferChannel[j].u4Dirtiness,
			prPreferChannel[j].u2IdleSlot);
		j++;
	...
	i = 0;
	ucRetChannel = prPreferChannel[i].ucChannel;

	for (j = 0; j < num_channels; j++) {
		if (prPreferChannel[j].ucChannel == 0)
			continue;

		prPreferChannel[j].ucRank =
			(uint8_t)(prWifiVar->ucWeightDirt *
			(prPreferChannel[j].u4Dirtiness - min_d) /
			(max_d - min_d)) +
			(prWifiVar->ucWeightIdle) -
			(uint8_t)(prWifiVar->ucWeightIdle *
			(prPreferChannel[j].u2IdleSlot - min_idle_slot) /
			(max_idle_slot - min_idle_slot));
		...
		if (prPreferChannel[j].ucChannel > 0 &&
			prPreferChannel[j].ucRank < prPreferChannel[i].ucRank) {
			i = j;
			ucRetChannel = prPreferChannel[i].ucChannel;
		}
	}
...
uint8_t p2pFuncAcsCountBits(uint32_t n)
{
	uint8_t count = 0;
...
enum ENUM_CHNL_EXT p2pFunGetSco(IN struct ADAPTER *prAdapter,
	...
	if (eBand == BAND_2G4) {
		...
		if (regd_is_single_sku_en(prAdapter)) {
			...
			calculateScoByChannel(prAdapter, eBand,
				ucPrimaryCh, &eSCO);
#endif
	}
...
#if (CFG_SUPPORT_SCAN_RADIOINFO == 1)
	for (uCnt = 0; uCnt < SCAN_COUNT; uCnt++) {
		for (ucChCount = 0; ucChCount < prScanInfo
			->ucSparseChannelArrayValidNum[uCnt];
				ucChCount++) {
			uint8_t ucIdx =
	#if (CFG_SUPPORT_WIFI_6G == 1)
				wlanGetChannelIndex(
				prScanInfo->rScanRadioInfo[uCnt][ucChCount].
					aeChannelBand,
				prScanInfo->rScanRadioInfo[uCnt][ucChCount].
					aucChannelNum);
	#else
				wlanGetChannelIndex(
				SCN_GET_EBAND_BY_CH_NUM(prScanInfo->
					rScanRadioInfo[uCnt][ucChCount].
					aucChannelNum),
				prScanInfo->rScanRadioInfo[uCnt][ucChCount].
					aucChannelNum);
	#endif

			prChnLoadInfo->rEachChnLoad[ucIdx].u2IdleSlot =
				prScanInfo->rScanRadioInfo[uCnt][ucChCount].
					au2ChannelIdleTime;

			DBGLOG(P2P, LOUD,
				"Channel %3d: ChIdleTimeSlot %6d\n",
				prScanInfo->rScanRadioInfo[uCnt][ucChCount].
					aucChannelNum,
				prScanInfo->rScanRadioInfo[uCnt][ucChCount].
					au2ChannelIdleTime);
...
#else /* CFG_SUPPORT_SCAN_RADIOINFO is 0*/
	for (ucChCount = 0; ucChCount < prScanInfo
		->ucSparseChannelArrayValidNum; ucChCount++) {
		uint8_t ucIdx =
#if (CFG_SUPPORT_WIFI_6G == 1)
			wlanGetChannelIndex(
			prScanInfo->aeChannelBand[ucChCount],
			prScanInfo->aucChannelNum[ucChCount]);
#else
			wlanGetChannelIndex(
			SCN_GET_EBAND_BY_CH_NUM(prScanInfo->
				aucChannelNum[ucChCount]),
			prScanInfo->aucChannelNum[ucChCount]);
#endif

		prChnLoadInfo->rEachChnLoad[ucIdx].u2IdleSlot =
			prScanInfo->au2ChannelIdleTime[ucChCount];
...
#if (CFG_SUPPORT_P2P_CSA == 1)
	if (prBssInfo && prBssInfo->eIftype == IFTYPE_P2P_GO)
		return ePolicy;
#endif
		...
		struct BSS_INFO *prP2pBssInfo)
{
	...
	kalMemZero(prGlueP2pInfo->chandef->chan,
		sizeof(struct ieee80211_channel));
	origin_chan = prGlueP2pInfo->chandef->chan;
	kalMemZero(prGlueP2pInfo->chandef,
		sizeof(struct cfg80211_chan_def));
	prGlueP2pInfo->chandef->chan = origin_chan;
...
uint32_t p2pFuncPreReboot(IN struct ADAPTER *prAdapter,
		 ...
		 OUT uint32_t *pu4QueryInfoLen)
{
	...
	if (!prAdapter) {
		DBGLOG(P2P, WARN, "Disconnect prAdapter Null fail\n");
		return WLAN_STATUS_FAILURE;
	}
...
#if (CFG_SUPPORT_DFS_OFFLOAD == 1)
void p2pFuncReadyStartAp(IN struct ADAPTER *prAdapter,
		IN struct P2P_ROLE_FSM_INFO *prP2pRoleFsmInfo)
{
	...
	if ((prAdapter == NULL) || (prP2pRoleFsmInfo == NULL)) {
		DBGLOG(P2P, ERROR, "prAdapter or prP2pRoleFsmInfo NULL\n");
		return;
	}

	ucRoleIdx = prP2pRoleFsmInfo->ucRoleIndex;
	DBGLOG(P2P, EVENT, "Role[%d]\n", ucRoleIdx);

	/* Trigger MID_MNY_P2P_START_AP*/
	prP2PInfo = prAdapter->prGlueInfo->prP2PInfo[ucRoleIdx];
	if (!prP2PInfo->pP2pStartAPMsg) {
		DBGLOG(P2P, ERROR, "Role[%d],P2pStart is NULL\n", ucRoleIdx);
		return;
	}

	p2pRoleFsmRunEventStartAP(prAdapter,
			(struct MSG_HDR *) prP2PInfo->pP2pStartAPMsg);

	if (prP2PInfo->pP2pStartAPMsg)
		prP2PInfo->pP2pStartAPMsg = NULL;
}

void p2pFuncChannelListFiltering(struct ADAPTER *prAdapter,
		uint8_t ucFilteredBw,
		enum ENUM_CHNL_EXT eFilteredSco,
...
uint8_t
p2pFuncSelect5GNonDFSChannel(struct ADAPTER *prAdapter,
		...
		uint8_t ucBandWidth)
{
	...
	u_int8_t ch_idx = 0;
	uint32_t u4ArrySize = 0;
	struct RF_CHANNEL_INFO *aucChannelList = NULL;
	struct RF_CHANNEL_INFO *aucChannelListRdd = NULL;

	if (!prAdapter) {
		DBGLOG(P2P, ERROR, " prAdapter is NULL\n");
		return 0;
	}

	u4ArrySize = sizeof(struct RF_CHANNEL_INFO) * MAX_5G_BAND_CHN_NUM;
	...
	aucChannelListRdd = kalMemAlloc(u4ArrySize, VIR_MEM_TYPE);

	if (aucChannelListRdd == NULL) {
		DBGLOG(P2P, WARN, "can't alloc aucChannelListRdd\n");
		kalMemFree(aucChannelList, VIR_MEM_TYPE, u4ArrySize);
		return 0;
	}

	kalMemZero(aucChannelList, u4ArrySize);
	kalMemZero(aucChannelListRdd, u4ArrySize);

	/* Get random ch */
	rlmDomainGetChnlList(prAdapter, BAND_5G, TRUE, MAX_5G_BAND_CHN_NUM,
			&ucNumOfChannel, aucChannelList);

	p2pFuncChannelListFiltering(prAdapter, ucBandWidth, eSco,
		ucNumOfChannel, aucChannelList,
		&ucNumOfChannel, aucChannelListRdd);

	ch_idx = kalRandomNumber() % ucNumOfChannel;
	ucNewChannel = aucChannelListRdd[ch_idx].ucChannelNum;

	kalMemFree(aucChannelList, VIR_MEM_TYPE, u4ArrySize);
	kalMemFree(aucChannelListRdd, VIR_MEM_TYPE, u4ArrySize);
		...
		IN uint8_t ch_num, IN uint8_t ucRoleIdx)
{
	...
	if (!prAdapter || !ch_num) {
		DBGLOG(P2P, ERROR, " prAdapter is NULL or CH[%d]\n", ch_num);
		return;
	}

	u4Ret = cnmIdcCsaReq(prAdapter, ch_num, ucRoleIdx);

	if (u4Ret < 0)
		DBGLOG(P2P, ERROR, "Switch %d Channel Failed\n", ch_num);
}
	...
	IN uint8_t ch_num, IN uint8_t ucRoleIdx)
{
	...
	rRfChnlInfo.ucChnlBw =
		cnmGetBssBandBw(prAdapter, prBssInfo, rRfChnlInfo.eBand);
	rRfChnlInfo.u2PriChnlFreq =
		nicChannelNum2Freq(ch_num, rRfChnlInfo.eBand) / 1000;
	rRfChnlInfo.u4CenterFreq1 =
		nicGetS1Freq(rRfChnlInfo.eBand,
	...
	DBGLOG(REQ, INFO,
	"CH=%d,Band=%d,BW=%d,PriFreq=%d,S1=%d\n",
		rRfChnlInfo.ucChannelNum,
		rRfChnlInfo.eBand,
		rRfChnlInfo.ucChnlBw,
		rRfChnlInfo.u2PriChnlFreq,
...
uint32_t p2pFuncNotifyDataPathStatus(
	struct ADAPTER *prAdapter,
	uint8_t ucBssIndex,
	uint8_t fgConnect,
	uint8_t fgDpConcurrent,
	struct RF_CHANNEL_INFO *prChannelInfo)
{
	struct MSG_P2P_NAN_DP_UPDATE *prP2pNanDpUpdateMsg = NULL;

	prP2pNanDpUpdateMsg = cnmMemAlloc(
		prAdapter,
		RAM_TYPE_MSG,
		sizeof(struct MSG_P2P_NAN_DP_UPDATE));

	if (!prP2pNanDpUpdateMsg)
		return WLAN_STATUS_FAILURE;

	if (ucBssIndex >= MAX_BSS_INDEX)
		return WLAN_STATUS_FAILURE;

	prP2pNanDpUpdateMsg->rMsgHdr.eMsgId = MID_P2P_NAN_DP_UPDATE;
	prP2pNanDpUpdateMsg->ucBssIndex = ucBssIndex;
	prP2pNanDpUpdateMsg->fgConnect = fgConnect;
	prP2pNanDpUpdateMsg->fgDpConcurrent = fgDpConcurrent;

	if (prChannelInfo) {
		kalMemCopy(
			&prP2pNanDpUpdateMsg->rChannelInfo,
			prChannelInfo,
			sizeof(struct RF_CHANNEL_INFO));
		DBGLOG(P2P, LOUD,
			"P2P notify NAN DP chnl[%d]\n",
			prChannelInfo->ucChannelNum);
	}

	DBGLOG(P2P, INFO,
		"P2P notify NAN DP Bss[%d] Connect[%d] DpConcurrent[%d]\n",
		ucBssIndex,
		fgConnect,
		fgDpConcurrent);
	...
	mboxSendMsg(prAdapter,
		MBOX_ID_0,
		(struct MSG_HDR *) prP2pNanDpUpdateMsg,
		MSG_SEND_METHOD_BUF);

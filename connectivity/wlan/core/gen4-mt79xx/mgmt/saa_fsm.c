	...
	IN struct STA_RECORD *prStaRec)
{
	...
	if (prStaRec->ucTxAuthAssocRetryCount >=
		...
		if (!fgIsSendAssoc) {
			...
			if ((prBssDesc != NULL) &&
				rsnPerformPolicySelection(prAdapter,
...
#if CFG_SUPPORT_NAN
	mtk_cfg80211_vendor_event_nan_infra_auth_rx_indication(prAdapter,
								prSwRfb);
				...
				IN struct SW_RFB *prSwRfb)
{
	...
	do {
		...
		if (IS_STA_IN_AIS(prStaRec)) {
			...
			/* Drop the deauth frame that differs from
			 * the target BSS in the connection process
			 */
			if (prTargetBssDesc && UNEQUAL_MAC_ADDR(
			...
			/* Drop abnormal deauth frame
			 * if TA/RA are not matched
			 */
			if ((UNEQUAL_MAC_ADDR(prStaRec->aucMacAddr,
				prDeauthFrame->aucSrcAddr)) ||
				(IS_UCAST_MAC_ADDR(prDeauthFrame->aucDestAddr)
			...
			if (kalGetMediaStateIndicated(prAdapter->prGlueInfo,
				...
				if (prStaRec->ucStaState == STA_STATE_1 ||
					...
					kalIndicateRxDeauthToUpperLayer(
						...
						(size_t)prSwRfb->u2PacketLen);
					prAdapter->rWifiVar.
						rConnSettings[prStaRec->
						ucBssIndex].
						bss = NULL;
					DBGLOG(SAA, INFO,
						"notify RX deauth for ReasonCode = 0x%x Done\n",
				  ...
				  IN struct SW_RFB *prSwRfb)
{
	...
	do {
		...
		if (IS_STA_IN_AIS(prStaRec)) {
			...
			/* Drop the disassoc frame that differs from
			 * the target BSS in the connection process
			 */
			if (prTargetBssDesc && UNEQUAL_MAC_ADDR(
				prTargetBssDesc->aucBSSID,
			...
			/* Drop abnormal disassoc frame
			 * if TA/RA are not matched
			 */
			if ((UNEQUAL_MAC_ADDR(prStaRec->aucMacAddr,
				prDisassocFrame->aucSrcAddr)) ||
				(IS_UCAST_MAC_ADDR(prDisassocFrame->aucDestAddr)
				&& UNEQUAL_MAC_ADDR(prAisBssInfo->aucOwnMacAddr,
				prDisassocFrame->aucDestAddr))) {
				DBGLOG(SAA, WARN,
					"Received a Disassoc SA[" MACSTR
					"] DA[" MACSTR "] unmatch Peer[" MACSTR
					"] Own[" MACSTR "]\n",
					MAC2STR(prDisassocFrame->aucSrcAddr),
					MAC2STR(prDisassocFrame->aucDestAddr),

...
#define CFG_SUPPORT_ONE_TIME_CAL_ALL_CH 1
...
#if (CFG_SUPPORT_PWR_LIMIT_COUNTRY && (CFG_BUILD_MOBILE_SKU == 1))
			    ...
			    struct MSDU_INFO *prMsduInfo)
{
	...
	if (IS_BSS_AIS(prBssInfo) &&
		(IS_FEATURE_ENABLED(prAdapter->rWifiVar.ucBtmCap))) {
		prExtCap->ucLength = ELEM_MAX_LEN_EXT_CAP;
		SET_EXT_CAP(prExtCap->aucCapabilities, ELEM_MAX_LEN_EXT_CAP,
					ELEM_EXT_CAP_BSS_TRANSITION_BIT);
	}
	...
	if (IS_BSS_AP(prAdapter, prBssInfo) && rttIsRstaEnable(prAdapter))
		SET_EXT_CAP(prExtCap->aucCapabilities, ELEM_MAX_LEN_EXT_CAP,
...
/*----------------------------------------------------------------------------*/
/*!
 * \brief For association request generate VHT Capability IE
 *
 * \param[in] prAdapter : Pointer to adapter
 * \param[in] prMsduInfo : Pointer to the MSDU Info
 *
 * \return none
 */
/*----------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------*/
/*!
 * \brief For probe request generate VHT Capability IE
 *
 * \param[in] prAdapter : Pointer to adapter
 * \param[in] prMsduInfo : Pointer to the MSDU Info
 *
 ...
 */
/*----------------------------------------------------------------------------*/
void rlmProbReqGenerateVhtCapIE(struct ADAPTER *prAdapter,
			    struct MSDU_INFO *prMsduInfo)
{
	struct BSS_INFO *prBssInfo = NULL;
	...
	if (fgIsOwnCap) {
		...
		if (prBssDesc) {
			...
			ucRfCenterFreqSeg1 = nicGetS1(prBssDesc->eBand,
				ucPrimaryChannel, eRfSco, eRfChannelWidth);
...
)
{
	...
	uint16_t u2Freq = 0;
	struct ONE_TIME_CAL_GROUP_DEF_ENTRY *prGroupFreqDefArr =
		prAdapter->chip_info->prGroupFreqDefArr;
#if (CFG_SUPPORT_ONE_TIME_CAL_ALL_CH == 1)
	...
	if (eBand == ONE_TIME_CAL_BAND_2G) {
		...
		ucChannelNum = FACT_CAL_CH_NUM_2G;
		u2Freq =
		...
		ucChannelNum = FACT_CAL_CH_NUM_5G;
		u2Freq =
			ieee80211_channel_to_frequency(
			ucChannel,
			KAL_BAND_5GHZ);
		...
		ucChannelNum = FACT_CAL_CH_NUM_6G;
		u2Freq =
			ieee80211_channel_to_frequency(
			ucChannel,
			KAL_BAND_6GHZ);
	...
	for (ucChIdx = 0; ucChIdx < ucChannelNum; ucChIdx++) {
		if (pu1ChList[ucChIdx] == ucChannel)
			break;
	}
	...
	for (ucGrpDefIdx = 0;
		...
		if (u2Freq >= prGroupFreqDefArr[ucGrpDefIdx].ucFreqStart &&
			u2Freq <= prGroupFreqDefArr[ucGrpDefIdx].ucFreqEnd) {
			*pucGroup =
				prGroupFreqDefArr[ucGrpDefIdx].ucGroupIdx;
			break;
		}
	}
...
/* Context for async one-time cal send */
/* Extended context for async one-time cal send sequence */
void rlmSendFWOneTimeCalData_next(struct ONE_TIME_CAL_SEND_CTX *prctx)
{
	struct ONE_TIME_CAL_BLK_INFO *prCmdBlkInfo;
	uint32_t u4BlkBufLen, u4FragBufLen;
	uint8_t ucFragCtr, ucFragSeqNum, ucBlkSeqNum, ucBlkCtr;
	enum ONE_TIME_CAL_TYPE eCalType;
	uint32_t u4CalParam;
	struct ADAPTER *prAdapter;

	/* Move all declarations to the top for C89 compatibility */
	prCmdBlkInfo = &(prctx->rCmdBlk.rOneTimeCalBlkInfo);
	ucFragCtr = prctx->ucFragCtr;
	ucFragSeqNum = prctx->ucFragSeqNum;
	ucBlkSeqNum = prctx->ucBlkSeqNum[prctx->u4SeqIdx];
	ucBlkCtr = prctx->ucBlkCtr;
	eCalType = prctx->eCalType;
	u4CalParam = prctx->u4CalParam;
	prAdapter = prctx->prAdapter;

	kalMemZero(&(prctx->rCmdBlk), sizeof(struct ONE_TIME_CAL_DATA_BLK));
	prCmdBlkInfo->ucCalType = eCalType;
	prCmdBlkInfo->u4CalParam = u4CalParam;
	prCmdBlkInfo->ucBlkSeqNum = (ucBlkSeqNum << 4) + ucBlkCtr;
	prCmdBlkInfo->ucBlkFragSeqNum = (ucFragSeqNum << 4) + ucFragCtr;
	prCmdBlkInfo->u4BlkLen =
		(ONE_TIME_CAL_DATA_BLK_LEN << 16) + ONE_TIME_CAL_DATA_BLK_LEN;

	if (rlmOneTimeCalUpdateStruct(prAdapter,
		&(prctx->rCmdBlk), FALSE) != WLAN_STATUS_SUCCESS) {
		return;
	}

	u4BlkBufLen =
		(prCmdBlkInfo->u4BlkLen & BITS(16, 31)) >> 16;
	if (ucFragCtr == ucFragSeqNum - 1)
		u4FragBufLen =
		u4BlkBufLen - ucFragCtr * ONE_TIME_CAL_DATA_BLK_LEN;
	else
		u4FragBufLen = ONE_TIME_CAL_DATA_BLK_LEN;

	DBGLOG(RLM, INFO,
		"BlkBufLen=%u, FragBufLen=%u\n",
		u4BlkBufLen, u4FragBufLen);

	prCmdBlkInfo->u4BlkLen = (u4BlkBufLen << 16) + u4FragBufLen;
	...
	wlanSendSetQueryCmd(prAdapter,
		...
		FALSE,
		FALSE,
		nicOneTimeCalSendCmdDoneHandler,
		nicOneTimeCalSendCmdTimeOutHandler,
		(uint32_t)(sizeof(struct ONE_TIME_CAL_DATA_BLK)),
		((uint8_t *)&(prctx->rCmdBlk)),
		prctx,
		sizeof(prctx));
	return;
}

/* setup ctx for next cal type and start sending */
void rlmSendFWOneTimeCalData_Sequence_next(struct ONE_TIME_CAL_SEND_CTX *prctx)
{
	struct ONE_TIME_CAL_DATA_BLK *prCmdBlk;
	struct ONE_TIME_CAL_BLK_INFO *prCmdBlkInfo;
	uint8_t ucBlkSeqNum = 0, ucFragSeqNum = 0, ucBlkCtr = 0;
	enum ONE_TIME_CAL_TYPE eCalType;
	uint32_t u4CalParam;
	struct ADAPTER *prAdapter;
	...
	if (!prctx) {
		DBGLOG(NIC, ERROR,
			"One time cal Sequence_next get null context!\n");
		return;
	}

	prAdapter = prctx->prAdapter;
	eCalType = prctx->aeCalType[prctx->u4SeqIdx];
	u4CalParam = prctx->au4CalParam[prctx->u4SeqIdx];
	ucBlkSeqNum = prctx->ucBlkSeqNum[prctx->u4SeqIdx];
	prCmdBlk = &(prctx->rCmdBlk);
	...
	ucBlkSeqNum = ((prCmdBlkInfo->ucBlkSeqNum) & BITS(4, 7)) >> 4;

	DBGLOG(RLM, INFO, "Seq[%u/%u] CalType=%u, CalParam=%u, BlkNum=%u\n",
		prctx->u4SeqIdx, prctx->u4SeqTotal,
		eCalType, u4CalParam, ucBlkSeqNum);

	/* Get the total number of frags from 1st frag */
	prCmdBlkInfo->ucCalType = eCalType;
	prCmdBlkInfo->u4CalParam = u4CalParam;
	prCmdBlkInfo->ucBlkSeqNum = (ucBlkSeqNum << 4) + 0;
	prCmdBlkInfo->ucBlkFragSeqNum = 0;
	prCmdBlkInfo->u4BlkLen =
		(ONE_TIME_CAL_DATA_BLK_LEN << 16) + ONE_TIME_CAL_DATA_BLK_LEN;
	ucFragSeqNum = ((prCmdBlkInfo->ucBlkFragSeqNum) & BITS(4, 7)) >> 4;

	/* Fill context for this cal type */
	prctx->eCalType = eCalType;
	prctx->u4CalParam = u4CalParam;
	prctx->ucFragSeqNum = ucFragSeqNum;
	prctx->ucBlkCtr = 0;
	prctx->ucFragCtr = 0;

	/* Start sending first fragment for this cal type */
...
)
{
	...
	ucBlkSeqNum = (prNewBlkInfo->ucBlkSeqNum & BITS(4, 7)) >> 4;
	...
	DBGLOG(RLM, INFO, "FileBlkDataOffset=%u ucBlkSeqNum=%d\n",
		u2FileBlkCalDataOffset, ucBlkSeqNum);
	...
	case ONE_TIME_CAL_TYPE_GROUP:
		...
		if (fgSet && ucFragSeqNumCur == 0)
			...
			u4CalParam = prFileBlkInfo->u4CalParam;
			prNewBlkInfo->u4CalParam = prFileBlkInfo->u4CalParam;
		}
...
uint32_t rlmOneTimeCalCheckInValidData(
	IN struct ADAPTER *prAdapter,
	IN enum ONE_TIME_CAL_TYPE eCalType,
	IN enum ONE_TIME_CAL_BAND eBand,
	IN uint8_t ucChannel,
	OUT struct ONE_TIME_CAL_SEND_CTX *prctx
)
{
	uint8_t ucidx = 0, ucCnt = 0, ucGroup = 0, ucChIdx = 0;
	uint32_t u4FileBlkSize = 0;
	struct ONE_TIME_CAL_FILE *prOneTimeCalFile;
	void *prFileBlk;
	uint8_t *prFileBlkInfo;
	struct ONE_TIME_CAL_BLK_INFO *prtmpFileBlkInfo;
	...
	switch (eCalType) {
	case ONE_TIME_CAL_TYPE_GROUP:
		ucCnt = ONE_TIME_CAL_GRP_BLK_NUM;
		prctx->au4CalParam[1] = ucGroup;
		prFileBlk = &(prOneTimeCalFile
				->rGrpCalData[ucGroup][0]);
		prFileBlkInfo =
			(uint8_t *)
			(&((struct ONE_TIME_CAL_FILE_BLK_GRP *)prFileBlk)
				->rOneTimeCalBlkInfo);
		u4FileBlkSize =
			sizeof(struct ONE_TIME_CAL_FILE_BLK_GRP);
		break;
	case ONE_TIME_CAL_TYPE_CHANNEL:
		ucCnt = ONE_TIME_CAL_DPD_BLK_NUM;
		prctx->au4CalParam[2] =
			(eBand << 12) + ucChannel;
		prFileBlk = &(prOneTimeCalFile
				->rDpdCalData[ucChIdx][0]);
		prFileBlkInfo =
			(uint8_t *)
			(&(((struct ONE_TIME_CAL_FILE_BLK_CH *)prFileBlk)
				->rOneTimeCalBlkInfo));
		u4FileBlkSize =
			sizeof(struct ONE_TIME_CAL_FILE_BLK_CH);
		break;
	default:
		DBGLOG(RLM, INFO,
			"Not support eCalType%d for valid cnt!", eCalType);
		return WLAN_STATUS_FAILURE;
	}
	...
	for (ucidx = 0; ucidx < ucCnt; ucidx++) {
		...
		if (prtmpFileBlkInfo->u4BlkLen == 0)
			break;
		prFileBlkInfo += u4FileBlkSize;
	}

	prctx->ucBlkSeqNum[eCalType] = ucidx;
	DBGLOG(RLM, INFO,
		"prctx->ucBlkSeqNum[%d] %d",
		eCalType, prctx->ucBlkSeqNum[eCalType]);
...
)
{
	...
	/* Allocate and fill sequence context */
	prctx = kalMemAlloc(sizeof(struct ONE_TIME_CAL_SEND_CTX), VIR_MEM_TYPE);
	if (!prctx) {
		DBGLOG(RLM, ERROR, "Alloc ctx mem fail (seq)\n");
		rStatus = WLAN_STATUS_RESOURCES;
		goto exit;
	}
	kalMemZero(prctx, sizeof(struct ONE_TIME_CAL_SEND_CTX));
	prctx->prAdapter = prAdapter;
	prctx->u4SeqIdx = 0;
	prctx->aeCalType[0] = ONE_TIME_CAL_TYPE_COMMON;
	prctx->au4CalParam[0] = eBand;
	prctx->ucBlkSeqNum[0] = 1;
	prctx->aeCalType[1] = ONE_TIME_CAL_TYPE_GROUP;
	prctx->aeCalType[2] = ONE_TIME_CAL_TYPE_CHANNEL;
	prctx->u4SeqTotal = 1;
	if (rlmOneTimeCalCheckInValidData(
		prAdapter, ONE_TIME_CAL_TYPE_GROUP,
		eBand, ucChannel, prctx) != WLAN_STATUS_SUCCESS) {
		DBGLOG(RLM, ERROR, "prctx grp allocate fail!\n");
		rStatus = WLAN_STATUS_INVALID_DATA;
		goto exit;
	}
	prctx->u4SeqTotal++;
	...
	if (rlmOneTimeCalCheckInValidData(
		prAdapter, ONE_TIME_CAL_TYPE_CHANNEL,
		eBand, ucChannel, prctx) != WLAN_STATUS_SUCCESS) {
		DBGLOG(RLM, ERROR, "prctx Chn allocate fail!\n");
		rStatus = WLAN_STATUS_INVALID_DATA;
		goto exit;
	}
	...
	if (prctx->u4SeqTotal > ONE_TIME_CAL_SEQ_MAX) {
		DBGLOG(RLM, ERROR, "prctx Seq allocate fail!\n");
	...
	OUT enum ENUM_CHNL_EXT *peSco)
{
	...
	rlmModifyHE6GBwPara(rlmGetBssOpBwByChannelWidth(*peSco,
			*pucChannelWidth),
			ucChannelNum,
			pucCenterFreqS1,
			pucCenterFreqS2,
...
#if (CFG_SUPPORT_STA_CSA == 1)
	struct IE_EX_CHANNEL_SWITCH *prExCSAIE;
#endif /* CFG_SUPPORT_STA_CSA */
#else
...
#if (CFG_SUPPORT_802_11AX == 1)
	fgChangeBssColor = updateHeBssColor(prBssInfo, prSwRfb, ucBssColorInfo);
			...
			uint8_t *pucIE, uint16_t u2IELength)
{
	...
	IE_FOR_EACH(pucIE, u2IELength, u2Offset)
	{
		...
		case ELEM_ID_VHT_CAP:
			...
			if ((prStaRec->u2HtCapInfo & HT_CAP_INFO_SUP_CHNL_WIDTH)
				!= 0) {
				prStaRec->ucVhtOpMode |=
				rlmGetOpModeBwByVhtAndHtOpInfo(prBssInfo);
			}
...
#endif
	DBGLOG(AAA, TRACE, "VhtOpMode from Assoc Req %1u\n",
		prStaRec->ucVhtOpMode);
#endif
}
		   ...
		   struct BSS_INFO *prBssInfo, uint8_t *pOutBuf)
{
	...
	if (IS_FEATURE_ENABLED(prAdapter->rWifiVar.ucRxStbc)) {
		uint8_t tempRxStbcNss = 0;

		tempRxStbcNss = prAdapter->rWifiVar.ucRxStbcNss;
		tempRxStbcNss =
			(tempRxStbcNss > prBssInfo->ucOpRxNss)
				? prBssInfo->ucOpRxNss : (tempRxStbcNss);
		if (tempRxStbcNss != prAdapter->rWifiVar.ucRxStbcNss) {
			DBGLOG(RLM, WARN, "Apply Nss:%d as RxStbcNss in HT Cap",
				wlanGetSupportNss(prAdapter,
				prBssInfo->ucBssIndex));
...
{
	struct IE_VHT_CAP *prVhtCap = NULL;
	struct VHT_SUPPORTED_MCS_FIELD *prVhtSupportedMcsSet = NULL;
	uint8_t i = 0;
	uint8_t ucMaxBw = 0;
	...
	if (IS_FEATURE_ENABLED(prAdapter->rWifiVar.ucRxStbc)) {
		...
		if (prAdapter->rWifiVar.u4SwTestMode ==
			...
			tempRxStbcNss = (prAdapter->rWifiVar.ucRxStbcNss
				> prBssInfo->ucOpRxNss) ?
				prBssInfo->ucOpRxNss :
				prAdapter->rWifiVar.ucRxStbcNss;
		}
		prVhtCap->u4VhtCapInfo |=
			((tempRxStbcNss << VHT_CAP_INFO_RX_STBC_OFFSET) &
			 VHT_CAP_INFO_RX_STBC_MASK);
...
void rlmResetCSAParams(struct BSS_INFO *prBssInfo, uint8_t fgClearAll)
{
	...
	DBGLOG(RLM, INFO, "Reset CSA count to %u CsaMode %d for BSS%d",
	    prCSAParams->ucCsaCount, prCSAParams->ucCsaMode,
		prBssInfo->ucBssIndex);
}
#else /* CFG_SUPPORT_STA_CSA */
			   ...
			   uintptr_t ulParamPtr)
{
	...
	eChannelWidth = (enum ENUM_CHANNEL_WIDTH)prBssInfo->ucVhtChannelWidth;
	...
	if (IS_BSS_AIS(prBssInfo) &&
		...
		aisReqJoinChPrivilegeForCSA(prAdapter, prAisFsmInfo, prBssInfo);
	}

	rlmResetCSAParams(prBssInfo, FALSE);
	/* Enable Tx Queue (kernel -> driver)*/
	kalIndicateAllQueueTxAllowed(
				prAdapter->prGlueInfo,
				prStaRec->ucBssIndex,
				TRUE);
	/* Enable Tx Queue (driver -> HIF)*/
	qmSetStaRecTxAllowed(prAdapter, prStaRec, TRUE);
#else /* CFG_SUPPORT_STA_CSA */
				   ...
				   u_int8_t fgIsSuccess)
{
	...
	do {
		...
		if (!prStaRec) {
			DBGLOG(RLM, WARN,
				"OP notification Tx done after disconnect!\n");
	...
	int8_t *pcMaxTxPwrPsd)
{
	...
	if (rlmTxPwrEnvGetBwShift(eChannelWidth, eSco, &ucBwShift)
		!= WLAN_STATUS_SUCCESS) {
		DBGLOG(RLM, ERROR,
			"Get PSD BW shift fail,ChBw[%u]Sco[%u]\n",
			eChannelWidth,
			eSco);
		return WLAN_STATUS_FAILURE;
	}
	...
	if (ucTxPwrEnvCnt == 0) {
		*pcMaxTxPwrPsd = prTxPwrEnvIE->acMaxTxPwr[0];
	} else {
		 ...
		 *      |      4        |             8           |
		 */
		ucNumMaxTxPwr = (uint8_t)(1 << (ucTxPwrEnvCnt - 1));

		/* Coverity check for TxPwrEnv count */
		if (ucNumMaxTxPwr > TX_PWR_ENV_INFO_TXPWR_COUNT_MAX ||
			ucPriChIdx >= ucNumMaxTxPwr) {
			DBGLOG(RLM, ERROR,
				"Get max TxPwr PSD idx fail,MaxNum[%d]Idx[%d]PriCh[%u]CenCh[%u]Cnt[%u]\n",
				ucNumMaxTxPwr,
				ucPriChIdx,
				ucPriCh,
				ucCenterCh,
		...
		*pcMaxTxPwrPsd = prTxPwrEnvIE->acMaxTxPwr[ucPriChIdx];
	}

	DBGLOG(RLM, TRACE,
		"[%d][RLM]TPE PSD,BW[%u]Sco[%u]BwShif[%u]PriCh[%u]CenCh[%u]Idx[%u]Cnt[%u]PSD[%d]\n",
		__LINE__,
		eChannelWidth,
		eSco,
		ucBwShift,
...
void rlmTxPwrEnvMaxPwrInit(int8_t *pcMaxTxPwr, uint8_t ucBufSize)
{
	enum TX_PWR_ENV_MAX_TXPWR_BW_TYPE eBwType = TX_PWR_ENV_MAX_TXPWR_BW_NUM;

	for (eBwType = TX_PWR_ENV_MAX_TXPWR_BW20;
		eBwType < ucBufSize; eBwType++) {
		pcMaxTxPwr[eBwType] = MAX_TX_POWER;
	}
}
...
{
	enum TX_PWR_ENV_MAX_TXPWR_BW_TYPE eBwType = TX_PWR_ENV_MAX_TXPWR_BW_NUM;
	int8_t cMaxTxPwrPsd = 0;
	uint8_t ucTxPwrDelta = 0;
	uint32_t u4Status = WLAN_STATUS_SUCCESS;

	if ((!prTxPwrEnvIE) || (!pcMaxTxPwr))
		return WLAN_STATUS_INVALID_DATA;
	 ...
	 *    - And will convert power LSB = 0.5dBm
	 */
	if (u4Status == WLAN_STATUS_SUCCESS) {
		for (eBwType = TX_PWR_ENV_MAX_TXPWR_BW20;
			eBwType < ucBufSize; eBwType++) {
			...
			pcMaxTxPwr[eBwType] =
				(int8_t)(cMaxTxPwrPsd + ucTxPwrDelta);
		}
	}
	return u4Status;
}
	...
	struct IE_TX_PWR_ENV_FRAME *prTxPwrEnvIE)
{
	uint32_t u4Status = WLAN_STATUS_SUCCESS;
	enum TX_PWR_ENV_MAX_TXPWR_BW_TYPE eBwType = TX_PWR_ENV_MAX_TXPWR_BW_NUM;
	int8_t acTxPwrEnvMaxTxPwr[TX_PWR_ENV_MAX_TXPWR_BW_NUM] = {0};
	uint8_t ucTxPwrEnvIntrpt = 0;
	uint8_t ucPwrLmtNum = 0;

	if ((!prAdapter) || (!prBssDesc) || (!prTxPwrEnvIE)) {
		DBGLOG(RLM, ERROR, "[%d][RLM] Invalid parameter\n", __LINE__);
		return WLAN_STATUS_INVALID_DATA;
	}

	rlmTxPwrEnvMaxPwrInit(acTxPwrEnvMaxTxPwr, TX_PWR_ENV_MAX_TXPWR_BW_NUM);
	...
	if (ucTxPwrEnvIntrpt == TX_PWR_ENV_LOCAL_EIRP
		...
		for (eBwType = TX_PWR_ENV_MAX_TXPWR_BW20;
			...
			acTxPwrEnvMaxTxPwr[eBwType]
				= prTxPwrEnvIE->acMaxTxPwr[eBwType];
		}
		 ...
		 * and store in the acTxPwrEnvMaxTxPwr
		 */
	...
	if (u4Status == WLAN_STATUS_SUCCESS) {
		for (eBwType = TX_PWR_ENV_MAX_TXPWR_BW20;
			eBwType < ucPwrLmtNum; eBwType++) {
			DBGLOG(RLM, TRACE,
				"Parse TPE,Band[%u]Itpt[%u]BW[%u]Lmt[%d]\n",
				eHwBand,
				ucTxPwrEnvIntrpt,
				eBwType,
				acTxPwrEnvMaxTxPwr[eBwType]);
	...
	uint8_t *pfgIsChange)
{
	...
	/* Coverity check for null pointer */
	if (!prAdapter || !pcTarget || !pcCompare || !pfgIsChange)
		return;

	/* Align IEEE spec, to avoid AP send power limit info too low */
	cMinPwrLmt = prAdapter->rWifiVar.cTxPwrEnvLmtMin;

	/* Coverity check for quantity to update */
	...
	for (eBwType = TX_PWR_ENV_MAX_TXPWR_BW20; eBwType < ucNum; eBwType++) {
		if (pcTarget[eBwType] != pcCompare[eBwType]) {
			pcTarget[eBwType] = pcCompare[eBwType];
			*pfgIsChange = TRUE;
		}

		/* Coverity check TxPower boundary */
		if (pcTarget[eBwType] > MAX_TX_POWER) {
			DBGLOG(RLM, TRACE,
			"TPE PwrLmt too big, use default[%d]Lmt[%d]BW[%u]\n",
			MAX_TX_POWER,
			pcTarget[eBwType],
			eBwType);

			pcTarget[eBwType] = MAX_TX_POWER;
		} else if (pcTarget[eBwType] < cMinPwrLmt) {
			DBGLOG(RLM, TRACE,
			"TPE PwrLmt too low, use default[%d]Lmt[%d]BW[%u]\n",
			cMinPwrLmt,
			pcTarget[eBwType],
			eBwType);

			pcTarget[eBwType] = cMinPwrLmt;
		}

		DBGLOG(RLM, TRACE,
			"Final TPE BW[%u]Lmt[%d]Change[%d]\n",
			eBwType,
			pcTarget[eBwType],
			*pfgIsChange);
	}

...
struct MSDU_INFO *cnmPktAlloc(struct ADAPTER *prAdapter, uint32_t u4Length)
{
	...
	if (u4Length) {
		...
		prMsduInfo->u4PacketBufferSize = u4Length;
	}
...
void cnmPktFree(struct ADAPTER *prAdapter, struct MSDU_INFO *prMsduInfo)
{
	...
	if (prMsduInfo->prPacket) {
		...
		prMsduInfo->u4PacketBufferSize = 0;
	}
...
void cnmMemFree(IN struct ADAPTER *prAdapter, IN void *pvMemory)
{
	...
	if (!prAdapter || !pvMemory) {
		log_dbg(MEM, WARN, "prAdapter or pvMemory is NULL!\n");
		return;
	}
	...
	enum ENUM_STA_TYPE eStaType, uint8_t ucBssIndex, uint8_t *pucMacAddr)
{
	...
	if (eStaType == STA_TYPE_NAN)
		ucBssIndex = nanGetNdlCurrentBssIdxByNDI(prAdapter, pucMacAddr);
#endif
...
#if CFG_SUPPORT_NAN
		if (eStaType == STA_TYPE_NAN) {
			nanDevGetNdiAddress(prAdapter,
				NAN_BSS_INDEX_MAIN,
				prStaRec->aucLocalNdiAddr);
			prStaRec->ucNdiIndex = (u_int8_t)NAN_BSS_INDEX_MAIN;
		}
#endif
	...
	uint8_t ucStaRecIndexExcluded)
{
	...
	for (i = 0; i < CFG_STA_REC_NUM; i++) {
		...
		/* The StaRec allocated by RTT should only be
		 * freed by RTT handler. We need to prevent RTT
		 * StaRec being freed in AIS's FSM.
		 */
		if (IS_STA_RTT_TYPE(prStaRec) && prStaRec->fgIsInUse) {
			log_dbg(CNM, INFO,
...
#if (CFG_SUPPORT_4ADDR_STATION == 1)
	if (prStaRec->ucBssIndex < KAL_AIS_NUM)
		prAisBssInfo = prAdapter->prAisBssInfo[prStaRec->ucBssIndex];
	else
		prAisBssInfo = NULL;
#endif /* end CFG_SUPPORT_4ADDR_STATION */
...
#if (CFG_SUPPORT_4ADDR_STATION == 1)
	prCmdContent->ucWdsEnable = FALSE;
#endif /* end CFG_SUPPORT_4ADDR_STATION */

	prCmdContent->ucStaIndex = prStaRec->ucIndex;
#if (CFG_SUPPORT_NAN == 1)
	prCmdContent->ucNdiIndex = prStaRec->ucNdiIndex;
#endif
...
#if (CFG_SUPPORT_4ADDR_STATION == 1)
	if (prAisBssInfo)
		prCmdContent->ucWdsEnable = prAisBssInfo->ucWdsEnable;

	log_dbg(REQ, TRACE, "Update StaRec[%u] WIDX[%u] State[%u] Type[%u] BssIdx[%u] AID[%u] 4addr[%d]\n",

		...
		IN uint8_t ucRoleIdx)
{
	...
	do {
		...
		atomic_set(&prP2pBssInfo->fgIsApStarting, FALSE);
		...
		IN struct MSG_HDR *prMsgHdr)
{
	...
	p2pFuncNotifyDataPathStatus(prAdapter,
		prP2pRoleFsmInfo->ucBssIndex,
		TRUE,
		FALSE,
...
#if (CFG_SUPPORT_DFS_OFFLOAD == 1)
	if (!p2pFuncGetRadarDetectMode()) {
		ucNewChannel = p2pFuncSelect5GNonDFSChannel(prAdapter,
				prP2pBssInfo->eBssSCO,
				rlmGetBssOpBwByVhtAndHtOpInfo(prP2pBssInfo));
		if (ucNewChannel == 0) {
			DBGLOG(P2P, WARN, "chan invalid, use chan 36\n");
			ucNewChannel = AP_DEFAULT_CHANNEL_5G;
		}
		if (IS_NET_PWR_STATE_ACTIVE(prAdapter,
			prP2pBssInfo->ucBssIndex)) {
			p2pFuncSwitchChannel(prAdapter,
				ucNewChannel, prP2pRoleFsmInfo->ucRoleIndex);
			DBGLOG(P2P, STATE, "Radar detected, start CSA\n");
		} else {
			p2pFuncUpdateChannel(prAdapter,
				ucNewChannel, prP2pRoleFsmInfo->ucRoleIndex);
			p2pFuncReadyStartAp(prAdapter, prP2pRoleFsmInfo);
			DBGLOG(P2P, STATE, "Radar detected, restart AP\n");
		}
	}
#endif /* CFG_SUPPORT_DFS_OFFLOAD */
		...
		struct MSG_HDR *prMsgHdr)
{
	...
	if (p2pFuncIsManualCac() && p2pFuncGetDriverCacTime() <= 2)
		bSkipCac = TRUE;

	if (prAdapter->prAisBssInfo[AIS_DEFAULT_INDEX]->eConnectionState
		== MEDIA_STATE_CONNECTED
		&& prAdapter->prAisBssInfo[AIS_DEFAULT_INDEX]->ucPrimaryChannel
		== ucChannelNum
		&& prAdapter->prAisBssInfo[AIS_DEFAULT_INDEX]->eBand == eBand)
		bSkipCac = TRUE;
	...
	if (chan && !(chan->flags & IEEE80211_CHAN_RADAR)) {
		...
		p2pFuncPreStartRdd(prAdapter,
			prP2pStartAPMsg->ucRoleIdx, &chandef,
		...
		IN struct MSG_HDR *prMsgHdr)
{
	...
	if (prP2pScanReqMsg->eScanReason == SCAN_REASON_ACS)
		prScanReqInfo->u2PassiveDwellTime =
			ACS_DEFAULT_DWELL_TIME_MS;
		...
		IN struct P2P_ROLE_FSM_INFO *prP2pRoleFsmInfo)
{
	...
	if (ucTokenID == prChnlReqInfo->ucSeqNumOfChReq) {
		...
		case P2P_ROLE_STATE_DFS_CAC:
			...
			if (p2pFuncCheckWeatherRadarBand(
					prAdapter, prChnlReqInfo))
				u4CacTimeMs =
		...
		IN struct MSG_MGMT_TX_REQUEST *prMgmtTxMsg)
{
	...
	if (p2pRoleAddTxReq2Queue(prAdapter, prP2pMgmtTxReqInfo,
		...
		DBGLOG(P2P, WARN, "Add Tx Req to Queue failed.\n");
		goto error;
	}
	...
	for (ucIdx = 0; ucIdx < MAX_BSS_INDEX; ucIdx++) {
		...
		if (!IS_BSS_P2P(prBssInfo)) {
			...
			if (eOPMode == OP_MODE_ACCESS_POINT) {
				...
				kalP2pNotifyApGoStoppedByIdx(prAdapter,
					ucIdx);
			}
...
#if (CFG_NAN_P2P_CONCURRENT_SUPPORT == 1)
void p2pRoleFsmRunEventNanDPUpdate(
	struct ADAPTER *prAdapter,
	struct MSG_HDR *prMsgHdr)
{
	struct BSS_INFO *prP2pBssInfo = (struct BSS_INFO *) NULL;
	struct P2P_ROLE_FSM_INFO *prP2pRoleFsmInfo =
		(struct P2P_ROLE_FSM_INFO *) NULL;
	struct STA_RECORD *prCurrStaRec = NULL;
	struct LINK *prClientList;
	uint8_t ucRoleIdx = 0;
	struct STA_RECORD *prStaRec = NULL;

	DBGLOG(P2P, INFO, "Received NAN DP Update Msg\n");

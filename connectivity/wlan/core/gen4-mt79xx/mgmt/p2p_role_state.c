		...
		IN struct BSS_INFO *prP2pBssInfo)
{
	...
	if (atomic_read(&prP2pBssInfo->fgIsApStarting) == TRUE) {
		...
		if (QUEUE_IS_NOT_EMPTY(&prAdapter->rStartApPendingMsgList)) {
			...
			atomic_set(&prP2pBssInfoOther->fgIsApStarting, TRUE);
		...
		IN struct P2P_CONNECTION_REQ_INFO *prConnReqInfo)
{
	...
	do {
		...
		if ((cnmPreferredChannel(prAdapter,
			...
			prScanReqInfo->u2PassiveDwellTime = 50;
		} else {
		...
		OUT struct P2P_CHNL_REQ_INFO *prChnlReqInfo)
{
	...
	do {
		...
		u4CacTimeMs = prAdapter->prGlueInfo
			->prP2PInfo[prP2pRoleFsmInfo->ucRoleIndex]->cac_time_ms;

...
#undef __MSG_ID__
		...
		case SCAN_STATE_SCANNING:
			if (scnDoAllowScanCheck(prAdapter)) {
				...
				GET_CURRENT_SYSTIME(
					&prScanInfo->u4ScanStartTime);
			} else {
				/* If other task is ongoing,
				 * directly report scan done.
				 */
...
void scnSendScanReqV2(IN struct ADAPTER *prAdapter)
{
	...
	else if (IS_FEATURE_ENABLED(prAdapter->rWifiVar.fgEnDbdcScan))
		prCmdScanReq->u4ScnFuncMaskExtend |= ENUM_SCN_ENABLE_DBDC_SCAN;
	...
	for (i = 0; i < prCmdScanReq->ucSSIDNum; i++) {
		...
		log_dbg(SCN, TRACE,
			"Ssid=%s, SsidLen=%d\n",
			SSID2STR(prCmdScanReq->arSSID[i].aucSsid,
			prCmdScanReq->arSSID[i].u4SsidLen),
			prCmdScanReq->arSSID[i].u4SsidLen);
	}
	for (i = 0; i < prCmdScanReq->ucSSIDExtNum; i++) {
		...
		log_dbg(SCN, TRACE,
			"Ssid=%s, SsidLen=%d\n",
			SSID2STR(prCmdScanReq->arSSIDExtend[i].aucSsid,
			prCmdScanReq->arSSIDExtend[i].u4SsidLen),
			prCmdScanReq->arSSIDExtend[i].u4SsidLen);
	}
...
void scnFsmMsgStart(IN struct ADAPTER *prAdapter, IN struct MSG_HDR *prMsgHdr)
{
	...
	if (prScanInfo->eCurrentState == SCAN_STATE_IDLE) {
		...
		/*send vender event scan start*/
		mtk_cfg80211_vendor_event_scan_status(prAdapter,
		SCN_VENDOR_COMMAND_SCAN_START);
#endif
...
#if CFG_SUPPORT_MTK_SCAN_EVENT
			/*send vender event scan done*/
			mtk_cfg80211_vendor_event_scan_status(prAdapter,
			SCN_VENDOR_COMMAND_SCAN_DONE);
#endif
...
#if (CFG_SUPPORT_WIFI_6G == 1) && (CFG_SUPPORT_6G_PASSIVE_SCAN == 1)
	/* 6G channel will judge whether do passive scan by REG & FILS */
	prScanParam->u4ScnFuncMaskExtend |= ENUM_SCN_NOT_FORCE_6G_ACTIVE_PROBE;
#endif
...
#define __MSG_ID__ prPendingMsgHdr->eMsgId
		if (__MSG_ID__ == MID_AIS_SCN_SCAN_REQ
			...
			if (ucSeqNum == prScanReqMsg->ucSeqNum
				...
				if (__MSG_ID__ != MID_RLM_SCN_SCAN_REQ)
			...
			if (ucSeqNum == prScanReqMsgV2->ucSeqNum
				...
				prRemoveMsgHdr = prPendingMsgHdr;
				if (__MSG_ID__ != MID_RLM_SCN_SCAN_REQ_V2)
					fgIsRemovingScan = TRUE;
			}
		}
#undef __MSG_ID__
		...
		if (prRemoveLinkEntry) {
			if (fgIsRemovingScan == TRUE) {
				/* generate scan-done event for caller */
				scnFsmGenerateScanDoneMsgWithMsgId(prAdapter,
					ucSeqNum,
					ucBssIndex,
					SCAN_STATUS_CANCELLED,
...
u_int8_t scnDoAllowScanCheck(struct ADAPTER *prAdapter)
{
	...
	if (rttIsRunning(prAdapter)) {
		DBGLOG(SCN, INFO, "Abort Scan due to RTT blocking\n");
		return FALSE;
	}
...
/*----------------------------------------------------------------------------*/
/*!
 * \brief            Copy RNR parameter to scan parameter.
 *
 * \param prScanParam       scan parameter
 * \param prNeighborParam   neighbor AP parameter
 *
 * \return none
 */
/*----------------------------------------------------------------------------*/
	...
	struct NEIGHBOR_AP_PARAM *prNeighborParam)
{
	...
	prScanParam->u4ScnFuncMaskExtend =
		prNeighborParam->u4ScnFuncMaskExtend;
	...
	/* reset unused parameters to init value */
	...
	IN struct EVENT_SCAN_DONE *prScanDone, u_int8_t fgIsNewVersion)
{
	...
	if (prScanDone->ucSparseChannelValid) {
		...
		char *strbuf = (char *)kalMemAlloc(
			SCN_SCAN_DONE_PRINT_BUFFER_LENGTH, VIR_MEM_TYPE);
		...
		if (!prScanInfo->ucScanCount)
			prScanInfo->
			ucSparseChannelArrayValidNum[SCAN_COUNT - 1] = 0;

		num = prScanInfo->
			ucSparseChannelArrayValidNum[prScanInfo->ucScanCount]
		...
		if (num > SCAN_DONE_EVENT_MAX_CHANNEL_NUM) {
			num = SCAN_DONE_EVENT_MAX_CHANNEL_NUM;
		}
...
#define print_info_ch(_Mod, _Clz, _Fmt, var) \
		do { \
			...
			for (u4ChCnt = 0; u4ChCnt < num; u4ChCnt++) { \
				...
				prScanInfo-> \
				rScanRadioInfo[prScanInfo-> \
				ucScanCount][u4ChCnt]. \
				aeChannelBand = eBand; \
				prScanInfo-> \
				rScanRadioInfo[prScanInfo-> \
				ucScanCount][u4ChCnt].var \
				= prScanDone->var[u4ChCnt]; \
...
#define print_info(_Mod, _Clz, _Fmt, var) \
		...
		print_info(SCN, INFO, "ScanTime : %s\n", u2ChannelScanTime);
...
#define print_info_ch(_Mod, _Clz, _Fmt, var) \
		do { \
			...
			int totalLen = SCN_SCAN_DONE_PRINT_BUFFER_LENGTH; \
			enum ENUM_BAND eBand = BAND_NULL; \
			for (u4ChCnt = 0; u4ChCnt < num && \
				written < totalLen; u4ChCnt++) { \
				eBand = \
				SCN_GET_EBAND_BY_CH_NUM( \
				prScanDone->var[u4ChCnt]); \
...
#undef	print_scan_info
	...
	if (prScanInfo->eCurrentState == SCAN_STATE_SCANNING
		...
		while (!LINK_IS_EMPTY(&prScanInfo->rNeighborAPInfoList)) {
			...
			DBGLOG(SCN, LOUD, "Skip NeighborAPInfo\n");
			u4BssidCnt = 0;
			cnmMemFree(prAdapter, prNeighborAPInfo);
		}
#endif
...
void
scnFsmGenerateScanDoneMsgWithMsgId(IN struct ADAPTER *prAdapter,
	IN uint8_t ucSeqNum, IN uint8_t ucBssIndex,
	IN enum ENUM_SCAN_STATUS eScanStatus, enum ENUM_MSG_ID eMsgId)
{
	struct MSG_SCN_SCAN_DONE *prScanDoneMsg;

	log_dbg(SCN, EVENT, "Gen ScanDone Num[%d]-ID[%d]-Idx[%d]\n",
		ucSeqNum, eMsgId, ucBssIndex);

	prScanDoneMsg = (struct MSG_SCN_SCAN_DONE *) cnmMemAlloc(prAdapter,
		RAM_TYPE_MSG, sizeof(struct MSG_SCN_SCAN_DONE));
	if (!prScanDoneMsg) {
		log_dbg(SCN, ERROR, "Alloc ScanDone Msg Failed\n");
		return;
	}
...
}	/* end of scnFsmGenerateScanDoneMsgWithMsgId() */
	...
	IN struct PARAM_SCHED_SCAN_REQUEST *prRequest)
{
	...
	for (i = 0; i < prSchedScanCmd->ucSsidNum; i++) {
		...
		log_dbg(SCN, TRACE, "ssid set(%d) %s\n", i,
			SSID2STR(prSsid[i].aucSsid, prSsid[i].u4SsidLen));
	}
	...
	for (i = 0; i < prSchedScanCmd->ucMatchSsidNum; i++) {
		...
		log_dbg(SCN, TRACE, "Match set(%d) %s, rssi>%d\n",
				i, SSID2STR(prMatchSets[i].aucSsid,
				prMatchSets[i].ucSsidLen),
	...
	if (prAdapter->rWifiVar.eDbdcMode == ENUM_DBDC_MODE_DISABLED)
		prSchedScanCmd->ucScnFuncMask |= ENUM_SCN_DBDC_SCAN_DIS;
	else if (IS_FEATURE_ENABLED(prAdapter->rWifiVar.fgEnDbdcScan))
		prSchedScanCmd->u4ScnFuncMaskExtend |=
			ENUM_SCN_ENABLE_DBDC_SCAN;

	scnSetSchedScanPlan(prAdapter, prSchedScanCmd);

	log_dbg(SCN, INFO, "V(%u)seq(%u)sz(%zu)chT(%u)chN(%u)ssid(%u)match(%u)IE(%u=>%u)MSP(%u)Func(0x%X,0x%X)\n",

		       ...
		       OUT struct RSN_INFO *prRsnInfo)
{
	...
	if (IE_ID(pucInfoElem) == ELEM_ID_VENDOR &&
		...
		DBGLOG(RSN, WARN, "Unsupported RSN IE\n");
		DBGLOG_MEM8(RSN, WARN,
	...
	do {
		...
		*/
		...
		if (u2RemainRsnIeLen < u2DesiredPmkidCnt * RSN_PMKID_LEN) {
			DBGLOG(RSN, TRACE,
		...
		IN uint8_t ucBssIndex)
{
	...
	uint8_t fgTryRsn = TRUE;
	uint8_t fgTryRsno2 = FALSE, fgTryRsno = FALSE;
	uint8_t fgIsApRsnOveride = FALSE;
	...
	DBGLOG(RSN, TRACE, "BSS["MACSTR
		"] AuthMode [%d], EncStatus [%d], Flag[rsn=%d rsno=%d rsno2=%d aprsno=%d]",
		MAC2STR(prBss->aucBSSID), eAuthMode, eEncStatus,
		fgTryRsn, fgTryRsno, fgTryRsno2, fgIsApRsnOveride);
...
try_again:
	if (eAuthMode ==
		...
		if (0) {
			...
			DBGLOG(RSN, INFO,
			       "RSNE does not exist. try rsn=%d rsno=%d rsno2=%d\n",
			       fgTryRsn, fgTryRsno, fgTryRsno2);
			return FALSE;
		}
#if CFG_SUPPORT_PASSPOINT
	} else if (eAuthMode ==
		   AUTH_MODE_WPA_OSEN) {
		if (fgTryRsn && prBss->fgIERSN) {
			prBssRsnInfo = &prBss->rRSNInfo;
			fgTryRsn = FALSE;
		}
	...
	if (prBssRsnInfo != NULL && !rsnIsSuitableBSS(prAdapter, prBssRsnInfo,
		ucBssIndex)) {
	...
	else if (eAuthMode == AUTH_MODE_WPA3_FT_SAE &&
		rsnSearchAKMSuite(prAdapter,
			RSN_AKM_SUITE_FT_OVER_SAE, &j, ucBssIndex))
	...
	*/
	...
	if (prBss->eRsnSelectedRSNOverride !=
		RSN_OVERRIDE_NOT_USED) {
		aisGetAisSpecBssInfo(prAdapter, ucBssIndex)
		->eRsnSelectedRSNOverride =
		prBss->eRsnSelectedRSNOverride;
	}
#endif /* CFG_SUPPORT_RSNO */
		      ...
		      IN struct MSDU_INFO *prMsduInfo)
{
	...
	DBGLOG(RSN, INFO, "rsnGenerateWPAIE");
	...
	if (GET_BSS_INFO_BY_INDEX(prAdapter, ucBssIndex)->
	    ...
	    (eAuthMode ==
	     ...
	     eAuthMode ==
			AUTH_MODE_WPA_PSK)) {
#endif
		DBGLOG(RSN, INFO, "BssIndex:0x%8x\n", ucBssIndex);
		DBGLOG(RSN, INFO, "GroupKeyCipherSuite:0x%8x\n",
			prBssInfo->u4RsnSelectedGroupCipher);
		DBGLOG(RSN, INFO, "PairwiseCipher:0x%8x\n",
			prBssInfo->u4RsnSelectedPairwiseCipher);
		DBGLOG(RSN, INFO, "WpaIeBuffer:0x%8x\n, WpaIeLen:0x%8x\n",
			prP2pSpecificBssInfo->aucWpaIeBuffer,
			prP2pSpecificBssInfo->u2WpaIeLen);

		if (IS_BSS_P2P(prBssInfo) &&
			prAdapter->fgIsP2PRegistered && prP2pSpecificBssInfo
	...
	struct MSDU_INFO *prMsduInfo)
{
	...
	prConnSettings = aisGetConnSettings(prAdapter, ucBssIndex);
	if (prConnSettings->rRnsSelectionInfo.ucLength == 0)
		return;

	ucLength = prConnSettings->rRnsSelectionInfo.ucLength + 2;
	...
	kalMemCopy(pucBuffer, &(prConnSettings->rRnsSelectionInfo),
		ucLength);
	prMsduInfo->u2FrameLength += IE_SIZE(pucBuffer);

	DBGLOG(RSN, INFO, "Generate RSN Selection\n");
	DBGLOG_MEM8(RSN, INFO,
...
void rsnSaQueryRequest(IN struct ADAPTER *prAdapter, IN struct SW_RFB *prSwRfb)
{
	...
	struct AIS_SPECIFIC_BSS_INFO *prBssSpecInfo = NULL;
#endif
	...
	if ((prStaRec->eStaType == STA_TYPE_LEGACY_AP) &&
		(prBssSpecInfo->fgIgnorSaQueryReq)) {
		DBGLOG(RSN, INFO, "Ignor SA Query Request\n");
		return;
	}
#endif
	DBGLOG(RSN, INFO,
...
#if (CFG_SUPPORT_RSNO == 1)
		if (rsnSupportRSNOverride(prAdapter, ucBssIdx)) {
			prAisSpecificBssInfo =
			aisGetAisSpecBssInfo(prAdapter, ucBssIdx);
			if (prAisSpecificBssInfo->eRsnSelectedRSNOverride
				== RSN_OVERRIDE_RSNE_OVERRIDE
				&& prBssDesc->fgIERSNO) {
				DBGLOG(RSN, LOUD, "use RSNOInfo to check\n");
				return rsnCheckWpaRsnInfo(prBssInfo,
					&prBssDesc->rRSNOInfo);
			} else if (prAisSpecificBssInfo->eRsnSelectedRSNOverride
				== RSN_OVERRIDE_RSNE_OVERRIDE_2
				&& prBssDesc->fgIERSNO2) {
				DBGLOG(RSN, LOUD, "use RSNO2Info to check\n");
				return rsnCheckWpaRsnInfo(prBssInfo,

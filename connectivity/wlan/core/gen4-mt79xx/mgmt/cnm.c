...
#define DBDC_INFO (prAdapter->rDbdcInfo)
#else
#define DBDC_INFO (g_rDbdcInfo)
#endif

#define DBDC_SET_GUARD_TIME(_prAdapter, _u4TimeoutMs) { \
	cnmTimerStartTimer(_prAdapter, \
		&DBDC_INFO.rDbdcGuardTimer, \
		_u4TimeoutMs); \
	DBDC_INFO.eDdbcGuardTimerType = \
		ENUM_DBDC_GUARD_TIMER_SWITCH_GUARD_TIME; \
}
...
#define DBDC_SET_WMMBAND_FW_AUTO_BY_CHNL(_ucCh, _ucWmmQIdx, _eBand , _ucBId) \
	{ \
		DBDC_INFO.ucPrimaryChannel = (_ucCh);\
		DBDC_INFO.ucWmmQueIdx = (_ucWmmQIdx);\
		DBDC_INFO.eRfBand = (_eBand);\
		DBDC_INFO.ucBssIdx = (_ucBId);\
	}

#define DBDC_SET_WMMBAND_FW_AUTO_DEFAULT() \
	{ \
		DBDC_INFO.ucPrimaryChannel = 0; \
		DBDC_INFO.ucWmmQueIdx = 0;\
		DBDC_INFO.ucBssIdx = 0;\
		DBDC_INFO.eRfBand = BAND_NULL;\
	}

#define DBDC_UPDATE_CMD_WMMBAND_FW_AUTO(_prCmdBody) \
	{ \
		(_prCmdBody)->ucPrimaryChannel = DBDC_INFO.ucPrimaryChannel; \
		(_prCmdBody)->ucWmmQueIdx = DBDC_INFO.ucWmmQueIdx; \
		(_prCmdBody)->ucRfBand = DBDC_INFO.eRfBand; \
...
#define DBDC_FSM_EVENT_HANDLER(_prAdapter, _event) { \
	if ((uint32_t) DBDC_INFO.eDbdcFsmCurrState \
		>= ENUM_DBDC_FSM_STATE_NUM) { \
		log_dbg(CNM, WARN, \
	...
	arDdbcFsmActionTable[(uint32_t) DBDC_INFO.eDbdcFsmCurrState] \
			       ...
			       struct MSG_HDR *prMsgHdr)
{
	...
	if (cnmDBDCIsReqPeivilegeLock(prAdapter)) {
		LINK_INSERT_TAIL(&DBDC_INFO.rPendingMsgList,
				 &prMsgHdr->rLinkEntry);
		log_dbg(CNM, INFO,
			    ...
			    struct WIFI_EVENT *prEvent)
{
	...
	if (!prAdapter || !prEvent) {
		DBGLOG(CNM, ERROR, "prAdapter or prEvent is NULL\n");
		return;
	}
	...
	if (prEventBody->ucBssIndex >= ARRAY_SIZE(prAdapter->aprBssInfo)) {
		DBGLOG(CNM, ERROR, "BssIndex: %u>[%u]\n",
			prEventBody->ucBssIndex,
			ARRAY_SIZE(prAdapter->aprBssInfo));
		return;
	}

	if (prEventBody->ucStatus != EVENT_CH_STATUS_GRANT) {
		DBGLOG(CNM, ERROR, "ucStatus:%u\n",
	...
	IN uint8_t ch_num, IN uint8_t ucRoleIdx)
{
	...
	/* 6G is not support*/
	eBandCsa = (ch_num <= HW_CHNL_NUM_MAX_2G4) ? BAND_2G4 : BAND_5G;
...
uint32_t cnmSCCAutoSwitchMode(IN struct ADAPTER *prAdapter,
		...
		IN uint8_t ucAISChannel)
{
	...
	if (!rlmDomainCheckChannelValidForP2pCsa(prAdapter,
		eAISBand, ucAISChannel)) {
		DBGLOG(CNM, STATE,
			"No CSA Check channel[%d]-Band[%d]\n",
			ucAISChannel, eAISBand);
		       ...
		       uint8_t ucBssIndex)
{
	...
	if (IS_BSS_AIS(prBssInfo) && prBssDesc)
		ucChannelBw = rlmDomainGetChannelBw(prAdapter,
...
uint8_t cnmChannelWidthToCnmChBw(enum WIFI_CHANNEL_WIDTH eChannelWidth)
{
...
void cnmFreqToChnl(uint32_t u4Freq, uint8_t *ucChannel, enum ENUM_BAND *eBand)
{
	uint32_t u4ChannelNum;

	/* Initialize data */
	u4ChannelNum = nicFreq2ChannelNum(u4Freq * 1000);
	*ucChannel = 0;
	*eBand = BAND_NULL;

	if (u4ChannelNum > 255) {
		DBGLOG(CNM, ERROR,
			"Channel num %d out of range\n", u4ChannelNum);
	} else {
		*ucChannel = (uint8_t) u4ChannelNum;
	}
				      ...
				      u_int8_t fgIsP2pDevice)
{
	...
	if (eNetworkType == NETWORK_TYPE_P2P && fgIsP2pDevice) {
		...
		for (i = 0; i < MAX_KEY_NUM; i++) {
			...
			if (i < MAX_PGTK_KEY_NUM)
				prBssInfo->u4DualGTKKeyIndex[i] = 0xFF;
#endif
		}
...
#if CFG_SUPPORT_NAN
	/* Fix NAN OwnMacIdx to be same */
	if (eNetworkType == NETWORK_TYPE_NAN) {
		if (ucOwnMacIdx < prAdapter->ucHwBssIdNum)
			nanDevSetOwnMacIdx(prAdapter, &ucOwnMacIdx);
		else
			DBGLOG(CNM, ERROR,
				"No enough OM(%u) for NAN\n", ucOwnMacIdx);
	}
#endif
...
void cnmInitDbdcSetting(IN struct ADAPTER *prAdapter)
{
	...
	DBDC_SET_WMMBAND_FW_AUTO_DEFAULT();
	DBDC_INFO.fgHasSentCmd = FALSE;
	DBDC_INFO.fgPostpondEnterAG = FALSE;
	DBDC_INFO.fgPostpondLeaveAG = FALSE;
#ifdef MT7926
	DBDC_INFO.ucOpBwOrignal = MAX_BW_UNKNOWN;
#endif
	...
	case ENUM_DBDC_MODE_DYNAMIC:
		DBDC_INFO.eDbdcFsmCurrState =
			ENUM_DBDC_FSM_STATE_DISABLE_IDLE;
		DBDC_INFO.eDbdcFsmPrevState =
			ENUM_DBDC_FSM_STATE_DISABLE_IDLE;
		DBDC_INFO.eDbdcFsmNextState =
			ENUM_DBDC_FSM_STATE_DISABLE_IDLE;
		...
		DBDC_INFO.eDdbcGuardTimerType =
			ENUM_DBDC_GUARD_TIMER_NONE;
		DBDC_INFO.fgReqPrivelegeLock = FALSE;
		LINK_INITIALIZE(&DBDC_INFO.rPendingMsgList);
		DBDC_INFO.fgDbdcDisableOpmodeChangeDone = TRUE;

		for (ucBssLoopIndex = 0;
		     ucBssLoopIndex < prAdapter->ucHwBssIdNum;
		     ucBssLoopIndex++)
			DBDC_INFO.eBssOpModeState[ucBssLoopIndex] =
				ENUM_OPMODE_STATE_DONE;

		prAdapter->rWifiVar.fgDbDcModeEn = FALSE;
		cnmUpdateDbdcSetting(prAdapter, FALSE);
		break;
	...
	for (ucBssIndex = 0;
		...
		if (IS_BSS_ALIVE(prAdapter, prBssInfo)) {
			...
			case CNM_OPMODE_REQ_STATUS_DEFER:
				DBDC_INFO.fgDbdcDisableOpmodeChangeDone
					= FALSE;
				DBDC_INFO.eBssOpModeState[ucBssIndex]
					= ENUM_OPMODE_STATE_WAIT;
	...
	IN bool fgSuccess)
{
	...
	log_dbg(CNM, INFO, "[DBDC] OPMODE STATE [%u/%u/%u/%u]\n",
	       DBDC_INFO.eBssOpModeState[BSSID_0],
	       DBDC_INFO.eBssOpModeState[BSSID_1],
	       DBDC_INFO.eBssOpModeState[BSSID_2],
	       DBDC_INFO.eBssOpModeState[BSSID_3]);

	for (ucBssLoopIndex = 0;
	     ucBssLoopIndex < prAdapter->ucHwBssIdNum;
	     ucBssLoopIndex++) {

		if (DBDC_INFO.eBssOpModeState[ucBssLoopIndex] ==
		    ENUM_OPMODE_STATE_WAIT)
			return;
	...
	if (!DBDC_INFO.fgDbdcDisableOpmodeChangeDone) {
		DBDC_INFO.fgDbdcDisableOpmodeChangeDone = true;
			  ...
			  IN u_int8_t fgDbdcEn)
{
	...
	if (DBDC_INFO.fgHasSentCmd == TRUE)
		log_dbg(CNM, WARN, "Not event came back for DBDC\n");

	DBDC_INFO.fgHasSentCmd = TRUE;
	DBDC_INFO.fgCmdEn = fgDbdcEn;

	/* Set DBDC A+A Mode to FW */
	if (DBDC_INFO.fgIsDBDCAAMode == TRUE)
		prCmdBody->ucDBDCAAMode = 1;

	log_dbg(CNM, WARN, "fgDbdcEn=%d, ucDBDCAAMode=%d\n",
		DBDC_INFO.fgCmdEn, prCmdBody->ucDBDCAAMode);
	...
	IN enum ENUM_DBDC_FSM_EVENT_T   eEvent)
{
	...
	DBDC_INFO.eDbdcFsmPrevState = DBDC_INFO.eDbdcFsmCurrState;
	DBDC_INFO.eDbdcFsmCurrState = eNextState;
	DBDC_INFO.eDbdcFsmNextState = eNextState;
	...
	if ((uint32_t) DBDC_INFO.eDbdcFsmPrevState >=
		sizeof(arDdbcFsmActionTable) / sizeof(struct DBDC_FSM_T) ||
		(uint32_t) DBDC_INFO.eDbdcFsmCurrState >=
		sizeof(arDdbcFsmActionTable) / sizeof(struct DBDC_FSM_T)) {
		log_dbg(CNM, INFO, "Invalid state[%d]\n",
			DBDC_INFO.eDbdcFsmPrevState);
		return;
	}
	if (DBDC_INFO.eDbdcFsmPrevState != DBDC_INFO.eDbdcFsmCurrState) {
		/* state change, call exit function of previous state */
		if (arDdbcFsmActionTable[
			(uint32_t) DBDC_INFO.eDbdcFsmPrevState]
			.pfExitFunc) {
...
{
	return DBDC_INFO.fgReqPrivelegeLock;
}

static void
cnmDBDCFsmActionReqPeivilegeLock(struct ADAPTER *prAdapter)
{
	DBDC_INFO.fgReqPrivelegeLock = TRUE;
...
cnmDBDCFsmActionReqPeivilegeUnLock(IN struct ADAPTER *prAdapter)
{
	...
	while (!LINK_IS_EMPTY(&DBDC_INFO.rPendingMsgList)) {

		LINK_REMOVE_HEAD(&DBDC_INFO.rPendingMsgList, prMsgHdr,
				 struct MSG_HDR *);
...
{
	if (timerPendingTimer(&DBDC_INFO.rDbdcGuardTimer)) {
		log_dbg(CNM, WARN,
	...
	IN enum ENUM_DBDC_FSM_EVENT_T	eEvent)
{
	...
	cnmDbdcFsmSteps(prAdapter, DBDC_INFO.eDbdcFsmNextState, eEvent);
}
	...
	IN enum ENUM_DBDC_FSM_EVENT_T   eEvent)
{
	...
	switch (eEvent) {
	case DBDC_FSM_EVENT_BSS_DISCONNECT_LEAVE_AG:
		DBDC_INFO.fgPostpondLeaveAG = TRUE;
		break;
	...
	IN enum ENUM_DBDC_FSM_EVENT_T   eEvent)
{
	...
	case DBDC_FSM_EVENT_BSS_CONNECTING_ENTER_AG:
		DBDC_INFO.fgPostpondEnterAG = TRUE;
		break;
...
#ifdef MT7926
	/* When enabling DBDC, the max BW of Besra's band0 is only 40 */
	if (USE_DBDC_CAPABILITY()) {
		if (prBssInfo->eBand == BAND_2G4 ||
			(prBssInfo->eBand == BAND_5G &&
				DBDC_INFO.fgIsDBDCAAMode))
			ucMaxBw = (ucMaxBw > MAX_BW_40MHZ) ?
			    ...
			    IN u_int8_t ucForceLeaveEnGuard)
{
	...
	if (timerPendingTimer(&DBDC_INFO.rDbdcGuardTimer) &&
		...
		if (DBDC_INFO.eDbdcFsmCurrState ==
		ENUM_DBDC_FSM_STATE_DISABLE_GUARD) {
			log_dbg(CNM, INFO,
			...
			cnmTimerStartTimer(prAdapter,
					   &DBDC_INFO.rDbdcGuardTimer,
					   DBDC_ENABLE_GUARD_TIME);
		} else if (DBDC_INFO.eDbdcFsmCurrState ==
					ENUM_DBDC_FSM_STATE_ENABLE_GUARD &&
					ucForceLeaveEnGuard) {
			       ...
			       IN unsigned long plParamPtr)
{
	...
	if (DBDC_INFO.eDdbcGuardTimerType ==
	    ENUM_DBDC_GUARD_TIMER_SWITCH_GUARD_TIME) {
	...
	} else if (DBDC_INFO.eDdbcGuardTimerType ==
		   ENUM_DBDC_GUARD_TIMER_DISABLE_COUNT_DOWN) {
			      ...
			      IN struct WIFI_EVENT *prEvent)
{
	...
	if (DBDC_INFO.eDbdcFsmCurrState ==
		...
		fgDbdcEn = false;
		DBDC_INFO.fgHasSentCmd = false;
	} else if (DBDC_INFO.fgHasSentCmd == true) {
		...
		log_dbg(CNM, INFO,
				...
				DBDC_INFO.eDbdcFsmCurrState);
		DBDC_INFO.fgHasSentCmd = FALSE;
		prAdapter->rWifiVar.fgDbDcModeEn = DBDC_INFO.fgCmdEn;
		return;
	} else {
		log_dbg(CNM, ERROR,
...
{
	return DBDC_INFO.fgIsDBDCEnByP2pLis;
}
#endif

u_int8_t cnmDbdcIsWaitHwDisable(IN struct ADAPTER *prAdapter)
{
	return DBDC_INFO.eDbdcFsmCurrState ==
		ENUM_DBDC_FSM_STATE_WAIT_HW_DISABLE;
}
...
#ifdef MT7926
	/* When enabling DBDC, the max BW of Besra's band0 is only 40 */
	/* When DBDC disabled, the max BW of Besra's band0 must be recover */
		if (fgEnable) {
			if (prBssInfo->eBand == BAND_5G &&
					DBDC_INFO.fgIsDBDCAAMode) {
				if (DBDC_INFO.ucOpBwOrignal ==
							MAX_BW_UNKNOWN)
					DBDC_INFO.ucOpBwOrignal =
						cnmOpModeGetMaxBw(prAdapter,
								prBssInfo);

				ucOpBwFinal = (ucOpBwFinal > MAX_BW_40MHZ) ?
						MAX_BW_40MHZ : ucOpBwFinal;
			}
		} else {
			if (prBssInfo->eBand == BAND_5G &&
				DBDC_INFO.ucOpBwOrignal != MAX_BW_UNKNOWN) {
				ucOpBwFinal = DBDC_INFO.ucOpBwOrignal;
				DBDC_INFO.ucOpBwOrignal = MAX_BW_UNKNOWN;
			}
		}
#endif
...
enum ENUM_DBDC_BN
cnmGetDbdcIdxByRfBand(struct ADAPTER *prAdapter,
	enum ENUM_BAND eBand)
{
#if CFG_SUPPORT_DBDC
	if ((eBand == BAND_2G4) && (DBDC_INFO.fgCmdEn))
		return ENUM_BAND_1;
#endif
	return ENUM_BAND_0;
}

...
#define CMD_RES_WAITING_LOOP  100

#if (CFG_SUPPORT_FMCW_RADAR == 1)
#define FMCW_RADAR_FREQ_1_5G 5787500
#define FMCW_RADAR_FREQ_2_5G 5800000
#define FMCW_RADAR_FREQ_1_6G 6095000
#endif
...
static const uint16_t g_u2CountryGroupEU[] = {
...
#if (CFG_SUPPORT_6G_PWR_REGS == 1)
#define SKU_TBL_VLP_VER 3
/* ant gain info for tpe feature */
...
struct DOMAIN_INFO_CATEGORY arSupportedRegCategory[] = {
	{
		(uint16_t *) g_u2CountryGroup0, sizeof(g_u2CountryGroup0) / 2,
		REG_CAT_ETSI
	}
...
#if (CFG_SUPPORT_UNII4 == 1)
	,
	{BAND_5G, UNII4_LOWER_BOUND, UNII4_UPPER_BOUND, 2, 0}
				/* 5G 167~177 */
#endif
#if (CFG_SUPPORT_WIFI_6G == 1)
	,
...
#define IS_CH_SPAN(_ch, _adj_ch, _span) \
((_ch >= _adj_ch) ? ((_ch - _adj_ch) == _span) : ((_adj_ch - _ch) == _span))
...
/*----------------------------------------------------------------------------*/
/*!
 * \brief Retrieve regulatory category by the current country code
 *
 * This function determines the regulatory category based on the country code
 * provided by the adapter. If no match is found for the country code,
 * the default regulatory category is used.
 *
 * \param[in] prAdapter: Pointer to adapter
 *
 * \return value : Enum of regulatory category
 */
/*----------------------------------------------------------------------------*/
uint8_t rlmDomainGetCategory(struct ADAPTER *prAdapter)
{
	struct DOMAIN_INFO_CATEGORY *prDomainInfo = NULL;
	enum ENUM_REGULATORY_CATEGORY eRegCat = REG_CAT_NULL;
	uint8_t ucFound = FALSE;
	uint16_t u2TargetCountryCode, i, j;
	uint32_t u4GroupNum, u4DefaultIdx;

	u4GroupNum = ARRAY_SIZE(arSupportedRegCategory);
	u4DefaultIdx = u4GroupNum - 1;

	/* Get current country code */
	u2TargetCountryCode =
		rlmDomainReverseAlpha2(prAdapter->rWifiVar.u2CountryCode);

	for (i = 0; i < u4GroupNum; i++) {
		prDomainInfo = &arSupportedRegCategory[i];
		if (!prDomainInfo->u4CountryNum ||
			!prDomainInfo->pu2CountryGroup)
			continue;
...
out:
	if (!ucFound) {
		/* Use default category if no matched country code */
		eRegCat = arSupportedRegCategory[u4DefaultIdx].eRegCat;
		DBGLOG(RLM, INFO,
			"No matched for country[%c%c], use the default[%u]\n",
			(u2TargetCountryCode & 0xff00) >> 8,
			(u2TargetCountryCode & 0x00ff),
			eRegCat);
	} else if ((prDomainInfo) && (prDomainInfo->eRegCat >= 0) &&
		(prDomainInfo->eRegCat < REG_CAT_NUM)) {
		/* Found matching country code, set corresponding category */
		eRegCat = prDomainInfo->eRegCat;
		DBGLOG(RLM, TRACE,
			"Country[%c%c/%u/%s] found in group[%u,%u]\n",
			(u2TargetCountryCode & 0xff00) >> 8,
			(u2TargetCountryCode & 0x00ff),
			eRegCat, apucRegCatType[eRegCat],
			i, j);
	} else {
		DBGLOG(RLM, ERROR,
			"DomainInfo is null or Category is invalid value\n");
	}

	return (uint8_t)eRegCat;
}
			...
			struct RF_CHANNEL_INFO *paucChannelList)
{
	...
	if (eSpecificBand == BAND_2G4) {
		i = 0;
		max_count = RLM_GET_ACTIVE_CHANNEL_COUNT(KAL_BAND_2GHZ);
	} else if (eSpecificBand == BAND_5G) {
		i = RLM_GET_ACTIVE_CHANNEL_COUNT(KAL_BAND_2GHZ);
		max_count = RLM_GET_ACTIVE_CHANNEL_COUNT(KAL_BAND_5GHZ) +
			RLM_GET_ACTIVE_CHANNEL_COUNT(KAL_BAND_2GHZ);
	}
#if (CFG_SUPPORT_WIFI_6G == 1)
	else if (eSpecificBand == BAND_6G) {
		i = RLM_GET_ACTIVE_CHANNEL_COUNT(KAL_BAND_2GHZ) +
				RLM_GET_ACTIVE_CHANNEL_COUNT(KAL_BAND_5GHZ);
		max_count = RLM_GET_ACTIVE_CHANNEL_COUNT(KAL_BAND_6GHZ) +
			RLM_GET_ACTIVE_CHANNEL_COUNT(KAL_BAND_5GHZ) +
			RLM_GET_ACTIVE_CHANNEL_COUNT(KAL_BAND_2GHZ);
	} else {
		i = 0;
	...
	ucNum = 0;
	for (; i < max_count; i++) {
		prCh = RLM_GET_ACTIVE_CHANNELS() + i;
		if (fgNoDfs && (prCh->eFlags & IEEE80211_CHAN_RADAR))
			continue; /*not match*/

		if (i < RLM_GET_ACTIVE_CHANNEL_COUNT(KAL_BAND_2GHZ))
			band = BAND_2G4;
#if (CFG_SUPPORT_WIFI_6G == 1)
		else if (i < RLM_GET_ACTIVE_CHANNEL_COUNT(KAL_BAND_2GHZ) +
			RLM_GET_ACTIVE_CHANNEL_COUNT(KAL_BAND_5GHZ))
			band = BAND_5G;
		else
			band = BAND_6G;
			  ...
			  struct RF_CHANNEL_INFO *paucChannelList)
{
	...
	for (idx = start_idx; idx < end_idx; idx++) {
		prCh = RLM_GET_ACTIVE_CHANNELS() + idx;
		if (!((prCh->eFlags & IEEE80211_CHAN_RADAR)
...
static bool isEUCountry(struct ADAPTER *prAdapter, uint32_t u4CountryCode)
{
	...
	for (i = 0; i < (sizeof(g_u2CountryGroupEU) / sizeof(uint16_t)); i++) {
		if (g_u2CountryGroupEU[i] == u2TargetCountryCode)
			return TRUE;
	}
...
void rlmDomainSendDomainInfoCmd_V2(struct ADAPTER *prAdapter)
{
...
#if (CFG_SUPPORT_FMCW_RADAR == 1)
#if ((CFG_WIFI_FMCW_RADAR_CTS2SELF == 1) && (CFG_SUPPORT_DYNAMIC_EDCCA == 1))
	if (isEUCountry(prAdapter, prCmd->u4CountryCode))
		prCmd->u4CountryCode = COUNTRY_CODE_EU;
#endif
#endif /* #if (CONFIG_SUPPORT_FMCW_RADAR == 1) */
...
#endif
	prCmd->ucRegCat = rlmDomainGetCategory(prAdapter);
	...
	DBGLOG(RLM, TRACE,
		"Country[%c%c] cmd_Country[%c%c] Reg[%u/%s]\n",
		(prAdapter->rWifiVar.u2CountryCode & 0x00ff),
		(prAdapter->rWifiVar.u2CountryCode & 0xff00) >> 8,
		(prCmd->u4CountryCode & 0x00ff),
		((prCmd->u4CountryCode & 0xff00) >> 8),
		prCmd->ucRegCat,
		apucRegCatType[prCmd->ucRegCat]);
		...
		enum ENUM_BAND eBand, uint8_t ucChannel)
{
	...
	for (idx = start_idx; idx < end_idx; idx++) {
		prCh = RLM_GET_ACTIVE_CHANNELS() + idx;
		if (prCh->u2ChNum == ucChannel &&
			((prCh->eFlags & IEEE80211_CHAN_RADAR)
				== IEEE80211_CHAN_RADAR)) {
			return TRUE;
		}
	}
	...
	struct ADAPTER **prAdapter)
{
	...
	*prAdapter = prGlueInfo->prAdapter;
	...
	eCurrentState = rlmDomainGetCtrlState(prGlueInfo);
	...
	u_int32_t u4CountryCode)
{
	...
	if (u4FinalCountryCode != u4CountryCode)
		rlmDomainSetCountryCode(prAdapter, acCountryCodeStr,
	...
	u2CountryCode =
		(uint16_t)rlmDomainGetCountryCode(prAdapter);
...
rlmDomainSetCountry(struct ADAPTER *prAdapter)
{
	...
	struct wiphy *prBaseWiphy = wlanGetWiphy(pprWdev[0]);
	struct ADAPTER *prBaseAdapter = NULL;
	...
	rlmDomainRegSetAddToQueue(prBaseWiphy,
	...
	struct TX_PWR_LIMIT_DATA *pTxPwrLimitData)
{
	...
	uint8_t *pucTxBfBackoffExists = &prGlueInfo->bTxBfBackoffExists;
#else
	uint8_t *pucTxBfBackoffExists = &g_bTxBfBackoffExists;
#endif
...
#define TXPWRLIMIT_FILE_LEN 64
	...
	uint8_t aucFileName[TXPWRLIMIT_FILE_LEN];
#if CFG_SUPPORT_MULTI_CARD
	struct GLUE_INFO *prGlueInfo = prAdapter->prGlueInfo;
	uint32_t u4Idx;
#endif
	...
	for (u4Idx = READ_DEV_CFG; u4Idx < READ_CFG_TOTAL_NUM; u4Idx++) {
		...
		if (u4Idx == READ_DEV_CFG)
			kalSnprintf(aucFileName, TXPWRLIMIT_FILE_LEN, "%s/%s",
				prGlueInfo->aucDevCfgPath, prFileName);
		else
#endif /* CFG_SUPPORT_MULTI_CARD */
			kalSnprintf(aucFileName, TXPWRLIMIT_FILE_LEN,
				"%s", prFileName);

		kalMemZero(aucPath, sizeof(aucPath));
		kalSnprintf(aucPath[0], TXPWRLIMIT_FILE_LEN,
			"%s", aucFileName);
		kalSnprintf(aucPath[1], TXPWRLIMIT_FILE_LEN,
			"/data/misc/%s", aucFileName);
			      ...
			      uint8_t ucPwrLimitNum)
{
	...
	for (i = 0; i < ucPwrLimitNum; i++, prPwrLimit++) {
		if (*prPwrLimit != kal_clamp_t(int8_t, *prPwrLimit,
	...
	uint32_t size)
{
	...
	for (i = 0; i < size; i++) {
		...
		target[i] = kal_clamp_t(int8_t, target[i],
			MIN_TX_POWER, MAX_TX_POWER);
	}
}
...
uint32_t
rlmDomainTxPwrLimitSendCmdWrap(
	struct ADAPTER *prAdapter,
	uint8_t ucCID,
	u_int8_t fgSetQuery,
	u_int8_t fgNeedResp,
	...
	uint32_t u4SetQueryBufferLen)
{
	...
	if (u4SetQueryInfoLen <= 0 || !pucInfoBuffer) {
		DBGLOG(RLM, ERROR, "pucInfoBuffer is Invalid\n");
		return WLAN_STATUS_INVALID_DATA;
	}
	if (kalIsResetting(prAdapter->prGlueInfo)) {
		DBGLOG(RLM, WARN, "Chip resetting, skip\n");
		return WLAN_STATUS_FAILURE;
	}

	/* Update tx max. power info to chip */
	while (cmd_cnt < CMD_RES_WAITING_LOOP) {
		/* Update tx max. power info to chip */
		rStatus = wlanSendSetQueryCmd(prAdapter,
			ucCID,
			fgSetQuery,
	...
	if (cmd_cnt >= CMD_RES_WAITING_LOOP)
		DBGLOG(RLM, ERROR,
			   "[TXPWR]Failed to send cmd, Status[%d]\n",
			   rStatus);
...
)
{
	...
	for (i = 0; i < prCmdBatchNum; i++) {
		...
		u4SetQueryInfoLen = u4BufSize;
		/* Update tx max. power info to chip */
		rStatus = rlmDomainTxPwrLimitSendCmdWrap(prAdapter,
...
#if (CFG_SUPPORT_6G_PWR_REGS == 1)
u_int8_t rlmDomainGetTxPwrVlpLimit(
	uint32_t country_code,
	uint8_t *pucVersion,
	struct GLUE_INFO *prGlueInfo,
	struct TX_PWR_LIMIT_DATA *pTxPwrLimitData)
	...
	struct CHANNEL_TX_PWR_LIMIT *pChTxPwrLimit)
{
	...
	for (section = 0; section < SINGLE_SKU_VLP_SECTION_NUM; section++) {
	...
	)
{
	...
	prTxPwrLimitVlpCmd->eLimitType = TXPOWER_LIMIT_FORMAT_CH_SKU_VLP;
...
void rlmDomainSendPwrLimitCmd_V2(struct ADAPTER *prAdapter)
{
	...
	if (!rlmDomainGetTxPwrLimit(rlmDomainGetCountryCode(prAdapter),
		&ucVersion,
		prGlueInfo,
		pTxPwrLimitData)) {
		DBGLOG(RLM, ERROR,
	...
	if (!rlmDomainGetTxPwrLegacyLimit(rlmDomainGetCountryCode(prAdapter),
		&ucVersion,
		prGlueInfo,
		pTxPwrLegacyLimitData)) {
		DBGLOG(RLM, ERROR,
	...
	prAdapter->chip_info->prTxPwrLimitFile = WIFI_TXPWR_6G_LPI_FN;
	if (!rlmDomainGetTxPwrLimit(rlmDomainGetCountryCode(prAdapter),
		&ucVersion,
		prGlueInfo,
		pTxPwrLimitData)) {
...
#if (CFG_SUPPORT_6G_PWR_REGS == 1)
	prAdapter->chip_info->prTxPwrLimitFile = WIFI_TXPWR_6G_SP_FN;
	if (!rlmDomainGetTxPwrLimit(rlmDomainGetCountryCode(prAdapter),
		&ucVersion,
		prGlueInfo,
		pTxPwrLimitData)) {
	...
	/* Get Max Tx Power from MT_TxPwrLimit_6G_VLP.dat */
	prAdapter->chip_info->prTxPwrLimitFile = WIFI_TXPWR_6G_VLP_FN;
	if (!rlmDomainGetTxPwrVlpLimit(rlmDomainGetCountryCode(prAdapter),
		&ucVersion,
		prGlueInfo,
		pTxPwrLimitData)) {
		prAdapter->fg6GSupVlpFile = FALSE;
		DBGLOG(RLM, ERROR,
			"Load VLP TxPwrLimitData failed\n");
		goto error;
	}
...
error:
	...
	prAdapter->chip_info->prTxPwrLimitFile = WIFI_TXPWR_FN;
	...
	uint8_t *pu1SubBandPwrIdx)
{
	...
	case PWR_LMT_SUBBAND_UNII3:
		*pu1SubBandPwrIdx = PWR_LMT_SUBBAND_PWR_UNII3;
		break;
#if (CFG_SUPPORT_UNII4 == 1)
	case PWR_LMT_SUBBAND_UNII4:
		*pu1SubBandPwrIdx = PWR_LMT_SUBBAND_PWR_UNII4;
		break;
	...
	enum ENUM_PWR_MODE_6G_TYPE e6GPwrModeBss)
{
	...
	e6GPwrModeFinal = rlmDomainPwrLmt6GPwrModeGet(prAdapter);

	if (prAdapter->e6GPwrModeCurr != e6GPwrModeFinal) {
		/* Resend power limit */
		DBGLOG(RLM, INFO, "Set to 6GPwrMode[%u->%u]",
	...
	DBGLOG(RLM, INFO, "Update BSS[%u]6GPwrMode[%d]Final[%d]",
			ucBssIndex,
			e6GPwrModeBss,
			e6GPwrModeFinal);
 ...
 */
/*----------------------------------------------------------------------------*/
uint8_t rlmDomainPwrLmt6GPwrModeGet(struct ADAPTER *prAdapter)
{
	uint8_t ucBssIdx = 0;
 ...
 */
/*----------------------------------------------------------------------------*/
uint32_t rlmDomainGetSubBandIdx(enum ENUM_BAND eBand,
	uint8_t ucCenterCh,
	...
	uint8_t uc6GHeRegInfo)
{
	...
	DBGLOG(RLM, TRACE,
		"HE6GPre[%u]HeRegInfo[%u]6GPwrMode[%d]\n",
		fgIsHE6GPresent,
		uc6GHeRegInfo,
		ePwrMode6G);
 ...
 */
/*----------------------------------------------------------------------------*/
uint8_t rlmDomain6GPwrModeSupportStatusGet(
...
void rlmDomainSetDfsRegion(struct ADAPTER *prAdapter,
...
enum nl80211_dfs_regions rlmDomainGetDfsRegion(struct ADAPTER *prAdapter)
{
#if CFG_SUPPORT_MULTI_CARD
	struct GLUE_INFO *prGlueInfo = prAdapter->prGlueInfo;
	struct mtk_regd_control *prRegdControl = &prGlueInfo->rMtkRegdControl;
...
#if (CFG_SUPPORT_6G_PWR_REGS == 1)
	if (flags & IEEE80211_CHAN_NO_6GHZ_VLP_CLIENT)
		LOGBUF(buf, ((int32_t)buf_size), buf_written, "NO_VLP ");
	if (flags & IEEE80211_CHAN_NO_6GHZ_AFC_CLIENT)
		LOGBUF(buf, ((int32_t)buf_size), buf_written, "NO_AFC ");
#endif /* CFG_SUPPORT_6G_PWR_REGS */
}
...
void rlmDomainParsingChannel(IN struct wiphy *pWiphy)
{
	...
	WIPHY_PRIV(pWiphy, prGlueInfo);
#else
	prGlueInfo = rlmDomainGetGlueInfo(NULL);
#endif
	prAdapter = prGlueInfo->prAdapter;
	...
	if (prGlueInfo && kalGetMediaStateIndicated(prGlueInfo) ==
		...
		ucChannelNum =
			wlanGetChannelNumberByNetwork(prAdapter,
	...
	for (band_idx = 0; band_idx < KAL_NUM_BANDS; band_idx++) {
		...
		for (ch_idx = 0; ch_idx < sband->n_channels; ch_idx++) {
			...
			pCh = RLM_GET_ACTIVE_CHANNELS() + ch_count;
			   ...
			   struct CMD_DOMAIN_ACTIVE_CHANNEL_LIST *prBuff)
{
	...
	prBuff->u1ActiveChNum2g = RLM_GET_ACTIVE_CHANNEL_COUNT(KAL_BAND_2GHZ);
	prBuff->u1ActiveChNum5g = RLM_GET_ACTIVE_CHANNEL_COUNT(KAL_BAND_5GHZ);

#if (CFG_SUPPORT_WIFI_6G == 1)
	prBuff->u1ActiveChNum6g = RLM_GET_ACTIVE_CHANNEL_COUNT(KAL_BAND_6GHZ);
	...
	for (idx = 0; idx < ch_count; idx++) {
		...
		pCh->u2ChNum =
			(RLM_GET_ACTIVE_CHANNELS() + idx)->u2ChNum;
		pCh->eFlags =
			(RLM_GET_ACTIVE_CHANNELS() + idx)->eFlags;
	}
...
struct GLUE_INFO *rlmDomainGetGlueInfo(struct ADAPTER *prAdapter)
{
	...
	return prGlueInfo->rMtkRegdControl.pGlueInfo;
#else
...
bool rlmDomainIsEfuseUsed(struct ADAPTER *prAdapter)
{
	...
	return prGlueInfo->rMtkRegdControl.isEfuseCountryCodeUsed;
#else
	return g_mtk_regd_control.isEfuseCountryCodeUsed;
#endif
}

uint8_t rlmDomainGetChannelBw(struct ADAPTER *prAdapter,
				enum ENUM_BAND eBand, uint8_t channelNum)
{
	...
	for (ch_idx = start_idx; ch_idx < end_idx; ch_idx++) {
		...
		if (ch_idx < RLM_GET_ACTIVE_CHANNEL_COUNT(KAL_BAND_2GHZ))
			eChBand = BAND_2G4;
#if (CFG_SUPPORT_WIFI_6G == 1)
		else if (ch_idx < RLM_GET_ACTIVE_CHANNEL_COUNT(KAL_BAND_2GHZ)
			+ RLM_GET_ACTIVE_CHANNEL_COUNT(KAL_BAND_5GHZ))
			eChBand = BAND_5G;
		else
			eChBand = BAND_6G;
		...
		DBGLOG(RLM, TRACE, "band=%d, ch=%d, flags=0x%x\n",
		...
		if (ch_idx >= RLM_GET_ACTIVE_CHANNEL_COUNT(KAL_BAND_2GHZ)) {
			...
			if (channelBw > MAX_BW_20MHZ) {
				...
				if ((u4ChnlSeq & 0x1) && (pCh->eFlags &
					IEEE80211_CHAN_NO_HT40PLUS))
					channelBw = MAX_BW_20MHZ;
				/* Check flag for 5G chnl 120 or 128 */
				else if ((!(u4ChnlSeq & 0x1)) && (pCh->eFlags &
					IEEE80211_CHAN_NO_HT40MINUS))
					channelBw = MAX_BW_20MHZ;
			}

			/* Limit MAX_BW_80MHz and above to 40 or 20MHz */
			if (channelBw > MAX_BW_40MHZ) {
				int32_t offset = 0;
				int32_t i4ChIdx = ch_idx;
				uint8_t ucAdj40ChnlSpan = CHNL_SPAN_40;

				switch (u4ChnlSeq) {
				case 1:
					/* 5G chnl 116 to check chnl 120 flag */
					if ((i4ChIdx + 1) < i4EndIdx)
						pAdj20Chnl = (
						RLM_GET_ACTIVE_CHANNELS() +
						(i4ChIdx + 1));
					offset = 3;
					ucAdj40ChnlSpan += CHNL_SPAN_20;
					break;
				case 2:
					/* 5G chnl 120 to check chnl 116 flag */
					if ((i4ChIdx - 1) >= i4StartIdx)
						pAdj20Chnl = (
						RLM_GET_ACTIVE_CHANNELS() +
						(i4ChIdx - 1));
					offset = 2;
					break;
				case 3:
					/* 5G chnl 124 to check chnl 128 flag */
					if ((i4ChIdx + 1) < i4EndIdx)
						pAdj20Chnl = (
						RLM_GET_ACTIVE_CHANNELS() +
						(i4ChIdx + 1));
				...
				/* To find adjacent 40MHz channel:
				 * 5G chnl 116 to check chnl 128 flag
				 * 5G chnl 120 to check chnl 128 flag
				 * 5G chnl 124 to check chnl 116 flag
				 * 5G chnl 128 to check chnl 116 flag
				 */
				...
				/* Limit Bandwidth to 40 or 20MHZ:
				 * 1. Unable to find adjacent channel
				 * 2. Incorrect adjacent channel span
				 */
				if ((pAdj20Chnl == NULL) ||
					!IS_CH_SPAN(channelNum, pAdj20Chnl
					->u2ChNum, CHNL_SPAN_20))
					channelBw = MAX_BW_20MHZ;
				else if ((pAdj40Chnl == NULL) ||
					!IS_CH_SPAN(channelNum, pAdj40Chnl
					->u2ChNum, ucAdj40ChnlSpan))
					channelBw = MAX_BW_40MHZ;
			}
		}
		break;
	...
	if (pAdj20Chnl)
		DBGLOG(RLM, LOUD, "adj20ch=%u, flag=0x%x\n",
			pAdj20Chnl->u2ChNum, pAdj20Chnl->eFlags);
	if (pAdj40Chnl)
		DBGLOG(RLM, LOUD, "adj40ch=%d, flag=0x%x\n",
			pAdj40Chnl->u2ChNum, pAdj40Chnl->eFlags);
...
#if CFG_SUPPORT_MULTI_CARD
	prRegdControl = &prAdapter->prGlueInfo->rMtkRegdControl;
#else
	prRegdControl = &g_mtk_regd_control;
#endif
			    ...
			    u8 size_of_country)
{
	...
	if (rlmDomainIsUsingLocalRegDomainDataBase() == FALSE) {
		DBGLOG(RLM, INFO,
		       "%s(): Using driver hint to query CRDA getting regd.\n",
		       __func__);
		regulatory_hint(GLUE_GET_WIPHY(prGlueInfo), country);
		return;
	}

	if (rlmDomainIsSameCountryCode(prAdapter, country, size_of_country)) {
		char acCountryCodeStr[MAX_COUNTRY_CODE_LEN + 1] = {0};
	...
	enum ENUM_PWR_MODE_6G_TYPE ePwrMode6G)
{
	...
	pTxPwrLimitData = rlmDomainInitTxPwrLimitData_6G(prAdapter);
	pTxPwrLegacyLimitData =
		rlmDomainInitTxLegacyPwrLimitData_6G(prAdapter);
	...
	if (ePwrMode6G == PWR_MODE_6G_LPI)
		prAdapter->chip_info->prTxPwrLimitFile = WIFI_TXPWR_6G_LPI_FN;
	else if (ePwrMode6G == PWR_MODE_6G_SP)
		prAdapter->chip_info->prTxPwrLimitFile = WIFI_TXPWR_6G_SP_FN;
	else {
		DBGLOG(RLM, ERROR,
	...
	if (!rlmDomainGetTxPwrLegacyLimit(
		rlmDomainGetCountryCode(prAdapter),
		&ucVersion,
		prGlueInfo,
		pTxPwrLegacyLimitData)) {
		DBGLOG(RLM, ERROR,
			"Load TxPwrLegacyLimitData6G failed\n");
		u4Ret = WLAN_STATUS_FAILURE;
		goto error;
	}

	/* Update Power table to FW */
	if (u4Ret == WLAN_STATUS_SUCCESS && ucVersion == 2) {
		rlmDomainSendTxLegacyPwrLimitPerRateCmd_6G(prAdapter,
			ucVersion, pTxPwrLegacyLimitData);
		rlmDomainSendTxPwrLimitPerRateCmd_6G(prAdapter,
			ucVersion, pTxPwrLimitData);
	...
	uint8_t fgPwrLmtEnable)
{
	...
	prTempCmd->eLimitType = TXPOWER_LIMIT_FORMAT_TPE;
	...
	struct TX_PWR_ANT_GAIN_DATA *pTxPwrAntGain)
{
	...
	if (BandIdx < (BAND_2G4 - 1) || BandIdx >= (BAND_NUM - 1)) {
		DBGLOG(RLM, ERROR, "Invalid band idx\n");
		return FALSE;
	}
	...
	if ((pucBuf[u4TmpPos] == 'g') && (pucBuf[u4TmpPos+1] == 'r') &&
		...
		*pu4Pos = u4TmpPos;
		DBGLOG(RLM, ERROR,
			"[%s] [RLM] Invalid subband setting starting chars: %c%c\n",
			__func__,
			pucBuf[u4TmpPos],
			pucBuf[u4TmpPos + 1]);
...
u_int8_t rlmDomainAntGaiLoadFromFile(
	uint8_t *prFileName, uint8_t *pucConfigBuf,
	size_t szConfigBufSize, uint32_t *pu4ConfigReadLen)
{
	u_int8_t bRet = FALSE;

	if ((!prFileName) || (!pu4ConfigReadLen) ||
		(!pucConfigBuf) || (szConfigBufSize == 0)) {
		bRet = FALSE;
		DBGLOG(RLM, ERROR,
			"[%d]Invalid parameter\n", __LINE__);
		goto error;
	...
	kalMemZero(pucConfigBuf, szConfigBufSize);
	*pu4ConfigReadLen = 0;

	if (kalRequestFirmware(prFileName,
				pucConfigBuf,
				(uint32_t)szConfigBufSize,
				pu4ConfigReadLen,
				NULL) != 0) {
		DBGLOG(RLM, ERROR, "Read File Method Fail. Path[%s]\n",
			prFileName);
		bRet = FALSE;
		goto error;
	}
	...
	if (pucConfigBuf[0] == '\0' || *pu4ConfigReadLen == 0) {
		DBGLOG(RLM, ERROR, "ConfigBuf is empty!!\n");
		bRet = FALSE;
		goto error;
	}
...
bool rlmDomainAntGainInit(void)
{
	bool bRet = FALSE;
	...
	pucConfigBuf = (uint8_t *)kalMemAlloc(WLAN_ANT_GAIN_FILE_BUF_SIZE,
						VIR_MEM_TYPE);

	if (!pucConfigBuf) {
		DBGLOG(RLM, ERROR, "Fail to allocate AntGainBuffer\n");
		return bRet;
	}

	// Load AntGain file to configBuf
	bRet = rlmDomainAntGaiLoadFromFile(
		arFileName,
		pucConfigBuf,
		(size_t) WLAN_ANT_GAIN_FILE_BUF_SIZE,
		&u4ConfigReadLen);
	...
	for (i = 0; i < TX_PWR_ANT_GAIN_GROUP_NUM; i++) {
		for (BandIdx = BAND_2G4; BandIdx < BAND_NUM; BandIdx++) {
			...
			DBGLOG(RLM, INFO, "[RLM]Ant_Group[%u]=[%d][%d]\n",
				...
				prAntGainGroup->
				rTxPwrAntGainValue[BandIdx - 1][0],
				prAntGainGroup->
				rTxPwrAntGainValue[BandIdx - 1][1]);
		}
	}
...
{
	struct TX_PWR_ANT_GAIN_GROUP *prAntGainGroup = NULL;
	uint8_t AntIdx = 0, BandIdx = eBand - 1;
	uint8_t u1BandGrp = POWER_ANT_GAIN_GROUP_1;

	if (!acAntGain || eBand == BAND_NULL)  {
		DBGLOG(RLM, ERROR, "[%d][RLM]Invalid data\n", __LINE__);
		return WLAN_STATUS_INVALID_DATA;
	}
...
#if (CFG_BUILD_MOBILE_SKU == 1)
void calculateScoByChannel(struct ADAPTER *prAdapter,
		enum ENUM_BAND eBand,
...
#if (CFG_SUPPORT_FMCW_RADAR == 1)
bool rlmDomainCheckRadarCountryCode(struct ADAPTER *prAdapter, uint32_t u4Freq)
{
	uint16_t u2CountryCode;
	...
	if (u2CountryCode && u4Freq) {
		switch (u4Freq) {
		case FMCW_RADAR_FREQ_1_5G:
			if ((u2CountryCode == COUNTRY_CODE_PE) ||
				(u2CountryCode == COUNTRY_CODE_MX))
				fgIsFreqVaild = TRUE;
			break;

		case FMCW_RADAR_FREQ_2_5G:
			if ((u2CountryCode != COUNTRY_CODE_PE) &&
				(u2CountryCode != COUNTRY_CODE_MX))
				fgIsFreqVaild = TRUE;
			break;

		case FMCW_RADAR_FREQ_1_6G:
			/* Not support yet*/
			break;
	...
	DBGLOG(RLM, ERROR, "Country(%c%c)\n",
				((u2CountryCode & 0xff00) >> 8),
				(u2CountryCode & 0x00ff));

	return fgIsFreqVaild;
}
#endif

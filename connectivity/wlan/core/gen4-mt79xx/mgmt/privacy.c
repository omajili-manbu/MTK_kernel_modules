			     ...
			     IN struct STA_RECORD *prStaRec)
{
	...
	if (prBssInfo->eNetworkType == NETWORK_TYPE_NAN) {
		...
		if (prMsdu->ucStaRecIndex == STA_REC_INDEX_BMCAST) {
			if (nanIsMcTxKeyExist(prAdapter,
				prMsdu->aucEthDestAddr,
				prMsdu->aucEthSrcAddr))
				return TRUE;
			else
				return FALSE;
		} else
...
u_int8_t secIsUsedByStaRecEntry(IN struct ADAPTER *prAdapter, IN uint8_t Index)
{
	...
	if (i % 8 == 0)
		...
		if (prWtbl[i].ucUsed
			...
			if ((prSta->eStaType == STA_TYPE_NAN)
				&& !EQUAL_MAC_ADDR(prSta->aucLocalNdiAddr,
				prWtbl[i].aucLocalNdiAddr))
				continue;
#endif
			ucEntry = i;
			DBGLOG(RSN, TRACE,
...
#if CFG_SUPPORT_NAN
		if (prSta->eStaType == STA_TYPE_NAN)
			COPY_MAC_ADDR(prWtbl[ucEntry].aucLocalNdiAddr,
			prSta->aucLocalNdiAddr);
#endif
 ...
 * For MC Tx(Data): pucLocalAddr = local NDI/NMI, pucPeerAddr = NDC ID
 * For MC TX(Beacon/SDF): pucLocalAddr = local NMI, pucPeerAddr = local NMI,
 * BssIdx = to select different wtbl entries for BIGTK and IGTK
 */
uint8_t secPrivacySeekForNanEntry(
			IN struct ADAPTER *prAdapter,
			IN uint8_t *pucLocalAddr,
			IN uint8_t *pucPeerAddr,
			IN uint8_t ucPairwise,
			IN uint8_t ucBssIdxForTxBmc,
			OUT uint8_t *pucReUsed)
{
	struct WLAN_TABLE *prWtbl = NULL;
	size_t szEntry = WTBL_RESERVED_ENTRY;
	size_t i = 0, j = 0;
	uint8_t fgReUsed = FALSE;
	...
	if (prAdapter->ucTxDefaultWlanIndex != 0) {
		j = kalRandomNumber() % prAdapter->ucTxDefaultWlanIndex;
	} else {
		DBGLOG(RSN, ERROR,
			"ucTxDefaultWlanIndex is 0, return FALSE\n");
		return WTBL_RESERVED_ENTRY;
	}

	for (i = 0; i <= (size_t)(prAdapter->ucTxDefaultWlanIndex - 1);
	  i++) {
#if CFG_WIFI_WORKAROUND_HWITS00012836_WTBL_SEARCH_FAIL
		if (i % 8 == 0)
			continue;
#endif
		if (prWtbl[i].ucUsed
			&& EQUAL_MAC_ADDR(pucPeerAddr, prWtbl[i].aucMacAddr)
			&& EQUAL_MAC_ADDR(
				pucLocalAddr, prWtbl[i].aucLocalNdiAddr)
			&& (prWtbl[i].ucPairwise == ucPairwise)
			&& (ucBssIdxForTxBmc &&
				(prWtbl[i].ucBssIndex == ucBssIdxForTxBmc))) {
			fgReUsed = TRUE;
			szEntry = i;
			DBGLOG(RSN, TRACE,
				"[Wlan index]: Reuse entry #%zu\n", i);
			break;
		}
	}

	if (i == (prAdapter->ucTxDefaultWlanIndex)) {
		for (i = 0;
		i <= (size_t)(prAdapter->ucTxDefaultWlanIndex - 1);
		i++, j = (j + 1) % prAdapter->ucTxDefaultWlanIndex) {
#if CFG_WIFI_WORKAROUND_HWITS00012836_WTBL_SEARCH_FAIL
			if (j % 8 == 0)
				continue;
#endif
			if (prWtbl[j].ucUsed == FALSE) {
				if (!secIsUsedByStaRecEntry(prAdapter, j)) {
					szEntry = j;
					DBGLOG(RSN, INFO,
						"[Wlan index]: Assign entry #%zu\n",
						j);
					break;
				}
			}
	...
	if (pucReUsed != NULL)
		*pucReUsed = fgReUsed;
	/* Save to the driver maintain table */
	if (szEntry < prAdapter->ucTxDefaultWlanIndex) {
		if (!fgReUsed) {
			prWtbl[szEntry].ucUsed = TRUE;
			prWtbl[szEntry].ucKeyId = 0xFF;
			prWtbl[szEntry].ucPairwise = ucPairwise;
			COPY_MAC_ADDR(prWtbl[szEntry].aucMacAddr,
				pucPeerAddr);
			COPY_MAC_ADDR(prWtbl[szEntry].aucLocalNdiAddr,
				pucLocalAddr);
			if (ucBssIdxForTxBmc)
				prWtbl[szEntry].ucBssIndex = ucBssIdxForTxBmc;
			else
				prWtbl[szEntry].ucBssIndex = nanGetBssIdxByRole(
					prAdapter, NAN_BSS_INDEX_MAIN);
			prWtbl[szEntry].ucStaIndex = STA_REC_INDEX_NOT_FOUND;
		}

		DBGLOG(RSN, INFO,
			"[Wlan index] BSS#%d keyid#%d P=%d use WlanIndex#%zu STAIdx=%d "
			MACSTR "\n",
			prWtbl[szEntry].ucBssIndex, 0,
			prWtbl[szEntry].ucPairwise, szEntry,
			prWtbl[szEntry].ucStaIndex, MAC2STR(pucPeerAddr));
#if DBG
		secCheckWTBLAssign(prAdapter);
#endif
		return szEntry;
...
#if CFG_SUPPORT_NAN
		kalMemZero(prWtbl[ucEntry].aucLocalNdiAddr, MAC_ADDR_LEN);
#endif
	...
	for (entry = 0; entry < WTBL_SIZE; entry++) {
		...
		if (prWtbl[entry].ucUsed &&
			...
			if ((prStaRec->eStaType == STA_TYPE_NAN)
				&& !EQUAL_MAC_ADDR(prStaRec->aucLocalNdiAddr,
				prWtbl[entry].aucLocalNdiAddr))
				continue;

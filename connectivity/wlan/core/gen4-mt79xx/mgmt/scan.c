	...
	IE_FOR_EACH(pucIE, u2IELength, u2Offset)
	{
		...
		IE_FOR_EACH(pucProfileIE, u2ProfileLen, u2ProfileOffset) {
			...
			if (IE_ID(pucProfileIE) == ELEM_ID_SSID) {
				rSSID.u4SsidLen =
				    SSID_IE(pucProfileIE)->ucLength;

				if (rSSID.u4SsidLen >= ELEM_MAX_LEN_SSID)
					rSSID.u4SsidLen = ELEM_MAX_LEN_SSID;

				kalMemCopy(rSSID.aucSsid,
				    SSID_IE(pucProfileIE)->aucSSID,
				    rSSID.u4SsidLen);
			}

			if (prMbssidIdxIe && rSSID.u4SsidLen)
				break;
		}
		...
		DBGLOG(SCN, TRACE,
			"trans bssid:" MACSTR " non-trans bssid:" MACSTR
			" indicator:%u idx:%u ssid:%s bssdesc:%p\n",
			MAC2STR(prTransBSS->aucBSSID),
			MAC2STR(aucBSSID),
			prMbssidIe->ucMaxBSSIDIndicator,
			prMbssidIdxIe->ucBSSIDIndex,
			SSID2STR(rSSID.aucSsid, rSSID.u4SsidLen),
			prBssDesc);
	}
}
#endif
	...
	uint8_t ucRnrChNum, uint8_t ucOpClass)
{
	...
	/* Check if channel is legal for current band */
	...
	IN struct BSS_DESC *prBssDesc, IN uint8_t *pucIE)
{
	...
	uint8_t ucBssidNum = 0;
	uint8_t ucShortSsidSize = (uint8_t)(OFFSET_OF(
		struct IE_SHORT_SSID_LIST, aucShortSsidList)
	...
	if (prScanInfo->eCurrentState != SCAN_STATE_SCANNING
		...
		DBGLOG(SCN, TRACE, "Skip 6G oob scan Rnr parsing\n");
		return;
	}

	while (u2CurrentLength + sizeof(struct NEIGHBOR_AP_INFO_FIELD)
		<= IE_LEN(pucIE)) {
		...
		*/
		...
		if (ucNewLink) {
			...
			kalMemCopy(rNeighborParam->aucIE,
				prAdapterScanParam->aucIE,
				prAdapterScanParam->u2IELen);
			rNeighborParam->u2IELen = prAdapterScanParam->u2IELen;
			...
			*/
			...
			if (prAdapter->rWifiVar.eDbdcMode ==
				...
				rNeighborParam->ucScnFuncMask |=
					ENUM_SCN_DBDC_SCAN_DIS;
			else if (IS_FEATURE_ENABLED(prAdapter
				->rWifiVar.fgEnDbdcScan))
				rNeighborParam->u4ScnFuncMaskExtend |=
					ENUM_SCN_ENABLE_DBDC_SCAN;
		}
		...
		for (i = 0; i < u2TbttInfoCount; i++) {
			...
			log_dbg(SCN, TRACE, "RnrIe[%x][" MACSTR "]\n", i,
			...
			if ((ucShortSsidOffset != 0) &&
				...
				*/
				...
				kalMemCopy(
					&prIeShortSsidList->aucShortSsidList,
					&prNeighborAPInfoField->
					aucTbttInfoSet[j + ucShortSsidOffset],
					SCAN_MAX_SHORT_SSID_LEN);

				log_dbg(SCN, TRACE,
					"TbttInfoCnt[%u],short SSID[%x %x %x %x]\n",
					i,
					prIeShortSsidList->aucShortSsidList[0],
					prIeShortSsidList->aucShortSsidList[1],
					prIeShortSsidList->aucShortSsidList[2],
					prIeShortSsidList->aucShortSsidList[3]);

				rNeighborParam->u2IELen += ucShortSsidSize;
		...
		if (fgHasBssid && fgScanEnable) {
			...
			log_dbg
			(SCN, TRACE, "Match[%d %d %d %d][%d %d %d %d]\n",
			...
			log_dbg
			(SCN, TRACE, "RnR (IE Length:%d)into list(%d)\n",
				rNeighborParam->u2IELen,
				  ...
				  IN struct SW_RFB *prSwRfb)
{
	...
	struct IE_TX_PWR_ENV_FRAME *prTxPwrEnvIE = NULL;
	...
	if (prSwRfb->u2PacketLen < sizeof(struct WLAN_BEACON_FRAME) ||
		...
		>= prSwRfb->pucRecvBuff + prSwRfb->u2RxByteCount)
		return NULL;
	...
	IE_FOR_EACH(pucIE, u2IELength, u2Offset) {
		...
		case ELEM_ID_RESERVED:
		{
			...
			if (IE_ID_EXT(pucIE) == ELEM_EXT_ID_HE_OP) {
				u4Offset = OFFSET_OF(
					struct _IE_HE_OP_T, aucVarInfo);
				if (IE_LEN(pucIE) <
					(u4Offset - ELEM_HDR_LEN)) {
					DBGLOG(SCN, WARN,
						"HE_OP IE_LEN err: %u < %u\n",
						IE_LEN(pucIE),
						(u4Offset - ELEM_HDR_LEN));
					break;
				}
	...
	if ((eHwBand == BAND_6G) && ((prAdapter->fg6GPwrModeForce) != TRUE)) {
		...
		if (fgPwrMode6GSupport == FALSE) {
			DBGLOG(SCN, WARN, "Skip scan, BSSID["MACSTR
				"] SSID:%s non support 6G pwr mode[%d]",
				MAC2STR(prWlanBeaconFrame->aucBSSID),
				SSID2STR(rSsid.aucSsid, rSsid.u4SsidLen),
				e6GPwrModeCurr);
			return NULL;
	...
	IE_FOR_EACH(pucIE, u2IELength, u2Offset) {
		...
		case ELEM_ID_TIM:
			if (TIM_IE(pucIE)->ucLength < 3) {
				DBGLOG(SCN, WARN, "TIM IE_LEN err(%u)!\n",
						TIM_IE(pucIE)->ucLength);
				break;
			}
...
#if (CFG_SUPPORT_802_11V_MBSSID == 1)
		case ELEM_ID_MBSSID:
			if (MBSSID_IE(pucIE)->ucLength < 2) {
				DBGLOG(SCN, TRACE, "MBSSID IE_LEN err(%u)!\n",
						MBSSID_IE(pucIE)->ucLength);
				break;
			}
	...
	/* 4 <4> Update information from HIF RX Header */
	{
		...
		/* 5G Band */
			...
			if (ucIeHtChannelNum >= 32 && ucIeHtChannelNum < 200) {
				/* Receive Beacon/ProbeResp frame
				 * from adjacent channel.
				 */
				if ((ucIeHtChannelNum == ucHwChannelNum)
			...
			if (prBssDesc->ucChannelNum != ucHwChannelNum) {
				...
				/* If driver can not get channel info
				 * from IE_HE_OP, update RCPI and ChNum
				 */
	...
	if (prTxPwrEnvIE) {
		...
		/* Update TPE power */
		rlmTxPwrEnvMaxPwrUpdate(prAdapter, prBssDesc, eHwBand,
					prTxPwrEnvIE);
	}
#endif  /* CFG_SUPPORT_6G_PWR_REGS == 1 */
	...
	if (prCountryIE && prCountryIE->ucLength ==
		...
		int8_t cNewPwrLimit;
		uint8_t ucChnlOfst =
			(eHwBand == BAND_2G4) ? CHNL_SPAN_5 : CHNL_SPAN_20;
			   ...
			   IN struct SW_RFB *prSwRfb)
{
	...
	log_dbg(SCN, LOUD, "ind %s %d %d\n",
		SSID2STR(prBssDesc->aucSSID, prBssDesc->ucSSIDLen),
				       ...
				       IN struct SW_RFB *prSwRfb)
{
	...
	if ((prSwRfb->u2PacketLen < prSwRfb->u2HeaderLen) ||
		...
		+ CAP_INFO_FIELD_LEN) || prSwRfb->u2HeaderLen !=
		sizeof(struct WLAN_MAC_HEADER)) {
	...
	if (u2TxFrameCtrl == MAC_FRAME_PROBE_RSP) {
		DBGLOG(SCN, INFO, "receive probe RSP\n");
		glCustomSendToUplayer((uint8_t *)prSwRfb->pvHeader,
			prSwRfb->u2PacketLen);
	}
#endif
	...
	for (u4Idx = 0; u4Idx < KAL_AIS_NUM; u4Idx++) {
...
#endif
			if (IS_AIS_ROAMING(prAdapter, u4Idx)) {
				...
				DBGLOG(SCN, TRACE,
				"Ignore security mode detection when roaming\n");
			} else if (rsnCheckSecurityModeChanged(prAdapter,
		...
		if ((prBssDesc->fgIsTxPwrEnvPresent) &&
			...
			(prBssDesc->e6GPwrMode < PWR_MODE_6G_LPI)) {
			/* Reset TPE change flag at first */
			prBssDesc->fgIsTxPwrEnvChange = FALSE;

			DBGLOG(RLM, INFO,
				"[%d][RLM] Update_TPE (" MACSTR ")\n", __LINE__,
				MAC2STR(prBssDesc->aucBSSID));

			/* Update TPE power limit */
			rlmSendTpeLimit(
				prAdapter,
				prBssDesc->eBand,
				prBssDesc->ucChannelNum,
	...
	IN struct ADAPTER *prAdapter, IN uint8_t ucBssIndex)
{
	...
	log_dbg(SCN, INFO, "SEARCH: Bss Num: %d, Look for SSID: %s, "
		...
		SSID2STR(prConnSettings->aucSSID, prConnSettings->ucSSIDLen),
	...
	LINK_FOR_EACH_ENTRY(
...
#endif
#endif
		/* bypass BssDesc timeout check only if
		 * roaming enhance is enabled and
		 * bss is under roaming condition
		 */
		if (CHECK_FOR_TIMEOUT(rCurrentTime, prBssDesc->rUpdateTime,
			...
			&& (!prAdapter->prGlueInfo->u4RoamingEnhanceEnable
			|| !IS_AIS_ROAMING(prAdapter, ucBssIndex))
	...
	if (SpecificprBssDesc) {
		{
			...
			if (!scanCheckBssIsLegal(prAdapter,
				...
				log_dbg(SCN, TRACE,
					...
					SSID2STR(SpecificprBssDesc->aucSSID,
					SpecificprBssDesc->ucSSIDLen),
...
void scanCheckEpigramVhtIE(IN uint8_t *pucBuf, IN struct BSS_DESC *prBssDesc)
{
	...
	if (prEpiIE->ucLength <= (uint8_t)(OFFSET_OF(
		struct IE_VENDOR_EPIGRAM_IE, pucData) - ELEM_HDR_LEN)) {
		DBGLOG(RLM, TRACE,
			"[Epigram] VHT length is invalid(%u), skip!\n",
			prEpiIE->ucLength);
		return;
	}

	u2IELength = prEpiIE->ucLength -
		(uint8_t)(OFFSET_OF(struct IE_VENDOR_EPIGRAM_IE, pucData)
		- ELEM_HDR_LEN);
	...
	IN enum ENUM_BAND eHwBand)
{
	...
	if (HE_IS_6G_OP_INFOR_PRESENT(prHeOp->ucHeOpParams) &&
		...
		if (IE_LEN(pucIE) < (u4Offset - ELEM_HDR_LEN)
	...
	log_dbg(SCN, TRACE, "HE6G_OPINFOR:%u,%u,%u,%u,%u,%u,%u,%u,%u,%u\n",
		prBssDesc->ucChannelNum,
		(uint8_t)pr6gOperInfor->rControl.bits.ChannelWidth,
		prBssDesc->eChannelWidth,
		pr6gOperInfor->ucChannelCenterFreqSeg0,

...
static void aisScanAddHtCapIE(struct ADAPTER *prAdapter,
	struct MSG_SCN_SCAN_REQ_V2 *prCmdScanReq);

#if (CFG_SUPPORT_802_11AC == 1)
static void aisScanAddVhtCapIE(struct ADAPTER *prAdapter,
	struct MSG_SCN_SCAN_REQ_V2 *prCmdScanReq);
#endif /* CFG_SUPPORT_802_11AC */

#if (CFG_SUPPORT_802_11AX == 1) && (CFG_SUPPORT_WIFI_6G == 1)
static void aisScanAddHeCapIE(struct ADAPTER *prAdapter,
	struct MSG_SCN_SCAN_REQ_V2 *prCmdScanReq);
#endif /* CFG_SUPPORT_802_11AX && CFG_SUPPORT_WIFI_6G */
...
void aisFsmInit(IN struct ADAPTER *prAdapter, uint8_t ucBssIndex)
{
	...
	prAisFsmInfo->fgIsStaRoaming = FALSE;
#endif /* CFG_SUPPORT_ROAMING */
	...
	cnmTimerInitTimer(prAdapter,
		&prAisFsmInfo->rDelayCarrierOffTimer,
		(PFN_MGMT_TIMEOUT_FUNC) aisFsmRunEventDelayCarrierOffTimeOut,
		(unsigned long)ucBssIndex,
		TIMER_WAKELOCK_AUTO);
	prAisSpecificBssInfo->fgDelayReportCarrierOff = FALSE;
#if CFG_SUPPORT_802_11W
	cnmTimerInitTimer(prAdapter,
		&prAisFsmInfo->rPmfWaitReconnectTimer,
		(PFN_MGMT_TIMEOUT_FUNC) aisFsmRunEventPmfWaitReconnectTimeout,
		(unsigned long)ucBssIndex,
		TIMER_WAKELOCK_AUTO);
	prAisSpecificBssInfo->fgIgnorSaQueryReq = FALSE;
#endif
#endif
...
#if CFG_MTK_SUPPLICANT_ROAMING_ENHANCE
	cnmTimerStopTimer(prAdapter, &prAisFsmInfo->rDelayCarrierOffTimer);
	prAisSpecificBssInfo->fgDelayReportCarrierOff = FALSE;
#if CFG_SUPPORT_802_11W
	cnmTimerStopTimer(prAdapter, &prAisFsmInfo->rPmfWaitReconnectTimer);
	prAisSpecificBssInfo->fgIgnorSaQueryReq = FALSE;
#endif
#endif
		...
		struct BSS_DESC *prBssDesc, uint8_t ucBssIndex)
{
	...
	if (prAisBssInfo->eConnectionState == MEDIA_STATE_DISCONNECTED
		...
		prAisFsmInfo->ucAvailableAuthTypes =
			(uint8_t) prWpaInfo->u4AuthAlg;
		DBGLOG(AIS, INFO,
			"JOIN INIT: Auth Algorithm for Roaming:%d\n",
			prAisFsmInfo->ucAvailableAuthTypes);
#else
	...
	enum ENUM_AIS_STATE eNextState, uint8_t ucBssIndex)
{
	...
	do {
		...
		if (prAisFsmInfo->ePreviousState !=
			...
			prAisFsmInfo->fgScanTryCount = 0;
#endif

#if (CFG_SUPPORT_ROAMING == 1)
			if (prAisFsmInfo->fgIsStaRoaming)
				prAisFsmInfo->fgIsStaRoaming = FALSE;
#endif /* CFG_SUPPORT_ROAMING */
...
#if (CFG_MTK_SUPPLICANT_ROAMING_ENHANCE == 1)
				/* check if we are under roaming condition */
				if (prAisBssInfo->eConnectionState ==
					MEDIA_STATE_CONNECTED)
					roamingFsmRunEventRoam(prAdapter,
					prAisBssInfo->ucBssIndex);
#endif /* CFG_MTK_SUPPLICANT_ROAMING_ENHANCE  */
			...
			if (prAisBssInfo->eConnectionState ==
				...
				if (prAisFsmInfo->ucConnTrialCount >
					...
					COPY_MAC_ADDR(prConnSettings->aucBSSID,
						aisGetCurrBssId(
						prAdapter,
						ucBssIndex)->arMacAddress);
					DBGLOG(INIT, INFO,
						"Keep Normal_TR state\n");
#endif
		...
		case AIS_STATE_LOOKING_FOR:
...
#if (CFG_SUPPORT_WIFI_6G == 1)
			/* Check if chip support 6G or not */
			...
			&& (prAdapter->rWifiVar.ucChannelMinDwellTime >= 8)
			&& (prAdapter->rWifiVar.ucChannelMaxDwellTime >
				prAdapter->rWifiVar.ucChannelMinDwellTime)) {
			...
			DBGLOG(SCN, TRACE, "SCAN MinDwell[%d],MaxDwell[%d]\n",
				prScanReqMsg->u2ChannelMinDwellTime,
				prScanReqMsg->u2ChannelDwellTime);
			...
			case SCAN_CHANNEL_6G:
				...
				break;
			default:
				DBGLOG(AIS, TRACE, "ScanChannel Type(%u)\n",
					prScanReqMsg->eScanChannel);
				break;
			}
			if (u2ScanIELen > 0) {
			...
			/* Generate Capability IE for each band :
			 * HT Capa IE for 2.4G band
			 * HT Capa IE, VHT Capa IE for 5G band
			 * HE Capa IE for 6G band
			 */
			aisScanAddHtCapIE(prAdapter, prScanReqMsg);
#if (CFG_SUPPORT_802_11AC == 1)
			aisScanAddVhtCapIE(prAdapter, prScanReqMsg);
#endif /* CFG_SUPPORT_802_11AC */
#if (CFG_SUPPORT_802_11AX == 1) && (CFG_SUPPORT_WIFI_6G == 1)
			aisScanAddHeCapIE(prAdapter, prScanReqMsg);
#endif /* CFG_SUPPORT_802_11AX && CFG_SUPPORT_WIFI_6G */
...
#if (CFG_SUPPORT_AUTO_SCC == 1)
			if ((prWifiVar->ucP2pGoAutoSCC == FEATURE_ENABLED) &&
		...
		case AIS_STATE_GO_SYNC_CHANNEL:
			...
			if (cnmSCCAutoSwitchMode(prAdapter,
				prAisFsmInfo->prTargetBssDesc->ucChannelNum)) {
				/* channel switch failed, fall into MCC */
				eNextState = AIS_STATE_GO_SYNC_CH_DONE;
				fgIsTransition = TRUE;
			} else {
				/* start timer
			    ...
			    IN struct MSG_HDR *prMsgHdr)
{
	...
	DBGLOG(AIS, INFO, "ScanDone %u, status(%d) native report(%u) req(%u)\n",
			 ...
			 IN struct MSG_HDR *prMsgHdr)
{
	...
	if (ucReasonOfDisconnect != DISCONNECT_REASON_CODE_ROAMING) {
		...
		if (prAdapter->prAisBssInfo[ucBssIndex] != NULL) {
			kalMemZero(prAisBssInfo->aucSSID,
			   sizeof(prAisBssInfo->aucSSID));
		...
		uint8_t ucBssIndex)
{
	...
	case AIS_STATE_JOIN_FAILURE:
		...
		rlmObssAbortScan(prAdapter, ucBssIndex);
		break;
				...
				IN struct MSG_HDR *prMsgHdr)
{
	...
	if (eNextState == AIS_STATE_NORMAL_TR) {
		DBGLOG(AIS, INFO,
			"stop carrier off timer when reconnect success.\n");
		aisGetAisSpecBssInfo(prAdapter,
			ucBssIndex)->fgDelayReportCarrierOff = FALSE;
		cnmTimerStopTimer(prAdapter,
			&prAisFsmInfo->rDelayCarrierOffTimer);
	}
#endif
...
static enum ENUM_AIS_STATE aisFsmHandleRoamFail(IN struct ADAPTER *prAdapter,
	IN uint8_t ucBssIndex,
	IN struct BSS_DESC *prBssDesc)
{
	struct CONNECTION_SETTINGS *prConnSettings;
...
#if CFG_MTK_SUPPLICANT_ROAMING_ENHANCE
	if (prAdapter->prGlueInfo->u4RoamingEnhanceEnable) {
		kalIndicateJoinFail(
			prAdapter,
			ucBssIndex,
			prBssDesc);
		COPY_MAC_ADDR(
			...
			aisGetCurrBssId(prAdapter, ucBssIndex)->arMacAddress);
		prConnSettings->bss = NULL;
#if (CFG_SUPPORT_ROAMING == 1)
		roamingFsmRunEventFail(prAdapter,
			ROAMING_FAIL_REASON_NOCANDIDATE,
			       ...
			       unsigned long ulParamPtr)
{
	...
	case AIS_STATE_JOIN:
		...
		if (prAisBssInfo->eConnectionState ==
			...
			prAisFsmInfo->prTargetBssDesc->u2JoinStatus =
				STATUS_CODE_AUTH_TIMEOUT;
			kalIndicateJoinFail(prAdapter,
				ucBssIndex,
				prAisFsmInfo->prTargetBssDesc);
			prConnSettings->bss = NULL;
			eNextState = AIS_STATE_NORMAL_TR;
...
#endif /* CFG_SUPPORT_ROAMING */
			DBGLOG(INIT, INFO,
				"Keep Normal_TR state if roaming fail\n");
		} else
#endif
		{
			...
			DBGLOG(AIS, WARN, "set ConnSetting bss NULL\n");
		}
#else
...
#if CFG_MTK_SUPPLICANT_ROAMING_ENHANCE
void aisFsmRunEventDelayCarrierOffTimeOut(IN struct ADAPTER *prAdapter,
				  unsigned long ulParamPtr)
{
	uint8_t ucBssIndex = (uint8_t)ulParamPtr;
	...
	DBGLOG(AIS, INFO, "Carrier Off Timer Timeout\n");
	...
	if (prBssInfo
		...
		DBGLOG(INIT, INFO, "[wifi] %s netif_carrier_off\n",
		       prDevHandler->name);
		aisGetAisSpecBssInfo(prAdapter, ucBssIndex)
			->fgDelayReportCarrierOff = FALSE;
		netif_carrier_off(prDevHandler);
	}
}
#if CFG_SUPPORT_802_11W
void aisFsmRunEventPmfWaitReconnectTimeout(IN struct ADAPTER *prAdapter,
				   unsigned long ulParamPtr)
{
	uint8_t ucBssIndex = (uint8_t)ulParamPtr;

	aisGetAisSpecBssInfo(prAdapter, ucBssIndex)->fgIgnorSaQueryReq = FALSE;
}
#endif
#endif
/*----------------------------------------------------------------------------*/
	...
	struct MSG_SCN_SCAN_REQ_V2 *prCmdScanReq)
{
	...
	uint8_t *pucIE = prCmdScanReq->aucIE;
	uint16_t u2IELength = prCmdScanReq->u2IELen;
	uint16_t u2Offset = 0;

	if (IS_FEATURE_DISABLED(prAdapter->rWifiVar.ucStaHt) ||
		IS_FEATURE_DISABLED(prAdapter->rWifiVar.fgScanAddCapIE))
		return;

	prBssInfo = GET_BSS_INFO_BY_INDEX(prAdapter,
				prCmdScanReq->ucBssIndex);
	if (!IS_BSS_AIS(prBssInfo)) {
		DBGLOG(AIS, TRACE, "Bss[%u] is not AIS\n",
			prCmdScanReq->ucBssIndex);
		return;
	}

	/* if IE has already existed, skip generate HT cap IE */
	IE_FOR_EACH(pucIE, u2IELength, u2Offset) {
		if (IE_ID(pucIE) == ELEM_ID_HT_CAP)
			return;
	}

	/* if no space left, skip generate HT cap IE */
	if ((sizeof(prCmdScanReq->aucIE) - u2IELength)
		< sizeof(struct IE_HT_CAP))
		return;

	/* alloc buffer to fill HT cap IE */
	prMsduInfo = cnmMgtPktAlloc(prAdapter, sizeof(struct IE_HT_CAP));
	if (prMsduInfo == NULL)
		return;

	prMsduInfo->ucBssIndex = prBssInfo->ucBssIndex;
	prMsduInfo->ucStaRecIndex = CFG_STA_REC_NUM;
	prMsduInfo->u2FrameLength = 0;

	/* Generate HT capability IE :
	 * Currently, we generate the capability IE for the probe request
	 * without considering the eBand, ucPrimaryChannel, or
	 * TargetBssDesc in BssInfo.
	 */
	rlmReqGenerateHtCapIE(prAdapter, prMsduInfo);

	/* copy rlm ie */
	kalMemCopy(prCmdScanReq->aucIE + prCmdScanReq->u2IELen,
			(uint8_t *) prMsduInfo->prPacket,
			prMsduInfo->u2FrameLength);
	prCmdScanReq->u2IELen += prMsduInfo->u2FrameLength;

	DBGLOG(AIS, TRACE,
			"Add HT Cap IE, IELen[%u], FrameLen[%u]\n",
	...
	struct MSG_SCN_SCAN_REQ_V2 *prCmdScanReq)
{
	...
	if (IS_FEATURE_DISABLED(prAdapter->rWifiVar.ucStaVht) ||
		IS_FEATURE_DISABLED(prAdapter->rWifiVar.fgScanAddCapIE))
		return;
	...
	/* if IE has already existed, skip generate VHT cap IE */
	IE_FOR_EACH(pucIE, u2IELength, u2Offset) {
		if (IE_ID(pucIE) == ELEM_ID_VHT_CAP)
			return;
	}

	/* if no space left, skip generate VHT cap IE */
	if ((sizeof(prCmdScanReq->aucIE) - u2IELength)
		< sizeof(struct IE_VHT_CAP))
		return;

	/* alloc buffer to fill VHT cap IE */
	prMsduInfo = cnmMgtPktAlloc(prAdapter, sizeof(struct IE_VHT_CAP));
	if (prMsduInfo == NULL)
		return;
	...
	/* Generate VHT capability IE
	 * Currently, we generate the capability IE for the probe request
	 * without considering the eBand, ucPrimaryChannel, or
	 * TargetBssDesc in BssInfo.
	 */
	rlmProbReqGenerateVhtCapIE(prAdapter, prMsduInfo);
	...
	DBGLOG(AIS, TRACE,
			"Add VHT Cap IE, IELen[%u], FrameLen[%u]\n",
			prCmdScanReq->u2IELen,
			prMsduInfo->u2FrameLength);
	...
	struct MSG_SCN_SCAN_REQ_V2 *prCmdScanReq)
{
	...
	if (IS_FEATURE_DISABLED(prAdapter->rWifiVar.ucStaHe) ||
		IS_FEATURE_DISABLED(prAdapter->rWifiVar.fgScanAddCapIE))
		return;
	...
	/* if IE has already existed, skip generate HE cap IE */
	IE_FOR_EACH(pucIE, u2IELength, u2Offset) {
		if (IE_ID(pucIE) == ELEM_ID_RESERVED &&
			(IE_ID_EXT(pucIE) == ELEM_EXT_ID_HE_CAP ||
			IE_ID_EXT(pucIE) == ELEM_EXT_ID_HE_6G_BAND_CAP))
			return;
	}

	/* calculate IE len */
	len = heRlmCalculateHeCapIELen(prAdapter, prBssInfo->ucBssIndex, NULL);
	len += sizeof(struct _IE_HE_6G_BAND_CAP_T);

	/* if no space left, skip generate HE cap IE */
	if ((sizeof(prCmdScanReq->aucIE) - u2IELength) < len)
		return;

	/* alloc buffer to fill HE cap IE */
	prMsduInfo = cnmMgtPktAlloc(prAdapter, len);
	if (prMsduInfo == NULL)
		return;
	...
	/* Generate HE capability IE :
	 * Currently, we generate the capability IE for the probe request
	 * without considering the eBand, ucPrimaryChannel, or
	 * TargetBssDesc in BssInfo.
	 */
	heRlmReqGenerateHeCapIE(prAdapter, prMsduInfo);
	heRlmProbReqGenerateHe6gBandCapIE(prAdapter, prMsduInfo);

	/* if FrameLength is too long, skip copy rlm ie */
	if (prMsduInfo->u2FrameLength > len) {
		DBGLOG(AIS, TRACE, "FrameLength[%u] > len[%u]\n",
	...
	DBGLOG(AIS, TRACE,
			"Add HE Cap IE, IELen[%u], FrameLen[%u]\n",
			prCmdScanReq->u2IELen,
			prMsduInfo->u2FrameLength);
			   ...
			   IN struct MSG_HDR *prMsgHdr)
{
	...
	DBGLOG(AIS, EVENT,
		"ChGrantEvent : StaRecOfAP[%d], fgIsAisSwitchingChnl[%d]\n",
			prAisBssInfo->prStaRecOfAP != NULL ? 1 : 0,
			prAisBssInfo->fgIsAisSwitchingChnl);
		   ...
		   IN uint8_t ucBssIndex)
{
	...
	do {
		...
		/* Check whether the frame is BMC management frame or not. */
		if (prStaRec == NULL &&
		    IS_BMCAST_MAC_ADDR(prWlanHdr->aucAddr1))
...
static void aisRemoveDisappearedBlacklist(struct ADAPTER *prAdapter)
{
	...
	LINK_FOR_EACH_ENTRY_SAFE(prEntry, prNextEntry, prBlackList, rLinkEntry,
		...
		if (!fgDisappeared || (u8Current - prEntry->u4DisapperTime) <
		    600 * USEC_PER_SEC)
			continue;
			  ...
			  uint8_t ucBssIndex)
{
	...
	for (c2BufLen = (int16_t)u2ApBufLen; c2BufLen >=
		...
		/* add boundary check c2ValidIELen for fixing fuzz issue */
...
void aisPreSuspendFlow(IN struct ADAPTER *prAdapter)
{
	...
	for (ucIdx = 0; ucIdx < prAdapter->ucHwBssIdNum; ucIdx++)
		rlmObssAbortScan(prAdapter, ucIdx);
	/* cancel scan */
	...
	for (ucIdx = AIS_DEFAULT_INDEX; ucIdx < KAL_AIS_NUM; ucIdx++) {
...
#if (CFG_SUPPORT_4ADDR_STATION == 0)
		if ((IS_FEATURE_ENABLED(prWifiVar->ucWow) &&
			...
			&& prAisBssInfo->eConnectionState ==
			   MEDIA_STATE_CONNECTED)
#else /* CFG_SUPPORT_4ADDR_STATION == 1 */
		if (((IS_FEATURE_ENABLED(prWifiVar->ucWow) &&
			IS_FEATURE_DISABLED(prAdapter->rWowCtrl.fgWowEnable) &&
			IS_FEATURE_DISABLED(prWifiVar->ucAdvPws))
			|| prAisBssInfo->ucWdsEnable)
			&& prAisBssInfo->eConnectionState ==
			   MEDIA_STATE_CONNECTED)
				...
				struct BSS_INFO *prBss)
{
	...
	if (!prAdapter || !prAisFsmInfo || !prBss) {
		DBGLOG(AIS, ERROR, "Invalid input argument\n");
		return;
	}

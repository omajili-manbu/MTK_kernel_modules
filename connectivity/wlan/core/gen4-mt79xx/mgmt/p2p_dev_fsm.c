...
static uint8_t *apucDebugP2pDevState[P2P_DEV_STATE_NUM] = {
	...
	(uint8_t *) DISP_STRING("P2P_DEV_STATE_NAN_CONCURRENT_LISTEN")
};
...
uint8_t p2pDevFsmInit(IN struct ADAPTER *prAdapter)
{
	...
	do {
		...
		cnmTimerInitTimer(prAdapter,
			...
			(PFN_MGMT_TIMEOUT_FUNC) p2pDevNanMapUpdatedDelayTimeout,
			(unsigned long) prP2pDevFsmInfo,
			TIMER_WAKELOCK_AUTO);
		...
		IN enum ENUM_P2P_DEV_STATE eNextState)
{
	...
	do {
		...
		case P2P_DEV_STATE_OFF_CHNL_TX:
			...
			break;
		case P2P_DEV_STATE_NAN_CONCURRENT_LISTEN:
			if (!fgIsLeaveState) {
				fgIsLeaveState =
					p2pDevStateInit_NAN_CONCURRENT_LISTEN(
					prAdapter,
					prP2pDevFsmInfo->ucBssIndex,
					prP2pDevFsmInfo,
					&(prP2pDevFsmInfo->rChnlReqInfo),
				...
				p2pDevStateAbort_NAN_CONCURRENT_LISTEN(
					prAdapter,
					prP2pDevFsmInfo,
					eNextState);
			}
		...
		IN struct MSG_HDR *prMsgHdr)
{
	...
	do {
		...
		prScanReqInfo->u2PassiveDwellTime = 0;
		...
		IN struct MSG_HDR *prMsgHdr)
{
	...
	do {
		...
		} else if (prP2pDevFsmInfo->fgIsP2pListenConcurrent) {
			DBGLOG(P2P, INFO,
			       "Early Leave p2p listen with NAN concurrent\n");
		...
		IN struct MSG_MGMT_TX_REQUEST *prMgmtTxMsg)
{
	...
	case P2P_DEV_STATE_NAN_CONCURRENT_LISTEN:
		DBGLOG(P2P, WARN,
			"do not serve off channel tx in this state\n");
		goto error;
...
#if (CFG_NAN_P2P_CONCURRENT_SUPPORT == 1)
void p2pDevNanMapUpdatedDelayTimeout(IN struct ADAPTER *prAdapter,
		IN unsigned long ulParamPtr)
{
	struct P2P_DEV_FSM_INFO *prP2pDevFsmInfo =
		(struct P2P_DEV_FSM_INFO *) ulParamPtr;
	...
	DBGLOG(P2P, INFO,
		"NAN has applied the chnl or NAN handle timeout\n");
	p2pDevFsmStateTransition(prAdapter,
		prP2pDevFsmInfo,
		P2P_DEV_STATE_CHNL_ON_HAND);
}

void p2pDevFsmRunEventChnlMapUpdated(struct ADAPTER *prAdapter,
		struct MSG_HDR *prMsgHdr)
{
	...
	do {
		...
		if (prP2pDevFsmInfo->fgIsP2pListenConcurrent == FALSE) {
			DBGLOG(P2P, INFO, "P2P is not listening\n");
			break;
		}
		...
		if (timerPendingTimer(
			&(prP2pDevFsmInfo->rP2pListenNanUpdatedTimer))) {
			DBGLOG(P2P, INFO,
				"NAN Timeline Map updated. Stop Timer\n");
			cnmTimerStopTimer(prAdapter,
				&(prP2pDevFsmInfo->rP2pListenNanUpdatedTimer));
		}
...
uint32_t p2pDevFsmSetParam(struct ADAPTER *prAdapter,
	...
	uint8_t fgSet)
{
	...
	struct CMD_P2P_DEV_SET_PARAMS *prCmdP2pDevSetParams;

	prCmdP2pDevSetParams = (struct CMD_P2P_DEV_SET_PARAMS *)
		cnmMemAlloc(prAdapter, RAM_TYPE_MSG,
			sizeof(struct CMD_P2P_DEV_SET_PARAMS));

	if (!prCmdP2pDevSetParams) {
		DBGLOG(P2P, ERROR, "Memory allocation fail\n");
		goto done;
	}

	prCmdP2pDevSetParams->ucBssIndex = P2P_DEV_BSS_INDEX;
	prCmdP2pDevSetParams->ucRfBand = (uint8_t)eBand;
	prCmdP2pDevSetParams->ucDBDCBand = (uint8_t)eDBDCBand;
	prCmdP2pDevSetParams->fgSet = fgSet;
	prCmdP2pDevSetParams->ucChannel = ucChannel;

	/* send command to fw */
	wlanSendSetQueryCmd(prAdapter,
	    CMD_ID_SET_P2P_DEV, /* ucCID */
	    TRUE, /* fgSetQuery */
	    FALSE, /* fgNeedResp */
	    FALSE, /* fgIsOid */
	    NULL, /* pfCmdDoneHandler */
	    nicCmdTimeoutCommon, /* pfCmdTimeoutHandler */
	    sizeof(struct CMD_P2P_DEV_SET_PARAMS), /* u4SetQueryInfoLen */
	    (uint8_t *) prCmdP2pDevSetParams, /* pucInfoBuffer */
	    NULL,  /* pvSetQueryBuffer */
	    0); /* u4SetQueryBufferLen */

	cnmMemFree(prAdapter, prCmdP2pDevSetParams);

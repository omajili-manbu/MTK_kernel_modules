	...
	uint8_t *pPPEThreshold)
{
	...
	int8_t nss = wlanGetSupportNss(prAdapter, prBssInfo->ucBssIndex) - 1;
	uint8_t ucSupportedNss = (nss > 0) ? nss : 0;
	uint8_t ucMaxBw =
...
void heRlmProbReqGenerateHe6gBandCapIE(
	struct ADAPTER *prAdapter,
	struct MSDU_INFO *prMsduInfo)
{
...
#if CFG_SUPPORT_BFEE
	nss = wlanGetSupportNss(prAdapter, prBssInfo->ucBssIndex) - 1;
	ucSupportedNss = (nss > 0) ? nss : 0;
#endif

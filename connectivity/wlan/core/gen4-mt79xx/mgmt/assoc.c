...
#if CFG_SUPPORT_802_11W
static void handlePmfReject(IN struct ADAPTER *prAdapter,
	IN struct STA_RECORD *prStaRec,
	IN struct SW_RFB *prSwRfb,
	IN struct WLAN_ASSOC_RSP_FRAME *prAssocRspFrame)
{
	...
	struct AIS_FSM_INFO *prAisFsmInfo =
		aisGetAisFsmInfo(prAdapter, prStaRec->ucBssIndex);
#endif

	u2IELength = (uint16_t) ((prSwRfb->u2PacketLen -
			prSwRfb->u2HeaderLen) -
			(OFFSET_OF(struct WLAN_ASSOC_RSP_FRAME, aucInfoElem) -
			WLAN_MAC_MGMT_HEADER_LEN));
			...
			if (pucTime[0] ==
				...
				if (prStaRec->eStaType ==
					...
					cnmTimerStartTimer(prAdapter,
						&(prAisFsmInfo->
						rPmfWaitReconnectTimer),
						TU_TO_MSEC(tu) +
						PMF_WAIT_RECONNECT_MARGIN);
				}
#endif
			}
			break;
		...
		IN uint32_t u4SetBufferLen, IN uint8_t ucBssIndex)
{
	...
	uint32_t u4DisconnectReason = DISCONNECT_REASON_CODE_NEW_CONNECTION;
	struct CONNECTION_SETTINGS *prConnSettings;
...
#if CFG_MTK_SUPPLICANT_ROAMING_ENHANCE
		if (u4DisconnectReason ==
			DISCONNECT_REASON_TEMPORARY_FOR_ROAM) {
			u4DisconnectReason =
				DISCONNECT_REASON_CODE_NEW_CONNECTION;
			aisGetAisSpecBssInfo(prAdapter, ucBssIndex)->
				fgDelayReportCarrierOff = TRUE;
		}
#endif
		prAisAbortMsg->ucReasonOfDisconnect =
			u4DisconnectReason;
	...
	IE_FOR_EACH(pucIE, u2IELength, u2Offset) {
		...
		case ELEM_ID_RSN:
			...
			if (prAdapter->fgIsP2PRegistered
				...
				rsnParserCheckForRSNCCMPPSK(prAdapter,
				RSN_IE(pucIE), prStaRec, &u2StatusCode);
		...
		case ELEM_ID_VENDOR:
...
#if CFG_ENABLE_WIFI_DIRECT
			{
				if ((prAdapter->fgIsP2PRegistered)) {
					...
					rsnParseCheckForWFASpecificElem

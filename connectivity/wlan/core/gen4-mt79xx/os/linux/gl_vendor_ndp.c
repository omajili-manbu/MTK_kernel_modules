...
const struct nla_policy
	mtk_wlan_vendor_ndp_policy[MTK_WLAN_VENDOR_ATTR_NDP_PARAMS_MAX + 1] = {
			...
			[MTK_WLAN_VENDOR_ATTR_NDP_PUB_ID] = {
				.type = NLA_U16 },
	};
...
nanNdiCreateRspEvent(struct ADAPTER *prAdapter,
	...
	prDevHandler = wlanGetNetDev(prAdapter->prGlueInfo, AIS_DEFAULT_INDEX);
	if (!prDevHandler) {
...
nanNdpInitiatorReqHandler(struct GLUE_INFO *prGlueInfo, struct nlattr **tb) {
...
#if (CFG_SUPPORT_NAN_REDIRECT_VENDOR_CMD == 0)
	...
	if (tb[MTK_WLAN_VENDOR_ATTR_NDP_CONFIG_SECURITY]) {
		if (tb[MTK_WLAN_VENDOR_ATTR_NDP_CSID])
			kalMemCpyS(cipher_suite_list,
			   ...
			   sizeof(cipher_suite_list));
		prNanCmdDataRequest->ucSecurity = cipher_suite_list[0];

		/* AP NAN TODO */
		if (prNanCmdDataRequest->ucSecurity ==
			NAN_CIPHER_SUITE_ID_NCS_PK_2WDH_128 ||
			prNanCmdDataRequest->ucSecurity ==
				NAN_CIPHER_SUITE_ID_NCS_PK_2WDH_256 ||
			prNanCmdDataRequest->ucSecurity ==
				NAN_CIPHER_SUITE_ID_NCS_GTK_CCM_128 ||
			prNanCmdDataRequest->ucSecurity ==
				NAN_CIPHER_SUITE_ID_NCS_GTK_GCM_256) {
			DBGLOG(NAN, ERROR,
				"[Data req] cipher_suite_id: %u not supported!",
				prNanCmdDataRequest->ucSecurity);
			kalMemFree(prNanCmdDataRequest,
				VIR_MEM_TYPE,
				sizeof(struct _NAN_CMD_DATA_REQUEST));
		...
		prNanCmdDataRequest->ucGtkCipher = cipher_suite_list[1];
		/* AP NAN TODO:
		 * no hook up ndp request yet
		 */
#endif
		DBGLOG(NAN, INFO, "Cipher:%u, GTKCipher:%u\n",
			prNanCmdDataRequest->ucSecurity,
	...
	/* Peer mac Addr */
	if (tb[MTK_WLAN_VENDOR_ATTR_NDP_PEER_DISCOVERY_MAC_ADDR]) {
		kalMemCpyS(prNanCmdDataRequest->aucResponderDataAddress,
			sizeof(prNanCmdDataRequest->aucResponderDataAddress),
			nla_data(
			tb[MTK_WLAN_VENDOR_ATTR_NDP_PEER_DISCOVERY_MAC_ADDR]),
			MAC_ADDR_LEN);
	}
	...
	if (tb[MTK_WLAN_VENDOR_ATTR_NDP_MULTICAST_ADDR]) {
		kalMemCpyS(prNanCmdDataRequest->aucMulticastAddress,
			sizeof(prNanCmdDataRequest->aucMulticastAddress),
			nla_data(
...
nanNdpResponderReqHandler(struct GLUE_INFO *prGlueInfo, struct nlattr **tb) {
	...
	struct ADAPTER *prAdapter = NULL;
#if (CFG_SUPPORT_NAN_REDIRECT_VENDOR_CMD == 1)
	uint32_t u4BufLen = 0;
#endif
	...
	if (tb[MTK_WLAN_VENDOR_ATTR_NDP_PUB_ID]) {
		prNanCmdDataResponse->u2PublishId = nla_get_u16(
			tb[MTK_WLAN_VENDOR_ATTR_NDP_PUB_ID]);
		DBGLOG(NAN, INFO, "Get publish_id =%u\n",
			prNanCmdDataResponse->u2PublishId);
	}
	...
	if (tb[MTK_WLAN_VENDOR_ATTR_NDP_CSID]) {
		...
		kalMemCpyS(cipher_suite_list,
			...
			tb[MTK_WLAN_VENDOR_ATTR_NDP_CSID]),
			sizeof(cipher_suite_list));
		prNanCmdDataResponse->ucSecurity = cipher_suite_list[0];
		...
		if (prNanCmdDataResponse->ucSecurity ==
			...
			prNanCmdDataResponse->ucSecurity ==
				NAN_CIPHER_SUITE_ID_NCS_GTK_GCM_256) {
			DBGLOG(NAN, ERROR,
				"[Data Resp] cipher_suite_id: %u not supported!",
				prNanCmdDataResponse->ucSecurity);
			goto exit;
		}
#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_FRAME_PROT == 1)
		if ((prInstance != NULL) && prInstance->fgGtkRequired)
			prNanCmdDataResponse->ucGtkCipher =
				nanSecGetGtkCipherByNdi(NAN_NDI_INDEX_0);
#endif
		DBGLOG(NAN, INFO, "Cipher:%u, GTKCipher:%u\n",
			prNanCmdDataResponse->ucSecurity,
			prNanCmdDataResponse->ucGtkCipher);
	...
	DBGLOG(NAN, INFO, "aucInitiatorDataAddress = " MACSTR "\n",
	...
	/* Multicast Address */
	if (tb[MTK_WLAN_VENDOR_ATTR_NDP_MULTICAST_ADDR]) {
		kalMemCpyS(prNanCmdDataResponse->aucMulticastAddress,
			sizeof(prNanCmdDataResponse->aucMulticastAddress),
			nla_data(
			tb[MTK_WLAN_VENDOR_ATTR_NDP_MULTICAST_ADDR]),
			MAC_ADDR_LEN);
	}
...
#else
		rStatus = kalIoctl(prGlueInfo,
					wlanoidSetNanDataEnd,
					&rNanCmdDataEnd,
					sizeof(rNanCmdDataEnd),
					FALSE, FALSE, TRUE,
					&u4BufLen);
#endif
	}
...
nanNdpDataConfirmEvent(IN struct ADAPTER *prAdapter,
	...
	if (prNDP == NULL) {
		DBGLOG(NAN, ERROR, "prNDP is NULL\n");
		return WLAN_STATUS_INVALID_DATA;
	}
	...
	if (prNDP->fgCarryIPV6)
		DBGLOG(NAN, INFO, "fgCarryIPV6 = %d\n",
		prNDP->aucRspInterfaceId);
...
nanNdpDataTerminationEvent(IN struct ADAPTER *prAdapter,
	...
	prWdev = prDevHandler->ieee80211_ptr;
...
mtk_cfg80211_vendor_ndp(struct wiphy *wiphy, struct wireless_dev *wdev,
	...
	DBGLOG(NAN, INFO, "NDP CMD Type %d\n",
		u4NdpCmdType);

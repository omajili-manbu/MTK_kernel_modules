...
struct NanP2pState g_rCurrNanP2pState; /* TODO: need extract from P2P FSM */
uint8_t g_fgManualWfdState = FALSE;
/*******************************************************************************
 *                                 M A C R O S
 *******************************************************************************
 */
		     ...
		     struct NanSdeaCtrlParams *prNanSdeaCtrlParms)
{
	...
	prNanSdeaCtrlParms->fgDataType = GET_SDEA_DATA_PATH_TYPE(*pIndata);
	...
	DBGLOG(NAN, INFO,
	       ...
	       prNanSdeaCtrlParms->config_nan_data_path,
	       prNanSdeaCtrlParms->fgDataType,
		   prNanSdeaCtrlParms->security_cfg);
	DBGLOG(NAN, INFO,
	...
	DBGLOG(NAN, INFO, "[%s]fgGAS: %u, fgQoS: %u, fgRangeLimit: %u\n",
	       ...
	       prNanSdeaCtrlParms->fgRangeLimit);
#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_FRAME_PROT == 1)
	DBGLOG(NAN, INFO, "fgGtkRequired: %u\n",
	       prNanSdeaCtrlParms->fgGtkRequired);
#endif
}
...
struct wfpal_channel nanCastChToVendorFormat(
		struct RF_CHANNEL_INFO rVendorChInfo,
		enum ENUM_CHNL_EXT eSco)
{
	struct wfpal_channel rCastedVendorChInfo = {0};
	uint32_t defaultBw = 0;

	rCastedVendorChInfo.channel = rVendorChInfo.ucChannelNum;
	switch (rVendorChInfo.eBand) {
	case BAND_2G4:
		rCastedVendorChInfo.flags = NAN_C_FLAG_2GHZ;
		defaultBw = NAN_C_FLAG_20MHZ;
		break;
	case BAND_5G:
		rCastedVendorChInfo.flags = NAN_C_FLAG_5GHZ;
		defaultBw = NAN_C_FLAG_80MHZ;
		break;
#if (CFG_SUPPORT_WIFI_6G == 1)
	case BAND_6G:
		rCastedVendorChInfo.flags = NAN_C_FLAG_6GHZ;
		defaultBw = NAN_C_FLAG_80MHZ;
		break;
	...
	switch (rVendorChInfo.ucChnlBw) {
	case CW_20_40MHZ:
		if (eSco == CHNL_EXT_SCN) {
			rCastedVendorChInfo.flags |= NAN_C_FLAG_20MHZ;
		} else if (eSco == CHNL_EXT_SCA) {
			rCastedVendorChInfo.flags |= NAN_C_FLAG_40MHZ;
			rCastedVendorChInfo.flags |= NAN_C_FLAG_EXTENSION_ABOVE;
		} else if (eSco == CHNL_EXT_SCB) {
			rCastedVendorChInfo.flags |= NAN_C_FLAG_40MHZ;
		}
		break;
	case CW_80MHZ:
		rCastedVendorChInfo.flags |= NAN_C_FLAG_80MHZ;
		break;
	case CW_160MHZ:
		rCastedVendorChInfo.flags |= NAN_C_FLAG_160MHZ;
		break;
	default:
		DBGLOG(REQ, ERROR, "Unknown Channel Bw: %d\n",
			rVendorChInfo.ucChnlBw);
		rCastedVendorChInfo.flags |= defaultBw;
		break;
	}
	return rCastedVendorChInfo;
}
void
nanAbortOngoingScan(struct ADAPTER *prAdapter)
{
	...
	DBGLOG(NAN, STATE, "Abort ongoing SCAN due to NAN on!\n");
...
uint32_t
nanAbortScan(struct ADAPTER *prAdapter)
{
	...
	if (!prAdapter->rWifiVar.fgNanOnAbortScan) {
		DBGLOG(NAN, INFO, "Not support NAN on abort scan\n");
		return WLAN_STATUS_SUCCESS;
	}

	return kalIoctl(prAdapter->prGlueInfo,
					nanOidAbortOngoingScan,
					NULL,
					0,
					FALSE, FALSE, TRUE,
					&u4SetInfoLen);
	...
	struct wireless_dev *wdev, const void *data, int data_len)
{
	...
	case NAN_MSG_ID_ENABLE_REQ: {
		uint32_t rStatus = 0;
		struct NanEnableRequest nanEnableReq = {0};
		struct NanEnableRspMsg nanEnableRsp = {0};
		...
		rStatus = nanAbortScan(prAdapter);
		if (rStatus != WLAN_STATUS_SUCCESS) {
			DBGLOG(NAN, WARN, "NAN on Abort Scan Fail\n");
		}
		...
		/* TODO: nanEnableReq not used so far
		 * Integrate with nanDevSendEnableRequest
		 */
		 ...
		 */
		if (nanDevSendEnableRequestToCnm(prAdapter)
			== WLAN_STATUS_SUCCESS)
			nanEnableRsp.status = NAN_I_STATUS_SUCCESS;
		else
			nanEnableRsp.status = NAN_I_STATUS_TIMEOUT;
		...
		/* Update Driver cache availability after enable
		 * to avoid FW unsync
		 */
...
exit:
	...
	case NAN_MSG_ID_PUBLISH_SERVICE_REQ: {
		...
		uint8_t aucCipherList[NAN_MAX_CIPHER_SUITE_NUM] = {0};
		...
		if (publish_id != 0) {
			...
			prInstance = nanDiscInstanceSearch(prAdapter,
					publish_id);
		}
		...
		if (pNanPublishReq->sdea_params.security_cfg
			...
			ucCipherType = (uint8_t) pNanPublishReq->cipher_type;
			aucCipherList[0] = ucCipherType;
			ucNumCsid = 1;
#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_FRAME_PROT == 1)
			if (prInstance && prInstance->fgGtkRequired)
			{
				aucCipherList[1] =
					nanSecGetGtkCipherByNdi(
						NAN_NDI_INDEX_0);
				ucNumCsid = 2;
			}
#endif /* CFG_NAN_SUPPORT_R4_GROUP_ADDR_FRAME_PROT */
			nanCmdAddCsid(prAdapter, publish_id,
				ucNumCsid, aucCipherList);
			nanSetPublishPmkid(
				prGlueInfo->prAdapter,
				pNanPublishReq);
	...
	case NAN_MSG_ID_TRANSMIT_FOLLOWUP_REQ: {
		...
		uint8_t fgIsSkda = FALSE;
		struct PAIRING_INFO *prPairingInfo = NULL;
		struct NanSkdaInfo *prSkdaInfo = NULL;
		struct _NAN_ATTR_SKDA_T *prAttrSkda = NULL;
		uint16_t u2KeyLength = 0;
		uint8_t *prKdeBuf = NULL;
#endif
		...
		while ((remainingLen > 0) &&
			...
			case NAN_TLV_TYPE_SDEA_SERVICE_SPECIFIC_INFO:
				...
				pNanXmitFollowupReq
					...
					outputTlv.length;
				break;
			case NAN_TLV_CUSTOM_ATTR:
				if (outputTlv.length >
					NAN_CUSTOM_ATTRIBUTE_MAX_SIZE) {
					DBGLOG(NAN, ERROR,
						"followup NAN_TLV_CUSTOM_ATTR length err[%u]\n",
						outputTlv.length);
					kfree(pNanXmitFollowupReq);
					kfree(pNanXmitFollowupRsp);
					return -EFAULT;
				...
				DBGLOG(NAN, INFO,
					"followup copy custom attr len:%u\n",
					outputTlv.length);
				kalMemCopy(pNanXmitFollowupReq
					       ->custom_attributes,
				       outputTlv.value, outputTlv.length);
				pNanXmitFollowupReq
					->custom_attribute_length =
					outputTlv.length;
				break;
			case NAN_TLV_TYPE_SHARED_KEY_DESCRIPTOR:
#if (CFG_SUPPORT_NAN_R4_VENDOR_PAIRING == 1)
				DBGLOG(NAN, INFO,
					"followup SDKA len%u DstMac>"MACSTR"\n",
					outputTlv.length,
					MAC2STR(pNanXmitFollowupReq->addr));
				fgIsSkda = TRUE;
				prSkdaInfo = kalMemAlloc(
					sizeof(struct NanSkdaInfo),
					PHY_MEM_TYPE);
				if (!prSkdaInfo) {
					DBGLOG(NAN, ERROR,
						"prSkdaInfo allocate fail\n");
					kfree(pNanXmitFollowupReq);
					kfree(pNanXmitFollowupRsp);
					return -EFAULT;
				}
				kalMemZero(prSkdaInfo,
					sizeof(struct NanSkdaInfo));
				kalMemCopy(prSkdaInfo,
					outputTlv.value,
					outputTlv.length);
				DBGLOG(NAN, INFO,
					"followup SKDA info\n");
				dumpMemory8((uint8_t *)prSkdaInfo,
					sizeof(struct NanSkdaInfo));
		...
		if (fgIsSkda) {
			DBGLOG(NAN, INFO,
				"followup SDKA DstMac["MACSTR"]\n",
				MAC2STR(pNanXmitFollowupReq->addr));
			prPairingInfo =
				nanPairingInfoSearch(prAdapter,
					pNanXmitFollowupReq->addr);

			if (prPairingInfo != NULL &&
				prSkdaInfo != NULL &&
				prSkdaInfo->append_nik_kde == TRUE)	{
				if (prPairingInfo->rPtk.kek_len != 0) {
					prAttrSkda = (struct _NAN_ATTR_SKDA_T *)
						((uint8_t *)pNanXmitFollowupReq
						->custom_attributes +
						pNanXmitFollowupReq->
						custom_attribute_length);
					prAttrSkda->ucAttribID =
					NAN_ATTR_ID_SHARED_KEY_DESCRIPTOR;
					prAttrSkda->ucPublishID =
						(uint8_t)pNanXmitFollowupReq->
						publish_subscribe_id;

					DBGLOG(NAN, INFO, "carry SKDA in it\n");
					prKdeBuf = (uint8_t *)
						kalMemAlloc(256, PHY_MEM_TYPE);
					u2KeyLength = nanSecAppendKde(
						prSkdaInfo->append_nik_kde,
						prSkdaInfo->nik.nik_length,
						prSkdaInfo->nik.nik,
						prSkdaInfo->nik.cipher_version,
						prSkdaInfo->
							append_nik_lifetime_kde,
						prSkdaInfo->
							nik_lifetime.key_bitmap,
						prSkdaInfo->
							nik_lifetime.
							key_lifetime,
						prSkdaInfo->append_igtk_kde,
						prPairingInfo->rPtk.kek,
						prPairingInfo->rPtk.kek_len,
						prKdeBuf);
					nanSecComposeEapolKey(prAttrSkda,
					prKdeBuf,
					u2KeyLength,
						prPairingInfo->rPtk.kck,
					...
					pNanXmitFollowupReq->
						...
						(prAttrSkda->u2Len +
							NAN_ATTR_HDR_LEN);
					dumpMemory8((uint8_t *)prAttrSkda,
						255);
				} else {
					DBGLOG(NAN, INFO,
						"KEK is NULL, Follow-up won't carry KDE\n");
				}
			}
		}
		if (prSkdaInfo)
			kalMemFree(prSkdaInfo,
				PHY_MEM_TYPE,
				sizeof(struct NanSkdaInfo));
#endif /* CFG_SUPPORT_NAN_R4_VENDOR_PAIRING */

		/* Follow up Command reply message */
		kalMemCpyS(&pNanXmitFollowupRsp->fwHeader,
			   sizeof(pNanXmitFollowupRsp->fwHeader),
			   &nanMsgHdr,
			   sizeof(struct _NanMsgHeader));
		skb = cfg80211_vendor_cmd_alloc_reply_skb(
		...
		rStatus = kalIoctl(prGlueInfo, wlanoidNANFollowupRsp,
	...
	case NAN_MSG_ID_VENDOR_GET_WFD_STATUS:
	{
		struct BSS_INFO *prBssInfo = NULL;
		struct P2P_DEV_FSM_INFO *prP2pDevFsmInfo = NULL;
		struct NanP2pStateIndMsg rP2pStatus = {0};
		enum ENUM_NAN_REPORT_P2P_STATE eP2pState =
		...
		kalMemCopy(&rP2pStatus.fwHeader, &nanMsgHdr,
			sizeof(struct _NanMsgHeader));

		prP2pDevFsmInfo = prAdapter->rWifiVar.prP2pDevFsmInfo;

		for (; i < MAX_BSSID_NUM; i++) {
		...
		if (prBssInfo && (i != MAX_BSSID_NUM)) {
			eP2pState = ENUM_NAN_P2P_STATE_OPERATING;
			ucChannel = prBssInfo->ucPrimaryChannel;
			eChannelWidth = CW_80MHZ;
		} else if (prP2pDevFsmInfo &&
			prP2pDevFsmInfo->eCurrentState
			== P2P_DEV_STATE_NAN_CONCURRENT_LISTEN) {
			eP2pState = ENUM_NAN_P2P_STATE_LISTENING;
			ucChannel =
				prP2pDevFsmInfo->rChnlReqInfo.ucReqChnlNum;
			eChannelWidth = CW_20_40MHZ;
		}
		rP2pStatus.rNanP2pState.eP2pState = eP2pState;
		rVendorChInfo.ucChannelNum = ucChannel;
		rVendorChInfo.eBand =
			SCN_GET_EBAND_BY_CH_NUM(ucChannel);
		rVendorChInfo.ucChnlBw = (uint8_t)eChannelWidth;
		rP2pStatus.rNanP2pState.channel =
		nanCastChToVendorFormat(rVendorChInfo,
			CHNL_EXT_SCN);

		/* For Debug purpose */
		if (g_fgManualWfdState == TRUE) {
			kalMemCopy(&rP2pStatus.rNanP2pState,
				&g_rCurrNanP2pState,
				sizeof(struct NanP2pState));
		}

		DBGLOG(NAN, INFO,
			"GET_P2P_STATUS[%u] state[%u] channel[%u] flag[%u]",
			i,
			rP2pStatus.rNanP2pState.eP2pState,
			rP2pStatus.rNanP2pState.channel.channel,
			rP2pStatus.rNanP2pState.channel.flags);

		skb = cfg80211_vendor_cmd_alloc_reply_skb(
			wiphy, sizeof(struct NanP2pStateIndMsg));
		if (!skb) {
			DBGLOG(REQ, ERROR, "Allocate skb failed\n");
			return -ENOMEM;
		}
		if (unlikely(nla_put_nohdr(skb,
			sizeof(struct NanP2pStateIndMsg),
					&rP2pStatus) < 0)) {
			kfree_skb(skb);
			return -EFAULT;
		}
	...
	case NAN_MSG_ID_GET_DEVICE_CAPABILITIES:
	{
		struct NanDeviceCapabilitiesMsg rDeviceCapabilities = {0};

		kalMemCopy(&rDeviceCapabilities.fwHeader, &nanMsgHdr,
			sizeof(struct _NanMsgHeader));

		nanDevGetDeviceCapabilities(prAdapter,
			&rDeviceCapabilities.rDeviceCapabilities);

		skb = cfg80211_vendor_cmd_alloc_reply_skb(
			wiphy, sizeof(struct NanDeviceCapabilitiesMsg));
		if (!skb) {
			DBGLOG(REQ, ERROR, "Allocate skb failed\n");
			return -ENOMEM;
		}
		if (unlikely(nla_put_nohdr(skb,
			sizeof(struct NanDeviceCapabilitiesMsg),
					&rDeviceCapabilities) < 0)) {
			kfree_skb(skb);
			return -EFAULT;
		}
		break;
	}
	case NAN_MSG_ID_GET_CLUSTER_ID:
	{
		struct NanClusterIdMsg rClusterId = {0};

		kalMemCopy(&rClusterId.fwHeader, &nanMsgHdr,
			sizeof(struct _NanMsgHeader));

		nanDevGetClusterId(prAdapter, rClusterId.cluster_id);

		DBGLOG(NAN, INFO,
			"GET_CLUSTER_ID ["MACSTR"] ",
			MAC2STR(rClusterId.cluster_id));

		skb = cfg80211_vendor_cmd_alloc_reply_skb(
			wiphy, sizeof(struct NanClusterIdMsg));
		if (!skb) {
			DBGLOG(REQ, ERROR, "Allocate skb failed\n");
			return -ENOMEM;
		}
		if (unlikely(nla_put_nohdr(skb,
			sizeof(struct NanClusterIdMsg),
					&rClusterId) < 0)) {
			kfree_skb(skb);
			return -EFAULT;
		}
	...
	case NAN_MSG_ID_GET_MAST_PREFERENCE:
	{
		struct NanMastPreferenceMsg rMastPreference = {0};

		kalMemCopy(&rMastPreference.fwHeader, &nanMsgHdr,
			sizeof(struct _NanMsgHeader));

		if (prAdapter->fgNanEnable) {
			DBGLOG(NAN, INFO,
				"Get MasPref before NAN enable, return default value");
			rMastPreference.mast_preference =
				prAdapter->rNanSyncInfo.ucCurrentMasPref;
		} else {
			rMastPreference.mast_preference =
				prAdapter->rWifiVar.ucMasterPref;
		}

		DBGLOG(NAN, INFO,
			"GET_MAST_PREFERENCE curr: %u, config: %u",
			rMastPreference.mast_preference,
			prAdapter->rWifiVar.ucMasterPref);

		skb = cfg80211_vendor_cmd_alloc_reply_skb(
			wiphy, sizeof(struct NanMastPreferenceMsg));
		if (!skb) {
			DBGLOG(REQ, ERROR, "Allocate skb failed\n");
			return -ENOMEM;
		}
		if (unlikely(nla_put_nohdr(skb,
			sizeof(struct NanMastPreferenceMsg),
					&rMastPreference) < 0)) {
			kfree_skb(skb);
			return -EFAULT;
		}
	...
	case NAN_MSG_ID_GET_RANDOM_FACTOR:
	{
		struct NanRandomFactorMsg rRandomFactor = {0};

		kalMemCopy(&rRandomFactor.fwHeader, &nanMsgHdr,
			sizeof(struct _NanMsgHeader));

		rRandomFactor.random_factor =
			prAdapter->rNanSyncInfo.ucCurrentRandomfactor;
		...
		skb = cfg80211_vendor_cmd_alloc_reply_skb(
			wiphy, sizeof(struct NanRandomFactorMsg));
		if (!skb) {
			DBGLOG(REQ, ERROR, "Allocate skb failed\n");
			return -ENOMEM;
		}
		if (unlikely(nla_put_nohdr(skb,
			sizeof(struct NanRandomFactorMsg),
					&rRandomFactor) < 0)) {
			kfree_skb(skb);
			return -EFAULT;
		}
	...
	case NAN_MSG_ID_SET_COMMITTED_AVAILABILITY:
	{
		...
		struct MSG_P2P_NAN_MAP_UPDATED *prMsgNanMapUpdated = NULL;
		...
		/* Skip setting NAN channel sequence to firmware
		 * since it's unused when NAN is disabled
		 */
		if (prAdapter->fgNanEnable) {
...
#if (CFG_NAN_P2P_CONCURRENT_SUPPORT == 1)
		prMsgNanMapUpdated = (struct MSG_P2P_NAN_MAP_UPDATED *)
			cnmMemAlloc(prAdapter,
			RAM_TYPE_MSG, sizeof(struct MSG_P2P_NAN_MAP_UPDATED));

		if (!prMsgNanMapUpdated) {
			/* Can't indicate P2P Dev for Map update */
			return -EINVAL;
		}
		prMsgNanMapUpdated->rMsgHdr.eMsgId =
			MID_MNY_P2P_NAN_MAP_UPDATED;
		mboxSendMsg(prAdapter,
			MBOX_ID_0,
			(struct MSG_HDR *) prMsgNanMapUpdated,
			MSG_SEND_METHOD_BUF);
#endif /* CFG_NAN_P2P_CONCURRENT_SUPPORT */
		return 0;
	}
...
#if (CFG_SUPPORT_NAN_R4_VENDOR_PAIRING == 1)
	case NAN_MSG_ID_SET_PAIRING_KEY:
	{
		struct NanPairingSetKey rNanPairingKey = {0};
		size_t message_len = sizeof(struct NanPairingSetKey);

		DBGLOG(NAN, INFO,
			"[pairing] NAN_MSG_ID_PAIRING_SET_KEY\n");

		kalMemCopy(&rNanPairingKey, (u8 *)data, message_len);

		DBGLOG(NAN, INFO, "pairing peer["MACSTR"]\n",
			MAC2STR(rNanPairingKey.peer_address));
		DBGLOG(NAN, INFO, "pairing NM_TK Scite_ID[%u] Key_Len[%u]\n",
			rNanPairingKey.nm_tk.cipher_suite_id,
			rNanPairingKey.nm_tk.key_length);
		DBGLOG(NAN, INFO, "pairing NM_KEK Scite_ID[%u] Key_Len[%u]\n",
			rNanPairingKey.nm_kek.cipher_suite_id,
			rNanPairingKey.nm_kek.key_length);
		DBGLOG(NAN, INFO, "pairing NM_KCK Scite_ID[%u] Key_Len[%u]\n",
			rNanPairingKey.nm_kck.cipher_suite_id,
			rNanPairingKey.nm_kck.key_length);
		DBGLOG(NAN, INFO, "pairing ND_TK Scite_ID[%u] Key_Len[%u]\n",
			rNanPairingKey.nd_tk.cipher_suite_id,
			rNanPairingKey.nd_tk.key_length);
		DBGLOG(NAN, INFO, "pairing IGTK Scite_ID[%u] Key_Len[%u]\n",
			rNanPairingKey.igtk.cipher_suite_id,
			rNanPairingKey.igtk.key_length);
		DBGLOG(NAN, INFO, "pairing BIGTK Scite_ID[%u] Key_Len[%u]\n",
			rNanPairingKey.bigtk.cipher_suite_id,
			rNanPairingKey.bigtk.key_length);
		DBGLOG(NAN, INFO, "pairing GTK Scite_ID[%u] Key_Len[%u]\n",
			rNanPairingKey.gtk.cipher_suite_id,
			rNanPairingKey.gtk.key_length);

		return nanSecSetKeyHandler(prAdapter, &rNanPairingKey);
	}
#endif /* CFG_SUPPORT_NAN_R4_VENDOR_PAIRING */
	case NAN_MSG_ID_SET_OOB_AF_TX_PARAMS:
	{
		struct _NAN_CMD_OOB_ACTION *prNanCmdOOBAction = NULL;

		DBGLOG(NAN, INFO, "[oob] NAN_MSG_ID_SET_OOB_AF_TX_PARAMS\n");

		prNanCmdOOBAction = (struct _NAN_CMD_OOB_ACTION *)data;

		nanVendorOobActionTxHandler(prAdapter, prNanCmdOOBAction);

		return 0;
	}
	case NAN_MSG_ID_VENDOR_SET_AWAKE_PERIOD:
	{
		struct NanAwakePeriod *prNanAwakeDw = NULL;
		struct CMD_CHIP_CONFIG rCmdChipConfig = {0};

		prNanAwakeDw = (struct NanAwakePeriod *)data;

		DBGLOG(NAN, INFO, "SET_AWAKE_PERIOD 2G[%u]5G[%u]\n",
			prNanAwakeDw->period_2_4G,
			prNanAwakeDw->period_5G);

		rCmdChipConfig.u2Id = 0;
		rCmdChipConfig.ucType = CHIP_CONFIG_TYPE_WO_RESPONSE;
		rCmdChipConfig.u2MsgSize =
			(uint16_t)kalSnprintf(rCmdChipConfig.aucCmd,
			CHIP_CONFIG_RESP_SIZE - 1, "Nan2gDftCommittedDw %u",
			prNanAwakeDw->period_2_4G);
		rCmdChipConfig.aucCmd[CHIP_CONFIG_RESP_SIZE - 1] = '\0';
		prAdapter->rWifiVar.ucNan2gCommittedDw =
			(uint8_t)prNanAwakeDw->period_2_4G;
		wlanCfgSetUint32(prAdapter,
			(const int8_t *)"Nan2gDftCommittedDw",
			(uint32_t)prNanAwakeDw->period_2_4G);
		...
		rCmdChipConfig.u2MsgSize =
			(uint16_t) kalSnprintf(rCmdChipConfig.aucCmd,
			CHIP_CONFIG_RESP_SIZE - 1, "Nan5gDftCommittedDw %u",
			prNanAwakeDw->period_5G);
		rCmdChipConfig.aucCmd[CHIP_CONFIG_RESP_SIZE - 1] = '\0';
		prAdapter->rWifiVar.ucNan5gCommittedDw =
			(uint8_t)prNanAwakeDw->period_5G;
		wlanCfgSetUint32(prAdapter,
			(const int8_t *)"Nan5gDftCommittedDw",
			(uint32_t)prNanAwakeDw->period_5G);
	...
	case NAN_MSG_ID_VENDOR_GET_AWAKE_PERIOD:
	{
		struct NanAwakePeriodMsg rNanAwakePeriod;

		kalMemCopy(&rNanAwakePeriod.fwHeader, &nanMsgHdr,
			sizeof(struct _NanMsgHeader));

		rNanAwakePeriod.awake_period.period_2_4G =
			prAdapter->rWifiVar.ucNan2gCommittedDw;
		rNanAwakePeriod.awake_period.period_5G =
			prAdapter->rWifiVar.ucNan5gCommittedDw;

		DBGLOG(NAN, INFO, "GET_AWAKE_PERIOD 2G[%u]5G[%u]\n",
			rNanAwakePeriod.awake_period.period_2_4G,
			rNanAwakePeriod.awake_period.period_5G);
		...
		if (unlikely(nla_put_nohdr(skb,
			sizeof(struct NanAwakePeriodMsg),
					   &rNanAwakePeriod) < 0)) {
			kfree_skb(skb);
			return -EFAULT;
		}
					       ...
					       uint8_t *pcuEvtBuf)
{
	...
	kalMemZero(prNanEventInd, message_len);
		...
		IN struct ADAPTER *prAdapter, uint8_t *pcuEvtBuf)
{
	...
	kalMemZero(prNanDisableInd, message_len);
					       ...
					       uint8_t *pcuEvtBuf)
{
	...
	/* AP NAN TODO: need to extract u2SdeaServiceControl for GTK
	 * Current no used this flag
	 */

	DBGLOG(NAN, LOUD,
	       "data_path_required : %d, security_required:%d, ranging_required:%d, gtk_required:%d\n",
	       nanPeerSdeaCtrlarms.data_path_required,
	       nanPeerSdeaCtrlarms.security_required,
	       nanPeerSdeaCtrlarms.ranging_required,
						  ...
						  uint8_t *pcuEvtBuf)
{
	...
	struct NAN_FOLLOW_UP_EVENT *prFollowupEvt = NULL;
	uint8_t *tlvs = NULL;
	size_t message_len = 0;
	uint32_t rStatus = WLAN_STATUS_FAILURE;
...
uint32_t
mtk_cfg80211_vendor_event_nan_vendor_followup_rx(IN struct ADAPTER *prAdapter,
						  uint8_t *pcuEvtBuf)
{
	struct sk_buff *skb = NULL;
	struct wiphy *prWiphy = NULL;
	struct wireless_dev *prWdev = NULL;
	struct NanVendorFollowupIndMsg *prNanFollowupInd = NULL;
	struct NAN_VENDOR_FOLLOW_UP_RX_EVENT *prFollowupEvt = NULL;
	size_t message_len = 0;
	uint32_t rStatus = WLAN_STATUS_FAILURE;
#if (CFG_SUPPORT_NAN_R4_VENDOR_PAIRING == 1)
	struct _NAN_ATTR_SKDA_T *prAttrSkda = NULL;
	...
	prFollowupEvt = (struct NAN_VENDOR_FOLLOW_UP_RX_EVENT *)pcuEvtBuf;

	message_len =
			sizeof(struct _NanMsgHeader) +
			sizeof(struct NAN_VENDOR_FOLLOW_UP_RX_EVENT);
	...
	prNanFollowupInd->fwHeader.msgVersion = 1;
	prNanFollowupInd->fwHeader.msgId = NAN_MSG_ID_VENDOR_FOLLOWUP_RX_IND;
	prNanFollowupInd->fwHeader.msgLen = message_len;
	prNanFollowupInd->fwHeader.handle = prFollowupEvt->ucLocalServiceId;

	/* Mapping datas */
	prNanFollowupInd->vendorfollowupIndParams.ucVersion =
		prFollowupEvt->ucVersion;
	prNanFollowupInd->vendorfollowupIndParams.ucLocalServiceId =
		prFollowupEvt->ucLocalServiceId;
	prNanFollowupInd->vendorfollowupIndParams.ucPeerServiceId =
		prFollowupEvt->ucPeerServiceId;
	COPY_MAC_ADDR(prNanFollowupInd->vendorfollowupIndParams.aucPeerAddr,
		prFollowupEvt->aucPeerAddr);

	prNanFollowupInd->vendorfollowupIndParams.i1FollowUpRssi =
		prFollowupEvt->i1FollowUpRssi;
	prNanFollowupInd->vendorfollowupIndParams.ucPrimaryCh =
		prFollowupEvt->ucPrimaryCh;
	prNanFollowupInd->vendorfollowupIndParams.ucChBand =
		prFollowupEvt->ucChBand;
	prNanFollowupInd->vendorfollowupIndParams.ucChBw =
		prFollowupEvt->ucChBw;
	prNanFollowupInd->vendorfollowupIndParams.u2AttrListLength =
		prFollowupEvt->u2AttrListLength;

#if (CFG_SUPPORT_NAN_R4_VENDOR_PAIRING == 1)
	prPairingInfo = nanPairingInfoSearch(
		prAdapter, prFollowupEvt->aucPeerAddr);
	if (prPairingInfo != NULL) {
	/* We should convert the SKDA endianness to host format */
		prAttrSkda = (struct _NAN_ATTR_SKDA_T *)nanRetrieveAttrById(
			prFollowupEvt->aucAttrList,
			prFollowupEvt->u2AttrListLength,
			NAN_ATTR_ID_SHARED_KEY_DESCRIPTOR);
		if (prAttrSkda != NULL)
			nanSecRxSkdaToHostFormat(
				prAttrSkda->aucKeyDescriptor,
			prPairingInfo->u4SelCipherType);
		else
			DBGLOG(NAN, ERROR, "prAttrSkda is NULL\n");
	} else {
		DBGLOG(NAN, ERROR, "prPairingInfo is NULL\n");
	}
#endif

	kalMemCopy(prNanFollowupInd->vendorfollowupIndParams.aucAttrList,
		prFollowupEvt->aucAttrList,
		prNanFollowupInd->vendorfollowupIndParams.u2AttrListLength);


	DBGLOG(NAN, INFO, "ch[%u] rssi[%d] attr_len[%u]\n",
		prNanFollowupInd->vendorfollowupIndParams.ucPrimaryCh,
		prNanFollowupInd->vendorfollowupIndParams.i1FollowUpRssi,
		prNanFollowupInd->vendorfollowupIndParams.u2AttrListLength);
...
uint32_t
mtk_cfg80211_vendor_event_nan_vendor_followup_tx(
		struct ADAPTER *prAdapter,
		uint8_t *pcuEvtBuf)
{
	struct sk_buff *skb = NULL;
	struct wiphy *prWiphy = NULL;
	struct wireless_dev *prWdev = NULL;
	struct NanVendorFollowupTxIndMsg *prNanFollowupInd = NULL;
	struct NAN_VENDOR_FOLLOW_UP_TX_EVENT *prFollowupEvt = NULL;
	size_t message_len = 0;
	uint32_t rStatus = WLAN_STATUS_FAILURE;
	...
	prFollowupEvt = (struct NAN_VENDOR_FOLLOW_UP_TX_EVENT *)pcuEvtBuf;

	message_len =
			sizeof(struct _NanMsgHeader) +
			sizeof(struct NAN_VENDOR_FOLLOW_UP_TX_EVENT);
	...
	prNanFollowupInd->fwHeader.msgVersion = 1;
	prNanFollowupInd->fwHeader.msgId = NAN_MSG_ID_VENDOR_FOLLOWUP_TX_IND;
	prNanFollowupInd->fwHeader.msgLen = message_len;
	...
	COPY_MAC_ADDR(prNanFollowupInd->vendorfollowupIndParams.aucPeerAddr,
		prFollowupEvt->aucPeerAddr);
	prNanFollowupInd->vendorfollowupIndParams.u4FollowUpToken =
		prFollowupEvt->u4FollowUpToken;
	prNanFollowupInd->vendorfollowupIndParams.u4TxStatus =
		prFollowupEvt->u4TxStatus;

	DBGLOG(NAN, INFO, "peer[" MACSTR "] tx_status:%u",
		MAC2STR(prNanFollowupInd->vendorfollowupIndParams.aucPeerAddr),
		prNanFollowupInd->vendorfollowupIndParams.u4TxStatus);
		...
		enum ENUM_CHNL_EXT eSco)
{
	...
	prMsg->channel = nanCastChToVendorFormat(rChannel, eSco);
						...
						uint8_t *pcuEvtBuf)
{
	...
	prAdapter->rNanSyncInfo.ucCurrentHopCount =
		prNanReportDwEvt->device_info.ucHopCount;
	prAdapter->rNanSyncInfo.ucCurrentRandomfactor =
		prNanReportDwEvt->device_info.ucRandomFactor;
	prAdapter->rNanSyncInfo.ucCurrentMasPref =
		prNanReportDwEvt->device_info.ucMasPreference;

	DBGLOG(NAN, LOUD, "channel[%u], flags[0x%X], eTsfL[%u]\n",
		prMsg->channel.channel, prMsg->channel.flags,
		prMsg->expected_tsf_l);
	DBGLOG(NAN, LOUD, "eTsfH[%u], aTsfL[%u], aTsfH[%u], dw_num[%u]\n",
		prMsg->expected_tsf_h,
		prMsg->actual_tsf_l,
		prMsg->actual_tsf_h,
		prMsg->dw_num);
	DBGLOG(NAN, LOUD, "hopCT[%u], RandomFac[%u], MasPref[%u]\n",
		prAdapter->rNanSyncInfo.ucCurrentHopCount,
		prAdapter->rNanSyncInfo.ucCurrentRandomfactor,
		prAdapter->rNanSyncInfo.ucCurrentMasPref);
...
uint32_t
mtk_cfg80211_vendor_event_nan_oob_af_tx(struct ADAPTER *prAdapter,
					uint16_t u2Token,
					uint32_t u4Status)
{
	struct sk_buff *skb = NULL;
	...
	struct wireless_dev *prWdev = NULL;
	size_t message_len = 0;
	struct NanOobAfTxStatusIndMsg *prMsg = NULL;
	uint32_t rStatus = WLAN_STATUS_FAILURE;
	...
	message_len = sizeof(struct NanOobAfTxStatusIndMsg);
	...
	prMsg->fwHeader.msgVersion = 1;
	prMsg->fwHeader.msgId = NAN_MSG_ID_OOB_AF_TX_STATUS_IND;
	prMsg->fwHeader.msgLen = message_len;

	prMsg->u2Token = u2Token;

	/* status code mapping
	 * enum ENUM_TX_RESULT_CODE
	 */
	if (u4Status == TX_RESULT_SUCCESS) {
		prMsg->ucResult =
			WFPAL_NAN_OUT_OF_BAND_ACTION_FRAME_TX_RESULT_ACK;
	} else if (u4Status == TX_RESULT_MPDU_ERROR) {
		prMsg->ucResult =
			WFPAL_NAN_OUT_OF_BAND_ACTION_FRAME_TX_RESULT_NO_ACK;
	} else if (u4Status == TX_RESULT_LIFE_TIMEOUT) {
		prMsg->ucResult =
			WFPAL_NAN_OUT_OF_BAND_ACTION_FRAME_TX_RESULT_EXPIRED;
	} else {
		DBGLOG(NAN, WARN, "TX error: 0x%08x\n", u4Status);
		prMsg->ucResult =
			WFPAL_NAN_OUT_OF_BAND_ACTION_FRAME_TX_RESULT_UNKNOWN;
	}

	DBGLOG(NAN, INFO,
		"[OobAF] Tx status token: %u, result: %u\n",
		prMsg->u2Token,
		prMsg->ucResult);
...
mtk_cfg80211_vendor_event_nan_oob_af_rx(struct ADAPTER *prAdapter,
					...
					uint8_t *ucSrcAddr,
					uint8_t *ucBssid,
					struct RF_CHANNEL_INFO *prVendorChInfo,
					uint8_t ucRssi,
					uint16_t u2DataLen,
					uint8_t *ucData,
					uint8_t u1FrameType)
{
	...
	rVendorChInfo.ucChannelNum = prVendorChInfo->ucChannelNum;
	rVendorChInfo.eBand = prVendorChInfo->eBand;
	rVendorChInfo.ucChnlBw = prVendorChInfo->ucChnlBw;
	...
	prMsg->channel = nanCastChToVendorFormat(rVendorChInfo, CHNL_EXT_SCN);
	prMsg->rx_rssi = ucRssi;
	prMsg->frameType = u1FrameType;
							...
							uint8_t *pcuEvtBuf)
{
	...
	rVendorChInfo.ucChannelNum = prNanBcnFrameEvt->ucHwChnl;
	rVendorChInfo.eBand =
		SCN_GET_EBAND_BY_CH_NUM(prNanBcnFrameEvt->ucHwChnl);
	rVendorChInfo.ucChnlBw = prNanBcnFrameEvt->ucBw;
	prMsg->channel = nanCastChToVendorFormat(rVendorChInfo, CHNL_EXT_SCN);
...
uint32_t
mtk_cfg80211_vendor_event_nan_report_p2p_state(struct ADAPTER *prAdapter,
				enum ENUM_NAN_REPORT_P2P_STATE eP2pState,
				uint8_t ucChannel,
				enum ENUM_CHANNEL_WIDTH eBw)
{
	struct sk_buff *skb = NULL;
	struct wiphy *prWiphy = NULL;
	struct net_device *prNetDev = NULL;
	struct wireless_dev *prWdev = NULL;
	size_t message_len = 0;
	struct NanP2pStateIndMsg *prMsg = NULL;
	uint32_t rStatus = WLAN_STATUS_FAILURE;
	struct RF_CHANNEL_INFO rVendorChInfo = {0};
	...
	message_len = sizeof(struct NanP2pStateIndMsg);
	...
	prMsg->fwHeader.msgVersion = 1;
	prMsg->fwHeader.msgId = NAN_MSG_ID_VENDOR_WFD_STATUS_IND;
	prMsg->fwHeader.msgLen = message_len;

	/* Indication doesn't have transition ID */
	prMsg->fwHeader.transactionId = 0;

	prMsg->rNanP2pState.eP2pState =
		(enum ENUM_NAN_REPORT_P2P_STATE)eP2pState;
	rVendorChInfo.ucChannelNum = ucChannel;
	rVendorChInfo.eBand =
		SCN_GET_EBAND_BY_CH_NUM(ucChannel);
	rVendorChInfo.ucChnlBw = (uint8_t)eBw;
	prMsg->rNanP2pState.channel =
		nanCastChToVendorFormat(rVendorChInfo,
			CHNL_EXT_SCN);

	DBGLOG(NAN, INFO, "P2P State[%u] Ch[%u]\n",
		prMsg->rNanP2pState.eP2pState,
		prMsg->rNanP2pState.channel.channel);

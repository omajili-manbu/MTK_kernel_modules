...
#include "precomp.h"
#if ((CFG_CONNAC2_LISTMODE_ENABLE == 1) || (CFG_SUPPORT_QA_TOOL == 1))
#include "gl_wext.h"
...
#if ((CFG_CONNAC2_LISTMODE_ENABLE == 1) || CFG_SUPPORT_BUFFER_MODE)
			   ...
			   struct HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	MT_ATESetTxCount(prNetDev, TxCount);
	MT_ATESetTxLength(prNetDev, (uint32_t) TxLength);
				   ...
				   struct HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	if (prGlueInfo->prAdapter->fgIsSupportQAAccessEfuse == TRUE) {
		...
		for (Loop = 0; Loop < EFUSE_BLOCK_SIZE/2; Loop++) {
			DBGLOG(INIT, INFO,
 ...
 */
/*----------------------------------------------------------------------------*/
static int32_t HQA_ReadBulkSecEEPROM(struct net_device
				  *prNetDev,
				  IN union iwreq_data *prIwReqData,
				  struct HQA_CMD_FRAME *HqaCmdFrame)
{
	int32_t i4Ret = -EINVAL;
#if CFG_SECEFUSE_ACCESS
	uint32_t u4BufLen = 0;
	uint16_t u2Offset = 0, u2Len = 0, u2CurOffset = 0, u2EndOffset = 0;
	uint16_t u2RemainDataSize = 0, u2BlockSize = 0;
	uint16_t u2DataOffset = 0;
	struct ADAPTER *prAdapter = NULL;
	struct GLUE_INFO *prGlueInfo = NULL;
	...
	prGlueInfo = *((struct GLUE_INFO **) netdev_priv(prNetDev));
	if (!prGlueInfo || !prGlueInfo->prAdapter) {
		DBGLOG(INIT, ERROR, "prGlueInfo or prAdapter is NULL\n");
		i4Ret = -EFAULT;
		goto end;
	}
	prAdapter = prGlueInfo->prAdapter;

	DBGLOG(INIT, INFO, "QA_AGENT HQA_ReadBulkSecEEPROM\n");

	/*
	 * HQA_ReadSecEEPROM (0x131D): HQA cmd for bulk read sec eeprom data
	 * Data[0..1]: offset
	 * Data[2..3]: length
	 */
	u2Offset = ntohs(*(uint16_t *)&HqaCmdFrame->Data[0]);
	u2Len = ntohs(*(uint16_t *)&HqaCmdFrame->Data[2]);

	/* out of bound access and overflow check */
	u2EndOffset = u2Offset + u2Len;
	if ((u2Offset >= MAX_SEC_EEPROM_BUFFER_SIZE) || (u2Len == 0) ||
		(u2EndOffset > MAX_SEC_EEPROM_BUFFER_SIZE) ||
		(u2EndOffset < u2Offset)) {
		DBGLOG(INIT, ERROR, "Offset (0x%x), Len (0x%x): error.\n",
			u2Offset, u2Len);
		i4Ret = -EINVAL;
		goto end;
	}

	/* check starting offset and length */
	u2CurOffset = u2Offset;
	while (u2CurOffset < u2EndOffset) {
		if ((u2CurOffset >= SECEFUSE_KEYHASH_OFFSET_BEGIN) &&
			(u2CurOffset < SECEFUSE_KEYHASH_OFFSET_END)) {
			u2BlockSize = SECEFUSE_KEYHASH_SIZE;
		} else {
			u2BlockSize = EFUSE_BLOCK_SIZE;
		}

		/* offset should be in block size alignment */
		if (((u2CurOffset % u2BlockSize) != 0) ||
			(u2CurOffset + u2BlockSize > u2EndOffset)) {
			DBGLOG(INIT, ERROR, "Offset (0x%x),Len (0x%x):error.\n",
				u2Offset, u2Len);
			i4Ret = -EINVAL;
			goto end;
		...
		u2CurOffset += u2BlockSize;
	}


	u2CurOffset = u2Offset;
	u2RemainDataSize = sizeof(HqaCmdFrame->Data) - 2;
	while (u2CurOffset < u2EndOffset) {
		if ((u2CurOffset >= SECEFUSE_KEYHASH_OFFSET_BEGIN) &&
			(u2CurOffset < SECEFUSE_KEYHASH_OFFSET_END)) {
			u2BlockSize = SECEFUSE_KEYHASH_SIZE;
		...
		rAccessEfuseInfo.u4Address = u2CurOffset;
		rAccessEfuseInfo.u4Size = u2BlockSize;

		i4Ret = (int32_t)kalIoctl(prGlueInfo,
				wlanoidQueryProcessAccessSecEfuseRead,
				&rAccessEfuseInfo,
				sizeof(struct PARAM_CUSTOM_ACCESS_SECEFUSE),
				TRUE, TRUE, TRUE, &u4BufLen);

		DBGLOG(INIT, INFO, "r-loop: offset(0x%x), len(0x%x), v:0x%x\n",
			u2CurOffset, u2BlockSize, prAdapter->aucEepromValid);

		u2DataOffset = u2CurOffset - u2Offset;
		if ((i4Ret == 0) && prAdapter->aucEepromValid) {
			i4Ret = kalMemCpyS(&HqaCmdFrame->Data[2 + u2DataOffset],
					u2RemainDataSize,
					&prAdapter->aucEepromVaule[0],
					u2BlockSize);
			if (i4Ret == 0) {
				u2RemainDataSize -= u2BlockSize;
				u2CurOffset += u2BlockSize;
			}
		}
	...
	ResponseToQA(HqaCmdFrame, prIwReqData, 2 + (u2CurOffset - u2Offset),
		!!i4Ret);
#endif /* CFG_SECEFUSE_ACCESS */
 ...
 */
/*----------------------------------------------------------------------------*/
static int32_t HQA_WriteBulkSecEEPROM(struct net_device
				   *prNetDev,
				   IN union iwreq_data *prIwReqData,
				   struct HQA_CMD_FRAME *HqaCmdFrame)
{
...
#if CFG_SECEFUSE_ACCESS
	uint32_t u4BufLen = 0;
	uint16_t u2Offset = 0, u2Len = 0, u2Loop = 0, u2LoopSize = 0;
	uint16_t u2CurOffset = 0, u2EndOffset = 0;
	uint16_t *Buffer = NULL;
	struct ADAPTER *prAdapter = NULL;
	struct GLUE_INFO *prGlueInfo = NULL;
	struct PARAM_CUSTOM_ACCESS_SECEFUSE rAccessEfuseInfo = { 0 };
	struct SEC_EFUSE_ACCESS_BLOCK {
		uint16_t u2Offset;
		uint16_t u2BlockSize;
	} rAccessBlock[(MAX_SEC_EEPROM_BUFFER_SIZE + EFUSE_BLOCK_SIZE - 1) /
		EFUSE_BLOCK_SIZE + 1] = { 0 };
	uint32_t u4BlockIdx = 0;
	uint16_t u2BlockSize = 0;
	uint32_t u4WithKeyHash = 0;
	...
	DBGLOG(INIT, INFO, "QA_AGENT HQA_WriteBulkSecEEPROM\n");

	/*
	 * HQA_WriteSecEEPROM (0x131E): HQA cmd for bulk write sec eeprom data
	 * Data[0..1]: offset
	 * Data[2..2+HqaCmdFrame->Length]: sec eeprom data to be written
	 */

	u2Offset = ntohs(*(uint16_t *)&HqaCmdFrame->Data[0]);
	u2Len = ntohs(HqaCmdFrame->Length) - 2;
	...
	if ((u2Offset >= MAX_SEC_EEPROM_BUFFER_SIZE) || (u2Len == 0) ||
		...
		(u2EndOffset < u2Offset)) {
		DBGLOG(INIT, ERROR,
			"Offset(0x%x), Len(0x%x) oob, overflow or len error.\n",
			u2Offset, u2Len);
		i4Ret = -EINVAL;
		goto end;
	}

	/* check and save seceeprom access block info */
	u2CurOffset = u2Offset;
	while (u2CurOffset < u2EndOffset) {
		if ((u2CurOffset >= SECEFUSE_KEYHASH_OFFSET_BEGIN) &&
			(u2CurOffset < SECEFUSE_KEYHASH_OFFSET_END)) {
		...
		if (((u2CurOffset % u2BlockSize) != 0) ||
			(u2CurOffset + u2BlockSize > u2EndOffset)) {
			DBGLOG(INIT, ERROR,
				"Offset(0x%x), Len(0x%x): not aligned error.\n",
				u2Offset, u2Len);
			i4Ret = -EINVAL;
			goto end;
		}

		rAccessBlock[u4BlockIdx].u2Offset = u2CurOffset;
		rAccessBlock[u4BlockIdx].u2BlockSize = u2BlockSize;
		u2CurOffset += u2BlockSize;
		u4BlockIdx++;
	}

	/*
	 * brick-safe-1: if key hash were not written or will not be written,
	 * don't enable secure boot.
	 */
	for (u2Loop = SECEFUSE_KEYHASH_OFFSET_BEGIN;
		(u2Loop < SECEFUSE_KEYHASH_OFFSET_END) &&
		(u4WithKeyHash == 0); u2Loop += SECEFUSE_KEYHASH_SIZE) {
		rAccessEfuseInfo.u4Address = u2Loop;
		rAccessEfuseInfo.u4Size = SECEFUSE_KEYHASH_SIZE;
		...
		if ((i4Ret == 0) && prAdapter->aucEepromValid) {
			...
			for (i = 0; i < SECEFUSE_KEYHASH_SIZE; i++) {
				if (prAdapter->aucEepromVaule[i]) {
					u4WithKeyHash = 1;
					break;
				}
			}
	...
	Buffer = kmalloc(sizeof(uint8_t) * u2Len, GFP_KERNEL);
	if (Buffer == NULL) {
		i4Ret = -ENOMEM;
		goto end;
	}
	kalMemSet(Buffer, 0, sizeof(uint8_t) * u2Len);
	i4Ret = kalMemCpyS(Buffer, sizeof(uint8_t) * u2Len,
			&HqaCmdFrame->Data[2], u2Len);
	if (i4Ret != 0)
		goto end;

	u2LoopSize = (u2Len >> 1);
	for (u2Loop = 0; u2Loop < u2LoopSize ; u2Loop++)
		Buffer[u2Loop] = ntohs(Buffer[u2Loop]);

	/*
	 * brick-safe-2: handle 32-byte keyhash block first, then 16 byte block.
	 */
	for (u2BlockSize = SECEFUSE_KEYHASH_SIZE; u2BlockSize > 0;
		u2BlockSize -= EFUSE_BLOCK_SIZE) {
		u4BlockIdx = 0;
		while (rAccessBlock[u4BlockIdx].u2BlockSize != 0) {
			uint16_t u2Tmp = 0, u2DataOffset = 0;

			if (rAccessBlock[u4BlockIdx].u2BlockSize !=
				u2BlockSize) {
				u4BlockIdx++;
				continue;
			}

			DBGLOG(INIT, INFO, "w-loop: offset(0x%x), len(0x%x)\n",
				rAccessBlock[u4BlockIdx].u2Offset, u2BlockSize);

			u2DataOffset = rAccessBlock[u4BlockIdx].u2Offset -
				u2Offset;
			for (u2Loop = 0; u2Loop < (u2BlockSize >> 1);
				u2Loop++) {
				u2Tmp |= Buffer[u2Loop + (u2DataOffset >> 1)];
			}
			if (u2Tmp == 0) {
				DBGLOG(INIT, INFO, "Nothing to write, skip.\n");
				u4BlockIdx++;
				continue;
			}

			rAccessEfuseInfo.u4Address =
				rAccessBlock[u4BlockIdx].u2Offset;
			rAccessEfuseInfo.u4Size = u2BlockSize;
			kalMemSet(rAccessEfuseInfo.aucData, 0,
				sizeof(rAccessEfuseInfo.aucData));
			i4Ret = kalMemCpyS(rAccessEfuseInfo.aucData,
				sizeof(rAccessEfuseInfo.aucData),
				(unsigned char *)Buffer + u2DataOffset,
				u2BlockSize);
			if (i4Ret != 0)
				goto end;

			DBGLOG(INIT, INFO, "Going for write e-Fuse,0x%x:0x%x\n",
					rAccessEfuseInfo.u4Address,
	...
	ResponseToQA(HqaCmdFrame, prIwReqData, 2 + u2Len, !!i4Ret);
	kfree(Buffer); /* null safe */
#endif /* CFG_SECEFUSE_ACCESS */
 ...
 */
/*----------------------------------------------------------------------------*/
static int32_t HQA_GetFmcwRawData(struct net_device *prNetDev,
			       IN union iwreq_data *prIwReqData,
			       struct HQA_CMD_FRAME *HqaCmdFrame)
{
	int32_t i4Ret = 0;
	int32_t u4BufLen = 0;
	uint32_t u4Length = 0;
	uint32_t u4FmcwDataCunt;
	struct GLUE_INFO *prGlueInfo = NULL;
	struct PARAM_MTK_FMCW_TEST_STRUCT rRfATInfo;
	...
	memcpy(&u4FmcwDataCunt, HqaCmdFrame->Data + 4 * 0, 4);
	u4FmcwDataCunt = ntohs(u4FmcwDataCunt);

	kalMemZero(&rRfATInfo, sizeof(struct PARAM_MTK_FMCW_TEST_STRUCT));

	rRfATInfo.u4FuncIndex = RF_AT_FUNCID_FMCW_RADAR_GET_DATA;
	rRfATInfo.u4FuncData = u4FmcwDataCunt;
	...
	i4Ret = kalIoctl(prGlueInfo,
			 wlanoidRftestFmcwQueryAutoTest, &rRfATInfo,
			 sizeof(rRfATInfo), TRUE, TRUE, TRUE, &u4BufLen);

	if (i4Ret < 0)
		return i4Ret;

	memcpy(HqaCmdFrame->Data + 2, rRfATInfo.u4FuncFmcwData,
		       sizeof(rRfATInfo.u4FuncFmcwData));
	memcpy(HqaCmdFrame->Data + 2 + sizeof(rRfATInfo.u4FuncFmcwData),
			&rRfATInfo.u4TxChainAdcTemp,
			sizeof(rRfATInfo.u4TxChainAdcTemp));

	u4Length = sizeof(rRfATInfo.u4TxChainAdcTemp) +
				sizeof(rRfATInfo.u4FuncFmcwData);

	ResponseToQA(HqaCmdFrame, prIwReqData, 2 + u4Length, i4Ret);

	return i4Ret;
}

static int32_t HQA_FmcwTestFlowCrtl(struct net_device *prNetDev,
				   IN union iwreq_data *prIwReqData,
				   struct HQA_CMD_FRAME *HqaCmdFrame)
{
	struct GLUE_INFO *prGlueInfo = NULL;
	struct RADAR_TEST_FLOW_CTRL rCmdFmcw = {0};
	uint32_t u4FmcwVal = 0;
	uint32_t u4SubCmdId = 0;
	uint32_t rStatus = WLAN_STATUS_SUCCESS;
	uint32_t u4BufLen = 0;
	...
	if (!prGlueInfo) {
		DBGLOG(RFTEST, ERROR, "NULL prGlueInfo!\n");
		return -EFAULT;
	}

	memcpy(&u4SubCmdId, HqaCmdFrame->Data + 4 * 0, 4);
	memcpy(&u4FmcwVal, HqaCmdFrame->Data + 4 * 1, 4);

	rCmdFmcw.u4SubCmdId = u4SubCmdId;
	rCmdFmcw.u4FmcwVal = u4FmcwVal;

	rStatus = kalIoctl(prGlueInfo, wlanoidFmcwTestFlowCtrl, &rCmdFmcw,
			   sizeof(struct RADAR_TEST_FLOW_CTRL),
			   TRUE, TRUE, TRUE, &u4BufLen);

	if (rStatus != WLAN_STATUS_SUCCESS) {
		DBGLOG(REQ, WARN, "fmcw test cmd error:%x\n", rStatus);
		return rStatus;
	}

	u4FmcwVal = rCmdFmcw.u4FmcwVal;
	memcpy(HqaCmdFrame->Data + 2, &u4FmcwVal,
		   sizeof(u4FmcwVal));

	ResponseToQA(HqaCmdFrame, prIwReqData, 2 + sizeof(u4FmcwVal), rStatus);
...
static HQA_CMD_HANDLER HQA_CMD_SET3[] = {
	...
	ToDoFunction,		/* 0x131B */
	ToDoFunction,		/* 0x131C */
	HQA_ReadBulkSecEEPROM,	/* 0x131D */
	HQA_WriteBulkSecEEPROM,	/* 0x131E */
};
				   ...
				   struct HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	ret = request_firmware_direct(&fw, aucEeprom, prGlueInfo->prDev);
...
static HQA_CMD_HANDLER HQA_CMD_SET5[] = {
	...
	ToDoFunction,		/* 0x153D */
#if CFG_SUPPORT_FMCW_RADAR
	HQA_GetFmcwRawData,	/* 0x153E */
	HQA_FmcwTestFlowCrtl,	/* 0x153F */
#endif
};
	...
	struct HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	kalSnprintf(prInBuf, sizeof(desire_bw),
		"%u", desire_bw);
	...
	struct HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	kalSnprintf(prInBuf, sizeof(desire_nc),
		"%u", desire_nc);
...
#if (CFG_CONNAC2_LISTMODE_ENABLE == 1)
static int32_t hqa_listmode_tx_seg(struct net_device
				   *prNetDev,
				   IN union iwreq_data *prIwReqData,
				   struct HQA_CMD_FRAME *HqaCmdFrame)
{
	int32_t i4Ret = 0;
	static struct list_mode_tx_seg_header ParserSegHeader = {0};
	struct list_mode_tx_seg_header *pSendSegHeader = NULL;
	uint32_t i = 0, seg_para_num = 0;
	uint32_t remain_seg_num = 0;
	u_char *pucPos = HqaCmdFrame->Data;

	memcpy(&ParserSegHeader.u4ExtId, pucPos, 4);
	pucPos += 4;
	ParserSegHeader.u4ExtId = ntohl(ParserSegHeader.u4ExtId);

	memcpy(&ParserSegHeader.u4FC, pucPos, 4);
	pucPos += 4;
	ParserSegHeader.u4FC = ntohl(ParserSegHeader.u4FC);

	memcpy(&ParserSegHeader.u4Dur, pucPos, 4);
	pucPos += 4;
	ParserSegHeader.u4Dur = ntohl(ParserSegHeader.u4Dur);

	memcpy(&ParserSegHeader.u4SeqID, pucPos, 4);
	pucPos += 4;
	ParserSegHeader.u4SeqID = ntohl(ParserSegHeader.u4SeqID);

	memcpy(&ParserSegHeader.u4TxLen, pucPos, 4);
	pucPos += 4;
	ParserSegHeader.u4TxLen = ntohl(ParserSegHeader.u4TxLen);

	memcpy(&ParserSegHeader.aucSourceAddr, pucPos, SERV_MAC_ADDR_LEN);
	pucPos += SERV_MAC_ADDR_LEN;

	memcpy(&ParserSegHeader.aucDestAddr, pucPos, SERV_MAC_ADDR_LEN);
	pucPos += SERV_MAC_ADDR_LEN;

	memcpy(&ParserSegHeader.aucBSSID, pucPos, SERV_MAC_ADDR_LEN);
	pucPos += SERV_MAC_ADDR_LEN;

	memcpy(&ParserSegHeader.u4STBC, pucPos, 4);
	pucPos += 4;
	ParserSegHeader.u4STBC = ntohl(ParserSegHeader.u4STBC);

	memcpy(&ParserSegHeader.u4SegNum, pucPos, 4);
	pucPos += 4;
	ParserSegHeader.u4SegNum = ntohl(ParserSegHeader.u4SegNum);

	memcpy(&ParserSegHeader.u4SegParaNum, pucPos, 4);
	pucPos += 4;
	ParserSegHeader.u4SegParaNum = ntohl(ParserSegHeader.u4SegParaNum);

	/* header parser */
	if (ParserSegHeader.u4SegParaNum >= SEGPARANUM_MAX)
		ParserSegHeader.u4SegParaNum = SEGPARANUM_MAX;

	pSendSegHeader = kalMemAlloc(
			sizeof(struct list_mode_tx_seg_header) +
			(LIST_MODE_FW_SEG_NUM_MAX * sizeof(uint32_t) *
			ParserSegHeader.u4SegParaNum),
			VIR_MEM_TYPE);

	if (pSendSegHeader == NULL)
		goto leave_hqa_listmode_tx_seg;
	...
	while (remain_seg_num != 0) {
		...
		memcpy(pSendSegHeader, &ParserSegHeader,
			sizeof(struct list_mode_tx_seg_header));
		...
		for (i = 0; i < seg_para_num; i++) {
			memcpy(&pSendSegHeader->au4Buffer[i],
			HqaCmdFrame->Data + 4 * 7 +
			SERV_MAC_ADDR_LEN * 3 + 4*(i+1),
			sizeof(uint32_t));
			pSendSegHeader->au4Buffer[i] =
				ntohl(pSendSegHeader->au4Buffer[i]);
		}

		i4Ret =
		MT_ATE_listmode_cmd(prNetDev, pSendSegHeader, LISTMODE_TX_SEG);
	}

leave_hqa_listmode_tx_seg:
	if (pSendSegHeader != NULL)
		kalMemFree(pSendSegHeader, VIR_MEM_TYPE,
			sizeof(struct list_mode_tx_seg_header) +
			(LIST_MODE_FW_SEG_NUM_MAX * sizeof(uint32_t) *
			ParserSegHeader.u4SegParaNum));

	/* Update hqa_frame with response: status (2 bytes) */
	memcpy(HqaCmdFrame->Data + 2, (u_char *)&ParserSegHeader.u4ExtId,
...
static int32_t hqa_listmode_tx_start(struct net_device
				   *prNetDev,
				   IN union iwreq_data *prIwReqData,
				   struct HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	memcpy(&ext_id, HqaCmdFrame->Data + 4 * 0, 4);
	ext_id = ntohl(ext_id);
	...
	if (pbuf == NULL)
		goto leave_hqa_listmode_tx_start;

	/* alloc buffer to receive from FW */
	pRsp =
	kalMemAlloc(sizeof(struct list_mode_event),
	VIR_MEM_TYPE);
	...
	ret = MT_ATE_listmode_cmd(prNetDev, ptx_seg, LISTMODE_TX_SEG);

leave_hqa_listmode_tx_start:
	if (pbuf != NULL)
		kalMemFree(pbuf,
			VIR_MEM_TYPE,
			sizeof(struct list_mode_tx_seg_header));
	...
	/* Update hqa_frame with response: status (2 bytes) */
	memcpy(HqaCmdFrame->Data + 2, (u_char *)&ext_id,
			sizeof(ext_id));
	ResponseToQA(HqaCmdFrame, prIwReqData, 2 + sizeof(ext_id), ret);

	return ret;
}

static int32_t hqa_listmode_tx_stop(struct net_device
				   *prNetDev,
				   IN union iwreq_data *prIwReqData,
				   struct HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	if (pbuf == NULL)
		goto leave_hqa_listmode_tx_stop;

	/* alloc buffer to receive from FW */
	pRsp = kalMemAlloc(sizeof(struct list_mode_event),
	VIR_MEM_TYPE);
...
leave_hqa_listmode_tx_stop:
	if (pbuf != NULL) {
		kalMemFree(pbuf,
			VIR_MEM_TYPE,
			izeof(struct list_mode_tx_seg_header));
	}

	if (pRsp != NULL)
		kalMemFree(pRsp, VIR_MEM_TYPE, sizeof(struct list_mode_event));
...
static int32_t hqa_listmode_rx_seg(struct net_device
				   *prNetDev,
				   IN union iwreq_data *prIwReqData,
				   struct HQA_CMD_FRAME *HqaCmdFrame)
{
	uint32_t ret = WLAN_STATUS_SUCCESS;
	u_char *ucData = HqaCmdFrame->Data + 4 * 3 + 6;
	struct list_mode_rx_seg_header ParserSegHeader = {0};
	struct list_mode_event *pRsp = NULL;
	struct list_mode_rx_seg_header *pSendSegHeader = NULL;
	uint32_t i = 0, seg_size = 0, seg_para_num = 0;
	uint32_t remain_seg_num = 0;

	/* header parser */
	memcpy(&ParserSegHeader.u4ExtId, HqaCmdFrame->Data + 4 * 0, 4);
	ParserSegHeader.u4ExtId =
	ntohl(ParserSegHeader.u4ExtId);

	memcpy(&ParserSegHeader.aucOwnMac,
	HqaCmdFrame->Data + 4 * 1, 6);
	// no need to change endian

	memcpy(&ParserSegHeader.u4SegNum,
	HqaCmdFrame->Data + 4 * 1 + 6, 4);
	ParserSegHeader.u4SegNum =
	ntohl(ParserSegHeader.u4SegNum);

	memcpy(&ParserSegHeader.u4SegParaNum,
	HqaCmdFrame->Data + 4 * 2 + 6, 4);
	ParserSegHeader.u4SegParaNum =
	ntohl(ParserSegHeader.u4SegParaNum);
	...
	if (pRsp == NULL)
		goto leave_hqa_listmode_rx_seg;

	/* alloc buffer to send FW */
	pSendSegHeader = kalMemAlloc(
		sizeof(struct list_mode_rx_seg_header) +
		(LIST_MODE_FW_SEG_NUM_MAX *
		sizeof(uint32_t) *	ParserSegHeader.u4SegParaNum),
		VIR_MEM_TYPE);
	...
	while (remain_seg_num != 0) {
		...
		for (i = 0; i < seg_para_num; i++) {
			ucData += 4*i;
			memcpy(&pSendSegHeader->au4Buffer[i],
			HqaCmdFrame->Data + 4 * 3 + 6 + 4*i, 4);
			pSendSegHeader->au4Buffer[i] =
			ntohl(pSendSegHeader->au4Buffer[i]);
		}

		ret = MT_ATE_listmode_cmd(prNetDev,
		pSendSegHeader, LISTMODE_RX_SEG);

		if (ret != WLAN_STATUS_SUCCESS)
			goto leave_hqa_listmode_rx_seg;
	}

leave_hqa_listmode_rx_seg:
	if (pSendSegHeader != NULL)
		kalMemFree(pSendSegHeader,
		VIR_MEM_TYPE,
		sizeof(struct list_mode_rx_seg_header));

	if (pRsp != NULL) {
		if (ret == WLAN_STATUS_SUCCESS)
			ret = (uint32_t)pRsp->u2Status;
		kalMemFree(pRsp, VIR_MEM_TYPE, sizeof(struct list_mode_event));
	}

	/* Update hqa_frame with response: status (2 bytes) */
	memcpy(HqaCmdFrame->Data + 2,
		(u_char *)&ParserSegHeader.u4ExtId,
		sizeof(ParserSegHeader.u4ExtId));
	ResponseToQA(HqaCmdFrame,
		prIwReqData,
		2 + sizeof(ParserSegHeader.u4ExtId),
		ret);

	return ret;
}

static int32_t hqa_listmode_rx_start(struct net_device
				   *prNetDev,
				   IN union iwreq_data *prIwReqData,
				   struct HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	if (pbuf == NULL)
		goto leave_hqa_listmode_rx_start;
	...
	ret = MT_ATE_listmode_cmd(prNetDev, prx_seg, LISTMODE_RX_SEG);

	if (ret != WLAN_STATUS_SUCCESS)
		goto leave_hqa_listmode_rx_start;

leave_hqa_listmode_rx_start:
	if (pbuf != NULL)
		kalMemFree(
		pbuf,
		VIR_MEM_TYPE,
...
static int32_t hqa_listmode_rx_stop(struct net_device
				   *prNetDev,
				   IN union iwreq_data *prIwReqData,
				   struct HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	if (pbuf == NULL)
		goto leave_hqa_listmode_rx_stop;
...
static int32_t hqa_listmode_rx_get_status(struct net_device
				   *prNetDev,
				   IN union iwreq_data *prIwReqData,
				   struct HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	uint8_t *ptr = HqaCmdFrame->Data + 2;
	uint32_t total_frame, item_num, convert, i, *cast = NULL;
	uint32_t seg_num_start = 0;
	...
	if (pSend == NULL) {
		ret = WLAN_STATUS_FAILURE;
		goto leave_hqa_listmode_rx_get_status;
	}
	...
	for (seg_num_start = 0; seg_num_start < LIST_SEG_MAX;) {
		...
		ret = MT_ATE_listmode_rx_get_status(
			prNetDev,
			pget_status,
			prx_event);
		...
		if (seg_num_start == 0) {
			...
			convert = htonl(prx_event->u4ExtId);
			memcpy(ptr, &convert, sizeof(convert));
			ptr += sizeof(convert);
			total_frame += sizeof(convert);

			convert = htonl(prx_event->u4SegNumTotal);
			memcpy(ptr, &convert, sizeof(convert));
			ptr += sizeof(convert);
			total_frame += sizeof(convert);
		}

		cast = (uint32_t *)&prx_event->tRxStatus;
	...
	/* Update hqa_frame with response: status (2 bytes) */
	ResponseToQA(HqaCmdFrame, prIwReqData, 2 + total_frame, ret);
	//update_hqa_frame(hqa_frame, total_frame + 2, ret);

leave_hqa_listmode_rx_get_status:
	if (ret != WLAN_STATUS_SUCCESS) {
		/* Update hqa_frame with response: status (2 bytes) */
		memcpy(HqaCmdFrame->Data + 2,
		(u_char *)&ext_id,
...
#if (CFG_CONNAC2_LISTMODE_ENABLE == 1)
	hqa_listmode_tx_seg,			/* 0x00000010 */
	hqa_listmode_tx_start,			/* 0x00000011 */
	ToDoFunction,					/* 0x00000012 */
	hqa_listmode_tx_stop,			/* 0x00000013 */
	hqa_listmode_rx_seg,			/* 0x00000014 */
	hqa_listmode_rx_start,			/* 0x00000015 */
	hqa_listmode_rx_get_status,		/* 0x00000016 */
	hqa_listmode_rx_stop,			/* 0x00000017 */
#else
 ...
 */
/*----------------------------------------------------------------------------*/
int __priv_qa_agent(IN struct net_device *prNetDev,
		  ...
		  IN union iwreq_data *prIwReqData, IN char *pcExtra)
{
	...
	if (!HqaCmdFrame) {
		DBGLOG(RFTEST, ERROR, "ERROR HqaCmdFrame ptr\n");
		i4Status = -ENOMEM;
		goto ERROR0;
	}

...
static const struct iw_priv_args rIwPrivTable[] = {
	...
	{IOCTL_IWPRIV_ATE,
		IW_PRIV_TYPE_CHAR | sizeof(struct HQA_CMD_FRAME), 0, ""},
...
/*----------------------------------------------------------------------------*/
/*!
 * \brief Find the desired RSN selection
 *	  Information Element according to desiredElemID.
 *
 * \param[in] pucIEStart IE starting address.
 * \param[in] i4TotalIeLen Total length of all the IE.
 * \param[in] ucDesiredElemId Desired element ID.
...
u_int8_t wextSrchDesiredRsnSelectionIE(IN uint8_t *pucIEStart,
		       IN int32_t i4TotalIeLen, OUT uint8_t **ppucDesiredIE)
{
	int32_t i4InfoElemLen;
	u_int8_t ret = FALSE;
	...
	while (i4TotalIeLen >= 2) {
		...
		if (pucIEStart[0] == ELEM_ID_VENDOR
		    && i4InfoElemLen <= i4TotalIeLen) {
			if (pucIEStart[1] >= 4) {
				if (memcmp(&pucIEStart[2], "\x50\x6f\x9a\x2c",
				    4) == 0) {
					*ppucDesiredIE = &pucIEStart[0];
					return TRUE;
				}
		...
		IN struct iw_point *prEnc, IN char *pcExtra)
{
	...
	if (prWepKey == NULL) {
		DBGLOG(INIT, INFO, "alloc buffer fail\n");
		return -ENOMEM;
	}
		    ...
		    IN struct iw_point *prEnc, IN char *pcExtra)
{
	...
	kalMemZero(keyStructBuf, KEY_BUF_SIZE);
...
static int __std_set_priv(struct net_device *prDev,
		struct iw_request_info *rIwReqInfo,
		union iwreq_data *prData,
		char *pcExtra)
{
		...
		char *pcExtra)
{
	...
	DBGLOG(REQ, LOUD, "cmd=%x, flags=%x\n",
	     rIwReqInfo->cmd, rIwReqInfo->flags);
	DBGLOG(REQ, LOUD, "mode=%x, flags=%x\n",
	     prData->mode, prData->data.flags);
	return compat_priv(prDev, rIwReqInfo,
	     prData, pcExtra, __std_set_priv);
}

...
uint8_t g_fgIsNanAtResetFlow = FALSE;
...
/*----------------------------------------------------------------------------*/
/*!
 * \brief To check NAN is in reset state or not
 *
 * \param[in] void
 *
 * \return   TRUE
 ...
 */
/*----------------------------------------------------------------------------*/
uint8_t isNanAtResetFlow(void)
{
	return g_fgIsNanAtResetFlow;
}

/*----------------------------------------------------------------------------*/
/*!
 * \brief To set the status if NAN start/end the reset state
 *
 ...
 */
/*----------------------------------------------------------------------------*/
void nanSetAtResetFlow(uint8_t ucValue)
{
	if ((ucValue != FALSE) && (ucValue != TRUE))
		g_fgIsNanAtResetFlow = FALSE;
	else
		g_fgIsNanAtResetFlow = ucValue;
}

/*----------------------------------------------------------------------------*/
/*!
 * \brief deinit NAN stack at reset flow.
 *
 ...
 */
/*----------------------------------------------------------------------------*/
uint32_t nanDisableAtReset(struct GLUE_INFO *prGlueInfo)
{
	enum NanStatusType rWlanStatus = NAN_STATUS_SUCCESS;
	struct net_device *prNetDev = NULL;
	struct ADAPTER *prAdapter = NULL;

	if (prGlueInfo == NULL) {
		DBGLOG(NAN, ERROR, "prGlueInfo is NULL\n");
		rWlanStatus = NAN_STATUS_INVALID_PARAM;
		goto end;
	}
	...
	if (!nanIsRegistered(prAdapter)) {
		DBGLOG(NAN, INFO, "NAN not registered yet\n");
		goto end;
	}

	/* do NAN reset */
	prNetDev = wlanGetNetDev(prGlueInfo, AIS_DEFAULT_INDEX);
	if (prNetDev == NULL) {
		DBGLOG(NAN, ERROR, "Cannot get netdev.\n");
		rWlanStatus = NAN_STATUS_INVALID_PARAM;
		goto end;
	}

	nanSetAtResetFlow(TRUE);

	if (prAdapter->fgNanEnable)
		rWlanStatus = nanDisableHandler(prAdapter);

	nanSetAtResetFlow(FALSE);

end:
	if (rWlanStatus == NAN_STATUS_SUCCESS)
		DBGLOG(NAN, INFO, "NAN reset success\n");
	else
		DBGLOG(NAN, INFO, "NAN reset fail: 0x%08x\n", rWlanStatus);
...
/*----------------------------------------------------------------------------*/
unsigned char
nanFreeInfo(struct GLUE_INFO *prGlueInfo)
{
	struct ADAPTER *prAdapter = NULL;
	struct WIFI_VAR *prWifiVar = NULL;
	uint8_t ucNdiIdx = 0;
	...
	for (u4Idx = 0; u4Idx < NAN_BSS_INDEX_NUM; u4Idx++) {
		if (prWifiVar->aprNanSpecificBssInfo[u4Idx] != NULL) {
			kalMemFree(prWifiVar->aprNanSpecificBssInfo[u4Idx],
				VIR_MEM_TYPE,
				sizeof(struct _NAN_SPECIFIC_BSS_INFO_T));
			prWifiVar->aprNanSpecificBssInfo[u4Idx] = NULL;
		}
	}
	for (ucNdiIdx = 0; ucNdiIdx < NAN_NDI_INDEX_NUM; ucNdiIdx++) {
		if (prGlueInfo->aprNANDevInfo[ucNdiIdx] != NULL) {
			kalMemFree(prGlueInfo->aprNANDevInfo[ucNdiIdx],
				VIR_MEM_TYPE,
				sizeof(struct _GL_NAN_INFO_T *));
			prGlueInfo->aprNANDevInfo[ucNdiIdx] = NULL;
		}
	}
	    ...
	    unsigned char fgIsRtnlLockAcquired)
{
	...
	uint8_t ucIdx = NAN_BSS_INDEX_MAIN;
...
glSetupNAN(struct GLUE_INFO *prGlueInfo, struct wireless_dev *prNanWdev,
	   struct net_device *prNanDev, uint8_t ucRoleIdx)
{
	struct ADAPTER *prAdapter = NULL;
	struct _GL_NAN_INFO_T *prNANInfo = NULL;
	...
	/*0. allocate naninfo */
	if (nanAllocInfo(prGlueInfo, ucRoleIdx) != TRUE) {
		DBGLOG(INIT, WARN, "Allocate memory for nan FAILED\n");
		return -1;
	}

	prNANInfo = prGlueInfo->aprNANDevInfo[ucRoleIdx];
	...
	for (ucIdx = 0; ucIdx < NAN_BSS_INDEX_NUM; ucIdx++) {
		...
		ucBssIndex = nanBssInit(prGlueInfo->prAdapter, ucIdx);

		if (ucBssIndex == MAX_BSS_INDEX) {
			DBGLOG(INIT, ERROR, "No BSS can be used!!\n");
			nanFreeInfo(prGlueInfo);
			return -1;
		}
		if (ucIdx == NAN_BSS_INDEX_MAIN)
...
mtk_nan_wext_set_Multicastlist(struct GLUE_INFO *prGlueInfo)
{
	...
	if (u4PacketFilter & PARAM_PACKET_FILTER_MULTICAST) {
		...
		if (!prMCAddrList) {
			DBGLOG(NAN, ERROR, "Allocate addr list fail!\n");
			return;
		}
...
uint32_t nanSyncInfoInit(struct ADAPTER *prAdapter)
{
	if (!prAdapter) {
		DBGLOG(NAN, ERROR, "prAdapter error!\n");
	...
	kalMemZero(&prAdapter->rNanSyncInfo, sizeof(struct _NAN_SYNC_INFO_T));
...
/* allocate netdev */
	...
	if (prGlueInfo->prAdapter->rWifiVar.ucNanMacAddrOverride > 0) {
		if (prGlueInfo->prAdapter->rWifiVar.ucNanMacAddrOverride == 1)
			...
			prAdapter->ucNanMacAddr);
		COPY_MAC_ADDR(rRandMacAddr,
			prAdapter->ucNanMacAddr);
	...
	kal_eth_hw_addr_set(prNanDev, rRandMacAddr);
	...
	nanDevEnableGtkReqChecker(prAdapter,
		NAN_NDI_INDEX_0, TRUE);
#endif
...
glNanCreateWirelessDevice(struct GLUE_INFO *prGlueInfo)
{
	...
	struct wireless_dev **prOrigWdev = wlanGetWirelessDevice(prGlueInfo);
	struct wiphy *prWiphy = wlanGetWiphy(*prOrigWdev);
...
nanSetMulticastList(IN struct net_device *prDev)
{
	...
	uint8_t ucRoleIdx = NAN_BSS_INDEX_MAIN;
...
nanHardStartXmit(IN struct sk_buff *prSkb, IN struct net_device *prDev)
{
	...
	struct TX_PACKET_INFO rTxPktInfo = {0};
	...
	if (kalQoSFrameClassifierAndPacketInfo(
			prGlueInfo, prSkb, &rTxPktInfo)) {

		if (IS_BMCAST_MAC_ADDR(rTxPktInfo.aucEthDestAddr)) {
			DBGLOG(NAN, INFO, "TX with DA = BMCAST\n");
#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_DATA == 1)
			prNanBssInfo = nanFindBssInfoByGroupAddr(
				prGlueInfo->prAdapter,
				rTxPktInfo.aucEthDestAddr,
				rTxPktInfo.aucEthSrcAddr);
			if (prNanBssInfo) {
				ucBssIndex = prNanBssInfo->ucBssIndex;
				DBGLOG(NAN, LOUD,
					"prNanBssInfo bssIndex:%d\n",
						ucBssIndex);
			} else {
				DBGLOG(NAN, ERROR,
					"nanFindBssInfoByGroupAddr has error\n");
			}
#endif
		}
		else {
			prStaRec = nanDataEngineSearchStaRec(
				prGlueInfo->prAdapter,
				NULL, rTxPktInfo.aucEthSrcAddr,
				rTxPktInfo.aucEthDestAddr);
			if (prStaRec != NULL) {
				...
				DBGLOG(NAN, ERROR,
					"nanDataEngineSearchStaRec has error\n");
			}
		}
	}
#endif
...
int nanSetMacAddress(struct net_device *ndev, void *addr)
{
	...
	DBGLOG(NAN, INFO, "Get Mac address from host: " MACSTR ".\n",
		MAC2STR(sa->sa_data));

	COPY_MAC_ADDR(prAdapter->ucNanMacAddr, sa->sa_data);

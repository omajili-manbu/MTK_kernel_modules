...
struct DYNAMIC_POWER_CTRL_INFO g_dynamicPowerCtrlInfo[CFG_MAX_WLAN_DEVICES];
#endif
/*******************************************************************************
 *                                 M A C R O S
 *******************************************************************************
...
#endif
#if CFG_SUPPORT_SER_DEBUGFS
static int32_t resetCreateSerDbgFs(struct GLUE_INFO *prGlueInfo);
static void resetRemoveSerDbgFs(struct GLUE_INFO *prGlueInfo);
#endif
...
static void glResetInitWakeLock(struct GLUE_INFO *prGlueInfo)
{
	...
	KAL_WAKE_LOCK_INIT(NULL, lock, "WLAN_ChipReset");
#endif
}
...
#if CFG_CONN_DYNAMIC_POWER_CTRL
struct DYNAMIC_POWER_CTRL_INFO *getDynamicPowerCtrlInfoByBusId(uint32_t busId)
{
	int idx = 0;
	...
	for (; idx < CFG_MAX_WLAN_DEVICES; idx++)
		if (g_dynamicPowerCtrlInfo[idx].busId == busId)
			return &g_dynamicPowerCtrlInfo[idx];
	return NULL;
}
#endif
			   ...
			   u_int8_t fgIsProbe)
{
	uint32_t bus_id = 0;
#if CFG_CONN_DYNAMIC_POWER_CTRL
	struct DYNAMIC_POWER_CTRL_INFO *prDynamicPowerCtrlInfo = NULL;
#endif
	...
	DBGLOG(INIT, INFO, "Enter with bus_id %d\n", bus_id);
#if CFG_CONN_DYNAMIC_POWER_CTRL
	prDynamicPowerCtrlInfo = getDynamicPowerCtrlInfoByBusId(bus_id);
#endif
...
#if CFG_CONN_DYNAMIC_POWER_CTRL
		/*probe done, clear chole chip rst status*/
...
#if CFG_CONN_DYNAMIC_POWER_CTRL
		if (prDynamicPowerCtrlInfo &&
			prDynamicPowerCtrlInfo->
			fgIsPendingForPowerOffReady) {
			prDynamicPowerCtrlInfo->
				fgIsPendingForPowerOffReady = FALSE;
			DBGLOG(INIT, INFO,
				"send ready event pending by powerOffReady\n");
			glSendResetEvent(bus_id,
...
#if CFG_SUPPORT_SER_DEBUGFS
	resetRemoveSerDbgFs(prGlueInfo);
#endif
}
			     ...
			     IN uint32_t u4RstFlag)
{
	...
	if ((prAdapter == NULL) ||
		...
		|| prAdapter->eWfsysResetState == WFSYS_RESET_STATE_POSTPONE) {
			glSetResettingFlag(prGlueInfo, TRUE);
			glSetRstPreventFwOwn(prGlueInfo, TRUE);
			prAdapter->eWfsysResetState = WFSYS_RESET_STATE_DETECT;
			schedule_work(
...
#if CFG_CONN_DYNAMIC_POWER_CTRL
		if (get_wifi_process_status() == WRITE_PROCESSING_OFF) {
			DBGLOG(INIT, ERROR, "wifi off process, skip reset!\n");
			return;
		}
		update_whole_chip_rst_status(TRUE);
#endif
		glSetResettingFlag(prGlueInfo, TRUE);
	}
...
void resetkoNotifyFunc(unsigned int event, void *data)
{
	...
	for (i = 0; i < CFG_MAX_WLAN_DEVICES; i++) {
		...
		if (prGlueInfo != NULL &&
		    prGlueInfo->rHifInfo.pdev != NULL &&
		    prGlueInfo->rHifInfo.pdev->bus != NULL) {
			tmp_bus_id = glGetBusId(prGlueInfo->rHifInfo.pdev);
	...
	/*found dynamic power ctrl info by bus id*/
#if CFG_CONN_DYNAMIC_POWER_CTRL
	prDynamicPowerCtrlInfo = getDynamicPowerCtrlInfoByBusId(bus_id);
#endif
	...
	if (prGlueInfo == NULL || (prGlueInfo != NULL &&
		...
		DBGLOG(INIT, WARN, "%s: prGlueInfo not ready\n",
			__func__);
		fgGlueInfoReady = FALSE;
	}

	if (event == MODULE_NOTIFY_MESSAGE && fgGlueInfoReady) {
...
#endif
	} else if (event == MODULE_NOTIFY_PRE_POWER_OFF && fgGlueInfoReady) {
	...
	} else if (event == MODULE_NOTIFY_PRE_FUNCTION_OFF) {
		DBGLOG(INIT, INFO,
			"RST ko notify wifi driver to do remove action\n");
#if (CFG_CONN_DYNAMIC_POWER_CTRL && defined(_HIF_PCIE))
			if (wlanIsProbingOrRemoving(TRUE, bus_id) ||
				wlanIsProbingOrRemoving(FALSE, bus_id)) {
				DBGLOG(INIT, INFO,
					"should pending ready event\n");
				if (prDynamicPowerCtrlInfo)
					prDynamicPowerCtrlInfo->
					fgIsPendingForPowerOffReady = TRUE;
			}
#endif
	}
}
...
enum SerTestType {
	SER_TEST_TYPE_FW_ASSERT = 0,
	SER_TEST_TYPE_BUS_HUNG,
	SER_TEST_TYPE_BUS_DISCONNECT,
	SER_TEST_TYPE_TRIGGER_L0,
	SER_TEST_TYPE_TRIGGER_L0P5_IN_RESUME,
	SER_TEST_TYPE_TRIGGER_L0P5_IN_SUSPEND,

	SER_TEST_TYPE_MAX
};

#define SER_DBGFS_BUF_SIZE 8
static char ser_dbgfs_write_buf[SER_DBGFS_BUF_SIZE];
static char ser_dbgfs_read_buf[] =
	"echo XX > ser_test\n"
	"XX:\n"
	"  0: SER_TEST_TYPE_FW_ASSERT\n"
	"  1: SER_TEST_TYPE_BUS_HUNG (USB)\n"
	"  2: SER_TEST_TYPE_BUS_DISCONNECT (USB)\n"
	"  3: SER_TEST_TYPE_TRIGGER_L0 (ResetKo)\n"
	"  4: SER_TEST_TYPE_TRIGGER_L0P5_IN_RESUME\n"
	"  5: SER_TEST_TYPE_TRIGGER_L0P5_IN_SUSPEND\n";


static ssize_t ser_dbgfs_read(struct file *file, char __user *buf,
			      size_t len, loff_t *ppos)
{
	return simple_read_from_buffer(buf, len, ppos, ser_dbgfs_read_buf,
					strlen(ser_dbgfs_read_buf));
}

ssize_t ser_dbgfs_write(struct file *file, const char __user *buf,
			size_t len, loff_t *ppos)
{
	struct GLUE_INFO *prGlueInfo = (struct GLUE_INFO *)file->private_data;
	...
	uint32_t u4BufLen = 0;
	int input;
	enum SerTestType eTestType;
	ssize_t ret;

	if (prGlueInfo == NULL)
		return 0;

	memset(ser_dbgfs_write_buf, 0, SER_DBGFS_BUF_SIZE);
	ret = simple_write_to_buffer(ser_dbgfs_write_buf,
				     SER_DBGFS_BUF_SIZE - 1, ppos, buf, len);
	if (ret < 0) {
		DBGLOG(INIT, ERROR, "[SER] input error\n");
		return ret;
	}

	if (kstrtoint(ser_dbgfs_write_buf, 10, &input) == 0) {
		eTestType = (enum SerTestType)input;
	} else {
		DBGLOG(INIT, ERROR, "[SER] input is not a number\n");
		return ret;
	}

	if (eTestType == SER_TEST_TYPE_FW_ASSERT) {
		/* fw assert */
		DBGLOG(INIT, STATE, "[SER] SER_TEST_TYPE_FW_ASSERT\n");
		rChipConfigInfo.ucType = CHIP_CONFIG_TYPE_WO_RESPONSE;
		rChipConfigInfo.u2MsgSize = kalStrnLen("assert",
		...
		kalIoctl(prGlueInfo, wlanoidSetChipConfig,
			&rChipConfigInfo, sizeof(rChipConfigInfo),
			FALSE, FALSE, TRUE,
			&u4BufLen);
#if defined(_HIF_USB)
	} else if (eTestType == SER_TEST_TYPE_BUS_HUNG) {
		/* bus busy */
		DBGLOG(INIT, STATE, "[SER] SER_TEST_TYPE_BUS_HUNG\n");
		rCmdAccessReg.u4Address = 0x74011804;
		rCmdAccessReg.u4Data = 0x00000001;

		kalIoctl(prGlueInfo, wlanoidSetMcrWrite,
			&rCmdAccessReg, sizeof(rCmdAccessReg),
			FALSE, FALSE, TRUE,
			&u4BufLen);
	} else if (eTestType == SER_TEST_TYPE_BUS_DISCONNECT) {
		/* bus disconnect */
		DBGLOG(INIT, STATE, "[SER] SER_TEST_TYPE_BUS_DISCONNECT\n");
...
#endif
#if CFG_CHIP_RESET_KO_SUPPORT
	} else if (eTestType == SER_TEST_TYPE_TRIGGER_L0) {
		/* trigger L0 reset */
		DBGLOG(INIT, STATE, "[SER] SER_TEST_TYPE_TRIGGER_L0\n");
#if CFG_SUPPORT_MULTI_CARD
#if defined(_HIF_USB)
		bus_id = glGetBusId(prGlueInfo->rHifInfo.intf);
#endif
		...
		glSendResetEvent(bus_id,
				...
				RFSM_EVENT_TRIGGER_RESET);
#endif
	} else if (eTestType == SER_TEST_TYPE_TRIGGER_L0P5_IN_RESUME) {
		/* pre resume cmd timeout */
		DBGLOG(INIT, STATE,
			"[SER] SER_TEST_TYPE_TRIGGER_L0P5_IN_RESUME\n");
		prGlueInfo->fgTestL0p5InResume = TRUE;
	} else if (eTestType == SER_TEST_TYPE_TRIGGER_L0P5_IN_SUSPEND) {
		/* trigger L0.5 reset durning suspend */
		DBGLOG(INIT, STATE,
			"[SER] SER_TEST_TYPE_TRIGGER_L0P5_IN_SUSPEND\n");
		prGlueInfo->fgTestL0p5InSuspend = TRUE;
	}
...
static const struct file_operations fops_ser_dbgfs = {
	.owner = THIS_MODULE,
	.read = ser_dbgfs_read,
	.write = ser_dbgfs_write,
	.open = simple_open,
};
...
static int32_t resetCreateSerDbgFs(struct GLUE_INFO *prGlueInfo)
{
	...
	if (u4Idx < CFG_MAX_WLAN_DEVICES)
		kalSnprintf(rootDir, 50, "mtk_ser_dbgfs_%d", u4Idx);
	else
#endif
		kalSnprintf(rootDir, 50,  "mtk_ser_dbgfs_0");

	prGlueInfo->fgTestL0p5InResume = FALSE;
	prGlueInfo->fgTestL0p5InSuspend = FALSE;

	prGlueInfo->serDbgFsDir = debugfs_create_dir(rootDir, NULL);
	if (!(prGlueInfo->serDbgFsDir)) {
		DBGLOG(INIT, ERROR, "create mtk_ser_dbgfs fail\n");
		return -1;
	}

	/* /sys/kernel/debug/mtk_ser_dbgfs_0/ser_test, mode: wr */
	if (!debugfs_create_file("ser_test",
				 0644, prGlueInfo->serDbgFsDir, prGlueInfo,
				 &fops_ser_dbgfs))
		DBGLOG(INIT, WARN,
			"create ser_test dgbfs node fail\n");
...
static void resetRemoveSerDbgFs(struct GLUE_INFO *prGlueInfo)
{
	...
	debugfs_remove_recursive(prGlueInfo->serDbgFsDir);
}
#endif  /* CFG_SUPPORT_SER_DEBUGFS */
#endif  /* CFG_CHIP_RESET_SUPPORT */

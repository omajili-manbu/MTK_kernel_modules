		...
		IN uint8_t ucRoleIndex)
{
	...
	if (prGlueP2pInfo == NULL || prGlueP2pInfo->chandef == NULL)
		return;
	 ...
	 */
	DBGLOG(INIT, INFO, "Rdd Detect Event Update to OS\n");
	cfg80211_radar_event(prGlueP2pInfo->prWdev->wiphy,
		prGlueP2pInfo->chandef, GFP_KERNEL);
	DBGLOG(INIT, INFO, "Rdd Detect Event Update to OS Done\n");
		...
		IN uint8_t ucRoleIndex)
{
	...
	&& prNetdevice->ieee80211_ptr) {
#if KERNEL_VERSION(6, 12, 0) <= CFG80211_VERSION_CODE
		prNetdevice->ieee80211_ptr->links[0].cac_time_ms =
			  p2pFuncGetDriverCacTime() * 1000;
#else
		prNetdevice->ieee80211_ptr->cac_time_ms =
			  p2pFuncGetDriverCacTime() * 1000;
#endif /* end KERNEL_VERSION(6, 12, 0) */
		DBGLOG(INIT, INFO, "set Manual cactime [%d]sec to wdev\n",
			p2pFuncGetDriverCacTime());
	}

	DBGLOG(INIT, INFO, "CAC End event Update to OS\n");
#if KERNEL_VERSION(6, 12, 0) <= CFG80211_VERSION_CODE
	cfg80211_cac_event(prNetdevice,
...
#else
	cfg80211_cac_event(prNetdevice, NL80211_RADAR_CAC_FINISHED, GFP_KERNEL);
#endif
#endif
	DBGLOG(INIT, INFO, "CAC End event Update to OS Done\n");
}				/* kalP2PRddDetectUpdate */
		...
		IN uint8_t ucRoleIndex)
{
	...
	cfg80211_ch_switch_notify(prNetdevice, prGlueP2pInfo->chandef
	...
	DBGLOG(INIT, INFO, "CAC Start event Update to OS\n");
#if KERNEL_VERSION(6, 12, 0) <= CFG80211_VERSION_CODE
	cfg80211_cac_event(prNetdevice,
			prGlueP2pInfo->chandef,
...
#else
	cfg80211_cac_event(prNetdevice, NL80211_RADAR_CAC_STARTED, GFP_KERNEL);
#endif
#endif
	DBGLOG(INIT, INFO, "CAC Start event Update to OS Done\n");
...
void kalP2pNotifyApGoStopped(struct ADAPTER *prAdapter)
{
	uint8_t ucIdx;
	...
	for (ucIdx = 0; ucIdx < MAX_BSS_INDEX; ucIdx++) {
		...
		prNetdev = wlanGetNetDev(prAdapter->prGlueInfo, ucIdx);
		if (!prNetdev)
			continue;

		DBGLOG(P2P, STATE,
			"Notify AP/GO stop, BssIdx %u\n", ucIdx);
		cfg80211_stop_iface(prNetdev->ieee80211_ptr->wiphy,
				prNetdev->ieee80211_ptr, GFP_KERNEL);
	}
}

void kalP2pNotifyApGoStoppedByIdx(IN struct ADAPTER *prAdapter,
	IN uint8_t ucBssIndex)
{
	struct net_device *prNetdev;
	...
	DBGLOG(P2P, STATE,
		"Notify [%s] stop, BssIdx [%u]\n", prNetdev->name, ucBssIndex);

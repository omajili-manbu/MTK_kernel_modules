...
#if (CFG_SUPPORT_6G_PWR_REGS == 1)
static int
priv_driver_set_6g_pwr_mode(struct net_device *prNetDev, char *pcCommand,
			int i4TotalLen);
#endif /* CFG_SUPPORT_6G_PWR_REGS */
...
#if (CFG_SUPPORT_RTT == 1)
static int priv_driver_set_rtt(struct net_device *prNetDev,
	char *pcCommand, int i4TotalLen);
static int priv_driver_control_rtt(struct net_device *prNetDev,
	char *pcCommand, int i4TotalLen);
#endif /* CFG_SUPPORT_RTT */
...
int compat_priv(IN struct net_device *prNetDev,
	     IN struct iw_request_info *prIwReqInfo,
	...
	case PRIV_CMD_GET_WIFI_TYPE:
		{
			...
			i4ResultLen = priv_driver_cmds(prGlueInfo, prNetDev,
						       aucOidBuf, u4CmdLen);
			if (i4ResultLen > 1) {
	...
	case PRIV_CMD_SET_PWR_CTRL:
		{
			...
			priv_driver_cmds(prGlueInfo, prNetDev,
							pCommand, u4CmdLen);
...
__priv_nan_struct(IN struct net_device *prNetDev,
	...
	case ENUM_NAN_PUBLISH: {
		...
		DBGLOG(NAN, INFO, "NDP Type: %d\n",
		       publishReq->sdea_params.fgDataType);
		...
		if (pid != 0) {
			nanDiscInstanceAdd(prGlueInfo->prAdapter, pid,
				NAN_PUBLISH,
				publishReq->aucServiceHash,
				publishReq->service_name,
				publishReq->service_name_len,
				FALSE);
		}

		if (publishReq->sdea_params.security_cfg && pid != 0) {
	...
	case ENUM_NAN_DATA_REQ: {
		...
		struct NanDataReqReceive rDataRcv = {0};
		...
		if (!prNanCmdDataRequest) {
			DBGLOG(NAN, ERROR,
				"Alloc mem failed for Data path req\n");
			return -ENOMEM;
		}
		kalMemZero(prNanCmdDataRequest, sizeof(*prNanCmdDataRequest));

		/* Retrieve to NDP */
		prNanCmdDataRequest->ucType = prDataReq->type;
		...
		if (prNanCmdDataRequest->ucSecurity) {
			...
			kalMemCopy(prNanCmdDataRequest->aucScid,
				   prDataReq->scid,
				   NAN_SCID_DEFAULT_LEN);
		...
		prNanCmdDataRequest->ucRequireQOS = prDataReq->ndp_cfg.qos_cfg;
		prNanCmdDataRequest->ucMinTimeSlot = prDataReq->ucMinTimeSlot;
		prNanCmdDataRequest->u2MaxLatency = prDataReq->u2MaxLatency;

		kalMemCopy(prNanCmdDataRequest->aucResponderDataAddress,
			   prDataReq->peer_disc_mac_addr, MAC_ADDR_LEN);

		prNanCmdDataRequest->fgCarryIpv6 = prDataReq->fgCarryIpv6;
		if (prNanCmdDataRequest->fgCarryIpv6 == TRUE)
			kalMemCopy(prNanCmdDataRequest->aucIPv6Addr,
		...
		rStatus = nanCmdDataRequest(
			prGlueInfo->prAdapter, prNanCmdDataRequest,
...
exit:
		kalMemFree(prNanCmdDataRequest,
			VIR_MEM_TYPE, sizeof(*prNanCmdDataRequest));
		break;
	}
	case ENUM_NAN_DATA_RESP: {
		...
		if (!prNanCmdDataResponse) {
			DBGLOG(NAN, ERROR,
				"Alloc mem failed for Data response\n");
			return -ENOMEM;
		}

		kalMemZero(prNanCmdDataResponse, sizeof(*prNanCmdDataResponse));

		prNanCmdDataResponse->ucType = prDataRes->type;
		prNanCmdDataResponse->ucDecisionStatus = NAN_DP_REQUEST_ACCEPT;
		prNanCmdDataResponse->ucNDPId = prDataRes->ndp_instance_id;
		prNanCmdDataResponse->ucRequireQOS = prDataRes->ndp_cfg.qos_cfg;
		prNanCmdDataResponse->ucSecurity = prDataRes->cipher_type;
		prNanCmdDataResponse->u2SpecificInfoLength =
			prDataRes->app_info.ndp_app_info_len;
		prNanCmdDataResponse->fgCarryIpv6 = prDataRes->fgCarryIpv6;
		prNanCmdDataResponse->u2PortNum = prDataRes->u2PortNum;
		prNanCmdDataResponse->ucServiceProtocolType =
			prDataRes->ucServiceProtocolType;
		prNanCmdDataResponse->ucMinTimeSlot = prDataRes->ucMinTimeSlot;
		prNanCmdDataResponse->u2MaxLatency = prDataRes->u2MaxLatency;
		...
		kalMemCopy(prNanCmdDataResponse->aucSpecificInfo,
			   ...
			   prDataRes->app_info.ndp_app_info_len);
		if (prNanCmdDataResponse->fgCarryIpv6 == TRUE)
	...
	case ENUM_NAN_DATA_END: {
		...
		struct _NAN_CMD_DATA_END rNanCmdDataEnd = {0};
		uint32_t rStatus = WLAN_STATUS_SUCCESS;
	...
	case ENUM_NAN_DATA_UPDTAE: {
		...
		struct NanDataPathInitiatorRequest *prDataUpd =
			(struct NanDataPathInitiatorRequest *)&aucOidBuf[0];
		struct _NAN_PARAMETER_NDL_SCH rNanUpdateSchParam = {0};
		uint32_t rStatus = WLAN_STATUS_SUCCESS;
		...
		rStatus = nanCmdDataUpdate(prGlueInfo->prAdapter,
					   &rNanUpdateSchParam);
	...
	case ENUM_NAN_ENABLE_REQ: {
		...
		enum NanStatusType nanRetStatus = WLAN_STATUS_SUCCESS;
...
{
	uint32_t i, j, k;
	uint32_t max = NUM_SUPPORTED_OIDS - 1;
	...
	while (i <= j) {
		...
		if (rOid == arWlanOidReqTable[k].rOid) {
			*ppWlanReqEntry = &arWlanOidReqTable[k];
			return TRUE;
		} else if (rOid < arWlanOidReqTable[k].rOid && k > 0) {
			j = k - 1;
		} else if (rOid > arWlanOidReqTable[k].rOid && k < max) {
			i = k + 1;
		} else {
			break;
		}
		...
		IN union iwreq_data *prIwReqData, IN OUT char *pcExtra)
{
	...
	if (pcExtra) {
		...
		DBGLOG(REQ, INFO, "%s prIwReqData->data.length = %d\n",
		       __func__, prIwReqData->data.length);
		i4BytesWritten = priv_driver_cmds(prGlueInfo, prNetDev, pcExtra,
					  IW_PRIV_BUF_SIZE);
		DBGLOG(REQ, INFO, "%s i4BytesWritten = %d\n", __func__,
		       i4BytesWritten);
	}
...
#define CMD_FAW_DEF_CONFIG "FAW_DEF_CONFIG"
#define CMD_GET_NAN_STAT "GET_NAN"
#if (CFG_NAN_P2P_CONCURRENT_SUPPORT == 1)
#define CMD_SET_NAN_P2P "SET_NAN_P2P"
#endif
#if CFG_NAN_SUPPORT_FAST_DISC
...
#if (CFG_SUPPORT_6G_PWR_REGS == 1)
#define CMD_TX_PWR_ENV_CTRL		"TPE_CTRL"
#endif  /* CFG_SUPPORT_6G_PWR_REGS == 1 */
#if (CFG_ADM_CTRL == 1)
...
#if (CFG_SUPPORT_SCAN_RADIOINFO == 1)
#define CMD_GET_CH_RADIOINFO	"GET_CH_RADIOINFO"
#endif /* end CFG_SUPPORT_SCAN_RADIOINFO */

#define CMD_EFUSE		"EFUSE"
#define CMD_SECEFUSE		"SECEFUSE"
...
#if CFG_WIFI_FMCW_RADAR_TRAFFIC_REPORT
#define CMD_FMCW_RADAR_TRAFFIC_REPORT  "RADAR_TRAFFIC_REPORT"
#endif
...
#if (CFG_SUPPORT_ANT_SMART_DIVERSITY == 1)
#define CMD_ANTENNA_SWITCH_ENABLE  "ANT_SWITCH_ENABLE"
#define CMD_ANTENNA_SWITCH_CONTROL  "ANT_SWITCH_CONTROL"
#endif
...
#define CMD_CONTROL_RTT         "CONTROL_RTT"
#endif
...
#if (CFG_WIFI_ULTRA_RADIO_OFF_CTRL == 1)
...
#define CMD_SET_SCAN_MIN_MAX_DWELL_TIME    "SET_SCAN_DWELL_TIME"

#if (CFG_SUPPORT_6G_PWR_REGS == 1)
#define CMD_SET_6G_POWER_MODE		"SET_6G_PWR_MODE"
#endif /*CFG_SUPPORT_6G_PWR_REGS */

#define CMD_SET_RADAR_PARAMS		"SET_RADAR_PARAMS"

#if CFG_SUPPORT_SAP_TX_FAIRNESS
#define CMD_SET_SAP_TX_FAIRNESS		"SET_SAP_TX_FAIRNESS"
#define CMD_RESET_AIRTIME_CNT		"RESET_AIRTIME_COUNTER"
#endif
					...
					IN char *pcCommand, IN int i4TotalLen)
{
	...
	i4Recv = sscanf(this_char, "%10u:%10u", &(u4WCID), &(u4DebugType));
	if (i4Recv < 0)
		return -1;
					...
					IN char *pcCommand, IN int i4TotalLen)
{
	...
	u4ParamNum = sscanf(this_char, "%3u:%3u", &ucParam, &ucBandIdx);
	if (u4ParamNum != 2)
		return -1;
					...
					IN char *pcCommand, IN int i4TotalLen)
{
	...
	u4ParamNum = sscanf(this_char, "%3u:%3u:%3u:%4d",
				    ...
				    IN char *pcCommand, IN int i4TotalLen)
{
	...
	if (i4Argc >= 4 && apcArgv[2] != NULL) {
	...
	} else if (i4Argc >= 3 && apcArgv[1] != NULL) {
		...
		} else if (i4Argc == 2 && apcArgv[1] != NULL) {
			if (strnicmp(apcArgv[1], CMD_STAT_RESET_CNT,
			    strlen(CMD_STAT_RESET_CNT)) == 0) {
				fgResetCnt = TRUE;
				fgSearchMacAddr = TRUE;
				     ...
				     IN char *pcCommand, IN int i4TotalLen)
{
	...
	prQueryDrvStatistics->u4RxFreeSwRfbMsduNumElem =
					getTotalFreeRfbCount(prAdapter);
				     ...
				     IN char *pcCommand, IN int i4TotalLen)
{
	...
	i4Ret = sscanf(apcArgv[1], "%2hhx:%2hhx:%2hhx:%2hhx:%2hhx:%2hhx",
				  ...
				  IN char *pcCommand, IN int i4TotalLen)
{
	...
	i4Recv = sscanf(this_char,
		"%3u-%3u-%3u-%3u-%5u-%5u-%5u-%5u-%5u-%5u-%5u",
		&(rSniffer.ucModule),
		&(rSniffer.ucAction),
		&(rSniffer.ucFilter),
			 ...
			 char *pcCommand, int i4TotalLen)
{
	...
	kalIoctl(prGlueInfo, wlanoidSetFwParam, (void *) (pcCommand + 13),
		 i4TotalLen - 13, FALSE, FALSE, TRUE, &i4BytesWritten);
...
int
priv_driver_get_nan_stat(IN struct net_device *prNetDev, IN char *pcCommand,
			  IN int i4TotalLen)
{
#define MAP_SHOW_FORMAT	"%02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x"
	struct ADAPTER *prAdapter = NULL;
	struct GLUE_INFO *prGlueInfo = NULL;
	int32_t i4Argc = 0, i4BytesWritten = 0;
	int8_t *apcArgv[WLAN_CFG_ARGV_MAX] = { 0 };
	uint8_t ucTimeLineIdx = 0, i = 0, j = 0;
	struct _NAN_TIMELINE_MGMT_T *prNanTimelineMgmt = NULL;
	struct _NAN_CHANNEL_TIMELINE_T *prNanChnlList = NULL;
	struct _NAN_SCHEDULER_T *prNanScheduler = NULL;
	struct _NAN_EVENT_DEVICE_INFO *prEventDeviceInfo = NULL;
	size_t szSize = sizeof(struct _NAN_EVENT_DEVICE_INFO);
	uint32_t u4BufLen = 0;
	...
	uint8_t aucBuf[16] = {0};
	uint8_t aucRole[5][20] = {"ROLE_NONE",
						"ANCHOR_MASTER",
						"ROLE_MASTER",
						"ROLE_NON_MASTER",
						"ROLE_NUM"};
	...
	if (!prNanScheduler) {
		DBGLOG(NAN, ERROR, "prNanScheduler error\n");
		return -1;
	}

	prNANSpecInfo = nanGetSpecificBssInfo(prAdapter, NAN_BSS_INDEX_MAIN);
	if (prNANSpecInfo == NULL) {
		DBGLOG(NAN, ERROR, "prNANSpecInfo error\n");
		return -1;
	}
	prBssInfo =
	...
	prEventDeviceInfo = (struct _NAN_EVENT_DEVICE_INFO *)
		   kalMemAlloc(szSize, VIR_MEM_TYPE);

	if (!prEventDeviceInfo) {
		DBGLOG(NAN, ERROR, "Memory allocation fail\n");
		return -1;
	}
	kalMemZero(prEventDeviceInfo, szSize);
	kalIoctl(prGlueInfo,
	...
	LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
		...
		prEventDeviceInfo->ucIsEnabled ? "Enabled" : "Disabled",
		prEventDeviceInfo->ucHopCount);

	LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
		"Role = %s, State : %s\n",
		aucRole[prEventDeviceInfo->ucNanDeviceRole],
		aucState[prEventDeviceInfo->ucNanDeviceState]);

	LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
		"AMBTT = %u, TSF = 0x%08x%08x\n",
	...
	LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
		...
		prEventDeviceInfo->ucMasPreference,
		prEventDeviceInfo->ucRandomFactor);

	LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
		"\nAnchor Master["MACSTR"]\n",
		MAC2STR(prEventDeviceInfo->aucAnchorMasMacAddr));
	LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
		"AM Master Preference      = %3u, AM Random Factor    = %3u\n",
		prEventDeviceInfo->ucAmMasPreference,
	...
	LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
		...
		prEventDeviceInfo->ucParentMasPreference,
	...
	LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
		"IGTK Packet Number = %02x %02x %02x %02x %02x %02x\n",
		prEventDeviceInfo->aucIgtkPn[0],
		prEventDeviceInfo->aucIgtkPn[1],
		prEventDeviceInfo->aucIgtkPn[2],
		prEventDeviceInfo->aucIgtkPn[3],
		prEventDeviceInfo->aucIgtkPn[4],
		prEventDeviceInfo->aucIgtkPn[5]);

	LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
		"BIGTK Packet Number = %02x %02x %02x %02x %02x %02x\n",
		prEventDeviceInfo->aucBigtkPn[0],
		prEventDeviceInfo->aucBigtkPn[1],
		prEventDeviceInfo->aucBigtkPn[2],
		prEventDeviceInfo->aucBigtkPn[3],
		prEventDeviceInfo->aucBigtkPn[4],
		prEventDeviceInfo->aucBigtkPn[5]);
#endif
	...
	for (ucTimeLineIdx = 0; ucTimeLineIdx < NAN_TIMELINE_MGMT_SIZE;
		...
		LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
			...
			ucTimeLineIdx);
		LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
			"[Committed Availiability]\n");

		for (i = 0; i < NAN_TIMELINE_MGMT_CHNL_LIST_NUM; i++) {
			prNanChnlList = &prNanTimelineMgmt->arChnlList[i];

			if (prNanChnlList->fgValid == FALSE)
				continue;

			LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
			   "[Chnl Entry #%u Chnl: %u Op_class: %u]\n",
			   i, prNanChnlList->rChnlInfo.rChannel.u4PrimaryChnl,
			   prNanChnlList->rChnlInfo.rChannel.u4OperatingClass);

			u4Length = sizeof(prNanChnlList->au4AvailMap);
			pucContent = (uint8_t *)prNanChnlList->au4AvailMap;
			if (u4Length >= 16) {
				LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
				MAP_SHOW_FORMAT"\n",
	...
	LINK_FOR_EACH_ENTRY(prPeerSchDesc, prPeerSchDescList, rLinkEntry,
		...
		if (prPeerSchDesc == NULL) {
			LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
				"No peer schedule desc\n");
			goto exit;
		}
		LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
			"Dump ["MACSTR"] Availability\n",
			MAC2STR(prPeerSchDesc->aucNmiAddr));

		for (u4Idx = 0; u4Idx < NAN_NUM_AVAIL_DB; u4Idx++) {
			prNanAvailAttr =
				&prPeerSchDesc->arAvailAttr[u4Idx];
			if (prNanAvailAttr->ucMapId ==
				NAN_INVALID_MAP_ID)
				continue;
			...
			for (u4Idx1 = 0;
				...
				u4Idx1++) {
				prNanAvailEntry =
					&prNanAvailAttr->arAvailEntryList
						[u4Idx1];
				if (prNanAvailEntry->fgActive == FALSE)
					continue;

				if (prNanAvailEntry->arBandChnlCtrl[0].
					rInfo.u4Type ==
					NAN_BAND_CH_ENTRY_LIST_TYPE_BAND)
					continue;

				LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
					"[%u][%u] MapID: %d, Ctrl:0x %x, ChnlRaw:0x %x, Class: %u\n",
				...
				if (u4Length >= 16) {
					...
					kalMemZero(aucBuf, 16);
					kalMemCpyS(aucBuf, sizeof(aucBuf),
						pucContent, u4Length);
...
exit:
	if (prEventDeviceInfo)
		kalMemFree(prEventDeviceInfo, VIR_MEM_TYPE, szSize);
...
#if (CFG_NAN_P2P_CONCURRENT_SUPPORT == 1)
int
priv_driver_set_nan_p2p(IN struct net_device *prNetDev, IN char *pcCommand,
			  IN int i4TotalLen)
{
	struct ADAPTER *prAdapter = NULL;
	struct GLUE_INFO *prGlueInfo = NULL;
	int32_t i4Argc = 0;
	uint8_t ucstate = 0, ucChannel = 0, ucBw = 0;
	int8_t *apcArgv[WLAN_CFG_ARGV_MAX] = {0};
	int32_t u4Ret = 0;
	int32_t i4BytesWritten = 0;
	uint8_t aucState[3][30] = {"WIFI_DIRECT_DISABLE",
						"WIFI_DIRECT_LISTENING",
						"WIFI_DIRECT_OPERATING"};
	uint8_t aucBw[3][20] = {"20_40MHZ",
						"80MHZ",
						"160MHZ"};
	struct MSG_P2P_NAN_DP_UPDATE rP2pNanDpUpdateMsg = {0};
	struct RF_CHANNEL_INFO rVendorChInfo = {0};
	...
	if (i4Argc >= 4) {
		...
		if (u4Ret) {
			DBGLOG(REQ, ERROR, "parse u4state error u4Ret=%d\n",
			       u4Ret);
			goto error_exit;
		}
		u4Ret = kalkStrtou8(apcArgv[2], 0, &ucChannel);
		if (u4Ret) {
			DBGLOG(REQ, ERROR, "parse u4Channel error u4Ret=%d\n",
			       u4Ret);
			goto error_exit;
		}
		u4Ret = kalkStrtou8(apcArgv[3], 0, &ucBw);
		if (u4Ret) {
			DBGLOG(REQ, ERROR, "parse ucBw error u4Ret=%d\n",
			       u4Ret);
			goto error_exit;
		}
		...
		LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
			"\n[%s], Channel[%u], Bw[%s]\n",
			aucState[ucstate], ucChannel, aucBw[ucBw]);

		if (ucstate == 2) {
			rP2pNanDpUpdateMsg.rChannelInfo.ucChannelNum =
				ucChannel;
			nanSchedP2pNotifyNanConnectMode(prAdapter,
				(struct MSG_HDR *)&rP2pNanDpUpdateMsg);
		} else if (ucstate == 1) {
			nanSchedP2pNotifyNanListenMode(prAdapter,
				(int32_t)ucChannel);
		} else {
			nanSchedP2pNotifyNanAbortMode(prAdapter);
		}

		g_fgManualWfdState = TRUE;
		g_rCurrNanP2pState.eP2pState = ucstate;
		rVendorChInfo.ucChannelNum = ucChannel;
		rVendorChInfo.eBand =
			SCN_GET_EBAND_BY_CH_NUM(ucChannel);
		rVendorChInfo.ucChnlBw = (uint8_t)ucBw;
		g_rCurrNanP2pState.channel =
			nanCastChToVendorFormat(rVendorChInfo,
			CHNL_EXT_SCN);
...
error_exit:
	...
	LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
		"\nset_nan_p2p [state] [channel] [bandwidth]\n");
	LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
		"\n[state] wifi direct state. 0: disable, 1: listen, 2: connect\n");
	LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
		"\n[channel] wifi_direct channel\n");
	LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
		"\n[bandwidth] channel bw. 0: bw20/40, 1: bw80, 2: bw160\n");
...
#if CFG_NAN_SUPPORT_FAST_DISC
static int priv_driver_set_fast_discovery(IN struct net_device *prNetDev,
				    IN char *pcCommand, IN int i4TotalLen)
{
	struct GLUE_INFO *prGlueInfo = NULL;
	struct ADAPTER *prAdapter = NULL;
	...
	if (i4Argc >= 2) {
		...
		DBGLOG(NAN, INFO,
			"priv_driver_set_fast_discovery:: set_fast_discovery %d\n",
			u4Slot);
	} else {
...
int
priv_driver_set_faw_def_config(
	IN struct net_device *prNetDev, IN char *pcCommand,
	IN int i4TotalLen)
{
	struct GLUE_INFO *prGlueInfo = NULL;
	...
	if (i4Argc >= i4ArgNum) {
		...
		if (u4Ret) {
			DBGLOG(REQ, LOUD,
			       "parse FAW DEF CONFIG band error u4Ret=%d\n",
			       u4Ret);
			return -1;
		}
		...
		if (u4Ret) {
			DBGLOG(REQ, LOUD,
			       "parse FAW DEF CONFIG channel error u4Ret=%d\n",
			       u4Ret);
			return -1;
		}
		...
		if (u4Ret) {
			DBGLOG(REQ, LOUD,
			       "parse FAW DEF CONFIG slotBitmap error u4Ret=%d\n",
			       u4Ret);
			return -1;
		}

		u4Ret = kalkStrtou8(apcArgv[4], 0, &(ucApply));
		if (u4Ret) {
			DBGLOG(REQ, LOUD,
			       "parse FAW DEF CONFIG Applied error u4Ret=%d\n",
			       u4Ret);
			return -1;
		}

		nanSchedFawDefConfigCmd(prGlueInfo->prAdapter, ucChnl, eBand,
					     u4SlotBitmap, ucApply);
	}
...
#define MIN_ARGC_OF_SET_COUNTRY_CMD 2
	...
	if (regd_is_single_sku_en(prGlueInfo->prAdapter)) {
			    ...
			    IN char *pcCommand, IN int i4TotalLen)
{
	...
	country = rlmDomainGetCountryCode(prGlueInfo->prAdapter);
			     ...
			     IN char *pcCommand, IN int i4TotalLen)
{
	...
	if (i4Argc >= 2 && (apcArgv[1][0] == '2') && (apcArgv[1][1] == 'g')) {
		start_idx = 0;
		end_idx = RLM_GET_ACTIVE_CHANNEL_COUNT(KAL_BAND_2GHZ);
	} else if (i4Argc >= 2 && (apcArgv[1][0] == '5') &&
	    (apcArgv[1][1] == 'g')) {
		start_idx = RLM_GET_ACTIVE_CHANNEL_COUNT(KAL_BAND_2GHZ);
		end_idx = RLM_GET_ACTIVE_CHANNEL_COUNT(KAL_BAND_2GHZ)
				+ RLM_GET_ACTIVE_CHANNEL_COUNT(KAL_BAND_5GHZ);
		...
		end_idx = RLM_GET_ACTIVE_CHANNEL_COUNT(KAL_BAND_2GHZ)
				+ RLM_GET_ACTIVE_CHANNEL_COUNT(KAL_BAND_5GHZ)
				+ RLM_GET_ACTIVE_CHANNEL_COUNT(KAL_BAND_6GHZ);
#endif
	} else {
		start_idx = 0;
		end_idx = RLM_GET_ACTIVE_CHANNEL_COUNT(KAL_BAND_2GHZ)
	...
	if (regd_is_single_sku_en(prAdapter)) {
		...
		for (ch_idx = start_idx; ch_idx < end_idx; ch_idx++) {
			...
			pCh = (RLM_GET_ACTIVE_CHANNELS() + ch_idx);
			maxbw = 160;
		  ...
		  IN OUT struct ifreq *prReq, IN int i4Cmd)
{
	...
	if ((prMdnsUplayerInfo->ucCmd < MDNS_CMD_ENABLE)
		...
		DBGLOG(REQ, ERROR, "%s: mdns cmd error!\n", __func__);
		ret = -EFAULT;
		goto exit;
	}

	if (prMdnsUplayerInfo->ucCmd == MDNS_CMD_ADD_RECORD) {
		if (!prMdnsParam
		|| (prMdnsParam->query[0].name_length == 0x0)
		|| (prMdnsParam->response_len > MDNS_RESPONSE_RECORD_MAX_LEN)
		|| (prMdnsParam->response_len == 0)) {
			DBGLOG(REQ, ERROR, "mdns param check failed!\n");
			ret = -EFAULT;
			goto exit;
		}
	} else {
		if (!prMdnsParam) {
			DBGLOG(REQ, ERROR, "mdns param is NULL!\n");
			ret = -EFAULT;
			goto exit;
		}
	}
		...
		IN char *pcCommand, IN int i4TotalLen)
{
	...
	for (i = 0; i < 5; i++) {
		...
		kalMemCopy(prMdnsUplayerInfo->name, test_passthrough[i], len);
		kalMdnsProcess(prGlueInfo, prMdnsUplayerInfo);
	}
...
static int priv_driver_resetall_passthrough(IN struct net_device *prNetDev,
		IN char *pcCommand, IN int i4TotalLen)
{
	struct GLUE_INFO *prGlueInfo = NULL;
	struct MDNS_INFO_UPLAYER_T *prMdnsUplayerInfo = NULL;
				 ...
				 IN char *pcCommand, IN int i4TotalLen)
{
	...
	if (i4Argc >= 2) {
		...
		updateWifiOnTimeStatistics(prNetDev);
#endif
				   ...
				   IN char *pcCommand, IN int i4TotalLen)
{
	...
	u4Offset += kalSnprintf(pcCommand + u4Offset, i4TotalLen - u4Offset,
		"t-neptune-main-mt7902-2046-MT7902_E1_ASIC_L_X86_GEN4M-20250926170854-GOOD.xml\nWiFi Driver Version %u.%u.%u %s\n",
					...
					IN char *pcCommand, IN int i4TotalLen)
{
	...
	u4ArrySize = sizeof(struct RF_CHANNEL_INFO) * MAX_PER_BAND_CHN_NUM;
	aucChannelList = kalMemAlloc(u4ArrySize, VIR_MEM_TYPE);
	if (aucChannelList == NULL) {
		DBGLOG(SCN, WARN, "can't alloc aucChannelList\n");
		return -1;
	}
	kalMemZero(aucChannelList, u4ArrySize);
	...
	kalMemFree(aucChannelList, VIR_MEM_TYPE, u4ArrySize);
	return i4BytesWritten;
}
	...
	for (ucBandIdx = BAND_2G4; ucBandIdx < BAND_NUM; ucBandIdx++) {
		...
		for (i = 0; i < ucNumOfChannel; i++) {
			...
			u4Offset = kalSnprintf(
				pcCommand + i4BytesWritten, i4TotalLen,
...
#if (CFG_SUPPORT_SCAN_RADIOINFO == 1)
static int priv_driver_get_ch_radioinfo(IN struct net_device *prNetDev,
					IN char *pcCommand, IN int i4TotalLen)
{
	struct GLUE_INFO *prGlueInfo = NULL;
	uint32_t i4BytesWritten = 0, u4Offset = 0;
	struct PARAM_GET_CHN_INFO *prChnLoadInfo = NULL;
	struct RF_CHANNEL_INFO *paucChannelList[BAND_NUM];
	uint8_t i = 0, j = 0, uScnt = 0, ucBandIdx = 0, ucChnIdx = 0;
	uint8_t ucNumOfChannel[BAND_NUM] = {0};
	int8_t ucChCount = -1;
	struct SCAN_INFO *prScanInfo = NULL;
	struct SCAN_RADIO_INFO *prScanRadioInfo = NULL;
	int16_t au2ChannelIdleTime = -1, u2Idle = -1, u2Busy = -1;
	int16_t u2ChannelScanTime = -1;
	int8_t aucChannelMDRDYCnt = -1;
	...
	for (i = BAND_2G4; i < BAND_NUM; i++) {
		paucChannelList[i] = (struct RF_CHANNEL_INFO *)
			kalMemAlloc(sizeof(struct RF_CHANNEL_INFO)
				* MAX_PER_BAND_CHN_NUM, VIR_MEM_TYPE);
		if (!paucChannelList[i]) {
			DBGLOG(REQ, ERROR,
			"Can not alloc memory for rf channel info\n");
			return -ENOMEM;
		}

		kalMemZero(paucChannelList[i],
	...
	for (ucBandIdx = BAND_2G4; ucBandIdx < BAND_NUM; ucBandIdx++)
		rlmDomainGetChnlList(prGlueInfo->prAdapter, ucBandIdx,
			TRUE, MAX_PER_BAND_CHN_NUM,
			&ucNumOfChannel[ucBandIdx],
			paucChannelList[ucBandIdx]);

	for (ucBandIdx = BAND_2G4; ucBandIdx < BAND_NUM; ucBandIdx++) {
		for (i = 0; i < ucNumOfChannel[ucBandIdx]; i++) {
			for (ucChnIdx = 0; ucChnIdx < MAX_CHN_NUM; ucChnIdx++) {

				if (paucChannelList[ucBandIdx][i].eBand
					== prChnLoadInfo->
					rChnRankList[ucChnIdx].eBand
					&& paucChannelList[ucBandIdx][i].
					ucChannelNum == prChnLoadInfo->
					rChnRankList[ucChnIdx].ucChannel) {
					ucChCount = ucChnIdx;
					break;
				}
			...
			for (uScnt = 0; uScnt < SCAN_COUNT; uScnt++) {
				prScanRadioInfo = prScanInfo->
					rScanRadioInfo[uScnt];
				for (j = 0; j < prScanInfo->
					ucSparseChannelArrayValidNum[uScnt];
					j++) {
					if (j >=
			...
			if (au2ChannelIdleTime >= 0
			&& u2ChannelScanTime >= 0) {
				u2Idle =
				(au2ChannelIdleTime*9)/u2ChannelScanTime;
				if (u2Idle >= 1000)
					u2Idle = 1000;
			...
			if (ucChCount >= 0 && i4BytesWritten < 2000) {
				u4Offset = kalSnprintf(
					pcCommand + i4BytesWritten,
					IW_PRIV_BUF_SIZE - i4BytesWritten,
					"\nB:%u C:%u D:%u I:%d S:%d",
					prChnLoadInfo->
					rChnRankList[ucChCount].eBand,
					prChnLoadInfo->
					rChnRankList[ucChCount].ucChannel,
					prChnLoadInfo->
					rChnRankList[ucChCount]
						.u4Dirtiness,
					u2Idle,
					u2Busy);
				DBGLOG(REQ, INFO,
					"2 B:%u C:%u D:%u I:%d S:%d ID:%d ST:%d\n",
					prChnLoadInfo->
					rChnRankList[ucChCount].eBand,
					prChnLoadInfo->
					rChnRankList[ucChCount].ucChannel,
					prChnLoadInfo->rChnRankList[ucChCount]
						.u4Dirtiness,
					u2Idle,
					u2Busy,
					au2ChannelIdleTime,
			...
			au2ChannelIdleTime = -1;
			u2ChannelScanTime = -1;
			aucChannelMDRDYCnt = -1;
			u2Idle = -1;
			u2Busy = -1;
		}
	}
	for (i = BAND_2G4; i < BAND_NUM; i++) {
		kalMemFree(paucChannelList[i], VIR_MEM_TYPE,
			sizeof(struct RF_CHANNEL_INFO)
			* MAX_PER_BAND_CHN_NUM);
	}
	return i4BytesWritten;
...
#if CFG_SECEFUSE_ACCESS
static int priv_driver_sec_efuse_ops(IN struct net_device *prNetDev,
		IN char *pcCommand, IN int i4TotalLen)
{
	enum EFUSE_OP_MODE {
		EFUSE_READ,
	...
	uint32_t u4Efuse_addr = 0;
	uint32_t u4Efuse_len = 0;
	char *ucStr = NULL;
	char ucStr2Hex[5] = {'0', 'x', '0', '0', '\0'};
	int32_t i = 0;
	...
	uint8_t  u4Index = 0;
	struct GLUE_INFO *prGlueInfo = NULL;
	struct PARAM_CUSTOM_ACCESS_SECEFUSE rAccessEfuseInfo = { 0 };

	wlanCfgParseArgument(pcCommand, &i4Argc, apcArgv);

	/* # of input arguments check */
	if (i4Argc < 2)
		goto efuse_op_invalid;
	...
	kalMemSet(&rAccessEfuseInfo, 0,
		sizeof(struct PARAM_CUSTOM_ACCESS_SECEFUSE));

	/* Fill cmd's u4Address and u4Size */
	if (ucOpMode == EFUSE_READ || ucOpMode == EFUSE_WRITE) {
		if (apcArgv[2][0] != '0' || apcArgv[2][1] != 'x')
			goto efuse_op_invalid;

		u4Ret = kalkStrtos32(apcArgv[2], 16, &i4Parameter);
		u4Efuse_addr = (uint32_t)i4Parameter;

		if (u4Efuse_addr < 0x20) {
			rAccessEfuseInfo.u4Address =
				(u4Efuse_addr / EFUSE_BLOCK_SIZE) *
				EFUSE_BLOCK_SIZE;
			u4Index = u4Efuse_addr % EFUSE_BLOCK_SIZE;
			rAccessEfuseInfo.u4Size = 16;
		} else {
			rAccessEfuseInfo.u4Address =
				(u4Efuse_addr / SECEFUSE_BLOCK_SIZE) *
				SECEFUSE_BLOCK_SIZE;
			u4Index = u4Efuse_addr % SECEFUSE_BLOCK_SIZE;
			rAccessEfuseInfo.u4Size = 32;
		}
	}

	/* Fill cmd's aucData */
	if (ucOpMode == EFUSE_WRITE) {
		if (apcArgv[3][0] != '0' || apcArgv[3][1] != 'x')
			goto efuse_op_invalid;

		ucStr = &apcArgv[3][2]; /* Ignore "0x" */
		u4Efuse_len = strlen(ucStr)/2;

		if (u4Efuse_len > SECEFUSE_BLOCK_SIZE) {
			DBGLOG(REQ, ERROR, "data len exceeds 0x%x\n",
				SECEFUSE_BLOCK_SIZE);
			goto efuse_op_invalid;
		} else if (u4Efuse_len == 1) { /* Byte access */
			ucStr2Hex[2] = ucStr[0];
			ucStr2Hex[3] = ucStr[1];
			u4Ret = kalkStrtos32(ucStr2Hex, 16, &i4Parameter);
			rAccessEfuseInfo.aucData[u4Index] = i4Parameter;
			rAccessEfuseInfo.u4Size = u4Index + 1;
		} else { /* Block access */
			if (u4Index != 0) {
				DBGLOG(REQ, ERROR, "data not align to 0x%x\n",
					SECEFUSE_BLOCK_SIZE);
				goto efuse_op_invalid;
			}

			for (i = 0; i < u4Efuse_len; i++) {
				ucStr2Hex[2] = ucStr[i*2];
				ucStr2Hex[3] = ucStr[i*2+1];
				u4Ret = kalkStrtos32(ucStr2Hex, 16,
					&i4Parameter);
				rAccessEfuseInfo.aucData[i] = i4Parameter;
			}

			rAccessEfuseInfo.u4Size = u4Efuse_len;
		}
	}

	DBGLOG(REQ, INFO, "rAccessEfuseInfo.u4Address:0x%x\n",
		rAccessEfuseInfo.u4Address);
	DBGLOG(REQ, INFO, "rAccessEfuseInfo.u4Type:0x%x\n",
		rAccessEfuseInfo.u4Type);
	DBGLOG(REQ, INFO, "rAccessEfuseInfo.u4Valid:0x%x\n",
		rAccessEfuseInfo.u4ReservedValid);
	DBGLOG(REQ, INFO, "rAccessEfuseInfo.u4Size:0x%x\n",
		rAccessEfuseInfo.u4Size);
	for (i = 0; i < 32; i++)
		DBGLOG(REQ, INFO, "rAccessEfuseInfo.aucData[0x%x]:0x%x\n",
			i, rAccessEfuseInfo.aucData[i]);
	...
	if (prGlueInfo->prAdapter->chip_info &&
		!prGlueInfo->prAdapter->chip_info->is_support_secefuse) {
		u4Offset += snprintf(pcCommand + u4Offset,
			i4TotalLen - u4Offset,
			"sec efuse ops is invalid\n");
		return (int32_t)u4Offset;
	}

	if (ucOpMode == EFUSE_READ) {
		rStatus = kalIoctl(prGlueInfo,
			wlanoidQueryProcessAccessSecEfuseRead,
			&rAccessEfuseInfo,
			sizeof
			(struct PARAM_CUSTOM_ACCESS_SECEFUSE),
			TRUE, TRUE, TRUE, &u4BufLen);
		if (rStatus == WLAN_STATUS_SUCCESS) {
			u4Offset += snprintf(pcCommand + u4Offset,
				i4TotalLen - u4Offset,
				"Read addr 0x%X = 0x%X, valid = 0x%X\n",
				u4Efuse_addr,
				prGlueInfo->prAdapter->
				aucEepromVaule[u4Index],
				prGlueInfo->prAdapter->aucEepromValid);
		}
	} else if (ucOpMode == EFUSE_WRITE) {
		rStatus = kalIoctl(prGlueInfo,
			wlanoidQueryProcessAccessSecEfuseWrite,
			&rAccessEfuseInfo,
			sizeof
			(struct PARAM_CUSTOM_ACCESS_SECEFUSE),
...
efuse_op_invalid:
	u4Offset += snprintf(pcCommand + u4Offset, i4TotalLen - u4Offset,
		"\nHelp menu\n");
	u4Offset += snprintf(pcCommand + u4Offset, i4TotalLen - u4Offset,
		"\tRead:\t\"secefuse read addr_hex\", ex: secefuse read 0x20\n");
	u4Offset += snprintf(pcCommand + u4Offset, i4TotalLen - u4Offset,
		"\tWrite:\t\"secefuse write addr_hex val_hex\", ex: secefuse write 0x0 0x50700000\n");
...
#if CFG_WIFI_FMCW_RADAR_TRAFFIC_REPORT
static int priv_driver_get_radar_traffic_report(IN struct net_device *prNetDev,
				 IN char *pcCommand, IN int i4TotalLen)
{
	struct GLUE_INFO *prGlueInfo = NULL;
	struct ADAPTER *prAdapter = NULL;
	...
	int32_t i4Argc = 0;
	int8_t *apcArgv[WLAN_CFG_ARGV_MAX] = { 0 };
	struct CMD_FMCW_RADAR_TRAFFIC_INFO *cmd = NULL;
	...
	cmd = (struct CMD_FMCW_RADAR_TRAFFIC_INFO *)
		kalMemAlloc(sizeof(*cmd), VIR_MEM_TYPE);
	...
	cmd->u2Type = CMD_GET_FMCW_RADAR_REPORT_TYPE;
	cmd->u2Len = sizeof(*cmd);

	if (strnicmp(apcArgv[1], "GET", strlen("GET")) == 0) {
		cmd->ucAction = CMD_FMCW_RADAR_REPORT_GET;
	} else if (strnicmp(apcArgv[1], "RESET", strlen("RESET")) == 0) {
		cmd->ucAction = CMD_FMCW_RADAR_REPORT_RESET;
		cmd->u2Type |= CMD_ADV_CONTROL_SET;
	} else {
		i4BytesWritten += snprintf(pcCommand + i4BytesWritten,
			i4TotalLen - i4BytesWritten,
	...
	if (rStatus != WLAN_STATUS_SUCCESS)
		return -1;
	else if (cmd->ucAction == CMD_FMCW_RADAR_REPORT_GET &&
			prAdapter->rFmcwRadarVar.ucRadarEnable) {
		i4BytesWritten += kalSnprintf(pcCommand + i4BytesWritten,
			i4TotalLen - i4BytesWritten,
			"%s\n", "\n");
		...
		i4BytesWritten += kalSnprintf(pcCommand + i4BytesWritten,
			i4TotalLen - i4BytesWritten,
			"%-20s%s%d%s\n", "Frequency", " = ",
			prAdapter->rFmcwRadarVar.u4Freg, " Hz");

		i4BytesWritten += kalSnprintf(pcCommand + i4BytesWritten,
			i4TotalLen - i4BytesWritten,
			"%-20s%s%d\n", "Bandwidth", " = ",
			prAdapter->rFmcwRadarVar.u4BandWidth);

		i4BytesWritten += kalSnprintf(pcCommand + i4BytesWritten,
			i4TotalLen - i4BytesWritten,
			"%-20s%s%d%s\n", "CTS2SELF NAV", " = ",
			prAdapter->rFmcwRadarVar.u2CTSNAV, " us");

		i4BytesWritten += kalSnprintf(pcCommand + i4BytesWritten,
			i4TotalLen - i4BytesWritten,
			"%-20s%s%d\n", "CBP Mode", " = ",
			prAdapter->rFmcwRadarVar.ucCBPEnable);

		i4BytesWritten += kalSnprintf(pcCommand + i4BytesWritten,
			i4TotalLen - i4BytesWritten,
			"%-20s%s%d\n", "Frame Per Second", " = ",
			prAdapter->rFmcwRadarVar.ucFpsCnt);

		i4BytesWritten += kalSnprintf(pcCommand + i4BytesWritten,
			i4TotalLen - i4BytesWritten,
			"%-20s%s%d\n", "Tx Burst Count", " = ",
			prAdapter->rFmcwRadarVar.ucBurstCnt);

		i4BytesWritten += kalSnprintf(pcCommand + i4BytesWritten,
			i4TotalLen - i4BytesWritten,
			"%-20s%s%d%s\n", "Tx Sample Per Frame", " = ",
			(IS_FEATURE_ENABLED(prWifiVar->fgRadarCoupling)) ?
			100 : 50, "\n");
		...
		i4BytesWritten += kalSnprintf(pcCommand + i4BytesWritten,
			i4TotalLen - i4BytesWritten,
			"%-20s%s%s\n", "Force CBP Tx", " = ",
			(IS_FEATURE_ENABLED(prWifiVar->fgRadarForceCBPTx)) ?
			"Enable" : "Disable");
		...
		i4BytesWritten += kalSnprintf(pcCommand + i4BytesWritten,
			i4TotalLen - i4BytesWritten,
			"%-20s%s%d\n", "CORRECT_DATA", " = ",
			cmd->u4ReasonCode[ENUM_CORRECT_DATA]);

		i4BytesWritten += kalSnprintf(pcCommand + i4BytesWritten,
			i4TotalLen - i4BytesWritten,
			"%-20s%s%d\n", "FAIL_PS_FRAME", " = ",
			cmd->u4ReasonCode[ENUM_FAIL_ERROR_CENTRAL_FREQ]);

		i4BytesWritten += kalSnprintf(pcCommand + i4BytesWritten,
			i4TotalLen - i4BytesWritten,
			"%-20s%s%d\n", "FAIL_CHANNEL_BUSY", " = ",
			cmd->u4ReasonCode[ENUM_FAIL_CHANNEL_BUSY]);

		i4BytesWritten += kalSnprintf(pcCommand + i4BytesWritten,
			i4TotalLen - i4BytesWritten,
			"%-20s%s%d\n", "FAIL_CTS_FAIL", " = ",
			cmd->u4ReasonCode[ENUM_FAIL_CTS_FAIL]);

		i4BytesWritten += kalSnprintf(pcCommand + i4BytesWritten,
			i4TotalLen - i4BytesWritten,
			"%-20s%s%d\n", "TSSI_CALIBRATION", " = ",
			cmd->u4ReasonCode[ENUM_TSSI_CALIBRATION]);

		i4BytesWritten += kalSnprintf(pcCommand + i4BytesWritten,
			i4TotalLen - i4BytesWritten,
			"%-20s%s%d\n", "DUMP_ACC_DATA", " = ",
			cmd->u4ReasonCode[ENUM_DUMP_ACC_DATA]);

		i4BytesWritten += kalSnprintf(pcCommand + i4BytesWritten,
			i4TotalLen - i4BytesWritten,
			"%-20s%s%d\n", "FAIL_PHASE_CAL", " = ",
			cmd->u4ReasonCode[ENUM_FAIL_PHASE_CAL]);

		i4BytesWritten += kalSnprintf(pcCommand + i4BytesWritten,
			i4TotalLen - i4BytesWritten,
			"%-20s%s%d\n", "FORCE_CBP_TX", " = ",
			cmd->u4ReasonCode[ENUM_FORCE_CBP_TX]);

		i4BytesWritten += kalSnprintf(pcCommand + i4BytesWritten,
			i4TotalLen - i4BytesWritten,
			"%-20s%s%d\n", "MOVE_DATA_FAIL", " = ",
			cmd->u4ReasonCode[ENUM_MOVE_DATA_FAIL]);

		i4BytesWritten += kalSnprintf(pcCommand + i4BytesWritten,
			i4TotalLen - i4BytesWritten,
			"%-20s%s%d\n", "SKIP_FOR_SCAN", " = ",
			cmd->u4ReasonCode[ENUM_SKIP_FOR_SCAN]);
	}
	return i4BytesWritten;
...
get_report_invalid:
	...
	i4BytesWritten += snprintf(pcCommand + i4BytesWritten,
		i4TotalLen - i4BytesWritten,
		"\nformat:radar_traffic_report [get|reset]");
	return i4BytesWritten;
}
#endif
...
#if (CFG_SUPPORT_ANT_SMART_DIVERSITY == 1)
static int32_t priv_driv_enable_ant_switch(
					IN struct net_device *prNetDev,
					IN char *pcCommand, IN int i4TotalLen)
{
	struct GLUE_INFO *prGlueInfo = NULL;
	...
	if (i4Argc >= 2) {
		...
		prAdapter->rWifiVar.ucAntSwDivEnable = ucEnable;

		DBGLOG(REQ, WARN, "ANT Switch  enable %d\n",
			prAdapter->rWifiVar.ucAntSwDivEnable);
...
static int32_t priv_driv_ant_switch_control(
					IN struct net_device *prNetDev,
					IN char *pcCommand, IN int i4TotalLen)
{
	struct GLUE_INFO *prGlueInfo = NULL;
	...
	if (i4Argc >= 2) {
		u4Ret = kalkStrtou8(apcArgv[1], 0, &ucForce);

		if (u4Ret)
			DBGLOG(REQ, LOUD, "parse ucForce error u4Ret=%d\n",
				u4Ret);

		if (ucForce == 1 || ucForce == 0)
			kalAntSwDivSwitchCtrl(prAdapter, ucForce);

		DBGLOG(REQ, WARN, "ANT Force %d\n", ucForce);
...
static int priv_driver_set_scan_min_max_dwell_time(
	IN struct net_device *prNetDev,
	IN char *pcCommand, IN int i4TotalLen)
{
	struct GLUE_INFO *prGlueInfo = NULL;
	...
	/* example: SET_SCAN_DWELL_TIME 8 10 (Set) {min, max} */
	/* example: SET_SCAN_DWELL_TIME 0 0(clean) */

	if (i4Argc >= 3) {

		u4Ret = kalkStrtou32(apcArgv[1], 0, &MinDwell);
		if (u4Ret) {
			DBGLOG(REQ, LOUD, "parse MinDwell error u4Ret=%d\n",
			       u4Ret);
			return -1;
		}

		u4Ret = kalkStrtou32(apcArgv[2], 0, &MaxDwell);
		if (u4Ret) {
			DBGLOG(REQ, LOUD,
			       "parse  MaxDwell error u4Ret=%d\n", u4Ret);
			return -1;
		}

		/*MinDwell need >= 8 && MaxDwellneed >= 10*/
		if (MinDwell >= 8 && MaxDwell >= 10) {
			prAdapter->rWifiVar.ucChannelMinDwellTime = MinDwell;
			prAdapter->rWifiVar.ucChannelMaxDwellTime = MaxDwell;
		} else {
			prAdapter->rWifiVar.ucChannelMinDwellTime = 0;
			prAdapter->rWifiVar.ucChannelMaxDwellTime = 0;
		}

		DBGLOG(REQ, INFO, "MinDwell[%d],MaxDwell[%d]\n",
			MinDwell, MaxDwell);
		return 0;
	}  else
		return -1;
...
#if (CFG_SUPPORT_RADAR_CMD == 1)

static int priv_driver_set_radar_params(
	IN struct net_device *prNetDev,
	IN char *pcCommand,
	IN int i4TotalLen)
{
	struct GLUE_INFO *prGlueInfo = NULL;
	struct ADAPTER *prAdapter = NULL;
	struct CMD_RADAR_CTRL_PARA *prRadarCtrl = NULL;
	struct CMD_RADAR_CTRL_SETTING_T *prRadarSetting = NULL;
	struct CMD_RADAR_CTRL_ENABLE_T *prRadarEnable = NULL;
	struct RADAR_CMD_RESP_REPORT *prRadarReport = NULL;
	...
	uint32_t u4Offset = 0;
	uint32_t u4Freq = 0;
	uint32_t u4BandWidth = 0;
	uint16_t u2CTSNAV = 0;
	uint8_t ucCBPEnable = 0;
	uint8_t ucSubCmdId = 0;
	uint8_t ucRadarEnable = 0;
	uint8_t ucFpsCnt = 2;
	uint8_t ucBurstCnt = 1;
	uint8_t ucCount = 0;
	uint32_t au4BandWidthTable[] = {120, 130, 140, 141, 150, 160, 320};
	bool fgIsFreqVaild = FALSE;
	...
	rStatus = kalkStrtou8(apcArgv[1], 0, &ucSubCmdId);

	if (rStatus) {
		DBGLOG(REQ, ERROR, "kalkStrtou8 fail.\n");
		u4Offset = -1;
		goto end;
	}

	switch (ucSubCmdId) {
	case ENUM_RADAR_CTRL_SET_RADAR:
		u4InfoBufLen = sizeof(struct CMD_RADAR_CTRL_PARA) +
			sizeof(struct CMD_RADAR_CTRL_SETTING_T);
		prRadarCtrl = kalMemAlloc(u4InfoBufLen, VIR_MEM_TYPE);
		if (prRadarCtrl == NULL) {
			DBGLOG(REQ, ERROR, "request RadarCtrl Mem fail.\n");
			u4Offset = -1;
			goto end;
		}

		prRadarCtrl->ucSubCmdId = ENUM_RADAR_CTRL_SET_RADAR;
		prRadarSetting = (struct CMD_RADAR_CTRL_SETTING_T *)
			prRadarCtrl->aucBody;

		if (i4Argc != 6) {
			DBGLOG(REQ, ERROR, "invalid params num.\n");
			goto cmd_invalid;
		}

		rStatus = kalkStrtou32(apcArgv[2], 0, &u4Freq);
		fgIsFreqVaild =
			rlmDomainCheckRadarCountryCode(prAdapter, u4Freq);
		if (rStatus || !fgIsFreqVaild) {
			DBGLOG(REQ, ERROR, "invalid Freq = %d\n", u4Freq);
			goto cmd_invalid;
		}

		prRadarSetting->u4Freg = u4Freq;
		prAdapter->rFmcwRadarVar.u4Freg = u4Freq;

		rStatus = kalkStrtou32(apcArgv[3], 0, &u4BandWidth);

		for (ucCount = 0;
			ucCount < ARRAY_SIZE(au4BandWidthTable);
			ucCount++) {
			if (u4BandWidth == au4BandWidthTable[ucCount]) {
				prRadarSetting->u4BandWidth = u4BandWidth;
				prAdapter->rFmcwRadarVar.u4BandWidth =
					u4BandWidth;
		...
		if (rStatus) {
			DBGLOG(REQ, ERROR, "invalid BandWidth = %d\n"
					, u4BandWidth);
			goto cmd_invalid;
		}

		rStatus = kalkStrtou16(apcArgv[4], 0, &u2CTSNAV);
		if (rStatus || (u2CTSNAV > 0x7FFF)) {
			DBGLOG(REQ, ERROR, "invalid CTSNAV = %d\n"
						, u2CTSNAV);
			goto cmd_invalid;
		} else if ((u4Freq == 6095000 && u2CTSNAV != 0)) {
			DBGLOG(REQ, ERROR, "6G no support CTSNAV\n");
			goto cmd_invalid;
		} else {
			prRadarSetting->u2CTSNAV = u2CTSNAV;
			prAdapter->rFmcwRadarVar.u2CTSNAV = u2CTSNAV;
		}

		rStatus = kalkStrtou8(apcArgv[5], 0, &ucCBPEnable);
		if (rStatus || (ucCBPEnable > 2)) {
			DBGLOG(REQ, ERROR, "invalid CBPEnable = %d\n"
						, ucCBPEnable);
			goto cmd_invalid;
		}

		prRadarSetting->ucCBPEnable = ucCBPEnable;
		prAdapter->rFmcwRadarVar.ucCBPEnable = ucCBPEnable;

		break;

	case ENUM_RADAR_CTRL_SET_ENABLE_RADAR:
		u4InfoBufLen = sizeof(struct CMD_RADAR_CTRL_PARA) +
			sizeof(struct CMD_RADAR_CTRL_ENABLE_T);
		prRadarCtrl = kalMemAlloc(u4InfoBufLen, VIR_MEM_TYPE);
		if (prRadarCtrl == NULL) {
			DBGLOG(REQ, ERROR, "request RadarCtrl Mem fail.\n");
			u4Offset = -1;
		...
		prRadarCtrl->ucSubCmdId = ENUM_RADAR_CTRL_SET_ENABLE_RADAR;
		prRadarEnable = (struct CMD_RADAR_CTRL_ENABLE_T *)
			prRadarCtrl->aucBody;

		prRadarEnable->ucFpsCnt = 2;
		...
		rStatus = kalkStrtou8(apcArgv[2], 0, &ucRadarEnable);

		if (rStatus || (ucRadarEnable > 1)) {
			DBGLOG(REQ, ERROR, "invalid RadarEnable = %d\n"
					, ucRadarEnable);
			goto cmd_invalid;
		}

		prRadarEnable->ucRadarEnable = ucRadarEnable;
		prAdapter->rFmcwRadarVar.ucRadarEnable = ucRadarEnable;

		rStatus = kalkStrtou8(apcArgv[3], 0, &ucFpsCnt);

		if (rStatus || (ucFpsCnt < 1) || (ucFpsCnt > 10)) {
			DBGLOG(REQ, ERROR, "invalid FpsCnt = %d\n", ucFpsCnt);
			goto cmd_invalid;
		}

		prRadarEnable->ucFpsCnt = ucFpsCnt;
		prAdapter->rFmcwRadarVar.ucFpsCnt = ucFpsCnt;

		rStatus = kalkStrtou8(apcArgv[4], 0, &ucBurstCnt);

		if (rStatus || (ucBurstCnt < 1) || (ucBurstCnt > 10)) {
			DBGLOG(REQ, ERROR, "invalid BurstCnt = %d\n"
					, ucBurstCnt);
			goto cmd_invalid;
		}

		prRadarEnable->ucBurstCnt = ucBurstCnt;
		prAdapter->rFmcwRadarVar.ucBurstCnt = ucBurstCnt;

		break;

	default:
		DBGLOG(REQ, ERROR, "invalid Sub-cmd ID = %d.\n", ucSubCmdId);
		goto cmd_invalid;
	}

	rStatus = kalIoctl(prGlueInfo,
		wlanoidSetRadarParams,
		prRadarCtrl,
		u4InfoBufLen,
		TRUE,
		TRUE,
	...
	prRadarReport = (struct RADAR_CMD_RESP_REPORT *)prRadarCtrl->aucBody;

	if (prRadarReport->ucStatus == ENUM_RADAR_CTRL_RESULT_SUCCESS) {
		u4Offset += kalSnprintf(pcCommand + u4Offset,
			i4TotalLen - u4Offset, "SUCCESS\n");
	} else {
		u4Offset += kalSnprintf(pcCommand + u4Offset,
			i4TotalLen - u4Offset, "FAIL\n");
	}
...
cmd_invalid:
	...
	u4Offset += kalSnprintf(pcCommand + u4Offset, i4TotalLen - u4Offset,
			"\nHelp Menu\n");
	u4Offset += kalSnprintf(pcCommand + u4Offset, i4TotalLen - u4Offset,
			"\tSet Radar: SET_RADAR_PARAMS 0 [Freq] [BW] [CTSNAV] [CBP]\n");
	u4Offset += kalSnprintf(pcCommand + u4Offset, i4TotalLen - u4Offset,
			"\t[Freq]: 5787500, 5800000\n");
	u4Offset += kalSnprintf(pcCommand + u4Offset, i4TotalLen - u4Offset,
			"\t[BW]: 150\n");
	u4Offset += kalSnprintf(pcCommand + u4Offset, i4TotalLen - u4Offset,
			"\t[CTSNAV]: 0 ~ 32767\n");
	u4Offset += kalSnprintf(pcCommand + u4Offset, i4TotalLen - u4Offset,
			"\t[CBP]: 0(off), 1(on), 2(5G off, 6G on)\n\n");
	u4Offset += kalSnprintf(pcCommand + u4Offset, i4TotalLen - u4Offset,
			"\tEnable Radar: SET_RADAR_PARAMS 1 [Control] [FPS] [BurstCnt]\n");
	u4Offset += kalSnprintf(pcCommand + u4Offset, i4TotalLen - u4Offset,
	...
	if (prRadarCtrl != NULL)
		kalMemFree(prRadarCtrl, VIR_MEM_TYPE, u4InfoBufLen);
...
#if CFG_SUPPORT_SAP_TX_FAIRNESS
static int priv_driver_set_sap_tx_fairness(IN struct net_device *prNetDev,
	IN char *pcCommand, IN int i4TotalLen)
{
	struct GLUE_INFO *prGlueInfo = NULL;
	int32_t i4BytesWritten = 0;
	...
	if (i4Argc == 2) {
		...
		fgEnable = (uint8_t) u4Parse;
		prGlueInfo->prAdapter->rWifiVar.fgSapTxFairness =
			(fgEnable ? TRUE : FALSE);

		DBGLOG(REQ, LOUD, "fgSapTxFairness = %u\n", fgEnable);
	} else {
		DBGLOG(INIT, LOUD,
			"iwpriv wlan0 driver SET_SAP_TX_FAIRNESS <en>\n");
		DBGLOG(INIT, LOUD, "<en> 1: enable, 0: disable.\n");
	}

	return i4BytesWritten;
}

static int priv_driver_reset_airtime_counter(IN struct net_device *prNetDev,
	IN char *pcCommand, IN int i4TotalLen)
{
	struct GLUE_INFO *prGlueInfo = NULL;
	struct ADAPTER *prAdapter = NULL;
	...
	int8_t *apcArgv[WLAN_CFG_ARGV_MAX] = {0};
	uint32_t u4Ret = 0, u4Parse = 0;
	uint32_t wtbl_lmac_adm_ctrl_addr = 0;
	uint8_t idx = 0, start_idx = 0, end_idx = 0;
	uint8_t i = 0;
	...
	prAdapter = prGlueInfo->prAdapter;
	prChipInfo = prAdapter->chip_info;
	end_idx = prGlueInfo->prAdapter->ucWtblEntryNum;
	...
	for (i = start_idx; i <= end_idx; i++) {
		CONNAC2X_LWTBL_CONFIG(prAdapter,
			prChipInfo->u4LmacWtblDUAddr, i);
		wtbl_lmac_adm_ctrl_addr =
			CONNAC2X_LWTBL_IDX2BASE(prChipInfo->
				u4LmacWtblDUAddr, i, 22);
		HAL_MCR_WR(prAdapter, wtbl_lmac_adm_ctrl_addr, 0);
	}

	return i4BytesWritten;
}
#endif /* CFG_SUPPORT_SAP_TX_FAIRNESS */
...
struct PRIV_CMD_HANDLER priv_cmd_handlers[] = {
	...
	{CMD_SET_NAN_P2P, priv_driver_set_nan_p2p},
#endif
#if CFG_NAN_SUPPORT_FAST_DISC
	{CMD_SET_NAN_FAST_DISC, priv_driver_set_fast_discovery},
#endif
#if (CFG_SUPPORT_NAN_CUSTOMIZATION_VERSION == 1)
	{CMD_FAW_DEF_CONFIG, priv_driver_set_faw_def_config},
#endif
#endif
#if (CFG_SUPPORT_DFS_MASTER == 1)
	...
	{CMD_RESETALL_PASSTHTOUGH, priv_driver_resetall_passthrough},
...
#if (CFG_SUPPORT_SCAN_RADIOINFO == 1)
	{CMD_GET_CH_RADIOINFO, priv_driver_get_ch_radioinfo},
#endif /* end CFG_SUPPORT_SCAN_RADIOINFO */
...
#endif
#if CFG_WIFI_FMCW_RADAR_TRAFFIC_REPORT
	{CMD_FMCW_RADAR_TRAFFIC_REPORT, priv_driver_get_radar_traffic_report},
#endif
...
#endif
#if (CFG_SUPPORT_RTT == 1)
	{CMD_SET_RTT, priv_driver_set_rtt},
	{CMD_CONTROL_RTT, priv_driver_control_rtt},
#endif
#if (CFG_SUPPORT_WIFI_DL_TEST_MODE == 1)
	{CMD_FWDL_TEST_MODE, priv_driver_fwdl_test_mode},
#endif
#if (CFG_SUPPORT_6G_PWR_REGS == 1)
	/* For Tx Power 6G SP TPE */
	{CMD_TX_PWR_ENV_CTRL, priv_driver_tx_pwr_env_ctrl},
#endif  /* CFG_SUPPORT_6G_PWR_REGS == 1 */
#if (CFG_SUPPORT_TSF_SYNC == 1)
...
#endif
#if (CFG_SUPPORT_ANT_SMART_DIVERSITY == 1)
	{CMD_ANTENNA_SWITCH_ENABLE, priv_driv_enable_ant_switch},
	{CMD_ANTENNA_SWITCH_CONTROL, priv_driv_ant_switch_control},
#endif
#if (CFG_WIFI_ULTRA_RADIO_OFF_CTRL == 1)
...
#if (CFG_SUPPORT_6G_PWR_REGS == 1)
	/* For Tx Power 6G REGULATIONS */
	{CMD_SET_6G_POWER_MODE, priv_driver_set_6g_pwr_mode},
#endif /*CFG_SUPPORT_6G_PWR_REGS */

#if (CFG_SUPPORT_RADAR_CMD == 1)
	{CMD_SET_RADAR_PARAMS, priv_driver_set_radar_params},
#endif
#if CFG_SUPPORT_SAP_TX_FAIRNESS
	{CMD_SET_SAP_TX_FAIRNESS, priv_driver_set_sap_tx_fairness},
	{CMD_RESET_AIRTIME_CNT, priv_driver_reset_airtime_counter},
#endif
	{CMD_GET_SLEEP_CNT_INFO, priv_driver_get_sleep_cnt_info},
	{CMD_SET_LP_KEEP_PWR_CTRL, priv_driver_set_lp_keep_pwr_ctrl},
	{CMD_SET_SCAN_MIN_MAX_DWELL_TIME,
		priv_driver_set_scan_min_max_dwell_time},
#if CFG_SECEFUSE_ACCESS
	{CMD_SECEFUSE, priv_driver_sec_efuse_ops},
#endif
#if CFG_SUPPORT_EASY_DEBUG
	{CMD_FW_PARAM, priv_driver_fw_param},
#endif /* CFG_SUPPORT_EASY_DEBUG */
};
...
int32_t priv_driver_cmds(struct GLUE_INFO *prGlueInfo,
			...
			int32_t i4TotalLen)
{
	int32_t *pu4HaltFlag = wlanGetHaltFlag(prGlueInfo);
	int32_t i4BytesWritten = 0;
	int32_t i4CmdFound = 0;
	int i;
	...
	for (i = 0; i < sizeof(priv_cmd_handlers) / sizeof(struct
		...
		if (strnicmp(pcCommand,
			...
			if (!prGlueInfo->prAdapter) {
				DBGLOG(REQ, WARN,
					"prAdapter is NUll skip priv_driver_cmds\n");
				return -1;
			}
	...
	if (i4CmdFound == 0) {
		...
		if (strnicmp(pcCommand, CMD_RSSI, strlen(CMD_RSSI)) == 0) {
			...
			i4BytesWritten = priv_driver_get_antenna_report(
					prNetDev, pcCommand, i4TotalLen);
#endif
#if CFG_WIFI_FMCW_RADAR_TRAFFIC_REPORT
		else if (strnicmp(pcCommand, CMD_FMCW_RADAR_TRAFFIC_REPORT,
			strlen(CMD_FMCW_RADAR_TRAFFIC_REPORT)) == 0)
			i4BytesWritten = priv_driver_get_radar_traffic_report(
					prNetDev, pcCommand, i4TotalLen);
#endif
#if CFG_WIFI_SUPPORT_NOISE_HISTOGRAM
			...
			i4BytesWritten = priv_driver_set_qos(prNetDev,
							pcCommand, i4TotalLen);
#if (CFG_SUPPORT_6G_PWR_REGS == 1)
		} else if (strnicmp(pcCommand, CMD_SET_6G_POWER_MODE,
				strlen(CMD_SET_6G_POWER_MODE)) == 0) {
			i4BytesWritten = priv_driver_set_6g_pwr_mode(prNetDev,
							pcCommand, i4TotalLen);
#endif /*CFG_SUPPORT_6G_PWR_REGS */
#if CFG_SUPPORT_CSI
...
#endif
#if CFG_SUPPORT_SAP_TX_FAIRNESS
		} else if (!strnicmp(pcCommand, CMD_SET_SAP_TX_FAIRNESS,
					strlen(CMD_SET_SAP_TX_FAIRNESS))) {
			i4BytesWritten = priv_driver_set_sap_tx_fairness(
				prNetDev, pcCommand, i4TotalLen);
		} else if (!strnicmp(pcCommand, CMD_RESET_AIRTIME_CNT,
					strlen(CMD_RESET_AIRTIME_CNT))) {
			i4BytesWritten = priv_driver_reset_airtime_counter(
				prNetDev, pcCommand, i4TotalLen);
#endif
		} else
	...
	IN OUT struct ifreq *prReq, IN int i4Cmd)
{
	...
	if (priv_cmd.total_len <= 0
		...
		DBGLOG(REQ, ERROR, "%s: total_len invalid\n", __func__);
		return -EINVAL;
	}
			    ...
			    IN OUT struct ifreq *prReq, IN int i4Cmd)
{
	...
	i4BytesWritten =
		priv_driver_cmds(prGlueInfo, prNetDev, pcCommand, i4TotalLen);
...
#if (CFG_SUPPORT_6G_PWR_REGS == 1)
int priv_driver_tx_pwr_env_ctrl(
	IN struct net_device *prNetDev,
	IN char *pcCommand,
	IN int i4TotalLen)
{
	...
	if (!prNetDev) {
		DBGLOG(REQ, ERROR,
		"null prNetDev\n");
		return (int)WLAN_STATUS_INVALID_DATA;
	}

	if (GLUE_CHK_PR2(prNetDev, pcCommand) == FALSE)
		return (int)WLAN_STATUS_FAILURE;

	prGlueInfo = *((struct GLUE_INFO **) netdev_priv(prNetDev));
	if (!prGlueInfo) {
		DBGLOG(REQ, ERROR, "Invalid prGlueInfo\n");
		return (int)WLAN_STATUS_FAILURE;
	}
	prAdapter = prGlueInfo->prAdapter;
	if (!prAdapter) {
		DBGLOG(REQ, ERROR, "Invalid prAdapter\n");
		return (int)WLAN_STATUS_FAILURE;
	}

	wlanCfgParseArgument(pcCommand, &i4Argc, apcArgv);

	/* Prevent Kernel Panic, set default i4ArgNum to 3 */
	if (i4Argc == i4ArgNum) {
		/* Get Command
		 * 1: Set enable/disable Tx Pwr Envelope
		 */
		...
		if (u4Cmd == 1) {
			u4Ret = kalkStrtou32(apcArgv[2], 0, &u4Enable);
			if (u4Ret)
				return (int)WLAN_STATUS_INVALID_DATA;

			if (u4Enable == 0) {
				prAdapter->fgTxPwrEnvEnable = FALSE;
				kalSnprintf(pcCommand, i4TotalLen,
					CMD_SET_CHIP " TpeEnableConfig 0");
			} else {
				prAdapter->fgTxPwrEnvEnable = TRUE;
				kalSnprintf(pcCommand, i4TotalLen,
					CMD_SET_CHIP " TpeEnableConfig 1");
			}

			priv_driver_set_chip_config(prNetDev, pcCommand,
						    i4TotalLen);
		} else {
			i4Offset += snprintf(pcCommand, (size_t)i4TotalLen,
				"\nUnknown command\n");
		}
	}
			...
			int i4TotalLen)
{
	...
	uint8_t ucMode = PWR_MODE_6G_NUM, c = 0, ucBssIndex = 0;
	uint8_t ucEnable = 0, ucForce = 0;
	int8_t *apcArgv[WLAN_CFG_ARGV_MAX] = { 0 };
	struct ADAPTER *prAdapter = NULL;
	struct WIFI_VAR *prWifiVar = NULL;
	...
	/* Usage:
	 * iwpriv wlan0 driver "SET_6G_PWR_MODE mode [num]"
	 * iwpriv wlan0 driver "SET_6G_PWR_MODE VlpSup6G [num]"
	 * iwpriv wlan0 driver "SET_6G_PWR_MODE force [num]"
	 * iwpriv wlan0 driver "SET_6G_PWR_MODE"
	 */
	DBGLOG(REQ, INFO, "command is %s\n", pcCommand);
	wlanCfgParseArgument(pcCommand, &i4Argc, apcArgv);
	DBGLOG(REQ, INFO, "argc is %i\n", i4Argc);
	...
	for (c = 0; (c < i4Argc) && (c + 1 < i4Argc); c++) {
		if (kalStrCmp(apcArgv[c], "mode") == 0) {
			...
			rStatus = kalIoctl(prGlueInfo, wlanoidSet6GPwrMode,
				   &ucMode, sizeof(uint8_t),
				   TRUE, TRUE, TRUE, &u4BufLen);

			if (rStatus != WLAN_STATUS_SUCCESS) {
				return WLAN_STATUS_FAILURE;
			} else {
				i4BytesWritten += kalScnprintf(pcCommand +
				i4BytesWritten, i4TotalLen - i4BytesWritten,
				"\nSet 6G Power Mode = [%u/%s] success",
				ucMode, apucPwrModeType[ucMode]);
			}
		} else if (kalStrCmp(apcArgv[c], "LpiSup6G") == 0) {
			u4Ret = kalkStrtou8(apcArgv[c+1], 0, &ucEnable);
			if (u4Ret) {
				DBGLOG(REQ, ERROR,
					"parse set_6g_pwr_mode error Ret=%d\n",
					u4Ret);
			...
			prWifiVar->fgLpiSup6G = ucEnable;
		} else if (kalStrCmp(apcArgv[c], "SpSup6G") == 0) {
			u4Ret = kalkStrtou8(apcArgv[c+1], 0, &ucEnable);
			if (u4Ret) {
				DBGLOG(REQ, ERROR,
					"parse set_6g_pwr_mode error Ret=%d\n",
			...
			prWifiVar->fgSpSup6G = ucEnable;
		} else if (kalStrCmp(apcArgv[c], "VlpSup6G") == 0) {
			u4Ret = kalkStrtou8(apcArgv[c+1], 0, &ucEnable);
			if (u4Ret) {
				DBGLOG(REQ, ERROR,
					"parse set_6g_pwr_mode error Ret=%d\n",
			...
			prWifiVar->fgVlpSup6G = ucEnable;
		} else if (kalStrCmp(apcArgv[c], "PwrRdutMd") == 0) {
			u4Ret = kalkStrtou8(apcArgv[c+1], 0, &ucEnable);
			if (u4Ret) {
				DBGLOG(REQ, ERROR,
					"parse set_6g_pwr_mode error Ret=%d\n",
			...
			prWifiVar->fgPwrRdutMd = ucEnable;
		} else if (kalStrCmp(apcArgv[c], "force") == 0) {
			u4Ret = kalkStrtou8(apcArgv[c+1], 0, &ucForce);
			if (u4Ret) {
				DBGLOG(REQ, ERROR,
					"parse set_6g_pwr_mode error Ret=%d\n",
					u4Ret);
			...
			if (ucForce)
				...
				prAdapter->fg6GPwrModeForce = FALSE;
		}
	}

	i4BytesWritten += kalScnprintf(pcCommand + i4BytesWritten,
	i4TotalLen - i4BytesWritten,
	"\nPwrMode(fw)[%s/VLP] Support LPI/SP/VLP/Num[%u %u %u %u]",
		apucPwrModeType[prAdapter->e6GPwrModeCurr],
	...
	for (ucBssIndex = 0; ucBssIndex < MAX_BSSID_NUM; ucBssIndex++) {
		i4BytesWritten += kalScnprintf(pcCommand +
		i4BytesWritten, i4TotalLen - i4BytesWritten,
		"BSS(%u) PwrMode = [%u/%s], Active = [%u]\n", ucBssIndex,
		prAdapter->e6GPwrMode[ucBssIndex],
		apucPwrModeType[prAdapter->e6GPwrMode[ucBssIndex]],
			...
			IN char *pcCommand, IN int i4TotalLen)
{
	...
	if (!prCSIInfo) {
		DBGLOG(REQ, ERROR,
			"[CSI] allocate memory for prCSIInfo failed\n");
		i4BytesWritten = -1;
		goto out;
	}
...
#if CFG_SUPPORT_DBDC
	prCSICtrl->ucBandIdx = (enum ENUM_DBDC_BN) prCSIInfo->ucBandIdx;
#else
	...
	if (prCSICtrl->ucMode == CSI_CONTROL_MODE_START &&
		rttIsActive(prGlueInfo->prAdapter)) {
		DBGLOG(REQ, WARN, "RTT is active, can not start CSI\n");
		goto out;
	}
#endif /* CFG_SUPPORT_RTT */

#if CFG_CSI_DYNAMIC_BUFFER
	if ((prCSICtrl->ucMode == CSI_CONTROL_MODE_START) &&
		csiAllocDynamicBuffer(prGlueInfo) != TRUE) {
		DBGLOG(REQ, ERROR,
			"[CSI] allocate memory for dynamic buffer failed\n");
		i4BytesWritten = -1;
		goto out;
...
#if (CFG_SUPPORT_CSI_VNI == 0)
		prCSIInfo->u4CSIBufferHead = 0;
		prCSIInfo->u4CSIBufferTail = 0;
		prCSIInfo->u4CSIBufferUsed = 0;
#endif /* CFG_SUPPORT_CSI_VNI == 0 */
		prCSIInfo->fgIsRunning = prCSICtrl->ucMode;
		goto send_cmd;
	}
...
#if CFG_SUPPORT_DBDC
	if (prCSICtrl->ucCfgItem == CSI_CONFIG_RSVD1) {
		enum ENUM_DBDC_BN eBand = 0;

		eBand = (enum ENUM_DBDC_BN) prCSICtrl->ucValue1;
		if (eBand < ENUM_BAND_NUM) {
			DBGLOG(REQ, INFO, "[CSI] set band: %d\n", eBand);
			prCSIInfo->ucBandIdx = eBand;
			goto out;
		} else {
...
#if CFG_CSI_DYNAMIC_BUFFER
	else if ((prCSICtrl->ucMode == CSI_CONTROL_MODE_STOP) &&
		csiFreeDynamicBuffer(prGlueInfo) != TRUE) {
		DBGLOG(REQ, ERROR,
			"[CSI] free memory for dynamic buffer failed\n");
	}
#endif
			...
			char *pcCommand, int i4TotalLen)
{
	...
	struct ADAPTER *prAdapter = NULL;
	struct GLUE_INFO *prGlueInfo = NULL;
	struct RTT_CAPABILITIES rRttCapabilities = {0};
	...
	if (!rttReq) {
		DBGLOG(REQ, ERROR, "Fail to alloc memory for rttReq.\n");
		return -1;
	}
	kalMemZero(rttReq, sizeof(struct PARAM_RTT_REQUEST));

	if (ucType == 0) {
		DBGLOG(REQ, INFO, "Test RTT CMD_LOC_GET_CAPA\n");
		rStatus = kalIoctl(prGlueInfo,
			...
			FALSE, FALSE, TRUE, &u4BufLen);
		DBGLOG(REQ, INFO,
				"RTT one_sided=%hhu, ftm=%hhu, lci=%hhu, lcr=%hhu, preamble=%hhu, bw=%hhu, responder=%hhu, ver=%hhu",
		...
		rttInfo = rttGetInfo(prAdapter);
		if (!rttInfo) {
			DBGLOG(REQ, ERROR, "rttInfo is NULL\n");
			goto out;
		}
		if (!rttInfo->fgIsIstaEnable) {
			DBGLOG(REQ, ERROR,
				"ISTA is disable, set CONTROL_RTT 2\n");
			goto out;
		}

		DBGLOG(REQ, INFO,
			"Test 11mc RTT CMD_LOC_RANGE_MC_ISTA Start\n");
		...
		config = &rttReq->arRttConfigs[0];
		COPY_MAC_ADDR(config->aucAddr, aucTestMacAddr);
		config->eType = RTT_TYPE_2_SIDED;
		config->ePeer = RTT_PEER_AP;
		config->rChannel.width =
			(enum WIFI_CHANNEL_WIDTH) ucWidth;
		config->rChannel.center_freq =
			u4Freq ? u4Freq : 5180;
		config->rChannel.center_freq0 = 0;
		config->rChannel.center_freq1 = 0;
		config->u2BurstPeriod = 0;
		config->u2NumBurstExponent = 0;
		config->u2PreferencePartialTsfTimer = 0;
		config->ucNumFramesPerBurst = 14;
		config->ucNumRetriesPerRttFrame = 3;
		config->ucNumRetriesPerFtmr = 0;
		config->ucLciRequest = 0;
		config->ucLcrRequest = 0;
		config->ucBurstDuration = 11;
		config->ePreamble = WIFI_RTT_PREAMBLE_VHT;
		config->eBw = rttBssBwToRttBw(ucWidth);
		config->eEventType = 0;
		config->ucASAP = 1;
		...
		DBGLOG(RTT, STATE,
			...
			MAC2STR(config->aucAddr), config->eType,
		...
		rStatus = kalIoctl(prGlueInfo, wlanoidHandleRttRequest,
				rttReq, sizeof(struct PARAM_RTT_REQUEST),
				FALSE, FALSE, TRUE, &u4BufLen);
	} else if (ucType == 2) {
		DBGLOG(REQ, ERROR, "Test 11az RTT, but Connac2 not support\n");
		goto out;
	} else if (ucType == 3) {
		DBGLOG(REQ, STATE,
			"Test 11mc RTT CMD_LOC_RANGE_MC_ISTA Stop\n");
		rttReq->fgEnable = false;
		rStatus = kalIoctl(prGlueInfo, wlanoidHandleRttRequest,
				rttReq, sizeof(struct PARAM_RTT_REQUEST),
				FALSE, FALSE, TRUE, &u4BufLen);
	} else {
		DBGLOG(REQ, ERROR, "Test RTT Invalid\n");
		goto out;
	}
			...
			char *pcCommand, int i4TotalLen)
{
	...
	if (!prNetDev) {
		DBGLOG(REQ, ERROR, "net_device is NULL\n");
		return -1;
	}
	prGlueInfo = *((struct GLUE_INFO **) netdev_priv(prNetDev));
	DBGLOG(REQ, LOUD, "command is %s\n", pcCommand);
	...
	if (i4Argc == 2) {
		...
		if (u4Parse > 2) {
			DBGLOG(REQ, ERROR,
				"<Setting> 0: disable. 1: mc rsta. 2: mc ista\n");
			return -1;
		}
		ucControl = (uint8_t) u4Parse;


		DBGLOG(REQ, INFO, "Control RTT: %d\n", ucControl);
		rStatus = kalIoctl(prGlueInfo, wlanoidControlRtt,
				&ucControl, sizeof(uint8_t),
				FALSE, FALSE, TRUE, &u4BufLen);
	} else {
		DBGLOG(REQ, ERROR,
			"iwpriv wlanXX driver CONTROL_RTT <Setting>\n");
		DBGLOG(REQ, ERROR,
			"<Setting> 0: disable. 1: enable mc rsta. 2: enable mc ista\n");
	}

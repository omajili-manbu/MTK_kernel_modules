...
#if CFG_SUPPORT_RX_PAGE_POOL_USE_CMA
#include "hif_pdma.h"
#endif
...
struct probe_remove_tracking_item {
	struct list_head node;
	uint32_t bus_id;
};
static LIST_HEAD(g_probe_list);
static LIST_HEAD(g_remove_list);
static spinlock_t g_probe_remove_lock;

#if CFG_CONN_DYNAMIC_POWER_CTRL
#define POWER_ON_TIMEOUT 4000
#define POWER_OFF_TIMEOUT 4000
#endif
...
#if defined(UT_TEST_MODE) && (CFG_BUILD_X86_PLATFORM == 1)
...
int kalDcSetWow(void)
{
	...
	WIPHY_PRIV(wlanGetWiphy(NULL), prGlueInfo);
	...
	if (prGlueInfo->prAdapter->chip_info->fgIsSupportL0p5Reset)
		glResetWaitL0p5WorkDone(prGlueInfo);
#endif
...
static const struct wiphy_vendor_command
...
#if CFG_MTK_SUPPLICANT_ROAMING_ENHANCE
	{
		{
			.vendor_id = GOOGLE_OUI,
			.subcmd = WIFI_SUBCMD_GET_ROAM_IMPROVE_FEATURE
		},
		.flags = WIPHY_VENDOR_CMD_NEED_WDEV |
			 WIPHY_VENDOR_CMD_NEED_NETDEV,
		.doit = mtk_cfg80211_vendor_get_roaming_improve_feature,
		VENDOR_OPS_SET_POLICY(VENDOR_CMD_RAW_DATA)
	},
#endif
	{
 ...
 */
#if CFG_CONN_DYNAMIC_POWER_CTRL
static void initBusid(void);
static void storeBusId(uint32_t busId);
#endif
/*******************************************************************************
 *                              F U N C T I O N S
 *******************************************************************************
...
#if CFG_WIFI_SUPPORT_WIFI_ON_STATISTICS
void updateWifiOnTimeStatistics(struct net_device *prDev)
{
...
u16 mtk_wlan_ndev_select_sap_tx_queue(
	...
	struct sk_buff *skb)
{
	...
	uint8_t ucBssIdx = 0;
	uint8_t aucEthDestAddr[MAC_ADDR_LEN] = {0};
	uint8_t *pucLookAheadBuf = NULL;
	uint8_t i = 0;

	prNetDevPrivate = (struct NETDEV_PRIVATE_GLUE_INFO *)netdev_priv(dev);
	if (!prNetDevPrivate)
		return CFG_NOT_SAP_TXQ_NUM;

	prAdapter = prNetDevPrivate->prGlueInfo->prAdapter;
	if (IS_FEATURE_DISABLED(prAdapter->rWifiVar.fgSapTxFairness))
		return CFG_NOT_SAP_TXQ_NUM;

	ucBssIdx = prNetDevPrivate->ucBssIdx;
	if (!IS_BSS_INDEX_APGO(prAdapter, ucBssIdx))
		return CFG_NOT_SAP_TXQ_NUM;

	pucLookAheadBuf = skb->data;
	kalMemCopy(aucEthDestAddr, pucLookAheadBuf, PARAM_MAC_ADDR_LEN);
	...
	for (i = 0; i < CFG_STA_REC_NUM; i++) {
		...
		if (EQUAL_MAC_ADDR(prStaRec->aucMacAddr, aucEthDestAddr)) {
			/* only one STA has traffic - AC */
			if (prAdapter->prGlueInfo->
				    aucTxPendingStaCnt[ucBssIdx] < 2)
				return CFG_NOT_SAP_TXQ_NUM;
			/* more than one STA has traffic - STA */
			return (u16) prStaRec->ucIndex;
		}
	}
...
static void glTaskletInit(struct GLUE_INFO *prGlueInfo)
{
	...
	prGlueInfo->u4RxTaskScheduleCnt = 0;
...
static u_int8_t glIsNapiInit(struct net_device *prDev)
{
	struct NETDEV_PRIVATE_GLUE_INFO *prNetDevPrivate = NULL;
	...
	if (atomic_dec_and_test(&prNetDevPrivate->napi_init_count))
		return TRUE;

	DBGLOG(INIT, WARN, "napi_init_count is error:%d\n",
		prNetDevPrivate->napi_init_count);
	return FALSE;
}

static void glSetNapiInit(struct net_device *prDev)
{
	struct NETDEV_PRIVATE_GLUE_INFO *prNetDevPrivate = NULL;
	...
	atomic_set(&prNetDevPrivate->napi_init_count, 1);
}
...
#if CFG_SUPPORT_RX_GRO
	glSetNapiInit(prGlueInfo->prDevHandler);
...
static void wlanFreeNetDev(struct net_device *prNetDev)
{
...
#if CFG_SUPPORT_MULTI_CARD
	struct wireless_dev *prWdev = prNetDev->ieee80211_ptr;

	WIPHY_PRIV(wlanGetWiphy(prWdev), prGlueInfo);
#endif
	...
	for (u4Idx = 0; u4Idx < KAL_AIS_NUM; u4Idx++) {
		...
		prDev = pprWdev[u4Idx]->netdev;

		DBGLOG(INIT, INFO, "free_netdev %s netdev\n", prDev->name);
		free_netdev(prDev);
	}
}
...
#if CFG_SUPPORT_PERSIST_NETDEV || CFG_SUPPORT_MULTI_CARD
	...
	uint8_t ucAisIndex)
{
	...
	if (pprWdev[ucAisIndex] && pprWdev[ucAisIndex]->netdev)
...
#if CFG_SUPPORT_PERSIST_NETDEV
static struct net_device **wlanGetNetDevice(struct GLUE_INFO *prGlueInfo)
{
#if CFG_SUPPORT_MULTI_CARD
	if (!prGlueInfo)
		return NULL;

	return prGlueInfo->prNetdev;
#else
	return gprNetdev;
#endif
}
...
int32_t *wlanGetHaltFlag(struct GLUE_INFO *prGlueInfo)
{
#if CFG_SUPPORT_MULTI_CARD
	if (!prGlueInfo)
		return NULL;

	return &prGlueInfo->u4HaltFlag;
#else
	return &g_u4HaltFlag;
#endif
}
 ...
 */
/*---------------------------------------------------------------------------*/
struct GLUE_INFO *wlanGetGlueInfo(struct wiphy *wiphy)
{
	struct GLUE_INFO *prGlueInfo = NULL;
...
{
	struct GLUE_INFO *prGlueInfo;
	struct delayed_work *prWorkq;
	struct net_device **pprNetDev;
...
#if CFG_SUPPORT_MULTI_CARD
	prWorkq = &prGlueInfo->workq;
	pprNetDev = &prGlueInfo->prNetDevice;
#else
	prWorkq = &workq;
	pprNetDev = &gPrDev;
#endif
	...
	*pprNetDev = prGlueInfo->prDevHandler;
	schedule_delayed_work(prWorkq, 0);
}
...
#if CFG_SUPPORT_MULTI_CARD
	prGlueInfo = container_of(work, struct GLUE_INFO, workq.work);
...
void wlanSchedScanStoppedWorkQueue(struct work_struct *work)
{
	...
	DBGLOG(SCN, INFO, "SchedScanReqid : %u\n",
		prGlueInfo->u8SchedScanReqid);
		      ...
		      struct net_device *prDev)
{
	...
	if (!wlanIsDriverReady(prGlueInfo,
		...
		prGlueInfo->u4ReadyFlag, kalIsResetting(prGlueInfo));
		dev_kfree_skb(prSkb);
		return NETDEV_TX_OK;
	}
#endif
...
{
	struct GLUE_INFO *prGlueInfo = NULL;
	struct delayed_work *prWorkq = NULL;
	...
	INIT_DELAYED_WORK(prWorkq, wlanSetMulticastListWorkQueue);

	/* 20150205 work queue for sched_scan */
	INIT_DELAYED_WORK(prSchedWorkq,
			  wlanSchedScanStoppedWorkQueue);

#if CFG_REDIRECT_OID_SUPPORT
	INIT_DELAYED_WORK(prOidWorkq,
...
static int wlanOpen(struct net_device *prDev)
{
	...
	if (ppe_dev_register_hook != NULL) {
		DBGLOG(INIT, STATE,
			"ppe_dev_register_hook prDev->name:%s\n", prDev->name);
		ppe_dev_register_hook(prDev);
	} else {
		DBGLOG(INIT, ERROR,
...
static int wlanStop(struct net_device *prDev)
{
	...
	if (ppe_dev_unregister_hook != NULL) {
		DBGLOG(INIT, STATE,
			"ppe_dev_unregister_hook prDev->name:%s\n",
			prDev->name);
		ppe_dev_unregister_hook(prDev);
	} else {
		DBGLOG(INIT, ERROR,
...
void wlanUpdateChannelTable(struct GLUE_INFO *prGlueInfo)
{
	...
	struct ieee80211_supported_band *pr2gBand = NULL, *pr5gBand = NULL;
#if (CFG_SUPPORT_WIFI_6G == 1)
	struct ieee80211_supported_band *pr6gBand = NULL;
#endif
	...
	if (aucChannelList == NULL) {
		DBGLOG(INIT, WARN, "can't alloc aucChannelList\n");
		return;
	}
	kalMemZero(aucChannelList, u4ArrySize);

	/* 1. Get all bands info */
#if CFG_SUPPORT_MULTI_CARD
	pr2gBand = &prGlueInfo->mtk_band_2ghz;
	pr5gBand = &prGlueInfo->mtk_band_5ghz;
#if (CFG_SUPPORT_WIFI_6G == 1)
	pr6gBand = &prGlueInfo->mtk_band_6ghz;
#endif
#else /* !CFG_SUPPORT_MULTI_CARD */
	pr2gBand = &mtk_band_2ghz;
	pr5gBand = &mtk_band_5ghz;
#if (CFG_SUPPORT_WIFI_6G == 1)
	pr6gBand = &mtk_band_6ghz;
#endif
#endif /* CFG_SUPPORT_MULTI_CARD */

	if (!pr2gBand || !pr5gBand
#if (CFG_SUPPORT_WIFI_6G == 1)
		|| !pr6gBand
#endif
	) {
		DBGLOG(INIT, WARN, "band is NULL\n");
		return;
	}

	/* 2. Disable all channels */
	for (i = 0; i < pr2gBand->n_channels; i++) {
		pr2gBand->channels[i].flags |= IEEE80211_CHAN_DISABLED;
		pr2gBand->channels[i].orig_flags |= IEEE80211_CHAN_DISABLED;
	}

	for (i = 0; i < pr5gBand->n_channels; i++) {
		pr5gBand->channels[i].flags |= IEEE80211_CHAN_DISABLED;
		pr5gBand->channels[i].orig_flags |= IEEE80211_CHAN_DISABLED;
	}

#if (CFG_SUPPORT_WIFI_6G == 1)
	for (i = 0; i < pr6gBand->n_channels; i++) {
		pr6gBand->channels[i].flags |= IEEE80211_CHAN_DISABLED;
		pr6gBand->channels[i].orig_flags |= IEEE80211_CHAN_DISABLED;
	}
#endif
	...
	for (u4BandIdx = BAND_2G4; u4BandIdx < BAND_NUM; u4BandIdx++) {
		...
		for (i = 0; i < ucNumOfChannel; i++) {
			...
			case BAND_2G4:
				for (j = 0; j <
					pr2gBand->n_channels; j++) {
					if (pr2gBand->channels[j].hw_value ==
					    aucChannelList[i].ucChannelNum) {
						pr2gBand->channels[j].flags &=
						~IEEE80211_CHAN_DISABLED;
						pr2gBand->channels[j].orig_flags
						&= ~IEEE80211_CHAN_DISABLED;
						break;
					}
				}
			...
			case BAND_5G:
				for (j = 0; j <
					pr5gBand->n_channels; j++) {
					if (pr5gBand->channels[j].hw_value ==
					    aucChannelList[i].ucChannelNum) {
						pr5gBand->channels[j].flags &=
						~IEEE80211_CHAN_DISABLED;
						pr5gBand->channels[j].orig_flags
						&= ~IEEE80211_CHAN_DISABLED;
						pr5gBand->channels[j].dfs_state
			...
			case BAND_6G:
				for (j = 0; j <
					pr6gBand->n_channels; j++) {
					if (pr6gBand->channels[j].hw_value ==
					aucChannelList[i].ucChannelNum) {
						pr6gBand->channels[j].flags &=
						~IEEE80211_CHAN_DISABLED;
						pr6gBand->channels[j].orig_flags
						&= ~IEEE80211_CHAN_DISABLED;
						pr6gBand->channels[j].dfs_state
						= (aucChannelList[i].eDFS != 0)
							? NL80211_DFS_USABLE :
							NL80211_DFS_UNAVAILABLE;
						break;
		...
		enum ENUM_BAND eBand)
{
	...
	struct ieee80211_supported_band *pr5gBand = NULL;
...
#endif /* CFG_SUPPORT_MULTI_CARD */
	if (!pr5gBand) {
		DBGLOG(INIT, WARN, "5G band is NULL\n");
		return;
	}

	/* 2. Get current domain DFS channel list */
	...
	/* 3. Enable specific channel based on domain channel list */
	for (i = 0; i < ucNumOfChannel; i++) {
		for (j = 0; j < pr5gBand->n_channels; j++) {
			if (aucChannelList[i].ucChannelNum !=
				pr5gBand->channels[j].hw_value)
				continue;
...
static struct wireless_dev *wlanCreateWirelessDevice(void)
{
...
#if defined(_HIF_USB)
	prGlueInfo->ePoweredoffWifiFlag =
		WIFI_STATE_POWER_OFF_NON;
#endif /* end _HIF_USB */
	...
	prGlueInfo->bTxBfBackoffExists = FALSE;
#endif
...
#if CFG_SUPPORT_MULTI_CARD
	prWiphy->bands[KAL_BAND_2GHZ] = &prGlueInfo->mtk_band_2ghz;
	/* always assign 5Ghz bands here, if the chip is not support 5Ghz,
	 *  bands[KAL_BAND_5GHZ] will be assign to NULL
	 */
	prWiphy->bands[KAL_BAND_5GHZ] = &prGlueInfo->mtk_band_5ghz;
#if (CFG_SUPPORT_WIFI_6G == 1)
	prWiphy->bands[KAL_BAND_6GHZ] = &prGlueInfo->mtk_band_6ghz;
...
#if (CFG_SUPPORT_4ADDR_STATION == 1)
	prWiphy->flags |= WIPHY_FLAG_4ADDR_STATION;
#endif /* end CFG_SUPPORT_4ADDR_STATION */
...
#endif
	for (u4GlIdx = 0; u4GlIdx < CFG_MAX_WLAN_DEVICES; u4GlIdx++) {
		if (!aprGlueInfo[u4GlIdx]) {
			aprGlueInfo[u4GlIdx] = prGlueInfo;
			break;
		}
	}
	if (u4GlIdx >= CFG_MAX_WLAN_DEVICES) {
 ...
 */
/*----------------------------------------------------------------------------*/
static void wlanDestroyAllWdev(struct net_device *prDev)
{
#if CFG_ENABLE_UNIFY_WIPHY
...
#if CFG_SUPPORT_MULTI_CARD
	if (prDev && prDev->ieee80211_ptr) {
		prWdev = prDev->ieee80211_ptr;
	} else {
		DBGLOG(INIT, ERROR, "fail to find wireless devices\n");
		return;
	}
#else
	prWdev = gprWdev[0];
	...
	prP2pWdev = wlanGetP2pWdev(prGlueInfo);
	pprP2pRoleWdev = wlanGetP2pRoleWdev(prGlueInfo);

#if CFG_CHIP_RESET_SUPPORT
	glResetUninit(prGlueInfo);
#endif
	...
	for (i = 0; i < KAL_P2P_NUM; i++) {
		 ...
		 */
		if (pprP2pRoleWdev[i] == prP2pWdev)
			prP2pWdev = NULL;

#if CFG_ENABLE_UNIFY_WIPHY
		wiphy = pprP2pRoleWdev[i]->wiphy;
#else
		set_wiphy_dev(pprP2pRoleWdev[i]->wiphy, NULL);
		wiphy_unregister(pprP2pRoleWdev[i]->wiphy);
		wiphy_free(pprP2pRoleWdev[i]->wiphy);
#endif
	...
	if (pprWdev[0]) {
#if CFG_ENABLE_UNIFY_WIPHY
		wiphy = wlanGetWiphy(pprWdev[0]);
#else
		/* trunk doesn't do set_wiphy_dev, but trunk-ce1 does. */
		/* set_wiphy_dev(gprWdev->wiphy, NULL); */
		wiphy_unregister(pprWdev[0]->wiphy);
		wiphy_free(pprWdev[0]->wiphy);
#endif
		...
		void *pvDriverData)
{
	...
	struct wireless_dev **pprWdev = NULL;
#if CFG_SUPPORT_PERSIST_NETDEV
	struct net_device **pprNetdev = NULL;
#endif
	struct lock_class_key *prSpinKey = NULL;
	...
	pprWdev = wlanGetWirelessDevice(prGlueInfo);
#if CFG_SUPPORT_PERSIST_NETDEV
	pprNetdev = wlanGetNetDevice(prGlueInfo);
#endif
#if CFG_SUPPORT_MULTI_CARD
	prSpinKey = prGlueInfo->rSpinKey;
#else
	prSpinKey = rSpinKey;
#endif
	...
	if (pprWdev[0]
		...
		kalMemZero(pprWdev[0], sizeof(struct wireless_dev));
	...
	kalMemZero(prAdapter->chip_info, sizeof(struct mt66xx_chip_info));
	kalMemCopy(prAdapter->chip_info, prChipInfo,
		sizeof(struct mt66xx_chip_info));

	/* Allocate each card's bus_info */
	prBusInfo = ((struct mt66xx_hif_driver_data *)
		      pvDriverData)->chip_info->bus_info;

	prAdapter->chip_info->bus_info =
		kalMemAlloc(sizeof(struct BUS_INFO), VIR_MEM_TYPE);

	if (!prAdapter->chip_info->bus_info) {
		DBGLOG(INIT, ERROR,
		       "Allocating memory to bus_info failed\n");
		goto netcreate_err;
	}

	kalMemZero(prAdapter->chip_info->bus_info, sizeof(struct BUS_INFO));
	kalMemCopy(prAdapter->chip_info->bus_info, prBusInfo,
...
#if CFG_SUPPORT_PERSIST_NETDEV
#if CFG_SUPPORT_MULTI_CARD
	pprNetdev = &prGlueInfo->prNetdev[0];
#else
	pprNetdev = &gprNetdev[0];
#endif
	...
	if (!pprNetdev[0]) {
		prDevHandler = alloc_netdev_mq(
			...
			ether_setup,
			CFG_MAX_TXQ_NUM);
		pprNetdev[0] = prDevHandler;
	} else
		prDevHandler = pprNetdev[0];
#else
	prDevHandler = alloc_netdev_mq(
		sizeof(struct NETDEV_PRIVATE_GLUE_INFO),
		prInfName,
	...
	for (i = 0; i < SPIN_LOCK_NUM; i++) {
		...
		lockdep_set_class(&prGlueInfo->rSpinLock[i], &prSpinKey[i]);
	}
	...
	init_waitqueue_head(&prGlueInfo->waitq_cmd);
...
static void wlanNetDestroy(struct wireless_dev *prWdev)
{
...
#ifdef CONFIG_WIRELESS_EXT
	{
		...
		for (u4Idx = 0; u4Idx < KAL_AIS_NUM; u4Idx++) {
			if (pprWdev[u4Idx] && pprWdev[u4Idx]->netdev) {
				rtnl_lock();
				pprWdev[u4Idx]->netdev->wireless_handlers =
					NULL;
				rtnl_unlock();
			}
		}
			...
			enum ENUM_SUSPEND_MODE_SOURCE eSource)
{
	...
	if (prGlueInfo->fgIsStaInSuspendMode == fgEnable
		&& eSource != SUSPEND_MODE_SOURCE_WOW)
		return;
			 ...
			 struct PARAM_CUSTOM_P2P_SET_STRUCT p2pmode)
{
	...
	uint32_t u4BufLen = 0;
#if CFG_SUPPORT_MULTI_CARD
	uint32_t *prP2pDevIdx = prGlueInfo->u4P2pDevIdx;
#else
	uint32_t *prP2pDevIdx = g_u4DevIdx;
#endif
	...
	if ((!rSetP2P.u4Enable) && (kalIsResetting(prGlueInfo) == FALSE))
		p2pNetUnregister(prGlueInfo, FALSE);
	...
	if ((rSetP2P.u4Enable)
	    && (prGlueInfo->prAdapter->fgIsP2PRegistered)
	    && (kalIsResetting(prGlueInfo) == FALSE))
...
int set_nan_handler(struct net_device *netdev, uint32_t ucEnable)
{
	...
	if ((ucEnable) && (nanIsRegistered(prGlueInfo->prAdapter)))
		return 0;
...
int set_csi_handler(struct net_device *netdev, uint8_t ucEnable)
{
	struct GLUE_INFO *prGlueInfo =
		*((struct GLUE_INFO **)netdev_priv(netdev));
	uint32_t rWlanStatus = WLAN_STATUS_SUCCESS;
	...
	if (kalIsResetting(prGlueInfo)) {
		DBGLOG(CSI, ERROR, "chip is Resetting\n");
		return -1;
	}

	if ((ucEnable) && (prGlueInfo->prAdapter->fgIsCsiRegistered))
		return 0;
	...
	if ((!ucEnable))
		if ((csiNetUnregister(prGlueInfo)) == FALSE)
			DBGLOG(INIT, ERROR, "Failed to unregister csidev\n");

	rWlanStatus = kalIoctl(prGlueInfo, wlanoidSetCsiMode, (void *)&ucEnable,
			       sizeof(uint32_t), FALSE, FALSE, TRUE, &u4BufLen);

	DBGLOG(INIT, INFO, "set_csi_handler ret = 0x%08lx\n",
	       (uint32_t)rWlanStatus);

	/* Need to check fgIsCsiRegistered, in case of whole chip reset.
	 * in this case, kalIOCTL return success always,
	 * and prGlueInfo->prCSIDevInfo may be NULL
	 */
	if ((ucEnable) && (prGlueInfo->prAdapter->fgIsCsiRegistered)) {
		if ((csiNetRegister(prGlueInfo)) == FALSE) {
			DBGLOG(INIT, ERROR, "Failed to register csidev\n");
			return -1;
		}
	}
...
uint8_t set_fmcw_handler(struct net_device *netdev, uint8_t ucEnable)
{
	struct GLUE_INFO *prGlueInfo =
		*((struct GLUE_INFO **)netdev_priv(netdev));
	struct ADAPTER *prAdapter = NULL;
	...
	if (kalIsResetting(prGlueInfo)) {
		DBGLOG(RDR, ERROR, "chip is Resetting\n");
		return FALSE;
	}
	...
	if (ucEnable) {
		if (prAdapter->fgIsFmcwRegistered)
			return TRUE;
	} else {
		if (!prAdapter->fgIsFmcwRegistered)
			return TRUE;

		if ((fmcwNetUnregister(prGlueInfo)) == FALSE)
			DBGLOG(RDR, ERROR, "Failed to unregister fmcwdev\n");
	}

	rWlanStatus = kalIoctl(prGlueInfo, wlanoidSetFmcwMode,
					(void *)&ucEnable, sizeof(uint32_t),
					FALSE, FALSE, TRUE, &u4BufLen);


	/* Need to check fgIsFmcwRegistered, in case of whole chip reset.
	 * in this case, kalIOCTL return success always,
	 * and prGlueInfo->prFmcwDevInfo may be NULL
	 */
	if ((ucEnable) && (prAdapter->fgIsFmcwRegistered)) {
		if ((fmcwNetRegister(prGlueInfo)) == FALSE) {
			DBGLOG(RDR, ERROR, "Failed to register fmcwdev\n");
			return FALSE;
		}
	}
...
const uint8_t *wlanGetWlanLog(void)
{
	...
	for (u4Idx = 0; u4Idx < CFG_MAX_WLAN_DEVICES; u4Idx++) {
		...
		if (!prGlueInfo ||
			(prGlueInfo->u4TxThreadPid != u4Pid &&
...
#define WIFI_DEVCFG_BUF_SIZE 256
	...
	pucConfigBuf = (uint8_t *) kalMemAlloc(
				   WIFI_DEVCFG_BUF_SIZE, VIR_MEM_TYPE);
	u4ConfigReadLen = 0;

	if (!pucConfigBuf)
		return WLAN_STATUS_FAILURE;

	kalMemZero(pucConfigBuf, WIFI_DEVCFG_BUF_SIZE);
	if (kalRequestFirmware(WIFI_DEVCFG_FN, pucConfigBuf,
		   WIFI_DEVCFG_BUF_SIZE, &u4ConfigReadLen,
		   prAdapter->prGlueInfo->prDev) == 0) {
		/* Nothing */
	} else if (kalReadToFile("/data/misc/wifi/" WIFI_DEVCFG_FN,
		   pucConfigBuf, WIFI_DEVCFG_BUF_SIZE,
		   &u4ConfigReadLen) == 0) {
		/* Nothing */
	} else if (kalReadToFile("/storage/sdcard0/" WIFI_DEVCFG_FN,
		   pucConfigBuf, WIFI_DEVCFG_BUF_SIZE,
		   &u4ConfigReadLen) == 0) {
		/* Nothing */
	}
	...
	kalMemFree(pucConfigBuf, VIR_MEM_TYPE, WIFI_DEVCFG_BUF_SIZE);
...
#define WIFI_CFG_FILE_LEN 64
	...
	uint8_t aucPath[3][WIFI_CFG_FILE_LEN];
	...
	for (u4Idx = READ_DEV_CFG; u4Idx < READ_CFG_TOTAL_NUM; u4Idx++) {
		...
		if (u4Idx == READ_DEV_CFG)
			kalSnprintf(aucFileName, WIFI_CFG_FILE_LEN, "%s/%s",
				prGlueInfo->aucDevCfgPath, WIFI_CFG_FN);
		else
#endif /* CFG_SUPPORT_MULTI_CARD */
		...
		kalMemZero(aucPath, sizeof(aucPath));
		kalSnprintf(aucPath[0], WIFI_CFG_FILE_LEN,
			"%s", aucFileName);
		kalSnprintf(aucPath[1], WIFI_CFG_FILE_LEN,
			"/data/misc/wifi/%s", aucFileName);
		kalSnprintf(aucPath[2], WIFI_CFG_FILE_LEN,
			"/storage/sdcard0/%s", aucFileName);
		...
		if (kalRequestFirmware("wifi_sigma.cfg", pucConfigBuf,
			   ...
			   prAdapter->prGlueInfo->prDev) == 0) {
			/* ToDo:: Nothing */
		} else if (kalRequestFirmware(aucPath[0], pucConfigBuf,
			   WLAN_CFG_FILE_BUF_SIZE, &u4ConfigReadLen,
			   prAdapter->prGlueInfo->prDev) == 0) {
			/* ToDo:: Nothing */
		} else if (kalReadToFile(aucPath[1],
			   pucConfigBuf, WLAN_CFG_FILE_BUF_SIZE,
			   &u4ConfigReadLen) == 0) {
			/* ToDo:: Nothing */
		} else if (kalReadToFile(aucPath[2],
			   pucConfigBuf, WLAN_CFG_FILE_BUF_SIZE,
			   &u4ConfigReadLen) == 0) {
			/* ToDo:: Nothing */
		}
	...
	if (prAdapter->rWifiVar.ucEfuseBufferModeCal
		...
		if (aucDevEeprom[0] &&
			kalRequestFirmware(aucDevEeprom, pucConfigBuf,
				MAX_EEPROM_BUFFER_SIZE, &u4ContentLen,
				prGlueInfo->prDev) == 0) {
			DBGLOG(INIT, INFO, "request dev file done\n");
		} else
#endif
		{
			if (kalRequestFirmware(aucEeprom, pucConfigBuf,
...
uint32_t wlanServiceInit(struct GLUE_INFO *prGlueInfo)
{
	...
	prServiceTest->test_winfo->net_dev = prNetDev;
...
static void consys_log_event_notification(struct net_device *prDev,
				int cmd, int value)
{
	...
	DBGLOG(INIT, INFO, "prDev=%p, cmd=%d, value=%d\n",
		prDev, cmd, value);
	...
	struct mt66xx_chip_info **pprChipInfo)
{
	...
	*pprRegInfo = &prGlueInfo->rRegInfo;
	...
	for (u4Idx = 0; u4Idx < MAX_BSSID_NUM; u4Idx++)
		prAdapter->e6GPwrMode[u4Idx] = PWR_MODE_6G_LPI;
	...
	const u_int8_t bAtResetFlow)
{
...
#if CFG_SUPPORT_CFG80211_QUEUE
#if CFG_SUPPORT_MULTI_CARD
	struct delayed_work *prCfg80211Workq = &prGlueInfo->cfg80211_workq;
#else
	struct delayed_work *prCfg80211Workq = &cfg80211_workq;
#endif
#endif

	DBGLOG(INIT, ERROR, "start.\n");
	...
	if (!bAtResetFlow) {
		...
		INIT_DELAYED_WORK(&prGlueInfo->rRxPktDeAggWork,
				halDeAggRxPktWorker);
#if CFG_SUPPORT_CFG80211_QUEUE
		INIT_DELAYED_WORK(prCfg80211Workq,
...
#if CFG_ENABLE_EARLY_SUSPEND
#if CFG_SUPPORT_MULTI_CARD
	struct early_suspend *prEarlySuspendDesc =
					&prGlueInfo->wlan_early_suspend_desc;
#else
	struct early_suspend *prEarlySuspendDesc =
					&wlan_early_suspend_desc;
#endif
#endif
	DBGLOG(INIT, TRACE, "start.\n");
	/* 4 <4> Register early suspend callback */
#if CFG_ENABLE_EARLY_SUSPEND
	glRegisterEarlySuspend(prEarlySuspendDesc,
...
#if CFG_SUPPORT_CSI_VNI
static
void wlanOnCsiRegistration(struct wireless_dev *prWdev)
{
	DBGLOG(INIT, TRACE, "start.\n");

	if (!prWdev) {
		DBGLOG(CSI, ERROR, "prWdev error\n");
		return;
	}
	if (!prWdev->netdev) {
		DBGLOG(CSI, ERROR, "prWdev->netdev error\n");
		return;
	}

	if (set_csi_handler(prWdev->netdev, TRUE) == 0)
		DBGLOG(INIT, INFO, "csi device registered\n");
	else
		DBGLOG(INIT, ERROR, "Failed to register csi device\n");
}
#endif /* CFG_SUPPORT_CSI_VNI */

#if (CFG_SUPPORT_FMCW_VNI == 1)
static
void wlanOnFmcwRegistration(struct wireless_dev *prWdev)
{
	if (!prWdev) {
		DBGLOG(RDR, ERROR, "prWdev error\n");
		return;
	}
	if (!prWdev->netdev) {
		DBGLOG(RDR, ERROR, "prWdev->netdev error\n");
		return;
	}

	if (set_fmcw_handler(prWdev->netdev, TRUE) == TRUE)
		DBGLOG(RDR, INFO, "fmcw device registered\n");
	else
		DBGLOG(RDR, ERROR, "Failed to register fmcw device\n");
}
#endif /* (CFG_SUPPORT_FMCW_VNI == 1) */
	...
	const u_int8_t bAtResetFlow)
{
	 ...
	 */
	wlanFeatureToFw(prGlueInfo->prAdapter, NULL, false);
#endif
#endif
	...
	if (!bAtResetFlow) {
		...
		kalAntSwDivInit(prGlueInfo->prAdapter);
#endif
...
#if CFG_MTK_SUPPLICANT_ROAMING_ENHANCE
	if (prGlueInfo->u4RoamingEnhanceEnable &&
		(wlanNotifyRoamingEnhanceToFw(prGlueInfo) !=
		WLAN_STATUS_SUCCESS)) {
		DBGLOG(REQ, WARN,
			"failed to enable roaming enhance feature\n");
	}
#endif
	...
	if (u4LogOnOffCache != -1) /* -1: connsysD does not set */
		consys_log_event_notification(prNetDev,
			(int)FW_LOG_CMD_ON_OFF,	u4LogOnOffCache);
#endif
...
*/
		...
		*/
		...
		/* It is configured in mt66xx_chip_info.fw_dl_ops */
...
#if (CFG_SUPPORT_FW_LOG_TIMESYNC == 1)
...
#endif
#if CFG_MTK_ANDROID_WMT
	glSetSimplifyResetFlowFlag(prGlueInfo, TRUE);
#endif /* CFG_MTK_ANDROID_WMT */
#if defined(_HIF_USB)
	prBusInfo = prGlueInfo->prAdapter->chip_info->bus_info;
...
#if CFG_SUPPORT_NAN
	kalNanPrivWorkUninit(prGlueInfo);
	nanDisableAtReset(prGlueInfo);
	mtk_cfg80211_vendor_event_nan_chip_reset(prAdapter, TRUE);
...
int32_t wlanOnAtReset(struct net_device *prDev)
{
	...
	if (rStatus == WLAN_STATUS_SUCCESS) {
...
#if defined(_HIF_PCIE) && (CFG_PCIE_FW_OWN_BYPASS == 1)
		/* PCIe FW Own bypass on Probe & L0.5 after FW downloaded */
		prAdapter->fgIsBypassFwOwn = TRUE;
#endif
		...
		kalP2pNotifyApGoStopped(prAdapter);

#if defined(_HIF_PCIE) && (CFG_PCIE_FW_OWN_BYPASS == 1)
		prAdapter->fgIsBypassFwOwn = FALSE;
#endif
	} else {
...
static u_int8_t wlanTrackingProbeRemoveCAS(u_int8_t fgIsProbe, uint32_t bus_id)
{
	u_int8_t fgRet = TRUE;
	unsigned long flags;
	struct probe_remove_tracking_item *cur, *next;
	struct probe_remove_tracking_item *item;

	struct list_head *pList =
		(fgIsProbe == TRUE) ? &g_probe_list : &g_remove_list;

	item = kalMemAlloc(sizeof(struct probe_remove_tracking_item),
			   VIR_MEM_TYPE);
	if (!item) {
		DBGLOG(INIT, ERROR, "kalMemAlloc for probe tracking failed!\n");
		return FALSE;
	}

	spin_lock_irqsave(&g_probe_remove_lock, flags);
	if (!list_empty(pList)) {
		list_for_each_entry_safe(cur, next, pList, node) {
			if (cur->bus_id == bus_id) {
				fgRet = FALSE;
				goto CAS_RETURN;
			}
		}
	...
	item->bus_id = bus_id;
	list_add_tail(&item->node, pList);

CAS_RETURN:
	spin_unlock_irqrestore(&g_probe_remove_lock, flags);

	if (fgRet == FALSE)
		kalMemFree(item, VIR_MEM_TYPE,
			   sizeof(struct probe_remove_tracking_item));

	return fgRet;
}

static void wlanTrackingProbeRemoveClear(u_int8_t fgIsProbe, uint32_t bus_id)
{
	u_int8_t fgRet = FALSE;
	unsigned long flags;
	struct probe_remove_tracking_item *cur, *next;
	struct probe_remove_tracking_item *item = NULL;
	...
	if (fgRet == FALSE)
		DBGLOG(INIT, ERROR, "untracked bus_id (%d)!\n", bus_id);
}

u_int8_t wlanIsProbingOrRemoving(u_int8_t fgIsProbe, uint32_t bus_id)
{
	u_int8_t fgRet = FALSE;
	unsigned long flags;
	struct probe_remove_tracking_item *cur, *next;
...
static int32_t wlanProbe(void *pvData, void *pvDriverData)
{
	...
	struct net_device **pprNetDev;
	struct ieee80211_supported_band *prGHzBand;

#if CFG_SUPPORT_MULTI_CARD
#if defined(_HIF_USB) || defined(_HIF_PCIE)
	bus_id = glGetBusId(pvData);
#endif
#endif
#if CFG_CONN_DYNAMIC_POWER_CTRL
	storeBusId(bus_id);
	...
	if (wlanTrackingProbeRemoveCAS(TRUE, bus_id) == FALSE) {
	...
	glSendResetEvent(bus_id,
			 RESET_MODULE_TYPE_WIFI, RFSM_EVENT_START_PROBE);
#endif  /* CFG_CHIP_RESET_KO_SUPPORT */
	if (kalGetSimplifyResetFlowFlag(NULL)) {
#if CFG_SUPPORT_MULTI_CARD
		glGetDev(pvData, &prDev);
		i = wlanSearchDevIdx(prDev);

		if (i < CFG_MAX_WLAN_DEVICES)
			i4Status = wlanOnAtReset(arWlanDevInfo[i].prDev);
#else
	...
	do {
		...
		if (prWdev == NULL) {
			i4Status = -ENOMEM;
			DBGLOG(INIT, ERROR, "wlanCreateWirelessDevice ERROR\n");
			break;
		}

		WIPHY_PRIV(wlanGetWiphy(prWdev), prGlueInfo);
		if (prWdev && (glP2pCreateWirelessDevice(prGlueInfo) == TRUE)) {
			/* P2PDev and P2PRole[0] share the same Wdev */
			prGlueInfo->prP2pWdev = prGlueInfo->prP2pRoleWdev[0];
		}
		...
		*pprNetDev = prGlueInfo->prDevHandler;
		...
		if (i4Status < 0) {
			DBGLOG(INIT, ERROR, "wlanProbe: create sysfs failed\n");
#if CFG_SUPPORT_MULTI_CARD
			sysUninitSysFs(prGlueInfo);
#endif
#if WLAN_INCLUDE_PROC
			procRemoveProcfs(prGlueInfo);
#endif
...
#if CFG_SUPPORT_MULTI_CARD
		prGHzBand = &prGlueInfo->mtk_band_5ghz;
#else
		prGHzBand = &mtk_band_5ghz;
#endif
		pprP2pRoleWdev = wlanGetP2pRoleWdev(prAdapter->prGlueInfo);
...
#if (CFG_SUPPORT_WIFI_6G == 1)
#if CFG_SUPPORT_MULTI_CARD
		prGHzBand = &prGlueInfo->mtk_band_6ghz;
#else
		...
		if (prAdapter->fgIsHwSupport6G)
			prWdev->wiphy->bands[KAL_BAND_6GHZ] =
				prGHzBand;
		else
...
#if CFG_SUPPORT_CSI_VNI
		wlanOnCsiRegistration(prWdev);
#endif /* CFG_SUPPORT_CSI_VNI */
#if (CFG_SUPPORT_FMCW_VNI == 1)
		wlanOnFmcwRegistration(prWdev);
#endif /* (CFG_SUPPORT_FMCW_VNI == 1) */
...
#endif
	wlanTrackingProbeRemoveClear(TRUE, bus_id);
	return i4Status;
}				/* end of wlanProbe() */
...
#endif
{
...
#endif /* CFG_SUPPORT_MULTI_CARD */
#if CFG_ENABLE_EARLY_SUSPEND
	struct early_suspend *prEarlySuspend = NULL;
#endif
	int32_t *pu4HaltFlag = NULL;
	struct semaphore *prHaltSem = NULL;
	uint32_t bus_id = 0;

	DBGLOG(INIT, STATE, "Remove wlan start!\n");

#if CFG_SUPPORT_MULTI_CARD
	glGetDev(pvData, &prDevice);
#if defined(_HIF_USB) || defined(_HIF_PCIE)
	bus_id = glGetBusId(pvData);
#endif
#endif

	if (wlanTrackingProbeRemoveCAS(FALSE, bus_id) == FALSE) {
		DBGLOG(INIT, ERROR, "%s in process\n", __func__);
		return;
	}
#if CFG_SUPPORT_NVRAM
	...
	if (u4DevIdx < CFG_MAX_WLAN_DEVICES) {
		prGlueInfo = *((struct GLUE_INFO **)
			netdev_priv(arWlanDevInfo[u4DevIdx].prDev));
		prDev = prGlueInfo->prDevHandler;
		prWlandevInfo = &arWlanDevInfo[u4DevIdx];
	}
#endif

#if CFG_WIFI_SUPPORT_WIFI_ON_STATISTICS
	/*need to update wifi on time statistics*/
	updateWifiOnTimeStatistics(prDev);
#endif
	...
	{
		uint32_t count = 0;
		uint8_t fgIsInSysReset = TRUE;

		while (true) {
			spin_lock_bh(&prAdapter->rWfsysResetLock);
			if (prAdapter->eWfsysResetState ==
				WFSYS_RESET_STATE_IDLE)
				fgIsInSysReset = FALSE;
			spin_unlock_bh(&prAdapter->rWfsysResetLock);

			if (fgIsInSysReset == FALSE)
				break;

			if (count > RST_D0_L0P5_MAX_TIME) {
				DBGLOG(HAL, ERROR,
					"system abnormal return directly\n");
				goto WLAN_REMOVE_RETURN;
			}
...
#endif
#if CFG_ENABLE_EARLY_SUSPEND
	prEarlySuspend  = &prGlueInfo->wlan_early_suspend_desc;
#endif
#else /* !CFG_SUPPORT_MULTI_CARD */
	prWorkq      = &workq;
	prSchedWorkq = &sched_workq;
...
#endif
#if CFG_ENABLE_EARLY_SUSPEND
	prEarlySuspend = &wlan_early_suspend_desc;
#endif
#endif /* CFG_SUPPORT_MULTI_CARD */
	...
	flush_delayed_work(prSchedWorkq);

#if CFG_REDIRECT_OID_SUPPORT
	flush_delayed_work(prOidWorkq);
#endif

#if CFG_SUPPORT_CFG80211_QUEUE
	flush_delayed_work(prCfg80211Workq);
#endif
...
#if CFG_SUPPORT_CSI_VNI
	if (prGlueInfo->prAdapter->fgIsCsiRegistered) {
		DBGLOG(INIT, INFO, "csiNetUnregister...\n");
		if ((csiNetUnregister(prGlueInfo)) == FALSE)
			DBGLOG(INIT, ERROR, "Failed to unregister csidev\n");
		DBGLOG(INIT, INFO, "csiRemove...\n");
		/* csiRemove must before wlanAdapterStop */
		csiRemove(prGlueInfo);
	}
#endif /* CFG_SUPPORT_CSI_VNI */

#if (CFG_SUPPORT_FMCW_VNI == 1)
	if (prGlueInfo->prAdapter->fgIsFmcwRegistered) {
		DBGLOG(INIT, INFO, "fmcwNetUnregister...\n");
		if ((fmcwNetUnregister(prGlueInfo)) == FALSE)
			DBGLOG(INIT, ERROR, "Failed to unregister fmcwdev\n");
		DBGLOG(INIT, INFO, "fmcwRemove...\n");
		/* fmcwRemove must before wlanAdapterStop */
		fmcwRemove(prGlueInfo);
	}
#endif /* (CFG_SUPPORT_FMCW_VNI == 1) */
...
#if WLAN_INCLUDE_SYS && CFG_SUPPORT_MULTI_CARD
	sysUninitSysFs(prGlueInfo);
#endif
...
#if CFG_ENABLE_EARLY_SUSPEND
	glUnregisterEarlySuspend(prEarlySuspend);
#endif
...
WLAN_REMOVE_RETURN:
	...
	wlanDestroyAllWdev(prDev);
#endif
	wlanTrackingProbeRemoveClear(FALSE, bus_id);
	DBGLOG(INIT, STATE, "wlanRemove end\n");
	return;
}				/* end of wlanRemove() */
...
static void initBusid(void)
{
	...
	for (; idx < CFG_MAX_WLAN_DEVICES; idx++)
		g_dynamicPowerCtrlInfo[idx].busId = INVALID_BUS_ID;
}
...
static void storeBusId(uint32_t busId)
{
	...
	DBGLOG(INIT, INFO, "Enter with busId %d\n", busId);
	for (; idx < CFG_MAX_WLAN_DEVICES; idx++) {
		if (g_dynamicPowerCtrlInfo[idx].busId == busId)
			return;

		if ((g_dynamicPowerCtrlInfo[idx].busId == INVALID_BUS_ID) &&
			(store_idx == -1))
			store_idx = idx;
	}

	if (store_idx >= 0)
		g_dynamicPowerCtrlInfo[store_idx].busId = busId;
}

static int wlanFuncOnImpl(void)
{
	int ret = 0;
...
static void wlanFuncOffImpl(void)
{
	glBusFuncOff();
}
...
int wlanFuncOn(void)
{
	...
	for (u4DevIdx = 0; u4DevIdx < CFG_MAX_WLAN_DEVICES; u4DevIdx++) {
		if (g_dynamicPowerCtrlInfo[u4DevIdx].busId == INVALID_BUS_ID)
			continue;

		ret = resetko_power_request(
			g_dynamicPowerCtrlInfo[u4DevIdx].busId,
			RESET_MODULE_TYPE_WIFI,
			POWER_ON, POWER_ON_TIMEOUT);
		if (ret != RESET_RETURN_STATUS_SUCCESS)
			goto connsys_pwr_off;
	}
#else
	ret = resetko_power_request(RESET_MODULE_TYPE_WIFI,
		POWER_ON, POWER_ON_TIMEOUT);
#endif
	if (ret != RESET_RETURN_STATUS_SUCCESS)
		goto exit;
...
connsys_pwr_off:
	...
	while (TRUE) {
		...
		resetko_power_request(g_dynamicPowerCtrlInfo[u4DevIdx].busId,
			RESET_MODULE_TYPE_WIFI,
			POWER_OFF, POWER_OFF_TIMEOUT);
		if (u4DevIdx == 0)
			break;
		u4DevIdx--;
	}
#else
	resetko_power_request(RESET_MODULE_TYPE_WIFI,
		POWER_OFF, POWER_OFF_TIMEOUT);
#endif
...
/* prPlatDev is already created by initWlan()->glRegisterBus()::axi.c */
...
#if CFG_SUPPORT_RX_PAGE_POOL_USE_CMA
	ret = wifi_page_pool_init();
	if (ret) {
		DBGLOG(INIT, INFO,
		       "wifi_page_pool_init failed (%d)\n", ret);
		return -EINVAL;
	}
#endif
	...
	spin_lock_init(&g_probe_remove_lock);
	INIT_LIST_HEAD(&g_probe_list);
	INIT_LIST_HEAD(&g_remove_list);
...
#if CFG_POWER_OFF_CTRL_SUPPORT && (CFG_SUPPORT_MULTI_CARD == 0)
	wlanRegisterRebootNotifier(prGlueInfo);
#endif
#if CFG_AP_80211KVR_INTERFACE
...
static void exitWlan(void)
{
	...
	for (u4DevIdx = 0; u4DevIdx < CFG_MAX_WLAN_DEVICES; u4DevIdx++) {
		 ...
		 */
		prGlueInfo->ePoweredoffWifiFlag =
			WIFI_STATE_POWER_OFF_START;
		if ((glUsbGetState(&prGlueInfo->rHifInfo)
			!= USB_STATE_LINK_DOWN)
		&& prGlueInfo->u4ReadyFlag
		&& prGlueInfo->prAdapter) {
			wlanPowerOffWifi(prGlueInfo->prAdapter);
		}
		prGlueInfo->ePoweredoffWifiFlag =
			WIFI_STATE_POWER_OFF_DONE;
#endif
...
#if CFG_SUPPORT_PERSIST_NETDEV
	for (u4DevIdx = 0; u4DevIdx < CFG_MAX_WLAN_DEVICES; u4DevIdx++) {
		if (!arWlanDevInfo[u4DevIdx].prDev)
			continue;

		prNetdev = arWlanDevInfo[u4DevIdx].prDev;
		prGlueInfo =
		...
		for (u4Idx = 0; u4Idx < KAL_AIS_NUM; u4Idx++) {
			if (pprNetdev[u4Idx] && pprWdev[u4Idx]->netdev &&
				pprWdev[u4Idx]->netdev->
					reg_state == NETREG_REGISTERED) {
				wlanClearDevIdx(pprWdev[u4Idx]->netdev);
				DBGLOG(INIT, INFO,
						"Unregister %s netdev start.\n",
						pprWdev[u4Idx]->netdev->name);
				unregister_netdev(pprWdev[u4Idx]->netdev);
				DBGLOG(INIT, INFO,
						"Unregister %s netdev end.\n",
						pprWdev[u4Idx]->netdev->name);
				pprWdev[u4Idx]->netdev = NULL;
				pprNetdev[u4Idx] = NULL;
			}
		}

		prGlueInfo->fgIsRegistered = FALSE;
		DBGLOG(INIT, INFO, "Free wlan device..\n");
		wlanFreeNetDev(prNetdev);
#endif /* CFG_SUPPORT_PERSIST_NETDEV*/

#if CFG_SUPPORT_PERSIST_NETDEV || (CFG_SUPPORT_MULTI_CARD == 0)
		/* For single wiphy case,
		 * it's hardly to free wdev & wiphy in 2 func.
		 * So that, use wlanDestroyAllWdev to replace
		 * wlanDestroyWirelessDevice and glP2pDestroyWirelessDevice.
		 */
		wlanDestroyAllWdev(prNetdev);
...
#if WLAN_INCLUDE_SYS && (CFG_SUPPORT_MULTI_CARD == 0)
	sysUninitSysFs(prGlueInfo);
#endif
#if defined(UT_TEST_MODE) && (CFG_BUILD_X86_PLATFORM == 1)
	...
	DBGLOG(INIT, STATE, "exit Wlan end\n");
		...
		unsigned long event, void *unused)
{
	...
	if (event == SYS_RESTART) {
		...
		if (rStatus != WLAN_STATUS_SUCCESS)
			DBGLOG(REQ, ERROR, "p2pFuncPreReboot\n");
		 ...
		 */
		wlanDestroyAllWdev(prGlueInfo->prDevHandler);
...
{
#if CFG_SUPPORT_MULTI_CARD
	struct notifier_block *prNotifier = &prGlueInfo->wf_pdwnc_notifier;
#else
	struct notifier_block *prNotifier = &wf_pdwnc_notifier;
#endif

	prNotifier->notifier_call = wf_pdwnc_notify;
	prNotifier->next = NULL;
	prNotifier->priority = 0;

	DBGLOG(HAL, STATE, "wlanRegisterRebootNotifier()\n");
	register_reboot_notifier(prNotifier);
...
void wlanUnregisterRebootNotifier(struct GLUE_INFO *prGlueInfo)
{
	...
	unregister_reboot_notifier(prNotifier);

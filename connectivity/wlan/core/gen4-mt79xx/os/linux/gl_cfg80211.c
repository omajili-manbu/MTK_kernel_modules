...
void dumpBmcWlanEntry(struct BSS_INFO *prBssInfo)
{
	uint32_t i = 0;
	...
	for (; i < MAX_PGTK_KEY_NUM; i++) {
		DBGLOG(RSN, INFO, "ucBMCWlanIndexSUsed[%u]=%u\n",
			i, prBssInfo->ucBMCWlanIndexSUsed[i]);
		DBGLOG(RSN, INFO, "u4DualGTKKeyIndex[%u]=%u\n",
			i, prBssInfo->u4DualGTKKeyIndex[i]);
	}
}
#endif
		     ...
		     struct key_params *params)
{
	...
	prBssInfo = GET_BSS_INFO_BY_INDEX(prGlueInfo->prAdapter,
			ucBssIndex);
	if (!prBssInfo) {
		DBGLOG(RSN, INFO, "invalid BssInfo\n");
		return -EINVAL;
	}
#endif
	...
	dumpBmcWlanEntry(prBssInfo);

	if (pairwise == FALSE
		&& prBssInfo->ucBMCWlanIndexSUsed[1] == FALSE
		&& prBssInfo->ucBMCWlanIndexSUsed[2] == FALSE
		&& prBssInfo->ucBMCWlanIndexSUsed[3] == FALSE) {
		/* if this is the first time add gtk */
		if ((rKey.u4KeyIndex == 1) ||
			(rKey.u4KeyIndex == 2) ||
			(rKey.u4KeyIndex == 3)) {
			kalMemCopy(&rDupKey, &rKey, sizeof(struct PARAM_KEY));

			for (u4TmpKeyIndex = 1;
				u4TmpKeyIndex < MAX_PGTK_KEY_NUM;
				u4TmpKeyIndex++) {
				if (rKey.u4KeyIndex == u4TmpKeyIndex)
					continue;

				rDupKey.u4KeyIndex = u4TmpKeyIndex;
				prBssInfo->u4DualGTKKeyIndex[u4TmpKeyIndex] =
					u4TmpKeyIndex;
				if (kalIoctl(prGlueInfo, wlanoidSetAddKey,
	...
	} else if ((pairwise == FALSE)
		&& (rKey.u4KeyIndex < MAX_PGTK_KEY_NUM)
		&& (rKey.u4KeyIndex ==
		prBssInfo->u4DualGTKKeyIndex[rKey.u4KeyIndex])) {
		prBssInfo->u4DualGTKKeyIndex[rKey.u4KeyIndex] = 0xFF;
	}
#endif
			 ...
			 const u8 *mac_addr)
{
	...
	uint8_t ucBssIndex = 0;
#if CFG_SUPPORT_DUAL_WTBL_GTK_REKEY_OFFLOAD
	struct PARAM_REMOVE_KEY rDupRemoveKey;
	struct BSS_INFO *prBssInfo;
	uint32_t usedCount = 0;
	uint32_t dupCount = 0;
	uint32_t tmpKeyIndex  = 0;
	uint32_t lastNonDupKeyIndex = 0;
#endif
	...
	/*calculate usedCount and dupCount*/
	for (tmpKeyIndex = 1; tmpKeyIndex < MAX_PGTK_KEY_NUM; tmpKeyIndex++) {
		if (prBssInfo->ucBMCWlanIndexSUsed[tmpKeyIndex])
			usedCount++;
		else
			continue;

		if (prBssInfo->u4DualGTKKeyIndex[tmpKeyIndex] != 0xFF)
			dupCount++;
		else
			lastNonDupKeyIndex = tmpKeyIndex;

	}

	if ((pairwise == FALSE) &&
		((rRemoveKey.u4KeyIndex == 1)
		|| (rRemoveKey.u4KeyIndex == 2)
		|| (rRemoveKey.u4KeyIndex == 3))
		&& (usedCount >= dupCount)
		&& (usedCount - dupCount == 1)
		&& (rRemoveKey.u4KeyIndex == lastNonDupKeyIndex)) {
		for (tmpKeyIndex = 1;
			tmpKeyIndex < MAX_PGTK_KEY_NUM;
			tmpKeyIndex++) {
			if (prBssInfo->u4DualGTKKeyIndex[tmpKeyIndex]
				== 0xFF) {
				continue;
			}
			kalMemCopy(&rDupRemoveKey, &rRemoveKey,
				sizeof(struct PARAM_REMOVE_KEY));
			rDupRemoveKey.u4KeyIndex = tmpKeyIndex;
			prBssInfo->u4DualGTKKeyIndex[tmpKeyIndex] = 0xFF;
			if (prBssInfo->ucBMCWlanIndexSUsed
				[rDupRemoveKey.u4KeyIndex]) {
				DBGLOG(RSN, INFO, "remove dup key id %d\n",
					rDupRemoveKey.u4KeyIndex);
				if (kalIoctl(prGlueInfo, wlanoidSetRemoveKey,
					&rDupRemoveKey, rDupRemoveKey.u4Length,
					FALSE, FALSE, TRUE,
					&u4BufLen) != WLAN_STATUS_SUCCESS)
					DBGLOG(RSN, WARN,
						"remove dup key failed\n");
			}
		}
	} else if (pairwise == FALSE
		&& key_index < MAX_PGTK_KEY_NUM) {
		prBssInfo->u4DualGTKKeyIndex[rRemoveKey.u4KeyIndex] = 0xFF;
	}
#endif
...
#if CFG_SUPPORT_NAN
	if (nanCheckScanAbort(prAdapter) == TRUE) {
		DBGLOG(REQ, ERROR, "skip scan, NAN NDP ongoing\n");
		return -EBUSY;
	}
	...
	struct net_device *ndev, struct cfg80211_auth_request *req)
{
	...
	if (req->bss->channel->center_freq) {
		...
		prConnSettings->eBand = eBand;
		DBGLOG(RSN, INFO,
			"set prConnSettings->ucChannelNum:%d B[%d]\n",
			prConnSettings->ucChannelNum,
			prConnSettings->eBand);
	} else {
		prConnSettings->ucChannelNum = 0;
		prConnSettings->eBand = BAND_NULL;
		DBGLOG(RSN, INFO,
	...
	struct net_device *ndev, struct cfg80211_deauth_request *req)
{
	...
	uint32_t rStatus;
	uint32_t u4BufLen;
	uint32_t u4Reason = DISCONNECT_REASON_CODE_NEW_CONNECTION;
	...
	*/
	...
	if (req && req->reason_code == DISCONNECT_REASON_TEMPORARY_FOR_ROAM)
		u4Reason = req->reason_code;
#endif
#if (KAL_AIS_NUM == 1)
	rStatus = kalIoctl(prGlueInfo, wlanoidSetDisassociate,
			&u4Reason, sizeof(u4Reason),
			FALSE, FALSE, TRUE, &u4BufLen);
#else
		...
		IN struct GLUE_INFO *prGlueInfo)
{
	...
	rStatistics.u4RstReason = glGetRstReason(prGlueInfo->prAdapter);
	rStatistics.u8RstTime = glGetRstTIme(prGlueInfo);
#endif
			  ...
			  IN struct cfg80211_sched_scan_request *request)
{
	...
	DBGLOG(SCN, TRACE, "schedScan reqid : %u->%u\n",
		prGlueInfo->u8SchedScanReqid, request->reqid);

	prGlueInfo->u8SchedScanReqid = (uint64_t)request->reqid;
	       ...
	       struct net_device *ndev, struct cfg80211_assoc_request *req)
{
	...
	*/
	...
	if (req->crypto.wpa_versions & NL80211_WPA_VERSION_1)
		...
		|| (req->crypto.wpa_versions & NL80211_WPA_VERSION_3)
#endif
		)
		prGlueInfo->rWpaInfo[ucBssIndex].u4WpaVersion =
					IW_AUTH_WPA_VERSION_WPA2;
	...
	if (req->ie && req->ie_len > 0) {
		...
		kalMemZero(&prConnSettings->rRnsSelectionInfo,
			sizeof(struct IE_WFA_RSN_SELECTION));
		if (wextSrchDesiredRsnSelectionIE(pucIEStart, req->ie_len,
					  (uint8_t **) &prDesiredIE)) {
			uint8_t ucLength = (*(prDesiredIE+1)+2);
				kalMemCopy(&prConnSettings->rRnsSelectionInfo,
					prDesiredIE, ucLength);
			DBGLOG(REQ, INFO,
						"DUMP RSN Selection, EID %x length %x\n",
						*prDesiredIE, ucLength);
			DBGLOG_MEM8(REQ, INFO,
				&prConnSettings->rRnsSelectionInfo,
				ucLength);
		}
#endif /*CFG_SUPPORT_PASSPOINT*/
		kalMemZero(&rRsnInfo, sizeof(struct RSN_INFO));
...
#endif
#if (CFG_SUPPORT_SUPPLICANT_SME == 1) && (CFG_SUPPORT_802_11R == 1)
		eAuthMode = prGlueInfo->prAdapter->rWifiVar
			.rConnSettings[ucBssIndex].eAuthMode;
...
static int
_mtk_cfg80211_tdls_mgmt(struct wiphy *wiphy,
		       struct net_device *dev,
...
mtk_cfg80211_tdls_mgmt(struct wiphy *wiphy,
		       ...
		       bool initiator, const u8 *buf, size_t len)
{
	return _mtk_cfg80211_tdls_mgmt(wiphy, dev, peer,
	       ...
	       IN struct regulatory_request *pRequest)
{
	...
	*/
	...
	rlmDomainSetDfsRegion(prAdapter, pRequest->dfs_region);
}
...
int mtk_cfg80211_resume(struct wiphy *wiphy)
{
	...
	for (; i < SCN_SSID_MATCH_MAX_NUM; i++) {
		...
		pprBssDesc[i]->u2RawLength = 0;
	}
	DBGLOG(SCN, INFO, "pending %d sched scan results\n", i);

#if CFG_WOW_SUPPORT && CFG_SUPPORT_SCHED_SCAN
	if ((prAdapter->rWifiVar.ucSchedScanDelayWakeup) &&
		...
		DBGLOG(SCN, TRACE, "Call cfg80211_sched_scan_results\n");
#if KERNEL_VERSION(4, 12, 0) <= CFG80211_VERSION_CODE
		     ...
		     struct net_device *ndev)
{
	...
	struct wireless_dev **pprP2pRoleWdev = wlanGetP2pRoleWdev(prGlueInfo);
	struct net_device **pprP2pDev = wlanGetP2pNetDev(prGlueInfo);
	...
	uint32_t u4SetBufferLen, uint32_t *pu4SetInfoLen)
{
	...
	pprP2pDev = wlanGetP2pNetDev(prAdapter->prGlueInfo);
...
#endif
{
	...
	if (ndev->ieee80211_ptr->iftype == type) {
		...
		&& params->use_4addr >= 0
		&& prNetdevPriv->ucBssIdx < KAL_AIS_NUM) {

			ucBssIdx = prNetdevPriv->ucBssIdx;

			if (params->use_4addr)
				prAdapter->prAisBssInfo[ucBssIdx]->ucWdsEnable
				= !!params->use_4addr;
			else
				prAdapter->prAisBssInfo[ucBssIdx]->ucWdsEnable
				= params->use_4addr;

			DBGLOG(REQ, INFO, "bssidx[%d] set 4addr [%d]\n",
			ucBssIdx,
			prAdapter->prAisBssInfo[ucBssIdx]->ucWdsEnable);
		}
#endif /* end CFG_SUPPORT_4ADDR_STATION */
...
int mtk_cfg_resume(struct wiphy *wiphy)
{
	...
	if (prGlueInfo->prAdapter &&
		...
		if (glResetReSchL0p5Work(prGlueInfo))

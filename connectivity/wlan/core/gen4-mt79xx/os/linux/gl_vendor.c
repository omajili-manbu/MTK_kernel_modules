					 ...
					 const void *data, int data_len)
{
	...
	prGlueInfo = wlanGetGlueInfo(wiphy);
	if (!prGlueInfo)
		return -EFAULT;
...
int mtk_cfg80211_vendor_get_roaming_improve_feature(struct wiphy *wiphy,
	struct wireless_dev *wdev, const void *data, int data_len)
{
	struct GLUE_INFO *prGlueInfo;
	struct sk_buff *skb;
	uint32_t feature_flags = 1;
	...
	if (unlikely(nla_put(skb, ROAMING_IMPROVE_ATTRIBUTE_FEATURE_ENABLE,
				sizeof(feature_flags), &feature_flags) < 0))
		goto nla_put_failure;

	DBGLOG(REQ, INFO, "feature_flags %d\n", feature_flags);

	if (wlanNotifyRoamingEnhanceToFw(prGlueInfo) != WLAN_STATUS_SUCCESS) {
		DBGLOG(REQ, WARN,
			"failed to enable roaming enhance feature\n");
		goto nla_put_failure;
	}

	prGlueInfo->u4RoamingEnhanceEnable = TRUE;
	return cfg80211_vendor_cmd_reply(skb);
	...
	const void *data, int data_len)
{
	...
	DBGLOG(REQ, INFO, "RTT data_len=%d, iftype=%d\n",
							data_len, wdev->iftype);
	...
	DBGLOG(REQ, INFO, "Get RTT Capabilities\n");
	...
	const void *data, int data_len)
{
	...
	nla_for_each_nested(attr, attrs[RTT_ATTRIBUTE_TARGET_INFO], tmp) {
		...
		if (i >= request->ucConfigNum || i >= RTT_MAX_CANDIDATES) {
			DBGLOG(RTT, ERROR, "Wrong num %hhu/%hhu/%d\n",
			      i, request->ucConfigNum, RTT_MAX_CANDIDATES);
			ret = -EINVAL;
			goto RETURN;
		}
	...
	rStatus = kalIoctl(prGlueInfo, wlanoidHandleRttRequest,
			   request, sizeof(struct PARAM_RTT_REQUEST),
			   FALSE, FALSE, TRUE, &u4BufLen);
	...
	struct wireless_dev *wdev, const void *data, int data_len)
{
	...
	prInfo = kalMemAlloc(sizeof(struct PARAM_OFLD_INFO), VIR_MEM_TYPE);
	if (prInfo == NULL) {
	...
	do {
		...
		if (u4SentLen > u4ProgLen ||
			prInfo->u4BufLen > u4ProgLen - u4SentLen) {
			DBGLOG(REQ, ERROR, "APF set fail for invalid len\n");
			ret = -EFAULT;
			goto exit;
		}
...
#else
			prInfo->ucOp = PKT_OFLD_OP_ENABLE;
#endif
	...
	struct wireless_dev *wdev, const void *data, int data_len)
{
	...
	do {
		rStatus = kalIoctl(prGlueInfo,
				wlanoidQueryOffloadInfo, prInfo,
				sizeof(struct PARAM_OFLD_INFO),
	...
	int data_len)
{
	...
	DBGLOG(INIT, INFO, "connect_ext features\n");
	DBGLOG_MEM8(INIT, INFO, buf, len);

...
#define CMD_RES_WAITING_TIME  50
/*******************************************************************************
 *                             D A T A   T Y P E S
 *******************************************************************************
 */
...
#else
	const struct firmware *prFwEntry = fw_entry;
#endif

	release_firmware(prFwEntry);
...
{
	const struct firmware *prFwEntry = NULL;
	...
	*pu4FileLength = 0;
	...
	do {
			...
			break;
		{
			...
			if (!prFwBuffer) {
				DBGLOG(INIT, ERROR, "prFwBuffer is null\n");
				return NULL;
			}
...
#if (CFG_SUPPORT_RX_PAGE_POOL_USE_CMA == 0)
...
fail:
	if (!pkt) {
		pkt = kalPacketAlloc(
			prGlueInfo, CFG_RX_MAX_MPDU_SIZE, ppucData);
	}
	return pkt;
#else
			     ...
			     *prGlueInfo, IN void *pvPkt)
{
...
#endif
#if CFG_SUPPORT_CSI_VNI
	u_int8_t fgIsCsiVniPkt = FALSE;
#endif /* CFG_SUPPORT_CSI_VNI */
...
#if CFG_SUPPORT_CSI_VNI
	if (GLUE_RX_GET_PKT_CSI_VNI_FLAG(prSkb) == TRUE) {
		fgIsCsiVniPkt = TRUE;
		GLUE_SET_PKT_BSS_IDX(prSkb, MAX_BSSID_NUM);
	}
#endif /* CFG_SUPPORT_CSI_VNI */
	...
	if (ucBssIdx < MAX_BSSID_NUM) {
		prNetDev = (struct net_device *)wlanGetNetInterfaceByBssIdx(
			prGlueInfo, ucBssIdx);
#if CFG_SUPPORT_CSI_VNI
	} else if ((ucBssIdx == MAX_BSSID_NUM) && (fgIsCsiVniPkt == TRUE)) {
		DBGLOG(RX, TRACE, "Is CSI VNI Packet\n");
		if ((prGlueInfo->prCSIDevInfo) &&
			(prGlueInfo->prCSIDevInfo->prDevHandler)) {
			prNetDev = prGlueInfo->prCSIDevInfo->prDevHandler;
		}
#endif /* CFG_SUPPORT_CSI_VNI */
	} else {
	...
	if (GLUE_RX_GET_PKT_FMCW_VNI_FLAG(prSkb) == TRUE) {
		DBGLOG(RX, TRACE, "Is FMCW VNI Packet\n");
		if ((prGlueInfo->prFmcwDevInfo) &&
			(prGlueInfo->prFmcwDevInfo->prDevHandler)) {
			prNetDev = prGlueInfo->prFmcwDevInfo->prDevHandler;
		}
	}
...
#if CFG_SUPPORT_SW_FAST_PATH
	if (ppe_hook_rx_wifi != NULL) {
		DBGLOG(RX, LOUD,
			"ppe_hook_rx_wifi exist call FOE_MAGIC_TAG\n");
		FOE_MAGIC_TAG(prSkb) = FOE_MAGIC_WLAN;
		if (ppe_hook_rx_wifi(prSkb)) {
			DBGLOG(RX, LOUD,
				"ppe_hook_rx_wifi return 1,can not speed up,send the skb to kernel\n");
		} else {
			DBGLOG(RX, LOUD,
				"ppe_hook_rx_wifi return 0,send by ppe hook\n");
			return WLAN_STATUS_SUCCESS;
		}
	} else {
		DBGLOG(RX, LOUD,
			"ppe_hook_rx_wifi NULL,send the skb to kernel\n");
	}
#endif

	/* When running iperf and radar VNI at the same time
	 * Need to bypass GRO when handling radar packet
	 * B/c GRO is designed for RX throughput enhancement
	 */
#if (CFG_SUPPORT_FMCW_VNI == 1)
	if (GLUE_RX_GET_PKT_FMCW_VNI_FLAG(prSkb) == TRUE) {
		DBGLOG(RX, TRACE,
			"Bypass Generic Receive Offload for Radar Packet\n");
		goto out;
	}
#endif /* (CFG_SUPPORT_FMCW_VNI == 1) */
	...
	if (kal_is_skb_gro(prGlueInfo->prAdapter, ucBssIdx)) {
		...
		if (HAL_IS_RX_DIRECT(prGlueInfo->prAdapter)) {
			...
			/*
			 * 1. napi handler context
			 * 2.rx reorder timeout handler
			 */
			preempt_disable();
			...
			IN uint32_t u4BufLen)
{
	...
	if (nlh == NULL) {
		DBGLOG(NAN, ERROR, "Fail to put nlmsg.\n");
		nlmsg_free(skb);
		return -10;
	}
...
void kalIndicateJoinFail(IN struct ADAPTER *prAdapter,
	...
	IN struct BSS_DESC *prBssDesc)
{
	...
	/* When Target BssDesc is NULL
	 * or only Auth/Assoc timeout occurs
	 * trigger auth/assoc timeout event
	 */
	if (prBssDesc
		&& (prBssDesc->u2JoinStatus
			== STATUS_CODE_ASSOC_TIMEOUT)
		&& prConnSettings->bss) {
		DBGLOG(REQ, EVENT, "assoc timeout notify\n");
	...
	KERNEL_VERSION(6, 0, 0) <= CFG80211_VERSION_CODE
		...
		cfg80211_assoc_failure(prDevHandler,
			&assoc_failure_data);
...
#endif
		DBGLOG(REQ, INFO, "assoc timeout Done\n");
		prConnSettings->bss = NULL;
	} else if ((prBssDesc
		&& (prBssDesc->u2JoinStatus
			== STATUS_CODE_AUTH_TIMEOUT))
		|| !prBssDesc) {
		DBGLOG(REQ, EVENT, "auth timeout notify");
		cfg80211_auth_timeout(prDevHandler,
			     ...
			     IN uint32_t u4BufLen, IN uint8_t ucBssIndex)
{
...
#elif (CFG_SUPPORT_6G_PWR_REGS == 1)
	struct BSS_DESC *prBssDesc = NULL;
#endif  /* CFG_SUPPORT_6G_PWR_REGS == 1 */
	...
	if (!prGlueInfo || !prGlueInfo->prAdapter) {
		DBGLOG(INIT, ERROR, "prGlueInfo or prAdapter invalid\n");
		return;
	}
	...
	case WLAN_STATUS_MEDIA_CONNECT:
		...
		if (prBssDesc && prBssDesc->eBand == BAND_6G)
	...
	case WLAN_STATUS_MEDIA_DISCONNECT_LOCALLY:
		...
		if (aisGetAisSpecBssInfo(
			prAdapter, ucBssIndex)->fgDelayReportCarrierOff) {
			/*start timer*/
			cnmTimerStartTimer(prAdapter,
				&aisGetAisFsmInfo(
				prAdapter, ucBssIndex)->rDelayCarrierOffTimer,
		...
		if (prBssDesc == NULL) {
			...
			if (prBssDesc) {
				/* Reinit Transmit Envelope Power limit */
				if (prBssDesc->fgIsTxPwrEnvPresent) {
					rlmTxPwrEnvMaxPwrInit(
					prBssDesc->acTxPwrEnvMaxTxPwr,
					TX_PWR_ENV_MAX_TXPWR_BW_NUM);
					prBssDesc->fgIsTxPwrEnvChange = FALSE;
	...
	case WLAN_STATUS_JOIN_FAILURE: {
...
#if (CFG_SUPPORT_SUPPLICANT_SME == 1)
		kalIndicateJoinFail(prAdapter, ucBssIndex, prBssDesc);
#else
		if (prBssDesc && prBssDesc->u2JoinStatus
			...
			cfg80211_connect_result(prDevHandler,
				...
				GFP_KERNEL);
#endif /* end CFG_SUPPORT_SUPPLICANT_SME */
		kalSetMediaStateIndicated(prGlueInfo,
			MEDIA_STATE_DISCONNECTED,
			ucBssIndex);
		 ...
		 uint8_t ucBssIndex)
{
	...
	while (u4TotalLen > 0) {
		...
		u4DataLeft = kal_min_t(int, skb_shinfo(prOrgSkb)->gso_size,
			u4TotalLen);
		...
		while (u4DataLeft > 0) {
			...
			u4Size = kal_min_t(int, rTso.size, u4DataLeft);
...
/*----------------------------------------------------------------------------*/
/*!
 * \brief Verify that no queues are stuck. Linux will resume transmission.
 *        ALL: all resource
 *        ATOTAL: the total resource for A band SAP (ALL - GTATOL)
 *        GTOTAL: the total resource for G band SAP
 *        sta_cnt: number of STAs with traffic
 *        Single SAP: ALL
 *        Dual SAP: ATOTAL and GTOTAL; or ALL
 *            if 2G && 5/6G sta_cnt > 0: ATOTAL and GTOTAL
 *            else: ALL
 *
 * \param[in] prGlueInfo     Pointer of GLUE Data Structure
 * \param[in] ucBssIndex     BSS index of this net device
 *
 * \return threshold of panding frames for the bss
 */
/*----------------------------------------------------------------------------*/
static uint32_t kalPandingFrameThreshold(IN struct GLUE_INFO *prGlueInfo,
	IN uint8_t ucBssIndex)

{
	struct ADAPTER *prAdapter = NULL;
	struct BSS_INFO *prBssInfo = NULL;
	static uint8_t ucBssIdx2G = MAX_BSSID_NUM;
	static uint8_t ucBssIdx5G = MAX_BSSID_NUM;
#if (CFG_SUPPORT_WIFI_6G == 1)
	static uint8_t ucBssIdx6G = MAX_BSSID_NUM;
#endif
	uint8_t ucSap0BssIdx = MAX_BSSID_NUM;
	uint8_t ucSap1BssIdx = MAX_BSSID_NUM;
	uint8_t i = 0;
	uint8_t ucTxPendingStaCnt = 0;

	prAdapter = prGlueInfo->prAdapter;
	if (!prAdapter)
		return 0;

	/* count the number of connected STAs with traffic */
	for (i = 0; i < CFG_MAX_TXQ_NUM; i++) {
		if (GLUE_GET_REF_CNT(prGlueInfo->
				au4TxPendingFrameNumPerSta[ucBssIndex][i]) >
			CFG_SAP_TXQ_STA_CNT_FRAME_THRESHOLD)
			ucTxPendingStaCnt++;
	}
	GLUE_SET_REF_CNT(prGlueInfo->aucTxPendingStaCnt[ucBssIndex],
		ucTxPendingStaCnt);

	/* sap0: 2G, sap1: 5/6G */
	if (ucBssIdx2G < MAX_BSSID_NUM)
		ucSap0BssIdx = ucBssIdx2G;
	if (ucBssIdx5G < MAX_BSSID_NUM)
		ucSap1BssIdx = ucBssIdx5G;
#if (CFG_SUPPORT_WIFI_6G == 1)
	else if (ucBssIdx6G < MAX_BSSID_NUM)
		ucSap1BssIdx = ucBssIdx6G;
#endif

	/* allocate number of frames for different scenarios */
	if (ucSap0BssIdx < MAX_BSSID_NUM &&
		ucSap1BssIdx < MAX_BSSID_NUM) {
		/* Dual SAP - both of them have traffic */
		if (GLUE_GET_REF_CNT(prGlueInfo->
				aucTxPendingStaCnt[ucSap0BssIdx]) > 0 &&
			GLUE_GET_REF_CNT(prGlueInfo->
				aucTxPendingStaCnt[ucSap1BssIdx]) > 0) {
			GLUE_SET_REF_CNT(prGlueInfo->
				au4TxPendingThreshold[ucSap0BssIdx],
				CFG_SAP_TXQ_2G_TOTAL_FRAME_THRESHOLD);
			GLUE_SET_REF_CNT(prGlueInfo->
				au4TxPendingThreshold[ucSap1BssIdx],
				HIF_TX_MSDU_TOKEN_NUM -
				CFG_SAP_TXQ_2G_TOTAL_FRAME_THRESHOLD);
		} else {
			GLUE_SET_REF_CNT(prGlueInfo->
			    au4TxPendingThreshold[ucBssIndex],
			    HIF_TX_MSDU_TOKEN_NUM);
		}
	} else if (ucSap0BssIdx < MAX_BSSID_NUM ||
			   ucSap1BssIdx < MAX_BSSID_NUM) {
		/* Single SAP */
		GLUE_SET_REF_CNT(prGlueInfo->
		    au4TxPendingThreshold[ucBssIndex],
		...
		for (i = 0; i < MAX_BSSID_NUM; i++) {
			...
			if (prBssInfo->eBand == BAND_2G4)
				ucBssIdx2G = prBssInfo->ucBssIndex;
			else if (prBssInfo->eBand == BAND_5G)
				ucBssIdx5G = prBssInfo->ucBssIndex;
#if (CFG_SUPPORT_WIFI_6G == 1)
			else if (prBssInfo->eBand == BAND_6G)
				ucBssIdx6G = prBssInfo->ucBssIndex;
#endif
		}
	}

	if (ucTxPendingStaCnt == 0)
		return 0;

	return (GLUE_GET_REF_CNT(prGlueInfo->
				au4TxPendingThreshold[ucBssIndex]) /
			ucTxPendingStaCnt);
}
 ...
 *
 * \param[in] prGlueInfo     Pointer of GLUE Data Structure
 * \param[in] ucStaRecIdx    STAREC index
 *
 * \return none
 */
/*----------------------------------------------------------------------------*/
void kalAwakeQueue(IN struct GLUE_INFO *prGlueInfo,
	IN uint8_t ucStaRecIdx)
{
	struct net_device *prDev = NULL;
	struct netdev_queue *prTxQue = NULL;
	...
	prStaRec = cnmGetStaRecByIndex(prGlueInfo->prAdapter, ucStaRecIdx);
	if (!prStaRec)
		return;
	...
	prDev = wlanGetNetDev(prGlueInfo, ucBssIdx);
	if (!prDev)
		return;

	// Verify that no queues are stuck. Traverse all of then
	for (ucQueIdx = 0; ucQueIdx < CFG_MAX_TXQ_NUM; ucQueIdx++) {
		prTxQue = netdev_get_tx_queue(prDev, ucQueIdx);
		if (!netif_tx_queue_stopped(prTxQue))
			continue;
		// If pending frames are below a threshold, resume transmission
		if (GLUE_GET_REF_CNT(
				prGlueInfo->au4TxPendingFrameNumPerSta
				[ucBssIdx][ucQueIdx]) <
				CFG_SAP_TXQ_NETIF_WAKE_THRESHOLD) {

			netif_wake_subqueue(prDev, ucQueIdx);

			DBGLOG(TX, TRACE, "%s WakeUp BSS[%u] QIDX[%u]\n",
				prDev->name, ucBssIdx, ucQueIdx);
		}
	}
}
		 ...
		 uint8_t ucBssIndex)
{
	...
	struct ADAPTER *prAdapter = NULL;
#if CFG_SUPPORT_SAP_TX_FAIRNESS
	uint32_t u4TxPendingFrameNum = 0;
	uint32_t u4TxPendingFrameTh = 0;
#endif
...
#if CFG_SUPPORT_SAP_TX_FAIRNESS
	if (IS_FEATURE_ENABLED(prAdapter->rWifiVar.fgSapTxFairness) &&
		IS_BSS_INDEX_APGO(prAdapter, ucBssIndex)) {
		/* dynamically set frame threshold for sap tx queue */
		u4TxPendingFrameTh =
			kalPandingFrameThreshold(prGlueInfo, ucBssIndex);
		if (u4TxPendingFrameTh > 0) {
			u4TxPendingFrameNum = GLUE_GET_REF_CNT(prGlueInfo->
			    au4TxPendingFrameNumPerSta[ucBssIndex][u2QueueIdx]);
			if (u4TxPendingFrameNum > u4TxPendingFrameTh) {
				netif_stop_subqueue(prDev, u2QueueIdx);

				DBGLOG(TX, TRACE,
					"%s Stop subqueue for BSS[%u] QIDX[%u] TH[%4u]\n",
					prDev->name, ucBssIndex,
					u2QueueIdx, u4TxPendingFrameTh);
			}
		}
	}
#endif
				  ...
				  *prGlueInfo, IN void *pvPacket)
{
	...
	if (fgIsValidDevice == TRUE) {
...
#if CFG_SUPPORT_SAP_TX_FAIRNESS
		if (IS_FEATURE_ENABLED(prGlueInfo->
			prAdapter->rWifiVar.fgSapTxFairness)) {
			if (IS_BSS_INDEX_APGO(prGlueInfo->
					prAdapter, ucBssIndex) &&
				prGlueInfo->aucTxPendingStaCnt
					[ucBssIndex] > 1)
				fgIsQueueByAc = FALSE;
		}
#endif /* CFG_SUPPORT_SAP_TX_FAIRNESS */
		...
		if (fgIsQueueByAc &&
		    netif_subqueue_stopped(prDev, prSkb) &&
		    prGlueInfo->ai4TxPendingFrameNumPerQueue[ucBssIndex]
...
#endif /* CFG_TCP_IP_CHKSUM_OFFLOAD */
#endif /* Automation */
	} else if (ucIpv6Proto == IP_PRO_FRAG_IPV6) {
#if CFG_IP_FRAG_DISABLE_HW_CHECKSUM
...
#if CFG_SUPPORT_NAN
	kalMemCpyS(prTxPktInfo->aucEthSrcAddr,
		sizeof(prTxPktInfo->aucEthSrcAddr),
		aucLookAheadBuf + PARAM_MAC_ADDR_LEN,
		PARAM_MAC_ADDR_LEN);
#endif
...
u_int8_t kalGetEthSrcAddr(IN struct GLUE_INFO *prGlueInfo,
			   IN void *prPacket,
			   OUT uint8_t aucEthSrcAddr[PARAM_MAC_ADDR_LEN])
{
	struct sk_buff *prSkb = (struct sk_buff *)prPacket;
	uint8_t *pucLookAheadBuf = NULL;

	if (!prPacket || !prGlueInfo)
		return FALSE;

	pucLookAheadBuf = prSkb->data;

	kalMemCpyS(aucEthSrcAddr, PARAM_MAC_ADDR_LEN,
		pucLookAheadBuf+PARAM_MAC_ADDR_LEN, PARAM_MAC_ADDR_LEN);
					...
					struct KAL_THREAD_SCHEDSTATS *pSched)
{
	...
	if (!pThread || kalIsResetting(NULL))
		return -2;
	 ...
	 IN uint8_t ucBssIndex)
{
	...
	int32_t *pu4HaltFlag = NULL;
	struct semaphore *prHaltSem = NULL;
	...
	*/
	...
	pu4HaltFlag = wlanGetHaltFlag(prGlueInfo);
	prHaltSem  = wlanGetHaltSem(prGlueInfo);

	if (down_interruptible(prHaltSem))
		return WLAN_STATUS_FAILURE;
	...
	for (waitCount = 0; waitCount < 30; waitCount++) {
		...
		if ((waitRet > 0) || kalIsResetting(prGlueInfo))
			break;
	}
	...
	while (u4TxLoopCount--) {
		while (QUEUE_IS_NOT_EMPTY(prTxQueue)) {
			...
			if (QUEUE_IS_NOT_EMPTY(prTempReturnQue)) {
				if (prTempReturnQue->u4NumElem >
					UINT_MAX - prTempQue->u4NumElem) {
					DBGLOG(INIT, WARN,
						"u4NumElem is overflow!\n");
					break;
				}
...
int rx_thread(void *data)
{
	...
	while (TRUE) {
		...
		if (test_and_clear_bit(GLUE_FLAG_RX_TO_OS_BIT,
			...
			prRxQueue = &prGlueInfo->prAdapter->rRxQueue;
			while (u4LoopCount--) {
				while (TRUE) {
					...
					if (QUEUE_IS_NOT_EMPTY(prRxQueue)) {
...
#endif
#if CFG_SUPPORT_FW_LOG_TIMESYNC
			   ...
			   uint32_t *pu4ReadSize, struct device *dev)
{
	 ...
	 * Linux path: "/lib/firmware", "/lib/firmware/update"
	 */
	ret = request_firmware_direct(&fw, pucPath, dev);
			 ...
			 u_int8_t fgDriverTriggerd)
{
	...
	struct delayed_work *prSchedWorkq = NULL;
...
#endif
#if CFG_SUPPORT_MULTI_CARD
	prSchedWorkq = &prGlueInfo->sched_workq;
#else
	prSchedWorkq = &sched_workq;
#endif
	...
	if (fgDriverTriggerd) {
		...
		schedule_delayed_work(prSchedWorkq, 0);
	 ...
	 OUT uint32_t *pu4QryInfoLen)
{
	...
	void *pvBuf = NULL;
	struct delayed_work *prOidWorkq = NULL;
	GLUE_SPIN_LOCK_DECLARATION();
	...
	kalMemZero(prOidReq, sizeof(struct PARAM_OID_REQ));

	if (u4InfoBufLen) {
		pvBuf = (void *) kalMemAlloc(u4InfoBufLen, VIR_MEM_TYPE);
		DBGLOG(REQ, TRACE, "Alloc pvInfoBuf 0x%p\n", pvBuf);

		if (pvBuf)
			kalMemCpyS(pvBuf, u4InfoBufLen, pvInfoBuf,
				u4InfoBufLen);
		else {
			DBGLOG(REQ, ERROR, "pvBuf Alloc Failed\n");
			DBGLOG(REQ, ERROR, "Free prOidReq 0x%p\n", prOidReq);
			kalMemFree(prOidReq, VIR_MEM_TYPE,
				sizeof(PARAM_OID_REQ));
			return;
		}
	}
...
#if CFG_SUPPORT_MULTI_CARD
	prOidWorkq = &prGlueInfo->oid_workq;
#else
	prOidWorkq = &oid_workq;
#endif
	if (!schedule_delayed_work(prOidWorkq, 0))
...
void wlanSchedOidWorkQueue(struct work_struct *work)
{
...
#if CFG_SUPPORT_MULTI_CARD
	prGlueInfo = container_of(work, struct GLUE_INFO, oid_workq.work);
#else
	...
	while (TRUE) {
		GLUE_ACQUIRE_SPIN_LOCK(prGlueInfo, SPIN_LOCK_OID_REQ);
		if (QUEUE_IS_NOT_EMPTY(&prGlueInfo->prAdapter->rOidQueue)) {
			QUEUE_MOVE_ALL(prTempQue,
				&prGlueInfo->prAdapter->rOidQueue);
		} else {
			GLUE_RELEASE_SPIN_LOCK(prGlueInfo, SPIN_LOCK_OID_REQ);
			break;
		}
			...
			uint8_t *prAssocRspFrame, struct cfg80211_bss *bss, uint16_t u2FrameLen)
{
	...
	struct cfg80211_rx_assoc_resp_data rx_assoc_resp_data = {0};
#else
#if (CFG_ADVANCED_80211_MLO == 1) || \
	KERNEL_VERSION(6, 0, 0) <= CFG80211_VERSION_CODE
...
#if KERNEL_VERSION(6, 7, 0) <= CFG80211_VERSION_CODE
	DBGLOG(INIT, TEMP, "WIPHY_LOCK Try to acquire\n");
	mutex_lock(&(pDev->ieee80211_ptr->wiphy)->mtx);
	DBGLOG(INIT, TEMP, "WIPHY_LOCK Acquired\n");
#else
			...
			uint8_t ucflagTx, uint8_t ucFrameType)
{
	...
	struct GLUE_INFO *prGlueInfo = NULL;
#if CFG_SUPPORT_MULTI_CARD
	struct net_device *prDev = prDevHandler;
#else
	struct net_device *prDev = gPrDev;
#endif
	struct delayed_work *prCfg80211Workq = NULL;
...
#if CFG_SUPPORT_MULTI_CARD
	prCfg80211Workq = &prGlueInfo->cfg80211_workq;
#else
	prCfg80211Workq = &cfg80211_workq;
#endif
	if (!schedule_delayed_work(prCfg80211Workq, 0))
...
void wlanSchedCfg80211WorkQueue(struct work_struct *work)
{
	...
	DBGLOG(REQ, INFO, "\n");
#if CFG_SUPPORT_MULTI_CARD
	prGlueInfo = container_of(work, struct GLUE_INFO, cfg80211_workq.work);
#else
	prGlueInfo = (prDev != NULL) ? *((struct GLUE_INFO **)
					 netdev_priv(prDev)) : NULL;
	if (!prGlueInfo) {
		 ...
		 IN uint8_t *pucIPv6Addr, IN uint32_t u4NumIPv6Addr)
{
	...
	uint32_t u4Len = sizeof(struct PARAM_NETWORK_ADDRESS_LIST);
...
{
#if CFG_SUPPORT_MULTI_CARD
	struct proc_dir_entry **prMetProcDir = &prGlueInfo->prProcRoot;
#else
	struct proc_dir_entry **prMetProcDir = &pMetProcDir;
#endif
	/* struct proc_dir_entry *pMetProcDir; */
#if (CFG_SUPPORT_MULTI_CARD == 0)
	 ...
	 */
	*prMetProcDir = proc_mkdir("wlan0", init_net.proc_net);
#endif
	if (*prMetProcDir == NULL)
		return -ENOENT;
	/*
	 *  /proc/net/wlan0
	 ...
	 */
#if CFG_SUPPORT_MULTI_CARD
	proc_create_data(PROC_MET_PROF_CTRL, 0000, *prMetProcDir,
		    &rMetProcCtrlFops, prGlueInfo);
	proc_create_data(PROC_MET_PROF_PORT, 0000, *prMetProcDir,
		    &rMetProcPortFops, prGlueInfo);
#else
	proc_create(PROC_MET_PROF_CTRL, 0000, *prMetProcDir,
		    &rMetProcCtrlFops);
	proc_create(PROC_MET_PROF_PORT, 0000, *prMetProcDir,
		    &rMetProcPortFops);
...
int kalMetRemoveProcfs(struct GLUE_INFO *prGlueInfo)
{
#if CFG_SUPPORT_MULTI_CARD
	struct proc_dir_entry **prMetProcDir = &prGlueInfo->prProcRoot;
	...
	remove_proc_entry(PROC_MET_PROF_CTRL, *prMetProcDir);
	remove_proc_entry(PROC_MET_PROF_PORT, *prMetProcDir);
#if CFG_SUPPORT_MULTI_CARD
	/*
	 * move PROC_ROOT_NAME to last since it's root directory of the others
	 * incorrect sequence would cause use-after-free error
	 */
	remove_proc_entry(wiphy_name(wiphy), init_net.proc_net);
#else
...
uint32_t kalOpenFwDumpFile(u_int8_t ucDumpFileType)
{
	...
	KAL_GET_TIME64(&tval);
	KAL_RTC_TIME_TO_TM(tval.tv_sec, &tm);
...
void kalMdnsOffloadInit(IN struct ADAPTER *prAdapter)
{
	...
	prMdnsSaveFlags->ucPassthroughBehavior = MDNS_PASSTHROUGH_DROP_ALL;
...
uint16_t kalGetMdnsUplPTSz(struct MDNS_INFO_UPLAYER_T *prMdnsUplayerInfo)
{
	...
	count += MDNS_QUESTION_NAME_MAX_LEN;
	return count;
}
...
void kalShowMdnsRecord(struct GLUE_INFO *prGlueInfo)
{
	...
	for (i = 0 ; i < MDNS_QURTRY_NUMBER; i++) {
		...
		DBGLOG(REQ, ERROR, "clas [%d]  type %d\n",
			prMdnsParamEntry->mdns_param.query[i].class,
			prMdnsParamEntry->mdns_param.query[i].type);
		}
...
uint16_t kalMdnsConvettoDataBlock(struct GLUE_INFO *prGlueInfo)
{
	...
	LINK_FOR_EACH_ENTRY(prMdnsParamEntry, prMdnsRecordList,
		...
		for (i = 0 ; i < MDNS_QURTRY_NUMBER; i++) {
			if (pMdnsParam->query[i].name_length > 0) {
				...
				DBGLOG(REQ, ERROR,
					"pMdnsParam->query[%d].type =%d\n",
					i, pMdnsParam->query[i].type);

				/* Save the query type  */
				prMdnsRecord->u2querytype[i] =
					pMdnsParam->query[i].type;
				DBGLOG(REQ, ERROR,
					"prMdnsRecord->u2querytype[%d] =%d\n",
					i, prMdnsRecord->u2querytype[i]);
				/* Save the u2DataIndex  */
	...
	if (prMdnsInfo->rMdnsPassthroughCout <= 0) {
		DBGLOG(REQ, ERROR, " no passthrough cnt %d\n",
			prMdnsInfo->rMdnsPassthroughCout);
		return FAIL_MDNS_OVERSIZE;
	}
...
#if (CFG_SUPPORT_ANT_SMART_DIVERSITY == 1)
uint32_t kalAntSwDivInit(struct ADAPTER *prAdapter)
{
	...
	if (!prAdapter) {
	 ...
	 prAdapter->rWifiVar.fgIsAntSwDivProbed = 0;
	 prAdapter->rWifiVar.i4AntSwDivMaxRssi = PARAM_WHQL_RSSI_MAX_DBM;

	 cnmTimerInitTimer(prAdapter,
		&prAdapter->rAntSwDivTimer,
	 ...
	 cnmTimerStartTimer(prAdapter,
			&prAdapter->rAntSwDivTimer,
			ANTENNA_SW_DIV_DETECT_INTERVAL);

	return WLAN_STATUS_SUCCESS;
}

uint32_t kalAntSwDivUninit(struct ADAPTER *prAdapter)
{
	uint32_t u4Status = WLAN_STATUS_FAILURE;
	...
	if (timerPendingTimer(&prAdapter->rAntSwDivTimer)) {
...
void  kalAntSwDivDetect(IN struct ADAPTER *prAdapter,
	IN unsigned long plParamPtr)
{
	struct CMD_ANT_SW_DIV_CTRL *prCmdAntSwDiv;
	struct GLUE_INFO *prGlueInfo = NULL;
	uint8_t ucBssIndex = AIS_DEFAULT_INDEX;

	DEBUGFUNC("kalAntSwDivDetect()");
	...
	prCmdAntSwDiv = (struct CMD_ANT_SW_DIV_CTRL *)
		cnmMemAlloc(prAdapter, RAM_TYPE_BUF,
			sizeof(struct CMD_ANT_SW_DIV_CTRL));

	if (!prCmdAntSwDiv) {
		DBGLOG(SW4, ERROR,
			"cnmMemAlloc for kalAntSwDivDetect  failed!\n");
		return;
	}
	kalMemZero(prCmdAntSwDiv, sizeof(struct CMD_ANT_SW_DIV_CTRL));
	...
	if (kalGetMediaStateIndicated(prGlueInfo,
		...
		if (aisGetCurrState(prAdapter, ucBssIndex) ==
			AIS_STATE_NORMAL_TR) {
			prCmdAntSwDiv->ucBssIndex = AIS_DEFAULT_INDEX;
			prCmdAntSwDiv->ucDetectionMode = 1;

			wlanSendSetQueryCmd(prAdapter,
				CMD_ID_ANT_SMART_DIV_SWITCH_TRIGGER,
				TRUE,
				FALSE,
				FALSE,
				NULL,
				NULL,
				sizeof(*prCmdAntSwDiv),
				(uint8_t *) prCmdAntSwDiv, NULL, 0);
		}
	}

	cnmTimerStartTimer(prAdapter,
		&prAdapter->rAntSwDivTimer,  ANTENNA_SW_DIV_DETECT_INTERVAL);

	cnmMemFree(prAdapter, prCmdAntSwDiv);
}

void  kalAntSwDivSwitchCtrl(IN struct ADAPTER *prAdapter, uint8_t ucForce)
{
	struct CMD_ANT_SW_DIV_CTRL *prCmdAntSwDiv;
	struct GLUE_INFO *prGlueInfo = NULL;
	uint8_t ucBssIndex = AIS_DEFAULT_INDEX;
	...
	if (kalGetMediaStateIndicated(prGlueInfo, ucBssIndex) ==
		...
		prCmdAntSwDiv->ucBssIndex = AIS_DEFAULT_INDEX;
		prCmdAntSwDiv->ucForceSwitch = ucForce;
		...
		DBGLOG(SW4, ERROR,
			"Drop ANT switch, Please check wlan if connected!\n");

	cnmMemFree(prAdapter, prCmdAntSwDiv);
}


uint32_t  kalAntSwDivDynamicSwitch(IN struct ADAPTER *prAdapter, int32_t i4Rssi)
{
	uint32_t u4Status = WLAN_STATUS_FAILURE;
	uint8_t ucBssIndex = AIS_DEFAULT_INDEX;
	struct GLUE_INFO *prGlueInfo = NULL;
	int32_t i4CurRssi = 0;
	int32_t i4MaxRssi;
	static int32_t i4LastRssi = PARAM_WHQL_RSSI_MIN_DBM;
	uint8_t ucNeedSwitch = 0;
	static uint8_t ucDebounce = 1;
	struct CMD_ANT_SW_DIV_CTRL *prCmdAntSwDiv;

	DEBUGFUNC("kalAntSwDivDynamicSwitch()");
	...
	if (!prAdapter->rWifiVar.fgAntSwDivEnable)
		return u4Status;
	...
	if (!prCmdAntSwDiv) {
		DBGLOG(SW4, ERROR,
			"cnmMemAlloc for kalAntSwDivDynamicSwitch failed!\n");
		return u4Status;
	}
	kalMemZero(prCmdAntSwDiv, sizeof(struct CMD_ANT_SW_DIV_CTRL));
	...
	if (kalGetMediaStateIndicated(prGlueInfo,
		...
		i4CurRssi = (int32_t) i4Rssi;

		if (i4CurRssi > PARAM_WHQL_RSSI_MAX_DBM)
			i4CurRssi = PARAM_WHQL_RSSI_MAX_DBM;
		else if (i4CurRssi < PARAM_WHQL_RSSI_MIN_DBM)
			i4CurRssi = PARAM_WHQL_RSSI_MIN_DBM;

		if (i4LastRssi == PARAM_WHQL_RSSI_MIN_DBM)
			i4LastRssi = i4CurRssi;

		DBGLOG(SW4, WARN, "cur RSSI: %d, last RSSI: %d, diff: %d\n",
			i4CurRssi, i4LastRssi,
			i4LastRssi - i4CurRssi);

		if ((abs(i4CurRssi-i4LastRssi)  >= SWITCH_DEBOUNCE)
			&& (ucDebounce == 1)) {
			ucDebounce = 0;
			goto exit;
		}

		if (prAdapter->rWifiVar.i4AntSwDivMaxRssi
			== PARAM_WHQL_RSSI_MAX_DBM)
			prAdapter->rWifiVar.i4AntSwDivMaxRssi = i4CurRssi;
		else if (i4CurRssi > prAdapter->rWifiVar.i4AntSwDivMaxRssi)
			prAdapter->rWifiVar.i4AntSwDivMaxRssi = i4CurRssi;

		DBGLOG(SW4, WARN, "Max RSSI: %d\n",
			prAdapter->rWifiVar.i4AntSwDivMaxRssi);

		i4MaxRssi = prAdapter->rWifiVar.i4AntSwDivMaxRssi;
		if (i4CurRssi <= prAdapter->rWifiVar.i4AntSwDivTheshold) {
			if (prAdapter->rWifiVar.fgIsAntSwDivProbed == 0)
				ucNeedSwitch = 1;
			else if ((i4LastRssi != PARAM_WHQL_RSSI_MIN_DBM)
				&& ((i4LastRssi - i4CurRssi)
				>= prAdapter->rWifiVar.ucAntSwDivTriggerTH))
				ucNeedSwitch = 1;
			else if ((i4MaxRssi - i4CurRssi)
				>= (prAdapter->rWifiVar.ucAntSwDivTriggerTH*2))
				ucNeedSwitch = 1;
		}

		DBGLOG(SW4, WARN, "AntSwDivProbed = %d,  ucNeedSwitch = %d\n",
			prAdapter->rWifiVar.fgIsAntSwDivProbed,
			ucNeedSwitch);
		...
		if (ucNeedSwitch == 1) {
			prCmdAntSwDiv->ucBssIndex = AIS_DEFAULT_INDEX;
			prCmdAntSwDiv->ucForceSwitch = 0;	/*dynamic */
			...
			i4LastRssi = PARAM_WHQL_RSSI_MIN_DBM;
			prAdapter->rWifiVar.i4AntSwDivMaxRssi
				= PARAM_WHQL_RSSI_MAX_DBM;
		}
...
#if (CFG_SUPPORT_MULTI_CARD)
	/* Process different Glue Info for handling different autoPerfCfg */
...
int32_t kalFbNotifierReg(IN struct GLUE_INFO *prGlueInfo)
{
	int32_t i4Ret = 0;
#if CFG_SUPPORT_MULTI_CARD
	struct notifier_block *prNotifier = &prGlueInfo->wlan_fb_notifier;
#else
	struct notifier_block *prNotifier = &wlan_fb_notifier;
#endif

#if (CFG_SUPPORT_MULTI_CARD == 0)
	wlan_fb_notifier_priv_data = prGlueInfo;
#endif
	prNotifier->notifier_call = wlan_fb_notifier_callback;

	i4Ret = fb_register_client(prNotifier);
	if (i4Ret)
...
void kalFbNotifierUnReg(struct GLUE_INFO *prGlueInfo)
{
	...
	fb_unregister_client(prNotifier);
#if (CFG_SUPPORT_MULTI_CARD == 0)
				...
				IN uint8_t ucChannelNum)
{
	...
	((KERNEL_VERSION(6, 3, 0) <= CFG80211_VERSION_CODE) && \
	 (KERNEL_VERSION(6, 9, 0) > CFG80211_VERSION_CODE)) || \
	((CFG_KERNEL_AN14_515 == 1) && \
	 (KERNEL_VERSION(5, 15, 144) <= LINUX_VERSION_CODE))
		, 0
#endif
#endif
		);
...
void kalScanReqLog(struct cfg80211_scan_request *request)
{
	...
	snum = kal_min_t(int, request->n_ssids, CFG_SCAN_SSID_MAX_NUM + 1);
	cnum = kal_min_t(u32, request->n_channels,
...
uint8_t kalNapiUnInit(struct net_device *prDev)
{
	...
	if (prNetDevPrivate == NULL) {
		DBGLOG(INIT, ERROR, "prNetDevPrivate is NULL");
		return -1;
	}
	netif_napi_del(&prNetDevPrivate->napi);
...
void kalNanHandleVendorEvent(IN struct ADAPTER *prAdapter, uint8_t *prBuffer)
{
	...
	case NAN_EVENT_ID_DE_EVENT_IND:
		rStatus = mtk_cfg80211_vendor_event_nan_event_indication(
			prAdapter, prTlvElement->aucbody);
		break;
	case NAN_EVENT_DISCOVERY_RESULT:
		rStatus = mtk_cfg80211_vendor_event_nan_match_indication(
			prAdapter, prTlvElement->aucbody);
		break;
	case NAN_EVENT_FOLLOW_EVENT:
		rStatus = mtk_cfg80211_vendor_event_nan_followup_indication(
			prAdapter, prTlvElement->aucbody);
		break;
	case NAN_EVENT_VENDOR_FOLLOW_UP_RX_EVENT:
		rStatus = mtk_cfg80211_vendor_event_nan_vendor_followup_rx(
			prAdapter, prTlvElement->aucbody);
		break;
	case NAN_EVENT_REPLIED_EVENT:
		rStatus = mtk_cfg80211_vendor_event_nan_replied_indication(
			prAdapter, prTlvElement->aucbody);
		break;
	case NAN_EVENT_PUBLISH_TERMINATE_EVENT:
		rStatus = mtk_cfg80211_vendor_event_nan_publish_terminate(
			prAdapter, prTlvElement->aucbody);
		break;
	case NAN_EVENT_SUBSCRIBE_TERMINATE_EVENT:
		rStatus = mtk_cfg80211_vendor_event_nan_subscribe_terminate(
			prAdapter, prTlvElement->aucbody);
		break;
	case NAN_EVENT_SELF_FOLLOW_EVENT:
		rStatus = mtk_cfg80211_vendor_event_nan_seldflwup_indication(
			prAdapter, prTlvElement->aucbody);
		break;
	case NAN_EVENT_VENDOR_FOLLOW_UP_TX_EVENT:
		rStatus = mtk_cfg80211_vendor_event_nan_vendor_followup_tx(
			prAdapter, prTlvElement->aucbody);
		break;
	...
	case NAN_EVENT_DISC_BCN_PERIOD:
		nanDevDiscBcnPeriodEvtHandler(prAdapter, prTlvElement->aucbody);
#endif
		break;
	...
	case NAN_EVENT_DEVICE_INFO:
		DBGLOG(NAN, INFO, "NAN_EVENT_DEVICE_INFO!\n");
		break;
#if CFG_NAN_SUPPORT_OOB_AF_SLOT_BASE
	case NAN_EVENT_ID_OOB_ACTION_TX_STATUS:
		rStatus = nanOobActionFrameTxDoneSlotBase(prAdapter,
			prTlvElement->aucbody);
		break;
...
#if CFG_MTK_SUPPLICANT_ROAMING_ENHANCE
void kalIndicateCqmBeaconLossNotify(struct GLUE_INFO *prGlueInfo,
				uint8_t ucBssIndex)
{
	struct net_device *prNetDev = (struct net_device *)
		wlanGetNetInterfaceByBssIdx(
		prGlueInfo, ucBssIndex);

	if (!prNetDev) {
		DBGLOG(ROAMING, WARN, "net_device null\n");
		return;
	}

	DBGLOG(ROAMING, INFO, "Indicate beacon loss event\n");
	cfg80211_cqm_beacon_loss_notify(prNetDev, GFP_KERNEL);
}
#endif
...
void kalCmdWaitQ(struct ADAPTER *prAdapter)
{
	int ret;
	...
	/* cmd wait queue starts until it's woken up */
	ret = wait_event_interruptible_timeout(prAdapter->prGlueInfo->waitq_cmd,
		QUEUE_IS_NOT_EMPTY(&prAdapter->rFreeCmdList),
		msecs_to_jiffies(CMD_RES_WAITING_TIME));
	if (ret == 0)
		DBGLOG(INIT, WARN, "kalCmdWaitQ timeout\n");
	else if (ret < 0)
		DBGLOG(INIT, WARN, "kalCmdWaitQ interrupted, ret=%ld\n", ret);
}

void kalCmdQAwake(struct ADAPTER *prAdapter)
{
	if (!prAdapter) {
		DBGLOG(INIT, INFO, "prAdapter is NULL\n");
		return;
	...
	/* wake up cmd wait queue if cmd buf avail */
	if (waitqueue_active(&prAdapter->prGlueInfo->waitq_cmd))
		wake_up_interruptible(&prAdapter->prGlueInfo->waitq_cmd);
}

#if CFG_SUPPORT_NAN
void kalNanPrivWorkInit(struct GLUE_INFO *prGlueInfo)
{
	prGlueInfo->prNANPrivCmdWorkQueue = create_workqueue("nan_priv_work");
	if (!prGlueInfo->prNANPrivCmdWorkQueue)
		DBGLOG(INIT, ERROR, "prNANPrivCmdWorkQueue is NULL\n");
}
...
{
	struct NanDrvPrivCmdWork *prWork = ENTRY_OF(work,
						struct NanDrvPrivCmdWork,
						work);
	struct net_device *prNetDev = NULL;

	prNetDev = wlanGetNetDev(prWork->prGlueInfo, AIS_DEFAULT_INDEX);

	if (!prNetDev)
		return;

	priv_driver_cmds(prWork->prGlueInfo,
		prNetDev, prWork->cmd, kalStrLen(prWork->cmd));
	kalMemFree(prWork, PHY_MEM_TYPE, sizeof(struct NanDrvPrivCmdWork));
}
#endif /* CFG_SUPPORT_NAN */

#if CFG_SUPPORT_FW_LOG_TIMESYNC
static void setTimeParameter(
		...
		unsigned int usecond)
{
	...
	if (WLAN_CFG_VALUE_LEN_MAX < CHIP_CONFIG_RESP_SIZE)
		u2CpyLen = WLAN_CFG_VALUE_LEN_MAX;
	else
		u2CpyLen = CHIP_CONFIG_RESP_SIZE;
...
kalSyncTimeToFW(struct ADAPTER *prAdapter, u_int8_t fgInitCmd)
{
	...
	kalSyncTimeToFwViaCmd(prAdapter, u4Sec, u4Usec);
...
#endif /* CFG_SUPPORT_FW_LOG_TIMESYNC */

/* Safe copy memory block with specific size */
int _kalMemCpyS(void *pvDst, size_t szDst, const void *pvSrc,
	size_t szSrc, const char *pucFile, const size_t szLine)
{

	if ((szDst > RSIZE_MAX) || (szDst < szSrc)) {
		LOG_FUNC("kalMemCpyS failed with invalid copy size at %s:%lu\n",
				pucFile, szLine);
		return -EINVAL;
	}

	if ((pvDst == NULL) || (pvSrc == NULL)) {
		LOG_FUNC("kalMemCpyS failed with NULL pointer at %s:%lu\n",
				pucFile, szLine);
		return -EFAULT;
	}

	memcpy(pvDst, pvSrc, szSrc);
	return 0;
}

#if (CFG_SUPPORT_RTT == 1)
void
kalProcessRttReportDone(struct ADAPTER *prAdapter)
{
	struct GLUE_INFO *prGlueInfo;
	struct wiphy *wiphy;
	struct wireless_dev *wdev;
	...
	uint32_t u4Count = 0, sz = sizeof(struct RTT_RESULT);
	uint32_t u4DataLen = 0, complete = TRUE;

	DBGLOG(RTT, STATE, "Send RTT Report to host\n");
	...
	if (!wiphy) {
		DBGLOG(RTT, ERROR, "wiphy is null\n");
		return;
	}
	...
	/* Copy result to skb and free memory */
	while (!LINK_IS_EMPTY(&rttInfo->rResultList) && complete) {
		uint32_t size;
		struct RTT_RESULT *r = NULL;
		...
		if (entry != NULL) {
			...
			DBGLOG(RTT, ERROR, "RTT Result is NULL");
		}
	}
	...
	if (complete)
		...
		kalKfreeSkb(skb, TRUE);
}
#endif /* CFG_SUPPORT_RTT */

#if CFG_CSI_DYNAMIC_BUFFER
u_int8_t csiAllocDynamicBuffer(struct GLUE_INFO *prGlueInfo)
{
	struct CSI_INFO_T *prCSIInfo = NULL;
	struct CSI_INFO_DATA_T *prCsiInfoData = NULL;
	...
	if (prCSIInfo->prCsiInfoData) {
		DBGLOG(CSI, INFO, "CSI Info Data already alloc\n");
		return TRUE;
	}

	prCsiInfoData = (struct CSI_INFO_DATA_T *)kalMemAlloc(
			sizeof(struct CSI_INFO_DATA_T), VIR_MEM_TYPE);
	if (!prCsiInfoData) {
		DBGLOG(REQ, ERROR,
			"[CSI] allocate memory for prCsiInfoData failed\n");
		return FALSE;
	}

	prCSIInfo->prCsiInfoData = prCsiInfoData;
	DBGLOG(CSI, INFO, "[CSI] allocate memory for prCsiInfoData success\n");
	return TRUE;
}

u_int8_t csiFreeDynamicBuffer(struct GLUE_INFO *prGlueInfo)
{
	struct CSI_INFO_T *prCSIInfo = NULL;
	...
	if (!prCSIInfo->prCsiInfoData) {
		DBGLOG(CSI, INFO, "CSI Info Data already free\n");
		return TRUE;
	}

	kalMemFree(prCSIInfo->prCsiInfoData, VIR_MEM_TYPE,
		sizeof(struct CSI_INFO_DATA_T));

	prCSIInfo->prCsiInfoData = NULL;
	DBGLOG(CSI, INFO, "[CSI] free memory for prCsiInfoData success\n");
	return TRUE;
}

#endif
#if CFG_SUPPORT_MULTI_SKB_SIZE
uint32_t getSkbSize(IN struct ADAPTER *prAdapter,
	IN enum ENUM_SKB_SIZE_TYPES eSkbSizeType)
{
#define SIZE_1K 1024
#define SIZE_2K 2048
	...
	if (!prAdapter)
		return SKB_SIZE_4K_SLAB;
	u4TxHeadRoomSize = NIC_TX_DESC_AND_PADDING_LENGTH +
			prAdapter->chip_info->txd_append_size;

	switch (eSkbSizeType) {
	case SKB_SIZE_TYPES_1K_SLAB:
		/*576 bytes*/
		return SIZE_1K - SKB_DATA_ALIGN(sizeof(struct skb_shared_info))
			- SKB_DATA_ALIGN(u4TxHeadRoomSize + NET_SKB_PAD);
	case SKB_SIZE_TYPES_2K_SLAB:
		/*1600 bytes*/
		return SIZE_2K - SKB_DATA_ALIGN(sizeof(struct skb_shared_info))
			- SKB_DATA_ALIGN(u4TxHeadRoomSize + NET_SKB_PAD);
	case SKB_SIZE_TYPES_4K_SLAB:
	default:
		return SKB_SIZE_4K_SLAB;
	}
}

uint32_t getSkbNum(IN struct ADAPTER *prAdapter,
	IN enum ENUM_SKB_SIZE_TYPES eSkbSizeType)
{
	if (!prAdapter)
		return 0;
	...
	switch (eSkbSizeType) {
	case SKB_SIZE_TYPES_1K_SLAB:
		return prAdapter->rWifiVar.u2SkbNum1kSlab;
	case SKB_SIZE_TYPES_2K_SLAB:
		return prAdapter->rWifiVar.u2SkbNum2kSlab;
	case SKB_SIZE_TYPES_4K_SLAB:
	default:
		return prAdapter->rWifiVar.u2SkbNum4kSlab;
	}
}

struct QUE *getFreeRfbList(IN struct ADAPTER *prAdapter,
	IN uint32_t size,
	IN uint32_t u4MinRfbCnt)
{
	enum ENUM_SKB_SIZE_TYPES eSkbSizeType = SKB_SIZE_TYPES_1K_SLAB;
	struct RX_CTRL *prRxCtrl = NULL;

	if (!prAdapter || (getTotalFreeRfbCount(prAdapter) <= u4MinRfbCnt)) {
		DBGLOG(INIT, STATE,
			"return NULL for size %d u4MinRfbCnt %d\n",
			size, u4MinRfbCnt);
		return NULL;
	}

	prRxCtrl = &prAdapter->rRxCtrl;
	for (; eSkbSizeType < SKB_SIZE_TYPES_NUM; eSkbSizeType++) {
		if (prRxCtrl->rFreeSwRfbList[eSkbSizeType].u4NumElem > 0 &&
			getSkbSize(prAdapter, eSkbSizeType) >= size)
			return &(prRxCtrl->rFreeSwRfbList[eSkbSizeType]);
	}
	DBGLOG(INIT, STATE, "No free Rfb for size %d u4MinRfbCnt %d\n",
		size, u4MinRfbCnt);
	return NULL;
}

#endif
uint32_t getTotalFreeRfbCount(IN struct ADAPTER *prAdapter)
{
	struct RX_CTRL *prRxCtrl = NULL;
#if CFG_SUPPORT_MULTI_SKB_SIZE
	enum ENUM_SKB_SIZE_TYPES eSkbSizeType = SKB_SIZE_TYPES_1K_SLAB;
...
#if CFG_SUPPORT_MULTI_SKB_SIZE
	for (; eSkbSizeType < SKB_SIZE_TYPES_NUM; eSkbSizeType++)
		freeNum += prRxCtrl->rFreeSwRfbList[eSkbSizeType].u4NumElem;

	return freeNum;
#else
	return prRxCtrl->rFreeSwRfbList.u4NumElem;
#endif
}

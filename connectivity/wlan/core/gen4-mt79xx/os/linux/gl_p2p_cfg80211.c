...
static void mtk_vif_destructor(struct net_device *dev)
{
	...
	if ((pprP2pRoleWdev) && (prWdev != prP2pWdev)) {
		...
		for (u4Idx = 0; u4Idx < KAL_P2P_NUM; u4Idx++) {
			 ...
			 */
			if (u4Idx == 0)
				pprP2pRoleWdev[u4Idx] = prP2pWdev;
			else
...
#endif
{
	...
	do {
		...
		prP2pWdev = wlanGetP2pWdev(prAdapter->prGlueInfo);
		...
		*((struct GLUE_INFO **) netdev_priv(prNewNetDevice)) =
		...
		kalMemCopy(prWdev, prP2pWdev, sizeof(struct wireless_dev));
		...
		if (prGlueInfo->prAdapter->rWifiVar.ucP2pShareMacAddr &&
			...
			rMacAddr[0] = pprP2pDev[0]->dev_addr[0];
		} else {
...
/* P_IE_SSID_T prSsidIE = (P_IE_SSID_T)NULL; */
	...
	do {
		...
		for (ucBssIdx = 0; ucBssIdx < prAdapter->ucHwBssIdNum;
			...
			if (!prP2pBssInfo)
				continue;
			if (atomic_read(&prP2pBssInfo->fgIsApStarting)
				== TRUE) {
		...
		if (ucBssIdx == prAdapter->ucHwBssIdNum) {
			if (p2pFuncRoleToBssIdx(prAdapter, ucRoleIdx, &ucBssIdx)
				...
				return -EINVAL;
			prP2pBssInfo = prAdapter->aprBssInfo[ucBssIdx];
			atomic_set(&prP2pBssInfo->fgIsApStarting, TRUE);
			mboxSendMsg(prGlueInfo->prAdapter,
				MBOX_ID_0,
				(struct MSG_HDR *) prP2pStartAPMsg,
				MSG_SEND_METHOD_BUF);
		...
		unsigned int cac_time_ms)
{
	...
	do {
		...
		i4Rslt = p2pFuncPreStartRdd(prGlueInfo->prAdapter, ucRoleIdx,
					chandef, cac_time_ms);
		if (!i4Rslt)
			i4Rslt = p2pFuncStartCAC(prGlueInfo->prAdapter,
				ucRoleIdx, chandef);
		...
		struct cfg80211_csa_settings *params)
{
	...
	do {
		...
		if (prGlueInfo->prP2PInfo[ucRoleIdx]->chandef == NULL) {
			DBGLOG(P2P, WARN, "prP2PInfo role(%d) is NULL.\n",
					ucRoleIdx);
			...
			if (prGlueInfo->prP2PInfo[ucRoleIdx]->chandef->chan
				== NULL) {
				kalMemFree(prGlueInfo->prP2PInfo[ucRoleIdx]
					->chandef, VIR_MEM_TYPE,

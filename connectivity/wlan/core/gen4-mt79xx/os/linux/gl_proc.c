...
#if CFG_SUPPORT_CSI && (CFG_SUPPORT_MULTI_CARD == 0)
#if (CFG_SUPPORT_CSI_VNI == 0)
...
static uint8_t *glCsiGetCSIBuf(struct GLUE_INFO *prGlueInfo)
{
#if CFG_SUPPORT_MULTI_CARD
	...
	char __user *buf, size_t count, loff_t *f_pos)
{
...
#if CFG_SUPPORT_MULTI_CARD
	prTmpCSIData = &prGlueInfo->prAdapter->rTmpCSIData;
#else
	prTmpCSIData = &rTmpCSIData;
#endif
	...
	if (prCSIInfo->bIncomplete == FALSE) {
		...
		if (kalIsHalted() || kalIsResetting(prGlueInfo)) {
			DBGLOG(INIT, WARN,
				"[CSI] kalIsHalted=%u kalIsResetting=%u\n",
				kalIsHalted(), kalIsResetting(prGlueInfo));
			return -EFAULT;
		}
		 ...
		 */
		if (wlanPopCSIData(prGlueInfo->prAdapter,
			prTmpCSIData))
			i4Pos = wlanCSIDataPrepare(temp,
				prGlueInfo->prAdapter, prTmpCSIData);
	...
	size_t count, loff_t *f_pos)
{
	 ...
	 */
	uint32_t u4CopySize = 0;
	int32_t *pi4NextDriverReadLen = NULL;
...
#if CFG_SUPPORT_MULTI_CARD
	prGlueInfo = (struct GLUE_INFO *) PDE_DATA(file_inode(filp));
	pucProcBuf = prGlueInfo->aucProcBuf;
	pi4NextDriverReadLen = &prGlueInfo->i4NextDriverReadLen;
#else
	pucProcBuf = g_aucProcBuf;
	pi4NextDriverReadLen = &g_i4NextDriverReadLen;
#endif

	if (*pi4NextDriverReadLen > 0)	/* Detect content to show */
		u4CopySize = *pi4NextDriverReadLen;
	...
	size_t count, loff_t *data)
{
	struct GLUE_INFO *prGlueInfo;
	uint8_t *pucProcBuf;
	uint8_t *pucTempProcBuf;
	uint32_t u4CopySize, u4ProcBufLen;
	int32_t *pi4NextDriverReadLen;
	int32_t i4Ret = count;

#if CFG_SUPPORT_MULTI_CARD
	prGlueInfo = (struct GLUE_INFO *) PDE_DATA(file_inode(file));
	pucProcBuf = prGlueInfo->aucProcBuf;
	u4ProcBufLen = sizeof(prGlueInfo->aucProcBuf);
	pi4NextDriverReadLen = &prGlueInfo->i4NextDriverReadLen;
#else
	prGlueInfo = g_prGlueInfo_proc;
	pucProcBuf = g_aucProcBuf;
	u4ProcBufLen = sizeof(g_aucProcBuf);
	pi4NextDriverReadLen = &g_i4NextDriverReadLen;
#endif

	u4CopySize = u4ProcBufLen;
	pucTempProcBuf = kalMemAlloc(u4ProcBufLen, VIR_MEM_TYPE);
	if (!pucTempProcBuf) {
		DBGLOG(INIT, ERROR, "error of memory allocation\n");
		i4Ret = -ENOMEM;
		goto freeBuf;
	}

	kalMemSet(pucTempProcBuf, 0, u4CopySize);
	u4CopySize = (count < u4CopySize) ? count : (u4CopySize - 1);

	if (copy_from_user(pucTempProcBuf, buffer, u4CopySize)) {
		DBGLOG(INIT, ERROR, "error of copy from user\n");
		i4Ret = -EFAULT;
		goto freeBuf;
	}
	pucTempProcBuf[u4CopySize] = '\0';

	/* if g_u4NextDriverReadLen >0,
	 * the content for next DriverCmdRead will be
	 *  in : g_aucProcBuf with length : g_u4NextDriverReadLen
	 */
	*pi4NextDriverReadLen =
		priv_driver_cmds(prGlueInfo, prGlueInfo->prDevHandler,
			pucTempProcBuf, u4ProcBufLen);
	kalMemCopy(pucProcBuf, pucTempProcBuf, u4ProcBufLen);

freeBuf:
	if (pucTempProcBuf)
		kalMemFree(pucTempProcBuf, VIR_MEM_TYPE, u4ProcBufLen);
	...
	size_t count, loff_t *data)
{
	...
	while (temp) {
		if (sscanf(temp,
			"0x%2x:0x%2x", &u4NewDbgModule, &u4NewDbgLevel) != 2) {
...
#if CFG_SUPPORT_MULTI_CARD
	prGlueInfo = (struct GLUE_INFO *) PDE_DATA(file_inode(filp));
	prMcrOffset = &prGlueInfo->u4McrOffset;
#else
	prGlueInfo = g_prGlueInfo_proc;
	prMcrOffset = &u4McrOffset;
#endif

	rMcrInfo.u4McrData = 0;
	rMcrInfo.u4McrOffset = *prMcrOffset;
	...
	size_t count, loff_t *data)
{
	...
	num =
	    sscanf(acBuf, "0x%8x 0x%8x", &rMcrInfo.u4McrOffset,
		   &rMcrInfo.u4McrData);
	...
	if (num) {
		...
		case 2:
			...
			*/
			...
			{
				*prMcrOffset = rMcrInfo.u4McrOffset;
...
#define MODULE_NAME_LEN_1 5
	...
	while (temp) {
		...
		if (sscanf(temp, "%4s %10u", aucModule, &u4Enabled) != 2) {
			pr_info("read param fail, aucModule=%s u4Enabled=%u\n",
				aucModule, u4Enabled);
			fgParamValue = FALSE;
			break;
		}
	...
	if (fgParamValue) {
		...
		prGlueInfo = (struct GLUE_INFO *) PDE_DATA(file_inode(file));
#else
		prGlueInfo = wlanGetGlueInfo(NULL);
#endif
		if (!prGlueInfo)
			goto freeBuf;
...
#define MODULE_RX 2
	...
	while (temp) {
		...
		/* pick up a string and teminated after meet : */
		if (sscanf
		    (temp, "%6s %8x %10u %10u", aucModule, &u4IpProto,
		     &u4PortNum, &u4DelayThreshold) != 4) {
	...
	size_t count, loff_t *f_pos)
{
	...
	rStatus =
	    kalIoctl(prGlueInfo, wlanoidGetRoamParams, pucProcBuf,
	...
	size_t count, loff_t *data)
{
	...
	if (kalStrnCmp(pucProcBuf, "force_roam", 10) == 0)
		rStatus =
		    kalIoctl(prGlueInfo, wlanoidSetForceRoam, NULL, 0,
			     FALSE, FALSE, TRUE, &u4BufLen);
	else
		rStatus =
		    kalIoctl(prGlueInfo, wlanoidSetRoamParams,
	...
	size_t count, loff_t *data)
{
	...
	i = sscanf(temp, "%10u:%10u", &u4CoreNum, &u4CoreFreq);
	if (i == 2) {
	...
	i = sscanf(temp, "%11s:%10u", aucBuf, &u4ForceEnable);
	...
	char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if (u4CopySize >= PROC_MAX_BUF_SIZE) {
		u4CopySize = PROC_MAX_BUF_SIZE;
		temp[PROC_MAX_BUF_SIZE - 1] = '\0';
	}
...
{
#if CFG_SUPPORT_MULTI_CARD
	struct proc_dir_entry *prProcRoot = prGlueInfo->prProcRoot;
#else
	struct proc_dir_entry *prProcRoot = gprProcRoot;
...
#if CFG_SUPPORT_SET_CAM_BY_PROC
	remove_proc_entry(PROC_SET_CAM, prProcRoot);
#endif
#if CFG_SUPPORT_DEBUG_FS
	remove_proc_entry(PROC_ROAM_PARAM, prProcRoot);
#endif
#if CFG_SUPPORT_CSI
#if (CFG_SUPPORT_CSI_VNI == 0)
	prGlueInfo->prAdapter->rCSIInfo.u4CSIBufferUsed = 1;
	wake_up_interruptible(&(prGlueInfo->prAdapter->rCSIInfo.waitq));
	remove_proc_entry(PROC_CSI_DATA_NAME, prProcRoot);
#endif /* CFG_SUPPORT_CSI_VNI == 0 */
#endif
#if CFG_SUPPORT_PROC_GET_WAKEUP_REASON
	remove_proc_entry(PROC_WAKEUP_REASON, prProcRoot);
#endif
...
#else
#define PROC_CREATE(NAME, MODE, ROOT, OPS) \
		proc_create(NAME, MODE, ROOT, OPS)
#endif
	...
	const uint8_t *root_name = wiphy_name(wiphy);
#endif
	...
	/*
	 * Directory: Root (/proc/net/phy0) or (/proc/net/phy1) or ...
	 */
	...
	prEntry = PROC_CREATE(PROC_MCR_ACCESS, 0660, prProcRoot, &mcr_ops);
	if (prEntry == NULL) {
	...
	prEntry =
	    PROC_CREATE(PROC_PKT_DELAY_DBG, 0660, prProcRoot,
...
#if CFG_SUPPORT_SET_CAM_BY_PROC
	prEntry =
	    PROC_CREATE(PROC_SET_CAM, 0660, prProcRoot, &proc_set_cam_ops);
	if (prEntry == NULL) {
...
#endif
#if CFG_SUPPORT_DEBUG_FS
	prEntry = PROC_CREATE(PROC_ROAM_PARAM, 0660, prProcRoot, &roam_ops);
	if (prEntry == NULL) {
		DBGLOG(INIT, ERROR,
...
#endif
	prEntry = PROC_CREATE(PROC_COUNTRY, 0660, prProcRoot, &country_ops);
	if (prEntry == NULL) {
	...
	prEntry =
		PROC_CREATE(PROC_DRIVER_CMD, 0660, prProcRoot, &drivercmd_ops);
	if (prEntry == NULL) {
		DBGLOG(INIT, ERROR,
...
#if CFG_SUPPORT_CFG_FILE
	prEntry = PROC_CREATE(PROC_CFG, 0660, prProcRoot, &cfg_ops);
	if (prEntry == NULL) {
		DBGLOG(INIT, ERROR,
...
#endif
	prEntry =
		PROC_CREATE(PROC_EFUSE_DUMP, 0660, prProcRoot, &efusedump_ops);
	if (prEntry == NULL) {
...
#endif
#if CFG_WIFI_TXPWR_TBL_DUMP
	prEntry = PROC_CREATE(PROC_GET_TXPWR_TBL, 0660, prProcRoot,
			      &get_txpwr_tbl_ops);
	if (prEntry == NULL) {
		DBGLOG(INIT, ERROR,
...
#if CFG_ASSERT_DUMP
	prEntry =
		PROC_CREATE(PROC_CORE_DUMP, 0660, prProcRoot, &coredump_ops);
	if (prEntry == NULL) {
...
#if CFG_SUPPORT_PROC_GET_WAKEUP_REASON
	prEntry = PROC_CREATE(PROC_WAKEUP_REASON, 0660, prProcRoot,
			&wakeup_reason_ops);
	if (prEntry == NULL) {
		DBGLOG(INIT, ERROR,

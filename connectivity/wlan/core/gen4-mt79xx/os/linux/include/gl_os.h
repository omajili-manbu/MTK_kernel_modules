...
/* frame num threshold for 2.4G SAP */
#define CFG_SAP_TXQ_2G_TOTAL_FRAME_THRESHOLD    256
/* frame num threshold for count SAP TX STA num with traffic */
#define CFG_SAP_TXQ_STA_CNT_FRAME_THRESHOLD     10
/* frame num threshold for wake a netif SAP TX queue */
#define CFG_SAP_TXQ_NETIF_WAKE_THRESHOLD        50
...
typedef void (*wifi_fwlog_event_func_cb)(struct net_device *, int, int);
/* adaptor ko */
...
#define DEFAULT_MAC_ADDRESS "FF:FF:FF:FF:FF:FF"
#endif

#if CFG_SUPPORT_CSI && CFG_SUPPORT_MULTI_CARD
#define CSI_MAX_BUFFER_SIZE 4300
#endif

#if CFG_SUPPORT_SAP_TX_FAIRNESS
#undef CFG_MAX_TXQ_NUM
/* STA has its own tx queue for SAP */
#define CFG_MAX_TXQ_NUM CFG_STA_REC_NUM
#define CFG_NOT_SAP_TXQ_NUM 0xFF
#endif
...
enum ENUM_PKT_FLAG {
	...
	ENUM_PKT_IPV6_FRAG, /* fragmented IPV6 packet*/
#endif
...
struct GLUE_INFO {
	...
	struct sk_buff_head rHifSkbList;
#endif
#endif
...
#if CFG_SUPPORT_SAP_TX_FAIRNESS
	uint8_t aucTxPendingStaCnt[MAX_BSSID_NUM];
	uint32_t au4TxPendingThreshold[MAX_BSSID_NUM];
	uint32_t au4TxPendingFrameNumPerSta[MAX_BSSID_NUM][CFG_STA_REC_NUM];
#endif
	...
	struct _GL_CSI_INFO_T *prCSIDevInfo;
	struct wireless_dev *prCsiRoleWdev;
#endif /* CFG_SUPPORT_CSI_VNI */
#if (CFG_SUPPORT_FMCW_VNI == 1)
	struct _GL_FMCW_INFO_T *prFmcwDevInfo;
	struct wireless_dev *prFmcwRoleWdev;
	...
	/*store the supplicant roaming improvemenet enable state*/
	uint32_t u4RoamingEnhanceEnable;
	...
	u_int8_t fgTestL0p5InResume;
	u_int8_t fgTestL0p5InSuspend;
#endif
#endif
	...
	/* Check featureMap of FW release information*/
	u_int8_t fgSupportTestMode;
	/* Flag of waiting Test Mode Done, True:waitiing */
	uint8_t fgWaitTestModeDone;
#endif

#if defined(_HIF_USB)
	enum POWER_OFF_STATE ePoweredoffWifiFlag;
#endif /* _HIF_USB */
	...
	struct net_device *prNetdev[KAL_AIS_NUM];
...
#if WLAN_INCLUDE_SYS
	struct kobject *wlan_kobj;
	uint8_t aucMacAddrOverride[sizeof(DEFAULT_MAC_ADDRESS)];
	u_int8_t fgIsSysFsMacAddrOverride;
	int32_t i4PM;
...
#if CFG_SUPPORT_CSI_VNI
	uint8_t fgIsCsiVniPkt;	/* 1byte */
#endif /* CFG_SUPPORT_CSI_VNI */
#if (CFG_SUPPORT_FMCW_VNI == 1)
	uint8_t fgIsFmcwVniPkt;	/* 1byte */
#endif /* (CFG_SUPPORT_FMCW_VNI == 1) */
};
...
#if CFG_SUPPORT_CSI_VNI
#define GLUE_RX_SET_PKT_CSI_VNI_FLAG(_p, _fgIsCsiVniPkt) \
	(GLUE_GET_PKT_PRIVATE_RX_DATA(_p)->fgIsCsiVniPkt = \
	(uint8_t)(_fgIsCsiVniPkt))

#define GLUE_RX_GET_PKT_CSI_VNI_FLAG(_p) \
	(GLUE_GET_PKT_PRIVATE_RX_DATA(_p)->fgIsCsiVniPkt)
#endif /* CFG_SUPPORT_CSI_VNI */

#if (CFG_SUPPORT_FMCW_VNI == 1)
#define GLUE_RX_SET_PKT_FMCW_VNI_FLAG(_p, _fgIsFmcwVniPkt) \
	(GLUE_GET_PKT_PRIVATE_RX_DATA(_p)->fgIsFmcwVniPkt = \
	(uint8_t)(_fgIsFmcwVniPkt))

#define GLUE_RX_GET_PKT_FMCW_VNI_FLAG(_p) \
	(GLUE_GET_PKT_PRIVATE_RX_DATA(_p)->fgIsFmcwVniPkt)
#endif /* (CFG_SUPPORT_FMCW_VNI == 1) */
...
#define GLUE_FENCE_ATOMIC_VOID(op, args...) \
({ \
	KAL_MB_RW(); \
	op(args); \
	KAL_MB_RW(); \
})
#define GLUE_SET_REF_CNT(_refCount, _value)  \
	GLUE_FENCE_ATOMIC_VOID(atomic_set, ((atomic_t *)&(_refCount)), (_value))
...
#endif
#if CFG_SUPPORT_SAP_TX_FAIRNESS
	u2QueIdx = mtk_wlan_ndev_select_sap_tx_queue(dev, skb);
	if (u2QueIdx < CFG_MAX_TXQ_NUM)
		return u2QueIdx;
#endif
...
extern int32_t glCustomSendToUplayer(int8_t *aucMsg, int32_t u4MsgLen);
#endif

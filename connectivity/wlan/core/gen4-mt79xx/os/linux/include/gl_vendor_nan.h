...
#define IS_NAN_CAPABILITY_ENABLED(_prAdapter, u8CapBit) \
	((((_prAdapter)->u8NanCardCap) & (u8CapBit)) != 0)
#define IS_NAN_CAPABILITY_DISABLED(_prAdapter, u8CapBit) \
	(!(IS_NAN_CAPABILITY_ENABLED(_prAdapter, u8CapBit)))
...
struct NAN_VENDOR_FOLLOW_UP_RX_EVENT {
	...
	u8 ucLocalServiceId;
	u8 ucPeerServiceId;
	u8 aucPeerAddr[NAN_MAC_ADDR_LEN];
	s8 i1FollowUpRssi;
	u8 ucPrimaryCh;
	u8 ucChBand;
	u8 ucChBw;
	u8 aucRsvd;
	u16 u2AttrListLength;
	u8 aucAttrList[MAX_SIZE_NAN_ATTRIBUTE_REPORT];
...
struct NanVendorFollowupIndMsg {
	struct _NanMsgHeader fwHeader;
	struct NAN_VENDOR_FOLLOW_UP_RX_EVENT vendorfollowupIndParams;
} PACKED;

struct NanVendorFollowupTxIndMsg {
	struct _NanMsgHeader fwHeader;
	struct NAN_VENDOR_FOLLOW_UP_TX_EVENT vendorfollowupIndParams;
} PACKED;
...
#define WFPAL_NAN_OUT_OF_BAND_ACTION_FRAME_MAX_SIZE 960
struct NanOobAfRxIndMsg {
	...
	u8 frameType;
	u16 length;
	u8 data[WFPAL_NAN_OUT_OF_BAND_ACTION_FRAME_MAX_SIZE];
} PACKED;

enum NanOobTxStatusResult {
	/// Tx Success
	WFPAL_NAN_OUT_OF_BAND_ACTION_FRAME_TX_RESULT_ACK = 1,
	/// Tx Failure
	WFPAL_NAN_OUT_OF_BAND_ACTION_FRAME_TX_RESULT_NO_ACK = 2,
	/// Tx Failure
	WFPAL_NAN_OUT_OF_BAND_ACTION_FRAME_TX_RESULT_EXPIRED = 3,
	/// Tx Failure
	WFPAL_NAN_OUT_OF_BAND_ACTION_FRAME_TX_RESULT_DISABLED = 4,
	WFPAL_NAN_OUT_OF_BAND_ACTION_FRAME_TX_RESULT_UNKNOWN = 5,
} PACKED;

struct NanOobAfTxStatusIndMsg {
	struct _NanMsgHeader fwHeader;
	uint16_t u2Token;
	u8 ucResult; /* NanOobTxStatusResult */
	u8 ucReason;
} PACKED;
...
#define WFPAL_NAN_PAIRING_KEY_MAX_LENGTH 32
enum ENUM_ENCRYPTION_KEY {
	ENCRYPTION_KEY_NM_TK,
	ENCRYPTION_KEY_NM_KEK,
	ENCRYPTION_KEY_NM_KCK,
	ENCRYPTION_KEY_ND_TK,
	ENCRYPTION_KEY_IGTK,
	ENCRYPTION_KEY_BIGTK,
	ENCRYPTION_KEY_GTK,
	ENCRYPTION_KEY_NUM
};
struct NanPairingKeyInfo {
	uint8_t cipher_suite_id;
	/// key length (0 implies not applicable).
	uint8_t key_length;
	uint8_t key[WFPAL_NAN_PAIRING_KEY_MAX_LENGTH];
} PACKED;
struct NanPairingSetKey {
	uint8_t peer_address[MAC_ADDR_LEN];
	struct NanPairingKeyInfo	nm_tk;
	struct NanPairingKeyInfo	nm_kek;
	struct NanPairingKeyInfo	nm_kck;
	struct NanPairingKeyInfo	nd_tk;
	struct NanPairingKeyInfo	igtk;
	struct NanPairingKeyInfo	bigtk;
	struct NanPairingKeyInfo	gtk;
} PACKED;
#endif /* CFG_SUPPORT_NAN_R4_VENDOR_PAIRING */


enum ENUM_NAN_REPORT_P2P_STATE {
	/// WiFi Direct is not working
	ENUM_NAN_P2P_STATE_DISABLED = 0,
	/// WiFi Direct is listening for incoming connection
	ENUM_NAN_P2P_STATE_LISTENING = 1,
	/// WiFi Direct data path established
	ENUM_NAN_P2P_STATE_OPERATING = 2,
} PACKED;
...
struct NanP2pStateIndMsg {
	struct _NanMsgHeader fwHeader;
	struct NanP2pState rNanP2pState;
} PACKED;

struct NanAwakePeriod {
	uint8_t period_2_4G;
	uint8_t period_5G;
} PACKED;

struct NanAwakePeriodMsg {
	struct _NanMsgHeader fwHeader;
	struct NanAwakePeriod awake_period;
} PACKED;

enum ENUM_NAN_DEVICE_BAND_FLAGS {
	WFPAL_NAN_BAND_TV_WHITE_SPACES = (1 << 0),
	WFPAL_NAN_BAND_SUB_1GHZ_EXCLUDING_TV_WHITE_SPACES = (1 << 1),
	WFPAL_NAN_BAND_GHZ_2_4 = (1 << 2),
	WFPAL_NAN_BAND_GHZ_3_6 = (1 << 3),
	WFPAL_NAN_BAND_GHZ_4_9_to_5_0 = (1 << 4),
	WFPAL_NAN_BAND_GHZ_60 = (1 << 5),
} PACKED;

enum ENUM_NAN_CIPHER_SUITE_FLAGS {
	/// Secure Key CCM 128 bit keys
	NAN_CIPHER_SUITE_SK_CCM_128 = (1 << 0),
	NAN_CIPHER_SUITE_SK_GCM_256 = (1 << 1),
	/// Public Key 2 Way Diffie Hellman CCM 128 bit
	NAN_CIPHER_SUITE_PK_2WDH_CCM_128 = (1 << 2),
	/// Public Key 2 Way Diffie Hellman GCM 128 bit
	NAN_CIPHER_SUITE_PK_2WDH_GCM_256 = (1 << 3),
} PACKED;

enum ENUM_NAN_OPERATION_MODE {
	WFPAL_NAN_OPERATION_MODE_VHT = (1 << 0),
	WFPAL_NAN_OPERATION_MODE_VHT80_80 = (1 << 1),
	WFPAL_NAN_OPERATION_MODE_VHT_160 = (1 << 2),
	WFPAL_NAN_OPERATION_MODE_NDL_PAGING = (1 << 3),
} PACKED;

struct NanDeviceCapabilities {
	enum ENUM_NAN_DEVICE_BAND_FLAGS supported_bands;
	enum ENUM_NAN_CIPHER_SUITE_FLAGS supported_cipher_suites;
	enum ENUM_NAN_OPERATION_MODE operation_mode_ht_vht;
	/* higher 4 bits is the number of rx antennas,
	 * lower 4 bits is the number of tx antennas
	 */
	...
	/* NAN restore support on chip-reset. 1 = true, 0 = false. */
	uint8_t unicast_insecure_datapath_restore_supported;
	uint8_t nan_host_pairing;
	/* MAX quota size for key storage. unit: byte */
	uint32_t max_key_storage_size;
} PACKED;

struct NanDeviceCapabilitiesMsg {
	struct _NanMsgHeader fwHeader;
	struct NanDeviceCapabilities rDeviceCapabilities;
} PACKED;

struct NanClusterIdMsg {
	struct _NanMsgHeader fwHeader;
	uint8_t cluster_id[MAC_ADDR_LEN];
} PACKED;

struct NanMastPreferenceMsg {
	struct _NanMsgHeader fwHeader;
	uint8_t mast_preference;
} PACKED;

struct NanRandomFactorMsg {
	struct _NanMsgHeader fwHeader;
...
enum NanMsgId {
	...
	NAN_MSG_ID_OOB_AF_TX_STATUS_IND = 65,
	NAN_MSG_ID_SET_OOB_AF_TX_PARAMS = 66,
	NAN_MSG_ID_SET_PAIRING_KEY = 67,
	NAN_MSG_ID_VENDOR_FOLLOWUP_TX_IND = 68,
	NAN_MSG_ID_VENDOR_FOLLOWUP_RX_IND = 69,
	NAN_MSG_ID_VENDOR_WFD_STATUS_IND = 70,
	NAN_MSG_ID_VENDOR_GET_WFD_STATUS = 71,
	NAN_MSG_ID_VENDOR_SET_AWAKE_PERIOD = 72,
	NAN_MSG_ID_VENDOR_GET_AWAKE_PERIOD = 73,
	NAN_MSG_ID_GET_DEVICE_CAPABILITIES = 74,
	NAN_MSG_ID_GET_CLUSTER_ID          = 75,
	NAN_MSG_ID_GET_MAST_PREFERENCE     = 76,
	NAN_MSG_ID_GET_RANDOM_FACTOR       = 77,
...
enum NanTlvType {
	...
	NAN_TLV_CUSTOM_ATTR = 30,
	NAN_TLV_TYPE_SHARED_KEY_DESCRIPTOR = 31,
...
#define SDEA_CTRL_PARMS_GTK_REQUIRED BIT(10)
#define SDEA_CTRL_PARMS_RESERVED_1 BITS(11, 15)
...
#define GET_SDEA_SERVICE_UPDATE_IND_PRESENT(flags)                             \
	((flags & SDEA_CTRL_PARMS_SERVICE_UPDATE_IND_PRESENT) >>               \
	 9) /* eServUpdateInd */
#define GET_SDEA_SERVICE_GTK_REQUIRED(flags)                                   \
	((flags & SDEA_CTRL_PARMS_GTK_REQUIRED) >> 10) /* fgGtkRequired  */

...
#if defined(_HIF_SDIO) || defined(_HIF_PCIE)
...
#define RSIZE_MAX (16 * 1024 * 1024)

#define kalMemCpyS(pvDst, szDst, pvSrc, szSrc) \
	_kalMemCpyS(pvDst, szDst, pvSrc, szSrc,\
...
/*
 * the skb data size which equal to
 * the largest MPDU size.
 * in fact it will use 4K slab.
 */
#define SKB_SIZE_4K_SLAB CFG_RX_MAX_MPDU_SIZE

/*
 * Ethernet data packet usually has length
 * about 1500 as ethernet MTU is usually not
 * larger than 1500.
 * Give very large numbers of 2K size slab
 * for throughput performance.
 */
#define SKB_SIZE_1K_SLAB_NUM 0
#define SKB_SIZE_4K_SLAB_NUM 16
#define SKB_SIZE_2K_SLAB_NUM (CFG_RX_MAX_PKT_NUM \
				- SKB_SIZE_4K_SLAB_NUM \
				- SKB_SIZE_1K_SLAB_NUM)

enum ENUM_SKB_SIZE_TYPES {
	SKB_SIZE_TYPES_1K_SLAB = 0,
	SKB_SIZE_TYPES_2K_SLAB,
	SKB_SIZE_TYPES_4K_SLAB,
	SKB_SIZE_TYPES_NUM
};
...
#define IEEE80211_CHAN_NO_6GHZ_VLP_CLIENT      (1<<22)
#define IEEE80211_CHAN_NO_6GHZ_AFC_CLIENT      (1<<23)
...
/* Consider on some Android platform, using request_firmware()
 * may cause system get into kobject uevent fallback mechanism.
 * This mechanism will cause system to sleep for a long time about 60s,
 * So we use request_firmware_direct().
 */
...
#define KAL_WAKE_LOCK_TIMEOUT_FORCED(_prAdapter, _prWakeLock, _u4Timeout) ({ \
	if (_prWakeLock) { \
		__pm_wakeup_event(_prWakeLock, _u4Timeout); \
	} \
})
...
#define kalMemCopy(pvDst, pvSrc, u4Size) ({ \
	void *pvAddr; \
	if (u4Size != 0) \
		pvAddr = memcpy(pvDst, pvSrc, u4Size); \
	else \
		pvAddr = pvDst; \
	pvAddr; \
})
...
void kalAntSwDivSwitchCtrl(IN struct ADAPTER *prAdapter,
	uint8_t ucForce);
uint32_t kalAntSwDivDynamicSwitch(IN struct ADAPTER *prAdapter,
	int32_t i4Rssi);
#endif
...
void kalProcessRttReportDone(struct ADAPTER *prAdapter);

...
static void halTxUSBRecycleReqInIRQ(struct USB_REQ *prUsbReq);
#endif
...
uint32_t halTxUSBSendCmd(IN struct GLUE_INFO *prGlueInfo, IN uint8_t ucTc, IN struct CMD_INFO *prCmdInfo)
{
	...
	ucPktFormat = HAL_MAC_CONNAC2X_TXD_GET_PKT_FORMAT(
	(struct HW_MAC_CONNAC2X_TX_DESC *)((uint8_t *)(prBufCtrl->pucBuf) +
				prChipInfo->u2HifTxdSize));

	if ((ucTc != TC4_INDEX) && (ucPktFormat == TXD_PKT_FORMAT_COMMAND)) {
		DBGLOG(HAL, ERROR, "TX Incorrect TC[%d] for CMD[0x%X]\n",
				ucTc, prCmdInfo->ucCID);
		DBGLOG_MEM32(HAL, ERROR, prBufCtrl->pucBuf,
			(prChipInfo->u2HifTxdSize + prCmdInfo->u4TxdLen));
		ucTc = TC4_INDEX;
	} else if (ucPktFormat != TXD_PKT_FORMAT_COMMAND) {
		DBGLOG(HAL, ERROR, "Incorrect TX PKT_FORMAT %d for CMD[0x%X]\n",
				ucPktFormat, prCmdInfo->ucCID);
		DBGLOG_MEM32(HAL, ERROR, prBufCtrl->pucBuf,
			(prChipInfo->u2HifTxdSize + prCmdInfo->u4TxdLen));
		return WLAN_STATUS_FAILURE;
	}
	DBGLOG(HAL, LOUD, "TX TC[%d] PKT_FORMAT %d for CMD[0x%X]\n",
			ucTc, ucPktFormat, prCmdInfo->ucCID);
...
#if (CFG_USB_RECYCLE_REQ_IN_IRQ == 1)
		halTxUSBRecycleReqInIRQ(prUsbReq);
#else
...
static void halTxUSBRecycleReqInIRQ(struct USB_REQ *prUsbReq)
{
	...
	/* free skb in bottom half */
	ucTc = *((uint8_t *)&prUsbReq->prPriv) & TC_MASK;
	prHifInfo->u4TxCompleteTcMap |= BIT(ucTc);
	QUEUE_CONCATENATE_QUEUES(
		&prHifInfo->rTxCompleteMsduList,
		&prUsbReq->rSendingDataMsduInfoList);
	/* recycle USB_REQ in irq */
...
#if (CFG_USB_RECYCLE_REQ_IN_IRQ == 1)
	spin_lock(&prHifInfo->rTxDataQLock);
	halTxUSBRecycleReqInIRQ(prUsbReq);
	spin_unlock(&prHifInfo->rTxDataQLock);
#else
...
#if CFG_SUPPORT_MULTI_SKB_SIZE
			pFreeRfbList = getFreeRfbList(prAdapter,
				u2RxByteCount, u4MinRfbCnt);
			if (pFreeRfbList)
				QUEUE_REMOVE_HEAD(pFreeRfbList,
...
void halWakeUpWiFi(IN struct ADAPTER *prAdapter)
{
	...
	if ((prHifInfo == NULL) || (prChipInfo == NULL)) {
		DBGLOG(INIT, ERROR, "prHifInfo or prChipInfo is NULL!\n");
		return;
	}
...
#else /* CFG_USB_DRIVEROWN_POLLING_BY_ACKCR */
	struct GL_HIF_INFO *prHifInfo;
	u_int8_t fgResult = FALSE;
	uint8_t ucCount = 0;
...
bool halHifSwInfoInit(IN struct ADAPTER *prAdapter)
{
	...
	if (prBusInfo->asicUdmaInit)
		prBusInfo->asicUdmaInit(prAdapter);
...
void halTxCompleteTasklet(unsigned long data)
{
	...
	QUEUE_MOVE_ALL(&rFreeQueue,
		&prHifInfo->rTxCompleteMsduList);

	for (ucTc = 0; ucTc < USB_TC_NUM; ucTc++) {
		if ((BIT(ucTc) & prHifInfo->u4TxCompleteTcMap) == 0)
			continue;

		if (!usb_anchor_empty(&prHifInfo->rTxDataAnchor[ucTc]))
			continue;

		prUsbReq = list_entry(prHifInfo->rTxDataFreeQ[ucTc].next,
			struct USB_REQ, list);
		if (prUsbReq->prBufCtrl->u4WrIdx == 0)
			continue;
	...
	nicTxMsduDoneCb(prGlueInfo, &rFreeQueue);
#else

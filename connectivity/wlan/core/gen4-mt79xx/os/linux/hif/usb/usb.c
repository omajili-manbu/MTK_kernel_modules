...
#if (CFG_USB_SHUTDOWN_VENDOR_CMD_WITH_WIDX == 0)
#define WIFI_POWER_OFF_WIDX     (0X00)
#else
#define WIFI_POWER_OFF_WIDX     (0X02)
#endif
#endif
...
#endif /* MT7902 */
#ifdef MT7926
	{	USB_DEVICE_AND_INTERFACE_INFO(0x0E8D, 0x7926, 0xff, 0xff, 0xff),
		.driver_info = (kernel_ulong_t)&mt66xx_driver_data_mt7926},
#endif /* MT7926 */
#ifdef MT6639
...
static int mtk_usb_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
	...
	if (g_fgDriverProbed &&
		wlanSearchDevIdx(prDev) < CFG_MAX_WLAN_DEVICES)
#else
	if (g_fgDriverProbed)
#endif
	{
...
static void mtk_usb_disconnect(struct usb_interface *intf)
{
	...
	while (prGlueInfo->ePoweredoffWifiFlag
		== WIFI_STATE_POWER_OFF_START) {

		if (u1Count > POWER_OFF_WIFI_MAX_WAIT_TIME
		|| (prGlueInfo->ePoweredoffWifiFlag
			== WIFI_STATE_POWER_OFF_DONE)) {
			DBGLOG_LIMITED(HAL, ERROR, "skip wait poweroff done\n");
			break;
		}

		kalUsleep_range(5000, 6000);
		u1Count++;
		DBGLOG_LIMITED(HAL, EVENT, "wait poweroff done %d\n", u1Count);
...
static int mtk_usb_suspend(struct usb_interface *intf, pm_message_t message)
{
	...
	if (prGlueInfo->fgTestL0p5InSuspend == TRUE) {
	...
	*/
	...
	cnmTimerStopTimer(prGlueInfo->prAdapter,
		&SER_SYN_TIMER(prGlueInfo->prAdapter));
...
void mtk_usb_shutdown_vnd_cmd(struct GLUE_INFO *prGlueInfo)
{
	...
	if (prGlueInfo) {
		...
		if (prGlueInfo->prAdapter) {
			HAL_MCR_RD(prGlueInfo->prAdapter, WIFI_POWER_OFF_DONE,
						&u4Data);
			...
			HAL_MCR_WR(prGlueInfo->prAdapter, WIFI_POWER_OFF_DONE,
						u4Data);
		}
#endif
		ret = usb_control_msg(prGlueInfo->rHifInfo.udev,
				...
				0, WIFI_POWER_OFF_WIDX, NULL, 0, 100);
...
void glSetHifInfo(struct GLUE_INFO *prGlueInfo, unsigned long ulCookie)
{
	...
	/* TX Data */
#if (CFG_USB_RECYCLE_REQ_IN_IRQ == 1)
	QUEUE_INITIALIZE(&prHifInfo->rTxCompleteMsduList);
	prHifInfo->u4TxCompleteTcMap = 0;
#endif
	for (ucTc = 0; ucTc < USB_TC_NUM; ++ucTc) {
...
void glClearHifInfo(struct GLUE_INFO *prGlueInfo)
{
	...
	spin_lock_irqsave(&prHifInfo->rTxDataQLock, flags);
	{
		for (ucTc = 0; ucTc < USB_TC_NUM; ++ucTc) {
			if (ucTc >= TC4_INDEX && ucTc < USB_DBDC1_TC)
				continue;
			list_for_each_entry_safe(prUsbReq, prUsbReqNext,
						 &prHifInfo->rTxDataFreeQ[ucTc],
						 list) {
#ifndef CFG_PREALLOC_MEMORY
				if (prUsbReq && prUsbReq->prBufCtrl)
		...
		list_for_each_entry_safe(prUsbReq, prUsbReqNext,
					 &prHifInfo->rTxDataCompleteQ, list) {
#ifndef CFG_PREALLOC_MEMORY
			if (prUsbReq && prUsbReq->prBufCtrl)
				kfree(prUsbReq->prBufCtrl->pucBuf);
#endif
	...
	spin_lock_irqsave(&prHifInfo->rTxCmdQLock, flags);
	{
		...
		list_for_each_entry_safe(prUsbReq, prUsbReqNext,
					 &prHifInfo->rTxCmdCompleteQ, list) {
#ifndef CFG_PREALLOC_MEMORY
			if (prUsbReq && prUsbReq->prBufCtrl)
				kfree(prUsbReq->prBufCtrl->pucBuf);
#endif
	...
	/* RX DATA */
	spin_lock_irqsave(&prHifInfo->rRxDataQLock, flags);
	{
		list_for_each_entry_safe(prUsbReq, prUsbReqNext,
					 &prHifInfo->rRxDataFreeQ, list) {
#ifndef CFG_PREALLOC_MEMORY
			if (prUsbReq && prUsbReq->prBufCtrl)
				kfree(prUsbReq->prBufCtrl->pucBuf);
#endif
		...
		list_for_each_entry_safe(prUsbReq, prUsbReqNext,
					 &prHifInfo->rRxDataCompleteQ, list) {
#ifndef CFG_PREALLOC_MEMORY
			if (prUsbReq && prUsbReq->prBufCtrl)
				kfree(prUsbReq->prBufCtrl->pucBuf);
#endif
	...
	spin_unlock_irqrestore(&prHifInfo->rRxDataQLock, flags);

	/* RX EVENT */
	spin_lock_irqsave(&prHifInfo->rRxEventQLock, flags);
	{
		list_for_each_entry_safe(prUsbReq, prUsbReqNext,
					 &prHifInfo->rRxEventFreeQ, list) {
#ifndef CFG_PREALLOC_MEMORY
			if (prUsbReq && prUsbReq->prBufCtrl)
				kfree(prUsbReq->prBufCtrl->pucBuf);
#endif
		...
		list_for_each_entry_safe(prUsbReq, prUsbReqNext,
					 &prHifInfo->rRxEventCompleteQ, list) {
#ifndef CFG_PREALLOC_MEMORY
			if (prUsbReq && prUsbReq->prBufCtrl)
				kfree(prUsbReq->prBufCtrl->pucBuf);
#endif
	...
	spin_unlock_irqrestore(&prHifInfo->rRxEventQLock, flags);

	/* RX WDT */
#if CFG_CHIP_RESET_SUPPORT
	if (prChipInfo->fgIsSupportL0p5Reset && prBusInfo->fgIsSupportWdtEp) {
		spin_lock_irqsave(&prHifInfo->rRxWdtQLock, flags);
		{
			list_for_each_entry_safe(prUsbReq, prUsbReqNext,
						 &prHifInfo->rRxWdtFreeQ,
						 list) {
		...
		spin_unlock_irqrestore(&prHifInfo->rRxWdtQLock, flags);
	}
#endif
...
uint32_t glGetBusId(void *ctx)
{
	...
	if (!ctx) {
		DBGLOG(HAL, ERROR, "ctx is NULL\n");
		return 0;
	}
	dev = interface_to_usbdev((struct usb_interface *) ctx);
	if (dev == NULL || dev->bus == NULL) {
		DBGLOG(HAL, ERROR, "fail to find usb bus\n");
		return 0;
	}
	return (dev->bus->busnum << 8) | (dev->portnum);
}
		      ...
		      int8_t **pprArgs, int8_t *prArgv_size, int32_t i4Nargs)
{
	...
	struct GLUE_INFO *prGlueInfo = NULL;
	struct usb_device *prUsbDev = NULL;
	...
	prUsbDev = prGlueInfo->rHifInfo.udev;

	u4Ret = kalkStrtou32(pprArgs[DEV_CFG_BUS_NUM], 0, &u4Bus);
	if (u4Ret)
		return WLAN_STATUS_FAILURE;

	u4Ret = kalkStrtou32(pprArgs[DEV_CFG_PORT_NUM], 0, &u4Port);
	if (u4Ret)
		return WLAN_STATUS_FAILURE;

	if (u4Bus  != prUsbDev->bus->busnum ||
		u4Port != prUsbDev->portnum) {
		return WLAN_STATUS_FAILURE;
	}

...
#if CFG_CHIP_RESET_KO_SUPPORT
	bus_id = glGetBusId(pdev);
#if CFG_SUPPORT_MULTI_CARD
	update_hif_info(bus_id, HIF_INFO_PCIE_HOST, &bus_id);
#else
	update_hif_info(HIF_INFO_PCIE_HOST, &bus_id);
#endif
#endif
...
#if (CFG_SUPPORT_PCIE_DISABLE_ASPM_IN_SUSPEND_FLOW == 1)
	prHifInfo->fgNeedDisableAspm = TRUE;
#endif
	/* FW own */
...
#if (CFG_SUPPORT_PCIE_DISABLE_ASPM_IN_SUSPEND_FLOW == 1)
	prHifInfo = &prGlueInfo->rHifInfo;
	prHifInfo->fgNeedDisableAspm = FALSE;
#endif
...
uint32_t glRegisterBus(probe_card pfProbe, remove_card pfRemove)
{
	...
	DBGLOG(INIT, INFO,
		"register pci driver when insmod wifi ko\n");
	ret = (pci_register_driver(&mtk_pci_driver) == 0) ?
		WLAN_STATUS_SUCCESS : WLAN_STATUS_FAILURE;

#if CFG_CONN_DYNAMIC_POWER_CTRL
	DBGLOG(INIT, INFO, "unregister pci driver when insmod wifi ko\n");
...
void glUnregisterBus(remove_card pfRemove)
{
	...
	for (u4DevIdx = 0; u4DevIdx < CFG_MAX_WLAN_DEVICES; u4DevIdx++) {
		...
		pdev = container_of(prGlueInfo->prDev, struct pci_dev, dev);
...
void glClearHifInfo(struct GLUE_INFO *prGlueInfo)
{
	...
	/* Use temporary list to minimize spin lock time */
	INIT_LIST_HEAD(&rTempList);

	KAL_ACQUIRE_SPIN_LOCK(prGlueInfo->prAdapter, SPIN_LOCK_TX_DATA_QUE);
	list_replace_init(&prHifInfo->rTxDataQ, &rTempList);
	prHifInfo->u4TxDataQLen = 0;
	KAL_RELEASE_SPIN_LOCK(prGlueInfo->prAdapter, SPIN_LOCK_TX_DATA_QUE);

	/* Process the list outside of spin lock */
...
#if CFG_SUPPORT_MULTI_CARD
	if (u4WlanDevNum >= CFG_MAX_WLAN_DEVICES)
...
{
	uint32_t u4Bus, u4Dev, u4Func, u4Domain;
	...
	u4Domain = pci_domain_nr(pdev->bus) & 0xFFFF;
	DBGLOG(INIT, INFO, "u4Domain=%d\n", u4Domain);
	u4Bus = (pdev->bus->number) & 0xFF;
	DBGLOG(INIT, INFO, "u4Bus=%d\n", u4Bus);
	u4Dev = PCI_SLOT(pdev->devfn) & 0x1F;
	u4Func = PCI_FUNC(pdev->devfn) & 0x07;

	return (u4Domain << 16) | (u4Bus << 8) | (u4Dev << 3) | u4Func;
}

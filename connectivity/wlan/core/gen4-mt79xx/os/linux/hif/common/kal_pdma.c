			     ...
			     uint16_t u2Port)
{
	...
	for (u4Count = 0; pRxD->DMADONE == 0; u4Count++) {
		...
		if (u4Count > DMA_DONE_WAITING_COUNT) {
			DBGLOG(HAL, INFO,
				   "Rx DMA done P[%u] CPU[%u]\n",
				   u2Port, prRxRing->RxCpuIdx);
			return false;
		}
#else
...
static bool kalDevKickMsduData(struct GLUE_INFO *prGlueInfo)
{
	...
	if (!fgRet && !list_empty(&rTempList)) {
		KAL_ACQUIRE_SPIN_LOCK(prGlueInfo->prAdapter,
			SPIN_LOCK_TX_DATA_QUE);
		list_splice_init(&rTempList, &prHifInfo->rTxDataQ);
...
static bool kalDevKickAmsduData(struct GLUE_INFO *prGlueInfo)
{
	...
	list_sort((void *)prGlueInfo, &rTempList, kalAmsduTxDCmp);

	for (prCur = rTempList.next; prCur != &rTempList; prCur = prNext) {
		u4Num = kalGetNumOfAmsdu(prGlueInfo, prCur,
						&rTempList, &u2Size);
		prNext = prCur->next;
		if (u4Num > 1) {

		...
		enum ENUM_WIFI_RSV_MEM_IDX u4RsvMemIdx)
{
	...
	if (ret) {
		...
		DBGLOG(INIT, INFO, "memory region fouund\n");
	}
...
#if CFG_SUPPORT_WIFI_RSV_MEM || CFG_SUPPORT_RX_PAGE_POOL_USE_CMA
...
static void pcieRsvMemAllocTxDataBuf(struct MSDU_TOKEN_ENTRY *prToken,
					uint32_t u4Idx)
{
	...
	struct GLUE_INFO *prGlueInfo, uint8_t **ppucData)
{
	...
	if (!page) {
		DBGLOG(HAL, ERROR, "fail to allocate skb from page pool\n");
		goto fail;
	}
...
u_int8_t kalCreateHifSkbList(struct GLUE_INFO *prGlueInfo)
{
	struct sk_buff_head *prHifSkbList = &prGlueInfo->rHifSkbList;
...
#if (CFG_SUPPORT_HOST_RX_WM_EVENT_FROM_PSE == 1)
			u4Num += MT7961_HOST_RX_WM_EVENT_FROM_PSE_RX_RING4_SIZE;
#else
			u4Num += RX_RING1_SIZE;
#endif
		} else if (u4Idx == WFDMA1_RX_RING_IDX_0) {
	...
	skb_queue_head_init(prHifSkbList);

	for (u4Idx = 0; u4Idx < u4Num; u4Idx++) {
		prSkb = kalAllocRxSkbFromCmaPp(prGlueInfo, &pucRecvBuff);
		if (!prSkb) {
			DBGLOG(HAL, ERROR, "hif skb reserve fail[%u]!\n",
			       u4Idx);
			kalReleaseHifSkbList(prGlueInfo);
			fgRet = FALSE;
			goto exit;
		}
		skb_queue_tail(prHifSkbList, prSkb);
	}
	len =  skb_queue_len(prHifSkbList);
...
void kalReleaseHifSkbList(struct GLUE_INFO *prGlueInfo)
{
	struct sk_buff *prSkb;
	struct sk_buff_head *prHifSkbList = &prGlueInfo->rHifSkbList;

	DBGLOG(HAL, INFO, "release all reserved hif skb!\n");
	while (!skb_queue_empty(prHifSkbList)) {
		prSkb = skb_dequeue(prHifSkbList);
		if (!prSkb) {
...
static struct sk_buff *kalAllocHifSkb(struct GLUE_INFO *prGlueInfo)
{
	struct sk_buff_head *prHifSkbList = &prGlueInfo->rHifSkbList;
	uint32_t len =  skb_queue_len(prHifSkbList);

	DBGLOG(HAL, INFO, "skb_queue_len [%u]!\n", len);
	if (skb_queue_empty(prHifSkbList)) {
		DBGLOG(HAL, ERROR, "hif skb list is empty\n");
		return NULL;
	}

	return skb_dequeue(prHifSkbList);
}

static void kalFreeHifSkb(struct GLUE_INFO *prGlueInfo, struct sk_buff *prSkb)
{
	struct sk_buff_head *prHifSkbList = &prGlueInfo->rHifSkbList;
					...
					uint32_t u4Num, uint32_t u4Idx)
{
	...
	prSkb = kalAllocHifSkb(prGlueInfo);
	if (!prSkb) {
					...
					void *pvPacket)
{
	...
	kalFreeHifSkb(prGlueInfo, (struct sk_buff *)pvPacket);
}

#endif /* CFG_SUPPORT_RX_PAGE_POOL_USE_CMA */

#if (CFG_SUPPORT_WIFI_RSV_MEM == 0)
...
#if CFG_SUPPORT_RX_PAGE_POOL
	pkt = (struct sk_buff *) kalAllocRxSkbFromPp(
						prHifInfo->prGlueInfo,
						(uint8_t **)&prDmaBuf->AllocVa,
						-1);
#else
	pkt = (struct sk_buff *) kalPacketAlloc(
						prHifInfo->prGlueInfo,
...
#endif /* CFG_SUPPORT_WIFI_RSV_MEM == 0 */

#if (CFG_SUPPORT_WIFI_RSV_MEM == 0) || CFG_SUPPORT_RX_PAGE_POOL_USE_CMA
...
#if CFG_SUPPORT_RX_PAGE_POOL_USE_CMA
	if (!prSkb->pp_recycle) {
		pcieRsvMemCopyRxData(prHifInfo, pRxCell, prDmaBuf, prSwRfb);
		goto dma_map;
	}
#endif /* CFG_SUPPORT_RX_PAGE_POOL_USE_CMA */
		       ...
		       uint32_t u4Idx, uint32_t u4DumpLen)
{
	...
	prDmaBuf->AllocPa = pcieMapRxBuf(prHifInfo,
						     prDmaBuf->AllocVa,
...
int halSetMemOps(struct HIF_MEM_OPS *prMemOps)
{

...
#if CFG_MTK_WIFI_DRV_OWN_MODE_CONTROL
		if (prAdapter->rWifiVar.u4DrvOwnMode == 0) {
			if (!prBusInfo->fgCheckDriverOwnInt ||
				test_bit(GLUE_FLAG_INT_BIT,
						&prAdapter->prGlueInfo->ulFlag))
				HAL_LP_OWN_RD(prAdapter, &fgResult);
		} else { /* (prAdapter->rWifiVar.u4DrvOwnMode == 1) */
			HAL_LP_OWN_RD(prAdapter, &fgResult);
		}
#else
		if (!prBusInfo->fgCheckDriverOwnInt ||
			...
			HAL_LP_OWN_RD(prAdapter, &fgResult);
#endif  /* CFG_MTK_WIFI_DRV_OWN_MODE_CONTROL */
...
/*----------------------------------------------------------------------------*/
/*!
 * \brief This routine is used to get WFDMA ring usable count.
 *
 * \param[in] pvAdapter Pointer to the Adapter structure.
 *
 * \return count of usable packets
 */
/*----------------------------------------------------------------------------*/
...
void halSetFWOwn(IN struct ADAPTER *prAdapter, IN u_int8_t fgEnableGlobalInt)
{
...
#endif
		if (prAdapter->u4PwrCtrlBlockCnt != 0) {
			DBGLOG(INIT, TRACE, "PwrCtrlBlockCnt = %d\n",
	...
	if (nicSerIsWaitingReset(prAdapter)) {
		DBGLOG(INIT, INFO,
		  "FW OWN Skipped due to  waiting L1 reset\n");
		/* waiting L1 reset */
		goto unlock;
	}
	...
	if (!prHifInfo->fgIsPowerOff && halGetWfdmaRxCnt(prAdapter)) {
		DBGLOG(INIT, INFO,
		  "FW OWN Skipped due to pending INT!\n");
		/* pending interrupts */
		goto unlock;
	}
...
#if !CFG_CONTROL_ASPM_BY_FW
#if CFG_SUPPORT_PCIE_ASPM
#if (CFG_SUPPORT_PCIE_DISABLE_ASPM_IN_SUSPEND_FLOW == 1)
	if (prHifInfo->fgNeedDisableAspm == FALSE) {
#endif
...
void halInitMsduTokenInfo(IN struct ADAPTER *prAdapter)
{
	...
	for (u4Idx = 0; u4Idx < HIF_TX_MSDU_TOKEN_NUM; u4Idx++) {
		...
		prToken->ucStaRecIdx = STA_REC_INDEX_NOT_FOUND;
#endif
	...
	struct MSDU_INFO *prMsduInfo)
{
	...
	if (!prMsduInfo) {
		DBGLOG(HAL, ERROR, "MsduInfo is NULL!\n");
		return NULL;
	}
...
#if CFG_SUPPORT_SAP_TX_FAIRNESS
	if (IS_BSS_INDEX_APGO(prAdapter, prMsduInfo->ucBssIndex)) {
		if (prMsduInfo->ucStaRecIndex < CFG_MAX_TXQ_NUM) {
			prToken->ucStaRecIdx = prMsduInfo->ucStaRecIndex;
			GLUE_INC_REF_CNT(prAdapter->prGlueInfo->
				au4TxPendingFrameNumPerSta
				[prMsduInfo->ucBssIndex][prToken->ucStaRecIdx]);
		}
	}
#endif
...
void halReturnMsduToken(IN struct ADAPTER *prAdapter, uint32_t u4TokenNum)
{
	...
	uint8_t ucStaRecIdx = 0, ucBssIdx = 0;
#endif
	unsigned long flags = 0;
#if CFG_SUPPORT_PCIE_ASPM_IMPROVE
	struct BUS_INFO *prBusInfo = NULL;
	...
	prToken->prMsduInfo = NULL;
#if CFG_SUPPORT_SAP_TX_FAIRNESS
	ucStaRecIdx = prToken->ucStaRecIdx;
	prToken->ucStaRecIdx = STA_REC_INDEX_NOT_FOUND;
	prStaRec = cnmGetStaRecByIndex(prAdapter, ucStaRecIdx);
	if (prStaRec) {
		ucBssIdx = prStaRec->ucBssIndex;
		if (IS_BSS_INDEX_APGO(prAdapter, ucBssIdx) &&
			GLUE_GET_REF_CNT(prAdapter->prGlueInfo->
				au4TxPendingFrameNumPerSta
				[ucBssIdx][ucStaRecIdx]) > 0) {
			GLUE_DEC_REF_CNT(prAdapter->prGlueInfo->
				au4TxPendingFrameNumPerSta
				[ucBssIdx][ucStaRecIdx]);
			/* netif awake stopped queue to resume transmission  */
			kalAwakeQueue(prAdapter->prGlueInfo, ucStaRecIdx);
		}
	}
#endif
	prTokenInfo->u4UsedCnt--;
...
u_int8_t halIsWfdmaRxReady(struct RTMP_RX_RING *prRxRing,
				  uint32_t u4CpuIdx)
{
...
u_int8_t halIsWfdmaTxReady(struct RTMP_TX_RING *prTxRing)
{
	...
	return pTxD->DMADONE ? TRUE : FALSE;
}

#endif /* CFG_SUPPORT_IPC_WITHOUT_MMIO_READ */
...
void halWpdmaInitTxRing(IN struct GLUE_INFO *prGlueInfo)
{
	...
	for (i = 0; i < NUM_OF_TX_RING; i++) {
		...
		if (prBusInfo->ring_cnt_base_ext_ctrl) {
			phy_addr_ext = (((uint64_t)prTxCell->AllocPa >>
				DMA_BITS_OFFSET) & DMA_HIGHER_4BITS_MASK);

			u4Cnt = TX_RING_SIZE;

			prBusInfo->ring_cnt_base_ext_ctrl(
				prGlueInfo, prTxRing->hw_cnt_addr,
				phy_addr_ext, u4Cnt);
		} else {
#if CFG_SUPPORT_IPC_WITHOUT_MMIO_READ
			kalDevRegRead(prGlueInfo,
				prTxRing->hw_cnt_addr, &u4Cnt);
#endif
			u4Cnt &= ~0xfff;
...
void halWpdmaInitRxRing(IN struct GLUE_INFO *prGlueInfo)
{
	...
	for (i = 0; i < NUM_OF_RX_RING; i++) {
		...
		if (prBusInfo->ring_cnt_base_ext_ctrl) {
			...
			prBusInfo->ring_cnt_base_ext_ctrl(
				prGlueInfo, prRxRing->hw_cnt_addr,
				phy_addr_ext, prRxRing->u4RingSize);
		} else {
#if CFG_SUPPORT_IPC_WITHOUT_MMIO_READ
			kalDevRegRead(prGlueInfo,
				prRxRing->hw_cnt_addr, &u4Cnt);
#endif
			u4Cnt &= ~0xfff;
	...
	IN uint16_t u2Port)
{
	...
	while (prTxRing->u4UsedCnt) {
		...
		INC_RING_INDEX(u4SwIdx, u4RingSize);
	}
	...
	IN uint8_t ucRingNum)
{
	...
	u4RxPktCnt = 0;
	INC_RING_INDEX(u4CpuIdx, u4MaxCnt);
		       ...
		       struct list_head *prCurList)
{
	...
	/* Acquire MSDU token */
	prToken = halAcquireMsduToken(prGlueInfo->prAdapter, prMsduInfo);
	if (!prToken) {
			...
			uint32_t u4Num, uint16_t u2Size)
{
	...
	for (u4Idx = 0; u4Idx < u4Num; u4Idx++) {
		...
		/* Acquire MSDU token */
		prToken = halAcquireMsduToken(
			prGlueInfo->prAdapter, prMsduInfo);
		if (!prToken) {
...
void halHwRecoveryFromError(IN struct ADAPTER *prAdapter)
{
	...
	case ERR_RECOV_STOP_PDMA0:
		if (u4Status & ERROR_DETECT_RESET_DONE) {
			...
			/* Initialize wfdma re-init handshake parameters */
			if (prChipInfo->asicWfdmaReInit_handshakeInit)
				prChipInfo->
				asicWfdmaReInit_handshakeInit(prAdapter);
...
uint32_t halToggleWfsysRst(IN struct ADAPTER *prAdapter)
{
	...
	if ((prAdapter == NULL) || (prAdapter->chip_info == NULL)) {
		DBGLOG(INIT, ERROR, "prAdapter or chip_info is NULL!\n");
		return WLAN_STATUS_FAILURE;
	}

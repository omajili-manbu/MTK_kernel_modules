...
int SetATEDa(struct net_device *prNetDev, uint8_t *prInBuf)
{
	...
	/* xx:xx:xx:xx:xx:xx */
	rv = sscanf(prInBuf, "%2x:%2x:%2x:%2x:%2x:%2x", &addr[0],
			   ...
			   uint8_t *prInBuf)
{
	...
	rv = sscanf(prInBuf, "%10u:%10u:%10u:%10u:%10u:%10u:%10u:%10u",
			      ...
			      uint8_t *prInBuf)
{
	...
	rv = sscanf(prInBuf, "%10u:%10u:%10u:%10u:%10u:%10u",
		    &aucInput[0], &aucInput[1], &aucInput[2], &aucInput[3],
			   ...
			   uint8_t *prInBuf)
{
	...
	rv = sscanf(prInBuf, "%10u:%10u:%10u:%10u", &aucInput[0],
			   ...
			   uint8_t *prInBuf)
{
	...
	rv = sscanf(prInBuf, "%10u:%10u", &profileIdx, &fgBFer);
	if (rv == 2) {
		DBGLOG(RFTEST, ERROR,
			    ...
			    uint8_t *prInBuf)
{
	...
	rv = sscanf(prInBuf, "%10u:%10u:%2x:%2x", &profileIdx, &fgBFer,
			     ...
			     uint8_t *prInBuf)
{
	...
	rv = sscanf(prInBuf,
		    "%2x:%4x:%4x:%2x:%4x:%2x:%4x:%2x:%4x:%2x:%4x:%2x:%4x:%2x:%2x:%2x:%2x:%2x",
			   ...
			   uint8_t *prInBuf)
{
	...
	rv = sscanf(prInBuf,
		    "%3u:%5u:%5u:%5u:%5u:%5u:%5u:%5u:%5u:%5u:%5u:%5u:%5u:%5u",
		    ...
		    uint8_t *prInBuf)
{
	...
	rv = sscanf(prInBuf, "%10u:%10u:%10u:%10u", &u4WlanId, &u4ETxBf,
		    &u4ITxBf, &u4MuTxBf);
	if (rv == 4) {
		DBGLOG(RFTEST, ERROR,
			...
			uint8_t *prInBuf)
{
	...
	rv = sscanf(prInBuf,
			"%2x:%2x:%2x:%2x:%2x:%2x:%2x:%3u:%2x:%2x:%2x:%3u:%2x:%2x:%2x:%3u",
			 ...
			 uint8_t *prInBuf)
{
	...
	rv = sscanf(prInBuf, "%10u:%10u", &ucSuMuMode, &ucWlanIdx);
	if (rv == 2) {
		DBGLOG(RFTEST, ERROR,
		      ...
		      uint8_t *prInBuf)
{
	...
	rv = sscanf
	    (prInBuf, "%10u:%2x:%2x:%2x:%2x:%2x:%2x:%10u",
	     &u4OwnMacIdx,
		      ...
		      uint8_t *prInBuf)
{
	...
	rv = sscanf
	     (prInBuf, "%10u:%10u:%2x:%2x:%2x:%2x:%2x:%2x", &u4OwnMacIdx,
		       ...
		       uint8_t *prInBuf)
{
	...
	rv = sscanf(prInBuf,
		    "%8x:%8x:%8x:%8x:%8x:%8x:%8x:%8x:%8x:%8x:%8x:%8x:%8x:%8x:%8x:%8x:%8x:%8x:%8x:%8x:%8x:%8x:%8x",
		     ...
		     uint8_t *prInBuf)
{
	...
	rv = sscanf
	     (prInBuf, "%2x:%2x:%2x:%2x:%2x:%2x:%2x:%2x:%2x", &u4NumOfUser,
		   ...
		   uint8_t *prInBuf)
{
	...
	if (sscanf(prInBuf,
		"%08x:%08x:%08x:%08x:%08x:%08x:%08x:%08x:%08x:%08x:%08x:%08x:%08x:%08x:%08x:%2x:%2x:%2x:%2x:%2x:%2x:%2x:%2x:%2x:%2x:%2x:%2x",
		...
		uint8_t *prInBuf)
{
	...
	if (sscanf(prInBuf, "%2x:%2x", &u4SubcarrierIndex,
		   &u4Length) == 2) {
		DBGLOG(RFTEST, ERROR,
		    ...
		    uint8_t *prInBuf)
{
	...
	if (rv == 6) {
		...
		DBGLOG(RFTEST,
			ERROR,
			"Set_MUSetGID_UP: sscanf failed to parse prInBuf = %s",
			prInBuf);
		return -EINVAL;
	}
		    ...
		    uint8_t *prInBuf)
{
	...
	rv = sscanf(prInBuf,
		  "%3u:%8x:%8x:%8x:%10u:%2x:%2x:%2x:%2x:%2x:%2x:%2x:%2x:%2x:%2x:%2x:%2x",
			      ...
			      uint8_t *prInBuf)
{
	...
	rv = sscanf(prInBuf, "%10u-%10u-%10u-%10u-%10u-%10u", &u4Lm, &u4Nr,
	       ...
	       uint8_t *prInBuf)
{
	...
	rv = sscanf(prInBuf, "%2x:%2x", &addr[0], &addr[1]);

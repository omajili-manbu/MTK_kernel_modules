...
#define MAGNUM_SECEFUSE_CMD 0x53454300
#endif
		   ...
		   OUT uint32_t *pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = sizeof(enum ENUM_PARAM_AUTH_MODE);
	...
	case AUTH_MODE_WPA3_FT_SAE:
		DBGLOG(RSN, TRACE, "New auth mode: FT-SAE\n");
		break;
	case AUTH_MODE_WPA2_FT:
		DBGLOG(RSN, TRACE, "New auth mode: WPA2-FT\n");
		break;
	case AUTH_MODE_WPA2_FT_PSK:
		DBGLOG(RSN, TRACE, "New auth mode: WPA2-FT-PSK\n");
		break;
#endif
	default:
		...
		if (prCmdKey->ucAlgorithmId == CIPHER_SUITE_BIP ||
			...
			if (prCmdKey->ucIsAuthenticator) {
				...
				if (prBssInfo->prStaRecOfAP) {
					...
					DBGLOG(RSN, ERROR, "StaRecOfAP null\n");
			}
...
wlanoidSetRemoveKey(IN struct ADAPTER *prAdapter,
	...
	*pu4SetInfoLen = sizeof(struct PARAM_REMOVE_KEY);
	...
	if (u4KeyIndex >= 4) {
#if (CFG_SUPPORT_802_11W && CFG_MTK_SUPPLICANT_ROAMING_ENHANCE)
		if (IS_BSS_AIS(prBssInfo) &&
			prGlueInfo->u4RoamingEnhanceEnable) {
			struct AIS_SPECIFIC_BSS_INFO *prAisSpecBssInfo =
				aisGetAisSpecBssInfo(prAdapter,
				prRemovedKey->ucBssIdx);
...
wlanoidSetEncryptionStatus(IN struct ADAPTER *prAdapter,
	...
	*pu4SetInfoLen = sizeof(enum ENUM_WEP_STATUS);
	...
	case ENUM_ENCRYPTION3_ENABLED:	/* Enable WEP, TKIP, AES */
		...
		break;
	case ENUM_ENCRYPTION4_ENABLED: /* Enable GCMP256 and CCMP */
		secSetCipherSuite(prAdapter,
				  CIPHER_FLAG_GCMP256 | CIPHER_FLAG_CCMP,
				  ucBssIndex);
...
/*----------------------------------------------------------------------------*/
/*!
 * \brief This routine is called to read sec efuse content.
 *
 * \param[in] pvAdapter Pointer to the Adapter structure.
 * \param[out] pvQueryBuf A pointer to the buffer that holds the result of
 *                           the query.
...
/*----------------------------------------------------------------------------*/
uint32_t
wlanoidQueryProcessAccessSecEfuseRead(IN struct ADAPTER *prAdapter,
				   IN void *pvSetBuffer,
				   IN uint32_t u4SetBufferLen,
				   OUT uint32_t *pu4SetInfoLen)
{
	struct PARAM_CUSTOM_ACCESS_SECEFUSE *prSetAccessEfuseInfo;
	struct CMD_ACCESS_SECEFUSE rCmdSetAccessEfuse;
	uint32_t rWlanStatus = WLAN_STATUS_SUCCESS;
	uint32_t i;

	DEBUGFUNC("wlanoidQueryProcessAccessSecEfuseRead");

	ASSERT(prAdapter);
	ASSERT(pu4SetInfoLen);

	*pu4SetInfoLen = sizeof(struct PARAM_CUSTOM_ACCESS_SECEFUSE);

	if (u4SetBufferLen < sizeof(struct
				    PARAM_CUSTOM_ACCESS_SECEFUSE))
		return WLAN_STATUS_INVALID_LENGTH;

	ASSERT(pvSetBuffer);

	prSetAccessEfuseInfo = (struct PARAM_CUSTOM_ACCESS_SECEFUSE *)
			       pvSetBuffer;

	kalMemSet(&rCmdSetAccessEfuse, 0,
		  sizeof(struct CMD_ACCESS_SECEFUSE));

	rCmdSetAccessEfuse.u4Address =
		prSetAccessEfuseInfo->u4Address;
	rCmdSetAccessEfuse.u4Type = prSetAccessEfuseInfo->u4Type;
	rCmdSetAccessEfuse.u4ReservedValid =
		prSetAccessEfuseInfo->u4ReservedValid;
	rCmdSetAccessEfuse.u4Size = prSetAccessEfuseInfo->u4Size;
	rCmdSetAccessEfuse.u4MagicNum = MAGNUM_SECEFUSE_CMD;
	kalMemCopy(rCmdSetAccessEfuse.aucData,
		   prSetAccessEfuseInfo->aucData,
		   sizeof(prSetAccessEfuseInfo->aucData));

	DBGLOG(OID, INFO, "u4Address:0x%x\n", rCmdSetAccessEfuse.u4Address);
	DBGLOG(OID, INFO, "u4Type:0x%x\n", rCmdSetAccessEfuse.u4Type);
	DBGLOG(OID, INFO, "u4Valid:0x%x\n", rCmdSetAccessEfuse.u4ReservedValid);
	DBGLOG(OID, INFO, "u4Size:0x%x\n", rCmdSetAccessEfuse.u4Size);
	DBGLOG(OID, INFO, "u4MagicNum:0x%x\n", rCmdSetAccessEfuse.u4MagicNum);
	for (i = 0; i < 32; i++)
		DBGLOG(OID, INFO, "aucData[0x%x]:0x%x\n",
			i, rCmdSetAccessEfuse.aucData[i]);

	rWlanStatus = wlanSendSetQueryExtCmd(prAdapter,
			     CMD_ID_LAYER_0_EXT_MAGIC_NUM,
			     EXT_CMD_ID_SECEFUSE_ACCESS,
			     FALSE, /* Query Bit: True->write False->read */
			     TRUE,
			     TRUE,
			     NULL, /* No Tx done function wait until fw ack */
...
/*----------------------------------------------------------------------------*/
/*!
 * \brief This routine is called to write sec efuse content.
 *
 * \param[in] pvAdapter Pointer to the Adapter structure.
 * \param[out] pvQueryBuf A pointer to the buffer that holds the result of
 *                           the query.
 * \param[in] u4QueryBufLen The length of the query buffer.
 * \param[out] pu4SetInfoLen Reserved
 *
 * \retval WLAN_STATUS_SUCCESS
 * \retval WLAN_STATUS_INVALID_LENGTH
 */
/*----------------------------------------------------------------------------*/
uint32_t
wlanoidQueryProcessAccessSecEfuseWrite(IN struct ADAPTER *prAdapter,
				    IN void *pvSetBuffer,
				    IN uint32_t u4SetBufferLen,
				    OUT uint32_t *pu4SetInfoLen)
{
	...
	DEBUGFUNC("wlanoidQueryProcessAccessSecEfuseWrite");
...
wlanoidBssInfoBasic(IN struct ADAPTER *prAdapter,
	...
	*pu4SetInfoLen = sizeof(struct BSSINFO_BASIC);
	...
	prBssinfoBasic = (struct BSSINFO_BASIC *)(pvSetBuffer);
	prBssInfoUpdateBasic->ucBssIndex = prBssinfoBasic->ucBcMcWlanidx;
	DBGLOG(RFTEST, INFO,
...
wlanoidBssInfoConOwnDev(IN struct ADAPTER *prAdapter,
	...
	*pu4SetInfoLen = sizeof(struct BSSINFO_CONNECT_OWN_DEV);
	...
	prBssinfoConOwnDev = (struct BSSINFO_CONNECT_OWN_DEV *)(pvSetBuffer);
	prBssInfoUpdateBasic->ucBssIndex = prBssinfoConOwnDev->ucHwBSSIndex;
	DBGLOG(RFTEST, INFO, "prBssInfoUpdateBasic->ucBssIndex =%d\n",
		prBssInfoUpdateBasic->ucBssIndex);
...
wlanoidDevInfoActive(IN struct ADAPTER *prAdapter,
	...
	*pu4SetInfoLen = sizeof(struct CMD_DEVINFO_ACTIVE);
	...
	prCmdDevinfoActive = (struct CMD_DEVINFO_ACTIVE *)pvSetBuffer;
	prDevInfoUpdateActive->ucOwnMacIdx = prCmdDevinfoActive->ucOwnMacIdx;
	DBGLOG(RFTEST, INFO, "prDevInfoUpdateActive->ucOwnMacIdx = %d\n",
		prDevInfoUpdateActive->ucOwnMacIdx);

	prDevInfoUpdateActive->ucDbdcIdx = prCmdDevinfoActive->ucBandNum;
...
wlanoidSetSwCtrlWrite(IN struct ADAPTER *prAdapter,
	...
	*pu4SetInfoLen = sizeof(struct PARAM_CUSTOM_SW_CTRL_STRUCT);
	...
	switch (u2Id) {
		...
		/* 0x9000 - 0x9EFF reserved for FW */
		/* 0xFFFE          reserved for FW */
#if CFG_SUPPORT_802_11W_DEBUG_SAP_MMIC
	case 0x1111:
		ucBssIndex = u2SubId;
		/*send broadcast deauth frame*/
		authSendDeauthFrame(prAdapter,
			GET_BSS_INFO_BY_INDEX(prAdapter, ucBssIndex),
			NULL, NULL, u4Data, NULL);
		break;
#endif
			   ...
			   OUT uint32_t *pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = u4SetBufferLen;
	 ...
	 * pvSetBuffer == NULL to clear exist Multicast List.
	 */
	/* NOTE2: u4SetBufferLen = 0 will lead to pass 0 to memcpy
	 * and violation MISRA-C coding rule
	 */
	if (u4SetBufferLen == 0 || pvSetBuffer == NULL) {
		DBGLOG(REQ, ERROR, "u4SetBufferLen is %d, pvSetBuffer is %p\n",
			u4SetBufferLen,
			pvSetBuffer);
		return WLAN_STATUS_INVALID_DATA;
	}
...
wlanoidSetDisassociate(IN struct ADAPTER *prAdapter,
	...
	*pu4SetInfoLen = 0;
	...
	wlanSetDisassociate(prAdapter, pvSetBuffer, u4SetBufferLen, ucBssIndex);
			 ...
			 OUT uint32_t *pu4SetInfoLen)
{
	...
	uint32_t u4IPv6AddrIdx = 0;
	uint32_t u4IPv6AddrCount = 0;
	struct CMD_IPV6_NETWORK_ADDRESS_LIST *prCmdIpv6AddressList;
	struct PARAM_NETWORK_ADDRESS_LIST *prNetworkAddressList =
		(struct PARAM_NETWORK_ADDRESS_LIST *) pvSetBuffer;
	struct PARAM_NETWORK_ADDRESS *prNetworkAddress;
	...
	/* 5 <2> Calculate ipv6 command buffer size */
	/* construct payload of command packet */
	...
	/* 5 <3> Allocate command buffer */
	prCmdIpv6AddressList = (struct CMD_IPV6_NETWORK_ADDRESS_LIST *)
					kalMemAlloc(u4CmdSize, VIR_MEM_TYPE);

	if (prCmdIpv6AddressList == NULL)
		return WLAN_STATUS_FAILURE;


	/* 5 <4> Fill P_CMD_IPV6_NETWORK_ADDRESS_LIST */
	prCmdIpv6AddressList->ucBssIndex = prNetworkAddressList->ucBssIdx;

	/* only to set IP address to FW once ARP filter is enabled */
	if (prAdapter->fgEnArpFilter) {
		prCmdIpv6AddressList->ucAddressCount =
			(uint8_t) u4IPv6AddrCount;
		prNetworkAddress = (struct PARAM_NETWORK_ADDRESS *)
					(prNetworkAddressList + 1);

		for (i = 0, u4IPv6AddrIdx = 0;
			 i < prNetworkAddressList->u4AddressCount; i++) {
			if (prNetworkAddress->u2AddressType ==
				PARAM_PROTOCOL_ID_TCP_IP &&
				prNetworkAddress->u2AddressLength ==
				...
				kalMemCopy(prCmdIpv6AddressList->
					arNetAddress[u4IPv6AddrIdx].aucIpAddr,
					prNetworkAddress->aucAddress,
					IPV6_ADDR_LEN);

				DBGLOG(OID, INFO,
				"%s:IPv6 Addr[%u]["IPV6STR"]BSS[%d]\n",
				__func__,
				u4IPv6AddrIdx,
				IPV6TOSTR(prNetworkAddress->aucAddress),
				prCmdIpv6AddressList->ucBssIndex);
		...
		prCmdIpv6AddressList->ucAddressCount = 0;
	}
	...
	rStatus = wlanSendSetQueryCmd(prAdapter,
				      ...
				      nicOidCmdTimeoutCommon,
				      u4CmdSize,
				      (uint8_t *) prCmdIpv6AddressList,
				      pvSetBuffer,
				      u4SetBufferLen);

	kalMemFree(prCmdIpv6AddressList, VIR_MEM_TYPE,
		   u4CmdSize);
	return rStatus;
}
#endif
...
/*----------------------------------------------------------------------------*/
uint32_t
wlanoidRftestFmcwQueryAutoTest(IN struct ADAPTER *prAdapter,
			   OUT void *pvQueryBuffer,
			   IN uint32_t u4QueryBufferLen,
			   OUT uint32_t *pu4QueryInfoLen)
{
	struct PARAM_MTK_FMCW_TEST_STRUCT *prRfATInfo;
	uint32_t rStatus = WLAN_STATUS_SUCCESS;
	...
	if (!pu4QueryInfoLen) {
		DBGLOG(RFTEST, ERROR, "pu4QueryInfoLen is NULL\n");
		return WLAN_STATUS_INVALID_DATA;
	}

	if (u4QueryBufferLen < sizeof(struct PARAM_MTK_FMCW_TEST_STRUCT)) {
		DBGLOG(RFTEST, ERROR, "Invalid length %lu\n", u4QueryBufferLen);
		return WLAN_STATUS_INVALID_LENGTH;
	}

	if (!pvQueryBuffer) {
		DBGLOG(RFTEST, ERROR, "pvQueryBuffer is NULL\n");
		return WLAN_STATUS_INVALID_DATA;
	}

	*pu4QueryInfoLen = sizeof(struct
				  PARAM_MTK_FMCW_TEST_STRUCT);

	prRfATInfo = (struct PARAM_MTK_FMCW_TEST_STRUCT *)
		     pvQueryBuffer;
	...
	rStatus = rftestFmcwQueryATInfo(prAdapter,
				    prRfATInfo->u4FuncIndex,
				    prRfATInfo->u4FuncData,
				    pvQueryBuffer, u4QueryBufferLen);
...
uint32_t wlanoidFmcwTestFlowCtrl(
			   IN struct ADAPTER *prAdapter,
			   IN void *pvSetBuffer,
			   IN uint32_t u4SetBufferLen,
			   OUT uint32_t *pu4SetInfoLen)
{
	struct RADAR_TEST_FLOW_CTRL *prCmdFmcw;
	...
	if (u4SetBufferLen < sizeof(struct RADAR_TEST_FLOW_CTRL)) {
	...
	*pu4SetInfoLen = sizeof(struct RADAR_TEST_FLOW_CTRL);

	prCmdFmcw = (struct RADAR_TEST_FLOW_CTRL *)pvSetBuffer;

	return wlanSendSetQueryCmd(prAdapter,
			CMD_ID_RADAR_TEST_FLOW_CTRL,
			TRUE,
			TRUE,
			TRUE,
			nicCmdEventFmcwTestFlowCtrl,
			nicOidCmdTimeoutCommon,
...
#if CFG_SUPPORT_FMCW_RADAR
uint32_t
rftestFmcwQueryATInfo(IN struct ADAPTER *prAdapter,
		  uint32_t u4FuncIndex, uint32_t u4FuncData,
		  OUT void *pvQueryBuffer, IN uint32_t u4QueryBufferLen)
{
	struct GLUE_INFO *prGlueInfo = NULL;
	struct CMD_INFO *prCmdInfo = NULL;
	struct CMD_FMCW_TEST_CTRL *pCmdTestCtrl = NULL;
	union EVENT_FMCW_TEST_STATUS *prTestStatus = NULL;
	...
	if (prGlueInfo == NULL || prChipInfo == NULL)
		return FALSE;

	if (u4FuncIndex == RF_AT_FUNCID_FW_INFO) {
		/* driver implementation */
		prTestStatus = (union EVENT_FMCW_TEST_STATUS *) pvQueryBuffer;

		prTestStatus->rATInfo.u4FuncData =
			(prAdapter->rVerInfo.u2FwProductID << 16) |
			(prAdapter->rVerInfo.u2FwOwnVersion);
		u4QueryBufferLen = sizeof(union EVENT_FMCW_TEST_STATUS);
	...
	cmd_size =
		prChipInfo->u2CmdTxHdrSize + sizeof(struct CMD_FMCW_TEST_CTRL);
	prCmdInfo = cmdBufAllocateCmdInfo(prAdapter, cmd_size);
	if (!prCmdInfo) {
		DBGLOG(INIT, ERROR, "Allocate CMD_INFO_T ==> FAILED.\n");
		return WLAN_STATUS_FAILURE;
	...
	prCmdInfo->eCmdType = COMMAND_TYPE_GENERAL_IOCTL;
	prCmdInfo->u2InfoBufLen = cmd_size;
	prCmdInfo->pfCmdDoneHandler = nicCmdEventQueryRfTestFmcwATInfo;
	prCmdInfo->pfCmdTimeoutHandler = nicOidCmdTimeoutCommon;
	prCmdInfo->fgIsOid = TRUE;
	prCmdInfo->ucCID = CMD_ID_FMCW_TEST_CTRL;
	prCmdInfo->fgSetQuery = FALSE;
	prCmdInfo->fgNeedResp = TRUE;
	prCmdInfo->u4SetInfoLen = sizeof(struct CMD_FMCW_TEST_CTRL);
	prCmdInfo->pvInformationBuffer = pvQueryBuffer;
	prCmdInfo->u4InformationBufferLength = u4QueryBufferLen;
...
/*----------------------------------------------------------------------------*/
/*!
 * \brief This routine is used to set the csi mode.
 *
 * \param[in] pvAdapter Pointer to the Adapter structure.
 * \param[in] pvSetBuffer A pointer to the buffer that holds the data to be set.
 * \param[in] u4SetBufferLen The length of the set buffer.
...
/*----------------------------------------------------------------------------*/
uint32_t
wlanoidSetCsiMode(struct ADAPTER *prAdapter, void *pvSetBuffer,
	uint32_t u4SetBufferLen, uint32_t *pu4SetInfoLen)
{
	uint32_t status = WLAN_STATUS_SUCCESS;
	uint32_t *prEnable = (uint32_t *)NULL;

	DEBUGFUNC("wlanoidSetCsiMode");
	...
	*pu4SetInfoLen = sizeof(uint32_t);
	...
	DBGLOG(INIT, INFO, "Set CSI enable[%ld]\n", *prEnable);

	if (*prEnable) {
		if (csiLaunch(prAdapter->prGlueInfo)) {
			if (!(prAdapter->fgIsCsiRegistered)) {
				DBGLOG(REQ, ERROR,
					"fgIsCSIRegistered is NULL\n");
				return WLAN_STATUS_FAILURE;
			}
		} else {
			status = WLAN_STATUS_FAILURE;
		...
		if (prAdapter->fgIsCsiRegistered)
			csiRemove(prAdapter->prGlueInfo);
	}
...
/*----------------------------------------------------------------------------*/
/*!
 * \brief This routine is used to set the fmcw mode.
 *
 * \param[in] pvAdapter Pointer to the Adapter structure.
 * \param[in] pvSetBuffer A pointer to the buffer that holds the data to be set.
 * \param[in] u4SetBufferLen The length of the set buffer.
...
/*----------------------------------------------------------------------------*/
uint32_t
wlanoidSetFmcwMode(struct ADAPTER *prAdapter, void *pvSetBuffer,
	uint32_t u4SetBufferLen, uint32_t *pu4SetInfoLen)
{
	uint32_t status = WLAN_STATUS_SUCCESS;
	uint32_t *prEnable = (uint32_t *)NULL;
	...
	*pu4SetInfoLen = sizeof(uint32_t);
	...
	DBGLOG(RDR, INFO, "Set FMCW enable[%ld]\n", *prEnable);

	if (*prEnable) {
		if (fmcwLaunch(prAdapter->prGlueInfo)) {
			if (!(prAdapter->fgIsFmcwRegistered)) {
				DBGLOG(RDR, ERROR,
					"fgIsFmcwRegistered is NULL\n");
				status = WLAN_STATUS_FAILURE;
			}
		} else {
			status = WLAN_STATUS_FAILURE;
		...
		if (prAdapter->fgIsFmcwRegistered)
			fmcwRemove(prAdapter->prGlueInfo);
	}
	...
	OUT uint32_t *pu4QueryInfoLen)
{
	...
	case CMD_GET_FMCW_RADAR_REPORT_TYPE:
		*pu4QueryInfoLen = sizeof(struct CMD_FMCW_RADAR_TRAFFIC_INFO);
		len = sizeof(struct CMD_FMCW_RADAR_TRAFFIC_INFO);
		break;
#endif
	default:
		return WLAN_STATUS_INVALID_LENGTH;
...
wlanoidSetNchoRoamTrigger(IN struct ADAPTER *prAdapter,
	...
	*pi4Param = dBm_TO_RCPI(*pi4Param);		/* DB to RCPI */
	...
	kalSnprintf(acCmd, NCHO_CMD_MAX_LENGTH, "%s %d",
...
wlanoidAbortScan(IN struct ADAPTER *prAdapter,
	...
	if (prAisFsmInfo->eCurrentState == AIS_STATE_SCAN ||
		...
		/* Abort scan remove pending scan */
		if (aisFsmIsRequestPending(prAdapter, AIS_REQUEST_SCAN,
			TRUE, ucBssIndex) == TRUE)
			DBGLOG(SCN, INFO, "Abort scan remove pending scan\n");
...
#if CFG_SUPPORT_TX_BEACON_RESP_STA_MODE
				    ...
				    uint32_t *pu4SetInfoLen)
{
	...
	u4SSIDIELen = u4SetBufferLen + sizeof(*prSSIDIE);
	prSSIDIE = kalMemAlloc(u4SSIDIELen, PHY_MEM_TYPE);
	if (!prSSIDIE) {
	...
	rrmTxNeighborReportRequest(prAdapter, prAisBssInfo->prStaRecOfAP,
				   prSSIDIE);
	kalMemFree(prSSIDIE, PHY_MEM_TYPE, u4SSIDIELen);
	return WLAN_STATUS_SUCCESS;
}
			    ...
			    OUT uint32_t *pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = 0;
	...
	if (ucPhyType & PHY_TYPE_SET_802_11AC)
		kalStrnCpy(pNameBuf, "11AC",
			(ucMaxCopySize < kalStrLen("11AC")) ?
			ucMaxCopySize : kalStrLen("11AC"));
	else if (ucPhyType & PHY_TYPE_SET_802_11N)
		kalStrnCpy(pNameBuf, "11N",
			(ucMaxCopySize < kalStrLen("11N")) ?
			ucMaxCopySize : kalStrLen("11N"));
	else if (ucPhyType & PHY_TYPE_SET_802_11G)
		kalStrnCpy(pNameBuf, "11G",
			(ucMaxCopySize < kalStrLen("11G")) ?
			ucMaxCopySize : kalStrLen("11G"));
	else if (ucPhyType & PHY_TYPE_SET_802_11A)
		kalStrnCpy(pNameBuf, "11A",
			(ucMaxCopySize < kalStrLen("11A")) ?
			ucMaxCopySize : kalStrLen("11A"));
	else if (ucPhyType & PHY_TYPE_SET_802_11B)
		kalStrnCpy(pNameBuf, "11B",
			(ucMaxCopySize < kalStrLen("11B")) ?
			ucMaxCopySize : kalStrLen("11B"));
	else
		DBGLOG(OID, INFO,
			     ...
			     OUT uint32_t *pu4SetInfoLen)
{
	...
	cnmTimerStopTimer(prAdapter, &SER_SYN_TIMER(prAdapter));

	cnmTimerStartTimer(prAdapter, &SER_SYN_TIMER(prAdapter),
	...
	uint32_t *pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = sizeof(uint8_t);
	...
	for (ucBssIdx = 0; ucBssIdx < MAX_BSSID_NUM; ucBssIdx++) {
		rStatus = rlmDomain6GPwrModeUpdate(
					prAdapter,
					ucBssIdx,
					ucMode);
		...
		if (rStatus != WLAN_STATUS_SUCCESS)
			return WLAN_STATUS_FAILURE;
		else if (IS_NET_ACTIVE(prAdapter, ucBssIdx))
			nicUpdateBss(prAdapter, ucBssIdx);
	}
...
uint32_t wlanoidSetRadarParams(
	IN struct ADAPTER *prAdapter,
	IN void *pvSetBuffer,
	IN uint32_t u4SetBufferLen,
	OUT uint32_t *pu4SetInfoLen
...
{
	struct CMD_RADAR_CTRL_PARA *prSetRadarCtrlInfo = NULL;
	...
	if (u4SetBufferLen < sizeof(struct CMD_RADAR_CTRL_PARA)) {
		DBGLOG(REQ, ERROR, "Invalid length %lu\n", u4SetBufferLen);
		return WLAN_STATUS_INVALID_LENGTH;
	}
	...
	*pu4SetInfoLen = u4SetBufferLen;
	...
	prSetRadarCtrlInfo = (struct CMD_RADAR_CTRL_PARA *)pvSetBuffer;
	...
	return wlanSendSetQueryCmd(prAdapter,
			...
			TRUE,
			TRUE,
			nicCmdEventRadarCtrl,
			nicOidCmdTimeoutCommon,
			u4SetBufferLen,
			(uint8_t *)prSetRadarCtrlInfo,
			pvSetBuffer,
			u4SetBufferLen);
...
/*----------------------------------------------------------------------------*/
/*!
 * \brief Send CMD to get RTT Capabilities
 *
 * \param[in] pvAdapter Pointer to the Adapter structure.
 * \param[out] pvQueryBuffer A pointer to the buffer that holds the result of
 *                           the query.
	...
	uint32_t *pu4QueryInfoLen)
{
	...
	u4InfoBufLen = sizeof(struct CMD_LOC_REQ);
	prLocReq = kalMemAlloc(u4InfoBufLen, VIR_MEM_TYPE);
	if (prLocReq == NULL) {
		DBGLOG(REQ, ERROR, "Request prLocReq Mem fail.\n");
		return WLAN_STATUS_FAILURE;
	}
	prLocReq->ucSubCmdId = CMD_LOC_GET_CAPA;

	DBGLOG(REQ, INFO, "RTT Send CMD CMD_ID_LOCATION, CMD_LOC_GET_CAPA.\n");
	rStatus = wlanSendSetQueryCmd(prAdapter,
				   CMD_ID_LOCATION,
				   FALSE, TRUE, TRUE,
				   rttEventGetCapabilities,
				   nicOidCmdTimeoutCommon,
				   u4InfoBufLen, (uint8_t *)prLocReq,
				   pvQueryBuffer, u4QueryBufferLen);

	kalMemFree(prLocReq, VIR_MEM_TYPE, u4InfoBufLen);
...
/*----------------------------------------------------------------------------*/
/*!
 * \brief Send CMD to start or stop RTT
 *
 * \param[in] pvAdapter Pointer to the Adapter structure.
 * \param[in] pvSetBuffer A pointer to the buffer that holds the data to be set.
 * \param[in] u4SetBufferLen The length of the set buffer.
	...
	uint32_t *pu4SetInfoLen)
{
	...
	if (!u4SetBufferLen) {
		DBGLOG(REQ, ERROR, "u4SetBufferLen is NULL\n");
		return WLAN_STATUS_INVALID_DATA;
	}
	if (!pvSetBuffer) {
		DBGLOG(REQ, ERROR, "pvSetBuffer is NULL\n");
	...
	if (prAdapter->rAcpiState == ACPI_STATE_D3) {
		DBGLOG(REQ, WARN, "Adapter is not ready. ACPI=D%d, Radio=%d\n",
		       prAdapter->rAcpiState, prAdapter->fgIsRadioOff);
		return WLAN_STATUS_ADAPTER_NOT_READY;
	...
	DBGLOG(REQ, INFO, "Handle RTT Request\n");
...
/*----------------------------------------------------------------------------*/
/*!
 * \brief Send CMD to control RTT enable/disable
 *
 * \param[in] pvAdapter Pointer to the Adapter structure.
 * \param[in] pvSetBuffer A pointer to the buffer that holds the data to be set.
 * \param[in] u4SetBufferLen The length of the set buffer.
 ...
 */
/*----------------------------------------------------------------------------*/
uint32_t wlanoidControlRtt(struct ADAPTER *prAdapter,
	void *pvSetBuffer, uint32_t u4SetBufferLen,
	uint32_t *pu4SetInfoLen)
{
	uint8_t ucControl;
	...
	if (u4SetBufferLen < sizeof(uint8_t)) {
		DBGLOG(REQ, ERROR,
			"u4SetBufferLen is insufficient: %u\n",
			u4SetBufferLen);
		return WLAN_STATUS_INVALID_DATA;
	}
	if (!pvSetBuffer) {
	...
	kalMemCopy(&ucControl, pvSetBuffer, sizeof(uint8_t));
	rttControl(prAdapter, ucControl);
	return WLAN_STATUS_SUCCESS;
}

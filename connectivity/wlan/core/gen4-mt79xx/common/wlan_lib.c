...
void wlanAdapterDestroy(IN struct ADAPTER *prAdapter)
{
	...
	if (prAdapter->chip_info)
		kalMemFree(prAdapter->chip_info->bus_info,
			VIR_MEM_TYPE, sizeof(struct BUS_INFO));

	kalMemFree(prAdapter->chip_info, VIR_MEM_TYPE,
		sizeof(struct mt66xx_chip_info));
#endif
#if CFG_CSI_DYNAMIC_BUFFER
	csiFreeDynamicBuffer(prAdapter->prGlueInfo);
#endif
...
/*----------------------------------------------------------------------------*/
/*!
 * \brief Start or stop image starting flow protection
 *
 * \param prAdapter      Pointer of Adapter Data Structure
 * \param fgIsStart      Start or stop the protection
 *
 * \retval WLAN_STATUS_SUCCESS: Success
 * \retval WLAN_STATUS_FAILURE: Failed
 */
/*----------------------------------------------------------------------------*/
uint32_t wlanImageStartProtect(IN struct ADAPTER *prAdapter,
		IN uint8_t fgIsStart)
{
	...
	if (prChipInfo->SecProtectFWDL)
		return prChipInfo->SecProtectFWDL(prAdapter, fgIsStart);
					...
					IN const u_int8_t bAtResetFlow)
{
	...
	do {
		...
		if (u4Status != WLAN_STATUS_SUCCESS) {
			...
			u4Status = WLAN_STATUS_FAILURE;
			eFailReason = INIT_WAKEUP_WIFI_FAIL;
			break;
		}
		...
		u4Status = wlanImageStartProtect(prAdapter, FALSE);
...
void wlanOffClearAllQueues(IN struct ADAPTER *prAdapter)
{
	...
	wlanClearTxDataQueue(prAdapter);
#endif
 ...
 */
/*----------------------------------------------------------------------------*/
void wlanIST(IN struct ADAPTER *prAdapter, u_int8_t fgEnInt)
{
	uint32_t u4Status = WLAN_STATUS_SUCCESS;
...
/*----------------------------------------------------------------------------*/
/*!
 * \brief This routine is used to clear all buffer in rTxDataQ
 * \param prAdapter  Pointer of Adapter Data Structure
 *
 * \retval none
 */
...
void wlanClearTxDataQueue(IN struct ADAPTER *prAdapter)
{
	...
	if (prAdapter == NULL || prAdapter->prGlueInfo == NULL) {
		DBGLOG(TX, ERROR, "prAdapter or prGlueInfo is NULL\n");
		return;
	}
	...
	KAL_ACQUIRE_SPIN_LOCK(prAdapter, SPIN_LOCK_TX_DATA_QUE);
	list_replace_init(&prHifInfo->rTxDataQ, &rTempList);
	prHifInfo->u4TxDataQLen = 0;
	KAL_RELEASE_SPIN_LOCK(prAdapter, SPIN_LOCK_TX_DATA_QUE);

	list_for_each_safe(prCur, prNext, &rTempList) {
...
#if CFG_SUPPORT_MULTI_SKB_SIZE
	if (nicRxSetupRFB(prAdapter, prSwRfb, prSwRfb->eSkbSizeType)) {
#else
...
uint32_t wlanSetChipEcoInfo(IN struct ADAPTER *prAdapter)
{
	...
	if (wlanAccessRegister(prAdapter,
		...
		/* success */
		nicSetChipHwVer(prAdapter->prGlueInfo,
				(uint8_t)(GET_HW_VER(hw_version) & 0xFF));
		nicSetChipFactoryVer(prAdapter->prGlueInfo,
				(uint8_t)((GET_HW_VER(hw_version) >> 8) & 0xF));
		nicSetChipSwVer(prAdapter->prGlueInfo,
...
#if WLAN_INCLUDE_SYS
	sysMacAddrOverride(prAdapter->prGlueInfo, rMacAddr);
#endif
...
uint8_t wlanGetRomVersion(IN struct ADAPTER *prAdapter)
{
	...
	ucRomVersion = nicGetChipSwVer(prAdapter->prGlueInfo);
...
void wlanInitFeatureOption(IN struct ADAPTER *prAdapter)
{
	...
	prWifiVar->u4ApChnlHoldTime = (uint32_t) wlanCfgGetUint32(prAdapter,
					"ApChnlHoldTime",
					SAP_CHNL_HOLD_TIME_MS);
	prWifiVar->u4P2pChnlHoldTime = (uint32_t) wlanCfgGetUint32(prAdapter,
					"P2pChnlHoldTime",
					P2P_CHNL_HOLD_TIME_MS);
#if CFG_SUPPORT_IOT_AP_BLACKLIST
	...
	prWifiVar->fgEnStaRSNO = (uint8_t)
	wlanCfgGetUint32(prAdapter, "EnStaRSNO", FEATURE_ENABLED);
	...
	*/
	...
	prWifiVar->ucWeightDirt = (uint8_t) wlanCfgGetUint32(
		prAdapter, "WeightDirt", 100);
	prWifiVar->ucWeightIdle = (uint8_t) wlanCfgGetUint32(
		prAdapter, "WeightIdle", 0);
	...
	prWifiVar->ucLocRstaEnable = (uint8_t) wlanCfgGetUint32(
				prAdapter, "LocRstaEnable",
				FEATURE_DISABLED);
#endif /* CFG_SUPPORT_RTT_RSTA */
	prWifiVar->ucRttBlockScan = (uint8_t) wlanCfgGetUint32(
				prAdapter, "RttBlockScan",
				FEATURE_DISABLED);
...
prWifiVar->uc11BDynamicEnable = (uint8_t) wlanCfgGetUint32(
					prAdapter, "11BDynamicEnable", 0);
#endif
	...
	prWifiVar->ucDbdcOMFrame = (uint8_t) wlanCfgGetUint32(
					prAdapter, "DbdcOMFrame", 3);
#else
	...
	prWifiVar->ucSchedScanDelayWakeup = (uint8_t) wlanCfgGetUint32(
		prAdapter, "ScheduleScanDelayWakeup", FEATURE_DISABLED);
#endif
	...
	prWifiVar->fgScanAddCapIE = (u_int8_t) wlanCfgGetUint32(
					prAdapter, "ScanAddCapIE",
					FEATURE_DISABLED);
	prWifiVar->fgEnDbdcScan = (u_int8_t) wlanCfgGetUint32(
					prAdapter, "EnDbdcScan",
					FEATURE_ENABLED);
	...
	prWifiVar->ucP2pGoAutoSCC = (uint8_t) wlanCfgGetUint32(
			prAdapter, "P2pGoAutoSCC",
			FEATURE_DISABLED);
#endif
...
#if CFG_SUPPORT_NAN
	prWifiVar->ucNanMacAddrOverride =
		(uint8_t) wlanCfgGetInt32(prAdapter, "NanMacOverride", 0);
	...
	prWifiVar->ucNan2gBandwidth =
		(uint8_t)wlanCfgGetUint32(prAdapter, "Nan2gBw", NAN_CHNL_BW_20);
	prWifiVar->ucNan5gBandwidth =
		(uint8_t)wlanCfgGetUint32(prAdapter, "Nan5gBw", NAN_CHNL_BW_80);
	prWifiVar->fgEnNanVHT =
		(uint8_t)wlanCfgGetUint32(prAdapter, "NanVHT", 1);
#if (CFG_SUPPORT_NAN_R4 == 1) && (CFG_SUPPORT_802_11AX == 1)
	prWifiVar->fgEnNanHE =
		(uint8_t)wlanCfgGetUint32(prAdapter, "NanHE", 0);
#endif
	...
	prWifiVar->fgNoPmf =
		(uint8_t)wlanCfgGetUint32(prAdapter, "NanForceNoPmf", 0);
	prWifiVar->fgNanIsSigma =
		(uint8_t)wlanCfgGetUint32(prAdapter, "NanIsSigma", 0);
	prWifiVar->ucNanNSS =
		(uint8_t)wlanCfgGetUint32(prAdapter, "NanNss", 1);
	prWifiVar->ucNanVendorIoctl =
		(uint8_t) wlanCfgGetUint32(prAdapter, "NanVendorIoctl", 0);
	prWifiVar->ucDiscBcnPeriod = (uint8_t) wlanCfgGetUint32(
			prAdapter, "NanDiscBcnPeriod", 100);
	prWifiVar->ucNan2gCommittedDw =
		(uint8_t)wlanCfgGetUint32(prAdapter, "Nan2gDftCommittedDw", 1);
	prWifiVar->ucNan5gCommittedDw =
		(uint8_t)wlanCfgGetUint32(prAdapter, "Nan5gDftCommittedDw", 1);
	prWifiVar->fgNanOnAbortScan =
		(uint8_t)wlanCfgGetUint32(prAdapter, "NanOnAbortScan", 0);
#if (CFG_SUPPORT_NAN_R4_VENDOR_PAIRING == 1)
	prWifiVar->fgEnNanPairing =
		(unsigned char)wlanCfgGetUint32(
			prAdapter, (const int8_t *)"NanPairingEn", 1);
#endif
	prWifiVar->fgUseNmiCxtKey =
		(uint8_t)wlanCfgGetUint32(
			prAdapter, (const int8_t *)"NanNmiCxtKey", 1);
	prWifiVar->fgEnNanKeyMgmt =
		(uint8_t)wlanCfgGetUint32(
			prAdapter, (const int8_t *)"NanKeyMgmt", 1);
#if (CFG_NAN_SUPPORT_R4_GROUP_ADDR_FRAME_PROT == 1)
	prWifiVar->ucNanGroupSecCap =
		(unsigned char)wlanCfgGetUint32(
			prAdapter, (const int8_t *)"NanSecCap",
			CSIA_CAP_GTKSA_IGTKSA_BIGTKSA_SUPPORTED);
	prWifiVar->ucNanIgtkCipher =
		(unsigned char)wlanCfgGetUint32(
			prAdapter, (const int8_t *)"NanIgtkCs", 0);
	...
	prWifiVar->fgSapTxFairness = (uint8_t) wlanCfgGetUint32(
		prAdapter, "SapTxFairness", FEATURE_ENABLED);
#endif
	...
	prWifiVar->ucChannelMinDwellTime = (uint8_t) wlanCfgGetUint32(
					prAdapter, "ChannelMinDwellTime", 0);
	prWifiVar->ucChannelMaxDwellTime = (uint8_t) wlanCfgGetUint32(
					prAdapter, "ChannelMaxDwellTime", 0);

#if (CFG_SUPPORT_6G_PWR_REGS == 1)
	prWifiVar->fgLpiSup6G = (u_int8_t) wlanCfgGetUint32(
		prAdapter, "LpiSup6G", FEATURE_ENABLED);
	prWifiVar->fgSpSup6G = (u_int8_t) wlanCfgGetUint32(
		prAdapter, "SpSup6G", FEATURE_ENABLED);
	prWifiVar->fgVlpSup6G = (u_int8_t) wlanCfgGetUint32(
		prAdapter, "VlpSup6G", FEATURE_ENABLED);
	prWifiVar->fgPwrRdutMd = (u_int8_t) wlanCfgGetUint32(
		prAdapter, "PwrRdutMd", FEATURE_DISABLED);
#endif /* CFG_SUPPORT_6G_PWR_REGS */

#if (CFG_SUPPORT_FW_IDX_LOG_TRANS == 1)
	prWifiVar->fgFwIdxLogTrans = (uint32_t) wlanCfgGetUint32(
		prAdapter, "FwIdxLogTrans", FEATURE_DISABLED);
#endif /* CFG_SUPPORT_FW_IDX_LOG_TRANS */
#if (CFG_SUPPORT_6G_PWR_REGS == 1)
	prWifiVar->cTxPwrEnvLmtMin = (int8_t) wlanCfgGetInt32(
	...
	prWifiVar->ucCsaDeauthClient = (uint8_t) wlanCfgGetUint32(
		prAdapter, "CsaDeauthClient", FEATURE_ENABLED);

#if (CFG_SUPPORT_RADAR_CMD == 1)
	prWifiVar->fgRadarCoupling = (uint8_t) wlanCfgGetUint32(
		prAdapter, "RadarCoupling", FEATURE_DISABLED);
	prWifiVar->fgRadarForceCBPTx = (uint8_t) wlanCfgGetUint32(
		prAdapter, "RadarForceCBPTx", FEATURE_DISABLED);
#if (CFG_SUPPORT_FMCW_VNI == 1)
	prWifiVar->fgRadarVniEnable = (uint8_t) wlanCfgGetUint32(
		prAdapter, "RadarVniEnable", FEATURE_DISABLED);
#endif /* (CFG_SUPPORT_FMCW_VNI == 1) */
#endif /* (CFG_SUPPORT_RADAR_CMD == 1) */

#if (CFG_SUPPORT_FW_IDX_LOG_SAVE == 1)
	prWifiVar->fgFwIdxLogSave = (uint32_t) wlanCfgGetUint32(
		prAdapter, "FwIdxLogSave", FEATURE_DISABLED);
#endif /* CFG_SUPPORT_FW_IDX_LOG_SAVE */

#if (CFG_SUPPORT_802_11V_BSS_TRANSITION_MGT == 1)
	prWifiVar->ucBtmCap = (uint8_t)wlanCfgGetUint32(
		prAdapter, "BtmCap", FEATURE_ENABLED);
#endif

#if (CFG_MTK_WIFI_DRV_OWN_MODE_CONTROL == 1)
	prWifiVar->u4DrvOwnMode = wlanCfgGetUint32(
		prAdapter, "drvOwnMode", FEATURE_DISABLED);
#endif

#if CFG_SUPPORT_MULTI_SKB_SIZE
	prWifiVar->u2SkbNum1kSlab = (uint16_t) wlanCfgGetUint32(
		prAdapter, "SkbNum1kSlab", SKB_SIZE_1K_SLAB_NUM);
	prWifiVar->u2SkbNum2kSlab = (uint16_t) wlanCfgGetUint32(
		prAdapter, "SkbNum2kSlab", SKB_SIZE_2K_SLAB_NUM);
	prWifiVar->u2SkbNum4kSlab = (uint16_t) wlanCfgGetUint32(
		prAdapter, "SkbNum4kSlab", SKB_SIZE_4K_SLAB_NUM);
#endif

#if (CFG_SUPPORT_ANT_SMART_DIVERSITY == 1)
	prWifiVar->i4AntSwDivTheshold = (int32_t) wlanCfgGetInt32(
		prAdapter, "AntSwDivTheshold", -60);
	prWifiVar->ucAntSwDivTriggerTH = (uint8_t) wlanCfgGetInt32(
			prAdapter, "AntSwDivTriggerTH", 4);
	prWifiVar->fgAntSwDivEnable = (uint32_t) wlanCfgGetUint32(
			prAdapter, "AntSwDivEnable", FEATURE_ENABLED);
#endif
...
void wlanCfgSetChip(IN struct ADAPTER *prAdapter)
{
	...
	for (i = 0; i < WLAN_CFG_SET_CHIP_LEN_MAX; i++) {
		...
		kalStrnCpy(rChipConfigInfo.aucCmd, aucValue,
			   (CHIP_CONFIG_RESP_SIZE < WLAN_CFG_VALUE_LEN_MAX) ?
			   CHIP_CONFIG_RESP_SIZE : WLAN_CFG_VALUE_LEN_MAX);
		    ...
		    const int8_t *pucKey, int8_t *pucValue, uint32_t u4Flags)
{
	...
	if (!pucKey) {
		DBGLOG(INIT, ERROR, "pucKey is NULL\n");
		return WLAN_STATUS_FAILURE;
	}
...
#if CFG_SUPPORT_EASY_DEBUG
static int8_t hexDigitToInt(uint8_t ch)
{
	if (ch >= 'a' && ch <= 'f')
		return 10 + ch - 'a';
	else if (ch >= 'A' && ch <= 'F')
 ...
 *
 * @param prAdapter  Pointer of ADAPTER_T
 * @param pucKey  Pointer of uint8_t, key word of Feature
 * @param partial_update  Whether to partially update the switch
 *
 * @return none
 */
/*----------------------------------------------------------------------------*/
			...
			bool partial_update)
{
	...
	for (i = 0; i < WLAN_CFG_ENTRY_NUM_MAX; i++) {
		...
		if (prWlanCfgEntry) {
			...
			if (partial_update &&
				kalStrnCmp(prWlanCfgEntry->aucKey, pucKey,
					kalStrLen(prWlanCfgEntry->aucKey))) {
				continue;
			}
	      ...
	      IN enum ENUM_TX_RESULT_CODE rTxDoneStatus)
{
...
#if CFG_IP_FRAG_DISABLE_HW_CHECKSUM
		(uint8_t *) DISP_STRING("IP FRAG"),
		(uint8_t *) DISP_STRING("IPV6 FRAG"),
#endif
#if CFG_SUPPORT_TX_MGMT_USE_DATAQ
	...
	if (prMsduInfo->ucPktType == ENUM_PKT_1X) {
		/* For AIS Only */
		prStaRec = aisGetStaRecOfAP(prAdapter, prMsduInfo->ucBssIndex);
...
struct wiphy *wlanGetWiphy(struct wireless_dev *prWdev)
{
	...
	IN uint8_t ucBssIndex)
{
	...
	/* AIS */
	if (ucBssIndex < KAL_AIS_NUM && pprWdev[ucBssIndex])
		return pprWdev[ucBssIndex]->netdev;
...
#if (CFG_SUPPORT_WIFI_6G == 1)
	if (ucIdx >= 39)
		ucChannel = ((ucIdx - 39) << 2) + 1U;
	else
#endif
	if (ucIdx >= 34)
		ucChannel = ((ucIdx - 34) << 2) + 149U;
	else if (ucIdx >= 22)
		ucChannel = ((ucIdx - 22) << 2) + 100U;
	else if (ucIdx >= 14)
		ucChannel = ((ucIdx - 14) << 2) + 36U;
	else
		ucChannel = ucIdx + 1U;
		  ...
		  IN uint8_t ucBssIndex)
{
	...
	if (IS_BSS_APGO(prBssInfo)) {
		...
		ucRetValNss = prAdapter->rWifiVar.ucNanNSS;
#endif
...
#if CFG_SUPPORT_CSI
struct CSI_INFO_T *wlanCSIGetInfo(struct ADAPTER *prAdapter)
{
	if (prAdapter)
		return &(prAdapter->rCSIInfo);
	else
		return NULL;
}

/* Initialize CSI_INFO_T */
void wlanCSIInit(struct ADAPTER *prAdapter)
{
	struct CSI_INFO_T *prCSIInfo = wlanCSIGetInfo(prAdapter);

	if (!prCSIInfo) {
		DBGLOG(CSI, ERROR, "prCSIInfo is NULL\n");
		return;
	}

#if CFG_SUPPORT_DBDC
	prCSIInfo->ucBandIdx = ENUM_BAND_0;
#endif /* CFG_SUPPORT_DBDC */
	prCSIInfo->fgIsRunning = false;
}

void wlanCSIUninit(struct ADAPTER *prAdapter)
{
	struct CSI_INFO_T *prCSIInfo = wlanCSIGetInfo(prAdapter);
...
uint8_t wlanCSIIsRunning(struct ADAPTER *prAdapter)
{
	struct CSI_INFO_T *prCSIInfo = wlanCSIGetInfo(prAdapter);
	...
	DBGLOG(CSI, INFO, "Running = %d\n",
		prCSIInfo->fgIsRunning);

	return prCSIInfo->fgIsRunning;
}
...
wlanPushCSIData(IN struct ADAPTER *prAdapter, struct CSI_DATA_T *prCSIData)
{
	...
	if (!prCSIInfo->prCsiInfoData) {
		DBGLOG(INIT, ERROR,
			"can not push CSI data as prCsiInfoData is NULL\n");
		return FALSE;
	}
#endif
...
#if CFG_CSI_DYNAMIC_BUFFER
	kalMemCopy(&(prCSIInfo->prCsiInfoData->arCSIBuffer[
		prCSIInfo->u4CSIBufferTail]),
...
wlanPopCSIData(IN struct ADAPTER *prAdapter, struct CSI_DATA_T *prCSIData)
{
	...
	if (!prCSIInfo->prCsiInfoData) {
		DBGLOG(INIT, ERROR,
			"can not pop CSI data as prCsiInfoData is NULL\n");
		return FALSE;
	}
#endif
	...
	kalMemCopy(prCSIData,
		&(prCSIInfo->prCsiInfoData->arCSIBuffer[
		prCSIInfo->u4CSIBufferHead]),
...
#define ZERO(index) \
{ \
	if ((index >= 0) && (index < CSI_MAX_DATA_COUNT)) { \
		ai2IData[index] = 0; \
		ai2QData[index] = 0; \
	} \
}
...
{ \
	if ((start >= 0) && (end >= 0) && (start < CSI_MAX_DATA_COUNT) \
		&& (end < CSI_MAX_DATA_COUNT) && (start <= end)) { \
	...
	struct CSI_DATA_T *prCSIData)
{
	...
	int32_t i4Pos = 0;
	uint8_t *tmpBuf = buf;
	uint16_t u2DataSize = 0;
	uint16_t u2Rsvd1Size = 0;
	...
	if (!prCSIData) {
		DBGLOG(CSI, ERROR, "prCSIData error!\n");
		return 0;
	}

	prCSIInfo = &(prAdapter->rCSIInfo);
	u2DataSize = prCSIData->u2DataCount * sizeof(int16_t);
	u2Rsvd1Size = prCSIData->ucRsvd1Cnt * sizeof(int32_t);
	...
	if ((i4Pos + sizeof(uint16_t)) > CSI_EVENT_MAX_SIZE)
		return i4Pos;
	put_unaligned(sizeof(prCSIData->aucTA), (uint16_t *) (tmpBuf + i4Pos));
	i4Pos += sizeof(uint16_t);

	if ((i4Pos + sizeof(prCSIData->aucTA)) > CSI_EVENT_MAX_SIZE)
		return i4Pos;
	kalMemCopy((tmpBuf + i4Pos), prCSIData->aucTA,
		sizeof(prCSIData->aucTA));
	i4Pos += sizeof(prCSIData->aucTA);
	...
	if ((i4Pos + sizeof(uint32_t)) > CSI_EVENT_MAX_SIZE)
		return i4Pos;
	...
	if ((i4Pos + u2DataSize) > CSI_EVENT_MAX_SIZE ||
		u2DataSize > sizeof(prCSIData->ac2IData))
		return i4Pos;
	...
	if (prCSIInfo->ucValue1[CSI_CONFIG_INFO] & CSI_INFO_RSVD1) {
		...
		if ((i4Pos + u2Rsvd1Size) > CSI_EVENT_MAX_SIZE ||
			u2Rsvd1Size > sizeof(prCSIData->ai4Rsvd1))
			return i4Pos;
		...
		if ((i4Pos + sizeof(uint16_t)) > CSI_EVENT_MAX_SIZE)
			return i4Pos;
		put_unaligned(sizeof(int32_t), (uint16_t *) (tmpBuf + i4Pos));
		i4Pos += sizeof(uint16_t);
	...
	if ((i4Pos + sizeof(uint8_t)) > CSI_EVENT_MAX_SIZE)
		return i4Pos;
	put_unaligned(prCSIData->ucDbdcIdx,
		(uint8_t *) (tmpBuf + i4Pos));
	i4Pos += sizeof(uint8_t);
...
void
wlanVniOutputCSIData(
	struct ADAPTER *prAdapter,
	struct CSI_DATA_T *prCSIData)
{
	struct GLUE_INFO *prGlueInfo = NULL;
	struct net_device *prNetDev = NULL;
	uint8_t *pucProcBuf = NULL;
	int32_t i4Pos = 0;
	struct sk_buff *prSkb = NULL;
	...
	if (!prNetDev) {
		DBGLOG(CSI, ERROR, "prNetDev error!\n");
		return;
	}

	pucProcBuf = kalMemAlloc(CSI_EVENT_MAX_SIZE, VIR_MEM_TYPE);
	if (!pucProcBuf) {
		DBGLOG(NIC, WARN, "[CSI] Alloc pucProcBuf failed!");
		return;
	}
	kalMemZero(pucProcBuf, CSI_EVENT_MAX_SIZE);

	i4Pos = wlanCSIDataPrepare(pucProcBuf, prAdapter, prCSIData);

	if (i4Pos > CSI_EVENT_MAX_SIZE)
		i4Pos = CSI_EVENT_MAX_SIZE;

	prSkb = kalPacketAlloc(prGlueInfo, i4Pos, &pucRecvBuff);
	if (!prSkb) {
		DBGLOG(NIC, WARN, "[CSI] Alloc prSkb failed!");
		goto exit;
	}
	kalMemCopy(pucRecvBuff, pucProcBuf, i4Pos);
	skb_put(prSkb, i4Pos);
	GLUE_RX_SET_PKT_CSI_VNI_FLAG(prSkb, TRUE);

	kalRxIndicateOnePkt(prGlueInfo, prSkb);

exit:
	kalMemFree(pucProcBuf, VIR_MEM_TYPE, CSI_EVENT_MAX_SIZE);
	pucProcBuf = NULL;
}
...
uint32_t wlanWakeUpWiFi(IN struct ADAPTER *prAdapter)
{
	...
	if (!prAdapter->chip_info) {
		DBGLOG(INIT, ERROR, "prAdapter->chip_info is NULL!\n");
		return WLAN_STATUS_FAILURE;
	}
			...
			uint32_t *pu4CurRate, uint32_t *pu4MaxRate)
{
	...
	if (txmode == TX_RATE_MODE_CCK) { /* 11B */
		...
		DBGLOG_LIMITED(SW4, ERROR, "Wrong txmode: %u\n", txmode);
	}
...
wlanWaitCfg80211SuspendDone(struct GLUE_INFO *prGlueInfo)
{
	...
	u1Count = 0;
	/* waiting deauth done when wowenable 0 */
	if (IS_FEATURE_DISABLED(prGlueInfo->prAdapter->rWowCtrl.fgWowEnable)
		&& IS_FEATURE_DISABLED(prWifiVar->ucAdvPws)) {
		for (ucIdx = AIS_DEFAULT_INDEX; ucIdx < KAL_AIS_NUM; ucIdx++) {
			prAisBssInfo
			= prGlueInfo->prAdapter->prAisBssInfo[ucIdx];
			if (prAisBssInfo == NULL)
				continue;

			while (prAisBssInfo->eConnectionStateIndicated
				== MEDIA_STATE_CONNECTED) {
				if (u1Count > HIF_SUSPEND_MAX_WAIT_TIME)
					break;
...
uint32_t wlanSetRFTestModeCMD(struct GLUE_INFO *prGlueInfo, bool fgEn)
{
	...
	DBGLOG(REQ, INFO, "set %s Test Mode CMD\n",
		(fgEn) ? "Enter" : "Abort");
	...
	DBGLOG(REQ, INFO, "kalIoctl status : %d\n", u4Status);
	return u4Status;
}
...
#if CFG_SUPPORT_NAN
uint32_t
wlanoidSetNanDataRequest(IN struct ADAPTER *prAdapter,
		       ...
		       OUT uint32_t *pu4SetInfoLen)
{
	...
	if (!prNanCmdDataRequest) {
		DBGLOG(NAN, ERROR, "Alloc mem failed\n");
		return WLAN_STATUS_RESOURCES;
	}
	kalMemZero(prNanCmdDataRequest, sizeof(*prNanCmdDataRequest));

	kalMemCpyS(prNanCmdDataRequest, sizeof(*prNanCmdDataRequest),
		pvSetBuffer, u4SetBufferLen);

	rStatus = nanCmdDataRequest(prAdapter, prNanCmdDataRequest,
				    &rDataRcv.ndpid,
...
uint32_t
wlanoidSetNanDataResponse(IN struct ADAPTER *prAdapter,
		       IN void *pvSetBuffer, IN uint32_t u4SetBufferLen,
		       OUT uint32_t *pu4SetInfoLen)
{
	...
	kalMemCpyS(prNanCmdDataResponse, sizeof(*prNanCmdDataResponse),
		pvSetBuffer, u4SetBufferLen);

	rStatus = nanCmdDataResponse(
		prAdapter, prNanCmdDataResponse);
...
uint32_t
wlanoidSetNanDataEnd(IN struct ADAPTER *prAdapter,
		       IN void *pvSetBuffer, IN uint32_t u4SetBufferLen,
		       OUT uint32_t *pu4SetInfoLen)
{
	struct _NAN_CMD_DATA_END rNanCmdDataEnd = {0};

	kalMemCpyS(&rNanCmdDataEnd, sizeof(rNanCmdDataEnd),
		pvSetBuffer, u4SetBufferLen);
	return nanCmdDataEnd(
		prAdapter, &rNanCmdDataEnd);
}
...
uint32_t wlanNotifyRoamingEnhanceToFw(struct GLUE_INFO *prGlueInfo)
{
	...
	rRoamingCtrl.ucRoamingExtDataBitmap =
		ROAMING_EXT_DATA_FORCE_MOBILE_LIKE_EN;
	u4Status = kalIoctl(prGlueInfo,
...
#if (CFG_SUPPORT_RADAR_CMD == 1)
#if (CFG_SUPPORT_FMCW_VNI == 1)
uint32_t wlanFmcwDataPrepare(
	uint8_t *buf,
	uint8_t ucRadarDataCnt,
	uint32_t u4Len,
	struct ADAPTER *prAdapter,
	struct RADAR_RAW_DATA *prRadarRawData)
{
	uint8_t *tmpBuf = buf;
	/* Use Local Experimental Ethertype*/
	uint16_t u2EtherType = CONST_HTONS(0x88B5);
	uint32_t u4BuffSize = 0;

	/*
	 * Radar Raw Data Format:
	 * [MAC address][MAC address][Type] ...
	 * [TimeStamp-sec][TimeStamp-ms][ReasonCode][BurstIndex] ...
	 * [Thermal][RSSI][CBP_Retry][TxAntIdx][Radar_Raw_Data]
	 */

	if ((u4BuffSize + sizeof(uint8_t) * MAC_ADDR_LEN) > u4Len)
		return 0;
	kalMemCopy((tmpBuf + u4BuffSize), prAdapter->rWifiVar.aucMacAddress,
		sizeof(uint8_t) * MAC_ADDR_LEN);
	u4BuffSize += sizeof(uint8_t) * MAC_ADDR_LEN;

	if ((u4BuffSize + sizeof(uint8_t) * MAC_ADDR_LEN) > u4Len)
		return 0;
	/*write same address, discuss with CPL*/
	kalMemCopy((tmpBuf + u4BuffSize), prAdapter->rWifiVar.aucMacAddress,
		sizeof(uint8_t) * MAC_ADDR_LEN);
	u4BuffSize += sizeof(uint8_t) * MAC_ADDR_LEN;

	if ((u4BuffSize + sizeof(uint16_t)) > u4Len)
		return 0;
	kalMemCopy((tmpBuf + u4BuffSize), &u2EtherType, sizeof(uint16_t));
	u4BuffSize += sizeof(uint16_t);

	if ((u4BuffSize + sizeof(uint32_t)) > u4Len)
		return 0;
	kalMemCopy((tmpBuf + u4BuffSize), &(prRadarRawData->rCurrentSec),
		sizeof(uint32_t));
	u4BuffSize += sizeof(uint32_t);

	if ((u4BuffSize + sizeof(uint32_t)) > u4Len)
		return 0;
	kalMemCopy((tmpBuf + u4BuffSize), &(prRadarRawData->rCurrentMsec),
		sizeof(uint32_t));
	u4BuffSize += sizeof(uint32_t);

	if ((u4BuffSize + sizeof(uint32_t)) > u4Len)
		return 0;
	kalMemCopy((tmpBuf + u4BuffSize), &(prRadarRawData->ucReasonCode),
		sizeof(uint8_t));
	u4BuffSize += sizeof(uint32_t);

	if ((u4BuffSize + sizeof(uint32_t)) > u4Len)
		return 0;
	kalMemCopy((tmpBuf + u4BuffSize), &(prRadarRawData->u4BurstCnt),
		sizeof(uint32_t));
	u4BuffSize += sizeof(uint32_t);

	if ((u4BuffSize + sizeof(uint32_t)) > u4Len)
		return 0;
	kalMemCopy((tmpBuf + u4BuffSize), &(prRadarRawData->u4TxChainAdcTemp),
		sizeof(uint32_t));
	u4BuffSize += sizeof(uint32_t);

	if ((u4BuffSize + sizeof(uint32_t)) > u4Len)
		return 0;
	kalMemCopy((tmpBuf + u4BuffSize), &(prRadarRawData->u4FMCWRssi),
		sizeof(uint32_t));
	u4BuffSize += sizeof(uint32_t);

	if ((u4BuffSize + sizeof(uint32_t)) > u4Len)
		return 0;
	kalMemCopy((tmpBuf + u4BuffSize), &(prRadarRawData->u4FMCWCBPRetryCnt),
		sizeof(uint32_t));
	u4BuffSize += sizeof(uint32_t);

	if ((u4BuffSize + sizeof(uint32_t)) > u4Len)
		return 0;
	kalMemCopy((tmpBuf + u4BuffSize), &(prRadarRawData->ucFMCWTxAntIdx),
		sizeof(uint8_t));
	u4BuffSize += sizeof(uint32_t);

	if ((u4BuffSize + ucRadarDataCnt * sizeof(uint32_t)) > u4Len)
		return 0;
	kalMemCopy((tmpBuf + u4BuffSize), prRadarRawData->au4RawData,
		ucRadarDataCnt * sizeof(uint32_t));
	u4BuffSize += ucRadarDataCnt * sizeof(uint32_t);

	return u4BuffSize;
}

void
wlanVniOutputFmcwData(
	struct ADAPTER *prAdapter,
	struct RADAR_RAW_DATA *prRadarRawData,
	uint8_t ucRadarDataCnt)
{
	...
	if (!prRadarRawData) {
		DBGLOG(RDR, ERROR, "prFmcwData error!\n");
		return;
	}
	...
	if (!prNetDev) {
		DBGLOG(RDR, ERROR, "prNetDev error!\n");
		return;
	}

	u4Len = (ucRadarDataCnt +
			FMCW_NONDATA_INFO_NUM) * sizeof(uint32_t);

	pucProcBuf = kalMemAlloc(u4Len, VIR_MEM_TYPE);
	if (!pucProcBuf) {
		DBGLOG(RDR, ERROR, "pucProcBuf Allocate fail!");
		return;
	}

	kalMemZero(pucProcBuf, u4Len);
	u4BuffSize = wlanFmcwDataPrepare(pucProcBuf, ucRadarDataCnt, u4Len,
		prAdapter, prRadarRawData);

	if (u4BuffSize == 0) {
		DBGLOG(RDR, ERROR, "[RDR] u4BuffSize Error!");
		goto exit;
	}

	prSkb = kalPacketAlloc(prGlueInfo, u4BuffSize, &pucRecvBuff);
	if (!prSkb) {
		DBGLOG(RDR, ERROR, "[RDR] Alloc prSkb failed!");
		goto exit;
	}

	kalMemCopy(pucRecvBuff, pucProcBuf, u4BuffSize);
	skb_put(prSkb, u4BuffSize);
	GLUE_RX_SET_PKT_FMCW_VNI_FLAG(prSkb, TRUE);
	kalRxIndicateOnePkt(prGlueInfo, prSkb);
exit:
	kalMemFree(pucProcBuf, VIR_MEM_TYPE, u4Len);
}
#endif /* (CFG_SUPPORT_FMCW_VNI == 1) */
#endif /* (CFG_SUPPORT_RADAR_CMD == 1) */

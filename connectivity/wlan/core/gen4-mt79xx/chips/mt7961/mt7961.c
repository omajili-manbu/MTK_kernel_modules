...
/* for image download protection */
#define CONN_SEMAPHORE_BASE		0x18070000u
#define CONN_SEMA_M0_OWN_BASE		CONN_SEMAPHORE_BASE
#define CONN_SEMA_M0_REL_BASE		(CONN_SEMAPHORE_BASE + 0x200u)
#define CONN_SEMA_OWN_BY_M0_STA_REP_1	(CONN_SEMAPHORE_BASE + 0x400u)
#define CONN_SEMA_M0_OWN_STA(index)	(CONN_SEMA_M0_OWN_BASE + 0x4u * index)
#define CONN_SEMA_M0_OWN_REL(index)	(CONN_SEMA_M0_REL_BASE + 0x4U * index)
#define CONN_SEMA_M0_HOST_PROT_FWDL_IDX	23u

#define CONN_SEMA_M0_SEC_FWDL_LOCK_SEMA_ADDR	\
	CONN_SEMA_M0_OWN_STA(CONN_SEMA_M0_HOST_PROT_FWDL_IDX)
#define CONN_SEMA_M0_SEC_FWDL_REL_SEMA_ADDR	\
	CONN_SEMA_M0_OWN_REL(CONN_SEMA_M0_HOST_PROT_FWDL_IDX)

#define MAX_POLLING_FWDL_GET_SEMA	2000u
...
struct ONE_TIME_CAL_GROUP_DEF_ENTRY mt7961_group_freq_def_arr[] = {
	{0, 2300, 2499},
	{1, 4850, 5149},
	{2, 5150, 5250},
	{3, 5251, 5330},
...
#if (!CFG_SUPPORT_IPC_WITHOUT_MMIO_READ)
...
void mt7961WfdmaRingCntBaseExtCtrl(
	struct GLUE_INFO *prGlueInfo, uint32_t hw_cnt_addr,
	uint32_t phy_addr_ext, uint32_t cnt)
{
	uint32_t u4WrValue = 0;

	u4WrValue |=
		((phy_addr_ext << BASE_PTR_EXT_SHFT) & BASE_PTR_EXT_MASK);
	u4WrValue |=
		((cnt << MAX_CNT_SHFT) & MAX_CNT_MASK);

	HAL_MCR_WR(prGlueInfo->prAdapter, hw_cnt_addr, u4WrValue);
}
	...
	uint32_t *pu4IntStatus)
{
	...
	*pu4IntStatus = 0;
	...
	txdone_bits = prBusInfo->host_int_txdone_bits;
#if CFG_SUPPORT_DISABLE_DATA_DDONE_INTR
	IntMask.word = 0;
#if CFG_SUPPORT_PCIE_WFDMA_WMM
	if (HAL_IS_TX_DIRECT()) {
	...
	if (HAL_IS_TX_DIRECT())
		...
		kalIsTputMode(prAdapter, PKT_PATH_ALL, MAX_BSSID_NUM)?0:1;
#endif
	txdone_bits = txdone_bits | IntMask.word;
#endif
...
void mt7961GetFlavorVer(struct ADAPTER *prAdapter,
	uint8_t *flavor_str,
	bool fgIsWifiFW)
{
	uint32_t u4Val = 0;
	...
	if (prAdapter == NULL) {
		...
		ret = snprintf(flavor_str,
			CFG_FW_FLAVOR_MAX_LEN,
			"%x",
			flavor_ver);
		if (ret < 0) {
			DBGLOG(INIT, ERROR, "get flavor_str failed\n");
		flavor_str[0] = MT7961_A_DIE_7921_FLAVOR + '0';
		flavor_str[1] = '\0';
		}
		return;
	}
	...
	if (prAdapter->prGlueInfo == NULL) {
		DBGLOG(INIT, ERROR, "prAdapter->prGlueInfo == NULL\n");
		return;
	}
	if (fgIsWifiFW &&
		prAdapter->prGlueInfo->fgTestFwDl)
		suffix = "_TEST";
#endif

	if (prAdapter->chip_info->u4ADieVer != 0xFFFFFFFF) {
		flavor_ver = prAdapter->chip_info->u4ADieVer;
		ret = snprintf(flavor_str,
			CFG_FW_FLAVOR_MAX_LEN,
			"%x%s",
			flavor_ver,
	...
	ret = snprintf(flavor_str, CFG_FW_FLAVOR_MAX_LEN, "%x%s",
		flavor_ver, suffix);
	if (ret < 0) {
		DBGLOG(INIT, ERROR, "get flavor_str failed\n");
		flavor_str[0] = MT7961_A_DIE_7921_FLAVOR + '0';
...
uint32_t mt7961GetFwVer(struct ADAPTER *prAdapter)
{
	...
	u4SwVer = nicGetChipSwVer(prAdapter->prGlueInfo) + 1;
...
{
	uint8_t sub_idx = 0;
	uint8_t flavor_str[CFG_FW_FLAVOR_MAX_LEN] = {0};
	uint32_t chip_id = 0;
	...
	mt7961GetFlavorVer(prAdapter, flavor_str, TRUE);

	for (sub_idx = 0; apucNameTable[sub_idx]; sub_idx++) {
		if (((*pucNameIdx) + 3) < ucMaxNameIdx) {
			/* Type 1. WIFI_RAM_CODE_MTxxxx_x.bin */
			if (snprintf(*(apucName + (*pucNameIdx)),
			    CFG_FW_NAME_MAX_LEN, "%s%x_%s.bin",
			    apucNameTable[sub_idx], chip_id, flavor_str) < 0) {
	...
	uint8_t **apucName, uint8_t *pucNameIdx)
{
	...
	if (snprintf(apucName[(*pucNameIdx)],
		CFG_FW_NAME_MAX_LEN, "WIFI_MT%x_patch_mcu_%s_%x_hdr.bin",
		prChipInfo->chip_id, flavor_str,
		mt7961GetFwVer(prAdapter)) < 0)
		DBGLOG(INIT, ERROR, "gen Patch File Name fail\n");
}

#if (CFG_SUPPORT_FW_IDX_LOG_TRANS == 1)
void mt7961ConstructIdxLogBinName(struct GLUE_INFO *prGlueInfo,
	uint8_t **apucName)
{
	int ret = 0;
	uint8_t flavor_str[CFG_FW_FLAVOR_MAX_LEN] = {0};
	...
	/* ex: WIFI_RAM_CODE_MT7961_1_2_idxlog.bin */
	ret = kalSnprintf(apucName[0],
			  CFG_FW_NAME_MAX_LEN,
			  "WIFI_RAM_CODE_MT%x_%s_%x_idxlog.bin",
			  prChipInfo->chip_id,
			  flavor_str,
	...
	uint8_t **apucName, uint8_t *pucNameIdx)
{
	...
	mt7961GetFlavorVer(prAdapter, flavor_str, FALSE);
...
uint32_t mt7961DownloadBtPatch(IN struct ADAPTER *prAdapter)
{
	...
	struct patch_dl_target target = { .patch_region = NULL,
	.num_of_region = 0 };
					  ...
					  uint8_t *aucEeprom)
{
	...
	if (snprintf(aucEeprom, 32, "EEPROM_MT%x_%s.bin",
		 prChipInfo->chip_id, flavor_str) < 0) {
...
uint32_t mt7961GetDongleType(struct ADAPTER *prAdapter)
{
	...
	ret = kstrtol((const char *)flavor_str, 16, &flavor_value);
	if (ret)
		return 0;

	if (flavor_value == MT7961_A_DIE_7921_FLAVOR)
		return 0x7921;
	else if (flavor_value == MT7961_A_DIE_7920_FLAVOR)
		return 0x7920;
...
#if defined(CFG_PROTECT_SEC_FWDL) && (CFG_PROTECT_SEC_FWDL == 1)
#if defined(_HIF_PCIE)
static uint8_t mt7961ConnSemaRemapSetup(struct ADAPTER *prAdapter)
{
	uint32_t u4Val = 0, u4Retry = 2;

	/* Set PCIE2AP public mapping CR4 to CONN_SEMAPHORE */
	u4Val = (CONN_INFRA_BUS_CR_PCIE2AP_REMAP_2_DEFAULT & ~BITS(0, 15)) |
		(CONN_SEMAPHORE_BASE >> 16);
	HAL_MCR_WR(prAdapter, CONN_INFRA_BUS_CR_PCIE2AP_REMAP_2_ADDR, u4Val);

	/* wait remap to take effect by checking the remap result */
	while (u4Retry) {
		u4Retry--;
		HAL_MCR_RD(prAdapter, CONN_INFRA_BUS_CR_PCIE2AP_REMAP_2_ADDR,
			&u4Val);
		if ((u4Val & BITS(0, 15)) == (CONN_SEMAPHORE_BASE >> 16))
			break;
	}

	return !!u4Retry ? TRUE : FALSE;
}

static void mt7961ConnSemaRemapRestore(struct ADAPTER *prAdapter)
{
	/* Reset PCIE2AP public mapping CR4 */
	HAL_MCR_WR(prAdapter, CONN_INFRA_BUS_CR_PCIE2AP_REMAP_2_ADDR,
			CONN_INFRA_BUS_CR_PCIE2AP_REMAP_2_DEFAULT);
}

static uint32_t mt7961GetRemapSemaAddr(uint32_t u4SemaAddr)
{
	return (u4SemaAddr & ~BITS(16, 31)) |
		R_PCIE2AP_PUBLIC_REMAPPING_4_BUS_ADDR;
}

#else /* _HIF_USB or other HIFs */
#define mt7961ConnSemaRemapSetup(prAdapter)    (TRUE)
#define mt7961ConnSemaRemapRestore(prAdapter)

static uint32_t mt7961GetRemapSemaAddr(uint32_t u4SemaAddr)
{
	return u4SemaAddr | CONN_INFRA_REMAPPING_OFFSET;
}

#endif /* defined(_HIF_PCIE) */

static uint32_t mt7961SecProtectFWDL(struct ADAPTER *prAdapter,
		uint8_t fgIsStart)
{
	uint32_t u4Status = WLAN_STATUS_FAILURE, u4PollingCnt = 0;
	uint32_t u4SemaAddr = 0, u4SemaRepAddr = 0, u4SemaVal = 0;
	...
	if (!mt7961ConnSemaRemapSetup(prAdapter)) {
		DBGLOG(INIT, ERROR, "Hif conn remap failed\n");
		return WLAN_STATUS_FAILURE;
	}

	if (fgIsStart) {
		/* Get Semaphore to prevent dl fw in the same time with BT
		 * result sec fw dl failed.
		 */
		u4SemaAddr = mt7961GetRemapSemaAddr(
				CONN_SEMA_M0_SEC_FWDL_LOCK_SEMA_ADDR);
		while (u4PollingCnt < (uint32_t) MAX_POLLING_FWDL_GET_SEMA) {
			HAL_MCR_RD(prAdapter, u4SemaAddr, &u4SemaVal);
			if (u4SemaVal & BIT(0)) {
				DBGLOG(INIT, INFO,
					"ProtFWDL sema get success\n");
				u4Status = WLAN_STATUS_SUCCESS;
				break;
			}
		...
		/* Release semaphore, write 0x1 to release */
		u4SemaAddr = mt7961GetRemapSemaAddr(
				CONN_SEMA_M0_SEC_FWDL_REL_SEMA_ADDR);
		u4SemaRepAddr = mt7961GetRemapSemaAddr(
				CONN_SEMA_OWN_BY_M0_STA_REP_1);
		HAL_MCR_WR(prAdapter, u4SemaAddr, 0x1);
		/* read semaphore status */
		HAL_MCR_RD(prAdapter, u4SemaRepAddr, &u4SemaVal);
		DBGLOG(INIT, INFO, "sema rel stat: 0x%x\n", u4SemaVal);
		u4Status = WLAN_STATUS_SUCCESS;
	}

	mt7961ConnSemaRemapRestore(prAdapter);

	return u4Status;
}
#endif /* defined(CFG_PROTECT_SEC_FWDL) && (CFG_PROTECT_SEC_FWDL == 1) */
...
#if CONFIG_PHYS_ADDR_T_64BIT
	.ring_cnt_base_ext_ctrl = mt7961WfdmaRingCntBaseExtCtrl,
#endif
...
#endif
#if (CFG_SUPPORT_FW_IDX_LOG_TRANS == 1)
	.constrcutIdxLogBin = mt7961ConstructIdxLogBinName,
#endif /* CFG_SUPPORT_FW_IDX_LOG_TRANS */
};
...
struct mt66xx_chip_info mt66xx_chip_info_mt7961 = {
	...
	.prTxPwrLimitFile = WIFI_TXPWR_FN,
...
#if CFG_SUPPORT_ONE_TIME_CAL
	.prGroupFreqDefArr = mt7961_group_freq_def_arr,
#endif
...
#endif
#if defined(CFG_PROTECT_SEC_FWDL) && (CFG_PROTECT_SEC_FWDL == 1)
	.SecProtectFWDL = mt7961SecProtectFWDL,
#endif
};

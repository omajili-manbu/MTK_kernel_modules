...
extern struct MT7961_DMASHDL_CFG rMT7961DmashdlCfg;
...
#if defined(MT7961) || defined(MT7922)
#if defined(_HIF_PCIE) || defined(_HIF_AXI) || defined(_HIF_USB)
void mt7961_DmashdlGetRefill(struct ADAPTER *prAdapter)
{
	uint32_t u4Val = 0;

	HAL_MCR_RD(prAdapter,
		       rMT7961DmashdlCfg.rGroup0RefillDisable.u4Addr, &u4Val);
	DBGLOG(HAL, INFO, "DMASHDL ReFill Control (0x%08x): 0x%08x\n",
		rMT7961DmashdlCfg.rGroup0RefillDisable.u4Addr, u4Val);
}

void mt7961_DmashdlGetPktMaxPage(struct ADAPTER *prAdapter)
{
	uint32_t u4Val = 0;
	...
	HAL_MCR_RD(prAdapter,
		       rMT7961DmashdlCfg.rPlePacketMaxSize.u4Addr, &u4Val);

	ple_pkt_max_sz = (u4Val &
	rMT7961DmashdlCfg.rPlePacketMaxSize.u4Mask) >>
	rMT7961DmashdlCfg.rPlePacketMaxSize.u4Shift;
	pse_pkt_max_sz = (u4Val &
	rMT7961DmashdlCfg.rPsePacketMaxSize.u4Mask) >>
	rMT7961DmashdlCfg.rPsePacketMaxSize.u4Shift;
...
void mt7961_DmashdlGetGroupControl(struct ADAPTER *prAdapter,
					       uint8_t ucGroup)
{
	uint32_t u4Addr;
	uint32_t u4Val = 0;
	...
	u4Addr = rMT7961DmashdlCfg.rGroup0ControlMaxQuota.u4Addr +
		(ucGroup << 2);
...
uint32_t mt7961_DmashdlGetRsvCount(struct ADAPTER *prAdapter,
					       uint8_t ucGroup)
{
	uint32_t u4Addr;
	uint32_t u4Val = 0;
	...
	u4Addr = rMT7961DmashdlCfg.rStatusRdGp0RsvCnt.u4Addr + (ucGroup << 2);

	HAL_MCR_RD(prAdapter, u4Addr, &u4Val);

	rsv_cnt = (u4Val & rMT7961DmashdlCfg.rStatusRdGp0RsvCnt.u4Mask) >>
		rMT7961DmashdlCfg.rStatusRdGp0RsvCnt.u4Shift;
...
uint32_t mt7961_DmashdlGetSrcCount(struct ADAPTER *prAdapter,
					       uint8_t ucGroup)
{
	uint32_t u4Addr;
	uint32_t u4Val = 0;
	...
	u4Addr = rMT7961DmashdlCfg.rStatusRdGp0SrcCnt.u4Addr + (ucGroup << 2);

	HAL_MCR_RD(prAdapter, u4Addr, &u4Val);

	src_cnt = (u4Val & rMT7961DmashdlCfg.rStatusRdGp0SrcCnt.u4Mask) >>
		rMT7961DmashdlCfg.rStatusRdGp0SrcCnt.u4Shift;

	DBGLOG(HAL, TRACE, "\tsrc_cnt = 0x%03x\n", src_cnt);
	return src_cnt;
}

void mt7961_DmashdlGetPKTCount(struct ADAPTER *prAdapter,
					   uint8_t ucGroup)
{
	uint32_t u4Addr;
	uint32_t u4Val = 0;
	...
	if ((ucGroup & 0x1) == 0)
		u4Addr = rMT7961DmashdlCfg.rRdGroupPktCnt0.u4Addr +
		(ucGroup << 1);
	else
		u4Addr = rMT7961DmashdlCfg.rRdGroupPktCnt0.u4Addr +
		((ucGroup-1) << 1);
	...
	struct ADAPTER *prAdapter)
{
	...
	mt7961_DmashdlGetRefill(prAdapter);
	mt7961_DmashdlGetPktMaxPage(prAdapter);

	HAL_MCR_RD(prAdapter,
			rMT7961DmashdlCfg.rErrorFlagCtrl.u4Addr, &value);

	DBGLOG(HAL, INFO, "DMASHDL ERR FLAG CTRL(0x%08x): 0x%08x\n",
	       rMT7961DmashdlCfg.rErrorFlagCtrl.u4Addr, value);

	for (idx = 0; idx <= ENUM_DMASHDL_GROUP_14; idx++) {
		if (rMT7961DmashdlCfg.afgRefillEn[idx] == 0)
			continue;
		DBGLOG(HAL, INFO, "Group %d info:\n", idx);
		mt7961_DmashdlGetGroupControl(prAdapter, idx);
		rsv_cnt = mt7961_DmashdlGetRsvCount(prAdapter, idx);
		src_cnt = mt7961_DmashdlGetSrcCount(prAdapter, idx);
		mt7961_DmashdlGetPKTCount(prAdapter, idx);
	...
	DBGLOG(HAL, INFO, "Group %d info:\n", idx);
	mt7961_DmashdlGetGroupControl(prAdapter, idx);
	mt7961_DmashdlGetRsvCount(prAdapter, idx);
	mt7961_DmashdlGetSrcCount(prAdapter, idx);
	mt7961_DmashdlGetPKTCount(prAdapter, idx);

	HAL_MCR_RD(prAdapter,
		       rMT7961DmashdlCfg.rStatusRdFfaCnt.u4Addr, &value);
	ffa_cnt = (value & rMT7961DmashdlCfg.rStatusRdFfaCnt.u4Mask) >>
		rMT7961DmashdlCfg.rStatusRdFfaCnt.u4Shift;
	free_pg_cnt = (value & rMT7961DmashdlCfg.rStatusRdFreePageCnt.u4Mask) >>
		rMT7961DmashdlCfg.rStatusRdFreePageCnt.u4Shift;
	DBGLOG(HAL, INFO, "\tDMASHDL Status_RD(0x%08x): 0x%08x\n",
		rMT7961DmashdlCfg.rStatusRdFreePageCnt.u4Addr, value);
	DBGLOG(HAL, INFO, "\tfree page cnt = 0x%03x, ffa cnt = 0x%03x\n",
		free_pg_cnt, ffa_cnt);

	DBGLOG(HAL, INFO, "DMASHDL Counter Check:\n");
	HAL_MCR_RD(prAdapter,
		       rMT7961DmashdlCfg.rHifPgInfoHifRsvCnt.u4Addr, &value);
	ple_rpg_hif = (value & rMT7961DmashdlCfg.rHifPgInfoHifRsvCnt.u4Mask) >>
		  rMT7961DmashdlCfg.rHifPgInfoHifRsvCnt.u4Shift;
	ple_upg_hif = (value & rMT7961DmashdlCfg.rHifPgInfoHifSrcCnt.u4Mask) >>
		rMT7961DmashdlCfg.rHifPgInfoHifSrcCnt.u4Shift;
	DBGLOG(HAL, INFO,

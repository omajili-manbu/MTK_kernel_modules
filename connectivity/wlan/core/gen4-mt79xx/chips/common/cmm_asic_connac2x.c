...
#else /* !CFG_MTK_WIFI_DRV_OWN_MODE_CONTROL */
...
void asicConnac2xUmdaInit(
	struct ADAPTER *prAdapter)
{
#if (CFG_SUPPORT_DEBUG_SOP == 1)
	uint32_t u4UdmaValue = 0;

	HAL_MCR_RD(prAdapter, CONNAC2X_UDMA_DBGCNT_CTRL_DBGCNT_EN_ADDR,
			&u4UdmaValue);
	u4UdmaValue &= ~CONNAC2X_UDMA_DBGCNT_CTRL_DBGCNT_EN_MASK;
	u4UdmaValue |= CONNAC2X_UDMA_DBGCNT_CTRL_DBGCNT_EN_MASK;

	HAL_MCR_WR(prAdapter, CONNAC2X_UDMA_DBGCNT_CTRL_DBGCNT_EN_ADDR,
			u4UdmaValue);
#endif
}
	...
	struct RX_DESC_OPS_T *prRxDescOps)
{
	...
	prRxDescOps->nic_rxd_get_bssid = nic_rxd_v2_get_bssid;
	...
	IN struct ADAPTER *prAdapter)
{
	...
	if (CONNAC2X_RXV_FROM_RX_RPT(prAdapter)) {
		...
		ucRcpi0 = (uint8_t)CONNAC2X_HAL_RXV_GET_RCPI0_RXRPT(u4RxvDw6);
		ucRcpi1 = (uint8_t)CONNAC2X_HAL_RXV_GET_RCPI1_RXRPT(u4RxvDw6);
		ucRcpi2 = (uint8_t)CONNAC2X_HAL_RXV_GET_RCPI2_RXRPT(u4RxvDw6);
		ucRcpi3 = (uint8_t)CONNAC2X_HAL_RXV_GET_RCPI3_RXRPT(u4RxvDw6);
		ucRxNum = (uint8_t)CONNAC2X_HAL_RXV_GET_NUM_RX_RXRPT(u4RxvDw2);
	} else {
		if (CONNAC2X_RX_DBG_INFO_GRP3(prAdapter)) {
			...
			ucRcpi0 = (uint8_t)CONNAC2X_HAL_RX_VECTOR_GET_RCPI0_V2(
				prGroup3);
			ucRcpi1 = (uint8_t)CONNAC2X_HAL_RX_VECTOR_GET_RCPI1_V2(
				prGroup3);
			ucRcpi2 = (uint8_t)CONNAC2X_HAL_RX_VECTOR_GET_RCPI2_V2(
				prGroup3);
			ucRcpi3 = (uint8_t)CONNAC2X_HAL_RX_VECTOR_GET_RCPI3_V2(
				prGroup3);
			ucRxNum = (uint8_t)CONNAC2X_HAL_RX_VECTOR_GET_NUM_RX_V2(
				prGroup3);
		} else {
			...
			ucRcpi0 = (uint8_t)CONNAC2X_HAL_RX_VECTOR_GET_RCPI0(
				prSwRfb->prRxStatusGroup5);
			ucRcpi1 = (uint8_t)CONNAC2X_HAL_RX_VECTOR_GET_RCPI1(
				prSwRfb->prRxStatusGroup5);
			ucRcpi2 = (uint8_t)CONNAC2X_HAL_RX_VECTOR_GET_RCPI2(
				prSwRfb->prRxStatusGroup5);
			ucRcpi3 = (uint8_t)CONNAC2X_HAL_RX_VECTOR_GET_RCPI3(
				prSwRfb->prRxStatusGroup5);
			ucRxNum = (uint8_t)CONNAC2X_HAL_RX_VECTOR_GET_NUM_RX(
				prSwRfb->prRxStatusGroup5);
		}
	}
	...
	if (ucRxNum == 0) {
		...
		case RCPI_MODE_MAX:
			if (ucRcpi0 >= RCPI_MEASUREMENT_NOT_AVAILABLE)
				ucRcpiValue = ucRcpi1;
			else if (ucRcpi1 >= RCPI_MEASUREMENT_NOT_AVAILABLE)
				ucRcpiValue = ucRcpi0;
			else if (ucRcpi0 > ucRcpi1)
				ucRcpiValue = ucRcpi0;
			else
				ucRcpiValue = ucRcpi1;
			break;
			       ...
			       IN uint8_t ucBssIndex)
{
	...
	uint8_t ucNss = 0;
	uint8_t ucStbc = 0;
	uint32_t u4PhyRate = 0;
	uint32_t u4MaxPhyRate = 0;
	/* Rate
	 * Bit Number 2
	 * Unit 500 Kbps
	 */
	...
	if (ucRxMode == RX_VT_LEGACY_CCK || ucRxMode == RX_VT_LEGACY_OFDM) {
		...
		if (ucRxMode >= RX_VT_HE_MODE) {
			wlanQueryRateByTable(ucRxMode, ucMcs, ucFrMode,
				ucShortGI, ucNss, &u4PhyRate, &u4MaxPhyRate);
		} else
...
{
	if (glGetRstReason(NULL) == RST_BT_TRIGGER) {
...
int wlan_func_on_by_chrdev(void)
{
	return wlanFuncOn();
}

int wlan_func_off_by_chrdev(void)
{
	wlanFuncOff();
	return 0;
}

 ...
 *******************************************************************************
 */
#define TEST_MODE_NOT_SUPPORTED 0x80
				  ...
				  *prAdapter, IN enum ENUM_IMG_DL_IDX_T eDlIdx)
{
	...
	/* Acquire semaphore for protect wifi/bt download concurrent racing
	 * issue (sec boot related).
	 */
	if (wlanImageStartProtect(prAdapter, TRUE) != WLAN_STATUS_SUCCESS) {
		DBGLOG(INIT, ERROR, "[RAM] SecProtectFWDL failed!\n");
		goto END;
	}
...
/* To support dynamic memory map for WiFi RAM code download::End */
	...
	/* Release semaphore, not just protect wifi start cmd/event is due to
	 * code maintainability, protect all the period for wifi start will be
	 * more clear (just more protect a little pure sw code, so it's fine).
	 */
	rCfgStatus = wlanImageStartProtect(prAdapter, FALSE);
...
#pragma message("ROM code supports no SEM-CTRL for ROM patch download")
	...
	do {
		...
		/*
		 * Acquire semaphore for protect wifi/bt download concurrent
		 * racing issue(sec boot related).
		 */
		u4Status = wlanImageStartProtect(prAdapter, TRUE);
		if (u4Status != WLAN_STATUS_SUCCESS) {
			DBGLOG(INIT, ERROR, "[Patch] SecProtectFWDL failed!\n");
			goto END;
		}
...
/* Dynamic memory map::End */
	...
	/* Release semaphore, not just protect patch finish cmd/event is due to
	 * code maintainability, protect all the period for patch finish will be
	 * more clear(just more protect a little pure sw code, so it's fine).
	 */
...
void fwDlGetReleaseInfoSection(struct ADAPTER *prAdapter, uint8_t *pucStartPtr)
{
	...
	prAdapter->prGlueInfo->fgSupportTestMode = TRUE;
#endif
	...
	while (u2Offset < prFirstInfo->u2Len) {
		...
		case 0x02:
			...
			break;
#if CFG_WIFI_TESTMODE_FW_REDOWNLOAD
		case 0x04: /* FW Featuer bitmap TAG ID*/
			fwDlGetReleaseFeatureMap(prAdapter, prRelInfo,
				pucCurPtr);
			break;
#endif
		default:
...
#if CFG_WIFI_TESTMODE_FW_REDOWNLOAD
void fwDlGetReleaseFeatureMap(struct ADAPTER *prAdapter,
				struct HEADER_RELEASE_INFO *prRelInfo,
				uint8_t *pucStartPtr)
{
	int i = 0;
	...
	copySize = (prRelInfo->u2Len >
			sizeof(prAdapter->rVerInfo.aucReleaseFeatureMap)) ?
			sizeof(prAdapter->rVerInfo.aucReleaseFeatureMap)
			: prRelInfo->u2Len;

	kalMemZero(&prAdapter->rVerInfo.aucReleaseFeatureMap,
			sizeof(prAdapter->rVerInfo.aucReleaseFeatureMap));
	kalMemCopy(&prAdapter->rVerInfo.aucReleaseFeatureMap,
			pucStartPtr, copySize);

	/* Check if the first byte is 0x80,does't support test mode */
	if (prAdapter->rVerInfo.aucReleaseFeatureMap[0] ==
		TEST_MODE_NOT_SUPPORTED)
		prAdapter->prGlueInfo->fgSupportTestMode = FALSE;

	for (i = 0; i < 4; i++)
		DBGLOG(INIT, INFO, "Release featuremap[%d]: 0x%x\n",
			i, prAdapter->rVerInfo.aucReleaseFeatureMap[i]);
}
#endif

	...
	IN struct ADAPTER *prAdapter)
{
	...
	ucRcpi0 = (uint8_t)((prGroup3->u4RxVector[3] & RX_VT_RCPI0_MASK)
			>> RX_VT_RCPI0_OFFSET);
	ucRcpi1 = (uint8_t)((prGroup3->u4RxVector[3] & RX_VT_RCPI1_MASK)
			>> RX_VT_RCPI1_OFFSET);
	ucRxNum = (uint8_t)((prGroup3->u4RxVector[0] & RX_VT_NUM_RX_MASK)
			>> RX_VT_NUM_RX_OFFSET);
	...
	if (ucRxNum == 0) {
		...
		case RCPI_MODE_AVG: /*Not recommended for CBW80+80*/
			if (ucRcpi0 <= RCPI_HIGH_BOUND &&
				...
				ucRcpiValue = (uint8_t)(ucRcpi0 + ucRcpi1) / 2;
			else
				if (ucRcpi0 <= RCPI_HIGH_BOUND)
					ucRcpiValue = ucRcpi0;
				else
			       ...
			       IN uint8_t ucBssIndex)
{
	...
	noise_level = ((prRxStatusGroup3->u4RxVector[5] &
		RX_VT_NF0_MASK) >> 1);

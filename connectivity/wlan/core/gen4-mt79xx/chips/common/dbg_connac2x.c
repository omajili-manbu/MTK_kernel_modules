		...
		uint32_t u4StatGroup)
{
	...
	if (u4StatGroup & 0x0040) {
		...
		for (ucIdx = 0; ucIdx < AGG_RANGE_SEL_NUM; ucIdx++) {
			ucInt = (ucIdx >> 2) % AGG_RANGE_SEL_4BYTE_NUM;
	...
	kalMemZero(prAdapter->arMibInfo, sizeof(prAdapter->arMibInfo));

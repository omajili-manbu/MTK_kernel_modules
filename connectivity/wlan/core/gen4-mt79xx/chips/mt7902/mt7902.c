...
struct ONE_TIME_CAL_GROUP_DEF_ENTRY mt7902_group_freq_def_arr[] = {
	{0, 2300, 2499},
	{1, 5170, 5250},
	{2, 5251, 5330},
	{3, 5331, 5490},
...
void mt7902GetFlavorVer(struct GLUE_INFO *prGlueInfo,
	uint8_t *flavor_str)
{
	int ret;
	...
	uint8_t *pucNameIdx, uint8_t ucMaxNameIdx)
{
	...
	mt7902GetFlavorVer(prGlueInfo, flavor_str);
	...
	for (sub_idx = 0; apucNameTable[sub_idx]; sub_idx++) {
		if (((*pucNameIdx) + 3) < ucMaxNameIdx) {
			...
			ret = snprintf(*(apucName + (*pucNameIdx)),
				...
				apucNameTable[sub_idx], chip_id, flavor_str);
			if (ret < 0)
				DBGLOG(INIT, ERROR,
...
void mt7902ConstructIdxLogBinName(struct GLUE_INFO *prGlueInfo,
	uint8_t **apucName)
{
	struct ADAPTER *prAdapter = NULL;
	struct mt66xx_chip_info *prChipInfo = NULL;
	...
	/* ex: WIFI_RAM_CODE_MT7902_1_1_idxlog.bin */
...
#if CONFIG_PHYS_ADDR_T_64BIT
	.ring_cnt_base_ext_ctrl = mt7902WfdmaRingCntBaseExtCtrl,
#endif
...
#endif
#if (CFG_SUPPORT_FW_IDX_LOG_TRANS == 1)
	.constrcutIdxLogBin = mt7902ConstructIdxLogBinName,
#endif /* CFG_SUPPORT_FW_IDX_LOG_TRANS */
};
...
#if CFG_SUPPORT_ONE_TIME_CAL
	.prGroupFreqDefArr = mt7902_group_freq_def_arr,
#endif

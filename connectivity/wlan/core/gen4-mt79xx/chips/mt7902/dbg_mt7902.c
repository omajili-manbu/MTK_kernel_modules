...
extern struct MT7902_DMASHDL_CFG rMT7902DmashdlCfg;
...
#if defined(_HIF_PCIE) || defined(_HIF_AXI) || defined(_HIF_USB)
void mt7902_DmashdlGetRefill(struct ADAPTER *prAdapter)
{
	uint32_t u4Val = 0;

	HAL_MCR_RD(prAdapter,
		       rMT7902DmashdlCfg.rGroup0RefillDisable.u4Addr, &u4Val);
	DBGLOG(HAL, INFO, "DMASHDL ReFill Control (0x%08x): 0x%08x\n",
		rMT7902DmashdlCfg.rGroup0RefillDisable.u4Addr, u4Val);
}

void mt7902_DmashdlGetPktMaxPage(struct ADAPTER *prAdapter)
{
	uint32_t u4Val = 0;
	...
	HAL_MCR_RD(prAdapter,
		       rMT7902DmashdlCfg.rPlePacketMaxSize.u4Addr, &u4Val);

	ple_pkt_max_sz = (u4Val &
	rMT7902DmashdlCfg.rPlePacketMaxSize.u4Mask) >>
	rMT7902DmashdlCfg.rPlePacketMaxSize.u4Shift;
	pse_pkt_max_sz = (u4Val &
	rMT7902DmashdlCfg.rPsePacketMaxSize.u4Mask) >>
	rMT7902DmashdlCfg.rPsePacketMaxSize.u4Shift;

	DBGLOG(HAL, INFO, "DMASHDL PACKET_MAX_SIZE (0x%08x): 0x%08x\n",
		rMT7902DmashdlCfg.rPlePacketMaxSize.u4Addr, u4Val);
	DBGLOG(HAL, INFO, "PLE/PSE packet max size=0x%03x/0x%03x\n",
		ple_pkt_max_sz, pse_pkt_max_sz);
}

void mt7902_DmashdlGetGroupControl(struct ADAPTER *prAdapter,
					       uint8_t ucGroup)
{
	uint32_t u4Addr;
	uint32_t u4Val = 0;
	...
	u4Addr = rMT7902DmashdlCfg.rGroup0ControlMaxQuota.u4Addr +
		(ucGroup << 2);
...
uint32_t mt7902_DmashdlGetRsvCount(struct ADAPTER *prAdapter,
					       uint8_t ucGroup)
{
	uint32_t u4Addr;
	uint32_t u4Val = 0;
	...
	u4Addr = rMT7902DmashdlCfg.rStatusRdGp0RsvCnt.u4Addr + (ucGroup << 2);

	HAL_MCR_RD(prAdapter, u4Addr, &u4Val);

	rsv_cnt = (u4Val & rMT7902DmashdlCfg.rStatusRdGp0RsvCnt.u4Mask) >>
		rMT7902DmashdlCfg.rStatusRdGp0RsvCnt.u4Shift;
...
uint32_t mt7902_DmashdlGetSrcCount(struct ADAPTER *prAdapter,
					       uint8_t ucGroup)
{
	uint32_t u4Addr;
	uint32_t u4Val = 0;
	...
	u4Addr = rMT7902DmashdlCfg.rStatusRdGp0SrcCnt.u4Addr + (ucGroup << 2);

	HAL_MCR_RD(prAdapter, u4Addr, &u4Val);

	src_cnt = (u4Val & rMT7902DmashdlCfg.rStatusRdGp0SrcCnt.u4Mask) >>
		rMT7902DmashdlCfg.rStatusRdGp0SrcCnt.u4Shift;

	DBGLOG(HAL, TRACE, "\tsrc_cnt = 0x%03x\n", src_cnt);
	return src_cnt;
}

void mt7902_DmashdlGetPKTCount(struct ADAPTER *prAdapter,
					   uint8_t ucGroup)
{
	uint32_t u4Addr;
	uint32_t u4Val = 0;
	...
	if ((ucGroup & 0x1) == 0)
		u4Addr = rMT7902DmashdlCfg.rRdGroupPktCnt0.u4Addr +
		(ucGroup << 1);
	else
		u4Addr = rMT7902DmashdlCfg.rRdGroupPktCnt0.u4Addr +
		((ucGroup-1) << 1);
	...
	struct ADAPTER *prAdapter)
{
	...
	mt7902_DmashdlGetRefill(prAdapter);
	mt7902_DmashdlGetPktMaxPage(prAdapter);

	HAL_MCR_RD(prAdapter,
			rMT7902DmashdlCfg.rErrorFlagCtrl.u4Addr, &value);

	DBGLOG(HAL, INFO, "DMASHDL ERR FLAG CTRL(0x%08x): 0x%08x\n",
	       rMT7902DmashdlCfg.rErrorFlagCtrl.u4Addr, value);

	for (idx = 0; idx <= ENUM_DMASHDL_GROUP_14; idx++) {
		if (rMT7902DmashdlCfg.afgRefillEn[idx] == 0)
			continue;
		DBGLOG(HAL, INFO, "Group %d info:\n", idx);
		mt7902_DmashdlGetGroupControl(prAdapter, idx);
		rsv_cnt = mt7902_DmashdlGetRsvCount(prAdapter, idx);
		src_cnt = mt7902_DmashdlGetSrcCount(prAdapter, idx);
		mt7902_DmashdlGetPKTCount(prAdapter, idx);
	...
	DBGLOG(HAL, INFO, "Group %d info:\n", idx);
	mt7902_DmashdlGetGroupControl(prAdapter, idx);
	mt7902_DmashdlGetRsvCount(prAdapter, idx);
	mt7902_DmashdlGetSrcCount(prAdapter, idx);
	mt7902_DmashdlGetPKTCount(prAdapter, idx);

	HAL_MCR_RD(prAdapter,
		       rMT7902DmashdlCfg.rStatusRdFfaCnt.u4Addr, &value);
	ffa_cnt = (value & rMT7902DmashdlCfg.rStatusRdFfaCnt.u4Mask) >>
		rMT7902DmashdlCfg.rStatusRdFfaCnt.u4Shift;
	free_pg_cnt = (value & rMT7902DmashdlCfg.rStatusRdFreePageCnt.u4Mask) >>
		rMT7902DmashdlCfg.rStatusRdFreePageCnt.u4Shift;
	DBGLOG(HAL, INFO, "\tDMASHDL Status_RD(0x%08x): 0x%08x\n",
		rMT7902DmashdlCfg.rStatusRdFreePageCnt.u4Addr, value);
	DBGLOG(HAL, INFO, "\tfree page cnt = 0x%03x, ffa cnt = 0x%03x\n",
		free_pg_cnt, ffa_cnt);

	DBGLOG(HAL, INFO, "DMASHDL Counter Check:\n");
	HAL_MCR_RD(prAdapter,
		       rMT7902DmashdlCfg.rHifPgInfoHifRsvCnt.u4Addr, &value);
	ple_rpg_hif = (value & rMT7902DmashdlCfg.rHifPgInfoHifRsvCnt.u4Mask) >>
		  rMT7902DmashdlCfg.rHifPgInfoHifRsvCnt.u4Shift;
	ple_upg_hif = (value & rMT7902DmashdlCfg.rHifPgInfoHifSrcCnt.u4Mask) >>
		rMT7902DmashdlCfg.rHifPgInfoHifSrcCnt.u4Shift;
	DBGLOG(HAL, INFO,

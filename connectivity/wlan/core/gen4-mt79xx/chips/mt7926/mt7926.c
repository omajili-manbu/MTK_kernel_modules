...
#include "hal_wfsys_reset_mt7926.h"
...
struct ONE_TIME_CAL_GROUP_DEF_ENTRY mt7926_group_freq_def_arr[] = {
	{0, 2300, 2499},
	{1, 5170, 5250},
	{2, 5251, 5330},
	{3, 5331, 5490},
...
struct PCIE_CHIP_CR_MAPPING mt7926_bus2chip_cr_mapping[] = {
	...
	{0x70000000, 0x1c0000, 0x10000}, /* Reserved for CBTOP, can't switch */
	{0x70010000, 0x1d0000, 0x10000}, /* Reserved for CBTOP, can't switch */
	{0x74030000, 0x1e0000, 0x10000}, /* Reserved for CBTOP, can't switch */
	{0x70020000, 0x1f0000, 0x10000}, /* Reserved for CBTOP, can't switch */
	{0x7c020000, 0xd0000, 0x10000}, /* CONN_INFRA, wfdma */
	{0x7c060000, 0xe0000, 0x10000}, /* CONN_INFRA, conn_host_csr_top */
	{0x7c000000, 0xf0000, 0x10000}, /* CONN_INFRA */
	{0x7c500000, 0x60000, 0x20000}, /* dyn map */
};
...
void mt7926GetFlavorVer(struct GLUE_INFO *prGlueInfo,
	uint8_t *flavor_str)
{
	int ret;
	uint32_t flavor_ver = FLAVOR_VERSION_DEFAULT;
	char *suffix = "";

#if CFG_WIFI_TESTMODE_FW_REDOWNLOAD
	if (prGlueInfo->fgTestFwDl)
		suffix = "_TEST";
#endif
	...
	if (ret < 0) {
		...
		flavor_str[0] = FLAVOR_VERSION_DEFAULT + '0';
		flavor_str[1] = '\0';
	}
}

#if (CFG_SUPPORT_FW_IDX_LOG_TRANS == 1)
void mt7926ConstructIdxLogBinName(struct GLUE_INFO *prGlueInfo,
	uint8_t **apucName)
{
	struct ADAPTER *prAdapter = NULL;
	struct mt66xx_chip_info *prChipInfo = NULL;
	uint8_t flavor_str[CFG_FW_FLAVOR_MAX_LEN];
	int32_t s4Result;
	...
	mt7926GetFlavorVer(prGlueInfo, flavor_str);

	/* ex: WIFI_RAM_CODE_MT7926_1_1_idxlog.bin */
	s4Result = snprintf(apucName[0],
		CFG_FW_NAME_MAX_LEN,
	...
	if (s4Result < 0)
		DBGLOG(INIT, ERROR, "Can't setup the IDXLOG file name\n");
}
#endif /* CFG_SUPPORT_FW_IDX_LOG_TRANS */

#if CFG_SUPPORT_WIFI_DL_BT_PATCH
void mt7926ConstructBtPatchName(struct GLUE_INFO *prGlueInfo,
	uint8_t **apucName, uint8_t *pucNameIdx)
{
	...
	s4Result = snprintf(apucName[(*pucNameIdx)],
		CFG_FW_NAME_MAX_LEN, "BT_RAM_CODE_MT%x_1_2_hdr.bin",
		prChipInfo->chip_id);
	if (s4Result < 0)
...
void mt7926ConstructZbPatchName(struct GLUE_INFO *prGlueInfo,
	uint8_t **apucName, uint8_t *pucNameIdx)
{
	struct ADAPTER *prAdapter = NULL;
	struct mt66xx_chip_info *prChipInfo = NULL;
	...
	s4Result = snprintf(apucName[(*pucNameIdx)],
		CFG_FW_NAME_MAX_LEN, "LRWPAN_RAM_CODE_MT%x_hdr.bin",
		prChipInfo->chip_id);
	if (s4Result < 0)
...
struct BUS_INFO mt7926_bus_info = {
	...
	.DmaShdlReInit = mt7902DmashdlReInit,
	.updateTxRingMaxQuota = mt7902UpdateDmashdlQuota,
	.asicUdmaInit = asicConnac2xUmdaInit,
	.asicUdmaRxFlush = asicConnac2xUdmaRxFlush,
#if CFG_CHIP_RESET_SUPPORT
	.asicUsbEpctlRstOpt = mt7926HalUsbEpctlRstOpt,
#endif
#endif /* _HIF_USB */
...
#if CFG_SUPPORT_WIFI_DL_BT_PATCH
	.constructBtPatchName = mt7926ConstructBtPatchName,
	.downloadBtPatch = mt7902DownloadBtPatch,
#endif
#if CFG_SUPPORT_WIFI_DL_ZB_PATCH
	.constructZbPatchName = mt7926ConstructZbPatchName,
	.downloadZbPatch = mt7902DownloadZbPatch,
#endif
#if (CFG_SUPPORT_FW_IDX_LOG_TRANS == 1)
	.constrcutIdxLogBin = mt7926ConstructIdxLogBinName,
#endif /* CFG_SUPPORT_FW_IDX_LOG_TRANS */
};
...
struct CHIP_DBG_OPS mt7926DebugOps = {
	...
	.showDmaschInfo = mt7902_show_dmashdl_info,
#else
	.showDmaschInfo = NULL,
	...
	.show_debug_sop_info = mt7926_show_debug_sop_info,
#endif
};
...
#if CFG_CHIP_RESET_SUPPORT
	.asicWfsysRst = mt7926HalCbtopRguWfRst,
	.asicPollWfsysSwInitDone = mt7926HalPollWfsysSwInitDone,
#if defined(_HIF_SDIO)
...
#if CFG_SUPPORT_ONE_TIME_CAL
	.prGroupFreqDefArr = mt7926_group_freq_def_arr,
#endif

#if defined(_HIF_PCIE) || defined(_HIF_AXI)
	/* owner set true when feature is ready. */
	.fgIsSupportL0p5Reset = TRUE,
	.dmashdlQuotaDecision = mt7902dmashdlQuotaDecision,
#elif defined(_HIF_USB)
	.fgIsSupportL0p5Reset = TRUE,
	.dmashdlQuotaDecision = mt7902dmashdlQuotaDecision,

...
void gps_dl_hw_print_hw_status(enum gps_dl_link_id_enum link_id, bool dump_rf_cr)
{
	...
	/* Dump early recorded usrt status for DE check */
	GDL_LOGXW(link_id, "dump early recorded usrt status");
	gps_dl_hw_print_usrt_status_struct(link_id, &g_gps_early_rec_usrt_status[link_id]);
...
void gps_dl_hw_do_gps_a2z_dump(void)
{
	...
	GDL_HW_RD_GPS_REG(0x80072200);
	GDL_HW_RD_GPS_REG(0x80072204);
	GDL_HW_RD_GPS_REG(0x80072208);
	GDL_HW_RD_GPS_REG(0x8007220c);
	GDL_HW_RD_GPS_REG(0x80072220);

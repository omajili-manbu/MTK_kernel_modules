...
#define BGF_ON_PRINT_LINE_MAX (8)
	...
	for (flag = flag_start; flag < (flag_start + len); flag++) {
		...
		gps_dl_bus_wr_opt(GPS_DL_CONN_INFRA_BUS,
			CONN_HOST_CSR_TOP_CR_HOSTCSR2BGF_ON_DBG_SEL_ADDR, flag, 0);
		out = gps_dl_bus_rd_opt(GPS_DL_CONN_INFRA_BUS,
			CONN_HOST_CSR_TOP_BGF_MONFLG_ON_OUT_ADDR, 0);
...
void gps_dl_hw_dep_dump_host_csr_gps_info(void)
{
	...
	for (i = 0; i < g_gps_v06x_bgf_on_dump_range_num; i++) {
		p_range = &g_gps_v06x_bgf_on_dump_range_ptr[i];
		gps_dl_hw_dep_set_bgf_on_dbg_sel(p_range->flag_start, p_range->len);
	}

	gps_dl_hw_dep_dump_cirq_info();
}

void gps_dl_hw_dep_dump_cirq_info(void)
{
	/* Max 20 elements.*/
	unsigned int out[20];
	int i;
	...
	for (i = 0; i < g_gps_cirq_dump_range_num; i++) {
		out[i] = gps_dl_bus_rd_opt(GPS_DL_CONN_INFRA_BUS, g_gps_cirq_dump_range_ptr[i], 0);
		print_index += snprintf(print_buf + print_index, sizeof(print_buf) - print_index, "0x%08x ", out[i]);
	}

	GDL_LOGW("dump: %s", print_buf);
}

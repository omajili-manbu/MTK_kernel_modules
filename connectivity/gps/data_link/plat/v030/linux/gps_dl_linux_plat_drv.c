...
const struct of_device_id gps_dl_of_ids[] = {
	...
	{ .compatible = "mediatek,mt6881-gps", },
...
static wait_queue_head_t gps_dl_suspend_waitq;
static atomic_t gps_dl_in_suspend = ATOMIC_INIT(0);

static bool gps_dl_is_in_suspend(void)
{
	return atomic_read(&gps_dl_in_suspend) != 0;
}
...
enum gps_dl_sys_suspend_status gps_dl_wait_if_system_in_suspend(int timeout_ms)
{
	...
	if (!gps_dl_is_in_suspend())
		return GPS_DL_SYS_NOT_IN_SUSPEND;

	ret = wait_event_timeout(
		gps_dl_suspend_waitq,
		!gps_dl_is_in_suspend(),
		msecs_to_jiffies(timeout_ms)
	);

	if (ret > 0)
		return GPS_DL_SYS_SUSPEND_EXITED;
	else
		return GPS_DL_SYS_SUSPEND_TIMEOUT;
}
		...
		unsigned long event, void *dummy)
{
	...
	case PM_SUSPEND_PREPARE:
		atomic_set(&gps_dl_in_suspend, 1);
	...
	case PM_POST_SUSPEND:
		atomic_set(&gps_dl_in_suspend, 0);
		wake_up_all(&gps_dl_suspend_waitq);
...
#if GPS_DL_HAS_MCUDL
	/* Initialize suspend waitqueue for system suspend handling */
	init_waitqueue_head(&gps_dl_suspend_waitq);
...
static struct wakeup_source *g_gps_dl_dev_wake_lock_ptr;
const char c_gps_dl_wake_lock_name[] = "gpsdl_wakelock";
const char c_gps_dl_ctrld_wake_lock_name[] = "gpsdl_kctrld";
const char c_gps_dl_dev_wake_lock_name[] = "gpsdl_dev";
void gps_dl_wake_lock_init(void)
{
	...
	g_gps_dl_dev_wake_lock_ptr = wakeup_source_register(NULL, c_gps_dl_dev_wake_lock_name);
#else
	...
	g_gps_dl_dev_wake_lock_ptr = wakeup_source_register(c_gps_dl_dev_wake_lock_name);
#endif
}
...
void gps_dl_wake_lock_deinit(void)
{
	...
	wakeup_source_unregister(g_gps_dl_dev_wake_lock_ptr);
}
...
void gps_dl_dev_wake_lock_hold(bool hold)
{
	GDL_LOGD_ONF("hold = %d", hold);
	if (hold)
		__pm_stay_awake(g_gps_dl_dev_wake_lock_ptr);
	else
		__pm_relax(g_gps_dl_dev_wake_lock_ptr);
}

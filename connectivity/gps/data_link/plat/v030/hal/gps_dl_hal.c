...
struct gps_dl_hw_usrt_status_struct g_gps_early_rec_usrt_status[GPS_DATA_LINK_NUM];
bool gps_dl_hal_mcub_flag_handler(enum gps_dl_link_id_enum link_id)
{
	...
	while (1) {
		...
		if (d2a.flag & GPS_MCUB_D2AF_MASK_DSP_RAMCODE_READY) {
			...
			/* rec usrt status after dsp ram ok for DE check*/
			gps_dl_hw_save_usrt_status_struct(link_id, &g_gps_early_rec_usrt_status[link_id]);

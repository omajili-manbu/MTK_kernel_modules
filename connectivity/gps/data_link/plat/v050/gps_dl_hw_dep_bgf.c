...
bool gps_dl_hw_gps_common_on_inner(void)
{
	...
	*/
	if ((gps_dl_hal_get_conn_infra_ver() == GDL_HW_CONN_INFRA_VER_MT6878) ||
		...
		(gps_dl_hal_get_conn_infra_ver() == GDL_HW_CONN_INFRA_VER_MT6881)) {
		if (adie_ver == 0x6631) {
	...
	if (adie_ver == 0x6686) {
		...
		else if (gps_dl_hal_get_conn_infra_ver() == GDL_HW_CONN_INFRA_VER_MT6881)
			gps_dl_hw_dep_gps_control_adie_on_6881();
	}
#endif
...
unsigned int gps_dl_hw_gps_get_adie_id_from_conninfra(void)
{
	...
	* so we need to check conninfra_get_ic_info(CONNSYS_GPS_ADIE_CHIPID) again for GNSS part
	*/
	if ((chip_ver == 0x6878) || (chip_ver == 0x6858) || (chip_ver == 0x6881)) {
		if (adie_ver == 0x6631) {
			adie_ver = conninfra_get_ic_info(CONNSYS_GPS_ADIE_CHIPID);
			if (adie_ver == 0x6686)
				adie_ver = 0x6686;

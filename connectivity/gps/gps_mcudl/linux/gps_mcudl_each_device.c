	...
	char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if (mutex_lock_interruptible(&dev->i_buf_lock)) {
		MDL_LOGXW_DRW(link_id, "len = %ld, pid = %d, mtx_int", count, pid);
		print_log = true;
		retlen = -ERESTARTSYS;
		goto _ret;
	}
	if (pid != dev->last_read_pid) {
		MDL_LOGXD_DRW(link_id, "len = %ld, pid = %d, last_read_pid = %d",
			count, pid, dev->last_read_pid);
		dev->last_read_pid = pid;
	}
	...
	const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if (mutex_lock_interruptible(&dev->o_buf_lock)) {
		MDL_LOGXW_DRW(link_id, "len = %ld, pid = %d, mtx_ret", count, pid);
		print_log = true;
		retlen = -ERESTARTSYS;
		goto _ret;
	}
	if (pid != dev->last_write_pid) {
		MDL_LOGXD_DRW(link_id, "len = %ld, pid = %d, last_write_pid = %d",
			count, pid, dev->last_write_pid);
		dev->last_write_pid = pid;
	}
...
int gps_mcudl_cdev_setup(struct gps_mcudl_each_device *dev, enum gps_mcudl_xid xid)
{
	...
	mutex_init(&dev->i_buf_lock);
	mutex_init(&dev->o_buf_lock);

...
/*L19 code for L19-37 by luziyang at 2021.11.29 start*/
#include "linux/fcntl.h"
#include "linux/fs.h"
#define BOARD_HW_INFO_FILE      "/sys/devices/virtual/huaqin/interface/hw_info/pcba_config"
/*L19 code for L19-37 by luziyang at 2021.11.29 end*/
...
/*L19 code for L19-37 by luziyang at 2021.11.29 start*/
/*function: INT32 wmt_get_cfg_name(void) Return: 0 - device is L19N    1 - device is L19_GL	2-device is L19_IN*/
INT32 wmt_get_cfg_name(void)
{
	INT32 ret = -1;
  	mm_segment_t fs;
  	struct file *fd = NULL;
	...
	if ( fd->f_op->read(fd, ptr, PAGE_SIZE << 2, &fd->f_pos) <= 0) {
		...
		WMT_INFO_FUNC("board config info:%s\n", ptr);
          	if (strstr(ptr, "L19N") != NULL) {
			//Use L19N config file.
        		osal_strncat(&(gDevWmt.cWmtcfgName[0]), CUST_CFG_WMT_SOC_L19N, osal_sizeof(CUST_CFG_WMT_SOC_L19N));
			ret = 0;
		/*L19 code for L19-75 by luziyang at 2021.12.29 start*/
		}else if(strstr(ptr, "L19P") != NULL){
                        //For now there is no requirement for L19P, so return -1 and use defualt config file.
                        osal_strncat(&(gDevWmt.cWmtcfgName[0]), CUST_CFG_WMT_SOC, osal_sizeof(CUST_CFG_WMT_SOC));
                        ret = -1;
                /*Enter next else-if statement means device is L19, so just judge the Region/Country.*/
                }else if(strstr(ptr, "GLOBAL") != NULL){
                        //Use L19 GLOBAL config file.
                        osal_strncat(&(gDevWmt.cWmtcfgName[0]), CUST_CFG_WMT_SOC_L19_GL, osal_sizeof(CUST_CFG_WMT_SOC_L19_GL));
                        ret = 1;
                }else if(strstr(ptr, "IN") != NULL){
                        //Use L19 IN config file.
                        osal_strncat(&(gDevWmt.cWmtcfgName[0]), CUST_CFG_WMT_SOC_L19_IN, osal_sizeof(CUST_CFG_WMT_SOC_L19_IN));
                        ret = 2;
                }else{
			WMT_INFO_FUNC("Use default WMT config file!\n");
                        osal_strncat(&(gDevWmt.cWmtcfgName[0]), CUST_CFG_WMT_SOC, osal_sizeof(CUST_CFG_WMT_SOC));
		}
                /*L19 code for L19-75 by luziyang at 2021.12.29 end*/
	}
        filp_close(fd,NULL);
	set_fs(fs);
	return ret;

...
#else
		timer_delete_sync(&cif_dev->fw_own_timer);
#endif
	} else
		BTMTK_DBG_LIMITTED("%s: not create yet", __func__);
}
...
static int btmtk_uart_send_query_uart_cmd(struct hci_dev *hdev)
{
	...
	ret = btmtk_main_send_cmd(bdev, cmd, GETBAUD_CMD_LEN, event, GETBAUD_EVT_LEN, DELAY_TIMES,
			...
			RETRY_TIMES, BTMTK_TX_CMD_FROM_DRV, CMD_NO_NEED_FILTER);
#else
			SEND_RETRY_ONE_TIMES_500MS, BTMTK_TX_PKT_SEND_NO_ASSERT, CMD_NO_NEED_FILTER);
#endif
	if (ret < 0) {
...
exit:
	/* clear status for fail case, avoid next bt on without connv3_pwr_on */
	bdev->reset_type = 0;
	if (btmtk_get_chip_state(bdev) == BTMTK_STATE_DISCONNECT
...

	{
		...
		if (wakeup_count > process_count) {
			...
			if (!IS_ERR(cif_dev->tx_task) && atomic_read(&cif_dev->thread_status)) {
				...
				BTMTK_WARN("%s: TX Debug: tx thread already stopped", __func__);
		}
	}
...
#else
		timer_delete_sync(&debug->debug_timer);
#endif
	} else {
		spin_unlock_irqrestore(&debug->debug_lock, flags);
	}
...
#if (KERNEL_VERSION(4, 15, 0) > LINUX_VERSION_CODE)
	init_timer(&debug->debug_timer);
	debug->debug_timer.function = btmtk_tx_debug_timer_callback;
	debug->debug_timer.data = (unsigned long)debug;
#else
...
#else
			timer_delete(&debug->debug_timer);
#endif
...
#else
			timer_delete(&cif_dev->fw_own_timer);
#endif
		} else
			BTMTK_DBG_LIMITTED("%s: not create yet", __func__);
...
static int btmtk_pm_notification(struct notifier_block *this, unsigned long event, void *ptr)
{
	...
	if (g_sbdev == NULL) {
		BTMTK_INFO("event = %ld, g_sbdev is NULL", event);
		return ret;
	}

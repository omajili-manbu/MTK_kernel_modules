	...
	case COMBO_IOCTL_BT_INTTRX:
		...
		if (copy_from_user(ioc_buf, (uint8_t __user *)arg, IOCTL_BT_HOST_INTTRX_SIZE))
			...
			if (ioc_buf[0] == 0x01 && ioc_buf[1] == 0x2D && ioc_buf[2] == 0xFC) {
				...
				if (ioc_buf[4] == 0x05) {
					BTMTK_INFO_RAW(g_sbdev, ioc_buf, ioc_buf[6] + 7, "%s: len[%d] TX: ", __func__, ioc_buf[6] + 7);
					if (ioc_buf[6] > 240) {
						BTMTK_ERR("eids buffer is large than expected");
						retval = -EFAULT;
					} else
						btmtk_send_finder_eids(ioc_buf, ioc_buf[6] + 7);
				} else if (ioc_buf[4] == 0x06) {
					BTMTK_INFO_RAW(g_sbdev, ioc_buf, ioc_buf[5] + 6, "%s set header: len[%d] TX: ", __func__, ioc_buf[5] + 6);
					btmtk_send_finder_cmd(ioc_buf, ioc_buf[5]+6);
				} else if (ioc_buf[4] == 0x04) {
					BTMTK_INFO_RAW(g_sbdev, ioc_buf, ioc_buf[3] + 4, "%s set intervel: len[%d] TX: ", __func__, ioc_buf[3] + 4);
					btmtk_send_finder_cmd(ioc_buf, ioc_buf[3]+4);
				}
			} else
				retval = -EFAULT;
		}
...
inline void btmtk_chardev_exit(void)
{
	return BT_exit();
}

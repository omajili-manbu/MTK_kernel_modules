...
int btmtk_intcmd_system_status(unsigned char system_state)
{
	...
	int ret = 0;
	int pkt_type = BTMTK_TX_PKT_SEND_DIRECT;

	if (system_state == WMT_PARA_RESUME)
		pkt_type = BTMTK_TX_CMD_FROM_DRV;

	BTMTK_INFO("%s: system_state[%d], pkt_type[%d]", __func__, system_state, pkt_type);
	...
	ret = btmtk_main_send_cmd(g_sbdev,
			...
			DELAY_TIMES, RETRY_TIMES, pkt_type, CMD_NO_NEED_FILTER);
...
int btmtk_connv3_sub_drv_init(void)
{
	struct btmtk_uart_dev *cif_dev = NULL;
	...
	BTMTK_DBG("%s start", __func__);
	if (!bdev) {
		BTMTK_ERR("[ERR] %s: bdev is NULL", __func__);
		return -1;
	}

	cif_dev = (struct btmtk_uart_dev *)bdev->cif_dev;
	if (!cif_dev) {
		BTMTK_ERR("[ERR] %s: cif_dev is NULL", __func__);
		return -1;
	}

	tty = cif_dev->tty;
	if (!tty) {
		BTMTK_ERR("[ERR] %s: tty is NULL", __func__);
		return -1;
	}
...
void btmtk_connv3_sub_drv_deinit(void)
{
	struct btmtk_dev *bdev = g_sbdev;
	BTMTK_DBG("%s start", __func__);
	if (!bdev) {
	...
	connv3_sub_drv_ops_unregister(CONNV3_DRV_TYPE_BT);
}
...
int btmtk_get_rst_pin_out_state(void)
{
	struct bt_gpio *gpio = NULL;
	...
	gpio = &g_platform_prop->rst_gpio;
	if (gpio->gpio_remap_base == NULL) {
		BTMTK_INFO("%s: gpio_remap_base is NULL", __func__);
		gpio->gpio_remap_base = ioremap(gpio->gpio_base, gpio->remap_len);
	}

	if (gpio->gpio_remap_base == NULL) {
		BTMTK_WARN("%s: gpio_remap_base failed", __func__);
		return 0;
	}

	return GET_BIT(CONSYS_REG_READ(gpio->gpio_remap_base + gpio->out.offset), gpio->out.bit);
}
...
int btmtk_send_finder_cmd(u8 *buf, u32 len) {
	int ret = 0;
	uint8_t evt[] = {0x04, 0x0E, 0x04, 0x01, 0x64, 0xFD};
...
int btmtk_uart_launcher_deinit(void)
{
	...
	if (g_sbdev != NULL) {
		...
		cif_dev = (struct btmtk_uart_dev *)g_sbdev->cif_dev;
		if (g_sbdev->interface_state != BTMTK_STATE_DISCONNECT &&
				cif_dev && cif_dev->tty && cif_dev->tty->port && cif_dev->tty->port->count == 0)

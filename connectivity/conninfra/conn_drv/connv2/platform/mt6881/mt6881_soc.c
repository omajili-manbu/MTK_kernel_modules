...
int consys_get_co_clock_type_mt6881(void)
{
	...
	if (conf != NULL && (conf->tcxo_gpio != 0 || conn_hw_env.tcxo_support)) {
#else
	...
	if (conf != NULL)
		...
		pr_info("[%s] conn_hw_env.tcxo_support=%d, %s",
			__func__, conn_hw_env.tcxo_support,
			clock_mng_get_schematic_name(clock_type));
...
int consys_platform_spm_conn_ctrl_mt6881(unsigned int enable)
{
	int ret = 0;
...
int consys_conninfra_on_power_ctrl_mt6881(unsigned int enable)
{
	...
	ret = consys_platform_spm_conn_ctrl_mt6881(enable);
	mdelay(5);
#else
	ret = consys_conninfra_on_power_ctrl_mt6881_gen(enable);
...
void consys_set_if_pinmux_mt6881(unsigned int enable, unsigned int curr_status, unsigned int next_status)
{
	...
	if (enable) {
		consys_set_if_pinmux_mt6881_gen(curr_status, next_status, 1);
	} else {
		consys_set_if_pinmux_mt6881_gen(curr_status, next_status, 0);
	}
...
int consys_reg_init_mt6881(struct platform_device *pdev)
{
	...
	if (node) {
		for (i = 0; i < CONSYS_BASE_ADDR_MAX; i++) {
			base_addr = &conn_reg_mt6881.reg_base_addr[i];
...
int consys_reg_deinit_mt6881(void)
{
	...
	for (i = 0; i < CONSYS_BASE_ADDR_MAX; i++) {
		if (conn_reg_mt6881.reg_base_addr[i].vir_addr) {
			pr_info("[%d] Unmap %s (0x%zx)",
				i, consys_base_addr_index_to_str[i],
				conn_reg_mt6881.reg_base_addr[i].vir_addr);
			iounmap((void __iomem *)conn_reg_mt6881.reg_base_addr[i].vir_addr);
			conn_reg_mt6881.reg_base_addr[i].vir_addr = 0;
		}
	}

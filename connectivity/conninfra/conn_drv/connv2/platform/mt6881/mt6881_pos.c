...
#include "include/mt6881.h" /* For function declaration */
#include "include/mt6881_atf.h"
#include "include/mt6881_consys_reg.h" /* cr base address */
#include "include/mt6881_consys_reg_offset.h" /* cr offset */
 ...
 ********************************************************************************
 */
unsigned int mt6881_chip_info[CHIP_INFO_SIZE];
/*******************************************************************************
...
unsigned int consys_emi_set_remapping_reg_mt6881(
	phys_addr_t con_emi_base_addr,
	phys_addr_t md_shared_emi_base_addr,
	phys_addr_t gps_emi_base_addr)
{
	return consys_emi_set_remapping_reg_mt6881_gen(con_emi_base_addr, md_shared_emi_base_addr,
							gps_emi_base_addr, 16);
}

int consys_conninfra_wakeup_mt6881(void)
{
	return consys_conninfra_wakeup_mt6881_gen();
}

int consys_conninfra_sleep_mt6881(void)
{
	return consys_conninfra_sleep_mt6881_gen();
}

int consys_polling_chipid_mt6881(void)
{
	return consys_polling_chipid_mt6881_gen(NULL);
}

int connsys_d_die_cfg_mt6881(void)
{
	unsigned int d_die_efuse = 0;

	/* Reset conninfra sysram */
	consys_init_conninfra_sysram_mt6881_gen();
	consys_config_multibit_cr_dmux_upd_mt6881_gen();

	/* Read D-die Efuse
	 * AP2CONN_EFUSE_DATA 0x1801_1020
	 * Write to conninfra sysram: CONN_INFRA_SYSRAM_SW_CR_D_DIE_EFUSE(0x1805_3820)
	 */
	connsys_get_d_die_efuse_mt6881_gen(&d_die_efuse);
	CONSYS_REG_WRITE(
		CONN_INFRA_SYSRAM_SW_CR_D_DIE_EFUSE, d_die_efuse);

	connsys_d_die_cfg_mt6881_gen();
					     ...
					     unsigned int next_status)
{
	...
	ret = connsys_adie_clock_buffer_setting_mt6881_gen(
...
int connsys_spi_master_cfg_mt6881(unsigned int curr_status, unsigned int next_status)
{
	if (curr_status != 0)
		return 0;

	if (consys_get_adie_chipid_mt6881() == ADIE_6637) {
		connsys_wt_slp_top_ctrl_adie6637_mt6881_gen();
	} else if (consys_get_adie_chipid_mt6881() == ADIE_6631) {
		connsys_wt_slp_top_ctrl_adie6631_mt6881_gen();
	} else if (consys_get_adie_chipid_mt6881() == ADIE_6686) {
		connsys_wt_slp_top_ctrl_adie6631_mt6881_gen();
		connsys_wt_slp_top_ctrl_adie6686_mt6881_gen();
	}

	return 0;
}

int consys_get_sleep_mode_mt6881(void)
{
	int platform_config;
...
int connsys_a_die_cfg_mt6881(unsigned int curr_status, unsigned int next_status)
{
	...
	int check = 0;
	struct consys_plat_thermal_data_mt6881 input;
	...
	if ((consys_get_adie_chipid_mt6881() == ADIE_6686) && (bt_wifi_fm_on == false)) {
		return 0;
	} else if ((consys_get_adie_chipid_mt6881() != ADIE_6686) && (curr_status != 0)) {
		return 0;
	}

	clock_type = consys_co_clock_type_mt6881();
	...
	memset(&input, 0, sizeof(struct consys_plat_thermal_data_mt6881));
	if (consys_get_adie_chipid_mt6881() == ADIE_6637) {
		connsys_a_die_switch_to_gpio_mode_mt6881_gen();
		connsys_adie_top_ck_en_ctl_mt6881_gen(1);

		if (consys_sema_acquire_timeout_mt6881(CONN_SEMA_RFSPI_INDEX, CONN_SEMA_TIMEOUT)
			== CONN_SEMA_GET_FAIL) {
		...
		connsys_a_die_switch_to_gpio_mode_mt6881_gen();
		connsys_adie_top_ck_en_ctl_mt6881_gen(1);
		consys_m10_srclken_cfg_mt6881_gen(curr_status, next_status, 1);
		...
		if (consys_sema_m4_acquire_timeout_mt6881(0, CONN_SEMA_TIMEOUT)
			== CONN_SEMA_GET_FAIL) {
			connsys_adie_top_ck_en_ctl_mt6881_gen(0);
			consys_sema_release_mt6881(CONN_SEMA_RFSPI_INDEX);
			consys_m10_srclken_cfg_mt6881_gen(curr_status, next_status, 0);
			pr_err("[SPI READ] Require semaphore fail\n");
			return CONNINFRA_SPI_OP_FAIL;
		}
	...
	connsys_a_die_cfg_deassert_adie_reset_mt6881_gen(curr_status, next_status);

	if (consys_get_adie_chipid_mt6881() == ADIE_6686)
		consys_sema_m4_release_mt6881(0);

	check = connsys_a_die_cfg_read_adie_id_mt6881_gen(curr_status, next_status, &adie_id, &hw_ver_id);
	if (check) {
		consys_sema_release_mt6881(CONN_SEMA_RFSPI_INDEX);
		return -1;
	}
	...
	connsys_a_die_efuse_read_get_efuse_info_mt6881_gen(
			curr_status, next_status, mt6881_chip_info,sysram_efuse_list,
			&(input.slop_molecule), &(input.thermal_b),&(input.offset));
	pr_info("slop_molecule=[%d], thermal_b =[%d], offset=[%d]",
		input.slop_molecule, input.thermal_b, input.offset);
	pr_info("Chip info = [%08x, %08x, %08x]",
		mt6881_chip_info[0], mt6881_chip_info[1], mt6881_chip_info[2]);
	update_thermal_data_mt6881(&input);

	connsys_a_die_cfg_PART2_mt6881_gen(curr_status, next_status, hw_ver_id);
	...
	if (consys_get_adie_chipid_mt6881() == ADIE_6637)
		connsys_a_die_switch_to_conn_mode_mt6881_gen();

	conn_hw_env.is_rc_mode = consys_is_rc_mode_enable_mt6881();

	if (consys_get_adie_chipid_mt6881() == ADIE_6637) {
		sleep_mode = consys_get_sleep_mode_mt6881();
		pr_info("sleep_mode = %d\n", sleep_mode);
		connsys_wt_slp_top_power_saving_ctrl_adie6637_mt6881_gen(adie_id, sleep_mode);
	} else {
		connsys_wt_slp_top_power_saving_ctrl_adie6631_mt6881_gen();
	}
	connsys_adie_top_ck_en_ctl_mt6881_gen(0);
...
void connsys_afe_sw_patch_mt6881(void)
{
}

int connsys_afe_wbg_cal_mt6881(void)
{
	...
	if (first_cal == 0)
		return 0;
	first_cal = 0;
	return connsys_afe_wbg_cal_mt6881_gen(CONN_SEMA_RFSPI_INDEX, CONN_SEMA_TIMEOUT);
}

int connsys_subsys_pll_initial_mt6881(void)
{
	return connsys_subsys_pll_initial_xtal_26000k_mt6881_gen();
}

int connsys_low_power_setting_mt6881(unsigned int curr_status, unsigned int next_status)
{
	if (consys_get_adie_chipid_mt6881() == ADIE_6637)
		connsys_adie_clock_buffer_setting(curr_status, next_status);

	if (curr_status == 0) {
		connsys_low_power_setting_mt6881_gen();
	} else {
		/* Not first on */
	}
	/* !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! */
...
int consys_sema_acquire_timeout_mt6881(unsigned int index, unsigned int usec)
{
	unsigned int i;
...
int consys_sema_m4_acquire_timeout_mt6881(unsigned int index, unsigned int usec)
{
	unsigned int i;
...
void consys_sema_release_mt6881(unsigned int index)
{
...
void consys_sema_m4_release_mt6881(unsigned int index)
{
	unsigned long duration;
...
int consys_spi_read_nolock_int_mt6881(enum sys_spi_subsystem subsystem, unsigned int addr,
...
int consys_spi_read_nolock_mt6881(enum sys_spi_subsystem subsystem, unsigned int addr,
				  unsigned int *data)
{
	int ret = 0;
	...
	ret = consys_spi_read_nolock_int_mt6881(subsystem, addr, data, spi_base_addr);
	return ret;
}

int consys_spi_read_mt6881(enum sys_spi_subsystem subsystem, unsigned int addr, unsigned int *data)
{
	if (subsystem == SYS_SPI_FM || subsystem == SYS_SPI_GPS)
		return consys_spi_read_nolock_mt6881(subsystem, addr, data);
	else
		return consys_spi_read_mt6881_atf(subsystem, addr, data);
	...
	ret = consys_spi_read_nolock_mt6881(subsystem, addr, data);
...
int consys_spi_1_read_mt6881(enum sys_spi_subsystem subsystem, unsigned int addr, unsigned int *data)
{
	int ret = 0;
	/* If (Dual A-die) {RFSPI0(with base addr 0x18042XXX)
	 * & RFSPI1(with base addr 0x18046XXX)}(GPS with 6686)
	...
	consys_spi_read_nolock_int_mt6881(subsystem, addr, data, spi_base_addr);
	return ret;
}


int consys_spi_write_nolock_int_mt6881(enum sys_spi_subsystem subsystem, unsigned int addr,
				  unsigned int data, mapped_addr spi_base_addr)
{
#ifndef CONFIG_FPGA_EARLY_PORTING
	int check = 0;
...
int consys_spi_write_nolock_mt6881(enum sys_spi_subsystem subsystem, unsigned int addr, unsigned int data)
{
	int ret = 0;
	/* If (Dual A-die) {RFSPI0(with base addr 0x18042XXX)
	 * & RFSPI1(with base addr 0x18046XXX)}(GPS with 6686)
	...
	ret = consys_spi_write_nolock_int_mt6881(subsystem, addr, data, spi_base_addr);
	return ret;
}


int consys_spi_write_mt6881(enum sys_spi_subsystem subsystem, unsigned int addr, unsigned int data)
{
	if (subsystem == SYS_SPI_FM || subsystem == SYS_SPI_GPS)
		return consys_spi_write_nolock_mt6881(subsystem, addr, data);
	else
		return consys_spi_write_mt6881_atf(subsystem, addr, data);
	...
	ret = consys_spi_write_nolock_mt6881(subsystem, addr, data);
...
int consys_spi_1_write_mt6881(enum sys_spi_subsystem subsystem, unsigned int addr, unsigned int data)
{
	int ret = 0;
	/* If (Dual A-die) {RFSPI0(with base addr 0x18042XXX)
	 * & RFSPI1(with base addr 0x18046XXX)}(GPS with 6686)
...
int consys_spi_update_bits_mt6881(enum sys_spi_subsystem subsystem, unsigned int addr,
				  unsigned int data, unsigned int mask)
{
	...
	if (subsystem == SYS_SPI_FM || subsystem == SYS_SPI_GPS) {
		ret = consys_spi_read_nolock_mt6881(subsystem, addr, &curr_val);
		if (ret) {
		...
		if (change)
			ret = consys_spi_write_nolock_mt6881(subsystem, addr, new_val);

	} else
		ret = consys_spi_update_bits_mt6881_atf(subsystem, addr, data, mask);

	return ret;
}

int consys_spi_1_update_bits_mt6881(enum sys_spi_subsystem subsystem, unsigned int addr,
				  unsigned int data, unsigned int mask)
{
	int ret = 0;
	unsigned int curr_val = 0;
	...
	ret = consys_spi_1_read_mt6881(subsystem, addr, &curr_val);

	new_val = (curr_val & (~mask)) | (data & mask);
	change = (curr_val != new_val);

	if (change) {
		ret = consys_spi_1_write_mt6881(subsystem, addr, new_val);
	}
...
int consys_spi_clock_switch_mt6881(enum connsys_spi_speed_type type)
{
	int check;
	int ret = 0;
...
int consys_subsys_status_update_mt6881(bool on, int radio)
{
	...
	if (consys_sema_acquire_timeout_mt6881(CONN_SEMA_CONN_INFRA_COMMON_SYSRAM_INDEX,
	...
	consys_sema_release_mt6881(CONN_SEMA_CONN_INFRA_COMMON_SYSRAM_INDEX);
	...
	if (on && (radio == CONNDRV_TYPE_BT) &&
	    ...
	    == 0x0) && consys_get_adie_chipid_mt6881() == ADIE_6637)
		consys_pre_cal_restore_mt6881();

	return 0;
}

bool consys_is_rc_mode_enable_mt6881(void)
{
#ifdef CONFIG_FPGA_EARLY_PORTING
	return false;
#else /* CONFIG_FPGA_EARLY_PORTING */
...
#ifndef CONFIG_FPGA_EARLY_PORTING
void consys_spi_write_offset_range_nolock_mt6881(
	...
	unsigned int reg_offset, unsigned int value_offset, unsigned int size)
{
	...
	data2 = data & (~reg_mask);
	data2 = (data2 | value);
	consys_spi_write_nolock_mt6881(subsystem, addr, data2);
}
...
int connsys_adie_top_ck_en_ctl_mt6881(bool onoff)
{
	return connsys_adie_top_ck_en_ctl_mt6881_gen(onoff);
}

static void splitValues(unsigned int val1, unsigned int val2, unsigned char *array) {
    array[0] = (val1 >> 24) & 0xFF;
    array[1] = (val1 >> 16) & 0xFF;
    array[2] = (val1 >> 8) & 0xFF;
    array[3] = val1 & 0xFF;
    array[4] = (val2 >> 24) & 0xFF;
    array[5] = (val2 >> 16) & 0xFF;
    array[6] = (val2 >> 8) & 0xFF;
    array[7] = val2 & 0xFF;
}
...
static int consys_get_chip_info(char *buf, unsigned int size, int print_log)
{
	...
	splitValues(mt6881_chip_info[1], mt6881_chip_info[2], split_info_list);

	ret = snprintf(buf_p, buf_sz,
			"MT6637: [%08x][%02x, %02x, %02x, %02x, %02x, %02x, %02x, %02x]",
				mt6881_chip_info[0],
				split_info_list[0],
				split_info_list[1],
				split_info_list[2],
...
int consys_get_chip_info_mt6881(char *buf, unsigned int size)
{

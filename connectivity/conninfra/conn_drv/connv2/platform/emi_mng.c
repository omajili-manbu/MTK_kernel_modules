...
static void emi_mng_get_gps_emi(struct platform_device *pdev)
{
	...
	if (!node)
		node = of_find_node_by_path("/soc/gps@18c00000");
	...
	if (of_property_read_u64(node, "emi-addr", &phy_addr) == 0)
		connsys_emi_addr_info.gps_emi_phy_addr = phy_addr;
	else if (of_property_read_u32(node, "emi-addr", &phy_addr_32) == 0)
		connsys_emi_addr_info.gps_emi_phy_addr = (u64)phy_addr_32;
	else {
		pr_info("%s: unable to get emi-addr\n", __func__);
		return;
	}

	if (of_property_read_u32(node, "emi-size", &phy_size)) {
		pr_info("%s: unable to get emi-size\n", __func__);
		return;
	}

	connsys_emi_addr_info.gps_emi_size = phy_size;
	pr_info("%s emi_addr %x, emi_size %x\n", __func__,
		connsys_emi_addr_info.gps_emi_phy_addr, phy_size);
}

...
int consys_clock_buffer_ctrl_mt6858(unsigned int enable)
{
	...
	if (vir_addr_consys_gen_cksys_reg_base)
		iounmap(vir_addr_consys_gen_cksys_reg_base);

	return 0;
}

int consys_platform_spm_conn_ctrl_mt6858(unsigned int enable)
{
	int ret = 0;
...
int consys_conninfra_on_power_ctrl_mt6858(unsigned int enable)
{
	...
	ret = consys_platform_spm_conn_ctrl_mt6858(enable);
	mdelay(5);
#else
	ret = consys_conninfra_on_power_ctrl_mt6858_gen(enable);
...
void consys_set_if_pinmux_mt6858(unsigned int enable, unsigned int curr_status, unsigned int next_status)
{
	...
	if (enable) {
		consys_set_if_pinmux_mt6858_gen(curr_status, next_status, 1);
	} else {
		consys_set_if_pinmux_mt6858_gen(curr_status, next_status, 0);
	}
...
int consys_reg_init_mt6858(struct platform_device *pdev)
{
	...
	if (node) {
		for (i = 0; i < CONSYS_BASE_ADDR_MAX; i++) {
			base_addr = &conn_reg_mt6858.reg_base_addr[i];
...
int consys_reg_deinit_mt6858(void)
{
	...
	for (i = 0; i < CONSYS_BASE_ADDR_MAX; i++) {
		if (conn_reg_mt6858.reg_base_addr[i].vir_addr) {
			pr_info("[%d] Unmap %s (0x%zx)",
				i, consys_base_addr_index_to_str[i],
				conn_reg_mt6858.reg_base_addr[i].vir_addr);
			iounmap((void __iomem *)conn_reg_mt6858.reg_base_addr[i].vir_addr);
			conn_reg_mt6858.reg_base_addr[i].vir_addr = 0;
		}
	}
...
{
	unsigned int bus_timeout_irq_status = 0;
	unsigned int light_security_irq_status = 0;

	pr_notice("[%s] ++++++\n", __func__);

	bus_timeout_irq_status = CONSYS_REG_READ(CONN_BUS_CR_CONN_INFRA_OFF_BUS_TIMEOUT_STATUS_ADDR);
	light_security_irq_status = CONSYS_REG_READ(CONN_DBG_CTL_CONN_INFRA_BUS_TIMEOUT_IRQ_ADDR);

	pr_notice("[%s] bus timeout irq = 0x%x, light security irq = 0x%x\n", __func__, bus_timeout_irq_status, light_security_irq_status);

	/* dump light security info if light security irq is set */
	if (light_security_irq_status == 0x80)
		pr_notice("[%s] light security vio addr = 0x%x, vio info = 0x%x\n", __func__, CONSYS_REG_READ(CONN_BUS_CR_EASY_SECURITY_AP2CONN_HADDR_LATCH_ADDR),
			CONSYS_REG_READ(CONN_BUS_CR_EASY_SECURITY_AP2CONN_INFO_LATCH_ADDR));

	conninfra_is_bus_hang();
	conninfra_trigger_whole_chip_rst(CONNDRV_TYPE_CONNINFRA,
									"conninfra bus timeout irq handling");
	/* clear irq */
	if (bus_timeout_irq_status != 0)
		CONSYS_SET_BIT(CONN_BUS_CR_CONN_INFRA_OFF_BUS_TIMEOUT_CTRL_ADDR, (0x1U << 1));

	if (light_security_irq_status == 0x80) {
		CONSYS_SET_BIT(CONN_BUS_CR_EASY_SECURITY_AP2CONN_IRQ_CLR_ADDR, (0x1U << 0));
		CONSYS_SET_BIT(CONN_BUS_CR_EASY_SECURITY_AP2CONN_HADDR_LATCH_CLR_ADDR, (0x1U << 0));
		CONSYS_SET_BIT(CONN_BUS_CR_EASY_SECURITY_AP2CONN_VIO_W_LATCH_CLR_ADDR, (0x1U << 0));
		CONSYS_SET_BIT(CONN_BUS_CR_EASY_SECURITY_AP2CONN_VIO_R_LATCH_CLR_ADDR, (0x1U << 0));

		CONSYS_CLR_BIT(CONN_BUS_CR_EASY_SECURITY_AP2CONN_IRQ_CLR_ADDR, (0x1U << 0));
		CONSYS_CLR_BIT(CONN_BUS_CR_EASY_SECURITY_AP2CONN_HADDR_LATCH_CLR_ADDR, (0x1U << 0));
		CONSYS_CLR_BIT(CONN_BUS_CR_EASY_SECURITY_AP2CONN_VIO_W_LATCH_CLR_ADDR, (0x1U << 0));
		CONSYS_CLR_BIT(CONN_BUS_CR_EASY_SECURITY_AP2CONN_VIO_R_LATCH_CLR_ADDR, (0x1U << 0));
	}
...
irqreturn_t consys_irq_handler_mt6858(int irq, void* data)
{
...
int consys_register_irq_mt6858(struct platform_device *pdev)
{
	int ret = 0;
...
void consys_unregister_irq_mt6858(void)
{

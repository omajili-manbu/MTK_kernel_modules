...
static struct consys_plat_thermal_data_mt6858 g_consys_plat_therm_data;

/* For calibration backup/restore */
#define MAX_CALIBRATION_DATA_BACKUP_SIZE	256

extern phys_addr_t g_con_emi_phy_base;
unsigned long mt6858_power_state_dump_data[POWER_STATE_DUMP_DATA_SIZE];
static uint32_t g_connsys_adie_sku = 0x0;

int consys_co_clock_type_mt6858(void)
{
	return conn_hw_env.clock_type;
}

int consys_enable_power_dump_mt6858(void)
{
...
#define CONN_TICK_TO_SEC(TICK) (TICK / CONN_32K_TICKS_PER_SEC)
	...
	if (print_log) {
		mt6858_power_state_dump_data[0] = round;
		mt6858_power_state_dump_data[1] = CONN_TICK_TO_SEC(conninfra_sleep_time);
		mt6858_power_state_dump_data[2] = CONN_TICK_TO_SEC((conninfra_sleep_time
							    % CONN_32K_TICKS_PER_SEC) * 1000);
		mt6858_power_state_dump_data[3] = conninfra_sleep_cnt;
		mt6858_power_state_dump_data[4] = CONN_TICK_TO_SEC(wf_sleep_time);
		mt6858_power_state_dump_data[5] = CONN_TICK_TO_SEC((wf_sleep_time
							    % CONN_32K_TICKS_PER_SEC) * 1000);
		mt6858_power_state_dump_data[6] = wf_sleep_cnt;
		mt6858_power_state_dump_data[7] = CONN_TICK_TO_SEC(bt_sleep_time);
		mt6858_power_state_dump_data[8] = CONN_TICK_TO_SEC((bt_sleep_time
							    % CONN_32K_TICKS_PER_SEC) * 1000);
		mt6858_power_state_dump_data[9] = bt_sleep_cnt;
		mt6858_power_state_dump_data[10] = CONN_TICK_TO_SEC(gps_sleep_time);
		mt6858_power_state_dump_data[11] = CONN_TICK_TO_SEC((gps_sleep_time
							     % CONN_32K_TICKS_PER_SEC) * 1000);
		mt6858_power_state_dump_data[12] = gps_sleep_cnt;
		mt6858_power_state_dump_data[13] = CONN_TICK_TO_SEC(t_conninfra_sleep_time);
		mt6858_power_state_dump_data[14] = CONN_TICK_TO_SEC((t_conninfra_sleep_time
							     % CONN_32K_TICKS_PER_SEC) * 1000);
		mt6858_power_state_dump_data[15] = t_conninfra_sleep_cnt;
		mt6858_power_state_dump_data[16] = CONN_TICK_TO_SEC(t_wf_sleep_time);
		mt6858_power_state_dump_data[17] = CONN_TICK_TO_SEC((t_wf_sleep_time
							     % CONN_32K_TICKS_PER_SEC) * 1000);
		mt6858_power_state_dump_data[18] = t_wf_sleep_cnt;
		mt6858_power_state_dump_data[19] = CONN_TICK_TO_SEC(t_bt_sleep_time);
		mt6858_power_state_dump_data[20] = CONN_TICK_TO_SEC((t_bt_sleep_time
							     % CONN_32K_TICKS_PER_SEC) * 1000);
		mt6858_power_state_dump_data[21] = t_bt_sleep_cnt;
		mt6858_power_state_dump_data[22] = CONN_TICK_TO_SEC(t_gps_sleep_time);
		mt6858_power_state_dump_data[23] = CONN_TICK_TO_SEC((t_gps_sleep_time
							     % CONN_32K_TICKS_PER_SEC) * 1000);
		mt6858_power_state_dump_data[24] = t_gps_sleep_cnt;
		...
		ret = snprintf(buf_p, buf_sz,
			...
			mt6858_power_state_dump_data[0],
			mt6858_power_state_dump_data[1],
			mt6858_power_state_dump_data[2],
			mt6858_power_state_dump_data[3],
			mt6858_power_state_dump_data[4],
			mt6858_power_state_dump_data[5],
			mt6858_power_state_dump_data[6],
			mt6858_power_state_dump_data[7],
			mt6858_power_state_dump_data[8],
			mt6858_power_state_dump_data[9],
			mt6858_power_state_dump_data[10],
			mt6858_power_state_dump_data[11],
			mt6858_power_state_dump_data[12],
			mt6858_power_state_dump_data[13],
			mt6858_power_state_dump_data[14],
			mt6858_power_state_dump_data[15],
			mt6858_power_state_dump_data[16],
			mt6858_power_state_dump_data[17],
			mt6858_power_state_dump_data[18],
			mt6858_power_state_dump_data[19],
			mt6858_power_state_dump_data[20],
			mt6858_power_state_dump_data[21],
			mt6858_power_state_dump_data[22],
			mt6858_power_state_dump_data[23],
			mt6858_power_state_dump_data[24]);
		if (ret > 0)
			pr_info("%s", buf_p);
...
int consys_reset_power_state_mt6858(void)
{
	return consys_power_state_dump(NULL, 0, 0);
}

int consys_power_state_dump_mt6858(char *buf, unsigned int size)
{
	return consys_power_state_dump(buf, size, 1);
}

unsigned int consys_get_hw_ver_mt6858(void)
{
	return CONN_HW_VER;
}

void update_thermal_data_mt6858(struct consys_plat_thermal_data_mt6858 *input)
{
	memcpy(&g_consys_plat_therm_data, input, sizeof(struct consys_plat_thermal_data_mt6858));
}
...
{
	struct consys_plat_thermal_data_mt6858 *data = &g_consys_plat_therm_data;
	int t;
	int const_offset = 25;
...
int consys_thermal_query_adie6631_mt6858(void)
{
...
#define THERMAL_TEMP_INVALID	-275
	...
	connsys_adie_top_ck_en_ctl_mt6858(true);

	/* Hold Semaphore, TODO: may not need this, because
	 * thermal cr separate for different
	 */
	if (consys_sema_acquire_timeout_mt6858(CONN_SEMA_THERMAL_INDEX, CONN_SEMA_TIMEOUT)
		== CONN_SEMA_GET_FAIL) {
		pr_err("[THERM QRY] Require semaphore fail\n");
		connsys_adie_top_ck_en_ctl_mt6858(false);
		iounmap(addr);
		return -1;
	}
	...
	consys_sema_release_mt6858(CONN_SEMA_THERMAL_INDEX);
	connsys_adie_top_ck_en_ctl_mt6858(false);
...
unsigned int consys_adie_detection_adie6631_mt6858(unsigned int drv_type)
{
	g_connsys_adie_sku = ADIE_6631;
	return ADIE_6631;
}

unsigned int consys_adie_detection_adie6686_mt6858(unsigned int drv_type)
{
...
unsigned int consys_get_adie_chipid_mt6858(void)
{
	/* It's only different gps adie between each sku in mt6858 */
	return g_connsys_adie_sku;
}

void consys_set_mcu_control_mt6858(int type, bool onoff)
{

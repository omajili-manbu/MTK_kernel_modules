...
#include "include/mt6858.h" /* For function declaration */
#include "include/mt6858_consys_reg.h" /* cr base address */
#include "include/mt6858_consys_reg_offset.h" /* cr offset */
 ...
 ********************************************************************************
 */
unsigned int mt6858_chip_info[CHIP_INFO_SIZE];
/*******************************************************************************
...
struct timespec64 sema_get_time[CONN_SEMA_NUM_MAX];
...
static void consys_write_bt_rcal_mt6858(mapped_addr *psysram_efuse_list);
#endif

unsigned int consys_emi_set_remapping_reg_mt6858(
	phys_addr_t con_emi_base_addr,
	phys_addr_t md_shared_emi_base_addr,
	phys_addr_t gps_emi_base_addr)
{
	return consys_emi_set_remapping_reg_mt6858_gen(con_emi_base_addr, md_shared_emi_base_addr,
							gps_emi_base_addr, 16);
}

int consys_conninfra_wakeup_mt6858(void)
{
	return consys_conninfra_wakeup_mt6858_gen();
}

int consys_conninfra_sleep_mt6858(void)
{
	return consys_conninfra_sleep_mt6858_gen();
}

int consys_polling_chipid_mt6858(void)
{
	return consys_polling_chipid_mt6858_gen(NULL);
}

int connsys_d_die_cfg_mt6858(void)
{
	unsigned int d_die_efuse = 0;

	/* Reset conninfra sysram */
	consys_init_conninfra_sysram_mt6858_gen();

	/* Read D-die Efuse
	 * AP2CONN_EFUSE_DATA 0x1801_1020
	 * Write to conninfra sysram: CONN_INFRA_SYSRAM_SW_CR_D_DIE_EFUSE(0x1805_3820)
	 */
	connsys_get_d_die_efuse_mt6858_gen(&d_die_efuse);
	CONSYS_REG_WRITE(
		CONN_INFRA_SYSRAM_SW_CR_D_DIE_EFUSE, d_die_efuse);

	connsys_d_die_cfg_mt6858_gen();
...
int connsys_spi_master_cfg_mt6858(unsigned int curr_status, unsigned int next_status)
{
	if (curr_status != 0)
		return 0;

	if (consys_get_adie_chipid_mt6858() == ADIE_6631) {
		connsys_wt_slp_top_ctrl_adie6631_mt6858_gen();
	} else if (consys_get_adie_chipid_mt6858() == ADIE_6686) {
		connsys_wt_slp_top_ctrl_adie6631_mt6858_gen();
		connsys_wt_slp_top_ctrl_adie6686_mt6858_gen();
	}

	return 0;
}

int connsys_a_die_cfg_mt6858(unsigned int curr_status, unsigned int next_status)
{
	...
	int check = 0;
	struct consys_plat_thermal_data_mt6858 input;
	...
	if ((consys_get_adie_chipid_mt6858() == ADIE_6686) && (bt_wifi_fm_on == false)) {
		return 0;
	} else if ((consys_get_adie_chipid_mt6858() != ADIE_6686) && (curr_status != 0)) {
		return 0;
	}

	clock_type = consys_co_clock_type_mt6858();
	...
	memset(&input, 0, sizeof(struct consys_plat_thermal_data_mt6858));
	if (consys_get_adie_chipid_mt6858() == ADIE_6631) {
		connsys_adie_top_ck_en_ctl_mt6858_gen(1);

		if (consys_sema_acquire_timeout_mt6858(CONN_SEMA_RFSPI_INDEX, CONN_SEMA_TIMEOUT)
			== CONN_SEMA_GET_FAIL) {
			connsys_adie_top_ck_en_ctl_mt6858_gen(0);
			pr_err("[SPI READ] Require semaphore fail\n");
			return CONNINFRA_SPI_OP_FAIL;
		}
	} else if (consys_get_adie_chipid_mt6858() == ADIE_6686) {
		connsys_adie_top_ck_en_ctl_mt6858_gen(1);
		consys_m10_srclken_cfg_mt6858_gen(1);

		if (consys_sema_acquire_timeout_mt6858(CONN_SEMA_RFSPI_INDEX, CONN_SEMA_TIMEOUT)
			== CONN_SEMA_GET_FAIL) {
			connsys_adie_top_ck_en_ctl_mt6858_gen(0);
			consys_m10_srclken_cfg_mt6858_gen(0);
			pr_err("[SPI READ] Require semaphore fail\n");
			return CONNINFRA_SPI_OP_FAIL;
		}
		if (consys_sema_m4_acquire_timeout_mt6858(0, CONN_SEMA_TIMEOUT)
			== CONN_SEMA_GET_FAIL) {
			connsys_adie_top_ck_en_ctl_mt6858_gen(0);
			consys_sema_release_mt6858(CONN_SEMA_RFSPI_INDEX);
			consys_m10_srclken_cfg_mt6858_gen(0);
			pr_err("[SPI READ] Require semaphore fail\n");
			return CONNINFRA_SPI_OP_FAIL;
		}
	...
	connsys_a_die_cfg_deassert_adie_reset_mt6858_gen(curr_status, next_status);

	if (consys_get_adie_chipid_mt6858() == ADIE_6686)
		consys_sema_m4_release_mt6858(0);

	check = connsys_a_die_cfg_read_adie_id_mt6858_gen(curr_status, next_status, &adie_id, &hw_ver_id);
	if (check) {
		consys_sema_release_mt6858(CONN_SEMA_RFSPI_INDEX);
		return -1;
	}
	...
	connsys_a_die_efuse_read_get_efuse_info_mt6858_gen(
			curr_status, next_status, mt6858_chip_info,sysram_efuse_list,
			&(input.slop_molecule), &(input.thermal_b),&(input.offset));
	pr_info("slop_molecule=[%d], thermal_b =[%d], offset=[%d]",
		input.slop_molecule, input.thermal_b, input.offset);
	update_thermal_data_mt6858(&input);

	connsys_a_die_cfg_PART2_mt6858_gen(curr_status, next_status, hw_ver_id);

	consys_write_bt_rcal_mt6858(sysram_efuse_list);
	...
	conn_hw_env.is_rc_mode = consys_is_rc_mode_enable_mt6858();

	connsys_wt_slp_top_power_saving_ctrl_adie6631_mt6858_gen();
...
void connsys_afe_sw_patch_mt6858(void)
{
	connsys_afe_sw_patch_mt6858_gen();
}

int connsys_afe_wbg_cal_mt6858(void)
{
	...
	if (first_cal == 0)
		return 0;
	first_cal = 0;
	return connsys_afe_wbg_cal_mt6858_gen(CONN_SEMA_RFSPI_INDEX, CONN_SEMA_TIMEOUT);
}

int connsys_subsys_pll_initial_mt6858(void)
{
	return connsys_subsys_pll_initial_xtal_26000k_mt6858_gen();
}

int connsys_low_power_setting_mt6858(unsigned int curr_status, unsigned int next_status)
{
	if (curr_status == 0) {
		connsys_low_power_setting_mt6858_gen();
	} else {
		/* Not first on */
	}
	/* !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! */
...
int consys_sema_acquire_timeout_mt6858(unsigned int index, unsigned int usec)
{
	unsigned int i;
	...
	for (i = 0; i < usec; i++) {
		...
		if (consys_sema_acquire(index) == CONN_SEMA_GET_SUCCESS) {
			osal_gettimeofday(&sema_get_time[index]);
			pr_info("%s start hold semaphore (%d), flags=%lu \n", __func__, index, g_sema_irq_flags);
			return CONN_SEMA_GET_SUCCESS;
		}
		udelay(1);
	}
...
int consys_sema_m4_acquire_timeout_mt6858(unsigned int index, unsigned int usec)
{
	unsigned int i;
...
void consys_sema_release_mt6858(unsigned int index)
{
	...
	osal_gettimeofday(&curr_time);
	duration = timespec64_to_ms(&sema_get_time[index], &curr_time);
	if (index == CONN_SEMA_RFSPI_INDEX) {
		local_irq_restore(g_sema_irq_flags);
		msleep(5);
	}
	if (index == CONN_SEMA_RFSPI_INDEX) {
		pr_info("rfspi semaphore status: [0x%08x][0x%08x][0x%08x][0x%08x] \n",
		CONSYS_REG_READ(CONN_SEMAPHORE_CONN_SEMA_M0_QUEUEING_STA_REP_1_ADDR),
		CONSYS_REG_READ(CONN_SEMAPHORE_CONN_SEMA_M1_QUEUEING_STA_REP_1_ADDR),
		CONSYS_REG_READ(CONN_SEMAPHORE_CONN_SEMA_M2_QUEUEING_STA_REP_1_ADDR),
		CONSYS_REG_READ(CONN_SEMAPHORE_CONN_SEMA_M3_QUEUEING_STA_REP_1_ADDR));
	}
...
void consys_sema_m4_release_mt6858(unsigned int index)
{
	unsigned long duration;
...
int consys_spi_read_nolock_int_mt6858(enum sys_spi_subsystem subsystem, unsigned int addr,
				  unsigned int *data, mapped_addr spi_base_addr)
{
	...
	*data = check;
	...
	osal_gettimeofday(&end_time);
	duration = timespec64_to_ms(&start_time, &end_time);
	if (duration > SEMA_HOLD_TIME_THRESHOLD)
		pr_notice("%s hold spi read semaphore, flags=%lu for %lu ms\n",
			__func__, g_sema_irq_flags, duration);

	return 0;
}

int consys_spi_read_nolock_mt6858(enum sys_spi_subsystem subsystem, unsigned int addr,
				  unsigned int *data)
{
	int ret = 0;
	...
	ret = consys_spi_read_nolock_int_mt6858(subsystem, addr, data, spi_base_addr);
	return ret;
}

int consys_spi_read_mt6858(enum sys_spi_subsystem subsystem, unsigned int addr, unsigned int *data)
{
	int ret = 0;

	if (subsystem == SYS_SPI_FM || subsystem == SYS_SPI_GPS)
		return consys_spi_read_nolock_mt6858(subsystem, addr, data);
	...
	ret = consys_spi_read_nolock_mt6858(subsystem, addr, data);

	consys_sema_release_mt6858(CONN_SEMA_RFSPI_INDEX);
	return ret;
}

int consys_spi_1_read_mt6858(enum sys_spi_subsystem subsystem, unsigned int addr, unsigned int *data)
{
	int ret = 0;
	/* If (Dual A-die) {RFSPI0(with base addr 0x18042XXX)
	 * & RFSPI1(with base addr 0x18046XXX)}(GPS with 6686)
	...
	consys_spi_read_nolock_int_mt6858(subsystem, addr, data, spi_base_addr);
	return ret;
}


int consys_spi_write_nolock_int_mt6858(enum sys_spi_subsystem subsystem, unsigned int addr,
				  unsigned int data, mapped_addr spi_base_addr)
{
#ifndef CONFIG_FPGA_EARLY_PORTING
	int check = 0;
	...
	duration = timespec64_to_ms(&start_time, &end_time);
	if (duration > SEMA_HOLD_TIME_THRESHOLD)
		pr_notice("%s hold spi write semaphore, flags=%lu for %lu ms\n",
			__func__, g_sema_irq_flags, duration);

	return 0;
}

int consys_spi_write_nolock_mt6858(enum sys_spi_subsystem subsystem, unsigned int addr, unsigned int data)
{
	int ret = 0;
	/* If (Dual A-die) {RFSPI0(with base addr 0x18042XXX)
	 * & RFSPI1(with base addr 0x18046XXX)}(GPS with 6686)
	...
	ret = consys_spi_write_nolock_int_mt6858(subsystem, addr, data, spi_base_addr);
	return ret;
}


int consys_spi_write_mt6858(enum sys_spi_subsystem subsystem, unsigned int addr, unsigned int data)
{
	int ret = 0;

	if (subsystem == SYS_SPI_FM || subsystem == SYS_SPI_GPS)
		return consys_spi_write_nolock_mt6858(subsystem, addr, data);
	...
	ret = consys_spi_write_nolock_mt6858(subsystem, addr, data);

	consys_sema_release_mt6858(CONN_SEMA_RFSPI_INDEX);
	return ret;
}

int consys_spi_1_write_mt6858(enum sys_spi_subsystem subsystem, unsigned int addr, unsigned int data)
{
	int ret = 0;
	/* If (Dual A-die) {RFSPI0(with base addr 0x18042XXX)
	 * & RFSPI1(with base addr 0x18046XXX)}(GPS with 6686)
...
int consys_spi_update_bits_nolock_mt6858(enum sys_spi_subsystem subsystem, unsigned int addr,
				  unsigned int data, unsigned int mask)
{
	...
	ret = consys_spi_read_nolock_mt6858(subsystem, addr, &curr_val);
	...
	if (change) {
		ret = consys_spi_write_nolock_mt6858(subsystem, addr, new_val);
	}

	return ret;
}

int consys_spi_update_bits_mt6858(enum sys_spi_subsystem subsystem, unsigned int addr,
				  unsigned int data, unsigned int mask)
{
	int ret = 0;

	if (subsystem == SYS_SPI_FM || subsystem == SYS_SPI_GPS) {
		return consys_spi_update_bits_nolock_mt6858(subsystem, addr, data, mask);
	}
	...
	consys_spi_update_bits_nolock_mt6858(subsystem, addr, data, mask);
	consys_sema_release_mt6858(CONN_SEMA_RFSPI_INDEX);

	return ret;
}

int consys_spi_1_update_bits_mt6858(enum sys_spi_subsystem subsystem, unsigned int addr,
				  unsigned int data, unsigned int mask)
{
	int ret = 0;
	unsigned int curr_val = 0;
	...
	ret = consys_spi_1_read_mt6858(subsystem, addr, &curr_val);

	new_val = (curr_val & (~mask)) | (data & mask);
	change = (curr_val != new_val);

	if (change) {
		ret = consys_spi_1_write_mt6858(subsystem, addr, new_val);
	}
...
int consys_spi_clock_switch_mt6858(enum connsys_spi_speed_type type)
{
	int check;
	int ret = 0;
...
int consys_subsys_status_update_mt6858(bool on, int radio)
{
	...
	if (consys_sema_acquire_timeout_mt6858(CONN_SEMA_CONN_INFRA_COMMON_SYSRAM_INDEX,
	...
	consys_sema_release_mt6858(CONN_SEMA_CONN_INFRA_COMMON_SYSRAM_INDEX);

	return 0;
}

bool consys_is_rc_mode_enable_mt6858(void)
{
#ifdef CONFIG_FPGA_EARLY_PORTING
	return false;
#else /* CONFIG_FPGA_EARLY_PORTING */
...
#ifndef CONFIG_FPGA_EARLY_PORTING
void consys_spi_write_offset_range_nolock_mt6858(
	...
	unsigned int reg_offset, unsigned int value_offset, unsigned int size)
{
	...
	data2 = data & (~reg_mask);
	data2 = (data2 | value);
	consys_spi_write_nolock_mt6858(subsystem, addr, data2);
}
...
int connsys_adie_top_ck_en_ctl_mt6858(bool onoff)
{
	return connsys_adie_top_ck_en_ctl_mt6858_gen(onoff);
}

static int consys_get_chip_info(char *buf, unsigned int size, int print_log)
{
	char temp_buf[CHIP_INFO_SIZE];
	char *buf_p = temp_buf;
	int buf_sz = CHIP_INFO_SIZE;
	int ret = 0;
	...
	ret = snprintf(buf_p, buf_sz, "MT6631: [%08x]", mt6858_chip_info[0]);
...
int consys_get_chip_info_mt6858(char *buf, unsigned int size)
{
	return consys_get_chip_info(buf, size, 1);
}

#ifndef CONFIG_FPGA_EARLY_PORTING
static void consys_write_bt_rcal_mt6858(mapped_addr *sysram_efuse_list)
{
	...
	/* turn on bt clk*/
	consys_spi_write_nolock_mt6858(SYS_SPI_TOP, 0xA08, 0xFFFFFFFF);
	/* write to 0x64 */
	consys_spi_write_nolock_mt6858(SYS_SPI_BT, 0x064, bt_rcal_w);
	consys_spi_read_nolock_mt6858(SYS_SPI_BT, 0x064, &bt_rcal_r);
	/* turn off bt clk */
	consys_spi_write_nolock_mt6858(SYS_SPI_TOP, 0xA08, 0x0);

...
{
	if (consys_hw_ops && consys_hw_ops->consys_plt_spi_1_read)
...
{
	if (consys_hw_ops && consys_hw_ops->consys_plt_spi_1_write)
...
int consys_hw_spi_1_update_bits(enum sys_spi_subsystem subsystem, unsigned int addr,
					 unsigned int data, unsigned int mask)
{
	if (consys_hw_ops && consys_hw_ops->consys_plt_spi_1_update_bits)
...
int consys_hw_tcxo_parser(struct platform_device *pdev)
{
	...
	ret = of_property_read_string(pdev->dev.of_node, "tcxo-support", &tcxo_support);
	if (!ret) {

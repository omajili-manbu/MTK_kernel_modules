...
#include "include/mt8875_debug_gen.h"
#include "include/mt8875_pos.h"
#include "include/mt8875_pos_gen.h"

#define CONSYS_DUMP_BUF_SIZE 1000
struct consys_base_addr g_conn_reg_mt8875;
static struct conn_debug_info_mt8875 *g_debug_info_ptr;
static struct conn_debug_info_mt8875 *g_debug_top_power_info_ptr;
...
int consys_reg_init_mt8875(struct platform_device *pdev)
{
	...
	if (node) {
		for (i = 0; i < CONSYS_BASE_ADDR_MAX; i++) {
			base_addr = &g_conn_reg_mt8875.reg_base_addr[i];
...
int consys_reg_deinit_mt8875(void)
{
	...
	for (i = 0; i < CONSYS_BASE_ADDR_MAX; i++) {
		if (g_conn_reg_mt8875.reg_base_addr[i].vir_addr) {
			pr_info("[%d] Unmap %s (0x%zx)",
				i, consys_base_addr_index_to_str[i],
				g_conn_reg_mt8875.reg_base_addr[i].vir_addr);
			iounmap((void __iomem*)g_conn_reg_mt8875.reg_base_addr[i].vir_addr);
			g_conn_reg_mt8875.reg_base_addr[i].vir_addr = 0;
		}
	}
...
int consys_check_reg_readable_mt8875(void)
{
	...
	return 1;
#endif
	if (consys_check_ap2conn_infra_on_mt8875())
		return 0;
	if (consys_check_ap2conn_infra_off_clock_mt8875())
		return 0;
	if (consys_check_ap2conn_infra_off_irq_mt8875())
		return 0;
	return 1;
}
...
int consys_is_bus_hang_mt8875(void)
{
	...
	if (!is_clock_fail) {
		...
		consys_print_pbus_debug_dbg_level_2_mt8875_debug_gen(g_debug_info_ptr);
...
int consys_debug_top_power_status_mt8875(void)
{
	...
	consys_print_top_power_debug_dbg_level_0_mt8875_debug_gen(
			g_debug_top_power_info_ptr);
	consys_print_log("[CONN_SUBSYS_POWER]", g_debug_top_power_info_ptr);

	consys_print_top_power_debug_dbg_level_1_mt8875_debug_gen(
			g_debug_top_power_info_ptr);
	consys_print_log("[CONN_TOP_POWER_A]", g_debug_top_power_info_ptr);

	consys_print_top_power_debug_dbg_level_2_mt8875_debug_gen(
			g_debug_top_power_info_ptr);
...
{
	g_debug_info_ptr =
		(struct conn_debug_info_mt8875 *)osal_malloc(sizeof(struct conn_debug_info_mt8875));
	if (g_debug_info_ptr == NULL) {
	...
	consys_debug_init_mt8875_debug_gen();
}
...
void consys_debug_deinit_mt8875(void)
{
	...
	consys_debug_deinit_mt8875_debug_gen();
}

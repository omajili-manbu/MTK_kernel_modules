...
#include "mt8875_consys_reg.h"
#include "mt8875_consys_reg_offset.h"
#include "mt8875.h"
#include "mt8875_pos.h"
#include "mt8875_pos_gen.h"
#include "mt8875_soc.h"
#include "../include/connsys_smc.h"

static u64 g_sema_get_time[CONN_SEMA_NUM_MAX];

static int consys_sema_acquire_timeout_mt8875(unsigned int index, unsigned int usec);
static int consys_sema_acquire_mt8875(unsigned int index);
static void consys_sema_release_mt8875(unsigned int index);
#ifndef CONFIG_FPGA_EARLY_PORTING
static const char *get_spi_sys_name(enum sys_spi_subsystem subsystem);
#endif

unsigned int consys_emi_set_remapping_reg_mt8875(
	phys_addr_t con_emi_base_addr,
	phys_addr_t md_shared_emi_base_addr,
	phys_addr_t gps_emi_base_addr)
{
	return consys_emi_set_remapping_reg_mt8875_gen(con_emi_base_addr, md_shared_emi_base_addr,
							gps_emi_base_addr, 16);
}

bool consys_is_rc_mode_enable_mt8875(void)
{
...
int consys_conninfra_on_power_ctrl_mt8875(unsigned int enable)
{
	int ret = 0;

#ifndef CONFIG_FPGA_EARLY_PORTING
#if MTK_CONNINFRA_CLOCK_BUFFER_API_AVAILABLE
	ret = consys_platform_spm_conn_ctrl_mt8875(enable);
#else
	ret = consys_conninfra_on_power_ctrl_mt8875_gen(1, 0, enable);
#endif
#endif
	if (enable)
		mdelay(5);
...
int consys_polling_chipid_mt8875(void)
{
	return consys_polling_chipid_mt8875_gen(NULL);
}

int connsys_d_die_cfg_mt8875(void)
{
	...
	consys_init_conninfra_sysram_mt8875_gen();

	clock_type = consys_co_clock_type_mt8875();
	 ...
	 */
	connsys_get_d_die_efuse_mt8875_gen(&d_die_efuse);
	CONSYS_REG_WRITE(
		CONN_INFRA_SYSRAM_SW_CR_D_DIE_EFUSE, d_die_efuse);

	connsys_d_die_cfg_mt8875_gen();
...
int connsys_spi_master_cfg_mt8875(unsigned int curr_status, unsigned int next_status)
{
	if (curr_status != 0)
		return 0;

	connsys_wt_slp_top_ctrl_adie6686_mt8875_gen();
	return 0;
}

void connsys_afe_sw_patch_mt8875_atf(void)
{
	CONNSYS_SMC_CALL_VOID(SMC_CONNSYS_AFE_SW_PATCH_OPID, CONN_HW_VER_MT8875,
			      CONNSYS_A_DIE_ID_MT8875, 0, 0, 0, 0);
}

int connsys_subsys_pll_initial_mt8875(void)
{
	return connsys_subsys_pll_initial_xtal_26000k_mt8875_gen();
}

int connsys_low_power_setting_mt8875(unsigned int curr_status, unsigned int next_status)
{
	if (curr_status == 0)
		connsys_low_power_setting_mt8875_gen();
...
int consys_conninfra_wakeup_mt8875(void)
{
	return consys_conninfra_wakeup_mt8875_gen();
}

int consys_conninfra_sleep_mt8875(void)
{
...
int consys_subsys_status_update_mt8875(bool on, int radio)
{
...
int consys_sema_acquire_timeout_mt8875(unsigned int index, unsigned int usec)
{
	unsigned int i;
	...
	if (index >= CONN_SEMA_NUM_MAX)
		return CONN_SEMA_GET_FAIL;
	for (i = 0; i < usec; i++) {
		if (consys_sema_acquire_mt8875(index) == CONN_SEMA_GET_SUCCESS) {
...
void consys_sema_release_mt8875(unsigned int index)
{
...
static int consys_spi_read_nolock_mt8875(
...
int consys_spi_read_mt8875(
	enum sys_spi_subsystem subsystem, unsigned int addr, unsigned int *data)
{
	int ret = 0;
	/* Get semaphore before read */
	if (consys_sema_acquire_timeout_mt8875(
		CONN_SEMA_RFSPI_INDEX, CONN_SEMA_TIMEOUT) == CONN_SEMA_GET_FAIL) {
		pr_err("[SPI READ] Require semaphore fail\n");
		return CONNINFRA_SPI_OP_FAIL;
	}

	ret = consys_spi_read_nolock_mt8875(subsystem, addr, data);

	consys_sema_release_mt8875(CONN_SEMA_RFSPI_INDEX);
	return ret;
}

static int consys_spi_write_nolock_mt8875(
	enum sys_spi_subsystem subsystem, unsigned int addr, unsigned int data)
{
#ifndef CONFIG_FPGA_EARLY_PORTING
	int check = 0;
...
int consys_spi_write_mt8875(enum sys_spi_subsystem subsystem, unsigned int addr, unsigned int data)
{
	int ret = 0;
	/* Get semaphore before read */
	if (consys_sema_acquire_timeout_mt8875(
	...
	ret = consys_spi_write_nolock_mt8875(subsystem, addr, data);

	consys_sema_release_mt8875(CONN_SEMA_RFSPI_INDEX);
	return ret;
}

int consys_spi_update_bits_mt8875(
	enum sys_spi_subsystem subsystem, unsigned int addr, unsigned int data, unsigned int mask)
{
	...
	ret = consys_spi_read_nolock_mt8875(subsystem, addr, &curr_val);
	...
	if (change)
		ret = consys_spi_write_nolock_mt8875(subsystem, addr, new_val);

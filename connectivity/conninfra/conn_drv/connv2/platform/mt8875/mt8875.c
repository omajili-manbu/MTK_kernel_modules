...
unsigned int consys_get_hw_ver_mt8875(void)
{
	return CONN_HW_VER_MT8875;
}

unsigned int consys_get_adie_chipid_mt8875(unsigned int drv_type)
{
	return CONNSYS_A_DIE_ID_MT8875;
}
...
static void consys_power_state(void)
{
	...
	consys_debug_top_power_status_mt8875();
}
...
int consys_reset_power_state_mt8875(void)
{
	return consys_power_state_dump(NULL, 0, 0);
}

int consys_power_state_dump_mt8875(char *buf, unsigned int size)
{
	return consys_power_state_dump(buf, size, 1);
}

int consys_enable_power_dump_mt8875(void)
{
	/* Return success because sleep count dump is enable on POS */
	return 0;
}

void consys_set_mcu_control_mt8875(int type, bool onoff)
{
